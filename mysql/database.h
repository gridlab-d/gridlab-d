...
#define DBO_OVERWRITE                                                          \
  0x0008 ///< overwrite existing file when dumping and backing up
...
#define TD_BACKUP 0x0002 ///< table dump is formatted as SQL backup dump
  ...
  char *get_sqldata(char *buffer, size_t size, gld_property &p,
                    double scale = 1.0);
  char *get_sqldata(char *buffer, size_t size, gld_property &p,
                    gld_unit *unit = NULL);
  bool get_sqlbind(MYSQL_BIND &value, gld_property &target,
                   my_bool *error = NULL);
  void check_schema();
