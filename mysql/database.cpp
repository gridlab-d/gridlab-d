...
database::database(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass =
        gld_class::create(module, "database", sizeof(database), PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_char256, "hostname", get_hostname_offset(), PT_ACCESS,
            PA_PUBLIC, PT_DESCRIPTION, "server hostname", PT_char256, "schema",
            get_schema_offset(), PT_ACCESS, PA_PUBLIC, PT_DESCRIPTION,
            "schema name", PT_char32, "username", get_username_offset(),
            PT_ACCESS, PA_PUBLIC, PT_DESCRIPTION, "user name", PT_char32,
            "password", get_password_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "user password", PT_int32, "port",
            get_port_offset(), PT_ACCESS, PA_PUBLIC, PT_DESCRIPTION,
            "TCP port number", PT_char1024, "socketname",
            get_socketname_offset(), PT_ACCESS, PA_PUBLIC, PT_DESCRIPTION,
            "unix socket name", PT_set, "clientflags", get_clientflags_offset(),
            PT_ACCESS, PA_PUBLIC, PT_DESCRIPTION, "mysql client flags",
            PT_KEYWORD, "COMPRESS", (int64)CLIENT_COMPRESS, PT_KEYWORD,
            "FOUND.rows()", (int64)CLIENT_FOUND.rows(), PT_KEYWORD,
            "IGNORE_SIGPIPE", (int64)CLIENT_IGNORE_SIGPIPE, PT_KEYWORD,
            "INTERACTIVE", (int64)CLIENT_INTERACTIVE, PT_KEYWORD, "LOCAL_FILES",
            (int64)CLIENT_LOCAL_FILES, PT_KEYWORD, "MULTI_RESULTS",
            (int64)CLIENT_MULTI_RESULTS, PT_KEYWORD, "MULTI_STATEMENTS",
            (int64)CLIENT_MULTI_STATEMENTS, PT_KEYWORD, "NO_SCHEMA",
            (int64)CLIENT_NO_SCHEMA, PT_KEYWORD, "ODBC", (int64)CLIENT_ODBC,
            PT_KEYWORD, "SSL", (int64)CLIENT_SSL, PT_KEYWORD,
            "REMEMBER_OPTIONS", (int64)CLIENT_REMEMBER_OPTIONS, PT_set,
            "options", get_options_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "database connection options", PT_KEYWORD,
            "SHOWQUERY", (int64)DBO_SHOWQUERY, PT_DESCRIPTION,
            "show SQL queries when running", PT_KEYWORD, "NOCREATE",
            (int64)DBO_NOCREATE, PT_DESCRIPTION,
            "prevent automatic creation of non-existent schemas", PT_KEYWORD,
            "NEWDB", (int64)DBO_DROPSCHEMA, PT_DESCRIPTION,
            "destroy existing schemas before use them (risky)", PT_KEYWORD,
            "OVERWRITE", (int64)DBO_OVERWRITE, PT_DESCRIPTION,
            "destroy existing files before output dump/backup results (risky)",
            PT_char1024, "on_init", get_on_init_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "SQL script to run when initializing", PT_char1024,
            "on_sync", get_on_sync_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "SQL script to run when synchronizing", PT_char1024,
            "on_term", get_on_term_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "SQL script to run when terminating", PT_double,
            "sync_interval[s]", get_sync_interval_offset(), PT_ACCESS,
            PA_PUBLIC, PT_DESCRIPTION, "interval at which on_sync is called",
            PT_int32, "tz_offset", get_tz_offset_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION, "timezone offset used by timestamp in the database",
            PT_bool, "uses_dst", get_uses_dst_offset(), PT_ACCESS, PA_PUBLIC,
            PT_DESCRIPTION,
...
int database::init(OBJECT *parent) {
  ...
  if (mysql_options(mysql_client, MYSQL_REPORT_DATA_TRUNCATION,
                    &report_truncate) != 0)
    gl_warning("unable to disable data truncation reporting in mysql");

  gld_string flags = get_clientflags_property().get_string();

  gl_verbose("mysql_connect(hostname='%s',username='%s',password='%s',schema='%"
             "s',port=%u,socketname='%s',clientflags=0x%016llx[%s])",
             (const char *)hostname, (const char *)username,
             (const char *)password, (const char *)schema, port,
             (const char *)socketname, get_clientflags(), (const char *)flags);

  mysql =
      mysql_real_connect(mysql_client, hostname, username, password, nullptr,
                         port, socketname, (unsigned long)clientflags);
  if (mysql == 0)
  ...
  // drop schema
  if (get_options() & DBO_DROPSCHEMA &&
      query("DROP DATABASE IF EXISTS `%s`", get_schema())) {
    if (strcmp(get_schema(), "gridlabd") == 0)
      gl_warning("%s uses NEWDB option on the default schema '%s' - this is "
                 "extremely risky",
                 get_name(), get_schema());
  ...
  if (mysql_num.rows()(res) == 0) {
    if (!(get_options() & DBO_NOCREATE)) {
      ...
      exception("NOCREATE option prevents automatic creation of schema '%s'",
                get_schema());
  } else
  ...
  if (strcmp(get_on_init(), "") != 0) {
    ...
    if (res <= 0)
      exception("on_init script '%s' failed at line %d: %s", get_on_init(),
                -res, get_last_error());
  }
  return 1;
}
...
void database::term(void) {
  if (strcmp(get_on_term(), "") != 0) {
    ...
    try {
      ...
      if (res <= 0)
        exception("on_term script '%s' failed at line %d: %s", get_on_term(),
                  -res, get_last_error());
...
TIMESTAMP database::commit(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (get_sync_interval() > 0) {
    ...
    if (strcmp(get_on_sync(), "") != 0 && mod == 0) {
      int mod1 = t1 % 86400, mod2 = ts.get_timestamp() % 86400,
          mod3 = ts.get_localtimestamp() % 86400;
      gld_clock ts(t0);
      char buffer[64];
      gl_verbose(
          "%s running on_init script '%s' at %s", get_name(), get_on_sync(),
          ts.to_string(buffer, sizeof(buffer)) ? buffer : "(unknown time)");
      int res = run_script(get_on_sync());
      if (res <= 0)
        exception("on_init script '%s' failed at line %d: %s", get_on_sync(),
                  -res, get_last_error());
    }
...
const char *database::get_last_error(void) { return mysql_error(mysql); }
bool database::get_sqlbind(MYSQL_BIND &value, gld_property &target,
                           my_bool *error) {
...
char *database::get_sqldata(char *buffer, size_t size, gld_property &prop,
                            double scale) {
  switch (prop.get_type()) {
  case PT_double:
  case PT_random:
...
bool database::query(char *fmt, ...) {
  ...
  if (mysql_query(mysql, command) != 0)
    exception("%s->query[%s] failed - %s", get_name(), command,
              mysql_error(mysql));
...
bool database::query(const char *command) {
  ...
  check_schema();
  if (mysql_query(mysql, command) != 0)
    exception("%s->query[%s] failed - %s", get_name(), buffer,
              mysql_error(mysql));
  else if (get_options() & DBO_SHOWQUERY)
    gl_verbose("%s->query[%s] ok", get_name(), command);
...
MYSQL_RES *database::select(char *fmt, ...) {
  ...
  check_schema();
  if (mysql_query(mysql, command) != 0)
    exception("%s->select[%s] query failed - %s", get_name(), command,
              mysql_error(mysql));
  else if (get_options() & DBO_SHOWQUERY)
  ...
  MYSQL_RES *res = mysql_store_result(mysql);
  if (res == nullptr)
    exception("%s->select[%s] result failed - %s", get_name(), command,
              mysql_error(mysql));
  int n = mysql_num.rows()(res);
...
unsigned int64 database::get_last_index(void) { return mysql_insert_id(mysql); }
...
int database::run_script(char *file) {
  ...
  while (fgets(line, sizeof(line) - 1, fp) != nullptr) {
    ...
    if (len + eol >= sizeof(buffer))
      exception("run_script(char *file='%s'): line '%s' command too long", num,
                file);
    ...
    if (buffer[eol - 1] == ';') {
      ...
      // regular SQL commands
        ...
        // run SQL query
        if (!query("%s", buffer))
          exception("run_script(char *file='%s'): line '%s' command failed",
                    num, file);
        MYSQL_RES *res = mysql_store_result(mysql);
        if (res) {
          gl_verbose("query [%.32s%s] ok - %d.rows() returned", buffer,
                     eol > 32 ? "..." : "", mysql_num.rows()(res));
          mysql_free_result(res);
        } else
          gl_verbose("query [%.32s%s] ok - %d.rows() affected", buffer,
                     eol > 32 ? "..." : "", mysql_affected.rows()(mysql));
      }
...
size_t database::dump(char *table, char *file, unsigned long options) {
  ...
  if (!(options & TD_APPEND)) {
    ...
    if (access(file, 0x00) == 0)
      exception("unable to dump '%s' to '%s' - OVERWRITE not specified but "
                "file exists",
                table, file);
  }
  ...
  if (result == nullptr) {
    fclose(fp);
    gl_error("dump of '%s' to '%s' failed - %s", table, file,
             mysql_error(mysql));
    return -1;
  }
    ...
    if (nfields == 0) {
      ...
      if (options & TD_BACKUP) {
        ...
        for (n = 0; n < nfields; n++) {
          ...
          if (type == nullptr) {
            gl_warning("unable to determine type of field '%s' in table '%s' - "
                       "using DOUBLE",
                       table, fields[n].name);
            type = "DOUBLE";
          }
          ...
          // dump data
          if (size > 0)
            fprintf(fp, "%s\n\t`%s` %s(%u)", n == 0 ? "" : ", ", fields[n].name,
                    type, size);
          else
            fprintf(fp, "%s\n\t`%s` %s", n == 0 ? "" : ",", fields[n].name,
                    type);
          if (fields[n].flags & NOT_nullptr_FLAG)
            fprintf(fp, " %s", "NOT nullptr");
          if (fields[n].flags & AUTO_INCREMENT_FLAG)
            fprintf(fp, " %s", "AUTO_INCREMENT");
          if (fields[n].flags & PRI_KEY_FLAG)
            fprintf(fp, " %s", "PRIMARY KEY");
        }
        fprintf(fp, "%s", "\n);\n");
...
Done:
  gl_verbose("dumped %u.rows() from table '%s' to file '%s'", .rows(), table,
             file);
...
size_t database::backup(char *file) {
  ...
  if (get_options() && DBO_OVERWRITE)
    unlink(file);
  if (access(file, 0x00) == 0)
    exception("unable to backup of '%s' to '%s' - OVERWRITE not specified but "
              "file exists",
              get_schema(), file);
...
TIMESTAMP database::convert_from_dbtime(TIMESTAMP ts) {
  // imcoming timestamp is dbtime -- convert it to UTC be subtracting offset and
  // dst
  gld_clock dt(ts);
  return ts - tz_offset - (uses_dst && dt.get_is_dst() ? 3600 : 0);
}
