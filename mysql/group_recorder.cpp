...
#endif
    oclass = gl_register_class(mod, "group_recorder", sizeof(group_recorder),
                               PC_POSTTOPDOWN);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);

    if (gl_publish_variable(
            oclass, PT_char1024, "file", PADDR(table), PT_DESCRIPTION,
            "output file name (legacy compatibility, applied to table field)",
            PT_char1024, "table", PADDR(table), PT_DESCRIPTION,
            "database table name", PT_char1024, "group", PADDR(group_def),
            PT_DESCRIPTION, "group definition string", PT_bool, "strict",
            PADDR(strict), PT_DESCRIPTION,
            ...
            "flag to record units for each written object, if applicable",
            PT_char256, "property", PADDR(property_name), PT_DESCRIPTION,
            "property to record", PT_int32, "limit", PADDR(limit),
            PT_DESCRIPTION,
            "the maximum number of lines to write to the database", PT_char32,
            "mode", PADDR(mode), PT_DESCRIPTION, "table output mode", PT_int32,
            "query_buffer_limit", PADDR(query_buffer_limit), PT_DESCRIPTION,
            "max number of queries to buffer before pushing to database",
            PT_int32, "column_limit", PADDR(column_limit), PT_DESCRIPTION,
            "maximum number of columns per MySQL table (default 200)",
            PT_char32, "datetime_fieldname", PADDR(datetime_fieldname),
            PT_DESCRIPTION, "name of date-time field", PT_char32,
            "recordid_fieldname", PADDR(recordid_fieldname), PT_DESCRIPTION,
            "name of record-id field", PT_char256, "complex_part",
            PADDR(complex_part), PT_DESCRIPTION,
            "the complex part(s) to record if complex properties are gathered "
            "(default REAL|IMAG)",
            PT_char256, "data_type", PADDR(data_type), PT_DESCRIPTION,
            "the data format MySQL should use to store the values (default is "
            "DOUBLE with no precision set). Acceptable types are DECIMAL(M,D), "
            "FLOAT(M,D), and DOUBLE(M,D)",
            PT_double, "interval[s]", PADDR(dInterval), PT_DESCRIPTION,
            "recording interval (0 'every iteration', -1 'on change')", PT_bool,
            "legacy_mode", PADDR(legacy_mode), PT_DESCRIPTION,
            "Produces output consistent with the tape group_recorder "
            "formatting, rather than the new database optimized formatting",

            // legacy compatibility components.
            PT_bool, "format", PADDR(format), PT_DESCRIPTION,
            "(unused) legacy variable for compatibility with tape "
            "group_recorder",
...
group_recorder::~group_recorder() { delete group_recorder_connection; }
...
int group_recorder::init(OBJECT *obj) {
  ...
  if (get_table()[0] == '\0') {
    gl_error("Both 'table' and 'file' fields are empty. Database target is "
             "unknown.");
    return 0;
  }
  ...
  group_recorder_connection = new query_engine(
      get_connection() != nullptr ? (database *)(get_connection() + 1) : db,
      query_buffer_limit, column_limit);
  ...
  if (strlen(mode) > 0) {
    ...
    struct {
      ...
      set bits;
        ...
        {"r", 0xffff},         {"r+", 0xffff}, {"w", MO_DROPTABLES},
        {"w+", MO_DROPTABLES}, {"a", 0x0000},  {"a+", 0x0000},
    };
    int n;
    ...
    if (options == 0xffffffff)
      ...
      exception("mode '%s' is not valid for group recorder",
                (const char *)mode);
  }
  ...
  if (0 == items) {
    if (strict) {
      gl_error("group_recorder::init(): unable to construct a set with group "
               "definition");
      /* TROUBLESHOOT
       An error occured while attempting to build and populate the find list
       with the specified group definition.
       */
      return 0;
    } else {
  ...
  for (gr_obj = gl_find_next(items, 0); gr_obj != 0;
    ...
    if (prop_ptr == nullptr) {
      ...
      /* TROUBLESHOOT
       An error occured while reading the specified property in one of the
       objects.
       */
      return 0;
    }
...
string setup_property_buffer(stringstream *property_name_buffer,
                             const char *suffix) {
...
int group_recorder::write_header() {
  ...
  if (get_property_name() != nullptr) {
    ...
    for (size_t n = 0; n < complex_part_vector.size(); n++) {
      std::transform(
          complex_part_vector[n].begin(), complex_part_vector[n].end(),
          complex_part_vector[n].begin(), std::ptr_fun<int, int>(std::toupper));
    ...
    for (qol = obj_list; qol != 0; qol = qol->next) {
      ...
      if (!target_prop.is_valid()) {
        ...
        /*  TROUBLESHOOT
         Check to make sure the target you are specifying is a published
         variable for the object that you are pointing to.  Refer to the
      ...
      if (target_prop.is_complex()) {
        while (complex_part_local_buffer != NONE) {
          ...
          case NONE:
          default:
            // FIXME: this is setting and wiping the buffer each iteration.
          ...
          case REAL:
            property_name_string =
                setup_property_buffer(&property_name_local_buffer, "_REAL");
            property_list << "`" << property_name_string << "` " << data_type
                          << ", ";
            grc->get_table_path()->add_table_header(
                grc, property_list, &property_name_string,
                target_prop.get_unit()->get_name());
          ...
          case IMAG:
            property_name_string =
                setup_property_buffer(&property_name_local_buffer, "_IMAG");
            property_list << "`" << property_name_string << "` " << data_type
                          << ", ";
            grc->get_table_path()->add_table_header(
                grc, property_list, &property_name_string,
          ...
          case MAG:
            property_name_string =
                setup_property_buffer(&property_name_local_buffer, "_MAG");
            property_list << "`" << property_name_string << "` " << data_type
                          << ", ";
            grc->get_table_path()->add_table_header(
                grc, property_list, &property_name_string,
          ...
          case ANG:
            property_name_string =
                setup_property_buffer(&property_name_local_buffer, "_ANG_DEG");
            property_list << "`" << property_name_string << "` " << data_type
                          << ", ";
            grc->get_table_path()->add_table_header(
                grc, property_list, &property_name_string,
          ...
          case ANG_RAD:
            property_name_string =
                setup_property_buffer(&property_name_local_buffer, "_ANG_RAD");
            property_list << "`" << property_name_string << "` " << data_type
                          << ", ";
            grc->get_table_path()->add_table_header(
                grc, property_list, &property_name_string,
        ...
        property_name_local_buffer << property_name_string_buffer;
        property_name_string =
            setup_property_buffer(&property_name_local_buffer, "");
        if (target_prop.is_double()) {
          ...
          property_list << "`" << property_name_string << "` "
                        << "INTEGER UNSIGNED, ";
          grc->get_table_path()->add_table_header(
              grc, property_list, &property_name_string,
              target_prop.get_unit()->get_name());
        ...
        //				else if (target_prop.is_character()) {
        //// TODO: Identify element of gld_property we want saved in this case.
        //					property_list << "`" <<
        //property_name_string << "` " << "VARCHAR(256), ";
        //					grc->get_table_path()->add_table_header(grc,
        //property_list, &property_name_string,
        //target_prop.get_unit()->get_name());
        //				}
        else {
          if (strict) {
            gl_error("group_recorder::write_header(): property is not an "
                     "allowed type");
            return 0;
          }
          gl_warning("group_recorder::write_header(): property is not an "
                     "allowed type");
          return 1;
        }
        cleanup_property_buffer(&property_name_local_buffer);
    ...
    for (int index = 0; index < grc->get_table_count(); index++) {
      ...
      if (db->table_exists(grc->get_table().get_string())) {
        if ((get_options() & MO_DROPTABLES) &&
            !db->query("DROP TABLE IF EXISTS `%s`",
                       grc->get_table().get_string()))
      ...
      if (db->table_exists(grc->get_table_references().get_string())) {
        if ((get_options() & MO_DROPTABLES) &&
            !db->query("DROP TABLE IF EXISTS `%s`",
                       grc->get_table_references().get_string()))
          exception("unable to drop table '%s'",
                    grc->get_table_references().get_string());
      }
      ...
      if (!db->table_exists(grc->get_table().get_string())) {
        if (!(get_options() & MO_NOCREATE)) {
          ...
          if (!db->query(query_char_string))
            exception("unable to create table '%s' in schema '%s'", get_table(),
                      db->get_schema());
          else
            gl_verbose("table %s created ok", grc->get_table().get_string());
        } else
          exception("NOCREATE option prevents creation of table '%s'",
                    grc->get_table().get_string());
      }

      // check row count
      else {
        if (db->select("SELECT count(*) FROM `%s`",
                       grc->get_table().get_string()) == nullptr)
          exception("unable to get row count of table '%s'",
                    grc->get_table().get_string());
    ...
    if (!db->table_exists(grc->get_table_references().get_string())) {
      if (!(get_options() & MO_NOCREATE)) {
        ...
        const char *query_char_string = query_string.c_str();
        if (!db->query(query_char_string))
          exception("unable to create table '%s' in schema '%s'",
                    grc->get_table_references().get_string(), db->get_schema());
        else
          gl_verbose("table %s created ok",
                     grc->get_table_references().get_string());
      } else
        exception("NOCREATE option prevents creation of table '%s'",
                  grc->get_table_references().get_string());
    ...
    // check row count
      ...
      if (db->select("SELECT count(*) FROM `%s`",
                     grc->get_table_references().get_string()) == nullptr)
        exception("unable to get row count of table '%s'",
                  grc->get_table_references().get_string());
...
int group_recorder::build_row(TIMESTAMP t1) {
  ...
  for (size_t n = 0; n < complex_part_vector.size(); n++) {
    std::transform(complex_part_vector[n].begin(), complex_part_vector[n].end(),
                   complex_part_vector[n].begin(),
  ...
  for (curr = obj_list; curr != 0; curr = curr->next) {
    ...
    if (!target_prop.is_valid()) {
      ...
      /*  TROUBLESHOOT
       ...
       variables can be published within the object you are pointing to with the
       assert function.
       */
      return 0;
    }
    ...
    if (target_prop.is_complex()) {
      ...
      while (complex_part_local_buffer != NONE) {
        ...
        case REAL:
          property_name_string =
              setup_property_buffer(&property_name_local_buffer, "_REAL");
          grc->get_table_path()->add_insert_values(grc, &property_name_string,
                                                   to_string(cptr.Re()));
          complex_part_local_buffer ^= compare;
          compare <<= 1;
          cleanup_property_buffer(&property_name_local_buffer);
        ...
        case ANG:
          property_name_string =
              setup_property_buffer(&property_name_local_buffer, "_ANG_DEG");
          grc->get_table_path()->add_insert_values(
              grc, &property_name_string, to_string(cptr.Arg() * 180 / PI));
          complex_part_local_buffer ^= compare;
          compare <<= 1;
          cleanup_property_buffer(&property_name_local_buffer);
          break;
      ...
      //			else if (target_prop.is_character()) { // TODO:
      //Identify element of gld_property we want saved in this case. 				gld_object*
      //property_value = target_prop.get_property();
      //				grc->get_table_path()->add_insert_values(grc,
      //&property_name_string, to_string(property_value));
      //			}

      else {
        if (strict) {
          gl_error(
              "group_recorder::build_row(): property is not an allowed type");
          return 0;
        }
        gl_warning(
            "group_recorder::build_row(): property is not an allowed type");
