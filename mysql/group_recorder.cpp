...
int group_recorder::write_header()
{
	...
	if (get_property_name() != nullptr)
	{
		...
		{
			std::transform(complex_part_vector[n].begin(), complex_part_vector[n].end(), complex_part_vector[n].begin(), std::ptr_fun<int, int>(std::toupper));
...
extern "C" MODULE_API TIMESTAMP sync_group_recorder(OBJECT *obj, ...)
{
	...
	template <class T>
	std::string group_recorder::to_string(T t)
	{
