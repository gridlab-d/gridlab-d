...
table_manager::table_manager(database *db_in, int threshold_in, int column_in,
                             int table_index_in, char1024 *table_name_in,
                             char32 recordid, char32 datetime,
...
std::vector<char *> *table_manager::get_table_units() { return &table_units; }
...
void table_manager::extend_list(query_engine *parent) {
  table_manager *new_table = new table_manager(
      db, threshold, column_limit, table_index + 1, table_root,
      recordid_fieldname, datetime_fieldname, false);
...
int table_manager::add_table_header(query_engine *parent,
...
int table_manager::add_table_header(string *property_name,
                                    string *property_full_header) {
  ...
  table_header_buffer << (column_count > 0 ? ", " : "")
                      << *property_full_header;
  column_count++;
...
int table_manager::add_insert_values(query_engine *parent, string *column_name,
                                     string value) {
  if (query_table(column_name)) {
    ...
    parent->next_table();
    return parent->get_table_path()->add_insert_values(parent, column_name,
                                                       value, table_index);
  }
  return 1; // Success
}
...
void table_manager::set_custom_sql(std::string sql) {
  ...
  while ((pos = sql.find(delimiter)) != std::string::npos || !last_run) {
    ...
    while ((inner_pos = token.find(inner_delimiter)) != std::string::npos ||
           !inner_last_run) {
...
std::string table_manager::get_custom_sql_columns() {
  ...
  for (int index = 0; index < custom_sql_headers.size(); index++) {
    column_values += "`" + custom_sql_headers[index] + "` " +
                     custom_sql_datatypes[index] + ", ";
  }
...
void table_manager::commit_values() {
  ...
  if (insert_count > 0) {
    ...
    query << "INSERT INTO `" << table.get_string() << "` (`"
          << datetime_fieldname << "`, `";
