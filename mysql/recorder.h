...
public:
  ...
  GL_ATOMIC(set, sql_options); // this is the actual flag set the user can set
                               // with the 'options' input.
...
private:
  ...
  template <class T>
  std::string to_string(T); // This template exists in both Recorder and Group
                            // Recorder, and could be cleaned up.
public:
