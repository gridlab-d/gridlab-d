...
collector::collector(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gld_class::create(module, "collector", sizeof(collector),
    ...
    if (gl_publish_variable(
            oclass, PT_char1024, "property", get_property_offset(),
            PT_DESCRIPTION, "target property name", PT_char1024, "group",
            get_group_offset(), PT_DESCRIPTION, "collector group specification",
            PT_char1024, "table", get_table_offset(), PT_DESCRIPTION,
            "table name to store samples", PT_char1024, "file",
            get_table_offset(), PT_DESCRIPTION,
            "file name (for tape compatibility)", PT_char32, "mode",
            get_mode_offset(), PT_DESCRIPTION, "table output mode", PT_int32,
            "limit", get_limit_offset(), PT_DESCRIPTION,
            "maximum number of records to output", PT_double, "interval[s]",
            get_interval_offset(), PT_DESCRIPTION, "sampling interval",
            PT_object, "connection", get_connection_offset(), PT_DESCRIPTION,
            "database connection", PT_set, "options", get_options_offset(),
            PT_DESCRIPTION, "SQL options", PT_KEYWORD, "PURGE",
            (int64)MO_DROPTABLES, PT_DESCRIPTION,
            "flag to drop tables before creation", nullptr) < 1) {
      char msg[256];
...
int collector::init(OBJECT *parent) {
  ...
  for (p = propspecs, n = 0; n < n_aggregates; n++) {
    ...
    gl_debug("%s: group '%s' aggregate '%s' initial value is '%lf'", get_name(),
             get_group(), p, list[n].get_value());
  ...
  if (db->table_exists(get_table())) {
    if (get_options() & MO_DROPTABLES &&
        !db->query("DROP TABLE IF EXISTS `%s`", get_table()))
  ...
  if (!db->table_exists(get_table())) {
    if (!(options & MO_NOCREATE)) {
      char buffer[4096];
      size_t eos = snprintf(buffer, sizeof(buffer),
      ...
      for (n = 0; n < n_aggregates; n++)
        eos += snprintf(buffer, sizeof(buffer) + eos, "`%s` double, ", names[n]);
      eos += snprintf(buffer, sizeof(buffer) + eos, "%s", "INDEX i_t (t))");
...
TIMESTAMP collector::commit(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (dt == 0 || (t1 == next_t && next_t != TS_NEVER)) {
    char buffer[4096];
    size_t eos = snprintf(buffer, sizeof(buffer), "INSERT INTO `%s` (t", get_table());
    int n;
    for (n = 0; n < n_aggregates; n++)
      eos += snprintf(buffer, sizeof(buffer) + eos, ",`%s`", names[n]);
    eos += snprintf(buffer, sizeof(buffer) + eos, ") VALUES (from_unixtime(%lli)",
                   db->convert_to_dbtime(gl_globalclock));
    for (n = 0; n < n_aggregates; n++)
      eos += snprintf(buffer, sizeof(buffer) + eos, ",%g", list[n].get_value());
    snprintf(buffer, sizeof(buffer) + eos, "%s", ")");

    if (!db->query(buffer))
      exception("unable to add data to '%s' - %s", get_table(),
                mysql_error(mysql));
    else
