...
void query_engine::next_table() { table_path = table_path->get_next_table(); }

void query_engine::init_tables(char32 recordid_fieldname,
                               char32 datetime_fieldname,
                               bool single_table_mode) {
  table_path = new table_manager(db, threshold, column_limit, 0, &table,
                                 recordid_fieldname, datetime_fieldname,
                                 single_table_mode);
  inc_table_count();
}
...
char1024 query_engine::get_table() { return table_path->get_table_name(); }
char1024 query_engine::get_table_references() { return table_references; }
...
void query_engine::build_table_references(bool print_units) {
  ...
  query << "INSERT INTO `" << table_references.get_string()
        << "` (`table_name`, `header`" << (print_units ? ",`units`" : "")
        << ") VALUES";
