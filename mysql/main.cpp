...
#define MYSQL_TS_NEVER                                                         \
  "'2038-01-01 00:00:00'" // near maximum mysql timestamp is used for TS_NEVER
#define MYSQL_TS_ZERO                                                          \
  "'1970-01-01 00:00:00'" // zero mysql timestamp is used for TS_ZERO
...
// options
bool new_database =
    false; ///< flag to drop a database before using it (very dangerous)
...
const char *get_table_name(const char *format, ...) {
  ...
  char fullfmt[1024] = "";
  if (table_prefix[0] != '\0' &&
      sscanf(table_prefix, "%[A-Za-z0-9_]", fullfmt) == 1 &&
      strcmp(table_prefix, fullfmt) != 0)
...
const char *process_command(const char *command) {
  ...
  if (table_prefix[0] == '\0')
    strcpy(table_prefix, default_table_prefix);
  ...
  while ((token = strtok_s(token ? nullptr : buffer, " \t", &next)) !=
         nullptr) {
    if (strcmp(token, "-h") == 0 || strcmp(token, "--hostname") == 0)
      ...
      if (verbose.is_valid() && verbose.get_bool() == false)
        gl_warning("option '--show_query' has no effect unless verbose mode is "
                   "enabled");
      show_query = true;
      state = PS_SCHEMA;
      ...
      case PS_SCHEMA:
        if (strlen(schema) > 0) {
          gl_error("schema '%s' is not allowed because only one schema may be "
                   "specified",
                   schema);
          return nullptr;
      ...
      case PS_FLAGS:
        gl_warning(
            "mysql import/export flags '%s' ignored, use module flags instead",
            token);
        state = PS_SCHEMA;
        break;
      case PS_PREFIX:
      ...
      default:
        gl_error(
            "process_command(const char *command='%s'): state = %d is invalid",
            command, state);
        return nullptr;
      }
    }
...
static MYSQL *get_connection(const char *schema, bool autocreate = false) {
  ...
  gl_debug("mysql_connect(hostname='%s',username='%s',password='%s',schema='%s'"
           ",port=%u,socketname='%s',clientflags=0x%016llx)",
           (const char *)hostname, (const char *)username,
           (const char *)password, (const char *)schema, port,
           (const char *)socketname, clientflags);
  mysql = mysql_real_connect(mysql, hostname, username, password, nullptr, port,
                             socketname, (unsigned long)clientflags);
  if (mysql == nullptr) {
...
static MYSQL_RES *query_result(MYSQL *mysql, const char *fmt, ...) {
  ...
  if (res == nullptr) {
    gl_error("query [%s] store result failed - %s", command,
             mysql_error(mysql));
    return nullptr;
  }
...
static bool import_modules(MYSQL *mysql) {
  MYSQL_RES *data = query_result(
      mysql, "SELECT `name`,`major`,`minor` FROM `%s` ORDER BY `id`",
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    ...
    gl_debug("import_modules(MYSQL*): row %d: `%s` = [%s]", n, fields[0].name,
             row[0]);
...
static bool import_globals(MYSQL *mysql) {
  MYSQL_RES *data =
      query_result(mysql, "SELECT `name`,`type`,`value` FROM `%s`",
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    MYSQL_ROW row = mysql_fetch_row(data);
    MYSQL_FIELD *fields = mysql_fetch_fields(data);
    gl_debug("import_globals(MYSQL*): row %d: `%s` = [%s], `%s` = [%s]", n,
             fields[0].name, row[0], fields[1].name, row[2]);
    char old[1024] = "(undefined)";
    gld_global var(row[0]);
    if (var.is_valid()) {
      var.to_string(old, sizeof(old));
      if (!var.from_string(row[2])) {
        gl_error("import_globals(MYSQL*): global set %s='%s' failed", row[0],
                 row[2]);
        return false;
      } else
        gl_verbose("set global %s: '%s' -> '%s'", row[0], old, row[2]);
    } else if (!var.create(row[0], (PROPERTYTYPE)atoi(row[1]), row[2])) {
      gl_error("import_globals(MYSQL*): global create type %s='%s' failed",
               row[0], row[2]);
      return false;
    } else {
      gld_type ptype((PROPERTYTYPE)atoi(row[1]));
      gl_verbose("create global %s type %s: '%s'", row[0],
                 ptype.get_spec()->name, row[2]);
    }
  }
  mysql_free_result(data);
  return true;
...
static bool import_classes(MYSQL *mysql) {
  MYSQL_RES *data = query_result(mysql,
                                 "SELECT `name`, `module`, `property`, `type`, "
                                 "`flags`, `units`, `description` "
                                 "FROM `%s` WHERE type<%d",
                                 get_table_name("classes"), _PT_LAST);
  if (data == nullptr)
    return false;
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    ...
    if ((flags & PF_EXTENDED) == 0)
      continue; // ignore classes that are not runtime extensions
    char *name = row[0];
    char *module = row[1];
    char *property = row[2];
    ...
    gl_debug("import_classes(MYSQL*): row %d, name=%s, module=%s, property=%s, "
             "type=%s, flags=%s, units=%s, description=%s",
             n, name, module, property, type, flags, unit, description);

    // create runtime classes
    CLASS *cls = gl_class_get_by_name(name);
    if (module == nullptr && cls == nullptr &&
        (cls = gl_register_class(nullptr, name, 0, 0)) == nullptr)
      return false;
    ...
    if (flags & PF_EXTENDED) {
      if (gl_class_add_extended_property(cls, property, type, unit) == nullptr)
        ...
        gl_verbose("property '%s[%s]' added to class '%s'", property, unit,
                   name);
    } else {
...
static bool import_objects(MYSQL *mysql) {
  MYSQL_RES *data = query_result(
      mysql,
      "SELECT "
      "`id`,`class`,`name`,`groupid`,`parent`,`rank`,`clock`,`valid_to`,`"
      "schedule_skew`,"
      "`latitude`,`longitude`,`in_svc`,`in_svc_micro`,`out_svc`,`out_svc_micro`"
      ",`rngstate`,`heartbeat`,`flags`,`module` FROM `%s` ORDER by `id`",
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    ...
    MYSQL_FIELD *fields = mysql_fetch_fields(data);
    gl_verbose(
        "import_objects(MYSQL*): row %d, id=%s, class=%s, name=%s, groupid=%s, "
        "parent=%s, rank=%s, clock='%s', valid_to='%s', schedule_skew='%s',"
        " latitude=%s, longtitude=%s, in_svc='%s', in_svc_micro=%s, "
        "out_svc='%s', out_svc_micro=%s, rngstate=%s, heartbeat='%s', flags=%s",
        n, row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7],
    ...
    ;
    while (cls != nullptr && !((cls->module == nullptr && row[18] == nullptr) ||
                               strcmp(cls->module->name, row[18]) == 0)) {
      gl_verbose("searching for class that matches module: '%s' -> %s:%s ",
                 row[18], cls->module, cls->name);
      do {
        cls = cls->next;
      } while (cls != nullptr && strcmp(cls->name, row[1]) != 0);
    }
    if (cls == nullptr) {
    ...
    if (row[3] != nullptr)
      strncpy(obj->groupid, row[3], sizeof(obj->groupid));
    obj->parent =
        row[4] == nullptr ? nullptr : gl_object_find_by_id(atoi(row[4]));
    obj->rank = atoi(row[5]);
  ...
  for (OBJECT *obj = first_object; obj != nullptr; obj = obj->next) {
    ...
    for (CLASS *cls = obj->oclass; cls != nullptr; cls = cls->parent) {
      MYSQL_RES *vars = query_result(
          mysql, "SELECT * FROM `%s` WHERE id=%d",
          get_table_name("%s_%s", cls->module ? cls->module->name : "",
                         cls->name),
          obj->id);
      if (vars == nullptr)
        return false;
      ...
      for (unsigned long m = 0; m < n_vars; m++) {
        ...
        for (unsigned long p = 1; p < n_props;
             p++) // don't process the first field, it's the id
        {
          if (var[p] == nullptr)
            continue; // nullptrs are ignored because core does not allow
                      // unsetting properties
          gl_debug("gl_set_value_by_name(obj=%0x, property='%s', value='%s')",
                   obj, prop[p].name, var[p]);
          gld_property value(obj, prop[p].name);
          if (!value.is_valid())
            gl_warning("object %s:%d property '%s' is not valid, value [%s] "
                       "cannot be imported",
                       obj->oclass->name, obj->id, prop[p].name, var[p]);
          else if (!value.get_access(PA_W | PA_L))
            gl_warning("object %s:%d property '%s' is not writeable and "
                       "loadable, value [%s] cannot be imported",
                       obj->oclass->name, obj->id, prop[p].name, var[p]);
          else if (value.from_string(var[p]) <= 0) {
            gl_error("unable to set object %s:%d property '%s' to value [%s]",
                     obj->oclass->name, obj->id, prop[p].name, var[p]);
            return false;
          } else
            gl_verbose(
                "setting object %s:%d property '%s' to value [%s] -> [%s]",
                obj->oclass->name, obj->id, value.get_name(), var[p],
  ...
  // resolve parents
  data =
      query_result(mysql, "SELECT `id`, `parent` FROM `%s` WHERE `parent`>`id`",
                   get_table_name("objects"));
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    MYSQL_ROW row = mysql_fetch_row(data);
    MYSQL_FIELD *fields = mysql_fetch_fields(data);
    gl_verbose("import_objects(MYSQL*): row %d, id=%s, parent=%s", n, row[0],
               row[1]);
    if (row[0] != nullptr && row[1] != nullptr) {
      ...
      if (obj == nullptr) {
        ...
        gl_error("import_objects(MYSQL*): parent object id %d is not found",
                 obj->id);
        return false;
      } else {
        obj->parent = parent;
...
bool import_properties(MYSQL *mysql) {
  MYSQL_RES *data =
      query_result(mysql, "SELECT `id`,`property`,`type`,`specs` FROM `%s`",
                   get_table_name("properties"));
  if (data == nullptr)
    return false;
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    ...
    MYSQL_FIELD *fields = mysql_fetch_fields(data);
    gl_verbose(
        "import_objects(MYSQL*): row %d, id=%s, property=%s, type=%s, specs=%s",
    ...
    if (!prop.is_valid()) {
      gl_error("import_objects(MYSQL*): object %s:%d property %s is not found",
               obj->oclass->name, obj->id, row[1]);
      return false;
    }
    if (strcmp(row[2], "randomvar") == 0) {
      if (prop.from_string(row[3]) <= 0) {
        gl_error("import_objects(MYSQL*): object %s:%d property %s value %s is "
                 "not valid",
                 obj->oclass->name, obj->id, row[1], row[3]);
        return false;
      }
    } else {
      gl_error("import_objects(MYSQL*): object %s:%d property %s type %s is "
               "not recognized",
               obj->oclass->name, obj->id, row[1], row[2]);
      return false;
    }
  }
  mysql_free_result(data);
...
EXPORT int import_schedules(MYSQL *mysql) {
  MYSQL_RES *data = query_result(mysql, "SELECT `name`,`definition` FROM `%s`",
                                 get_table_name("schedules"));
  if (data == nullptr)
    return false;
...
static bool add_linear_transform(unsigned int source_type, char *source,
                                 char *target, char *spec) {
  char name[64], prop[64] = "";
  if (sscanf(target, "%[^.].%s", name, prop) != 2) {
    gl_error("add_linear_transform(char *source='%s', char *target='%s', char "
             "*spec='%s'): target is not valid",
             source, target, spec);
    return false;
  }
  ...
  if (sscanf(spec, "*%g+%g", &scale, &bias) != 2) {
    gl_error("add_linear_transform(char *source='%s', char *target='%s', char "
             "*spec='%s'): linear transform specification is invalid",
             source, target, spec);
    return false;
  }
  ...
  case 1: // source is a schedule
  {
    ...
    if (schedule == nullptr) {
      gl_error("add_linear_transform(char *source='%s', char *target='%s', "
               "char *spec='%s'): source schedule is not found",
               source, target, spec);
      return false;
    }
    // TODO
  ...
  case 2: // source is a property
  {
    ...
    if (!src.is_valid()) {
      gl_error("add_linear_transform(char *source='%s', char *target='%s', "
               "char *spec='%s'): source is not found",
               source, target, spec);
      return false;
    }
    if (!gl_transform_add_linear((TRANSFORMSOURCE)source_type,
                                 (double *)src.get_addr(), dst.get_addr(),
                                 scale, bias, dst.get_object(),
                                 dst.get_property(), nullptr)) {
      gl_error("add_linear_transform(char *source='%s', char *target='%s', "
               "char *spec='%s'): add transform failed - probable memory "
               "allocation failure",
               source, target, spec);
      return false;
    }
  ...
  default:
    gl_error("add_linear_transform(char *source='%s', char *target='%s', char "
             "*spec='%s'): error parsing source",
             source, target, spec);
    return false;
  }
  return true;
}
static bool add_external_transform(unsigned int source_type, char *source,
                                   char *target, char *spec) {
  // TODO
...
EXPORT int import_transforms(MYSQL *mysql) {
  MYSQL_RES *data = query_result(
      mysql,
      "SELECT `source`,`target`,`transform_type`,`source_type`,`specification` "
      "FROM `%s`",
      get_table_name("transforms"));
  if (data == nullptr)
    return false;
  ...
  for (unsigned long n = 0; n < n.rows(); n++) {
    ...
    char *stype = row[3];
    char *spec = row[4];
    if (source == nullptr || target == nullptr || ttype == nullptr ||
        stype == nullptr || spec == nullptr) {
      gl_error("import_transforms cannot have a nullptr source, target, type "
               "or specification");
      return false;
    }
    ...
    case XT_LINEAR:
      if (sscanf(stype, "%d", &source_type) == 0 ||
          !add_linear_transform(source_type, source, target, spec))
        return false;
      break;
    case XT_EXTERNAL:
      if (sscanf(stype, "%d", &source_type) == 0 ||
          !add_external_transform(source_type, source, target, spec))
        return false;
      break;
    default:
...
EXPORT int import_file(const char *info) {
  ...
  if (mysql == nullptr)
    return 0;
  int rc = import_modules(mysql) && import_globals(mysql) &&
           import_classes(mysql) && import_objects(mysql) &&
           import_properties(mysql) && import_schedules(mysql) &&
...
static bool export_modules(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("modules")))
    return false;
  if (!no_create && !query(mysql,
  ...
  while (true) {
    if (!query(
            mysql,
            "REPLACE INTO `%s` (`name`,`major`,`minor`) VALUES (\"%s\",%d,%d)",
            get_table_name("modules"), mod.get_name(), mod.get_major(),
            mod.get_minor()))
      return false;
    if (mod.is_last())
      break;
...
static bool export_globals(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("globals")))
    return false;
  if (!no_create && !query(mysql,
                           "CREATE TABLE IF NOT EXISTS `%s` ("
  ...
  while (true) {
    ...
    if (!query(mysql,
               ...
               "(\"%s\",%d,%d,\"%s\",%s,\"%s\")",
               get_table_name("globals"), prop->name, prop->ptype,
               var.get_flags(), quoted, unit, prop->description))
      return false;
    if (var.is_last())
      return true;
    else
...
static bool export_class(MYSQL *mysql, CLASS *cls) {
  ...
  if (cls->parent != nullptr) {
    if (!export_class(mysql, cls->parent) ||
        !query(mysql,
               "REPLACE INTO `%s` (`name`,`module`,`property`,`type`) "
               "VALUES (\"%s\",%s,\"%s\",%d)",
               get_table_name("classes"), cls->name, modname, cls->parent->name,
               PT_INHERIT))
      return false;
  }
  ...
  // create class tables
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`",
  ...
  int len = sprintf(query_string,
                    ...
                    get_table_name("%s_%s", mod ? mod->name : "", cls->name));
  for (PROPERTY *prop = cls->pmap; prop != nullptr && prop->oclass == cls;
       prop = prop->next) {
    ...
    if (!query(
            mysql,
            "REPLACE INTO `%s` "
            "(`name`,`module`,`property`,`type`,`flags`,`units`,`description`) "
            "VALUES (\"%s\",%s,\"%s\",%d,%d,%s,%s)",
            get_table_name("classes"), prop->oclass->name, modname, prop->name,
            prop->ptype, prop->flags, units, description))
      return false;
    ...
    for (KEYWORD *keyword = prop->keywords; keyword != nullptr;
         keyword = keyword->next) {
      if (!query(mysql,
                 "REPLACE INTO `%s` "
                 "(`name`,`module`,`property`,`keyword`,`type`,`flags`) "
                 "VALUES (\"%s\",%s,\"%s\",\"%s\",%d,%lld)",
                 get_table_name("classes"), prop->oclass->name, modname,
                 prop->name, keyword->name, PT_KEYWORD, keyword->value))
        return false;
    }
  }
...
static bool export_classes(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("classes")))
    return false;
  if (!no_create &&
      !query(mysql,
             "CREATE TABLE IF NOT EXISTS `%s` ("
             ...
             "`units` text, "
             "`description` text, "
             " KEY (`name`,`module`,`property`,`keyword`))",
             get_table_name("classes")))
    return false;
...
static bool export_objects(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("objects")))
    return false;
  if (!no_create &&
      !query(mysql,
             "CREATE TABLE IF NOT EXISTS `%s` ("
  ...
  for (OBJECT *obj = gl_object_get_first(); obj != nullptr; obj = obj->next) {
    ...
    if (!query(mysql,
               ...
               "latitude`,`longitude`,"
               " `clock`,`valid_to`,`schedule_skew`,`in_svc`,`in_svc_micro`,`"
               "out_svc`,`out_svc_micro`,"
               " `rngstate`,`heartbeat`,`flags`)"
               " VALUES (%d,%s,\"%s\",%s,%s,%s,%d,%s,%s,"
               " %s,%s,%s,%s,%d,%s,%d,"
               " %d,%s,%d)",
               get_table_name("objects"), obj->id, modname, cls->name, name,
               groupid, parent, obj->rank, latitude, longitude, clock, valid_to,
               schedule_skew, in_svc, obj->in_svc_micro, out_svc,
               obj->out_svc_micro, obj->rng_state, heartbeat, obj->flags))
      return false;
...
bool export_properties(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("properties")))
    return false;
  if (!no_create && !query(mysql,
                           "CREATE TABLE IF NOT EXISTS `%s` ("
  ...
  for (OBJECT *obj = gl_object_get_first(); obj != nullptr; obj = obj->next) {
    ...
    for (PROPERTY *prop = cls->pmap; prop != nullptr && prop->oclass == cls;
      ...
      if (type != nullptr && specs[0] == '\0') {
        gl_error("unable to get extended property specs for object %s:%d "
                 "property %s",
                 cls->name, obj->id, prop->name);
        return false;
      }
      if (type != nullptr &&
          !query(mysql,
                 "REPLACE INTO `%s` (`id`,`property`,`type`,`specs`) VALUES "
                 "(%d,'%s','%s','%s')",
                 get_table_name("properties"), obj->id, prop->name, type,
                 specs))
        return false;
    }
  }
...
bool export_transforms(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("transforms")))
    return false;
  if (!no_create && !query(mysql,
                           "CREATE TABLE IF NOT EXISTS `%s` ("
                           ...
                           get_table_name("transforms")))
    return false;
  for (TRANSFORM *xform = gl_transform_getfirst(); xform != nullptr;
       xform = gl_transform_getnext(xform)) {
    char source[256];
    ...
    case XS_ENDUSE: {
      ...
      if (!prop.is_valid()) {
        gl_error("export transform source address cannot be resolved to a "
                 "valid object");
        return false;
      }
    ...
    case XS_SCHEDULE:
      ...
      break;
    default:
      gl_error("export transform source type is not valid (type=%d)",
               xform->source_type);
      return false;
    }
    ...
    case XT_EXTERNAL:
      ...
      for (unsigned int n = 1; n < xform->nlhs; n++) {
        ...
        if (!prop.is_valid()) {
          gl_error("export transform external function lhs[%u] property '%s' "
                   "address cannot be resolved to an object",
      ...
      for (unsigned int n = 1; n < xform->nrhs; n++) {
        ...
        if (!prop.is_valid()) {
          gl_error("export transform external function rhs[%u] property '%s' "
                   "address cannot be resolved to an object",
    ...
    if (!query(mysql,
               "REPLACE INTO `%s` "
               "(`source`,`target`,`transform_type`,`source_type`,`"
               "specification`) "
               "VALUES (\"%s\",\"%s:%u.%s\",%d,%d,\"%s\")",
               get_table_name("transforms"), source,
               xform->target_obj->oclass->name, xform->target_obj->id,
               xform->target_prop->name, xform->function_type,
               xform->source_type, specs))
      return false;
  }
  return true;
}
bool export_schedules(MYSQL *mysql) {
  if (overwrite &&
      !query(mysql, "DROP TABLE IF EXISTS `%s`", get_table_name("schedules")))
    return false;
  if (!no_create && !query(mysql,
                           "CREATE TABLE IF NOT EXISTS `%s` ("
                           " `name` char(64) PRIMARY KEY,"
                           " `definition` text NOT nullptr)",
                           get_table_name("schedules")))
    return false;
  for (SCHEDULE *schedule = gl_schedule_getfirst(); schedule != nullptr;
       schedule = schedule->next) {
    char quoted[sizeof(schedule->definition) * 2 + 1];
    mysql_real_escape_string(mysql, quoted, schedule->definition,
                             strlen(schedule->definition));
    if (!query(mysql,
               "REPLACE INTO `%s` (`name`,`definition`) VALUES (\"%s\",\"%s\")",
               get_table_name("schedules"), schedule->name, quoted))
      return false;
  }
  return true;
}
EXPORT int export_file(char *info) {
  ...
  if (mysql == nullptr)
    return 0;
  int rc = export_modules(mysql) && export_globals(mysql) &&
           export_classes(mysql) && export_objects(mysql) &&
           export_properties(mysql) && export_transforms(mysql) &&
