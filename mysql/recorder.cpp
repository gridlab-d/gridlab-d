...
recorder::recorder(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gld_class::create(module, "recorder", sizeof(recorder),
    ...
    if (gl_publish_variable(
            ...
            PT_DESCRIPTION, "target property name", PT_char32, "trigger",
            get_trigger_offset(), PT_DESCRIPTION, "recorder trigger condition",
            ...
            "file name (for tape compatibility)", PT_char1024, "group",
            get_group_offset(), PT_DESCRIPTION,
            "group name for group_recorder mode", PT_char1024, "recorder_name",
            get_recorder_name_offset(), PT_DESCRIPTION,
            "name added to every row generated by an individual recorder for "
            "many-to-one recording",
            PT_char32, "mode", get_mode_offset(), PT_DESCRIPTION,
            "table output mode", PT_int32, "limit", get_limit_offset(),
            PT_DESCRIPTION, "maximum number of records to output", PT_double,
            "interval[s]", get_interval_offset(), PT_DESCRIPTION,
            "sampling interval", PT_object, "connection",
            get_connection_offset(), PT_DESCRIPTION, "database connection",
            PT_set, "options", get_sql_options_offset(), PT_DESCRIPTION,
            "SQL options", PT_KEYWORD, "PURGE", (gld::set)MO_DROPTABLES,
            PT_DESCRIPTION, "flag to drop tables before creation", PT_KEYWORD,
            "UNITS", (gld::set)MO_USEUNITS, PT_DESCRIPTION,
            "include units in column names", PT_char32, "datetime_fieldname",
            get_datetime_fieldname_offset(), PT_DESCRIPTION,
            "name of date-time field", PT_char32, "recordid_fieldname",
            get_recordid_fieldname_offset(), PT_DESCRIPTION,
            "name of record-id field", PT_char1024, "header_fieldnames",
            get_header_fieldnames_offset(), PT_DESCRIPTION,
            "name of header fields to include", PT_int32, "query_buffer_limit",
            get_query_buffer_limit_offset(), PT_DESCRIPTION,
            "max number of queries to buffer before pushing to database",
            PT_bool, "minimize_data", get_minified_offset(), PT_DESCRIPTION,
            "sets a signal for size minimization to the database. will use "
            "VARCHAR and FLOAT instead of CHAR and DOUBLE for most types",
...
int recorder::init(OBJECT *parent) {
  ...
  if (tag_mode)
    rc->get_table_path()->add_table_header(
        new string("recorder_name"),
        new string(
            "`recorder_name` " +
            string((get_minified() ? "SMALLINT UNSIGNED" : "VARCHAR(64)"))));
  ...
  if (group_mode) {
    ...
    for (size_t index = 0; index < group_items->get_size(); index++) {
      for (int property_index = 0; property_index < property_specs.size();
        ...
        group_obj_count++;
        if (group_obj_list == 0) {
          group_obj_list = new recorder_quickobjlist(
              obj_builder, obj_prop.get_property_struct());
        } else {
    ...
    rc->get_table_path()->add_table_header(
        new string("name"),
        new string("name " +
                   string((get_minified() ? "VARCHAR(64)"
                                          : "CHAR(64), index i_name (name)"))));
    ...
    for (size_t n = 0; n < property_specs.size(); n++) {
      ...
      if (spec.size() > 0) {
        ...
        OBJECT *obj_builder =
            group_items->get(0); // Maybe not the best way to do this?
        ...
        if (unit.is_valid()) {
          rc->get_table_path()->add_table_header(new string(tmp[0][0]),
                                                 new string(tmp[0][1]));
          rc->get_table_path()->add_table_header(new string(tmp[1][0]),
                                                 new string(tmp[1][1]));
        } else {
          rc->get_table_path()->add_table_header(new string(tmp[0][0]),
                                                 new string(tmp[0][1]));
    ...
    for (size_t n = 0; n < header_specs.size(); n++) {
      if (header_specs[n].compare("name") == 0 && !group_mode) {
        ...
        rc->get_table_path()->add_table_header(
            ...
            new string(
                "class " +
                string((get_minified() ? "VARCHAR(32)"
                                       : "CHAR(32), index i_class (class)"))));
        ...
        rc->get_table_path()->add_table_header(
            ...
            new string("latitude " +
                       string((get_minified()
                                   ? "FLOAT"
                                   : "DOUBLE, index i_latitude (latitude)"))));
        ...
        rc->get_table_path()->add_table_header(
            ...
            new string(
                ...
                string((get_minified()
                            ? "FLOAT"
                            : "DOUBLE, index i_longitude (longitude)"))));
      } else
        exception("header field %s does not exist",
                  (const char *)header_specs[n].c_str());
    }
  ...
  if (n_properties > 0) {
    ...
    if (db->table_exists(get_table())) {
      if (get_options() & MO_DROPTABLES &&
          !db->query("DROP TABLE IF EXISTS `%s`", rc->get_table().get_string()))
    ...
    if (!get_minified()) {
      query << ", INDEX `i_" << datetime_fieldname << "` (`"
            << datetime_fieldname << "`))";
    } else {
      query << ")";
    }
...
TIMESTAMP recorder::commit(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  gl_debug("%s: interval=%.0f, clock=%lld", get_name(), interval,
           gl_globalclock);
  if (interval > 0) {
  ...
  if (enabled) {
    ...
    if (group_mode) {
      ...
      for (curr = group_obj_list; curr != 0;) {
        ...
        if (tag_mode)
          rc->get_table_path()->add_insert_values(
              rc, new string("recorder_name"),
              string("'" + string(get_recorder_name()) + "'"));
        rc->get_table_path()->add_insert_values(
            rc, new string("name"),
            string("'" + string(target_prop.get_object()->name) + "'"));
        ...
        for (size_t n = 0; n < property_target.size() && curr != 0;
          ...
          char name_buffer[64];
          string *name_string =
              new string(property_target[n].get_sql_safe_name(name_buffer));
          ...
          if (valid_unit_test && (get_options() & MO_USEUNITS)) {
            db->get_sqldata(buffer, sizeof(buffer), target_prop,
                            target_prop.get_unit());
            rc->get_table_path()->add_insert_values(rc, name_string,
                                                    string(buffer));
            rc->get_table_path()->add_insert_values(
                rc, new string(*name_string + "_units"),
                "'" + string(target_prop.get_unit()->get_name()) + "'");
          } else if (get_options() & MO_USEUNITS) {
            rc->get_table_path()->add_insert_values(rc, name_string,
                                                    string("nullptr"));
            rc->get_table_path()->add_insert_values(
                rc, new string(*name_string + "_units"), string("nullptr"));
          } else {
            db->get_sqldata(buffer, sizeof(buffer), target_prop,
                            target_prop.get_unit());
            rc->get_table_path()->add_insert_values(rc, name_string,
      ...
      if (get_limit() > 0 && group_limit_counter >= get_limit()) {
        ...
        rc->set_tables_done();
        enabled = false;
        gl_verbose("table '%s' size limit %d reached",
                   rc->get_table().get_string(), get_limit());
      } else {
        group_limit_counter++;
      }
    } else {
      ...
      for (size_t n = 0; n < property_target.size(); n++) {
        ...
        if (property_unit[n].is_valid()) {
          db->get_sqldata(buffer, sizeof(buffer), property_target[n],
                          &property_unit[n]);
          rc->get_table_path()->add_insert_values(rc, name_string,
                                                  string(buffer));
          rc->get_table_path()->add_insert_values(
              rc, new string(*name_string + "_units"),
              "'" + string(property_unit[n].get_name()) + "'");
        } else {
          db->get_sqldata(buffer, sizeof(buffer), property_target[n],
                          &property_unit[n]);
          rc->get_table_path()->add_insert_values(rc, name_string,
      ...
      for (size_t n = 0; n < header_specs.size(); n++) {
        if (header_specs[n].compare("name") == 0) {
          rc->get_table_path()->add_insert_values(
              rc, new string("name"),
              string("'" + string(get_parent()->get_name()) + "'"));
        } else if (header_specs[n].compare("class") == 0) {
          rc->get_table_path()->add_insert_values(
              rc, new string("class"),
              string("'" + string(get_parent()->get_oclass()->get_name()) +
                     "'"));
        } else if (header_specs[n].compare("latitude") == 0) {
          if (isnan(get_parent()->get_latitude()))
            rc->get_table_path()->add_insert_values(rc, new string("latitude"),
                                                    string("nullptr"));
          else
            rc->get_table_path()->add_insert_values(
                rc, new string("latitude"),
                string(to_string(get_parent()->get_latitude())));
        } else if (header_specs[n].compare("longitude") == 0) {
          if (isnan(get_parent()->get_longitude()))
            rc->get_table_path()->add_insert_values(rc, new string("longitude"),
                                                    string("nullptr"));
          else
            rc->get_table_path()->add_insert_values(
                rc, new string("longitude"),
                string(to_string(get_parent()->get_longitude())));
        } else
          exception("header field %s does not exist",
                    (const char *)header_specs[n].c_str());
      }
