/** $Id: office.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The building simultion uses a single zone ETP model with first order
 ODEs:

        \f[ T'_i = \frac{1}{c_a} \left[ T_m U_m - T_i (U_a+U_m) + T_o U_a +
 \Sigma Q_x \right] \f]
 ...
 - \f$T_m\f$ is the temperature of the mass inside the buildnig (e.g, furniture,
 inside walls etc)
 ...
 - \f$c_m\f$ is the heat capacity of the mass of the furniture inside the walls
 etc
 - \f$c_a\f$ is the heat capacity of the air inside the building
 - \f$Q_i\f$ is the heat rate from internal heat gains of the building (e.g.,
 plugs, lights, people)
 ...
 where:
   ...
   - \f$T_i(t)\f$ is the temperature of the air inside the building at time
 \f$t\f$
   - \f$T_{i,0}\f$ is \f$T_i(t=0)\f$, e.g initial temperature of the air inside
 the building
   - \f$T'_{i,0}\f$ is \f$T'_i(t=0)\f$ e.g the initial temperature gradient of
...
office::office(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gld_class::create(module, "office", sizeof(office),
    ...
    if (gl_publish_variable(
            oclass, PT_double, "floor_area[sf]", PADDR(zone.design.floor_area),

            PT_double, "floor_height[ft]", PADDR(zone.design.floor_height),
            PT_double, "exterior_ua[Btu/degF/h]",
            PADDR(zone.design.exterior_ua), PT_double,
            "interior_ua[Btu/degF/h]", PADDR(zone.design.interior_ua),
            PT_double, "interior_mass[Btu/degF]",
            PADDR(zone.design.interior_mass),

            PT_double, "glazing[sf]", PADDR(zone.design.window_area[0]),
            PT_SIZE,
            sizeof(zone.design.window_area) /
                sizeof(zone.design.window_area[0]),
            PT_double, "glazing.north[sf]",
            PADDR(zone.design.window_area[CP_N]), PT_double,
            "glazing.northeast[sf]", PADDR(zone.design.window_area[CP_NE]),
            PT_double, "glazing.east[sf]", PADDR(zone.design.window_area[CP_E]),
            PT_double, "glazing.southeast[sf]",
            PADDR(zone.design.window_area[CP_SE]), PT_double,
            "glazing.south[sf]", PADDR(zone.design.window_area[CP_S]),
            PT_double, "glazing.southwest[sf]",
            PADDR(zone.design.window_area[CP_SW]), PT_double,
            "glazing.west[sf]", PADDR(zone.design.window_area[CP_W]), PT_double,
            "glazing.northwest[sf]", PADDR(zone.design.window_area[CP_NW]),
            PT_double, "glazing.horizontal[sf]",
            PADDR(zone.design.window_area[CP_H]), PT_double,
            "glazing.coefficient[pu]", PADDR(zone.design.glazing_coeff),

            // to be ripped out/updated with new schedule system
            PT_double, "occupancy", PADDR(zone.current.occupancy), PT_double,
            "occupants", PADDR(zone.design.occupants), PT_char256, "schedule",
            PADDR(zone.design.schedule),

            PT_double, "air_temperature[degF]",
            PADDR(zone.current.air_temperature), PT_double,
            "mass_temperature[degF]", PADDR(zone.current.mass_temperature),
            PT_double, "temperature_change[degF/h]",
            PADDR(zone.current.temperature_change), PT_double,
            "outdoor_temperature[degF]", PADDR(zone.current.out_temp),

            PT_double, "Qh[Btu/h]", PADDR(Qh), PT_double, "Qs[Btu/h]",
            PADDR(Qs), PT_double, "Qi[Btu/h]", PADDR(Qi), PT_double,
            "Qz[Btu/h]", PADDR(Qz),

            /* HVAC loads */
            PT_enumeration, "hvac_mode", PADDR(zone.hvac.mode), PT_KEYWORD,
            "HEAT", HC_HEAT, PT_KEYWORD, "AUX", HC_AUX, PT_KEYWORD, "COOL",
            HC_COOL, PT_KEYWORD, "ECON", HC_ECON, PT_KEYWORD, "VENT", HC_VENT,
            PT_KEYWORD, "OFF", HC_OFF, PT_double,
            "hvac.cooling.balance_temperature[degF]",
            PADDR(zone.hvac.cooling.balance_temperature), PT_double,
            "hvac.cooling.capacity[Btu/h]", PADDR(zone.hvac.cooling.capacity),
            PT_double, "hvac.cooling.capacity_perF[Btu/degF/h]",
            PADDR(zone.hvac.cooling.capacity_perF), PT_double,
            "hvac.cooling.design_temperature[degF]",
            PADDR(zone.hvac.cooling.design_temperature), PT_double,
            "hvac.cooling.efficiency[pu]", PADDR(zone.hvac.cooling.efficiency),
            PT_double, "hvac.cooling.cop[pu]", PADDR(zone.hvac.cooling.cop),

            PT_double, "hvac.heating.balance_temperature[degF]",
            PADDR(zone.hvac.heating.balance_temperature), PT_double,
            "hvac.heating.capacity[Btu/h]", PADDR(zone.hvac.heating.capacity),
            PT_double, "hvac.heating.capacity_perF[Btu/degF/h]",
            PADDR(zone.hvac.heating.capacity_perF), PT_double,
            "hvac.heating.design_temperature[degF]",
            PADDR(zone.hvac.heating.design_temperature), PT_double,
            "hvac.heating.efficiency[pu]", PADDR(zone.hvac.heating.efficiency),
            ...
            /* End-use data */
            PT_complex, "demand[kW]", PADDR(zone.total.demand), PT_complex,
            "total_load[kW]", PADDR(zone.total.power), PT_complex,
            "energy[kWh]", PADDR(zone.total.energy), PT_double, "power_factor",
            PADDR(zone.total.power_factor), PT_complex, "power[kW]",
            PADDR(zone.total.constant_power), PT_complex, "current[A]",
            PADDR(zone.total.constant_current), PT_complex, "admittance[1/Ohm]",
            PADDR(zone.total.constant_admittance),
            ...
            PT_double, "hvac.power_factor",
            PADDR(zone.hvac.enduse.power_factor),
            ...
            PT_double, "lights.power_factor",
            PADDR(zone.lights.enduse.power_factor), PT_double,
            "lights.heatgain_fraction",
            PADDR(zone.lights.enduse.heatgain_fraction), PT_double,
            "lights.heatgain[kW]", PADDR(zone.lights.enduse.heatgain),
            ...
            PT_double, "plugs.power_factor",
            PADDR(zone.plugs.enduse.power_factor), PT_double,
            "plugs.heatgain_fraction",
            PADDR(zone.plugs.enduse.heatgain_fraction), PT_double,
            "plugs.heatgain[kW]", PADDR(zone.plugs.enduse.heatgain),

            PT_double, "cooling_setpoint[degF]",
            PADDR(zone.control.cooling_setpoint), PT_double,
            "heating_setpoint[degF]", PADDR(zone.control.heating_setpoint),
            PT_double, "thermostat_deadband[degF]",
            PADDR(zone.control.setpoint_deadband), PT_double,
            "control.ventilation_fraction",
            PADDR(zone.control.ventilation_fraction), PT_double,
            "control.lighting_fraction", PADDR(zone.control.lighting_fraction),
    ...
    static double Tout = 59, RHout = 0.75,
                  Solar[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
...
/* convert a schedule into an occupancy buffer
   ...
   e.g.,
        "1-5 8-17;6,7 10,11,13,14" means Mon-Fri 8a-5p, and Sat & Holidays
   10a-noon and 1p-3p with Day 0 = Sunday, etc., day 7 = holiday Hour 0 =
   midnight to 1am
 */
static void occupancy_schedule(char *text, char occupied[24]) {
  ...
  for (p = text; true; p++) {
    if (*p == ';') { /* recursion on the rest of the schedule */
...
int office::init(OBJECT *parent) {
  ...
  /* sets up default office parameters if none were passed:  floor height = 9ft;
  interior mass = 2000 Btu/degF; interior/exterior ua = 2 Btu/degF/h; floor area
  = 4000 sf*/
  ...
  if (strcmp(zone.design.schedule, "") == 0)
    strcpy(zone.design.schedule,
           "1-5 8-17"); /* default is unoccupied, MTWRF 8a-5p is occupied */
  occupancy_schedule(zone.design.schedule, occupied);

  /* automatic sizing of HVAC equipment */
  if (zone.hvac.heating.capacity == 0)
    zone.hvac.heating.capacity =
        oversize * (zone.design.exterior_ua *
                        (zone.control.heating_setpoint -
                         zone.hvac.heating.design_temperature) /* envelope */
                    - (zone.hvac.heating.design_temperature -
                       zone.control.heating_setpoint) *
                          (0.2402 * 0.0735 * zone.design.floor_height *
                           zone.design.floor_area) *
                          zone.hvac.minimum_ach); /* vent */
  if (zone.hvac.cooling.capacity == 0)
    zone.hvac.cooling.capacity =
        oversize * (-zone.design.exterior_ua *
                        (zone.hvac.cooling.design_temperature -
                         zone.control.cooling_setpoint) /* envelope */
                    - (zone.design.window_area[0] + zone.design.window_area[1] +
                       zone.design.window_area[2] + zone.design.window_area[3] +
                       zone.design.window_area[4] + zone.design.window_area[5] +
                       zone.design.window_area[6] + zone.design.window_area[7] +
                       zone.design.window_area[8]) *
                          100 * 3.412 * zone.design.glazing_coeff /* solar */
                    - (zone.hvac.cooling.design_temperature -
                       zone.control.cooling_setpoint) *
                          (0.2402 * 0.0735 * zone.design.floor_height *
                           zone.design.floor_area) *
                          zone.hvac.minimum_ach /* vent */
                    - (zone.lights.capacity + zone.plugs.capacity) *
                          3.412); /* lights and plugs */
  ...
  if (climates->hit_count > 0) {
    ...
    zone.current.pTemperature =
        (double *)get_addr(obj, gl_get_property(obj, "temperature"));
    zone.current.pHumidity =
        (double *)get_addr(obj, gl_get_property(obj, "humidity"));
    zone.current.pSolar =
        (double *)get_addr(obj, gl_get_property(obj, "solar_flux"));
  }
  ...
  struct {
    ...
    bool test;
      ...
      {"control setpoint deadpoint is invalid",
       zone.control.setpoint_deadband <= 0},
      ...
      {"economizer cutin is above cooling setpoint deadband",
       zone.control.economizer_cutin >=
           zone.control.cooling_setpoint - zone.control.setpoint_deadband},
  };
  int i;
...
TIMESTAMP office::sync(TIMESTAMP t1) {
  ...
  if (dt1 > 0) {
    ...
    if (c2 != 0) {
      ...
      if (warn_control) {
        ...
        if (Ti < warn_low_temp || Ti > warn_high_temp) {
          ...
          gl_localtime(t1, &dt1);
          char ts0[64], ts1[64];
          gl_warning("office:%d (%s) air temperature excursion (%.1f degF) at "
                     "between %s and %s",
                     obj->id, obj->name ? obj->name : "anonymous", Ti,
                     gl_strtime(&dt0, ts0, sizeof(ts0)) ? ts0 : "UNKNOWN",
                     gl_strtime(&dt1, ts1, sizeof(ts1)) ? ts1 : "UNKNOWN");
        }
        ...
        if (Tm < warn_low_temp || Tm > warn_high_temp) {
          ...
          gl_localtime(t1, &dt1);
          char ts0[64], ts1[64];
          gl_warning("office:%d (%s) mass temperature excursion (%.1f degF) at "
                     "between %s and %s",
    ...
    const double Ca =
        0.2402 * 0.0735 * zone.design.floor_height * zone.design.floor_area;
    ...
    double f_air =
        1.0; /* adjust the fraction of gains that goes to air vs mass */
    ...
    /* calculate power */
    zone.total.power = zone.lights.enduse.power + zone.plugs.enduse.power +
                       zone.hvac.enduse.power;
    ...
    if (warn_control) {
      ...
      if ((mode == HC_HEAT || mode == HC_AUX) && Teq < TheatOff) {
        ...
        gl_warning("office:%d (%s) %s heating undersized between %s and %s",
                   ...
                   mode == HC_HEAT ? "primary" : "auxiliary",
                   gl_strtime(&dt0, ts0, sizeof(ts0)) ? ts0 : "UNKNOWN",
                   gl_strtime(&dt1, ts1, sizeof(ts1)) ? ts1 : "UNKNOWN");
      }
  ...
  if (Tevent == Teq)
    return -(
        t1 +
        (TIMESTAMP)(3600 * TS_SECOND)); /* soft return not more than an hour */
  ...
  if (isnan(dt2) || !isfinite(dt2) || dt2 < 0) {
    ...
    /* do not allow more than 1 degree/hour temperature change before solving
     * again */
    dt2 = fabs(3600 / dTi);
    if (dt2 > 3600)
      dt2 =
          3600; /* never more than an hour because of the occupancy schedule */
    return -(t1 + (TIMESTAMP)(dt2 * TS_SECOND)); /* soft return */
  }
  if (dt2 < TS_SECOND)
    return t1 + 1; /* need to do a second pass to get next state */
  else
    return t1 + (TIMESTAMP)(dt2 * TS_SECOND); /* return t2>t1 on success, t2=t1
                                                 for retry, t2<t1 on failure */
}
...
void office::update_control_setpoints() {
  ...
  if (TcoolOff - TheatOff <=
      0) // deadband needs to be smaller/ setpoints need to be farther apart
    throw (char *)"thermostat deadband causes heating/cooling turn-off points "
                  "to overlap";
  zone.control.ventilation_fraction =
      zone.current.occupancy > 0 ? zone.hvac.minimum_ach : 0;
}
...
TIMESTAMP office::update_lighting(TIMESTAMP t1) {
  ...
  zone.lights.enduse.heatgain =
      zone.lights.enduse.power.Mag() * zone.lights.enduse.heatgain_fraction;
  return TS_NEVER;
}
...
TIMESTAMP office::update_plugs(TIMESTAMP t1) {
  ...
  // heatgain calculation
  zone.plugs.enduse.heatgain =
      zone.plugs.enduse.power.Mag() * zone.plugs.enduse.heatgain_fraction;
  return TS_NEVER;
}
...
double office::update_hvac() {
  ...
  case HC_HEAT: /* system is heating normally */
    ...
    Qactive = zone.hvac.heating.capacity +
              zone.hvac.heating.capacity_perF *
                  (zone.hvac.heating.balance_temperature - Tout);
    Qvent =
        ((*zone.current.pTemperature) - zone.current.air_temperature) *
        (0.2402 * 0.0735 * zone.design.floor_height * zone.design.floor_area) *
        zone.control.ventilation_fraction;
    Tevent = TheatOff;
    break;
  ...
  case HC_COOL: /* system is actively cooling */
    cop = -1.0 -
          (zone.hvac.cooling.cop + 1) * (Tout - TmaxCool) / (TmaxCool - Tecon);
    Qactive = zone.hvac.cooling.capacity -
              zone.hvac.cooling.capacity_perF *
                  (Tout - zone.hvac.cooling.balance_temperature);
    Qvent =
        ((*zone.current.pTemperature) - zone.current.air_temperature) *
        (0.2402 * 0.0735 * zone.design.floor_height * zone.design.floor_area) *
        zone.control.ventilation_fraction;
  ...
  if (Qactive != 0)
    zone.hvac.enduse.power.SetPowerFactor(Qactive / cop / 1000,
                                          zone.hvac.enduse.power_factor);
  else
  ...
  if (Qvent != 0)
    zone.hvac.enduse.power +=
        gld::complex(.1, -0.01) / 1000 * zone.design.floor_area; /* ~ 1 W/sf */

  zone.hvac.enduse.energy += zone.hvac.enduse.power;
  if (zone.hvac.enduse.power.Re() < 0)
    throw "hvac unit is generating electricity";
  else if (!isfinite(zone.hvac.enduse.power.Re()) ||
           !isfinite(zone.hvac.enduse.power.Im()))
...
TIMESTAMP office::plc(TIMESTAMP t1) {
  ...
  const double &Taux =
      zone.control.heating_setpoint - zone.control.auxiliary_cutin;
  ...
  } else if (Tair <= TheatOn ||
             (mode == HC_AUX || mode == HC_HEAT)) // enter HEAT/AUX mode
  {
    if (Tair <= Taux)
      mode = HC_AUX;
    else
      mode = HC_HEAT;
             ...
             (mode == HC_ECON || mode == HC_COOL)) // enter ECON/COOL mode
  {
    if (Tout < Tecon) {
      /* compute ventilation needed to meet cooling demand */
      double Qgain = Qs + Qi - zone.design.exterior_ua * (Tair - Tout) -
                     zone.design.interior_ua * (Tair - Tmass);
      vent =
          Qgain / ((Tair - Tout) * (0.2402 * 0.0735 * zone.design.floor_height *
                                    zone.design.floor_area));
      if (vent < MinAch) {
        vent = MinAch;
        mode = HC_ECON;
