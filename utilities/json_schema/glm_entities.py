...
class Entity:
    ...
    def merge_instance(self, object_name, params, current_conditionals=None):
        """Merge parameters into an existing Entity instance.
        
        This method is used when a module or object is declared multiple times
        (typically in conditional blocks like #ifdef/#else). Instead of overwriting,
        it merges the new parameters with existing ones, creating conditional
        structures for properties that differ.

        Args:
            object_name (str): the name of the instance
            params (dict): list of the attribute parameters to merge
            current_conditionals (list): current conditional context (ifdef/ifndef/etc)
            
        Returns:
            Entity instance: the updated object with merged values
        """
        ...
        try:
            ...
            # If instance doesn't exist, just create it normally
            return self.set_instance(object_name, params)
        
        # Get the previous conditional context (stored during first instance creation)
        # Make a deep copy so we don't modify the original when merging
        import copy
        prev_conditionals = copy.deepcopy(instance.get('_conditionals', {}))
        current_conditionals = current_conditionals or []
        
        # Convert current conditionals list to dict format
        current_cond_dict = {}
        for cond in current_conditionals:
            t = cond["type"]
            if t in ("ifdef", "ifndef", "ifexist", "if", "ifnot"):
                if t not in current_cond_dict:
                    current_cond_dict[t] = []
                current_cond_dict[t].append(cond["condition"])
        
        # Merge conditionals
        if '_conditionals' in instance:
            for cond_type, cond_list in current_cond_dict.items():
                if cond_type not in instance['_conditionals']:
                    instance['_conditionals'][cond_type] = []
                for cond in cond_list:
                    if cond not in instance['_conditionals'][cond_type]:
                        instance['_conditionals'][cond_type].append(cond)
        elif current_cond_dict:
            instance['_conditionals'] = current_cond_dict
        ...
        for attr in params:
            ...
            if attr.startswith(("inline_comments", "inside_comments", "comment", 
                ...
                if attr in instance:
                    # Merge comment lists/dicts
                    if isinstance(instance[attr], list) and isinstance(params[attr], list):
                        instance[attr].extend(params[attr])
                    elif isinstance(instance[attr], dict) and isinstance(params[attr], dict):
                        instance[attr].update(params[attr])
                    else:
                        instance[attr] = params[attr]
                else:
                    instance[attr] = params[attr]
            ...
            if attr in instance:
                existing_value = instance[attr]
                new_value = params[attr]
                
                # If values are the same, no merge needed
                if existing_value == new_value:
                    continue
                
                # Values differ - create conditional structure
                # Check if already a conditional structure
                if isinstance(existing_value, dict) and any(k in existing_value for k in ['ifdef', 'ifndef', 'ifexist', 'if', 'ifnot']):
                    # Already conditional, add new branch
                    for cond_type, cond_list in current_cond_dict.items():
                        if cond_type not in existing_value:
                            existing_value[cond_type] = {}
                        for cond_val in cond_list:
                            existing_value[cond_type][cond_val] = new_value
                else:
                    # Convert to conditional structure
                    conditional_struct = {}
                    # Add existing value under its conditional
                    for cond_type, cond_list in prev_conditionals.items():
                        if cond_type not in conditional_struct:
                            conditional_struct[cond_type] = {}
                        for cond_val in cond_list:
                            conditional_struct[cond_type][cond_val] = existing_value
                    # Add new value under its conditional
                    for cond_type, cond_list in current_cond_dict.items():
                        if cond_type not in conditional_struct:
                            conditional_struct[cond_type] = {}
                        for cond_val in cond_list:
                            conditional_struct[cond_type][cond_val] = new_value
                    instance[attr] = conditional_struct
            else:
                # Attribute doesn't exist in instance yet
                # If we're in a conditional context, mark it as conditional
                if current_cond_dict:
                    conditional_struct = {}
                    for cond_type, cond_list in current_cond_dict.items():
                        if cond_type not in conditional_struct:
                            conditional_struct[cond_type] = {}
                        for cond_val in cond_list:
                            conditional_struct[cond_type][cond_val] = params[attr]
                    instance[attr] = conditional_struct
                else:
                    # No conditional context, just add it
                    item = self.find_item(attr)
                    if not isinstance(item, Item):
                        ...
                        if (self.find_item("parent") or self.find_item("configuration")):
                            ...
                            print(f"Unrecognized parameter {attr} in {self.entity} named {object_name}")
                    instance[attr] = params[attr]
        
        # Check for attributes that exist in the instance but not in params
        # These should also be marked as conditional (only exist in one branch)
        if prev_conditionals and current_cond_dict:
            for attr in list(instance.keys()):
                ...
                if attr.startswith(("inline_comments", "inside_comments", "comment",
                                   ...
                                   "_conditionals")):
                    continue
                # If attribute exists in instance but not in new params, mark as conditional
                if attr not in params:
                    existing_value = instance[attr]
                    # Only convert if not already conditional
                    if not (isinstance(existing_value, dict) and any(k in existing_value for k in ['ifdef', 'ifndef', 'ifexist', 'if', 'ifnot'])):
                        conditional_struct = {}
                        # Add existing value under its conditional
                        for cond_type, cond_list in prev_conditionals.items():
                            if cond_type not in conditional_struct:
