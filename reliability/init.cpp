...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  gl_global_create(
      "reliability::enable_subsecond_models", PT_bool, &enable_subsecond_models,
      PT_DESCRIPTION,
      "Flag to enable deltamode functionality in the reliability module",
      nullptr);
  gl_global_create("reliability::maximum_event_length", PT_double,
                   &event_max_duration, PT_UNITS, "s", PT_DESCRIPTION,
                   "Maximum duration of any faulting event", nullptr);
  gl_global_create("reliability::report_event_log", PT_bool,
                   &metrics::report_event_log, PT_DESCRIPTION,
                   "Should the metrics object dump a logfile?", nullptr);
  gl_global_create("reliability::deltamode_timestep", PT_double,
...
void schedule_deltamode_start(TIMESTAMP tstart) {
  if (enable_subsecond_models == true) // Make sure the overall mode is enabled
  {
    ...
    GL_THROW("reliability: a call was made to deltamode functions, but "
             "subsecond models are not enabled!");
    /*  TROUBLESHOOT
    The schedule_deltamode_start function was called by an object when
    reliability's overall enabled_subsecond_models flag was not set.  The
    module-level flag indicates that no devices should use deltamode, but one
    made the call to a deltamode function.  Please check the deltamode_inclusive
    flag on all objects.  If deltamode is desired, please set the module-level
...
EXPORT unsigned long preupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models == true) {
    if (deltamode_timestep_publish <= 0.0) {
      gl_error("reliability::deltamode_timestep must be a positive, non-zero "
               "number!");
      /*  TROUBLESHOOT
      The value for deltamode_timestep, as specified as the module level in
      reliability, must be a positive, non-zero number. Please use such a number
      and try again.
      */
...
EXPORT SIMULATIONMODE interupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models == true) {
    ...
    for (curr_object_number = 0; curr_object_number < eventgen_object_count;
         curr_object_number++) {
      ...
      if ((delta_objects[curr_object_number]->in_svc_double <=
        ...
        if (function_status == SM_DELTA) {
          gl_verbose(
              "Reliability object:%d - %s - requested deltamode to continue",
          ...
          gl_verbose(
              "Reliability object:%d - %s - requested a deltamode reiteration",
              delta_objects[curr_object_number]->id,
              (delta_objects[curr_object_number]->name
                   ? delta_objects[curr_object_number]->name
                   : "Unnamed"));
          ...
          gl_error("Reliability object:%d - %s - deltamode function returned "
                   "an error!",
                   delta_objects[curr_object_number]->id,
                   (delta_objects[curr_object_number]->name
                        ? delta_objects[curr_object_number]->name
