/** $Id: eventgen.cpp 1182 2008-12-22 22:08:36Z dchassin $
        ...
        The current implementation generates events with an exponential
 distribution.
...
CLASS *eventgen::oclass =
    nullptr; /**< a pointer to the CLASS definition in GridLAB-D's core */
eventgen *eventgen::defaults = nullptr; /**< a pointer to the default values
                                           used when creating new objects */

/* Class registration is only called once to register the class with the core */
eventgen::eventgen(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "eventgen", sizeof(eventgen),
    ...
    if (gl_publish_variable(
            oclass, PT_char1024, "target_group", PADDR(target_group),
            PT_char256, "fault_type", PADDR(fault_type), PT_enumeration,
            "failure_dist", PADDR(failure_dist), PT_KEYWORD, "UNIFORM",
            (enumeration)UNIFORM, PT_KEYWORD, "NORMAL", (enumeration)NORMAL,
            PT_KEYWORD, "LOGNORMAL", (enumeration)LOGNORMAL, PT_KEYWORD,
            "BERNOULLI", (enumeration)BERNOULLI, PT_KEYWORD, "PARETO",
            (enumeration)PARETO, PT_KEYWORD, "EXPONENTIAL",
            (enumeration)EXPONENTIAL, PT_KEYWORD, "RAYLEIGH",
            (enumeration)RAYLEIGH, PT_KEYWORD, "WEIBULL", (enumeration)WEIBULL,
            PT_KEYWORD, "GAMMA", (enumeration)GAMMA, PT_KEYWORD, "BETA",
            (enumeration)BETA, PT_KEYWORD, "TRIANGLE", (enumeration)TRIANGLE,
            PT_enumeration, "restoration_dist", PADDR(restore_dist), PT_KEYWORD,
            "UNIFORM", (enumeration)UNIFORM, PT_KEYWORD, "NORMAL",
            (enumeration)NORMAL, PT_KEYWORD, "LOGNORMAL",
            (enumeration)LOGNORMAL, PT_KEYWORD, "BERNOULLI",
            (enumeration)BERNOULLI, PT_KEYWORD, "PARETO", (enumeration)PARETO,
            PT_KEYWORD, "EXPONENTIAL", (enumeration)EXPONENTIAL, PT_KEYWORD,
            "RAYLEIGH", (enumeration)RAYLEIGH, PT_KEYWORD, "WEIBULL",
            (enumeration)WEIBULL, PT_KEYWORD, "GAMMA", (enumeration)GAMMA,
            PT_KEYWORD, "BETA", (enumeration)BETA, PT_KEYWORD, "TRIANGLE",
            (enumeration)TRIANGLE, PT_double, "failure_dist_param_1",
            PADDR(fail_dist_params[0]), PT_double, "failure_dist_param_2",
            PADDR(fail_dist_params[1]), PT_double, "restoration_dist_param_1",
            PADDR(rest_dist_params[0]), PT_double, "restoration_dist_param_2",
            PADDR(rest_dist_params[1]), PT_char1024, "manual_outages",
            PADDR(manual_fault_list), PT_double, "max_outage_length[s]",
            PADDR(max_outage_length_dbl), PT_int32, "max_simultaneous_faults",
            PADDR(max_simult_faults), PT_char256, "controlled_switch",
            PADDR(controlled_switch), PT_DESCRIPTION,
            "Name of a switch to manually fault/un-fault", PT_int32,
            "switch_state", PADDR(switch_state), PT_DESCRIPTION,
            "Current state (1=closed, 0=open) for the controlled switch",
            PT_char1024, "external_fault_event", PADDR(external_fault_event),
            PT_DESCRIPTION,
            "This variable is populated from external programs with a fault "
            "they would like to add/remove to the system.",
            PT_bool, "use_external_faults", PADDR(use_external_faults),
            PT_DESCRIPTION,
            "Boolean to let the object know to check for faults messages from "
            "an external source.",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    if (gl_publish_function(oclass, "add_event", (FUNCTIONADDR)add_event) ==
        nullptr)
      GL_THROW("Unable to publish reliability event adding function");
    if (gl_publish_function(oclass, "interupdate_event_object",
                            (FUNCTIONADDR)interupdate_eventgen) == nullptr)
...
int eventgen::create(void) {
  ...
  fail_dist_params[0] =
      1.0 / 2592000.0;       // 30 days mean time to failure (exponential)
  ...
  fault_implement_mode =
      false; // Assume by default that we are in randomized fault mode
...
int eventgen::init(OBJECT *parent) {
  ...
  if (glob_min_timestep > 1) // Now check us
  {
    ...
    gl_warning("eventgen:%s - minimum_timestep set - all events at least 1 "
               "minimum step long",
               hdr->name);
    /*  TROUBLESHOOT
    A minimum timestep value is set in the file.  Reliability works not-so-well
    when the events are starting and stopping less than this value.  All events
    are therefore going to be set to be at least 1 minimum timestep long.  This
    may alter your results, so please plan appropriately.
    */
  }
  ...
  if ((hdr->flags & OF_DELTAMODE) == OF_DELTAMODE) {
    ...
    {
      gl_warning("Eventgen:%s is flagged for deltamode, but reliability "
                 "deltamode is not enabled",
                 hdr->name);
      /*  TROUBLESHOOT
      The deltamode flag was set for the particular eventgen object, but the
      enabled_subsecond_models flag for the entire reliability module is not
      set.  Deltamode support is not enabled.  If this is not desired, be sure
      to set the module-level variable enabled_subsecond_models to true and try
      again.
      */
    }
  }

  // Secondary check -- theoretically core would catch and complain about this,
  // but do it here too
  if ((deltamode_inclusive == true) && (off_nominal_time == true)) {
    GL_THROW("event_gen:%s is flagged for deltamode, but minimum timesteps are "
             "enabled!",
             hdr->name);
    /*  TROUBLESHOOT
    An event_gen object is flagged for deltamode, but the simulation also has
    minimum timesteps enabled. This is not supported at this time.  Please
    disable one or the other to continue.
    */
  }
  ...
  if (max_outage_length_dbl > event_max_duration) {
    ...
    gl_warning(
        "Maximum outage for %s was above global max - restricted to global max",
        hdr->name);
    /*  TROUBLESHOOT
    The max_outage_length value exceeds the module global event length defined
    in maximum_event_length.  Th max_outage_length has been truncated to the
    module global value.  If this is undesired, specify and increase the global
    maximum_event_length value.
    */
  }
  ...
  {
    gl_warning("event_gen:%s does not have a metrics object as a parent -- "
               "metrics are not calculated.",
               hdr->name);
    /*  TROUBLESHOOT
    The event_gen object needs to be parented to a metrics object to ensure
    reliability metrics are being properly computed.  Please ensure the parent
    of this event_gen object is specified, and is a metrics object if
    reliability metrics are desired.
    */
    ...
    if (gl_object_isa(hdr->parent, "metrics", "reliability")) {
      ...
      // Map the functions
      metrics_object_event_ended = (FUNCTIONADDR)(gl_get_function(
          metrics_obj_hdr, "metrics_event_ended"));

      // Check it
      if (metrics_object_event_ended == nullptr) {
        GL_THROW(
            "Eventgen:%d - %s - Unable to map link power calculation function",
            hdr->id, (hdr->name ? hdr->name : "Unnamed"));
        /*  TROUBLESHOOT
        While attempting to map the metrics functions for the evengen object, an
      ...
      // Do the secondary
      metrics_object_event_ended_sec = (FUNCTIONADDR)(gl_get_function(
          metrics_obj_hdr, "metrics_event_ended_secondary"));
      ...
      metrics_get_interrupted_count = (FUNCTIONADDR)(gl_get_function(
          metrics_obj_hdr, "metrics_get_interrupted_count"));
      ...
      metrics_get_interrupted_count_sec = (FUNCTIONADDR)(gl_get_function(
          metrics_obj_hdr, "metrics_get_interrupted_count_secondary"));
    ...
    {
      GL_THROW("event_gen:%s must be parented to a metrics object to properly "
               "function.",
               hdr->name);
      // Defined above
    }
    ...
    secondary_interruption_cnt = new gld_property(
        metrics_obj_hdr, "perform_secondary_interruptions_count");

    // Make sure it worked
    if ((secondary_interruption_cnt->is_valid() != true) ||
        (secondary_interruption_cnt->is_bool() != true)) {
      GL_THROW("event_gen:%d - %s - Unable to map property for remote object",
               hdr->id, (hdr->name ? hdr->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attmepting to map to a remote object's property, an error occurred.
      Please try again.  If the error persists, please submit an issue via the
      ticketing system.
      */
    }
  }
  ...
  // And convert it back - maximum outage is being forced to whole seconds, just
  // because
  max_outage_length_dbl = (double)(max_outage_length);
  if (!use_external_faults) {
    // Determine how we should proceed - are we group-finding objects, or
    // list-finding objects?
    if (target_group[0] ==
        '\0') // Empty group list implies that manual list mode is desired
              // (programmer defined. i.e., I say so)
    {
      fault_implement_mode = true; // Flag as manually populated list mode

      // Count the number of commas in the list - use that as an initial basis
      // for validity
      index = 0;
      comma_count = 1;
      ...
      {
        GL_THROW("Manual event generation specified, but no events listed in "
                 "event_gen:%s",
                 hdr->name);
        /*  TROUBLESHOOT
        An eventgen object was set to manual mode, but no list of objects and
        fault times was found.  If target_group is left blank, specify objects
        and appropriate times using the manual_outages property.
        */
      }

      // Assuming it was more than one, make sure the count is in 3s (object,
      // fault time, rest time) before proceeding
      ...
      if (temp_double != 0.0) // odd amount, a comma error has occurred
      {
        ...
        /*  TROUBLESHOOT
        The manual list (manual_outages) is improperly specified.  The list must
        occur in sets of three comma separated values (object name, fault time,
        restoration time).  A comma is missing, or an extra comma is in place
        somewhere.  Please fix this to proceed.
        */
      }

      // See how many we found and make a big ol' structural array!
      UnreliableObjs = (OBJEVENTDETAILS *)gl_malloc(UnreliableObjCount *
                                                    sizeof(OBJEVENTDETAILS));

      // Make sure it worked
      if (UnreliableObjs == nullptr) {
        GL_THROW("Failed to allocate memory for object list in %s", hdr->name);
        /*  TROUBLESHOOT
        The event_gen object failed to allocate memory for the "unreliable"
        object list. Please try again.  If the error persists, submit your code
        and a bug report using the trac website.
        */
      }

      // Loop through the count and try parsing out what everything is - do
      ...
      for (index = 0; index < UnreliableObjCount; index++) {
        ...
        if (temp_obj == nullptr) {
          ...
          GL_THROW("eventgen:%s: fault object %s was not found!", hdr->name,
                   token_a);
          /*  TROUBLESHOOT
          While parsing the manual fault list for the evengen object, a fault
          point was not found.  Please check the name and try again.  If the
          problem persists, please submit your code and a bug report via the
          trac website.
          */
        }
        ...
        // Fault time
        token_a1 = time_token(token_a, &temp_time_A, &temp_time_A_nano,
                              &temp_time_A_dbl); // Pull next - starts on obj,
                                                 // we want to do a count

        token_a = token_a1; // Update the pointer

        // Restoration time
        token_a1 = time_token(token_a, &temp_time_B, &temp_time_B_nano,
                              &temp_time_B_dbl); // Pull next - starts on obj,
                                                 // we want to do a count
        ...
        {
          GL_THROW(
              "Manual event time in %s must be AFTER the simulation start time",
              hdr->name);
          /*  TROUBLESHOOT
          A manual event specified into eventgen has a start time before the
          simulation has actually started. Please fix this and try again.
          */
        }
        ...
        if (deltamode_inclusive == true) {
          ...
          // If over max outage length, warn here (it's manual mode, so let user
          // shoot themselves in the foot)
          if (temp_time_double > max_outage_length_dbl) {
            gl_warning("outage length for object:%s in eventgen:%s exceeds the "
                       "defined maximum outage.",
                       temp_obj->name, hdr->name);
            // Defined below
          }
          ...
          UnreliableObjs[index].rest_length_ns =
              (unsigned int)(1.0e9 *
                                 (temp_time_double -
                                  (double)(UnreliableObjs[index].rest_length)) +
                             0.5);
        } else //"Traditional" operation
        {
          ...
          if (tempTime > max_outage_length) {
            gl_warning("outage length for object:%s in eventgen:%s exceeds the "
                       ...
                       temp_obj->name, hdr->name);
            /*  TROUBLESHOOT
            An outage length specified for the manual event generation mode is
            longer than the value defined in max_outage_length. If this is not
            desired, fix the input.  If it is acceptable, the simulation will
            run normally.
            */
          }
          ...
          if ((off_nominal_time == true) &&
              (((double)(tempTime)) < glob_min_timestep)) {
            // Force to be at least one
            tempTime = (TIMESTAMP)(glob_min_timestep);

            // Update the restoration time appropriately
            UnreliableObjs[index].rest_time =
                UnreliableObjs[index].fail_time + tempTime;

            // Toss a warning, for giggles
            gl_warning("Outage length for object:%s is less than the minimum "
                       "timestep of %.0f seconds, rounded to minimum timestep",
                       temp_obj->name, glob_min_timestep);
            /*  TROUBLESHOOT
            The selected outage length is less than the mininum timestep set. It
            has been forced to be at least one minimum timestep long.
            */
          }
      ...
      // Pass in parameters for statistics - for tracking - put as 0's and
      // "none" to flag out
    ...
    } else // Random mode, proceed with that assumption
    {
      ...
      if (ObjListVals == nullptr) {
        GL_THROW("Failure to find devices for %s specified as: %s", hdr->name,
                 target_group.get_string());
        /*  TROUBLESHOOT
        While attempting to populate the list of devices to test reliability on,
        the event_gen object failed to find any desired objects.  Please make
        sure the objects exist and try again. If the bug persists, please submit
        your code using the trac website.
        */
      }
      ...
      // Loop through and init them - can't compute exact time, but can populate
      // array
      temp_obj = nullptr;
      for (index = 0; index < UnreliableObjCount; index++) {
        // Find the object
        ...
        gen_random_time(failure_dist, fail_dist_params[0], fail_dist_params[1],
                        &UnreliableObjs[index].fail_length,
                        &UnreliableObjs[index].fail_length_ns,
                        &UnreliableObjs[index].fail_length_dbl);

        // Find restoration time
        gen_random_time(restore_dist, rest_dist_params[0], rest_dist_params[1],
                        &temp_time_A, &temp_time_A_nano, &temp_time_A_dbl);

        // If over max outage length, cap it - side note - minimum timestep
        // issues handled inside gen_random_time
  ...
  if (((max_simult_faults == -1) || (max_simult_faults > 1)) &&
      (metrics_obj_hdr !=
       nullptr)) // infinite or more than 1 - and metrics are on, so we care
  {
    gl_warning("event_gen:%s has the ability to generate more than 1 "
               "simultaneous fault - metrics may not be accurate",
               hdr->name);
    /*  TROUBLESHOOT
    The event_gen object has the ability to induce simultaneous and concurrent
    fault conditions.  This may affect the accuracy of your reliability metrics.
    */
  } else if ((max_simult_faults == 0) || (max_simult_faults < -1)) {
    GL_THROW("event_gen:%s must have a valid maximum simultaneous fault count "
             "number",
             hdr->name);
    /*  TROUBLESHOOT
    Event_gen objects require the max_simultaneous_faults property to be
    properly defined to run. This must be either -1, or a number greater than or
    equal to 1.  -1 indicates "infinite" simultaneous faults, so it is
    conceivable that every candidate object can be a fault condition
    concurrently.
    */
  }
  ...
  if (deltamode_inclusive) {
    ...
    if (enable_subsecond_models != true) {
      gl_warning("eventgen:%s indicates it wants to run deltamode, but the "
                 "module-level flag is not set!",
                 hdr->name ? hdr->name : "unnamed");
      /*  TROUBLESHOOT
      The eventgen object has the deltamode_inclusive flag set, but not the
      module-level enable_subsecond_models flag.  The event will not simulate
...
int eventgen::precommit(TIMESTAMP t1) { return 1; }
...
TIMESTAMP eventgen::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  // All passes start out assuming they don't need a new count update and
  ...
  if (!use_external_faults) {
    ...
    if (strlen(controlled_switch) > 0) {
      if (switch_state != last_switch_state) {
        cout << "Switch " << controlled_switch << " changing status to "
             << switch_state << " at " << t0 << " going to " << t1 << endl;
    ...
    if (next_event_time == 0) {
      ...
      for (index = 0; index < UnreliableObjCount; index++) {
        ...
        if (fault_implement_mode == false) {
          ...
          if (deltamode_inclusive == true) {
            UnreliableObjs[index].fail_time_dbl =
                t1_dbl + UnreliableObjs[index].fail_length_dbl;
            UnreliableObjs[index].fail_time =
                (TIMESTAMP)(floor(UnreliableObjs[index].fail_time_dbl));
            UnreliableObjs[index].fail_time_ns =
                (unsigned int)((UnreliableObjs[index].fail_time_dbl -
                                (double)(UnreliableObjs[index].fail_time)) *
                                   1.0e9 +
                               0.5);
          } else // Nope
          {
            UnreliableObjs[index].fail_time =
                t1 + UnreliableObjs[index].fail_length;
          }
        }

        // See if it is a new minimum - don't need to check fault state - first
        // run assumes all are good
      ...
      // Linked list is ignored on this first run - it will get caught as part
                                   ...
                                   // yet
      {
        if ((eventgen_object_current == -1) || (delta_objects == nullptr)) {
          ...
          delta_objects =
              (OBJECT **)gl_malloc(eventgen_object_count * sizeof(OBJECT *));

          // Make sure it worked
          if (delta_objects == nullptr) {
            GL_THROW("Failed to allocate deltamode objects array for "
                     "reliability module!");
            /*  TROUBLESHOOT
            While attempting to create a reference array for reliability module
          ...
          delta_functions = (FUNCTIONADDR *)gl_malloc(eventgen_object_count *
                                                      sizeof(FUNCTIONADDR));
        ...
        if (eventgen_object_current >= eventgen_object_count) {
          GL_THROW("Too many objects tried to populate deltamode objects array "
                   "in the reliability module!");
          /*  TROUBLESHOOT
          While attempting to populate a reference array of deltamode-enabled
          objects for the reliability module, an attempt was made to write
          beyond the allocated array space.  Please try again.  If the error
          persists, please submit a bug report and your code via the trac
          website.
          */
        }
        ...
        delta_functions[eventgen_object_current] =
            (FUNCTIONADDR)(gl_get_function(hdr, "interupdate_event_object"));

        // Make sure it worked
        if (delta_functions[eventgen_object_current] == nullptr) {
          GL_THROW("Failure to map deltamode function for device:%s",
                   hdr->name);
          /*  TROUBLESHOOT
          Attempts to map up the interupdate function of a specific device
          failed.  Please try again and ensure the object supports deltamode. If
    ...
    // If the next time point is the whole second right before the time for a
    // deltamode event, we need to schedule to enter deltamode.
    if ((next_event_time == t1) && deltamode_inclusive) {
      // Provide a check for if the event is beyond stop time -- if it is, we
      // get stuck here and iterate forever
      ...
      if (next_event_time_dbl <= gld_stoptime) {
        ...
        /*********************** May need to revisit this portion -- might still
         * be needed ********************************************/

        return -next_event_time; // Return here and we will do all the work in
                                 // deltamode
      } else // Arbitrarily return one more - it will trip the core
      {
        ...
        // Send this out
        return -next_event_time; // Theoretically 1 forward in time, which means
                                 // core will quit us
      }
    }
    ...
    {
      return -next_event_time; // Negative return, we'd like to go there if
    ...
    if (!external_events.empty()) {
      ...
      for (i = external_events.begin(); i != external_events.end();) {
        if ((*i)->enable_event && !(*i)->event_enabled) {
          ...
          funadd = (FUNCTIONADDR)(gl_get_function((*i)->fault_object,
                                                  "create_fault"));
          ...
          returnval = ((int (*)(OBJECT *, OBJECT **, char *, int *,
                                TIMESTAMP *))(*funadd))(
              (*i)->fault_object, &(*i)->effected_safety_device, (*i)->type,
              &(*i)->implemented_fault, &mean_repair_time);
          ...
          returnval = ((int (*)(OBJECT *, int *, char *))(*funadd))(
              (*i)->fault_object, &(*i)->implemented_fault, impl_fault);
...
TIMESTAMP eventgen::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  int after_count, after_count_sec, differential_count, differential_count_sec,
      index;
  ...
  // See if we need a "post-fault" count - assumes all customers will determine
  // their outage state by either presync or sync (or before this in postsync)
  if ((diff_count_needed == true) && (metrics_obj_hdr != nullptr)) {
    ...
    if (temp_bool == true) {
      ...
      temp_status = ((STATUS (*)(OBJECT *, int *, int *))(
          *metrics_get_interrupted_count_sec))(metrics_obj_hdr, &after_count,
                                               &after_count_sec);
      ...
      if (temp_status == FAILED) {
        GL_THROW("Eventgen:%d - %s - an error occurred while trying to get the "
                 "object interruption count",
                 hdr->id, (hdr->name ? hdr->name : "Unnamed"));
        /*  TROUBLESHOOT
        While attempting to get the interrupted object count, an error occurred.
        Please try again.  If the error persists, please submit an issue via the
        ticketing system.
        */
      }
      ...
      // See how it sits - if it is negative, toss a warning and just use the
      // after_count value
      if (differential_count <
          0) // Must be positive or zero, otherwise things were restored
      {
        gl_warning(
            "Afflicted object count went negative - using after fault count");
        /*  TROUBLESHOOT
        While determining the number of objects faulted by an event, a negative
        count was encountered. This indicates some objects magically restored
        themselves before the final count could be obtained.  This may be a
        logic fault in the code, or the event generation logic.  Pleas submit
      ...
      // See how it sits - if it is negative, toss a warning and just use the
      // after_count value
      if (differential_count_sec <
          0) // Must be positive or zero, otherwise things were restored
      {
        gl_warning("Afflicted secondary object count went negative - using "
                   "after fault count");
        /*  TROUBLESHOOT
        While determining the number of objects secondarily faulted by an event,
        a negative count was encountered. This indicates some objects magically
        restored themselves before the final count could be obtained.  This may
        be a logic fault in the code, or the event generation logic.  Pleas
      ...
      if (Unhandled_Events.next != nullptr) // Something is in there!
      {
        ...
        while (temp_struct->next != nullptr) {
          ...
          if (temp_struct->objdetails.customers_affected_sec ==
              -1) // We need it
            temp_struct->objdetails.customers_affected_sec =
                differential_count_sec;
        } // End LL while
      } // end LL if
    } else // No secondaries
    {
      ...
      // Get current number of customers out of service
      temp_status = ((STATUS (*)(OBJECT *, int *))(
          *metrics_get_interrupted_count))(metrics_obj_hdr, &after_count);
...
void eventgen::gen_random_time(enumeration rand_dist_type, double param_1,
                               double param_2, TIMESTAMP *event_time,
                               unsigned int *event_nanoseconds,
                               double *event_double) {
  ...
  if ((off_nominal_time == true) && (dbl_random_time < glob_min_timestep)) {
    ...
    // Toss a warning, for giggles
    gl_warning("Outage length is less than the minimum timestep of %.0f "
               "seconds, rounded to minimum timestep",
               glob_min_timestep);
    // Defined above
  }
  ...
  *event_time = random_time;
  ...
  if (deltamode_inclusive == true) {
    ...
    ns_random_time =
        (unsigned int)((dbl_random_time - (double)(random_time)) * 1e9 + 0.5);
...
// Function to parse a comma-separated list to get the next timestamp (or the
// last timestamp)
//  start_token - pointer to character field
//  time_val - pointer to TIMESTAMP variable of "normal" time
//  micro_time_val - pointer to microsecond portion of TIMESTAMP variable, if
//  available dbl_time_val - pointer to double-precision (combined) time
//  variable
char *eventgen::time_token(char *start_token, TIMESTAMP *time_val,
                           unsigned int *micro_time_val, double *dbl_time_val) {
...
char *eventgen::obj_token(char *start_token, OBJECT **obj_val) {
  char workArray[128]; // Hopefully, names will never be over 128 characters -
...
// Function to add new event into structure - since externally called, presumes
// calling object will handle mean_repair_time calls
int eventgen::add_unhandled_event(OBJECT *obj_to_fault, const char *event_type,
                                  TIMESTAMP fail_time, TIMESTAMP rest_length,
                                  int implemented_fault, bool fault_state) {
  OBJECT *obj = object_header(this);
  ...
  if (new_struct == nullptr) {
    GL_THROW(
        "Eventgen:%s - Failed to allocate memory for new reliability event!",
        obj->name);
    /*  TROUBLESHOOT
    While attempting to allocate memory for a new "unhandled" reliability event,
    the memory allocation failed.  Please try again.  If the error persists,
    plesa submit your code and a bug report via the trac website.
    */
  }

  // Force rest_length to at least one timestep, if it isn't
  if ((off_nominal_time == true) &&
      (((double)(rest_length)) < glob_min_timestep)) {
    rest_length = (TIMESTAMP)(glob_min_timestep); // Force to one

    // Warning
    gl_warning("Outage length for object:%s is less than the minimum timestep "
               "of %.0f seconds, rounded to minimum timestep",
               obj_to_fault->name, glob_min_timestep);
    // Defined above
  }
...
void eventgen::regen_events(TIMESTAMP t1_ts, double t1_dbl) {
  ...
  // See if the distribution parameters have changed - if so, regen all faults
  // (items in fault condition will wait until restoration to update) Only do
  // this if in random mode
  if ((fault_implement_mode == false) &&
      ((curr_fail_dist_params[0] != fail_dist_params[0]) ||
       (curr_fail_dist_params[1] != fail_dist_params[1]) ||
       (curr_rest_dist_params[0] != rest_dist_params[0]) ||
       (curr_rest_dist_params[1] != rest_dist_params[1]) ||
       (curr_fail_dist != failure_dist) || (curr_rest_dist != restore_dist))) {
    ...
    for (index = 0; index < UnreliableObjCount; index++) {
      if (UnreliableObjs[index].in_fault ==
          false) // Not faulting, so we don't care if we are now a fault or if
                 // we have one upcoming
      {
        // Update the failure and restoration length (do this now) - mininmum
        // timestep issues are handled inside gen_random_time
        gen_random_time(failure_dist, fail_dist_params[0], fail_dist_params[1],
                        &UnreliableObjs[index].fail_length,
                        &UnreliableObjs[index].fail_length_ns,
                        &UnreliableObjs[index].fail_length_dbl);
        ...
        } else // Standard operation
        {
          // Update failure time
          UnreliableObjs[index].fail_time =
              t1_ts + UnreliableObjs[index].fail_length;
        }
      ...
      else // Implies it is in a fault - update next_event_time (if it is done,
...
void eventgen::do_event(TIMESTAMP t1_ts, double t1_dbl, bool entry_type) {
  ...
  for (index = 0; index < UnreliableObjCount; index++) {
    // Check failure time
    if ((((UnreliableObjs[index].fail_time <= t1_ts) &&
          (entry_type == false)) ||
         ((UnreliableObjs[index].fail_time_dbl <= t1_dbl) &&
          (entry_type == true))) &&
        (UnreliableObjs[index].in_fault == false)) // Failure!
    {
      // See if we're allowed to fault
      if ((faults_in_prog < max_simult_faults) ||
          (max_simult_faults == -1)) // Room to fault or infinite amount
      {
        ...
        if ((diff_count_needed == false) && (metrics_obj_hdr != nullptr)) {
          ...
          if (temp_bool == true) {
            ...
            // Get current number of customers out of service
            temp_status = ((STATUS (*)(OBJECT *, int *, int *))(
                *metrics_get_interrupted_count_sec))(
                metrics_obj_hdr, &curr_time_interrupted,
                &curr_time_interrupted_sec);
            ...
            if (temp_status == FAILED) {
              GL_THROW("Eventgen:%d - %s - an error occurred while trying to "
                       "get the object interruption count",
                       hdr->id, (hdr->name ? hdr->name : "Unnamed"));
              // Defined above
            }
          } else // No secondaries
          {
            ...
            // Get current number of customers out of service
            temp_status =
                ((STATUS (*)(OBJECT *, int *))(*metrics_get_interrupted_count))(
                    metrics_obj_hdr, &curr_time_interrupted);
        ...
        funadd = (FUNCTIONADDR)(gl_get_function(
            UnreliableObjs[index].obj_of_int, "create_fault"));

        // Make sure it was found
        if (funadd == nullptr) {
          GL_THROW("Unable to induce event on %s",
                   UnreliableObjs[index].obj_of_int->name);
          /*  TROUBLESHOOT
          While attempting to create or restore a fault, the eventgen failed to
          find the fault-inducing or fault-fixing function on the object of
          interest.  Ensure this object type supports faulting and try again. If
          the problem persists, please submit a bug report and your code to the
          trac website.
          */
        }
        ...
        // replace wlock with SharedMutexManager
        auto &v =
            SharedMutexManager::get_mutex(UnreliableObjs[index].obj_of_int);
        std::unique_lock<std::shared_mutex> lock(v);

        if (metrics_obj_hdr != nullptr) {
          returnval = ((int (*)(OBJECT *, OBJECT **, char *, int *,
                                TIMESTAMP *))(*funadd))(
              UnreliableObjs[index].obj_of_int,
              &UnreliableObjs[index].obj_made_int, fault_type.get_string(),
              &UnreliableObjs[index].implemented_fault, &mean_repair_time);
        } else {
          returnval = ((int (*)(OBJECT *, OBJECT **, char *, int *,
                                TIMESTAMP *))(*funadd))(
              UnreliableObjs[index].obj_of_int,
        ...
        {
          GL_THROW("Failed to induce event on %s",
                   UnreliableObjs[index].obj_of_int->name);
          /*  TROUBLESHOOT
          Eventgen attempted to induce a fault on the object, but the object
          failed to induce the fault. Check the object's create_fault code.  If
          necessary, submit your code and a bug report using the trac website.
          */
        }
        ...
        if (fault_implement_mode == false) // If random mode
        {
          ...
          // Deltamode check - handle times "traditionally" or not
          if (deltamode_inclusive == true) {
            UnreliableObjs[index].rest_time_dbl =
                t1_dbl + UnreliableObjs[index].rest_length_dbl;
            UnreliableObjs[index].rest_time =
                (TIMESTAMP)(floor(UnreliableObjs[index].rest_time_dbl));
            UnreliableObjs[index].rest_time_ns =
                (unsigned int)((UnreliableObjs[index].rest_time_dbl -
                                (double)(UnreliableObjs[index].rest_time)) *
                                   1.0e9 +
                               0.5);
          } else // Nope
          {
            UnreliableObjs[index].rest_time =
                t1_ts + UnreliableObjs[index].rest_length;
          }
        }
        ...
        // Increment the simultaneous fault counter (prevents later objects from
        // faulting if they are ready)
        faults_in_prog++;
      } else // Fault limit has been hit, give us a new time to play
      {
        if (fault_implement_mode == false) // Random mode - OK to do this
        {
          // Update the failure and restoration length (do this now) - minimum
          // timestep issues handled inside gen_random_time
          gen_random_time(failure_dist, fail_dist_params[0],
                          fail_dist_params[1],
                          &UnreliableObjs[index].fail_length,
                          &UnreliableObjs[index].fail_length_ns,
                          &UnreliableObjs[index].fail_length_dbl);
        ...
        } else // Deterministic mode - if this happens, a parameter was set
               // wrong.  Flag this as already occurred and move on
        {
          // Toss the warning
          gl_warning("Manual event %d of eventgen:%s exceeded the simultaneous "
                     "fault limit and was ignored",
                     index, hdr->name);
          /*  TROUBLESHOOT
          While processing manual fault conditions, a fault attempted to exceed
          the maximum number of faults limit set for this eventgen object.  This
          fault is now ignored.  Increase the maximum simultaneous fault limit
          and try again.
          */
    ...
    } else if (((UnreliableObjs[index].rest_time <= t1_ts) &&
                (entry_type == false)) ||
               ((UnreliableObjs[index].rest_time_dbl <= t1_dbl) &&
                (entry_type == true))) // Restoration time!
    {
      // Call the object back into service
      funadd = (FUNCTIONADDR)(gl_get_function(UnreliableObjs[index].obj_of_int,
                                              "fix_fault"));
      ...
      if (metrics_obj_hdr != nullptr) {
        returnval = ((int (*)(OBJECT *, int *, char *))(*funadd))(
            UnreliableObjs[index].obj_of_int,
            &UnreliableObjs[index].implemented_fault, impl_fault);
      } else {
        returnval = ((int (*)(OBJECT *, int *, char *))(*funadd))(
            UnreliableObjs[index].obj_of_int,
            &UnreliableObjs[index].implemented_fault, impl_fault);
      ...
      if (returnval == 0) // Restoration is no go :(
      {
        GL_THROW("Failed to induce repair on %s",
                 UnreliableObjs[index].obj_of_int->name);
        /*  TROUBLESHOOT
        Eventgen attempted to induce a repair on the object, but the object
        failed to perform the repair. Check the object's fix_fault code.  If
        necessary, submit your code and a bug report using the trac website.
        */
      }
      ...
      if (metrics_obj_hdr != nullptr) {
        ...
        if (temp_bool == true) {
          temp_status = ((STATUS (*)(OBJECT *, OBJECT *, OBJECT *, OBJECT *,
                                     TIMESTAMP, TIMESTAMP, char *, char *, int,
                                     int))(*metrics_object_event_ended_sec))(
              metrics_obj_hdr, hdr, UnreliableObjs[index].obj_of_int,
              UnreliableObjs[index].obj_made_int,
              UnreliableObjs[index].fail_time, UnreliableObjs[index].rest_time,
              fault_type.get_string(), impl_fault,
              UnreliableObjs[index].customers_affected,
              UnreliableObjs[index].customers_affected_sec);
        } else // no secondaries
        {
          temp_status = ((STATUS (*)(OBJECT *, OBJECT *, OBJECT *, OBJECT *,
                                     TIMESTAMP, TIMESTAMP, char *, char *,
                                     int))(*metrics_object_event_ended))(
              metrics_obj_hdr, hdr, UnreliableObjs[index].obj_of_int,
              UnreliableObjs[index].obj_made_int,
              UnreliableObjs[index].fail_time, UnreliableObjs[index].rest_time,
              fault_type.get_string(), impl_fault,
        ...
        if (temp_status == FAILED) {
          GL_THROW("Eventgen:%d - %s - an error occurred while trying to end "
                   "an event",
                   hdr->id, (hdr->name ? hdr->name : "Unnamed"));
          /*  TROUBLESHOOT
          While attempting to end a reliability event, an error occurred. Please
          try again.  If the error persists, please submit an issue via the
          ticketing system.
          */
        }
      }
      ...
      // Decrement us out of the simultaneous fault count (allows possibility of
      // later object in list to fault before count updated)
      faults_in_prog--;
  ...
  if (Unhandled_Events.next != nullptr) // Something is in there!
  {
    ...
    while (temp_struct->next != nullptr) {
      ...
      // Check failure time
      if ((((temp_struct->objdetails.fail_time <= t1_ts) &&
            (entry_type == false)) ||
           ((temp_struct->objdetails.fail_time_dbl <= t1_dbl) &&
            (entry_type == true))) &&
          (temp_struct->objdetails.in_fault == false)) // Failure!
      {
        ...
        // Put a fault on the system
        funadd = (FUNCTIONADDR)(gl_get_function(
            temp_struct->objdetails.obj_of_int, "create_fault"));

        // Make sure it was found
        if (funadd == nullptr) {
          GL_THROW("Unable to induce event on %s",
                   temp_struct->objdetails.obj_of_int->name);
          // Defined above
        }
        ...
        // replace wlock with SharedMutexManager
        auto &v =
            SharedMutexManager::get_mutex(temp_struct->objdetails.obj_of_int);
        std::unique_lock<std::shared_mutex> lock(v);

        if (metrics_obj_hdr != nullptr) {
          returnval = ((int (*)(OBJECT *, OBJECT **, char *, int *,
                                TIMESTAMP *))(*funadd))(
              temp_struct->objdetails.obj_of_int,
              &temp_struct->objdetails.obj_made_int, temp_struct->event_type,
              &temp_struct->objdetails.implemented_fault, &mean_repair_time);
        } else {
          returnval = ((int (*)(OBJECT *, OBJECT **, char *, int *,
                                TIMESTAMP *))(*funadd))(
              temp_struct->objdetails.obj_of_int,
        ...
        if ((mean_repair_time != 0) &&
            (temp_struct->objdetails.rest_time != TS_NEVER)) {
        ...
        // See if the restoration time is TS_NEVER - if so, we are an induced
        // fault that will be undone separately - delete us
      ...
      } else if (((temp_struct->objdetails.rest_time <= t1_ts) &&
                  (entry_type == false)) ||
                 ((temp_struct->objdetails.rest_time_dbl <= t1_dbl) &&
                  (entry_type == true))) // Restoration time!
      {
        // Call the object back into service
        funadd = (FUNCTIONADDR)(gl_get_function(
            temp_struct->objdetails.obj_of_int, "fix_fault"));
        ...
        if (metrics_obj_hdr != nullptr) {
          returnval = ((int (*)(OBJECT *, int *, char *))(*funadd))(
              temp_struct->objdetails.obj_of_int,
              &temp_struct->objdetails.implemented_fault, impl_fault);
        } else {
          returnval = ((int (*)(OBJECT *, int *, char *))(*funadd))(
              temp_struct->objdetails.obj_of_int,
              &temp_struct->objdetails.implemented_fault, impl_fault);
        ...
        if (metrics_obj_hdr != nullptr) {
          ...
          if (temp_bool == true) {
            temp_status = ((STATUS (*)(
                OBJECT *, OBJECT *, OBJECT *, OBJECT *, TIMESTAMP, TIMESTAMP,
                char *, char *, int, int))(*metrics_object_event_ended_sec))(
                metrics_obj_hdr, hdr, temp_struct->objdetails.obj_of_int,
                temp_struct->objdetails.obj_made_int,
                temp_struct->objdetails.fail_time,
                temp_struct->objdetails.rest_time, temp_struct->event_type,
                impl_fault, temp_struct->objdetails.customers_affected,
                temp_struct->objdetails.customers_affected_sec);
          } else // no secondaries
          {
            temp_status = ((STATUS (*)(OBJECT *, OBJECT *, OBJECT *, OBJECT *,
        ...
        // Decrement us out of the simultaneous fault count (allows possibility
        // of later object in list to fault before count updated)
        faults_in_prog--;
  ...
  // Reset and update our current fault counter (just to ensure things are
  // accurate)
  faults_in_prog = 0;
...
//////////////////////////////////////////////////////////////////////////
// Module-level call
EXPORT SIMULATIONMODE interupdate_eventgen(OBJECT *obj,
...
SIMULATIONMODE eventgen::inter_deltaupdate(unsigned int64 delta_time,
                                           unsigned long dt,
                                           unsigned int iteration_count_val) {
  ...
  double delta_epsilon =
      (double)dt / (double)DT_SECOND /
      2; // A small increase to t1 to make sure we stay in deltamode if an
         // event's time lands on the whole second and the next event's time is
         // within a second

  t1_ts = (TIMESTAMP)(t1_dbl + 1.0 / (2.0 * ((double)(DT_SECOND))));

  if (next_event_time_dbl <
      ceil(t1_dbl + delta_epsilon)) { // Is there an event to do between now and
    ...
    for (auto &i : json_events.items()) {
      ...
      if (!event_exists) {
        ...
        std::string obj_name_str =
            json_event["fault_object"].template get<std::string>();
        ...
        if (new_event->fault_object != nullptr) {
          ...
          gl_warning("eventgen::parse_external_fault_event: no object with "
                     "name %s was found. Ignoring fault event.",
                     obj_name_str.c_str());
        }
      }
    }
  } catch (...) {
    GL_THROW("eventgent::parse_external_fault_events():Json parse failed to "
             "parse string = %s",
             events_char);
  }
}
...
EXPORT int add_event(OBJECT *event_obj, OBJECT *obj_to_fault, char *event_type,
                     TIMESTAMP fail_time, TIMESTAMP rest_length,
                     int implemented_fault, bool fault_state) {
  int ret_value;
  ...
  // Call the function
  ret_value = eventgenobj->add_unhandled_event(obj_to_fault, event_type,
                                               fail_time, rest_length,
                                               implemented_fault, fault_state);
