/** $Id: range.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The range simulation is based on a demand profile attached to the
 object. The internal heat gain is calculated as the demand fraction of
 installed power.
...
range::range(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "range", sizeof(range),
    ...
    if (gl_publish_variable(
            ...
            "oven_volume[gal]", PADDR(oven_volume), PT_DESCRIPTION,
            "the volume of the oven", PT_double, "oven_UA[Btu/degF*h]",
            PADDR(oven_UA), PT_DESCRIPTION,
            "the UA of the oven (surface area divided by R-value)", PT_double,
            "oven_diameter[ft]", PADDR(oven_diameter), PT_DESCRIPTION,
            "the diameter of the oven",

            PT_double, "oven_demand[gpm]", PADDR(oven_demand), PT_DESCRIPTION,
            "the hot food take out from the oven",

            PT_double, "heating_element_capacity[kW]",
            PADDR(heating_element_capacity), PT_DESCRIPTION,
            "the power of the heating element", PT_double,
            "inlet_food_temperature[degF]", PADDR(Tinlet), PT_DESCRIPTION,
            "the inlet temperature of the food", PT_enumeration, "heat_mode",
            PADDR(heat_mode), PT_DESCRIPTION,
            "the energy source for heating the oven", PT_KEYWORD, "ELECTRIC",
            (enumeration)ELECTRIC, PT_KEYWORD, "GASHEAT", (enumeration)GASHEAT,
            PT_enumeration, "location", PADDR(location), PT_DESCRIPTION,
            "whether the range is inside or outside", PT_KEYWORD, "INSIDE",
            (enumeration)INSIDE, PT_KEYWORD, "GARAGE", (enumeration)GARAGE,
            PT_double, "oven_setpoint[degF]", PADDR(oven_setpoint),
            PT_DESCRIPTION,
            "the temperature around which the oven will heat its contents",
            PT_double, "thermostat_deadband[degF]", PADDR(thermostat_deadband),
            PT_DESCRIPTION,
            "the degree to heat the food in the oven, when needed", PT_double,
            "temperature[degF]", PADDR(Tw), PT_DESCRIPTION,
            "the outlet temperature of the oven", PT_double, "height[ft]",
            PADDR(h), PT_DESCRIPTION, "the height of the oven",

            PT_double, "food_density", PADDR(food_density), PT_DESCRIPTION,
            "food density", PT_double, "specificheat_food",
            PADDR(specificheat_food),

            PT_double, "queue_cooktop[unit]", PADDR(enduse_queue_cooktop),
            PT_DESCRIPTION, "number of loads accumulated",

            PT_double, "queue_oven[unit]", PADDR(enduse_queue_oven),
            PT_DESCRIPTION, "number of loads accumulated",

            PT_double, "queue_min[unit]", PADDR(queue_min), PT_double,
            "queue_max[unit]", PADDR(queue_max),

            PT_double, "time_cooktop_operation", PADDR(time_cooktop_operation),
            PT_double, "time_cooktop_setting", PADDR(time_cooktop_setting),

            PT_double, "cooktop_run_prob", PADDR(cooktop_run_prob), PT_double,
            "oven_run_prob", PADDR(oven_run_prob),

            PT_double, "cooktop_coil_setting_1[kW]",
            PADDR(cooktop_coil_power[0]), PT_double,
            "cooktop_coil_setting_2[kW]", PADDR(cooktop_coil_power[1]),
            PT_double, "cooktop_coil_setting_3[kW]",
            PADDR(cooktop_coil_power[2]),
            ...
            PT_double, "demand_cooktop[unit/day]", PADDR(enduse_demand_cooktop),
            PT_DESCRIPTION, "number of loads accumulating daily", PT_double,
            "demand_oven[unit/day]", PADDR(enduse_demand_oven), PT_DESCRIPTION,
            "number of loads accumulating daily",

            PT_double, "stall_voltage[V]", PADDR(stall_voltage), PT_double,
            "start_voltage[V]", PADDR(start_voltage), PT_complex,
            "stall_impedance[Ohm]", PADDR(stall_impedance), PT_double,
            "trip_delay[s]", PADDR(trip_delay), PT_double, "reset_delay[s]",
            PADDR(reset_delay),

            PT_double, "time_oven_operation[s]", PADDR(time_oven_operation),
            PT_double, "time_oven_setting[s]", PADDR(time_oven_setting),

            PT_enumeration, "state_cooktop", PADDR(state_cooktop), PT_KEYWORD,
            "CT_STOPPED", CT_STOPPED, PT_KEYWORD, "STAGE_6_ONLY",
            CT_STAGE_1_ONLY, PT_KEYWORD, "STAGE_7_ONLY", CT_STAGE_2_ONLY,
            PT_KEYWORD, "STAGE_8_ONLY", CT_STAGE_3_ONLY, PT_KEYWORD,
            "CT_STALLED", CT_STALLED, PT_KEYWORD, "CT_TRIPPED", CT_TRIPPED,

            PT_double, "cooktop_energy_baseline[kWh]",
            PADDR(cooktop_energy_baseline), PT_double, "cooktop_energy_used",
            PADDR(cooktop_energy_used), PT_double, "Toff", PADDR(Toff),
            PT_double, "Ton", PADDR(Ton),

            PT_double, "cooktop_interval_setting_1[s]",
            PADDR(cooktop_interval[0]), PT_double,
            "cooktop_interval_setting_2[s]", PADDR(cooktop_interval[1]),
            PT_double, "cooktop_interval_setting_3[s]",
            PADDR(cooktop_interval[2]), PT_double, "cooktop_energy_needed[kWh]",
            PADDR(cooktop_energy_needed),

            PT_bool, "heat_needed", PADDR(heat_needed), PT_bool, "oven_check",
            PADDR(oven_check), PT_bool, "remainon", PADDR(remainon), PT_bool,
            "cooktop_check", PADDR(cooktop_check),

            PT_double, "actual_load[kW]", PADDR(actual_load), PT_DESCRIPTION,
            "the actual load based on the current voltage across the coils",
            PT_double, "previous_load[kW]", PADDR(prev_load), PT_DESCRIPTION,
            "the actual load based on current voltage stored for use in "
            "controllers",
            PT_complex, "actual_power[kVA]", PADDR(range_actual_power),
            PT_DESCRIPTION,
            "the actual power based on the current voltage across the coils",
            PT_double, "is_range_on", PADDR(is_range_on), PT_DESCRIPTION,
...
int range::create() {
  ...
  oven_UA = clip(gl_random_normal(&hdr->rng_state, 2.0, 0.20), 0.1, 10) *
            oven_volume / 50;
  if (oven_UA <= 1.0)
    oven_UA = 2.0; // "R-13"
...
/** Initialize oven model properties - randomized defaults for all published
  * variables
 **/
int range::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("range::init(): deferring initialization on %s",
  ...
  if (pTair == 0) {
    pTair = &sTair;
    gl_warning(
        "range parent lacks \'air_temperature\' property, using default");
  }
  if (pTout == 0) {
    pTout = &sTout;
    gl_warning(
        "range parent lacks \'outside_temperature\' property, using default");
  }

  /* sanity checks */
  /* initialize oven volume */
  if (oven_volume <= 0.0) {
    //		oven_volume = 5*floor((1.0/5.0)*gl_random_uniform(0.90, 1.10)
    //* 50.0 * (pHouse->get_floor_area() /2000.0));  // [gal]
  ...
  if (oven_setpoint < 90 || oven_setpoint > 160)
    GL_THROW("This model is experimental and not validated: oven thermostat is "
             "set to %f and is outside the bounds of 90 to 160 degrees "
             "Fahrenheit (32.2 - 71.1 Celsius).",
             oven_setpoint);
  /*	TROUBLESHOOT
          TODO.
  */
  ...
  if (thermostat_deadband > 10 || thermostat_deadband < 0.0)
    GL_THROW("oven deadband of %f is outside accepted bounds of 0 to 10 "
             "degrees (5.6 degC).",
             thermostat_deadband);
  ...
  if (Tw < Tinlet) { // uninit'ed temperature
    Tw = gl_random_uniform(&hdr->rng_state, oven_setpoint - thermostat_deadband,
                           oven_setpoint + thermostat_deadband);
  }
  current_model = NONE;
  load_state = STABLE;

  // initial demand
  Tset_curtail = oven_setpoint - thermostat_deadband / 2 -
  ...
  case MT_UNKNOWN:
    ...
    gl_warning("This device, %s, is considered very experimental and has not "
               "been validated.",
               get_name());
    break;
  case MT_ANALOG:
  ...
  case MT_PULSED:
    ...
    if (shape.params.pulsed.pulsetype == MPT_TIME) {
      ...
      ; /* constant power pulse ~ oven demand X kW, limited by C + Q * h ~ Vdot
           proportional to power/time */
      oven_demand = gl_get_loadshape_value(&shape) / 2.4449;
    }
    break;
  case MT_MODULATED:
    if (shape.params.modulated.pulsetype == MPT_TIME) {
      GL_THROW("Amplitude modulated oven usage is nonsensical for residential "
               "ovens");
      /*	TROUBLESHOOT
...
int range::isa(char *classname) {
  return (strcmp(classname, "range") == 0 ||
...
void range::thermostat(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  case FULL:
    ...
    if (enduse_queue_oven > 1) // && dryer_on == true)
    {
      ...
      oven_run_prob =
          double(gl_random_uniform(&hdr->rng_state, queue_min, queue_max));

      if (Tw - TSTAT_PRECISION < Ton &&
          (oven_run_prob > enduse_queue_oven || oven_run_prob > 1) &&
          (time_oven_operation < time_oven_setting))
    ...
    if (Tw + TSTAT_PRECISION > Toff ||
        (time_oven_operation >= time_oven_setting)) {
      heat_needed = false;
      oven_check = false;
    }
    if (Tw - TSTAT_PRECISION < Ton &&
        (time_oven_operation < time_oven_setting) && remainon == true) {
      heat_needed = true;
      oven_check = true;
    }
  ...
  default:
    GL_THROW(
        "range thermostat() detected that the oven is in an unknown state");
  }
...
TIMESTAMP range::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (Tw > 212.0) {
    ...
    gl_warning("range:%i is using an experimental model and should not be "
               "considered reliable",
               my->id);
    /*	TROUBLESHOOT
            The range object has a number of VERY experimental features and
       development is incomplete. If you are receiving this error message,
       reccommend no longer using this particular feature without considerable
       overhaul.
    */
  }
...
TIMESTAMP range::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (heat_needed == true) {
    /* power_kw */ load.total =
        heating_element_capacity * (heat_mode == GASHEAT ? 0.01 : 1.0);
    is_range_on = 1;
  } else {
...
double range::update_state(double dt1, TIMESTAMP t1) {
  ...
  case CT_STOPPED:
    ...
    if (enduse_queue_cooktop > 1 && (cooktop_run_prob > enduse_queue_cooktop)) {
      ...
      cycle_duration_cooktop = 1000 *
                               (cooktop_energy_needed - cooktop_energy_used) /
                               cooktop_coil_power[0] * 60 * 60;
  ...
  case CT_STAGE_1_ONLY:
    ...
    if (cooktop_energy_used >= cooktop_energy_needed ||
        time_cooktop_operation >= time_cooktop_setting ||
        cycle_time_cooktop <= 0) {
      if (cooktop_energy_used >= cooktop_energy_needed ||
        ...
        double cycle_t = 1000 * (cooktop_energy_needed - cooktop_energy_used) /
                         cooktop_coil_power[2] * 60 * 60;
  ...
  case CT_STAGE_2_ONLY:
    ...
    if (cooktop_energy_used >= cooktop_energy_needed ||
      ...
      if (cooktop_energy_used >= cooktop_energy_needed ||
        ...
        cooktop_check = true;
        cycle_time_cooktop =
            time_cooktop_setting - cooktop_interval[0] - cooktop_interval[1];
      }
  ...
  case CT_STAGE_3_ONLY:
    ...
    if (cooktop_energy_used >= cooktop_energy_needed ||
        cycle_time_cooktop <= 0) { // The dishes are clean
      state_cooktop = CT_STOPPED;
      cycle_time_cooktop = 0;
      cooktop_energy_used = 0;
      cooktop_check = false;
  ...
  case CT_STOPPED:
    ...
    // nothing running
    load.constant_power = load.constant_current = load.constant_admittance =
        gld::complex(0, 0, J);

    // time to next expected state_cooktop change
    // dt = (enduse_demand_cooktop<=0) ? -1 : 	dt = 3600/enduse_demand_cooktop;
    dt1 = (enduse_queue_cooktop >= 1) ? 0
                                      : ((1 - enduse_queue_cooktop) * 3600) /
                                            (enduse_queue_cooktop * 24);
  ...
  case CT_STAGE_1_ONLY:
    ...
    load.constant_admittance =
        gld::complex((cooktop_coil_power[0]) / 1000, 0, J);
  ...
  case CT_STAGE_2_ONLY:
    ...
    load.constant_power = load.constant_current = gld::complex(0, 0, J);
    load.constant_admittance =
        gld::complex((cooktop_coil_power[1]) / 1000, 0, J);
  ...
  case CT_STAGE_3_ONLY:
    ...
    load.constant_power = load.constant_current = gld::complex(0, 0, J);
    load.constant_admittance =
        gld::complex((cooktop_coil_power[2]) / 1000, 0, J);
  ...
  default:
    ...
    /*	TROUBLESHOOT
            This is an error.  Please submit a bug report along with at the
       range object & class sections from the relevant GLM file, and from the
       dump file.
    */
    break;
  }
  ...
  load.total +=
      load.constant_power + load.constant_current + load.constant_admittance;
  total_power_cooktop = (load.constant_power.Re() +
                         (load.constant_current.Re() +
                          load.constant_admittance.Re() * load.voltage_factor) *
                             load.voltage_factor) *
                        1000;
...
TIMESTAMP range::postsync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
int range::commit() {
  Tw_old = Tw;
...
/** Set the oven model and its state based on the estimated
        temperature differential along the height of the food column when it is
 full, emplty or partial at the current height.
 **/
  ...
  */
  ...
  SingleZone:
    ...
    if (h < ROUNDOFF) {
      ...
      double energy_over = vol_over * food_density * specificheat_food *
                           (/*Tupper*/ Tw - Tlower);
...
/* the key to picking the equations apart is that the goal is to calculate the
 temperature differences relative to the *	temperature of the lower node
 (or inlet temp, if 1node).
...
double range::dhdt(double h) {
  ...
  const double c1 = food_density * specificheat_food * area *
                    (/*Tupper*/ Tw - Tlower); // Btu/ft...
  ...
  const double cA =
      -mdot / (food_density * area) +
      (actual_kW() * BTUPHPKW + oven_UA * (get_Tambient(location) - Tlower)) /
          c1;
...
double range::actual_kW(void) {
  ...
  if (heat_needed && re_override != OV_OFF) {
    ...
    if (actual_voltage > 2.0 * default_line_voltage) {
      if (trip_counter++ > 10)
        GL_THROW("oven line voltage for range:%d is too high, exceeds twice "
                 "nominal voltage.",
                 obj->id);
      /*	TROUBLESHOOT
              The range is receiving twice the nominal voltage consistantly, or
         about 480V on what should be a 240V circuit.  Please sanity check your
         powerflow model as it feeds to the meter and to the house.
      */
      else
        return 0.0; // @TODO:  This condition should trip the breaker with a
                    // counter
    }
    double test = heating_element_capacity * (actual_voltage * actual_voltage) /
...
inline double range::new_time_1node(double T0, double T1) {
  const double mdot_Cp =
      specificheat_food * oven_demand * 60 * food_density / GALPCF;

  if (Cw <= ROUNDOFF)
    return -1.0;

  const double c1 =
      ((actual_kW() * BTUPHPKW + oven_UA * get_Tambient(location)) +
       mdot_Cp * Tinlet) /
      Cw;
...
inline double range::new_temp_1node(double T0, double delta_t) {
  ...
  const double mdot_Cp =
      specificheat_food * oven_demand_old * 60 * food_density / GALPCF;
  ...
  const double c2 = (actual_kW() * BTUPHPKW + mdot_Cp * Tinlet +
                     oven_UA * get_Tambient(location)) /
                    (oven_UA + mdot_Cp);
...
//	OBJECT *obj = object_header(this);
//	gl_warning("%s (range:%d): %s", obj->name?obj->name:"(anonymous
//object)", obj->id, errtxt[msg]); 	throw msg; // this must be caught by the
//range code, not by the core
// }
...
static TIMESTAMP sync_range_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  try {
    ...
    gl_error("%s (range:%d) model zone exception (code %d) not caught",
             obj->name ? obj->name : "(anonymous range)", obj->id, m);
  } catch (char *msg) {
    gl_error("%s (range:%d) %s", obj->name ? obj->name : "(anonymous range)",
             obj->id, msg);
  }
  return TS_INVALID;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_range(OBJECT *obj, TIMESTAMP t0,
                                           PASSCONFIG pass) {
...
EXPORT TIMESTAMP plc_range(OBJECT *obj, TIMESTAMP t0) {
  ...
  /// but not later than oven time to transition (ticket #147)
  return TS_NEVER;
}
...
/** $Id: range.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The residential electric range uses a hybrid thermal model that is
 capable of tracking a single-mass of food.
