...
int waterheater::init(OBJECT *parent)
{
    ...
    */
    ...
    if (current_model == MULTILAYER)
    {
        ...
        if (Vdot_circ < 0.0)
        { // if no user input, set a default value
            Vdot_circ = 2.0;
        }
...
TIMESTAMP waterheater::sync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    {
        if (t1 == fwh_sim_time)
        { // update power, cop, water temperature, and heat gain from the fortran water heater model and tell the model to simulate again.
