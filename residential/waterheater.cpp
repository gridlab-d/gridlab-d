/** $Id: waterheater.cpp,v 1.51 2008/02/15 00:24:14 d3j168 Exp $
        ...
        The residential electric waterheater uses a hybrid thermal model that is
 capable of tracking either a single-mass of water, or a dual-mass with a
 varying thermocline.

        The driving dynamic parameters of the waterheater are
        - <b>demand</b>: the current consumption of water in gallons/minute; the
 higher the demand, the more quickly the thermocline drops.
        - <b>voltage</b>: the line voltage for the coil; the lower the voltage,
 the more slowly the thermocline rises.
        - <b>inlet water temperature</b>: the inlet water temperature; the lower
 the inlet water temperature, the more heat is needed to raise it to the set
 point
        - <b>indoor air temperature</b>: the higher the indoor temperature, the
 less heat loss through the jacket.
 @{
 **/
...
waterheater::waterheater(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "waterheater", sizeof(waterheater),
    ...
    if (gl_publish_variable(
            ...
            "tank_volume[gal]", PADDR(tank_volume), PT_DESCRIPTION,
            "the volume of water in the tank when it is full", PT_double,
            "tank_UA[Btu*h/degF]", PADDR(tank_UA), PT_DESCRIPTION,
            "the UA of the tank (surface area divided by R-value)", PT_double,
            "tank_diameter[ft]", PADDR(tank_diameter), PT_DESCRIPTION,
            "the diameter of the water heater tank", PT_double,
            "tank_height[ft]", PADDR(tank_height), PT_DESCRIPTION,
            "the height of the water heater tank", PT_double,
            "water_demand[gpm]", PADDR(water_demand), PT_DESCRIPTION,
            "the hot water draw from the water heater", PT_double,
            "heating_element_capacity[kW]", PADDR(heating_element_capacity),
            PT_DESCRIPTION, "the power of the heating element", PT_double,
            "inlet_water_temperature[degF]", PADDR(Tinlet), PT_DESCRIPTION,
            "the inlet temperature of the water tank", PT_enumeration,
            "waterheater_model", PADDR(current_model), PT_DESCRIPTION,
            "the water heater model to use", PT_KEYWORD, "ONEZNODE",
            (enumeration)ONENODE, PT_KEYWORD, "TWONODE", (enumeration)TWONODE,
            PT_KEYWORD, "FORTRAN", (enumeration)FORTRAN, PT_KEYWORD,
            "MULTILAYER", (enumeration)MULTILAYER, PT_KEYWORD, "NONE",
            (enumeration)NONE, PT_enumeration, "heat_mode", PADDR(heat_mode),
            PT_DESCRIPTION, "the energy source for heating the water heater",
            PT_KEYWORD, "ELECTRIC", (enumeration)ELECTRIC, PT_KEYWORD,
            "GASHEAT", (enumeration)GASHEAT, PT_KEYWORD, "HEAT_PUMP",
            (enumeration)HEAT_PUMP, PT_enumeration, "location", PADDR(location),
            PT_DESCRIPTION, "whether the water heater is inside or outside",
            PT_KEYWORD, "INSIDE", (enumeration)INSIDE, PT_KEYWORD, "GARAGE",
            (enumeration)GARAGE, PT_double, "heatpump_predicted_energy[Wh]",
            PADDR(energytake), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "Amount of energy heat pump predicts to need to reach setpoint - "
            "energy increases as water temperature decreases",
            PT_double, "tank_setpoint[degF]", PADDR(tank_setpoint),
            PT_DESCRIPTION,
            "the temperature around which the water heater will heat its "
            "contents",
            PT_double, "thermostat_deadband[degF]", PADDR(thermostat_deadband),
            PT_DESCRIPTION, "the degree to heat the water tank, when needed",
            PT_double, "temperature[degF]", PADDR(Tw), PT_DESCRIPTION,
            "the outlet temperature of the water tank", PT_double, "height[ft]",
            PADDR(h), PT_DESCRIPTION,
            "the height of the hot water column within the water tank",
            PT_complex, "demand[kVA]", PADDR(load.total), PT_DESCRIPTION,
            "the water heater power consumption", PT_double, "actual_load[kW]",
            PADDR(actual_load), PT_DESCRIPTION,
            "the actual load based on the current voltage across the coils",
            PT_double, "previous_load[kW]", PADDR(prev_load), PT_DESCRIPTION,
            "the previous load based on voltage across the coils at the last "
            "sync operation",
            PT_complex, "actual_power[kVA]", PADDR(waterheater_actual_power),
            PT_DESCRIPTION,
            "the actual power based on the current voltage across the coils",
            PT_double, "is_waterheater_on", PADDR(is_waterheater_on),
            PT_DESCRIPTION,
            "simple logic output to determine state of waterheater (1-on, "
            "0-off)",
            PT_double, "gas_fan_power[kW]", PADDR(gas_fan_power),
            PT_DESCRIPTION, "load of a running gas waterheater", PT_double,
            "gas_standby_power[kW]", PADDR(gas_standby_power), PT_DESCRIPTION,
            "load of a gas waterheater in standby", PT_double,
            "heat_pump_coefficient_of_performance[Btu/kWh]", PADDR(HP_COP),
            PT_DESCRIPTION,
            "current COP of the water heater pump - currently calculated "
            "internally and not an input",
            PT_double, "Tcontrol[degF]", PADDR(Tcontrol), PT_DESCRIPTION,
            "in heat pump operation, defines the blended temperature used for "
            "turning on and off HP - currently calculated internally and not "
            "an input",
            PT_enumeration, "current_tank_status", PADDR(current_tank_state),
            PT_KEYWORD, "FULL", (enumeration)FULL, PT_KEYWORD, "PARTIAL",
            (enumeration)PARTIAL, PT_KEYWORD, "EMPTY", (enumeration)EMPTY,
            ...
            PT_double, "COP", PADDR(fwh_cop_current), PT_DESCRIPTION,
            "the cop of the fortran heat pump water heater model.", PT_double,
            "operating_mode", PADDR(operating_mode), PT_DESCRIPTION,
            "the operating mode the fortran water heater should be using.",
            PT_double, "fortran_sim_time[s]", PADDR(simulation_time),
            PT_DESCRIPTION,
            "the amount of time the fortran model should simulate.", PT_double,
            "waterheater_power[kW]", PADDR(fwh_power_now), PT_DESCRIPTION,
            "the current power draw from the fortran water heater.",
            PT_enumeration, "load_state", PADDR(load_state), PT_KEYWORD,
            "DEPLETING", (enumeration)DEPLETING, PT_KEYWORD, "RECOVERING",
            (enumeration)RECOVERING, PT_KEYWORD, "STABLE", (enumeration)STABLE,
            PT_double, "actual_voltage", PADDR(actual_voltage), PT_ACCESS,
            PA_HIDDEN, PT_double, "nominal_voltage", PADDR(nominal_voltage),
            PT_ACCESS, PA_HIDDEN, PT_enumeration, "re_override",
            PADDR(re_override), PT_DESCRIPTION,
            "the override setting for the water heater", PT_KEYWORD, "OV_ON",
            (enumeration)OV_ON, PT_KEYWORD, "OV_NORMAL", (enumeration)OV_NORMAL,
            PT_KEYWORD, "OV_OFF", (enumeration)OV_OFF,
            // published variables for the multi layer waterheater model
            PT_double, "lower_tank_setpoint[degF]", PADDR(tank_setpoint_1),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The setpoint for the lower heating element "
            "thermostat in the tank",
            PT_double, "upper_tank_setpoint[degF]", PADDR(tank_setpoint_2),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The setpoint for the upper heating element "
            "thermostat in the tank",
            PT_double, "lower_tank_deadband[degF]", PADDR(deadband_1),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The deadband for the lower heating element "
            "thermostat in the tank",
            PT_double, "upper_tank_deadband[degF]", PADDR(deadband_2),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The deadband for the upper heating element "
            "thermostat in the tank",
            PT_double, "lower_tank_temperature[degF]", PADDR(Tw_1),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The water temperature at the lower heating "
            "element thermostat in the tank",
            PT_double, "upper_tank_temperature[degF]", PADDR(Tw_2),
            PT_DESCRIPTION,
            "MULTILAYER_MODEL: The water temperature for the upper heating "
            "element thermostat in the tank",
            PT_double, "discrete_step_size[s]", PADDR(discrete_step_size),
            PT_DESCRIPTION,
            "MULTILAYER MODEL: The step size in seconds to use in the discrete "
            "tank temperature dynamics loop.",
            PT_double, "circular_flow_rate[gpm]", PADDR(Vdot_circ),
            PT_DESCRIPTION,
            "MULTILAYER MODEL: Heuristic flow activated once the heating "
            "elements turn on.",
            PT_double, "T_mixing_valve[degF]", PADDR(T_mixing_valve),
            PT_DESCRIPTION,
            "MULTILAYER MODEL: Reference temperature to which mixing valve "
            "operate.",
            PT_enumeration, "lower_heating_element_state",
            PADDR(control_switch_1), PT_DESCRIPTION,
            "MULTILAYER MODEL: The state of the lower heating element in the "
            "tank.",
            PT_KEYWORD, "OFF", (enumeration)OFF, PT_KEYWORD, "ON",
            (enumeration)ON, PT_enumeration, "upper_heating_element_state",
            PADDR(control_switch_2), PT_DESCRIPTION,
            "MULTILAYER MODEL: The state of the upper heating element in the "
            "tank.",
            PT_KEYWORD, "OFF", (enumeration)OFF, PT_KEYWORD, "ON",
            (enumeration)ON, nullptr) < 1)
...
int waterheater::create() {
  ...
  tank_diameter = 0;
  tank_height = 0;
  Tinlet = 60.0; // default set here, but published by the model for users to
                 // set this value
  ...
  tank_UA =
      clip(gl_random_normal(RNGSTATE, 2.0, 0.20), 0.1, 10) * tank_volume / 50;
  if (tank_UA <= 1.0)
    tank_UA = 2.0; // "R-13"
  ...
  energytake = 0.0;
  compressor_max_threshold = 2000; // Heating element replaces compressor when
                                   // EnergyTake hits this threshold
  compressor_min_threshold =
      ((thermostat_deadband / 2) * 2.44 *
       tank_volume); // Compressor turns on when it hits this threshold
  heating_element_min_threshold =
      1100; // compressor replaces heating element when it hits this threshold.
  interval = 0;
  // =================================
...
/** Initialize water heater model properties - randomized defaults for all
  * published variables
 **/
int waterheater::init(OBJECT *parent) {
  ...
  nominal_voltage =
      (2.0 * default_line_voltage); //@TODO:  Determine if this should be
                                    //published or how we want to obtain this
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("waterheater::init(): deferring initialization on %s",
  ...
  if (pTair == 0) {
    pTair = &sTair;
    gl_warning(
        "waterheater parent lacks \'air_temperature\' property, using default");
  }
  if (pTout == 0) {
    pTout = &sTout;
    gl_warning("waterheater parent lacks \'outside_temperature\' property, "
               "using default");
  }
  if (pRH == 0) {
    pRH = &sTout;
  ...
  if (tank_volume <= 0.0) {
    if (tank_diameter <= 0) {
      if (tank_height <= 0) {
        ...
        gl_warning("waterheater::init() : tank volume, diameter, and height "
                   "were not specified, defaulting to 50 gallons and 3.78 ft");

        tank_volume = 50;
        tank_height = 3.782; // was the old default for a 1.5 ft diameter
        tank_diameter =
            2 * sqrt(tank_volume * (1 / GALPCF) / (pi * tank_height));
        area = (pi * pow(tank_diameter, 2)) / 4;
      } else {
        // Only height was set, so defaulting to a standard gallon size
        gl_warning("waterheater::init() : tank volume and diameter were not "
                   "specified, defaulting to 50 gallons");
      ...
      if (tank_height <= 0) {
        ...
        gl_warning("waterheater::init() : tank volume and height were not "
                   "specified, defaulting to 50 gallons");
        ...
        gl_verbose("waterheater::init() : tank volume was not specified, "
                   "calculating from height and diameter");
    ...
    if (tank_volume > 100.0 || tank_volume < 20.0) {
      gl_error("watertank volume of %f outside the volume bounds of 20 to 100 "
               "gallons.",
               tank_volume);
      /*	TROUBLESHOOT
              All waterheaters must be set between 20 and 100 gallons.  Most
         waterheaters are assumed to be 50 gallon tanks.
      */
    }
    ...
    if (tank_height <= 0) {
      if (tank_diameter <= 0) {
        ...
        gl_warning("waterheater::init() : height and diameter were not "
                   "specified, defaulting to 3.78 ft");
        ...
        gl_verbose("waterheater::init() : tank height was not specified, "
                   "calculating from volume and diameter");
      ...
      if (tank_diameter <= 0) {
        ...
        gl_verbose("waterheater::init() : diameter was not specified, "
                   "calculating from volume and height");
        ...
        if (abs(temp_tank_diameter - tank_diameter) > 0.05) {
          gl_error("waterheater::init() : tank volume, diameter, and height "
                   "were all set, but did not agree with each other.  Please "
                   "unspecify one variable to be calculated.");
          /*  TROUBLESHOOT
                  The three variables, tank_volume, tank_diameter, and height
             are geometrically dependent.  The values given are not properly
             dependent ( volume = pi * (diameter/2)^2 * height ) to a relative
             precision of 0.05 feet in the calculation of the tank_diameter.
             Please check calculations or un-specify one of the variables.
          */
        } else {
          area = (pi * pow(tank_diameter, 2)) / 4;
        }
  ...
  if (height <= 0) {
    ...
    gl_warning(
        "waterheater::init() : height of water column was set to value greater "
        "than tank height. setting initial height to tank height.");
    height = tank_height;
  }
  if (tank_setpoint < 90 || tank_setpoint > 160)
    gl_error("watertank thermostat is set to %f and is outside the bounds of "
             "90 to 160 degrees Fahrenheit (32.2 - 71.1 Celsius).",
             tank_setpoint);
  /*	TROUBLESHOOT
  ...
  */
  ...
  if (thermostat_deadband > 10 || thermostat_deadband < 0.0)
    GL_THROW("watertank deadband of %f is outside accepted bounds of 0 to 10 "
             "degrees (5.6 degC).",
             thermostat_deadband);
  if (current_model != FORTRAN) {
    // initial tank UA
  ...
  if (Tw < Tinlet) { // uninit'ed temperature
    Tw = gl_random_uniform(RNGSTATE, tank_setpoint - thermostat_deadband,
                           tank_setpoint + thermostat_deadband);
  }
  ...
  if (current_model != FORTRAN && current_model != MULTILAYER) {
    ...
    // initial demand
    Tset_curtail =
        tank_setpoint - thermostat_deadband / 2 -
        10; // Allow T to drop only 10 degrees below lower cut-in T...
  ...
  case MT_ANALOG:
    if (shape.params.analog.energy == 0.0) {
      ...
      */
      ...
      //				double heat_per_gallon = RHOWATER * //
      //lb/cf 										 CFPGAL *	// lb/gal 										 CWATER *	// BTU/degF / gal 										 KWBTUPH /
      //// kW/gal 										 1000.0;	// W/gal
    ...
    break;
  case MT_PULSED:
    /* pulsed loadshapes "emit one or more pulses at random times s. t. the
     * total energy is accumulated over the period of the loadshape". pulsed
     * loadshapes can either user time or kW values per pulse. */
    if (shape.params.pulsed.pulsetype == MPT_TIME) {
      ; /* constant time pulse ~ consumes X gallons to drive heater for Y hours
           ~ but what's Vdot, what's t? */
    } else if (shape.params.pulsed.pulsetype == MPT_POWER) {
      ; /* constant power pulse ~ draws water to consume X kW, limited by C + Q
           * h ~ Vdot proportional to power/time */
      water_demand = gl_get_loadshape_value(&shape) / 2.4449;
    }
    break;
  case MT_MODULATED:
    if (shape.params.modulated.pulsetype == MPT_TIME) {
      GL_THROW("Amplitude modulated water usage is nonsensical for residential "
               "water heaters");
      /*	TROUBLESHOOT
              Though it is possible to put a constant, low-level water draw on a
         water heater, it is thoroughly counterintuitive to the normal usage of
         the waterheater.
       */
  ...
  case MT_QUEUED:
    if (shape.params.queued.pulsetype == MPT_TIME) {
      ; /* constant time pulse ~ consumes X gallons/minute to consume Y thermal
           energy */
  ...
  if (current_model == FORTRAN) {
    if (simulation_time <= 0) {
      GL_THROW("The simulation time for the fortran water heater model must be "
               "greater than 0.");
    }
    ...
    if (tank_volume == 40) {
      ...
      GL_THROW("Invalid tank volume for the fortran water heater_model. Valid "
               "volumes are 40 or 80 gallons.");
    }
  }
  if (current_model == MULTILAYER) {
    ...
    if (discrete_step_size < 1) {
      ...
      gl_warning("waterheater::init() : The discretization step size is chosen "
                 "too large. To reduce error setting it to 60 seconds.");
      discrete_step_size = 60;
    }
    if (Vdot_circ < 0.0) { // if no user input, set a default value
      Vdot_circ = 2.0;
    } else if (Vdot_circ < 1.0) {
      gl_warning("waterheater::init() : The Heuristic flow rate is too small "
                 "for 10 layers. Setting it to minimum 1 gpm.");
      Vdot_circ = 1.0;
    } else if (Vdot_circ > 3.0) {
      gl_warning("waterheater::init() : The Heuristic flow rate is too large "
                 "for 10 layers. Setting it to maximum 3 gpm.");
      Vdot_circ = 3.0;
    }
    ...
    if (T_mixing_valve < 1.0) {
      gl_warning("waterheater::init() : Mixing valve temperature was not set, "
                 "setting it to minimum upper element setpoint.");
      T_mixing_valve =
          Tmin_upper; // if mixing valve temperature is not set by the user then
                      // just try to provide small mixing so consumer get a bit
                      // less temperature in their home than the upper heating
                      // layer
    } else if (T_mixing_valve > Tmax_upper) {
      gl_warning("waterheater::init() : Mixing valve temperature is set very "
                 "high, it will not be very efficient. Try choosing a value "
                 "less than upper element setpoint.");
    }
    ...
    bottom_layer_disk = 1;
    top_layer_disk = 1;
    number_of_layers = (number_of_mixing_zone_disks * total_mixing_zones) +
                       (number_of_regular_disks * total_regular_zones) +
                       bottom_layer_disk + top_layer_disk;
    ...
    a_diffusion_coefficient =
        thermal_conductivity / (RHOWATER * Cp * H_layer * H_layer);
    a_loss_layer_coefficient = A_layer * U_val / (RHOWATER * Cp * V_layer);
    a_loss_bottom_coefficient =
        ((A_layer + A_bottom) * U_val / (RHOWATER * Cp * V_layer));
    a_loss_top_coefficient =
        ((A_bottom + A_top) * U_val / (RHOWATER * Cp * V_layer));
    a_circular_const = (Vdot_circ * 60.0) / (GALPCF * V_layer);
    b_matrix_coefficient =
        heating_element_capacity * BTUPHPKW /
        (RHOWATER * Cp * V_layer * number_of_mixing_zone_disks);
    ...
    int start_index_1 = 1;
    int end_index_1 =
        start_index_1 + number_of_mixing_zone_disks + number_of_regular_disks;
    int start_index_2 = end_index_1 + 1;
    int end_index_2 =
        start_index_2 + number_of_mixing_zone_disks + number_of_regular_disks;
    for (int i = start_index_1 + 1;
         i <= start_index_1 + number_of_mixing_zone_disks; i++) {
      B_control[i][0] = b_matrix_coefficient;
    }
    for (int i = start_index_2;
         i <= start_index_2 + number_of_mixing_zone_disks - 1; i++) {
    ...
    } else if (Tw >= Tmin_lower && Tw <= Tmax_lower &&
               control_upper[0] == 0.0) {
...
int waterheater::isa(char *classname) {
  return (strcmp(classname, "waterheater") == 0 ||
...
void waterheater::thermostat(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  case EMPTY:
    ...
    break;
  default:
    GL_THROW("waterheater thermostat() detected that the water heater tank is "
             "in an unknown state");
  }
  // return TS_NEVER; // this thermostat is purely reactive and will never drive
  // the system
}

/** Water heater plc control code to set the water heater 'heat_needed' state
        The thermostat set point, deadband, tank state(height of hot water
 column) and current water temperature are used to determine 'heat_needed'
 state.
 **/
TIMESTAMP waterheater::presync(TIMESTAMP t0, TIMESTAMP t1) {
  /* time has passed ~ calculate internal gains, height change, temperature
   * change */
  ...
  if (current_model == MULTILAYER) {
    ...
    if (water_demand != last_water_demand || Tinlet != last_inlet_temperature ||
        tank_setpoint_1 != last_lower_thermostat_setpoint ||
        tank_setpoint_2 != last_upper_thermostat_setpoint ||
        re_override != last_override_value ||
        control_upper[idx] != control_upper[idx2] ||
        control_lower[idx] != control_lower[idx2]) {
  ...
  if (Tw > 212.0 || Tw_1 > 212.0 || Tw_2 > 212.0) {
    ...
    /*	TROUBLESHOOT
            The temperature model for the waterheater has broken, or the
       environment around the waterheater has burst into flames.  Please post
       this with your model and dump files attached to the bug report.
     */
  }
...
void waterheater::sync_energytake() {
  ...
  double Tw_temp;
  /*
  Some information about Energytake that might help whoever tries to update or
  work in the code in the future.

  EnergyTake is the amount of energy that the HPWH would need to consume to heat
  the water in the tank to the temperature set-point. When HPWH is in idle mode
  (losing heat over time), energytake increases. Further, energytake drops
  significantly if a water draw occurs.
  */
  ...
  */
  ...
  if (interval == 60 && (water_demand > 0) && turn_fan_on)
    heating_element_capacity =
        nominal_voltage * 0.000171; // rated fan current is 0.000171.

  if ((Toff >= Tw) && (Tw >= Ton) &&
      (heating_element_capacity ==
       0)) // when energy take is between setpoints, HPWH is in idle mode.
  {

    energytake =
        log(70 / (get_Tambient(location) * 0.39)) *
        time_step; // Constants are based on trial and error to get the most
                   // appropraite behavior similar to the physical unit.
  ...
  } else // when energy take is not between setpoints (i.e. waterheater is on)
  {
    ...
    if (Tw <= Ton || heating_element_capacity != 0) {
      ...
      if (water_demand > 0) {
        ...
        // If water demand is less than 10, the +2 factor will get the model to
        // behave as the physical unit. Otherwise, a correction factor (0.65 *
        // water demand) is needed to get the model to behave nearly the same as
        // the physical unit
        double tank_volume_post_draw = tank_volume - water_demand;
        if (water_demand < 10)
          Tw_temp =
              ((((tank_volume_post_draw)*tank_setpoint) + (water_demand * 60)) /
               (tank_volume)) +
              2;
        else
          Tw_temp =
        ...
        energytake = ((tank_setpoint - Tw_temp) * tank_volume * 2.44) +
                     energy_increment_value;
      }

      if (heating_element_capacity <=
          0.42) // compressor operation. The resistive heating element should
                // not trigger when the compressor is ON and Energytake within
                // compressor's limits.
      {
        energytake -=
            (heating_element_capacity - 0.447) /
            (-(get_Tambient(location) / pow(get_Tambient(location), 2.4)));
      }
      if (heating_element_capacity > 0.42) {
        energytake -=
            (heating_element_capacity - 4.872) /
            (-0.0014); // The 4.87 is the operation of the heating element. The
                       // -0.0014 is the result of the line fit equation.
      }
    }
...
TIMESTAMP waterheater::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (location == INSIDE) {
    ...
    if (this->current_model == ONENODE) {
      ...
      internal_gain = A_bottom * U_val * (T_layers[1][0] - Tamb) +
                      A_top * U_val * (T_layers[10][0] - Tamb);
      for (int i = 2; i <= 9; i++) {
  ...
  if (current_model == FORTRAN) {
    if (t1 == fwh_sim_time) { // update power, cop, water temperature, and heat
                              // gain from the fortran water heater model and
                              // tell the model to simulate again.
      ...
      if (location == INSIDE) {
        for (i = 0; i < coarse_tank_grid * fine_tank_grid; i++) {
          internal_gain +=
              (tank_heat_loss_rate *
               (((init_tank_temp[i] * (9.0 / 5.0)) + 32.0) - Tamb)) /
              (coarse_tank_grid * fine_tank_grid);
        }
  ...
  if (current_model != FORTRAN) {
    ...
    if (heat_needed == true) {
      /* power_kw */ load.total =
          (heat_mode == GASHEAT ? gas_fan_power : heating_element_capacity);
      is_waterheater_on = 1;
    } else {
      /* power_kw */ load.total =
          (heat_mode == GASHEAT ? gas_standby_power : 0.0);
      is_waterheater_on = 0;
    }
  }
  ...
  waterheater_actual_power =
      load.constant_power +
      (load.constant_current + load.constant_admittance * load.voltage_factor) *
          load.voltage_factor;
    ...
    if (current_model != MULTILAYER) {
      if (re_override == OV_NORMAL) {
        if (time_to_transition >=
            (1.0 / 3600.0)) // 0.0167 represents one second
        {
          TIMESTAMP t_to_trans =
              (TIMESTAMP)(t1 + time_to_transition * 3600.0 / TS_SECOND);
...
/** Set the water heater model and tank state based on the estimated
        temperature differential along the height of the water column when it is
 full, emplty or partial at the current height, given the current water draw.
 **/
enumeration waterheater::set_current_model_and_load_state(void) {
  ...
  case EMPTY:
    if (dhdt_empty <= 0.0) {
      ...
      /* it is reconized that this causes a discontinuous jump in the water
       * temperature. despite that, energy is mostly conserved, since Q => dh
       * until h = 0 (thus no heat in the water). the +0.01 degF fudge factor
       * for the dhdt() T_diff=0 catch adds about 0.05% of a tank of heat, less
       * than expected errors from other sources. */
    } else if (dhdt_full > 0) {
    ...
    case RECOVERING:
      try {
        ...
        if (m == MODEL_NOT_2ZONE) {
          ...
          GL_THROW("unexpected exception in update_T_and_or_h(%+.1f hrs)",
                   nHours);
      }
      break;
    }
...
/* the key to picking the equations apart is that the goal is to calculate the
 * temperature differences relative to the temperature of the lower node (or
 * inlet temp, if 1node). cA is the volume change from water draw, heating
 * element, and thermal jacket given a uniformly cold tank cB is the volume
 * change from the extra energy within the hot water node
 */
double waterheater::dhdt(double h) {
  ...
  const double c1 =
      RHOWATER * Cp * area * (/*Tupper*/ Tw - Tlower); // Btu/ft...

  // check c1 before dividing by it
  if (c1 <= ROUNDOFF)
    return 0.0; // Possible only when /*Tupper*/ Tw and Tlower are very close,
                // and the difference is negligible

  double cA;
  if (heat_mode == HEAT_PUMP) {
    // @TODO: These values were created from HPWH project; need to make them
    // more accessible
    HP_COP =
        (1.04 + (1.21 - 1.04) * (get_Tambient(location) - 50) / (70 - 50)) *
        (5.259 - 0.0255 * Tw); // Added by GLD folks

    cA = -mdot / (RHOWATER * area) +
         (actual_kW() * BTUPHPKW * HP_COP +
          tank_UA * (get_Tambient(location) - Tlower)) /
             c1;
  } else {
    cA =
        -mdot / (RHOWATER * area) +
        (actual_kW() * BTUPHPKW + tank_UA * (get_Tambient(location) - Tlower)) /
            c1;
  }
...
double waterheater::actual_kW(void) {
  ...
  if (heat_needed && re_override != OV_OFF) {
    ...
    if (actual_voltage > 2.0 * nominal_voltage) {
      if (trip_counter++ > 10)
        gl_error("Water heater line voltage for waterheater:%d is too high, "
                 "exceeds twice nominal voltage.",
                 obj->id);
      /*	TROUBLESHOOT
              The waterheater is receiving twice the nominal voltage
         consistantly, or about 480V on what should be a 240V circuit.  Please
         sanity check your powerflow model as it feeds to the meter and to the
         house.
      */
      else
        return 0.0; // @TODO:  This condition should trip the breaker with a
                    // counter
    }
    double test;

    if (heat_mode == ELECTRIC) {
      test = heating_element_capacity * (actual_voltage * actual_voltage) /
             (nominal_voltage * nominal_voltage);
    } else {
      if (heat_mode == HEAT_PUMP) {
        // compressor should turn on?
        if ((energytake < compressor_max_threshold) &&
            (energytake > compressor_min_threshold) && !heating_element_on &&
            !turn_fan_on) {
          // 0.42
          heating_element_capacity = nominal_voltage * 0.00175;
        } else if ((energytake >= heating_element_min_threshold) &&
                   !turn_fan_on) {
          // 4.8
          heating_element_capacity =
              (nominal_voltage * 0.020); // Threshold for the compressor
    ...
    // @TODO: We don't have a voltage dependence for the heat pump yet...but we
    // should
...
inline double waterheater::new_time_1node(double T0, double T1) {
  ...
  if (heat_mode == HEAT_PUMP) {
    ...
    HP_COP =
        (1.04 + (1.21 - 1.04) * (get_Tambient(location) - 50) / (70 - 50)) *
        (5.259 - 0.0255 * Tw);
    c1 = ((actual_kW() * BTUPHPKW * HP_COP + tank_UA * get_Tambient(location)) +
          mdot_Cp * Tinlet) /
         Cw;
  } else {
    c1 = ((actual_kW() * BTUPHPKW + tank_UA * get_Tambient(location)) +
          mdot_Cp * Tinlet) /
         Cw;
  }
  const double c2 = -(tank_UA + mdot_Cp) / Cw;

  if (fabs(c1 + c2 * T1) <= ROUNDOFF || fabs(c1 + c2 * T0) <= ROUNDOFF ||
      fabs(c2) <= ROUNDOFF)
    return -1.0;

  const double new_time =
      (log(fabs(c1 + c2 * T1)) - log(fabs(c1 + c2 * T0))) / c2; // [hr]
  return new_time;
}
...
inline double waterheater::new_temp_1node(double T0, double delta_t) {
  ...
  if (heat_mode == HEAT_PUMP) {
    ...
    c2 = (actual_kW() * BTUPHPKW * HP_COP + mdot_Cp * Tinlet +
          tank_UA * get_Tambient(location)) /
         (tank_UA + mdot_Cp);
  } else {
    c2 = (actual_kW() * BTUPHPKW + mdot_Cp * Tinlet +
          tank_UA * get_Tambient(location)) /
         (tank_UA + mdot_Cp);
  }
...
inline double waterheater::new_time_2zone(double h0, double h1) {
  ...
  if (fabs(c0) <= ROUNDOFF || height <= ROUNDOFF)
    return -1.0; // c0 or height should never be zero.  if one of these is zero,
                 // there is no definite time to transition
...
inline double waterheater::new_h_2zone(double h0, double delta_t) {
  ...
  const double c1 =
      RHOWATER * Cp * area *
      (/*Tupper*/ Tw - Tlower); // lb/ft^3 * ft^2 * degF * Btu/lb.degF = lb/lb *
                                // ft^2/ft^3 * degF/degF * Btu = Btu/ft

  // check c1 before division
  if (fabs(c1) <= ROUNDOFF)
    return height; // if /*Tupper*/ Tw and Tlower are real close, then the new
                   // height is the same as tank height
...
void waterheater::wrong_model(WRONGMODEL msg) {
  ...
  OBJECT *obj = object_header(this);
  gl_warning("%s (waterheater:%d): %s",
             obj->name ? obj->name : "(anonymous object)", obj->id,
             errtxt[msg]);
...
int waterheater::multilayer_time_to_transition() {
  ...
  while (!state_changed) {
    ...
    if (T_layers[1][time_new] >= Tmax_lower || control_upper[time_new] == 1.0) {
      control_lower.push_back(0.0);
    } else if (T_layers[1][time_new] <= Tmin_lower &&
    ...
    if (control_upper[time_now] != control_upper[time_new] ||
        control_lower[time_now] != control_lower[time_new]) {
      state_changed = true;
    }
    time_now += 1;
  }
...
void waterheater::calculate_waterheater_matrices(int time_now) {
  ...
  if (T_mixing_valve > T_layers[10][time_now]) {
    ...
    mixing_fraction =
        (T_mixing_valve - Tinlet) / (T_layers[10][time_now] - Tinlet);
  }
  ...
  A_circular_flow[start_index_1][start_index_1] =
      -1.0 * a_circular_const * control_lower[time_now];
  A_circular_flow[end_index_1][end_index_1] =
      -1.0 * a_circular_const * control_lower[time_now];
  ...
  A_circular_flow[start_index_2][start_index_2] =
      -1.0 * a_circular_const * control_upper[time_now];
  A_circular_flow[end_index_2][end_index_2] =
      -1.0 * a_circular_const * control_upper[time_now];
  A_circular_flow[end_index_2][end_index_2 + 1] = 0.0;
  for (i = 0; i < number_of_states; i++) {
    for (int j = 0; j < number_of_states; j++) {
      A_matrix[i][j] = A_diffusion[i][j] + A_loss[i][j] + A_plug[i][j] +
                       A_circular_flow[i][j];
    }
  }
}
...
std::vector<double>
waterheater::multiply_waterheater_matrices(vector<vector<double>> &a,
                                           vector<double> &b) {
...
static TIMESTAMP sync_waterheater_impl(OBJECT *obj, TIMESTAMP t0,
                                       PASSCONFIG pass) {
  try {
    waterheater *my = object_data<waterheater>(obj);
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_waterheater(OBJECT *obj, TIMESTAMP t0,
                                                 PASSCONFIG pass) {
...
EXPORT TIMESTAMP plc_waterheater(OBJECT *obj, TIMESTAMP t0) {
  ...
  // no changes to timestamp will be made by the internal water heater
  // thermostat
  /// @todo If external plc codes return a timestamp, it will allow sync sooner
  /// but not later than water heater time to transition (ticket #147)
  return TS_NEVER;
}
