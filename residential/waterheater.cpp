...
waterheater::waterheater(MODULE *module) : residential_enduse(module) {
    ...
    if (oclass == NULL) {
        ...
        if (gl_publish_variable(oclass,
                                ...
                                PT_double, "tank_volume[gal]", PADDR(tank_volume), PT_DESCRIPTION,
                                "the volume of water in the tank when it is full",
                                PT_double, "tank_UA[Btu*h/degF]", PADDR(tank_UA), PT_DESCRIPTION,
                                "the UA of the tank (surface area divided by R-value)",
                                PT_double, "tank_diameter[ft]", PADDR(tank_diameter), PT_DESCRIPTION,
                                "the diameter of the water heater tank",
                                PT_double, "tank_height[ft]", PADDR(tank_height), PT_DESCRIPTION,
                                "the height of the water heater tank",
                                PT_double, "water_demand[gpm]", PADDR(water_demand), PT_DESCRIPTION,
                                "the hot water draw from the water heater",
                                PT_double, "heating_element_capacity[kW]", PADDR(heating_element_capacity),
                                PT_DESCRIPTION, "the power of the heating element",
                                PT_double, "inlet_water_temperature[degF]", PADDR(Tinlet), PT_DESCRIPTION,
                                ...
                                PT_double, "height[ft]", PADDR(h), PT_DESCRIPTION,
                                ...
                                PT_complex, "actual_power[kVA]", PADDR(waterheater_actual_power), PT_DESCRIPTION,
                                "the actual power based on the current voltage across the coils",
                                PT_double, "is_waterheater_on", PADDR(is_waterheater_on), PT_DESCRIPTION,
                                "simple logic output to determine state of waterheater (1-on, 0-off)",
                                PT_double, "gas_fan_power[kW]", PADDR(gas_fan_power), PT_DESCRIPTION,
                                "load of a running gas waterheater",
                                PT_double, "gas_standby_power[kW]", PADDR(gas_standby_power), PT_DESCRIPTION,
                                "load of a gas waterheater in standby",
                                PT_double, "heat_pump_coefficient_of_performance[Btu/kWh]", PADDR(HP_COP),
                                PT_DESCRIPTION,
                ...
                //published variables for fortran water heater
                                ...
                                PT_double, "operating_mode", PADDR(operating_mode), PT_DESCRIPTION,
                                "the operating mode the fortran water heater should be using.",
                                PT_double, "fortran_sim_time[s]", PADDR(simulation_time), PT_DESCRIPTION,
                ...
                //published variables for the multi layer waterheater model
                                ...
                                PT_double, "upper_tank_setpoint[degF]", PADDR(tank_setpoint_2), PT_DESCRIPTION,
                                "MULTILAYER_MODEL: The setpoint for the upper heating element thermostat in the tank",
                                PT_double, "lower_tank_deadband[degF]", PADDR(deadband_1), PT_DESCRIPTION,
                                "MULTILAYER_MODEL: The deadband for the lower heating element thermostat in the tank",
                                PT_double, "upper_tank_deadband[degF]", PADDR(deadband_2), PT_DESCRIPTION,
                                "MULTILAYER_MODEL: The deadband for the upper heating element thermostat in the tank",
                                PT_double, "lower_tank_temperature[degF]", PADDR(Tw_1), PT_DESCRIPTION,
                                "MULTILAYER_MODEL: The water temperature at the lower heating element thermostat in the tank",
                                PT_double, "upper_tank_temperature[degF]", PADDR(Tw_2), PT_DESCRIPTION,
                                "MULTILAYER_MODEL: The water temperature for the upper heating element thermostat in the tank",
                                PT_double, "discrete_step_size[s]", PADDR(discrete_step_size), PT_DESCRIPTION,
                                "MULTILAYER MODEL: The step size in seconds to use in the discrete tank temperature dynamics loop.",
                                PT_double, "circular_flow_rate[gpm]", PADDR(Vdot_circ), PT_DESCRIPTION,
...
int waterheater::init(OBJECT *parent) {
    ...
    nominal_voltage = (2.0 *
                       default_line_voltage); //@TODO:  Determine if this should be published or how we want to obtain this from the equipment/network
    ...
    if (tank_volume <= 0.0) {
        if (tank_diameter <= 0) {
            if (tank_height <= 0) {
                ...
                gl_warning(
                        "waterheater::init() : tank volume, diameter, and height were not specified, defaulting to 50 gallons and 3.78 ft");
                ...
                gl_warning(
                        "waterheater::init() : tank volume and diameter were not specified, defaulting to 50 gallons");
        ...
        if (tank_height <= 0) {
            ...
            if (tank_diameter <= 0) {
                ...
                if (abs(temp_tank_diameter - tank_diameter) > 0.05) {
                    gl_error(
                            "waterheater::init() : tank volume, diameter, and height were all set, but did not agree with each other.  Please unspecify one variable to be calculated.");
                    /*  TROUBLESHOOT
    ...
    if (height <= 0) {
        ...
        gl_warning(
                "waterheater::init() : height of water column was set to value greater than tank height. setting initial height to tank height.");
        height = tank_height;
    }
    if (tank_setpoint < 90 || tank_setpoint > 160)
        gl_error(
                "watertank thermostat is set to %f and is outside the bounds of 90 to 160 degrees Fahrenheit (32.2 - 71.1 Celsius).",
                tank_setpoint);
    /*	TROUBLESHOOT
    ...
    */
    ...
    if (thermostat_deadband > 10 || thermostat_deadband < 0.0)
        GL_THROW("watertank deadband of %f is outside accepted bounds of 0 to 10 degrees (5.6 degC).",
                 thermostat_deadband);
    if (current_model != FORTRAN) {
        // initial tank UA
        if (tank_UA <= 0.0)
    ...
    if (current_model != FORTRAN && current_model != MULTILAYER) {
        ...
        // initial demand
        Tset_curtail = tank_setpoint - thermostat_deadband / 2 -
...
//										 1000.0;	// W/gal
        ...
        case MT_PULSED:
            ...
            if (shape.params.pulsed.pulsetype ==
                MPT_TIME) { ; /* constant time pulse ~ consumes X gallons to drive heater for Y hours ~ but what's Vdot, what's t? */
            } else if (shape.params.pulsed.pulsetype ==
                       MPT_POWER) { ; /* constant power pulse ~ draws water to consume X kW, limited by C + Q * h ~ Vdot proportional to power/time */
                water_demand = gl_get_loadshape_value(&shape) / 2.4449;
            }
            break;
        case MT_MODULATED:
            ...
            break;
        case MT_QUEUED:
            if (shape.params.queued.pulsetype ==
                MPT_TIME) { ; /* constant time pulse ~ consumes X gallons/minute to consume Y thermal energy */
            } else if (shape.params.queued.pulsetype ==
                       MPT_POWER) { ; /* constant power pulse ~ draws water to consume X kW, limited by C + Q * h */
                water_demand = gl_get_loadshape_value(&shape) / 2.4449;
            }
            break;
        default:
    ...
    if (current_model == MULTILAYER) {
        ...
        if (discrete_step_size < 1) {
            ...
            gl_warning(
                    "waterheater::init() : The discretization step size is chosen too large. To reduce error setting it to 60 seconds.");
        ...
        if (Vdot_circ < 0.0) { // if no user input, set a default value
            ...
            gl_warning(
                    "waterheater::init() : The Heuristic flow rate is too small for 10 layers. Setting it to minimum 1 gpm.");
            Vdot_circ = 1.0;
        } else if (Vdot_circ > 3.0) {
            gl_warning(
                    "waterheater::init() : The Heuristic flow rate is too large for 10 layers. Setting it to maximum 3 gpm.");
            Vdot_circ = 3.0;
        }
        ...
        if (T_mixing_valve < 1.0) {
            gl_warning(
                    "waterheater::init() : Mixing valve temperature was not set, setting it to minimum upper element setpoint.");
            ...
            gl_warning(
                    "waterheater::init() : Mixing valve temperature is set very high, it will not be very efficient. Try choosing a value less than upper element setpoint.");
        }
        ...
        top_layer_disk = 1;
        number_of_layers =
                (number_of_mixing_zone_disks * total_mixing_zones) + (number_of_regular_disks * total_regular_zones) +
...
//		Vdot_circ = 2;
        ...
        b_matrix_coefficient =
                heating_element_capacity * BTUPHPKW / (RHOWATER * Cp * V_layer * number_of_mixing_zone_disks);
...
TIMESTAMP waterheater::presync(TIMESTAMP t0, TIMESTAMP t1) {
    ...
    if (current_model == MULTILAYER) {
        ...
        if (water_demand != last_water_demand || Tinlet != last_inlet_temperature ||
            tank_setpoint_1 != last_lower_thermostat_setpoint || tank_setpoint_2 != last_upper_thermostat_setpoint ||
            re_override != last_override_value || control_upper[idx] != control_upper[idx2] ||
...
TIMESTAMP waterheater::sync(TIMESTAMP t0, TIMESTAMP t1) {
    ...
    if (current_model == FORTRAN) {
        if (t1 ==
            fwh_sim_time) {//update power, cop, water temperature, and heat gain from the fortran water heater model and tell the model to simulate again.
            ...
            if (location == INSIDE) {
                for (i = 0; i < coarse_tank_grid * fine_tank_grid; i++) {
                    internal_gain += (tank_heat_loss_rate * (((init_tank_temp[i] * (9.0 / 5.0)) + 32.0) - Tamb)) /
    ...
    waterheater_actual_power =
            load.power + (load.current + load.admittance * load.voltage_factor) * load.voltage_factor;
...
double waterheater::dhdt(double h) {
    ...
    if (heat_mode == HEAT_PUMP) {
        ...
        cA = -mdot / (RHOWATER * area) +
             (actual_kW() * BTUPHPKW * HP_COP + tank_UA * (get_Tambient(location) - Tlower)) / c1;
    } else {
...
double waterheater::actual_kW(void) {
    ...
    if (heat_needed && re_override != OV_OFF) {
        ...
        if (actual_voltage > 2.0 * nominal_voltage) {
            if (trip_counter++ > 10)
                gl_error("Water heater line voltage for waterheater:%d is too high, exceeds twice nominal voltage.",
                         obj->id);
                /*	TROUBLESHOOT
        ...
        if (heat_mode == ELECTRIC) {
            ...
            heating_element_capacity =
                    (1.09 + (1.17 - 1.09) * (get_Tambient(location) - 50) / (70 - 50)) * (0.379 + 0.00364 * Tw);
...
inline double waterheater::new_temp_1node(double T0, double delta_t) {
    ...
    if (heat_mode == HEAT_PUMP) {
        // @TODO: These values were created from HPWH project; need to make them more accessible
        HP_COP = (1.04 + (1.21 - 1.04) * (get_Tambient(location) - 50) / (70 - 50)) * (5.259 - 0.0255 * Tw);
        c2 = (actual_kW() * BTUPHPKW * HP_COP + mdot_Cp * Tinlet + tank_UA * get_Tambient(location)) /
             (tank_UA + mdot_Cp);
    } else {
...
inline double waterheater::new_h_2zone(double h0, double delta_t) {
    ...
    const double c1 = RHOWATER * Cp * area * (/*Tupper*/ Tw -
                                                         Tlower); // lb/ft^3 * ft^2 * degF * Btu/lb.degF = lb/lb * ft^2/ft^3 * degF/degF * Btu = Btu/ft
...
void waterheater::purge_control_maps(unsigned dt) {
    double buffer;

    buffer = control_lower[dt];
    control_lower.clear();
    control_lower[dt] = buffer;
    buffer = control_upper[dt];
    control_upper.clear();
    control_upper[dt] = buffer;

    for (int i = 0; i < number_of_states; i++) {
        buffer = T_layers[i][dt];
        T_layers[i].clear();
        T_layers[i][dt] = buffer;
    }
}
