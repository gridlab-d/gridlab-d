...
//////////////////////////////////////////////////////////////////////////
// ev_charger CLASS FUNCTIONS
//////////////////////////////////////////////////////////////////////////
CLASS* ev_charger::oclass = NULL;
CLASS* ev_charger::pclass = NULL;

ev_charger::ev_charger(MODULE *module) : residential_enduse(module)
{
	// first time init
	if (oclass==NULL)
	{
		// register the class definition
		oclass = gl_register_class(module,"ev_charger",sizeof(ev_charger),PC_BOTTOMUP|PC_POSTTOPDOWN);
		if (oclass==NULL)
			throw "unable to register class ev_charger";
		...
		if (gl_publish_variable(oclass,
			...
			if (gl_publish_function(oclass,	"interupdate_res_object", (FUNCTIONADDR)interupdate_ev_charger)==NULL)
				GL_THROW("Unable to publish ev_charger deltamode function");
	}
}

int ev_charger::create() 
{
...
int ev_charger::init(OBJECT *parent)
{
	if(parent != NULL){
		if((parent->flags & OF_INIT) != OF_INIT){
			char objname[256];
			gl_verbose("ev_charger::init(): deferring initialization on %s", gl_name(parent, objname, 255));
			return 2; // defer
		}
	}
	...
	{
		GL_THROW("ev_charger:%s - Failed to map global minimum timestep variable",hdr->name?hdr->name:"unnamed");
		/*  TROUBLESHOOT
	...
	if (glob_min_timestep > 1)					//Now check us
	{
		...
		gl_verbose("ev_charger:%s - minimum_timestep set - problems may emerge",hdr->name);
		/*  TROUBLESHOOT
	...
	if (CarInformation.MaxChargeRate > 1700)
	{
		...
		{
			gl_warning("ev_charger:%s - The max charge rate is over 1.7 kW (Level 1), but the load is still 110-V connected.",hdr->name ? hdr->name : "unnamed");
			/*  TROUBLESHOOT
			The ev_charger maximum_charge_rate implies a level 2 or higher charge rate, but it still set up to be connected as a normal level 1 110/120 VAC device.
	...
	{
		gl_warning("ev_charger:%s - the breaker rating may be too low for this maximum charge rate.",hdr->name ? hdr->name : "unnamed");
		/*  TROUBLESHOOT
		The ev_charger maximum_charge_rate and configuration calculated a maximum breaker current that may be above the current setting.
	...
	if (deltamode_inclusive)
	{
		...
		{
			gl_warning("ev_charger:%s indicates it wants to run deltamode, but the module-level flag is not set!",hdr->name?hdr->name:"unnamed");
			/*  TROUBLESHOOT
			The ev_charger object has the deltamode_inclusive flag set, but not the module-level enable_subsecond_models flag.  The ev_charger
	...
	else	//Not enabled for this model
	{
		if (enable_subsecond_models == true)
		{
			gl_warning("ev_charger:%d %s - Deltamode is enabled for the module, but not this house!",hdr->id,(hdr->name ? hdr->name : "Unnamed"));
			/*  TROUBLESHOOT
			The ev_charger is not flagged for deltamode operations, yet deltamode simulations are enabled for the overall system.  When deltamode
			triggers, this ev_charger may no longer contribute to the system, until event-driven mode resumes.  This could cause issues with the simulation.
...
int ev_charger::isa(char *classname)
{
	return (strcmp(classname,"ev_charger")==0 || residential_enduse::isa(classname));
}

TIMESTAMP ev_charger::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
...
TIMESTAMP ev_charger::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	double ret_value;
...
double ev_charger::sync_ev_function(double curr_time_dbl)
{
	OBJECT *obj = OBJECTHDR(this);
...
double ev_charger::check_J2894_values(double diff_time, double tstep_value, bool *floored_trip)
{
...
SIMULATIONMODE ev_charger::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
...
EXPORT int create_ev_charger(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(ev_charger::oclass);

		if (*obj!=NULL)
		{
			ev_charger *my = OBJECTDATA(*obj,ev_charger);
	...
	CREATE_CATCHALL(ev_charger);
}

EXPORT int init_ev_charger(OBJECT *obj)
{
	try {
		ev_charger *my = OBJECTDATA(obj,ev_charger);
		return my->init(obj->parent);
	}
	INIT_CATCHALL(ev_charger);
}

EXPORT int isa_ev_charger(OBJECT *obj, char *classname)
{
	if(obj != 0 && classname != 0){
		return OBJECTDATA(obj,ev_charger)->isa(classname);
	} else {
		return 0;
	}
}

EXPORT TIMESTAMP sync_ev_charger(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	TIMESTAMP t1;
	...
	try {
		...
		case PC_POSTTOPDOWN:
			...
			break;
		default:
			gl_error("ev_charger::sync- invalid pass configuration");
			t1 = TS_INVALID; // serious error in exec.c
		}
		return t1;
	} 
	SYNC_CATCHALL(ev_charger);
}

//Deltamode linkage function
//Deltamode exposed functions
EXPORT SIMULATIONMODE interupdate_ev_charger(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	ev_charger *my = OBJECTDATA(obj,ev_charger);
	...
	{
		gl_error("interupdate_ev_charger(obj=%d;%s): %s", obj->id, obj->name?obj->name:"unnamed", msg);
		return status;
	}
}
