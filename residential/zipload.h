...
public:
  ...
  bool demand_response_mode; ///< Activates equilibrium dynamic representation
                             ///< of demand response
  ...
  int16 x;    ///< temperature of the device's controlled media (eg air temp or
              ///< water temp)
  double phi; ///< duty cycle of the device
  double PHI; ///< diversity of a population of devices
  double eta; ///< consumer demand rate that prematurely turns on a device or
              ///< population
  double rho; ///< effect rate at which devices heats up or cools down under
              ///< consumer demand
  double nominal_power;
  int64 next_time, last_time; ///< used to keep track of time in "special" modes
                              ///< - DR, duty-cycle
  ...
  double multiplier; ///< static multiplier to modify base power ( load =
                     ///< base_power * multiplier )
  double recovery_duty_cycle; ///< duty cycle during recovery interval
  bool heatgain_only; ///< Activates a heat only mode - no load electric load is
  ...
  DRMODEL drm;
  DRMODEL previous_drm; ///< structures to save drm population and previous
                        ///< population
