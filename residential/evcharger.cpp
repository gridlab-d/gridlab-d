/** $Id: evcharger.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - ELECTRIC	pure electric vehicle with limited range based on
 battery capacity
        - HYBRID	hybrid electric vehicle with longer range based on fuel
 capacity

        The evcharger simulation is based on demand state profile of the
 vehicle. When the vehicle is at home, it has a probability of leaving on one of
 3 trips.

        \image html residential/docs/evcharger/Slide1.PNG "Figure 1 - EV charger
 state diagram"

        - WORK		trip to work (standard distance defined by trip.d_work)
        - SHORTTRIP	random trip up to 50 miles (battery will not be fully
 discharged)
        - LONGTRIP	random trip over 50 miles (battery will be discharged up
 to 25%) but only possible for HYBRID vehicles

        \image html residential/docs/evcharger/Slide2.PNG "Figure 2 - EV charger
 home/work arrival/departure statistics"

        When away, the probability of a return is used to determine when the
 vehicle returns.
        ...
        The trip distances is used to estimate the battery charge upon return
 according to the following rules:

        1. A work trip discharges the battery depending on whether
 charge_at_work is defined. If charging at work is allowed, the battery will
 discharge for 1 trip, otherwise it will discharge for 2 trips.

        2. A short trip discharges the battery based on the distance traveled.

        3. A long trip discharges the battery to 25%, but is only possible with
 hybrids.

        In all cases, if the trip distance is greater than 50 miles and the car
 is a hybrid, the discharge will be down to 25%.

        Heat fraction ratio is used to calculate the internal gain from plug
 loads.
        ...
        is either \p HOME, \p WORK, \p SHRT, or \p LONG.  There must
 be 24.rows() of numbers, the numbers must be positive numbers between 0 and 1,
 and each column must be normalized (they must add up to 1.000 over the 24 hour
 period).

        You may introduce as many daytype blocks as are supported simultaneously
 (2 max at this time).

        @todo the 24.rows() are not check for compliance with the 24 hour rule,
 the 0-1 rule, or the normalized rule.
 @{
 **/
...
EVDEMAND *load_demand_profile(char *filename) {
  char filepath[1024];
  if (gl_findfile(filename, nullptr, R_OK, filepath, sizeof(filepath)) ==
      nullptr) {
  ...
  if (fp == nullptr) {
    ...
    GL_THROW("unable to read demand profile in '%s' at line %d", filename,
             linenum);
  }
  char line[1024];
  double *hdr[8];
    ...
    if (line[0] == '[') // daytype start
    {
      ...
      if (sscanf(line, "[%7s]", blockname) == 1) {
        if (strcmp(blockname, "WEEKDAY") == 0)
          ...
          GL_THROW("%s(%d): '%s' is an invalid daytype name", filename, linenum,
                   blockname);
          // throw "invalid block name";
        }
        data->n_daytypes++;
      } else {
        GL_THROW("%s(%d): daytype name syntax is not recognized", filename,
                 linenum, blockname);
    ...
    } else if (isalpha(line[0])) // header
    {
      ...
      while (sscanf(line + offset, "%3s%4s", dir, trip) == 2) {
        if (nhdr > sizeof(hdr) / sizeof(hdr[0])) {
          gl_output("%s(%d): too many heading items listed (max is %d)",
                    filename, linenum, sizeof(hdr) / sizeof(hdr[0]));
        ...
        if (strcmp(dir, "ARR") == 0)
          ...
          GL_THROW("%s(%d): '%s' is not an appropriate event direction (e.g., "
                   "ARR, DEP)",
                   filename, linenum, dir);
        ...
        if (strcmp(trip, "HOME") == 0)
          ...
          GL_THROW("%s(%d): '%s' is not an appropriate trip location (e.g., "
                   "HOME, WORK)",
                   filename, linenum, dir);
...
evcharger::evcharger(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "evcharger", sizeof(evcharger),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "residential_enduse", PT_enumeration,
            "charger_type", PADDR(charger_type), PT_KEYWORD, "LOW",
            (enumeration)CT_LOW, PT_KEYWORD, "MEDIUM", (enumeration)CT_MEDIUM,
            PT_KEYWORD, "HIGH", (enumeration)CT_HIGH, PT_enumeration,
            "vehicle_type", PADDR(vehicle_type), PT_KEYWORD, "ELECTRIC",
            (enumeration)VT_ELECTRIC, PT_KEYWORD, "HYBRID",
            (enumeration)VT_HYBRID, PT_enumeration, "state",
            PADDR(vehicle_state), PT_KEYWORD, "UNKNOWN",
            (enumeration)VS_UNKNOWN, PT_KEYWORD, "HOME", (enumeration)VS_HOME,
            PT_KEYWORD, "WORK", (enumeration)VS_WORK,
            // these are not yet supported
            //	PT_KEYWORD,"SHORTTRIP",SHORTTRIP,
            //	PT_KEYWORD,"LONGTRIP",LONGTRIP,
            PT_double, "p_go_home[unit/h]", PADDR(demand.home), PT_double,
            "p_go_work[unit/h]", PADDR(demand.work),
            // these are not yet supported
            ...
            PT_double, "capacity[kWh]", PADDR(capacity), PT_double,
            "charge[unit]", PADDR(charge), PT_bool, "charge_at_work",
            PADDR(charge_at_work), PT_double, "charge_throttle[unit]",
            PADDR(charge_throttle), PT_double, "charger_efficiency[unit]",
            PADDR(charging_efficiency), PT_DESCRIPTION,
            "Efficiency of the charger in terms of energy in to battery stored",
            PT_double, "power_train_efficiency[mile/kWh]", PADDR(mileage),
            PT_DESCRIPTION, "Miles per kWh of battery charge", PT_double,
            "mileage_classification[mile]", PADDR(mileage_classification),
            PT_DESCRIPTION, "Miles expected range on battery only", PT_char1024,
            "demand_profile", PADDR(demand_profile), nullptr) < 1)
...
int evcharger::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("evcharger::init(): deferring initialization on %s",
                 gl_name(parent, objname, 255));
      return 2; // defer
    }
  }
  static double sizes[] = {20, 30, 30, 40, 40, 40, 50, 50,
                           50, 50, 60, 60, 60, 70, 70, 80};

  if (mileage == 0)
    mileage = gl_random_uniform(RNGSTATE, 0.8, 1.2);

  // See if mileage classification and mileage are defined
  if ((mileage_classification > 0.0) && (mileage > 0.0)) {
    capacity = mileage_classification /
               mileage; // capacity = miles expected * kWh/mile
  }

  if (capacity == 0)
    capacity =
        gl_random_sampled(RNGSTATE, sizeof(sizes) / sizeof(sizes[0]), sizes);
  if (power_factor == 0)
    power_factor = 0.95;
  if (charge == 0)
    charge = gl_random_uniform(RNGSTATE, 0.25, 1.0);
  if (charge_throttle == 0)
    charge_throttle = 1.0;
  if (mileage == 0)
    mileage = gl_random_uniform(RNGSTATE, 0.8, 1.2);
  if (distance.work == 0)
    distance.work = gl_random_lognormal(RNGSTATE, 3, 1);
  // these are not yet supported
  ...
  if ((charging_efficiency > 1.0) || (charging_efficiency < 0.0)) {
    ...
    /*  TROUBLESHOOT
    A charger_efficiency value outside of the possible values was specified.
...
double evcharger::update_state(double dt /* seconds */) {
  ...
  if (obj->clock > TS_ZERO && dt > 0) {
    ...
    /*	TROUBLESHOOT
            The clock likely was not properly initialized, or the object clock
       has been overwritten with garbage.  Please verify that the clock is set
       by the input model.
    */
    ...
    case VS_HOME:
      if (gl_random_bernoulli(RNGSTATE, demand.home * dt / 3600)) {
        gl_debug("%s (%s:%d) leaves for work with %.0f%% charge",
                 obj->name ? obj->name : "anonymous", obj->oclass->name,
                 obj->id, charge * 100);
    ...
    case VS_WORK:
      if (gl_random_bernoulli(RNGSTATE, demand.work * dt / 3600)) {
        ...
        if (charge <= 0)
          gl_warning(
              "%s (%s:%d) vehicle has run out of charge while away from home",
              obj->name ? obj->name : "anonymous", obj->oclass->name, obj->id);
        else
          gl_debug("%s (%s:%d) returns from work with %.0f%% charge",
                   obj->name ? obj->name : "anonymous", obj->oclass->name,
                   obj->id, charge * 100);
      }
      break;
    ...
    default:
      ...
      /*	TROUBLESHOOT
              The evcharger vehicle has found itself in an ambiguous state while
         calculating its state and level of charge.  Please verify that the
         object was initialized with "state" as either "HOME" or "WORK".  If it
         is, please fill out a bug report and include the relevant sections of
         the evcharger objects and class blocks from your input model.
      */
      break;
    }
  }
  ...
  case VS_HOME:
    if (charge < 1.0 && obj->clock > 0) {
      ...
      case CT_HIGH:
        ...
        charge_kw = amps[(int)charger_type] * temp_voltage_magnitude *
                    charge_throttle / 1000;
        break;
      default:
        GL_THROW("invalid charger_type");
        /*	TROUBLESHOOT
                The vehicle charger is not in a valid state.  It may not have
           been declared properly in the model file.  Please verify that the
           "charger_type" property is either "LOW", "MEDIUM", or "HIGH" in the
           model file.  If it is defined properly, please fill out a bug report
           and include the relevant sections with the evcharger objects and
           class blocks from your input model.
        */
        break;
      }
      ...
      if (charge > 1.0) {
        gl_warning("%s (%s:%d) overcharge by %.0f%% truncated",
                   obj->name ? obj->name : "anonymous", obj->oclass->name,
                   obj->id, (charge - 1) * 100);
        // TODO: the balance of charge should be dumped as heat into the garage
        charge = 1.0;
      } else if (charge < 0.0) {
        gl_warning("%s (%s:%d) 100%% discharged",
                   obj->name ? obj->name : "anonymous", obj->oclass->name,
                   obj->id);
        // TODO: the balance of charge should be dumped as heat into the garage
        charge = 0.0;
      } else if (charge > 0.999) {
        gl_debug("%s (%s:%d) charge complete",
                 obj->name ? obj->name : "anonymous", obj->oclass->name,
                 obj->id);
        charge = 1.0;
      }
  ...
  default:
    ...
    /*	TROUBLESHOOT
            The evcharger vehicle has found itself in an ambiguous state while
       calculating its effects on its parent house.  Please verify that the
       object was initialized with "state" as either "HOME" or "WORK".  If it
       is, please fill out a bug report and include the relevant sections of the
       evcharger objects and class blocks from your input model.
    */
    break;
  }
  ...
  /*	TROUBLESHOOT
          The charge value is no longer a finite value.  Please submit a bug
     report with the entire offending model file, or create a simplified model
     using subsections of the offending model to isolate or eradicate the error.
  */
...
TIMESTAMP evcharger::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (dt == 0)
    gl_warning("%s (%s:%d) didn't advance the clock",
               obj->name ? obj->name : "anonymous", obj->oclass->name, obj->id);
