...
residential_enduse::residential_enduse(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass =
        gld_class::create(mod, "residential_enduse", sizeof(residential_enduse),
                          PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);
    /* TROUBLESHOOT
            The registration for the residential_enduse class failed.   This is
    ...
    if (gl_publish_variable(
            oclass, PT_loadshape, "shape", PADDR(shape), PT_enduse, "",
            PADDR(load), PT_enumeration, "override", PADDR(re_override),
            PT_KEYWORD, "NORMAL", (enumeration)OV_NORMAL, PT_KEYWORD, "ON",
            (enumeration)OV_ON, PT_KEYWORD, "OFF", (enumeration)OV_OFF,
            PT_enumeration, "power_state", PADDR(power_state), PT_KEYWORD,
            "OFF", (enumeration)PS_OFF, PT_KEYWORD, "ON", (enumeration)PS_ON,
            PT_KEYWORD, "UNKNOWN", (enumeration)PS_UNKNOWN, nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    /* TROUBLESHOOT
            The registration for the residential_enduse properties failed. This
...
int residential_enduse::init(OBJECT *parent) {
  ...
  if (pParent != nullptr && pParent->is_valid()) {
    ...
    ATTACHFUNCTION attach =
        (ATTACHFUNCTION)pParent->get_function("attach_enduse");
    if (attach)
      ...
      gl_warning("%s (%s:%d) parent %s (%s:%d) does not export attach_enduse "
                 ...
                 pParent->get_id());
    /* TROUBLESHOOT
            Enduses must have a voltage source from a parent object that exports
       an attach_enduse function. The residential_enduse object references a
       parent object that does not conform with this requirement. Fix the parent
  ...
  if (load.shape != nullptr) {
    if (load.shape->schedule == nullptr) {
      gl_verbose(
          "%s (%s:%d) schedule is not specified so the load may be inactive",
          get_name(), get_oclass()->get_name(), get_id());
      /* TROUBLESHOOT
              The residential_enduse object requires a schedule that defines how
              the load behaves.  Omitting this schedule effectively shuts the
         enduse load off and this is not typically intended.
       */
    }
  }
...
TIMESTAMP residential_enduse::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (load.voltage_factor > 1.2 || load.voltage_factor < 0.8)
    gl_verbose(
        "%s voltage is out of normal +/- 20%% range of nominal (vf=%.2f)",
        get_name(), load.voltage_factor);
  /* TROUBLESHOOT
     The voltage on the enduse circuit is outside the expected range for that
     enduse. This is usually caused by an impropely configure circuit (e.g.,
     110V on 220V or vice versa). Fix the circuit configuration for that enduse
     and try again.
   */
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_residential_enduse(OBJECT *obj, TIMESTAMP t0,
                                             PASSCONFIG pass) {
