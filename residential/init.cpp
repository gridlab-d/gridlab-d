...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  gl_global_create("residential::default_line_voltage", PT_double,
                   &default_line_voltage, PT_UNITS, "V", PT_DESCRIPTION,
                   "line voltage (L-N) to use when no circuit is attached",
                   nullptr);
  gl_global_create(
      "residential::default_outdoor_temperature", PT_double,
      &default_outdoor_temperature, PT_UNITS, "degF", PT_DESCRIPTION,
      "outdoor air temperature when no climate data is found", nullptr);
  gl_global_create("residential::default_humidity", PT_double,
                   &default_humidity, PT_UNITS, "%", PT_DESCRIPTION,
                   "humidity when no climate data is found", nullptr);
  gl_global_create(
      "residential::default_horizontal_solar", PT_double,
      &default_horizontal_solar, PT_UNITS, "Btu/sf", PT_DESCRIPTION,
      "horizontal solar gains when no climate data is found", nullptr);
  gl_global_create("residential::default_etp_iterations", PT_int64,
                   &default_etp_iterations, PT_DESCRIPTION,
                   "number of iterations ETP solver will run", nullptr);
  gl_global_create("residential::default_grid_frequency", PT_double,
                   &default_grid_frequency, PT_UNITS, "Hz", PT_DESCRIPTION,
                   "grid frequency when no powerflow attachment is found",
                   nullptr);
  gl_global_create("residential::ANSI_voltage_check", PT_bool,
                   &ANSI_voltage_check, PT_DESCRIPTION,
                   "enable or disable messages about ANSI voltage limit "
                   "violations in the house",
                   nullptr);
  gl_global_create(
      "residential::enable_subsecond_models", PT_bool, &enable_subsecond_models,
      PT_DESCRIPTION,
      "Enable deltamode capabilities within the residential module", nullptr);
  gl_global_create("residential::deltamode_timestep", PT_double,
                   &deltamode_timestep_publish, PT_UNITS, "ns", PT_DESCRIPTION,
                   "Desired minimum timestep for deltamode-related simulations",
                   nullptr);
  gl_global_create(
      "residential::all_residential_delta", PT_bool, &all_residential_delta,
      PT_DESCRIPTION,
      "Modeling convenient - enables all residential objects in deltamode",
      nullptr);
...
void schedule_deltamode_start(TIMESTAMP tstart) {
  if (enable_subsecond_models == true) // Make sure the overall mode is enabled
  {
    ...
    GL_THROW("residential: a call was made to deltamode functions, but "
             "subsecond models are not enabled!");
    /*  TROUBLESHOOT
    The schedule_deltamode_start function was called by an object when
    residential's overall enabled_subsecond_models flag was not set.  The
...
void allocate_deltamode_arrays(void) {
  ...
  if ((res_object_current == -1) || (delta_objects == nullptr)) {
    ...
    if (delta_objects == nullptr) {
      GL_THROW(
          "Failed to allocate deltamode objects array for residential module!");
      /*  TROUBLESHOOT
      While attempting to create a reference array for residential module
      deltamode-enabled objects, an error was encountered.  Please try again. If
    ...
    delta_functions =
        (FUNCTIONADDR *)gl_malloc(res_object_count * sizeof(FUNCTIONADDR));
...
EXPORT unsigned long deltamode_desired(int *flags) {
  ...
  if (enable_subsecond_models ==
      true) // Make sure we even want to run deltamode
  {
...
EXPORT unsigned long preupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models == true) {
    if (deltamode_timestep_publish <= 0.0) {
      gl_error("residential::deltamode_timestep must be a positive, non-zero "
               "number!");
      /*  TROUBLESHOOT
      The value for deltamode_timestep, as specified as the module level in
      residential, must be a positive, non-zero number. Please use such a number
      and try again.
      */
...
EXPORT SIMULATIONMODE interupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models == true) {
    // See if this is the first instance -- if so, update the timestep
    // (flagging-related)
    ...
    for (curr_object_number = 0; curr_object_number < res_object_count;
         curr_object_number++) {
      ...
      if ((delta_objects[curr_object_number]->in_svc_double <=
        ...
        try {
          ...
          gl_error("residential:interupdate - pre-pass function call: unknown "
                   "exception");
          function_status = SM_ERROR; // Flag as an error too
        }
      ...
      if (function_status == SM_DELTA) {
        gl_verbose(
            "Residential object:%d - %s - requested deltamode to continue",
        ...
        gl_verbose(
            "Residential object:%d - %s - requested a deltamode reiteration",
            delta_objects[curr_object_number]->id,
            (delta_objects[curr_object_number]->name
                 ? delta_objects[curr_object_number]->name
                 : "Unnamed"));
        ...
        gl_error("Residential object:%d - %s - deltamode function returned an "
                 "error!",
                 delta_objects[curr_object_number]->id,
                 (delta_objects[curr_object_number]->name
                      ? delta_objects[curr_object_number]->name
                      : "Unnamed"));
        /*  TROUBLESHOOT
        While performing a deltamode update, one object returned an error code.
        Check to see if the object itself provided more details and try again.
    ...
    for (curr_object_number = 0; curr_object_number < res_object_count;
      ...
      if (post_delta_functions[curr_object_number] != nullptr) {
        ...
        if (function_status == FAILED) {
          gl_error("Residential object:%s - failed post-deltamode update",
                   delta_objects[curr_object_number]->name);
          /*  TROUBLESHOOT
