...
double lights::power_fraction[_MAXTYPES][3] = {
    ...
    0.6, // CFL
    0.80, 0.1,
    0.1, // SSL - these values are unvalidated
    0.80, 0.1,
    0.1, // HID - these values are unvalidated
};

// the constructor registers the class and properties and sets the defaults
lights::lights(MODULE *mod) : residential_enduse(mod) {
  // first time init
  if (oclass == nullptr) {
    pclass = residential_enduse::oclass;

    // register the class definition
    oclass = gl_register_class(mod, "lights", sizeof(lights),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "residential_enduse", PT_enumeration, "type",
            PADDR(type), PT_DESCRIPTION, "lighting type (affects power_factor)",
            PT_KEYWORD, "INCANDESCENT", (enumeration)INCANDESCENT, PT_KEYWORD,
            "FLUORESCENT", (enumeration)FLUORESCENT, PT_KEYWORD, "CFL",
            (enumeration)CFL, PT_KEYWORD, "SSL", (enumeration)SSL, PT_KEYWORD,
            "HID", (enumeration)HID, PT_enumeration, "placement",
            PADDR(placement), PT_DESCRIPTION,
            "lighting location (affects where heatgains go)", PT_KEYWORD,
            "INDOOR", (enumeration)INDOOR, PT_KEYWORD, "OUTDOOR",
            (enumeration)OUTDOOR, PT_double, "installed_power[kW]",
            PADDR(shape.params.analog.power), PT_DESCRIPTION,
            "installed lighting capacity", PT_double, "power_density[W/sf]",
            PADDR(power_density), PT_DESCRIPTION, "installed power density",
            PT_double, "curtailment[pu]", PADDR(curtailment), PT_DESCRIPTION,
            "lighting curtailment factor", PT_double, "demand[pu]",
            PADDR(shape.load), PT_DESCRIPTION, "the current lighting demand",
            PT_complex, "actual_power[kVA]", PADDR(lights_actual_power),
            PT_DESCRIPTION, "actual power demand of lights object",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    /* TROUBLESHOOT
            The file that implements the specified class cannot publisht the
...
int lights::create(void) {
  ...
  // load.power_factor = 0.95; commenting out this line means a default power
  // factor of 1.00
  load.breaker_amps = 0;
...
int lights::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("lights::init(): deferring initialization on %s",
  ...
  if (load.power_factor == 0) {
    ...
    gl_warning("No value was given for power factor. Defaulting to power "
               "factor 1.00 and incandescent light type.");
  }
  if (load.voltage_factor == 0) {
    load.voltage_factor = 1.0;
    gl_warning("No value was given for voltage factor.  Defaulting to voltage "
               "factor 1.00");
  }
  if ((load.power_fraction + load.current_fraction + load.impedance_fraction) ==
      0.0) {
    ...
    gl_warning(
        "No value was given for power fraction. Defaulting to incandescent "
        "light type and corresponding power fraction 1,0,0.");
  }
  ...
  /* TROUBLESHOOT
          It is not possible to define the total energy consumed by a lighting
     load in relation to a schedule. Use either scaled power or installed power
     to define the lighting load and try again.
   */

  // installed power intially re_overrides use of power density
  double *floor_area =
      parent ? gl_get_double_by_name(parent, "floor_area") : nullptr;
  if (shape.params.analog.power == 0 && shape.schedule == nullptr) {
    // set basic properties
    if (power_density == 0)
      power_density = gl_random_triangle(RNGSTATE, 0.75, 1.25); // W/sf

    if (floor_area == nullptr) {
      gl_error("lights parent must publish \'floor_area\' to work properly if "
               "no installed_power is given ~ default 2500 sf");
      /* TROUBLESHOOT
              The lights did not reference a parent object that publishes a
         floor are, so 2500 sf was assumed. Change the parent reference and try
         again.
       */
...
int lights::isa(char *classname) {
  return (strcmp(classname, "lights") == 0 ||
...
TIMESTAMP lights::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (pCircuit != nullptr) {
    ...
    load.voltage_factor =
        temp_voltage_magnitude / default_line_voltage; // update voltage factor
  }
  ...
  if (shape.type == MT_UNKNOWN) { /* manual power calculation*/
    ...
    if (shape.load < 0) {
      ...
      gl_warning("lights shape demand exceeds installed lighting power, "
                 "capping to 100%%");
      shape.load = 1.0;
    }
    load.constant_power = shape.params.analog.power * shape.load;
    if (fabs(load.power_factor) < 1) {
      val = (load.power_factor < 0 ? -1.0 : 1.0) * load.constant_power.Re() *
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_lights(OBJECT *obj, TIMESTAMP t0,
                                             PASSCONFIG pass) {
