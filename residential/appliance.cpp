...
appliance::appliance(MODULE *module) : residential_enduse(module) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "appliance", sizeof(appliance),
                               PC_PRETOPDOWN | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);
    if (gl_publish_variable(
            oclass, PT_INHERIT, "residential_enduse", PT_complex_array,
            "powers", PADDR(power), PT_complex_array, "impedances",
            PADDR(impedance), PT_complex_array, "currents", PADDR(current),
            PT_double_array, "durations", PADDR(duration), PT_double_array,
            "transitions", PADDR(transition), PT_double_array, "heatgains",
            PADDR(heatgain), nullptr) < 1)
...
int appliance::init(OBJECT *parent) {
  ...
  if (duration.rows() != 1)
    exception("duration must have 1 rows (it has %d)", n_states,
              duration.rows());

  // number of states if length of duration vector
  n_states = (unsigned int)duration.cols();
  if (state < 0 || state >= n_states)
    exception("initial state must be between 0 and %d, inclusive",
              n_states - 1);
  gl_debug("n_states = %d (initial state is %d)", n_states, state);

  // transition must be either 1xN or NxN
  if ((transition.rows() != 1 && transition.rows() != n_states) ||
      transition.cols() != n_states)
    exception(
        "transition must have either 1r x %dc or %dr x %dc (it is %dr c %dc)",
        n_states, n_states, n_states, transition.rows(), transition.cols());
  ...
  if (impedance.rows() != 1 || impedance.cols() != n_states)
    exception("impedance must 1r x %dc (it is %dr x %dc)", n_states,
              impedance.rows(), impedance.cols());
  ...
  if (current.rows() != 1 || current.cols() != n_states)
    exception("current must 1r x %dc (it is %dr x %dc)", n_states,
  ...
  if (power.rows() != 1 || power.cols() != n_states)
    exception("power must 1r x %dc (it is %dr x %dc)", n_states, power.rows(),
              power.cols());
...
void appliance::update_next_t(void) {
  ...
  if (!isfinite(transition_probability)) {
    ...
    gl_debug("%s: non-probabilistic transition scheduled at %lld", get_name(),
             next_t);
  } else if (gl_random_uniform(&my()->rng_state, 0, 1) <
             transition_probability) {
    // transition is uncertain
    next_t = gl_globalclock + (TIMESTAMP)gl_random_uniform(&my()->rng_state, 1,
                                                           duration(0, state));
...
void appliance::update_power(void) {
  ...
  load.constant_admittance =
      Z.Mag() == 0 ? gld::complex(0) : gld::complex(1) / Z;
...
TIMESTAMP appliance::presync(TIMESTAMP t1) { return next_t; }
