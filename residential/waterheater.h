...
private:
  double standby_load; ///< typical power loss through thermal jacket losses (UA
                       ///< 2, 60 to 140 degF, 160 BTU/hr, 47W, 411kWh/year,
                       ///< ~10% energy star guesstimate)
public:
  typedef enum {
  ...
  typedef enum {
    ...
    HEAT_PUMP ///< tank heats with a heat pump (currently ignores all electric
              ///< coil usage)
  } HEATMODE; ///<
  ...
  enumeration
      heat_mode; ///< method of heating the water (gas or electric) [enum]
  ...
  double
      h; ///< boundary between hot and cold water layers [ft from top of tank]
  ...
  double energytake; ///< The amount of energy available in the tank to heat the
                     ///< water to the setpoint temperaute.
  ...
  bool heat_needed; ///< need to maintain this bit of state because of Tstat
                    ///< deadband...
  double is_waterheater_on; ///< Simple logic for determining state of
                            ///< waterheater 1-on, 0-off
public:
  ...
  double water_demand_old; ///< previous water demand, needed for temperature
                           ///< change (reflects heat loss from hot water draw)
  double heating_element_capacity; ///< rated Q of (each) heating element, input
                                   ///< in W, converted to[Btu/hr]
  double tank_setpoint;            ///< setpoint T of heating element [F]
  double thermostat_deadband; ///< deadband around Tset (half above, half below)
                              ///< [F]
  double *pTair;
  double *pTout;
  double *pRH;
  double HP_COP; ///< coefficient of performance for heat pump; currently
                 ///< calculated
  double
      gas_fan_power; ///< fan power draw when a gas waterheater is burning fuel
  double gas_standby_power; ///< standby power draw when a gas waterheater is
                            ///< NOT burning fuel
  ...
  double prev_load;
  gld::complex
      waterheater_actual_power; ///< the actual power draw of the object after
                                ///< accounting for voltage
...
public:
  ...
  double Tw_1;
  double Tw_2;
  typedef enum { OFF = 0, ON = 1 } HEATELEMENTSTATE;
...
public:
  ...
  void thermostat(TIMESTAMP t0, TIMESTAMP t1); // Thermostat plc control code -
                                               // determines whether to heat...
...
public:
  enumeration
      current_model;      ///< Discerns which water heater model we need to use
  ...
  enumeration set_current_model_and_load_state(
      void);                      // set the model and state for each cycle
  ...
  double new_temp_1node(double T0,
                        double delta_t);       // Calcs temp after transition...
  ...
  vector<double> multiply_waterheater_matrices(vector<vector<double>> &,
                                               vector<double> &);
  void reinitialize_internals(int dt);

  double get_Tambient(
      enumeration water_heater_location); // ambient T [F] -- either an indoor
                                          // house temperature or a garage
