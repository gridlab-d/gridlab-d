/** $Id: refrigerator.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The refrigerator model is based on performance profile that can
 specified by a tape and using a method developed by Ross T. Guttromson

        Original ODE:

        \f$ \frac{C_f}{UA_r + UA_f} \frac{dT_{air}}{dt} + T_{air} = T_{out} +
 \frac{Q_r}{UA_r} \f$
         ...
         where
                 ...
                 \f$ T_{out} \f$ is the ambient temperature around the
 refrigerator
...
refrigerator::refrigerator(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "refrigerator", sizeof(refrigerator),
                               PC_PRETOPDOWN | PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);
    /* TROUBLESHOOT
            The file that implements the lights in the residential module cannot
       register the class. This is an internal error.  Contact support for
       assistance.
     */

    // publish the class properties
    if (gl_publish_variable(
            oclass, PT_INHERIT, "residential_enduse", PT_double, "size[cf]",
            PADDR(size), PT_DESCRIPTION, "volume of the refrigerator",
            PT_double, "rated_capacity[Btu/h]", PADDR(rated_capacity),
            PT_double, "temperature[degF]", PADDR(Tair), PT_double,
            "setpoint[degF]", PADDR(Tset), PT_double, "deadband[degF]",
            PADDR(thermostat_deadband), PT_double, "cycle_time[s]",
            PADDR(cycle_time), PT_double, "output", PADDR(Qr), PT_double,
            "event_temp", PADDR(Tevent), PT_double, "UA[Btu/degF*h]", PADDR(UA),
            PT_double, "compressor_off_normal_energy",
            PADDR(compressor_off_normal_energy), PT_double,
            "compressor_off_normal_power[W]",
            PADDR(compressor_off_normal_power), PT_double,
            "compressor_on_normal_energy", PADDR(compressor_on_normal_energy),
            PT_double, "compressor_on_normal_power[W]",
            PADDR(compressor_on_normal_power), PT_double, "defrost_energy",
            PADDR(defrost_energy), PT_double, "defrost_power[W]",
            PADDR(defrost_power), PT_double, "icemaking_energy",
            PADDR(icemaking_energy), PT_double, "icemaking_power[W]",
            PADDR(icemaking_power), PT_double, "ice_making_probability",
            PADDR(ice_making_probability), PT_int32, "FF_Door_Openings",
            PADDR(FF_Door_Openings), PT_int32, "door_opening_energy",
            PADDR(door_opening_energy), PT_int32, "door_opening_power",
            PADDR(door_opening_power), PT_double, "DO_Thershold",
            PADDR(DO_Thershold), PT_double, "dr_mode_double",
            PADDR(dr_mode_double), PT_double, "energy_needed",
            PADDR(energy_needed), PT_double, "energy_used", PADDR(energy_used),
            PT_double, "refrigerator_power", PADDR(refrigerator_power), PT_bool,
            "icemaker_running", PADDR(icemaker_running), PT_int32, "check_DO",
            PADDR(check_DO), PT_bool, "is_240", PADDR(is_240), PT_double,
            "defrostDelayed", PADDR(defrostDelayed), PT_bool,
            "long_compressor_cycle_due", PADDR(long_compressor_cycle_due),
            PT_double, "long_compressor_cycle_time",
            PADDR(long_compressor_cycle_time), PT_double,
            "long_compressor_cycle_power", PADDR(long_compressor_cycle_power),
            PT_double, "long_compressor_cycle_energy",
            PADDR(long_compressor_cycle_energy), PT_double,
            "long_compressor_cycle_threshold",
            PADDR(long_compressor_cycle_threshold), PT_enumeration,
            "defrost_criterion", PADDR(defrost_criterion), PT_KEYWORD, "TIMED",
            (enumeration)DC_TIMED, PT_KEYWORD, "DOOR_OPENINGS",
            (enumeration)DC_DOOR_OPENINGS, PT_KEYWORD, "COMPRESSOR_TIME",
            (enumeration)DC_COMPRESSOR_TIME, PT_bool, "run_defrost",
            PADDR(run_defrost), PT_double, "door_opening_criterion",
            PADDR(door_opening_criterion), PT_double, "compressor_defrost_time",
            PADDR(compressor_defrost_time), PT_double, "delay_defrost_time",
            PADDR(delay_defrost_time), PT_int32, "daily_door_opening",
            PADDR(daily_door_opening), PT_enumeration, "state", PADDR(state),
            PT_KEYWORD, "DEFROST", (enumeration)RS_DEFROST, PT_KEYWORD,
            "COMPRESSSOR_OFF_NORMAL", (enumeration)RS_COMPRESSSOR_OFF_NORMAL,
            PT_KEYWORD, "COMPRESSSOR_ON_LONG",
            (enumeration)RS_COMPRESSSOR_ON_LONG, PT_KEYWORD,
            "COMPRESSSOR_ON_NORMAL", (enumeration)RS_COMPRESSSOR_ON_NORMAL,
            nullptr) < 1)
...
int refrigerator::create() {
  ...
  /* TROUBLESHOOT
          The refrigerator explicit model has some serious issues and should be
...
int refrigerator::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("refrigerator::init(): deferring initialization on %s",
  ...
  if (size == 0)
    size = gl_random_uniform(&hdr->rng_state, 20, 40); // cf
  if (thermostat_deadband == 0)
    thermostat_deadband = gl_random_uniform(&hdr->rng_state, 2, 3);
  if (Tset == 0)
    Tset = gl_random_uniform(&hdr->rng_state, 35, 39);
  if (UA == 0)
    UA = 0.6;
  if (UAr == 0)
    UAr = UA + size / 40 * gl_random_uniform(&hdr->rng_state, 0.9, 1.1);
  if (UAf == 0)
    UAf = gl_random_uniform(&hdr->rng_state, 0.9, 1.1);
  if (COPcoef == 0)
    COPcoef = gl_random_uniform(&hdr->rng_state, 0.9, 1.1);
  if (Tout == 0)
    Tout = 59.0;
  ...
  if (pTout == nullptr) {
    ...
    gl_warning("%s (%s:%d) parent object lacks air temperature, using %0f degF "
               "instead",
               hdr->name, hdr->oclass->name, hdr->id, default_air_temperature);
    pTout = &default_air_temperature;
  }

  /* derived values */
  Tair = gl_random_uniform(&hdr->rng_state, Tset - thermostat_deadband / 2,
                           Tset + thermostat_deadband / 2);

  // size is used to couple Cw and Qrated
  Cf = size / 10.0 * RHOWATER * CWATER; // cf * lb/cf * BTU/lb/degF = BTU / degF

  rated_capacity = BTUPHPW * size * 10; // BTU/h ... 10 BTU.h / cf (34W/cf, so
                                        // ~700 for a full-sized refrigerator)

  start_time = 0;

  if (compressor_off_normal_energy == 0)
    compressor_off_normal_energy = 15 * 45 * 60; // watt-secs
  if (compressor_off_normal_power == 0)
    compressor_off_normal_power = 15; // watt

  if (long_compressor_cycle_energy == 0)
    long_compressor_cycle_energy = 120 * 100 * 60; // watt-secs
  if (long_compressor_cycle_power == 0)
    long_compressor_cycle_power = 120; // watt

  if (compressor_on_normal_energy == 0)
    compressor_on_normal_energy = 120 * 35 * 60; // watt-secs
  if (compressor_on_normal_power == 0)
    compressor_on_normal_power = 120; // watt

  if (defrost_energy == 0)
    defrost_energy = 40 * 550 * 60; // watt-secs
  if (defrost_power == 0)
    defrost_power = 550; // watt

  if (icemaking_energy == 0)
    icemaking_energy = 300 * 60; // watt-secs
  if (icemaking_power == 0)
    icemaking_power = 300; // watt

  if (ice_making_probability == 0)
    ice_making_probability = 0.02; // watt

  if (DO_Thershold == 0)
    DO_Thershold = 24;
  if (long_compressor_cycle_threshold == 0)
    long_compressor_cycle_threshold = 0.05;

  if (FF_Door_Openings == 0)
    FF_Door_Openings = 0;

  if (door_opening_power == 0)
    door_opening_power = 16;

  if (delay_defrost_time == 0)
    delay_defrost_time = 28800;

  if (defrost_criterion == 0)
    defrost_criterion = DC_TIMED;
  ...
  case RS_COMPRESSSOR_OFF_NORMAL:
    ...
    cycle_time =
        ceil((energy_needed - energy_used) / compressor_off_normal_power);
    break;
  case RS_COMPRESSSOR_ON_NORMAL:
    if (energy_needed == 0)
      energy_needed = compressor_on_normal_energy;
    cycle_time =
        ceil((energy_needed - energy_used) / compressor_on_normal_power);
    break;
  }
...
int refrigerator::isa(char *classname) {
  return (strcmp(classname, "refrigerator") == 0 ||
...
/* exclusively modifies Tevent and motor_state, nothing the reflects current
 * properties should be affected by the PLC code. */
...
double refrigerator::update_refrigerator_state(double dt0, TIMESTAMP t1) {
  ...
  if (defrostDelayed <= 0) {
    if (DC_TIMED == defrost_criterion) {
      run_defrost = true;
    } else if (DC_COMPRESSOR_TIME == defrost_criterion &&
               total_compressor_time >= compressor_defrost_time) {
  ...
  if (door_next_open_time <= 0 && hourly_door_opening > 0) {
    ...
    FF_Door_Openings++;
    door_return_time +=
        ceil(gl_random_uniform(&hdr->rng_state, 0, door_open_time));
    door_energy_calc = true;
    hourly_door_opening--;

    if (hourly_door_opening > 0) {
      door_next_open_time = int(gl_random_uniform(
          &hdr->rng_state, 0,
          (3600 - (t1 - start_time) % 3600))); // next_door_openings[hourly_door_opening-1]
                                               // - ((t1-start_time)%3600);
      door_to_open = true;
    } else {
      door_to_open = false;
  ...
  if (((t1 - start_time) % 3600) == 0 && start_time > 0) {
    double temp_hourly_door_opening =
        (door_opening_criterion * daily_door_opening) -
        floor(door_opening_criterion * daily_door_opening);
    ...
    hourly_door_opening = floor(gl_random_normal(
        &hdr->rng_state, hourly_door_opening, ((hourly_door_opening) / 3)));
    ...
    if (hourly_door_opening >
        long_compressor_cycle_threshold * daily_door_opening) {
      long_compressor_cycle_due = true;
      long_compressor_cycle_time =
          int(gl_random_uniform(&hdr->rng_state, 0, (3600)));
    }
  ...
  case RS_COMPRESSSOR_OFF_NORMAL:
    ...
    if (run_defrost == true) {
      ...
      if (long_compressor_cycle_due == true &&
  ...
  if (true == door_open && true == door_to_open) {
    door_time = door_return_time < door_next_open_time ? door_return_time
                                                       : door_next_open_time;
    ...
    if (cycle_time > return_time)
      if (return_time > door_time)
        if (door_time > long_compressor_cycle_time)
          ...
          dt1 = door_time;
      else if (return_time > long_compressor_cycle_time)
        dt1 = long_compressor_cycle_time;
      else
        dt1 = return_time;
    else if (cycle_time > door_time)
      if (door_time > long_compressor_cycle_time)
        dt1 = long_compressor_cycle_time;
      else
        dt1 = door_time;
    else if (cycle_time > long_compressor_cycle_time)
      dt1 = long_compressor_cycle_time;
    else
      dt1 = cycle_time;
  }
