 ...
 * contains all utility functions and common definitions for the residential
 module
 */
...
typedef struct s_circuit {
  ...
  enduse *pLoad;    ///< pointer to the load struct (ENDUSELOAD* in house_a,
                    ///< enduse* in house_e)
...
typedef CIRCUIT *(*ATTACHFUNCTION)(
    OBJECT *, enduse *, double,
    int is220); ///< type definition for attach function
...
// Globals
GLOBAL double
    default_line_voltage INIT(120.0); // Value for the default nominal_voltage
GLOBAL bool ANSI_voltage_check
    INIT(true); // Flag to enable/disable ANSI voltage violation checks
GLOBAL double default_outdoor_temperature
    INIT(74.0); // Value for default outdoor air temperature
GLOBAL double default_humidity INIT(75.0); // Value for default humidity
GLOBAL
    int64 default_etp_iterations INIT(100); // Value for etp solution iterations
GLOBAL double
    default_horizontal_solar INIT(0.0); // Value for horizontal solar gains
...
GLOBAL bool all_residential_delta
    INIT(false); /* Cheater flag -- may not make it into the merge -- basically
                    allows all residential objects to use deltamode */
...
GLOBAL double deltatimestep_running
    INIT(-1.0); /* Flagging variable - starts indicating QSTS */
...
void allocate_deltamode_arrays(
