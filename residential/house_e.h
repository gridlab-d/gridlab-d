...
class house_e
    : public residential_enduse { /*inherits due to HVAC being a load */
public:
  ...
  static gld::set
      implicit_enduses_active; ///< implicit enduses that are to be activated
  static enumeration
      implicit_enduse_source; ///< source of implicit enduses (e.g., ELCAP1990,
                              ///< ELCAP2010, RBSA2014)
public:
  ...
  double
      interior_exterior_wall_ratio; ///< ratio of internal to external wall area
  double exterior_wall_fraction;    ///< exterior-wall ratio
  double
      exterior_ceiling_fraction; ///< ratio of external ceiling sf to floor area
  double exterior_floor_fraction;
  double aspect_ratio;                      ///< building footprint aspect ratio
  double solar_aperture[N_SOLAR_SURFACES];  ///< Future: Solar aperture(WWR) by
                                            ///< orientation
  ...
  double total_thermal_mass_per_floor_area; ///< Total thermal mass per unit of
                                            ///< floor area (Rob's rule of thumb
                                            ///< is 2 for wood frame)
  double interior_surface_heat_transfer_coeff; ///< Rob's rule of thumb is 1
  double air_heat_fraction; ///< fraction of gains that go to air - this
                            ///< variable is not needed consider deleting?
  double mass_solar_gain_fraction; ///< fraction of solar gains that goes to the
                                   ///< mass
  double mass_internal_gain_fraction; ///< fraction of the internal gains that
                                      ///< goes to the mass
  ...
  double solar_heatgain_factor; ///< product of the window solar heatgain
                                ///< coefficient and the exterior transmission
                                ///< coefficient
  ...
  double window_a; // quadratic coefficient for function between low and high
                   // cutoffs
  double
      window_b; // linear coefficient for function between low and high cutoffs
  double window_c; // constant coefficient for function between low and high
                   // cutoffs
  double window_temp_delta; // difference in outdoor temperature that triggers a
                            // re-calculation of the model
  double last_temperature;  // last temperature of a state change
  int window_first_time_through; // determine if this is the first time through
                                 // the model or not
  ...
  int16
      thermostat_off_cycle_time;  ///< the minimum amount of time the thermostat
                                  ///< cycle must stay in the off state
  int16 thermostat_on_cycle_time; ///< the minimum amount of time the thermostat
                                  ///< cycle must stay in the on state
  ...
  double rated_heating_capacity; ///< rated heating capacity of the system
                                 ///< (BTUh/sf; varies w.r.t Tout),
  double rated_cooling_capacity; ///< rated cooling capacity of the system
                                 ///< (BTUh/sf; varies w.r.t Tout)
  ...
  double aux_heat_capacity; ///< auxiliary (resistive) heat is COP 1 and has no
                            ///< adjustment
  double
      aux_heat_deadband; ///< additional deadband before auxiliary heat engages
  double aux_heat_temp_lockout; ///< outdoor temperature at which the auxiliary
                                ///< heat will automatically engage
  double aux_heat_time_delay; ///< minimum time the heat pump must run until the
                              ///< auxiliary heating engages
  // fan characteristics
  double
      fan_design_power; ///< designed maximum power draw of the ventilation fan
  double fan_low_power_fraction; ///< fraction of ventilation fan power draw
                                 ///< during low-power mode (two-speed only)
  double fan_power;          ///< current power being fed to the ventilation fan
  double fan_design_airflow; ///< designed airflow for the ventilation system
                             ///< (cf/min)
  ...
  ///
  double duct_pressure_drop; ///< pressure drop across the ventilation ducting,
  ...
  double latent_load_fraction; ///< fractional increase in cooling load due to
                               ///< latent heat
  ...
  gld::complex hvac_power; ///< actual power draw of the hvac system (includes
                           ///< fan and motor where applicable)
  ...
  double last_heating_load; ///< stores the previous heater load for use in the
                            ///< controller
  double last_cooling_load; ///< stores the previous A/C load for use in the
                            ///< controller
  bool compressor_on;
  ...
  bool thermal_storage_present; // Indication of if thermal storage is present
                                // and available for drawing
  bool thermal_storage_inuse;   // Flag to indicate thermal storage is being
  ...
  typedef enum {         ///< Thermal integrity level is an "easy" to use
    ...
    TI_ABOVE_NORMAL =
        4,       ///< Advanced user's who wish to specify the characteristics
  ...
  typedef enum {
    ...
    MM_BASIC =
        1,      ///< in calculating the efficiency of the hvac motor.  NONE will
    MM_FULL = 2 ///< indicate that the electrical motor is 100% electrically
                ///< efficient.
  } MOTORMODEL; ///< BASIC incorporates shunt losses, while FULL also includes
                ///< series losses.
  enumeration motor_model;

  // Motor variables and enumerations
  double hvac_motor_efficiency; ///< Actual percent electrical efficiency of
                                ///< hvac motor
  double hvac_motor_loss_power_factor; ///< Between this and efficiency, derives
                                       ///< the real and reactive losses
  double hvac_motor_real_loss;     ///< Actual real power loss of motor at rated
                                   ///< voltage
  double hvac_motor_reactive_loss; ///< Actual reactive power loss of motor at
                                   ///< rated voltage
  typedef enum {
    ME_VERY_POOR =
        0, ///< Gives an "easy" to use parameter for specifying the efficiency
  ...
  // gross_wall_area*window_wall_ratio*glazing_shgc*window_exterior_transmission_coefficient
  // should be the same as the equivalent solar aperature located in Rob Pratt's
  // ETP Excel spreadsheet.
  ...
  double incident_solar_radiation; ///< This variable hold the average incident
                                   ///< solar radiation hitting the house in
                                   ///< Btu/(hr*sf)
  ...
  double is_AUX_on, is_HEAT_on,
      is_COOL_on; // A logic statement so that the population dynamics of the AC
                  // can be observed with collectors.
...
private:
  ...
  double dTair;
  double a, b, c, d, c1, c2, A3, A4, k1, k2, r1, r2, Teq, Tevent, Qi, Qa, Qm,
      adj_cooling_cap, adj_heating_cap, adj_cooling_cop, adj_heating_cop;
  double Qlatent;
  static bool warn_control;
  static double warn_low_temp;
  static double warn_high_temp;
  static double system_dwell_time; // time interval at which hvac checks its
                                   // state (approximates true dwell time)
  bool check_start;
  bool heat_start;

  bool deltamode_inclusive;  // Boolean for deltamode calls - pulled from object
                             // flags
  bool deltamode_registered; // Boolean for deltamode registration -- basically
                             // a "first run" flag
  bool
      proper_meter_parent; // Flag to see if powerflow interactions should occur
  bool proper_climate_found; // Flag to see if climate interactions should occur
  bool commercial_load_parent; // proper_meter_parent is true, but the parent is
                               // actually a load

  // Variables for fault_impedance method
  bool powerflow_impedance_conversion_enabled; // Boolean for powerflow - see if
                                               // "convert all to impedance" is
                                               // active
  double
      powerflow_impedance_conversion_level; // Threshold from powerflow where
                                            // impedance conversion should occur
  ...
  gld_property
      *pMeterStatus; ///< Pointer to service_status variable on triplex parent
  ...
  gld::complex value_Circuit_V[3]; ///< value holder for the three voltages on
                                   ///< three lines
  gld::complex
      value_Line_I[3]; ///< value holder for the three current on three lines
  gld::complex
      value_Shunt[3]; ///< value holder for shunt value on triplex parent
  gld::complex
      value_Power[3]; ///< value holder for power value on triplex parent
  enumeration value_MeterStatus; ///< value holder for service_status variable
                                 ///< on triplex parent
  double value_Frequency; ///< value holder for measured frequency on triplex
                          ///< parent
  typedef enum {
    XPFV_NONE = 0, // no external power flow
    XPFV_ONEV = 1, // set just external_v1N, assume v2N equal and opposite
    XPFV_TWOV =
        2, // will set both external_v1N and v2N, require v12 = v1N - v2N
  } EXTERNALPFMODE;
  enumeration external_pf_mode;
  gld::complex
      external_v1N; // from OpenDSS or another external power flow program
  gld::complex
      external_v2N; // from OpenDSS or another external power flow program
  ...
  bool external_motor_attached; //< Flag for external powerflow motor being used
                                //- removes that panel contributions

  void circuit_voltage_factor_update(
      void); ///< Functionalized version of the voltage_factor update, so can be
             ///< called in deltamode
  void powerflow_accumulator_remover(
      void); ///< Functioanlized item that removes current accumulator values
             ///< from powerflow (mostly for XML stuff)
