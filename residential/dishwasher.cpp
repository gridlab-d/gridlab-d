...
dishwasher::dishwasher(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "dishwasher", sizeof(dishwasher),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "residential_enduse",

            PT_double, "control_power[W]", PADDR(coil_power[0]), PT_double,
            "dishwasher_coil_power_1[W]", PADDR(coil_power[1]), PT_double,
            "dishwasher_coil_power_2[W]", PADDR(coil_power[2]), PT_double,
            "dishwasher_coil_power_3[W]", PADDR(coil_power[3]), PT_double,
            "motor_power[W]", PADDR(motor_power),

            PT_double, "circuit_split", PADDR(circuit_split), PT_double,
            "queue[unit]", PADDR(enduse_queue), PT_DESCRIPTION,
            "number of loads accumulated", PT_double, "stall_voltage[V]",
            PADDR(stall_voltage), PT_double, "start_voltage[V]",
            PADDR(start_voltage), PT_complex, "stall_impedance[Ohm]",
            PADDR(stall_impedance), PT_double, "trip_delay[s]",
            PADDR(trip_delay), PT_double, "reset_delay[s]", PADDR(reset_delay),
            PT_double, "total_power[W]", PADDR(total_power),

            PT_enumeration, "state", PADDR(state), PT_KEYWORD, "STOPPED",
            (enumeration)dishwasher_STOPPED, PT_KEYWORD, "STALLED",
            (enumeration)dishwasher_STALLED, PT_KEYWORD, "TRIPPED",
            (enumeration)dishwasher_TRIPPED, PT_KEYWORD, "MOTOR_ONLY",
            (enumeration)dishwasher_MOTOR_ONLY, PT_KEYWORD, "MOTOR_COIL_ONLY",
            (enumeration)dishwasher_MOTOR_COIL_ONLY, PT_KEYWORD, "COIL_ONLY",
            (enumeration)dishwasher_COIL_ONLY, PT_KEYWORD, "CONTROL_ONLY",
            (enumeration)dishwasher_CONTROL_ONLY, PT_KEYWORD, "HEATEDDRY_ONLY",
            (enumeration)dishwasher_HEATEDDRY_ONLY,

            PT_double, "energy_baseline[kWh]", PADDR(energy_baseline),
            PT_double, "energy_used[kWh]", PADDR(energy_used),

            PT_bool, "control_check1", PADDR(control_check1), PT_bool,
            "control_check2", PADDR(control_check2), PT_bool, "control_check3",
            PADDR(control_check3), PT_bool, "control_check4",
            PADDR(control_check4), PT_bool, "control_check5",
            PADDR(control_check5), PT_bool, "control_check6",
            PADDR(control_check6), PT_bool, "control_check7",
            PADDR(control_check7), PT_bool, "control_check8",
            PADDR(control_check8), PT_bool, "control_check9",
            PADDR(control_check9), PT_bool, "control_check10",
            PADDR(control_check10), PT_bool, "control_check11",
            PADDR(control_check11), PT_bool, "control_check12",
            PADDR(control_check12), PT_bool, "control_check_temp",
            PADDR(control_check_temp),

            PT_bool, "motor_only_check1", PADDR(motor_only_check1), PT_bool,
            "motor_only_check2", PADDR(motor_only_check2), PT_bool,
            "motor_only_check3", PADDR(motor_only_check3), PT_bool,
            "motor_only_check4", PADDR(motor_only_check4), PT_bool,
            "motor_only_check5", PADDR(motor_only_check5), PT_bool,
            "motor_only_check6", PADDR(motor_only_check6), PT_bool,
            "motor_only_check7", PADDR(motor_only_check7), PT_bool,
            "motor_only_check8", PADDR(motor_only_check8), PT_bool,
            "motor_only_check9", PADDR(motor_only_check9),

            PT_bool, "motor_only_temp1", PADDR(motor_only_temp1), PT_bool,
            "motor_only_temp2", PADDR(motor_only_temp2),

            PT_bool, "motor_coil_only_check1", PADDR(motor_coil_only_check1),
            PT_bool, "motor_coil_only_check2", PADDR(motor_coil_only_check2),

            PT_bool, "heateddry_check1", PADDR(heateddry_check1), PT_bool,
            "heateddry_check2", PADDR(heateddry_check2),

            PT_bool, "coil_only_check1", PADDR(coil_only_check1), PT_bool,
            "coil_only_check2", PADDR(coil_only_check2), PT_bool,
            "coil_only_check3", PADDR(coil_only_check3),
            ...
            PT_double, "energy_needed[kWh]", PADDR(energy_needed), PT_double,
            "dishwasher_demand[kWh]", PADDR(dishwasher_demand), PT_double,
            "daily_dishwasher_demand[kWh]", PADDR(daily_dishwasher_demand),
            PT_double, "actual_dishwasher_demand[kWh]",
            PADDR(actual_dishwasher_demand),

            PT_double, "motor_on_off", PADDR(motor_on_off), PT_double,
            "motor_coil_on_off", PADDR(motor_coil_on_off),

            PT_bool, "is_240", PADDR(is_240), PT_DESCRIPTION,
            "load is 220/240 V (across both phases)", nullptr) < 1)
...
int dishwasher::create() {
  ...
  gl_warning("explicit %s model is experimental and has not been validated",
             object_header(this)->oclass->name);
  /* TROUBLESHOOT
          The dishwasher explicit model has some serious issues and should be
     considered for complete removal.  It is highly suggested that this model
     NOT be used.
  */
...
int dishwasher::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("dishwasher::init(): deferring initialization on %s",
  ...
  // default properties
  if (motor_power == 0)
    motor_power = gl_random_uniform(&hdr->rng_state, 150, 350);
  if (heat_fraction == 0)
    heat_fraction = 0.2;
  if (is_240) {
    load.config = EUC_IS220;
    if (stall_voltage == 0)
      stall_voltage = 1.2 * default_line_voltage; // 0.6 * 2 * line_voltage
  } else if (stall_voltage == 0)
    stall_voltage = 0.6 * default_line_voltage;
  ...
  case MT_ANALOG:
    if (shape.params.analog.energy == 0.0) {
      ...
      daily_dishwasher_demand =
          gl_get_loadshape_value(&shape); /* unitless ~ drive gpm */
    }
    break;
  case MT_PULSED:
    ...
    if (shape.params.pulsed.pulsetype == MPT_TIME) {
      ...
      ; /* constant power pulse ~ dishwasher demand X kW, limited by C + Q * h ~
           Vdot proportional to power/time */
      daily_dishwasher_demand = gl_get_loadshape_value(&shape) / 2.4449;
    }
    break;
  case MT_MODULATED:
    if (shape.params.modulated.pulsetype == MPT_TIME) {
      GL_THROW("Amplitude modulated dishwasher usage is nonsensical for "
               "residential dishwashers");
      /*	TROUBLESHOOT
              Though it is possible to put a constant, low-level dishwasher
         demand, it is thoroughly counterintuitive to the normal usage of the
         dishwasher.
       */
...
int dishwasher::isa(char *classname) {
  return (strcmp(classname, "dishwasher") == 0 ||
...
double dishwasher::update_state(double dt) //,TIMESTAMP t1)
{
  ...
  case dishwasher_STOPPED:
    ...
    if (enduse_queue > 1 && (dishwasher_run_prob > enduse_queue)) {
      ...
      cycle_duration = cycle_time =
          1000 * (energy_needed - energy_used) / coil_power[0] * 60 * 60;
  ...
  case dishwasher_CONTROL_ONLY:
    ...
    if (energy_used >= energy_needed || cycle_time <= 0) {
      if (energy_used >= energy_needed) {
        ...
        if (cycle_time <= 0 &&
            (control_check1 == false || control_check_temp == true)) {
          state = dishwasher_MOTOR_ONLY;
          double cycle_t =
              1000 * (energy_needed - energy_used) / motor_power * 60 * 60;
        ...
        if (cycle_time <= 0 && control_check6 == true) {
          ...
          double cycle_t =
              1000 * (energy_needed - energy_used) / coil_power[1] * 60 * 60;
        ...
        if (cycle_time <= 0 && (control_check7 == true)) {
          ...
          double cycle_t = 1000 * (energy_needed - energy_used) /
                           (motor_power + coil_power[3]) * 60 * 60;
        ...
        if (cycle_time <= 0 && control_check8 == true &&
            count_control_only < 5) {
          state = dishwasher_COIL_ONLY;
        ...
        if (cycle_time <= 0 && control_check9 == true &&
            count_control_only1 < 7) {
          state = dishwasher_MOTOR_ONLY;
        ...
        if (cycle_time <= 0 && control_check11 == true &&
            Heateddry_option_check == true) {
          state = dishwasher_HEATEDDRY_ONLY;

          double cycle_t =
              1000 * (energy_needed - energy_used) / (coil_power[2]) * 60 * 60;
        ...
        if (cycle_time <= 0 && control_check12 == true &&
            Heateddry_option_check == true) {
          state = dishwasher_HEATEDDRY_ONLY;
  ...
  case dishwasher_MOTOR_ONLY:
    ...
    if (energy_used >= energy_needed || cycle_time <= 0) {
      if (energy_used >= energy_needed) {
        ...
        if (cycle_time <= 0 && motor_only_check3 == true &&
            count_motor_only_68 < 3)
        ...
        if (cycle_time <= 0 && motor_only_check5 == true &&
            count_motor_only_25 < 6) {
        ...
        if (cycle_time <= 0 && motor_only_check7 == true &&
            count_motor_only < 6) {
  ...
  case dishwasher_COIL_ONLY:
    ...
    if (energy_used >= energy_needed || cycle_time <= 0) {
      if (energy_used >= energy_needed) {
        ...
        if (cycle_time <= 0 && coil_only_check2 == true &&
            count_coil_only < 4) {
          double interval = pulse_interval[4];
  ...
  case dishwasher_STOPPED:
    ...
    dt = ((enduse_queue >= 1) || (enduse_queue == 0))
             ? 0
             : ((1 - enduse_queue) * 3600) / (enduse_queue * 24);
  ...
  case dishwasher_MOTOR_COIL_ONLY:
    ...
    load.constant_admittance =
        gld::complex((coil_power[3]) / 1000, 0, J); // assume pure resistance
  ...
  case dishwasher_COIL_ONLY:
    ...
    load.constant_admittance =
        gld::complex((coil_power[1]) / 1000, 0, J); // assume pure resistance
  ...
  case dishwasher_HEATEDDRY_ONLY:
    ...
    load.constant_power = load.constant_current = gld::complex(0, 0, J);
    load.constant_admittance =
        gld::complex((coil_power[2]) / 1000, 0, J); // assume pure resistance
  ...
  default:
    ...
    /*	TROUBLESHOOT
       ...
       dishwasher object & class sections from the relevant GLM file, and from
       the dump file.
    */
    break;
  }

  // compute the total electrical load - first for the enduse structure and
  // second for an internal variable
  load.total =
      load.constant_power + load.constant_current + load.constant_admittance;
  total_power = (load.constant_power.Re() +
...
static TIMESTAMP sync_dishwasher_impl(OBJECT *obj, TIMESTAMP t0,
                                      PASSCONFIG pass) {
  TIMESTAMP tret;
  ...
  try {
    ...
    gl_error("%s (dishwasher:%d) model zone exception (code %d) not caught",
             obj->name ? obj->name : "(anonymous dishwasher)", obj->id, m);
  } catch (char *msg) {
    gl_error("%s (dishwasher:%d) %s",
             obj->name ? obj->name : "(anonymous dishwasher)", obj->id, msg);
  }
  return TS_INVALID;
}

#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_dishwasher(OBJECT *obj, TIMESTAMP t0,
                                                PASSCONFIG pass) {
