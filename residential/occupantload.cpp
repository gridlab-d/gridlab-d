/** $Id: occupantload.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        DOE-2 assumptions are used for calculating the internal gain from
 occupant load.
...
occupantload::occupantload(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "occupantload", sizeof(occupantload),
    ...
    if (gl_publish_variable(oclass, PT_INHERIT, "residential_enduse", PT_int32,
                            "number_of_occupants", PADDR(number_of_occupants),
                            PT_double, "occupancy_fraction[unit]",
                            PADDR(occupancy_fraction), PT_double,
                            "heatgain_per_person[Btu/h]",
                            PADDR(heatgain_per_person), nullptr) < 1)
...
int occupantload::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("occupantload::init(): deferring initialization on %s",
  ...
  if (number_of_occupants == 0)
    number_of_occupants =
        4; // defaulted to 4, but perhaps define it based on house size??
  if (heatgain_per_person == 0)
    heatgain_per_person =
        400.0; // Based on DOE-2, includes latent and sensible heatgain

  OBJECT *hdr = object_header(this);
  hdr->flags |= OF_SKIPSAFE;

  if (parent == nullptr ||
      (!gl_object_isa(parent, "house") && !gl_object_isa(parent, "house_e"))) {
    gl_error("occupantload must have a parent house");
    /*	TROUBLESHOOT
            The occupantload object, being an enduse for the house model, must
       have a parent house that it is connected to.  Create a house object and
       set it as the parent of the offending occupantload object.
    */
    return 0;
  }
  ...
  if (attach == nullptr) {
    ...
    /*	TROUBLESHOOT
            The occupantload object attempt to attach itself to its parent,
       which must implement the attach_enduse function.
    */
    return 0;
  }
  // Needed to pass heat gain up to the house
  // "true" on 220 keeps the circuits "balanced"
  ((CIRCUIT * (*)(OBJECT *, enduse *, double, int))(*attach))(
      hdr->parent, &(this->load), 20, true);

  load.heatgain =
      number_of_occupants * occupancy_fraction * heatgain_per_person;
  ...
  if (shape.type != MT_UNKNOWN && shape.type != MT_ANALOG) {
    ...
    gl_warning("occupancy_load \'%s\' may not work properly with a non-analog "
               "load shape.",
               hdr->name ? hdr->name : outname);
  }
  return 1;
}

int occupantload::isa(char *classname) {
  return (strcmp(classname, "occupantload") == 0 ||
...
TIMESTAMP occupantload::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (heatgain_per_person > 1600) {
    ...
    gl_error(
        "heatgain per person above 1600 Btu/hr (470W), reseting to 400 Btu/hr");
    heatgain_per_person = 400.0;
  }
  ...
  if (shape.type == MT_UNKNOWN) {
    ...
    if (occupancy_fraction * number_of_occupants > 300.0) {
      gl_error(
          "attempting to fit 300 warm bodies into a house, reseting to zero");
