/** $Id: freezer.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The freezer model is based on performance profile that can specified by
 a tape and using a method developed by Ross T. Guttromson
...
freezer::freezer(MODULE *module) : residential_enduse(module) {
  // first time init
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "freezer", sizeof(freezer),
    ...
    if (gl_publish_variable(
            ...
            PADDR(size), PT_double, "rated_capacity[Btu/h]",
            PADDR(rated_capacity), PT_double, "temperature[degF]", PADDR(Tair),
            PT_double, "setpoint[degF]", PADDR(Tset), PT_double,
            "deadband[degF]", PADDR(thermostat_deadband), PT_timestamp,
            "next_time", PADDR(last_time), PT_double, "output", PADDR(Qr),
            PT_double, "event_temp", PADDR(Tevent), PT_double, "UA[Btu/degF*h]",
            PADDR(UA), PT_enumeration, "state", PADDR(motor_state), PT_KEYWORD,
            "OFF", (enumeration)S_OFF, PT_KEYWORD, "ON", (enumeration)S_ON,
            nullptr) < 1)
...
int freezer::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("freezer::init(): deferring initialization on %s",
  ...
  if (size == 0)
    size = gl_random_uniform(RNGSTATE, 20, 40); // cf
  if (thermostat_deadband == 0)
    thermostat_deadband = gl_random_uniform(RNGSTATE, 2, 3);
  if (Tset == 0)
    Tset = gl_random_uniform(RNGSTATE, 10, 20);
  if (UA == 0)
    UA = 0.3;
  if (UAr == 0)
    UAr = UA + size / 40 * gl_random_uniform(RNGSTATE, 0.9, 1.1);
  if (UAf == 0)
    UAf = gl_random_uniform(RNGSTATE, 0.9, 1.1);
  if (COPcoef == 0)
    COPcoef = gl_random_uniform(RNGSTATE, 0.9, 1.1);
  if (Tout == 0)
    Tout = 59.0;
  if (power_factor == 0)
    power_factor = 0.95;
  ...
  /* derived values */
  Tair = gl_random_uniform(RNGSTATE, Tset - thermostat_deadband / 2,
                           Tset + thermostat_deadband / 2);

  // size is used to couple Cw and Qrated
  // Cf = 8.43 * size/10; // BTU equivalent gallons of water for only 10% of the
  // size of the refigerator
  Cf = size / 10.0 * RHOWATER * CWATER; // cf * lb/cf * BTU/lb/degF = BTU / degF

  rated_capacity =
      BTUPHPW * size *
      10; // BTU/h ... 10 BTU.h / cf (34W/cf, so ~700 for a full-sized freezer)
...
int freezer::isa(char *classname) {
  return (strcmp(classname, "freezer") == 0 ||
...
TIMESTAMP freezer::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (pTout == nullptr) {
    GL_THROW("Parent house of freezer lacks property \'air_temperature\' at "
             "sync time?");
  }
  Tout = *pTout;
  ...
  if (nHours > 0 && t0 > 0) { /* skip this on TS_INIT */
    const double COP =
        COPcoef * ((-3.5 / 45) * (Tout - 70) + 4.5); /* come from ??? */

    if (t1 == next_time) {
      /* lazy skip-ahead */
      load.heatgain = -((Tair - Tout) * exp(-(UAr + UAf) / Cf) + Tout - Tair) *
                          Cf * nHours +
                      Qr * nHours * COP;
      Tair = Tevent;
    } else {
...
TIMESTAMP freezer::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (temp_voltage_magnitude <
      (default_line_voltage * 0.6)) { /* stall voltage */
  ...
  // if fridge is undersized or time exceeds balance of time or external event
  // pending
  next_time = (TIMESTAMP)(t1 + (t > 0 ? t : -t) * (3600.0 / TS_SECOND) + 1);
  return next_time > TS_NEVER ? TS_NEVER : -next_time;
}

TIMESTAMP freezer::postsync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_freezer(OBJECT *obj, TIMESTAMP t0,
                                             PASSCONFIG pass) {
