...
public:
  ...
  double zero_power;
  double motor_power;   ///< installed dishwasher motor power [W] (default =
                        ///< random uniform between 150-350 W)
  double coil_power[4]; ///< installed heating coil power [W] (default = random
                        ///< uniform between 3500-5000 W, 0 for gas)
  ...
  //****Changes by Niru
  TIMESTAMP next_change_time; ///< time when the current change changes to the
                              ///< next state (s)
  //***changes stop here
  ...
  // double load_relative_humidity;		///< relative amount of water in
  // load - will be used to determine cycle length double slope_of_rh_vs_time;
  // ///< slope of how fast water is removed from load as function of time
  double energy_baseline; ///< amount of energy needed to dry clothes in
                          ///< baseline mode
  double energy_used; ///< amount of energy used in current cycle to dry clothes
  double actual_dishwasher_demand;
  double pulse_interval[19]; ///< length of time of each pulse section for a
                             ///< dishwasher operation

  // double time_baseline;				///< length of time
  // needed to run baseline at nominal voltage double time_eco;
  // ///< length of time needed to run eco at nominal voltage
  double energy_needed; ///< total energy needed to dry clothes (used for
                        ///< switching modes)
...
public:
  ...
  double update_state(
      double dt); //, TIMESTAMP t1=0);		///< updates the load struct and
                  //returns the time until expected state change
};
