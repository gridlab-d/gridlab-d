...
public:
  ...
  double recharge_power;  ///< Scaled power usage based on outdoor air temp to
                          ///< recharge [kW]
  double recharge_power_factor;  ///< Power factor associated with recharging
                                 ///< operation
  double discharge_power;        ///< Scaled power usage to cool the air [kW]
  double discharge_power_factor; ///< Power factor associated with discharging
                                 ///< operation
  double discharge_rate; ///< Based on design cooling from house [Btu/hr]
  bool recharge;         ///< set 1 if recharging, 0 not
  double recharge_time;  ///< recharge cycle set on by schedule, can not overlap
                         ///< with discharge schedule
  double discharge_time; ///< discharge cycle set on by schedule, can not
                         ///< overlap with recharge schedule
  double state_of_charge; ///< Thermal storage state of charge variable
  double k;               ///< Coefficient of thermal conductivity [W/m/K]
  double water_capacity; ///< Amount of water contained in the system for use as
                         ///< ice storage [m^3]
  double
      surface_area; ///< Surface area of the water capacity stored in cube [m^2]
  TIMESTAMP
      last_timestep; ///< Last time the simulation stopped to calculate [sec]
  TIMESTAMP
      next_timestep; ///< Next time thermal_storage will need to run [delta sec]
  ...
  enumeration
      recharge_schedule_type; ///< Determines if charging should occur via
                              ///< internal schedule, or an external property
  enumeration
      discharge_schedule_type; ///< Determines if discharging should occur via
                               ///< internal schedule, or an external property
