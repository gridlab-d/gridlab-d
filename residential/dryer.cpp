/** $Id: dryer.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The dryer simulation is based on an hourly demand profile attached to
 the object. The internal heat gain is calculated using a specified fraction of
 installed power. The queue is used to determine the probability of a load being
 run during that hour. Demand is added to the queue until the queue becomes
 large enough to trigger an event. The motor model is simplified in that it
 assume the motor all the time during the load run (which is not really true).
 The duration of the load is a configurable parameter.
...
dryer::dryer(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "dryer", sizeof(dryer),
    ...
    if (gl_publish_variable(
            ...
            "motor_power[W]", PADDR(motor_power), PT_double,
            "dryer_coil_power[W]", PADDR(coil_power[0]), PT_double,
            "controls_power[W]", PADDR(controls_power), PT_double,
            "circuit_split", PADDR(circuit_split), PT_double, "queue[unit]",
            PADDR(enduse_queue), PT_DESCRIPTION, "number of loads accumulated",
            PT_double, "queue_min[unit]", PADDR(queue_min), PT_double,
            "queue_max[unit]", PADDR(queue_max), PT_double, "stall_voltage[V]",
            ...
            (enumeration)DRYER_STOPPED, PT_KEYWORD, "STALLED",
            (enumeration)DRYER_STALLED, PT_KEYWORD, "TRIPPED",
            (enumeration)DRYER_TRIPPED, PT_KEYWORD, "MOTOR_ONLY",
            (enumeration)DRYER_MOTOR_ONLY, PT_KEYWORD, "MOTOR_COIL_ONLY",
            (enumeration)DRYER_MOTOR_COIL_ONLY, PT_KEYWORD, "CONTROL_ONLY",
            (enumeration)DRYER_CONTROL_ONLY,

            PT_double, "energy_baseline[kWh]", PADDR(energy_baseline),
            PT_double, "energy_used[kWh]", PADDR(energy_used), PT_double,
            "next_t", PADDR(next_t), PT_bool, "control_check",
            PADDR(control_check),
            ...
            "motor_only_check6", PADDR(motor_only_check6),

            PT_bool, "dryer_on", PADDR(dryer_on), PT_bool, "dryer_ready",
            PADDR(dryer_ready),
            ...
            PT_double, "dryer_run_prob", PADDR(dryer_run_prob), PT_double,
            "dryer_turn_on", PADDR(dryer_turn_on),
            ...
            "daily_dryer_demand[kWh]", PADDR(daily_dryer_demand), PT_double,
            "actual_dryer_demand[kWh]", PADDR(actual_dryer_demand), PT_double,
            "motor_on_off", PADDR(motor_on_off), PT_double, "motor_coil_on_off",
            PADDR(motor_coil_on_off),
...
int dryer::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("dryer::init(): deferring initialization on %s",
  ...
  case MT_PULSED:
    ...
    if (shape.params.pulsed.pulsetype == MPT_TIME) {
      ...
      ; /* constant power pulse ~ dryer demand X kW, limited by C + Q * h ~ Vdot
  ...
  case MT_MODULATED:
    if (shape.params.modulated.pulsetype == MPT_TIME) {
      GL_THROW("Amplitude modulated dryer usage is nonsensical for residential "
               "dryers");
      /*	TROUBLESHOOT
              Though it is possible to put a constant, low-level dryer demand,
         it is thoroughly counterintuitive to the normal usage of the dryer.
       */
...
int dryer::isa(char *classname) {
  return (strcmp(classname, "dryer") == 0 ||
...
double dryer::update_state(double dt) //,TIMESTAMP t1)
{
  ...
  case DRYER_STOPPED:
    ...
    if (enduse_queue > 1 && (dryer_run_prob > enduse_queue)) {
      ...
      cycle_duration = cycle_time =
          1000 * (energy_needed - energy_used) / controls_power * 60 * 60;
  ...
  case DRYER_CONTROL_ONLY:
    ...
    if (energy_used >= energy_needed &&
        cycle_time <= 0) { // The clothes are dry
      state = DRYER_STOPPED;
      cycle_time = 0;
      energy_used = 0;
    ...
    } else if (cycle_time <= 0 && control_check == false) // one-over
    {
      ...
      double cycle_t = 1000 * (energy_needed - energy_used) /
                       (motor_power + coil_power[0]) * 60 * 60;
  ...
  case DRYER_MOTOR_ONLY:
    ...
    if (energy_used >= energy_needed) { // The clothes are dry
      state = DRYER_STOPPED;
      cycle_time = 0;
  ...
  case DRYER_MOTOR_COIL_ONLY:
    ...
    load.constant_admittance =
        gld::complex((coil_power[0]) / 1000, 0, J); // assume pure resistance
  ...
  default:
    ...
    /*	TROUBLESHOOT
       ...
       dryer object & class sections from the relevant GLM file, and from the
       dump file.
    */
    break;
  }
...
static TIMESTAMP sync_dryer_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  try {
    ...
    gl_error("%s (dryer:%d) model zone exception (code %d) not caught",
             obj->name ? obj->name : "(anonymous dryer)", obj->id, m);
  } catch (char *msg) {
    gl_error("%s (dryer:%d) %s", obj->name ? obj->name : "(anonymous dryer)",
             obj->id, msg);
  }
  return TS_INVALID;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_dryer(OBJECT *obj, TIMESTAMP t0,
                                           PASSCONFIG pass) {
