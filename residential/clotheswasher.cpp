...
int clotheswasher::init(OBJECT *parent)
{
    ...
    if (shape.params.analog.power == 0)
        shape.params.analog.power = gl_random_uniform(&hdr->rng_state, 0.100, 0.750); // clotheswasher size [W]
    ...
    if (NORMAL_SPIN_HIGH_ENERGY == 0)
        NORMAL_SPIN_HIGH_ENERGY = 2 * 220 * 60; // Not sure what these values represent, so unclear if the 220 is a voltage or not (for global substitution)
