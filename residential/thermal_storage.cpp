...
struct s_thermal_default_schedule_list {
  ...
  const char *schedule_definition;
    ...
    {"thermal_storage_discharge_default", "dischargetime {"
                                          "* 0-10 * * * 0.0;"
                                          "* 11-20 * * * 1.0;"
                                          "* 21-23 * * * 0.0;"
                                          "}"},
    {"thermal_storage_charge_default", "chargetime {"
                                       "* 0-10 * * * 1.0;"
                                       "* 11-20 * * * 0.0;"
                                       "* 21-23 * * * 1.0;"
                                       "}"}};
...
thermal_storage::thermal_storage(MODULE *mod) : residential_enduse(mod) {
  // first time init
  if (oclass == nullptr) {
    pclass = residential_enduse::oclass;

    // register the class definition
    oclass = gl_register_class(mod, "thermal_storage", sizeof(thermal_storage),
                               PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register class thermal_storage");
    /* TROUBLESHOOT
            The file that implements the thermal_storage in the residential
       module cannot register the class. This is an internal error.  Contact
    ...
    if (gl_publish_variable(
            ...
            "total_capacity[Btu]", PADDR(total_capacity), PT_DESCRIPTION,
            "total storage capacity of unit", PT_double, "stored_capacity[Btu]",
            PADDR(stored_capacity), PT_DESCRIPTION,
            "amount of capacity that is stored", PT_double,
            "recharge_power[kW]", PADDR(recharge_power), PT_DESCRIPTION,
            "installed compressor power usage", PT_double,
            "discharge_power[kW]", PADDR(discharge_power), PT_DESCRIPTION,
            "installed pump power usage", PT_double, "recharge_pf",
            PADDR(recharge_power_factor), PT_DESCRIPTION,
            "installed compressor power factor", PT_double, "discharge_pf",
            PADDR(discharge_power_factor), PT_DESCRIPTION,
            "installed pump power factor", PT_enumeration,
            "discharge_schedule_type", PADDR(discharge_schedule_type),
            PT_DESCRIPTION, "Scheduling method for discharging", PT_KEYWORD,
            "INTERNAL", (enumeration)INTERNAL, PT_KEYWORD, "EXTERNAL",
            (enumeration)EXTERNAL, PT_enumeration, "recharge_schedule_type",
            PADDR(recharge_schedule_type), PT_DESCRIPTION,
            "Scheduling method for charging", PT_KEYWORD, "INTERNAL",
            (enumeration)INTERNAL, PT_KEYWORD, "EXTERNAL",
            (enumeration)EXTERNAL, PT_double, "recharge_time",
            PADDR(recharge_time), PT_DESCRIPTION,
            "Flag indicating if recharging is available at the current time (1 "
            "or 0)", // schedule?
            PT_double, "discharge_time", PADDR(discharge_time), PT_DESCRIPTION,
            "Flag indicating if discharging is available at the current time "
            "(1 or 0)", // schedule?
            PT_double, "discharge_rate[Btu/h]", PADDR(discharge_rate),
            PT_DESCRIPTION, "rating of discharge or cooling", PT_double,
            "SOC[%]", PADDR(state_of_charge), PT_DESCRIPTION,
            "state of charge as percentage of total capacity", // storage/stored
                                                               // capacity
            PT_double, "k[W/m/K]", PADDR(k), PT_DESCRIPTION,
            "coefficient of thermal conductivity (W/m/K)", nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    /* TROUBLESHOOT
            The file that implements the specified class cannot publish the
       variables in the class. This is an internal error.  Contact support for
       assistance.
     */
  }
}
...
int thermal_storage::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("thermal_storage::init(): deferring initialization on %s",
  ...
  design_cooling_capacity_prop =
      new gld_property(parent, "design_cooling_capacity");

  // Make sure it worked
  if ((design_cooling_capacity_prop->is_valid() != true) ||
      (design_cooling_capacity_prop->is_double() != true)) {
    GL_THROW("thermal_storage:%d - %s - Unable to map house interface property",
  ...
  // Check it
  if ((outside_temperature->is_valid() != true) ||
      (outside_temperature->is_double() != true)) {
    GL_THROW("thermal_storage:%d - %s - Unable to map house interface property",
             hdr->id, (hdr->name ? hdr->name : "Unnamed"));
    // Defined above
  }
  ...
  thermal_storage_available =
      new gld_property(parent, "thermal_storage_present");

  // Check it
  if ((thermal_storage_available->is_valid() != true) ||
      (thermal_storage_available->is_bool() != true)) {
    GL_THROW("thermal_storage:%d - %s - Unable to map house interface property",
             hdr->id, (hdr->name ? hdr->name : "Unnamed"));
    // Defined above
  }
  ...
  // Check it
  if ((thermal_storage_active->is_valid() != true) ||
      (thermal_storage_active->is_bool() != true)) {
    GL_THROW("thermal_storage:%d - %s - Unable to map house interface property",
             hdr->id, (hdr->name ? hdr->name : "Unnamed"));
    // Defined above
  }
  ...
  if (design_cooling_capacity == 0) {
    gl_warning("\'design_cooling_capacity\' not specified in parent ~ default "
               "to 5 ton or 60,000 Btu/hr");
    /* TROUBLESHOOT
            The thermal_storage did not reference a parent object that publishes
       design_cooling_capacity, so 5 ton was assumed. Confirm or change the
       parent reference and try again.
    */
  ...
  surface_area =
      6 * pow(water_capacity,
              0.6667); // suface area of a cube calculated from volume

  if (total_capacity == 0)
    total_capacity = (30 / 5) * discharge_rate; // Btu
  ...
  if (state_of_charge < 0 && stored_capacity < 0) // Btu
  {
    ...
    stored_capacity = (state_of_charge / 100) * total_capacity;
    gl_warning("stored_capacity and SOC are both defined, SOC being used for "
               "initial energy state");
    /*  TROUBLESHOOT
    During the initialization of the system, a value was specified for both the
    stored_capacity and SOC (state of charge). The thermal energy storage object
    gives precedence to the SOC variable, so the initial stored_capacity will be
    the SOC percentage of the total_capacity.
    */
  }

  if (recharge_power == 0)
    recharge_power = (3.360 * discharge_rate) / (5 * 12000); // kW
  if (discharge_power == 0)
    discharge_power = (0.300 * discharge_rate) / (5 * 12000); // kW
  if (recharge_power_factor == 0)
    recharge_power_factor = 0.97; // same as used for HVAC compressor in house_e
  if (discharge_power_factor == 0)
    discharge_power_factor = 1; // assume ideal pump
  if (k < 0)
    k = 0;            // assume no thermal conductivity
  ...
  if (recharge_schedule_type == INTERNAL) {
    ...
    recharge_schedule_vals =
        gl_schedule_find(thermal_default_schedule_list[1].schedule_name);

    // If not found, create
    if (recharge_schedule_vals == nullptr) {
      // Populate schedules - charging
      recharge_schedule_vals = gl_schedule_create(
          thermal_default_schedule_list[1].schedule_name,
          thermal_default_schedule_list[1].schedule_definition);
  ...
  if (discharge_schedule_type == INTERNAL) {
    // See if someone else has already created such a schedule
    discharge_schedule_vals =
        gl_schedule_find(thermal_default_schedule_list[0].schedule_name);

    // If not found, create
    if (discharge_schedule_vals == nullptr) {
      // Populate schedules - discharging
      discharge_schedule_vals = gl_schedule_create(
          thermal_default_schedule_list[0].schedule_name,
          thermal_default_schedule_list[0].schedule_definition);
...
int thermal_storage::isa(char *classname) {
  return (strcmp(classname, "thermal_storage") == 0 ||
...
TIMESTAMP thermal_storage::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (t0 != 0) {
    ...
    if (*recharge_time_ptr == 1 && *discharge_time_ptr == 1) {
      gl_warning("recharge and discharge can not both be scheduled to be "
                 "concurrently on ~ defaulting to recharge");
      /*  TROUBLEHSOOT
      The schedules or control values used to determine when the thermal energy
      storage can charge or discharge are overlapping.  In this case, the system
      gives precedence to recharging the thermal energy storage.  If this
      behavior is undesired, please ensure proper time separation of the
      controlling charge and discharge signals.
      */
    ...
    if (*recharge_time_ptr == 1 && *discharge_time_ptr != 1 &&
      ...
      if (recharge == 0)
        last_timestep =
            t0; // don't run energy calculations when first turned on
      recharge = 1;
    } else {
      recharge = 0;
    }
    ...
    // k is the coefficient of thermal conductivity [W/m/K - converted to
    // BTU/sec/m/degF] A is the surface area [m^2]
    ...
    if (stored_capacity > 0) {
      ...
      stored_capacity =
          stored_capacity +
          ((k * surface_area * (32 - outside_temperature_val) / 0.05) *
           (t0 - last_timestep));
    ...
    // Recharge cycle
    actual_recharge_power =
        recharge_power * (1 + (75 - outside_temperature_val) * 0.0106);
    if (recharge && *recharge_time_ptr == 1 && outside_temperature_val >= 15 &&
        outside_temperature_val <= 115)
    // needs to recharge, is set to recharge and the outside temperature is
    // between 15 and 155 deg F
    {
      ...
      if (last_timestep != t0) // only calculate energy once per timestep
      {
        stored_capacity =
            stored_capacity +
            (((total_capacity / 30) / (9 * outside_temperature_val + 705)) *
             (t0 - last_timestep));
      }
      if (stored_capacity >= total_capacity) {
        recharge = 0;
        ...
        recharge = 1;
        state_of_charge = stored_capacity / total_capacity * 100;
        next_timestep = (TIMESTAMP)((total_capacity - stored_capacity) /
                                    ((total_capacity / 30) /
                                     (9 * outside_temperature_val + 705)));
        if (next_timestep == 0)
          next_timestep = 1;
    ...
    if (*recharge_time_ptr != 1 && *discharge_time_ptr == 1 &&
        state_of_charge > 0 && outside_temperature_val >= 15)
    // not set to recharge, set to discharge, has charge to use and the outside
    // air temperature is above 15 deg F
    {
      thermal_storage_active->getp<bool>(temp_bool_val, test_rlock);
      if (temp_bool_val == false)
        last_timestep =
            t0; // don't run energy calculations when first turned on
    ...
    if (recharge == 0 && *discharge_time_ptr == 1 && (temp_bool_val == true)) {
      if (stored_capacity > 0) {
        ...
        {
          stored_capacity =
              stored_capacity - (discharge_rate * (t0 - last_timestep) / 3600);
        }
        state_of_charge = stored_capacity / total_capacity * 100;
  ...
  if (*obj != nullptr) {
    ...
    try {
      ...
      gl_error("%s::%s.create(OBJECT **obj={name='%s', id=%d},...): %s",
               (*obj)->oclass->module->name, (*obj)->oclass->name, (*obj)->name,
               (*obj)->id, msg);
      /* TROUBLESHOOT
...
EXPORT int init_thermal_storage(OBJECT *obj) {
  ...
  try {
    ...
    gl_error("%s::%s.init(OBJECT *obj={name='%s', id=%d}): %s",
             obj->oclass->module->name, obj->oclass->name, obj->name, obj->id,
             msg);
    /* TROUBLESHOOT
