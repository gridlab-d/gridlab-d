/** $Id: microwave.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The microwave simulation is based on a demand profile attached to the
 object. The internal heat gain is calculated using a specified fraction of
 installed power.
...
microwave::microwave(MODULE *module) : residential_enduse(module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "microwave", sizeof(microwave),
    ...
    if (gl_publish_variable(
            ...
            "installed_power[kW]", PADDR(shape.params.analog.power),
            PT_DESCRIPTION, "rated microwave power level", PT_double,
            "standby_power[kW]", PADDR(standby_power), PT_DESCRIPTION,
            "standby microwave power draw (unshaped only)", PT_double,
            "circuit_split", PADDR(circuit_split), PT_enumeration, "state",
            PADDR(state), PT_DESCRIPTION, "on/off state of the microwave",
            PT_KEYWORD, "OFF", OFF, PT_KEYWORD, "ON", ON,

            PT_double, "cycle_length[s]", PADDR(cycle_time), PT_DESCRIPTION,
            "length of the combined on/off cycle between uses", PT_double,
            "runtime[s]", PADDR(runtime), PT_DESCRIPTION, "", PT_double,
            "state_time[s]", PADDR(state_time), PT_DESCRIPTION, "",
            nullptr) < 1)
...
int microwave::create() {
  ...
  standby_power = 0.01;
  shape.load = gl_random_uniform(
      RNGSTATE, 0, 0.1); // assuming a default maximum 10% of the sync time
...
/* basic checks on unshaped microwaves.  on failure, don't play games, just
 * throw exceptions. */
void microwave::init_noshape() {
  if (shape.params.analog.power < 0) {
    GL_THROW("microwave power must be positive (read as %f)",
             shape.params.analog.power);
  } else if (shape.params.analog.power > 4.000) {
    GL_THROW(
        "microwave power can not exceed 4 kW (and most don't exceed 2 kW)");
  }
  if (shape.params.analog.power < 0.700) {
    gl_warning("microwave installed power is smaller than traditional "
               "microwave ovens");
  } else if (shape.params.analog.power > 1.800) {
    gl_warning("microwave installed power is greater than traditional "
               "microwave ovens");
  }
  if (standby_power < 0) {
    ...
    gl_warning("standby power exceeds installed power, resetting to 1%% of "
               "installed power");
    standby_power = shape.params.analog.power * 0.01;
  }
  if (cycle_time < 0) {
...
int microwave::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("microwave::init(): deferring initialization on %s",
  ...
  if (shape.type == MT_UNKNOWN) {
    ...
    gl_error("queued loadshapes not supported ~ will attempt to run as an "
             "unshaped load");
...
int microwave::isa(char *classname) {
  return (strcmp(classname, "microwave") == 0 ||
          residential_enduse::isa(classname));
}

// periodically activates for the tail demand % of a cycle_time period.  Has a
// random offset to prevent
...
double microwave::update_state(double dt) {
  ...
  // run times (used for gl_random_sample()) - DPC: this is an educated guess,
  // the true PDF needs to be researched
  static double rt[] = {30, 30, 30, 30, 30, 30,  30,  30,  30,  30, 60,
                        60, 60, 60, 90, 90, 120, 150, 180, 450, 600};
  ...
  case OFF:
    ...
    if (state_time == 0 || prev_demand != shape.load) {
      ...
      state_time = 0; // important that state time be reset to prevent increase
                      // in demand from causing immediate state change
    }
  ...
  default:
    ...
    /*	TROUBLESHOOT
            The microwave is neither on nor off.  Please initialize the "state"
...
TIMESTAMP microwave::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (shape.type == MT_UNKNOWN) {
    ...
    load.constant_power.SetPowerFactor(
        (state == ON ? shape.params.analog.power : standby_power),
        load.power_factor);
  }
  ...
  if (shape.type == MT_UNKNOWN) {
    if (cycle_time == 0)
      return dt > 0 ? -(TIMESTAMP)(t1 + dt * TS_SECOND)
                    : TS_NEVER; // negative time means soft transition
    else
...
static TIMESTAMP sync_microwave_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  try {
    microwave *my = object_data<microwave>(obj);
    TIMESTAMP t2 = TS_NEVER;
    switch (pass) {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_microwave(OBJECT *obj, TIMESTAMP t0,
                                             PASSCONFIG pass) {
