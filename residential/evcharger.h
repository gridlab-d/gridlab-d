...
class evcharger : public residential_enduse {
  typedef enum {
    ...
    //	VS_SHORTTRIP=2,				///< vehicle is on short trip
    //(<50 miles -- discharging to 25%) 	VS_LONGTRIP=3,
    /////< vehicle is on long trip (>50 miles -- discharged to 25%)
  } VEHICLESTATE;
  typedef enum {
...
public:
  double heat_fraction; ///< fraction of the evcharger that is transferred as
                        ///< heat (default = 0.90)
  ...
  struct {
    ...
    // these are not yet supported
    //	double shorttrip;			///< probability of leaving on a
    //short trip 	double longtrip;			///< probability of
    //leaving on a long trip
  } demand; ///< vehicle demand characteristics
  struct {
    double work; ///< distance to work
    // these are not yet supported
    //	double shorttrip;			///< typical distance for short
    //trip 	double longtrip;			///< typical distance for long
    //trip
  ...
  double mileage_classification; ///< Mileage classification of PHEV - e.g. 33
                                 ///< for a PHEV33 (33 miles on electric)
