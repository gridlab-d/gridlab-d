...
EXPORT CIRCUIT *attach_enduse_house_e(OBJECT *obj, enduse *target, double breaker_amps, int is220)
{
    ...
    if (breaker_amps < 0 || breaker_amps > 1000)
    { /* at 3kA, we're looking into substation power levels, not enduses */
...
house_e::house_e(MODULE *mod) : residential_enduse(mod)
{
    ...
    if (oclass == nullptr)
    {
        ...
        if (gl_publish_variable(oclass,
                                ...
                                PT_timestamp, "thermostat_last_off_cycle_time", PADDR(thermostat_last_off_cycle_time), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "last time the thermostat cycle went off",
                                PT_timestamp, "thermostat_last_on_cycle_time", PADDR(thermostat_last_on_cycle_time), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "last time the thermostat cycle went on",
                                ...
                                PT_double, "TcoolOff_dbg[degF]", PADDR(TcoolOff), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "DEBUG_TEMP",
                                PT_double, "TheatOn_dbg[degF]", PADDR(TheatOn), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "DEBUG_TEMP",
                                PT_double, "TheatOff_dbg[degF]", PADDR(TheatOff), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "DEBUG_TEMP",
                                PT_double, "TauxOn_dbg[degF]", PADDR(TauxOn), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "DEBUG_TEMP",
                                PT_enumeration, "re_override_dbg", PADDR(re_override), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "DEBUG_TEMP",
...
int house_e::init(OBJECT *parent)
{
    ...
    { // using legacy format
        if (heating_system_type == HT_UNKNOWN)
            heating_system_type = HT_NONE; // should not be possible, but good to put in.
    ...
    if (gross_wall_area == 0)
        gross_wall_area = 2.0 * number_of_stories * (aspect_ratio + 1.0) * ceiling_height * sqrt(floor_area / aspect_ratio / number_of_stories);
    ...
    if (interior_exterior_wall_ratio == 0)
        interior_exterior_wall_ratio = 1.5; // Based partions for six rooms per floor
    ...
    if (window_exterior_transmission_coefficient <= 0)
        window_exterior_transmission_coefficient = 0.60; // 0.6 represents a window with a screen
    ...
    if (air_thermal_mass == 0)
        air_thermal_mass = 3 * air_heat_capacity * air_mass; // thermal mass of air [BTU/F]  //*3 multiplier is to reflect that the air mass includes surface effects from the mass as well.
    ...
    if (mass_solar_gain_fraction == 0)
        mass_solar_gain_fraction = 0.5; // Rob Pratt's implimentation for heat gain from the solar gains
    if (mass_internal_gain_fraction == 0)
        mass_internal_gain_fraction = 0.5; // Rob Pratt's implimentation for heat gain from internal gains
    ...
    if (design_internal_gains <= 0.0)
        design_internal_gains = 167.09 * pow(floor_area, 0.442); // Numerical estimate of internal gains
    ...
    if (house_content_thermal_mass == 0)
        house_content_thermal_mass = total_thermal_mass_per_floor_area * floor_area - 2 * air_heat_capacity * air_mass; // thermal mass of house_e [BTU/F]
    if (house_content_heat_transfer_coeff == 0)
        house_content_heat_transfer_coeff =
            interior_surface_heat_transfer_coeff * (
                                                       ...
                                                       (gross_wall_area - window_area - door_area) + gross_wall_area * interior_exterior_wall_ratio + number_of_stories * exterior_ceiling_area / exterior_ceiling_fraction); // heat transfer coefficient of house_e contents [BTU/hr*F]
...
void house_e::update_system(double dt)
{
    ...
    if (heating_cop_curve == HC_LINEAR)
    { // rated temperature is 47F/8.33C
        if (value_Tout >= 47)
        {
            heating_cop_adj = heating_COP;
        }
    ...
    if (cooling_cop_curve == CC_LINEAR)
    { // rated temperature is at 95F/35C
        if (temp_c <= 35)
        {
            cooling_cop_adj = cooling_COP;
        }
...
#pragma warning("house_e: add update_system voltage adjustment for heating")
    ...
    if ((value_MeterStatus != 0) && (pHVAC_EnduseLoad->status == BRK_CLOSED))
    {
        ...
        case HT_RESISTANCE:
            ...
            break;
        case HT_HEAT_PUMP:
            if (system_mode == SM_AUX)
            { // only when we're 'running hot'
...
TIMESTAMP house_e::sync_thermostat(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    {
        char buffer[64];
        gl_error("%s: thermostat setpoints deadbands overlap (TcoolOff=%.1f < TheatOff=%.1f)", gl_name(object_header(this), buffer, sizeof(buffer)), TcoolOff, TheatOff);
        return TS_INVALID;
    }
    ...
    */
    ...
    if (thermostat_control != TC_NONE)
    {
        ...
        case SM_HEAT:
            ...
            if (thermostat_mode == TM_HEAT || thermostat_mode == TM_AUTO)
            { // heating is allowed
                if (re_override == OV_NORMAL)
                {
                    if (auxiliary_system_type != AT_NONE &&
                            ((auxiliary_strategy & AX_DEADBAND && Tair < TauxOn) || (auxiliary_strategy & AX_TIMER && t0 >= thermostat_last_cycle_time + aux_heat_time_delay)) ||
        ...
        case SM_AUX:
            ...
            break;
        case SM_COOL:
            if (thermostat_mode == TM_COOL || thermostat_mode == TM_AUTO)
            { // cooling is allowed
...
void house_e::push_complex_powerflow_values()
{
    ...
    if (commercial_load_parent == true)
    {
        ...
        */
        ...
        gld::complex balShunt = (value_Shunt[0] + value_Shunt[1] + value_Shunt[2] * 4.0) / (internalTurnsRatio * internalTurnsRatio) / denom;
        int insertS = 0;
        if (balShunt.Mag() > 0.0)
        {
            insertS = 1;
        ...
        gld::complex balCurrent = (value_Line_I[0] + value_Line_I[1] + value_Line_I[2] * 2.0) / internalTurnsRatio / denom;
        int insertI = 0;
...
EXPORT int init_house(OBJECT *obj)
{
    try
    {
        ...
        if (rv == 0)
        { // climate object has not initialized
            return 2;
        }
        else
        {
