...
mpi_network::mpi_network(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "mpi_network", sizeof(mpi_network), PC_BOTTOMUP);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);
    /* TROUBLESHOOT
            The registration for the mpi_network class failed.   This is usually
    ...
    if (gl_publish_variable(oclass, PT_int64, "interval", PADDR(interval),
                            PT_int32, "mpi_target", PADDR(mpi_target), PT_int64,
                            "reply_time", PADDR(reply_time), PT_ACCESS,
...
int mpi_network::init(OBJECT *parent) {
  ...
  if (mpi_target == 0) {
    gl_error(
        "mpi_network init(): target is 0, which is assumed to be GridLAB-D");
    return 0;
  }
...
TIMESTAMP mpi_network::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (next_time >= t1) {
    last_time = t1;
    rv = MPI_Send(&last_time, sizeof(last_time), MPI_LONG_LONG, mpi_target, 0,
                  MPI_COMM_WORLD);
    next_time += interval;
  }
  ...
  rv = MPI_Recv(&their_time, sizeof(their_time), MPI_LONG_LONG, mpi_target, 0,
