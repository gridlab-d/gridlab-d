...
int network_interface::check_buffer()
{
	...
	else if (send_message_mode == NSM_UPDATE || send_message_mode == NSM_UPDATE_PERIOD)
	{ // send on update
...
network_message *network_interface::handle_inbox(TIMESTAMP t1, network_message *nm)
{
	...
	if (nm != 0)
	{ // it's been stacked, process in reverse order
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_network_interface(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
