...
network::network(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    /* TROUBLESHOOT
            The registration for the network class failed.   This is usually
       caused by a coding error in the core implementation of classes or the
       module implementation. Please report this error to the developers.
     */
    ...
    if (gl_publish_variable(
            ...
            PT_double, "latency[s]", PADDR(latency), PT_char32, "latency_mode",
            PADDR(latency_mode), PT_double, "latency_period[s]",
            PADDR(latency_period), PT_double, "latency_arg1",
            PADDR(latency_arg1), PT_double, "latency_arg2", PADDR(latency_arg2),
            PT_double, "bandwidth[MB/s]", PADDR(bandwidth), PT_enumeration,
            "queue_resolution", PADDR(queue_resolution), PT_KEYWORD, "REJECT",
            QR_REJECT, PT_KEYWORD, "QUEUE", QR_QUEUE, PT_double,
            "buffer_size[MB]", PADDR(buffer_size), PT_double,
            "bandwidth_used[MB/s]", PADDR(bandwidth_used), PT_ACCESS,
            PA_REFERENCE, nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    /* TROUBLESHOOT
            The registration for the network properties failed.   This is
...
void network::update_latency() {
  // NOTE: latency_period == 0 means that the latency will update every
  // iteration, even though it is processed during 'commit'.
  ...
  if (random_type != RT_INVALID) {
    ...
    latency_next_update = TS_NEVER; // latency does not use a distribution and
...
int network::isa(char *classname) { return strcmp(classname, "network") == 0; }
...
TIMESTAMP network::commit(TIMESTAMP t1, TIMESTAMP t2) {
  ...
  double net_bw = 0.0;
  double nif_bw = 0.0;
  double net_bw_used = 0.0; // tallies how much network bandwidth has been used
  ...
  // not O(n^2) but limited to the count of messages that exist, could iterate
  // on those iff messages were stored in a list THIS SHOULD NOT USE
  // nif->send_rate_used OR nif->recv_rate_used!! at the moment, this is
  //	making some guesses on how much bandwidth is wanted to send all the
  ...
  for (nif = first_if; nif != 0; nif = nif->next) {
    ...
    if (nif->has_outbound()) {
      ...
      for (netmsg = nif->peek_outbox(); netmsg != 0; netmsg = netmsg->next) {
        // note: if applicable, would check rx's bandwidth to limit tx's sent
        // rate constrain by interface send rate
        if (nif_bw + netmsg->size - netmsg->bytes_sent > nif->send_rate) {
          nif_bw = nif->send_rate; // unable to send all the existing message
                                   // AND the rest of this message
          break;
        } else {
          nif_bw += netmsg->size -
                    netmsg->bytes_sent; // try to send the rest of this message
        }
      }
      net_bw += nif_bw;
    }
  ...
  /*	if(net_bw > bandwidth){
                  switch(queue_resolution){
                          case QR_REJECT:
                                  bandwidth_used = 0;
                                  bytes_rejected = net_bw - bandwidth; // for
     reference, how far over we are break; case QR_QUEUE: bandwidth_used =
     bandwidth; break; case QR_SCALE: gl_error("communication network does not
     yet support interface transmission scaling"); return 0; default: break;
                  }
          } else {
                  ;
          }*/
  ...
  for (nif = first_if; nif != 0; nif = nif->next) {
    if (nif->has_outbound()) {
      ...
      for (netmsg = nif->peek_outbox(); netmsg != 0; netmsg = netmsg->next) {
        // note: if applicable, would check rx's bandwidth to limit tx's sent
        // rate.  may involve another pass to pre-determine nif transmission
        // constraints determine how much is being sent
        ...
        case QR_REJECT:
          ...
          if (bandwidth - bandwidth_used >
              nif->send_rate - nif->send_rate_used) {
            // send
            ...
            if (bandwidth == net_bw_used) {
              ...
              netmsg->send_rate =
                  bandwidth - bandwidth_used; // already know is l.t. NIF BW
              netmsg->msg_state = NMS_SENDING;
            }
          }
          break;
          ...
          if (bandwidth - bandwidth_used >
            ...
            if (bandwidth == bandwidth_used && buffer_size == buffer_used) {
              ...
              netmsg->send_rate =
                  bandwidth - bandwidth_used; // already know is l.t. NIF BW
              netmsg->buffer_rate =
                  nif->send_rate - nif->send_rate_used - netmsg->send_rate;
              netmsg->msg_state = NMS_SENDING;
            }
          }
          break;
        case QR_SCALE:
          gl_error("communication network does not yet support interface "
  ...
  // if there's additional bandwidth, use it to unload the buffer back into the
  // network
  if (bandwidth_used < bandwidth) {
    ...
    if (buffer_used > 0) {
      for (nif = first_if; nif != 0 && bandwidth_left > 0.0; nif = nif->next) {
        if (nif->has_outbound()) {
          //	for each message,
          for (netmsg = nif->peek_outbox(); netmsg != 0;
               netmsg = netmsg->next) {
      ...
      for (netmsg = nif->peek_outbox(); netmsg != 0; netmsg = netmsg->next) {
        ...
        if (netmsg->bytes_recv >= netmsg->size) {
          ...
          if (netmsg->end_time < rv)
            rv = netmsg->end_time;
          //					memcpy(netmsg->pTo->data_buffer,
          //netmsg->message, netmsg->buffer_size);
          ...
          if (nif->outbox == netmsg) { // make sure we pull this off the outbox,
                                       // if it's the first one there
          ...
          if (netmsg->send_rate > 0.0) {
            predicted_tx_sec =
                (netmsg->size - netmsg->bytes_recv) / netmsg->send_rate;
