...
public:
  ...
  bool has_outbound() {
    return (outbox != 0);
  } // returns true if there is a message being sent from this interface
  bool has_inbound() {
    return (inbox != 0);
  } // returns true if there is a message completely received by this interface
  network_message *peek_outbox() {
    return outbox;
  } // returns the first message in the outbox without modifying it
  network_message *peek_inbox() {
    return inbox;
  } // returns the first message in the inbox without modifying it
};
