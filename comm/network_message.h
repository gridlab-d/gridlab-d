...
public:
  ...
  double bytes_resent;
  double bytes_recv;
  double bytes_buffered; // bytes caught up in a buffered network's router
                         // storage spaces
  MSGSTATE msg_state;
  ...
  int send_message(OBJECT *f, OBJECT *t, char *m, int len, double sz,
                   TIMESTAMP ts, double frac, double latency);
