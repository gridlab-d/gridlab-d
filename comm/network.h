...
typedef enum {
  ...
  QR_QUEUE,  // once bandwidth capacity is met, edge routers buffer messages
             // before transmitting across the network
  QR_SCALE,
  /*x*/ // once bandwidth capacity is met, transmitters are scaled back so as to
...
// future idea
typedef enum {
  MXM_FIFO,    // the messages are sent first-in first out and will block
               // additional messages
  MXM_BALANCE, // messages are sent at a rate proportional to the receiver's
               // bandwidth
  MXM_CASCADE  // messages are sent FIFO, but if Rx limits Tx, additional
               // messages may try to be sent
} MXMODE;
...
public:
  ...
  QUEUERESOLUTION
      queue_resolution; // determines what the network does when the sum of the
                        // interface traffic wants to exceed the bandwidth
  double buffer_size;   // how many bytes can be stored 'in the network' when
                        // traffic exceeds network bandwidth
  double buffer_used;   // how many bytes are stored by edge-routers before
