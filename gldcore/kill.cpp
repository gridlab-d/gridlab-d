/** $Id: kill.c 4738 2014-07-03 00:55:39Z dchassin $
   ...
   @file kill.c
   @addtogroup exec
   This program provide the same kill functionality that is available to Linux
 versions
...
void kill_stophandler(void) { handler_stop = 1; }
...
void kill_starthandler(void) {
  if (_beginthread(&msghandler, 0, (void *)SIGINT) == 1 ||
      _beginthread(&msghandler, 0, (void *)SIGTERM) == 1)
...
/** Send a kill signal to a windows version of GridLAB-D
    @return 0 on successfull completion, -1 on error (e.g., no such signal, no
 such process)
 **/
         ...
         int sig)   /**< the signal id (see signal.h) */
{
  ...
  /* valid signal needs to be sent */
    ...
    output_error(
        "unable to signal gridlabd process %d with signal %d (error %d)", pid,
        sig, GetLastError());
    return -1;
  } else {
    SetEvent(hEvent);
