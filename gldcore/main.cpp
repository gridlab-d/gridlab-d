...
void delete_pidfile(void) { unlink(global_pidfile); }
...
fs::path findExecutable(const std::string &name, const std::string &execName,
    ...
    char *argv[]) /**< a list of pointers to the command-line arguments */
{
  ...
  global_gl_path = std::string(
      (getenv("GLPATH") != nullptr ? std::string(getenv("GLPATH")) + env_delim
                                   : "") +
      global_gl_lib.string() + env_delim + global_gl_share.string() +
      env_delim + global_gl_include.string() + env_delim +
  ...
  for (i = 0; i < argc; i++) {
    if (pos < (int)(sizeof(global_command_line) - strlen(argv[i])))
      pos += sprintf(global_command_line + pos, "%s%s", pos > 0 ? " " : "",
                     argv[i]);
  }
