...
struct s_loadshape {
  ...
  union {
    struct {
      double energy; /**< the total energy used over the shape (0 if power is
                        used) */
      double power;  /**< the power scaling factor of the shape (0 if energy is
                        used) */
    } analog;
    struct {
    ...
    struct {
      ...
      double on_time, off_time; /**< the hour of day when the high value comes
                                   on and goes off */
      double on_ramp,
          off_ramp; /**< the rate at which the high value comes on and off */
      double on_end, off_end; /**< ramp end times are computed internally to
                                 ease sync calcs */
      unsigned char weekdays; /**< bitfield indicating which weekdays the
                                 schedule is active (Sun=b0, Mon=b1, etc.) */
      unsigned int
          dt; /**< the time resolution of the schedule (default is 3600s) */
    } scheduled;
...
int convert_from_loadshape(
    char *string, int size, void *data,
    PROPERTY *prop); /**< convert from a loadshape to a string */
int convert_to_loadshape(
    char *string, void *data,
    PROPERTY *prop); /**< convert from a string to a loadshape */
