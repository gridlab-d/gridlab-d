                    ...
                    char *argv[]) /**< arguments passed from the command line */
{
  ...
  // if (!mod)
  // {
  // 	fprintf(stderr, "module_load: Failed to find module %s\n", file);
  // 	abort();
  // }
...
// --- ACQUIRE MUTEX BEFORE DYNAMIC LOADING ---
  ...
  if (fmod != nullptr && strcmp(fmod, file) != 0) {
    ...
    if (parent_mod != nullptr &&
        parent_mod->subload !=
            nullptr) { /* if we've defined a subload routine and already loaded
                          the parent module*/
      MODULE *child_mod;
      ...
      if (mod != nullptr) { /* if we want to register another module */
...
int module_saveall_xml_old(FILE *fp) {
  ...
  for (mod = first_module; mod != nullptr; mod = mod->next) {
    ...
    while (module_getvar(mod, varname, nullptr, 0)) {
      ...
      if (module_getvar(
              mod, varname, value,
              sizeof(value))) { /* TODO: support other types (ticket #46) */
...
void module_libinfo(const char *module_name) {
  ...
  if (mod != nullptr) {
    ...
    if (mod->setvar != nullptr)
      output_raw("setvar ");
    if (mod->import_file != nullptr)
      output_raw("import_file ");
    if (mod->export_file != nullptr)
      output_raw("export_file ");
    if (mod->check != nullptr)
      output_raw("check ");
                   ...
                   int line)           /**< source line (for context) */
{
  ...
  // FILE *def = fopen("exports.def", "w");
  // if (def)
  // {
  // 	fprintf(def, "EXPORTS\nmyfunction2 @1\n");
  // 	fclose(def);
  // }
  if ((rc = execf("cmd /C build_dll.bat")) != 0)
    return rc;
...
void sched_update(TIMESTAMP clock, enumeration status) {
  int t;
  if (my_proc == nullptr || my_proc->list == nullptr)
    return;
...
void sched_init(int readonly) {
  ...
  if (shmid < 0) {
    ...
    case ENOENT:
      ...
      break;
    case ENOMEM:
      output_error("process map too big");
      break;
    /* TROUBLESHOOT
