/** $Id: complex.h 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The complex number implementation is specifically designed to use in
 power systems calculations.  Hence it supports some things that a peculiar to
 power systems engineering. In particular, the imaginary portion can be
 identified using \e i, \e j or \e d (for polar coordinates).

        Note that the functions are only available to C++ code and only the \p
 struct is available to C code.
...
#define complex_set_polar(X, M, A)                                             \
  ((X).r = ((M) * cos(A)), (X).i = ((M) * sin(A)), (X))
#define complex_set_power_factor(X, M, P)                                      \
  complex_set_polar((X), (M) / (P), acos(P))
#define complex_get_mag(X) (sqrt((X).r * (X).r + (X).i * (X).i))
#define complex_get_arg(X)                                                     \
  ((X).r == 0 ? ((X).i > 0 ? PI / 2 : ((X).i < 0 ? -PI / 2 : 0))               \
              : ((X).r > 0 ? atan((X).i / (X).r) : PI + atan((X).i / (X).r)))
double complex_get_part(void *c, const char *name);
#else
public:
