/** $Id: unit.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        Based on these units, all the other units are derived using simple
 formulae, e.g.:
        @verbatim
        ; Length
        cm=0.01 m
        ...
        The theory behind the unit management system is based on the fact that
 every physical quantity can be expressed in terms of a linear combination of
 exponents of the five basic physical constants (to which a currency constant is
 added to allow cost-based units).  This is best understood by considering the
 equation
        ...
        where c, e, h, k, m, and s are the constants defined above, and the k
 terms are the unit terms defined for that unit.

        Thus, two values can only be added (or subtracted) only if their units
 are the same (i.e., unit is a common factor), and when two values are
 multiplied (or divided), the exponential terms of the units are added (or
 subtracted).
...
/* unit_find_raw is both used to detect the presence and absence of units in the
   list. not finding a given unit is normal behavior, and this function should
   run silently.
*/
...
UNIT *unit_primary(const char *name, double c, double e, double h, double k,
                   double m, double s, double a, double b, int prec);
...
UNIT *unit_find_underived(const char *unit) {
  ...
  if (p == nullptr) {
    ...
    /*	TROUBLESHOOT
            The given unit had a recognized scalar prefix, but the underlying
       base unit was not identified.   This will cascade a null pointer to the
       calling method.
    */
    return nullptr;
  }
  ...
  return unit_primary(unit, p->c, p->e, p->h, p->k, p->m, p->s,
                      p->a * pow(10, s->scalar), p->b, p->prec);
}
...
int unit_scalar(char *name, int scalar) {
  ...
  if (ptr == nullptr) {
    throw_exception("%s(%d): scalar definitinon of '%s' failed: %s", filepath,
                    linenum, name, strerror(errno));
    /*	TROUBLESHOOT
            This is actually triggered by a malloc failure, and the rest of the
       system is likely unstable.
    */
  }
...
int unit_constant(char name, double value) {
  ...
  if (unit_find_underived(buffer) != nullptr) {
    throw_exception(
        "%s(%d): constant definition of '%s' failed; unit already defined",
        filepath, linenum, buffer);
    /*	TROUBLESHOOT
            The value of the specified scalar name has already been used as a
       unit and cannot be used as a constant.  Please remove or comment out the
       offending line from the unit file.
    */
  }
  switch (name) {
  case 'c':
  ...
  case 's':
    ...
    break;
  default:
    throw_exception("%s(%d): constant '%c' is not valid", filepath, linenum,
                    name);
    /*	TROUBLESHOOT
            The specified constant name does not conform to the internally
...
UNIT *unit_primary(const char *name, double c, double e, double h, double k,
...
#endif
  if (p != nullptr &&
      (c != p->c || e != p->e || h != p->h || k != p->k || m != p->m ||
       s != p->s || a != p->a || b != p->b || prec != p->prec)) {
    throw_exception("%s(%d): primary definition of '%s' failed; unit already "
                    "defined with a different parameter set",
                    filepath, linenum, name);
    /*	TROUBLESHOOT
            Since the specified unit was defined, the values of the universal
       constants OR the global precision have changed.  No units should be
       created until after all of the constants have been specified, nor should
       those constants be modified between unit definitions.
    */
  }
  ...
  if (p == nullptr) {
    ...
    /* TROUBLESHOOT
            The unit conversion system was unable to allocate memory needed to
       define the unit located at the specified file at the given line. Try
       freeing up system memory and try again.
     */
    return 0;
  }
...
int unit_precision(char *term) {
  ...
  /* get mantissa */
  if ((p = strchr(term, 'e')) != nullptr ||
      (p = strchr(term, 'E')) != nullptr) {
...
int unit_derived(const char *name, const char *derivation) {
  ...
  if (unit_find_raw(name) != nullptr) {
    throw_exception(
        "%s(%d): derived definition of '%s' failed; unit already defined",
        filepath, linenum, name);
    /*	TROUBLESHOOT
            The specified derived unit has already been defined within the unit
       file.  Please review the unit definition file and remove the offending
       line.
    */
  }
  ...
  while (*p != '\0') {
    ...
    if (sscanf(p, "%[^-*/^+]", term) != 1) {
      ...
      /*	TROUBLESHOOT
              The unit definition contains a character that prevents it from
         being parsed.  No unit definitions should contain the characters ^, -,
         *, /, or +.
      */
    }
    ...
    if (nextOp != '^') {
      ...
      if (pUnit == nullptr) {
        ...
        if (sscanf(term, "%lf", &val) == 1) {
          ...
          throw_exception("%s(%d): unable to find or derive unit '%s'",
                          filepath, linenum, p);
          /*	TROUBLESHOOT
                  The specified term was neither found nor derived, and likely
             does not exist in the unit subsystem at the point the exception is
             raised.  Review the unit file and either move the offending unit
             down the file, or verify that the base unit exists.
          */
        }
      }
    }
    ...
    case '^':
      if (lastUnit != nullptr) {
        ...
        if (sscanf(term, "%d", &repeat) != 1 || repeat < 2) {
          throw_exception(
              "%s(%d): syntax error at '%s', exponent must be greater than 1",
              filepath, linenum, term);
          /*	TROUBLESHOOT
                  The unit was defined akin to "m^2", but the exponent was an
             integer value less than 1, which is nonsensical for a physical
             simulation
          */
        }
        repeat--;
        ...
        case '/':
          ...
          break;
        default:
          throw_exception("%s(%d): ^ not allowed after '%c' at '%s'", filepath,
                          linenum, lastOp, term);
          /* TROUBLESHOOT
                  The unit file used an invalid syntax to define a unit. Correct
      ...
      break;
    default:
      throw_exception("%s(%d): '%c' is not recognized at '%s'", filepath,
                      linenum, lastOp, term);
      /* TROUBLESHOOT
              The unit file used an invalid syntax to define a unit.  Correct
         the syntax and try again.
...
void unit_init(void) {
  ...
  std::unique_lock<std::shared_mutex> lock(
      SharedMutexManager::get_mutex(&trylock));
  if (tried) {
    // wunlock(&trylock);
    return;
  } else
  ...
  if (fp == nullptr) {
    ...
    /*	TROUBLESHOOT
            The unit subsystem was not able to locate the unit file in the
       working directoy or in the directories specified in GLPATH.
    */
    // wunlock(&trylock);
    return;
  }
  ...
  while (!feof(fp) && !ferror(fp)) {
    ...
    if (buffer[0] == '#') {
      ...
      if (sscanf(buffer + 1, "%c=%lf", &name, &value) != 2) {
        output_error("%s(%d): constant definition '%s' is not valid", filepath,
                     linenum, buffer);
      } else {
    ...
    if (buffer[0] == '*') {
      ...
      if (sscanf(buffer + 1, "%7[^=]=%d", name, &scalar) == 2) {
        ...
        output_error("%s(%d): scalar '%s' is not valid", filepath, linenum,
                     buffer);
      }
      continue;
    }
    ...
    /* primary definition */
    {
      ...
      int prec;
      if (sscanf(buffer, "%[^=]=%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%d", name, &c,
                 &e, &h, &k, &m, &s, &a, &b, &prec) == 10) {
    ...
    /* not recognized */
    throw_exception("%s(%d): unit '%s' is not recognized", filepath, linenum,
                    buffer);
    /*	TROUBLESHOOT
            The specified unit was not recognized as a constant, a primary unit,
       or a derived unit.
    */
  }
  ...
  if (strcmp(from, to) == 0)
    ...
    if (pFrom != nullptr) {
      if (pTo != nullptr) {
        ...
        /*	TROUBLESHOOT
                The specified unit name was not found by the unit system. Verify
           that it is a valid unit name.
        */
      }
    } else {
...
int unit_convert_ex(UNIT *pFrom, UNIT *pTo, double *pValue) {
  if (pFrom == nullptr || pTo == nullptr || pValue == nullptr) {
    ...
    /*	TROUBLESHOOT
            An error occured earlier in processing that caused a null pointer to
       be used as an arguement.  Review other error messages for details, but
       either a property was not found, or a unit definition was not found.
    */
    return 0;
  }
  if (pTo->c == pFrom->c && pTo->e == pFrom->e && pTo->h == pFrom->h &&
      pTo->k == pFrom->k && pTo->m == pFrom->m && pTo->s == pFrom->s) {
    *pValue = (*pValue - pFrom->b) * (pFrom->a / pTo->a) + pTo->b;
    return 1;
  } else {
    output_error(
        "could not convert units from %s to %s, mismatched constant values",
        pFrom->name, pTo->name);
    return 0;
  }
}
...
#define PI 3.141592635
  ...
  for (n = 0; n < sizeof(test) / sizeof(test[0]); n++) {
    ...
    if (!unit_convert(test[n].from.unit, test[n].to.unit, &v)) {
      output_test("FAILED: conversion from %s to %s not possible",
                  test[n].from.unit, test[n].to.unit);
      failed++;
    } else if (fabs(v - test[n].to.value) > test[n].precision) {
      output_test("FAILED: incorrect unit conversion %g %s -> %g %s (got %g %s "
                  "instead)",
                  test[n].from.value, test[n].from.unit, test[n].to.value,
                  test[n].to.unit, v, test[n].to.unit);
      failed++;
    } else if (!unit_convert(test[n].to.unit, test[n].from.unit,
                             &v)) { /* reverse test */
      output_test("FAILED: conversion from %s to %s not possible",
                  test[n].to.unit, test[n].from.unit);
      failed++;
    } else if (fabs(v - test[n].from.value) > test[n].precision) {
      output_test("FAILED: incorrect unit conversion %g %s -> %g %s (got %g %s "
                  "instead)",
                  test[n].to.value, test[n].to.unit, test[n].from.value,
                  test[n].from.unit, v, test[n].from.unit);
      failed++;
    } else {
      output_test("SUCCESS: %g %s = %g %s", test[n].from.value,
                  test[n].from.unit, test[n].to.value, test[n].to.unit);
      succeeded++;
    }
  }
  output_test("END: %d units tested", n);
  output_verbose("units tested: %d ok, %d failed (see '%s' for details).",
