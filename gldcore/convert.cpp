    ...
    PROPERTY *prop)     /**< a pointer to keywords that are supported */
{
  ...
  if (n > 1) /* something else given */
  {
    if (prop->unit != nullptr) /* Unit allowed - see if it is a valid unit */
    {
      ...
      if (from != prop->unit &&
          unit_convert_ex(from, prop->unit, (double *)data) == 0)
      {
        output_error(
            "convert_to_double(const char *buffer='%s', void *data=0x%*p, "
            "PROPERTY *prop={name='%s',...}): unit conversion failed",
            buffer, sizeof(void *), data, prop->name);
        /* TROUBLESHOOT
        This error is caused by an attempt to convert a value from a unit that
        is incompatible with the unit of the target property.  Check your units
        and try again.
        */
        return 0;
      }
    ...
    else // Unit not specified, give a more general error
    {
      output_error(
          "convert_to_double(const char *buffer='%s', void *data=0x%*p, "
          "PROPERTY *prop={name='%s',...}): conversion failed",
          buffer, sizeof(void *), data, prop->name);
      /* TROUBLESHOOT
      This error is caused by either an invalid entry in the conversion (extra
      decimal points), or with a unit specified where no unit was on the
      original property.  Check your source data (GLM entry or player file) and
      try again.
      */
      return 0;
    }
    ...
    PROPERTY *prop)     /**< a pointer to keywords that are supported */
{
  ...
  if (n == 1) /* only real part */
    v->SetRect(a, 0, v->Notation());
           ...
           strchr("ijdr", notation[0]) ==
               nullptr) /* incomplete imaginary part or missing notation */
  {
    output_error(
        "convert_to_complex('%s',%s): complex number format is not valid",
        buffer, prop->name);
    /* TROUBLESHOOT
            A complex number was given that doesn't meet the formatting
       requirements, e.g., <real><+/-><imaginary><notation>. Check the format of
       your complex numbers and try again.
     */
    return 0;
  }
  /* appears ok */
