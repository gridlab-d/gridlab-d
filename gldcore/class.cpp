...
/** Get the first property in a class's property list.
        ...
        or \p nullptr is none defined
 **/
PROPERTY *class_get_first_property(CLASS *oclass) /**< the object class */
{
  if (oclass == nullptr)
    throw_exception(
        "class_get_first_property(CLASS *oclass=nullptr): oclass is nullptr");
  /* TROUBLESHOOT
          A call to <code>class_get_first_property()</code> was made with a
     nullptr pointer. This is a bug and should be reported.
   */
  return oclass->pmap;
}

/** Get the next property of within the current class
        @return a pointer to the PROPERTY, or \p nullptr if there are no
 properties left
 **/
...
/* though improbable, this is to prevent more complicated, specifically crafted
        inheritence loops.  these should be impossible if a class_register call
   is immediately followed by a class_define_map call. -d3p988 */
PROPERTY *class_find_property_rec(CLASS *oclass, const PROPERTYNAME name,
                                  CLASS *pclass) {
  PROPERTY *prop;
  ...
  if (oclass->parent == pclass) {
    output_error(
        "class_find_property_rec(CLASS *oclass='%s', PROPERTYNAME name='%s', "
        "CLASS *pclass='%s') causes an infinite class inheritance loop",
        oclass->name, name, pclass->name);
    /*	TROUBLESHOOT
            A class has somehow specified itself as a parent class, either
       directly or indirectly. This means there is a problem with the module
       that publishes the class.
     */
    return nullptr;
...
static PROPERTY *find_header_property(CLASS *oclass, const PROPERTYNAME name) {
...
/** Find the named property in the class
        ...
        @return a pointer to the PROPERTY, or \p nullptr if the property is not
 found.
 **/
                              ...
                              const PROPERTYNAME name) /**< the property name */
{
  ...
  for (prop = oclass->pmap; prop != nullptr && prop->oclass == oclass;
    ...
    if (strcmp(name, prop->name) == 0) {
      if (prop->flags & PF_DEPRECATED &&
          ...
          !global_suppress_deprecated_messages) {
        output_warning("class_find_property(CLASS *oclass='%s', PROPERTYNAME "
                       "name='%s': property is deprecated",
                       oclass->name, name);
        /* TROUBLESHOOT
                You have done a search on a property that has been flagged as
           deprecated and will most likely not be supported soon. Correct the
           usage of this property to get rid of this message.
         */
  ...
  if (oclass->parent == oclass) {
    output_error("class_find_property(oclass='%s', name='%s') causes an "
                 "infinite class inheritance loop",
                 oclass->name, name);
    /*	TROUBLESHOOT
            A class has somehow specified itself as a parent class, either
       directly or indirectly. This means there is a problem with the module
 ...
 **/
void class_add_property(
    CLASS *oclass,  /**< the class to which the property is to be added */
 ...
 **/
PROPERTY *class_add_extended_property(
    CLASS *oclass,      /**< the class to which the property is to be added */
    ...
    char *unit)         /**< the unit of the property */
{
  ...
  if (prop == nullptr)
    throw_exception("class_add_extended_property(oclass='%s', name='%s', ...): "
                    "memory allocation failed",
                    oclass->name, name);
  /* TROUBLESHOOT
          The system has run out of memory.  Try making the model smaller and
     trying again.
   */
  if (ptype <= _PT_FIRST || ptype >= _PT_LAST)
    throw_exception("class_add_extended_property(oclass='%s', name='%s', ...): "
                    ...
                    oclass->name, name);
  /* TROUBLESHOOT
          The function was called with a property type that is not recognized.
     This is a bug that should be reported.
   */
  if (unit != nullptr && pUnit == nullptr)
    throw_exception("class_add_extended_property(oclass='%s', name='%s', ...): "
                    "unit '%s' is not found",
                    oclass->name, name, unit);
  /* TROUBLESHOOT
          The function was called with unit that is defined in units file
     <code>.../etc/unitfile.txt</code>.  Try using a defined unit or adding the
     desired unit to the units file and try again.
   */
 ...
 **/
const char *
class_get_property_typexsdname(PROPERTYTYPE type) /**< the property type */
{
  if (type <= _PT_FIRST || type >= _PT_LAST)
    return "//UNDEF//";
  else
 ...
 **/
PROPERTYTYPE class_get_propertytype_from_typename(
    char *name) /**< a string containing the name of the property type */
{
  int i;
 ...
 **/
int class_string_to_property(
    PROPERTY *prop,    /**< the type of the property at the \p addr */
 ...
 **/
int class_property_to_string(
    PROPERTY *prop, /**< the property type */
    void *addr,     /**< the address of the property's data */
    char *value, /**< the value buffer to which the string is to be written */
    int size) /**< the maximum number of characters that can be written to the
                 \p value buffer*/
{
  int rv = 0;
  if (prop->ptype == PT_delegated) {
    ...
    /*	TROUBLESHOOT
            Property delegation is not yet fully implemented, so you should
       never get this error. If you do, there is a problem with the system that
       is causing it to become unstable.
     */
    return 0;
...
CLASS *class_register(
    MODULE *module,        /**< the module that implements the class */
    ...
    PASSCONFIG passconfig) /**< the passes for which \p sync should be called */
{
  ...
  if (_PT_LAST - _PT_FIRST - 1 !=
      sizeof(property_type) / sizeof(property_type[0])) // This is always false.
  {
    output_fatal("property_type[] in class.c has an incorrect number of "
                 "members (%i vs %i)",
                 a / b, c);
    /* TROUBLESHOOT
            This error occurs when an improper definition of a class is used.
       This is not usually caused by an error in a GLM file but is most likely
       caused by a bug in a module or incorrectly defined class.
     */
    exit(XC_EXCEPTION);
  }
  if (oclass != nullptr) {
    if (strcmp(oclass->module->name, module->name) == 0) {
      output_error("module %s cannot register class %s, it is already "
                   "registered by module %s",
                   module->name, name, oclass->module->name);
      /*	TROUBLESHOOT
              This error is caused by an attempt to define a new class which is
         already defined in the module or namespace given.  This is generally
         caused by bug in a module or an incorrectly defined class.
       */
      return nullptr;
    } else {
      output_verbose(
          "module %s is registering a 2nd class %s, previous one in module %s",
          module->name, name, oclass->module->name);
    }
  }
...
/** Get the first registered class
        ...
        or \p nullptr if none registered.
 **/
CLASS *class_get_first_class(void) { return first_class; }
...
/** Get the class from the class name.
        ...
        or \p nullptr if no match found.
 **/
CLASS *class_get_class_from_classname(
    const char *name) /**< a pointer to a \p nullptr -terminated string
                         containing the class name */
{
  ...
  MODULE *mod = nullptr;
  char *ptr = nullptr;
  char temp[1024]; /* we get access violations when name is from another DLL.
                      -mh */
  strcpy(temp, name);
  ptr = strchr(temp, '.');
  if (ptr != nullptr) { /* check module for the class */
    ...
    if (mod == nullptr) {
      output_verbose("could not search for '%s.%s', module not loaded", name,
                     ptr);
      return nullptr;
    }
    for (oclass = first_class; oclass != nullptr; oclass = oclass->next) {
...
/** Define one or more class properties.
        ...
        The variable argument list must be \p nullptr -terminated.
        ...
        - \p PT_SIZE will cause memory to be allocated for the value.  If
          \p addr is non-nullptr, the data is points to will be converted to
          the property.
        ...
        - \p PT_UNITS will allow you to specify the units (instead of using []
 syntax)
        - \p PT_EXTEND will expand the size of the class by the size of the
 property being mapped
        - \p PT_EXTENDBY will expand the size of the class by the unsigned int
    ...
    if (proptype > _PT_LAST) {
      if (proptype == PT_INHERIT) {
        if (oclass->parent != nullptr) {
          errno = EINVAL;
          output_error(
              "class_define_map(oclass='%s',...): PT_INHERIT unexpected; class "
              "already inherits properties from class %s",
              oclass->name, oclass->parent);
          /* TROUBLESHOOT
                  This error is caused by an attempt to incorrectly specify a
             class that inherits variables from more than one other class.  This
             is almost always caused by a bug in the module's constructor for
             that class.
           */
          goto Error;
        } else {
          ...
          oclass->parent = class_get_class_from_classname_in_module(
              classname, oclass->module);
          if (oclass->parent == nullptr) {
            errno = EINVAL;
            output_error("class_define_map(oclass='%s',...): parent property "
                         "class name '%s' is not defined",
                         oclass->name, classname);
            /*	TROUBLESHOOT
                    A class is trying to inherit properties from another class
               that has not been defined. This is usually caused by a problem in
               the module(s) that publishes the classes.  Either the child class
               is in not specifying the parent class correctly, or the parent
               class is not created before the child class.  If the child class
               depends on a class implemented in another module, the module
               publishing the child class must load the module publishing the
               parent class before attempting to use it.
             */
            goto Error;
          }
          if (oclass->parent == oclass) {
            errno = EINVAL;
            output_error("class_define_map(oclass='%s',...): parent property "
                         "class name '%s' attempting to inherit from self!",
                         oclass->name, classname);
            /*	TROUBLESHOOT
                    A class is attempting to directly inherit properties from
               itself.  This is caused by a problem with the module that
               publishes the class.
             */
            goto Error;
          }
          no_override =
              ~(~oclass->parent->passconfig |
                oclass->passconfig); /* parent bool-implies child (p->q=~p|q) */
          if (oclass->parent->passconfig & PC_UNSAFE_OVERRIDE_OMIT &&
              !(oclass->passconfig & PC_PARENT_OVERRIDE_OMIT) &&
              no_override & PC_PRETOPDOWN)
            output_warning("class_define_map(oclass='%s',...): class '%s' "
                           "suppresses parent class '%s' PRETOPDOWN sync "
                           "behavior by omitting override",
                           oclass->name, oclass->name, oclass->parent->name);
          /*	TROUBLESHOOT
                  A class is suppressing the <i>presync</i> event implemented by
             its parent even though the parent is published with a flag that
             indicates this is unsafe.  Presumably this is deliberate, but the
             warning is given just in case it's not intended.
           */
          if (oclass->parent->passconfig & PC_UNSAFE_OVERRIDE_OMIT &&
              !(oclass->passconfig & PC_PARENT_OVERRIDE_OMIT) &&
              no_override & PC_BOTTOMUP)
            output_warning(
                "class_define_map(oclass='%s',...): class '%s' suppresses "
                "parent class '%s' BOTTOMUP sync behavior by omitting override",
                oclass->name, oclass->name, oclass->parent->name);
          /*	TROUBLESHOOT
                  A class is suppressing the <i>sync</i> event implemented by
             its parent even though the parent is published with a flag that
             indicates this is unsafe.  Presumably this is deliberate, but the
             warning is given just in case it's not intended.
           */
          if (oclass->parent->passconfig & PC_UNSAFE_OVERRIDE_OMIT &&
            ...
            output_warning("class_define_map(oclass='%s',...): class '%s' "
                           "suppresses parent class '%s' POSTTOPDOWN sync "
                           "behavior by omitting override",
                           oclass->name, oclass->name, oclass->parent->name);
          /*	TROUBLESHOOT
                  A class is suppressing the <i>postsync</i> event implemented
             by its parent even though the parent is published with a flag that
             indicates this is unsafe.  Presumably this is deliberate, but the
             warning is given just in case it's not intended.
           */
          if (oclass->parent->passconfig & PC_UNSAFE_OVERRIDE_OMIT &&
            ...
            output_warning(
                "class_define_map(oclass='%s',...): class '%s' does not assert "
                "UNSAFE_OVERRIDE_OMIT when parent class '%s' does",
                oclass->name, oclass->name, oclass->parent->name);
          /*	TROUBLESHOOT
                  A class is not asserting that it is unsafe to suppress
             synchronization behavior but its parent does assert that this is
             unsafe.  This permits stealth omission by any classes that inherits
             behavior from this class and the warning is given in case this is
             not intended.
           */
          count++;
        }
      ...
      /* this test will catch use of PT_? tokens outside the context of a
         property */
      else if (prop == nullptr) {
        errno = EINVAL;
        output_error("class_define_map(oclass='%s',...): expected keyword "
                     "missing after '%s'",
                     oclass->name, class_get_property_typename(proptype));
        /*	TROUBLESHOOT
                The structure of class is being published with some special
           properties that only work in the context of a published variable.
           This is caused by a problem with the module that publishes the class.
         */
        goto Error;
      } else if (proptype == PT_KEYWORD && prop->ptype == PT_enumeration) {
        char *keyword = va_arg(arg, char *);
        int32 keyvalue = va_arg(arg, int32);
        if (!class_define_enumeration_member(oclass, prop->name, keyword,
                                             keyvalue)) {
          errno = EINVAL;
          output_error("class_define_map(oclass='%s',...): property keyword "
                       "'%s' could not be defined as value %d",
                       oclass->name, keyword, keyvalue);
          /*	TROUBLESHOOT
                  An attempt to define an <i>enumeration</i> property is using a
             value that cannot be used, either because it is already being used,
             or because it is outside of range of allowed values for that
             property.  That is caused by a problem in the module that publishes
             the class.
           */
          goto Error;
        }
        ...
        if (!class_define_set_member(oclass, prop->name, keyword, keyvalue)) {
          ...
          output_error("class_define_map(oclass='%s',...): property keyword "
                       ...
                       oclass->name, keyword, keyvalue);
          /*	TROUBLESHOOT
                  An attempt to define an <i>set</i> property is using a value
             that cannot be used, either because it is already being used, or
             because it is outside of range of allowed values for that property.
             That is caused by a problem in the module that publishes the class.
           */
          goto Error;
        }
        ...
        default:
          errno = EINVAL;
          output_error("class_define_map(oclass='%s',...): unrecognized "
                       "property access code (value=%d is not valid)",
                       oclass->name, pa);
          /*	TROUBLESHOOT
                  A class is attempting specify a type variable access that is
             unknown.  This is caused by a problem in the module that publishes
             the class.
           */
          goto Error;
          break;
        ...
        if (prop->size < 1) {
          errno = EINVAL;
          output_error("class_define_map(oclass='%s',...): property size must "
                       "be greater than 0",
                       oclass->name, proptype);
          /*	TROUBLESHOOT
                  A class is attempting to define a repeated variable (such as
             an array) that contains less than 1 item. This is caused by a
             problem in the module that published the class.
           */
          goto Error;
        }
        ...
        //				TRY {
        //					if ((prop->unit =
        //unit_find(unitspec))==nullptr) 						throw_exception("unable to define unit
        //'%s'", unitspec); 				} CATCH (const char *msg) {
        //						output_error("class_define_map(oclass='%s',...):
        //property %s unit '%s' is not recognized: %s",oclass->name,
        //prop->name,unitspec,msg);
        //						/*	TROUBLESHOOT
        //							A class is
        //attempting to publish a variable using a unit that is not defined.
        //							This is caused
        //by an incorrect unit specification in a variable publication (in C++)
        //or declaration (in GLM). 							Units are defined in the unit file located in
        //the GridLAB-D <b>etc</b> folder. 							This error immediately follows a
        //throw event with the same message.
        //						 */
        //				} ENDCATCH;
        ...
        try {
          ...
          output_error("class_define_map(oclass='%s',...): property %s unit "
                       "'%s' is not recognized: %s",
                       oclass->name, prop->name, unitspec, msg);
          /*	TROUBLESHOOT
                  A class is attempting to publish a variable using a unit that
             is not defined. This is caused by an incorrect unit specification
             in a variable publication (in C++) or declaration (in GLM). Units
             are defined in the unit file located in the GridLAB-D <b>etc</b>
             folder. This error immediately follows a throw event with the same
             message.
           */
        };
        ...
        if (prop->notify == 0) {
          errno = EINVAL;
          output_error("Unable to find function '%s' in %s module",
                       notify_fname, prop->oclass->module->name);
          goto Error;
        }
        ...
        if (strcmp(ptypestr, "//UNDEF//") == 0)
          ptypestr = tcode;
        errno = EINVAL;
        output_error("class_define_map(oclass='%s',...): unrecognized extended "
                     "property (PROPERTYTYPE=%s)",
                     oclass->name, ptypestr ? ptypestr : tcode);
        /*	TROUBLESHOOT
                A property extension given in a published class specification
           uses a property type (PT_*) is that not valid. This is caused by a
           problem in the module that publishes the class.
         */
        goto Error;
      }
    } else if (proptype ==
               PT_enduse) /// @todo class_define_map support for enduse has to
                          /// be done explicitly until structures are supported
    {
      char *name = va_arg(arg, char *);
      PROPERTYADDR addr = va_arg(arg, PROPERTYADDR);
      if (enduse_publish(oclass, addr, name) <= 0) {
        output_error("class_define_map(oclass='%s',...): substructure of "
                     "property '%s' substructure could not be published",
                     oclass->name, prop->name);
        /*	TROUBLESHOOT
                A class is publishing a property that has a substructure, which
           couldn't be published. This must be corrected in the code the
           declares the property or publishes the class.
         */
        errno = E2BIG;
        goto Error;
      }
      ...
      DELEGATEDTYPE *delegation =
          (proptype == PT_delegated ? va_arg(arg, DELEGATEDTYPE *) : nullptr);
      char *name = va_arg(arg, char *);
      PROPERTYADDR addr = va_arg(arg, PROPERTYADDR);
      if (prop != nullptr && strlen(name) >= sizeof(prop->name)) {
        output_error(
            "class_define_map(oclass='%s',...): property name '%s' is too big",
            oclass->name, name);
        /*	TROUBLESHOOT
                A class is publishing a property using a name that is too big
           for the system. Property names are limited in length. This must be
           corrected in the code the declares the property or publishes the
           class.
         */
        errno = E2BIG;
        goto Error;
      ...
      if (strcmp(name, "parent") == 0) {
        output_error("class_define_map(oclass='%s',...): property name '%s' "
                     "conflicts with built-in property",
                     oclass->name, name);
        /*	TROUBLESHOOT
                A class is attempting to publish a variable with a name normally
           reserved for object headers. This is not allowed.  If the class is
           implemented in a module, this is problem with the module. If the
           class is declared in a GLM file, you must correct the problem to
           avoid unpredictable simulation behavior.
         */
        goto Error;
...
Error:
  if (prop != nullptr)
    output_error("class_define_map(oclass='%s',...): processed up to '%s' "
                 "before encountering error",
                 oclass->name, prop->name);
  return -count;
}
 ...
 **/
int class_define_enumeration_member(
    CLASS *oclass, /**< pointer to the class which implements the enumeration */
...
/** Define a set member
 **/
int class_define_set_member(
    CLASS *oclass, /**< pointer to the class which implements the set */
    ...
    unsigned int64 value)      /**< set value to associate with the name */
{
  ...
  if (prop->keywords == nullptr)
    prop->flags |= PF_CHARSET; /* enable single character keywords until a long
                                  keyword is defined */
  key->next = prop->keywords;
  strncpy(key->name, member, sizeof(key->name));
  key->name[sizeof(key->name) - 1] =
      '\0'; /* null terminate name in case is was too long for strncpy */
  if (strlen(key->name) > 1 &&
      (prop->flags & PF_CHARSET)) /* long keyword detected */
...
/**	Define a class function.
        @return A structure with the function pointer and the function's
   published name.
 */
FUNCTION *class_define_function(CLASS *oclass, const FUNCTIONNAME functionname,
                                FUNCTIONADDR call) {
  FUNCTION *func, *tempfunc;

  // DEBUG: Log the inputs
  // fprintf(stderr, "class_define_function called with oclass: %s,
  // functionname: %s\n", 		oclass ? oclass->name : "nullptr", functionname ?
  // functionname : "nullptr");

  // Check if `oclass` is null
  if (!oclass) {
    output_error("class_define_function: oclass is nullptr. Ensure class is "
                 "registered before adding functions.");
    errno = EINVAL;
    return nullptr;
  }
  ...
  if (class_get_function(oclass->name, functionname) != nullptr) {
    output_error(
        "class_define_function(CLASS *class={name='%s',...}, FUNCTIONNAME "
        "functionname='%s', ...) the function name has already been defined",
        oclass->name, functionname);
    /* TROUBLESHOOT
  ...
  else // More than one attached
  {
    ...
    while (tempfunc->next != nullptr) {
      // fprintf(stderr, "Traversing fmap for class '%s': current function:
      ...
      {
        output_error("class_define_function: Detected corrupted function map "
                     "(fmap) during traversal for class '%s'",
                     oclass->name);
...
FUNCTIONADDR class_get_function(char *classname, const char *functionname) {
  // fprintf(stderr, "class_get_function called with classname: %s,
  ...
  for (func = oclass->fmap; func != nullptr && func->oclass == oclass;
       func = func->next) {
...
int class_saveall(FILE *fp) /**< a pointer to the stream FILE structure */
{
  ...
  count += fprintf(fp, "// classes\n");
  {
    ...
    for (oclass = class_get_first_class(); oclass != nullptr;
      ...
      if (oclass->parent)
        count +=
            fprintf(fp, "#ifdef INCLUDE_PARENT_CLASS\n\tparent %s;\n#endif\n",
                    oclass->parent->name);
      for (func = oclass->fmap; func != nullptr && func->oclass == oclass;
           func = func->next)
        count +=
            fprintf(fp, "#ifdef INCLUDE_FUNCTIONS\n\tfunction %s();\n#endif\n",
                    func->name);
      for (prop = oclass->pmap; prop != nullptr && prop->oclass == oclass;
           prop = prop->next) {
        ...
        if (ptype != nullptr) {
          if (strchr(prop->name, '.') == nullptr)
            ...
            count += fprintf(
                fp, "#ifdef INCLUDE_DOTTED_PROPERTIES\t%s %s;\n#endif\n", ptype,
                prop->name);
        }
      }
      count += fprintf(fp, "}\n");
...
void class_profiles(void) {
  ...
  output_profile("%-16.16s %7.3f %8.1f%% %8.1f\n", "Total",
                 (double)total / global_ms_per_second, 100.0,
                 1000 * (double)total / global_ms_per_second /
                     object_get_count());
}

/** Register a type delegation for a property
        @return a pointer DELEGATEDTYPE struct if successful, \p nullptr if
 delegation failed

        Type delegation is used to transform data to string and string to data
 conversion to routines implemented in a module, instead of in the core.   This
 allows custom data type to be implemented, including enumerations, sets, and
 special objects.
 **/
DELEGATEDTYPE *class_register_type(
    CLASS *oclass, /**< the object class */
    char *type,    /**< the property type */
    int (*from_string)(void *,
                       char *), /**< the converter from string to data */
    int (*to_string)(void *, char *,
                     int)) /**< the converter from data to string */
{
...
int class_add_loadmethod(CLASS *oclass, const char *name,
                         int (*call)(void *, char *)) {
...
int class_define_type(CLASS *oclass, DELEGATEDTYPE *delegation, ...) {
  output_error("delegated types not supported using class_define_type (use "
               "class_define_map instead)");
  /*	TROUBLESHOOT
                  Property delegation is not supported yet so this should never
     happen. This is most likely caused by a lack of memory or an unstable
     system.
   */
  return 0;
}
...
static int check =
    0; /* there must be a better way to do this, but this works. -MH */

/**	Writes a formatted string into a temporary buffer prior and verifies
 that enough space exists prior to writing to the destination.
        @return the number of characters written to the buffer
 **/
static int buffer_write(
    char *buffer, /**< buffer into which string is written */
    size_t len,   /**< size of the buffer into which the string is written */
    const char *format, /**< format of string to write into buffer, followed by
 ...
 **/
int class_get_xsd(
    CLASS *oclass, /**< a pointer to the class to convert to XSD */
    ...
    size_t len)    /**< the size of the buffer */
{
  ...
  struct {
    ...
    KEYWORD *keys;
      ...
      {"id", "integer", nullptr},       {"parent", "string", nullptr},
      {"rank", "integer", nullptr},     {"clock", "string", nullptr},
      {"valid_to", "string", nullptr},  {"latitude", "string", nullptr},
      {"longitude", "string", nullptr}, {"in_svc", "string", nullptr},
      {"out_svc", "string", nullptr},   {"flags", "string", oflags},
  };
  check = 1;
  ...
  for (i = 0; i < sizeof(attribute) / sizeof(attribute[0]); i++) {
    ...
    if (attribute[i].keys == nullptr) {
      ...
      n +=
          buffer_write(buffer + n, len - n, "\t\t\t\t\t\t<xs:pattern value=\"");
      for (key = attribute[i].keys; key != nullptr; key = key->next) {
        n += buffer_write(buffer + n, len - n, "%s%s",
                          key == attribute[i].keys ? "" : "|", key->name);
      }
      n += buffer_write(buffer + n, len - n, "\"/>\n");
  ...
  for (; oc != 0; oc = oc->parent) {
    for (prop = oc->pmap; prop != nullptr && prop->oclass == oc;
         prop = prop->next) {
