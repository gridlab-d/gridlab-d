...
int mti_debug(MTI *mti, const char *fmt, ...) {
  if (mti_debug_mode) {
    ...
    va_list ptr;
    va_start(ptr, fmt);
    len += fprintf(stderr, "MTIDEBUG [%s] %s : ",
                   convert_from_timestamp(global_clock, ts, sizeof(ts)) ? ts
                                                                        : "???",
                   mti ? mti->name : "(init)");
...
static MTICODE iterator_proc(MTIPROC *tp) {
  ...
  while (tp->enabled) {
    ...
    std::unique_lock<std::shared_mutex> lock(
        SharedMutexManager::get_mutex(&mti->start.lock));
    ...
    std::unique_lock<std::shared_mutex> lock2(
        SharedMutexManager::get_mutex(&mti->stop.lock));
...
MTI *mti_init(const char *name, MTIFUNCTIONS *fn, size_t minitems) {
  ...
  mti_debug_mode =
      global_getvar("mti_debug", buffer, sizeof(buffer)) ? atoi(buffer) : 0;
  mti_debug(mti, "MTI init started for %s", name);

  /* sizing pass */
  while ((item = fn->get(item)) != nullptr)
    nitems++;
  if (nitems == 0)
    return nullptr;
  ...
  if (mti->n_processes > 1) {
    ...
    for (p = 0; p < (int)(mti->n_processes); p++) {
      ...
      /*proc->enabled  = (
       * pthread_create(&proc->thread_id,nullptr,(void*(*)(void*))iterator_proc,proc)==0
       * );*/
      ...
      try {
        proc->thread = std::thread(
            [proc]() { ((MTICODE (*)(MTIPROC *))iterator_proc)(proc); });
        ...
        // Optionally log the error: std::cerr << "Thread creation failed: " <<
        // e.what() << std::endl;
      }

      mti_debug(mti, "proc=%d; enabled=%d, nitems=%d", p, proc->enabled.load(),
                proc->n_items);
    }
  } else
    mti->process = nullptr;
...
int mti_run(MTIDATA result, MTI *mti, MTIDATA input) {
  ...
  /* multi threaded update */
    ...
    // pthread_mutex_lock(mti->stop.lock);
    {
      std::unique_lock<std::shared_mutex> stop_lock(
          SharedMutexManager::get_mutex(&mti->stop.lock));
      ...
      // pthread_mutex_lock(mti->start.lock);
      {
        ...
        mti->start.cond
            .notify_all(); // Notify all threads waiting on the condition
