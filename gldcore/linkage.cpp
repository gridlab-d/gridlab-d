 ...
 **/
int linkage_create_writer(instance *inst, char *fromobj, char *fromvar,
                          char *toobj, char *tovar) {
  /* allocate linkage */
  linkage *lnk = (linkage *)malloc(sizeof(linkage));
  if (!lnk) {
    output_error("unable to allocate memory for linkage %s:%s -> %s:%s",
                 fromobj, fromvar, toobj, tovar);
    return 0;
  }
  ...
  if (!instance_add_linkage(inst, lnk)) {
    output_error("unable to attach linkage %s:%s -> %s:%s to instance %s",
                 lnk->local.obj, lnk->local.prop, lnk->remote.obj,
                 lnk->remote.prop, inst->model);
    return 0;
  } else {
    output_verbose("created write linkage from local %s:%s to remote %s:%s",
                   fromobj, fromvar, toobj, tovar);
    return 1;
  }
}
...
/** linkage_create_reader
    Add a master<-slave linkage to an instance object.
        @returns 1 on success, 0 on failure
 **/
int linkage_create_reader(instance *inst, char *fromobj, char *fromvar,
                          char *toobj, char *tovar) {
  /* allocate linkage */
  linkage *lnk = (linkage *)malloc(sizeof(linkage));
  if (!lnk) {
  ...
  if (!instance_add_linkage(inst, lnk)) {
    ...
    output_verbose("created read linkage from local %s:%s to remote %s:%s",
                   fromobj, fromvar, toobj, tovar);
    return 1;
  }
}
...
STATUS linkage_master_to_slave(char *buffer, linkage *lnk) {
  ...
  case MRM_MASTER:
    //			rv =
    //class_property_to_string(lnk->target.prop,GETADDR(lnk->target.obj,lnk->target.prop),(char
    //*)((int64)lnk->addr),size);
    rv = object_get_value_by_addr(
        lnk->target.obj, get_addr(lnk->target.obj, lnk->target.prop),
        (char *)((int64)lnk->addr), size, lnk->target.prop);
    output_debug("prop %s, addr %x, addr2 %x, val %s", lnk->target.prop->name,
                 get_addr(lnk->target.obj, lnk->target.prop),
                 (char *)((int64)lnk->addr), lnk->addr);
    break;
  case MRM_SLAVE:
    //			rv =
    //class_string_to_property(lnk->target.prop,GETADDR(lnk->target.obj,lnk->target.prop),(char
    //*)((int64)lnk->addr));
    rv = object_set_value_by_addr(lnk->target.obj,
                                  get_addr(lnk->target.obj, lnk->target.prop),
                                  (char *)((int64)lnk->addr), lnk->target.prop);
    output_debug("prop %s, addr %x, addr2 %x, val %s", lnk->target.prop->name,
                 get_addr(lnk->target.obj, lnk->target.prop),
                 (char *)((int64)lnk->addr), lnk->addr);
    break;
  ...
  if (0 == rv) {
    output_error("linkage_master_to_slave failed for link %s.%s",
                 lnk->target.obj->name, lnk->target.prop->name);
...
STATUS linkage_slave_to_master(char *buffer, linkage *lnk) {
  ...
  if (0 == rv) {
    output_error("linkage_slave_to_master failed for link %s.%s",
                 lnk->target.obj->name, lnk->target.prop->name);
    output_debug("str=%8s", (char *)((int64)lnk->addr));
    return FAILED;
  }
...
STATUS linkage_init(instance *inst, linkage *lnk) {
  ...
  /* find local property */
  lnk->target.prop =
      object_get_property(lnk->target.obj, lnk->local.prop, nullptr);
  if (!lnk->target.prop) {
    output_error("unable to find linkage source property '%s' in object '%s'",
                 lnk->local.prop, lnk->local.obj);
    return FAILED;
  }
  ...
  output_verbose("initialized linkage between local %s:%s and remote %s:%s",
                 lnk->local.obj, lnk->local.prop, lnk->remote.obj,
                 lnk->remote.prop);
  return SUCCESS;
}
