...
/**************************************************************************************
   IMPORTANT NOTE: it is vital that the platform specific implementations be
 tested for first because some compilers support __sync_bool_compare_and_swap on
 platforms that have a native (often better) implementation of atomic
 operations.
 **************************************************************************************/
#if defined(__APPLE__)
#include <libkern/OSAtomic.h>
#define atomic_compare_and_swap(dest, comp, xchg)                              \
  OSAtomicCompareAndSwap32Barrier(comp, xchg, (volatile int32_t *)dest)
...
#define atomic_compare_and_swap(dest, comp, xchg)                              \
  (_InterlockedCompareExchange((volatile long *)dest, xchg, comp) == comp)
...
#define atomic_increment(ptr)                                                  \
  __sync_add_and_fetch((volatile unsigned int *)ptr, 1)
#else
static inline unsigned int atomic_increment(unsigned int *ptr) {
  ...
  do {
    value = *(volatile long *)ptr;
      ...
      !__sync_bool_compare_and_swap((volatile long *)ptr, value, value + 1));
  return value;
}
#endif
#else
...
/** Enable lock trace
 **/
#ifdef LOCKTRACE // this code should only be used in care is mystery lock
                 // timeouts
...
void check_lock(unsigned int *lock, bool write, bool unlock) {
  ...
  do {
    ...
    if (timeout-- == 0)
      throw_exception("check lock timeout");
           ...
           !atomic_compare_and_swap(&check_lock, value, value + 1));
  ...
  if (item == nullptr) {
    ...
    //			printf("%s %slock(%p) = %d (%s) - possible damage
    //(last=%d)\n", write?"write":"read ", unlock?"un":"
    //",lock,*lock,item->name, item->last_value);
    if (no_lock) // found a registered lock that was not locked
      printf("%s %slock(%p) = %d (%s) - no lock detected (last=%d)\n",
             write ? "write" : "read ", unlock ? "un" : "  ", lock, *lock,
...
/* This locking method uses a standard spinlock.
   ...
   (2) an atomic compare-and-swap (CAS) operation is performed to take the lock
   by setting the low bit to 1 (3) if the CAS operation fails, the lock process
   starts over at (1) (4) to unlock the lock value is incremented (which clears
   the low bit and increments the lock count).
 */
/** Read lock
 **/
// extern "C"
std::shared_lock<std::shared_mutex> rlock(unsigned int *obj_ptr) {
  ...
  /*if(global_lock_enabled){
          ...
          } while ((value&1) || !atomic_compare_and_swap(lock, value, value +
  1));
  }*/
}
/** Write lock
...
//		} while ((value&1) || !atomic_compare_and_swap(lock, value,
//value + 1));
//	}
// }
/** Read unlock
...
/* This locking method uses a modified double spinlock
   ...
   (2) an atomic compare-and-swap (CAS) operation is performed to take the lock
   by setting the high bit to 1 (3) if the CAS operation fails, the lock process
   starts over at (1) (4) to unlock the lock value is set to zero (which clears
   all the bits) The write lock operation works as follows: (1) a lock is
   attempted when both the low and high bits are zero (2) an atomic CAS
   operation is performed to take the lock by setting the low bit to 1 (3) to
   unlock the lock value is set to zero (which clears all the bits)
 */
static inline void rlock(unsigned int *lock) {
  ...
  do {
    value = (*lock);
           ...
           !atomic_compare_and_swap(lock, value, value | 0x80000000));
}
static inline void wlock(unsigned int *lock) {
  ...
  do {
    value = (*lock);
  } while ((value & 0x80000001) ||
           !atomic_compare_and_swap(lock, value, value + 1));
}
static inline void _unlock(unsigned int *lock) { *lock = 0; }
...
#define runlock(lock)                                                          \
  ...
  while (lock_tmp != *(lock) || lock_tmp & 1)                                  \
    ;                                                                          \
  }
