/** $Id$
    ...
    block1-name { // each block must have a unique name
        ...
        minutes hours days months weekdays value // multiple entries separate by
...
struct s_schedule {
...
#ifdef _DEBUG
  unsigned int magic1; /* values between magic1 and magic2 should never change
                          once compiled */
#endif
  ...
  unsigned char
      *index[MAXCALENDARS]; /**< the schedule index (enough room for all 14
                               annual calendars to 1 minute resolution) */
  unsigned char *dtnext[MAXCALENDARS]; /**< the time until the next schedule
                                          change (in minutes) */
  double
  ...
  TIMESTAMP since;
  double
      duration; /**< the duration of the current scheduled value (in hours) */
  double fraction; /**< the fractional weight of the block of the current value
                      (pu time) */
...
#define SN_NORMAL 0x0001 /**< schedule normalization flag - normalize enabled  \
                          */
#define SN_ABSOLUTE                                                            \
  0x0002 /**< schedule normalization flag - use absolute values */
#define SN_WEIGHTED                                                            \
  0x0004 /**< schedule normalization flag - use weighted values */
#define SN_INTERPOLATED                                                        \
  0x0008 /**< schedule values are interpolated between defined values */
#define SN_BOOLEAN                                                             \
  0x8000 /**< schedule is boolean (only one/zero values are expected) */
#define SN_NONZERO                                                             \
  0x4000 /**< schedule is non-zero (no zero values are expected) */
#define SN_POSITIVE                                                            \
  0x2000 /**< schedule is positive (no negative values are expected) */
#define SN_IS_NORMALIZED                                                       \
  0x0020 /**< schedule normalization flag - indicates that the schedule has    \
            been nomalized already */
