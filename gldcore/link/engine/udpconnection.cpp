...
bool bindUDPSocket(ENGINELINK *given) {
  ...
  if (bind(data->sd, (const struct sockaddr *)&data->receiver,
           sizeof(data->receiver)) == SOCKET_ERROR) {
    // TODO collect error info
    gl_error("socket bind failed");
    return false;
  } else
    gl_debug("socket bind to addr %ld port %d ok",
             ntohl(data->receiver.sin_addr.s_addr),
             ntohs(data->receiver.sin_port));
...
int recvUDPSocket(ENGINELINK *engine, char *buffer, int maxlen) {
  ...
  int rv = recvfrom(data->sd, buffer, maxlen, 0,
                    (struct sockaddr *)&data->sender, (socklen_t *)&len);
  if (rv == SOCKET_ERROR) {
    // TODO collect error info
  ...
  gl_debug("incoming message from %d.%d.%d.%d/%d [%s]",
           get_addr(&data->sender, 1), get_addr(&data->sender, 2),
           get_addr(&data->sender, 3), get_addr(&data->sender, 4),
...
int sendUDPSocket(ENGINELINK *engine, char *buffer, int len) {
  ...
  gl_debug("outgoing message to %d.%d.%d.%d/%d [%s]",
           get_addr(&engine->sender, 1), get_addr(&engine->sender, 2),
           get_addr(&engine->sender, 3), get_addr(&engine->sender, 4),
           get_port(&engine->sender), buffer);
  int rv;
  // rv = sendto(engine->sd,(char*)&len,sizeof(size_t),0,(const struct
  // sockaddr*)&engine->sender,sizeof(engine->sender));
  rv = sendto(data->sd, buffer, len, 0, (const struct sockaddr *)&data->sender,
              sizeof(data->sender));
  if (rv == SOCKET_ERROR) {
    // TODO collect error info
