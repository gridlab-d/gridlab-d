...
private:
  ...
  pid_t gldpid;
#endif
  void startGLD(const char *gldexecpath, const char *args[]);
  /// options
private:
...
public:
  /// establish a local connection to a gridlabd instance, this will launch
  /// gridlabd
  void connect(ENGINE_SOCK_TYPE socktype, const char *pathToGLD,
               const char *parameters, ...);
  /// establish a local connection using UDP to a gridlabd instance, this will
  /// launch gridlabd
  void connect(const char *pathToGLD, const char *parameters, ...);

  /// establish a connectiong to a remote gridlabd instance, remote instance
  /// should be manually started!!
  void connect(ENGINE_SOCK_TYPE socktype, const string &ip,
               const int &port = 6267);
