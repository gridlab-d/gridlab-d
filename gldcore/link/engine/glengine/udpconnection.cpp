...
udpconnection::udpconnection(const string &ip, const int &port)
    : absconnection(ip, port) {
#ifdef _WIN32
  // init socket system
...
int udpconnection::recv(char *buffer, const int &size) {
  ...
  return recvfrom(sd, buffer, size, 0, (struct sockaddr *)&serv_addr,
                  (socklen_t *)&slen);
}
...
int udpconnection::send(const char *buffer, const int &size) {
  ...
  return sendto(sd, buffer, size, 0, (struct sockaddr *)&this->togld,
                sizeof(this->togld));
}
