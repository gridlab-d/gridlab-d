...
void glengine::addtocache(glproperty *given) {
  ...
  this->cacheIndex.insert(
      pair<string, pair<PROPOERTYCONTEXT, int>>(given->name, toindex));
}

glproperty *glengine::getFromCache(string name) const {
  map<string, pair<PROPOERTYCONTEXT, int>>::const_iterator it =
      this->cacheIndex.find(name);
...
int glengine::set(const string name, const string value) {
  ...
  else if (it->second.first == IMPORT)
...
void glengine::startGLD(const char *gldexecpath, const char **args) {
  ...
  if (!CreateProcess(nullptr, s, nullptr, nullptr, FALSE, 0, nullptr, nullptr,
                     &glsi, &this->gldpid)) {
  ...
  this->gldpid = fork();
  if (gldpid < 0)
    exception("fork gld process failed!") if (gldpid == 0) { // child
...
void glengine::protocolConnect() {
  ...
  debug(1, "connection open with GridLAB-D Version %d.%d.%d-%d (%s)", major,
        minor, patch, build, branch);
...
void glengine::connect(const char *pathToGLD, const char *parameters, ...) {
  ...
  this->interface =
      absconnection::getconnection(UDP, string("127.0.0.1"), 6267);
  if (interface == nullptr)
    exception("Interface creation error, unsupported socket type or socket "
              "operation error");
  protocolConnect();
}

void glengine::connect(ENGINE_SOCK_TYPE socktype, const char *pathToGLD,
  ...
  this->interface =
      absconnection::getconnection(socktype, string("127.0.0.1"), 6267);
  if (interface == nullptr)
    exception("Interface creation error, unsupported socket type or socket "
              "operation error");
...
void glengine::connect(ENGINE_SOCK_TYPE socktype, const string &ip,
                       const int &port) {
...
int glengine::send(const char *fmt, ...) {
  ...
  if (slen < 0)
    exception("unable to send message [%s] (errno=%s)", buffer,
...
bool glengine::recv_gldversion() {
  ...
  if (sscanf(buffer, "GRIDLABD %d.%d.%d (%32[^)])", &major, &minor, &patch,
             branch) != 4)
    return false;
  return true;
}
...
void glengine::send_imports() {
  for (int i = 0; i < imports_cache.size(); i++) {
    ...
    if (send("%d %s", imports_cache[i]->index,
             imports_cache[i]->value.c_str()) <= 0) {
