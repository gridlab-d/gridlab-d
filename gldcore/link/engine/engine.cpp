...
bool recv_status(ENGINELINK *engine, ENGINELINKSTATUS *status,
                 char *msg = nullptr, int maxlen = 0) {
  char buffer[1500];
  ...
  if (strcmp(buffer, "INIT") == 0) {
    if (msg != nullptr && tag != nullptr && maxlen > 1)
      strncpy(msg, tag + 1, maxlen - 1);
    *status = ELS_INIT;
    return true;
...
bool recv_init(ENGINELINK *engine) {
  ...
  if (len > 0) {
    if (strcmp(buffer, "INIT") == 0)
      ...
      gl_error("recv_init() did not receive an init message, message received "
               "was '%s'",
               buffer);
      return false;
    }
...
bool send_status(ENGINELINK *engine, ENGINELINKSTATUS status,
                 char *msg = nullptr) {
  char buffer[1500];
  int len;
...
bool recv_sync(ENGINELINK *engine, TIMESTAMP *t) {
  ...
  if (len > 0) {
    gld_clock ts(buffer);
    char timestr[64];
    if (sscanf(buffer, "SYNC %63[-0-9. :A-Z]", timestr) == 1 &&
        ts.from_string(timestr)) {
      *t = ts.get_timestamp();
      return true;
    } else if (sscanf(buffer, "SYNC %lld", (int64 *)t) == 1)
      return true;
    else {
      gl_error("recv_sync() did not receive a valid timestamp, message "
               "received was '%s'",
               buffer);
      return false;
    }
  ...
  for (item = engine->recv; item != nullptr; item = item->next) {
    ...
    if (index >= 0 && index < sync_index_size) {
      ...
      if (prop->from_string(value) <= 0) {
        gl_warning("unable to read import value '%s' for %s.%s",
                   prop->get_object()->name, prop->get_property()->name);
        true;
      }
    } else {
...
bool add_import(ENGINELINK *engine, unsigned int index,
...
bool add_export(ENGINELINK *engine, unsigned int index,
                OBJECTPROPERTY *objprop) {
  gld_property *prop = new gld_property(objprop->obj, objprop->prop);
  engine->send = add_property(engine->send, index, prop);
  char buffer[1500];
...
EXPORT bool glx_init(glxlink *mod) {
  ...
  for (LINKLIST *item = mod->get_imports(); item != nullptr;
    ...
    if (objprop != nullptr && !add_import(engine, index++, objprop)) {
      gl_error("unable to send import specs for %s.%s", objprop->obj->name,
               objprop->prop->name);
      return false;
    }
  }
  ...
  for (LINKLIST *item = mod->get_exports(); item != nullptr;
    ...
    if (objprop != nullptr && !add_export(engine, index++, objprop)) {
      gl_error("unable to send export specs for %s.%s", objprop->obj->name,
               objprop->prop->name);
      return false;
    }
  }
