...
// 	if (already_called == 0 && sig == SIGABRT && strstr(global_modelname, "_err") != nullptr)
// 	{
// 		already_called = 1;
// 		output_verbose("SIGABRT caught during error test - this may be expected");
// 		exec_setexitcode(XC_RUNERR); // Set appropriate exit code for expected error
// 		exit(XC_RUNERR);
// 	}
// 	if (sig == SIGTERM)
// 	{
...
void exec_sighandler(int sig)
{
	...
	{
		already_called = 1;
		output_verbose("Windows: SIGABRT caught during error test '%s' - this may be expected",
					   global_modelname);
		...
		_exit(XC_TSTERR); // Use _exit instead of exit to bypass cleanup that might change code
	}
	...
	else if (sig == SIGINT)
	{
		...
		if (global_debug_mode)
		{
			...
			{
				output_debug("Windows: debugger activation failed, signal processing disabled");
				debug_active = 1;
				signal(sig, nullptr);
			}
		}
...
void exec_sighandler(int sig)
{
	...
	{
		already_called = 1;
		output_verbose("Linux/WSL2: SIGABRT caught during error test '%s' - this may be expected",
					   global_modelname);
		exec_setexitcode(XC_TSTERR); // Use test error code
	...
	else if (sig == SIGINT)
	{
		...
		if (global_debug_mode)
		{
			...
			{
				output_debug("Linux/WSL2: debugger activation failed, signal processing disabled");
				debug_active = 1;
				signal(sig, nullptr);
			}
		}
...
typedef enum
{
	...
	DBG_RUN,
	DBG_NEXT
} DEBUGCMD; /**< commands that are automatically repeated when a \p nullptr command is entered */
typedef enum
{
	BP_MODULE,
	BP_CLASS,
...
static void list_object(OBJECT *obj, PASSCONFIG pass)
{
	...
	output_message("%1s%1s%1s%1s%1s%1s %4d %-24s %-16s %-16s %s",
				   ...
				   obj->name ? obj->name : convert_from_object(buf1, sizeof(buf1), &obj, nullptr) ? buf1
																								  : "(error)",
				   obj->parent ? (obj->parent->name ? obj->parent->name : convert_from_object(buf2, sizeof(buf2), &(obj->parent), nullptr) ? buf2
																																		   : "(error)")
							   : "ROOT",
				   details);
}
	...
	Retry:
		...
		else if (strncmp(cmd, "break", std::max<unsigned long>(1, strlen(cmd))) == 0)
		{
			...
			{ /* create pass breakpoint */
				int pass;
				if (strnicmp_portable(bpval, "pretopdown", std::max<unsigned long>(2, strlen(bpval))) == 0)
					pass = PC_PRETOPDOWN;
				else if (strnicmp_portable(bpval, "bottomup", strlen(bpval)) == 0)
					pass = PC_BOTTOMUP;
				else if (strnicmp_portable(bpval, "posttopdown", std::max<unsigned long>(2, strlen(bpval))) == 0)
					pass = PC_POSTTOPDOWN;
				else
				{
		...
		else if (strncmp(cmd, "set", std::max<unsigned long>(1, strlen(cmd))) == 0)
		{
			...
			int scanct = 0;
			value[0] = '\0';
			if ((scanct = sscanf(buffer, "%*s %s %s %s", objname.get_string(), propname.get_string(), value.get_string())) >= 2)
			{
				OBJECT *obj;
				if (strcmp(objname, "global") == 0)
				{
					if (global_setvar(propname, value.get_string()) == FAILED)
						output_error("unable to set global value %s", propname.get_string());
					/* TROUBLESHOOT
