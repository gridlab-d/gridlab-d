/** $Id: debug.c 4738 2014-07-03 00:55:39Z dchassin $
 ...
 Breakpoints halt the simulator and start the debugger whenever a situation
 arises that matches the breakpoint criterion.  For example, a breakpoint on the
 bottom-up pass will stop the simulation every time an object sync is called
 during a bottom-up pass.

        Watchpoints are different from breakpoints in that the debugger is only
 stopped when the value being watched changes.  For example, a breakpoint on
 node:12 voltage would only stop the simulation when that value is changed.  In
 contrast, a breakpoint on node:12 would stop each time node:12 is sync'd.

        While the debugger is running \p help will print a list of all the
 available commands.

        @section debug_start Getting started

        To start the debugger you must include the \p --debug option on the
 command-line.  Note that while the debugger is running, the system will only
 operate in single-threaded mode.

        Each time the debugger stops to prompt for input, it displays the
 current simulation time and simulator status.  The status include which pass is
 currently running (see PASSCONFIG), which rank is being processed, which object
 is about to be updated, and which iteration is being run (if the time has not
 advanced yet).
        ...
        Debugging commands may be abbreviated to the extent that they are
 unambiguous.  For example \p b may be used instead of \p break, but \p wa must
 be used for \p watch to distinguish it from \p where.
        ...
        - \p t indicates that the object has yet to be called on the PRETOPDOWN
 pass
        - \p T indicates that the object has already been called on the
 PRETOPDOWN pass In the third character:
        - \p - indicates that the object is not called on the BOTTOMUP pass
        - \p b indicates that the object has yet to be called on the BOTTOMUP
 pass
        - \p B indicates that the object has already been called on the BOTTOMUP
 pass In the fourth character:
        - \p - indicates that the object is not called on the POSTTOPDOWN pass
        - \p t indicates that the object has yet to be called on the POSTTOPDOWN
 pass
        - \p T indicates that the object has already been called on the
 POSTTOPDOWN pass In the fifth character:
        ...
        The third field is the object's internal clock (or \p INIT) if the
 object has not yet been sync'd.

        The fourth field is the name (\p class:\p id) of the object.

        The fifth field is the name of the object's parent (or \p ROOT) if is
 has none.

        @subsection debug_print

        To inspect the properties of an object, you can use the \p print
 command.  With no option, the current object is printed:
        ...
        When an object name (\p class:\p id) is provided, that object is
 printed:
        ...
        You can run a script containing debug commands using the \p script
 command:
...
static int debug_active =
    1; /**< flag indicating that the debugger is currently active */
static int error_caught =
    0; /**< flag indicating that the debugger has seen an error */
static int watch_sync =
    0; /**< flag indicating that sync times should be reported */
static int sigint_caught =
    0; /**< flag indicating that \p SIGINT has been caught */
static int sigterm_caught =
    0; /**< flag indicating that \p SIGTERM has been caught */
static int list_details =
    0; /**< flag indicating that listing includes details */
static int list_unnamed =
    1; /**< flag indicating that listing includes unnamed objects */
static int list_inactive =
    1; /**< flag indicating that listing includes inactive objects */
static int list_sync =
    1; /**< flag indicating that listing includes objects that have syncs */
...
void debug_notify_error(void) { error_caught = 1; }
...
// 	// If we're processing an assertion error test, handle differently
// 	if (already_called == 0 && sig == SIGABRT && strstr(global_modelname,
// "_err") != nullptr)
// 	{
// 		already_called = 1;
// 		output_verbose("SIGABRT caught during error test - this may be
// expected"); 		exec_setexitcode(XC_RUNERR); // Set appropriate exit
// code for expected error 		exit(XC_RUNERR);
// 	}
// 	if (sig == SIGTERM)
// 	{
...
// halts 			the current simulation.  This is not normal, but
// may be intentional. 			If it is not intentional, make sure that
// the signal is not being 			raised accidentally by another
// application.
// 		 */
...
// 			else
// 			{
// 				output_debug("debugger activation failed, signal
// processing disabled"); 				debug_active = 1;
// signal(sig, nullptr);
// 			}
// 		}
// 		else
...
void exec_sighandler(int sig) {
  static int already_called = 0;
  output_debug("Windows: signal %s caught", strsignal(sig));

  // Special handling for assertion error tests
  if (already_called == 0 && sig == SIGABRT &&
      strstr(global_modelname, "_err") != nullptr) {
    already_called = 1;
    output_verbose(
        "Windows: SIGABRT caught during error test '%s' - this may be expected",
        global_modelname);
    exec_setexitcode(XC_TSTERR); // Use test error code
    // For Windows, we must ensure the exit code is properly returned
    // Windows tends to lose exit codes in signals without explicit handling
    _exit(XC_TSTERR); // Use _exit instead of exit to bypass cleanup that might
                      // change code
  }

  // Regular signal handling
  if (sig == SIGTERM) {
    output_error("Windows: received SIGTERM; exiting now");
    /* TROUBLESHOOT
     ...
     */
    _exit(XC_SIGNAL | SIGTERM); // Use _exit for Windows
  } else if (sig == SIGABRT) {
    output_error("Windows: received SIGABRT; exiting now");
    /* TROUBLESHOOT
            A signal from the operating system was caught, which aborts
            the current simulation.   This is not normal, but may be
       intentional. If it is not intentional, make sure that the signal is not
       being raised accidentally by another application.
     */
    _exit(XC_SIGNAL | SIGABRT); // Use _exit for Windows
  } else if (sig == SIGINT) {
    sigint_caught = 1;
    if (global_debug_mode) {
      if (debug_active == 0) {
        output_debug("Windows: debugger activated");
        debug_active = 1;
      } else {
        output_debug(
            "Windows: debugger activation failed, signal processing disabled");
        debug_active = 1;
        signal(sig, nullptr);
      }
    } else
    ...
    output_error("Windows: ignored signal %d", sig);
    /* TROUBLESHOOT
            A signal was received that is not handled.  This is not serious, but
            may indicate that a behavior was expected by another application
...
void exec_sighandler(int sig) {
  static int already_called = 0;
  output_debug("Linux/WSL2: signal %s caught", strsignal(sig));

  // Special handling for assertion error tests
  if (already_called == 0 && sig == SIGABRT &&
      strstr(global_modelname, "_err") != nullptr) {
    already_called = 1;
    output_verbose("Linux/WSL2: SIGABRT caught during error test '%s' - this "
                   "may be expected",
                   global_modelname);
    exec_setexitcode(XC_TSTERR); // Use test error code
    exit(XC_TSTERR);             // Standard exit is fine on Linux
  }

  // Regular signal handling
  if (sig == SIGTERM) {
    output_error("Linux/WSL2: received SIGTERM; exiting now");
    /* TROUBLESHOOT
            A signal from the operating system was caught, which halts
            the current simulation.  This is not normal, but may be intentional.
            If it is not intentional, make sure that the signal is not being
    ...
    output_error("Linux/WSL2: received SIGABRT; exiting now");
    /* TROUBLESHOOT
            A signal from the operating system was caught, which aborts
            the current simulation.   This is not normal, but may be
       intentional. If it is not intentional, make sure that the signal is not
    ...
    if (global_debug_mode) {
      if (debug_active == 0) {
        output_debug("Linux/WSL2: debugger activated");
        debug_active = 1;
      } else {
        output_debug("Linux/WSL2: debugger activation failed, signal "
                     "processing disabled");
        debug_active = 1;
        signal(sig, nullptr);
      }
    ...
    output_error("Linux/WSL2: ignored signal %d", sig);
    /* TROUBLESHOOT
            A signal was received that is not handled.  This is not serious, but
            may indicate that a behavior was expected by another application
       that GridLAB-D does not support.  If this is unexpected, make sure that
...
typedef enum {
  ...
  DBG_RUN,
  DBG_NEXT
} DEBUGCMD; /**< commands that are automatically repeated when a \p nullptr
               command is entered */
typedef enum {
  BP_MODULE,
  BP_CLASS,
  ...
  BP_CLOCK,
  BP_ERROR
} BREAKPOINTTYPE; /**< breakpoint types that are supported */
...
static BREAKPOINT *first_breakpoint =
                      nullptr,  /**< pointer to the first breakpoint */
    *last_breakpoint = nullptr; /**< pointer to the last breakpoint */
static int breakpoint_count =
    0; /**< the number of breakpoints defined so far */

/** This function adds a breakpoint to the list of breakpoints that examined by
 *the debugger
 **/
static int
exec_add_breakpoint(BREAKPOINTTYPE type, /**< the breakpoint type */
...
static WATCHPOINT *first_watchpoint =
                      nullptr,  /**< a pointer to the first watchpoint */
    *last_watchpoint = nullptr; /**< a pointer to the last watchpoint */
static int watchpoint_count =
    0; /**< the number of watchpoints defined so far */

/** This function adds a watchpoint to the watchpoint list
 **/
static int
exec_add_watchpoint(OBJECT *obj,    /**< the object being watched */
                    PROPERTY *prop) /**< the property being watched */
{
  ...
  if (prop == nullptr)
    ...
    strcpy(wp->buffer,
           object_property_to_string(obj, prop->name, buffer, 1023));
  wp->next = nullptr;
...
static void list_object(OBJECT *obj, PASSCONFIG pass) {
  ...
  if (list_unnamed == 0 && obj->name == nullptr)
    return;
  if (list_inactive == 0 &&
      (global_clock < obj->in_svc || global_clock > obj->out_svc))
    return;
  ...
  output_message(
      "%1s%1s%1s%1s%1s%1s %4d %-24s %-16s %-16s %s",
      global_clock < obj->in_svc
          ? "P"
          : (global_clock < obj->out_svc
                 ? "A"
                 : "R"), /* service status: P=planned, A=active, R=retired */
      (obj->oclass->passconfig & PC_PRETOPDOWN)
          ? (pass < PC_PRETOPDOWN ? "t" : "T")
          : "-", /* pre-topdown sync status */
      (obj->oclass->passconfig & PC_BOTTOMUP) ? (pass < PC_BOTTOMUP ? "b" : "B")
                                              : "-", /* bottom-up sync status */
      (obj->oclass->passconfig & PC_POSTTOPDOWN)
          ? (pass < PC_POSTTOPDOWN ? "t" : "T")
          : "-", /* post-topdown sync status */
      (obj->flags & OF_LOCKED) == OF_LOCKED ? "l"
                                            : "-", /* object lock status */
      (obj->flags & OF_HASPLC) == OF_HASPLC ? "x" : "-", /* object PLC status */
      obj->rank,
      obj->clock > 0
          ? (convert_from_timestamp(obj->clock, buf3, sizeof(buf3)) ? buf3
                                                                    : "(error)")
          : "INIT",
      obj->name                                                ? obj->name
      : convert_from_object(buf1, sizeof(buf1), &obj, nullptr) ? buf1
                                                               : "(error)",
      obj->parent
          ? (obj->parent->name ? obj->parent->name
             : convert_from_object(buf2, sizeof(buf2), &(obj->parent), nullptr)
                 ? buf2
                 : "(error)")
          : "ROOT",
      details);
...
int load_from_file = 0; /** flag whether commands are being read from a \p
                           script file (see #load_fp) */

/** The function executes the next debug command provided by the user or read
 *from a file
 **/
DEBUGCMD
exec_debug_cmd(std::shared_ptr<struct sync_data>
               ...
               OBJECT *obj) /**< the current object being processed */
{
  ...
  output_debug("time %s\r",
               convert_from_timestamp(global_clock, tmp, sizeof(tmp))
                   ? tmp
                   : "(invalid)");
  output_debug("pass %s, rank %d, object %s, iteration %d",
               passtype[pass] == PC_PRETOPDOWN
                   ? "PRETOPDOWN"
                   : (passtype[pass] == PC_BOTTOMUP
                          ? "BOTTOMUP"
                          : (passtype[pass] == PC_POSTTOPDOWN ? "POSTTOPDOWN"
                                                              : "(invalid)")),
  ...
  Retry:
    ...
    if (load_from_file == 0) {
      ...
      result =
          fgets(buffer, 1023,
                stdin); /* "gets() is dangerous and should not be used" -gcc */
    ...
    if (strncmp(cmd, "quit", std::max<unsigned long>(1, strlen(cmd))) == 0)
      return DBG_QUIT;
    else if (strncmp(cmd, "run", std::max<unsigned long>(1, strlen(cmd))) ==
                 0 ||
             (strlen(cmd) == 0 && last == DBG_RUN)) {
      output_debug("resuming simulation, Ctrl-C interrupts");
      debug_active = 0;
      return last = DBG_RUN;
    } else if (strncmp(cmd, "next", std::max<unsigned long>(1, strlen(cmd))) ==
                   0 ||
               (strlen(cmd) == 0 && last == DBG_NEXT)) {
      return last = DBG_NEXT;
    } else if (strncmp(cmd, "module",
                       std::max<unsigned long>(1, strlen(cmd))) == 0) {
      char modname[128];
    ...
    } else if (strncmp(cmd, "namespace",
                       std::max<unsigned long>(2, strlen(cmd))) == 0) {
      char space[1024];
    ...
    } else if (strncmp(cmd, "list", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      char lclass[256] = "";
      OBJECT *obj = object_get_first();
      sscanf(buffer, "%*s %s", lclass);
      /* output_message("%-16.16s %4s %6s %-16.16s %-32.32s",	"Object
         name","Rank","Status","Parent","Clock"); output_message("%-16.16s %4s
         %6s %-16.16s %-32.32s",
         "----------------","----","------","----------------","--------------------------------");
       */
      ...
      /* TROUBLESHOOT
              ...
              and try again.
       */
    } else if (strncmp(cmd, "nsync", std::max<unsigned long>(2, strlen(cmd))) ==
               0) {
      char cmd[1024];
      int n = sscanf(buffer, "%*s %s", cmd);
      if (n == 1) {
      ...
      /* TROUBLESHOOT
              ...
              and try again.
       */
    } else if (strncmp(cmd, "script",
                       std::max<unsigned long>(1, strlen(cmd))) == 0) {
      char load_filename[_MAX_PATH + 32]; /* Made it a little longer than
                                             max_path to be safe */
      if (sscanf(buffer, "%*s %[^\x20]", load_filename) <=
...
#endif
    } else if (strncmp(cmd, "break", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      char bptype[256] = "";
      char bpval[256] = "";
      if (sscanf(buffer, "%*s %s %s", bptype, bpval) == 0) {
        ...
        for (bp = first_breakpoint; bp != nullptr; bp = bp->next) {
          ...
          switch (bp->type) {
          case BP_MODULE:
            output_debug("breakpoint %2d - module %s %s", bp->num,
                         bp->mod->name, bp->enabled ? "" : "(disabled)");
            break;
          case BP_CLASS:
            output_debug("breakpoint %2d - class %s %s", bp->num,
                         bp->oclass->name, bp->enabled ? "" : "(disabled)");
            break;
          case BP_OBJECT:
            output_debug("breakpoint %2d object %s %s", bp->num,
                         get_objname(bp->obj), bp->enabled ? "" : "(disabled)");
            break;
          case BP_PASS:
            output_debug(
                "breakpoint %2d pass %s %s", bp->num,
                (int)bp->pass == PC_PRETOPDOWN
                    ? "pretopdown"
                    : ((int)bp->pass == PC_BOTTOMUP
                           ? "bottomup"
                           : ((int)bp->pass == PC_POSTTOPDOWN ? "posttopdown"
                                                              : "(invalid)")),
                bp->enabled ? "" : "(disabled)");
            break;
          case BP_RANK:
            output_debug("breakpoint %2d rank %d %s", bp->num, (int)bp->rank,
                         bp->enabled ? "" : "(disabled)");
            break;
          case BP_TIME:
            output_debug("breakpoint %2d time %s (%" FMT_INT64 "d) %s", bp->num,
                         convert_from_timestamp(bp->ts, tmp, sizeof(tmp))
                             ? tmp
                             : "(invalid)",
                         bp->ts, bp->enabled ? "" : "(disabled)");
            break;
          case BP_CLOCK:
            output_debug("breakpoint %2d clock %s", bp->num,
                         bp->enabled ? "" : "(disabled)");
            break;
          case BP_ERROR:
            output_debug("breakpoint %2d on error", bp->num,
                         bp->enabled ? "" : "(disabled)");
            break;
          default:
            break;
        ...
        /* TROUBLESHOOT
                ...
                and try again.
         */
      } else if (strncmp(bptype, "object", strlen(bptype)) ==
                 0) { /* create object breakpoint */
        OBJECT *obj = object_find_name(bpval);
        if (obj != nullptr || convert_to_object(bpval, &obj, nullptr)) {
          if (!exec_add_breakpoint(BP_OBJECT,
                                   (int64)obj)) /* warning: cast from pointer to
                                                   integer of different size */
            output_error("unable to add object breakpoint");
          /* TROUBLESHOOT
                  The <b>break <i>object</i></b> could not be completed because
             of a problem with the internal operation needed to perform it.
                  Follow the guidance for the message which precedes this
         ...
         */
      } else if (strncmp(bptype, "module", strlen(bptype)) ==
                 0) { /* create module breakpoint */
        MODULE *mod = module_find(bpval);
        if (mod != nullptr) {
          if (!exec_add_breakpoint(BP_MODULE,
                                   (int64)mod)) /* warning: cast from pointer to
                                                   integer of different size */
         ...
         */
      } else if (strncmp(bptype, "class", strlen(bptype)) ==
                 0) { /* create class breakpoint */
        CLASS *oclass = class_get_class_from_classname(bpval);
        if (oclass != nullptr) {
          if (!exec_add_breakpoint(
                  BP_CLASS, (int64)oclass)) /* warning: cast from pointer to
                                               integer of different size */
      ...
      } else if (strncmp(bptype, "pass", strlen(bptype)) ==
                 0) { /* create pass breakpoint */
        int pass;
        if (strnicmp_portable(bpval, "pretopdown",
                              std::max<unsigned long>(2, strlen(bpval))) == 0)
          pass = PC_PRETOPDOWN;
        else if (strnicmp_portable(bpval, "bottomup", strlen(bpval)) == 0)
          pass = PC_BOTTOMUP;
        else if (strnicmp_portable(bpval, "posttopdown",
                                   std::max<unsigned long>(2, strlen(bpval))) ==
                 0)
          pass = PC_POSTTOPDOWN;
        else {
          ...
          /* TROUBLESHOOT
                  ...
                  valid.  Only <b>pretopdown</b>, <b>bottomup</b>, and
             <b>posttopdown</b> are supported.
           */
          continue;
        }
        ...
        /* TROUBLESHOOT
                ...
                and try again.
         */
      } else if (strncmp(bptype, "rank", strlen(bptype)) ==
                 0) { /* create rank breakpoint */
         ...
         */
      } else if (strncmp(bptype, "time", strlen(bptype)) ==
                 0) { /* create time breakpoint */
        ...
        if (strcmp(bpval, "") != 0) {
          if (isdigit(bpval[0]))
            ...
            /* TROUBLESHOOT
                    The <b>break disable</b> command specified a breakpoint
               number that doesn't exist.  Check the list of breakpoints and try
               again.
             */
            continue;
          }
       ...
       */
    } else if (strncmp(cmd, "watch", std::max<unsigned long>(2, strlen(cmd))) ==
               0) {
      char wptype[256] = "";
      char wpval[256] = "";
      OBJECT *obj;
      if (sscanf(buffer, "%*s %s %s", wptype, wpval) == 0) {
        ...
        for (wp = first_watchpoint; wp != nullptr; wp = wp->next) {
          if (wp->prop == nullptr)
            output_debug("watchpoint %d - object %s %s", wp->num,
                         get_objname(wp->obj), wp->enabled ? "" : "(disabled)");
          else
            output_debug("watchpoint %d - object %s %s - property %s", wp->num,
                         get_objname(wp->obj), wp->enabled ? "" : "(disabled)",
                         wp->prop->name);
        }
        ...
        if (strcmp(wpval, "") != 0) {
          if (isdigit(wpval[0]))
            ...
            /* TROUBLESHOOT
                    The <b>watch</b> command refers to a watchpoint that doesn't
               exist. Check the list of watchpoints and try again.
             */
            continue;
          }
        }
      ...
      } else if ((obj = object_find_name(wptype)) ||
                 convert_to_object(wptype, &obj, nullptr)) {
        if (strcmp(wpval, "") != 0) {
          PROPERTY *prop = object_get_property(obj, wpval, nullptr);
          if (prop == nullptr)
            output_error("object %s does not have a property named '%s'",
                         wptype, wpval);
          /* TROUBLESHOOT
                  The <b>watch</b> command refers to an object property that
             isn't defined. Check the list of properties for that class of
      ...
      /* TROUBLESHOOT
              ...
              Check the command syntax and try again.
       */
    } else if (strncmp(cmd, "where", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      char ts[64];
      output_debug("Global clock... %s (%" FMT_INT64 "d)",
                   convert_from_timestamp(global_clock, ts, sizeof(ts))
                       ? ts
                       : "(invalid)",
                   global_clock);
      output_debug("Hard events.... %d", data->hard_event);
      output_debug("Sync status.... %s",
                   data->status == FAILED ? "FAILED" : "SUCCESS");
      output_debug("Step to time... %s (%" FMT_INT64 "d)",
                   convert_from_timestamp(data->step_to, ts, sizeof(ts))
                       ? ts
                       : "(invalid)",
                   data->step_to);
    ...
    } else if (strncmp(cmd, "print", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      char tmp[4096];
      if (sscanf(buffer, "%*s %s", tmp) == 0) {
        ...
        /* TROUBLESHOOT
                The <b>print</b> was unable to complete the dump of the
           specified object because of an internal error. Follow the guidance
           for the message that precedes this message and try again.
         */
      } else {
      ...
      for (var = global_getnext(nullptr); var != nullptr;
        ...
        if (val != nullptr && strlen(val) > 64)
          strcpy(val + 28, "...");
        output_message("%-32.32s: \"%s\"", var->prop->name,
                       val == nullptr ? "(error)" : val);
      }
    } else if (strncmp(cmd, "set", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      char256 objname;
      char256 propname;
      char256 value;
      int scanct = 0;
      value[0] = '\0';
      if ((scanct = sscanf(buffer, "%*s %s %s %s", objname.get_string(),
                           propname.get_string(), value.get_string())) >= 2) {
        OBJECT *obj;
        if (strcmp(objname, "global") == 0) {
          if (global_setvar(propname, value.get_string()) == FAILED)
            output_error("unable to set global value %s",
                         propname.get_string());
          /* TROUBLESHOOT
                  The <b>set</b> debug command was unable to set the specified
             global variable because of an internal error. Follow the guidance
             for the message that precedes this message and try again.
           */
        } else if ((obj = object_find_name(objname)) ||
                   convert_to_object(objname, &obj, nullptr)) {
          if (!object_set_value_by_name(obj, propname, value))
            output_error("unable to set value of object %s property %s",
                         objname.get_string(), propname.get_string());
          /* TROUBLESHOOT
                  The <b>set</b> debug command was unable to set the value given
             could not be converted to the type required for the property. Check
             the command syntax and try again.
           */
          else if (!object_get_property(obj, propname, nullptr))
            output_error("invalid property for object %s",
                         objname.get_string());
          /* TROUBLESHOOT
                  This error should not occur and in harmless.
                  If it does occur, please report the problem to the core
             development team.
           */
        } else
        ...
        /* TROUBLESHOOT
                The <b>set</b> debug command was unable to set the specified
           property either because the object could not be found, or the value
           given could not be converted to the type required for the property.
           Check the command syntax and try again.
         */
      } else {
        output_error("set syntax error (%i items)", scanct);
        /* TROUBLESHOOT
                The <b>set</b> syntax doesn't have the correct number of
           arguments. Check the command syntax and try again.
         */
      }
    } else if (strncmp(cmd, "find ", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      FINDLIST *fl = nullptr;
      OBJECT *obj = nullptr;
      ...
      if (exec_cmd("gdb --quiet %s --pid=%d", global_execname,
                   global_process_id) <= 0)
        output_debug("unable to start gdb");
    } else if (strncmp(cmd, "help", std::max<unsigned long>(1, strlen(cmd))) ==
               0) {
      output_debug(
          ...
          "   script <filename> read a script file containing debugger "
          "commands\n"
          "   nsync on|off      enables/disables listing of non-synchronizing "
          "objects\n"
          ...
          "         <X> <Y>     stop the simulation if object X property Y "
          "changes\n"
 ...
 **/
int exec_debug(std::shared_ptr<struct sync_data>
                   &data,   /**< the current sync status of the mail loop */
               int pass,    /**< the current pass on the main loop */
               int index,   /**< the rank index */
               OBJECT *obj) /**< the current object being processed */
{
  ...
  if (debug_active == 0) {
    ...
    if (convert_from_timestamp(global_clock, buffer, sizeof(buffer)) &&
        strcmp(buffer, timebuf) != 0) {
      output_raw("DEBUG: global_clock = '%s' (%" FMT_INT64 "d)\r", buffer,
                 global_clock);
    ...
    for (wp = first_watchpoint; wp != nullptr; wp = wp->next) {
      ...
      if (obj == wp->obj) {
        if (wp->prop == nullptr) {
          char tmp[65536];
          if (object_dump(tmp, sizeof(tmp), obj) &&
              strcmp(tmp, wp->buffer) != 0) {
            output_debug("watchpoint %d stopped on object %s", wp->num,
                         get_objname(obj));
            output_debug("%s", tmp);
          ...
          char temp[1024];
          char *tmp =
              object_property_to_string(obj, wp->prop->name, temp, 1023);
          if (tmp != nullptr && strcmp(tmp, wp->buffer) != 0) {
            output_debug("watchpoint %d stopped on object %s property %s",
                         wp->num, get_objname(obj), wp->prop->name);
    ...
    for (bp = first_breakpoint; bp != nullptr; bp = bp->next) {
      ...
      if (bp->type == BP_ERROR && error_caught) {
        output_debug("breakpoint %d (error) stopped on object %s", bp->num,
                     get_objname(obj));
        error_caught = 0;
        debug_active = 1;
      }
      if (bp->type == BP_MODULE && bp->mod == obj->oclass->module) {
        output_debug("breakpoint %d (module %s) stopped on object %s", bp->num,
                     bp->mod->name, get_objname(obj));
        debug_active = 1;
      } else if (bp->type == BP_CLASS && bp->oclass == obj->oclass) {
        output_debug("breakpoint %d (class %s) stopped on object %s", bp->num,
                     bp->oclass->name, get_objname(obj));
        debug_active = 1;
      } else if (bp->type == BP_OBJECT && bp->obj == obj) {
        char buf1[1024];
        strcpy(buf1, get_objname(bp->obj));
        output_debug("breakpoint %d (object %s) stopped on object %s", bp->num,
                     buf1, get_objname(obj));
        debug_active = 1;
      } else if (bp->type == BP_RANK && bp->rank == (int64)obj->rank) {
        output_debug("breakpoint %d (rank %d) stopped on object %s", bp->num,
                     (int)bp->rank, get_objname(obj));
        debug_active = 1;
      } else if (bp->type == BP_PASS && bp->pass == (int64)pass) {
        output_debug("breakpoint %d (pass %d) stopped on object %s", bp->num,
                     (int)bp->pass, get_objname(obj));
        debug_active = 1;
      } else if (bp->type == BP_TIME && global_clock >= bp->ts) {
        char tmp[64];
        output_debug(
            "breakpoint %d (time %s (%d)) stopped on object %s:%" FMT_INT64 "d",
            bp->num,
            convert_from_timestamp(bp->ts, tmp, sizeof(tmp)) ? tmp
                                                             : "(invalid)",
            bp->ts, obj->oclass->name, obj->id);
        debug_active = 1;
      } else if (bp->type == BP_CLOCK && global_clock != bp->ts) {
        char tmp[64];
        output_debug(
            "breakpoint %d (clock %s (%d)) stopped on clock update", bp->num,
            convert_from_timestamp(bp->ts, tmp, sizeof(tmp)) ? tmp
                                                             : "(invalid)",
            bp->ts);
  ...
  if (global_clock < obj->in_svc) {
    if (debug_active)
      output_debug("%s not in service yet", get_objname(obj));
    ...
    if (debug_active || watch_sync) {
      ...
      convert_from_timestamp(this_t < 0 ? -this_t : this_t, buffer,
                             sizeof(buffer));
      output_debug("%s next sync %s %s", get_objname(obj), buffer,
                   this_t < 0 ? "(soft sync)" : "");
    }
  } else {
    if (debug_active)
      output_debug("%s out of service", get_objname(obj));
  ...
  if (this_t < global_clock) {
    char b[64];
    output_error("%s: object %s stopped its clock (debug)!", simtime(),
    ...
    if (error_caught)
      ...
      data->status = FAILED; /* the debugger isn't going to handle the error so
                                make exec handle it */
  } else {
    ...
    if (iteration_counter == 2 && this_t == global_clock) {
      ...
      output_error("convergence iteration limit reached for object %s (debug)",
                   get_objname(obj));
      /* TROUBLESHOOT
