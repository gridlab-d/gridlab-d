...
/* TODO: Fix "method" - was missing from list and causing segfaults, so
 * populated.  No idea if it works or what it does */
PROPERTYSPEC property_type[_PT_LAST] = {
    {"void", "string", 0, 0, convert_from_void, convert_to_void},
    {
        ...
        convert_to_double,
        nullptr,
        reinterpret_cast<size_t (*)(FILE *, int, void *, PROPERTY *)>(
            stream_double),
        {TCOPS(double)},
    },
    {"complex",
     ...
     complex_get_part},
    {
        ...
        sizeof(int32),
        32,
        convert_from_enumeration,
        convert_to_enumeration,
        nullptr,
        nullptr,
        {TCOPS(uint64)},
    },
    ...
    {"object",
     ...
     {TCOPB(object)},
     object_get_part},
    {"delegated", "string", (unsigned int)-1, 0, convert_from_delegated,
     convert_to_delegated},
    {
        "bool",
        "string",
        sizeof(bool),
    ...
    {"complex_array",
     "string",
     sizeof(Eigen::MatrixXcd),
     0,
     convert_from_complex_array,
     convert_to_complex_array,
     nullptr,
     nullptr,
     {TCNONE},
     nullptr},
    {"real", "decimal", sizeof(real_type), 24, convert_from_real,
     convert_to_real},
    {"float", "decimal", sizeof(float), 24, convert_from_float,
     convert_to_float},
    {
        "loadshape",
        "string",
        sizeof(loadshape),
        0,
        convert_from_loadshape,
        reinterpret_cast<int (*)(const char *, void *, PROPERTY *)>(
            convert_to_loadshape),
        reinterpret_cast<int (*)(void *)>(loadshape_create),
        nullptr,
        {TCOPS(double)},
    },
    {"enduse",
     ...
     reinterpret_cast<int (*)(const char *, void *, PROPERTY *)>(
         convert_to_enduse),
     reinterpret_cast<int (*)(void *)>(enduse_create),
     nullptr,
     {TCOPS(double)},
     enduse_get_part},
    {"randomvar",
     "string",
     sizeof(randomvar_struct),
     24,
     convert_from_randomvar,
     reinterpret_cast<int (*)(const char *, void *, PROPERTY *)>(
         convert_to_randomvar),
     reinterpret_cast<int (*)(void *)>(randomvar_create),
     nullptr,
     {TCOPS(double)},
     random_get_part},
...
int property_check(void) {
  ...
  for (ptype = _PT_FIRST, ++ptype; ptype < _PT_LAST; ++ptype) {
    ...
    case PT_double:
      ...
      break;
    case PT_complex:
      sz = sizeof(gld::complex);
      break;
    case PT_enumeration:
      sz = sizeof(enumeration);
      break;
    case PT_set:
      sz = sizeof(gld::set);
      break;
    ...
    case PT_double_array:
      sz = sizeof(Eigen::MatrixXd);
      break;
    case PT_complex_array:
      sz = sizeof(Eigen::MatrixXcd);
      break;
    case PT_real:
      sz = sizeof(real_type);
      break;
...
PROPERTY *property_malloc(PROPERTYTYPE proptype, CLASS *oclass,
  ...
  if (prop == nullptr) {
    output_error("property_malloc(oclass='%s',...): memory allocation failed",
                 oclass->name, name);
    /*	TROUBLESHOOT
            This means that the system has run out of memory while trying to
       define a class.  Trying freeing up some memory by unloading applications
       or configuring your system so it has more memory.
     */
    errno = ENOMEM;
    goto Error;
  }
  ...
  if (sscanf(name.data(), "%[^[][%[^]]]", prop->name, unitspec) == 2) {
    ...
    if (prop->ptype != PT_double && prop->ptype != PT_complex)
      output_error("property_malloc(oclass='%s',...): property %s cannot have "
                   "unit '%s' because it is not a double or complex value",
                   oclass->name, prop->name, unitspec);
    /*	TROUBLESHOOT
            Only <b>double</b> and <b>complex</b> properties can have units.
            Either change the type of the property or remove the unit
       specification from the property's declaration.
     */
    ...
    /* verify that the requested unit exists or can be derived */
      ...
      if ((prop->unit = unit_find(unitspec)) == nullptr)
        throw_exception("property_malloc(oclass='%s',...): property %s unit "
                        ...
                        oclass->name, prop->name, unitspec);
      /*	TROUBLESHOOT
              A class is attempting to publish a variable using a unit that is
         not defined. This is caused by an incorrect unit specification in a
         variable publication (in C++) or declaration (in GLM). Units are
         defined in the unit file located in the GridLAB-D <b>etc</b> folder.
       */
    }
  }
  prop->addr = addr;
  ...
  // 	output_warning("property_malloc(oclass='%s',...): property name '%s' is
  // defined more than once", oclass->name, prop->name);
  /*	TROUBLESHOOT
          A class is attempting to publish a variable more than once.
          This is caused by an repeated specification for a variable publication
     (in C++) or declaration (in GLM).
   */
  return prop;
Error:
  free(prop);
...
double complex_get_part(void *x, const char *name) {
  ...
  if (strcmp(name, "mag") == 0)
    return c->Mag(); // complex_get_mag(*c);
  if (strcmp(name, "arg") == 0)
    return c->Arg(); // complex_get_arg(*c);
  if (strcmp(name, "ang") == 0)
    return c->Arg() * 180 / PI; // (complex_get_arg(*c)*180/PI);
  return QNAN;
}
