...
#define PC_AUTOLOCK                                                            \
  0x200 /**< used to flag that sync operations should not be automatically     \
           write locked */
#define PC_OBSERVER                                                            \
  0x400 /**< used to flag whether commit process needs to be delayed with      \
           respect to ordinary "in-the-loop" objects */
...
#define SET_DEL(set, value)                                                    \
  (set = (value ^ SET_MASK) & set) /*SET_HAS(set,value)?set^value:set) */
#define SET_CLEAR(set) (set = 0)
#define SET_HAS(set, value) (set & value)

typedef enum {
  CLASSVALID = 0xc44d822e
} CLASSMAGIC; /* this is used to uniquely identify classes */
...
PROPERTY *class_add_extended_property(CLASS *oclass, char *name,
                                      PROPERTYTYPE ptype, char *unit);
...
CLASS *class_register(MODULE *module, const CLASSNAME classname,
                      unsigned int datasize, PASSCONFIG passconfig);
int class_define_map(CLASS *oclass, ...);
int class_define_enumeration_member(CLASS *oclass, const char *property_name,
                                    const char *member, enumeration value);
int class_define_set_member(CLASS *oclass, const char *property_name,
                            const char *member, unsigned int64 value);
...
DELEGATEDTYPE *class_register_type(CLASS *oclass, char *type,
