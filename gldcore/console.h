...
inline void refresh(void) { /* currently unbuffered output */ }
...
Retry:
  while (GetNumberOfConsoleInputEvents(keyboard, &n) && n == 0) {
    Sleep(20);
    ftime(&t1);
    dt = ((double)t1.time + (double)t1.millitm / 1e3) -
         ((double)t0.time + (double)t0.millitm / 1e3);
    if (dt * 10 >= delay)
      return ERR;
  }
  if (n > 0) {
    INPUT_RECORD input;
    n = -1;
    if (!ReadConsoleInput(keyboard, &input, (DWORD)1, &n))
      goto Error;
