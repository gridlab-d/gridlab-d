...
static bool edit_bool(int row, int col, int len, PROPERTY *prop) {
  ...
  while (true) {
    ...
    if (key == KEY_ESC)
      return false;
    if (key == 'Y' || key == 'T' || key == '1' || key == KEY_DOWN)
      value = true;
    if (key == 'N' || key == 'F' || key == '0' || key == KEY_UP)
      value = false;
...
static bool edit_in_place(int row, int col, int len, PROPERTY *prop) {
  mvprintw(row, col - 1, "[");
  if (prop->width > 0 && prop->width < (size_t)len)
    len = prop->width;
  if (col + len > width - 2)
    len = width - 2 - col;
...
static int edit_globals(void) {
  ...
  if (var == nullptr) { // load the var list
    GLOBALVAR *v = nullptr;
  ...
  case KEY_ENTER:
    if (var[sel]->prop->access == PA_PUBLIC)
      edit_in_place(4 + sel - first, nsize + 1, width - nsize - 1,
                    var[sel]->prop);
    break;
  default:
...
extern "C" int setup(int argc, char *argv[]) {
  ...
  while (!done) {
    ...
    mvprintw(0, 0, "GridLAB-D %d.%d.%d-%d (%s) Setup Editor",
             ...
             global_version_build, global_version_branch);
