                  ...
                  double tol)
{
    ...
    else if (a.is_array())
    {
        ...
        for (size_t i = 0; i < max_sz; i++)
        {
            ...
            if (i >= a.size())
                diffs.push_back({child, "<missing>", b[i].dump()});
            else if (i >= b.size())
                diffs.push_back({child, a[i].dump(), "<missing>"});
            else
                compare_json(a[i], b[i], child, diffs, tol);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        if (checkpoint_mode)
            output_file = base + "_partial_checkpoint.json";
        else if (restore_mode)
            output_file = base + "_restored_checkpoint.json";
        else
            output_file = base + "_full_checkpoint.json";
    }
    ...
    for (auto &a : args)
        test_argv.push_back(const_cast<char *>(a));
