...
size_t stream_compress(char *buf, size_t len) {
    ...
    if (state == RAW) {
      ...
      if (runlen == 8) {
        ...
        if (rawlen > runlen) {
          ...
          if (fwrite(&rawlen, 1, sizeof(rawlen), fp) < 0)
            return -1;
          if (fwrite(raw, 1, rawlen, fp) < 0)
            return -1;
          count += rawlen + 2;
        }
      ...
      if (dp == diff) // run continues
      {
        ...
        if (runlen == 32767) {
          ...
          // dump run buffer
          if (fwrite(&runlen, 1, sizeof(runlen), fp) < 0)
            return -1;
          if (fwrite(&diff, 1, sizeof(diff), fp) < 0)
            return -1;
          if (fputc(*run, fp) < 0)
            return -1;
          count += 4;
  ...
  rawlen = 0;
  if (fwrite(&rawlen, sizeof(rawlen), 1, fp) < 0)
    return -1;
  else
    count += 2;

  // stream len confirmation code
  if (fwrite(&count, sizeof(count), 1, fp) < 0)
    return -1;
  else
    count += 2;

  output_debug("stream_compress(): %d kB -> %d kB (%.1f%%)",
               original / 1000 + 1, count / 1000 + 1,
               (double)count * 100 / (double)original);
  return count;
}
...
size_t stream_decompress(char *buf, const size_t len) {
  ...
  if (buflen > len)
    return stream_error(
        "stream_decompress(): stream overrun--possible invalid stream");

  // read confirmation code
  if (fread(&confirm, sizeof(confirm), 1, fp) == 1 && confirm == count)
    return count;
  else
    return stream_error("stream_decompress(): stream confirmation code "
                        "mismatched--probable invalid stream");
}
...
size_t stream(void *ptr,   ///< pointer to buffer
              ...
              void *match) ///< optional match when reading (mismatch causes an
                           ///< const char* exception)
{
  ...
  if (flags & SF_IN) {
    ...
    for (i = 0; i < a; i++) {
      int b = fgetc(fp);
      if (b == '\\' && fscanf(fp, "%02x", &b) < 1)
        throw -1;
      ((char *)ptr)[i] = (char)b;
    }
    while (fgetc(fp) != '\n') {
    }
    if (match != nullptr && memcmp(ptr, match, a) != 0)
      throw 0;
...
void stream(CLASS *oclass, PROPERTY *prop) {
  ...
  for (n = 0; n < count; n++) {
    ...
    char unit[64];
    if (prop)
      strcpy(unit, prop->unit ? prop->unit->name : "");
    ...
    if (flags & SF_OUT)
      prop = prop->next;
    if (flags & SF_IN)
      class_add_extended_property(oclass, name, ptype, unit);
  }
  stream("/RTC");
}
...
void stream(CLASS *oclass) {
  ...
  for (n = 0; n < count; n++) {
    ...
    if (oclass)
      passconfig = oclass->passconfig;
    stream(passconfig);

    if (flags & SF_IN)
      oclass = class_register(nullptr, name, size, passconfig);
    ...
    if (flags & SF_IN)
      module_load(oclass->name, 0, nullptr);
  }
  stream("/RTC");
}
...
void stream(OBJECT *obj) {
  ...
  for (n = 0; n < count; n++) {
    char cname[64];
    if (obj)
      strcpy(cname, obj->oclass->name);
    stream(cname, sizeof(cname));

    unsigned int size;
    if (obj)
      size = sizeof(OBJECT) + obj->oclass->size;
    stream(&size, sizeof(size));

    char oname[64];
    if (obj)
      strcpy(oname, obj->name ? obj->name : "");
...
void stream(GLOBALVAR *var) {
  ...
  for (n = 0; n < count; n++) {
    PROPERTYNAME name;
    if (var)
      strcpy(name, var->prop->name);
    stream(name, sizeof(name));

    char value[1024];
    if (var)
      global_getvar(name, value, sizeof(value));
...
size_t stream(FILE *fileptr, int opts) {
  ...
  try {
    ...
    // runtime classes
    try {
      stream(class_get_first_runtime());
    } catch (int) {
    };

    // modules
    try {
      stream(module_get_first());
    } catch (int) {
    }

    // objects
    try {
      stream(object_get_first());
    } catch (int) {
    };

    // globals
    try {
      stream(global_getnext(nullptr));
    } catch (int) {
    };
    ...
    output_debug("done processing stream on file %d with options %x",
                 fileno(fp), flags);
    return stream_pos;
  } catch (const char *msg) {
    output_error("stream() unexpected %s at offset %lld", msg,
                 (int64)stream_pos);
    return -1;
  } catch (...) {
...
#define stream_type(T)                                                         \
  extern "C" size_t stream_##T(void *ptr, size_t len, PROPERTY *prop) {        \
    return stream((T *)ptr, len);                                              \
  }
