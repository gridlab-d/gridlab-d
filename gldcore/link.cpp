...
int link_initall(void) {
  ...
  for (mod = glxlink::get_first(); mod != nullptr; mod = mod->get_next()) {
    ...
    if (mod->get_globals() == nullptr) {
      ...
      while ((var = global_getnext(var)) != nullptr) {
        if (var->prop != nullptr && var->prop->name != nullptr) {
          ...
          output_warning(
              "link_initall(): a variable property definition is null");
      }
    } else {
      ...
      for (item = mod->get_globals(); item != nullptr;
        ...
        if (item->data == nullptr)
          output_error("link_initall(target='%s'): global '%s' is not found",
                       mod->get_target(), item->name);
      }
    }
    ...
    if (mod->get_objects() == nullptr) {
      ...
      OBJECT *obj = nullptr;
      for (obj = object_get_first(); obj != nullptr;
        ...
        if (obj->name != nullptr)
          ...
          char id[256];
          snprintf(id, sizeof(id), "%s:%d", obj->oclass->name, obj->id);
    ...
    for (item = mod->get_exports(); item != nullptr;
      ...
      if (sscanf(item->name, "%[^.].%s %s", objname, propname, varname) == 3) {
        ...
        if (objprop->obj) {
          ...
          output_error(
              "link_initall(target='%s'): export '%s' object not found",
              mod->get_target(), item->name);
      } else
        output_error("link_initall(target='%s'): '%s' is not a valid export "
                     "specification",
                     mod->get_target(), item->name);
    }

    // link imports
    for (item = mod->get_imports(); item != nullptr;
         item = mod->get_next(item)) {
      char objname[64], propname[64], varname[64];
      if (sscanf(item->name, "%[^.].%s %s", objname, propname, varname) == 3) {
        OBJECTPROPERTY *objprop =
        ...
        if (objprop->obj) {
          ...
          if (objprop->prop == nullptr)
            output_error(
                "link_initall(target='%s'): import '%s' property not found",
                mod->get_target(), item->name);
          else {
            item->data = objprop;
            strcpy(item->name, varname);
          ...
          output_error(
              "link_initall(target='%s'): import '%s' object not found",
              mod->get_target(), item->name);
      } else
        output_error("link_initall(target='%s'): '%s' is not a valid import "
                     "specification",
                     mod->get_target(), item->name);
    }
  ...
  while (fgets(line, sizeof(line), fp) != nullptr) {
    ...
    if (sscanf(line, "%s %[^\r\n]", tag, data) > 0) {
      ...
      if (settag != nullptr) {
        ...
        if (!set_target(data)) {
          output_error("%s(%d): target '%s' is not valid", filename, linenum,
                       data);
          ok = false;
        }
      } else
        output_warning("%s(%d): tag '%s' cannot be processed until target "
                       "module is loaded",
                       filename, linenum, tag);
    }
  }
...
bool glxlink::set_target(char *name) {
  char libname[1024];
  char path[1024];
  snprintf(libname, sizeof(libname), PREFIX "glx%s" DLEXT, name);
  if (find_file(libname, nullptr, X_OK | R_OK, path, sizeof(path)) != nullptr) {
    // load library
    handle = DLLOAD(path);
    if (handle == nullptr) {
      output_error("unable to load '%s' for target '%s': %s", path, name,
                   DLERR);
      return false;
    }
    ...
    // call create routine
    bool (*create)(glxlink *, CALLBACKS *) =
        (bool (*)(glxlink *, CALLBACKS *))DLSYM(handle, "glx_create");
    if (create != nullptr && create(this, module_callbacks())) {
      strcpy(target, name);
      return true;
    } else {
      output_error("library '%s' for target '%s' does not define/export "
                   "glx_create properly",
                   path, name);
      return false;
    }
