...
typedef enum {
  ...
  MH_CLASS = 0x5d37,
  MH_MODULE = 0xb40f
} MEXHANDLETYPE; /* number carefully chosen to mean nothing */
...
#define ERROR "(error)"
#define NONE "(none)"
  char *fnames[1024] = {"id",      "class",    "parent",    "rank",
                        "clock",   "latitude", "longitude", "in_svc",
                        "out_svc", "flags",    nullptr}; // };
  int nFields = 0;
  int nData = 0;
  char value[1024];
  PROPERTY *prop;
  mxArray *pId = mxCreateString(
      convert_from_object(value, sizeof(value), &obj, nullptr) ? value : ERROR);
  mxArray *pClass = mxCreateString(obj->oclass->name);
  mxArray *pParent = mxCreateString(
      obj->parent != nullptr &&
              convert_from_object(value, sizeof(value), &(obj->parent), nullptr)
          ? value
          : NONE);
  mxArray *pRank = mxCreateNumericMatrix(1, 1, mxUINT32_CLASS, mxREAL);
  mxArray *pClock = mxCreateString(
      convert_from_timestamp(obj->clock, value, sizeof(value)) ? value : ERROR);
  mxArray *pLatitude = mxCreateString(
      convert_from_latitude(obj->latitude, value, sizeof(value)) ? value
                                                                 : NONE);
  mxArray *pLongitude = mxCreateString(
      convert_from_longitude(obj->longitude, value, sizeof(value)) ? value
                                                                   : NONE);
  mxArray *pInSvc = mxCreateString(
      convert_from_timestamp(obj->in_svc, value, sizeof(value)) ? value
                                                                : ERROR);
  mxArray *pOutSvc = mxCreateString(
      convert_from_timestamp(obj->out_svc, value, sizeof(value)) ? value
                                                                 : ERROR);
  mxArray *pFlags =
      mxCreateString(convert_from_set(value, sizeof(value), (void *)&obj->flags,
  ...
  *(OBJECTRANK *)mxGetPr(pRank) = obj->rank;
  ...
  while (fnames[nFields] != nullptr)
    nFields++;

  /* count the number of object properties and assign the field names */
  for (prop = class_get_first_property(obj->oclass); prop != nullptr;
  ...
  for (prop = class_get_first_property(obj->oclass); prop != nullptr;
       nFields++, prop = class_get_next_property(prop)) {
    mxArray *pValue;
    if (prop->ptype == PT_double) {
      ...
      *(double *)mxGetPi(pValue) = pData->i;
      ...
      pValue = mxCreateString(
          object_get_value_by_name(obj, prop->name, value, sizeof(value))
              ? value
              : ERROR);
    }
...
static void set_object_data(const mxArray *data) {
  ...
  char id[256];
  if (pId == nullptr)
    output_error("set_object_data(const mxArray *data={...}) did not find a "
                 "required object id field");
  else if (mxGetString(pId, id, sizeof(id)))
    output_error("set_object_data(const mxArray *data={...}) couldn't read the "
                 "object id field");
  else if ((obj = object_find_name(id)) == nullptr)
    output_error("set_object_data(const mxArray *data={id='%s',...}) couldn't "
                 "find object id",
                 id);
  else {
    int i;
    for (i = 0; i < mxGetNumberOfFields(data); i++) {
      ...
      if ((name = mxGetFieldNameByNumber(data, i)) == nullptr)
        output_error("set_object_data(const mxArray *data={id='%s',...}) "
                     "couldn't read the name of field %d",
                     id, i);
      else if (strcmp(name, "id") == 0 ||
               strcmp(name, "class") == 0) { /* these may not be changed */
      } else if (pField == nullptr)
        output_error("set_object_data(const mxArray *data={id='%s',...}) "
                     "couldn't read the object field '%s' for object '%s'",
                     id, name);
        ...
        output_error("set_object_data(const mxArray *data={id='%s',...}) "
                     "couldn't use error value '%s'",
                     id, value);
      else if (strcmp(value, NONE) == 0 && strcpy(value, "") == nullptr)
        output_error("set_object_data(const mxArray *data={id='%s',...}) "
                     "couldn't clear empty value '%s'",
                     id, value);
      else if (!object_set_value_by_name(obj, name, value))
        output_error("set_object_data(const mxArray *data={id='%s',...}) "
                     "couldn't read the value '%s' into property '%s'",
                     id, value, name);
    }
  }
}
                      ...
                      int nrhs, const mxArray *prhs[]) /**< () */
{
  ...
  char *fields[] = {"name",    "class", "parent", "flags", "location",
                    "service", "rank",  "clock",  "handle"};
  FINDLIST *list = nullptr;
  if (nrhs > 0 && mxGetString(prhs[0], criteria, sizeof(criteria)) != 0)
    output_error(
        "gl('list',type='object'): unable to read search criteria (arg 2)");
  else if (nrhs > 0 && (search = find_mkpgm(criteria)) == nullptr)
    output_error("gl('list',type='object'): unable to run search '%s'",
                 criteria);
  else if (search == nullptr &&
           (list = find_objects(nullptr, nullptr)) == nullptr)
  ...
  else if ((plhs[0] = mxCreateStructMatrix(list->hit_count, 1,
                                           sizeof(fields) / sizeof(fields[0]),
                                           fields)) == nullptr)
    output_error(
        "gl('list',type='object'): unable to allocate memory for result list");
  else {
    unsigned int n;
    for (n = 0, obj = find_first(list); obj != nullptr;
         n++, obj = find_next(list, obj)) {
      char tmp[1024];
      mxArray *data;
      double *pDouble;
      unsigned int *pInt;
      ...
      mxSetFieldByNumber(
          plhs[0], n, 2,
          mxCreateString(obj->parent ? object_name(obj->parent) : NONE));
      mxSetFieldByNumber(
          plhs[0], n, 3,
          mxCreateString(convert_from_set(tmp, sizeof(tmp), &(obj->flags),
                                          object_flag_property())
                             ? tmp
                             : ERROR));
      pDouble = mxGetPr(data = mxCreateDoubleMatrix(1, 2, mxREAL));
      pDouble[0] = obj->longitude;
      pDouble[1] = obj->latitude;
      mxSetFieldByNumber(plhs[0], n, 4, data);
      pDouble = mxGetPr(data = mxCreateDoubleMatrix(1, 2, mxREAL));
      pDouble[0] = (double)obj->in_svc / TS_SECOND;
      pDouble[1] = (double)obj->out_svc / TS_SECOND;
      mxSetFieldByNumber(plhs[0], n, 5, data);
      pInt = (unsigned int *)mxGetPr(
          data = mxCreateNumericMatrix(1, 1, mxINT32_CLASS, mxREAL));
      pInt[0] = obj->rank;
      mxSetFieldByNumber(plhs[0], n, 6, data);
      pDouble = mxGetPr(data = mxCreateDoubleMatrix(1, 1, mxREAL));
      pDouble[0] = (double)obj->clock / TS_SECOND;
                 ...
                 int nrhs, const mxArray *prhs[]) /**< (name,value) */
{
  ...
  if (nlhs != 0)
    ...
    if (mxIsChar(prhs[1])) {
      ...
      if (mxGetString(prhs[1], buffer, 1024) &&
          global_create(name, PT_char1024, buffer, PT_SIZE, 1, nullptr) ==
              nullptr)
        output_error("unable to register string variable '%s' in globals",
                     name);
      ...
      if (global_create(name, PT_double, x, PT_SIZE, size, nullptr) == nullptr)
        output_error("unable to register double array variable '%s' in globals",
                     name);
      ...
      if (global_create(name, PT_complex, x, PT_SIZE, size, nullptr) == nullptr)
        output_error(
            "unable to register complex array variable '%s' in globals", name);
      free(x);
 ...
 **/
void cmex_create(
    int nlhs, mxArray *plhs[],       /**< {properties} */
    int nrhs, const mxArray *prhs[]) /**< (class, property1, value1, property2,
  ...
  /* copy properties */
    ...
    for (i = 0; i < num_properties; i++) {
      ...
      if (!mxIsChar(p[0]))
        ...
        output_error("property %s (arg %d) couldn't be set to '%s'", name, n,
                     value);
                 ...
                 int nrhs, const mxArray *prhs[]) /**< (modulename) */
{
  if (nrhs > 0) {
    ...
    if (!mxIsChar(prhs[0]))
    ...
    else if ((mod = module_find(fname)) == nullptr &&
             (mod = module_load(fname, 0, nullptr)) == nullptr)
      output_error("Module load failed");
    else if (nlhs = 0)
      output_message("Module '%s(%d.%d)' loaded ok", mod->name, mod->major,
                     mod->minor);
    else {
      /* set the standard info */
      char *fnames[256] = {"handle", "name", "major", "minor"};
      mxArray *name = mxCreateString(mod->name);
      mxArray *handle = mxCreateNumericMatrix(
          1, 1, sizeof(int32) == sizeof(int) ? mxUINT32_CLASS : mxUINT64_CLASS,
          mxREAL);
      ...
      *pMinor = (unsigned char)mod->minor;
      ...
      while (module_getvar(mod, varname, nullptr, 0)) {
        char32 buffer;
        if (module_getvar(mod, varname, buffer, sizeof(buffer)) &&
            nFields < sizeof(fname) / sizeof(fname[0])) {
          double *pVal;
...
void cmex_set(int nlhs, mxArray *plhs[], /**< () */
              int nrhs,
              const mxArray *prhs[]) /**< (object_id,property_name,value) */
{
  if (nlhs > 0)
    ...
    output_error("set requires either a structure, or object id, property name "
                 "and value");
  else if (!mxIsChar(prhs[0]))
    ...
    if (mxGetString(prhs[0], value, sizeof(value)) != 0)
      ...
      output_error("property name (arg 1) %s not found in object %s:%d", value,
      ...
      if (prop->ptype == PT_double &&
          object_set_double_by_name(obj, prop->name, v) == 0)
              ...
              int nrhs, const mxArray *prhs[]) /**< (name) */
{
  if (nrhs > 0) {
    ...
    if (!mxIsChar(prhs[0]))
      ...
      mxArray *pTimestring = mxCreateString(
      ...
      *(double *)mxGetPr(pTimestamp) = ((double)global_clock) / TS_SECOND;
      plhs[0] = mxCreateStructMatrix(1, 1, sizeof(fnames) / sizeof(fnames[0]),
                                     fnames);
      ...
      if (mxGetString(prhs[1], name, sizeof(name)) != 0)
        ...
        if (sscanf(name, "%[^.].%s", classname, propname) == 2) {
          ...
          if (pClass) {
            ...
            if (pProp) {
              char *fields[] = {"class",  "name", "type",       "size",
                                "access", "unit", "delegation", "keywords"};
              int fn = 0;
              mxArray *oclass = mxCreateString(classname);
              mxArray *prop = mxCreateString(pProp->name);
              mxArray *type =
                  mxCreateString(class_get_property_typename(pProp->ptype));
              mxArray *size = mxCreateDoubleMatrix(1, 1, mxREAL);
              mxArray *access =
                  mxCreateString("(na)"); /** @todo implement get_property
                                             access info (ticket #187) */
              mxArray *unit = mxCreateString(pProp->unit->name);
              mxArray *delegation = mxCreateString(
                  pProp->delegation ? pProp->delegation->oclass->name
                                    : "(none)");
              mxArray *keywords =
                  mxCreateString("(na)"); /** @todo implement get_property
                                             keywords (ticket #188) */
              *(mxGetPr(size)) = pProp->size == 0 ? 1 : pProp->size;
              plhs[0] = mxCreateStructMatrix(
                  1, 1, sizeof(fields) / sizeof(fields[0]), fields);
              ...
              output_error("property %s is not found in class %s", propname,
                           classname);
          } else
      ...
      if (var == nullptr)
        ...
        memcpy(mxGetPr(plhs[0]), (void *)var->prop->addr,
               sizeof(double) * size);
        ...
        output_error("cannot retrieve globals that are of type %s",
                     class_get_property_typename(var->prop->ptype));
...
static CMDMAP cmdMap[] = {
    ...
    {"version", cmex_version, "Get GridLAB version information",
     "\tgl('version')\n"
     "  Returns the major, minor, and build numbers of the installed version "
     "of GridLAB.\n"},
    ...
    {"create", cmex_create, "Create an object",
     "\tgl('create',class, property1,value1, property2,value2, ..., "
     "propertyN,valueN)\n"
     "  Returns the object structure\n"},
          ...
          for (j = 0; j < sizeof(cmdMap) / sizeof(cmdMap[0]); j++) {
            if (strcmp(cmd, cmdMap[j].name) == 0) {
              output_raw("Help for command '%s'\n\n%s\n", cmd,
                         cmdMap[j].detail ? cmdMap[j].detail
