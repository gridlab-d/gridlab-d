/** $Id: loadshape.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        @par MT_PULSED
        <code>loadshape_create(s,MT_PULSED,double energy, double scalar,
MACHINEPULSETYPE pulsetype, double pulsevalue)</code> where #energy is the total
energy used over the schedule definition, #scalar is the number of pulses
generated over the schedule definition, #pulsetype is the type of pulse
generated (fixed duration or fixed power), and #pulsevalue is the value of the
fixed part of the pulse.
...
fixed part of the pulse.
        ...
        @par MT_QUEUED
        <code>loadshape_create(s,MT_PULSED,double energy, double scalar,
MACHINEPULSETYPE pulsetype, double pulsevalue, double q_on, double q_off)</code>
        where #energy is the total energy used over the schedule definition,
#scalar is the number of pulses generated over the schedule definition,
#pulsetype is the type of pulse generated (fixed duration or fixed power),
#pulsevalue is the value of the fixed part of the pulse, #q_on is the pulse
queue value at which the pulses are started, and #q_off is the pulse queue value
at which the pulses are stopped.
...
static void sync_pulsed(loadshape *ls, double dt) {
  ...
  TurnOff:
    ...
    if (ls->schedule->value > 0) {
      ...
      ls->r = ls->schedule->value * ls->params.pulsed.scalar /
              (ls->params.pulsed.energy);
      if (ls->r < 0)
        output_warning("loadshape %s: r not positive while load is off!",
                       ls->schedule->name);
    }
    ...
    /* zero value scheduled */
      ...
      ls->r = 0;
      output_warning("loadshape %s: pulsed shape suspended because schedule "
                     "has zero value",
                     ls->schedule->name);
    }
  }
  ...
  TurnOn:
    ...
    if (ls->params.pulsed.pulsetype == MPT_POWER) {
      ...
      if (ls->r >= 0)
        output_warning("loadshape %s: r not negative while load is on!",
                       ls->schedule->name);
    } else if (ls->params.pulsed.pulsevalue != 0) {
      /* load has fixed duration so power is energy/duration */
      ls->load =
          ls->params.pulsed.energy /
          (ls->params.pulsed.pulsevalue / 3600 * ls->params.pulsed.scalar) *
          ls->dPdV;
      ...
      /* can't have load now */
      output_warning("loadshape %s: load value is zero in 'on' state",
                     ls->schedule->name);
    }
  }
}
    ...
    if (ls->schedule->value > 0) {
      if (ls->params.modulated.modulation == MMT_AMPLITUDE) {
        ...
        double duty_cycle = (ls->params.modulated.pulsetype == MPT_TIME)
                                ...
                                : ls->params.modulated.energy * 3600 /
                                      ls->params.modulated.pulsevalue / period;
      ...
      // pulse-width modulation
        ...
        double power = (ls->params.modulated.pulsetype == MPT_TIME)
                           ...
                           : ls->params.modulated.pulsevalue;
        double period = ls->schedule->duration / ls->params.modulated.scalar;
        double ton = ls->schedule->value * ls->params.modulated.scalar /
                     ls->params.modulated.energy / ls->params.modulated.scalar;
      ...
      // frequency modulation
        ...
        if (ls->params.modulated.pulsetype == MPT_TIME)
          power = ls->params.modulated.pulseenergy *
                  ls->params.modulated.scalar / ton * 3600;
        else
          ton = ls->params.modulated.pulseenergy * ls->params.modulated.scalar /
                power * 3600;
        dutycycle = ls->schedule->value / ls->params.modulated.energy /
                    ls->params.modulated.scalar;
        ...
        output_warning("loadshape %s: modulation type is not determined!",
                       ls->schedule->name);
    } else {
      ls->r = 0;
      output_warning("loadshape %s: modulated shape suspended because schedule "
                     "has zero value",
                     ls->schedule->name);
    }
  }
  ...
  TurnOn:
    ...
    // pulse-width modulation
      ...
      double power = (ls->params.modulated.pulsetype == MPT_TIME)
                         ...
                         : ls->params.modulated.pulsevalue;
      double pulsecount =
          ls->params.modulated.energy / power * ls->schedule->duration / 3600;
      double period = ls->schedule->duration / pulsecount;
      double ton = ls->schedule->value * ls->params.modulated.scalar /
                   ls->params.modulated.energy / pulsecount;
      ls->r = -3600 / ton;
      ls->load = power;
    }

    // frequency modulation
    else if (ls->params.modulated.modulation ==
             MMT_FREQUENCY) // frequency modulation
    {
      double ton = ls->params.modulated.pulsevalue;
      double power = ls->params.modulated.pulsevalue;
      if (ls->params.modulated.pulsetype == MPT_TIME)
        power = ls->params.modulated.pulseenergy * ls->params.modulated.scalar /
                ton * 3600;
      else
        ton = ls->params.modulated.pulseenergy * ls->params.modulated.scalar /
              power * 3600;
...
static void sync_queued(loadshape *ls, double dt) {
  ...
  if (ls->params.queued.pulsetype == MPT_POWER)
    ...
    ls->load = ls->s * ls->params.queued.energy / ls->params.queued.pulsevalue /
               ls->params.queued.scalar * ls->dPdV;
...
#define duration ((ls->params.queued.energy * queue_value) / ls->load)
  ...
  if (ls->q > ls->d[0]) {
    ...
    ls->s = static_cast<MACHINESTATE>(0);
    ls->r = 1 / random_exponential(&(ls->rng_state),
                                   ls->schedule->value *
                                       ls->params.pulsed.scalar * queue_value);
  }
  ...
  if (t1 >= ls->t2) {
    ...
    if (ls->t2 == TS_ZERO) {
      ...
      if (!local_datetime(t1, &now))
        throw_exception("unable to determine schedule loadshape initial state: "
                        "time is not valid");
      ...
      if (hour < ls->params.scheduled.on_time) {
        ...
        dt = ls->params.scheduled.on_time - hour;
                 ...
                 ls->params.scheduled.on_time +
                     (ls->params.scheduled.high - ls->params.scheduled.low) /
                         ls->params.scheduled.on_ramp) {
        ls->s = MS_RAMPUP;
        ls->q = ls->params.scheduled.low;
        ls->r = skipday ? 0 : ls->params.scheduled.on_ramp;
        dt = hour - ls->params.scheduled.on_time +
             (ls->params.scheduled.high - ls->params.scheduled.low) /
                 ls->params.scheduled.on_ramp;
        ...
        dt = hour - ls->params.scheduled.off_time;
                 ...
                 ls->params.scheduled.off_time - ls->params.scheduled.on_time -
                     (ls->params.scheduled.high - ls->params.scheduled.low) /
                         ls->params.scheduled.on_ramp) {
        ls->s = MS_RAMPDOWN;
        ls->q = skipday ? ls->params.scheduled.low : ls->params.scheduled.high;
    ...
    /* state change now */
      ...
      case MS_OFF:
        ...
        ls->q = ls->params.scheduled.low;
        ls->s = MS_RAMPUP;
        dt = (ls->params.scheduled.high - ls->params.scheduled.low) /
             ls->params.scheduled.on_ramp;
        break;
      case MS_RAMPUP:
        ls->r = 0;
        ls->q = ls->params.scheduled.low;
        ls->s = MS_ON;
        dt = (ls->params.scheduled.off_time - ls->params.scheduled.on_time -
              (ls->params.scheduled.high - ls->params.scheduled.low) /
                  ls->params.scheduled.on_ramp);
        break;
      case MS_ON:
        ...
        ls->q = skipday ? ls->params.scheduled.low : ls->params.scheduled.high;
        ls->s = MS_RAMPDOWN;
        dt = (ls->params.scheduled.low - ls->params.scheduled.high) /
             ls->params.scheduled.off_ramp;
        break;
      case MS_RAMPDOWN:
        ls->r = 0;
        ls->q = skipday ? ls->params.scheduled.low : ls->params.scheduled.high;
        ls->s = MS_OFF;
        dt =
            (24 - ls->params.scheduled.off_time + ls->params.scheduled.on_time);
        break;
      default:
        dt = 0;
        break;
...
/** Convert a scheduled loadshape weekday parameter to string representing the
 weekdays (UMTWRFSH)
        @return A string containing bits found in the bitfield (U=0, M=1, T=2,
 W=3, R=4, F=5, S=6, H=7)
 **/
...
/** Convert a string representing weekdays (UMTWRFSH) to a scheduled loadshape
 parameter
    @return the days found in the string as a bitfield (U=0, M=1, T=2, W=3, R=4,
 F=5, S=6, H=7)
 **/
...
/** Convert a truncated normal distribution description string
 (min<mean~stdev<max) to a sample
...
void loadshape_recalc(loadshape *ls) {
  ...
  case MT_PULSED:
    ls->d[MS_OFF] =
        1; /* scalar determine how many pulses per period are emitted */
    ls->d[MS_ON] = 0;
  ...
  case MT_QUEUED:
    ...
    if (ls->s == 0 && ls->schedule != nullptr) /* load is off */
    {
      ...
      ls->r = 1 / random_exponential(
                      &(ls->rng_state),
                      ls->schedule->value * ls->params.pulsed.scalar *
                          (ls->params.queued.q_on - ls->params.queued.q_off));
    }
    break;
  case MT_SCHEDULED:
...
int loadshape_init(loadshape *ls) /**< load shape */
{
  ...
  if (ls->schedule == nullptr && ls->type != MT_SCHEDULED) {
    output_warning(
        "loadshape_init(): a loadshape without a schedule is meaningless");
    ls->t2 = TS_NEVER;
    return 0;
  }
  ...
  case MT_ANALOG:
    if (ls->params.analog.energy < 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "analog energy must be a positive number",
                   ls->schedule->name);
      return 1;
    } else if (ls->params.analog.power < 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "analog power must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.analog.power > 0.0 && ls->params.analog.energy > 0.0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "analog schedules cannot set both power and energy",
                   ls->schedule->name);
      return 1;
    }
    break;
  case MT_PULSED:
    if (ls->params.pulsed.energy <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "pulsed energy must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.pulsed.pulsetype == MPT_UNKNOWN) {
      output_error(
          "loadshape_init(loadshape *ls={schedule->name='%s',...}) pulse type "
          "could not be inferred because either duration or power is missing",
          ls->schedule->name);
      return 1;
    }
    if (ls->params.pulsed.pulsetype == MPT_TIME &&
        ls->params.pulsed.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "pulse duration must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.pulsed.pulsetype == MPT_POWER &&
        ls->params.pulsed.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "pulse power must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.pulsed.scalar <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "pulse count must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    break;
  case MT_MODULATED:
    if (ls->params.modulated.energy <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulated energy must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (static_cast<int>(ls->params.modulated.pulsetype) ==
        static_cast<int>(MT_UNKNOWN)) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulated pulse type could not be inferred because either "
                   "duration or power is missing",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.modulated.pulsetype == MPT_TIME &&
        ls->params.modulated.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulated pulse period must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.modulated.pulsetype == MPT_POWER &&
        ls->params.modulated.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulated pulse power must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.modulated.scalar <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulated pulse count must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.modulated.pulseenergy <= 0) {
      if (ls->params.modulated.pulseenergy == 0 &&
          ls->params.modulated.pulsevalue == 0) {
        output_error(
            "loadshape_init(loadshape *ls={schedule->name='%s',...}) either "
            "modulated pulse or count must be a positive number",
            ls->schedule->name);
        return 1;
      } else if (ls->params.modulated.pulseenergy < 0) {
        output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                     "modulated pulse must be a positive number",
                     ls->schedule->name);
        return 1;
      } else
        ls->params.modulated.pulseenergy =
            ls->params.modulated.energy / ls->params.modulated.pulsevalue;
    }
    if (ls->params.modulated.modulation <= MMT_UNKNOWN ||
        ls->params.modulated.modulation > MMT_FREQUENCY) {
      const char *modulation[] = {"unknown", "amplitude", "pulsewidth",
                                  "frequency"};
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "modulation type %s is invalid",
                   ls->schedule->name,
                   modulation[ls->params.modulated.modulation]);
      return 1;
    }
    break;
  case MT_QUEUED:
    if (ls->params.queued.energy <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "queue energy must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (static_cast<int>(ls->params.queued.pulsetype) ==
        static_cast<int>(MT_UNKNOWN)) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "queue pulse type could not be inferred because either "
                   "duration or power is missing",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.queued.pulsetype == MPT_TIME &&
        ls->params.queued.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "queue pulse duration must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.queued.pulsetype == MPT_POWER &&
        ls->params.queued.pulsevalue <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "queue pulse power must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.queued.scalar <= 0) {
      output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) "
                   "queue pulse count must be a positive number",
                   ls->schedule->name);
      return 1;
    }
    if (ls->params.queued.q_on <= ls->params.queued.q_off) {
      output_error(
          "loadshape_init(loadshape *ls={schedule->name='%s',...}) queue q_on "
          "threshold must be greater than q_off threshold (q_off=%f, q_on=%f)",
          ls->schedule->name, ls->params.queued.q_off, ls->params.queued.q_on);
      return 1;
    }
    break;
  case MT_SCHEDULED:
    ...
    if (ls->params.scheduled.on_time < 0 || ls->params.scheduled.on_time > 24) {
      output_error(
          "loadshape_init() scheduled on-time must be between 0 and 24");
      return 1;
    }

    if (ls->params.scheduled.off_time < 0 ||
        ls->params.scheduled.off_time > 24) {
      output_error(
          "loadshape_init() scheduled off-time must be between 0 and 24");
      return 1;
    }
    ...
    ls->params.scheduled.on_end =
        ls->params.scheduled.on_time +
        (ls->params.scheduled.high - ls->params.scheduled.low) /
            ls->params.scheduled.on_ramp;
    ls->params.scheduled.off_end =
        ls->params.scheduled.off_time +
        (ls->params.scheduled.low - ls->params.scheduled.high) /
            ls->params.scheduled.off_ramp;

    if (ls->params.scheduled.off_time <= ls->params.scheduled.on_end &&
        ls->params.scheduled.on_end <= ls->params.scheduled.off_end) {
      output_error("loadshape_init() scheduled on ramp overlaps with off time");
      return 1;
    }

    if (ls->params.scheduled.on_time <= ls->params.scheduled.off_end &&
        ls->params.scheduled.off_end <= ls->params.scheduled.on_end) {
    ...
    return 0;
  default:
    output_error("loadshape_init(loadshape *ls={schedule->name='%s',...}) load "
  ...
  if (ls->q == 0)
    ls->q = ls->d[0] < ls->d[1]
                ? random_uniform(&(ls->rng_state), ls->d[0], ls->d[1])
                : random_uniform(&(ls->rng_state), ls->d[1], ls->d[0]);
  ;
...
//	int ps = ls->s;
  ...
  if (ls->schedule != nullptr && t1 > ls->t0) {
    ...
    case MT_PULSED:
...
#ifdef _DEBUG
      if (ls->s == 0 && ls->r < 0) {
        output_error("loadshape %s: state inconsistent (s=on, r<0)!",
                     ls->schedule->name);
        return ls->t2 = TS_NEVER;
      } else if (ls->s == 1 && ls->r > 0) {
        output_error("loadshape %s: state inconsistent (s=off, r>0)!",
                     ls->schedule->name);
        return ls->t2 = TS_NEVER;
      }
#endif
      ...
      /* time to next event */
      ls->t2 = ls->r != 0
                   ? t1 + (TIMESTAMP)((ls->d[ls->s] - ls->q) / ls->r * 3600)
                   : TS_NEVER;
      /* This was to address a reported bug - every once in awhile, when ls->q
         was very near 1.0 but slighly less, it would lead to t2=t1 and fail
         simulation; this is a litte bump to get it out of the rut and try one
         more time before failing out */
      if (ls->t2 == t1)
...
#ifdef _DEBUG
      {
        char buf[64];
        output_debug(
            "schedule %s: value = %5.3f, q = %5.3f, r = %+5.3f, t2 = '%s'",
            ls->schedule->name, ls->schedule->value, ls->q, ls->r,
            convert_from_timestamp(ls->t2, buf, sizeof(buf)) ? buf : "(error)");
      }
#endif
      /* choose sooner of schedule change or state change */
      if (ls->schedule->next_t < ls->t2)
        ls->t2 = ls->schedule->next_t;
      break;
    ...
    case MT_MODULATED:
      ...
      /* time to next event */
      ls->t2 = ls->r != 0
                   ? t1 + (TIMESTAMP)((ls->d[ls->s] - ls->q) / ls->r * 3600) + 1
                   : TS_NEVER;
...
//	output_debug("loadshape %s: t2-t1 = %d", ls->schedule->name, ls->t2 -
//global_clock);
#endif
  ls->t0 = t1;
...
void loadshape_syncproc(LOADSHAPESYNCDATA *data) {
  ...
  while (data->ok) {
    ...
    {
      std::unique_lock<std::shared_mutex> start_lock(
          SharedMutexManager::get_mutex(&startlock_ls));
    ...
    {
      std::unique_lock<std::shared_mutex> done_lock(
          SharedMutexManager::get_mutex(&donelock_ls));
  ...
  // No need for pthread_exit or return value - C++ threads automatically clean
  // up
}
  ...
  if (n_threads_ls == 0) {
    ...
    if (n_threads_ls > 1) {
      ...
      output_debug("loadshape_syncall is using %d of %d available threads",
                   n_threads_ls, global_threadcount);
      output_debug("loadshape_syncall is assigning %d shapes per thread",
                   n_items);
...
//			n_items = n_shapes/n_threads_ls;
//			n_threads_ls = n_shapes/n_items;
//			if (n_threads_ls*n_items<n_shapes) // not enough slots
//yet 				n_threads_ls++; // add one underused threads
//
//			output_debug("loadshape_syncall is using %d of %d
//available threads", n_threads_ls, global_threadcount);
//			output_debug("loadshape_syncall is assigning %d shapes
//per thread", n_items);
//
//			// allocate thread list
//			thread_ls =
//(LOADSHAPESYNCDATA*)malloc(sizeof(LOADSHAPESYNCDATA)*n_threads_ls);
...
//				thread_ls[n].ok = true;
//				if (
//pthread_create(&(thread_ls[n].pt),nullptr,loadshape_syncproc,&(thread_ls[n]))!=0
//)
//				{
//					output_fatal("loadshape_sync thread
//creation failed"); 					thread_ls[n].ok = false;
//				}
//				else
//					thread_ls[n].n = n;
//			}
...
int convert_from_loadshape(char *string, int size, void *data, PROPERTY *prop) {
  ...
  case MT_ANALOG:
    if (ls->params.analog.energy > 0)
      ...
      return sprintf(string, "type: analog; schedule: %s; power: %g kW",
                     ls->schedule->name, ls->params.analog.power);
    else
  ...
  case MT_PULSED:
    if (ls->params.pulsed.pulsetype == MPT_TIME)
      return sprintf(string,
                     "type: pulsed; schedule: %s; energy: %g kWh; count: %g; "
                     "duration: %g s",
                     ls->schedule->name, ls->params.pulsed.energy,
                     ls->params.pulsed.scalar, ls->params.pulsed.pulsevalue);
    else if (ls->params.pulsed.pulsetype == MPT_POWER)
      return sprintf(
          string,
          "type: pulsed; schedule: %s; energy: %g kWh; count: %g; power: %g kW",
          ls->schedule->name, ls->params.pulsed.energy,
          ls->params.pulsed.scalar, ls->params.pulsed.pulsevalue);
    else {
      output_error("convert_from_loadshape(...,data={schedule->name='%s',...},"
                   "prop={name='%s',...}) has an invalid pulsetype",
                   ls->schedule->name, prop->name);
      return 0;
    }
    break;
  case MT_MODULATED:
    if (ls->params.pulsed.pulsetype == MPT_TIME)
      return sprintf(string,
                     "type: modulated; schedule: %s; energy: %g kWh; count: "
                     "%g; duration: %g s; pulse: %g kWh; modulation: %s",
                     ls->schedule->name, ls->params.modulated.energy,
                     ls->params.modulated.scalar,
                     ls->params.modulated.pulsevalue,
                     ls->params.modulated.pulseenergy,
                     modulation[ls->params.modulated.modulation]);
      ...
      return sprintf(string,
                     "type: modulated; schedule: %s; energy: %g kWh; count: "
                     "%g; power: %g kW; pulse: %g kWh; modulation: %s",
                     ls->schedule->name, ls->params.modulated.energy,
                     ls->params.modulated.scalar,
                     ls->params.modulated.pulsevalue,
                     ls->params.modulated.pulseenergy,
  ...
  case MT_QUEUED:
    if (ls->params.pulsed.pulsetype == MPT_TIME)
      return sprintf(string,
                     "type: queue; schedule: %s; energy: %g kWh; count: %g; "
                     "duration: %g s; q_on: %g; q_off: %g",
                     ls->schedule->name, ls->params.queued.energy,
                     ls->params.queued.scalar, ls->params.queued.pulsevalue,
                     ls->params.queued.q_on, ls->params.queued.q_off);
    else if (ls->params.pulsed.pulsetype == MPT_POWER)
      return sprintf(string,
                     "type: queued; schedule: %s; energy: %g kWh; count: %g; "
                     "power: %g kW; q_on: %g; q_off: %g",
                     ls->schedule->name, ls->params.queued.energy,
                     ls->params.queued.scalar, ls->params.queued.pulsevalue,
                     ls->params.queued.q_on, ls->params.queued.q_off);
    else {
  ...
  case MT_SCHEDULED:
    return sprintf(
        string,
        "type: scheduled; weekdays: %s; on-time: %.3g; off-time: %.3g; "
        "on-ramp: %.3g; off-ramp: %.3g; low: %.3g; high: %.3g; dt: %u m",
        schedule_weekday_to_string(ls->params.scheduled.weekdays, buffer,
                                   sizeof(buffer)),
        ls->params.scheduled.on_time, ls->params.scheduled.off_time,
        ls->params.scheduled.on_ramp, ls->params.scheduled.off_ramp,
        ls->params.scheduled.low, ls->params.scheduled.high,
        ls->params.scheduled.dt / 60);
  }
  return 1;
}
int convert_to_loadshape(char *string, void *data, PROPERTY *prop) {
  ...
  if (strlen(string) > sizeof(buffer) - 1) {
    output_error("convert_to_loadshape(string='%-.64s...', ...) input string "
  ...
  while ((token = strtok(token == nullptr ? buffer : nullptr, ";")) !=
         nullptr) {
    ...
    if (strcmp(param, "type") == 0) {
      if (strcmp(value, "analog") == 0) {
        ...
        output_error("convert_to_loadshape(string='%-.64s...', ...) type '%s' "
                     "is invalid",
                     string, value);
        return 0;
      }
      ...
      if (s == nullptr) {
        output_error("convert_to_loadshape(string='%-.64s...', ...) schedule "
                     "'%s' does not exist",
                     string, value);
        return 0;
      }
      ...
      if (ls->type == MT_ANALOG)
        ...
        result =
            convert_unit_double(value, "kWh", &ls->params.modulated.energy);
      else if (ls->type == MT_QUEUED)
        result = convert_unit_double(value, "kWh", &ls->params.queued.energy);
      else {
        output_error("convert_to_loadshape(string='%-.64s...', ...) unable to "
                     "parse energy before type is specified",
                     string);
        return 0;
      }
      if (result == 0) {
        output_error("convert_to_loadshape(string='%-.64s...', ...) unit of "
                     "energy parameter '%s' is incompatible with kWh",
                     string, value);
        return 0;
      }
    } else if (strcmp(param, "count") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) count is "
                       "not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        ...
        output_error("convert_to_loadshape(string='%-.64s...', ...) unable to "
                     "parse count before type is specified",
                     string);
        return 0;
      }
    } else if (strcmp(param, "duration") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) duration "
                       "is not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED) {
        if (ls->params.pulsed.pulsetype == MPT_POWER)
          output_warning("convert_to_loadshape(string='%-.64s...', ...) "
                         "duration ignored because power has already been "
                         "specified and is mutually exclusive",
                         string);
        else {
          ls->params.pulsed.pulsetype = MPT_TIME;
          if (!convert_unit_double(value, "s", &ls->params.pulsed.pulsevalue)) {
            output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                         "to convert duration '%s' to seconds",
                         string, value);
            return 0;
          }
        }
        ...
        output_warning("convert_to_loadshape(string='%-.64s...', ...) duration "
                       "is not used by modulated loadshapes",
                       string);
      else if (ls->type == MT_QUEUED) {
        if (ls->params.queued.pulsetype == MPT_POWER)
          output_warning("convert_to_loadshape(string='%-.64s...', ...) "
        ...
        output_error("convert_to_loadshape(string='%-.64s...', ...) unable to "
                     "parse duration before type is specified",
                     string);
        return 0;
      }
    } else if (strcmp(param, "period") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) period "
                       "is not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) duration "
        ...
        if (ls->params.modulated.pulsetype == MPT_POWER)
          output_warning("convert_to_loadshape(string='%-.64s...', ...) period "
                         "ignored because power has already been specified and "
                         "is mutually exclusive",
                         string);
        else {
          ls->params.modulated.pulsetype = MPT_TIME;
          if (!convert_unit_double(value, "s",
                                   &ls->params.modulated.pulsevalue)) {
            output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                         "to convert period '%s' to seconds",
                         string, value);
            return 0;
          }
        }
      ...
      if (ls->type == MT_ANALOG) {
        if (!convert_unit_double(value, "kW", &ls->params.analog.power)) {
          output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                       "to convert power '%s' to unit kW",
                       string, value);
          return 0;
        }
      } else if (ls->type == MT_PULSED)
        if (ls->params.pulsed.pulsetype == MPT_TIME)
          output_warning("convert_to_loadshape(string='%-.64s...', ...) power "
                         "ignored because duration has already been specified "
                         "and is mutually exclusive",
                         string);
        else {
          ls->params.pulsed.pulsetype = MPT_POWER;
          if (!convert_unit_double(value, "kW",
                                   &ls->params.pulsed.pulsevalue)) {
            output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                         "to convert power '%s' to unit kW",
                         string, value);
            return 0;
        ...
        if (ls->params.modulated.pulsetype == MPT_TIME)
          output_warning("convert_to_loadshape(string='%-.64s...', ...) power "
                         "ignored because period has already been specified "
                         "and is mutually exclusive",
                         string);
        else {
      ...
      if (ls->type == MT_ANALOG) {
        if (ls->params.analog.energy != 0) {
          if (!convert_unit_double(value, "kWh", &dev)) {
            output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                         "to convert stdev '%s' to unit kWh",
                         string, value);
            return 0;
          }
      ...
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) "
                       "modulation is not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) "
                       "modulation is not used by pulsed loadshapes",
                       string);
      else if (ls->type == MT_MODULATED) {
        if (strcmp(value, "amplitude") == 0) {
          output_warning("convert_to_loadshape(string='%-.64s...', ...) "
                         "amplitude modulation is not fully supported",
                         string);
          ...
          output_error("convert_to_loadshape(string='%-.64s...', ...) '%s' is "
                       "not a recognized modulation",
                       string, value);
          return 0;
        }
      } else if (ls->type == MT_QUEUED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) "
                       "modulation is not used by queued loadshapes",
                       string);
    } else if (strcmp(param, "pulse") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) pulse "
                       "energy is not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) pulse "
                       "energy is not used by pulsed loadshapes",
                       string);
      else if (ls->type == MT_MODULATED) {
        if (!convert_unit_double(value, "kWh",
                                 &ls->params.modulated.pulseenergy)) {
          output_error("convert_to_loadshape(string='%-.64s...', ...) unable "
                       "to convert pulse energy '%s' to unit kWh",
                       string, value);
          return 0;
        }
      } else if (ls->type == MT_QUEUED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) pulse "
                       "energy is not used by queued loadshapes",
                       string);
    } else if (strcmp(param, "q_on") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) q_on is "
                       "not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) q_on is "
                       "not used by pulsed loadshapes",
                       string);
      else if (ls->type == MT_MODULATED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) q_on is "
                       "not used by modulated loadshapes",
                       string);
      else if (ls->type == MT_QUEUED)
        ls->params.queued.q_on = atof(value);
    } else if (strcmp(param, "q_off") == 0) {
      if (ls->type == MT_ANALOG)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) q_off is "
                       "not used by analog loadshapes",
                       string);
      else if (ls->type == MT_PULSED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) q_off is "
      ...
      if (ls->type != MT_SCHEDULED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) %s is "
                       "not used by analog loadshapes",
                       string, param);
      else
      ...
      if (ls->type != MT_SCHEDULED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) %s is "
                       "not used by analog loadshapes",
                       string, param);
      else if (sample_from_diversity(&(ls->rng_state),
                                     &ls->params.scheduled.low, value) == 0)
        output_error("convert_to_loadshape(string='%-.64s...', ...) %s syntax "
                     "error, '%s' not valid",
                     string, param, value);
      ...
      if (ls->type != MT_SCHEDULED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) %s is "
                       ...
                       string, param);
                                     ...
                                     &ls->params.scheduled.on_time, value) == 0)
        output_error("convert_to_loadshape(string='%-.64s...', ...) %s syntax "
                     "error, '%s' not valid",
                     string, param, value);
      ...
      if (ls->type != MT_SCHEDULED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) %s is "
                       ...
                       string, param);
                                     ...
                                     &ls->params.scheduled.on_ramp, value) == 0)
        output_error("convert_to_loadshape(string='%-.64s...', ...) %s syntax "
                     "error, '%s' not valid",
                     string, param, value);
      ...
      if (ls->type != MT_SCHEDULED)
        output_warning("convert_to_loadshape(string='%-.64s...', ...) %s is "
                       ...
                       string, param);
                                     ...
                                     &ls->params.scheduled.high, value) == 0)
        output_error("convert_to_loadshape(string='%-.64s...', ...) %s syntax "
                     "error, '%s' not valid",
                     string, param, value);
      ...
      output_error("convert_to_loadshape(string='%-.64s...', ...) parameter "
                   "'%s' is not valid",
                   string, param);
      return 0;
    }
...
int loadshape_test(void) {
  ...
  /* report results */
  if (failed) {
    output_error("loadshapetest: %d loadshape tests failed--see test.txt for "
                 "more information",
                 failed);
    output_test("!!! %d loadshape tests failed, %d errors found", failed,
                errorcount);
  } else {
    output_verbose(
        "%d loadshape tests completed with no errors--see test.txt for details",
        ok);
    output_test("loadshapetest: %d loadshape tests completed, %d errors found",
                ok, errorcount);
  }
