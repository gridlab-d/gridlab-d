                    ...
                    int ordinal)  /**< the ordinal to which the item belongs */
{
  ...
  if (ordinal < index->first_ordinal) /* grow on bottom end */
  {
    ...
    output_fatal("ordinal %d is too small (first is %d)", ordinal,
                 index->first_ordinal);
    /*	TROUBLESHOOT
            This is an internal error caused by an attempt to index something
       improperly. Because the indexing system start at index 0, this means a
       negative index number was attempting, which is most likely a bug.  This
       usually occurs during object initialization and might be caused by
       invalid object numbers, either negative or greater than about 2 billion.
       If your model is using a the <b><i>class</i>:<i>id</i></b> naming
       convention, check the object id numbers to make sure they are correct.
    */
    errno = EINVAL;
    return FAILED;
  } else if (ordinal >= index->last_ordinal) /* grow on to end */
  {
    ...
    if (newblock == nullptr) {
      ...
      /*	TROUBLESHOOT
              An internal memory allocation error cause an index operation to
         fail. The message will usually include some explanation as to what
         cause the failure.  Remedy the indicated memory problem to fix the
         indexing problem.
      */
      return FAILED;
    }
  ...
  if (list_append(index->ordinal[pos], data) == nullptr)
    return FAILED;
  if (ordinal < index->first_used)
    index->first_used = ordinal;
  if (ordinal > index->last_used)
    index->last_used = ordinal;
  return SUCCESS;
}
