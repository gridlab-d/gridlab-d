...
typedef struct s_aggregate
    AGGREGATION; /**< forward declaration of the aggregation type */
...
public:
  ...
  complex(double re, double im,
          CNOTATION nf = CNOTATION_DEFAULT) /**< create a complex number with
  ...
  void SetRect(double rp, double ip,
               CNOTATION nf = CNOTATION_DEFAULT) /**< set rectangular value */
  {
    r = rp;
    i = ip;
    f = nf;
  ...
  {
    double lm = log(Mag()), a = Arg(), b = exp(x.r * lm - x.i * a),
           c = x.r * a + x.i * lm;
    r = (b * cos(c));
    i = (b * sin(c));
    return *this;
...
using gld::complex; // TODO: GLD Runtime compile does not properly handle
                    // namespaces.
...
#define TS_TPS                                                                 \
  ((int64)1) /* ticks per second time resolution - this needs to be compatible \
                with TS_SECOND is timestamp.h */
#define TS_REALNOW ((TIMESTAMP)(time(NULL)) * TS_TPS)
#define TS_SIMNOW (*(callback->global_clock))
#define TS_ZERO ((int64)0)
#define Ts_INIT                                                                \
  ((int64)3600 *                                                               \
   12) /* 12 hours after time zero to avoid negative timezone values */
#define TS_MAX                                                                 \
...
inline double gl_minutes(TIMESTAMP t) {
  return (double)t / (double)(TS_TPS * 60);
}
inline double gl_hours(TIMESTAMP t) {
  return (double)t / (double)(TS_TPS * 60 * 60);
}
inline double gl_days(TIMESTAMP t) {
  return (double)t / (double)(TS_TPS * 60 * 60 * 24);
}
inline double gl_weeks(TIMESTAMP t) {
  return (double)t / (double)(TS_TPS * 60 * 60 * 24 * 7);
}
...
//	RT_DEGENERATE,	/**< degenerate distribution (Dirac delta function);
//double only_value */ 	RT_UNIFORM,		/**< uniform distribution;
//double minimum_value, double maximum_value */ 	RT_NORMAL,		/**<
//normal distribution; double arithmetic_mean, double arithmetic_stdev */
//	RT_LOGNORMAL,	/**< log-normal distribution; double geometric_mean,
//double geometric_stdev */ 	RT_BERNOULLI,	/**< Bernoulli distribution;
//double probability_of_observing_1 */ 	RT_PARETO,		/**< Pareto
//distribution; double minimum_value, double gamma_scale */ 	RT_EXPONENTIAL, /**<
//exponential distribution; double coefficient, double k_scale */ 	RT_SAMPLED,
///**< sampled distribution; unsigned number_of_samples, double
//samples[n_samples] */ 	RT_RAYLEIGH,	/**< Rayleigh distribution; double sigma
//*/ 	RT_WEIBULL,		/**< Weibull distribution; double lambda, double
//k */ 	RT_GAMMA,		/**< Gamma distribution; double alpha, double
//beta */ 	RT_BETA,		/**< Beta distribution; double alpha, double
//beta */ 	RT_TRIANGLE,	/**< Triangle distribution; double a, double b */
// }; // RANDOMTYPE;
...
using randomvar = s_randomvar; // behavior for GLM runtime compilation, which
                               // does not handle underscore
...
typedef enum {
  ...
  PA_REFERENCE = (PA_R | PA_W | PA_S), /**< property is FYI (readable, writable
                                          and saved, but not loaded */
  PA_PROTECTED =
      (PA_R), /**< property is semipublic (readable, but not saved or loaded) */
  PA_PRIVATE =
      (PA_N),     /**< property is nonpublic (not visible, saved or loaded) */
...
#define PC_PRETOPDOWN                                                          \
  0x01 /**< used when the class requires synchronization on the first top-down \
          pass */
#define PC_BOTTOMUP                                                            \
  0x02 /**< used when the class requires synchronization on the bottom-up pass \
        */
#define PC_POSTTOPDOWN                                                         \
  0x04 /**< used when the class requires synchronization on the second         \
          top-down pass */
#define PC_FORCE_NAME                                                          \
  0x20 /**< used to indicate the this class must define names for all its      \
          objects */
#define PC_PARENT_OVERRIDE_OMIT                                                \
  0x40 /**< used to ignore parent's use of PC_UNSAFE_OVERRIDE_OMIT */
#define PC_UNSAFE_OVERRIDE_OMIT                                                \
  0x80 /**< used to flag that omitting overrides is unsafe */
#define PC_ABSTRACTONLY                                                        \
  0x100 /**< used to flag that the class should never be instantiated itself,  \
           only inherited classes should */
...
typedef struct s_exception_handler {
  ...
  jmp_buf buf;    /**< the \p jmpbuf containing the context for the exception
                     handler */
...
struct s_delegatedtype {
  ...
  int (*from_string)(
      void *addr,
      char *value); /**< the function that converts from a string to the data */
  int (*to_string)(
...
struct s_module_list {
  ...
  SIMULATIONMODE (*interupdate)(void *, int64, unsigned int64, unsigned long,
                                unsigned int);
...
/* Technology readiness levels (see
 * http://sourceforge.net/apps/mediawiki/gridlab-d/index.php?title=Technology_Readiness_Levels)
 */
typedef enum {
  TRL_UNKNOWN = 0,
  TRL_PRINCIPLE = 1,
...
} CLASSMAGIC; ///< this is used to uniquely identify class structure
...
struct s_class_list {
  ...
  bool has_runtime; ///< flag indicating that a runtime dll, so, or dylib is in
                    ///< use
...
typedef struct s_forecast {
  char1024 specification; /**< forecast specification (see forecasting docs for
                             details) */
  ...
  TIMESTAMP (*external)(void *obj,
                        void *fc); /**< external forecast update call */
...
struct s_object_list {
  ...
  unsigned int
      child_count;    /**< number of object that have this object as a parent */
  OBJECTRANK rank;    /**< object's rank */
  TIMESTAMP clock;    /**< object's private clock */
  TIMESTAMP valid_to; /**< object's valid-until time */
  TIMESTAMP schedule_skew;    /**< time skew applied to schedule operations
  ...
  double
      out_svc_double; /**< Double value representation of out of service time */
...
struct s_schedule {
  ...
  unsigned char
      index[14][366 * 24 * 60]; /**< the schedule index (enough room for all 14
                                   annual calendars to 1 minute resolution) */
  uint32 dtnext[14][366 * 24 * 60]; /**< the time until the next schedule change
                                       (in minutes) */
  double
      data[MAXBLOCKS * MAXVALUES]; /**< the list of values used in each block */
  unsigned int weight[MAXBLOCKS * MAXVALUES]; /**< the weight (in minutes)
                                                 associate with each value */
  double sum[MAXBLOCKS]; /**< the sum of values for each block -- used to
                            normalize */
  double abs[MAXBLOCKS]; /**< the sum of the absolute values for each block --
                            used to normalize */
  unsigned int
      count[MAXBLOCKS]; /**< the number of values given in each block */
  unsigned int minutes[MAXBLOCKS]; /**< the total number of minutes associate
                                      with each block */
...
typedef enum {
  ...
  MT_PULSED, /**< machine outputs pulses of fixed area with varying frequency to
                match value */
  MT_MODULATED, /**< machine outputs pulses of fixed frequency with varying area
                   to match value */
  MT_QUEUED,    /**< machine accrues values and output pulses of fixed area with
                   varying frequency */
} MACHINETYPE;  /** type of machine */
typedef enum {
  MPT_UNKNOWN = 0,
  MPT_TIME,  /**< pulses are of fixed time (either total period or on-time
                duration); power is energy/duration */
...
struct s_loadshape_core {
  ...
  union {
    ...
    struct {
      ...
      MACHINEPULSETYPE
          pulsetype;     /**< the fixed part of the pulse (time or power) */
...
typedef enum {
  ...
  EUMT_MOTOR_B, /**< induction motors driving high inertia speed-squares torque
                   loads */
  EUMT_MOTOR_C, /**< induction motors driving low inertia loads speed-squared
                   torque loads */
...
typedef struct s_enduse {
  ...
  double heatgain_fraction; /* fraction of power that goes to internal heat (pu
                               Btu/h) */
...
#define OF_HASPLC                                                              \
  0x0001 /**< Object flag; external PLC is attached, disables local PLC */
#define OF_LOCKED                                                              \
  0x0002 /**< Object flag; data write pending, reread recommended after lock   \
            clears */
#define OF_RECALC                                                              \
  0x0008 /**< Object flag; recalculation of derived values is needed */
#define OF_FOREIGN                                                             \
  0x0010 /**< Object flag; indicates that object was created in a DLL and      \
            memory cannot be freed by core */
#define OF_SKIPSAFE                                                            \
  0x0020 /**< Object flag; indicates that skipping updates is safe */
#define OF_FORECAST                                                            \
  0x0040 /**< Object flag; inidcates that the object has a valid forecast      \
            available */
#define OF_DEFERRED                                                            \
  0x0080 /**< Object flag; indicates that the object started to be             \
            initialized, but requested deferral */
#define OF_INIT                                                                \
  0x0100 /**< Object flag; indicates that the object has been successfully     \
            initialized */
...
#define atomic_compare_and_swap(dest, comp, xchg)                              \
  (_InterlockedCompareExchange((int32 *)dest, xchg, comp) == comp)
...
#define atomic_compare_and_swap(dest, comp, xchg)                              \
  OSAtomicCompareAndSwap32Barrier(comp, xchg, (int32_t *)dest)
...
static inline void unlock(unsigned int *lock) { atomic_increment(lock); }
...
typedef struct s_property_specs { /**<	the property type conversion
  ...
  struct {
    ...
    int trinary;
                            ...
                            this type */
  double (*get_part)(
      void *, char *name); /**< the function to get a part of a property */
...
typedef struct s_transform {
  ...
  TRANSFORMFUNCTIONTYPE
      function_type; ///< function type (linear, external, etc.)
  union {
...
public:
  ...
  PROPERTY *(*class_add_extended_property)(CLASS *, char *, PROPERTYTYPE,
                                           char *);
  struct {
    FUNCTION *(*define)(CLASS *, const FUNCTIONNAME, FUNCTIONADDR);
    FUNCTIONADDR (*get)(char *, const char *);
  } function;
  int (*define_enumeration_member)(CLASS *, const char *, const char *,
                                   enumeration);
  ...
  struct {
    ...
    bool (*compare_basic)(PROPERTYTYPE ptype, PROPERTYCOMPAREOP op, void *x,
                          void *a, void *b, char *part);
  ...
  void (*free)(void *);
  struct {
    std::shared_ptr<struct s_aggregate> (*create)(char *aggregator,
  ...
  struct {
    ...
    int (*depends)(const char *name, unsigned char major, unsigned char minor,
  ...
  DELEGATEDTYPE *(*register_type)(CLASS *oclass, char *type,
  ...
  struct {
    ...
    int (*convert_from_deltatime_timestamp)(double ts_v, char *buffer,
                                            int size);
  } time;
  ...
  struct {
    char *(*find_file)(const char *name, char *path, int mode, char *buffer,
                       int len);
  } file;
  ...
  struct {
    ...
    int (*property_to_string)(PROPERTY *prop, void *addr, char *value,
                              int size);
  } convert;
  ...
  struct {
    ...
    double (*quadratic)(double t, double x0, double y0, double x1, double y1,
                        double x2, double y2);
  } interpolate;
  struct {
    FORECAST *(*create)(
        OBJECT *obj,
        char *specs); /**< create a forecast using the specifications and append
                         it to the object's forecast block */
    FORECAST *(*find)(
        OBJECT *obj,
        char *name); /**< find the forecast for the named property, if any */
    double (*read)(
        FORECAST *fc,
        TIMESTAMP ts); /**< read the forecast value for the time ts */
    void (*save)(FORECAST *fc, TIMESTAMP ts, int32 tstep, int n_values,
                 double *data);
  } forecast;
  struct {
  ...
  struct {
    struct s_objlist *(*create)(CLASS *oclass, PROPERTY *match_property,
                                char *match_part, char *match_op,
                                void *match_value1, void *match_value2);
    struct s_objlist *(*search)(char *group);
    void (*destroy)(struct s_objlist *list);
    size_t (*add)(struct s_objlist *list, PROPERTY *match_property,
                  char *match_part, char *match_op, void *match_value1,
                  void *match_value2);
    size_t (*del)(struct s_objlist *list, PROPERTY *match_property,
                  char *match_part, char *match_op, void *match_value1,
                  void *match_value2);
    size_t (*size)(struct s_objlist *list);
    struct s_object_list *(*get)(struct s_objlist *list, size_t n);
    int (*apply)(struct s_objlist *list, void *arg,
                 int (*function)(struct s_object_list *, void *, int pos));
  } objlist;
  struct {
    struct {
  ...
  struct {
    ...
    int (*add_linear)(TRANSFORMSOURCE, double *, void *, double, double,
                      OBJECT *, PROPERTY *, SCHEDULE *);
    int (*add_external)(OBJECT *, PROPERTY *, const char *, OBJECT *,
                        PROPERTY *);
...
#define gl_verbose                                                             \
  (*callback->output_verbose) ///< Send a printf-style message to the verbose
                              ///< stream
#define gl_output                                                              \
  (*callback->output_message) ///< Send a printf-style message to the output
                              ///< stream
#define gl_warning                                                             \
  (*callback->output_warning) ///< Send a printf-style message to the warning
                              ///< stream
#define gl_error                                                               \
  (*callback->output_error) ///< Send a printf-style message to the error stream
#define gl_fatal                                                               \
  (*callback->output_fatal) ///< Send a printf-style message to the error stream
#define gl_debug                                                               \
  (*callback->output_debug) ///< Send a printf-style message to the debug stream
#define gl_testmsg                                                             \
  (*callback                                                                   \
        ->output_test) ///< Send a printf-style message to the testmsg stream

#define gl_globalclock                                                         \
  (*(callback->global_clock)) ///< Get the current value of the global clock
...
inline char *gl_global_getvar(
    char *name, char *buffer,
    int size) ///< pointer to string containing the name of the global variable
{
...
inline OBJECT *gl_create_object(
    CLASS *oclass) ///< a pointer to the class of the object to be created
{
  return (*callback->create.single)(oclass);
}

inline OBJECT *gl_create_foreign(OBJECT *obj) {
  return (*callback->create.foreign)(obj);
};
...
template <class T>
inline void
gl_get_value(OBJECT *obj, ///< the object whose property value is being obtained
                       ...
                       ///< value is being copied
{
  ...
  if (ptr == NULL)
    gl_throw("property %s not found in object %s", propname,
...
inline FUNCTIONADDR gl_get_function(OBJECT *obj, char *fname) {
  return callback->function.get(obj->oclass->name, fname);
};
inline FUNCTIONADDR gl_get_function(CLASS *oclass, char *fname) {
  return callback->function.get(oclass->name, fname);
};
inline FUNCTIONADDR gl_get_function(char *classname, char *fname) {
  return callback->function.get(classname, fname);
};

inline double gl_random_uniform(const double lo, const double hi) {
  return callback->random.uniform(NULL, lo, hi);
};
inline double gl_random_normal(const double mu, const double sigma) {
  return callback->random.normal(NULL, mu, sigma);
};
inline double gl_random_lognormal(const double gmu, const double gsigma) {
  return callback->random.lognormal(NULL, gmu, gsigma);
};
inline double gl_random_exponential(const double lambda) {
  return callback->random.exponential(NULL, lambda);
};
inline double gl_random_pareto(const double m, const double k) {
  return callback->random.pareto(NULL, m, k);
};
inline double gl_random_bernoulli(double p) {
  return callback->random.bernoulli(NULL, p);
};
inline double gl_random_sampled(unsigned int n, double *x) {
  return callback->random.sampled(NULL, n, x);
};
inline double gl_random_triangle(double a, double b) {
  return callback->random.triangle(NULL, a, b);
};
inline double gl_random_beta(double a, double b) {
  return callback->random.beta(NULL, a, b);
};
inline double gl_random_gamma(double a, double b) {
  return callback->random.gamma(NULL, a, b);
};
inline double gl_random_weibull(double a, double b) {
  return callback->random.weibull(NULL, a, b);
};
inline double gl_random_rayleigh(double a) {
  return callback->random.rayleigh(NULL, a);
};

inline bool gl_object_isa(OBJECT *obj, char *type) {
  return callback->object_isa(obj, type) == 1;
};
inline DATETIME *gl_localtime(TIMESTAMP ts, DATETIME *dt) {
  return callback->time.local_datetime(ts, dt) ? dt : NULL;
};
inline DATETIME *gl_localtime_delta(double ts, DATETIME *dt) {
  return callback->time.local_datetime_delta(ts, dt) ? dt : NULL;
};
inline TIMESTAMP gl_mkdatetime(DATETIME *dt) {
  return callback->time.mkdatetime(dt);
};
inline TIMESTAMP gl_mkdatetime(short year, short month, short day,
                               short hour = 0, short minute = 0,
                               short second = 0, char *tz = NULL,
                               unsigned int nsec = 0) {
  DATETIME dt;
  dt.year = year;
  dt.month = month;
...
inline int gl_unit_convert(char *from, char *to, double &value) {
  return callback->unit_convert(from, to, &value);
};
inline int gl_unit_convert(UNIT *from, UNIT *to, double &value) {
  return callback->unit_convert_ex(from, to, &value);
};
inline UNIT *gl_unit_find(char *name) { return callback->unit_find(name); };
inline char *gl_find_file(const char *name, char *path, int mode, char *buffer,
                          int len) {
  return callback->file.find_file(name, path, mode, buffer, len);
};
...
inline void trace(char *fn, OBJECT *obj) {
  callback->output_message("TRACE: %s(%s[%s:%d])", fn,
...
inline OBJECT *gl_find_next(struct s_findlist *list, OBJECT *obj) {
  return callback->find.next(list, obj);
};
inline struct s_findlist *gl_find_copy(struct s_findlist *list) {
  return callback->find.copy(list);
};
inline void gl_find_add(struct s_findlist *list, OBJECT *obj) {
  callback->find.add(list, obj);
};
inline void gl_find_del(struct s_findlist *list, OBJECT *obj) {
  callback->find.del(list, obj);
};
inline void gl_find_clear(struct s_findlist *list) {
  callback->find.clear(list);
};
...
private:
  ...
  inline bool tst_flag(const size_t r, size_t c, const unsigned char b) const {
    return (f[r * m + c] & b) == b;
  };
...
public:
  inline double_vector operator[](const size_t n) {
    return double_vector(x[n]);
  }
  inline const double_vector operator[](const size_t n) const {
    return double_vector(x[n]);
  }
  double_array(const size_t.rows() = 0, const size_t cols = 0,
               double **data = NULL) {
...
public:
  ...
  bool is_valid(const size_t r, const size_t c) const {
    return r < n && c < m;
  };
  ...
  double get_at(const size_t r, const size_t c) {
    return is_nan(r, c) ? QNAN : *(x[r][c]);
  };
