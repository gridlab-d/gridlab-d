...
public:
	...
	void set_max(const size_t size)
	{
		...
		size_t r;
		gld::complex ***z = (gld::complex***)malloc(sizeof(gld::complex**)*size);
		// create new rows
		for ( r=0 ; r<max_val ; r++ )
		{
			if ( x[r]!=NULL )
			{
				gld::complex **y = (gld::complex**)malloc(sizeof(gld::complex*)*size);
	...
	{ 
		...
		if ( m<c )
		{
			...
			for ( i=0 ; i<n ; i++ )
			{
				gld::complex **y = (gld::complex**)malloc(sizeof(gld::complex*)*c);
				if ( x[i]!=NULL )
				{
