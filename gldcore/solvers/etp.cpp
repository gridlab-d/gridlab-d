...
EXPORT int etp_set(char *param, ...) {
  ...
  while (tag != nullptr) {
    if (strcmp(param, "max_iterations") == 0)
      ...
      gl_error("etp_set(char *param='%s',...): tag '%s' is not recognized",
               param, tag);
      return n;
    }
    tag = va_arg(arg, char *);
  ...
  while (tag != nullptr) {
    if (strcmp(param, "max_iterations") == 0)
      ...
      *va_arg(arg, unsigned int *) = version;
      ...
      gl_error("etp_get(char *param='%s',...): tag '%s' is not recognized",
               param, tag);
      return n;
    }
    tag = va_arg(arg, char *);
...
/** solve an equation of the form $f[ ae^{nt} + be^{mt} + c = 0 $f]
        @returns 1 = t is set ok, 0 if not converged; t is NaN if no solution
 found
 **/
EXPORT int etp_solve(ETPDATA *etp) {
  ...
  if (iter == 0) {
    gl_error("etp::solve(a=%.4f,n=%.4f,b=%.4f,m=%.4f,c=%.4f,prec=%.g) failed "
             "to converge",
             a, n, b, m, c, p);
