...
static unsigned int solver_version =
...
EXPORT int newton_method_set(char *param, ...) {
  ...
  while (tag != nullptr) {
    if (strcmp(param, "max_iterations") == 0)
      ...
      gl_error(
          "newton_method_set(char *param='%s',...): tag '%s' is not recognized",
          param, tag);
      return n;
    }
    tag = va_arg(arg, char *);
  ...
  while (tag != nullptr) {
    if (strcmp(param, "solver_version") == 0)
      ...
      *va_arg(arg, unsigned int *) = dimensions;
      ...
      gl_error(
          "newton_method_get(char *param='%s',...): tag '%s' is not recognized",
          param, tag);
      return n;
    }
    tag = va_arg(arg, char *);
...
EXPORT int newton_method_solve(NMDATA *data) {
  ...
  for (n = 0; n < data->n; n++) {
    ...
    do {
      ...
      if (i++ > max_iterations) {
        if (s > 0)
          s = 2; // only flag if not already flagged for failure
        data->s[n] = 2;
      }
    } while (fabs(dx) > p);
  }
