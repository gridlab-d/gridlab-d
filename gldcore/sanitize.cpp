...
/** sanitize
        ...
        Sanitizes a gridlabd model by clear names and position from object
 headers
...
extern "C" int sanitize(int argc, char *argv[]) {
  ...
  else if (sscanf(global_sanitizeoffset.get_string(), "%lf%*[,/]%lf",
                  &delta_latitude, &delta_longitude) != 2) {
    output_error("sanitize_offset lat/lon '%s' is not valid",
                 global_sanitizeoffset.get_string());
    return -2;
  }
  ...
  for (obj = object_get_first(); obj != nullptr; obj = object_get_next(obj)) {
    ...
    if (isfinite(obj->latitude) &&
        (global_sanitizeoptions & SO_GEOCOORDS) == SO_GEOCOORDS) {
  ...
  if (strcmp(global_sanitizeindex, ".xml") == 0) {
    ...
    if (ext && strcmp(ext, ".glm") == 0)
      strcpy(ext, "-index.xml");
    else
      strcat(global_sanitizeindex, "-index.xml");
  } else if (strcmp(global_sanitizeindex, ".txt") == 0) {
    strcpy(global_sanitizeindex, global_modelname);
    char *ext = strrchr(global_sanitizeindex, '.');
    if (ext && strcmp(ext, ".glm") == 0)
      strcpy(ext, "-index.txt");
    else
      strcat(global_sanitizeindex, "-index.txt");
  } else if (global_sanitizeindex[0] == '.') {
    output_error("sanitization index file spec '%s' is not recognized",
                 global_sanitizeindex.get_string());
    return -2;
  }
