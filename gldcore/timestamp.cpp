...
#define MICROSECOND \
    (TS_SECOND / 1000000) /**< the number of ticks in one microsecond */
#define NANOSECOND \
    (TS_SECOND / 1000000000) /**< the number of ticks in one nanosecond */
 ...
 **/
char *timestamp_current_timezone(void) { return current_tzname; }
...
int timestamp_year(TIMESTAMP ts, TIMESTAMP *remainder)
{
...
#else
    unsigned int year =
        (unsigned int)(ts / 86400 / 365.24); /* estimate the year */
#endif
    int tsyear = 0;

    if (tszero[0] == -1)
    { /* need to initialize tszero array */
        TIMESTAMP ts = 0;
        int year0 = YEAR0;
    ...
    if ((year + YEAR0) < 0)
    {
        ...
        /*	TROUBLESHOOT
                This is indicative of something strange occuring within
           timestamp_year().  This is a "kicked up" error message that was
...
int isdst(TIMESTAMP t)
{
    ...
    if (dststart[year] >= 0) // If it's -1, no sense going forth
    {
        ...
        if (DSTstart_year != DSTend_year)
        {
            ...
            else // In "early-year" DST region - see if we can do some different
                 // checks
            {
                ...
                if (year > 0)
                {
                    ...
                    {
                        if (dstend[year - 1] <
                            t) // See if we're above "last year's" ending date
                        {
                            return 0; // We're greater than last year's, but less than this
                                      // year's - clearly not in DST
                        }
                ...
                else // First year of array, probably not wise to go -1 on it, just go
                     // like normal
                {
...
int local_datetime(TIMESTAMP ts, DATETIME *dt)
{
    ...
    if (rem < 0)
    {
        // DPC: note that as of 3.0, the clock is initialized by default, so this
        // error can only
        //      occur when an invalid timestamp is being converted to local time. It
        //      should no longer occur as a result of a missing clock directive.
        // THROW("local_datetime(ts=%lli, ...): invalid timestamp cannot be
        // converted to local time", ts);
        /*	TROUBLESHOOT
    ...
    while (rem >= n)
    {
        ...
        n = (daysinmonth[dt->month] +
             ((dt->month == 1 && ISLEAPYEAR(dt->year)) ? 1 : 0)) *
            86400 * TS_SECOND;
        if (n < 86400 * 28)
        { /**/
            output_fatal(
                "Breaking an infinite loop in local_datetime! (ts = %" FMT_INT64 "ds",
                ts);
            /*	TROUBLESHOOT
                    An internal protection against infinite loops in the time
               calculation module has encountered a critical problem.  This is often
               caused by an incorrectly initialized timezone system, a missing
               timezone specification before a timestamp was used, or a missing
               timezone localization in your system. Correct the timezone problem and
               try again.
             */
            return 0;
        }
    ...
    /* determine timezone */
    strncpy(dt->tz, tzvalid ? (dt->is_dst ? tzdst : tzstd) : "GMT",
            sizeof(dt->tz));
...
int local_datetime_delta(double tsdbl, DATETIME *dt)
{
    ...
    if (rem < 0)
    {
        ...
        //      occur when an invalid timestamp is being converted to local time. It
        //      should no longer occur as a result of a missing clock directive.
        // THROW("local_datetime_delta(ts=%lli, ...): invalid timestamp cannot be
        // converted to local time", ts);
        /*	TROUBLESHOOT
                This is the result of an internal core or module coding error which
           resulted in an invalid UTC clock time being converted to local time.
    ...
    while (rem >= n)
    {
        ...
        { /**/
            output_fatal(
                "Breaking an infinite loop in local_datetime_delta! (ts = %" FMT_INT64
                "ds",
                ts);
            /*	TROUBLESHOOT
                    An internal protection against infinite loops in the time
...
TIMESTAMP mkdatetime(DATETIME *dt)
{
    ...
    if (dt->year < YEAR0 ||
        dt->year >= YEAR0 + sizeof(tszero) / sizeof(tszero[0]))
    {
        return TS_INVALID;
    }
    ...
    for (n = 1; n < dt->month; n++)
    {
        ...
        // ts += (daysinmonth[n - 1] + (n == 2 && ISLEAPYEAR(dt->year) ? 1 : 0)) *
        // DAY;
    }

    if (dt->hour < 0 || dt->hour > 23 || dt->minute < 0 || dt->minute > 60 ||
        dt->second < 0 || dt->second > 62)
    {
    ...
    ts +=
        (TIMESTAMP)((dt->day - 1) * DAY + dt->hour * HOUR + dt->minute * MINUTE +
                    dt->second * SECOND + dt->nanosecond / 1.0e9);
    ...
    else if (strcmp(dt->tz, tzstd) == 0 ||
             ((strcmp(dt->tz, "") == 0 && !isdst(ts) &&
               (ts < dststart[dt->year - YEAR0] ||
                ts >= dstend[dt->year - YEAR0]))))
    {
        /* adjust to standard local time */
        return ts + tzoffset;
    }
    else if (strcmp(dt->tz, tzdst) == 0 ||
             ((strcmp(dt->tz, "") == 0 && !isdst(ts) &&
               ts >= dststart[dt->year - YEAR0] &&
               ts < dstend[dt->year - YEAR0])))
    {
...
int strdatetime(DATETIME *t, char *buffer, int size)
{
    ...
    {
        throw_exception(const_cast<char *>("global_dateformat=%d is not valid"),
                        global_dateformat);
        /* TROUBLESHOOT
                The value of the global variable 'global_dateformat' is not valid.
                Check for attempts to set this variable and make sure that is one of
           the valid values (e.g., DF_ISO, DF_US, DF_EURO)
         */
    }
    ...
    else
    {
        ...
        /*	TROUBLESHOOT
                The buffer provided to strdatetime was insufficiently large, or was
           otherwise packed with data beforehand.  The relevant code should use a
           larger buffer, or the output data set should be smaller.
        */
        return 0;
    }
}
...
TIMESTAMP compute_dstevent(int year, SPEC *spec, time_t offset)
{
    ...
    /* check values */
    if (spec->day < 0 || spec->day > 7 || spec->hour < 0 || spec->hour > 23 ||
        spec->minute < 0 || spec->minute > 59 || spec->month < 0 ||
        spec->month > 11 || spec->nth < 0 || spec->nth > 5)
    {
...
int tz_info(char *tzspec, char *tzname, char *std, char *dst, time_t *offset)
{
    ...
    {
        output_error("timezone %s (%s) has out-of-bounds hour offset of %i", tzname,
                     std, hours);
        return 0;
    }
    ...
    {
        output_error("timezone %s (%s) has out-of-bounds minutes offset of %i",
                     tzname, std, minutes);
        return 0;
    }
...
char *tz_locale(char *country, char *province, char *city)
{
    ...
    if (find_file(TZFILE, nullptr, R_OK, filepath, sizeof(filepath)) == nullptr)
    {
        throw_exception(
            const_cast<char *>(
                "timezone specification file %s not found in GLPATH=%s: %s"),
            TZFILE, global_gl_path.c_str(), strerror(errno));
        /* TROUBLESHOOT
                The system could not locate the timezone file
           <code>tzinfo.txt</code>. Check that the <code>etc</code> folder is
           included in the '''GLPATH''' environment variable and try again.
         */
    }
    fp = fopen(filepath, "r");
    if (fp == nullptr)
    {
        throw_exception(const_cast<char *>("%s: access denied: %s"), filepath,
                        strerror(errno));
        /* TROUBLESHOOT
                The system was unable to read the timezone file.  Check that the
           file has the correct permissions and try again.
         */
    }
    while (fgets(buffer, sizeof(buffer), fp) != nullptr)
    {
        ...
        {
            while (iswspace(locale[0]) && locale[0] != '\0')
                locale++; /* trim left white */
    ...
    throw_exception(
        const_cast<char *>("tz_locale(char *country='%s', char *province='%s', "
                           "char *city='%s'): not tzinfo entry found"),
...
char *tz_name(char *tzspec)
{
    ...
    else
    {
        ...
        /*	don'tTROUBLESHOOT
                The timezone specification was not found in the timezone subsystem.
           Double-check the spelling and format of the specification.
        */
        return nullptr;
    }
...
void load_tzspecs(char *tz)
{
    ...
    {
        throw_exception(
            const_cast<char *>("timezone '%s' was not understood by tz_name."), tz);
        /* TROUBLESHOOT
                The specific timezone is not valid.
                Try using a valid timezone or add the desired timezone to the
           timezone file <code>.../etc/tzinfo.txt</code> and try again.
         */
    }
    ...
    while (fgets(buffer, sizeof(buffer), fp))
    {
        ...
        /* TZ spec */
        form =
            sscanf(buffer, "%[^,],M%d.%d.%d/%d:%d,M%d.%d.%d/%d:%d", tzname,
        ...
        if (tz != nullptr && pTzname != nullptr &&
            strcmp(pTzname, current_tzname) != 0)
        {
            continue;
        }
        ...
        {
            throw_exception(
                const_cast<char *>("%s(%d): %s is not a valid timezone spec"),
                filepath, linenum, buffer);
            /* TROUBLESHOOT
                    The timezone specification is not valid.  Verify the syntax of the
               timezone spec and that it is defined in the timezone file
    ...
    if (found == 0)
    {
        output_warning("%s(%d): timezone spec '%s' not found in 'tzinfo.txt', will "
                       "include no DST information",
                       filepath, linenum, current_tzname);
    }
                                 ...
                                 int size)
{
    ...
    if (delta_t != 0)
    {
        ...
        /* Figure out the nanosecond portion - bias slightly - similar code in
         * recorders*/
    ...
    if (ts >= 365 * DAY)
    {
        ...
        if (ts >= 0)
        {
            if (ts < TS_NEVER)
            {
                ...
                /* TROUBLESHOOT
                        An attempt to convert a timestamp to a date/time string has
                   failed because the timezone isn't valid. This is most likely an
...
TIMESTAMP convert_to_timestamp(const char *value)
{
    ...
    if (sscanf(value, "%d-%d-%d %d:%d:%d %[-+:A-Za-z0-9]", &Y, &m, &d, &H, &M, &S,
               tz) >= 3)
    {
        int isdst = (strcmp(tz, tzdst) == 0) ? 1 : 0;
        DATETIME dt = {static_cast<short>(Y),
                       static_cast<short>(m),
                       static_cast<short>(d),
                       static_cast<short>(H),
                       static_cast<short>(M),
                       static_cast<short>(S),
                       0,
                       static_cast<short>(isdst)}; /* use GMT if tz is omitted */
        strncpy(dt.tz, tz, sizeof(dt.tz));
        return mkdatetime(&dt);
    }
    /* scan ISO format date/time */
    else if (global_dateformat == DF_ISO &&
             sscanf(value, "%d/%d/%d %d:%d:%d %[-+:A-Za-z0-9]", &Y, &m, &d, &H,
                    &M, &S, tz) >= 3)
    {
        int isdst = (strcmp(tz, tzdst) == 0) ? 1 : 0;
        DATETIME dt = {static_cast<short>(Y),
    ...
    /* scan US format date/time */
             ...
             sscanf(value, "%d/%d/%d %d:%d:%d %[-+:A-Za-z0-9]", &m, &d, &Y, &H,
                    &M, &S, tz) >= 3)
    {
        int isdst = (strcmp(tz, tzdst) == 0) ? 1 : 0;
        DATETIME dt = {static_cast<short>(Y),
    ...
    /* scan EURO format date/time */
             ...
             sscanf(value, "%d/%d/%d %d:%d:%d %[-+:A-Za-z0-9]", &d, &m, &Y, &H,
                    &M, &S, tz) >= 3)
    {
        int isdst = (strcmp(tz, tzdst) == 0) ? 1 : 0;
        DATETIME dt = {static_cast<short>(Y),
                                     ...
                                     double *dbl_time_value)
{
    ...
    *dbl_time_value = 0.0;
        ...
        strchr(value, 'z') != nullptr)
    {
        ...
        if (len > 0 && (normalized[len - 1] == 'Z' || normalized[len - 1] == 'z'))
        {
            normalized[len - 1] = '\0';
            if (len + 4 < sizeof(normalized))
            {
                strcat(normalized, " GMT");
            }
        }
    ...
    int n1 =
        sscanf(parse_value, "%hd-%hd-%hd %hd:%hd:%lf %[-+:A-Za-z0-9]", &dt.year,
               &dt.month, &dt.day, &dt.hour, &dt.minute, &seconds_w_nano, tz);
    if (n1 >= 3)
    {
        dt.second = (unsigned int)seconds_w_nano;
        dt.nanosecond =
            (unsigned int)(1e9 * (seconds_w_nano - (double)dt.second) + 0.5);
    ...
    /* scan ISO format date/time -- nanosecond inclusive */
             ...
             sscanf(value, "%hd/%hd/%hd %hd:%hd:%lf %[-+:A-Za-z0-9]", &dt.year,
                    &dt.month, &dt.day, &dt.hour, &dt.minute, &seconds_w_nano,
                    tz) >= 3)
    {
        dt.second = (unsigned int)seconds_w_nano;
        dt.nanosecond =
    ...
    /* scan US format date/time -- nanosecond inclusive */
             ...
             sscanf(value, "%hd/%hd/%hd %hd:%hd:%lf %[-+:A-Za-z0-9]", &dt.month,
                    &dt.day, &dt.year, &dt.hour, &dt.minute, &seconds_w_nano,
                    tz) >= 3)
    {
        dt.second = (unsigned int)seconds_w_nano;
        dt.nanosecond =
    ...
    /* scan EURO format date/time -- nanosecond inclusive */
             ...
             sscanf(value, "%hd/%hd/%hd %hd:%hd:%lf %[-+:A-Za-z0-9]", &dt.day,
                    &dt.month, &dt.year, &dt.hour, &dt.minute, &seconds_w_nano,
                    tz) >= 3)
    {
        dt.second = (unsigned int)seconds_w_nano;
        dt.nanosecond =
    ...
    *dbl_time_value =
        (double)(output_value) + ((double)(dt.nanosecond) / 1000000000.0);

    return output_value;
}

double timestamp_to_days(TIMESTAMP t) { return (double)t / DAY; }

double timestamp_to_hours(TIMESTAMP t) { return (double)t / HOUR; }

double timestamp_to_minutes(TIMESTAMP t) { return (double)t / MINUTE; }

double timestamp_to_seconds(TIMESTAMP t) { return (double)t / SECOND; }
...
#define NYEARS 50
    ...
    output_test("BEGIN: daylight saving time event test for TZ=%s...",
                current_tzname);
    ...
    for (year = 0; year < NYEARS; year++)
    {
        ...
        {
            for (ts = (event[test])[year] - 2 * step;
                 ts < (event[test])[year] + 2 * step; ts += step)
            {
                DATETIME t;
                if (local_datetime(ts, &t))
                {
                    if (last_t.is_dst != t.is_dst)
                        output_test(
                            "%s + %s = %s",
                            strdatetime(&last_t, buf1, sizeof(buf1)) ? buf1 : "(invalid)",
                            steptxt,
                            strdatetime(&t, buf2, sizeof(buf2)) ? buf2 : "(invalid)");
                    last_t = t;
                    succeeded++;
                }
                else
                {
                    output_test(
                        "FAILED: unable to convert ts=%" FMT_INT64 "d to local time", ts);
                    failed++;
                }
            }
        }
    ...
    for (ts = DAY + tzoffset; ts < DAY * 365 * NYEARS; ts += step)
    {
        ...
        if (local_datetime(ts, &t))
        {
            ...
            if (tt == TS_INVALID)
            {
                output_test("FAILED: unable to extract %04d-%02d-%02d %02d:%02d:%02d "
                            "%s (dow=%s, doy=%d)",
                            t.year, t.month, t.day, t.hour, t.minute, t.second, t.tz,
                            dow[t.weekday], t.yearday);
                failed++;
            }
            else if (tt != ts)
            {
                output_test("FAILED: unable to match %04d-%02d-%02d %02d:%02d:%02d %s "
                            "(dow=%s, doy=%d)\n    from=%s, to=%s",
                            t.year, t.month, t.day, t.hour, t.minute, t.second, t.tz,
                            dow[t.weekday], t.yearday, buf1, buf2);
                failed++;
            }
            else if (convert_to_timestamp(buf1) != ts)
            {
                output_test(
                    "FAILED: unable to convert %04d-%02d-%02d %02d:%02d:%02d %s "
                    "(dow=%s, doy=%d) back to a timestamp\n    from=%s, to=%s",
                    t.year, t.month, t.day, t.hour, t.minute, t.second, t.tz,
                    dow[t.weekday], t.yearday, buf1, buf2);
                output_test("        expected %" FMT_INT64 "d but got %" FMT_INT64 "d",
                            ts, convert_to_timestamp(buf1));
                failed++;
            }
            else
                succeeded++;
        ...
        {
            output_test("FAILED: timestamp_test: unable to convert ts=%" FMT_INT64
                        "d to local time",
                        ts);
            failed++;
        }
    ...
    output_test("END: daylight saving time tests for %d to %d", YEAR0,
                YEAR0 + NYEARS);
    output_verbose("daylight saving time tests: %d succeeded, %d failed (see "
                   "'%s' for details)",
                   succeeded, failed, global_testoutputfile);
    return failed;
}
...
double timestamp_get_part(void *x, const char *name)
{
    ...
    if (strcmp(name, "seconds") == 0)
        return (double)ts;
    if (strcmp(name, "minutes") == 0)
        return (double)ts / 60;
    if (strcmp(name, "hours") == 0)
        return (double)ts / 3600;
    if (strcmp(name, "days") == 0)
        return (double)ts / 86400;
    if (local_datetime(ts, &dt))
    {
        ...
        if (strcmp(name, "weekday") == 0)
            return (double)dt.weekday;
        if (strcmp(name, "yearday") == 0)
            return (double)dt.yearday;
        if (strcmp(name, "isdst") == 0)
            return (double)dt.is_dst;
    }
    return QNAN;
}
...
int is_soft_timestamp(TIMESTAMP t) { return t >= -TS_MAX && t < 0 ? 1 : 0; }
