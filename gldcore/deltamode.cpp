...
DELTAPROFILE *delta_getprofile(void) { return &profile; }

/** Initialize the delta mode code

        This call must be completed before the first call to any delta mode
 code. If the call fails, no delta mode code can be executed.  Failure does not
...
STATUS delta_init(void) {
  ...
  if (global_deltamode_force_preferred_order == true) {
    ...
    if (ordered_module == nullptr) {
      output_error(
          "deltamode: unable to allocate memory for module order tracking");
      /*  TROUBLESHOOT
      While attempting to allocate memory for a deltamode tracking array, an
      error occurred. Please try again.  If the error persists, please submit an
      issue via the ticketing system.
      */
      return FAILED;
    }
  ...
  for (module = module_get_first(); module != nullptr;
    ...
    if (0 != module->deltadesired) {
      ...
      if (global_deltamode_force_preferred_order == true) {
        ...
        // Default else - other module, don't care - already set false in
        // tracker
      }
  ...
  if (0 == delta_modulelist) {
    ...
    /* TROUBLESHOOT
      ...
      Try freeing up memory by making more heap available or making the model
      smaller.
     */
    return FAILED;
  }
  ...
  for (obj = object_get_first(); obj != nullptr; obj = object_get_next(obj)) {
    if (obj->flags & OF_DELTAMODE) {
      if (!obj->oclass->update)
        output_debug("object '%s' requested deltamode updates but the class "
                     "'%s' does not export the update function",
                     object_name(obj, temp_name_buff, 63), obj->oclass->name);
      delta_objectcount++;
      if (obj->rank > toprank) {
        toprank = obj->rank;
      }
  ...
  for (n = 0; n <= toprank; n++) {
    if (rankcount[n] > 0) {
      ...
      if (!ranklist[n]) {
        ...
        /* TROUBLESHOOT
          Deltamode operation requires more memory than is available.
          Try freeing up memory by making more heap available or making the
          model smaller.
         */
        return FAILED;
      }
...
/** Determine whether any modules desire operation in delta mode and if so at
 what DT
        @return DT=0 if no modules want to run in delta mode; DT>0 if at least
 one desires running in delta mode; DT=DT_INVALID on error.
 **/
DT delta_modedesired(DELTAMODEFLAGS *flags) {
  DT dt_desired = DT_INFINITY;
  DT dt;
  MODULE **module;
  for (module = delta_modulelist; module < delta_modulelist + delta_modulecount;
       module++) {
    dt = (*(*module)->deltadesired)(flags);

    if (dt == DT_INVALID) {
      output_error(
          "module '%s' return invalid/infinite dt for delta mode inquiry",
          (*module)->name);
      /* TROUBLESHOOT
         A module returned an invalid time-difference for deltamode operation.
         Generally, this is an internal error and should be reported to the
         GridLAB-D developers.
       */
      return DT_INVALID;
    }
...
DT delta_update(void) {
  ...
  unsigned int delta_iteration_remaining, delta_iteration_count,
      delta_forced_iteration, delta_federation_iteration_remaining;
  ...
  if (0 == timestep) {
    ...
    /* TROUBLESHOOT
       A module failed to complete a preupdate operation correctly while
       operating in deltamode. Generally, this is an internal error and should
       be reported to the GridLAB-D developers.
     */
    return DT_INVALID;
  }
  ...
  global_deltamode_maximumtime =
      (DELTAT)(global_deltamode_maximumtime_pub + 0.5);

  /* process updates until mode is switched or time limit elapses */
  for (global_deltaclock = 0; global_deltaclock < global_deltamode_maximumtime;
       global_deltaclock += timestep) {
    /* Check to make sure we haven't reached a stop time */
    global_delta_curr_clock =
        dbl_curr_clk_time + (double)global_deltaclock / (double)DT_SECOND;
    ...
    // Call some "internals" - replicate what was in exec - done after the
    // transform to sequence properly
    ...
    while (delta_federation_iteration_remaining > 0) {
      ...
      while (delta_iteration_remaining > 0) /* Iterate on this delta timestep */
      {
        ...
        for (n = 0; n < delta_objectcount; n++) {
          d_obj = delta_objectlist[n]; /* Shouldn't need nullptr checks, since
                                          they were done above */
          d_oclass = d_obj->oclass;

          /* See if the object is in service or not */
          if ((d_obj->in_svc_double <= global_delta_curr_clock) &&
              (d_obj->out_svc_double >= global_delta_curr_clock)) {
            if (d_oclass->update) /* Make sure it exists - init should handle
                                     this */
            {
              /* Call the object-level interupdate */
              interupdate_mode_result = static_cast<SIMULATIONMODE>(
                  d_oclass->update(d_obj, global_clock, global_deltaclock,
                                   timestep, delta_iteration_count));
              ...
              case SM_DELTA:
                ...
                break;
              case SM_ERROR:
                output_error("delta_update(): update failed for object \'%s\'",
                             object_name(d_obj, temp_name_buff, 63));
                /* TROUBLESHOOT
                   An object failed to update correctly while operating in
                   deltamode. Generally, this is an internal error and should be
                   reported to the GridLAB-D developers.
                 */
                return DT_INVALID;
              case SM_EVENT:
        ...
        interupdate_mode_result =
            delta_interupdate(timestep, delta_iteration_count);

        /* Check interupdate return statii */
        if (interupdate_mode_result == SM_ERROR) {
          output_error("delta_update(): interupdate failed");
          /* TROUBLESHOOT
             A module failed to complete an interupdate correctly while
             operating in deltamode. Generally, this is an internal error and
             should be reported to the GridLAB-D developers.
           */
          return DT_INVALID;
        }
        ...
        /* Now reconcile with object-level (if called) -- error is already
         * handled */
        if ((interupdate_mode_result != SM_EVENT) &&
            (interupdate_mode == SM_EVENT)) {
          interupdate_mode =
              interupdate_mode_result; /* It is SM_DELTA or SM_DELTA_ITER, which
                                          trumps SM_EVENT */
        } else if ((interupdate_mode_result == SM_DELTA_ITER) &&
                   (interupdate_mode == SM_DELTA)) {
          interupdate_mode =
              interupdate_mode_result; /* Sets us to SM_DELTA_ITER */
        }
        /* defaulted else - interupdate is SM_DELTA_ITER or both are, so it
         * doesn't matter */
      ...
      if (delta_iteration_remaining == 0) {
        ...
        /*  TROUBLESHOOT
        While performing the interupdate portion of the deltamode updates, too
        many reiterations were requested, so the simulation failed to progress
        forward.  You can increase the iteration count with the global variable
      ...
      if (clockupdate_result == SM_DELTA_ITER) {
        ...
        if (interupdate_mode == SM_EVENT) {
          output_error(
              "delta_update(): delta_interupdate() wanted to exit deltamode. "
              "However, delta_clockupdate() wanted to continue in deltamode. "
              "This shouldn't happen. Please check your object's "
              "delta_clockupdate function to determine what went wrong.");
          /*  TROUBLESHOOT
          While performing the interupdate portion of the deltamode updates,
          all objects determined that it was ok to exit deltamode. When this is
          the case, objects delta_clockupdate can only return DELTA_ITER
          indicating that there needs to be a reiteration of the current
          deltamode timestep or EVENT indicating that the simulation can exit
          deltamode. Please check your object's delat_clockupdate function to
          make sure it adhere's to this behavior.
          */
          return DT_INVALID;
        } else {
          break;
        ...
        if (interupdate_mode == SM_DELTA) {
          output_error(
              "delta_update(): delta_interupdate() wanted to move to the next "
              "deltamode timestep. However, delta_clockupdate() wanted to exit "
              "deltamode. This shouldn't happen. Please check your object's "
              "delta_clockupdate function to determine what went wrong.");
          /*  TROUBLESHOOT
          While performing the interupdate portion of the deltamode updates,
          all objects determined that it was ok move to the next deltamode
          timestep. When this is the case, objects delta_clockupdate can only
          return DELTA_ITER indicating that there needs to be a reiteration of
          the current deltamode timestep or DELTA indicating that the simulation
          can move to the next deltamode timestep. Please check your object's
          delta_clockupdate function to make sure it adhere's to this behavior.
          */
          return DT_INVALID;
        } else {
          break;
        ...
        /* TROUBLESHOOT
         * A module failed to complete an clockupdate correctly while operating
         * in deltamode. Generally, this is an internal error and should be
         * reported to the GridLAB-D developers.
         */
        return DT_INVALID;
      }
    ...
    if (delta_federation_iteration_remaining == 0) {
      ...
      /*  TROUBLESHOOT
      While performing the clockupdate portion of the deltamode updates, too
      many federate reiterations were requested, so the simulation failed to
      progress forward. You can increase the iteration count with the global
      variable deltamode_iteration_limit.
      */
      return DT_INVALID;
    }
    ...
    if (interupdate_mode == SM_EVENT) {
      ...
      if (delta_forced_iteration < 1) {
        ...
        /* Normal continue - next delta timestep -- allows global overrides to
         * still take place */

        output_verbose("delta_update(): Forced deltamode reiteration - %d left",
                       delta_forced_iteration);
      }
  ...
  /* profile */
  if (profile.t_min == 0 || timestep < profile.t_min)
    profile.t_min = timestep;
  if (profile.t_max == 0 || timestep > profile.t_max)
    profile.t_max = timestep;
  ...
  if (FAILED == delta_postupdate()) {
    ...
    /* TROUBLESHOOT
       A module failed to complete an postupdate correctly while operating in
       deltamode. Generally, this is an internal error and should be reported to
       the GridLAB-D developers.
     */
    return DT_INVALID;
  }
  ...
  seconds_advance =
      (DT)(global_deltaclock / DT_SECOND); /* Initial guess, check rounding */
...
static DT delta_preupdate(void) {
  ...
  /* Would theoretically get caught by the overall zero check, but not really
   * indicate why */
  if (timestep == 0) {
    output_error("delta_preupdate(): global deltamode_timestep is zero!");
    /*  TROUBLESHOOT
    The value for the global deltamode_timestep returned as zero.  It needs to
    be a positive number, with 1 ns being the smallest timestep.  If it was
    specified below 1 ns, try again with a larger timestep.
    */
    return 0;
  }
  ...
  for (module = delta_modulelist; module < delta_modulelist + delta_modulecount;
    ...
    if ((dt == 0) || (dt == DT_INVALID)) {
      ...
      /* TROUBLESHOOT
         A module failed to complete an preupdate correctly while operating in
         deltamode. Generally, this is an internal error and should be reported
         to the GridLAB-D developers.
       */
      return 0;
    } else if (dt < timestep) {
      timestep = dt;
...
static SIMULATIONMODE delta_interupdate(DT timestep,
  ...
  for (module = delta_modulelist; module < delta_modulelist + delta_modulecount;
       module++) {
    result = (*module)->interupdate(*module, global_clock, global_deltaclock,
                                    timestep, iteration_count_val);
    switch (result) {
    case SM_DELTA_ITER:
      mode = SM_DELTA_ITER;
      break;
...
static SIMULATIONMODE delta_clockupdate(DT timestep,
                                        SIMULATIONMODE interupdate_result) {
  clock_t t = clock();
  double nextTime = 0;
  ...
  if (interupdate_result == SM_DELTA) {
    ...
    nextTime =
        global_delta_curr_clock + ((double)timestep) / ((double)DT_SECOND);
    for (module = delta_modulelist;
         module < (delta_modulelist + delta_modulecount); module++) {
      if ((*module)->deltaClockUpdate != nullptr) {
        result = (*module)->deltaClockUpdate(*module, nextTime, timestep,
                                             interupdate_result);
        switch (result) {
        case SM_DELTA_ITER:
          output_error("delta_clockupdate(): It is too late to reiterate on "
    ...
    rv = SM_EVENT;
    // we are exiting deltamode so we need to determine the timestep to the next
    // whole second.
    ...
    for (module = delta_modulelist;
         module < (delta_modulelist + delta_modulecount); module++) {
      if ((*module)->deltaClockUpdate != nullptr) {
        result = (*module)->deltaClockUpdate(
            *module, nextTime, exitDeltaTimestep, interupdate_result);
        switch (result) {
        case SM_DELTA_ITER:
          output_error("delta_clockupdate(): It is too late to reiterate on "
                       "the current time step.");
          return SM_ERROR;
        case SM_DELTA:
          output_error("delta_clockupdate(); Every object wishes to exit delta "
                       "mode. It is too late to stay in it.");
          /* default else - leave it as is (SM_DELTA_ITER) */
          return SM_ERROR;
          break;
        case SM_ERROR:
