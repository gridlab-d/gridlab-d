...
int gldvar_isset(GLDVAR *var, unsigned int n) { return var[n].addr != nullptr; }
...
void *gldvar_getaddr(GLDVAR *var, unsigned int n) { return var[n].addr; }
void *gldvar_getprop(GLDVAR *var, unsigned int n) { return var[n].prop; }
PROPERTYTYPE gldvar_gettype(GLDVAR *var, unsigned int n) {
  return var[n].prop->ptype;
}
char *gldvar_getname(GLDVAR *var, unsigned int n) { return var[n].prop->name; }
...
int write_term(char *buffer, double a, char *x, int n, bool first) {
  ...
  if (fabs(a) > 1e-6) // non-zero
  {
    ...
    {
      if (fabs(a - 1) < 1e-6)
        len += snprintf(buffer + len, sizeof(buffer) - len, "%s", first ? "1" : "+1");
      else if (fabs(-a - 1) < 1e-6)
        len += snprintf(buffer + len, sizeof(buffer) - len, "%s", "-1");
      else
        len += snprintf(buffer + len, sizeof(buffer) - len, first ? "%.4f" : "%+.4f", a);
    } else // z^n term
    {
      if (fabs(a - 1) > 1e-6) // non-unitary coefficient
        len += snprintf(buffer + len, sizeof(buffer) - len, "%.4f", a);
      else if (fabs(-a - 1) < 1e-6) // -1 coefficient
        len += snprintf(buffer + len, sizeof(buffer) - len, "-");
      len += snprintf(buffer + len, sizeof(buffer) - len, "%s", x); // domain variable
      if (n > 1)                             // higher-order
                          ...
                          double *b)       ///< numerator coefficients
{
  ...
  if (global_debug_output) {
    ...
    output_debug("defining transfer function %s(%s) = %s%s%s/%s%s%s, "
                 "step=%.0fs, skew=%.0fs",
                 name, domain, count1 > 1 ? "(" : "", num,
                 count1 > 1 ? ")" : "", count2 > 1 ? "(" : "", den,
                 count2 > 1 ? ")" : "", timestep, timeskew);
  }
  return 1;
}
...
int get_source_type(PROPERTY *prop) {
  ...
  case PT_random:
    ...
    break;
  default:
    output_error("tranform/get_source_type(PROPERTY *prop='%s'): unsupported "
                 "source property type '%s'",
                 prop->name, property_getspec(prop->ptype)->name);
    break;
  }
  return source_type;
}
int transform_add_filter(
    OBJECT *target_obj,    /* pointer to the target object (lhs) */
    ...
    PROPERTY *source_prop) /* schedule object associated with target value, used
                              if stype == XS_SCHEDULE */
{
  ...
  if (tf == nullptr) {
    output_error("transform_add_filter(source='%s:%s',filter='%s',target='%s:%"
                 "s'): transfer function not defined",
                 object_name(target_obj, buffer1, sizeof(buffer1)),
                 target_prop->name, filter,
                 object_name(source_obj, buffer2, sizeof(buffer2)),
                 source_prop->name);
    return 0;
  }
  ...
  // build tranform
  xform->source =
      static_cast<double *>(object_get_addr(source_obj, source_prop->name));
  xform->source_type =
      static_cast<TRANSFORMSOURCE>(get_source_type(source_prop));
  xform->target_obj = target_obj;
  xform->target_prop = target_prop;
  xform->function_type = XT_FILTER;
  xform->tf = tf;
  xform->y =
      static_cast<double *>(object_get_addr(target_obj, target_prop->name));
  xform->t2 =
      (int64)(global_starttime / tf->timestep) * tf->timestep + tf->timeskew;
  xform->t2_dbl =
      floor((double)global_starttime / tf->timestep) * tf->timestep +
      tf->timeskew;
  ...
  if (global_debug_output) {
    output_debug(
        "added filter '%s' from source '%s:%s' to target '%s:%s'", filter,
        object_name(target_obj, buffer1, sizeof(buffer1)), target_prop->name,
        object_name(source_obj, buffer2, sizeof(buffer2)), source_prop->name);
  }
  return 1;
}

int transform_add_external(
    OBJECT *target_obj,    /* pointer to the target object (lhs) */
    PROPERTY *target_prop, /* pointer to the target property */
                              ...
                              if stype == XS_SCHEDULE */
{
  ...
  if ((xform->function = module_get_transform_function(function)) == nullptr) {
    output_error("transform_add_external(source='%s:%s',function='%s',target='%"
                 "s:%s'): function is not defined (probably a missing or "
                 "invalid extern directive)",
                 object_name(target_obj, buffer1, sizeof(buffer1)),
                 target_prop->name, function,
                 object_name(source_obj, buffer2, sizeof(buffer2)),
                 source_prop->name);
    free(xform);
    return 0;
  ...
  gldvar_set(xform->plhs, 0, object_get_addr(target_obj, target_prop->name),
             target_prop);
  xform->target_obj = target_obj;
  xform->target_prop = target_prop;
  ...
  gldvar_set(xform->prhs, 0, object_get_addr(source_obj, source_prop->name),
             source_prop);

  xform->next = schedule_xformlist;
  schedule_xformlist = xform;
  output_debug("added external transform %s:%s <- %s(%s:%s)",
               object_name(target_obj, buffer1, sizeof(buffer1)),
               target_prop->name, function,
               object_name(source_obj, buffer2, sizeof(buffer2)),
               source_prop->name);
...
int transform_add_linear(
    TRANSFORMSOURCE stype, /* specifies the type of source */
    ...
    SCHEDULE *sched) /* schedule object assoicated with target value, if stype
                        == XS_SCHEDULE */
{
  char buffer[1024];
  TRANSFORM *xform = (TRANSFORM *)malloc(sizeof(TRANSFORM));
  ...
  xform->nlhs = 1;
  xform->source_addr =
      source; /* this assumes the double is the first member of the structure */
  ...
  xform->next = schedule_xformlist;
  schedule_xformlist = xform;
  output_debug("added linear transform %s:%s <- scale=%.3g, bias=%.3g",
               object_name(obj, buffer, sizeof(buffer)), prop->name, scale,
               bias);
  return 1;
}
...
void cast_from_double(PROPERTYTYPE ptype, void *addr, double value) {
  ...
  case PT_double:
    *(double *)addr = value;
    break;
  case PT_complex:
    ((gld::complex *)addr)->SetReal(value);
    ((gld::complex *)addr)->SetImag(0);
    break;
  case PT_bool:
    *(int32 *)addr = (value != 0);
  case PT_int16:
    *(int16 *)addr = (int16)value;
    break;
  case PT_int32:
  ...
  case PT_int64:
    *(int64 *)addr = (int64)value;
    break;
  case PT_enumeration:
    *(enumeration *)addr = (enumeration)value;
    break;
  case PT_set:
    *(gld::set *)addr = (gld::set)value;
    break;
  case PT_object:
    break;
  case PT_timestamp:
  ...
  case PT_float:
    *(float *)addr = (float)value;
    break;
  case PT_loadshape:
    ((loadshape *)addr)->load = value;
    break;
  case PT_enduse:
    ((enduse *)addr)->total.SetReal(value);
    break;
  default:
    break;
  }
                       ...
                       double *dm_time)     ///< deltamode time interaction
{
  ...
  // observable form
  if (n > sizeof(dx) / sizeof(double))
    throw_exception(const_cast<char *>("transfer function %s order too high"),
                    f->name);
  for (i = 0; i < n; i++) {
    if (i == 0)
      dx[i] = -a[i] * x[n - 1];
...
/** apply the transform, source is optional and xform.source is used when source
is nullptr
...
TIMESTAMP transform_apply(TIMESTAMP t1, TRANSFORM *xform, double *source,
  ...
  case XT_LINEAR:
#ifdef _DEBUG
    output_debug("running linear transform for %s:%s",
                 object_name(xform->target_obj, buffer, sizeof(buffer)),
                 xform->target_prop->name);
#endif
    cast_from_double(xform->target_prop->ptype, xform->target,
                     (source ? (*source) : (*(xform->source))) * xform->scale +
                         xform->bias);
    t2 = TS_NEVER;
  ...
  case XT_EXTERNAL:
#ifdef _DEBUG
    output_debug("running external transform for %s:%s",
                 object_name(xform->target_obj, buffer, sizeof(buffer)),
                 xform->target_prop->name);
#endif
    xform->retval =
        (*xform->function)(xform->nlhs, xform->plhs, xform->nrhs, xform->prhs);
  ...
  case XT_FILTER:
#ifdef _DEBUG
    output_debug("running filter transform for %s:%s",
                 object_name(xform->target_obj, buffer, sizeof(buffer)),
                 xform->target_prop->name);
#endif
    if (xform->t2 <= t1)
    ...
    *dm_time = xform->t2_dbl;
    break;
  default:
    output_error("transform_apply(): invalid function type %d",
                 xform->function_type);
    t2 = TS_INVALID;
    break;
  }
...
TIMESTAMP transform_syncall(TIMESTAMP t1, TRANSFORMSOURCE source,
  ...
  // Convert the deltamode time to an integer (in case we've been in deltamode
  // longer)
  if (t1_dbl != nullptr) {
  ...
  for (xform = schedule_xformlist; xform != nullptr; xform = xform->next) {
    ...
    if (xform->source_type & source) {
      if ((xform->source_type == XS_SCHEDULE) &&
        ...
        tskew =
            t1 -
            xform->target_obj->schedule_skew; // subtract so the +12 is 'twelve
                                              // seconds later', not earlier
