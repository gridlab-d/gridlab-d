...
gld_loadHndl::gld_loadHndl()
    : fFormatter("LATIN1", 0, this, XMLFormatter::NoEscapes,
                 (const XMLFormatter::UnRepFlags)0),
      fExpandNS(false) {
  depth = 0;
  first = last = -1;
  stack_state = EMPTY;
...
gld_loadHndl::gld_loadHndl(const char *const encodingName,
...
void gld_loadHndl::error(const SAXParseException &e) {
  output_error("XML_Load: %ls(%i:char %i)\n Message: %ls",
               (char *)e.getSystemId(), e.getLineNumber(), e.getColumnNumber(),
               (char *)e.getMessage());
  load_state = false;
}
...
void gld_loadHndl::warning(const SAXParseException &e) {
  output_warning("XML_Load: %ls(%i:char %i)\n Message: %ls",
                 (char *)e.getSystemId(), e.getLineNumber(),
                 e.getColumnNumber(), (char *)e.getMessage());
  // load_state = false;
}

void gld_loadHndl::notationDecl(const XMLCh *const name,
                                const XMLCh *const publicId,
                                const XMLCh *const systemId) {
  ;
}

/** parse_property() breaks a string into its propname and propval before
 * handing both to the module getvar/setvar methods.
 */
...
char *gld_loadHndl::read_module_prop(char *buffer, size_t len) {
  ...
  if (strcmp(propname, "major") == 0) {
    ...
    if (major == major_in) { //	current version, continue
      ...
      output_warning("The input file was built using an older major version of "
                     "the module.");
      return nullptr;
    } else if (major < major_in) { //	newer version
      output_warning("The file was built using a newer major version of the "
                     "module loading it!");
      return nullptr;
    }
    ...
    if (minor == minor_in) { //	current version, continue
      ...
      output_warning("XML::read_module_prop(): The input file was built using "
                     "an older minor version of the module.");
    } else if (minor < minor_in) { //	new version
      output_warning("XML::read_module_prop(): The file was built using a "
                     "newer minor version of the module loading it!");
    }
  } else {
...
char *gld_loadHndl::read_global_prop(char *buffer, size_t len) { return 0; }
  ...
  if (this->prop == nullptr) {
    if (strcmp(propname, "parent") == 0) {
      if (strcmp(propname, "root") == 0) {
        ...
        add_unresolved(obj, PT_object, (void *)&obj->parent, oclass, buffer,
                       "XML", 42, UR_RANKS);
      }
 ...
 */
void gld_loadHndl::characters(const XMLCh *const chars,
...
/**	endDocument() is called when the end of the file is reached.  Any
 * post-processing within the parser should occur here.
 */
void gld_loadHndl::endDocument() {
  ...
  output_verbose("XML_Load: %d object%s loaded.", object_get_count(),
                 object_get_count() > 0 ? "s" : "");
  if (object_count > 0)
    if (object_get_count() != this->object_count)
      output_warning("XML_Load: we expected %i objects instead!",
                     this->object_count);
  output_verbose("XML_Load: %d class%s loaded.", class_get_count(),
                 class_get_count() > 0 ? "es" : "");
  if (class_count > 0)
    if (class_get_count() != this->class_count)
      output_warning("XML_Load: we expected %i classes instead!",
                     this->class_count);
}

char *gld_loadHndl::end_element_empty(char *buffer,
                                      size_t len) { //	why are we here?
  return nullptr;
}
...
/**	endElement() is called for every tag, and should zero whatever level of
 * the stack corresponds to the tag.
 */
void gld_loadHndl::endElement(const XMLCh *const uri,
...
void gld_loadHndl::ignorableWhitespace(const XMLCh *const chars,
                                       const unsigned int length) {
  output_verbose("XML_Load: ignored: %i spaces.", length);
}

void gld_loadHndl::processingInstruction(const XMLCh *const target,
...
char *gld_loadHndl::start_element_empty(char *buffer, size_t len,
...
char *gld_loadHndl::start_element_load(char *buffer, size_t len,
                                       const Attributes &attributes) {
  ...
  if (strcmp(buffer, "global") == 0) {
    ...
    wcstombs(module_name,
             (const wchar_t *)attributes.getValue((const XMLCh *)wcs_type),
             1024);
    ...
    if ((module = module_load(module_name, 0, nullptr)) == nullptr) {
      ...
      if (major != 0) {
        ...
        if (major < major_in) {
          output_warning("XML::read_module_prop(): The input file was built "
                         "using an older major version of the module.");
        ...
        if (minor == minor_in) { //	current version, continue
          ...
          output_warning("XML::read_module_prop(): The input file was built "
                         "using an older minor version of the module.");
        } else if (minor < minor_in) { //	new version
          output_warning("XML::read_module_prop(): The file was built using a "
                         "newer minor version of the module loading it!");
        }
...
char *
gld_loadHndl::start_element_module_build_object(const Attributes &attributes) {
  static wchar_t str_id[3], str_name[5], str_type[5];
  static size_t str_id_len = mbstowcs(str_id, "id", 3),
                str_name_len = mbstowcs(str_name, "name", 5),
                str_type_len = mbstowcs(str_type, "type", 5);
  char object_type[64], object_id[64], object_name[64];
  char *temp = nullptr;
  char *retval = nullptr;
  int first = -1, last = -1;
  ...
  if (attributes.getIndex((const XMLCh *)str_id) < 0) {
    ...
    wcstombs(object_id, (wchar_t *)attributes.getValue((const XMLCh *)str_id),
             64);
  ...
  if (attributes.getIndex((const XMLCh *)str_name) < 0) {
    ...
    wcstombs(object_name,
             (wchar_t *)attributes.getValue((const XMLCh *)str_name), 64);
...
char *gld_loadHndl::start_element_module(char *buffer, size_t len,
                                         const Attributes &attributes) {
  //	check generic props
  if (0 == strcmp(buffer, "object")) {
    ...
    return start_element_module_build_object(attributes);
           ...
           strcmp(buffer,
                  "properties")) { //	check for module props ~ note plural tag
...
char *gld_loadHndl::start_element_module_prop(char *buffer, size_t len,
  ...
  if (global_find(pname)) {
    ...
    output_warning("XML: start_element_module_prop: property \"\" not found, "
                   "initializing");
    strcpy(propname, pname);
  }
  return nullptr;
...
char *gld_loadHndl::start_element_object(char *buffer, size_t len,
                                         const Attributes &attributes) {
  //	check generic props
  ...
  prop = class_find_property(
      obj->oclass,
      buffer); //	prop = nullptr is valid for hardcoded properties. -MH
  return nullptr;
}

char *gld_loadHndl::start_element_object_prop(char *buffer, size_t len,
                                              const Attributes &attributes) {
  //	we shouldn't be here.
  return nullptr;
}

char *gld_loadHndl::start_element_global(char *buffer, size_t len,
                                         const Attributes &attributes) {
  //	check generic props
...
char *gld_loadHndl::start_element_global_prop(char *buffer, size_t len,
                                              const Attributes &attributes) {
  //	we may not supposed to be here?
  return nullptr;
}

char *gld_loadHndl::start_element_clock(char *buffer, size_t len,
                                        const Attributes &attributes) {
...
char *gld_loadHndl::start_element_clock_prop(char *buffer, size_t len,
                                             const Attributes &attributes) {
  return nullptr;
}

#define ADD_KEYWORD(A)                                                         \
  stack_ptr = new gldStack();                                                  \
  strcpy(stack_ptr->object_type, "module");                                    \
  strcpy(stack_ptr->keyword, (A));

void gld_loadHndl::startElement(const XMLCh *const uri,
                                const XMLCh *const localname,
                                const XMLCh *const qname,
                                const Attributes &attributes) {
  char buffer[128];
  ...
  if (wcstombs(buffer, (const wchar_t *)qname, 128) < 1) {
    output_error(
        "startElement: Unable to convert element tag name from wchar to char!");
    load_state = 0;
    return;
  }
...
char *gld_loadHndl::build_object_vect(int start, int end) {
  ...
  for (i = (start == -1) ? 0 : start; i <= last;
       ++i) { /* "if start == -1, use 0, else use start" */
