...
#define OF_DELTAMODE                                                           \
  0x0040 /**< Object flag; indicates that delta mode is enabled on this object \
          */
#define OF_FORECAST                                                            \
...
public:
  ...
  struct {
    char *(*find_file)(const char *name, const char *path, int mode,
                       char *buffer, int len);
  } file;
...
PROPERTY *object_get_property(OBJECT *obj, const PROPERTYNAME name,
                              PROPERTYSTRUCT *part);
...
int object_set_value_by_addr(OBJECT *obj, void *addr, char *value,
                             PROPERTY *prop);
...
int object_get_value_by_name(OBJECT *obj, const PROPERTYNAME name, char *value,
                             int size);
int object_get_value_by_addr(OBJECT *obj, void *addr, char *value, int size,
                             PROPERTY *prop);
...
FUNCTIONADDR object_get_function(CLASSNAME classname,
...
int object_get_namespace(OBJECT *obj, char *buffer,
                         int size); /**< get the object's namespace */
int object_open_namespace(
    char *space);             /**< open a new namespace and make it current */
int object_close_namespace(); /**< close the current namespace and restore the
                                 previous one */
int object_select_namespace(char *space); /**< change to another namespace */
int object_push_namespace(char *space);   /**< change to another namespace and
                                             push the one onto a stack */
NAMESPACE *
object_pop_namespace(); /**< restore the previous namespace from stack */

/* forecasting API */
FORECAST *
forecast_create(OBJECT *obj,
                char *specs); /**< create a forecast using the specifications
                                 and append it to the object's forecast block */
FORECAST *forecast_find(
    OBJECT *obj,
    char *name); /**< find the forecast for the named property, if any */
double
forecast_read(FORECAST *fc,
              TIMESTAMP ts); /**< read the forecast value for the time ts */
void forecast_save(FORECAST *fc, TIMESTAMP ts, int32 tstep, int n_values,
                   double *data);

/* remote data access */
void *object_remote_read(void *local, OBJECT *obj,
                         PROPERTY *prop); /** access remote object data */
void object_remote_write(void *local, OBJECT *obj,
                         PROPERTY *prop); /** access remote object data */
...
// #define object_parent(X) ((X)?(X)->parent:nullptr) /**< get the parent of the
// object */ #define object_rank(X) ((X)?(X)->name:-1) /**< get the rank of the
// object */
//
// #define OBJECTDATA(X,T) ((T*)((X)?((X)+1):nullptr)) /**< get the object data
// structure */ #define GETADDR(O,P) ((O)?((void*)((char*)((O)+1)+(unsigned
// int64)((P)->addr))):nullptr) /**< get the addr of an object's property */
// #define OBJECTHDR(X) ((X)?(((OBJECT*)X)-1):nullptr) /**< get the header from
// the object's data structure */
//
// #define MY (((OBJECT*)this)-1)
// #define MYPARENT (MY->parent) /**< get the parent from the object's data
// structure */ #define MYCLOCK (MY->clock) /**< get an object's own clock */
// #define MYRANK (MY->rank) /**< get an object's own rank */

// OBJECT ACCESSORS
template <typename T> constexpr int object_size(const T *obj) {
  return obj ? obj->size : -1;
}

template <typename T> constexpr int object_id(const T *obj) {
  return obj ? obj->id : -1;
}
...
template <typename T> constexpr int object_rank(const T *obj) {
  return obj ? obj->name
             : -1; // Note: 'name' used as rank? Double check that field!
}
...
template <typename T, typename U> constexpr T *object_data(U *obj) {
  static_assert(alignof(U) >= alignof(T),
                "U does not meet alignment requirements for T");
  return obj ? reinterpret_cast<T *>(obj + 1) : nullptr;
}

// template<typename T>
// constexpr OBJECT* object_header(T* data) {
//	return data ? static_cast<OBJECT*>(reinterpret_cast<OBJECT*>(data) - 1)
//: nullptr;
// }

template <typename T> constexpr OBJECT *object_header(T *data) {
  return data ? const_cast<OBJECT *>(reinterpret_cast<const OBJECT *>(data) - 1)
              : nullptr;
}

template <typename O, typename P>
constexpr void *get_addr(O *obj, const P *prop) {
  return obj ? static_cast<void *>(reinterpret_cast<char *>(obj + 1) +
                                   reinterpret_cast<std::uintptr_t>(prop->addr))
             : nullptr;
}

// INTERNAL this ACCESSORS
template <typename T> constexpr OBJECT *MY(T *ptr) {
...
template <typename T> constexpr auto MYRANK(T *ptr) -> decltype(MY(ptr)->rank) {
  return MY(ptr)->rank;
}
