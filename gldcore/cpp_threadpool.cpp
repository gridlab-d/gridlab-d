...
// This is a special single-threaded queue handler, for when sync mode is
// active.
void cpp_threadpool::sync_wait_on_queue() {
  ...
  while (!exiting.load()) {
    if (!lock.owns_lock())
      lock.lock();
    sync_condition.wait(lock, [=] { return !job_queue.empty(); });
...
void cpp_threadpool::wait_on_queue() {
  ...
  while (!exiting.load() && !(std::this_thread::get_id() == shutdown_id)) {
    if (!lock.owns_lock())
      lock.lock();
    condition.wait(lock, [=] { return !job_queue.empty(); });
...
void cpp_threadpool::await() {
  ...
  wait_condition.wait_for(lock, std::chrono::milliseconds(50),
