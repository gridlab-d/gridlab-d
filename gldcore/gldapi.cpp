...
int run_single_test(const std::filesystem::path &test_dir,
...
void print_test_summary(const TestSummary &summary) {
  ...
  std::cout << "Passed:       " << summary.passed_tests << " (" << std::fixed
            << std::setprecision(2)
            << (summary.total_tests > 0
                    ? (100.0 * summary.passed_tests / summary.total_tests)
                    : 0.0)
            << "%)\n";
  std::cout << "Failed:       " << summary.failed_tests << " (" << std::fixed
            << std::setprecision(2)
            << (summary.total_tests > 0
  ...
  for (const auto &[module, stats] : module_stats) {
    std::cout << "  " << module << ": " << stats.first << "/" << stats.second
              << " passed\n";
  }
  ...
  if (!failed_tests.empty()) {
    ...
    for (const auto &result : failed_tests) {
      std::cout << "  [FAIL] " << result.module << "/" << result.test_name
                << "\n";
      ...
      if (pid == -1) {
        ...
        if (WIFEXITED(status)) {
          ...
          if (is_error_test) {
            ...
            result.error_message =
                "Test terminated by signal " + std::to_string(WTERMSIG(status));
            summary.failed_tests++;
          }
        } else {
...
GLDErrorCode GridLabD::validate_api(bool verbose) {
  ...
  if (verbose)
    std::cout << "Test 3: Property get... " << std::flush;
  try {
    if (test_house_obj) {
      ...
      if (verbose)
        std::cout << "[FAIL] (no object from Test 2)\n";
      failed++;
    }
  } catch (...) {
    if (verbose)
  ...
  if (verbose)
    std::cout << "Test 4: Simulation execution... " << std::flush;
  try {
  ...
  if (verbose)
    std::cout << "Test 5: JSON checkpoint (in-memory)... " << std::flush;
  try {
    nlohmann::json checkpoint = this->get_checkpoint_json();
    bool valid = !checkpoint.empty() && checkpoint.contains("globals") &&
...
cleanup:
  ...
  if (verbose) {
    ...
    std::cout << "Passed:       " << passed << " (" << std::fixed
              << std::setprecision(2) << (100.0 * passed / (passed + failed))
              << "%)\n";
    std::cout << "Failed:       " << failed << " (" << std::fixed
              << std::setprecision(2) << (100.0 * failed / (passed + failed))
              << "%)\n\n";

    if (failed == 0) {
      std::cout << "[PASS] GridLAB-D API is functioning correctly\n";
    } else {
      std::cout << "[FAIL] GridLAB-D API has issues - some tests failed\n";
    }
    std::cout << "\n=============================================================\n\n";
  }
