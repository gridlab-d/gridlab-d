/** $Id: output.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        Implements functions that send output to the current environment's
 console, error stream, and test result string using printf style args.

        Debug, warning, errors, fatal messages should provide an indicate of the
 context in which the message is generated.

        - <b>Core messages</b> should be produced any time the core takes a
 off-normal path in the flow of the codes.  The severity of the situation
 determines which type of message it is and this is left to the programmers
 judgement, save to say that if the situation is not expected to seriously
 affect the result, it should be a warning; if the results are likely to be
 seriously affected, it should be an error; and if the result is hopelessly
 compromised it should be a fatal error. \code
        output_fatal("module_load(file='%s', argc=%d, argv=['%s',...]):
 intrinsic %s is not defined in module", file, argc,argc>0?argv[0]:"",fname);
        \endcode

        - <b>Module messages</b> should be produced un the same guidelines as
 core messages, except that fatal messages are not supported for modules.
 However, the context should provide the object, if known \code
         gl_error("office:%d occupancy_schedule '%s' day '%c' is invalid",
 object_header(this)->id, block, *p); \endcode
...
void output_prefix_enable(void) {
  ...
  case MRM_STANDALONE:
    snprintf(prefix, sizeof(prefix), "-%02d(%05d): ", cpuid, procid);
    break;
  case MRM_MASTER:
    flush_output = 1;
    snprintf(prefix, sizeof(prefix), "M%02d(%05d): ", cpuid, procid);
    break;
  case MRM_SLAVE:
    flush_output = 1;
    snprintf(prefix, sizeof(prefix), "S%02d(%05d): ", cpuid, procid);
    break;
  default:
    break;
  }
...
/**	prep_stream() sets the curr_stream objects to stdin, stdout, and stderr,
since VS2005 is picky about non-constant initializers.
**/
...
/**	Sets stderr to stdout
        ...
        This was requested to keep all the output consistantly going to the same
 output for the Java GUI, since catching messages from both stderr and stdout
...
/** Set the stdout print function
        ...
        The default function sends normal message strings to the stdout stream.
 Applications that need a handle such messages differently can request they be
 sent to the function \p call.
 **/
PRINTFUNCTION
output_set_stdout(PRINTFUNCTION call) /**< The \b printf style function to call
                                         to send text to \e stdout */
{
...
/** Set the stderr print function
        ...
        The default function sends error message strings to the stderr stream.
 Applications that need a handle such messages differently can request they be
 sent to the function \p call.
 **/
PRINTFUNCTION
output_set_stderr(PRINTFUNCTION call) /**< The \b printf style function to call
                                         to send text to \e stderr */
{
...
void output_set_delta_time_context(TIMESTAMP ts, DELTAT delta_ts) {
  convert_from_timestamp_delta(ts, delta_ts, time_context,
                               sizeof(time_context) - 1);
}
char *output_get_time_context(void) { return time_context; }
...
int output_fatal(const char *format, ...) /**< \bprintf style argument list */
{
  ...
  if (format != nullptr && strcmp(lastfmt, format) == 0 &&
    ...
    if (count > 0 && global_suppress_repeat_messages && !global_verbose_mode) {
      len = snprintf(buffer, sizeof(buffer), "last fatal error message was repeated %d times",
                    count);
      count = 0;
      if (format == nullptr)
        goto Output;
      ...
      return 0;
    va_start(ptr, format);
    vsnprintf(buffer + len, sizeof(buffer) - len, format,
...
int output_error(const char *format, ...) /**< \bprintf style argument list */
{
  ...
  if (format != nullptr && strcmp(lastfmt, format) == 0 &&
    ...
    strncpy(lastfmt, format ? format : "", sizeof(lastfmt) - 1);
    if (count > 0 && global_suppress_repeat_messages && !global_verbose_mode) {
      len = snprintf(buffer, sizeof(buffer), "last error message was repeated %d times", count);
      count = 0;
      if (format == nullptr)
        goto Output;
      else
...
int output_test(const char *format, ...) /**< \bprintf style argument list */
{
  ...
  va_start(ptr, format);
  vsnprintf(buffer, sizeof(buffer), format, ptr); /* note the lack of check on buffer overrun */
  va_end(ptr);
  ...
  if (fp == nullptr) {
    ...
    fprintf(fp, "GridLAB-D Version %s.%s\n",
            global_getvar("version.major", major_b, 32),
            global_getvar("version.minor", minor_b, 32));
...
/** Output a warning message to the stdout stream using printf style argument
 processing
        ...
        output_warning() will produce output to the standard output stream only
 when the \p global_warn_mode variable is not \b 0.  Warning messages are always
 preceded by the string "WARNING: " and a newline is always appended to the
 message.
 **/
  ...
  if (global_warn_mode) {
    ...
    if (format != nullptr && strcmp(lastfmt, format) == 0 &&
      ...
      if (count > 0 && global_suppress_repeat_messages &&
          !global_verbose_mode) {
        len = snprintf(buffer, sizeof(buffer), "last warning message was repeated %d times",
                      count);
        count = 0;
        if (format == nullptr)
          goto Output;
...
/** Output a debug message to the stdout stream using printf style argument
 processing

        output_debug() will produce output to the standard output stream only
 when the \p global_debug_output variable is not \b 0.  Debug messages are
 always preceded by the string "DEBUG: " and a newline is always appended to the
 message.
 **/
  ...
  if (global_debug_output) {
    ...
    if (format != nullptr && strcmp(lastfmt, format) == 0 &&
      ...
      if (count > 0 && global_suppress_repeat_messages &&
          !global_verbose_mode) {
        len =
            snprintf(buffer, sizeof(buffer), "last debug message was repeated %d times", count);
        count = 0;
        if (format == 0)
          goto Output;
        else
...
/** Output a verbose message to the stdout stream using printf style argument
 processing

        output_verbose() will produce output to the standard output stream only
 when the \p global_verbose_mode variable is not \b 0.  Verbose message always
 have leading spaces and an ellipsis printed before the string, and a newline is
 always appended to the message.
 **/
int output_verbose(const char *format, ...) /**< \bprintf style argument list */
{
  ...
  if (global_verbose_mode) {
    ...
    if (format != nullptr && strcmp(lastfmt, format) == 0 &&
      ...
      if (count > 0 && global_suppress_repeat_messages &&
          !global_verbose_mode) {
        len = snprintf(buffer, sizeof(buffer), "%slast verbose message was repeated %d times\n   ... ",
                      prefix, count);
        count = 0;
        if (format == 0)
          goto Output;
...
/** Output a message to the stdout stream using printf style argument processing
        ...
        output_message() will produce output to the standard output stream only
 when the \p global_quiet_mode variable is not \b 0.  A newline is always
  ...
  if (!global_quiet_mode) {
    ...
    if (count > 0 && global_suppress_repeat_messages && !global_verbose_mode) {
      len = snprintf(buffer, sizeof(buffer),
                     "%sLast message was repeated %d times\n", prefix, count);
      count = 0;
...
/** Output a raw string to the stdout stream using printf style argument
 processing

        output_raw() will produce output to the standard output stream only when
 the \p global_quiet_mode variable is not \b 0.
 **/
...
int output_xsd(char *spec) {
  ...
  char buffer[65536];
  /*if(sscanf(spec, "%[A-Za-z_0-9]::%[A-Za-z_0-9]:%s",modulename, submodulename,
  classname) == 3)
  {
          snprintf(jointname, sizeof(jointname), "%s::%s", modulename, submodulename);
  ...
  else if(sscanf(spec, "%[A-Za-z_0-9]::%[A-Za-z_0-9]",modulename, submodulename)
  == 2)
  {
          snprintf(jointname, sizeof(jointname), "%s::%s", modulename, submodulename);
          mod = module_load(jointname, 0, nullptr);
  ...
  if (classname[0] != '\0' &&
      (oclass = class_get_class_from_classname(classname)) == nullptr) {
    output_error("unable to find class '%s' in module '%s'", classname,
                 modulename);
    return 0;
  }
  // if ((strlen(submodulename) > 1))
  //	strcpy(modulename, submodulename);
  output_message("<?xml version=\"1.0\" encoding=\"utf-%d\"?>",
                 global_xml_encoding);
  output_message(
      "<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" "
      "targetNamespace=\"http://www.w3.org/\" xmlns=\"http://www.w3.org/\" "
      "elementFormDefault=\"qualified\">\n");
  for (oclass = (classname[0] != '\0' ? oclass : class_get_first_class());
       oclass != nullptr; oclass = oclass->next) {
...
int output_xsl(char *fname, int n_mods, char *p_mods[]) {
  ...
  /* document */
  fprintf(fp, "<html xsl:version=\"1.0\" "
              ...
              "xmlns=\"http://www.w3.org/1999/xhtml\">\n");
  {
    fprintf(fp, "<xsl:for-each select=\"/gridlabd\">\n");
    {
      fprintf(fp, "<head>\n");
      {
        ...
        fprintf(fp, "<title>GridLAB-D <xsl:value-of "
                    "select=\"version.major\"/>.<xsl:value-of "
                    "select=\"version.minor\"/> - <xsl:value-of "
                    "select=\"modelname\"/></title>\n");
        if (stylesheet == nullptr ||
            stylesheet->prop->ptype !=
                PT_char1024) /* only char1024 is allowed */
          fprintf(fp,
                  "<link rel=\"stylesheet\" href=\"%sgridlabd-%d_%d.css\" "
                  "type=\"text/css\"/>\n",
      ...
      fprintf(fp, "<body>\n");
      {
        ...
        fprintf(fp, "<H2><A NAME=\"global_variables\">GridLAB-D system "
                    "variables</A></H2>\n");
        ...
        while (var = global_getnext(var)) {
          ...
          fprintf(
              fp,
              "<TR><TH>%s</TH><TD><xsl:value-of select=\"%s\"/></TD></TR>\n",
        ...
        fprintf(fp, "<xsl:sort select=\"ordinal\" data-type=\"number\" "
                    "order=\"descending\"/>\n");
        fprintf(fp, "<DT>Rank <xsl:value-of select=\"ordinal\"/></DT>");
        fprintf(fp, "<xsl:for-each select=\"object\">\n");
        fprintf(fp, "<DD><a href=\"#{name}\"><xsl:value-of "
                    "select=\"name\"/></a></DD>\n");
        fprintf(fp, "</xsl:for-each>\n");
        fprintf(fp, "</xsl:for-each>\n");
        ...
        for (mod = module_get_first(); mod != nullptr; mod = mod->next) {
          ...
          fprintf(fp, "<H3><A NAME=\"modules_%s\">%s</A></H3>", mod->name,
                  mod->name);

          /* globals */
          fprintf(fp, "<TABLE BORDER=\"1\">\n");
          fprintf(fp,
                  "<TR><TH>version.major</TH><TD><xsl:value-of "
                  "select=\"%s/version.major\"/></TD></TR>",
                  mod->name);
          fprintf(fp,
                  "<TR><TH>version.minor</TH><TD><xsl:value-of "
                  "select=\"%s/version.minor\"/></TD></TR>",
                  mod->name);
          while (var = global_getnext(var)) {
            if (strncmp(var->prop->name, mod->name, strlen(mod->name)) == 0) {
              char *name = var->prop->name + strlen(mod->name) +
                           2; // offset name to after ::
              fprintf(fp,
                      "<TR><TH>%s</TH><TD><xsl:value-of "
                      "select=\"%s/%s\"/></TD></TR>",
                      name, mod->name, name);
            }
          }
          ...
          /* object dump */
          for (oclass = mod->oclass; oclass != nullptr && oclass->module == mod;
               oclass = oclass->next) {
            CLASS *pclass = oclass;
            ...
            for (pclass = oclass; pclass != nullptr; pclass = pclass->parent)
              for (prop = class_get_first_property(pclass); prop != nullptr;
                   prop = class_get_next_property(prop))
                fprintf(fp, "<TH>%s</TH>", prop->name);
            fprintf(fp, "</TR>\n");
            {
              fprintf(fp, "<xsl:for-each select=\"%s/%s_list/%s\">", mod->name,
                      oclass->name, oclass->name);
              {
                fprintf(
                    fp,
                    "<TR><TD><a name=\"#{name}\"/><xsl:value-of "
                    "select=\"name\"/> (#<xsl:value-of select=\"id\"/>)</TD>");
                for (pclass = oclass; pclass != nullptr;
                     pclass = pclass->parent)
                  for (prop = class_get_first_property(pclass); prop != nullptr;
                       prop = class_get_next_property(prop)) {
                    if (prop->ptype == PT_object)
                      fprintf(fp,
                              "<TD><a href=\"#{%s}\"><xsl:value-of "
                              "select=\"%s\"/></a></TD>",
                              prop->name, prop->name);
                    else
                      fprintf(fp, "<TD><xsl:value-of select=\"%s\"/></TD>",
                              prop->name);
                  }
                fprintf(fp, "</TR>\n");
              }
        ...
        fprintf(fp, "<table border=\"1\" width=\"100%%\"><tr><td><pre>\n");
        {
          ...
          /* heading info */
          fprintf(fp, "# Generated by GridLAB-D <xsl:value-of "
                      "select=\"version.major\"/>.<xsl:value-of "
          ...
          for (mod = module_get_first(); mod != nullptr; mod = mod->next) {
            ...
            fprintf(fp, "<xsl:for-each select=\"%s\">", mod->name);
            {
              ...
              fprintf(fp, "# %s module\n", mod->name);
              fprintf(fp, "module %s {\n", mod->name);
              fprintf(fp, "\tversion.major <xsl:value-of "
                          "select=\"version.major\"/>;\n");
              fprintf(fp, "\tversion.minor <xsl:value-of "
                          "select=\"version.minor\"/>;\n");
              while (var = global_getnext(var)) {
                if (strncmp(var->prop->name, mod->name, strlen(mod->name)) ==
                    0) {
                  char *name = var->prop->name + strlen(mod->name) +
                               2; // offset name to after ::
              ...
              for (oclass = mod->oclass;
                   oclass != nullptr && oclass->module == mod;
                   oclass = oclass->next) {
                fprintf(fp, "\n# %s::%s objects\n", mod->name, oclass->name);
                {
                  fprintf(fp, "<xsl:for-each select=\"%s_list/%s\">",
                          oclass->name, oclass->name);
                  {
                    PROPERTY *prop;
                    fprintf(fp,
                            "<a name=\"#GLM.{name}\"/>object %s:<xsl:value-of "
                            "select=\"id\"/> {\n",
                            oclass->name);
                    fprintf(fp,
                            "<xsl:if test=\"name!=''\">\tname \"<xsl:value-of "
                            "select=\"name\"/>\";\n</xsl:if>");
                    fprintf(fp, "<xsl:if test=\"parent!=''\">\tparent \"<a "
                                "href=\"#GLM.{parent}\"><xsl:value-of "
                                "select=\"parent\"/></a>\";\n</xsl:if>");
                    fprintf(fp,
                            "<xsl:if test=\"clock!=''\">\tclock '<xsl:value-of "
                            "select=\"clock\"/>';\n</xsl:if>");
                    fprintf(fp,
                            "<xsl:if test=\"in_svc!=''\">\tin_svc "
                            "'<xsl:value-of select=\"in_svc\"/>';\n</xsl:if>");
                    fprintf(fp,
                            "<xsl:if test=\"out_svc!=''\">\tout_svc "
                            "'<xsl:value-of select=\"out_svc\"/>';\n</xsl:if>");
                    fprintf(fp,
                            "<xsl:if test=\"latitude!=''\">\tlatitude "
                            "<xsl:value-of select=\"latitude\"/>;\n</xsl:if>");
                    fprintf(fp,
                            "<xsl:if test=\"longitude!=''\">\tlongitude "
                            "<xsl:value-of select=\"longitude\"/>;\n</xsl:if>");
                    fprintf(fp, "<xsl:if test=\"rank!=''\">\trank "
                                "<xsl:value-of select=\"rank\"/>;\n</xsl:if>");
                    for (prop = class_get_first_property(oclass);
                         prop != nullptr;
                         prop = class_get_next_property(prop)) {
                      if (prop->ptype == PT_object)
                        fprintf(fp,
                                "<xsl:if test=\"%s\">\t%s <a "
                                "href=\"#GLM.{%s}\"><xsl:value-of "
                                "select=\"%s\"/></a>;\n</xsl:if>",
                                prop->name, prop->name, prop->name, prop->name);
                      else
                        fprintf(fp,
                                "<xsl:if test=\"%s\">\t%s <xsl:value-of "
                                "select=\"%s\"/>;\n</xsl:if>",
