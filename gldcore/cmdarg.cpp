...
static int pidfile(int argc, char *argv[])
{
	...
	if (filename_arg == nullptr)
	{
		...
		snprintf(global_pidfile, sizeof(global_pidfile), "gridlabd-%d.pid", GETPID());
	}
	else
	{
		// Use user-specified filename
		strncpy(global_pidfile, filename_arg + 1, sizeof(global_pidfile) - 1);
		global_pidfile[sizeof(global_pidfile) - 1] = '\0'; // Ensure null-termination
	}
	return 0;
}
static int kml(int argc, char *argv[])
{
	...
	if (filename_arg == nullptr)
	{
		...
		snprintf(global_kmlfile, sizeof(global_kmlfile), "gridlabd-%d.kml", GETPID());
	}
	else
	{
		// strcpy(global_kmlfile, filename + 1);
		strncpy(global_kmlfile, filename_arg + 1, sizeof(global_kmlfile) - 1);
		global_kmlfile[sizeof(global_kmlfile) - 1] = '\0'; // Ensure null-termination
	}
	return 0;
}
				   ...
				   char *argv[]) /**< a list pointers to the argument string */
{
	...
	while (argv++, --argc > 0)
	{
		...
		for (i = 0; i < sizeof(main_cmd) / sizeof(main_cmd[0]); i++)
		{
			...
			if ((arg.sopt && strncmp(*argv, "-", 1) == 0 && strcmp((*argv) + 1, arg.sopt) == 0) || (arg.lopt && strncmp(*argv, "--", 2) == 0 && strcmp((*argv) + 2, arg.lopt) == 0) || (arg.lopt && strncmp(*argv, "--", 2) == 0 && strncmp((*argv) + 2, tmp, strlen(tmp)) == 0))
			{
