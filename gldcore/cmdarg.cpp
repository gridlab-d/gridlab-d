/** $Id: cmdarg.c 5518 2016-07-15 00:55:28Z andyfisher $
        ...
        The command-line argument processing module processes arguments as they
 are encountered.
...
void set_tabs(char *tabs, int tabdepth) {
  if (tabdepth > 32) {
    ...
    /* TROUBLESHOOT
            ...
            This suggests a problem with the internal model and should be
       reported.
     */
  } else {
    int i = 0;
  ...
  for (prop = oclass->pmap; prop != nullptr && prop->oclass == oclass;
    ...
    if (propname != nullptr) {
      ...
      if (prop->unit != nullptr) {
        ...
        for (key = prop->keywords; key != nullptr; key = key->next)
          printf("%s=%" FMT_INT64 "u%s", key->name, (int64)key->value,
                 key->next == nullptr ? "" : ", ");
        printf("} %s;", prop->name);
      } else {
      ...
      if (prop->description != nullptr)
        printf(" // %s%s", prop->flags & PF_DEPRECATED ? "(DEPRECATED) " : "",
               prop->description);
      printf("\n");
    }
  }
...
void print_class(CLASS *oclass) { print_class_d(oclass, 0); }
 ...
 * and the return status is the current status.  A return value of CMDERR
 * indicates that processing must stop immediately and FAILED status is
 * returned.
 *
 */
...
static STATUS no_cmdargs() {
  char htmlfile[1024];
  if (global_autostartgui && find_file("gridlabd.htm", nullptr, R_OK, htmlfile,
                                       sizeof(htmlfile) - 1) != nullptr) {
    char cmd[1024];
    ...
    output_error("default html file '%s' not found (workdir='%s')",
                 "gridlabd.htm", global_workdir);
...
static int server_inaddr(int argc, char *argv[]) {
  if (argc > 1) {
    strncpy(global_server_inaddr, (argc--, *++argv),
            sizeof(global_server_inaddr) - 1);
    return 1;
  } else {
...
static int version(int argc, char *argv[]) {
  output_message("GridLAB-D %d.%d.%d-%d (%.48s) %d-bit %s %s",
                 ...
                 global_version_patch, global_version_build,
...
static int pidfile(int argc, char *argv[]) {
  ...
  char *filename_arg = strchr(*argv, '=');
  if (filename_arg == nullptr) {
    // If no filename is specified, default to gridlabd-<PID>.pid
    snprintf(global_pidfile, sizeof(global_pidfile), "gridlabd-%d.pid",
             GETPID());
  } else {
    // Use user-specified filename
    strncpy(global_pidfile, filename_arg + 1, sizeof(global_pidfile) - 1);
    global_pidfile[sizeof(global_pidfile) - 1] =
...
static int kml(int argc, char *argv[]) {
  ...
  if (filename_arg == nullptr) {
    ...
    snprintf(global_kmlfile, sizeof(global_kmlfile), "gridlabd-%d.kml",
             GETPID());
  } else {
    // strcpy(global_kmlfile, filename + 1);
    strncpy(global_kmlfile, filename_arg + 1, sizeof(global_kmlfile) - 1);
    global_kmlfile[sizeof(global_kmlfile) - 1] =
        '\0'; // Ensure null-termination
  }
  return 0;
}
...
static int modattr(int argc, char *argv[]) {
  if (argc > 1) {
    ...
    pntree *ctree;
    /* lexographically sort all elements from class_get_first_class &
     * oclass->next */
    ...
    if (ctree == nullptr) {
      ...
      /* TROUBLESHOOT
              The memory allocation needed for module help to function has
         failed.  Try freeing up system memory and try again.
       */
    }
...
static int modhelp(int argc, char *argv[]) {
  if (argc > 1) {
    ...
    if (strchr(argv[0], ':') == 0) { // no class
      ...
      if (oclass == nullptr) {
        output_fatal("Unable to find class '%s' in module '%s'", cname,
                     argv[0]);
        /*	TROUBLESHOOT
                The <b>--modhelp</b> parameter was found on the command line,
           but if was followed by a class specification that isn't valid. Verify
           that the class exists in the module you specified.
        */
        return FAILED;
      }
      ...
      while ((var = global_getnext(var)) != nullptr) {
        ...
        if (proptype != nullptr) {
          if (prop->unit != nullptr) {
            printf("\t%s %s[%s];", proptype, strrchr(prop->name, ':') + 1,
                   prop->unit->name);
          } else if (prop->ptype == PT_set || prop->ptype == PT_enumeration) {
            KEYWORD *key;
            printf("\t%s {", proptype);
          ...
          if (prop->description != nullptr)
            printf(" // %s%s",
                   prop->flags & PF_DEPRECATED ? "(DEPRECATED) " : "",
                   prop->description);
          printf("\n");
        }
      }
...
static int define(int argc, char *argv[]) {
  if (argc > 1) {
    ...
    output_fatal("definition is missing");
    /* TROUBLESHOOT
            The <b>-D</b> or <b>--define</b> command line parameters was given,
       but it was not followed by a variable definition.  The correct syntax
...
static int globals(int argc, char *argv[]) {
  ...
  while ((var = global_getnext(var)) != nullptr) {
    if (n < sizeof(list) / sizeof(list[0]))
      ...
      output_fatal(
          "--globals has insufficient buffer space to sort globals list");
      return FAILED;
    }
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((var->prop->access & PA_HIDDEN) == PA_HIDDEN)
      continue;
    printf("%s=%s;", var->prop->name,
           global_getvar(var->prop->name, buffer, sizeof(buffer)) ? buffer
                                                                  : "(error)");
    ...
    if (strcmp(buffer, "none") == 0) {
      ...
      *p++ = '\0';
      if (output_redirect(buffer, p) == nullptr) {
        output_fatal("redirection of %s to '%s' failed: %s", buffer, p,
                     strerror(errno));
        /*	TROUBLESHOOT
                An attempt to redirect a standard stream from the
                command line using <b>--redirect
           <i>stream</i>:<i>destination</i></b> has failed.  The message should
           provide an indication of why the attempt failed. The remedy will
           depend on the nature of the problem.
         */
        return FAILED;
      }
    } else if (output_redirect(buffer, nullptr) == nullptr) {
      output_fatal("default redirection of %s failed: %s", buffer,
                   strerror(errno));
      /*	TROUBLESHOOT
              ...
              attempt failed. The remedy will depend on the nature of the
         problem.
...
static int environment(int argc, char *argv[]) {
  if (argc > 1)
    ...
    /*	TROUBLESHOOT
            ...
            correct syntax is <b>-e <i>keyword</i></b> or <b>--environment
       <i>keyword</i></b>.
     */
    return CMDERR;
  }
...
static int xsl(int argc, char *argv[]) {
  if (argc - 1 > 0) {
    ...
    /*	TROUBLESHOOT
            ...
            correct syntax is <b>--xsl
       <i>module1</i>[,<i>module2</i>[,...]]</b>.
     */
    return CMDERR;
  }
}
...
static int example(int argc, char *argv[]) {
  ...
  if (n != 2) {
    output_error("--example: %s name is not valid",
                 n == 0 ? "module" : "class");
    return CMDERR;
  }
  ...
  if (object == nullptr) {
    output_error("--example: unable to create example object from class %s",
                 classname);
    return CMDERR;
  }
  ...
  if (!object_init(object))
    output_warning(
        "--example: unable to initialize example object from class %s",
        classname);
...
static int mclassdef(int argc, char *argv[]) {
  ...
  n = sscanf(argv[1], "%1023[A-Za-z_]:%1024[A-Za-z_0-9]", modname, classname);
  if (n != 2) {
    output_error("--mclassdef: %s name is not valid",
                 n == 0 ? "module" : "class");
    return CMDERR;
  }
  module = module_load(modname, 0, nullptr);
  ...
  if (obj == nullptr) {
    output_error("--mclassdef: unable to create mclassdef object from class %s",
                 classname);
    return CMDERR;
  }
  global_clock = time(nullptr);
  ...
  if (!object_init(obj))
    output_warning(
        "--mclassdef: unable to initialize mclassdef object from class %s",
        classname);
...
static int workdir(int argc, char *argv[]) {
  ...
  output_verbose("working directory is '%s'",
...
static CMDARG main_cmd[] = {
    ...
    {"mt_profile", nullptr, mt_profile, "<n-threads>",
    ...
    {"kml", nullptr, kml, "[=<filename>]",
     "Output to KML (Google Earth) file of model (only supported by some "
     "modules)"},
    {"stream", nullptr, _stream, nullptr, "Toggles streaming I/O"},
    {"sanitize", nullptr, sanitize, "<options> <indexfile> <outputfile>",
     "Output a sanitized version of the GLM model"},
    {"xmlencoding", nullptr, xmlencoding, "8|16|32",
     "Set the XML encoding system"},
    {"xmlstrict", nullptr, xmlstrict, nullptr,
     "Toggle strict XML formatting (default is enabled)"},
    {"xsd", nullptr, xsd, "[module[:class]]",
     "Prints the XSD of a module or class"},
    {"xsl", nullptr, xsl, "module[,module[,...]]]",
    ...
    {"clearmap", nullptr, clearmap, nullptr,
     "Clears the process map of defunct jobs (deprecated form)"},
    {"pclear", nullptr, clearmap, nullptr,
    ...
    {"redirect", nullptr, redirect, "<stream>[:<file>]",
     "Redirects an output to stream to a file (or null)"},
    {"server_portnum", "P", server_portnum, nullptr,
     "Sets the server port number (default is 6267)"},
    {"server_inaddr", nullptr, server_inaddr, nullptr,
     "Sets the server interface address (default is INADDR_ANY, any "
     "interface)"},
...
static int help(int argc, char *argv[]) {
  ...
  for (i = 0; i < sizeof(main_cmd) / sizeof(main_cmd[0]); i++) {
    ...
    size_t len = (arg.sopt ? strlen(arg.sopt) : 0) +
                 (arg.lopt ? strlen(arg.lopt) : 0) +
                 (arg.args ? strlen(arg.args) : 0);
    if (len > indent)
      indent = len;
  }
  ...
  for (i = 0; i < sizeof(main_cmd) / sizeof(main_cmd[0]); i++) {
    ...
    if (arg.lopt == nullptr && arg.sopt == nullptr && arg.call == nullptr &&
        arg.args == nullptr) {
      ...
      if (arg.args == nullptr ||
          (arg.args[0] != '=' && strncmp(arg.args, "[=", 2) != 0))
        strcat(buffer, " ");
      if (arg.args) {
                   ...
                   char *argv[]) /**< a list pointers to the argument string */
{
  ...
  if (argc == 1 && strcmp(global_environment, "batch") != 0)
    return no_cmdargs();
  ...
  while (argv++, --argc > 0) {
    ...
    for (i = 0; i < sizeof(main_cmd) / sizeof(main_cmd[0]); i++) {
      ...
      if ((arg.sopt && strncmp(*argv, "-", 1) == 0 &&
          ...
          (arg.lopt && strncmp(*argv, "--", 2) == 0 &&
           strcmp((*argv) + 2, arg.lopt) == 0) ||
          (arg.lopt && strncmp(*argv, "--", 2) == 0 &&
           strncmp((*argv) + 2, tmp, strlen(tmp)) == 0)) {
    ...
    if (!found) {
      ...
      if (**argv != '-') {
        ...
        /* TROUBLESHOOT
           This warning is caused by an attempt to read an input file in
           self-test mode. The use of self-test model precludes reading model
           files.  Try running the system in normal more or leaving off the
           model file name.
         */
        else {
          clock_t start = clock();
      ...
      /* send cmdarg to modules */
        ...
        if (n == 0) {
          ...
          /* TROUBLESHOOT
                  ...
                  Check the command line for correct syntax and order of
             options.
           */
          status = FAILED;
        }
