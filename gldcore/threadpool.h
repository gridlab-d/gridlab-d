...
* Data get operation (#MTIGETFN) - This function gets items in the iteration
list
...
* Iterator reject test (#MTIREJECTFN) - This function allows for contingent
rejection of iteration based on data
...
Aside from giving the iterator all the functions it should use, the iterator
must also be given a name (which is used by mti_debug output) and a minimum
number of items needed before a thread is created.

Step 3 - Call the iterator using #mti_run()

If the #mti_run() call succeeds or no iterations is needed, it returns 1.  If it
fails it returns 0 and a single threaded iteration should be performed by the
caller instead.
...
typedef struct s_mtifunctions {
  ...
  /** Data compare object function prototype
      These functions compare to data object and return -1,0,1 to indicate
   relation -1 indicates the first value needs to be updated based on the second
   value 0 indicates the values are the same. 1 indicates no update to the first
   value is needed based on the second value 2 indicates no particularly
   important relation found If a value is NULL, the comparison is made with a
   reset value (e.g., zero)
   **/
  ...
  /** Data reject function prototype
      These functions determine whether to reject the iteration based on the
   value given
...
/** Multithread iterator initialization
    ...
    The return function must combine the current return data with the newly
 returned data.
