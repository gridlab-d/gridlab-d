...
STATUS instance_slave_get_data(void *buffer, size_t offset, size_t sz) {
  ...
  case MRC_MEM:
    ...
    break;
  default:
    output_error("instance_slave_get_data(): unrecognized "
                 "global_multirun_connection state");
    return FAILED;
  }
  return SUCCESS;
}
...
/** instance_slave_get_header()
        Reads a MESSAGE header off the exchange medium, copies it into a
 temporary buffer, instantiates a new full-sized message that should hold
 everything, then initializes the values for a MESSAGEWRAPPER.
 **/
...
STATUS instance_slave_parse_prop_list(char *line, linkage **root,
                                      LINKAGETYPE type) {
  char *token = 0;
  ...
  while (token != 0) {
    ...
    if (2 != tokenct) {
      ...
      output_error(
          "instance_slave_link_properties(): unable to parse '%s' (ct = %d)",
          token, tokenct);
      return FAILED;
    }
    ...
    if (obj == 0) {
      output_error(
          "instance_slave_link_properties(): unable to find object '%s'",
          objname);
      return FAILED;
    }
    prop = object_get_property(obj, propname, nullptr);
    if (prop == 0) {
      output_error("instance_slave_link_properties(): prop '%s' not found in "
                   "object '%s'",
                   propname, objname);
      return FAILED;
    }
    ...
    if (0 == link) {
      output_error(
          "instance_slave_link_properties(): malloc failure for linkage");
      return FAILED;
    }
    ...
    link->name_size =
        strlen(token) + 1; // +1 since there was a comma or space trimmed off
...
//		output_error("instance_slave_link_properties(): local_inst
//message wrapper not initialized"); 		return FAILED;
//	}
//	if(local_inst.name_size > 0){
//		buffer = (char *)malloc(local_inst.name_size);
//		if(0 == buffer){
//			output_error("instance_slave_link_properties(): malloc
//failure"); 			return FAILED;
//		}
//		memset(buffer, 0, local_inst.name_size);
//	} else {
//		output_error("instance_slave_link_properties(): non-positive
//local_inst.name_size"); 		return FAILED;
//	}
//
//
...
//	// a.b,c.d,e.f' '
//	rv = instance_slave_parse_prop_list(buffer, &(local_inst.write),
//LT_MASTERTOSLAVE); 	if(FAILED == rv){
//		output_error("instance_slave_link_properties(): writer list
//parsing failed"); 		return FAILED;
//	}
//
//	// parse readers (second half of the original string)
//	// a.b,c.d,e.f'\0'
//	rv = instance_slave_parse_prop_list(read, &(local_inst.read),
//LT_SLAVETOMASTER); 	if(FAILED == rv){
//		output_error("instance_slave_link_properties(): reader list
//parsing failed"); 		return FAILED;
//	}
//
...
//	}
//	// data_buffer will be unallocated if we do not know the link sizes a
//priori 	if(0 == local_inst.message->data_buffer){ 		local_inst.message->data_size
//= (int16 *)&(local_inst.prop_size); 		local_inst.message->data_buffer = (char
//*)malloc(local_inst.prop_size);
//	}
//	offset = 0;
//	for(link = local_inst.write; link != 0; link = link->next){
...
//			output_warning("instance_slave_link_properties(): pickle
//prop_size and calculated prop_size do not match!");
//		}
//	}
//	output_debug("instance_slave_link_properties(): exiting for slave %"
//FMT_INT64, local_inst.cacheid); 	return SUCCESS;
//}
...
//	output_verbose("wait_mmap: started wait with with fmap ts = %lli",
//tc->ts); 	rc = WaitForSingleObject(local_inst.hSlave,global_signal_timeout);
//	switch ( rc ) {
//		case WAIT_ABANDONED:
//			output_error("instance_slave_wait_mmap(): slave %d wait
//abandoned", slave_id); 			break; 		case WAIT_OBJECT_0:
//			output_verbose("instance_slave_wait_mmap(): slave %d
//wait completed", slave_id); 			status = 1; 			break; 		case WAIT_TIMEOUT:
//			output_error("instance_slave_wait_mmap(): slave %d wait
//timeout", slave_id); 			break; 		case WAIT_FAILED:
//			output_error("instance_slave_wait_mmap(): slave %d wait
//failed (error code %d)",slave_id,GetLastError()); 			break; 		default:
//			output_error("instance_slave_wait_mmap(): slave %d wait
//return code not recognized (rc=%d)", slave_id,rc); 			break;
//	}
...
////	output_verbose("instance_slave_wait_mmap(): slave %d controller reading
///links", slave_id);
//	memcpy(local_inst.cache, local_inst.filemap, local_inst.cachesize);
//
//	return status;
...
////		output_verbose("instance_slave_wait_mmap(): local_inst.buffer
///unallocated");
//		local_inst.buffer_size = 1 + local_inst.prop_size +
//sizeof(MESSAGE) + strlen(MSG_DATA); 		local_inst.buffer = (char
//*)malloc(local_inst.buffer_size); 		if(0 == local_inst.buffer){
//			output_fatal("instance_slave_wait_socket(): malloc
//failure"); 			return 0;
//		}
//	}
//
//	// read socket
//	rv = recv(local_inst.sockfd, local_inst.buffer,
//(int)local_inst.buffer_size, 0); 	printcontent(local_inst.buffer, rv);
////	output_debug("instance_slave_wait_socket(): %d = recv(%d, %x, %d, 0)",
///rv, local_inst.sockfd, local_inst.buffer, (int)local_inst.buffer_size);
//	if(0 == rv){
//		output_error("instance_slave_wait_socket(): socket closed");
//		closesocket(local_inst.sockfd);
//		return 0;
//	} else if(0 > rv){
//		output_error("instance_slave_wait_socket(): error %d when
//recv'ing data", rv); 		closesocket(local_inst.sockfd); 		return 0;
//	}
//	// copy into cache
...
//	}
//
////	output_debug("instance_slave_wait_socket(): msg memcpy(%x, %x, %d)",
///local_inst.cache, local_inst.buffer + offset, sizeof(MESSAGE));
//	memcpy(local_inst.cache, local_inst.buffer + offset, sizeof(MESSAGE));
//	offset += sizeof(MESSAGE);
//
////	output_debug("instance_slave_wait_socket(): db memcpy(%x, %x, %d)",
///local_inst.message->data_buffer, local_inst.buffer + offset,
///local_inst.prop_size);
//	memcpy(local_inst.message->data_buffer, local_inst.buffer + offset,
//local_inst.prop_size); 	status = 1; 	output_debug("instance_slave_wait_socket():
//exiting"); 	return status;
//}
...
//{
//	int status = 0;
//	output_verbose("instance_slave_wait(): slave %d entering wait state with
//t2=%lli (%x)", slave_id, local_inst.cache->ts,
//((char*)&local_inst.cache->ts-(char*)local_inst.cache)); 	if(local_inst.cnxtype
//== CI_MMAP){ #ifdef _WIN32 		status = instance_slave_wait_mmap(); #else
...
//) 	output_verbose("instance_slave_done_mmap(): copying %d bytes from %x to %x",
//local_inst.cachesize, local_inst.cache, local_inst.filemap);
...
//sizeof(MESSAGE) + strlen(MSG_DATA); 		local_inst.buffer = (char
//*)malloc(local_inst.buffer_size); 		if(0 == local_inst.buffer){
//			output_fatal("instance_slave_done_socket(): malloc
//failure"); 			return -1;
//		}
//	}
//	offset = strlen(MSG_DATA);
...
//	offset += sizeof(MESSAGE);
//	memcpy(local_inst.buffer + offset, local_inst.message->data_buffer,
//local_inst.prop_size); 	offset += local_inst.prop_size;
//
//	// feed through socket
//	rv = send(local_inst.sockfd, local_inst.buffer, (int)offset, 0);
//	printcontent(local_inst.buffer, rv);
////	output_debug("instance_slave_done_socket(): %d = send(%d, %x, %d, 0)",
///rv, local_inst.sockfd, local_inst.buffer, offset);
//	if(rv < 0){
//		output_error("instance_slave_done_socket(): error when sending
//data"); 		closesocket(local_inst.sockfd);
//		// TODO this should be a valid exitcode (see exec.c XC_*)
//		return -1;
//	} else if(0 == rv){
//		output_error("instance_slave_done_socket(): socket was closed
//before data sent"); 		closesocket(local_inst.sockfd);
//		// TODO this should be a valid exitcode (see exec.c XC_*)
//		return -1;
//	}
...
//	// @todo cnx exchange
//	/* signal the master */
//	output_verbose("instance_slave_done(): slave %d signaling master",
//slave_id); 	switch(local_inst.cnxtype){ 		case CI_MMAP: 			rv =
//instance_slave_done_mmap(); 			break; 		case CI_SHMEM: 			break; 		case CI_SOCKET: 			rv =
//instance_slave_done_socket(); 			break; 		default:
//			output_error("instance_slave_done(): unrecognized
...
//	linkage *lnk;
//	STATUS rv = SUCCESS;
//	output_verbose("instance_slaveproc(): slave %d controller startup in
//progress", slave_id);
//
...
//		output_verbose("instance_slaveproc(): slave %d controller
//waiting", slave_id); 		if ( 0 == instance_slave_wait() )
//		{
//			/* stop the main loop and exit the slave controller */
//			output_error("instance_slaveproc(): slave %d controller
//wait failure, thread stopping", slave_id); 			exec_setexitcode(XC_PRCERR);
//			pthread_cond_broadcast(&mls_inst_signal);
//			break;
//		}
//
//		output_debug("instance_slaveproc(): slave %d controller reading
//links", slave_id); 		for ( lnk=local_inst.write ; lnk!=nullptr ; lnk=lnk->next
//){ 			if(FAILED == linkage_master_to_slave(0, lnk)){
//				output_warning("instance_slaveproc():
//linkage_master_to_slave failed"); 				rv = FAILED; 				break;
//			}
//		}
//
...
//		// note, if TS_NEVER, we want the slave's exec loop to end
//normally
//		//output_debug("slave %d controller resuming exec with %lli",
//slave_id, local_inst.cache->ts); 		std::shared_ptr<sync_data> sync_data_nullptr
//= nullptr; 		output_debug("slave %d controller resuming exec with %lli",
//local_inst.cache->id, local_inst.cache->ts); 		output_debug("slave %d controller
//setting step_to %lli to cache->ts %lli", local_inst.cache->id,
//exec_sync_get(sync_data_nullptr), local_inst.cache->ts);
//
//		//exec_sync_merge(sync_data_nullptr, reinterpret_cast<struct
//sync_data *>(&local_inst.cache)); 		std::shared_ptr<sync_data> temp_from =
//std::make_shared<sync_data>(reinterpret_cast<sync_data&>(local_inst.cache));
//
...
//		output_verbose("slave %d controller waiting for main to
//complete", slave_id);
//
//		pthread_mutex_lock(&mls_inst_lock);
//		pthread_cond_wait(&mls_inst_signal, &mls_inst_lock);
...
//				output_warning("linkage_slave_to_master
//failed"); 				rv = FAILED; 				break;
//			}
//		}
////		output_debug("continuing");
//
//		/* copy the next time stamp */
//		/* how about we copy the time we want to step to and see what
//the master says, instead? -MH */
//
//		std::shared_ptr<sync_data> d =
//std::make_shared<sync_data>(reinterpret_cast<sync_data&>(local_inst.cache));
//
//		exec_sync_reset(d);
...
//	output_verbose("slave %" FMT_INT64 " completion state reached",
//local_inst.cacheid); 	pthread_exit(nullptr); 	return nullptr;
//}
...
//	{
//		output_error("unable to open cache '%s' for slave (error code
//%d)", cacheName, GetLastError()); 		return FAILED;
//	}
//	else
//	{
//		output_debug("cache '%s' opened for slave", cacheName);
//	}
//	local_inst.filemap = static_cast<char *>(MapViewOfFile(local_inst.hMap,
//FILE_MAP_ALL_ACCESS, 0, 0, (DWORD)0)); // not sure how big it is, so grab it
//all 	output_debug("MapViewOfFile returned"); 	output_debug("resulting handle is
//%x", local_inst.filemap); 	rv = (int64)local_inst.filemap;
//
//	output_debug("copying %d bytes from map %x to local message", sz,
//local_inst.filemap); 	memcpy(&tmsg, local_inst.filemap, sz);
//
//	output_debug("data copied");
//	output_debug("namesz %d, datesz %d", tmsg.name_size, tmsg.data_size);
//	output_debug("TMSG: usize %d, asize %d, id %x, nsz %d, psz %d",
//tmsg.usize, tmsg.asize, tmsg.id, tmsg.name_size, tmsg.data_size);
//	if(tmsg.name_size < 0){
//		return FAILED;
//	}
//	if(tmsg.data_size < 0){
...
//	{
//		output_error("unable to open event signal '%s' for slave %d",
//eventName, slave_id); 		return static_cast<STATUS>(0);
//	}
//	else
//	{
//		output_debug("opened event signal '%s' for slave %d", eventName,
//slave_id);
//	}
//
...
//	{
//		output_error("unable to open event signal '%s' for slave %d",
//eventName, slave_id); 		return FAILED;
//	}
//	else
//	{
//		output_debug("opened event signal '%s' for slave %d ",
//eventName, slave_id);
//	}
//	return SUCCESS;
// #else
...
//	{
//		output_error("instance_cnx_socket(): socket library
//initialization failed: %s",strerror(GetLastError())); 		return FAILED;
//	}
// #endif
//
//	// connect to master
...
//		output_fatal("instance_slave_init_socket(): unable to open
//socket"); 		return FAILED;
//	}
//	memset(&connaddr, 0, sizeof(connaddr));
//	connaddr.sin_addr.s_addr = inet_addr(global_master);
//	if(INADDR_NONE == connaddr.sin_addr.s_addr){
//		output_fatal("instance_slave_init_socket(): unrecognized
//inet_addr \'%s\'", global_master); 		return FAILED;
//	}
//	connaddr.sin_port = htons((unsigned short)global_master_port);
//	connaddr.sin_family = AF_INET;
//	rv = connect(local_inst.sockfd, (struct sockaddr *)&connaddr,
//sizeof(connaddr)); 	if(0 != rv){ 		output_fatal("instance_slave_init_socket():
//could not connect to %s:%d", global_master, global_master_port); 		#ifdef _WIN32
...
//	if(0 == rv){
//		output_fatal("instance_slave_init_socket(): socket closed before
//slave handshake sent"); 		return FAILED; 	} else if(0 > rv){
//		output_fatal("instance_slave_init_socket(): error sending slave
...
//	if(rv == 0){
//		output_fatal("instance_slave_init_socket(): socket closed before
//slave handshake response recv'd"); 		return FAILED; 	} else if (0 > rv){
//		output_fatal("instance_slave_init_socket(): error receiving
//slave handshake response"); 		return FAILED;
//	}
//	if(0 == memcmp(cmd, HS_FAIL, strlen(HS_FAIL))){
//		output_fatal("instance_slave_init_socket(): master reports bad
//ID/handshake"); 		return FAILED;
//	}
//	if(0 != memcmp(cmd, HS_RSP, strlen(HS_RSP))){
//		output_fatal("instance_slave_init_socket(): unrecognized master
//handshake reply response"); 		return FAILED;
//	}
//
...
//	if(rv == 0){
//		output_fatal("instance_slave_init_socket(): socket closed before
//instance data recv'd"); 		return FAILED; 	} else if (0 > rv){
//		output_fatal("instance_slave_init_socket(): error receiving
//instance data"); 		return FAILED;
//	}
//	if(0 != memcmp(cmd, MSG_INST, strlen(MSG_INST))){
//		;
//	}
//	memcpy(&pickle, cmd+strlen(MSG_INST), sizeof(pickle));
//	output_debug("pickle: %" FMT_INT64 "d %d %d %d %d %" FMT_INT64 "d",
//pickle.cacheid, pickle.cachesize, pickle.name_size, pickle.prop_size,
//pickle.id, pickle.ts); 	if(local_inst.cacheid != pickle.cacheid){ 		; // error
//	}
...
//
//	// FIXME: the next two lines compile, but the types are disparate enough
//that the reinterpret cast could be a problem going forward.
//	std::shared_ptr<sync_data> d =
//std::make_shared<sync_data>(reinterpret_cast<sync_data&>(local_inst.cache));
//
...
//	}
//	messagewrapper_init(&(local_inst.message), local_inst.cache);
//	output_debug("mw: m %x, n %x (%d), d %x (%d)", local_inst.message->msg,
//local_inst.message->name_buffer, *(local_inst.message->name_size),
//local_inst.message->data_buffer, *(local_inst.message->data_size));
...
//	if(0 == rv){
//		output_fatal("instance_slave_init_socket(): socket closed before
//instance OK sent"); 		return FAILED; 	} else if(0 > rv){
//		output_fatal("instance_slave_init_socket(): error sending
//instance OK"); 		return FAILED;
//	}
//
...
//	if(rv == 0){
//		output_fatal("instance_slave_init_socket(): socket closed before
//linkage names recv'd"); 		return FAILED; 	} else if (0 > rv){
//		output_fatal("instance_slave_init_socket(): error receiving
//linkage names "); 		return FAILED;
//	}
//	if(0 != memcmp(cmd, MSG_LINK, strlen(MSG_LINK))){
//		output_fatal("instance_slave_init_socket(): master did not send
//MSG_LINK message when expected"); 		return FAILED;
//	}
//	memcpy(local_inst.message->name_buffer, cmd+strlen(MSG_LINK),
//local_inst.name_size);
//
//
//	// receipt linkage data
//	strcpy(cmd, MSG_OK);
...
//	if(0 == rv){
//		output_fatal("instance_slave_init_socket(): socket closed before
//link name OK sent"); 		return FAILED; 	} else if(0 > rv){
//		output_fatal("instance_slave_init_socket(): error sending link
//name OK"); 		return FAILED;
//	}
//
...
//	if(rv == 0){
//		output_fatal("instance_slave_init_socket(): socket closed before
//start message recv'd"); 		return FAILED; 	} else if (0 > rv){
//		output_fatal("instance_slave_init_socket(): error receiving
//start message"); 		return FAILED;
//	}
//	if(0 != memcmp(cmd, MSG_START, (int)strlen(MSG_START))){
//		output_fatal("instance_slave_init_socket(): master did not send
//MSG_START message when expected"); 		output_debug(cmd); 		return FAILED; 	} else {
//		output_debug("instance_slave_init_socket(): recv start
//message");
//	}
//
//	// send initial link data
//
//	/* NOTE: may be feeding stdio down the socket to the master after this
//point */
//
//	// fdopen master socket
///*	local_inst.stream = fdopen(local_inst.sockfd, "w+");
//	if(0 == local_inst.stream){
//		output_fatal("instance_slave_init_socket(): unable to fdopen()
//the socket"); 		return FAILED;
//	}
...
//	if(rv != 0){
//		output_error("error with pthread_mutex_init() in
//instance_slave_init_pthreads()"); 		return FAILED;
//	}
//	rv = pthread_cond_init(&mls_inst_signal,nullptr);
//	if(rv != 0){
//		output_error("error with pthread_cond_init() in
//instance_slave_init_pthreads()"); 		return FAILED;
//	}
//	output_verbose("opened slave end of master-slave comm channel for slave
//%d", slave_id);
//
////	exec_mls_create(); // need to do this before we start the thread
//
//	/* start the slave controller */
//	// !!! &local_inst.pid causes a warning, pthread_t * -> us*__w64, but
//pt_t is a struct with [void * + uint], bad recast 	if (
//pthread_create(&(local_inst.threadid), nullptr, instance_slaveproc, nullptr) )
//	{
//		output_error("unable to start slave controller for slave %d",
//slave_id); 		return FAILED;
//	}
//	else
//	{
//		output_verbose("started slave controller for slave %d",
//slave_id);
//	}
//	return SUCCESS;
//}
...
//	int rv = 0;
////	STATUS stat;
//	size_t cacheSize = sizeof(MESSAGE)+MSGALLOCSZ; /* @todo size instance
//cache dynamically from linkage list */
//
...
//	// this is where we open a connection, snag the first header, and
//initialize everything from there 	switch(global_multirun_connection){ 		case
//MRC_MEM: #ifdef _WIN32 			local_inst.cnxtype = CI_MMAP; #else 			local_inst.cnxtype
//= CI_SHMEM; #endif 			rv = instance_slave_init_mem(); 			break; 		case MRC_SOCKET:
//			local_inst.cnxtype = CI_SOCKET;
//			rv = instance_slave_init_socket();
//			break;
//		case MRC_NONE:
//			rv = FAILED;
//			output_error("Not running in slave mode, or
//multirun_connection not set (internal error)"); 			break; 		default: 			rv = FAILED;
//			output_error("Unrecognized multirun_connection type
...
//	}
//
//	output_debug("li: %llx %d %d %d %d", local_inst.cacheid,
//local_inst.cachesize, local_inst.name_size, local_inst.prop_size,
//local_inst.id);
////	output_debug("slave %" FMT_INT64 " entering init_pthreads()",
///local_inst.cacheid);
//	rv = instance_slave_init_pthreads(); // starts slaveproc() thread
////	output_debug("slave %" FMT_INT64 " exited init_pthreads()",
///local_inst.cacheid); /	output_debug("li: %" FMT_INT64 " %d %d %d %d",
///local_inst.cacheid, local_inst.cachesize, local_inst.name_size,
///local_inst.prop_size, local_inst.id);
//
//	std::shared_ptr<sync_data> sync_data_nullptr = nullptr;
//
//	global_clock = exec_sync_get(sync_data_nullptr); // copy time signal to
//gc, legit since it's from msg 	output_debug("inst_slave_init(): gc = %lli",
//global_clock);
//
