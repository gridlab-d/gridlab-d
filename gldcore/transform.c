...
TIMESTAMP transform_syncall(TIMESTAMP t1, TRANSFORMSOURCE source)
{
	...
	{	
		if ( xform->source_type == XS_UNKNOWN )
 			output_warning("transform_syncall(...): transform to property '%s' of object '%s' has an unknown source type, it will always be run",
				xform->target_prop->name, xform->target_obj->name?xform->target_obj->name:"(unnamed)");
