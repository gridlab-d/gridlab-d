...
extern "C" int job(int argc, char *argv[]) {
  ...
  if (count == 0) {
    output_warning("no models found to process job in workdir '%s'",
                   global_workdir);
    exit(XC_RUNERR);
  }
  ...
  output_debug("starting job with cmdargs '%s' using %d threads", job_cmdargs,
  n_procs); for ( i=0 ; i<fmin(count,n_procs) ; i++ )
