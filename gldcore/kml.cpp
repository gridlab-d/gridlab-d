  ...
  for (oclass = class_get_first_class(); oclass != nullptr;
    ...
    for (obj = object_get_first(); obj != nullptr; obj = obj->next) {
      ...
      /* first instance of this class needs folder */
        ...
        kml_write("    <description>Module %s",
                  oclass->module ? oclass->module->name : "(runtime)");
        if (oclass->module != nullptr)
          kml_write(" (V%d.%02d)", oclass->module->major,
                    oclass->module->minor);
...
int kml_dump(char *filename) {
  ...
  fp = fopen(fname, "w");
  if (fp == nullptr)
    throw_exception("kml_dump(char *filename='%s'): %s", filename,
                    strerror(errno));
  /* TROUBLESHOOT
          The system was unable to output the KML data to the specified file.
          Follow the recommended solution based on the error message provided
     and try again.
   */
