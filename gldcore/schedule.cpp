							   ...
							   double *scale, double *bias)
{
	...
	*bias = 0.0;
	...
	if (strchr(expression, '*') == nullptr && strchr(expression, '+') == nullptr &&
		strchr(expression, '-') == nullptr && strchr(expression, '/') == nullptr)
	{
...
#endif
			if (sch->value != value)
			{ // This will not update sch->since to the starttime if value == 0 at the starttime.
				sch->since = t;
			}
			if (sch->since == TS_ZERO)
			{ // This is the value of sch->since at starttime so we should update it to starttime.
				sch->since = t;
			}
			sch->value = value;
			sch->duration = schedule_duration(sch, index) / 60.0;
...
void schedule_syncproc(SCHEDULESYNCDATA *data)
{
	...
	while (data->ok) {
		...
		while (!schedule_ready)
			start_sch.wait(start_sch_mutex);
		...
		{
			TIMESTAMP t3 = schedule_sync(sch, next_t1_sch);
			if (t3 < t2)
				t2 = t3;
		}
...
TIMESTAMP schedule_syncall(TIMESTAMP t1) /**< the time to which the schedule is synchronized */
{
	...
	static std::vector<SCHEDULESYNCDATA> thread_sch;
	...
	if (n_threads_sch == 0) {
		...
		if (n_threads_sch > 1)
		{
			...
			thread_sch.resize(n_threads_sch);
			...
			for (n = 0; n < n_threads_sch; n++)
			{
				...
				thread_sch[n].n = n;
				thread_sch[n].worker = std::thread(schedule_syncproc, &(thread_sch[n]));
			}
		}
	}
	...
	else
	{
		...
		if (setSCH) {
			for (unsigned int n = 0; n < n_threads_sch; n++) {
				thread_sch[n].worker.detach();  // Let thread run independently
			}
			setSCH = false;
		}
	}
