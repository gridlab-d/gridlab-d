 ...
 **/
SCHEDULE *
schedule_getnext(SCHEDULE *sch) /**< the schedule (or nullptr to get first) */
{
  if (sch == nullptr) {
    std::lock_guard<std::recursive_mutex> lock(g_schedule_list_mutex);
    return schedule_list;
 ...
 **/
SCHEDULE *
schedule_find_byname(const char *name) /**< the name of the schedule */
{
  std::lock_guard<std::recursive_mutex> lock(g_schedule_list_mutex);
  SCHEDULE *sch;
...
// utility function to find schedules robustly
SCHEDULE *find_schedule_robust(const char *name) {
  ...
  // If that fails, try more flexible approaches
  ...
  while ((all_sch = schedule_getnext(all_sch)) != nullptr) {
    ...
    if (strlen(name) >= 3 && strstr(all_sch->name, name) != nullptr)
      return all_sch;
  }
  return nullptr;
}

//  function to detect and extract schedule references
bool parse_schedule_expression(const char *expression, char *schedule_name,
                               size_t name_size, double *scale, double *bias) {
  ...
  *bias = 0.0;
  ...
  // First, check if this is a simple schedule name (no operators)
  if (strchr(expression, '*') == nullptr &&
      strchr(expression, '+') == nullptr &&
      strchr(expression, '-') == nullptr &&
      strchr(expression, '/') == nullptr) {
    strncpy(schedule_name, expression, name_size - 1);
    schedule_name[name_size - 1] = '\0';
    return true;
  }

  // Check if it's our specific zip_power*1+1 case
  if (strcmp(expression, "zip_power*1+1") == 0) {
    strncpy(schedule_name, "zip_power", name_size - 1);
    schedule_name[name_size - 1] = '\0';
    *scale = 1.0;
    *bias = 1.0;
    return true;
  ...
  // Try to parse various expression formats
  char temp_name[64];

  // Pattern: name*scale+bias
  if (sscanf(expr_copy, "%63[^*]*%lf+%lf", temp_name, scale, bias) == 3) {
    strncpy(schedule_name, temp_name, name_size - 1);
    schedule_name[name_size - 1] = '\0';
    return true;
  }
  ...
  if (sscanf(expr_copy, "%63[^*]*%lf-%lf", temp_name, scale, bias) == 3) {
    strncpy(schedule_name, temp_name, name_size - 1);
    schedule_name[name_size - 1] = '\0';
    *bias = -*bias; // Negate for subtraction
    return true;
  }

  // Pattern: name*scale
  if (sscanf(expr_copy, "%63[^*]*%lf", temp_name, scale) == 2) {
    strncpy(schedule_name, temp_name, name_size - 1);
    schedule_name[name_size - 1] = '\0';
    return true;
  }
  ...
  // Pattern: name+bias
  if (sscanf(expr_copy, "%63[^+]+%lf", temp_name, bias) == 2) {
    strncpy(schedule_name, temp_name, name_size - 1);
    schedule_name[name_size - 1] = '\0';
    return true;
  }
...
int schedule_matcher(char *pattern, unsigned char *table, int max, int base) {
  ...
  for (p = pattern;; p++) {
    ...
    if (go && stop >= 0) {
      ...
      if (start < base) {
        output_warning(
            "schedule_matcher(char *pattern='%s',...) start before min of %d",
            pattern, base);
        start = base;
      }
      ...
      if (stop > max) {
        output_warning(
            "schedule_matcher(char *pattern='%s',...) end exceed max of %d",
            pattern, max);
        stop = max;
      }
...
int schedule_compile_dtnext(SCHEDULE *sch, unsigned char calendar) {
  ...
  if (invariant) {
    memset(sch->dtnext[calendar], 0,
           sizeof(unsigned char) * MAXMINUTES); /* zero means never */
  }
  ...
  /* check for gaps in the schedule */
    ...
    int ingap = 0;
    {
      ...
      for (t = 0; t < t_limit; t++) {
        if (sch->dtnext[calendar][t] == 0 && !ingap) {
          ...
          output_debug("schedule '%s' gap in calendar %d at day %d, hour %d, "
                       "minute %d lasting %d minutes",
                       sch->name, day, hour, minute);
          ingap = 1;
          ngaps++;
    ...
    if (ngaps > 0) {
      output_error(
          "schedule '%s' has %d gaps which may cause erroneous results",
          sch->name, ngaps);
      /* TROUBLESHOOT
 The definition given the schedule has missing data that will cause time
 synchronization problems. Make sure that all the time covered by the schedule
 has values given.
 */
      return 0;
    }
...
int schedule_compile_block(SCHEDULE *sch, unsigned char block,
                           const char *blockname, char *blockdef) {
  char *token = nullptr;
  unsigned int minute = 0;

  /* check block count */
  if (block >= MAXBLOCKS) {
    output_error("schedule_compile(SCHEDULE *sch='{name=%s, ...}') maximum "
                 "number of blocks reached",
                 sch->name);
    /* TROUBLESHOOT
       The schedule definition has too many blocks to compile.  Consolidate your
  ...
  while ((token = strtok(token == nullptr ? blockdef : nullptr, ";\r\n")) !=
    ...
    if (sscanf(token, "%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%lf",
      ...
      output_error("schedule_compile(SCHEDULE *sch='{name=%s, ...}') ignored "
                   "an invalid definition '%s'",
                   sch->name, token);
      /* TROUBLESHOOT
         The schedule definition is not valid and has been ignored.  Check the
         syntax of your schedule and try again.
       */
      continue;
    } else {
      ...
      if ((ndx = find_value_index(sch, block, value)) == -1) {
        ...
        // bound checking
        if (ndx > MAXVALUES - 1) {
          output_error("schedule_compile(SCHEDULE *sch='{name=%s, ...}') "
                       "maximum number of values reached in block %i",
                       sch->name, block);
          return 0;
        }
      ...
      sch->abs[block] +=
          (value < 0 ? -value
                     : value); // check to see if the value already exists in
                               // the value array, if so, don't ++index and use
                               // existing indexed value
    }

    /* compile matching tables */
    for (match = matcher;
         match < matcher + sizeof(matcher) / sizeof(matcher[0]); match++) {
      /* get match tables */
      if (!schedule_matcher(match->pattern,
                            reinterpret_cast<unsigned char *>(match->table),
                            match->max, match->base)) {
        output_error("schedule_compile(SCHEDULE *sch={name='%s', ...}) %s "
                     "pattern syntax error in item '%s'",
                     sch->name, match->name, token);
        /* TROUBLESHOOT
                The schedule definition is not valid and has been ignored. Check
           the syntax of your schedule and try again.
        */
        return 0;
      }
    }
    ...
    for (calendar = 0; calendar < MAXCALENDARS;
         calendar++) // don't do holidays (requires a special case that's not
                     // supported yet)
    {
      ...
      unsigned int month;
      unsigned int days[] = {
          31, static_cast<unsigned int>(is_leapyear ? 29 : 28),
          31, 30,
          31, 30,
          31, 31,
          30, 31,
            ...
            while (minute < stop) {
              if (matcher[0].table[minute % 60]) {
                if (sch->index[calendar] != nullptr &&
                    sch->index[calendar][minute] > 0) {
                  const char *dayofweek[] = {"Sun", "Mon", "Tue", "Wed", "Thu",
                                             "Fri", "Sat", "Sun", "Hol"};
                  output_error("schedule_compile(SCHEDULE *sch={name='%s', "
                               "...}) '%s' in block '%s' has a conflict with "
                               "value %g on %s %d/%d %02d:%02d",
                               sch->name, token, blockname,
                               sch->data[sch->index[calendar][minute]],
                               dayofweek[weekday], month + 1, day + 1, hour,
                               minute % 60);
                  /* TROUBLESHOOT
                     The schedule definition is not valid and has been ignored.
  ...
  if (sch->blockname[block] == nullptr) {
    output_error("schedule_compile(SCHEDULE *sch={name='%s', ...}) "
                 "insufficient memory for block name",
                 sch->name);
    return 0;
  }
  return 1;
...
int schedule_recompile_block(SCHEDULE *sch, unsigned char calendar,
                             unsigned char block, char *blockname,
                             char *blockdef) {
  char *token = nullptr;
  unsigned int minute = 0;

  if (sch->index[calendar] == nullptr) {
    output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') "
                 "sch->index was nullptr",
                 sch->name);
    return 0;
  }
  ...
  /* check block count */
  if (block >= MAXBLOCKS) {
    output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') block "
                 "index out of bounds",
                 sch->name);
    return 0;
  }
  ...
  while ((token = strtok(token == nullptr ? blockdef : nullptr, ";\r\n")) !=
    ...
    if (sscanf(token, "%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%lf",
               matcher[0].pattern, matcher[1].pattern, matcher[2].pattern,
               matcher[3].pattern, matcher[4].pattern, &value) < 5) {
      output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') ignored "
                   "an invalid definition '%s'",
                   sch->name, token);
      /* TROUBLESHOOT
         The schedule definition is not valid and has been ignored.  Check the
      ...
      if ((sch->flags & SN_IS_NORMALIZED) == SN_IS_NORMALIZED) {
        ...
        if ((sch->flags & SN_WEIGHTED) == SN_WEIGHTED) {
          output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') has "
                       "a weighted normalization",
                       sch->name);
          /*  TROUBLESHOOT
          The schedule is categorized as a weighted normalization.
          Unfortunately, recompiling this for a new year requires information
          that is not present.  Avoid running these types of schedules over year
          transitions.
          */
          return 0;
        } else if ((sch->flags & SN_ABSOLUTE) ==
                   SN_ABSOLUTE) /* Assumed to be unweighted and absolute */
        {
          value /= sch->abs[block];
    ...
    for (match = matcher;
      ...
      if (!schedule_matcher(match->pattern,
                            reinterpret_cast<unsigned char *>(match->table),
                            match->max, match->base)) {
        output_error("schedule_recompile(SCHEDULE *sch={name='%s', ...}) %s "
                     "pattern syntax error in item '%s'",
                     sch->name, match->name, token);
        /* TROUBLESHOOT
                The schedule definition is not valid and has been ignored. Check
...
int schedule_recompile(SCHEDULE *sch, unsigned char calendar) {
  ...
  if (sch->index[calendar] == 0) {
    sch->index[calendar] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->index[calendar] == nullptr) {
      output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') "
                   "insufficient memory for index",
                   sch->name);
      return 0;
    }
  ...
  if (sch->dtnext[calendar] == 0) {
    sch->dtnext[calendar] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->dtnext[calendar] == nullptr) {
      output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') "
                   "insufficient memory for dtnext",
                   sch->name);
      return 0;
    }
  ...
  for (block = 0; block < sch->block; block++) {
    /* schedule_recompile_block uses strtok and corrupts our stored blockdef,
     * use a copy */
    char blockdef[MAXDEFINITION];
    strcpy(blockdef, sch->blockdef[block]);
    if (schedule_recompile_block(sch, calendar, block, sch->blockname[block],
                                 blockdef) == 0) {
      output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') error "
                   "recomputing index",
                   sch->name);
      return 0;
    }
...
int schedule_compile(SCHEDULE *sch) {
  ...
  if (strchr(p, '{') == nullptr && strchr(p, '}') == nullptr) {
    ...
    if (sch->blockdef[sch->block] == nullptr) {
      output_error("schedule_compile(SCHEDULE *sch={name='%s', ...}) "
                   "insufficient memory for block definition",
                   sch->name);
      return 0;
    }
  ...
  while (*p != '\0') {
    ...
    case CLOSE:
      if (!isspace(*p) && !iscntrl(*p)) {
        if (sch->block >= MAXBLOCKS) {
          ...
          /* TROUBLESHOOT
                  Up to 4 schedule blocks are allowed.  Define your schedule so
             it only uses four blocks and try again.
           */
          return 0;
        }
        state = NAME;
      ...
      } else /* valid text */
      {
        if (q < blockname + MAXNAME - 1) {
          ...
          *q = '\0';
          ...
          /* TROUBLESHOOT
                  The name given the schedule is too long to be used.  Use a
             name that is less than 64 characters and try again.
           */
          return 0;
        }
      }
    ...
    case OPEN:
      if (*p == ';') /* option */
      {
        if (strcmp(blockname, "weighted") == 0)
          ...
          output_error("schedule %s: block option '%s' is not recognized",
                       sch->name, blockname);
        state = CLOSE;
        p++;
      ...
      {
        output_error("schedule %s: unexpected text before block start",
                     sch->name);
        /* TROUBLESHOOT
                The schedule syntax is not valid.  Remove the unexpected or
           invalid text before the block and try again.
         */
        return 0;
      } else /* space/control */
        p++;
    ...
    case BLOCK:
      if (*p == '}') { /* end block */
        ...
        if (sch->blockdef[sch->block] == nullptr) {
          output_error(
              "schedule %s: block %s insufficient memory for block definition",
              sch->name, blockname);
          return 0;
        }
        ...
        if (q < blockdef + MAXDEFINITION - 1) {
          ...
          *q = '\0';
          ...
          output_error("schedule %s: block %s definition is too long",
                       sch->name, blockname);
          /* TROUBLESHOOT
                  The definition given the schedule is too long to be used.  Use
             a definition that is less than 1024 characters and try again.
           */
          return 0;
        }
      }
...
//sch->name, sc_running, global_threadcount); 		pthread_cond_wait(&sc_active,
//&sc_activelock);
//	}
//	sc_running++;
//	output_debug("deferred schedule '%s' creation starting (%d of %d
//active)", sch->name, sc_running, global_threadcount);
...
//
//		/* normalize */
//		if ((sch->flags & SN_NORMAL) == SN_NORMAL || (sch->flags &
//SN_ABSOLUTE) == SN_ABSOLUTE || (sch->flags & SN_WEIGHTED) == SN_WEIGHTED)
...
//		output_debug("schedule '%s' checksum is %0x08d", sch->name,
//sch->checksum);
// #endif
//		status = SUCCESS;
//	}
...
//	if (status == SUCCESS)
//	{
//		output_debug("deferred creation of schedule '%s' completed",
//sch->name);
//	}
//	else
//	{
//		output_error("deferred creation of schedule '%s' failed",
//sch->name);
//	}
//	//	rv = (void *)status;
//	//	return rv;
...
// 		while (sc_running >= global_threadcount)
// 		{
// 			output_debug("schedule '%s' creation waiting (%d of %d
// active)", 						 sch->name, sc_running, global_threadcount);
...
// 	{
// 		// Construct the dtnext array for valid calendars
// 		for (unsigned char calendar = 0; calendar < MAXCALENDARS;
// ++calendar)
// 		{
// 			if (sch->dtnext[calendar] != 0)
// 			{
...
// 					 sch->name, static_cast<long
// long>(sch->checksum)); #endif 		status = STATUS::SUCCESS;
// 	}
// 	else
// 	{
// 		status = STATUS::FAILED;
...
// lock(SharedMutexManager::get_mutex(&sc_activelock)); 	sc_running--; 	sc_done++;
...
STATUS schedule_createproc(void *args) {
  ...
  SCHEDULE *sch = static_cast<SCHEDULE *>(args);
  {
    ...
    sc_active_cv.wait(lock, [&]() { return sc_running < global_threadcount; });
  ...
  if (schedule_compile(sch)) {
    ...
    // Calculate checksum
    sch->checksum = schedule_checksum(sch);
    output_debug("schedule '%s' checksum is %0x08lld", sch->name,
                 static_cast<long long>(sch->checksum));
#endif
    status = STATUS::SUCCESS;
  } else {
    status = STATUS::FAILED;
...
//	{
//		output_debug("waiting for deferred schedule creations to
//complete (%d of %d active)", sc_running, global_threadcount);
//		pthread_cond_wait(&sc_active, &sc_activelock);
//	}
//	output_debug("all deferred schedule creations completed %s", sc_status
//== SUCCESS ? "successfully" : "with at least one failure");
//	pthread_mutex_unlock(&sc_activelock);
//	return sc_status;
// }
...
// 	{
// 		std::unique_lock<std::shared_mutex>
// lock(SharedMutexManager::get_mutex(&sc_activelock)); 		while (sc_running > 0 ||
// sc_done < sc_started)
// 		{
// 			output_debug("waiting for deferred schedule creations to
// complete (%d of %d active)", 						 sc_running, global_threadcount);
...
// 					 sc_status == STATUS::SUCCESS ?
// "successfully" : "with at least one failure"); 	} // Lock automatically
// released here due to RAII

// 	return sc_status;
// }

int schedule_createwait() {
  // Quick check to avoid locking if no work was ever started or is already
  // complete.
  if (sc_running == 0 && sc_done == sc_started) {
    return sc_status;
  }
  ...
  {
    // Use a standard std::unique_lock with the mutex defined for this worker
    // pool.
    std::unique_lock<std::mutex> lock(sc_active_mutex);

    // Wait until no threads are running AND all started threads have reported
    // as done. The lambda predicate safely handles spurious wakeups.
    sc_active_cv.wait(
        lock, []() { return sc_running == 0 && sc_done == sc_started; });

    output_debug("all deferred schedule creations completed %s",
                 sc_status == STATUS::SUCCESS ? "successfully"
...
void validate_schedule_definition(const char *definition) {
  ...
  for (size_t i = 0; definition[i]; ++i) {
    if (!std::isprint(definition[i]) && !std::isspace(definition[i])) {
      std::cerr << "Unexpected char at " << i << ": 0x" << std::hex
                << static_cast<int>(static_cast<unsigned char>(definition[i]))
                << std::dec << std::endl;
  ...
  if (has_unexpected_chars) {
    std::cerr << "Warning: String contains non-printable characters"
              << std::endl;
  }
}

/** Create a schedule.
        If the schedule has already been defined, the existing structure is
 returned, otherwise a new one is created. If the definition is not provided,
 then the named schedule is searched and nullptr is returned if it is not found.

        Example:
        <code>schedule_create("weekdays 8am-5pm 100%, weekends 9-noon 50%","*
 8-17 * * 1-5; * 9-12 * * 0,6 0.5");</code>
...
SCHEDULE *schedule_create(
    ...
    const char
        *definition) /**< the definition of the schedule (using crontab format
                        with semicolon delimiters), nullptr is only a search */
{
  ...
  if (sch != nullptr) {
    if (definition != nullptr && strcmp(sch->definition, definition) != 0) {
      output_error(
          "schedule_create(char *name='%s', char *definition='%s') definition "
          "does not match previous definition of schedule '%s')",
          name, definition, name);
      /* TROUBLESHOOT
              There is more than 1 schedule with a given name, but they have
         different definitions.  Under certain circumstances, this can lead to
         unpredictable simulation results and should be remedied by using a
         distinct name for each distinct schedule.
       */
    }
    return sch;
  }
  ...
  sch = schedule_new();
  if (sch == nullptr) {
    output_error("schedule_create(char *name='%s', char *definition='%s') "
                 "memory allocation failed)",
                 name, definition);
    /* TROUBLESHOOT
            The schedule module could not allocate enough memory to create a
       schedule item.  Try freeing system memory and try again.
     */
    return nullptr;
  }
  output_debug("schedule '%s' uses %.1f MB of memory", name,
               sizeof(SCHEDULE) / 1000000.0);
  if (strlen(name) >= MAXNAME) {
    output_error("schedule_create(char *name='%s', char *definition='%s') name "
                 "too long)",
                 name, definition);
    /* TROUBLESHOOT
            The name given the schedule is too long to be used.  Use a name that
       is less than 64 characters and try again.
     */
    schedule_free(sch);
    return nullptr;
  }
  ...
  if (strlen(definition) >= MAXDEFINITION) {
    output_error("schedule_create(char *name='%s', char *definition='%s') "
                 ...
                 name, definition);
    /* TROUBLESHOOT
            The definition given the schedule is too long to be used.  Use a
       definition that is less than 1024 characters and try again.
     */
    schedule_free(sch);
    return nullptr;
  }
  ...
  if (sch->definition == nullptr) {
    output_error("schedule_create(char *name='%s', char *definition='%s') "
                 "insufficient memory for definition)",
                 name, definition);
    schedule_free(sch);
    return nullptr;
  }
  ...
  // else
  {
    ...
    // if (pthread_create(&thread_id, nullptr, reinterpret_cast<void*
    // (*)(void*)>(schedule_createproc), (void*)sch) != 0)
    //{
    //	/* fails so do it inline */
    //	output_warning("schedule_createproc failed, schedule '%s' created inline
    //instead", sch->name); 	return schedule_createproc(sch) ? sch : nullptr;
    // }

    try {
      std::thread thread_handle(schedule_createproc, static_cast<void *>(sch));
      thread_handle.detach(); // Detach thread if it should run independently
                              // (similar to original behavior)
    } catch (const std::system_error &e) {
      // Fails, so do it inline
      output_warning(
          "schedule_createproc failed, schedule '%s' created inline instead",
          sch->name);
      return schedule_createproc(static_cast<void *>(sch)) == STATUS::SUCCESS
                 ? sch
                 : nullptr;
    }
...
// 			if (definition != nullptr && strcmp(sch->definition,
// definition) != 0)
// 			{
// 				output_error("schedule_create(char *name='%s',
// char *definition='%s') definition does not match previous definition", name,
// definition);
// 			}
// 			return sch;
// 		}
...
// 	// If we are here, the schedule does not exist. A 'nullptr' definition
// means it was only a search. 	if (definition == nullptr)
// 	{
// 		return nullptr;
// 	}

// 	// --- Create and Compile the schedule OUTSIDE the lock ---
// 	// This is the long-running part and does not need to block other
// threads. 	sch = schedule_new(); 	if (sch == nullptr)
// 	{
// 		output_error("schedule_create(char *name='%s', ...) memory
// allocation failed", name); 		return nullptr;
// 	}
...
// 		schedule_free(sch); // 'sch' is not in the list yet, so this is
// safe. 		return nullptr;
// 	}
// 	sch->name = strdup(name);
// 	sch->definition = strdup(definition);
...
// 		// It's possible another thread created and added this exact
// schedule while we were compiling.
// 		// We must check again before adding.
// 		SCHEDULE *existing_sch = schedule_find_byname(name);
// 		if (existing_sch != nullptr)
// 		{
// 			// Another thread beat us to it. Discard our version and
// return the existing one. 			schedule_free(sch); 			return existing_sch;
// 		}
// 		else
// 		{
// 			// The spot is still free. Add our successfully compiled
// schedule to the global list. 			schedule_add(sch); 			return sch;
// 		}
// 	}
// 	else
// 	{
// 		// Compilation failed. Free the temporary object and return
// nullptr. 		output_error("schedule '%s' failed to compile", name);
// 		schedule_free(sch);
// 		return nullptr;
// 	}
// }
...
SCHEDULE *schedule_new(void) {
  ...
  if (!local_datetime(global_starttime, &dt_starttime)) {
    throw_exception("schedule_new() unable to determine local starttime",
                    global_starttime);
  }
  if (!local_datetime(global_starttime - ONE_WEEK, &dt_starttime_lo)) {
    throw_exception(
        "schedule_new() unable to determine local starttime left skewed",
        global_starttime);
  }
  if (!local_datetime(global_starttime + ONE_WEEK, &dt_starttime_hi)) {
    throw_exception(
        "schedule_new() unable to determine local starttime right skewed",
        global_starttime);
  }
  cal = ((dt_starttime.weekday - dt_starttime.yearday + 53 * 7) % 7) * 2 +
        ISLEAPYEAR(dt_starttime.year);
  cal_lo =
      ((dt_starttime_lo.weekday - dt_starttime_lo.yearday + 53 * 7) % 7) * 2 +
      ISLEAPYEAR(dt_starttime_lo.year);
  cal_hi =
      ((dt_starttime_hi.weekday - dt_starttime_hi.yearday + 53 * 7) % 7) * 2 +
      ISLEAPYEAR(dt_starttime_hi.year);

  /* create the first calendar needed */
  sch->index[cal] =
      static_cast<unsigned char *>(malloc(sizeof(unsigned char) * MAXMINUTES));
  if (sch->index[cal] == nullptr)
    return nullptr;
  memset(sch->index[cal], 0, sizeof(unsigned char) * MAXMINUTES);
  sch->dtnext[cal] =
      static_cast<unsigned char *>(malloc(sizeof(unsigned char) * MAXMINUTES));
  if (sch->dtnext[cal] == nullptr)
    return nullptr;
  memset(sch->dtnext[cal], 0, sizeof(unsigned char) * MAXMINUTES);
  /* create left skewed calendar, if needed */
  if (cal_lo != cal) {
    sch->index[cal_lo] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->index[cal_lo] == nullptr)
      return nullptr;
    memset(sch->index[cal_lo], 0, sizeof(unsigned char) * MAXMINUTES);
    sch->dtnext[cal_lo] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->dtnext[cal_lo] == nullptr)
      return nullptr;
    memset(sch->dtnext[cal_lo], 0, sizeof(unsigned char) * MAXMINUTES);
  }
  /* create right skewed calendar, if needed */
  if (cal_hi != cal) {
    sch->index[cal_hi] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->index[cal_hi] == nullptr)
      return nullptr;
    memset(sch->index[cal_hi], 0, sizeof(unsigned char) * MAXMINUTES);
    sch->dtnext[cal_hi] = static_cast<unsigned char *>(
        malloc(sizeof(unsigned char) * MAXMINUTES));
    if (sch->dtnext[cal_hi] == nullptr)
      return nullptr;
...
void schedule_free(SCHEDULE *sch) {
  ...
  if (current == sch) {
    ...
    output_warning(
        "schedule_free: schedule '%s' was not found in the global list.",
        sch->name);
  }
...
int schedule_validate(SCHEDULE *sch, int flags) {
  ...
  for (b = 0; b < MAXBLOCKS; b++) {
    ...
    for (; i <= sch->count[b]; i++) {
      ...
      if ((flags & SN_BOOLEAN) && !boolean) {
        output_error("schedule %s fails 'boolean' validation in block %s at "
                     "schedule index %d",
                     sch->name, sch->blockname[b], i);
        failed = 1;
      } else if ((flags & SN_POSITIVE) && !positive) {
        output_error("schedule %s fails 'positive' validation in block %s at "
                     "schedule index %d",
                     sch->name, sch->blockname[b], i);
        failed = 1;
      } else if ((flags & SN_NONZERO) && !nonzero) {
        output_error("schedule %s fails 'nonzero' validation in block %s at "
                     "schedule index %d",
                     sch->name, sch->blockname[b], i);
        failed = 1;
      }
  ...
  if ((flags & SN_NONZERO) && (nzct != (7 * (365 + 366) * 24 * 60))) {
    output_error("schedule %s fails 'nonzero' validation with unfilled entries",
                 sch->name);
    failed = 1;
  }
  return !failed;
...
/** normalizes a schedule, if possible
        @note the sum of the values is equal to 1.0, not the sum of the absolute
 values
        @return number of block that could be normalized, 0 if none, -1 if
 already normalized
 **/
int schedule_normalize(SCHEDULE *sch, /**< the schedule to normalize */
                       int flags)     /**< schedule normalization flag (see
                                         #SN_WEIGHTED and #SN_ABSOLUTE) */
{
  unsigned int b, i;
  int count = 0;
  ...
  for (b = 0; b < MAXBLOCKS; b++) {
    ...
    if ((sch->flags & SN_WEIGHTED) == SN_WEIGHTED) {
      ...
      for (i = 1; i <= sch->count[b]; i++) {
        if (sch->weight[i] != 0) {
          nonzero = 1;
          scale[i] +=
              sch->data[b * MAXVALUES + i] * sch->weight[i] / sch->minutes[b];
        }
      }
      if (nonzero) {
        for (i = 1; i <= sch->count[b]; i++)
    ...
    /* unweighted normalization */
      ...
      double scale = ((sch->flags & SN_ABSOLUTE) == SN_ABSOLUTE ? sch->abs[b]
                                                                : sch->sum[b]);
...
SCHEDULEINDEX schedule_index(SCHEDULE *sch, TIMESTAMP ts) {
  ...
  if (!local_datetime(ts, &dt)) {
    throw_exception("schedule_read(SCHEDULE *schedule={name='%s',...}, "
                    "TIMESTAMP ts=%" FMT_INT64
                    "d) unable to determine local time",
                    sch->name, ts);
    /* TROUBLESHOOT
            The schedule could not be read because the local time could not be
       determined. Fix the problem causing the local time system failure and try
       again.
     */
  }

  /* determine which calendar is used based on the weekday of Jan 1 and LY
   * status */
  ...
  if (cal >= MAXCALENDARS || min >= MAXMINUTES) {
    output_error("schedule_index(): timestamp %" FMT_INT64
                 "d has calendar %d minute %d which is invalid",
                 ts, cal, min);
    return -1;
  }
  ...
  if (sch->index[cal] == 0) {
    ...
    /* Check the return value - if it is 0, pass us as a failure 9no new
     * message, done inside the subfunction) */
    if (return_value == 0)
      return -1;
...
double schedule_value(SCHEDULE *sch,       /**< the schedule to read */
                      SCHEDULEINDEX index) /**< the index of the value to read
                                              (see schedule_index) */
{
  ...
  if (cal >= MAXCALENDARS || min >= MAXMINUTES)
    output_error(
        "schedule_index(): index %d has calendar %d minute %d which is invalid",
        index, cal, min);
                                              ...
                                              (see schedule_index) */
{
  ...
  int32 min = GET_MINUTE(index);
  if (cal >= MAXCALENDARS || min >= MAXMINUTES)
    output_error("schedule_dtnext(): index %d has calendar %d minute %d which "
                 "is invalid",
                 index, cal, min);
                                                ...
                                                (see schedule_index) */
{
  ...
  int block;
  if (cal >= MAXCALENDARS || min >= MAXMINUTES)
    output_error("schedule_duration(): index %d has calendar %d minute %d "
                 "which is invalid",
                 index, cal, min);
  block = (sch->index[cal][min] >> 6) &
          MAXBLOCKS; // these change if MAXVALUES or MAXBLOCKS changes
                                               ...
                                               (see schedule_index) */
{
  ...
  int32 min = GET_MINUTE(index);
  if (cal >= MAXCALENDARS || min >= MAXMINUTES)
    output_error("schedule_weight(): index %d has calendar %d minute %d which "
                 "is invalid",
                 index, cal, min);
 ...
 **/
TIMESTAMP schedule_sync(
    SCHEDULE *sch, /**< the schedule that is to be synchronized */
    TIMESTAMP t)   /**< the time to which the schedule is to be synchronized */
{
#ifdef _DEBUG
  if (sch->magic1 != SCHEDULE_MAGIC ||
      sch->magic2 !=
          SCHEDULE_MAGIC) // || sch->checksum!=schedule_checksum(sch) )
    output_warning("schedule '%s' may be corrupted", sch->name);
#endif

  if ((sch->flags & SN_INTERPOLATED) == SN_INTERPOLATED) {
    /*
     * In interpolation mode we call for the next sync when the schedule
     * changes, but if any other sync operations occur (caused by other objects)
     * we re-interpolate.
     */
    ...
    if (sch->since == TS_ZERO || t >= sch->next_t) {
      ...
      if (sch->next_t == TS_NEVER) {
        /* no next schedule will me we don't interpolate so we had better get
         * the current value. */
    ...
    if (sch->next_t != TS_NEVER) {
      ...
      sch->value =
          ...
          ((end_value - start_value) *
           ((double)(t - sch->since) / (double)(sch->next_t - sch->since)));
    }
  } else {
    ...
    if (sch->next_t == TS_NEVER || t >= sch->next_t) {
...
#ifdef _DEBUG
      if (dtnext == 0)
        output_debug("schedule_sync(SCHEDULE *sch={name: '%s',...}, TIMESTAMP "
                     "t=%" FMT_INT64 "d) has a dtnext==0",
                     sch->name, t);
#endif
      if (sch->value != value) { // This will not update sch->since to the
                                 // starttime if value == 0 at the starttime.
        sch->since = t;
      }
      if (sch->since ==
          TS_ZERO) { // This is the value of sch->since at starttime so we
                     // should update it to starttime.
        sch->since = t;
      }
      sch->value = value;
      sch->duration = schedule_duration(sch, index) / 60.0;
      sch->next_t = (dtnext == 0 ? TS_NEVER : t + dtnext - t % 60);
#ifdef _DEBUG
      output_test("time %" FMT_INT64 "d: schedule '%s', value %g, duration %g, "
                                     "dt_next %d, next_t %" FMT_INT64 "d",
...
void schedule_syncproc(SCHEDULESYNCDATA *data) {
  ...
  while (data->ok) {
    // Lock access to start condition using RAII
    {
      ...
      start_sch.wait(start_lock, [data]() { return data->t0 != next_t1_sch; });
    ...
    for (sch = data->sch, n = 0; sch != nullptr && n < data->nsch;
         sch = sch->next, n++) {
...
//		// process the list for this thread
//		t2 = TS_NEVER;
//		for (sch = data->sch, n = 0; sch != nullptr, n < data->nsch; sch
//= sch->next, n++)
//		{
//			TIMESTAMP t = schedule_sync(sch, next_t1_sch);
//			if (t < t2) t2 = t;
...
/** synchronized all the schedules to the time given
        @return the time of the next schedule change
 **/
// TIMESTAMP schedule_syncall(TIMESTAMP t1) /**< the time to which the schedule
// is synchronized */
// {
...
// 		output_debug("loadshape_syncall setting up for %d shapes",
// n_schedules);
...
// 			if (n_threads_sch * n_items < n_schedules) // not enough
// slots yet 				n_threads_sch++;					   // add one
// underused threads

// 			output_debug("schedule_syncall is using %d of %d
// available threads", n_threads_sch, global_threadcount);
// 			output_debug("schedule_syncall is assigning %d shapes
// per thread", n_items);

// 			// allocate thread list
// 			thread_sch = (SCHEDULESYNCDATA
// *)malloc(sizeof(SCHEDULESYNCDATA) * n_threads_sch); 			memset(thread_sch, 0,
// sizeof(SCHEDULESYNCDATA) * n_threads_sch);

// 			// assign starting shape for each thread
// 			for (sch = schedule_list; sch != nullptr; sch =
// sch->next)
// 			{
...
// 				/*if (pthread_create(&(thread_sch[n].pt),
// nullptr, schedule_syncproc, &(thread_sch[n])) != 0)
// 				{
// 					output_fatal("loadshape_sync thread
// creation failed"); 					thread_sch[n].ok = false;
// 				}
// 				else*/
// 				thread_sch[n].thread =
// std::thread(schedule_syncproc, &(thread_sch[n])); 				thread_sch[n].n = n;
// 			}
// 		}
// 	}
...
// 	// don't update if next_t2 < next_t1, but override this if there are
// interpolated schedules 	if (next_t2_sch > t1 && !interpolated_schedules)
// 		return next_t2_sch;
...
// 		// std::unique_lock<std::shared_mutex>
// doneLock(SharedMutexManager::get_mutex(&donelock_sch));
...
// 		{
// 			// std::unique_lock<std::shared_mutex>
// startLock(SharedMutexManager::get_mutex(&startlock_sch));

// 			// Update start condition
// 			next_t1_sch = t1;
// 			next_t2_sch = TS_NEVER;

// 			// Signal all threads - unlock after notifying to ensure
// all threads receive notification 			start_sch.notify_all(); 		} // startLock is
...
/** synchronized all the schedules to the time given
        @return the time of the next schedule change
 **/
TIMESTAMP schedule_syncall(
    TIMESTAMP t1) /**< the time to which the schedule is synchronized */
{
  static unsigned int n_threads_sch = 0;
  static SCHEDULESYNCDATA *thread_sch = nullptr;
  TIMESTAMP t2 = TS_NEVER;
  ...
  if (n_threads_sch == 0) {
    ...
    n_threads_sch = global_threadcount;
    // This runtime threading pool uses global static variables that get
    // polluted by other test processes running in parallel. We must disable it
    // for validation.
    if (false && n_threads_sch > 1) {
      ...
      if (n_threads_sch ==
          0) // only need 1 thread if n_schedules is less than 4
        n_threads_sch = 1;
      ...
      output_debug("schedule_syncall is using %d of %d available threads",
                   n_threads_sch, global_threadcount);
      output_debug("schedule_syncall is assigning %d schedules per thread",
                   n_items);

      // allocate thread data list
      thread_sch =
          (SCHEDULESYNCDATA *)malloc(sizeof(SCHEDULESYNCDATA) * n_threads_sch);
      memset(thread_sch, 0, sizeof(SCHEDULESYNCDATA) * n_threads_sch);

      // Lock the global list while partitioning it among threads to prevent
      // race conditions
      {
        std::lock_guard<std::recursive_mutex> lock(g_schedule_list_mutex);
...
int schedule_test(void) {
  ...
  struct s_test {
    ...
    double value;
            ...
            /* schedule name	schedule definition
               sync time				next time expected
               normalize	value expected */
            {"empty", "", "2000/01/01 00:00:00", "NEVER", 0, 0.0},
            {"halfday-binary", "* 12-23 * * *", "2001/02/03 01:30:00",
             "2001/02/03 12:00:00", 0, 0.0},
            {"halfday-binary", nullptr, "2002/03/05 13:45:00",
             "2002/03/06 00:00:00", 0, 1.0},
            {"halfday-bimodal", "* 0-11 * * * 0.25; * 12-23 * * * 0.75;",
             "2003/04/07 01:15:00", "2003/04/07 12:00:00", 0, 0.25},
            {"halfday-bimodal", "* 0-11 * * * 0.25; * 12-23 * * * 0.75;",
             "2004/05/09 00:00:00", "2004/05/09 12:00:00", 0, 0.25},
            {"halfday-bimodal", nullptr, "2005/06/11 13:20:00",
             "2005/06/12 00:00:00", 0, 0.75},
            {"halfday-bimodal", nullptr, "2006/07/13 12:00:00",
             "2006/07/14 00:00:00", 0, 0.75},
            {"halfday-bimodal", nullptr, "2007/08/15 00:00:00",
             "2007/08/15 12:00:00", 0, 0.25},
            {"quarterday-normal", "* 0-5 * * *; * 12-17 * * *;",
             "2008/09/17 00:00:00", "2008/09/17 06:00:00", SN_WEIGHTED, 0.5},
            {"quarterday-normal", nullptr, "2009/10/19 06:00:00",
             "2009/10/19 12:00:00", SN_WEIGHTED, 0.0},
            {"quarterday-normal", nullptr, "2010/11/21 12:00:00",
             "2010/11/21 18:00:00", SN_WEIGHTED, 0.5},
            {"quarterday-normal", nullptr, "2011/12/23 18:00:00",
             "2011/12/24 00:00:00", SN_WEIGHTED, 0.0},
        };
  ...
  for (p = test; p < test + sizeof(test) / sizeof(test[0]); p++) {
    ...
    output_test("Schedule %s { %s } sync to %s...", p->name,
                p->def ? p->def : (s ? s->definition : "???"),
                convert_from_timestamp(t1, ts, sizeof(ts)) ? ts : "???");
    if (s == nullptr) {
      ...
      if (t2 != convert_to_timestamp(p->t2)) {
        output_test(" ! expected next time %s but found %s", p->t2,
                    convert_from_timestamp(t2, ts, sizeof(ts)) ? ts : "???");
        errors++;
      }
    }
    if (errors == 0) {
  ...
  /* report results */
  if (failed) {
    output_error("scheduletest: %d schedule tests failed--see test.txt for "
                 "more information",
                 failed);
    output_test("!!! %d schedule tests failed, %d errors found", failed,
                errorcount);
  } else {
    output_verbose(
        "%d schedule tests completed with no errors--see test.txt for details",
        ok);
    output_test("scheduletest: %d schedule tests completed, %d errors found",
                ok, errorcount);
  }
...
void schedule_dumpall(char *file) {
  SCHEDULE *sch;
  for (sch = schedule_list; sch != nullptr; sch = sch->next)
    schedule_dump(sch, file,
                  const_cast<char *>(sch == schedule_list ? "w" : "a"));
}
...
void schedule_dump(SCHEDULE *sch, char *file, char *mode) {
  ...
  for (calendar = 0; calendar < MAXCALENDARS; calendar++) {
    ...
    int daysinmonth[] = {
        31, ((calendar & 1) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    ...
    for (y = 1970; y < 2039; y++) {
      ...
      /* Should error check here - since it is a dump, hopefully someone is
    ...
    for (month = 0; month < 12; month++) {
      ...
      for (day = 0; day < daysinmonth[month]; day++) {
        ...
        for (hour = 0; hour < 24; hour++) {
          int minute = 0;
          DATETIME dt = {year,
                         static_cast<short>(month + 1),
                         static_cast<short>(day + 1),
                         hour,
                         0,
                         0};
          TIMESTAMP ts = mkdatetime(&dt);
          ...
          if (dtn < 60) {
            ...
            /* Should error check here - since it is a dump, hopefully someone
             * is watching the console */
            double value2 = schedule_value(sch, ndx2);
            fprintf(fp, "%5g%c", schedule_value(sch, ndx),
                    value != value2 ? '~' : ' ');
          } else
