        ...
        int ingap = 0;
        {
            ...
            for (t = 0; t < t_limit; t++) {
                if (sch->dtnext[calendar][t] == 0 && !ingap) {
                    ...
                    output_debug("schedule '%s' gap in calendar %d at day %d, hour %d, minute %d lasting %d minutes",
...
int schedule_compile_block(SCHEDULE *sch, unsigned char block, const char *blockname, char *blockdef) {
    ...
    while ((token = strtok(token == NULL ? blockdef : NULL, ";\r\n")) != NULL) {
        ...
        if (sscanf(token, "%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%lf",
            ...
            output_error("schedule_compile(SCHEDULE *sch='{name=%s, ...}') ignored an invalid definition '%s'",
                         sch->name, token);
            /* TROUBLESHOOT
            ...
            if ((ndx = find_value_index(sch, block, value)) == -1) {
                ...
                if (ndx > MAXVALUES - 1) {
                    output_error(
                            "schedule_compile(SCHEDULE *sch='{name=%s, ...}') maximum number of values reached in block %i",
                            sch->name, block);
                    return 0;
                }
            ...
            sch->sum[block] += value;
            sch->abs[block] += (value < 0 ? -value
                                          : value); // check to see if the value already exists in the value array, if so, don't ++index and use existing indexed value
        }

        /* compile matching tables */
        for (match = matcher; match < matcher + sizeof(matcher) / sizeof(matcher[0]); match++) {
            /* get match tables */
            if (!schedule_matcher(match->pattern, reinterpret_cast<unsigned char *>(match->table), match->max,
                                  match->base)) {
                output_error("schedule_compile(SCHEDULE *sch={name='%s', ...}) %s pattern syntax error in item '%s'",
        ...
        for (calendar = 0; calendar <
                           MAXCALENDARS; calendar++) // don't do holidays (requires a special case that's not supported yet)
        {
            unsigned int weekday = calendar % 7;
            unsigned int is_leapyear = (calendar >= 7 ? 1 : 0);
            unsigned int calendar = weekday * 2 + is_leapyear;
            unsigned int month;
            unsigned int days[] = {31, static_cast<unsigned int>(is_leapyear ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31,
                                   30, 31};
                        ...
                        while (minute < stop) {
                            if (matcher[0].table[minute % 60]) {
                                if (sch->index[calendar] != NULL && sch->index[calendar][minute] > 0) {
                                    const char *dayofweek[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun",
                                                               "Hol"};
                                    output_error(
                                            "schedule_compile(SCHEDULE *sch={name='%s', ...}) '%s' in block '%s' has a conflict with value %g on %s %d/%d %02d:%02d",
                                            sch->name, token, blockname, sch->data[sch->index[calendar][minute]],
                                            dayofweek[weekday], month + 1, day + 1, hour, minute % 60);
                                    /* TROUBLESHOOT
                                       The schedule definition is not valid and has been ignored.  Check the syntax of your schedule and try again.
                                     */
                                    return 0;
...
int
schedule_recompile_block(SCHEDULE *sch, unsigned char calendar, unsigned char block, char *blockname, char *blockdef) {
    char *token = NULL;
    unsigned int minute = 0;
    ...
    while ((token = strtok(token == NULL ? blockdef : NULL, ";\r\n")) != NULL) {
        ...
        if (sscanf(token, "%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%s%*[ \t]%lf",
                   matcher[0].pattern, matcher[1].pattern, matcher[2].pattern,
                   matcher[3].pattern, matcher[4].pattern, &value) < 5) {
            output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') ignored an invalid definition '%s'",
                         sch->name, token);
            /* TROUBLESHOOT
               The schedule definition is not valid and has been ignored.  Check the syntax of your schedule and try again.
             */
            ...
            if ((sch->flags & SN_IS_NORMALIZED) == SN_IS_NORMALIZED) {
                ...
                if ((sch->flags & SN_WEIGHTED) == SN_WEIGHTED) {
                    output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') has a weighted normalization",
                                 sch->name);
                    /*  TROUBLESHOOT
        ...
        for (match = matcher; match < matcher + sizeof(matcher) / sizeof(matcher[0]); match++) {
            ...
            if (!schedule_matcher(match->pattern, reinterpret_cast<unsigned char *>(match->table), match->max,
                                  match->base)) {
                output_error("schedule_recompile(SCHEDULE *sch={name='%s', ...}) %s pattern syntax error in item '%s'",
                             sch->name, match->name, token);
                /* TROUBLESHOOT
                    The schedule definition is not valid and has been ignored.  Check the syntax of your schedule and try again.
                */
                            ...
                            if (matcher[0].table[minute % 60]) {
                                if (sch->index[calendar][minute] > 0) {
                                    const char *dayofweek[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun",
                                                               "Hol"};
                                    output_error(
                                            "schedule_recompile(SCHEDULE *sch={name='%s', ...}) '%s' in block '%s' has a conflict with value %g on %s %d/%d %02d:%02d",
                                            sch->name, token, blockname, sch->data[sch->index[calendar][minute]],
                                            dayofweek[weekday], month + 1, day + 1, hour, minute % 60);
                                    /* TROUBLESHOOT
                                       The schedule definition is not valid and has been ignored.  Check the syntax of your schedule and try again.
...
int schedule_recompile(SCHEDULE *sch, unsigned char calendar) {
    ...
    if (sch->dtnext[calendar] == 0) {
        ...
        if (sch->dtnext[calendar] == NULL) {
            output_error("schedule_recompile(SCHEDULE *sch='{name=%s, ...}') insufficient memory for dtnext",
                         sch->name);
            return 0;
        }
...
int schedule_compile(SCHEDULE *sch) {
    ...
    if (strchr(p, '{') == NULL && strchr(p, '}') == NULL) {
        ...
        if (sch->blockdef[sch->block] == NULL) {
            output_error("schedule_compile(SCHEDULE *sch={name='%s', ...}) insufficient memory for block definition",
                         sch->name);
            return 0;
        }
		...
		case BLOCK:
			...
			{	/* end block */
				...
				sch->blockdef[sch->block] = strdup(blockdef);
                    if (sch->blockdef[sch->block] == NULL) {
                        output_error("schedule %s: block %s insufficient memory for block definition", sch->name,
                                     blockname);
                        return 0;
                    }
...
static std::condition_variable sc_active;
static std::mutex sc_activelock;
...
STATUS schedule_createproc(void *args) {
    ...
    std::unique_lock active_lock{sc_activelock};
    while (sc_running >= global_threadcount) {
        ...
        sc_active.wait(active_lock);
    }
    sc_running++;
    output_debug("deferred schedule '%s' creation starting (%d of %d active)", sch->name, sc_running,
                 global_threadcount);
    sc_active.notify_all();
    active_lock.unlock();
    ...
    if (schedule_compile(sch)) {
        ...
        /* normalize */
        if ((sch->flags & SN_NORMAL) == SN_NORMAL || (sch->flags & SN_ABSOLUTE) == SN_ABSOLUTE ||
        ...
        status = FAILED;
    Done:
    std::unique_lock done_lock{sc_activelock};
    sc_running--;
    sc_done++;
...
int schedule_createwait(void) {
    ...
    std::unique_lock schedule_lock{sc_activelock};
    while (sc_running > 0 || sc_done < sc_started) {
        output_debug("waiting for deferred schedule creations to complete (%d of %d active)", sc_running,
                     global_threadcount);
        sc_active.wait(schedule_lock);
    }
    output_debug("all deferred schedule creations completed %s",
                 sc_status == SUCCESS ? "successfully" : "with at least one failure");
    return sc_status;
}
                          ...
                          const char *definition)    /**< the definition of the schedule (using crontab format with semicolon delimiters), NULL is only a search */
{
    ...
    if (sch != NULL) {
        if (definition != NULL && strcmp(sch->definition, definition) != 0) {
            output_error(
                    "schedule_create(char *name='%s', char *definition='%s') definition does not match previous definition of schedule '%s')",
    ...
    sch = schedule_new();
    if (sch == NULL) {
        output_error("schedule_create(char *name='%s', char *definition='%s') memory allocation failed)", name,
                     definition);
        /* TROUBLESHOOT
    ...
    if (sch->name == NULL) {
        output_error("schedule_create(char *name='%s', char *definition='%s') insufficient memory for name)", name,
                     definition);
        schedule_free(sch);
        return NULL;
    }
    ...
    if (sch->definition == NULL) {
        output_error("schedule_create(char *name='%s', char *definition='%s') insufficient memory for definition)",
                     name, definition);
        schedule_free(sch);
        return NULL;
    }
    ...
    if (global_threadcount <= 1) {
        ...
        std::thread t1(schedule_createproc, sch);
        thread_id = t1.get_id();
        sc_started++;
        return sch;
    }
    ...
    for (b = 0; b < MAXBLOCKS; b++) {
        ...
        for (; i <= sch->count[b]; i++) {
            ...
            if ((flags & SN_BOOLEAN) && !boolean) {
                output_error("schedule %s fails 'boolean' validation in block %s at schedule index %d", sch->name,
                             sch->blockname[b], i);
                failed = 1;
            } else if ((flags & SN_POSITIVE) && !positive) {
                output_error("schedule %s fails 'positive' validation in block %s at schedule index %d", sch->name,
                             sch->blockname[b], i);
                failed = 1;
            } else if ((flags & SN_NONZERO) && !nonzero) {
                output_error("schedule %s fails 'nonzero' validation in block %s at schedule index %d", sch->name,
                             sch->blockname[b], i);
                failed = 1;
            }
        }
...
SCHEDULEINDEX schedule_index(SCHEDULE *sch, TIMESTAMP ts) {
    ...
    if (!local_datetime(ts, &dt)) {
        throw_exception(
                "schedule_read(SCHEDULE *schedule={name='%s',...}, TIMESTAMP ts=%" FMT_INT64 "d) unable to determine local time",
                sch->name, ts);
        /* TROUBLESHOOT
    ...
    if (cal >= MAXCALENDARS || min >= MAXMINUTES) {
        output_error("schedule_index(): timestamp %" FMT_INT64 "d has calendar %d minute %d which is invalid", ts, cal,
                     min);
        return -1;
    }
                        ...
                        TIMESTAMP t)    /**< the time to which the schedule is to be synchronized */
{
    ...
    if ((sch->flags & SN_INTERPOLATED) == SN_INTERPOLATED) {
        ...
        if (sch->next_t != TS_NEVER) {
            ...
            sch->value = start_value + ((end_value - start_value) *
        ...
        if (sch->next_t == TS_NEVER || t >= sch->next_t) {
            ...
            if (dtnext == 0)
                output_debug(
                        "schedule_sync(SCHEDULE *sch={name: '%s',...}, TIMESTAMP t=%" FMT_INT64 "d) has a dtnext==0",
                        sch->name, t);
#endif
            ...
            if (sch->since ==
                TS_ZERO) {//This is the value of sch->since at starttime so we should update it to starttime.
                sch->since = t;
            }
            sch->value = value;
            sch->duration = schedule_duration(sch, index) / 60.0;
...
#ifdef _DEBUG
            output_test(
                    "time %" FMT_INT64 "d: schedule '%s', value %g, duration %g, dt_next %d, next_t %" FMT_INT64 "d",
...
static std::condition_variable start_sch;
static std::mutex startlock_sch;
static std::condition_variable done_sch;
static std::mutex donelock_sch;
...
void *schedule_syncproc(void *ptr) {
    ...
    while (data->ok) {
        ...
        std::unique_lock startlock{startlock_sch};

        // wait for thread start condition
        while (data->t0 == next_t1_sch)
            start_sch.wait(startlock);
        ...
        std::unique_lock donelock{donelock_sch};
    ...
    if (n_threads_sch == 0) {
        ...
        if (n_threads_sch > 1) {
            ...
            for (n = 0; n < n_threads_sch; n++) {
                ...
                try {
                    thread_sch[n].pt = std::move(std::thread(schedule_syncproc, &(thread_sch[n])));
                    thread_sch[n].n = n;
                }
                catch (std::system_error &ex) {
                    output_fatal("loadshape_sync thread creation failed with error: %s", ex.what());
...
int schedule_test(void) {
    ...
    for (p = test; p < test + sizeof(test) / sizeof(test[0]); p++) {
        ...
        output_test("Schedule %s { %s } sync to %s...", p->name, p->def ? p->def : (s ? s->definition : "???"),
