...
static counters run_test(char *file, double *elapsed_time = nullptr) {
  ...
  *ext = '\0'; // remove extension from dir
...
#endif
  {
    output_error("run_test(char *file='%s'): unable to create test folder",
                 dir);
    result.inc_access(file);
    return result;
  } else {
    ...
    // std::cerr << "Thread " <<
    // std::hash<std::thread::id>{}(std::this_thread::get_id()) << "using
  ...
  std::string command_line =
      std::format("\"{}\" -W {} {} {}.glm",
                  executable_to_run_path
                      .string(), // Get string representation for formatting
                  dir, validate_child_cmdargs, name);
...
static size_t process_dir(const char *path, bool runglms = false) {
  ...
  while ((dp = readdir(dirp)) != nullptr) {
    ...
    else if (runglms == true && strstr(item, "/test_") != 0 &&
             strcmp(ext, ".glm") == 0) {
      pushdir(item);
      count++;
    }
