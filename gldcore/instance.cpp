...
unsigned int mls_inst_lock = 0;
std::condition_variable_any mls_inst_signal;
int inst_created = 0;

// used to balance control between instance_master_wait_socket and
// instance_runproc
...
static void message_init(
    MESSAGE *msg,    /**< pointer to the message cache */
...
STATUS messagewrapper_init(MESSAGEWRAPPER **msgwpr, MESSAGE *msg) {
  MESSAGEWRAPPER *wpr;
  size_t sz = sizeof(MESSAGE), sz2 = sizeof(MESSAGEWRAPPER);
  int64 a, b;
  b = (int64)msg;
  if (msgwpr == 0) {
...
void *instance_runproc_socket(void *ptr) {
  ...
  while (running) {
    ...
    //		output_debug("%d = recv(%d, %x, %d, 0)", rv, inst->sockfd,
    //inst->buffer, inst->buffer_size);
    if (0 == rv) {
      output_error(
          "instance_runproc_socket(): socket was closed before receiving data");
      running = 0;
    } else if (0 > rv) {
    ...
    if (0 == memcmp(inst->buffer, MSG_DATA, strlen(MSG_DATA))) {
      ...
      output_error("instance_runproc_socket(): slave indicated an error "
                   "occured"); // error occured
      running = 0;
    } else if (0 == memcmp(inst->buffer, MSG_DONE, strlen(MSG_DONE))) {
      output_verbose("instance_runproc_socket(): slave indicated run "
                     "completion"); // other side is done and is closing down
      running = 0;
    } else {
      // stdio message
      ;
    ...
    memcpy(inst->message->data_buffer,
           inst->buffer + strlen(MSG_DATA) + sizeof(MESSAGE), inst->prop_size);

    /* copy to inst->somewhere */
    // output_debug("i_rp_s(): waiting to send broadcast %d",
    // inst->sock_signal); pthread_mutex_lock(&inst->wait_lock);
...
/** instance_runproc
    This function is the slave instance creation and monitoring thread.
        If the instance host is the local machine, a new instance of gridlabd is
 started with the connection string as the first command argument. The global
 values of verbose and debug are consulted and the appropriate command line
 options are added if needed to ensure the slave has the same mode.
        @todo If the instance host is a remote machine, an RPC call is made to
 start the new instance of gridlabd on the remote machine with the connection
 string as the first command argument.
...
void *instance_runproc(void *ptr) {
  ...
  case CI_MMAP:
    ...
    break;
#else
    output_error("Memory Map (mmap) instance mode only supported under "
                 "Windows, please use Shared Memory (shmem) instead.");
    rc = -1;
    break;
#endif
  case CI_SHMEM:
  ...
  default:
    output_error(
        "unrecognized connection interface type in instance_runproc()");
    rc = -1;
  }

  /* instance exited */
  output_verbose("instance for model '%s' terminated with exit code %d (%s)",
                 inst->model, rc, rc < 0 ? strerror(errno) : "gridlabd error");
  instances_exited++;
...
instance *instance_create(char *host) {
  ...
  if (!inst) {
    output_error(
        "instance_create(): unable to allocate memory for instance on %s",
        host);
    return nullptr;
  }
...
int instance_master_wait_mmap(instance *inst) {
  ...
  case WAIT_TIMEOUT:
    ...
    break;
  case WAIT_FAILED:
    output_error("slave %d wait failed (error code %d)", inst->id,
                 GetLastError());
    break;
  default:
    output_error("slave %d wait return code not recognized (rc=%d)", inst->id,
                 rc);
    break;
  }
  ...
  return status;
#else
  output_error("instance_master_wait_mmap(): should not have been called "
               "outside Windows");
  return 0;
#endif
}
...
int instance_master_wait_socket(instance *inst) {
  ...
  if (sock_created) {
    // Wait for message using std::mutex and std::condition_variable
    std::unique_lock<std::shared_mutex> lock(
        SharedMutexManager::get_mutex(&inst->sock_lock));

    if (inst->has_data > 0) { // maybe 'while' this?
      output_debug("instance_master_wait_socket(): already has data for %d",
                   inst->id);
    } else {
      output_debug("instance_master_wait_socket(): inst %d waiting on %p",
                   inst->id, static_cast<void *>(&inst->sock_signal));

      // Wait for notification
      output_debug("inst %d waiting on signal %p", inst->id,
                   static_cast<void *>(&inst->sock_signal));

      inst->sock_signal.wait(lock);
      // Lock is automatically released while waiting
      // and reacquired when wait returns
    }
...
int instance_master_wait(void) {
  ...
  for (inst = instance_list; inst != nullptr; inst = inst->next) {
    ...
    if (status == 0)
      break;
    //		output_verbose("slave %d resumed with t2=%lli (%x)", inst->id,
    //inst->cache->ts, (&inst->cache->ts-inst->cache));
  }
...
void instance_master_done_mmap(instance *inst) {
  ...
  // output_verbose("master signaling slave %d with timestamp %lli", inst->id,
  // inst->cache->ts);
...
void instance_master_done_socket(instance *inst) {
  ...
  memcpy(inst->buffer + offset, inst->message->data_buffer,
         *(inst->message->data_size));
  ...
  // output_debug("%d = send(%d, %x, %d, 0)", rv, inst->sockfd, inst->buffer,
  // offset);
  if (0 == rv) {
    // socket closed
    output_error("instance_master_done_socket(): inst %d closed its socket",
                 inst->id);
    closesocket(inst->sockfd);
  } else if (0 > rv) {
    // socket error
    output_error("instance_master_done_socket(): error sending data to inst %d",
                 inst->id);
    closesocket(inst->sockfd);
  }
}
...
/** instance_master_done
    Signal all the slaves that the master is done processing the next sync
 state.
 **/
void instance_master_done(TIMESTAMP t1) {
  instance *inst;
  for (inst = instance_list; inst != nullptr; inst = inst->next) {
    // output_debug("master setting slave %d controller c->ts from %lli to t1
    // %lli", inst->cache->id, inst->cache->ts, t1);
...
/** instance_init
    Initialize an instance object.  This occurs on the master side for each
 slave instance. return 1 on failure, 0 on success.
 **/
...
//	{
//		output_error("instance_init(): slave instance model '%s' is the
//same as the master model '%'", inst->model,global_modelname); 		return FAILED;
//	}
//
//	// validate cnxtype
...
//			}
//		}
//		if(i == cnxtypecnt){ // exhausted the list without finding a
//match 			output_error("instance_init(): unrecognized connection type '%s' for
//instance '%s'", inst->cnxtypestr.get_string(), inst->model); 			return FAILED;
//		}
//	} else {
//		// default
// #ifdef _WIN32
...
//			output_error("instance_init(): linkage_init failed for
//inst '%s.%s' on link '%s.%s'", inst->hostname, inst->model, lnk->local.obj,
//lnk->local.prop); 			return FAILED; 		} else { 			inst->cachesize += lnk->size;
...
//	inst->buffer_size = inst->cachesize = sizeof(MESSAGE) +
//(size_t)(inst->name_size + inst->prop_size); 	inst->cache = (MESSAGE
//*)malloc(inst->cachesize); 	memset(inst->cache, 0, inst->cachesize);
//	message_init(inst->cache, inst->cachesize, (int16)inst->name_size,
//(int16)inst->prop_size); 	if(FAILED == messagewrapper_init(&(inst->message),
//inst->cache)){ 		return FAILED;
//	}
//
//	//	initialize cache
//	inst->cache->id = inst->id = instances_count;
//
//	//output_verbose("inst_init(): slave %d cache at %x, ts at %x",
//instances_count, inst->cache, &(inst->cache->ts));
//
//	// write property lists
//	// properties are written into the buffer as "obj1.prop1,obj2.prop2
...
//lnk->name_size; 		prop_offset += lnk->prop_size;
//	}
//	// note that for the first go-around, we cannot write the property
//values.
//	//	the system is still in init_all() and has not gotten to object
//	//	initialization, where defaults, calculated values, etc will be
//set.
//
...
//	if ( pthread_create(&(inst->threadid), nullptr, instance_runproc,
//(void*)inst) )
//	{
//		output_error("instance_init(): unable to starte instance slave
//%d", inst->id); 		return FAILED;
//	}
//
//	output_verbose("instance_init(): started instance for slave %d",
//inst->id); 	instances_count++; 	return SUCCESS;
// }

/** instance_initall
        Initialize all instance objects.
        @note This function waits for the slaves to signal their initialization
 has completed using instance_slave_done().
...
//lock(SharedMutexManager::get_mutex(&mls_inst_lock));
//		//pthread_cond_wait(&mls_inst_signal, &mls_inst_lock);
//		mls_inst_signal.wait(lock);
//
//
//	} else {
//		return SUCCESS;
...
//		if(FAILED == res){
//			output_error("instance_write_slave():
//linkage_master_to_slave failed"); 			return FAILED;
//		}
//	}
//	//output_verbose("copying %d bytes from %x to %x (%lli)",
//inst->cachesize, inst->cache, inst->buffer, inst->cache->ts);
...
/** instance_syncall
    Synchronize all slave instances
        @return the next time, TS_NEVER if slave is done, and TS_INVALID is sync
 failed.
...
//		{
//			output_debug("%d instance exit detected, stopping main
//loop", instances_exited);
//
