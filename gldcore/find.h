...
typedef struct s_findpgm {
  PGMCONSTFLAGS
      constflags; /* bits to indicate which criteria result in constant sets */
  COMPAREFUNC op;
...
char *find_file(const char *name, const char *path, int mode, char *buffer,
                int len);
