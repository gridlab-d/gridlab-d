...
static TIMESTAMP enduse_sync_impl(enduse *e, TIMESTAMP t1, PASSCONFIG pass) {
  ...
  if (pass == PC_PRETOPDOWN) // && t1>e->t_last)
  {
    ...
    if (e->shape &&
        e->shape->type != MT_UNKNOWN) // shape driven -> use fractions
    {
      // non-electric load
      ...
      // electric load
        ...
        double P =
            e->voltage_factor > 0
                ? e->shape->load * (e->power_fraction + e->current_fraction +
                                    e->impedance_fraction)
                : 0.0;
        e->total.Re() = P;
        if (fabs(e->power_factor) < 1)
          e->total.Im() = (e->power_factor < 0 ? -1 : 1) * P *
                          sqrt(1 / (e->power_factor * e->power_factor) - 1);
        else
          e->total.Im() = 0;

        // beware: these are misnomers (they are e->constant_power,
        // e->constant_current, ...)
    ...
    } else if (e->voltage_factor > 0 &&
               !(e->config & EUC_HEATLOAD)) // no shape electric - use ZIP
                                            // component directly
    {
      e->total.Re() = e->constant_power.Re() + e->constant_current.Re() +
                      e->constant_admittance.Re();
      e->total.Im() = e->constant_power.Im() + e->constant_current.Im() +
                      e->constant_admittance.Im();
    } else {
    ...
    // electric load
      ...
      if (e->heatgain_fraction > 0.0)
        e->heatgain =
            e->total.Re() * e->heatgain_fraction * 3412.1416 /* Btu/h/kW */;
    }
...
// #ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP enduse_sync(enduse *obj, TIMESTAMP t1,
                                            PASSCONFIG pass) {
...
void enduse_syncproc(ENDUSESYNCDATA *data) {
  ...
  while (data->ok) {
    // Lock access to start condition
    std::unique_lock<std::shared_mutex> startlock(
        SharedMutexManager::get_mutex(&startlock_ed));
    ...
    // Signal completion condition
    {
      std::unique_lock<std::shared_mutex> done_lock(
          SharedMutexManager::get_mutex(&donelock_ed));
      data->t0 = next_t1_ed;
      data->ran++;
      donecount_ed--;
      if (t2 < next_t2_ed)
  ...
  return; // Equivalent to pthread_exit (C++ exception-safe threads
...
TIMESTAMP enduse_syncall(TIMESTAMP t1) {
  ...
  if (n_threads_ed == 0) {
    ...
    if (n_threads_ed > 1) {
      ...
      output_debug("enduse_syncall is using %d of %d available threads",
                   n_threads_ed, global_threadcount);
      output_debug("enduse_syncall is assigning %d enduses per thread",
                   n_items);
  ...
  // Multi-threaded processing
    ...
    // Coordinate thread execution
    {
      ...
      // std::unique_lock<std::shared_mutex> done_wait(done_lock,
      // std::adopt_lock); done_ed.wait(done_wait, []() { return donecount_ed ==
      // 0; }); Wait for all threads to complete using the existing lock
...
//		int n_items, en = 0;
//
//		output_debug("enduse_syncall setting up for %d enduses",
//n_enduses);
//
...
//			n_items = n_enduses/n_threads_ed;
//			n_threads_ed = n_enduses/n_items;
//			if (n_threads_ed*n_items<n_enduses) // not enough slots
//yet 				n_threads_ed++; // add one underused thread
//
//			output_debug("enduse_syncall is using %d of %d available
//threads", n_threads_ed,global_threadcount); 			output_debug("enduse_syncall is
//assigning %d enduses per thread", n_items);
//
//			// allocate thread list
//			thread_ed =
//(ENDUSESYNCDATA*)malloc(sizeof(ENDUSESYNCDATA)*n_threads_ed);
...
//				thread_ed[n].ok = true;
//				if
//(pthread_create(&(thread_ed[n].pt),nullptr,enduse_syncproc,&(thread_ed[n]))!=0)
//				{
//					output_fatal("enduse_sync thread
//creation failed"); 					thread_ed[n].ok = false;
//				}
//				else
//					thread_ed[n].n = n;
//			}
...
int enduse_publish(CLASS *oclass, PROPERTYADDR struct_address, char *prefix) {
  enduse *self =
      nullptr; // temporary enduse structure used for mapping variables
  int result = 0;
  struct s_map_enduse {
    PROPERTYTYPE type;
    const char *name;
  ...
  PROPERTY *prop = property_malloc(
      PT_enduse, oclass,
      const_cast<char *>(strcmp(prefix, "") == 0 ? "load" : prefix),
      struct_address, nullptr);
  prop->description = "the enduse load description";
  prop->flags = 0;
  class_add_property(oclass, prop);

  for (p = prop_list; p < prop_list + sizeof(prop_list) / sizeof(prop_list[0]);
       p++) {
    ...
    if (p->type < _PT_LAST) {
      prop = property_malloc(p->type, oclass, name,
                             p->addr + (int64)struct_address, nullptr);
      prop->description = p->description;
      prop->flags = p->flags;
      class_add_property(oclass, prop);
      result++;
    } else if (last == nullptr) {
      output_error("PT_KEYWORD not allowed unless it follows another property "
                   "specification");
      /* TROUBLESHOOT
              The enduse_publish structure is not defined correctly.  This is an
         internal error and cannot be corrected by users.  Contact technical
         support and report this problem.
       */
      return -result;
    } else if (p->type == PT_KEYWORD) {
      switch (last->type) {
      case PT_enumeration:
        if (!class_define_enumeration_member(oclass, last->name, p->name,
                                             p->type)) {
          output_error(
              "unable to publish enumeration member '%s' of enduse '%s'",
              p->name, last->name);
          /* TROUBLESHOOT
          The enduse_publish structure is not defined correctly.  This is an
          internal error and cannot be corrected by users.  Contact technical
      ...
      case PT_set:
        if (!class_define_set_member(oclass, last->name, p->name, p->value)) {
          output_error("unable to publish set member '%s' of enduse '%s'",
                       p->name, last->name);
          /* TROUBLESHOOT
          The enduse_publish structure is not defined correctly.  This is an
          internal error and cannot be corrected by users.  Contact technical
      ...
      default:
        output_error(
            "PT_KEYWORD not supported after property '%s %s' in enduse_publish",
            class_get_property_typename(last->type), last->name);
        /* TROUBLESHOOT
        The enduse_publish structure is not defined correctly.  This is an
        internal error and cannot be corrected by users.  Contact technical
        support and report this problem.
      ...
      output_error("property type '%s' not recognized in enduse_publish",
                   class_get_property_typename(last->type));
      /* TROUBLESHOOT
              The enduse_publish structure is not defined correctly.  This is an
         internal error and cannot be corrected by users.  Contact technical
         support and report this problem.
...
int convert_to_enduse(char *string, void *data, PROPERTY *prop) {
  ...
  if (string[0] == '{') {
    ...
    PROPERTY eus[] = {
        {nullptr, "total", PT_complex, 0, 0, PA_PUBLIC, unit,
         (PROPERTYADDR)((char *)(&e->total) - (char *)e), nullptr, nullptr,
         nullptr, eus + 1},
        {nullptr, "energy", PT_complex, 0, 0, PA_PUBLIC, unit,
         (PROPERTYADDR)((char *)(&e->energy) - (char *)e), nullptr, nullptr,
         nullptr, eus + 2},
        {nullptr, "demand", PT_complex, 0, 0, PA_PUBLIC, unit,
         (PROPERTYADDR)((char *)(&e->demand) - (char *)e), nullptr, nullptr,
         nullptr, nullptr},
    };
  ...
  if (strlen(string) > sizeof(buffer) - 1) {
    output_error("convert_to_enduse(string='%-.64s...', ...) input string is "
                 "too long (max is 1023)",
                 string);
    return 0;
  }
  ...
  while ((token = strtok(token == nullptr ? buffer : nullptr, ";")) !=
    ...
    if (strcmp(param, "current_fraction") == 0)
      ...
      if (pref == nullptr) {
        output_warning("convert_to_enduse(string='%-.64s...', ...) loadshape "
                       "'%s' not found in class '%s'",
                       string, value, prop->oclass->name);
        return 0;
      }
      e->shape =
          (loadshape *)((char *)e - (int64)(prop->addr) + (int64)(pref->addr));
    } else {
      output_error("convert_to_enduse(string='%-.64s...', ...) parameter '%s' "
                   "is not valid",
                   string, param);
      return 0;
    }
...
int enduse_test(void) {
  ...
  if (failed) {
    output_error(
        "endusetest: %d enduse tests failed--see test.txt for more information",
        failed);
    output_test("!!! %d enduse tests failed, %d errors found", failed,
                errorcount);
  } else {
    output_verbose(
        "%d enduse tests completed with no errors--see test.txt for details",
        ok);
    output_test("endusetest: %d schedule tests completed, %d errors found", ok,
                errorcount);
  }
