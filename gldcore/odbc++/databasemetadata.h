...
#define SQL_NOT_DEFERRABLE 7
   ...
   *
   * @return <code>true</code> if the data source supports both data
   * manipulation (eg. <code>UPDATE</code>, <code>INSERT</code>) and data
   * definition (eg. <code>CREATE TABLE</code>) within a transaction.
   *
   *
   ...
   *
   * @return <code>true</code> if the data source only supports data
   * manipulation (eg. <code>UPDATE</code>, <code>INSERT</code>) within a
   * transaction.
   *
   ...
   *
   * @return <code>true</code> if (<code>"UPDATE ... WHERE CURRENT OF
   * ..."</code>) is supported
   */
   ...
   * <li><b>NULLABLE</b> - short - whether this type can contain
   * <code>NULL</code>s: <ul> <li><code>typeNoNulls</code> - no
   ...
   *     </ul>
   * <li><b>UNSIGNED_ATTRIBUTE</b> - bool - <code>true</code> if this type is
   * unsigned <li><b>FIXED_PREC_SCALE</b> - bool - whether this type has
   * predefined fixed precision and scale (eg is useful for money)
   ...
   * <li><b>IS_GRANTABLE</b> - string - <code>"YES"</code> if
   * <code>GRANTEE</code> can grant this privilege to other users.
   * <code>"NO"</code> if not. <code>NULL</code> if unknown.
   * </ol>
   */
   ...
   *     <ul>
   *     <li><code>tableIndexStatistic</code> - no real index - a statistic for
   * the table <li><code>tableIndexClustered</code> - this index is clustered
   *     <li><code>tableIndexHashed</code> - this index is hashed
   *     <li><code>tableIndexOther</code> - this is some other kind of index
   *     </ul>
   * <li><b>ORDINAL_POSITION</b> - short - column sequence number in index
   * (1-based). <code>NULL</code> if <code>TYPE</code> is
   * <code>tableIndexStatistic</code>. <li><b>COLUMN_NAME</b> - string - column
   * name. <code>NULL</code> if <code>TYPE</code> is
   * <code>tableIndexStatistic</code>. <li><b>ASC_OR_DESC</b> - string -
   * <code>"A"</code> for ascending, <code>"D"</code> for descending index.
  ...
  ResultSet *getIndexInfo(const ODBCXX_STRING &catalog,
                          const ODBCXX_STRING &schema,
                          const ODBCXX_STRING &table, bool unique,
                          bool approximate);
  ...
  ResultSet *getImportedKeys(const ODBCXX_STRING &catalog,
    ...
    return this->getCrossReference(
        ODBCXX_STRING_CONST(""), ODBCXX_STRING_CONST(""),
        ODBCXX_STRING_CONST(""), catalog, schema, table);
  }
  ...
  ResultSet *getExportedKeys(const ODBCXX_STRING &catalog,
                             ...
                             const ODBCXX_STRING &table) {
    return this->getCrossReference(
        catalog, schema, table, ODBCXX_STRING_CONST(""),
        ODBCXX_STRING_CONST(""), ODBCXX_STRING_CONST(""));
  }
   ...
   *     <ul>
   *     <li><code>bestRowUnknown</code> - it is unknown whether this is a
   * pseudo column <li><code>bestRowNotPseudo</code> - it is definitely not a
   * pseudo column <li><code>bestRowPseudo</code> - it is definitely a pseudo
   * column
   *     </ul>
   * </ol>
   * @param catalog the catalog name
  ...
  ResultSet *getBestRowIdentifier(const ODBCXX_STRING &catalog,
                                  const ODBCXX_STRING &schema,
                                  const ODBCXX_STRING &table, int scope,
                                  bool nullable);
   ...
   * <li><b>PSEUDO_COLUMN</b> - short - whether this is a pseudo column:
   *     <ul>
   *     <li><code>versionColumnUnknown</code> - it is unknown whether this is a
   * pseudo column <li><code>versionColumnNotPseudo</code> - it is definitely
   * not a pseudo column <li><code>versionColumnPseudo</code> - it is definitely
   * a pseudo column
   *     </ul>
   * </ol>
   */
