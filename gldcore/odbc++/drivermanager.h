...
public:
  ...
  const ODBCXX_STRING &getDescription() const { return description_; }

  /** Return a list of keys that can appear in a connect string using this
   * driver */
...
public:
  ...
  const ODBCXX_STRING &getName() const { return name_; }
...
public:
   ...
   * @param connectString Usually something like
   * <tt>"DSN=db;uid=user;pwd=password"</tt>
   */
