...
private:
  ...
  ResultSet *_getIndexInfo(const ODBCXX_STRING &catalog,
                           const ODBCXX_STRING &schema,
                           const ODBCXX_STRING &tableName, bool unique,
                           bool approximate);

  ResultSet *
  _getCrossReference(const ODBCXX_STRING &pc, const ODBCXX_STRING &ps,
                     const ODBCXX_STRING &pt, const ODBCXX_STRING &fc,
                     const ODBCXX_STRING &fs, const ODBCXX_STRING &ft);
  ...
  ResultSet *_getSpecialColumns(const ODBCXX_STRING &catalog,
                                const ODBCXX_STRING &schema,
                                const ODBCXX_STRING &table, int what, int scope,
                                int nullable);

protected:
  Statement(Connection *con, SQLHSTMT hstmt, int resultSetType,
...
public:
   ...
   *
   * @return the current result's update count (affected.rows()), or
   * <code>-1</code> if the result is a ResultSet or if there are no more
   * results.
   */
  int getUpdateCount();
   ...
   */
  int getFetchSize() { return fetchSize_; }
  ...
  int getResultSetConcurrency() { return resultSetConcurrency_; }

  /** Get the type for resultsets created by this statement */
  int getResultSetType() { return resultSetType_; }
