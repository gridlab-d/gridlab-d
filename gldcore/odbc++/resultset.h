...
public:
   ...
   */
  ResultSetMetaData *getMetaData() { return metaData_; }
  ...
  int getFetchSize() { return newFetchSize_; }
   ...
   */
  bool wasNull() { return lastWasNull_; }
   ...
   * @param len The number of bytes in the stream
   */
  void updateAsciiStream(const ODBCXX_STRING &colName, ODBCXX_STREAM *s,
                         int len);
   ...
   * @param len The number of bytes in the stream
   */
  void updateBinaryStream(const ODBCXX_STRING &colName, ODBCXX_STREAM *s,
                          int len);
