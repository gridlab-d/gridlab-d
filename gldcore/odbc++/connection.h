...
private:
  ...
  void _connect(const ODBCXX_STRING &dsn, const ODBCXX_STRING &user,
  ...
  const DriverInfo *_getDriverInfo() const { return driverInfo_; }
...
public:
   ...
   * Example:
   * <tt>std::auto_ptr&lt;PreparedStatement&gt; pstmt =
   *     std::auto_ptr&lt;PreparedStatement&gt;(cnt-&gt;prepareStatement(s, x,
   * y));</tt>
   *
   * @param sql The string to prepare, optionally containing parameter
   * markers.
   ...
   * Example:
   * <tt>std::auto_ptr&lt;CallableStatement&gt; cstmt =
   *     std::auto_ptr&lt;CallableStatement&gt;(cnt-&gt;prepareCall(s, x,
   * y));</tt>
   *
   * @param sql The string to prepare, optionally containing parameter
   * markers for input and/or output parameters
   ...
   * by this statement
   */
  CallableStatement *prepareCall(const ODBCXX_STRING &sql, int resultSetType,
                                 int resultSetConcurrency);
