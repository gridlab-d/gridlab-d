...
private:
  struct Rep {
    ...
    size_t len_;
    int refCount_;
    Rep(const ODBCXX_SIGNED_CHAR_TYPE *b, size_t l) : len_(l), refCount_(0) {
      if (len_ > 0) {
...
public:
  /** Default constructor */
  Bytes() : rep_(new Rep(NULL, 0)) { rep_->refCount_++; }
  ...
  /** Copy constructor */
  Bytes(const Bytes &b) : rep_(b.rep_) { rep_->refCount_++; }
  ...
  const ODBCXX_SIGNED_CHAR_TYPE *getData() const { return rep_->buf_; }

  /** Returns the size of the data */
  size_t getSize() const { return rep_->len_; }
};
#endif
...
protected:
  ...
  int _validateYear(int y) { return y; }
...
public:
   ...
   */
  Date(std::time_t t) { this->setTime(t); }

  /** Constructor.
   *
   * Sets this date to the specified string in the <tt>YYYY-MM-DD</tt> format.
   */
  Date(const ODBCXX_STRING &str) { this->parse(str); }
  ...
  void setYear(int year) { year_ = this->_validateYear(year); }

  /** Sets the month of this date */
  void setMonth(int month) { month_ = this->_validateMonth(month); }

  /** Sets the day of this date */
  void setDay(int day) { day_ = this->_validateDay(day); }
...
public:
   ...
   */
  Time(std::time_t t) { this->setTime(t); }

  /** Constructor.
   *
   * Sets the time to the specified string in the <tt>HH:MM:SS</tt> format.
   */
  Time(const ODBCXX_STRING &str) { this->parse(str); }

  /** Copy constructor */
  Time(const Time &t)
      : hour_(t.hour_), minute_(t.minute_), second_(t.second_) {}
  ...
  void setHour(int h) { hour_ = this->_validateHour(h); }

  /** Sets the minute of this time */
  void setMinute(int m) { minute_ = this->_validateMinute(m); }

  /** Sets the second of this time */
  void setSecond(int s) { second_ = this->_validateSecond(s); }
...
public:
   ...
   */
  Timestamp(std::time_t t) { this->setTime(t); }

  /** Constructor.
   *
   * Sets this timestamp from a <tt>YYYY-MM-DD HH:MM:SS[.NNN...]</tt> format
   */
  Timestamp(const ODBCXX_STRING &s) { this->parse(s); }

  /** Copy constructor */
  Timestamp(const Timestamp &t) : Date(t), Time(t), nanos_(t.nanos_) {}
  ...
  int getNanos() const { return nanos_; }

  /** Sets the nanoseconds value of this timestamp */
  void setNanos(int nanos) { nanos_ = this->_validateNanos(nanos); }
...
private:
  ...
  DriverMessage() {}
#if ODBCVER < 0x0300
  static DriverMessage *fetchMessage(SQLHENV henv, SQLHDBC hdbc,
                                     SQLHSTMT hstmt);
#else
  static DriverMessage *fetchMessage(SQLINTEGER handleType, SQLHANDLE h,
                                     int idx);
#endif
...
public:
  ...
  const ODBCXX_CHAR_TYPE *getSQLState() const { return state_; }

  const ODBCXX_CHAR_TYPE *getDescription() const { return description_; }

  int getNativeCode() const { return nativeCode_; }
};
...
public:
  ...
  SQLException(const ODBCXX_STRING &reason = ODBCXX_STRING_CONST(""),
               ...
               int vendorCode = 0)
      : reason_(reason), sqlState_(sqlState), errorCode_(vendorCode)
#if defined(ODBCXX_UNICODE)
  {
  ...
  SQLException(const DriverMessage &dm)
      : reason_(dm.getDescription()), sqlState_(dm.getSQLState()),
   ...
   */
  const ODBCXX_STRING &getSQLState() const { return sqlState_; }

  /** Gets the description of this message */
  const ODBCXX_STRING &getMessage() const { return reason_; }
...
public:
  ...
  /** Copy from a DriverMessage */
  SQLWarning(const DriverMessage &dm) : SQLException(dm) {}
