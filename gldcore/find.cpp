...
#include <cstring>    // For std::strncpy, strncpy_s
#include <filesystem> // For std::filesystem::path, exists, etc. (C++17)
#include <fstream>  // For std::ifstream, std::fstream (to attempt file opening)
#include <optional> // For std::optional (C++17)
#include <string>   // For std::string

// Define access mode constants to match the Windows _access() function's values
// as inferred from the waccess.cpp assertion (bits 2 and 4).
// These are standard POSIX names, but we define them with Windows values for
// clarity.
#ifndef F_OK // Test for existence of file
#define F_OK 0
#endif
#ifndef W_OK // Test for write permission
#define W_OK 2
#endif
#ifndef R_OK // Test for read permission
#define R_OK 4
#endif
// Note: X_OK (execute) is 1, but wasn't part of the assertion's logic.

static FINDTYPE invar_types[] = {FT_ID,
                                 FT_SIZE,
                                 FT_CLASS,
                                 FT_PARENT,
                                 FT_RANK,
...
void findlist_destroy(FINDLIST *fl) {
  ...
  module_free(
      (void **)&fl); // use module_free since allocation used module_malloc
}
...
static int compare_int(int64 a, FINDOP op, int64 b) {
  ...
  default:
    ...
    /* TROUBLESHOOT
            This error is caused when an object find procedure uses a comparison
       operator that isn't allowed on a that type of property.  Make sure the
       property type and the comparison operator are compatible and try again.
       If your GLM file isn't the cause of the problem, try reducing the
       complexity of the GLM file you are using to isolate which module is
       causing the error and file a report with the GLM file attached.
     */
    return 0;
  }
}
...
static int compare_string(char *a, FINDOP op, char *b) {
  ...
  case NE:
    ...
    /* TROUBLESHOOT
            An attempt to compare two numbers using a string comparison
       operation is not generally going to work as expected. Check that this is
       truly the desired comparison and correct it if not.  If so and the
       warning error is not desired you may suppress warnings using <code>#set
       warning=0</code> in the model file.
     */
...
CompareInt:
  return compare_int(a != nullptr && b != nullptr && (int64)strcmp(a, b), op,
                     0);
}

static int compare_property(OBJECT *obj, char *propname, FINDOP op,
                            void *value) {
  /** @todo comparisons should type based and not using string representation
   * (ticket #20) */
  char buffer[1024];
...
**/
static int compare_property_alt(OBJECT *obj, char *propname, FINDOP op,
                                void *value) {
...
#endif
  default:
    output_error("comparison operators not supported for property type %s",
                 class_get_property_typename(prop->ptype));
    /* TROUBLESHOOT
            This error is caused when an object find procedure uses a comparison
    operator that isn't allowed on a that type of property.  Make sure the
    property type and the comparison operator are compatible and try again.  If
    your GLM file isn't the cause of the problem, try reducing the complexity of
    the GLM file you are using to isolate which module is causing the error and
    file a report with the GLM file attached.
     */
    return 0;
  }
  return -1;
...
static int compare(OBJECT *obj, FINDTYPE ftype, FINDOP op, void *value,
                   char *propname) {
  switch (ftype) {
  case FT_ID:
    return compare_int((int64)obj->id, op, (int64) * (OBJECTNUM *)value);
  case FT_SIZE:
    return compare_int((int64)obj->oclass->size, op, (int64) * (int *)value);
  case FT_CLASS:
    return obj->oclass->module != nullptr &&
           compare_string((char *)obj->oclass->name, op, (char *)value);
  case FT_ISA:
    return object_isa(obj, (char *)value);
  case FT_MODULE:
    return (
        obj->oclass->module != nullptr &&
        compare_string((char *)obj->oclass->module->name, op, (char *)value));
  case FT_GROUPID:
    return compare_string((char *)obj->groupid, op, (char *)value);
  case FT_RANK:
    return compare_int((int64)obj->rank, op, (int64) * (int *)value);
  case FT_CLOCK:
    return compare_int((int64)obj->clock, op, (int64) * (TIMESTAMP *)value);
  ...
  default:
    ...
    /* TROUBLESHOOT
            This error is caused when an object find procedure uses a comparison
       operator that isn't allowed on a that header item.  Make sure the header
       item and the comparison operator are compatible and try again.  If your
       GLM file isn't the cause of the problem, try reducing the complexity of
       the GLM file you are using to isolate which module is causing the error
       and file a report with the GLM file attached.
     */
    return 0;
  }
}
...
FINDLIST *new_list(unsigned int n) {
  ...
  FINDLIST *list =
      static_cast<FINDLIST *>(module_malloc(sizeof(FINDLIST) + size - 1));
  if (list == nullptr) {
    errno = ENOMEM;
    return nullptr;
  }
...
#define ADDOBJ(L, N)                                                           \
  (!FOUND((L), (N))                                                            \
       ? ((L).result[(N) >> 3] |= (1 << ((N) & 0x7)), ++((L).hit_count))       \
       : (L).hit_count)
#define DELOBJ(L, N)                                                           \
  (FOUND((L), (N))                                                             \
       ? ((L).result[(N) >> 3] &= ~(1 << ((N) & 0x7)), --((L).hit_count))      \
       : (L).hit_count)
#define ADDALL(L)                                                              \
  ((L).hit_count = object_get_count(),                                         \
   memset((L).result, 0xff, (L).result_size))
#define DELALL(L)                                                              \
  ((L).hit_count = object_get_count(),                                         \
   memset((L).result, 0x00, (L).result_size))
...
/** Search for objects that match criteria
        ...
        Searches criteria may be as follows:
                ...
                find_objects(\p FT_NEW, \p ftype, \p compare, \p value[, ...],
\p nullptr);
        ...
        Values of \p ftype are:
                - \p FT_ID		 compares object ids (expects \e long
value)
                - \p FT_SIZE	 compares object size excluding header (expects
\e long value)
                - \p FT_CLASS	 compares object class name (expects \e char*
value)
                - \p FT_PARENT	 uses parent for comparison (must be followed by
another \p ftype)
                - \p FT_RANK	 compares object rank by number (expects \e long
value)
                - \p FT_CLOCK	 compares object clock (expects \e TIMESTAMP
value)
                - \p FT_PROPERTY compares property (expects \e char* value)
                - \p FT_MODULE	 compares module name
                - \p FT_ISA		 compares object class name including
parent classes (expected \e char* value)
        ...
        Conjunctions are \e AND and \e OR, and can be used to do complex
searches

        OBJECT *find_first(FINDLIST *list) returns the first object in the
result list

        OBJECT *find_next(FINDLIST *list, OBJECT *previous) returns the next
object in the result list

        @return a pointer for FINDLIST structure used by find_first(),
find_next, and find_makearray(), will return nullptr if an error occurs.
**/
FINDLIST *find_objects(FINDLIST *volatile start, ...) {
  ...
  /* FL_GROUP is something of an interrupt option that constructs a program by
  ...
  /* if we're not using FL_GROUP, we break apart the va_arg list, taking data
   * inputs in the "correct" type. */
  for (obj = object_get_first(); obj != nullptr; obj = obj->next) {
    ...
    while ((ftype = FINDTYPE(va_arg(ptr, int))) != FT_END) {
      ...
      /* conjunction */
      if (ftype == AND || ftype == OR) { /* expect another op */
      ...
      case FT_ISA:
        ...
        break;
      default:
        output_error("invalid findtype means search is specified incorrectly "
                     "or FT_END is probably missing");
        /*	TROUBLESHOOT
                The search rule is invalid because it contains a term that isn't
           recognized or the search rule is not correctly terminated by the
           FT_END term.  Check the syntax of the search rule and try again.  If
           the search rule comes from a module, report the problem.
         */
        if (start == FL_NEW)
                   ...
                   OBJECT ***objs) /**< the object found */
{
  ...
  if (n <= 0 || obj == nullptr)
    return 0;
  (*objs) = (OBJECT **)module_malloc(
      sizeof(OBJECT *) *
      (n + 1)); /* one extra for handy nullptr to terminate list */
...
#define VALUE(X, T, M)                                                         \
  ((X).isconstant ? (X).value.constant.M : *((X).value.ref.T))

int compare_pointer_eq(void *a, FINDVALUE b) {
  return *(void **)a == b.pointer;
}
...
int compare_integer16_eq(void *a, FINDVALUE b) {
  return *(int16 *)a == (int16)b.integer;
}
...
int compare_integer32_eq(void *a, FINDVALUE b) {
  return *(int32 *)a == (int32)b.integer;
}
...
int compare_integer64_eq(void *a, FINDVALUE b) {
  return *(int64 *)a == (int64)b.integer;
}
...
void findlist_clear(FINDLIST *volatile list) { DELALL(*list); }
...
PGMCONSTFLAGS find_pgmconstants(FINDPGM *pgm) {
  ...
  while (pgm->next != nullptr)
    pgm = pgm->next;
  return pgm->constflags;
}

static FINDPGM *add_pgm(FINDPGM **pgm, COMPAREFUNC op, unsigned short target,
                        FINDVALUE value, FOUNDACTION pos, FOUNDACTION neg) {
  ...
  if (item != nullptr) {
    ...
    if (*pgm != nullptr) {
      ...
      item->constflags =
          tail->constflags; /* inherit flags from previous result set */
...
static int value(PARSER, char *result,
                 int size) { /* everything to a semicolon */
  START;
...
static int token(PARSER, char *result,
                 int size) { /* everything to a semicolon */
  START;
  while (size > 1 && *_p != '\0' && *_p != ';' && *_p != '\n' && !isspace(*_p))
    COPY(result);
...
static int time_value_datetime(PARSER, TIMESTAMP *t) {
  ...
  if WHITE
    ACCEPT;
  if ((LITERAL("'") || LITERAL("\"")) && TERM(integer(HERE, &Y)) &&
      LITERAL("-") && TERM(integer(HERE, &m)) && LITERAL("-") &&
      TERM(integer(HERE, &d)) && LITERAL(" ") && TERM(integer(HERE, &H)) &&
      LITERAL(":") && TERM(integer(HERE, &M)) && LITERAL(":") &&
      TERM(integer(HERE, &S)) && (LITERAL("'") || LITERAL("\""))) {
    DATETIME dt = {(short)Y, (short)m, (short)d, (short)H, (short)M, (short)S};
    TIMESTAMP tt = mkdatetime(&dt);
    if (tt != TS_INVALID) {
      *t = (int64)tt * TS_SECOND;
      ACCEPT;
      DONE;
    }
  }
...
static int time_value(PARSER, TIMESTAMP *t) {
  ...
  if WHITE
    ACCEPT;
  if TERM (time_value_seconds(HERE, t)) {
    ACCEPT;
    DONE;
  }
  OR if TERM (time_value_minutes(HERE, t)) {
    ACCEPT;
    DONE;
  }
  OR if TERM (time_value_hours(HERE, t)) {
    ACCEPT;
    DONE;
  }
  OR if TERM (time_value_days(HERE, t)) {
    ACCEPT;
    DONE;
  }
  OR if TERM (time_value_datetime(HERE, t)) {
    ACCEPT;
    DONE;
  }
  OR if TERM (integer(HERE, t)) {
    ACCEPT;
    DONE;
  }
  else REJECT;
...
struct {
  COMPAREFUNC pointer, integer, real, string;
    ...
    {
        {compare_pointer_eq, compare_integer_eq, compare_real_eq,
         compare_string_eq}, // EQ=0
        {compare_pointer_lt, compare_integer_lt, compare_real_lt,
         compare_string_lt}, // LT=1
        {compare_pointer_gt, compare_integer_gt, compare_real_gt,
         compare_string_gt}, // GT=2
        {compare_pointer_ne, compare_integer_ne, compare_real_ne,
         compare_string_ne}, // NE=3
        {compare_pointer_le, compare_integer_le, compare_real_le,
         compare_string_le}, // LE=4
        {compare_pointer_ge, compare_integer_ge, compare_real_ge,
         compare_string_ge}, // GE=5
        {compare_pointer_li, compare_integer_li, compare_real_li,
         compare_string_li}, //
        {compare_pointer_nl, compare_integer_nl, compare_real_nl,
         compare_string_nl},
};

struct {
  COMPAREFUNC pointer, integer, real, string, int_16, int_32,
      int_64; /*	int_size to avoid #define int64 (platform.h) */
} comparemap_ext[] = {
    {compare_pointer_eq, compare_integer_eq, compare_real_eq, compare_string_eq,
     compare_integer16_eq, compare_integer32_eq, compare_integer64_eq},
    {compare_pointer_lt, compare_integer_lt, compare_real_lt, compare_string_lt,
     compare_integer16_lt, compare_integer32_lt, compare_integer64_lt},
    {compare_pointer_gt, compare_integer_gt, compare_real_gt, compare_string_gt,
     compare_integer16_gt, compare_integer32_gt, compare_integer64_gt},
    {compare_pointer_ne, compare_integer_ne, compare_real_ne, compare_string_ne,
     compare_integer16_ne, compare_integer32_ne, compare_integer64_ne},
    {compare_pointer_le, compare_integer_le, compare_real_le, compare_string_le,
     compare_integer16_le, compare_integer32_le, compare_integer64_le},
    {compare_pointer_ge, compare_integer_ge, compare_real_ge, compare_string_ge,
     compare_integer16_ge, compare_integer32_ge, compare_integer64_ge},
    {compare_pointer_li, compare_integer_li, compare_real_li, compare_string_li,
     compare_integer16_li, compare_integer32_li, compare_integer64_li},
    {compare_pointer_nl, compare_integer_nl, compare_real_nl, compare_string_nl,
     compare_integer16_nl, compare_integer32_nl, compare_integer64_nl},
};

/* int compare_integer32_eq(void *a, FINDVALUE b) { return
 * *(int32*)a==(int32)b.integer;} */
...
static int expression(PARSER, FINDPGM **pgm) {
  ...
  if WHITE
    ACCEPT;
  if (TERM(name(HERE, pname, sizeof(pname))) && WHITE,
      TERM(compare_op(HERE, &op)) && WHITE,
      TERM(token(HERE, pvalue, sizeof(pvalue)))) {
...
#define OFFSET(X) (unsigned short)((char *)&(_t.X) - (char *)&_t)
    ...
    if (strcmp(pname, "class") == 0) {
       ...
       */
        ...
        add_pgm(pgm, comparemap[op].pointer, OFFSET(oclass), v, nullptr,
                findlist_del);
        (*pgm)->constflags |=
            CF_CLASS; /* this will always reduce in a set class of fixed class,
                         leaving it invariant if already so */
        ACCEPT;
        DONE;
      }
    }
    ...
    if (strcmp(pname, "groupid") == 0) {
      ...
      add_pgm(pgm, comparemap[op % 7].string, OFFSET(groupid), v, nullptr,
              findlist_del);
    ...
    if (strcmp(pname, "module") == 0) {
      ...
      if (idnum < 0) {
        ...
        v.integer = idnum;
        add_pgm(pgm, comparemap[op].integer, OFFSET(id), v, nullptr,
                findlist_del);
      ...
      FINDVALUE v;
      strcpy(v.string, pvalue);
      add_pgm(pgm, comparemap[op].string, OFFSET(name), v, nullptr,
              findlist_del);
      (*pgm)->constflags |= CF_NAME;
      ACCEPT;
      DONE;
      ...
      if (parent == nullptr && strcmp(pvalue, "root") != 0 &&
          strcmp(pvalue, "ROOT") != 0)
       ...
       */
        ...
        add_pgm(pgm, comparemap[op].pointer, OFFSET(parent), v, nullptr,
                findlist_del);
       ...
       */
        ...
        v.integer = rank;
        add_pgm(pgm, comparemap[op].integer, OFFSET(rank), v, nullptr,
                findlist_del);
      ...
      if (sscanf(pvalue, "%d%c%d'%lf\"", &d, &ns, &m, &s) > 0) /* 12N34'56" */
      {
        ...
        v.real = val;
        add_pgm(pgm, comparemap[op].real, OFFSET(latitude), v, nullptr,
                findlist_del);
      ...
      if (sscanf(pvalue, "%d%c%d'%lf\"", &d, &ns, &m, &s) > 0) /* 12N34'56" */
      {
        ...
        v.real = val;
        add_pgm(pgm, comparemap[op].real, OFFSET(longitude), v, nullptr,
                findlist_del);
      ...
      if (v.integer == TS_NEVER)
        REJECT;
      add_pgm(pgm, comparemap[op].integer, OFFSET(clock), v, nullptr,
              findlist_del);
      (*pgm)->constflags |= CF_CLOCK;
      ACCEPT;
      DONE;
    } else if (strcmp(pname, "insvc") == 0 ||
               strcmp(pname, "in") == 0) /* format? */
    {
      FINDVALUE v;
      v.integer = convert_to_timestamp(pvalue);
      ...
      if (v.integer == TS_NEVER)
        REJECT;
      add_pgm(pgm, comparemap[op].integer, OFFSET(in_svc), v, nullptr,
              findlist_del);
      (*pgm)->constflags |= CF_INSVC;
      ACCEPT;
      DONE;
    } else if (strcmp(pname, "outsvc") == 0 ||
               strcmp(pname, "out") == 0) /* format? */
    {
      FINDVALUE v;
      v.integer = convert_to_timestamp(pvalue);
      if (v.integer == TS_NEVER)
        REJECT;
      add_pgm(pgm, comparemap[op].integer, OFFSET(out_svc), v, nullptr,
              findlist_del);
      ...
      /* still need to think about how to input flags without hardcoding the
       * flag name. -mh */
      output_error("find expression on %s not supported", pname.get_string());
      /* TROUBLESHOOT
              A search criteria attempted to search on the flags of an object,
         which is supported yet. Remove the "flags" criteria and try again.
       */
    } else
      output_error(
          "find expression refers to unknown or unsupported property '%s'",
          pname.get_string());
    /* TROUBLESHOOT
...
static int expression_list(PARSER, FINDPGM **pgm) {
  START;
  if TERM (expression(HERE, pgm))
    ACCEPT;
  if (WHITE, LITERAL(";") && TERM(expression_list(HERE, pgm))) {
    ACCEPT;
    DONE;
  }
  if (WHITE, LITERAL("AND") && TERM(expression_list(HERE, pgm))) {
    ACCEPT;
    DONE;
  }
  if (WHITE, LITERAL("and") && TERM(expression_list(HERE, pgm))) {
...
/**
 * @brief Checks if a given path has the specified access modes.
 *        Mitigates TOCTOU by attempting to open the file for requested
 * permissions.
 * @param path_to_check The path to the file.
 * @param mode The desired access mode (F_OK, R_OK, W_OK, or combinations).
 * @return An optional containing the path if all requested accesses are
 * granted, std::nullopt otherwise.
 */
std::optional<std::filesystem::path>
check_path_access_portable(const std::filesystem::path &path_to_check,
                           int mode) {
  // Use std::error_code for non-throwing filesystem operations
  std::error_code ec;

  // 1. Check for existence (implicitly or explicitly requested)
  // If R_OK or W_OK is requested, existence is implied.
  // If only F_OK (mode == 0) is requested, we only check existence.
  if (!std::filesystem::exists(path_to_check, ec) || ec) {
    // File does not exist, or an error occurred during existence check
    return std::nullopt;
  }

  // 2. Check for read permission (R_OK)
  if ((mode & R_OK) != 0) {
    // Most reliable way to check for read access is to attempt to open the file
    // for reading. std::ios::binary is often good practice for general file
    // streams to avoid text mode translations.
    std::ifstream file(path_to_check, std::ios::in | std::ios::binary);
    if (!file.is_open()) {
      return std::nullopt; // Cannot open for read
    }
    // File will be automatically closed when 'file' goes out of scope.
  }

  // 3. Check for write permission (W_OK)
  if ((mode & W_OK) != 0) {
    // Most reliable way to check for write access is to attempt to open the
    // file for read/write. Using std::ios::in | std::ios::out ensures it won't
    // create if non-existent, and won't truncate if exists.
    std::fstream file(path_to_check,
                      std::ios::in | std::ios::out | std::ios::binary);
    if (!file.is_open()) {
      return std::nullopt; // Cannot open for write
    }
    // File will be automatically closed when 'file' goes out of scope.
    // Note: This operation might update the file's last modified timestamp,
    // even if no data is written. This is a common side-effect when "testing"
    // write access this way.
  }

  // If all requested checks passed
 ...
 **/
char *find_file(
    const char *name, /**< the name of the file to find */
    const char *path, /**< the path to search (or nullptr to search the GLPATH
                         environment) */
    ...
    int len)      /**< the len of the buffer */
{
  ...
  // Assuming 'name' is 'const char*', 'mode' is 'int', 'buffer' is 'char*',
  // 'len' is 'size_t' These types are kept for compatibility with the
  // surrounding C-style code.

  // Convert the C-style string 'name' to a modern std::filesystem::path object.
  std::filesystem::path path_to_check_obj(name);

  // Call our new portable and safer access check function
  if (auto accessible_path_opt =
          check_path_access_portable(path_to_check_obj, mode);
      accessible_path_opt.has_value()) {
    // If access is granted, copy the path back into the provided buffer.
    // Use platform-specific safe string copy for robustness.
    const std::string &path_str =
        accessible_path_opt
            ->string(); // Get the string representation of the path

#ifdef _MSC_VER // Microsoft Visual C++ compiler (for Windows)
                // Use strncpy_s for better security on Windows
    strncpy_s(buffer, len, path_str.c_str(), path_str.length());
    buffer[len - 1] = '\0'; // Ensure null-termination, strncpy_s does this too
                            // if size permits
#else                       // POSIX systems (Linux, macOS, GCC/Clang)
    // Use std::strncpy, ensuring null-termination manually
    std::strncpy(buffer, path_str.c_str(), len - 1);
    buffer[len - 1] = '\0'; // Always null-terminate
#endif
    return buffer; // Return the C-style buffer as per original function
                   // signature
  }
  ...
  if (glpath != nullptr) {
    ...
    while (dir) {
      ...
      // 1. Convert the input 'filepath' to std::filesystem::path
      std::filesystem::path fs_filepath(filepath);
      // if (!access(filepath, mode))
      // 2. Call the new portable check function
      // The `if` condition for `!access(...)` means "if access is granted".
      // Our `check_path_access_portable` returns an optional that has a value
      // if access is granted.
      if (check_path_access_portable(fs_filepath, mode).has_value()) {
  ...
  if (module_get_exe_path(filepath, 1024)) {
    ...
    if (check_path_access_portable(std::filesystem::path(tempfp), mode)
            .has_value()) {
...
OBJLIST *objlist_create(CLASS *oclass, PROPERTY *match_property, char *part,
                        char *match_op, void *match_value1,
                        void *match_value2) {
  OBJLIST *list = (OBJLIST *)(malloc(sizeof(OBJLIST)));

  /* check parameters */
  if (!list)
    return (OBJLIST *)(output_error("find_create(): memory allocation failed"),
                       nullptr);
...
OBJLIST *objlist_search(char *group) {
  ...
  list->objlist =
      (s_object_list **)(malloc(sizeof(OBJECT *) * result->hit_count));
  if (!list->objlist)
    return nullptr;
  for (obj = find_first(result), n = 0; obj != nullptr;
       obj = find_next(result, obj), n++)
    list->objlist[n] = obj;
  return list;
}
...
size_t objlist_add(OBJLIST *list, PROPERTY *match, char *match_part,
                   char *match_op, void *match_value1, void *match_value2) {
  OBJECT *obj;
  ...
  for (obj = object_get_first(); obj != nullptr; obj = object_get_next(obj)) {
    ...
    if (obj->oclass != list->oclass)
      continue;
    if (property_compare_basic(match->ptype, op, x, match_value1, match_value2,
                               match_part)) {
      if (list->size == list->asize) { // grow the list
...
size_t objlist_del(OBJLIST *list, PROPERTY *match, char *match_part,
                   char *match_op, void *match_value1, void *match_value2) {
  PROPERTYCOMPAREOP op = property_compare_op(match->ptype, match_op);
  int n, m;
  ...
  for (n = 0; n < list->size; n++) {
    ...
    if (list->oclass != nullptr && obj->oclass != list->oclass)
      continue;
    if (property_compare_basic(match->ptype, op, x, match_value1, match_value2,
                               match_part))
  ...
  for (n = 0, m = 0; n < list->size; n++) {
    if (list->objlist[n] == nullptr)
      continue;
    if (n > m)
      list->objlist[m++] = list->objlist[n];
  }
  list->size = m;
  return list->size;
}
size_t objlist_size(OBJLIST *list) { return list->size; }
...
/** Apply the function to each object in the object list.
        ...
        For the last call the object pointer is nullptr and the position
 indicate is -1.

        The function's return value must indicate success (non-zero) or failure
 (zero).

        @example The following example illustrates how to use #objlist_apply by
 performing a log mean calculation on a property of objects in a particular:
        ...
        double example(char *classname, char *propertyname)
        {
                ...
                PROPERTY *prop =
 oclass?class_find_property(oclass,propertyname):nullptr; double zero=0; if (
 prop==nullptr ) return QNAN; OBJLIST *list =
 objlist_create(oclass,prop,nullptr,">",&zero,nullptr); if ( list==nullptr )
 return QNAN; arg.addr = prop->addr; return objlist_apply(list,&arg,logmean);
        }

        @returns a positive value if successful, zero or negative if failed
        (negative value indicate how many objects were successfully processed
 before failure).
 **/
int objlist_apply(
