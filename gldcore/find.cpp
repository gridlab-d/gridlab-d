...
// These are standard POSIX names, but we define them with Windows values for clarity.
...
static int compare(OBJECT *obj, FINDTYPE ftype, FINDOP op, void *value, char *propname)
{
	...
	case FT_CLASS:
		return obj->oclass->module != nullptr && compare_string((char *)obj->oclass->name, op, (char *)value);
	case FT_ISA:
		return object_isa(obj, (char *)value);
	case FT_MODULE:
		return (obj->oclass->module != nullptr && compare_string((char *)obj->oclass->module->name, op, (char *)value));
	case FT_GROUPID:
...
static int time_value_datetime(PARSER, TIMESTAMP *t)
{
	...
	if WHITE
		ACCEPT;
	if ((LITERAL("'") || LITERAL("\"")) && TERM(integer(HERE, &Y)) && LITERAL("-") && TERM(integer(HERE, &m)) && LITERAL("-") && TERM(integer(HERE, &d)) && LITERAL(" ") && TERM(integer(HERE, &H)) && LITERAL(":") && TERM(integer(HERE, &M)) && LITERAL(":") && TERM(integer(HERE, &S)) && (LITERAL("'") || LITERAL("\"")))
	{
 ...
 *        Mitigates TOCTOU by attempting to open the file for requested permissions.
 * @param path_to_check The path to the file.
 * @param mode The desired access mode (F_OK, R_OK, W_OK, or combinations).
 * @return An optional containing the path if all requested accesses are granted, std::nullopt otherwise.
 */
std::optional<std::filesystem::path> check_path_access_portable(const std::filesystem::path &path_to_check, int mode)
{
	...
	if ((mode & R_OK) != 0)
	{
		// Most reliable way to check for read access is to attempt to open the file for reading.
		// std::ios::binary is often good practice for general file streams to avoid text mode translations.
	...
	if ((mode & W_OK) != 0)
	{
		// Most reliable way to check for write access is to attempt to open the file for read/write.
		// Using std::ios::in | std::ios::out ensures it won't create if non-existent, and won't truncate if exists.
		std::fstream file(path_to_check, std::ios::in | std::ios::out | std::ios::binary);
		if (!file.is_open())
		{
		...
		// even if no data is written. This is a common side-effect when "testing" write access this way.
	}
				...
				int len)		  /**< the len of the buffer */
{
	...
	// Assuming 'name' is 'const char*', 'mode' is 'int', 'buffer' is 'char*', 'len' is 'size_t'
	// These types are kept for compatibility with the surrounding C-style code.
	...
	if (auto accessible_path_opt = check_path_access_portable(path_to_check_obj, mode); accessible_path_opt.has_value())
	{
		// If access is granted, copy the path back into the provided buffer.
		// Use platform-specific safe string copy for robustness.
		const std::string &path_str = accessible_path_opt->string(); // Get the string representation of the path
		...
		buffer[len - 1] = '\0'; // Ensure null-termination, strncpy_s does this too if size permits
...
#endif
		return buffer; // Return the C-style buffer as per original function signature
	}
	...
	if (glpath != nullptr)
	{
		...
		while (dir)
		{
			...
			// Our `check_path_access_portable` returns an optional that has a value if access is granted.
	...
	if (module_get_exe_path(filepath, 1024))
	{
		...
		if (check_path_access_portable(std::filesystem::path(tempfp), mode).has_value())
		{
...
OBJLIST *objlist_create(CLASS *oclass, PROPERTY *match_property, char *part, char *match_op, void *match_value1, void *match_value2)
{
	...
	/* check parameters */
	if (!list)
		return (OBJLIST *)(output_error("find_create(): memory allocation failed"), nullptr);
	...
	if (!list->objlist)
		return (OBJLIST *)(output_error("find_create(): memory allocation failed"), free(list), nullptr);
	list->oclass = oclass;
