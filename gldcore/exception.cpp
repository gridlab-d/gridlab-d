/** $Id: exception.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - <b>Core exceptions</b> should include the offending function
          call and the specifics of the error in context, as in
                \code
                THROW("object_tree_add(obj='%s:%d', name='%s'): memory
 allocation failed (%s)", obj->oclass->name, obj->id, name, strerror(errno));
                \endcode
          For functions that a \p static, you should include the file in which
          it is implemented, as in
                \code
                THROW("object.c/addto_tree(tree.name='%s', item.name='%s'):
 strcmp() returned unexpected value %d", tree->name, item->name, rel);
                \endcode
          One important situation to consider are exception that occur during
 I/O where the context in GridLAB may not be as important the context in the I/O
 stream, as in
                \code
...
EXCEPTIONHANDLER *create_exception_handler(void) {
  EXCEPTIONHANDLER *ptr =
      static_cast<EXCEPTIONHANDLER *>(malloc(sizeof(EXCEPTIONHANDLER)));
  if (ptr == nullptr) {
 ...
 **/
void delete_exception_handler(
    EXCEPTIONHANDLER *ptr) /**< a pointer to the exception handler */
{
  EXCEPTIONHANDLER *target;
  if (ptr == nullptr) {
    output_fatal("delete_exception_handler(): ending an exception handler "
                 "block where no exception handler was present");
    return;
  }
  target = ptr->next;
 ...
 **/
char *exception_msg(void) { return handlers->msg; }
