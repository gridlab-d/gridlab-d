/** $Id: random.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        @note The random number generators are not thread-safe.  This isn't
 generally a problem, unless you are using the pseudo-random sequences.  In that
 case, you need to lock the state variable you are using when generating random
 numbers.
...
static inline int gettimeofday(struct timeval *tv, struct timezone *tz)
{
  if (tv)
  {
    ...
    // Convert FILETIME (100-ns ticks since 1601-01-01) to microseconds since
    // 1970-01-01
    ULARGE_INTEGER uli;
    uli.LowPart = ft.dwLowDateTime;
    uli.HighPart = ft.dwHighDateTime;

    const unsigned long long EPOCH_1601_TO_1970_100NS =
        116444736000000000ULL; // correct offset
    const unsigned long long unix100ns =
        uli.QuadPart - EPOCH_1601_TO_1970_100NS;
    const unsigned long long usec = unix100ns / 10ULL;
  ...
  if (tz)
  {
    ...
    // Set DST flag based on current state
    tz->tz_dsttime = (tzid == TIME_ZONE_ID_DAYLIGHT) ? 1 : 0;
  }
...
static struct
{
  ...
  int nargs;
          ...
          /** @todo Add some other distributions (e.g., Cauchy, Laplace) (ticket
             #56)*/
};
/** Converts a distribution name to a #RANDOMTYPE
 **/
...
#define MULTIPLIER 44485709377909ULL
    ...
    *state = (unsigned int)((MULTIPLIER * (unsigned int64)(*state)) &
                            0xffffffffffffULL); /* %2^48 same as &(2^48-1) */
...
unsigned int64 random_id(void)
{
  ...
  int64 rv = 0;
  if (state == 0)
    state = (unsigned int)time(nullptr);
  rv = randwarn(&state);
...
double random_degenerate(unsigned int *state, double a)
{
  ...
  if (a != 0 && (aa < 1e-30 || aa > 1e30))
    output_warning(
        "random_degenerate(a=%g): a is outside normal bounds of +/-1e(+/-30)",
        a);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
     outside the expected range of real numbers. Correct the functional
...
/** Generate a uniformly distributed random number
        ...
        Note that this uniform distribution includes \e a but does not include
 \e b.
 **/
                      ...
                      double b)            /**< the maximum number */
{
  ...
  if (a != 0 && (aa < 1e-30 || aa > 1e30))
    output_warning("random_uniform(a=%g, b=%g): a is outside normal bounds of "
                   "+/-1e(+/-30)",
                   a, b);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
  ...
  if (b != 0 && (ab < 1e-30 || ab > 1e30))
    output_warning("random_uniform(a=%g, b=%g): b is outside normal bounds of "
                   "+/-1e(+/-30)",
                   a, b);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
...
is
        \f[ \varphi\left(x\right) =
\frac{1}{\sqrt{2\pi}\sigma}e^{\frac{\left(x-\mu\right)^2}{2\sigma^2}}	\f]

        Normally distributed random numbers are generated using the Box-Muller
method:
                        ...
                        double p)            /**< the probability of generating a 1 */
{
  ...
  if (ap != 0 && (ap < 1e-30 || ap > 1e30))
    output_warning("random_bernoulli(p=%g): p is not within the normal bounds "
                   "of +/-1e(+/-30)",
                   p);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
                      ...
                      double *x)           /**< the sample list */
{
  if (n > 0)
  {
    ...
    if (v != 0 && (v < 1e-30 || v > 1e30))
      output_warning("random_sampled(n=%d,...): sampled value is not within "
                     "normal bounds of +/-1e(+/-30)",
                     n);
    /* TROUBLESHOOT
            An attempt to generate a random number used a parameter that was
       outside the expected range of real numbers. Correct the functional
...
/** Generate a Pareto distributed random number
        ...
        The cumulative density function is \f[ \varphi\left( x<k \right) =
1-(\frac{m}{x})^k \f].
**/
                     ...
                     double k)            /**< the k value */
{
  ...
  if (am != 0 && (am < 1e-03 || am > 1e30))
    output_warning("random_pareto(m=%g, k=%g): m is not within the normal "
                   "bounds of +/-1e(+/-30)",
                   m, k);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
   ...
   */
  if (ak > 1e30)
    output_warning("random_pareto(m=%g, k=%g): k is not within the normal "
                   "bounds of +/-1e(+/-30)",
                   m, k);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
   ...
   */
  if (k <= 0)
    throw_exception("random_pareto(m=%g, k=%g): k must be greater than 1", m,
                    k);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
     outside the expected range of real numbers. Correct the functional
...
/** Generate a log Gaussian distributed random number
        ...
        \f[ \varphi\left(x\right) =
 \frac{1}{\sqrt{2\pi}x\sigma}e^{\frac{\left(\ln x-\mu\right)^2}{2\sigma^2}}
 \f]
 **/
                          ...
                          double lambda)       /**< the rate parameter lambda */
{
  double r = randunit(state);
  if (lambda <= 0)
    throw_exception("random_exponential(l=%g): l must be greater than 0",
                    lambda);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
     outside the expected range of real numbers. Correct the functional
  ...
  if (lambda < 1e-30 || lambda > 1e30)
    output_warning("random_exponential(l=%g): l is not within the normal "
                   "bounds of 1e(+/-30)",
                   lambda);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
...
/** Generate a Weibull distributed random number
        ...
        \f[ \varphi\left(x;k,\lambda\right) = \frac{k}{\lambda} \left(
 \frac{x}{\lambda} \right)^{k-1} e^{-\left(\frac{x}{\lambda}\right)^k} \f]

        @note This distribution is not tested because the test requires the
 Gamma function, which itself would have to be implemented and tested.
                      ...
                      double k)            /**< rate shape parameter */
{
  double r = randunit(state);
  if (k <= 0)
    throw_exception("random_weibull(l=%g, k=%g): k must be greater than 0",
                    lambda, k);
  /* TROUBLESHOOT
          An attempt to generate a random number used a parameter that was
     outside the expected range of real numbers. Correct the functional
...
/** Generate a Rayleigh distributed random number
        ...
        \f[ \varphi \left( x;\sigma \right) = \frac{x e^{-\frac{x^2}{2\sigma^2}}
 }{\sigma^2} \f]
...
/** Generate a Gamma distributed random number
        ...
        \f[ \varphi \left( x; \alpha,\beta\right) = \frac{1}{\Gamma(\alpha)
 \beta^\alpha} x^{\alpha-1} e^{-x/\beta} \f]
                    ...
                    double alpha, double beta)
{
  ...
  if (fabs(na - alpha) < 1e-8 &&
      na < 12) /* alpha is an integer andsafe against underflow */
  {
    unsigned int i;
    double prod = 1;
    for (i = 0; i < na; i++)
  ...
  else /* a is large */
  {
    ...
    do
    {
      ...
      v = randunit(state);
             ...
             (1 + y * y) * exp((alpha - 1) * log(x / (alpha - 1)) - sqrta * y));
    return beta * x;
  }
}
...
/** Generate a Beta distributed random number
        ...
        \f[ \varphi \left( x; \alpha,\beta\right) =
...
static double _random_value(RANDOMTYPE type, unsigned int *state, va_list ptr)
{
  ...
  case RANDOMTYPE::RT_SAMPLED: /* ... unsigned n_samples, double
                                  samples[n_samples] */
  {
  ...
  default:
    ...
    /* TROUBLESHOOT
            An attempt to generate a random number specific a distribution type
       that isn't recognized. Check that the distribution is valid and try
       again.
     */
  }
  return QNAN; /* never gets here */
...
int _random_specs(RANDOMTYPE type, double a, double b, char *buffer, int size)
{
  ...
  {
  case RANDOMTYPE::RT_DEGENERATE: /* ... double value */
    return snprintf(buffer, size, "degenerate(%lf)", a);
  case RANDOMTYPE::RT_UNIFORM: /* ... double min, double max */
    return snprintf(buffer, size, "uniform(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_NORMAL: /* ... double mean, double stdev */
    return snprintf(buffer, size, "normal(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_BERNOULLI: /* ... double p */
    return snprintf(buffer, size, "bernoulli(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_SAMPLED: /* ... unsigned n_samples, double
                                  samples[n_samples] */
    return snprintf(buffer, size, "sampled(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_PARETO: /* ... double base, double gamma */
    return snprintf(buffer, size, "pareto(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_LOGNORMAL: /* ... double gmean, double gsigma */
    return snprintf(buffer, size, "lognormal(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_EXPONENTIAL: /* ... double lambda */
    return snprintf(buffer, size, "exponential(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_RAYLEIGH: /* ... double sigma */
    return snprintf(buffer, size, "rayleigh(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_WEIBULL: /* ... double lambda, double k */
    return snprintf(buffer, size, "weibull(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_GAMMA: /* ... double alpha, double beta */
    return snprintf(buffer, size, "gamma(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_BETA: /* ... double alpha, double beta */
    return snprintf(buffer, size, "beta(%lf,%lf)", a, b);
  case RANDOMTYPE::RT_TRIANGLE: /* ... double a, double b */
    return snprintf(buffer, size, "triangle(%lf,%lf)", a, b);
  default:
    throw_exception("_random_specs(type=%d,...); type is not valid", type);
    /* TROUBLESHOOT
            An attempt to describe a random number specific a distribution type
       that isn't recognized. Check that the distribution is valid and try
       again.
     */
  }
 ...
 **/
int random_apply(
    char *group_expression, /**< the group definition; see find_objects() */
                    ...
                    ...)             /**< the distribution's parameters */
{
  ...
  va_list ptr;
  va_start(ptr, type);
  x = _random_value(type, nullptr, ptr);
  va_end(ptr);
  return x;
}
...
/** Generate a pseudo-random value using the known state that is updated.
        @return a double containing the random number
 **/
double pseudorandom_value(
    RANDOMTYPE type,     /**< the type of distribution desired */
  ...
  else
  {
    ...
    if (expected == 0)
      output_test("%-16.16s %10.4f %10.4f %9.6f  %s", parameter, actual,
                  expected, actual - expected, iserror ? "ERROR" : "");
    else
      output_test("%-16.16s %10.4f %10.4f %9.6f%% %s", parameter, actual,
                  expected, (actual - expected) / expected * 100,
                  iserror ? "ERROR" : "");
    return iserror;
  }
}
...
/** Test random distributions
        ...
        The test results are output to the file specified by the \p
...
int random_test(void)
{
  ...
  errorcount += report(nullptr, 0, 0, 0);
  ...
  errorcount += report("Stdev", samp_stdev(sample, count),
                       sqrt((b - a) * (b - a) / 12), 0.01);
  errorcount += report("Min", samp_min(sample, count), a, 0.01);
  ...
  errorcount += report(nullptr, 0, 0, 0.01);
  errorcount += report("Mean", samp_mean(sample, count), a, 0.01);
  errorcount +=
      report("Stdev", samp_stdev(sample, count), sqrt(a * (1 - a)), 0.01);
  ...
  errorcount += report(nullptr, 0, 0, 0.01);
  errorcount +=
      report("Mean", samp_mean(sample, count), exp(a + b * b / 2), 0.01);
  errorcount += report("Stdev", samp_stdev(sample, count),
                       sqrt((exp(b * b) - 1) * exp(2 * a + b * b)), 0.1);
  ...
  errorcount += report(nullptr, 0, 0, 0.01);
  errorcount +=
      report("Mean", samp_mean(sample, count), a * sqrt(3.1415926 / 2), 0.01);
  errorcount += report("Stdev", samp_stdev(sample, count),
                       sqrt((4 - 3.1415926) / 2 * a * a), 0.01);
  if (preverrors == errorcount)
    ok++;
  else
    failed++;
  ...
  errorcount +=
      report("Stdev", samp_stdev(sample, count), sqrt(a * b * b), 0.01);
  if (preverrors == errorcount)
    ok++;
  else
    failed++;
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (sample[i] != v)
      failed++,
          output_test("Sample %d did not match (%f!=%f)", i, sample[i], v);
  }
  if (preverrors == errorcount)
    ok++;
  else
  ...
  else
  {
    output_verbose("%d random distributions tests completed with no "
                   ...
                   ok);
    output_test(
        "randtest: %d random distributions tests completed, %d errors found",
        ok, errorcount);
  }
...
static randomvar_struct *randomvar_list = nullptr;
...
{
  randomvar_struct *var = (randomvar_struct *)data;
  return snprintf(string, size, "%lf", var->value);
}
  ...
  do
  {
    ...
    if (var->flags & RNF_INTEGRATE)
      ...
      var->value = v;
           ...
           !(var->low < var->value && var->value < var->high));
  return 1;
}
...
size_t randomvar_getspec(char *str, size_t size, const randomvar_struct *var)
{
  ...
  if (_random_specs(var->type, var->a, var->b, specs, sizeof(specs)) <= 0)
    return 0;
  len = snprintf(buffer, sizeof(buffer), "state: %u; type: %s; min: %g; max: %g; refresh: %u%s",
