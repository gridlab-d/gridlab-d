...
// IMPORTANT: include winsock2.h BEFORE windows.h (timeval is defined here on Windows)
...
static inline int gettimeofday(struct timeval *tv, struct timezone *tz)
{
	if (tv)
	{
		...
		// Convert FILETIME (100-ns ticks since 1601-01-01) to microseconds since 1970-01-01
		ULARGE_INTEGER uli;
		uli.LowPart = ft.dwLowDateTime;
		uli.HighPart = ft.dwHighDateTime;

		const unsigned long long EPOCH_1601_TO_1970_100NS = 116444736000000000ULL; // correct offset
		const unsigned long long unix100ns = uli.QuadPart - EPOCH_1601_TO_1970_100NS;
	...
	if (tz)
	{
		...
		// Bias is minutes difference between UTC and local time (commonly used for tz_minuteswest)
...
int convert_to_randomvar(char *string, void *data, PROPERTY *prop)
{
	...
	while ((token = strtok_s(token == nullptr ? buffer : nullptr, ";", &last)) != nullptr)
	{
