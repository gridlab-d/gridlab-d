...
Transient mode is triggered in the "data_cap_deltamode_trigger_3.player" file at
8.5 seconds. The use case requires that the simulation continue in transient mode
past ten seconds so that when the user reaches ten seconds, the next step would
continue in transient mode. This is the key difference between the use case #2.
To ensure this happens the model being used must be adjusted to create a longer
transient event.
...
This tests use case #3 which is defined as the following sequence of events:
1. Run in QSTS to t=5
2. Call `step()` advancing to a target time of 10 seconds - Run QSTS from t=5
to t=8, and then trigger transient mode at t=8.5, continuing in transient mode
past t=10.
