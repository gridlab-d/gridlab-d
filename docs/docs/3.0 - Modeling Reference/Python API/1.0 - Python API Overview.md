...
The Python API is distributed as a PyPI package that users can `pip install ...`. Users can `import ...` the package like any other Python package, allowing them to make the API calls to GridLAB-D™; more specifically this will allow users to instantiate a GridLAB-D™ engine, load a GridLAB-D™ model into it, advance the GridLAB-D™ model through simulation time in a controlled manner, reading and writing to model parameters as simulation time is being advanced.
