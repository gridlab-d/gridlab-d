...
[This example](../../../../examples/api/example_sim_stepping.py) shows the use of both API calls. The first call is to `step_to()` specifying a time 20 minutes after the "starttime" of the model. After reaching this point, the simulation is advanced a few time steps with the `step()` API. Lastly, the script demonstrates the fatal error that is generated when trying to step beyond the end of the specified "stoptime" in the simulation.
