...
Adding implicit enduses to your model is as simple as:
...
Add or remove items from the list as necessary. To learn more about how an *enduse* is configured, read further.
...
* **shape->schedule** -
    the schedule is updated to adjust the schedule value according to the target time of the sync event. The value _next_t' is updated to give the time of the next expected schedule change.
