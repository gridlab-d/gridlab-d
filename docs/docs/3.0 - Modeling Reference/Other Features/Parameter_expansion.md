...
`${parameter:=word}` | Assign default value. If parameter is unset or null, the expansion of word is assigned to parameter. The value of parameter is then substituted. Positional parameters and special parameters may not be assigned to in this way.
...
`${name+=value}` | TODO - Empty - `${name+=value}`
`${name-=value}` | TODO - Empty - `${name-=value}`
`${name*=value}` | TODO - Empty - `${name*=value}` 
`${name/=value}` | TODO - Empty - `${name/=value}`
`${name%=value}` | Operates on the global variable (only for int32 values). The addition (+=), substract (-=), multiplication (*=), division (/=), and modulo (%=) operators are performed before the value is expanded.
`${name<value[?yes:no]}` | TODO - Empty - `${name<value[?yes:no]}`
`${name>value[?yes:no]}` | TODO - Empty - `${name>value[?yes:no]}`
`${name<=value[?yes:no]}` | TODO - Empty - `${name<=value[?yes:no]}`
`${name>=value[?yes:no]}` | TODO - Empty - `${name>=value[?yes:no]}` 
`${name==value[?yes:no]}` | TODO - Empty - `${name==value[?yes:no]}`
`${name!=value[?yes:no]}` | Performs a comparison test of the global variable to a value (only for int32 values). If the test is true, the value 1 is expanded, otherwise the value 0 is expanded. If the string _yes_ is provided or empty, it is used in place of the value 1. If the string for _no_ is provided or empty, it is used in place of the value 0.
`${name&=value}` | TODO - Empty - `${name&=value}`
`${name|=~value}` | TODO - Empty - `${name|=~value}`
`${name^=value}` | TODO - Empty - `${name^=value}`
`${name^=~value}` | Perform bitwise operations on the int32 variable. The bitwise and (&=), or (|=), xor (^=), and equiv (^+~) operations are performed on the value before it is expanded.
`${name&value[?yes:no]}` | TODO - Empty - `${name&value[?yes:no]}`
`${name|~value[?yes:no]}` | Perform bitwise tests on the int32 variable. The bitwise is-set (&) and is-clear (|~) return 1 (or _yes_) if the result is non-zero and 0 (or _no_) if the result is zero.
