...
**Step 1**: Specify in the model file (the GLM file) that the output will be a plot file by adding the following directive in the object recorder section of the model file:

        file plot:residential_loads.plot;

**Step 2**: Specify what type of output file is desired. Current supported options are:
...
**Step 3**: The next line consists of several Gnuplot directives typed together as one single string. The directives should be separated by a vertical line `|` including the last directive. The whole string should be ended with a semicolon just like the other model file directives. An example follows:
...
!!! note
        ...
        For the Windows version: Gridlab for Windows will try to plot using wgnuplot which it assumes is installed in the path
