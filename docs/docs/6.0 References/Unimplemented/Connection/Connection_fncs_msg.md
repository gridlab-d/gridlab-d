...
Remember that the actual topic that gets published is prepended with the **fncs_msg** objects name. So If the name of the **fncs_msg** object is `federate1` then the topic that house's indoor air temperature its published on is `federate1/house1_indoor_air_temperature`. The `precommit|presync|sync|postsync|commit` choice determines when GridLAB-D™ will publish or update the properties during each exec cycle. If you are not sure where in the exec cycle to publish or update a property then, in general, precommit is best for subscriptions and commit is best for publications. A tolerance can be specified for which a property is published. An example is shown below: 
    
    
    publish "commit:house1.air_temperature -> house1_indoor_air_temperature; 5";
    

Now the GridLAB-D™ federate will only publish house1's `air_temperature` property only if the property has changed by 5 degrees. **Please Note:** Object names cannot contain "<", ">", or "-" as the line parser will fail to parse the publish and subscribe lines correctly if they do. 
