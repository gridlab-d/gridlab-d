...
The clock block declares how GridLAB-D should manage time during the simulation. The clock block controls the following features of the internal GridLAB-D clock: 
  ...
  * The default timezone used for schedules, and reading and writing. (If the `TZ` environment variable is not set, the `timezone` should be the first feature specified because it is needed to read the `starttime` and `stoptime`.)
  * The clock starttime.
  * The clock stoptime.
