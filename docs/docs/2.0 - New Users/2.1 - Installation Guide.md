...
!!! note
    ...
    Windows 11 may inadvertantly attempt to use `gridlabd.js` rather than `gridlabd.exe` to run simulation files via:

            gridlabd my_simulation.glm

    If this occurs, the user will likely be prompted to open a .js file. To ensure that GridLAB-D is correctly called to run your simulation, specify the .exe in this call:

            gridlabd.exe my_simulation.glm
