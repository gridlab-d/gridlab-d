...
GridLAB-D™ uses modules to define classes of objects. *For example, the **residential** module contains the definitions of the **house**, the **water heater**, and **end-use** load classes.* Each class *of object* must be defined in a module, *and cannot exist without one*. Classes define which properties are allowed in objects. *For example, it makes sense to define the number of doors and windows in a house object, but it wouldn't for a EV charger object*. Classes also define how the behaviors of those objects are implemented, *such as the thermodynamics of a house based on its insulation, thermostat setpoint, outside air temperature, and other relevant properties*. Objects are instances of classes, so each object can have its own values for each property while sharing behaviors with other objects of the same class. *Just as every house on the street isn't identical nor sets their thermostat to the same temperature, but each house does obey the laws of thermodynamics*. During simulations, GridLAB-D™ will keep the objects' properties synchronized with each other as time advances *to accurately model the behavior of the object with respect to time*. *Note also that* modules can either be static, meaning they are implemented in a dynamic link library (e.g., `.dll` in Windows, `.so` in Linux, `.dylib` on Macs), or they can be dynamic, meaning they are compiled and linked at runtime.
