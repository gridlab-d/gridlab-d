...
Consider a simple example.  We need to define a [house](../3.2%20Modules/Residential/house-e.md), each instance of which has a `floor_area`, drawn from a normal distribution about a mean of 2500 square feet [sf] with a standard deviation of 250 sf. To instantiate the house object, both require the `residential` module to be added first. Compare the GLM object definition:
...
In practice, GridLAB-D™ concerns itself with **modeling** in the context of GLM files, using objects, the class of each of those objects, and the DLL modules that registered those classes: 
  ...
  * **Objects** describe specific instances of a thing that can respond to and/or act on other objects. A simulation can contain very many objects. It is not uncommon to see a model that contains thousands of objects. Some of the large GridLAB-D™ models have been built that contain well over 100,000 objects. GridLAB-D™ objects are malloc’ed (memory allocation) structures that include enough space for a GridLAB-D™ object header and an instance of the class. In practice, the C++ classes are created using `malloc()` in the core, rather than using `new()` in the modules. Using the properties defined for the GridLAB-D™ class, it is possible for modules with no definition for an object’s C++ class to access any published variable in that foreign class. 
