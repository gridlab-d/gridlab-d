...
Consider a simple example. We need to define a `house`, each instance of which has a `floor_area`, drawn from a normal distribution about a mean of 2500 square feet [sf] with a standard deviation of 250 sf. To instantiate the house object, both require the `residential` module to be added first. Compare the GLM object definition: 
...
The third thing to observe is that GridLAB-D™ doesn’t do a great deal by itself. Without a model file or any modules, it becomes a big “Hello World!” exercise. If a model file includes, but does not use, objects of a particular type, that functionality is never realized. Without some sort of action from GridLAB-D™, it looks much like a large blank monolith. 
...
*TODO - Module Versioning - Is this still a thing?* To enforce verification of the module's version information, simply include the desired version in the module block:    
