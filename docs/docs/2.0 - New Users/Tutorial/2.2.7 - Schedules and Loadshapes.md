...
The **schedule object** stems from the separate schedule methods present in several objects in GridLAB-D™. Since the same basic system is in place in three different objects, it made sense to turn the schedule into a standalone class that can be used by many different classes with a minimum of effort. 
...
The schedule object uses the same formatting as the POSIX crontab utility. Absent scale values will be assumed to be 1.0. 
...
There are several ways to use a schedule and the most straightforward is to define the scheduled values as the values to be directly used. Open up the [schedules-direct.glm](https://github.com/gridlab-d/course/blob/master/Tutorial/Chapter%204%20-%20Recorders%20and%20Players/Schedules/Direct/schedules-direct.glm) file and you'll see that there are two schedules that only change based on the minute value in the current simulation time.
...
Running this simulation and looking at the "*overhead_line_losses.csv*" files shows the following:
...
We can see the values changing every fifteen minutes as the load changes. If the simulation is extended in duration to run from midnight to 2am, the values will repeat each hour since the **hour** field in the schedule definitions was left as a `*`, indicating it is valid for all hours. Go ahead and adjust the simulation to run until 2am and also change the hour fields in both schedules to `0`. This will make the schedules only applicable during the times when the hour is 00 (from midnight to 1am). Re-running the simulation shows the following:
...
An alternative application for schedulers is to use them to modify existing values rather than defining them directly. This allows schedules to be used to scale a given value in a system (load, voltage, etc.) by a linear amount while allowing unique specific quantities for each object. Without this capability, getting equivalent functionality would require unique schedules for each object and it would not be obvious from examining the model file that they were all being scaled together.

Open ["*schedules-modify.glm*"](https://github.com/gridlab-d/course/blob/master/Tutorial/Chapter%204%20-%20Recorders%20and%20Players/Schedules/Modify/schedules-modify.glm) to see how this can be implemented. As we can see from `#include "schedules.glm";`, the schedules in this model have been defined in an external file and are associated with the main model by using `#include`. Opening up the referenced file "*schedules.glm*" we can see the two schedules used in the previous example with new values being applied.

Back in the main file, you can see the load has been declared slightly differently with the real and reactive components as separate parameters of the object; we'll discuss why that is shortly. For each of these components the name of a schedule object is referenced multiplied by a constant value representing a baseline load level. 
    ...
    object load {
        ...
        phases ABC;
      constant_power_A_real load_schedule_2*20000;
      constant_power_A_reac load_schedule_2*20000;
    }

Running this model produces the following from the "*underground_line_losses.csv*"
...
!!! note
    ...
    The value of the property set by a schedule transform is updated before the presync pass, immediately after the schedules are updated.
...
!!! caveat
    ...
    Because all times are considered in local time, there is a possibility that scheduled changes during the daylight-savings/summer time (DST) shifts could result in a missing or duplicate value. For example, scheduling an event at 2am the night DST ends may result in a duplicate value. The solution is to schedule the event either before or after the time change so the ambiguity is resolved internally as appropriate. Similarly, scheduling an event at 2am the night DST starts could result in a gap in the schedule, the problem is also resolved by scheduling the event either before or after 2am so the gap is automatically filled by the schedule compiler. 
...
* Schedule transforms don't support the use of external units such as kV, MVA, mA, etc. GridLAB-D™ assumes all values are the base units even if you do something like:
  * `constant_power_A_real load_schedule_2*20 kVA;`
  ...
  GridLAB-D™ will not throw an error but simply assumes the load is 20 Watts and will compute the powerflow accordingly.
...
The **boolean** flag can be used to ensure that the schedule contains only 0 or 1 values: 
...
* **Modulated** - the loadshape value is the energy of a series of pulses generated during the interval and is equal to the shape value multiplied by the scalar
...
The power of the load is the above computed average energy divided by the load shape interval. Typically, this is 1 hour, so the power in (kW) is equal to the energy/duration in (kWh/h). Do not assume that this is always true. 

![Example of an analog loadshape](../../../images/300px-Analog_loadshape.png){ #fig:example-of-an-analog-loadshape }


Table: Analog load shape parameters { #tbl:analog-load-shape-parameters }
...
![Example of a pulsed loadshape](../../../images/300px-Pulsed_loadshape.png){ #fig:example-of-a-pulsed-loadshape }


Table: Random pulse load shape parameters { #tbl:random-pulse-load-shape-parameters }
Parameter | Unit | Default | Source | Description   
---|---|---|---|---  
**raw_value** | kWh/h | 0.0 | load shape | The value of the load shape for the end use at the given time   
**value** | pu.kWh/h | 0.0 | load shape | The raw load shape value / the total area of the load shape   
...
Pulsed loadshapes emit 1 or more pulses at random times such that the total energy is accumulated over the period of the loadshape. A pulsed loadshape is defined using the following terms: 
...
The loadshape value is the energy of a series of pulses generated during the interval and is equal to the shape value multiplied by the scalar: $count = value \times scalar$. 

The energy specifies the total energy used during a complete load shape. The period is the interval divided by the loadshape value: $period = interval / value / scalar$. 
...
![Example of a amplitude modulated loadshapes](../../../images/300px-Modulated_loadshape.png){ #fig:example-of-a-amplitude-modulated-loadshapes }


![Example of a pulse-width modulated loadshapes](../../../images/300px-Pw_modulated_loadshape.png){ #fig:example-of-a-pulse-width-modulated-loadshapes }


![Example of a frequency modulated loadshapes](../../../images/300px-F_modulated_loadshape.png){ #fig:example-of-a-frequency-modulated-loadshapes }


Table: Random pulse load shape parameters { #tbl:random-pulse-load-shape-parameters-2 }
...
![Example of an amplitude modulated loadshape](../../../images/300px-Modulated_loadshape.png){ #fig:example-of-an-amplitude-modulated-loadshape }


Modulated loadshapes where the power varies as a function of load have fixed duty cycle and period have amplitude modulation. AM is indicated by `modulation = MMT_AMPLITUDE`, and 
...
![Example of a pulse-width modulated loadshapes](../../../images/300px-Pw_modulated_loadshape.png){ #fig:example-of-a-pulse-width-modulated-loadshapes-2 }
...
Table: Random pulse load shape parameters { #tbl:random-pulse-load-shape-parameters-3 }
...
**q_off** | pu | 0 | load shape | The queue accrual value at which the machine turns off 

![Example of a queued loadshape](../../../images/300px-Queued_loadshape.png){ #fig:example-of-a-queued-loadshape }
...
![Simple Schedule](../../../images/300px-Simple_schedule.png){ #fig:simple-schedule }

A scheduled loadshape is defined for a population of objects and the aggregate value (heavy black line) diversity is built in.

A schedule-based loadshape syntax has a number of important advantages. 
...
If the min or the max are omitted, then 3σ is used. If the stdev is omitted, then 0 is used (meaning the value is invariant). 
...
Objects should use load data as a part of **enduse** properties. The **enduse** property encapsulates information about how a load is converted to a power, accumulated to an energy and heat gain. The structure is 
    ...
    typedef struct s_enduse {
           loadshape *shape; /* reference to the loadshape from which load is read
...
!!! caveat
    ...
    To implement defaults, note the following:
      ...
      2. All other object property defaults must be done in **create**.
