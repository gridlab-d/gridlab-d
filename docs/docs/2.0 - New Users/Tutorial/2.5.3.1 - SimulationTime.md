...
In the absence of specific time zone information, GridLAB-D™ uses the **TODO - based on IP address or something? Looks like no - "GLD does not use the operating system's time zone specifications... below** current time zone. Time zones are specified in the *"tzinfo.txt"* file that is installed with GridLAB-D™ under the `share` folder. The following time zones are currently supported for the United States:
