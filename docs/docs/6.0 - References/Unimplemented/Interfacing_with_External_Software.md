...
To enable this interaction, GridLAB-D™ must first be compiled from source with additional options; see [Building GridLAB-D™ from Source](../../4.0%20-%20Developing%20Reference/Building%20from%20Source/4.2.1%20-%20Building_from_Source.md). These options are enabled when running `./configure `:
