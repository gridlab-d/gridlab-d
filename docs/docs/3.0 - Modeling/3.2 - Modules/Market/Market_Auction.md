...
`yyyy` `price_mean_x` | currency  | Represents the average price associated with the `x` interval defined previously (defined as a module global or a through a property scanning list). `yyyy` represents the interval of this statistic, `current` or `past`. For example, if `x` was "1d" and `yyyy` was "current", `current_price_mean_1d` would represent the mean clearing price of the market for the last 24 hour period.   
`yyyy` `price_stdev_x` | currency  | Represents the standard deviation associated with the `x` interval defined previously (defined as a module global or a through a property scanning list). `yyyy` represents the interval of this statistic, `current` or `past`. For example, if `x` was "1d" and `yyyy` was "current", `current_price_stdev_1d` would represent the standard deviation of the clearing price of the market for the last 24 hour period.   
