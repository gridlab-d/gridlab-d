...
Assert objects can be used to monitor the properties of other objects. The assert class is used purely for validation purposes. They are used to assert that a given parameter will have a certain value at a certain time. In most case, this is used in conjunction with the auto-validation process performed on the nightly builds. More information on the validation process can be found at: [Integrated Testing](../Development%20Fundamentals/4.3.6%20-%20Integrated_Testing.md)
