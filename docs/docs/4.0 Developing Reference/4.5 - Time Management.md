...
### Coding Guide
To support delta mode processing, developers first need to decide whether they would like to offer module-level and/or object-level support. GridLAB-D modules that currently support module-level transient mode are generators, powerflow, and residential, while object-level support is offered by assert.
...
The following changes to `init.cpp` are required to enable delta mode operation for a module. 
