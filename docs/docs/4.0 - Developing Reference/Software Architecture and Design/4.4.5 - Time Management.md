...
### Object ranks
An object rank is a relationship structure that affects the order in which objects are accessed during the [synchronization procedure](./4.4.4%20-%20Object%20Synchronization%20Process.md). Objects are assigned ranks when they are created, which can later be altered through parent-child declarations or explicit promotion commands. Normally, the modeler only controls the parent-child relationship by either nesting objects when they are defined, or by using the name property. Modelers cannot strictly control the rank of objects because modules will generally apply their own ranking rules to ensure a stable solution. A good example of the utilization of object ranks is the [powerflow module](../../3.0%20-%20Modeling%20Reference/Modules/Powerflow/1.0%20-%20Power_Flow_User_Guide.md).
...
![Quasi-Static Mode](../../../images/300px-Deltamode.png){ fig: Quasi-Static Mode Synchronization Procedure }
