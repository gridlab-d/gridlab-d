...
GridLAB-D™ is an open-source project for developing a next-generation power system simulation technology. GridLAB-D™ is an electric power distribution system **TODO - Scope - Is it just distribution, or transmission also? [JK - specifically distribution--from the substation down]** simulation and analysis tool that provides valuable information to users who design and operate distribution systems, and to utilities that wish to take advantage of the **TODO - Scope - "latest energy technologies" can mean a lot... generation (solar technology, etc.), or grid modeling? Based on #1 below, seems like DER, distribution, markets? [JK- agree, let's be more specific. DER, power flow, load, and energy market modeling]** latest energy technologies. GridLAB-D™ incorporates advanced load modeling techniques, with high-performance solution algorithms to deliver end-use load modeling, coupled with power system models, market models, distribution automation models, and software integration tools for users of many power system analysis tools. 

GridLAB-D™ was developed by the U.S. Department of Energy (DOE) at Pacific Northwest National Laboratory (PNNL) with funding from the Office of Electricity and has been continued since in collaboration with industry and academia. 
...
1. Unusual situations are managed with greater accuracy.
2. It handles widely-disparate time scales, ranging from sub-seconds to many years.
3. Easy integration with new modules and third-party systems.
