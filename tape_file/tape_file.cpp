...
EXPORT void set_csv_data_only() { csv_data_only = 1; }
EXPORT void set_csv_keep_clean() { csv_keep_clean = 1; }
...
EXPORT int rewind_shaper(struct shaper *my) { return -1; }
...
EXPORT void flush_recorder(struct recorder *my) { fflush(my->fp); }
  ...
  if (!csv_data_only) {
    ...
    if (my->bins[0] != 0) {
      ...
      for (i = 0; i < my->bin_count; ++i) {
        fprintf(my->fp, "%s%f..%f%s", (my->bin_list[i].low_inc ? "[" : "("),
                my->bin_list[i].low_val, my->bin_list[i].high_val,
                (my->bin_list[i].high_inc ? "]" : ")"));
        if (i + 1 < my->bin_count) {
          fprintf(my->fp, ",");
        }
      }
...
EXPORT void flush_collector(struct collector *my) { fflush(my->fp); }
