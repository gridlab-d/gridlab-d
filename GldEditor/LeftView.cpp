...
void CLeftView::OnInitialUpdate() {
  ...
  SetWindowLong(m_hWnd, GWL_STYLE,
                ws | TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS);
}
...
void CLeftView::OnUpdate(CView *pSender, LPARAM lHint, CObject *pHint) {
  ...
  for (MODULE *mod = module_get_first(); mod != nullptr; mod = mod->next) {
    ...
    for (CLASS *oclass = class_get_first_class(); oclass != nullptr;
         oclass = oclass->next) {
      if (oclass->module == mod) {
        ...
        for (OBJECT *obj = object_get_first(); obj != nullptr;
             obj = obj->next) {
...
void CLeftView::Dump(CDumpContext &dc) const { CTreeView::Dump(dc); }
...
void CLeftView::OnTvnSelchanged(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  CGldEditorView *pEditor =
      ((CMainFrame *)(theApp.GetMainWnd()))->GetEditorPane();
  ...
  if (pRef != nullptr) {
    ...
    if (type == "object")
      ...
      if (strchr(type, ':'))
        pEditor->LoadScheduleBlock((SCHEDULE *)(pRef->GetData()),
                                   atoi(type.Mid(9)));
      else
