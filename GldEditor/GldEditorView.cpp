...
void CGldEditorView::OnInitialUpdate() {
  ...
  ListView_SetExtendedListViewStyleEx(
      GetListCtrl().m_hWnd, LVS_EX_FUL.rows() ELECT | LVS_EX_GRIDLINES,
...
#ifdef _DEBUG
void CGldEditorView::AssertValid() const { CListView::AssertValid(); }

void CGldEditorView::Dump(CDumpContext &dc) const { CListView::Dump(dc); }

CGldEditorDoc *
CGldEditorView::GetDocument() const // non-debug version is inline
{
...
void CGldEditorView::OnStyleChanged(int nStyleType,
...
void CGldEditorView::LoadObject(OBJECT *obj) {
  ...
  int nColumns =
      list.GetHeaderCtrl() ? list.GetHeaderCtrl()->GetItemCount() : 0;
...
#define W(X) (nWid += X, X)
  ...
  int Description = list.InsertColumn(nCol++, "Description", LVCFMT_LEFT,
                                      wr.Width() - nWid, nCol);
#undef W
  ...
  list.SetItemText(
      nItem, Data,
      convert_from_timestamp(obj->clock, buffer, sizeof(buffer)) ? buffer : "");
  ...
  list.SetItemText(nItem, Data,
                   convert_from_timestamp(obj->in_svc, buffer, sizeof(buffer))
                       ? buffer
                       : "");
  ...
  list.SetItemText(nItem, Access, "PUBLIC");
  list.SetItemText(nItem, Data,
                   convert_from_timestamp(obj->out_svc, buffer, sizeof(buffer))
                       ? buffer
                       : "");
  ...
  list.SetItemText(nItem, Access, "PUBLIC");
  list.SetItemText(nItem, Data,
                   convert_from_latitude(obj->longitude, buffer, sizeof(buffer))
                       ? buffer
                       : "");
  ...
  for (oclass = obj->oclass; oclass != nullptr; oclass = oclass->parent) {
    ...
    for (prop = class_get_first_property(oclass); prop != nullptr;
      ...
      list.SetItemText(nItem, Access,
                       convert_from_enumeration(buffer, sizeof(buffer),
                                                &(prop->access),
                                                object_access_property())
                           ? buffer
                           : "");
      list.SetItemText(
          nItem, Data,
...
#define W(X) (nWid += X, X)
  ...
  int Access = list.InsertColumn(nCol++, "Access", LVCFMT_LEFT, W(100), nCol);
  int Data =
      list.InsertColumn(nCol++, "Data", LVCFMT_LEFT, wr.Width() - nWid, nCol);
#undef W
  ...
  if (oclass->passconfig & PC_BOTTOMUP)
    strcat(buffer, buffer[0] == '\0' ? "" : "|"), strcat(buffer, "BOTTOMUP");
  if (oclass->passconfig & PC_POSTTOPDOWN)
    strcat(buffer, buffer[0] == '\0' ? "" : "|"), strcat(buffer, "POSTTOPDOWN");
  list.SetItemText(nItem, Data, buffer);
  ...
  for (prop = class_get_first_property(oclass); prop != nullptr;
    ...
    convert_from_enumeration(buffer, sizeof(buffer), &(prop->access),
                             object_access_property());
...
#define W(X) (nWid += X, X)
  ...
  int Data = list.InsertColumn(nCol++, "Value(s)", LVCFMT_LEFT,
                               wr.Width() - nWid, nCol);
#undef W
  ...
  for (oclass = mod->oclass; oclass != nullptr && oclass->module == mod;
       oclass = oclass->next) { // if (oclass!=nullptr && oclass->module==mod)
    nItem = list.InsertItem(list.GetItemCount(),
                            oclass == mod->oclass ? "Classes" : "");
    list.SetItemText(nItem, Data, oclass->name);
  }

  nItem = list.InsertItem(list.GetItemCount(), "Implementations");
  if (mod->cmdargs != nullptr) {
    list.SetItemText(nItem, Data, "cmdargs ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->getvar != nullptr) {
    list.SetItemText(nItem, Data, "getvar ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->setvar != nullptr) {
    list.SetItemText(nItem, Data, "setvar ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->import_file != nullptr) {
    list.SetItemText(nItem, Data, "import_file ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->export_file != nullptr) {
    list.SetItemText(nItem, Data, "export_file ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->check != nullptr) {
    list.SetItemText(nItem, Data, "check ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
  if (mod->kmldump != nullptr) {
    list.SetItemText(nItem, Data, "kmldump ");
    nItem = list.InsertItem(list.GetItemCount(), "");
  }
#ifndef _NO_CPPUNIT
  if (mod->module_test != nullptr) {
    list.SetItemText(nItem, Data, "module_test ");
  }
#endif
  PROPERTY *prop;
  ...
  while ((global = global_getnext(global)) != nullptr) {
    ...
    if (sscanf(global->name, "%[^:]::%s", modname, varname) == 2 &&
...
#undef W
  ...
  while ((var = global_getnext(var)) != nullptr) {
    ...
    list.SetItemText(nItem, Type,
                     class_get_property_typename(var->prop->ptype));

    convert_from_enumeration(buffer, sizeof(buffer), &(var->prop->access),
                             object_access_property());
    list.SetItemText(nItem, Access, buffer);
      ...
      for (i = PASSINIT(pass); PASSCMP(i, pass); i += PASSINC(pass)) {
        ...
        for (item = ranks[pass]->ordinal[i]->first; item != nullptr;
          ...
          list.SetItemText(nItem, Status,
                           convert_from_set(buffer, sizeof(buffer),
                                            &(obj->flags),
                                            object_flag_property())
                               ? buffer
                               : "(na)");
...
#define W(X) (nWid += X, X)
  ...
  int Text =
      list.InsertColumn(nCol++, "Text", LVCFMT_LEFT, wr.Width() - nWid, nCol);
#undef W
  ...
  char buffer[1024];
  int line = 0;
  while (fp != nullptr && !ferror(fp) && !feof(fp) &&
...
#undef W
  ...
  TIMESTAMP tstart =
      global_clock; // TODO: this should be global_starttime but it's already
                    // been set to wall clock by this time
  ...
  DATETIME last = {
      0,  0, 0,
      -1, 0, 0}; // -1 forces first time to be displayed even when it's midnight
  int max = 100;
  double lastvalue =
      -1; // -1 forces first time to be displayed even when it's zero value
  while (t < tstop) // && max-->0)
  {
