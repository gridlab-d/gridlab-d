...
#ifdef _DEBUG
void CGldEditorDoc::AssertValid() const { CDocument::AssertValid(); }

void CGldEditorDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
void CGldEditorDoc::EndGroup(BOOL status) {
  ...
  m_OutputBuffer.SetAt(pos, CString(status ? "=" : "-") +
                                m_OutputBuffer.GetAt(pos).Mid(1));
}
