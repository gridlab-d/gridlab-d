...
static int send_element_headers(struct mg_event *event)
{
	...
	{
		int has_location = !(isnan(gld_obj->get_latitude()) || isnan(gld_obj->get_longitude()) || (gld_obj->get_name() == nullptr) || (gld_obj->get_oclass() == nullptr));
		if (has_location)
		{
			stream << "\t<" << gld_obj->get_oclass()->get_name() << ">\n";
			stream << "\t\t<name>" << gld_obj->get_name() << "</name>\n";
			stream << "\t\t<latitude>" << std::fixed << std::setprecision(6) << gld_obj->get_latitude() << "</latitude>\n";
			stream << "\t\t<longitude>" << std::fixed << std::setprecision(6) << gld_obj->get_longitude() << "</longitude>\n";
			stream << "\t<" << "/" << gld_obj->get_oclass()->get_name() << ">\n";
		}
		else
		{
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_server(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
