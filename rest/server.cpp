...
#endif
{
  ...
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(oclass,
                            ...
                            PT_int16, "port", PADDR(port), PT_DESCRIPTION,
                            "server port", nullptr) < 1)
...
static int send_element_headers(struct mg_event *event) {
  ...
  stream << "<Elements>\n";
  for (gld_obj = gld_object::get_first(); (gld_obj->my()) != nullptr;
       gld_obj = (gld_obj->get_next())) {
    int has_location = !(
        isnan(gld_obj->get_latitude()) || isnan(gld_obj->get_longitude()) ||
    ...
    if (has_location) {
      ...
      if (fromProperty.get_property() && toProperty.get_property() &&
          (gld_obj->get_oclass())) {
...
int server::init(OBJECT *parent) {
  ...
  if (callback->file.find_file("rest/gui_root", nullptr, X_OK, path,
    ...
    const char *options[] = {"listening_ports", portString, "document_root",
                             path, nullptr};
...
EXPORT int init_server(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    gl_error("init_server(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
  }
  return 0;
}

static TIMESTAMP sync_server_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  TIMESTAMP t2 = TS_NEVER;
  ...
  try {
    ...
    gl_error("sync_server(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
  }
  return TS_INVALID;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_server(OBJECT *obj, TIMESTAMP t1,
                                            PASSCONFIG pass) {
  return sync_server_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_server(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
