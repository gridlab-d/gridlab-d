...
public:
  ...
  void test_create() {
    ...
    // TODO: Need to attach a TMY Tape object before this call will actually
  ...
  void test_tmy2() {
    ...
    CPPUNIT_ASSERT(obj != NULL);
    // S,       SE,      SW,E,       W, NE,      NW,N
    double test_data[9] = {33.70209, 36.79533, 29, 35.32216, 29,
                           30.14556, 29,       29, 30.38584};
    ...
    for (COMPASS_PTS c_point = CP_S; c_point < CP_LAST;
         c_point = COMPASS_PTS(c_point + 1)) {
      double data = OBJECTDATA(obj, climate)->solar_flux[c_point];
      double test_diff = data < test_data[c_point] ? test_data[c_point] - data
                                                   : data - test_data[c_point];
