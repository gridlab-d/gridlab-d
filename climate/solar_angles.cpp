...
double SolarAngles::eq_time(short day_of_yr) {
  ...
  return ((0.5501 * cos(rad) - 3.0195 * cos(2 * rad) - 0.0771 * cos(3 * rad) -
           7.3403 * sin(rad) - 9.4583 * sin(2 * rad) - 0.3284 * sin(3 * rad)) /
          60.0);
}
...
double SolarAngles::solar_time(
    ...
    double std_meridian, // Standard meridian (longitude) of local time zone
                         // (radians---e.g., Pacific timezone is 120 degrees
                         // times pi/180)
    double longitude     // Local longitude (radians east)
) {
  return std_time + eq_time(day_of_yr) +
         HR_PER_RADIAN * (longitude - std_meridian);
}
...
double SolarAngles::local_time(
    ...
    short day_of_yr, // Day of year from Jan 1
    double
        std_meridian, // Standard meridian (longitude) of local time zone
                      // (radians---e.g., Pacific timezone is 120 times pi/180)
    double longitude  // Local longitude (radians east)
) {
  // std_meridian and longitude swapped to account for negative west convention
  // (see solar_time above) Unchecked since nothing uses local_time
  return sol_time - eq_time(day_of_yr) -
         HR_PER_RADIAN * (longitude - std_meridian);
}
...
double SolarAngles::cos_incident(
  ...
  double sindecl, cosdecl, sinlat, coslat, sinslope, cosslope, sinaz, cosaz,
      sinhr, coshr; // For efficiency

  double hr_ang =
      -(15.0 * PI_OVER_180) * (sol_time - 12.0); // morning +, afternoon -
...
/*****************function zenith*****************************/
// PURPOSE: Compute the solar zenith angle (radians, the angle between solar
// beam
...
//
double SolarAngles::zenith(short day_of_yr, // day of year from Jan 1
                           double latitude, // latitude (radians)
                           double sol_time  // solar time (decimal hours)
) {
  double hr_ang =
  ...
  return acos(sin(decl) * sin(latitude) +
              cos(decl) * cos(latitude) * cos(hr_ang));
}
...
// SOURCE:  Duffy & Beckman.
//
double SolarAngles::altitude(short day_of_yr, // day of year from Jan 1
                             double latitude, // latitude (radians)
...
//
double
SolarAngles::hr_sunrise(short day_of_yr, // solar declination angle (radians)
                        double latitude  // latitude (radians)
) {
  // Calculate hour angle of sunrise (converted to degrees)
  double hr_ang =
      acos(-tan(latitude) * tan(declination(day_of_yr))) / PI_OVER_180;
...
// SOURCE:  Duffy & Beckman.
//
double SolarAngles::day_len(short day_of_yr, // Day of year from Jan 1
                            double latitude  // latitude (radians)
) {
  return 2.0 * acos(-tan(latitude) * tan(declination(day_of_yr))) /
         (15.0 * PI_OVER_180);
}
...
//
short SolarAngles::day_of_yr(short month, // Month of year (1-12)
...
double SolarAngles::elevation(short day_of_yr, // day of year from Jan 1
                              double latitude, // latitude (radians)
                              double sol_time  // solar time (decimal hours)
) {
  double hr_ang =
  ...
  return asin(sin(decl) * sin(latitude) +
              cos(decl) * cos(latitude) * cos(hr_ang));
}

// TODO: CDC - Fix this function; this function does not compute azimuth
// correctly
double SolarAngles::azimuth(short day_of_yr, // day of year from Jan 1
                            double latitude, // latitude (radians)
                            double sol_time  // solar time (decimal hours)
) {
  ...
  double rs =
      (sin(decl) * cos(latitude) - cos(decl) * sin(latitude) * cos(hr_ang)) /
      cos(alpha);
  if (rs > 1.0) {
    rs = 1.0;
  } else if (rs < -1.0) {
...
void SolarAngles::S_init(SOLPOS_POSDATA *pdat) {
  ...
  pdat->aspect =
      PI; // Azimuth of panel surface (direction it
  ...
  pdat->solcon =
      126.998456;         // Solar constant, 1367 W/sq m - 126.998456; W/sq ft
...
// Extracted from Perez et al., "Modeling Dayling Availability and Irradiance
// Components from Direct and Global Irradiance," Solar Energy, vol. 44, no. 7,
// pp. 271-289, 1990.
void SolarAngles::perez_tilt(SOLPOS_POSDATA *pdat) {
  double zen_rad, zen_rad_cubed, a_val, b_val;
  short index;

  // See if there's even any diffuse radiation to care about - i.e., is it not
  // night
  if (pdat->diff_horz != 0.0) {
    ...
    // Compute sky clearness - if there's no direct on the surface, it's 1.0
    // (save some calculations)
    if (pdat->dir_norm != 0.0)
      pdat->perez_skyclear =
          ((pdat->diff_horz + pdat->dir_norm) / pdat->diff_horz +
           zen_rad_cubed) /
          (1 + zen_rad_cubed);
    else
    ...
    for (index = 0; index < 7; index++) {
      if ((pdat->perez_skyclear >= perez_clearness_limits[index]) &&
          (pdat->perez_skyclear < perez_clearness_limits[index + 1])) {
        pdat->perez_skyclear_idx = index; // Indices are offset by one since
                                          // they'll be used as indices later
    ...
    pdat->perez_F1 = perez_tilt_coeff_F1[0][pdat->perez_skyclear_idx] +
                     perez_tilt_coeff_F1[1][pdat->perez_skyclear_idx] *
                         pdat->perez_brightness +
                     perez_tilt_coeff_F1[2][pdat->perez_skyclear_idx] * zen_rad;
    pdat->perez_F2 = perez_tilt_coeff_F2[0][pdat->perez_skyclear_idx] +
                     perez_tilt_coeff_F2[1][pdat->perez_skyclear_idx] *
                         pdat->perez_brightness +
                     perez_tilt_coeff_F2[2][pdat->perez_skyclear_idx] * zen_rad;

    // Maximum check
    if (pdat->perez_F1 < 0)
      pdat->perez_F1 = 0.0;

    // Now compute the Perez horizontal scalar (to be applied to diffuse) - get
    // a & b values first
    if (pdat->cosinc > 0.0)
      a_val = pdat->cosinc;
    else
    ...
    // Compute the scalar
    pdat->perez_horz =
        ((1 - pdat->perez_F1) * (1 + cos(pdat->tilt)) / 2.0 +
         pdat->perez_F1 * a_val / b_val + pdat->perez_F2 * sin(pdat->tilt));
