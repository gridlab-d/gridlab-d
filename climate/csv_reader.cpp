...
csv_reader::csv_reader(MODULE *module) {
  ...
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            oclass, PT_int32, "index", PADDR(index), PT_ACCESS, PA_REFERENCE,
            PT_char32, "city_name", PADDR(city_name), PT_char32, "state_name",
            PADDR(state_name), PT_double, "lat_deg", PADDR(lat_deg), PT_double,
            "lat_min", PADDR(lat_min), PT_double, "long_deg", PADDR(long_deg),
            PT_double, "long_min", PADDR(long_min), PT_double, "low_temp",
            PADDR(low_temp), PT_ACCESS, PA_REFERENCE, PT_double, "high_temp",
            PADDR(high_temp), PT_ACCESS, PA_REFERENCE, PT_double, "peak_solar",
            PADDR(peak_solar), PT_ACCESS, PA_REFERENCE, PT_int32, "elevation",
            PADDR(elevation), PT_enumeration, "status", PADDR(status),
            PT_ACCESS, PA_REFERENCE, PT_KEYWORD, "INIT", (enumeration)CR_INIT,
            PT_KEYWORD, "OPEN", (enumeration)CR_OPEN, PT_KEYWORD, "ERROR",
            (enumeration)CR_ERROR, PT_char32, "timefmt", PADDR(timefmt),
            PT_char32, "timezone", PADDR(timezone), PT_double,
            "timezone_offset", PADDR(tz_numval), PT_char256, "columns",
            PADDR(columns_str), PT_char256, "filename", PADDR(filename),
            nullptr) < 1)
...
int csv_reader::open(const char *file) {
  ...
  if (file == 0) {
    ...
    /* TROUBLESHOOT
            No input file was specified for the csv_reader object.  Double-check
       the input model and re-run GridLAB-D.
    */
    return 0;
  }
  ...
  if (infile == 0) {
    ...
    /* TROUBLESHOOT
            The specified input file could not be opened for reading.  Verify
       that no other applications are using that file, double-check the input
       model, and re-run GridLAB-D.
    */
    return 0;
  }
  ...
  if (columns_str[0] != 0) {
    if (0 == read_header(columns_str)) {
      gl_error("csv_reader::open ~ column header read failure from explicit "
               "headers");
      return 0;
    } else {
      has_cols = 1;
  ...
  while (fgets(line, 1024, infile) != nullptr) {
    ...
    if (line[0] == '#') { // comment
      ...
      if (0 == read_prop(line + 1)) {
        gl_error("csv_reader::open ~ property read failure on line %i",
                 linenum);
        return 0;
      } else {
        continue;
      ...
      if (0 == read_header(line)) {
        gl_error("csv_reader::open ~ column header read failure on line %i",
                 linenum);
        return 0;
      } else {
        has_cols = 1;
      ...
      if (0 == line_rv) {
        gl_error("csv_reader::open ~ data line read failure on line %i",
                 linenum);
        return 0;
      } else if (1 == line_rv) { // good read
        ++sample_ct;
      } else if (2 == line_rv) { // read went 'backwards' or was blank, line
                                 // discarded.
        ;
      }
    }
  ...
  for (i = 0, wtr = weather_root.get(); i < sample_ct && wtr != nullptr;
       ++i, wtr = wtr->next.get()) {
    if (!wtr) {
...
int csv_reader::read_prop(char *line) { // already pulled the '$' off the front
  ...
  if (split == nullptr) {
    ...
    /* TROUBLESHOOT
            Property lines must have the property name and property value
       seperated by an equals sign.  Please correct the CSV file and re-run
       GridLAB-D.
    */
    return 0;
  }
  ...
  if (2 != sscanf(line, "%[^=]=%[^\n#]", propstr, valstr)) {
    ...
    /* TROUBLESHOOT
            The line was not read properly by the parser.  Property lines must
       be of the format "prop=val".  Please review the CSV file and re-run
       GridLAB-D.
    */
    return 0;
  }
  ...
  if (prop == 0) {
    gl_error("csv_reader::read_prop ~ unrecognized csv_reader property \'%s\'",
             propstr);
    /* TROUBLESHOOT
            The property specified within the CSV file is not published by
       csv_reader. Please review the list of published variables, correct the
       CSV file, and re-run GridLAB-D.
    */
    return 0;
  }

  // Windows pointers on my Dell Precision 390 are apparently ulli -MH
  /* BAD BAD BAD!  cannot use 'my' with this, it may not have an associate
   * object header!!! */
  //	if(0 == gl_set_value(my, (void *)((unsigned long long int)this +
  //(unsigned long long int)prop->addr), valstr, prop)){
  //		gl_error("csv_reader::read_prop ~ unable to set property \'%s\'
  //to \'%s\'", propstr, valstr); 		return 0;
  //	}
  void *addr = (void *)((uint64)this + (uint64)prop->addr);
  if (prop->ptype == PT_double) {
    if (1 != sscanf(valstr, "%lg", static_cast<double *>(addr))) {
      gl_error(R"(csv_reader::read_prop ~ unable to set property '%s' to '%s')",
               propstr, valstr);
      /* TROUBLESHOOT
              The double parser was not able to convert the property value into
         a number.  Please review the input line for non-numeric characters and
         re-run GridLAB-D.
      */
      return 0;
    }
    ...
    if (1 != sscanf(valstr, "%ld", static_cast<long *>(addr))) {
      gl_error(R"(csv_reader::read_prop ~ unable to set property '%s' to '%s')",
               propstr, valstr);
      /* TROUBLESHOOT
              The int parser was not able to convert the property value into a
         number.  Please review the input line for non-numeric characters and
         re-run GridLAB-D.
      */
      return 0;
    }
  } else {
    gl_error("csv_reader::read_prop ~ unable to convert property \'%s\' due to "
             "type restrictions",
             propstr);
    /* TROUBLESHOOT
            This is a programming problem.  The property parser within the
       csv_reader is only able to properly handle char32, int32 and double
       properties.  Please contact matthew.hauer@pnl.gov for technical support.
     */
    return 0;
  }
  return 1;
...
int csv_reader::read_header(char *line) {
  struct cmnlist {
    ...
    PROPERTY *column;
    // struct cmnlist *next;
    std::unique_ptr<cmnlist>
        next; // Use unique_ptr for automatic memory management
  };
  char buffer[1024];
  ...
  std::unique_ptr<cmnlist>
      temp; // Use unique_ptr for automatic memory management
  ...
  while (index < 1024 && 0 == done) {
    ...
    temp = std::make_unique<cmnlist>(); // Use unique_ptr for automatic memory
                                        // management
  ...
  while (temp != 0 && i < column_ct) {
    ...
    if (temp->column == 0) {
      gl_error(
          "csv_reader::read_header ~ unable to find column property \'%s\''",
          temp->name);
      /* TROUBLESHOOT
              The specified property in the header was not found published by
         the weather class.  Please check the column header input and re-run
         GridLAB-D.
      */
      return 0;
    }
...
int csv_reader::read_line(char *line, int linenum) {
  ...
  try {
    ...
    int parsed_year = 0, parsed_month = 0, parsed_day = 0, parsed_hour = 0,
        parsed_minute = 0, parsed_second = 0;
    char timezone[32] = {0};

    // Try parsing with different format specifiers
    if (sscanf(timestamp_str.c_str(),
               "%d-%d-%d %d:%d:%d %31[^ \t\n\r]", // weather_dst.csv
               &parsed_year, &parsed_month, &parsed_day, &parsed_hour,
               &parsed_minute, &parsed_second, timezone) >= 6) {
    ...
    } else if (sscanf(timestamp_str.c_str(), "%d-%d-%d %d:%d:%d", &parsed_year,
                      &parsed_month, &parsed_day, &parsed_hour, &parsed_minute,
                      &parsed_second) >= 6) {
      ...
      sample->minute = 0;
      sample->second = 0;
    } else if (sscanf(timestamp_str.c_str(),
                      "%d:%d:%d:%d:%d", // weather_elev.csv
                      &parsed_month, &parsed_day, &parsed_hour, &parsed_minute,
                      &parsed_second) >= 5) {
      // Successfully parsed full timestamp without timezone
      // sample->month = parsed_month;
      ...
      if (ts != TS_INVALID && ts != TS_NEVER &&
          callback->time.local_datetime(ts, &dt)) {
        ...
        gl_error("csv_reader::read_line ~ unable to read time string '%s'",
                 timestamp_str.c_str());
    ...
    for (size_t col = 1; col < tokens.size() && col <= column_ct; ++col) {
      if (columns[col - 1]->ptype == PT_double) {
        ...
        if (name == "temperature")
          ...
          // delete sample;  // Clean up on error.  Important to handle this
          // correctly.
          return 0;
        }
      }
...
TIMESTAMP csv_reader::get_data(TIMESTAMP t0, double *temp, double *humid,
                               double *direct, double *diffuse, double *global,
                               double *extra_global, double *wind,
                               double *winddir, double *opaque, double *total,
                               double *rain, double *snow, double *pressure) {
  DATETIME now, then;
  //	TIMESTAMP until;
  int next_year = 0;
  int i = 0;
...
//			strcpy(guess_dt.tz, "GMT");
    ...
    if (index > -1 && index < sample_ct) {
      ...
      *pressure = samples[index]->pressure;
      ...
      if (!samples[sample_ct - 1]) {
        gl_error("csv_reader::get_data ~ sample at index %d is null",
                 sample_ct - 1);
  ...
  if (sample_ct == 1) { /* only one sample ~ ignore it and keep feeding the same
                           data back, but in a year */
    next_ts += 365 * 24 * 3600;
    return -next_ts;
  }
  ...
  do {
    ...
    next_ts = (TIMESTAMP)gl_mktime(&then);
           ...
           index != start); // skip samples that try to reverse the time
  ...
  *pressure = samples[index]->pressure;
  ...
  if (index == start) {
    ...
    /*	TROUBLESHOOT
            An unidentified error occured while reading data and constructing
       the weather data schedule.  Please post a ticket detailing this event on
       the GridLAB-D SourceForge page.
    */
  }
