...
static short days_thru_month[] = {
    0,   31,  59,  90,  120, 151,
    181, 212, 243, 273, 304, 334}; // Ignores leap years...
...
static double perez_tilt_coeff_F1[3][8] = {
...
static double perez_tilt_coeff_F2[3][8] = {
...
// Boundaries for Perez model "discrete sky clearness" categories - from Perez
// et al., 1990
static double perez_clearness_limits[8] = {1.0,   1.065, 1.230, 1.500,
                                           1.950, 2.800, 4.500, 6.200};
...
public:
  ...
  double solar_time(double std_time, short day_of_yr, double std_meridian,
                    double longitude); // decimal hours
  double local_time(double std_time, short day_of_yr, double std_meridian,
                    double longitude); // decimal hours
  double declination(short day_of_yr); // radians
  double cos_incident(double latitude, double slope, double az, double sol_time,
                      short day_of_yr); // unitless
  double incident(double latitude, double slope, double az, double sol_time,
                  short day_of_yr);                                   // radians
  ...
  // Perez model function at the end (perez_tilt) extracted from referenced
  // paper
  struct SOLPOS_TRIGDATA // used to pass calculated values locally
  {
  ...
  struct SOLPOS_POSDATA {
    ...
    int interval; /* I:              Interval of a measurement period in
                                                                         seconds.
                     Forces solpos to use the time and date from the interval
                                                                         midpoint.
                     The INPUT time (hour, minute, and second) is assumed to be
    ...
    int month;    /* I/O: S_DOY      Month number (Jan = 1, Feb = 2, etc.)
                                                                         solpos
                     will CALCULATE this by default,    or will optionally require it
                     as input    depending on the setting of the S_DOY    function
                     switch. */
    ...
    double aspect;  /* I:             Azimuth of panel surface (direction it
                                                                         faces)
                       N=0, E=90, S=180, W=270,  DEFAULT = 180 */
    ...
    double coszen;  /* O:  S_REFRAC   Cosine of refraction corrected solar
                       ...
                       angle */
    double
        dayang;     /* T:  S_GEOM     Day angle (daynum*360/year-length)
                                                                         degrees */
    double declin;  /* T:  S_GEOM     Declination--zenith angle of solar noon
                                                                         at
                       equator, degrees NORTH */
    ...
    double julday;  /* T:  S_GEOM     Julian Day of 1 JAN 2000 minus
                                                                         2,400,000
                       days (in order to regain  single precision) */
    double
        latitude; /* I:             Latitude, degrees north (south negative) */
    double
        longitude; /* I:             Longitude, degrees east (west negative) */
    ...
    double diff_horz; // Diffuse horizontal radiation for Perez tilt model
                      // calculations
    double dir_norm; // Direct normal radiation for Perez tilt model
                     // calculations
