...
enum class CI { CI_NONE = 0, CI_LINEAR, CI_QUADRATIC }; // CI;

#ifdef CM_NONE
#undef CM_NONE
#endif

enum class CLOUDMODEL { CM_NONE = 0, CM_CUMULUS = 1 }; // CLOUDMODEL;
...
EXPORT int64 calc_solar_solpos_shading_deg(OBJECT *obj, double tilt,
                                           double orientation,
                                           double shading_value, double *value);
...
public:
   ...
   */
  int header_info(char *city, char *state, int *degrees, int *minutes,
                  int *long_deg, int *long_min);
   ...
   * @param extra_terr_dni - Extra terrestrial direct normal irradiance (top of
   * atmosphere)
   */
  int read_data(double *dnr, double *dhr, double *ghr, double *tdb, double *rh,
                int *month, int *day, int *hour, double *wind = 0,
                double *winddir = 0, double *precip = 0, double *snowDepth = 0,
                double *pressure = 0, double *extra_terr_dni = 0,
                double *extra_terr_ghi = 0, double *tot_sky_cov = 0,
                double *opq_sky_cov = 0);

  /** obtain records **/

  double calc_solar(COMPASS_PTS cpt, short doy, double lat, double sol_time,
                    double dnr, double dhr, double ghr, double gnd_ref,
                    double vert_angle);
};
...
class climate : public gld_object {
  ...
  // get_/set_ accessors for classes in this module only (non-atomic data need
  // locks on access) GL_STRING(char32,city); ///< the city
  ...
  // GL_ATOMIC(double,solar_cloud_global); //< READ ONLY: global solar flux
  // after modification by the cloud model(W/sf)
  // GL_ATOMIC(double,solar_cloud_direct); ///< READ ONLY; direct solar flux
  // after modification by the cloud modelW/sf)
  // GL_ATOMIC(double,solar_cloud_diffuse); ///< READ ONLY: diffuse solar flux
  // after modification by the cloud model(W/sf) GL_ATOMIC(double,cloud_alpha);
  // //Determines the distance between the shading layers of the normalized
  // patterns.
  //										//Smaller
  //values lead to a larger distance between shading layers and greater
  //variation within a cloud, closer to continuous.
  //										//Larger
  //values lead to a smaller distance between shading layers and less variation
  //within a cloud, closer to binary.
  //										//Minimum
  //value is num_cloud_layers.
  // GL_ATOMIC(double,cloud_num_layers); //Higher number of layers makes for the
  // possibility of wispier clouds.
  // GL_ATOMIC(double,cloud_aerosol_transmissivity); //Attenuation factor of
  // no-cloud (clear-sky) radiation due to aerosols
  // GL_ATOMIC(double,ground_reflectivity); // flux reflectivity of ground
  // (W/sf) GL_STRUCT(CLIMATERECORD,record); ///< record values
  // (low,low_day,high,high_day,solar) GL_ATOMIC(double,rainfall); ///< rainfall
  // rate (in/h) GL_ATOMIC(double,snowdepth); ///< snow accumulation (in)
  ...
  // GL_ARRAY(double, solar_flux, CP_LAST); ///< Solar flux array (W/sf)
  // Elements are in order: [S, SE, SW, E, W, NE, NW, N, H]
...
protected:
  ...
  double
      solar_cloud_global; ///< global solar flux after cloud modification (W/sf)
  double
      solar_cloud_direct; ///< direct solar flux after cloud modification (W/sf)
  double solar_cloud_diffuse; ///< diffuse solar flux after cloud modification
                              ///< (W/sf)
  double cloud_alpha;         ///< cloud layer distance factor
  double cloud_num_layers;    ///< number of cloud layers
  double cloud_aerosol_transmissivity; ///< clear-sky radiation attenuation
                                       ///< factor due to aerosols
  ...
  enumeration interpolate = (enumeration)CI::CI_NONE; ///< interpolation method
...
public:
  ...
  // *>(&current_defaults->solar_cloud_global) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->solar_cloud_direct) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->solar_cloud_diffuse) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->cloud_alpha) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->cloud_num_layers) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->cloud_aerosol_transmissivity) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	return
  // get_cloud_aerosol_transmissivity_property().get_string().get_buffer();
  // }
...
public:
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->city) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	return
  // std::string(get_temperature_property().get_string().get_buffer());
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->humidity) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
...
public:
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->temperature_raw) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->solar_raw) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->wind_gust) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->interpolate) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->solar_elevation) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->solar_azimuth)
  // - reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->global_horizontal_extra) - reinterpret_cast<const
  // char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->tot_sky_cov) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->cloud_opacity)
  // - reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->cloud_reflectivity) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char
  // *>(&current_defaults->cloud_speed_factor) - reinterpret_cast<const char
  // *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->update_time) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
...
public:
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->record) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->rainfall) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->snowdepth) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->forecast_spec)
  // - reinterpret_cast<const char *>(current_defaults);
  // }
  ...
  static inline size_t get_tmyfile_offset(void) {
    climate *current_defaults = get_defaults();
    return reinterpret_cast<const char *>(&current_defaults->tmyfile) -
           reinterpret_cast<const char *>(current_defaults);
  }
  ...
  // {
  // 	climate *current_defaults = get_defaults();
  // 	return reinterpret_cast<const char *>(&current_defaults->reader) -
  // reinterpret_cast<const char *>(current_defaults);
  // }
   ...
   */
protected:
  double solar_flux[CP_LAST]; ///< Solar flux array (W/sf). Elements are in
                              ///< order: [S, SE, SW, E, W, NE, NW, N, H].
...
public:
  ...
  // {
  // 	return reinterpret_cast<const char *>(&defaults->solar_flux) -
  // reinterpret_cast<const char *>(defaults);
  // }
...
public:
  ...
  int get_solar_for_location(double latitude, double longitude, double *direct,
                             double *global, double *diffuse);
...
private:
  ...
  int get_fuzzy_cloud_value_for_location(double latitude, double longitude,
                                         double *cloud);
  int get_binary_cloud_value_for_location(double latitude, double longitude,
                                          int *cloud);
  double convert_to_binary_cloud();
  void convert_to_fuzzy_cloud(double cut_elevation, int num_fuzzy_layers,
                              double alpha);
