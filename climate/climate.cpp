...
//  These have been set under the assumption of a max wind speed (after altitude
//  adjustment to 1000m) of 80 m/s.  At 256 pixles/tile * 20m/pixle  the
//  off-screen pattern size is sufficient to ensure that  no undefined areas of
//  the pattern make it on-screen in the 60 s update period. Adjusting either of
//  these breaks that assumption and may force other change to be made, such as
//  reducing the time between cloud pattern updates.

// EDITME
const int CLOUD_TILE_SIZE = 512;
const int PIXEL_EDGE_SIZE =
    20; // Pixel size example: 10m per edge, 100 m^2 area.
...
EXPORT int64 calculate_solar_radiation_degrees(OBJECT *obj, double tilt,
                                               double orientation,
                                               double *value) {
  return calculate_solar_radiation_shading_radians(
      obj, RAD(tilt), RAD(orientation), 1.0, value);
}

EXPORT int64 calculate_solar_radiation_radians(OBJECT *obj, double tilt,
                                               double orientation,
                                               double *value) {
  return calculate_solar_radiation_shading_radians(obj, tilt, orientation, 1.0,
                                                   value);
}

EXPORT int64 calculate_solar_radiation_shading_degrees(OBJECT *obj, double tilt,
                                                       double orientation,
                                                       double shading_value,
                                                       double *value) {
  return calculate_solar_radiation_shading_radians(
      obj, RAD(tilt), RAD(orientation), shading_value, value);
}

EXPORT int64 calculate_solar_radiation_shading_radians(OBJECT *obj, double tilt,
                                                       double orientation,
                                                       double shading_value,
                                                       double *value) {
  return calculate_solar_radiation_shading_position_radians(
      obj, tilt, orientation, NaN, NaN, shading_value, value);
}

EXPORT int64 calculate_solar_radiation_shading_position_radians(
    OBJECT *obj, double tilt, double orientation, double latitude,
    double longitude, double shading_value, double *value) {
  ...
  std_time =
      (double)(dt.hour) + ((double)dt.minute) / 60.0 + (dt.is_dst ? -1.0 : 0.0);
  doy = sa.day_of_yr(dt.month, dt.day);
  solar_time =
      sa.solar_time(std_time, doy, RAD(cli->get_tz_meridian()), RAD(longitude));
  cos_incident =
      sa.cos_incident(RAD(latitude), tilt, orientation, solar_time, doy);
  *value = (shading_value * dnr * cos_incident) + dhr * (1 + cos(tilt)) / 2. +
           ghr * (1 - cos(tilt)) * cli->get_ground_reflectivity() / 2.;

  return 1;
}

// Degree version of the new solpos-based solar position/radiation algorithms
EXPORT int64 calc_solar_solpos_shading_deg(OBJECT *obj, double tilt,
                                           double orientation,
                                           double shading_value,
                                           double *value) {
  return calc_solar_solpos_shading_rad(obj, RAD(tilt), RAD(orientation),
                                       shading_value, value);
}

// Solar radiation calcuation based on solpos and Perez tilt models
EXPORT int64 calc_solar_solpos_shading_rad(OBJECT *obj, double tilt,
                                           double orientation,
                                           double shading_value,
                                           double *value) {
  return calc_solar_solpos_shading_position_rad(obj, tilt, orientation, NaN,
                                                NaN, shading_value, value);
}

// Solar radiation calcuation based on solpos and Perez tilt models
EXPORT int64 calc_solar_solpos_shading_position_rad(
    OBJECT *obj, double tilt, double orientation, double latitude,
    double longitude, double shading_value, double *value) {
  static SolarAngles sa; // just for the functions
  double ghr, dhr, dnr;
  ...
  {
    // Adjust time by half an hour - adjusts per TMY "reading" intervals - what
  ...
  // Convert temperature back to centrigrade - since we seem to like imperial
  // units
  ...
  // Solar constant associated with extraterrestrial DNI, 1367 W/sq m - pull
  // from TMY for now sa.solpos_vals.solcon = 126.998456;	//Use constant
  // value for direct normal extraterrestrial irradiance - doesn't seem right to
  // me
  sa.solpos_vals.solcon =
      cli->get_direct_normal_extra(); // Use weather-read version (TMY)
  ...
  *value = (shading_value * dnr * cos_incident) +
           dhr * sa.solpos_vals.perez_horz +
           ghr * ((1 - cos(tilt)) * cli->get_ground_reflectivity() / 2.0);

  return 1;
}

EXPORT int64 calc_solar_ideal_shading_position_radians(OBJECT *obj, double tilt,
                                                       double latitude,
                                                       double longitude,
                                                       double shading_value,
                                                       double *value) {
  ...
  cli->get_solar_for_location(latitude, longitude, &dnr, &ghr, &dhr);
  // strictly speaking, the diffuse horizontal value should be modified to
  // account for the fraction of the sky dome seen by the panel, but this is
  // copied from solar.c
  *value = shading_value * (dnr) + dhr +
           ghr * (1 - cos(tilt)) * (cli->get_ground_reflectivity()) / 2.0;
  return 1;
}
...
/**
        ...
        Opens a TMY2 file for reading and reads in the header data.  Some of the
 data in the header is used in solar radiation calculations, so this data needs
 to be held in memory for future use.  This also sets the file pointer used for
        later reads.
...
int tmy2_reader::open(const char *file) {
  ...
  if (fgets(buf, 500, fp)) {
    ...
    if (s == "tmy2") {
      sscan_rv = sscanf(buf, "%*s %75s %3s %d %c %d %d %c %d %d %d", data_city,
                        data_state, &tz_offset, temp_lat_hem, &lat_degrees,
                        &lat_minutes, temp_long_hem, &long_degrees,
                        &long_minutes, &elevation);
      gl_warning(
          "Daylight saving time (DST) is not handled correctly when using TMY2 "
          "datasets; please use TMY3 for DST-corrected weather data.");
      is_TMY2 = 1;
    } else if (s == "tmy3") {
      sscan_rv =
          sscanf(buf, "%*[^','],%[^','],%[^','],%[^','],%[^','],%[^','],%s",
                 data_city, data_state, tz, tlad, tlod, el);
    ...
    if ((strcmp(temp_long_hem, "W") == 0) ||
        (strcmp(temp_long_hem, "w") == 0)) {
...
*/
int tmy2_reader::header_info(char *city, char *state, int *degrees,
                             int *minutes, int *long_deg, int *long_min) {
  if (city)
...
int tmy2_reader::read_data(double *dnr, double *dhr, double *ghr, double *tdb,
                           double *rh, int *month, int *day, int *hour,
                           double *wind, double *winddir, double *precip,
                           double *snowDepth, double *pressure,
                           double *extra_terr_dni, double *extra_terr_ghi,
                           double *tot_sky_cov, double *opq_sky_cov) {
  int rct = 0;
  int rct_ymd = 0;
  int rct_hm = 0;
  int tmp_dnr, tmp_dhr, tmp_tot_sky_cov, tmp_opq_sky_cov, tmp_tdb, tmp_rh,
      tmp_wd, tmp_ws, tmp_precip, tmp_sf, tmp_ghr, tmp_extra_ghr, tmp_extra_dni,
      tmp_press;
  // sscanf(buf,
  // "%*2s%2d%2d%2d%*14s%4d%*2s%4d%*40s%4d%8*s%3d%*s",month,day,hour,&tmp_dnr,&tmp_dhr,&tmp_tdb,&tmp_rh);
  int tmh, tday, thr, t_mon, t_d, t_hr;
  char t_ymd[11], t_hm[10], t_ehr[10], t_dni[10], t_ghr[10], t_dnr[10],
      t_dhr[10], t_tkc[10], t_osc[10], t_tdb[10], t_rh[10], t_press[10],
      t_wd[10], t_ws[10], t_precip[10], t_sf[10], t_month[2], t_day[2],
      t_year[5], t_hour[2], t_min[2];
  if (month == nullptr)
    month = &tmh;
  if (day == nullptr)
    day = &tday;
  ...
  if (buf[2] == '/') {
    // rct = sscanf(buf,
    // "%d/%d/%*s,%d:%*s,%*s,%d,%d,%*s,%*s,%d,%*s,%*s,%d,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%d,%*s,%*s,%*s,%*s,%*s,%d,%*s,%*s,%d,%*s,%*s,%*s,%*s,%*s,%d,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%d,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%*s,%d",month,day,hour,&tmp_extra_dni,&tmp_ghr,&tmp_dnr,&tmp_dhr,&tmp_tdb,&tmp_rh,&tmp_press,&tmp_ws,&tmp_precip,&tmp_sf);
    rct = sscanf(
        buf,
        "%[^','],%[^','],%[^','],%[^','],%[^','],%*[^','],%*[^','],%[^','],%*[^"
        "','],%*[^','],%[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*[^','],%*["
        ...
        t_rh, t_press, t_wd, t_ws, t_precip, t_sf);
    ...
    tmp_tdb = (int)(atof(t_tdb) *
                    10); // Since for TMY3, measurement is 1/10. So we are
                         // multiplying this by 10 to avoid code changes later
                         // on. tmp_tdb is now scaled same as TMY2 value
    tmp_rh = atoi(t_rh);
    tmp_press = atoi(t_press);
    tmp_wd = atoi(t_wd);
    tmp_ws = (int)(atof(t_ws) * 10);
    tmp_precip = (int)(atof(t_precip) *
                       10); // converting from centimeters to millimeters
    tmp_sf = atoi(t_sf);
    rct = rct + 1;
    tmp_sf = 0; // tmy3 doesnt have this field (snow depth). so set to 0
  } else {
    // rct = sscanf(buf,
    // "%*2s%2d%2d%2d%*4s%4d%4d%*2s%4d%*2s%4d%*34s%4d%*8s%3d%*2s%4d%*7s%3d%*25s%3d%*7s%3d",month,day,hour,&tmp_extra_dni,&tmp_ghr,&tmp_dnr,&tmp_dhr,&tmp_tdb,&tmp_rh,&tmp_press,&tmp_ws,&tmp_precip,&tmp_sf);
    rct =
        sscanf(buf,
               "%*2s%2d%2d%2d%4d%4d%4d%*2s%4d%*2s%4d%*26s%2d%*2s%2d%*2s%4d%*8s%"
               "3d%*2s%4d%*2s%3d%*2s%3d%*25s%3d%*7s%3d",
               month, day, hour, &tmp_extra_ghr, &tmp_extra_dni, &tmp_ghr,
               &tmp_dnr, &tmp_dhr, &tmp_tot_sky_cov, &tmp_opq_sky_cov, &tmp_tdb,
               &tmp_rh, &tmp_press, &tmp_wd, &tmp_ws, &tmp_precip, &tmp_sf);
    /* 3__5__7__9__13__17_20_23_27__29_33___67_71_79__82__85__95_98_ */
  }
  if (rct != 17) {
    gl_warning("TMY reader did not get 17 values for line time %d/%d %d00",
               *month, *day, *hour);
  }
  if (dnr)
    *dnr = tmp_dnr;
  ...
  // Assign extra_dni and pressure - extra DNI converted outside with other
  // solar values
  if (extra_terr_dni)
...
/**
        Calculate the solar radation for a surface facing the given compass
   point.
        ...
        @param vert_angle the angle of the surface relative to the horizon
   (Default is 90 degrees)
*/
double tmy2_reader::calc_solar(COMPASS_PTS cpt, short doy, double lat,
                               double sol_time, double dnr, double dhr,
                               double ghr, double gnd_ref,
                               double vert_angle = 90) {
  SolarAngles *sa = new SolarAngles();
  double surface_angle = surface_angles[cpt];
  double cos_incident =
      sa->cos_incident(lat, RAD(vert_angle), RAD(surface_angle), sol_time, doy);
  delete sa;
...
*/
void tmy2_reader::close() { fclose(fp); }
...
climate::climate(MODULE *module) {
  ...
  if (oclass == nullptr) {
    oclass = gld_class::create(module, "climate", sizeof(climate),
                               PC_PRETOPDOWN | PC_AUTOLOCK);
    if (gl_publish_variable(
            oclass, PT_double, "solar_elevation",
            PADDR(solar_elevation), // sjin: publish solar elevation variable
            PT_double, "solar_azimuth",
            PADDR(solar_azimuth), // sjin: publish solar azimuth variable
            PT_double, "solar_zenith", PADDR(solar_zenith), PT_char32, "city",
            PADDR(city), PT_char1024, "tmyfile", PADDR(tmyfile), PT_double,
            "tz_meridian", PADDR(tz_meridian), PT_DESCRIPTION,
            "Specify 15*(time zone offset of weather source) if using CSV, "
            "manual, FNCS or HELICS control. Negative value in the U.S.",
            PT_double, "temperature[degF]", PADDR(temperature), PT_double,
            "humidity[pu]", PADDR(humidity), PT_double, "solar_flux[W/sf]",
            PADDR(solar_flux), PT_SIZE, 9, PT_double, "solar_direct[W/sf]",
            PADDR(solar_direct), PT_double, "solar_diffuse[W/sf]",
            PADDR(solar_diffuse), PT_double, "solar_global[W/sf]",
            PADDR(solar_global), PT_double,
            "extraterrestrial_global_horizontal[W/sf]",
            PADDR(global_horizontal_extra), PT_double,
            "extraterrestrial_direct_normal[W/sf]", PADDR(direct_normal_extra),
            PT_double, "pressure[mbar]", PADDR(pressure), PT_double,
            "wind_speed[mph]", PADDR(wind_speed), PT_double, "wind_dir[deg]",
            PADDR(wind_dir), PT_double, "wind_gust[mph]", PADDR(wind_gust),
            PT_double, "record.low[degF]", PADDR(record.low), PT_int32,
            "record.low_day", PADDR(record.low_day), PT_double,
            "record.high[degF]", PADDR(record.high), PT_int32,
            "record.high_day", PADDR(record.high_day), PT_double,
            "record.solar[W/sf]", PADDR(record.solar), PT_double,
            "rainfall[in/h]", PADDR(rainfall), PT_double, "snowdepth[in]",
            PADDR(snowdepth), PT_enumeration, "interpolate", PADDR(interpolate),
            PT_DESCRIPTION, "the interpolation mode used on the climate data",
            PT_KEYWORD, "NONE", (enumeration)CI::CI_NONE, PT_KEYWORD, "LINEAR",
            (enumeration)CI::CI_LINEAR, PT_KEYWORD, "QUADRATIC",
            (enumeration)CI::CI_QUADRATIC, PT_double, "solar_horiz",
            PADDR(solar_flux[CP_H]), PT_double, "solar_north",
            PADDR(solar_flux[CP_N]), PT_double, "solar_northeast",
            PADDR(solar_flux[CP_NE]), PT_double, "solar_east",
            PADDR(solar_flux[CP_E]), PT_double, "solar_southeast",
            PADDR(solar_flux[CP_SE]), PT_double, "solar_south",
            PADDR(solar_flux[CP_S]), PT_double, "solar_southwest",
            PADDR(solar_flux[CP_SW]), PT_double, "solar_west",
            PADDR(solar_flux[CP_W]), PT_double, "solar_northwest",
            PADDR(solar_flux[CP_NW]), PT_double, "solar_raw[W/sf]",
            PADDR(solar_raw), PT_double, "ground_reflectivity[pu]",
            PADDR(ground_reflectivity), PT_object, "reader", PADDR(reader),
            PT_char1024, "forecast", PADDR(forecast_spec), PT_DESCRIPTION,
            "forecasting specifications", PT_enumeration, "cloud_model",
            PADDR(cloud_model), PT_DESCRIPTION, "the cloud model to use",
            PT_KEYWORD, "NONE", (enumeration)CLOUDMODEL::CM_NONE, PT_KEYWORD,
            "CUMULUS", (enumeration)CLOUDMODEL::CM_CUMULUS, PT_double,
            "cloud_opacity[pu]", PADDR(cloud_opacity), PT_double,
            "opq_sky_cov[pu]", PADDR(opq_sky_cov),
            // PT_double,"cloud_reflectivity[pu]",PADDR(cloud_reflectivity),
            // //Unused in the cloud model at this time.
            ...
            PT_double, "cloud_alpha[pu]", PADDR(cloud_alpha), PT_double,
            "cloud_num_layers[pu]", PADDR(cloud_num_layers), PT_double,
            "cloud_aerosol_transmissivity[pu]",
            PADDR(cloud_aerosol_transmissivity), PT_double, "update_time",
            PADDR(update_time), nullptr) < 1)
    ...
    gl_publish_function(oclass, "calculate_solar_radiation_degrees",
                        (FUNCTIONADDR)calculate_solar_radiation_degrees);
    gl_publish_function(oclass, "calculate_solar_radiation_radians",
                        (FUNCTIONADDR)calculate_solar_radiation_radians);
    gl_publish_function(
        oclass, "calculate_solar_radiation_shading_degrees",
        (FUNCTIONADDR)calculate_solar_radiation_shading_degrees);
    gl_publish_function(
        oclass, "calculate_solar_radiation_shading_radians",
        (FUNCTIONADDR)calculate_solar_radiation_shading_radians);
    // New solar position algorithm stuff
    gl_publish_function(
        oclass, "calculate_solar_radiation_shading_position_radians",
        (FUNCTIONADDR)calculate_solar_radiation_shading_position_radians);
    gl_publish_function(oclass,
                        "calculate_solpos_radiation_shading_position_radians",
                        (FUNCTIONADDR)calc_solar_solpos_shading_position_rad);
    gl_publish_function(
        oclass, "calc_solar_ideal_shading_position_radians",
        (FUNCTIONADDR)calc_solar_ideal_shading_position_radians);
  }
}
...
int climate::create(void) {
  ...
  // solar_flux[0] = solar_flux[1] = solar_flux[2] = solar_flux[3] =
  // solar_flux[4] = solar_flux[5] = solar_flux[6] = solar_flux[7] =
  // solar_flux[8] = 0.0; // W/sf normal
  // // solar_flux_S = solar_flux_SE = solar_flux_SW = solar_flux_E =
  // solar_flux_W = solar_flux_NE = solar_flux_NW = solar_flux_N = 0.0; // W/sf
  // normal cloud_opacity = 1.0; cloud_speed_factor = 1;
  ...
  this->cloud_aerosol_transmissivity =
      defaults_storage.cloud_aerosol_transmissivity;
...
int climate::init(OBJECT *parent) {
  ...
  if (strcmp(tmyfile, "") == 0) {
    gl_verbose(
        "Manual or FNCS/HELICS climate control; initializing to the starttime");
  ...
  if (gl_findfile(tmyfile.get_string(), nullptr, R_OK, found_file,
                  sizeof(found_file)) ==
      nullptr) // TODO: get proper values for solar
  {
  ...
  if (cloud_model != (enumeration)CLOUDMODEL::CM_NONE) {
    ...
    if (cloud_opacity > 1) {
      gl_warning("climate:%s - Cloud opacity must be no greater than 1.0, "
                 "setting to 1.0",
                 obj->name);
      cloud_opacity = 1.0;
    } else if (cloud_opacity < 0) {
      gl_warning(
          "climate:%s - Cloud opacity must be no less than 0.0, setting to 0.0",
          obj->name);
      cloud_opacity = 0.0;
    }
    /*
    if (cloud_reflectivity > 1){
            gl_warning("climate:%s - Cloud reflectivity must be no greater
    than 1.0, setting to 1.0",obj->name); cloud_opacity = 1.0;
    }
    else if (cloud_reflectivity < 0){
            gl_warning("climate:%s - Cloud reflectivity must be no less than
    0.0, setting to 0.0",obj->name); cloud_opacity = 0.0;
    }
    */
    if (cloud_speed_factor < 0) {
      gl_warning("climate:%s - Cloud speed adjustment cannot be negative, "
                 "setting to 1.0",
                 obj->name);
      cloud_speed_factor = 1.0;
    }
    if (cloud_alpha < cloud_num_layers) {
      gl_warning("climate:%s - Cloud model alpha value must be less than or "
                 "equal to cloud_num_layers, setting to cloud_num_layers",
                 obj->name);
      cloud_alpha = cloud_num_layers;
    }
    if (cloud_aerosol_transmissivity < 0) {
      gl_warning("climate:%s - Cloud model aerosol transmissivity must be "
                 "greater than or equal to 0, setting to default value of 0.9",
                 obj->name);
      cloud_aerosol_transmissivity = 0.9;
    }
    if (cloud_aerosol_transmissivity > 1) {
      gl_warning("climate:%s - Cloud model aerosol transmissivity must be less "
                 "than or equal to 1, setting to 1",
                 obj->name);
      cloud_aerosol_transmissivity = 1.0;
    }
    gl_warning(
        "This cloud model places a large burden on computational resources. "
        "Patience and/or a more capable computer may be required.");
    init_cloud_pattern();
  ...
  if (reader_type == (enumeration)RT::RT_CSV) {
    ...
    if (reader == nullptr) {
      gl_error("climate::init(): no csv_reader specified for tmyfile %s",
               tmyfile.get_string());
      /* TROUBLESHOOT
              The weather file provided is for the csv_reader object but not
         csv_reader object was specified in the reader property. Please specify
         the name of the csv_reader object in the climate object in the glm.
      */
      return 0;
    } else {
      if ((reader->flags & OF_INIT) != OF_INIT) {
        char objname[256];
        gl_verbose("climate::init(): deferring initialization on %s",
                   gl_name(reader, objname, 255));
        return 2; // defer
      }
      csv_reader *my =
          /*OBJECTDATA(reader, csv_reader)*/ object_data<csv_reader>(reader);
      reader_hndl = my;
      rv = my->open(my->filename);
      //			my->get_data(t0, &temperature, &humidity,
      //&solar_direct, &solar_diffuse, &wind_speed, &rainfall, &snowdepth);
      ...
      if (obj->latitude < 0) {
        gl_warning("climate:%s - Southern hemisphere solar position model may "
                   "have issues",
                   obj->name);
        /*  TROUBLESHOOT
        The default solar position model was built around a northern hemisphere
        assumption.  As such, it doesn't always produce completely accurate
        results for southern hemisphere locations.  Calculated insolation values
        are approximately correct, but may show discrepancies against measured
        data.  If this climate is associated with a solar object, use the
        SOLAR_TILT_MODEL SOLPOS to ensure proper results (this warning will
        still pop up).
        */
      }
  ...
  double dnr, dhr, ghr, wspeed, wdir, precip, snowdepth, pressure, extra_dni,
      extra_ghi, tot_sky_cov, opq_sky_cov;
  // char cty[50];
  // char st[3];
  ...
  if (fabs(obj->latitude) > 90) {
    ...
    than 90 or less than -90 degrees.  This is not a valid value.  Please
    specify the latitude in this range, with positive values representing the
    northern hemisphere and negative values representing the southern
    hemisphere.
    */
    return 0;
  }
  ...
  if (fabs(obj->longitude) > 180) {
    ...
    than 180 or less than -180 degrees.  This is not a valid value.  Please
    specify the longitude in this range, with positive values representing the
    eastern hemisphere and negative values representing the western hemisphere.
    */
    return 0;
  }
  ...
  if (obj->latitude < 0) {
    gl_warning(
        "climate:%s - Southern hemisphere solar position model may have issues",
        obj->name);
    // Defined above
  }
  ...
  while (line < 8760 && file.next()) {
    ...
    file.read_data(&dnr, &dhr, &ghr, &temperature, &humidity, &month, &day,
                   &hour, &wspeed, &wdir, &precip, &snowdepth, &pressure,
                   &extra_dni, &extra_ghi, &tot_sky_cov, &opq_sky_cov);
    ...
    if (hoy >= 0 && hoy < 8760) {
      ...
      double sol_time = sa->solar_time((double)hour, doy, RAD(tz_meridian),
                                       RAD(get_longitude()));
      double sol_rad = 0.0;

      tmy[hoy].solar_elevation =
          sa->altitude(doy, RAD(obj->latitude), sol_time);
      ...
      for (COMPASS_PTS c_point = CP_H; c_point < CP_LAST;
        ...
        if (c_point == CP_H)
          sol_rad = file.calc_solar(CP_E, doy, RAD(get_latitude()), sol_time,
                                    dnr, dhr, ghr, ground_reflectivity,
                                    0.0); //(double)dnr * cos_incident + dhr;
        else
          sol_rad = file.calc_solar(
              c_point, doy, RAD(get_latitude()), sol_time, dnr, dhr, ghr,
              ground_reflectivity); //(double)dnr * cos_incident + dhr;
      ...
      gl_error("%s(%d): day %d, hour %d is out of allowed range 0-8759 hours",
               tmyfile.get_string(), line, day, hour);
...
int climate::get_solar_for_location(double latitude, double longitude,
                                    double *direct, double *global,
                                    double *diffuse) {
  int retval = 1;
  ...
  case (enumeration)CLOUDMODEL::CM_CUMULUS: {
    ...
    // retval = get_fuzzy_cloud_value_for_location(latitude, longitude, &cloud);
    // // Fuzzy cloud pattern evaluation f = 1 - (cloud * cloud_opacity);
    // // f=1 -> clear view of sun, f=0 -> very dark cloud blocking view of sun.
    int cloud_bin = 1; // default to "blue sky"
    retval = get_binary_cloud_value_for_location(
        latitude, longitude, &cloud_bin); // Binary cloud pattern evaluation
    ...
    *diffuse =
        get_solar_diffuse(); //(0.07 +
                             //0.45*get_tot_sky_cov())*global_transmissivity*ETR;
                             // get_solar_diffuse();
    *global = std::max(
        (*direct) * std::max(cos(get_solar_zenith()), 0.0) + *diffuse, 0.0);
...
int climate::get_binary_cloud_value_for_location(double latitude,
                                                 double longitude, int *cloud) {
  int pixel_x =
      floor(gl_lerp(latitude, MIN_LAT, MIN_LAT_INDEX, MAX_LAT, MAX_LAT_INDEX));
  int pixel_y =
      floor(gl_lerp(longitude, MIN_LON, MIN_LON_INDEX, MAX_LON, MAX_LON_INDEX));
  *cloud = binary_cloud_pattern[pixel_x][pixel_y];
  // Debugging and validation
  // write_out_cloud_pattern('C');
  //	write_out_cloud_pattern('B');
  //	gl_output("%i,%f,%f,%i,%i,%i", prev_NTime, latitude, longitude, pixel_x,
  //pixel_y,*cloud); 	gl_output("%f,%i,%f,%i", latitude, pixel_x, longitude,
  //pixel_y); 	gl_output(" ");
  return 1;
}

int climate::get_fuzzy_cloud_value_for_location(double latitude,
                                                double longitude,
                                                double *cloud) {
  ...
  *cloud = fuzzy_cloud_pattern[0][pixel_x][pixel_y];
  ...
  //	write_out_cloud_pattern('F');
  //	write_out_cloud_pattern('B');
  //	gl_output("%i,%f,%f,%i,%i,%f", prev_NTime, latitude, longitude, pixel_x,
  //pixel_y,*cloud); //fuzzy clouds 	gl_output("%f,%i,%f,%i", latitude, pixel_x,
  //longitude, pixel_y); 	gl_output(" ");
  return 1;
}
...
void climate::init_cloud_pattern() {
  ...
  cloud_pattern_size =
      num_tile_edge * CLOUD_TILE_SIZE + 1; // pattern must be 2^x + 1 square
  on_screen_size = (num_tile_edge - 2) *
                   CLOUD_TILE_SIZE; // Off-screen area is one tile width around
                                    // the perimeter of the on-screen area.
  ...
  for (int i = 0; i < num_tile_edge; i++) {
    for (int j = 0; j < num_tile_edge; j++) {
      ...
      build_cloud_pattern(
          col_min, col_max, row_min,
          row_max); // Min/Max x/y must always define a 2^x + 1 region of
                    // cloud_pattern write_out_cloud_pattern('C');
    }
  }
...
void climate::update_cloud_pattern(TIMESTAMP delta_t) {
  ...
  // Using European Wind Atlas terrain roughness class and length to translate
  // TMY2 windspeed at 10m to
  ...
  // log(1000/roughness_length)/log(10/roughness_length);

  // C. W. Hansen, J. S. Stein, and A. Ellis,
  //"Simulation of One-Minute Power Output from Utility-Scale Photovoltaic
  //Generation Systems," SAND2011-5529, 2011.
  //  Shows distribution of wind speeds at cumulus cloud elevations based on
  //  weather balloon launches (pg. 18) A cursory inspection of the TMY3
  //  measured wind speeds shows this is roughly consistent with histogram
  //  presented in the paper.
  //
  //  As part of validation I found I was having to scale back the windspeed to
  //  roughly the same speeds as the original source data. (Thus the
  //  "cloud_speed_factor" parameter.) Removing windspeed adjustment and using
  //  source data as is.
  double windspeed = windspeed_tmy2;
  ...
  double time_step = delta_t; // Placeholder until we dig into the actual GLD
                              // synchronization process.

  double col_shift_px =
      (windspeed * cos(wind_direct) * time_step) / PIXEL_EDGE_SIZE;
  double row_shift_px =
      (windspeed * sin(wind_direct) * time_step) / PIXEL_EDGE_SIZE;

  col_shift_request_px = col_shift_request_px + col_shift_px;
  row_shift_request_px = row_shift_request_px + row_shift_px;

  // Accumulating wind shift vector so that small persistent winds still have an
  // effect.
  int col_shift = 0;
  int row_shift = 0;
  ...
  if (row_shift != 0 || col_shift != 0) {
    if (row_shift >= 0 && col_shift >= 0) { // Wind blows from SW to NE
      if (col_shift > 0) {
        ...
        // Checking to see if barely off-screen values are empty before shifting
        // the pattern.
        //  If check is not done, may result in EMPTY_VALUES getting shifted
        //  on_screen.
        for (row = CLOUD_TILE_SIZE; row < CLOUD_TILE_SIZE + on_screen_size;
             row++) {
      ...
      if (row_shift > 0) {
        ...
        for (col = CLOUD_TILE_SIZE; col < CLOUD_TILE_SIZE + on_screen_size;
             col++) {
          if (cloud_pattern[row][col] == EMPTY_VALUE) {
      ...
      for (row = cloud_pattern_size - 1; row >= 0; row--) {
        for (col = cloud_pattern_size - 1; col >= 0; col--) {
          if (row + row_shift > cloud_pattern_size - 1) {
            ...
            cloud_pattern[row + row_shift][col + col_shift] =
                cloud_pattern[row][col];
            cloud_pattern[row][col] = EMPTY_VALUE;
          }
        }
      }
    ...
    if (solar_zenith <
        (110 * PI / 180)) { // Only do these things if the sun is above (or
                            // slightly below) the horizon).
      // Fractal cloud pattern is preserved and shifted appropriately but since
      // the sun is below the horizon
      //   and the solar radiation on the surface is zero, we don't need to
      //   fully define the clouds.
...
double climate::convert_to_binary_cloud() {
  // Convert fractal cloud pattern to binary value based on TMY2 opaque sky
  // value. Place-holder location.  Eventually needs to be driven by objects
  // asking for updated
  //   cloud values.  If the time since the last conversion is non-zero, this
  //   function needs to run to generate the new binary value.
  double cloud_value =
      get_opq_sky_cov(); // get_tot_sky_cov() or get_opq_sky_cov()

  /* TDH:
   * Trying to counteract the fuzzification process by artificially boosting the
   * coverage. This is a bit of a hack. The best way to handle this is to
   * include the fuzzification process in the elevation cut adjustment. This
   * way, the percentage of the sky covered by opaque clouds is ensured after
   * the fuzzification has taken place. I've tested this and it REALLY slows
   * things down (in an already slow function). For now, this is what I've
   * chosen to do because it more or less works.
   */
  ...
  double search_tolerance = 0.005; // Defines how close is close enough when
                                   // dialing in the binary cloud pattern.
  // OBJECT *obj=object_header(this);
  ...
  for (int i = 0; i < cloud_pattern_size; i++) {
    for (int j = 0; j < cloud_pattern_size; j++) {
      if (cloud_pattern[i][j] != EMPTY_VALUE) {
        normalized_cloud_pattern[i][j] =
            (cloud_pattern[i][j] - cloud_pattern_min) / cloud_pattern_range;
      }
    }
  }
  ...
  do {
    ...
    for (int i = CLOUD_TILE_SIZE; i < CLOUD_TILE_SIZE + on_screen_size; i++) {
      for (int j = CLOUD_TILE_SIZE; j < CLOUD_TILE_SIZE + on_screen_size; j++) {
        if (normalized_cloud_pattern[i][j] != EMPTY_VALUE &&
            normalized_cloud_pattern[i][j] <=
                cut_elevation) { // Values less than cut elevation are clouds
          running_count++;
        }
      }
    }
    measured_coverage =
        ...
        (on_screen_size * on_screen_size); // Factor, range [0 1]
  ...
  } while (measured_coverage < (cloud_value - search_tolerance) ||
           measured_coverage > (cloud_value + search_tolerance));
...
void climate::convert_to_fuzzy_cloud(double cut_elevation, int num_fuzzy_layers,
                                     double alpha) {

  double shade_step_size = 1.0 / alpha;

  if (cut_elevation == EMPTY_VALUE) { // Initialization call uses EMPTY_VALUE as
                                      // the cut elevation.
    // TDH: trivially parallelizable
  ...
  for (int i = 0; i < num_fuzzy_layers; i++) {
    double rand_upper =
        ((double)(i + 1) / (double)num_fuzzy_layers) * cut_elevation;
    double rand_lower =
        (((double)(i + 1) - 1) / (double)num_fuzzy_layers) * cut_elevation;
    for (int j = 0; j < cloud_pattern_size; j++) {
      for (int kk = 0; kk < cloud_pattern_size; kk++) {
        ...
        if (binary_cloud_pattern[j][kk] == 0.0 &&
            normalized_cloud_pattern[j][kk] != EMPTY_VALUE &&
            fuzzy_cloud_pattern[0][j][kk] !=
                EMPTY_VALUE) { // Areas with 0 in the binary pattern are cloudy
          if (normalized_cloud_pattern[j][kk] <=
              cut_elevation -
                  ((i + 1) * shade_step_size)) { // only values below the cut
                                                 // elevation accumulate
            fuzzy_cloud_pattern[0][j][kk] =
                gl_random_uniform(RNGSTATE, rand_lower, rand_upper) +
                fuzzy_cloud_pattern[0][j][kk];
...
void climate::rebuild_cloud_pattern_edge(char edge_needing_rebuilt) {
  ...
  if (edge_needing_rebuilt == 'W') {
    ...
    for (i = 0; i < 1 + on_screen_size / CLOUD_TILE_SIZE + 1; i++) {
      build_cloud_pattern(
          col_min, col_min + CLOUD_TILE_SIZE, row_min,
          row_min + CLOUD_TILE_SIZE); // Min/Max x/y must always define a 2^x +
                                      // 1 region of cloud_pattern
    ...
    for (i = 0; i < 1 + on_screen_size / CLOUD_TILE_SIZE + 1; i++) {
      build_cloud_pattern(
          col_min, col_min + CLOUD_TILE_SIZE, row_min,
          row_min + CLOUD_TILE_SIZE); // Min/Max row/col must always define a
                                      // 2^x + 1 region of cloud_pattern
...
void climate::trim_pattern_edge(char rebuilt_edge) {
  ...
  if (rebuilt_edge == 'W' || rebuilt_edge == 'E') {
    ...
    for (i = 0; i < CLOUD_TILE_SIZE; i++) {
      if (cloud_pattern[i][CLOUD_TILE_SIZE + on_screen_size + 10] !=
          EMPTY_VALUE) {
        min_edge_3 = i;
        break;
      }
    ...
    for (j = 0; j < CLOUD_TILE_SIZE; j++) {
      if (cloud_pattern[CLOUD_TILE_SIZE + on_screen_size + 10][j] !=
          EMPTY_VALUE) {
        min_edge_3 = j;
        break;
      }
...
void climate::erase_off_screen_pattern(char edge_to_erase) {
  ...
  if (edge_to_erase == 'W') {
    ...
    row_min = 0;
    for (int i = 0; i < cloud_pattern_size; i++) { // rows
      for (int j = cloud_pattern_size - CLOUD_TILE_SIZE + 1;
           j < cloud_pattern_size; j++) { // cols
        cloud_pattern[i][j] = EMPTY_VALUE;
      }
    }
    ...
    col_min = 0;
    row_min = 0;
    for (int i = cloud_pattern_size - CLOUD_TILE_SIZE + 1;
         i < cloud_pattern_size; i++) {              // rows
...
void climate::write_out_pattern_shift(
    int row_shift, int col_shift) { // Used only for verification.
  using namespace std;
  ofstream out_file;
...
void climate::write_out_cloud_pattern(
    char pattern) { // Used only for verification.
  using namespace std;
  ofstream out_file;
...
void climate::build_cloud_pattern(
    int col_min, int col_max, int row_min,
    int row_max) { // Min/Max row/col must always define a 2^x + 1 region of
                   // cloud_pattern
  int const SIGMA = 5;
  int step = col_max - col_min;
  int half_step = step / 2;
  int max_num_recursions =
      log((double)(col_max - col_min)) /
      log(2.0); // Change of base since c++ doesn't have a built-in log base 2.
  ...
  if (cloud_pattern[row_start + step][col_start] < EMPTY_VALUE * 0.98) {
    cloud_pattern[row_start + step][col_start] =
        gl_random_normal(RNGSTATE, 0, stdev);
  }
  if (cloud_pattern[row_start][col_start + step] < EMPTY_VALUE * 0.98) {
    cloud_pattern[row_start][col_start + step] =
        gl_random_normal(RNGSTATE, 0, stdev);
  }
  if (cloud_pattern[row_start + step][col_start + step] < EMPTY_VALUE * 0.98) {
    cloud_pattern[row_start + step][col_start + step] =
        gl_random_normal(RNGSTATE, 0, stdev);
  }
  ...
  for (int k = 0; k < max_num_recursions; k++) {
    ...
    for (int i = 0; i < end; i++) {
      for (int j = 0; j < end; j++) {
        ...
        double x =
            (c1 + c2 + c3 + c4) / 4 + gl_random_normal(RNGSTATE, 0, stdev);
        ...
        if (cloud_pattern[row_start + half_step][col_start + half_step] <
            EMPTY_VALUE * 0.98) {
          cloud_pattern[row_start + half_step][col_start + half_step] = x;
        }
        if (cloud_pattern[row_start + half_step][col_start] <
            EMPTY_VALUE * 0.98) {
          cloud_pattern[row_start + half_step][col_start] = e_a;
        }
        if (cloud_pattern[row_start][col_start + half_step] <
            EMPTY_VALUE * 0.98) {
          cloud_pattern[row_start][col_start + half_step] = e_b;
        }
        if (cloud_pattern[row_start + half_step][col_start + step] <
            EMPTY_VALUE * 0.98) {
          cloud_pattern[row_start + half_step][col_start + step] = e_c;
        }
        if (cloud_pattern[row_start + step][col_start + half_step] <
            EMPTY_VALUE * 0.98) {
...
int climate::calc_cloud_pattern_size(
  ...
  int num_tile_edge = 1; // Minimum size (1 on-screen tile with 1 off-screen on
                         // all sides; 9 tiles total).
  if (location_list.size() > 1) {
    ...
    double y_size_km =
        long_delta * cos(RAD(lat_min)) *
        KM_PER_DEG; // Assumes the lat_delta is small enough that  the polar
                    // convergence of the longitudinal lines is insignificant.
    double max_dim_km = std::max(x_size_km, y_size_km);
    num_tile_edge = ceil(((max_dim_km * 1000. / (double)PIXEL_EDGE_SIZE) /
                          (double)CLOUD_TILE_SIZE));
  }
  MIN_LAT_INDEX = CLOUD_TILE_SIZE; // because there is always a buffer
  MAX_LAT_INDEX =
      CLOUD_TILE_SIZE +
      num_tile_edge * CLOUD_TILE_SIZE; // buffer plus the number of pixels
  MIN_LON_INDEX = MIN_LAT_INDEX;       // square
  MAX_LON_INDEX =
      MAX_LAT_INDEX; // still square
  ...
  degree_range_lat =
      ((double)(num_tile_edge * CLOUD_TILE_SIZE * PIXEL_EDGE_SIZE)) / 1000. /
      KM_PER_DEG;
  degree_range_lon =
      ((double)(num_tile_edge * CLOUD_TILE_SIZE * PIXEL_EDGE_SIZE)) / 1000. /
      KM_PER_DEG / cos(RAD(lat_min));
  ...
  //	gl_output("lat min: %i=%f\tlat max: %i=%f", MIN_LAT_INDEX, MIN_LAT,
  //MAX_LAT_INDEX, MAX_LAT); 	gl_output("lon min: %i=%f\tlon max: %i=%f",
  //MIN_LON_INDEX, MIN_LON, MAX_LON_INDEX, MAX_LON); 	gl_output(" ");
  return num_tile_edge +
         2; // Adding extra tiles for off-screen buffer around perimeter;
}
...
TIMESTAMP climate::presync(TIMESTAMP t0) /* called in presync */
{
  ...
  if (t0 > TS_ZERO && tmy == nullptr &&
      reader_type !=
          (enumeration)RT::RT_CSV) { // no file was read, so it's probably
                                     // manual, FNCS or HELICS control
    gld_clock now(t0);
    // calculate the solar radiation
    OBJECT *obj = object_header(this);
    double longitude = obj->longitude;
    double sol_time = sa->solar_time(
        (double)now.get_hour() + (now.get_minute() / 60.0) +
            (now.get_second() / 3600.0) + (now.get_is_dst() ? -1 : 0),
        now.get_yearday(), RAD(tz_meridian), RAD(longitude));
    gl_localtime(t0, &dt);
    ...
    for (COMPASS_PTS c_point = CP_H; c_point < CP_LAST;
         c_point = COMPASS_PTS(c_point + 1)) {
      if (c_point == CP_H) {
        sol_rad = file.calc_solar(CP_E, now.get_yearday(), RAD(obj->latitude),
                                  sol_time, solar_direct, solar_diffuse,
                                  solar_global, ground_reflectivity, 0.0);
      } else {
        sol_rad = file.calc_solar(
            c_point, now.get_yearday(), RAD(obj->latitude), sol_time,
            solar_direct, solar_diffuse, solar_global, ground_reflectivity);
      }
      solar_flux[c_point] = sol_rad;
    }
  }
  // TODO: need to read the cloud stuff from the csv file
  // changes appear to be limited to weather.h, weather.cpp, csv_reader.h,
  // csv_reader.cpp
  if (t0 > TS_ZERO && reader_type == (enumeration)RT::RT_CSV) {
    ...
    csv_reader *cr =
        /*OBJECTDATA(reader, csv_reader)*/ object_data<csv_reader>(reader);
    csv_rv = cr->get_data(
        t0, &temperature, &humidity, &solar_direct, &solar_diffuse,
        &solar_global, &global_horizontal_extra, &wind_speed, &wind_dir,
        &opq_sky_cov, &tot_sky_cov, &rainfall, &snowdepth, &pressure);
    // calculate the solar radiation
    double sol_time = sa->solar_time(
        (double)now.get_hour() + now.get_minute() / 60.0 +
            now.get_second() / 3600.0 + (now.get_is_dst() ? -1 : 0),
        now.get_yearday(), RAD(tz_meridian), RAD(reader->longitude));
    // std::cout << now.get_hour() << "," << now.get_minute() << "," <<
    // now.get_second() <<"," << now.get_is_dst() << "," << now.get_yearday() <<
    // "," << tz_meridian << "," << reader->longitude << std::endl; std::cout <<
    // sol_time << std::endl;
    gl_localtime(t0, &dt);
    short day_of_yr = sa->day_of_yr(dt.month, dt.day);
    ...
    for (COMPASS_PTS c_point = CP_H; c_point < CP_LAST;
      ...
      if (c_point == CP_H)
        sol_rad = file.calc_solar(
            CP_E, now.get_yearday(), RAD(reader->latitude), sol_time,
            solar_direct, solar_diffuse, solar_global, ground_reflectivity,
            0.0); //(double)dnr * cos_incident + dhr;
      else
        sol_rad = file.calc_solar(
            c_point, now.get_yearday(), RAD(reader->latitude), sol_time,
            solar_direct, solar_diffuse, solar_global,
            ground_reflectivity); //(double)dnr * cos_incident + dhr;
      /* TMY2 solar radiation data is in Watt-hours per square meter. */
      solar_flux[c_point] = sol_rad;
    }
  ...
  if (t0 > TS_ZERO && tmy != nullptr) {
    ...
    // One hour shift as TMY are summarized values for the preceding hour. To
    // accurately calculate the solar time
    //     we need to start from the beginning of the hour and advance through
    //     it.
    ...
    case (enumeration)CI::CI_LINEAR:
      ...
      hoy1 = hoy + 1.0;
      temperature =
          (gl_lerp(now, hoy0, tmy[hoy].temp, hoy1, tmy[hoy + 1 % 8760].temp));
      humidity =
          (gl_lerp(now, hoy0, tmy[hoy].rh, hoy1, tmy[hoy + 1 % 8760].rh));
      solar_direct =
          (gl_lerp(now, hoy0, tmy[hoy].dnr, hoy1, tmy[hoy + 1 % 8760].dnr));
      solar_diffuse =
          (gl_lerp(now, hoy0, tmy[hoy].dhr, hoy1, tmy[hoy + 1 % 8760].dhr));
      solar_global =
          (gl_lerp(now, hoy0, tmy[hoy].ghr, hoy1, tmy[hoy + 1 % 8760].ghr));
      wind_speed = (gl_lerp(now, hoy0, tmy[hoy].windspeed, hoy1,
                            tmy[hoy + 1 % 8760].windspeed));
      rainfall = (gl_lerp(now, hoy0, tmy[hoy].rainfall, hoy1,
                          tmy[hoy + 1 % 8760].rainfall));
      snowdepth = (gl_lerp(now, hoy0, tmy[hoy].snowdepth, hoy1,
                           tmy[hoy + 1 % 8760].snowdepth));
      solar_azimuth = (gl_lerp(now, hoy0, tmy[hoy].solar_azimuth, hoy1,
                               tmy[hoy + 1 % 8760].solar_azimuth));
      solar_elevation = (gl_lerp(now, hoy0, tmy[hoy].solar_elevation, hoy1,
                                 tmy[hoy + 1 % 8760].solar_elevation));
      solar_zenith = (gl_lerp(now, hoy0, tmy[hoy].solar_zenith, hoy1,
                              tmy[hoy + 1 % 8760].solar_zenith));
      temperature_raw = (gl_lerp(now, hoy0, tmy[hoy].temp_raw, hoy1,
                                 tmy[hoy + 1 % 8760].temp_raw));
      solar_raw = (gl_lerp(now, hoy0, tmy[hoy].solar_raw, hoy1,
                           tmy[hoy + 1 % 8760].solar_raw));
      pressure = gl_lerp(now, hoy0, tmy[hoy].pressure, hoy1,
                         tmy[hoy + 1 % 8760].pressure);
      direct_normal_extra =
          gl_lerp(now, hoy0, tmy[hoy].direct_normal_extra, hoy1,
                  tmy[hoy + 1 % 8760].direct_normal_extra);
      global_horizontal_extra =
          gl_lerp(now, hoy0, tmy[hoy].global_horizontal_extra, hoy1,
                  tmy[hoy + 1 % 8760].global_horizontal_extra);
      wind_dir = gl_lerp(now, hoy0, tmy[hoy].wind_dir, hoy1,
                         tmy[hoy + 1 % 8760].wind_dir);
      tot_sky_cov = gl_lerp(now, hoy0, tmy[hoy].tot_sky_cov, hoy1,
                            tmy[hoy + 1 % 8760].tot_sky_cov);
      opq_sky_cov = gl_lerp(now, hoy0, tmy[hoy].opq_sky_cov, hoy1,
                            tmy[hoy + 1 % 8760].opq_sky_cov);
      for (int pt = 0; pt < CP_LAST; ++pt) {
        solar_flux[pt] = gl_lerp(now, hoy0, tmy[hoy].solar[pt], hoy1,
                                 tmy[hoy + 1 % 8760].solar[pt]);
      }
      break;
    case (enumeration)CI::CI_QUADRATIC:
      ...
      hoy2 = hoy + 2.0;
      temperature =
          (gl_qerp(now, hoy0, tmy[hoy].temp, hoy1, tmy[hoy + 1 % 8760].temp,
                   hoy2, tmy[hoy + 2 % 8760].temp));
      humidity = (gl_qerp(now, hoy0, tmy[hoy].rh, hoy1, tmy[hoy + 1 % 8760].rh,
                          hoy2, tmy[hoy + 2 % 8760].rh));
      if (humidity < 0.0) {
        humidity = 0.0;
        gl_verbose("Setting humidity to zero. Quadratic interpolation caused "
                   "the humidity to drop below zero.");
      }
      solar_direct =
          (gl_qerp(now, hoy0, tmy[hoy].dnr, hoy1, tmy[hoy + 1 % 8760].dnr, hoy2,
                   tmy[hoy + 2 % 8760].dnr));
      if (solar_direct < 0.0) {
        solar_direct = 0.0;
        gl_verbose("Setting solar_direct to zero. Quadratic interpolation "
                   "caused the solar_direct to drop below zero.");
      }
      solar_diffuse =
          (gl_qerp(now, hoy0, tmy[hoy].dhr, hoy1, tmy[hoy + 1 % 8760].dhr, hoy2,
                   tmy[hoy + 2 % 8760].dhr));
      if (solar_diffuse < 0.0) {
        solar_diffuse = 0.0;
        gl_verbose("Setting solar_diffuse to zero. Quadratic interpolation "
                   "caused the solar_diffuse to drop below zero.");
      }
      solar_global =
          (gl_qerp(now, hoy0, tmy[hoy].ghr, hoy1, tmy[hoy + 1 % 8760].ghr, hoy2,
                   tmy[hoy + 2 % 8760].ghr));
      if (solar_global < 0.0) {
        solar_global = 0.0;
        gl_verbose("Setting solar_global to zero. Quadratic interpolation "
                   "caused the solar_global to drop below zero.");
      }
      wind_speed = (gl_qerp(now, hoy0, tmy[hoy].windspeed, hoy1,
                            tmy[hoy + 1 % 8760].windspeed, hoy2,
                            tmy[hoy + 2 % 8760].windspeed));
      if (wind_speed < 0.0) {
        wind_speed = 0.0;
        gl_verbose("Setting wind_speed to zero. Quadratic interpolation caused "
                   "the wind_speed to drop below zero.");
      }
      rainfall = (gl_qerp(now, hoy0, tmy[hoy].rainfall, hoy1,
                          tmy[hoy + 1 % 8760].rainfall, hoy2,
                          tmy[hoy + 2 % 8760].rainfall));
      if (rainfall < 0.0) {
        rainfall = 0.0;
        gl_verbose("Setting rainfall to zero. Quadratic interpolation caused "
                   "the rainfall to drop below zero.");
      }
      snowdepth = (gl_qerp(now, hoy0, tmy[hoy].snowdepth, hoy1,
                           tmy[hoy + 1 % 8760].snowdepth, hoy2,
                           tmy[hoy + 2 % 8760].snowdepth));
      if (snowdepth < 0.0) {
        snowdepth = 0.0;
        gl_verbose("Setting snowdepth to zero. Quadratic interpolation caused "
                   "the snowdepth to drop below zero.");
      }
      solar_azimuth = (gl_qerp(now, hoy0, tmy[hoy].solar_azimuth, hoy1,
                               tmy[hoy + 1 % 8760].solar_azimuth, hoy2,
                               tmy[hoy + 2 % 8760].solar_azimuth));
      solar_elevation = (gl_qerp(now, hoy0, tmy[hoy].solar_elevation, hoy1,
                                 tmy[hoy + 1 % 8760].solar_elevation, hoy2,
                                 tmy[hoy + 2 % 8760].solar_elevation));
      solar_zenith = (gl_qerp(now, hoy0, tmy[hoy].solar_zenith, hoy1,
                              tmy[hoy + 1 % 8760].solar_zenith, hoy2,
                              tmy[hoy + 2 % 8760].solar_zenith));
      temperature_raw = (gl_qerp(now, hoy0, tmy[hoy].temp_raw, hoy1,
                                 tmy[hoy + 1 % 8760].temp_raw, hoy2,
                                 tmy[hoy + 2 % 8760].temp_raw));
      solar_raw = (gl_qerp(now, hoy0, tmy[hoy].solar_raw, hoy1,
                           tmy[hoy + 1 % 8760].solar_raw, hoy2,
                           tmy[hoy + 2 % 8760].solar_raw));
      if (solar_raw < 0.0) {
        solar_raw = 0.0;
        gl_verbose("Setting solar_raw to zero. Quadratic interpolation caused "
                   "the solar_raw to drop below zero.");
      }
      pressure = gl_qerp(now, hoy0, tmy[hoy].pressure, hoy1,
                         tmy[hoy + 1 % 8760].pressure, hoy2,
                         tmy[hoy + 2 % 8760].pressure);
      if (pressure < 0.0) {
        pressure = 0.0;
        gl_verbose("Setting pressure to zero. Quadratic interpolation caused "
                   "the pressure to drop below zero.");
      }
      direct_normal_extra =
          gl_qerp(now, hoy0, tmy[hoy].direct_normal_extra, hoy1,
                  tmy[hoy + 1 % 8760].direct_normal_extra, hoy2,
                  tmy[hoy + 2 % 8760].direct_normal_extra);
      if (direct_normal_extra < 0.0) {
        direct_normal_extra = 0.0;
        gl_verbose("Setting extraterrestrial_direct_normal to zero. Quadratic "
                   "interpolation caused the extraterrestrial_direct_normal to "
                   "drop below zero.");
      }
      global_horizontal_extra =
          gl_qerp(now, hoy0, tmy[hoy].global_horizontal_extra, hoy1,
                  tmy[hoy + 1 % 8760].global_horizontal_extra, hoy2,
                  tmy[hoy + 2 % 8760].global_horizontal_extra);
      if (global_horizontal_extra < 0.0) {
        global_horizontal_extra = 0.0;
        gl_verbose(
            "Setting global_horizontal_extra to zero. Quadratic interpolation "
            "caused the global_horizontal_extra to drop below zero.");
      }
      wind_dir = gl_qerp(now, hoy0, tmy[hoy].wind_dir, hoy1,
                         tmy[hoy + 1 % 8760].wind_dir, hoy2,
                         tmy[hoy + 2 % 8760].wind_dir);
      if (wind_dir < 0.0) {
        wind_dir = 360.0 + wind_dir;
        gl_verbose("Setting wind_dir to 360+wind_dir. Quadratic interpolation "
                   "caused the wind_dir to drop below zero.");
      }
      if (wind_dir > 360.0) {
        wind_dir = wind_dir - 360.0;
        gl_verbose("Setting wind_dir to wind_dir-360. Quadratic interpolation "
                   "caused the wind_dir to rise above 360.");
      }
      tot_sky_cov = gl_qerp(now, hoy0, tmy[hoy].tot_sky_cov, hoy1,
                            tmy[hoy + 1 % 8760].tot_sky_cov, hoy2,
                            tmy[hoy + 2 % 8760].tot_sky_cov);
      if (tot_sky_cov < 0.0) {
        tot_sky_cov = 0.0;
        gl_verbose("Setting tot_sky_cov to zero. Quadratic interpolation "
                   "caused the tot_sky_cov to drop below zero.");
      }
      opq_sky_cov = gl_qerp(now, hoy0, tmy[hoy].opq_sky_cov, hoy1,
                            tmy[hoy + 1 % 8760].opq_sky_cov, hoy2,
                            tmy[hoy + 2 % 8760].opq_sky_cov);
      if (opq_sky_cov < 0.0) {
        opq_sky_cov = 0.0;
        gl_verbose("Setting opq_sky_cov to zero. Quadratic interpolation "
                   "caused the opq_sky_cov to drop below zero.");
      }
      ...
      for (int pt = 0; pt < CP_LAST; ++pt) {
        if (tmy[hoy].solar[pt] == tmy[hoy + 1].solar[pt]) {
          ...
          solar_flux[pt] = gl_qerp(now, hoy0, tmy[hoy].solar[pt], hoy1,
                                   tmy[hoy + 1 % 8760].solar[pt], hoy2,
                                   tmy[hoy + 2 % 8760].solar[pt]);
    ...
    update_forecasts(t0);
    tmy_rv = -(t0 + (3600 * TS_SECOND -
                     t0 % (3600 * TS_SECOND))); /// negative means soft event
  }
  ...
  // Extra logic to return the correct timestamp based on the weather data
  // source and the use of the cloud model.
  if (t0 <= TS_ZERO)
    return TS_NEVER;
    ...
    if (reader_type == (enumeration)RT::RT_CSV)
      ...
      return TS_NEVER;
  else if (reader_type == (enumeration)RT::RT_CSV)
