...
TIMESTAMP double_controller::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (*pMarketID != lastmkt_id)
	{
		...
		if (*pNextP > *pAvg)
		{ // high prices un-cool and un-heat
		...
		if (bid_mode == BM_ON && *pNextP > bid_price)
		{ // only if we care about 'beating the market'
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_double_controller(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
