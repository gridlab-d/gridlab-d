/** $Id: double_controller.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        @defgroup double_controller Two-setpoint transactive controller,
 modified OlyPen experiment style for combined heating & cooling logic
        @ingroup market
...
double_controller::double_controller(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(
        module, "double_controller", sizeof(double_controller),
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "thermostat_mode", PADDR(thermostat_mode),
            PT_KEYWORD, "INVALID", (enumeration)TM_INVALID, PT_KEYWORD, "OFF",
            (enumeration)TM_OFF, PT_KEYWORD, "HEAT", (enumeration)TM_HEAT,
            PT_KEYWORD, "COOL", (enumeration)TM_COOL, PT_enumeration,
            "last_mode", PADDR(last_mode), PT_ACCESS, PA_REFERENCE, PT_KEYWORD,
            "INVALID", (enumeration)TM_INVALID, PT_KEYWORD, "OFF",
            (enumeration)TM_OFF, PT_KEYWORD, "HEAT", (enumeration)TM_HEAT,
            PT_KEYWORD, "COOL", (enumeration)TM_COOL, PT_enumeration,
            "resolve_mode", PADDR(resolve_mode), PT_KEYWORD, "NONE",
            (enumeration)RM_NONE, PT_KEYWORD, "DEADBAND",
            (enumeration)RM_DEADBAND, PT_KEYWORD, "STICKY",
            (enumeration)RM_STICKY, PT_enumeration, "setup_mode",
            PADDR(setup_mode), PT_KEYWORD, "NONE", (enumeration)ST_NONE,
            PT_KEYWORD, "HOUSE", (enumeration)ST_HOUSE, PT_enumeration,
            "bid_mode", PADDR(bid_mode), PT_KEYWORD, "ON", (enumeration)BM_ON,
            PT_KEYWORD, "OFF", (enumeration)BM_OFF, PT_int64, "last_mode_timer",
            PADDR(last_mode_timer), PT_double, "cool_ramp_low",
            PADDR(cool_ramp_low), PT_double, "cool_ramp_high",
            PADDR(cool_ramp_high), PT_double, "cool_range_low",
            PADDR(cool_range_low), PT_double, "cool_range_high",
            PADDR(cool_range_high), PT_double, "cool_set_base",
            PADDR(cool_set_base), PT_double, "cool_setpoint",
            PADDR(cool_setpoint), PT_double, "heat_ramp_low",
            PADDR(heat_ramp_low), PT_double, "heat_ramp_high",
            PADDR(heat_ramp_high), PT_double, "heat_range_low",
            PADDR(heat_range_low), PT_double, "heat_range_high",
            PADDR(heat_range_high), PT_double, "heat_set_base",
            PADDR(heat_set_base), PT_double, "heat_setpoint",
            PADDR(heat_setpoint), PT_char32, "temperature_name",
            PADDR(temperature_name), PT_char32, "cool_setpoint_name",
            PADDR(cool_setpoint_name), PT_char32, "cool_demand_name",
            PADDR(cool_demand_name), PT_char32, "heat_setpoint_name",
            PADDR(heat_setpoint_name), PT_char32, "heat_demand_name",
            PADDR(heat_demand_name), PT_char32, "load_name", PADDR(load_name),
            PT_char32, "total_load_name", PADDR(total_load_name), PT_char32,
            "deadband_name", PADDR(deadband_name), PT_char32, "state_name",
            PADDR(state_name), PT_object, "market", PADDR(pMarket),
            PT_DESCRIPTION, "the market to bid into", PT_double,
            "market_period", PADDR(market_period), PT_double, "bid_price",
            PADDR(bid_price), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION,
            "the bid price", PT_double, "bid_quant", PADDR(bid_quant),
            PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "the bid quantity",
            PT_char32, "load", PADDR(load), PT_DESCRIPTION,
            "the current controlled load", PT_char32, "total", PADDR(total),
            PT_DESCRIPTION, "the uncontrolled load (if any)", PT_double,
            "last_price", PADDR(last_price), PT_double, "temperature",
            PADDR(temperature), PT_double, "cool_bid", PADDR(cool_bid),
            PT_double, "heat_bid", PADDR(heat_bid), PT_double, "cool_demand",
            PADDR(cool_demand), PT_double, "heat_demand", PADDR(heat_demand),
            PT_double, "price", PADDR(price), PT_double, "avg_price",
            PADDR(avg_price), PT_double, "stdev_price", PADDR(stdev_price),
...
void double_controller::fetch(double **value, char *name, OBJECT *parent,
                              PROPERTY **prop, const char *goal) {
...
int double_controller::init(OBJECT *parent) {
  ...
  if (parent == nullptr) {
    gl_error(
        "%s: double_controller has no parent, therefore nothing to control",
        namestr);
    return 0;
  }

  if (pMarket == nullptr) {
    gl_error("%s: double_controller has no market, therefore no price signals",
             namestr);
    return 0;
  }
  ...
  if (pAvg == 0) {
    gl_error("%s: double_controller market has no %s average property", namestr,
             avg_target.get_string());
    return 0;
  }
  if (pStd == 0) {
    gl_error(
        "%s: double_controller market has no %s standard deviation property",
        namestr, std_target.get_string());
    return 0;
  }
  if (pPeriod == 0) {
    gl_error("%s: double_controller market has no period (and is not a market)",
             namestr);
    return 0;
  }
  if (pNextP == 0) {
    gl_error("%s: double_controller market has no next.P (and is not a market)",
             namestr);
    return 0;
  }
  if (pMarketID == 0) {
    gl_error(
        "%s: double_controller market has no market_id (and is not a market)",
        namestr);
    return 0;
  }
  fetch(&temperature_ptr, temperature_name, parent, &temperature_prop,
        "temperature_name");
  fetch(&cool_setpoint_ptr, cool_setpoint_name, parent, &cool_setpoint_prop,
        "cool_setpoint_name");
  fetch(&heat_setpoint_ptr, heat_setpoint_name, parent, &heat_setpoint_prop,
        "heat_setpoint_name");
  if (bid_mode == BM_ON) {
    fetch(&load_ptr, load_name, parent, &load_prop, "load_name");
    fetch(&total_load_ptr, total_load_name, parent, &total_load_prop,
          "total_load_name");
    fetch(&cool_demand_ptr, cool_demand_name, parent, &cool_demand_prop,
          "cool_demand_prop");
    fetch(&heat_demand_ptr, heat_demand_name, parent, &heat_demand_prop,
          "heat_demand_name");
  }
  ...
  if (cool_range_low > 0 || heat_range_low > 0) {
    gl_error("%s: heating and cooling minimum temperature ranges must be "
             "non-positive",
             namestr);
    return 0;
  }
  if (cool_range_high < 0 || heat_range_high < 0) {
    gl_error("%s: heating and cooling maximum temperature ranges must be "
             "non-negative",
             namestr);
    return 0;
  }
...
TIMESTAMP double_controller::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (temperature > cool_temp_max) {
    ...
    if (*pStd != 0.0) {
      cool_bid = *pAvg + (temperature - cool_set_base) * (cool_ramp * *pStd) /
                             fabs(cool_limit - cool_set_base);
    } else {
      cool_bid = *pAvg;
    }
  }
  ...
  if (temperature > heat_temp_max) {
    ...
    if (*pStd != 0.0) {
      heat_bid = *pAvg + (temperature - heat_set_base) * (heat_ramp * *pStd) /
                             fabs(heat_limit - heat_set_base);
    } else {
      heat_bid = *pAvg;
    }
  }
  ...
  case RM_NONE:
    ...
    break;
  default:
    // if the two limits are close, clip whichever one has not been used
    // recently (else cooling) if new limit would not bid at current
    // temperature, update accordingly
    ...
    if (bid_quant != 0) {
      ...
      if (state_ptr != 0) {
        ...
        ((void (*)(char *, char *, const char *, const char *, void *, size_t))(
            *submit))((char *)gl_name(hdr, ctrname, 1024),
                      (char *)gl_name(pMarket, mktname, 1024),
...
TIMESTAMP double_controller::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (*pMarketID != lastmkt_id) {
    ...
    avg_price = 0.0;
    if (*pAvg == 0.0 || *pStd == 0.0 || cool_set_base == 0.0 ||
        heat_set_base == 0.0) {
    ...
    if (bid_mode == BM_ON &&
        *pNextP > bid_price) { // only if we care about 'beating the market'
      ...
      if (cool_bid > 0.0 || bid_mode == BM_OFF) {
        double ramp =
            (temperature > cool_set_base ? cool_ramp_high : cool_ramp_low);
        double cool_offset = (*pNextP - *pAvg) *
                             fabs(cool_bound - cool_set_base) / (ramp * *pStd);
      ...
      if (heat_bid > 0.0 || bid_mode == BM_OFF) {
        double ramp =
            (temperature > heat_set_base ? heat_ramp_high : heat_ramp_low);
        double heat_offset = (*pNextP - *pAvg) *
                             fabs(heat_bound - heat_set_base) / (ramp * *pStd);
    ...
    *obj = gl_create_object(double_controller::oclass);
    if (*obj != nullptr) {
      double_controller *my =
          /*OBJECTDATA(*obj, double_controller)*/ object_data<
              double_controller>(*obj);
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_double_controller(OBJECT *obj,
                                                       TIMESTAMP t1,
                                                       PASSCONFIG pass) {
  return sync_double_controller_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_double_controller(OBJECT *obj, ...) {
  va_list args;
