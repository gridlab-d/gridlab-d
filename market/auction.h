...
typedef enum { AM_NONE = 0, AM_DENY = 1, AM_PROB = 2 } MARGINMODE;
...
private:
  ...
  void record_bid(char *from, double quantity, double real_price,
                  BIDDERSTATE state);
...
public:
  ...
  int16 lasthr, thishr;
  OBJECT *linkref; /**< reference link object that contains the total load (used
                      to determine unresponsive load when not all load bid) */
  ...
  double future_mean_price; // right now the future mean price will be fed with
                            // a player
  ...
  double
      capacity_reference_bid_price; // the bid price the capacity reference bids
  double max_capacity_reference_bid_quantity; // the maximum bid quantity
  double capacity_reference_bid_quantity;     // the bid quantity the capacity
                                              // reference bids
  ...
  double *new_prices;
  double
      *new_market_failures; // 0 indicates it did NOT fail, 1 indicates failure
  double *statdata;
...
public:
  int submit(char *from, double quantity, double real_price, KEY key,
             BIDDERSTATE state, bool rebid, int64 mkt_id);

private:
  int submit_nolock(char *from, double quantity, double real_price, KEY key,
                    BIDDERSTATE state, bool rebid, int64 mkt_id);
