...
public:
    ...
    bool operator<(const supervisor_bid &rhs) const {
      ...
      if (sort_by == 1) { // sort by power increasing
        ...
        return_val = voltage_deviation < rhs.voltage_deviation;
                 ...
                 4) { // sort by furtherst away from nominal in each direction
        return_val = voltage_deviation < rhs.voltage_deviation;
      } else {
        throw std::logic_error(
            "sort method is not defined!"); // This fixes no-return state.
      }
      return return_val;
    }
  } SUPERVISORBID;
  ...
  ~collect(void);
  void clear(void);
  int submit_on(OBJECT *from, double power, double voltage_deviation, int key,
                int state);
  int submit_off(OBJECT *from, double power, double voltage_deviation, int key,
                 int state);
  int calculate_freq_thresholds(double droop, double nom_freq,
                                double frequency_deadband, int PFC_mode);
