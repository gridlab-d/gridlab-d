...
stubauction::stubauction(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "stubauction", sizeof(stubauction),
    ...
    if (gl_publish_variable(
            ...
            PT_double, "last.P", PADDR(last_price), PT_DESCRIPTION,
            "last cleared price", PT_double, "current_market.clearing_price",
            PADDR(next_price), PT_DESCRIPTION, "next cleared price", PT_double,
            "past_market.clearing_price", PADDR(last_price), PT_DESCRIPTION,
            "last cleared price", PT_double, "next.P", PADDR(next_price),
            PT_DESCRIPTION, "next cleared price", PT_double, "avg24",
            PADDR(avg24), PT_DESCRIPTION, "daily average of price", PT_double,
            "std24", PADDR(std24), PT_DESCRIPTION, "daily stdev of price",
            PT_double, "avg72", PADDR(avg72), PT_DESCRIPTION,
            "three day price average", PT_double, "std72", PADDR(std72),
            PT_DESCRIPTION, "three day price stdev", PT_double, "avg168",
            PADDR(avg168), PT_DESCRIPTION, "weekly average of price", PT_double,
            "std168", PADDR(std168), PT_DESCRIPTION, "weekly stdev of price",
            PT_int64, "market_id", PADDR(market_id), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "unique identifier of market clearing", PT_bool,
            "verbose", PADDR(verbose), PT_DESCRIPTION,
            "enable verbose stubauction operations", PT_enumeration,
            "control_mode", PADDR(control_mode), PT_DESCRIPTION,
            "the control mode to use for determining average and deviation "
            "calculations",
            PT_KEYWORD, "NORMAL", (enumeration)CON_NORMAL, PT_KEYWORD,
            "DISABLED", (enumeration)CON_DISABLED, nullptr) < 1)
 ...
 */
TIMESTAMP stubauction::presync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
...
TIMESTAMP stubauction::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (t1 >= clearat) {
    ...
    //		if (verbose) gl_output("   ...%s clearing process started at
    //%s", gl_name(object_header(this),myname,sizeof(myname)),
    ...
    clearat = nextclear();
    gl_localtime(clearat, &dt);
    //		if (verbose) gl_output("   ...%s opens for clearing of market_id
    //%d at %s", gl_name(object_header(this),name,sizeof(name)),
    //(int32)market_id, gl_strtime(&dt,buffer,sizeof(buffer))?buffer:"unknown
    //time");
  }
  return (retry ? t1 : -clearat); /* soft return t2>t1 on success, t2=t1 for
                                     retry, t2<t1 on failure */
}
