...
public:
  ...
  typedef enum { PDT_NORMAL, PDT_EXPONENTIAL, PDT_UNIFORM } DISTRIBUTIONTYPE;
  ...
  bool input_chained;    ///< flag to determine if there are any attached
                         ///< controllers
  ...
  char32 observation_stdev_propname; ///< observation standard devation property
                                     ///< name
  ...
  bool pool_pump_model; ///< Boolean flag for turning on the pool pump model
                        ///< version of duty cycle control
  double base_duty_cycle;
  double cycle_on, cycle_off; ///< used to specify the number of seconds of on
                              ///< and off during the cycling dlc mode
  bool first_period; ///< boolean for determing whether we need to "seed" the
                     ///< dlc cycling mode
