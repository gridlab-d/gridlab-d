...
controller::controller(MODULE *module) {
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "simple_mode", PADDR(simplemode),
            PT_KEYWORD, "NONE", (enumeration)SM_NONE, PT_KEYWORD, "HOUSE_HEAT",
            (enumeration)SM_HOUSE_HEAT, PT_KEYWORD, "HOUSE_COOL",
            (enumeration)SM_HOUSE_COOL, PT_KEYWORD, "HOUSE_PREHEAT",
            (enumeration)SM_HOUSE_PREHEAT, PT_KEYWORD, "HOUSE_PRECOOL",
            (enumeration)SM_HOUSE_PRECOOL, PT_KEYWORD, "WATERHEATER",
            (enumeration)SM_WATERHEATER, PT_KEYWORD, "DOUBLE_RAMP",
            (enumeration)SM_DOUBLE_RAMP, PT_enumeration, "bid_mode",
            PADDR(bidmode), PT_KEYWORD, "ON", (enumeration)BM_ON, PT_KEYWORD,
            "OFF", (enumeration)BM_OFF, PT_KEYWORD, "PROXY",
            (enumeration)BM_PROXY, PT_enumeration, "use_override",
            PADDR(use_override), PT_KEYWORD, "OFF", (enumeration)OU_OFF,
            PT_KEYWORD, "ON", (enumeration)OU_ON, PT_double, "ramp_low[degF]",
            PADDR(ramp_low), PT_DESCRIPTION,
            "the comfort response below the setpoint", PT_double,
            "ramp_high[degF]", PADDR(ramp_high), PT_DESCRIPTION,
            "the comfort response above the setpoint", PT_double, "range_low",
            PADDR(range_low), PT_DESCRIPTION,
            "the setpoint limit on the low side", PT_double, "range_high",
            PADDR(range_high), PT_DESCRIPTION,
            "the setpoint limit on the high side", PT_char32, "target",
            PADDR(target), PT_DESCRIPTION,
            "the observed property (e.g., air temperature)", PT_char32,
            "setpoint", PADDR(setpoint), PT_DESCRIPTION,
            "the controlled property (e.g., heating setpoint)", PT_char32,
            "demand", PADDR(demand), PT_DESCRIPTION,
            "the controlled load when on", PT_char32, "load", PADDR(load),
            PT_DESCRIPTION, "the current controlled load", PT_char32, "total",
            PADDR(total), PT_DESCRIPTION, "the uncontrolled load (if any)",
            PT_char32, "market", PADDR(pMkt), PT_DESCRIPTION,
            "the market to bid into", PT_char32, "state", PADDR(state),
            PT_DESCRIPTION, "the state property of the controlled load",
            PT_char32, "avg_target", PADDR(avg_target), PT_char32, "std_target",
            PADDR(std_target), PT_double, "bid_price", PADDR(last_p), PT_ACCESS,
            PA_REFERENCE, PT_DESCRIPTION, "the bid price", PT_double,
            "bid_quantity", PADDR(last_q), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the bid quantity", PT_double, "set_temp[degF]",
            PADDR(set_temp), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION,
            "the reset value", PT_double, "base_setpoint[degF]",
            PADDR(setpoint0),
            // new stuff
            PT_double, "market_price", PADDR(clear_price), PT_DESCRIPTION,
            "the current market clearing price seen by the controller.",
            PT_double, "period[s]", PADDR(dPeriod), PT_DESCRIPTION,
            "interval of time between market clearings", PT_enumeration,
            "control_mode", PADDR(control_mode), PT_KEYWORD, "RAMP",
            (enumeration)CN_RAMP, PT_KEYWORD, "DOUBLE_RAMP",
            (enumeration)CN_DOUBLE_RAMP, PT_KEYWORD, "DEV_LEVEL",
            (enumeration)CN_DEV_LEVEL, PT_KEYWORD, "DOUBLE_PRICE",
            (enumeration)CN_DOUBLE_PRICE, PT_enumeration, "resolve_mode",
            PADDR(resolve_mode), PT_KEYWORD, "DEADBAND",
            (enumeration)RM_DEADBAND, PT_KEYWORD, "SLIDING",
            (enumeration)RM_SLIDING, PT_double, "slider_setting",
            PADDR(slider_setting), PT_double, "slider_setting_heat",
            PADDR(slider_setting_heat), PT_double, "slider_setting_cool",
            PADDR(slider_setting_cool), PT_char32, "override",
            PADDR(re_override),
            // double ramp
            PT_double, "heating_range_high[degF]", PADDR(heat_range_high),
            PT_double, "heating_range_low[degF]", PADDR(heat_range_low),
            PT_double, "heating_ramp_high", PADDR(heat_ramp_high), PT_double,
            "heating_ramp_low", PADDR(heat_ramp_low), PT_double,
            "cooling_range_high[degF]", PADDR(cool_range_high), PT_double,
            "cooling_range_low[degF]", PADDR(cool_range_low), PT_double,
            "cooling_ramp_high", PADDR(cool_ramp_high), PT_double,
            "cooling_ramp_low", PADDR(cool_ramp_low), PT_double,
            "heating_base_setpoint[degF]", PADDR(heating_setpoint0), PT_double,
            "cooling_base_setpoint[degF]", PADDR(cooling_setpoint0), PT_char32,
            "deadband", PADDR(deadband), PT_char32, "heating_setpoint",
            PADDR(heating_setpoint), PT_char32, "heating_demand",
            PADDR(heating_demand), PT_char32, "cooling_setpoint",
            PADDR(cooling_setpoint), PT_char32, "cooling_demand",
            PADDR(cooling_demand), PT_double, "sliding_time_delay[s]",
            PADDR(sliding_time_delay), PT_DESCRIPTION,
            "time interval desired for the sliding resolve mode to change from "
            "cooling or heating to off",
            PT_bool, "use_predictive_bidding", PADDR(use_predictive_bidding),

            // DEV_LEVEL
            PT_double, "device_actively_engaged", PADDR(is_engaged), PT_int32,
            "cleared_market", PADDR(last_market), PT_int32, "locked",
            PADDR(locked), PT_double, "p_ON", PADDR(P_ON), PT_double, "p_OFF",
            PADDR(P_OFF), PT_double, "p_ONLOCK", PADDR(P_ONLOCK), PT_double,
            "p_OFFLOCK", PADDR(P_OFFLOCK), PT_double, "delta_u", PADDR(delta_u),
            PT_char32, "regulation_market_on", PADDR(pMkt2), PT_DESCRIPTION,
            "the willing to change state from ON->OFF market to bid into for "
            "regulation case",
            PT_char32, "regulation_market_off", PADDR(pMkt), PT_DESCRIPTION,
            "the willing to change state from OFF->ON market to bid into for "
            "regulation case",
            PT_double, "fast_regulation_signal[s]", PADDR(fast_reg_signal),
            PT_DESCRIPTION,
            "the regulation signal that the controller compares against (i.e., "
            "how often is there a control action",
            PT_double, "market_price_on", PADDR(clear_price2), PT_DESCRIPTION,
            "the current market clearing price seen by the controller in "
            "ON->OFF regulation market",
            PT_double, "market_price_off", PADDR(clear_price), PT_DESCRIPTION,
            "the current market clearing price seen by the controller  in "
            "OFF->ON regulation market",
            PT_double, "period_on[s]", PADDR(dPeriod2), PT_DESCRIPTION,
            "interval of time between market clearings in ON->OFF regulation "
            "market",
            PT_double, "period_off[s]", PADDR(dPeriod), PT_DESCRIPTION,
            "interval of time between market clearings in  OFF->ON regulation "
            "market",
            PT_int32, "regulation_period", PADDR(reg_period), PT_DESCRIPTION,
            "fast regulation signal period", PT_double, "r1", PADDR(r1),
            PT_DESCRIPTION, "temporary diagnostic variable", PT_double, "mu0",
            PADDR(mu0), PT_DESCRIPTION, "temporary diagnostic variable",
            PT_double, "mu1", PADDR(mu1), PT_DESCRIPTION,
            "temporary diagnostic variable",

            // redefinitions
            PT_char32, "average_target", PADDR(avg_target), PT_char32,
            "standard_deviation_target", PADDR(std_target), PT_int64, "bid_id",
            PADDR(bid_id),
#ifdef _DEBUG
            PT_enumeration, "current_mode", PADDR(thermostat_mode), PT_KEYWORD,
            "INVALID", (enumeration)TM_INVALID, PT_KEYWORD, "OFF",
            (enumeration)TM_OFF, PT_KEYWORD, "HEAT", (enumeration)TM_HEAT,
            PT_KEYWORD, "COOL", (enumeration)TM_COOL, PT_enumeration,
            "dominant_mode", PADDR(last_mode), PT_KEYWORD, "INVALID",
            (enumeration)TM_INVALID, PT_KEYWORD, "OFF", (enumeration)TM_OFF,
            PT_KEYWORD, "HEAT", (enumeration)TM_HEAT, PT_KEYWORD, "COOL",
            (enumeration)TM_COOL, PT_enumeration, "previous_mode",
            PADDR(previous_mode), PT_KEYWORD, "INVALID", TM_INVALID, PT_KEYWORD,
            "OFF", (enumeration)TM_OFF, PT_KEYWORD, "HEAT",
            (enumeration)TM_HEAT, PT_KEYWORD, "COOL", (enumeration)TM_COOL,
            PT_double, "heat_max", PADDR(heat_max), PT_double, "cool_min",
            PADDR(cool_min),
#endif
            PT_int32, "bid_delay", PADDR(bid_delay), PT_char32,
            "thermostat_state", PADDR(thermostat_state), PT_DESCRIPTION,
            "The name of the thermostat state property within the object being "
            "controlled",
            // PROXY PROPERTIES
            PT_double, "proxy_average", PADDR(proxy_avg), PT_double,
            "proxy_standard_deviation", PADDR(proxy_std), PT_int64,
            "proxy_market_id", PADDR(proxy_mkt_id), PT_int64,
            "proxy_market2_id", PADDR(proxy_mkt_id2), PT_double,
            "proxy_clear_price[$]", PADDR(proxy_clear_price), PT_double,
            "proxy_clear_price2[$]", PADDR(proxy_clear_price2), PT_double,
            "proxy_price_cap", PADDR(proxy_price_cap), PT_double,
            "proxy_price_cap2", PADDR(proxy_price_cap2), PT_char32,
            "proxy_market_unit", PADDR(proxy_mkt_unit), PT_double,
            "proxy_initial_price", PADDR(proxy_init_price), PT_double,
            "proxy_marginal_fraction", PADDR(proxy_marginal_fraction),
            PT_double, "proxy_marginal_fraction2",
            PADDR(proxy_marginal_fraction2), PT_double,
            "proxy_clearing_quantity", PADDR(proxy_clearing_quantity),
            PT_double, "proxy_clearing_quantity2",
            PADDR(proxy_clearing_quantity2), PT_double,
            "proxy_seller_total_quantity", PADDR(proxy_seller_total_quantity),
            PT_double, "proxy_seller_total_quantity2",
            PADDR(proxy_seller_total_quantity2), PT_enumeration,
            "proxy_margin_mode", PADDR(proxy_margin_mode), PT_KEYWORD, "NORMAL",
            (enumeration)AM_NONE, PT_KEYWORD, "DENY", (enumeration)AM_DENY,
            PT_KEYWORD, "PROB", (enumeration)AM_PROB, PT_enumeration,
            "proxy_clearing_type", PADDR(proxy_clearing_type), PT_KEYWORD,
            "MARGINAL_SELLER", (enumeration)CT_SELLER, PT_KEYWORD,
            ...
            (enumeration)CT_NULL, PT_enumeration, "proxy_clearing_type2",
            PADDR(proxy_clearing_type2), PT_KEYWORD, "MARGINAL_SELLER",
            (enumeration)CT_SELLER, PT_KEYWORD, "MARGINAL_BUYER",
            (enumeration)CT_BUYER, PT_KEYWORD, "MARGINAL_PRICE",
            (enumeration)CT_PRICE, PT_KEYWORD, "EXACT", (enumeration)CT_EXACT,
            PT_KEYWORD, "FAILURE", (enumeration)CT_FAILURE, PT_KEYWORD,
            "nullptr", (enumeration)CT_NULL, nullptr) < 1)
...
int controller::init(OBJECT *parent) {
  ...
  if (control_mode == CN_RAMP || control_mode == CN_DEV_LEVEL) {
    ...
    if (fetch_property(&pCm, "mass_heat_capacity", parent) == 0)
      return 0;
    if (fetch_property(&pMassInternalGainFraction,
                       "mass_internal_gain_fraction", parent) == 0)
      return 0;
    if (fetch_property(&pMassSolarGainFraction, "mass_solar_gain_fraction",
                       parent) == 0)
      return 0;
    ...
    if (fetch_property(&pTmass, "mass_temperature", parent) == 0)
      return 0;
    if (fetch_property(&pRated_cooling_capacity, "design_cooling_capacity",
                       parent) == 0)
      return 0;
  ...
  if (dir == 0) {
    ...
    if (ramp_low * ramp_high < 0) {
      gl_warning("%s: controller price curve is not injective and may behave "
                 "strangely");
      /* TROUBLESHOOT
              The price curve 'changes directions' at the setpoint, which may
         create odd conditions in a number of circumstances.
       */
    }
  }
  if (setpoint0 == 0)
...
TIMESTAMP controller::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (control_mode == CN_DOUBLE_PRICE) {
    ...
    if (marketId !=
      ...
      // T_set,a = T_set + (P_clear - P_avg) * | T_lim - T_set | / (k_T *
      // stdev24)
      ...
      // gl_verbose("controller::postsync(): temp %f given p %f vs avg
...
TIMESTAMP controller::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  int64 no_bid =
      0; // flag gets set when the current temperature drops in between the the
         // heating setpoint and cooling setpoint curves
  double rampify = 0.0;
  extern double bid_offset;
  double deadband_shift = 0.0;
  double shift_direction = 0.0;
  ...
  if (control_mode == CN_RAMP) {
    ...
    if (demandP > 0 && no_bid != 1) {
      ...
      // lastbid_id = market->submit(object_header(this), -last_q, last_p,
      // bid_id, (BIDDERSTATE)(pState != 0 ? *pState : 0));
    ...
    if (demandP > 0 && no_bid != 1) {
      ...
      if (ps == *PS_ON) {
        ...
        controller_bid2.state = BS_UNKNOWN;
        ((void (*)(char *, char *, const char *, const char *, void *, size_t))(
            *submit2))((char *)gl_name(hdr, ctrname, 1024), (char *)(&pMkt2),
        ...
        if (controller_bid.rebid) {
          ...
          ((void (*)(char *, char *, const char *, const char *, void *,
                     size_t))(*submit))((char *)gl_name(hdr, ctrname, 1024),
                                        (char *)(&pMkt), "submit_bid_state",
        ...
        if (controller_bid2.rebid) {
          ...
          controller_bid2.state = BS_UNKNOWN;
          ((void (*)(char *, char *, const char *, const char *, void *,
                     size_t))(*submit2))((char *)gl_name(hdr, ctrname, 1024),
                                         (char *)(&pMkt2), "submit_bid_state",
    ...
    */
    ...
    if (cool_min - heat_max < dBand) {
      ...
      case RM_SLIDING:
        ...
        break;
      default:
        gl_error("unrecognized resolve_mode when double_ramp overlap "
    ...
    if (lastmkt_id != marketId) {
      ...
      if (clear_price == last_p) {
        ...
        case CT_NULL:  // q zero or logic error ~ should not occur
          ...
          // gl_warning("clearing price and bid price are equal with a market
          // clearing type that involves inequal prices");
          break;
        default:
          break;
        }
...
TIMESTAMP controller::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (control_mode == CN_DOUBLE_PRICE) {
    if (t1 == next_run - bid_delay) {
      ...
      double bid = -1.0;
      int64 no_bid =
          0; // flag gets set when the current temperature drops in between the
             // the heating setpoint and cooling setpoint curves
      double pCap = 0.0;
      double deadband_shift = 0.0;
      double dBand = 0.0;
      double avgP = 0.0;
    ...
    if (heatState == 1 || auxState == 1) {
      ...
      if (resolve_mode == RM_SLIDING)
        return TS_INVALID; // If the HVAC is in two modes at once then the
                           // sliding resolve mode will have conflicting state
                           // input so stop the simulation.
    }
  }
        ...
        // Ensure that it stays in the position we have decided on
          ...
          if (use_override == OU_ON) {
            if (locked == 1)
              ...
              override_prop.setp(
                  OV_OFF
                      ->get_enumeration_value()); // keep it in the OFF position
          }
        }
...
double controller::calcTemp2_double_price(
  ...
  double Qa_OFF = ((1 - MassInternalGainFraction) * Qi) +
                  ((1 - MassSolarGainFraction) * Qs);
  double Qa_ON = Qh + ((1 - MassInternalGainFraction) * Qi) +
                 ((1 - MassSolarGainFraction) * Qs);
