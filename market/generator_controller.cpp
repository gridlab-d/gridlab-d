...
TIMESTAMP generator_controller::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	int index;
	int64 bidID_val;
	double per_phase_power, per_phase_old_power;
	...
	if (last_market_id != *curr_market_id)
	{
		...
		//If timestamps don't match, copy in current bid curve
		if (bid_curve_values[latency_write_section].bid_curve_parsed != bid_curve_current.bid_curve_parsed)
			copy_bid_curve(obj);
