...
generator_controller::generator_controller(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "generator_controller",
                               sizeof(generator_controller),
                               PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);

    if (gl_publish_variable(
            oclass, PT_enumeration, "generator_state", PADDR(gen_state),
            PT_DESCRIPTION, "Current generator state", PT_KEYWORD, "OFF",
            (enumeration)GEN_OFF, PT_KEYWORD, "RUNNING",
            (enumeration)GEN_ACTIVE, PT_KEYWORD, "STARTING",
            (enumeration)GEN_STARTUP, PT_enumeration, "amortization_type",
            PADDR(amort_type), PT_DESCRIPTION,
            "Amortization compounding method", PT_KEYWORD, "EXPONENTIAL",
            (enumeration)EXPONENTIAL, PT_KEYWORD, "LINEAR_COST",
            (enumeration)LINEAR_COST, PT_KEYWORD, "LINEAR_BID",
            (enumeration)LINEAR_BID, PT_int32, "generator_state_number",
            PADDR(gen_state_num), PT_DESCRIPTION,
            "Current generator state as numeric value", PT_object, "market",
            PADDR(market_object), PT_DESCRIPTION,
            "Market the object will watch and bid into", PT_char1024,
            "bid_curve", PADDR(bidding_curve_txt), PT_DESCRIPTION,
            "Bidding curve text format", PT_char1024, "bid_curve_file",
            PADDR(bidding_curve_file), PT_DESCRIPTION,
            "Bidding curve file name", PT_double, "bid_generator_rating[VA]",
            PADDR(bid_gen_rating), PT_DESCRIPTION,
            "Size of generator in VA for the bid curve", PT_bool,
            "update_curve", PADDR(update_curve), PT_DESCRIPTION,
            "Flag to force updating of bidding curve parse", PT_bool,
            "is_marginal_gen", PADDR(is_marginal_gen), PT_DESCRIPTION,
            "Flag to indicate if the generator is a marginal generator",
            PT_double, "generator_rating[VA]", PADDR(gen_rating),
            PT_DESCRIPTION, "Size of generator in VA for the active bid",
            PT_double, "generator_output", PADDR(current_power_output),
            PT_DESCRIPTION,
            "Current real power output of generator", // Units removed for now
                                                      // -For now, "market
                                                      // units" is assumed as
                                                      // MWh - TODO: this needs
                                                      // to be fixed as part of
                                                      // ticket #574 */
            PT_double, "input_unit_base[MW]", PADDR(input_unit_base),
            PT_DESCRIPTION,
            "Base multiplier for generator bid units", // This should become
                                                       // irrelevant if #574 is
                                                       // fixed
            PT_double, "startup_cost[$]", PADDR(startup_cost), PT_DESCRIPTION,
            "Cost to start the generator from OFF status", PT_double,
            "shutdown_cost[$]", PADDR(shutdown_cost), PT_DESCRIPTION,
            "Cost to shut down the generator prematurely", PT_double,
            "minimum_runtime[s]", PADDR(minimum_runtime_dbl), PT_DESCRIPTION,
            "Minimum time the generator should run to avoid shutdown cost",
            PT_double, "minimum_downtime[s]", PADDR(minimum_downtime_dbl),
            PT_DESCRIPTION,
            "Minimum down time for the generator before it can bid again",
            PT_double, "capacity_factor", PADDR(capacity_factor),
            PT_DESCRIPTION,
            "Calculation of generator's current capacity factor based on the "
            "market period",
            PT_double, "amortization_factor[1/s]", PADDR(amortization_value),
            PT_DESCRIPTION,
            "Exponential decay factor in 1/s for shutdown cost repayment",
            PT_double, "bid_delay", PADDR(bid_delay_dbl), PT_DESCRIPTION,
            "Time before a market closes to bid", PT_enumeration,
            "generator_attachment", PADDR(gen_mode), PT_DESCRIPTION,
            "Generator attachment type - determines interactions", PT_KEYWORD,
            "STANDALONE", (enumeration)STANDALONE, PT_KEYWORD, "BUILDING",
            (enumeration)BUILDING, PT_double, "building_load_curr",
            PADDR(curr_building_load), PT_DESCRIPTION,
            "Present building load value (if BUILDING attachment)", PT_double,
            "building_load_bid", PADDR(bid_building_load), PT_DESCRIPTION,
            "Expected building load value for currently bidding market period "
            "(if BUILDING attachment)",
            PT_double, "year_runtime_limit[h]", PADDR(total_hours_year),
            PT_DESCRIPTION,
            "Total number of hours the generator can run in a year", PT_double,
            "current_year_runtime[h]", PADDR(hours_run_this_year),
            PT_DESCRIPTION, "Total number of hours generator has run this year",
            PT_char1024, "runtime_year_end", PADDR(runtime_year_end),
            PT_DESCRIPTION, "Date and time the generator runtime year resets",
            PT_double, "scaling_factor[unit]", PADDR(scaling_factor),
            PT_DESCRIPTION,
            "scaling factor applied to license premium calculation", PT_double,
            "license_premium", PADDR(license_premium), PT_DESCRIPTION,
            "current value of the generator license premium calculated",
            PT_double, "hours_in_year[h]", PADDR(hours_in_year), PT_DESCRIPTION,
            "Number of hours assumed for the total year", PT_double,
            "op_and_maint_cost[$]", PADDR(op_and_maint_cost), PT_DESCRIPTION,
            "Operation and maintenance cost per runtime year", PT_int64,
            "bid_id", PADDR(bid_id), nullptr) < 1)
...
int generator_controller::create(void) {
  ...
  runtime_hours_left =
      true; // Generator is assumed to have life left by default
  ...
  update_output = false;
  curve_file_mode =
      false; // By default, assumes we're parsing a text field, not a file
  ...
  capacity_factor = 0.0;
  amortization_value =
      6.0; // 1/2 in 5 minutes - gets translated by market intervals later
  amortization_scaled = 1.0; // Initial value
  shutdown_current_cost = 0.0;
  bid_delay_dbl = 0.0; // Start with 1 second delay
  bid_delay = 0; // Starts off just being one second before next (gets over
                 // written anyways)
  ...
  input_unit_base = 1.0; // Input base is assumed to be MW, in line with what
                         // the typical market it (part of ticket #574)
  unit_scaling_value = 1000000.0; // Multiplier for bids to get from "input" to
                                  // load posting value
...
int generator_controller::init(OBJECT *parent) {
  ...
  if (parent == nullptr) // No parent >:|
  {
    ...
    /*  TROUBLESHOOT
    The genererator_control object must be parented to a load or triplex_node in
    powerflow at this time.  It had no parent.  Please specify a parent and try
    again.
    */
  }
  if (gl_object_isa(parent, "load",
                    "powerflow")) // Parent is a normal three-phase load
  {
    ...
    if ((ptemp == nullptr) || (ptemp->ptype != PT_complex)) {
      GL_THROW("generator_controller:%s failed to map the power property of "
               "the parent device",
               obj->name);
      /*  TROUBLESHOOT
      The generator_controller object failed to find the proper variables for
      mapping the power values.
      */
    }
    ...
    if ((ptemp == nullptr) || (ptemp->ptype != PT_set)) {
      GL_THROW("generator_controller:%s failed to map the power property of "
               "the parent device",
               obj->name);
      // Defined above - technically different, but it is part of the same idea,
      // so meh
    }
    ...
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("generator_controller::init(): deferring initialization on %s",
  ...
  } else if (gl_object_isa(parent, "triplex_node",
                           "powerflow")) // Parent is a triplex node
  {
    // Grab the link to the constant_power properties
  ...
  } else                      // Invalid parent
  {
    ...
    /*  TROUBLESHOOT
    The genererator_control object must be parented to a load or triplex_node in
    powerflow at this time.  Please specify a valid parent and try again.
    */
  }
  ...
  if (market_object == nullptr) {
    ...
    /*  TROUBLESHOOT
    A generator_controller appears to be missing a market tie.  Please specify a
    valid market object and try again.  If the error persists, please submit
  ...
  if ((market_object->flags & OF_INIT) != OF_INIT) {
    char objname[256];
    gl_verbose("generator_controller::init(): deferring initialization on %s",
               gl_name(market_object, objname, 255));
    return 2; // defer
  }
  // Get this object
  ...
  price_cap_value =
      auction_object->pricecap +
      0.01; // Set to 0.01 higher - used as an initialization variable
  ...
  if (submit == nullptr) {
    ...
    gl_error("Unable to find function, submit_bid_state(), for object %s.",
             (char *)gl_name(market_object, buf, 255));
    return 0;
  }

  // Determine number of latency slots we need
  number_latency_sections = (int)((market_latency / market_period) +
                                  2); // Delay # + current bid + current active
  ...
  if (gen_rating > 0.0) // A value is in here
  {
    ...
    {
      gl_warning("Generator_rating and bid_generator_rating both have values "
                 "in generator_controller:%s",
                 obj->name);
      /*  TROUBLESHOOT
      Both the generator_rating and bid_generator_rating have non-zero values at
      the start of the simulation.  bid_generator_rating will eventually
      override generator_rating, so please ensure your values are entered
      appropriately.
      */
    }
  }
  // Defaulted else - gen rating is invalid, so hopefully bid_gen_rating is
  // right (otherwise next catch gets it)

  // Make sure the rating is non-negative and non-zero
  if (bid_gen_rating <= 0) {
    GL_THROW("Generator rating in generator_controller:%s must be positive!",
             obj->name);
    /*  TROUBLESHOOT
    The generator size specified for the current bid generator_controller must
    be a positive, non-zero value.  Please try again.
    */
  }

  // Do the initial latency mallocing
  bid_curve_values =
      (CURVEENTRY *)gl_malloc(number_latency_sections * sizeof(CURVEENTRY));

  // Make sure it worked
  if (bid_curve_values == nullptr) {
    GL_THROW("Failed to allocate bid_curve memory in %s", obj->name);
    /*  TROUBLESHOOT
    While attempting to allocate memory for the generator_controller bid curve,
  ...
  for (index = 0; index < number_latency_sections; index++) {
    ...
    bid_curve_values[index].expected_state = (GENERATOR_STATE)
        gen_state; // Initial bidding needs to know how we are running - assume
                   // it populates all the way back
    bid_curve_values[index].generator_capacity =
        0.0; // Initially has no capacity
    bid_curve_values[index].expected_building_load =
        0.0; // Assume no building load - either not needed, or is just zeroed
  }

  // Set initial pointer values appropriately (technically messes up the
  // indexing, but it gets fixed after 1 latency cycle)
  latency_write_section = (number_latency_sections - 1);
  latency_read_section = 0;

  // Allocate the first "old" value, just for the sake of doing so (so it
  // doesn't fail on first run)
  bid_curve_values[latency_write_section].Curve_Info =
      (CURVEDETAILS *)gl_malloc(sizeof(CURVEDETAILS));

  // Make sure it worked
  if (bid_curve_values[latency_write_section].Curve_Info == nullptr) {
    GL_THROW("generator_controller:%s failed to allocate the initial bid curve "
             "memory",
             obj->name);
    /*  TROUBLESHOOT
    While attempting to preallocate the memory for the temporary bid curve, an
  ...
  if (minimum_runtime_dbl <= 0.0) {
    GL_THROW("generator_controller:%s must have a positive, non-zero "
             "minimum_runtime!",
             obj->name);
    /*  TROUBLESHOOT
    To properly bid into the market, generator_controller objects must have a
    minimum_runtime that is greater than zero.  If no shutdown effects are
    desired, set the shutdown_cost to 0.  minimum_downtime must always be
    populated correctly.
    */
  }
  ...
  if (minimum_downtime_dbl < 0.0) {
    GL_THROW(
        "generator_controller:%s must have a non-negative minimum_downtime!",
        obj->name);
    /*  TROUBLESHOOT
    To properly bid into the market, generator_controller objects must have a
    minimum_downtime that is positive or zero.
    */
  }
  ...
  // See if a minimum timestep is in place - if so, make sure bid delay is set
  // right Retrieve the global value, only does so as a text string for some
  // reason
  ...
  if (glob_min_timestep > 1) // Now check us
  {
    ...
    if ((bid_delay != 0) && (bid_delay < (TIMESTAMP)(glob_min_timestep))) {
      ...
      // Let user know
      gl_warning("bid_delay was less than the current minimum_timestep setting "
                 "- rounded up to one whole timestep");
      /*  TROUBLESHOOT
      The time for bid_delay was less than the value set for minimum_timestep in
      the GLM.  It was rounded to be at least one minimum_timestep in value.  If
      this is not desired, fix your bid delay or minimum_timestep
      */
    }
  }
  ...
  if (bid_delay > market_period) {
    ...
    /*  TROUBLESHOOT
    The value for bid_delay is larger than the period of the market
    generator_controller is bidding into. Adjust one or the other and try again.
    */
  }

  // Update the previous state variable - just to be safe
  if (gen_state != GEN_ACTIVE) {
    prev_bid_state = GEN_OFF; // Assumes we were shutdown - if are OFF, were
                              // OFF.  If are starting, were OFF
  }
  ...
  if (total_hours_year != 0.0) {
    ...
    if (total_hours_year < 0.0) {
      GL_THROW("The yearly runtime limit a generator can run must be a "
               "positive number");
      /*  TROUBLESHOOT
      The value for yearly_runtime_limit was a negative value.  Please set it to
      zero (infinite runtime) or a positive value.
      */
    } else // Good value
    {
      ...
      if (hours_run_this_year < 0.0) {
        GL_THROW(
            "The current year's runtime must be a positive number, or zero");
        /*  TROUBLESHOOT
        The value for current_year_runtime is a negative number.  It must be
        zero or positive.
        */
      } else // Non-negative
      {
        if (hours_run_this_year >= total_hours_year) {
          gl_warning("The generator has already exhausted its yearly runtime "
                     "quota, it will not run this year");
          /*  TROUBLESHOOT
          The value specified for current_year_runtime is larger or equal to
          yearly_runtime_limit.  This means the generator has exhausted all of
          its available runtime hours for the year and will no longer run.
          */
        ...
        } else                        // Value in range
        {
          ...
          if (hours_in_year <= 0.0) {
            GL_THROW("The number of hours in a year must be a positive, "
                     "non-zero number");
            /*  TROUBLESHOOT
            The hours_in_year value must be a positive, non-negative number
            (typically 8760).  This value is the calendar year, not the runtime
            year.
            */
          } else // Valid
          {
            ...
            if (runtime_year_end[0] == '\0') // Empty
            {
              ...
              /*  TROUBLESHOOT
              The generator_controller can not properly calculate the license
              premium unless the value for the end of the run time year is
              specified in runtime_year.  Specify this in normal GridLAB-D
              date/time syntax. e.g., "2007-04-30 00:00:00 PST" for April 30,
              2007 at midnight PST
              */
            } else // Entry there, see if it works
            {
              ...
              if (end_of_runtime_year <= gl_globalclock) {
                ...
                /*  TROUBLESHOOT
                An error occurred while converting the value in runtime_year_end
                into a date.  Ensure you have written a proper date and have not
                specified a value in the simulation's past.
                */
              }
      ...
      // Compute O&M per "generator unit hour" - done by "hours per year allowed
      // to run"
      if (bid_gen_rating != 0.0) // Make sure it isn't zero - if it is, we can't
                                 // calculate anything
      {
        op_maint_cost_pwr =
            op_and_maint_cost / bid_gen_rating / total_hours_year;
      } else {
        op_maint_cost_pwr = 0.0; // Assumes no O&M;

        gl_warning(
            "No value for gen_rating set, assuming $0.0 Op and Maint cost");
        // Defined below
      }
  ...
  else // Total year hours is zero = infinite
  {
    ...
    if (hours_in_year > 0.0) {
                                 ...
                                 // calculate anything
      {
        ...
        /*  TROUBLESHOOT
        While attempting to convert the operations and maintenance annual cost
        into an energy cost, the generator rating was not set.  If this is not
        correct, please specify a value for bid_gen_rating.
        */
      }
    ...
    } else // Failure
    {
      ...
      /*  TROUBLESHOOT
      The hours_in_year value must be a positive or zero number (typically 8760)
      for computing infinite-runtime O&M costs.
      */
    }
  ...
  if (input_unit_base <= 0.0) {
    ...
    /*  TROUBLESHOOT
    The value of input_unit_base must be a positive, non-zero number.  Please
    specify it as such.
    */
  }
...
TIMESTAMP generator_controller::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  double per_phase_power, per_phase_old_power, shutdown_cost_temp,
      prev_section_power, prev_section_price, temp_time_var, temp_power_value;
  char mktname[1024];
  char ctrname[1024];
  ...
  if (total_hours_year != 0.0) {
    ...
    if (t1 != last_update_time) {
      ...
      if ((gen_state == GEN_ACTIVE) || (gen_state == GEN_STARTUP)) {
        hours_run_this_year +=
            temp_time_var / 3600.0; // Accumulate this run time
      }
  ...
  // If market changes - update our output appropriately - update tracking
  // variables for market bidding as well
  if (bid_market_id != *curr_market_id) {
    // Populate the previous state - check against 0 bid
    if (auction_object->cleared_frame.clearing_price >=
        bid_curve_values[latency_write_section]
            .Curve_Info[0]
            .price) // Price met
    {
      // See if it was a startup bid - if so, flag it as running now
      if (bid_curve_values[latency_write_section].expected_state ==
          GEN_STARTUP) {
        // Change over to running - otherwise subsequent bids might get a little
        // squirrely
        bid_curve_values[latency_write_section].expected_state = GEN_ACTIVE;

        // Update our amortization value - only read this each time the
        // generator kicks on
        if (amort_type == EXPONENTIAL) {
          amortization_scaled =
              exp(-1.0 * amortization_value * (double)(market_period));
    ...
    else if (bid_curve_values[latency_write_section].expected_state ==
             GEN_ACTIVE) // Was on, but didn't meet continued running
                         // requirements
    {
      ...
      minimum_downtime =
          market_bidding_time + (TIMESTAMP)(minimum_downtime_dbl);
    ...
    // Flag an update to the output - if the market changed, the "latencied"
    // active market has to, so recompute the output
    update_output = true;

    // if we've exceeded runtime limits, flag us as off, just to be safe (should
    // already be there)
    ...
    if (gen_mode == BUILDING)
      bid_curve_values[latency_write_section].expected_building_load =
          bid_building_load;
  ...
  if ((last_market_id != *curr_market_id) && (t1 >= next_bid)) {
    ...
    // Copy in the new bid curve - ALWAYS do this - since startup and such costs
    // may factor in
    copy_bid_curve(obj);
    ...
    if (total_hours_year == 0.0) // Infinite run
      license_premium = 1.0;
    else if (total_hours_year >
             hours_run_this_year) // Track - but still have life left
    {
      temp_time_var = (double)(end_of_runtime_year -
                               t1); // See how much time is left in the year

      // Make sure we have enough runtime to continue
      if (((total_hours_year - hours_run_this_year) * 3600.0) >=
          (double)(market_period)) {
        // Update the license premium
        license_premium = scaling_factor * temp_time_var / seconds_in_year *
                          total_hours_year /
                          (total_hours_year - hours_run_this_year);
      } else // Not enough for another period, shut us down
      {
        license_premium = 0.0;
        runtime_hours_left =
            false; // Flag us as done - just in case was missed above
      }
    } else // No life left
    {
    ...
    if (prev_bid_state == GEN_OFF) // Generator was off or just went through a
                                   // startup attempt - needs a start up value
    {
      // See if we're allowed to even try
      if (runtime_hours_left == true) {
        // Figure out how many market periods we need to run for - regardless of
        // if we need to or not
        min_num_run_periods =
            (int)((TIMESTAMP)(minimum_runtime_dbl) / market_period);

        // If it is zero - set costs appropriately
        if (min_num_run_periods < 1) {
          shutdown_cost_remaining =
              0.0; // No costs for mininum run time - if we clear, we met it
          shutdown_current_cost = 0.0;
        } else // Normal operation
        {
          if (amort_type == LINEAR_BID) {
            // Determine the startup cost and amortization constant - base on
            // shutdown cost (repay over full interval) Put into the tracking
            // variable
            shutdown_current_cost =
                shutdown_cost * 2.0 / ((double)(min_num_run_periods));

            // Put into amortization variable for future use
            amortization_scaled =
                shutdown_current_cost / ((double)(min_num_run_periods + 1));
          ...
          // Ensure the shutdown cost remaining is set to the full cost for
          // these bids - all types use this the same
          shutdown_cost_remaining = shutdown_cost;
        }

        // See if we've been off long enough - correct for the minimum shutdown
        // time
        if (market_bidding_time >= minimum_downtime) {
          ...
          for (index = 0; index < bid_curve_values[latency_write_section]
                                      .number_bid_curve_sections;
               index++) {
            // Add in the O&M Cost value
            bid_curve_values[latency_write_section].Curve_Info[index].price +=
                op_maint_cost_pwr;
            ...
            if (min_num_run_periods == 0) {
              // Apply offset to all portions of the bid - FC + SU - special
              // case, no shutdown for no minimum run time
              bid_curve_values[latency_write_section].Curve_Info[index].price +=
                  startup_cost;
            } else {
        ...
        } else // Still needs to be off
        {
          ...
          // Set all bids to the price cap value, +4 (just cause) - won't bid
          // in, but this will help ensure nothing wierd happens
          for (index = 0; index < bid_curve_values[latency_write_section]
                                      .number_bid_curve_sections;
               index++) {
      ...
      else // Runtime exceeded
      {
        ...
        // Set all bids to the price cap value, +4 (just cause) - won't bid in,
        // but this will help ensure nothing wierd happens
        for (index = 0;
             index <
             bid_curve_values[latency_write_section].number_bid_curve_sections;
             index++) {
    ...
    else if (prev_bid_state == GEN_ACTIVE) // Running, bid appropriately
    {
      if (runtime_hours_left == true) // We can still bid
      {
        ...
        if (num_runs_completed < min_num_run_periods) {
          ...
          // Make sure we won't go negative on shutdown cost - if so, just bid
          // the difference
          if (shutdown_current_cost > shutdown_cost_remaining) {
            // Just bid in the remainder - basically like below
            shutdown_cost_temp =
                shutdown_cost_remaining;   // Last bit goes in on this bid
          ...
          } else                           // Normal process, so bid like normal
          {
            ...
            shutdown_cost_remaining -=
                shutdown_current_cost; // Decrement appropriately
          }
        } else if (num_runs_completed ==
                   min_num_run_periods) // Right at the mininum number, just put
                                        // the rest of our bid in
        {
          shutdown_cost_temp =
              shutdown_cost_remaining;   // Last bit goes in on this bid
          shutdown_cost_remaining = 0.0; // No more penalty remains to offset
      ...
      GL_THROW("generator_controller:%s invalid generator state encountered",
               obj->name);
      /*  TROUBLESHOOT
      While attempting to bid into the market, the generator_controller found
      itself in an unknown state. Please try again.  If the error persists,
    ...
    if (bid_curve_values[latency_write_section].valid_bid_period == true) {
      if (gen_mode == STANDALONE) {
        ...
        for (index = 0;
          ...
          /* TODO: this needs to be fixed as part of ticket #574 */
          temp_power_value = bid_curve_values[latency_write_section]
                                 .Curve_Info[index]
                                 .power_delta *
                             input_unit_base;

          controller_bid.market_id = *curr_market_id;
          controller_bid.price =
              bid_curve_values[latency_write_section].Curve_Info[index].price;
          ...
          ((void (*)(char *, char *, const char *, const char *, void *,
                     size_t))(*submit))(
              (char *)gl_name(obj, ctrname, 1024),
              (char *)gl_name(market_object, mktname, 1024), "submit_bid_state",
          ...
          // Store the bidID
          bid_curve_values[latency_write_section].Curve_Info[index].lastbid_id =
              controller_bid.bid_id;
        }
      } // End Standalone bidding mode
      else // Must be building mode
      {
        // Assumes there's only 1 bidding section - if more, this doesn't work
        if (bid_curve_values[latency_write_section].number_bid_curve_sections >
            1) {
          GL_THROW("BUILDING mode only supports single section fuel curves "
                   "right now");
          /*  TROUBLESHOOT
          When operating generator_controller in BUILDING mode, it can only take
          a single-value fuel curve.  If the fuel curve has more than one
          section, generator_controller can't handle it properly and would do
          odd things.  This may be fixed in future releases.
          */
        } // More than 1
        else // Only 1
        {
          ...
          /* TODO: this needs to be fixed as part of ticket #574 */
          // temp_power_value =
          // bid_curve_values[latency_write_section].Curve_Info[0].power_delta*input_unit_base;
          temp_power_value =
              bid_curve_values[latency_write_section].expected_building_load *
              input_unit_base;
          ...
          controller_bid.market_id = *curr_market_id;
          controller_bid.price =
              bid_curve_values[latency_write_section].Curve_Info[0].price;
          controller_bid.quantity = -temp_power_value;
          if (gen_state == GEN_OFF) {
            controller_bid.state = BS_ON;
          } else {
          ...
          // Store the bidID
          bid_curve_values[latency_write_section].Curve_Info[0].lastbid_id =
              controller_bid.market_id;
  ...
  if (update_output == true) // Calculate a new value
  {
    ...
    if (gen_mode == BUILDING)
      curr_building_load =
          bid_curve_values[latency_read_section].expected_building_load;
    ...
    if (gen_state == GEN_OFF) {
      ...
      if (bid_curve_values[latency_read_section].valid_bid_period == true) {
        ...
        for (index = 0;
             index <
             bid_curve_values[latency_read_section].number_bid_curve_sections;
             index++) {
          // See if the price for this section was cleared
          if (bid_curve_values[latency_read_section].Curve_Info[index].price <=
              auction_object->current_frame.clearing_price) {
            // For now, "market units" is assumed as MWh
            /* TODO: this needs to be fixed as part of ticket #574 */
            current_power_output += bid_curve_values[latency_read_section]
                                        .Curve_Info[index]
                                        .power_delta;

            // Update the last value valid - used for marginal quantities
            prev_section_power = bid_curve_values[latency_read_section]
                                     .Curve_Info[index]
                                     .power_delta;
            prev_section_price =
                bid_curve_values[latency_read_section].Curve_Info[index].price;
          }
        }
        ...
        if (current_power_output != 0.0) {
          ...
          if (prev_section_price ==
              auction_object->current_frame.clearing_price) {
            // See if it was a marginal seller condition
            if (auction_object->cleared_frame.clearing_type == CT_SELLER) {
              // Figure out our contribution (base on ratio of marginal - in
              // case there is more than one of us) - subtract off the extra
              current_power_output -=
                  prev_section_power *
                  (1 - auction_object->cleared_frame.marginal_frac);
      ...
      if (gen_mode == STANDALONE) {
        ...
        if (bid_curve_values[latency_read_section].valid_bid_period == true) {
          ...
          for (index = 0;
               ...
               index++) {
            // See if the price for this section was cleared
            if (bid_curve_values[latency_read_section]
                    .Curve_Info[index]
                    .price <= auction_object->current_frame.clearing_price) {
              current_power_output += bid_curve_values[latency_read_section]
                                          .Curve_Info[index]
                                          .power_delta;
              ...
              prev_section_power = bid_curve_values[latency_read_section]
                                       .Curve_Info[index]
                                       .power_delta;
              prev_section_price = bid_curve_values[latency_read_section]
                                       .Curve_Info[index]
                                       .price;
            } // end
          }

          // See if our output is non-zero.  If it isn't, we just turned
          // ourselves off
          if (current_power_output == 0.0) {
            gen_state = GEN_OFF;
          } else // Valid output
      ...
      else // Must be building mode
      {
        ...
        if (bid_curve_values[latency_read_section].valid_bid_period == true) {
          ...
          if (current_power_output >
              curr_building_load) // If bigger, it's "marginal" to the building
                                  // load
    ...
    if (gen_state != bid_curve_values[latency_read_section].expected_state) {
      ...
      gl_warning("generator_controller:%s entered an unexpected state - "
                 "results may be invalid!",
                 obj->name);
      /*  TROUBLESHOOT
      While determining the output state for the generator, an unexpected
      condition was encountered (it was turned on when it expected to be off, or
      vice-versa).  This means all subsequent bids are suspect or incorrect.
      Please ensure nothing has changed cleared markets before the latency
      period expired.  If this message persists, please submit your code and a
    ...
    } else // If "no output" (easy way to turn a generator off), just set
           // capacity factor to zero
    {
      capacity_factor = 0.0;
    }
  } // End update output
  ...
  if (gen_mode == BUILDING) {
    if (gen_state !=
        GEN_OFF) // Generator is starting/running - building goes away -
                 // generator assumed to be handling everything
    {
      current_power_output = 0.0;
    } else // Generator off - implies full building load still there - negative
           // to make it load (posted as generation)
    {
      current_power_output = -curr_building_load;
    }
  }
  ...
  /* TODO: this may need to be fixed as part of ticket #574 - if bidding units
   * get fixed, these may need fixing as well */
  ...
  // Apply it appropriately - "added" as negative load - remove old contribution
  // as well (way being applied causes it to just accumulate forever)
...
void generator_controller::parse_bid_curve(OBJECT *thisobj, TIMESTAMP t0) {
  ...
  char temp_char_value[33]; // Assumes no number in the bid curve will ever be
                            // over 32 characters
  FILE *FHandle;
  ...
  {
    // Set the very end character of the temp variable - terminator for various
    // things
    temp_char_value[32] = '\0';

    // Make sure the file field isn't empty - just in case it is being "played"
    // in
    if (bidding_curve_file[0] == '\0') {
      GL_THROW("generator_controller:%s has an empty bid curve filename!",
               thisobj->name);
      /*  TROUBLESHOOT
      The bid_curve_file property of the generator_controller object is empty.
      If the controller is reading the bid curve from a file, it can only
      operate in that mode for the full duration of the simulation.  Swapping
      modes mid-simulation is not allowed at this time.
      */
    }
    ...
    if (FHandle == nullptr) {
      GL_THROW(
          "generator_controller:%s has an invalid bid_curve_file specified!",
          thisobj->name);
      /*  TROUBLESHOOT
      The generator_controller was unable to open the CSV containing the bid
      curve information.  Please ensure the file is pointed to properly and is a
      valid file and try again.  If the error persists, please submit your code
      and bid curve file in a bug report via the trac website.
      */
    }
    ...
    if (num_delims == 0) {
      ...
      // Toss the error
      GL_THROW("Bid curve file is either empty or improperly formatted in "
               "generator_controller:%s",
               thisobj->name);
      /*  TROUBLESHOOT
      While parsing the bid curve file, the generator_controller did not find
      any commas.  Either the file is empty, or is not formatted correctly.
      Please ensure the file is correct and try again.  If the error persists,
      please submit your code and a bug report via the trac website.
      */
    }

    // Set the number of entries - it is the same as the number of delimiters
    // minus 1
  ...
  else // Text field parsing
  {
    ...
    if (bidding_curve_txt[0] == '\0') {
      GL_THROW("generator_controller:%s has an empty bid curve!",
               thisobj->name);
      /*  TROUBLESHOOT
      The bid_curve property of the generator_controller object is empty. Please
      specify it as pairs of power values and prices.
      */
    }
    ...
    while ((index < 1025) && (bidding_curve_txt[index] != '\0')) {
      if (bidding_curve_txt[index] == ' ') {
        if (last_was_space == true) {
          GL_THROW("generator_controller:%s has two spaces in part of its bid "
                   "curve - please fix so it is single-space delineated!",
                   thisobj->name);
          /*  TROUBLESHOOT
          The bid_curve specified has to be delineated by single spaces.  Two
          spaces were found between one item.  Please fix them and try again.
          */
        } else // Not true
        {
    ...
    num_entries = (num_delims >> 1);
    if ((num_delims - (num_entries << 1)) !=
        1) // Must be odd, since odd number of spaces needed to separate even
           // number of numbers
    {
      GL_THROW("generator_controller:%s has an improperly formatted bid_curve",
               thisobj->name);
      /*  TROUBLESHOOT
      While parsing the bid_curve property of the generator object, an error was
      encountered.  Please ensure all values are valid numbers and that they
      occur in proper x-value and y-value pairs.
      */
    }
  } // End text field initial parsing

  // Start with the assumption that we need 2 more than indicated (for end
  // point) - we'll allocate that and if we don't need it, change the limit size
  number_bid_curve_sections =
      num_entries + 2; // 1 is just base (1 space becomes 0, which is 1 set) and
                       // 1 is the addition mentioned

  // See if a realloc is needed
  if ((bid_curve_current.Curve_Info != nullptr) &&
      (bid_curve_current.number_bid_curve_sections !=
       number_bid_curve_sections)) {
    // Free the current one
    gl_free((void **)&bid_curve_current.Curve_Info);
  ...
  if (bid_curve_current.Curve_Info == nullptr) {
    // Malloc the new one
    bid_curve_current.Curve_Info = (CURVEDETAILS *)(gl_malloc(
        number_bid_curve_sections * sizeof(CURVEDETAILS)));

    // Make sure it worked
    if (bid_curve_current.Curve_Info == nullptr) {
      GL_THROW("generator_controller:%s failed to map memory for the bid curve "
               "parsing.",
               thisobj->name);
      /*  TROUBLESHOOT
      While attempting to allocate memory for the current bid curve
      implementation, a memory allocation error occurred.  Please try again.  If
  ...
  // Initialize to "off" position as well - this shouldn't be needed, but let's
  // be paranoid
  ...
  if (curve_file_mode == true) // File mode
  {
    ...
    while (result_val > 0) {
      ...
      } else // Not a comma, store it
      {
        ...
        if (temp_index > 31) {
          ...
          32 characters long was found.  Please truncate this number and try
          again.
          */
        }
      }
    ...
    for (index = 0; index < (num_entries + 1); index++) {
      ...
      if ((index == 0) &&
          (temp_double == 0.0)) // First bid and zero - basically we're off
      {
        // Zero full items
        bid_curve_current.Curve_Info[index].power_stop = 0.0;
        bid_curve_current.Curve_Info[index].price = 0.0;
      ...
      {
        GL_THROW("generator_controller:%s does not have a monotonically "
      ...
      // Populate the delta
      bid_curve_current.Curve_Info[index].power_delta =
          bid_curve_current.Curve_Info[index].power_stop -
          bid_curve_current.Curve_Info[index].power_start;
    ...
    for (index = 0; index < (num_entries + 1); index++) {
      ...
      if (plast >= temp_double) // Same or greater, implies wrong direction!
      {
        GL_THROW("generator_controller:%s does not have a monotonically "
                 ...
                 thisobj->name);
        /*  TROUBLESHOOT
        A bid curve in the generator_controller object must have incrementing
        price points.  If they are the same, or negative, the bidding process
        will fail.  Please fix this error and try again.
        */
      }
  ...
  if (bid_curve_current.Curve_Info[num_entries].power_stop < bid_gen_rating) {
    ...
    bid_curve_current.Curve_Info[(num_entries + 1)].power_start =
        bid_curve_current.Curve_Info[num_entries]
            .power_stop; // Last point is our start
    bid_curve_current.Curve_Info[(num_entries + 1)].power_stop =
        bid_gen_rating; // End point is the max capacity
    bid_curve_current.Curve_Info[(num_entries + 1)].power_delta =
        bid_curve_current.Curve_Info[(num_entries + 1)].power_stop -
        bid_curve_current.Curve_Info[(num_entries + 1)].power_start; // Delta

    bid_curve_current.Curve_Info[(num_entries + 1)].price =
        bid_curve_current.Curve_Info[num_entries]
            .price; // Price is just last price - if it is supposed to be
                    // different, they should have said so
  } else if (bid_curve_current.Curve_Info[num_entries].power_stop >
             bid_gen_rating) // Bigger - oh noes!
  {
    GL_THROW("Bid curve in generator_controller:%s exceeds the maximum "
             "generator rating!",
             thisobj->name);
    /*  TROUBLESHOOT
    The parsed bid curve in the generator_controller object had a stopping power
    value greater than the capacity. This is not allowed.  Please fix this and
    try again.
    */
...
void generator_controller::copy_bid_curve(OBJECT *thisobj) {
  ...
  if (bid_curve_values[latency_write_section].number_bid_curve_sections !=
    ...
    if (bid_curve_values[latency_write_section].Curve_Info != nullptr) {
      gl_free((void **)&bid_curve_values[latency_write_section].Curve_Info);
    }

    // Allocate
    // Malloc the new one
    bid_curve_values[latency_write_section].Curve_Info =
        (CURVEDETAILS *)(gl_malloc(bid_curve_current.number_bid_curve_sections *
                                   sizeof(CURVEDETAILS)));
  ...
  // copy the values
  memcpy(bid_curve_values[latency_write_section].Curve_Info,
         bid_curve_current.Curve_Info,
         bid_curve_current.number_bid_curve_sections * sizeof(CURVEDETAILS));

  // Update the timestamp
  bid_curve_values[latency_write_section].bid_curve_parsed =
      bid_curve_current.bid_curve_parsed;

  // Set the allowed bidding flag to false by default
  bid_curve_values[latency_write_section].valid_bid_period = false;

  // By default, assume we will not be running - this will get populated
  // properly elsewhere
  bid_curve_values[latency_write_section].expected_state = GEN_OFF;

  // Copy in the stored capacity value
  bid_curve_values[latency_write_section].generator_capacity =
      bid_curve_current.generator_capacity;
}
 ...
 */
static TIMESTAMP sync_generator_controller_impl(OBJECT *obj, TIMESTAMP t1,
                                                PASSCONFIG pass) {
  ...
  try {
    ...
    gl_error("generator_controller %s (%s:%d): %s", obj->name,
             obj->oclass->name, obj->id, msg);
    return TS_INVALID;
  } catch (...) {
    gl_error("generator_controller %s (%s:%d): unknown exception", obj->name,
...
#ifndef __APPLE__
extern "C" MODULE_API int sync_generator_controller(OBJECT *obj, TIMESTAMP t1,
                                                    PASSCONFIG pass) {
