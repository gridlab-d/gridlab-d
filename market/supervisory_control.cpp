/** $Id: supervisory_control.cpp$
        ...
        The supervisory_control object implements the supervisory control
 algorithm for frequency responsive loads.
...
supervisory_control::supervisory_control(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "supervisory_control",
                               sizeof(supervisory_control),
    ...
    if (gl_publish_variable(
            ...
            "unit of quantity", PT_double, "period[s]", PADDR(period),
            PT_DESCRIPTION, "interval of time between market clearings",
            PT_int32, "market_id", PADDR(market_id), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "unique identifier of market clearing", PT_double,
            "nominal_frequency[Hz]", PADDR(nom_freq), PT_DESCRIPTION,
            "nominal frequency", PT_double, "droop[%]", PADDR(droop),
            PT_DESCRIPTION, "droop value for the supervisor", PT_double,
            "frequency_deadband[Hz]", PADDR(frequency_deadband), PT_DESCRIPTION,
            "frequency deadband for assigning trigger frequencies",
            PT_enumeration, "PFC_mode", PADDR(PFC_mode), PT_DESCRIPTION,
            ...
            PT_enumeration, "bid_sort_mode", PADDR(sort_mode), PT_DESCRIPTION,
            "Determines how the bids into the market is sorted to contruct the "
            "PF curve",
            PT_KEYWORD, "NONE", (enumeration)SORT_NONE, PT_KEYWORD,
            "POWER_INCREASING", (enumeration)SORT_POWER_INCREASE, PT_KEYWORD,
            "POWER_DECREASING", (enumeration)SORT_POWER_DECREASE, PT_KEYWORD,
            "VOLTAGE_DEVIAION_FROM_NOMINAL", (enumeration)SORT_VOLTAGE,
...
void supervisory_control::fetch_double(double **prop, char *name,
                                       OBJECT *parent) {
...
TIMESTAMP supervisory_control::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (t1 >= clearat) {
    gl_verbose(
        "I received #%d bids on and #%d bid off and we are at #%d market id\n",
    ...
    PFC_collect.calculate_freq_thresholds(droop, nom_freq, frequency_deadband,
                                          PFC_mode);
...
int supervisory_control::submit_nolock(OBJECT *from, double power,
                                       double voltage_deviation, int key,
                                       int state) {
  char biddername[64];
  if (key > market_id) {
    gl_error("bids from %s is for future market id",
             gl_name(from, biddername, sizeof(biddername)));
  } else if (key < market_id) {
    gl_error("bids from %s is for previous market id",
             gl_name(from, biddername, sizeof(biddername)));
  } else {
    gl_verbose("I got a bid from %s  ---> Power: (%.1f) PFC state: (%.1f) Key: "
               "(%d) State (%d)\n",
               gl_name(from, biddername, sizeof(biddername)), power,
               voltage_deviation, key, state);
    ...
    if (state == 1) { // water heater is on
      n_bids_on =
          PFC_collect.submit_on(from, power, voltage_deviation, key, state);
    } else if (state == 0) { // water heater is off
      n_bids_off =
          PFC_collect.submit_off(from, power, voltage_deviation, key, state);
    } else {
...
TIMESTAMP supervisory_control::nextclear(void) const {
  return gl_globalclock +
         (TIMESTAMP)(period - fmod((double)gl_globalclock, period));
}
