...
int collect::submit_on(OBJECT *from, double power, double voltage_deviation,
                       int key, int state) {
  if (len_on ==
      0) { // if this is the first time, create the array of bid structs
    len_on = 8;
    ...
    memcpy(new_supervisor_bid, supervisor_bid_on,
           len_on * sizeof(SUPERVISORBID));
...
int collect::submit_off(OBJECT *from, double power, double voltage_deviation,
                        int key, int state) {
  if (len_off ==
      0) { // if this is the first time, create the array of bid structs
    len_off = 8;
    ...
    memcpy(new_supervisor_bid, supervisor_bid_off,
           len_off * sizeof(SUPERVISORBID));
...
int collect::calculate_freq_thresholds(double droop, double nom_freq,
  ...
  if (PFC_mode == 1 || PFC_mode == 2) { // under frequengy and both
    ...
    for (int i = 0; i < number_of_bids_on;
         i++) { // loop to assign trigger frequencies
      power_cum_on += supervisor_bid_on[i].power;
      fetch_double(&device, "trigger_point_under_frequency",
                   supervisor_bid_on[i].from);
  ...
  if (PFC_mode == 0 || PFC_mode == 2) { // over frequency and both
    ...
    int *mode;
    double temp_trig_freq = 0;
    for (int i = 0; i < number_of_bids_off;
         i++) { // loop to assign trigger frequencies
      power_cum_off += supervisor_bid_off[i].power;
      fetch_double(&device, "trigger_point_over_frequency",
                   supervisor_bid_off[i].from);
