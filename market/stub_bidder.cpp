...
stub_bidder::stub_bidder(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "stub_bidder", sizeof(stub_bidder),
                               PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);

    if (gl_publish_variable(
            oclass, PT_double, "bid_period[s]", PADDR(bid_period), PT_int16,
            "count", PADDR(count), PT_object, "market", PADDR(market),
            PT_enumeration, "role", PADDR(role), PT_KEYWORD, "BUYER",
            (enumeration)BUYER, PT_KEYWORD, "SELLER", (enumeration)SELLER,
            PT_double, "price", PADDR(price), PT_double, "quantity",
            PADDR(quantity), PT_int64, "bid_id", PADDR(bid_id), nullptr) < 1)
...
int stub_bidder::init(OBJECT *parent) {
  ...
  if (submit == nullptr) {
    gl_error("Unable to find function, submit_bid_state(), for object %s.",
             (char *)gl_name(market, mktname, 1024));
    return 0;
  }
  return SUCCESS;
}
...
TIMESTAMP stub_bidder::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (t1 == next_t || next_t == 0) {
    ...
    ((void (*)(char *, char *, const char *, const char *, void *, size_t))(
        *submit))((char *)gl_name(hdr, ctrname, 1023),
                  (char *)gl_name(market, mktname, 1023), "submit_bid_state",
...
EXPORT int init_stub_bidder(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    char name[64];
    gl_error("init_stub_bidder(obj=%s): %s", gl_name(obj, name, sizeof(name)),
             msg);
    return 0;
  }
  return 1;
