...
public:
  typedef struct s_curvedetails { // Declare in here in case we ever need it
                                  // externally
    ...
    double power_delta; /// Power delta of this linear section (power_stop -
                        /// power_start)
  ...
  typedef enum e_amorttype {
    ...
    LINEAR_COST = 1, // Amortization is handled via a linear function reducing
                     // the shutdown payback
    LINEAR_BID = 2, // Amortization is handled as a linearly decreasing
                    // component to pay back shutdown in minimum period
  } AMORTIZATION_TYPE;

  typedef enum e_genoptype {
    STANDALONE = 0, // Generator operates as a standalone device
    BUILDING = 1, // Generator is attached to a building load and "removes" the
                  // building if it wins
  } GENERATOR_MODE;

  int gen_state_num; // Numeric indication of the generator state - mainly to
                     // make post-processing easier for some people
  ...
  typedef struct s_curveentry {
    ...
    TIMESTAMP bid_curve_parsed; // Tracking variable - so know if need to update
                                // or not
    bool valid_bid_period; // Variable to indicate if the generator was allowed
                           // to bid here, or not
    GENERATOR_STATE expected_state; // Expected state of generator at this bid
                                    // period - simple "post-clear" update to
                                    // let next interval know how to bid
    double
        generator_capacity; // Capacity value at the time of the bid - used to
                            // track varying capacity generators (e.g., wind)
    double expected_building_load; // Expected building load for the time of the
                                   // bid
  } CURVEENTRY; // High-level structure for curves

  enumeration
      gen_state; // Current state of the generator (used to influence bids)
  CURVEENTRY *bid_curve_values; // Storage of bid curve for all latency periods
  CURVEENTRY bid_curve_current; // Currently parsed bid-curve - used to populate
                                // the other intervals
  enumeration amort_type;        // Type of amortization utilized
  int number_bid_curve_sections; // Number of sections in the current bid curve
                                 // parse
  ...
  int latency_read_section; // Current latency period to read the curve out of
                            // (this is when the market is active)
  void parse_bid_curve(OBJECT *thisobj,
                       TIMESTAMP t0);   // Function to parse bid curve
  void copy_bid_curve(OBJECT *thisobj); // Function to copy currently parsed bid
                                        // curve into latency storage
  ...
  double bid_gen_rating; // Generator output rating for current bid-curve
                         // parsing
  ...
  bool is_marginal_gen; // Flag to indicate if the generator is operating on the
                        // margin
  ...
  double minimum_runtime_dbl; // Minimum time for shutdown_cost to be
                              // accumulated over
  double minimum_downtime_dbl; // Minimum time a generator must remain off
                               // before it can bid again
  ...
  double curr_building_load; // Value of the building load the generators are
                             // attached to (if building mode)
  double bid_building_load; // Value of the building load the generators are
                            // attached to (if building mode) for the period
                            // being bid
  ...
  double
      input_unit_base; // Base multiple of [W] that bidding units are specified
                       // in - work around for market mismathc issue right now
  int64 bid_id;
...
private:
  ...
  TIMESTAMP last_update_time; // Time object was last updated - used for license
                              // premium hours track
  double seconds_in_year; // Number of seconds in "runtime year" - for divisions
  bool runtime_hours_left; // Flag to determine if the generator is allowed to
                           // run, or if it has exceeded its hourly runtime
