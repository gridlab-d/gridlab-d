...
BID *curve::getbid(KEY n) { return bids + keys[n]; }
...
KEY curve::resubmit(BID *bid) {
  ...
  for (i = 0; i < n_bids; i++) {
    if (bid_ids[i] == bid->bid_id) {
      if (bid_hitcount == 1) {
        gl_error("curve::resubmit - There is more than one bid with the same "
                 "bid id in the bid curve.");
        return -1;
      }
      bid_index = i;
      bid_hitcount++;
  ...
  if (bid_hitcount == 0) {
    gl_warning("The bid was flagged as a rebid but there is no bid in the bid "
               "curve with the bid id provided. Submitting the bid.");
    if (len == 0) // create the bid list
    {
      len = 8;
      bids = new BID[len];
...
// This function is for removing a from a curve if the rebid places the bidder
// in the opposite curve.(i.e. switching from a seller to a buyer or vice versa)
...
void curve::sort(bool reverse) { sort(bids, keys, n_bids, reverse); }
...
void curve::sort(BID *list, KEY *key, const int len, const bool reverse) {
  ...
  if (len > 1) {
    ...
    if (split > 1)
      sort(list, a, split, reverse);
    if (len - split > 1)
      sort(list, b, len - split, reverse);
    KEY *res = new KEY[len];
    KEY *p = res;
    do {
