...
BID_CCSI *curve_ccsi::getbid(KEY n)
{
	return bids+keys[n];
}

KEY curve_ccsi::submit(BID_CCSI *bid)
{
	if (len==0) // create the bid list
	{
		len = 8;
		bids = new BID_CCSI[len];
		keys = new KEY[len];
		bid_ids = new KEY[len];
	}
	else if (n_bids==len) // grow the bid list
	{
		BID_CCSI *newbids = new BID_CCSI[len*2];
		KEY *newkeys = new KEY[len*2];
		KEY *newbid_ids = new KEY[len*2];
		memcpy(newbids,bids,len*sizeof(BID_CCSI));
...
KEY curve_ccsi::resubmit(BID_CCSI *bid)
{
	int i = 0;
	int bid_index = 0;
	int bid_hitcount = 0;
	for(i = 0; i < n_bids; i++) {
		if(bid_ids[i] == bid->bid_id) {
			if(bid_hitcount == 1) {
				gl_error("curve_ccsi::resubmit - There is more than one bid with the same bid id in the bid curve.");
				return -1;
			}
			bid_index = i;
			bid_hitcount++;
	...
	if(bid_hitcount == 0) {
		...
		*next = *bid;
		...
		BID_CCSI *old = &(bids[keys[bid_index]]);
		switch (old->state) {
		case BS_OFF:
		...
		gl_error("curve_ccsi::resubmit - the bid failed to be captured in the curve.");
		return -1;
	}
}
...
int curve_ccsi::remove_bid(KEY bid_id)
{
	int i = 0;
	int bid_index = 0;
	int bid_hitcount = 0;
	...
	if (bid_index < n_bids && bid_hitcount == 1) {
		...
		BID_CCSI *newbids = new BID_CCSI[len];
...
void curve_ccsi::sort(bool reverse)
{
	sort(bids, keys, n_bids, reverse);
}

void curve_ccsi::sort(BID_CCSI *list, KEY *key, const int len, const bool reverse)
{
	//merge sort
	if (len>1)
	{
...
double curve_ccsi::get_total_at(double price){
	double sum = 0.0;
	int i = 0;
	if(n_bids > 0){
		for(i = 0; i < n_bids; ++i){
