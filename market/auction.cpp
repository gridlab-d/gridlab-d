...
EXPORT int64 get_market_for_time(OBJECT *obj, TIMESTAMP ts) {
  ...
  if (obj == 0) {
    ...
    /* TROUBLESHOOT
            The call was made without specifying the object that called it.
       Double-check the code where this method was called and verify that a
       non-null object pointer is passed in.
            */
    return -1;
  }
...
/** Auction/register_participant allow an auction to interact propertly with a
 participating controller. This makes it unnecessary to manually set the market
 rank high in the GLM file and avoids the use of gl_set_dependent during init()
 of participants.
 **/
EXPORT int64 register_participant(OBJECT *mkt, OBJECT *part) {
  ...
  /* @todo other actions that may required with market participant are
   * initialized */
  return 1;
}
...
auction::auction(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "auction", sizeof(auction),
    ...
    if (gl_publish_variable(
            oclass, PT_char32, "unit", PADDR(unit), PT_DESCRIPTION,
            "unit of quantity", PT_double, "period[s]", PADDR(dPeriod),
            PT_DESCRIPTION, "interval of time between market clearings",
            PT_double, "latency[s]", PADDR(dLatency), PT_DESCRIPTION,
            "latency between market clearing and delivery", PT_int64,
            "market_id", PADDR(market_id), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "unique identifier of market clearing",
            /**/ PT_object, "network", PADDR(network), PT_DESCRIPTION,
            "the comm network used by object to talk to the market (if any)",
            PT_bool, "verbose", PADDR(verbose), PT_DESCRIPTION,
            "enable verbose auction operations", PT_double, "price_cap",
            PADDR(pricecap), PT_DESCRIPTION,
            "the maximum price (magnitude) allowed",

            PT_enumeration, "special_mode", PADDR(special_mode), PT_KEYWORD,
            "NONE", (enumeration)MD_NONE, PT_KEYWORD, "SELLERS_ONLY",
            (enumeration)MD_SELLERS, PT_KEYWORD, "BUYERS_ONLY",
            (enumeration)MD_BUYERS, PT_enumeration, "statistic_mode",
            PADDR(statistic_mode), PT_KEYWORD, "ON", (enumeration)ST_ON,
            PT_KEYWORD, "OFF", (enumeration)ST_OFF, PT_double, "fixed_price",
            PADDR(fixed_price), PT_double, "fixed_quantity",
            PADDR(fixed_quantity), PT_object, "capacity_reference_object",
            PADDR(capacity_reference_object), PT_char32,
            "capacity_reference_property", PADDR(capacity_reference_propname),
            PT_double, "capacity_reference_bid_price",
            PADDR(capacity_reference_bid_price), PT_double,
            "max_capacity_reference_bid_quantity",
            PADDR(max_capacity_reference_bid_quantity), PT_double,
            "fixed_uncontrollable_load", PADDR(fixed_uncontrollable_load),
            PT_double, "capacity_reference_bid_quantity",
            PADDR(capacity_reference_bid_quantity), PT_double, "init_price",
            PADDR(init_price), PT_double, "init_stdev", PADDR(init_stdev),
            PT_double, "future_mean_price", PADDR(future_mean_price), PT_bool,
            "use_future_mean_price", PADDR(use_future_mean_price),

            PT_timestamp, "current_market.start_time",
            PADDR(current_frame.start_time), PT_timestamp,
            "current_market.end_time", PADDR(current_frame.end_time), PT_double,
            "current_market.clearing_price[$]",
            PADDR(current_frame.clearing_price), PT_double,
            "current_market.clearing_quantity",
            PADDR(current_frame.clearing_quantity), PT_enumeration,
            "current_market.clearing_type", PADDR(current_frame.clearing_type),
            PT_KEYWORD, "MARGINAL_SELLER", (enumeration)CT_SELLER, PT_KEYWORD,
            "MARGINAL_BUYER", (enumeration)CT_BUYER, PT_KEYWORD,
            "MARGINAL_PRICE", (enumeration)CT_PRICE, PT_KEYWORD, "EXACT",
            (enumeration)CT_EXACT, PT_KEYWORD, "FAILURE",
            (enumeration)CT_FAILURE, PT_KEYWORD, "nullptr",
            (enumeration)CT_NULL, PT_double,
            "current_market.marginal_quantity_load",
            PADDR(current_frame.marginal_quantity), PT_double,
            "current_market.marginal_quantity",
            PADDR(current_frame.marginal_quantity), PT_double,
            "current_market.marginal_quantity_bid",
            PADDR(current_frame.total_marginal_quantity), PT_double,
            "current_market.marginal_quantity_frac",
            PADDR(current_frame.marginal_frac), PT_double,
            "current_market.seller_total_quantity",
            PADDR(current_frame.seller_total_quantity), PT_double,
            "current_market.buyer_total_quantity",
            PADDR(current_frame.buyer_total_quantity), PT_double,
            "current_market.seller_min_price",
            PADDR(current_frame.seller_min_price), PT_double,
            "current_market.buyer_total_unrep",
            PADDR(current_frame.buyer_total_unrep), PT_double,
            "current_market.cap_ref_unrep", PADDR(current_frame.cap_ref_unrep),

            PT_timestamp, "next_market.start_time",
            PADDR(next_frame.start_time), PT_timestamp, "next_market.end_time",
            PADDR(next_frame.end_time), PT_double,
            "next_market.clearing_price[$]", PADDR(next_frame.clearing_price),
            PT_double, "next_market.clearing_quantity",
            PADDR(next_frame.clearing_quantity), PT_enumeration,
            "next_market.clearing_type", PADDR(next_frame.clearing_type),
            PT_KEYWORD, "MARGINAL_SELLER", (enumeration)CT_SELLER, PT_KEYWORD,
            "MARGINAL_BUYER", (enumeration)CT_BUYER, PT_KEYWORD,
            "MARGINAL_PRICE", (enumeration)CT_PRICE, PT_KEYWORD, "EXACT",
            (enumeration)CT_EXACT, PT_KEYWORD, "FAILURE",
            ...
            PADDR(next_frame.marginal_quantity), PT_double,
            "next_market.marginal_quantity_bid",
            PADDR(next_frame.total_marginal_quantity), PT_double,
            "next_market.marginal_quantity_frac",
            PADDR(next_frame.marginal_frac), PT_double,
            "next_market.seller_total_quantity",
            PADDR(next_frame.seller_total_quantity), PT_double,
            "next_market.buyer_total_quantity",
            PADDR(next_frame.buyer_total_quantity), PT_double,
            "next_market.seller_min_price", PADDR(next_frame.seller_min_price),
            PT_double, "next_market.cap_ref_unrep",
            PADDR(next_frame.cap_ref_unrep),

            PT_timestamp, "past_market.start_time",
            PADDR(past_frame.start_time), PT_timestamp, "past_market.end_time",
            PADDR(past_frame.end_time), PT_double,
            "past_market.clearing_price[$]", PADDR(past_frame.clearing_price),
            PT_double, "past_market.clearing_quantity",
            PADDR(past_frame.clearing_quantity), PT_enumeration,
            "past_market.clearing_type", PADDR(past_frame.clearing_type),
            PT_KEYWORD, "MARGINAL_SELLER", (enumeration)CT_SELLER, PT_KEYWORD,
            "MARGINAL_BUYER", (enumeration)CT_BUYER, PT_KEYWORD,
            "MARGINAL_PRICE", (enumeration)CT_PRICE, PT_KEYWORD, "EXACT",
            (enumeration)CT_EXACT, PT_KEYWORD, "FAILURE",
            ...
            PADDR(past_frame.marginal_quantity), PT_double,
            "past_market.marginal_quantity_bid",
            PADDR(past_frame.total_marginal_quantity), PT_double,
            "past_market.marginal_quantity_frac",
            PADDR(past_frame.marginal_frac), PT_double,
            "past_market.seller_total_quantity",
            PADDR(past_frame.seller_total_quantity), PT_double,
            "past_market.buyer_total_quantity",
            PADDR(past_frame.buyer_total_quantity), PT_double,
            "past_market.seller_min_price", PADDR(past_frame.seller_min_price),
            PT_double, "past_market.cap_ref_unrep",
            PADDR(past_frame.cap_ref_unrep),

            PT_enumeration, "margin_mode", PADDR(margin_mode), PT_KEYWORD,
            "NORMAL", (enumeration)AM_NONE, PT_KEYWORD, "DENY",
            (enumeration)AM_DENY, PT_KEYWORD, "PROB", (enumeration)AM_PROB,
            PT_int32, "warmup", PADDR(warmup), PT_enumeration,
            "ignore_failed_market", PADDR(ignore_failedmarket), PT_KEYWORD,
            "FALSE", (enumeration)IFM_FALSE, PT_KEYWORD, "TRUE",
            (enumeration)IFM_TRUE, PT_enumeration, "ignore_pricecap",
            PADDR(ignore_pricecap), PT_KEYWORD, "FALSE", (enumeration)IP_FALSE,
            PT_KEYWORD, "TRUE", (enumeration)IP_TRUE, PT_char256,
            "transaction_log_file", PADDR(trans_log), PT_int32,
            "transaction_log_limit", PADDR(trans_log_max), PT_char256,
            "curve_log_file", PADDR(curve_log), PT_int32, "curve_log_limit",
            PADDR(curve_log_max), PT_enumeration, "curve_log_info",
            PADDR(curve_log_info), PT_KEYWORD, "NORMAL", (enumeration)CO_NORMAL,
    ...
    gl_publish_function(oclass, "submit_bid_state",
                        (FUNCTIONADDR)submit_bid_state);
    gl_publish_function(oclass, "get_market_for_time",
                        (FUNCTIONADDR)get_market_for_time);
    gl_publish_function(oclass, "register_participant",
                        (FUNCTIONADDR)register_participant);
    defaults = this;
    //		immediate = 1;
...
int auction::isa(char *classname) { return strcmp(classname, "auction") == 0; }
...
int auction::init(OBJECT *parent) {
  ...
  if (capacity_reference_object != nullptr) {
    ...
    if ((capacity_reference_object->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("auction::init(): deferring initialization on %s",
  ...
  if (trans_log[0] != 0) {
    ...
    if (trans_file == 0) {
      gl_error("%s (auction:%d) unable to open '%s' as a transaction log "
               "output file",
               obj->name ? obj->name : "anonymous", obj->id,
    ...
    fprintf(
        trans_file, "# %s",
        asctime(localtime(&now))); // return char already included in asctime
    fprintf(trans_file, "# %s\n", obj->name ? obj->name : "anonymous");
    fprintf(
        trans_file,
        "# market_id,timestamp,bidder_name,bid_price,bid_quantity,bid_state\n");
    fflush(trans_file);
  ...
  if (curve_log[0] != 0) {
    ...
    if (curve_file == 0) {
      gl_error("%s (auction:%d) unable to open '%s' as an aurction curve log "
               "output file",
               obj->name ? obj->name : "anonymous", obj->id,
               trans_log.get_string());
      return 0;
    ...
    fprintf(
        curve_file,
        "# "
        "market_id,timestamp,sort_index,bidder_name,bid_quantity,bid_price\n");
    fflush(curve_file);
  ...
  for (statprop = stats; statprop != nullptr; statprop = statprop->next) {
    if (statprop->interval < this->period) {
      ...
      if (was_warned == 0) {
        gl_warning("market statistic '%s' samples faster than the market "
                   "updates and will be filled with immediate data",
                   statprop->prop->name);
        /* TROUBLESHOOT
                Market statistics are only calculated when the market clears.
           Statistics shorter than the market clearing rate wind up being equal
           to either the immediate price or to zero, due to the constant price
      ...
      gl_warning("market statistic '%s' interval not a multiple of market "
                 "period, rounding towards one interval",
                 statprop->prop->name);
      /* TROUBLESHOOT
              Market statistics are only calculated when the market clears, and
         will be calculated assuming intervals that are multiples of the
         market's period.
              */
  ...
  if (capacity_reference_object != nullptr) {
    if (capacity_reference_propname[0] != 0) {
      capacity_reference_property = gl_get_property(
          capacity_reference_object, capacity_reference_propname.get_string());
      if (capacity_reference_property == nullptr) {
        gl_error("%s (auction:%d) capacity_reference_object of type '%s' does "
                 "not contain specified reference property '%s'",
                 obj->name ? obj->name : "anonymous", obj->id,
                 capacity_reference_object->oclass->name,
                 capacity_reference_propname.get_string());
        /* TROUBLESHOOT
                capacity_reference_object must contain a property named by
           capacity_reference_property.  Review the published properties of the
           object specified in capacity_reference_object.
                */
        return 0;
      }
      if (capacity_reference_property->ptype != PT_double) {
        gl_warning("%s (auction:%d) capacity_reference_property '%s' is not a "
                   "double type property",
                   obj->name ? obj->name : "anonymous", obj->id,
                   capacity_reference_propname.get_string());
        /* TROUBLESHOOT
                capacity_reference_property must specify a double property to
           work properly, and may behave unpredictably should it be pointed at
      ...
      gl_error("%s (auction:%d) capacity_reference_object specified without a "
               "reference property",
               obj->name ? obj->name : "anonymous", obj->id);
      /* TROUBLESHOOT
              capacity_reference_object can only be used when
         capacity_reference_property is specified.
              */
      return 0;
    }
  }
  ...
  if (special_mode != MD_NONE) {
    if (fixed_quantity < 0.0) {
      gl_error("%s (auction:%d) is using a one-sided market with a negative "
               "fixed quantity",
               obj->name ? obj->name : "anonymous", obj->id);
      /* TROUBLESHOOT
              capacity_reference_object can only be used when
  ...
  if (init_stdev < 0.0) {
    ...
    /* TROUBLESHOOT
            By mathematical definition, real standard deviations cannot be
       negative.
            */
    return 0;
  }
...
int auction::init_statistics() {
  ...
  for (prop = oclass->pmap; prop != nullptr; prop = prop->next) {
    ...
    period_unit[0] = 0;
    if (sscanf(prop->name, "%[^\n_]_%[^\n_]_%[^\n_]_%[0-9]%[A-Za-z]", frame,
               price, stat, period, period_unit) >= 4) {
      ...
      if (statprop.interval <= 0) {
        gl_warning(
            "market statistic interval for '%s' is not positive, skipping",
            prop->name);
        /* TROUBLESHOOT
      ...
      if (period_unit[0] == 0) {
        ...
        gl_warning("market statistic period scalar '%c' not recognized, "
                   "statistic ignored",
                   period_unit[0]);
...
int auction::update_statistics() {
  ...
  for (current = stats; current != 0; current = current->next) {
    ...
    start = (unsigned int)((history_count + stop - sample_need) %
                           history_count); // one off for initial period delay
    for (i = 0; i < sample_need; ++i) {
      idx = (start + i + history_count) % history_count;
      if ((ignore_pricecap == IP_TRUE) &&
          ((new_prices[idx] == pricecap) || (new_prices[idx] == -pricecap))) {
        ++skipped;
      } else if ((ignore_failedmarket == IFM_TRUE) &&
                 (new_market_failures[idx] == CT_FAILURE)) {
        ++skipped;
      } else {
        mean += new_prices[idx];
      }
    ...
    if (skipped != sample_need) {
      ...
      mean = 0; // problem!
      gl_warning("All values in auction statistic calculations were skipped. "
                 "Setting mean to zero.");
      /* TROUBLESHOOT
              You may need to check your auction setup.  At least two flags are
         used to skip values in the calculation of the statistics
         (ignore_pricecap & ignore_failedmarket).  This error indicates that
         every value was skipped.  This may be by design, but one should be
         aware of it.  Note, standard deviation will also be set to zero after
         the initial statistics collection period.
      */
    }
    ...
    if (current->stat_type == SY_MEAN) {
      ...
      double x = 0.0;
      if (sample_need + (current->stat_mode == ST_PAST ? 1 : 0) >
          total_samples) { // extra sample for 'past' values
...
/*	Take the current market values and enqueue them on the end of the
 * latency frame queue. */
int auction::push_market_frame(TIMESTAMP t1) {
  ...
  if ((latency_back + 1) % latency_count == latency_front) {
    gl_error("market latency queue is overwriting as-yet unused data, so is "
             "not long enough or is not consuming data");
    /* TROUBLESHOOT
            This is indicative of problems with how the internal circular queue
       is being handled.  Please report this error on the GridLAB-D TRAC site.
            */
    return 0;
  }
  ...
  // set stats
  for (i = 0, stat = this->stats; i < statistic_count && stat != 0;
       ++i, stat = stat->next) {
    stats[i] = stat->value;
  }
  if (back != 0) {
...
int auction::check_next_market(TIMESTAMP t1) {
  MARKETFRAME *frame = 0;
  frame =
      (MARKETFRAME *)(latency_front * this->latency_stride + (int64)framedata);
...
TIMESTAMP auction::pop_market_frame(TIMESTAMP t1) {
  ...
  memcpy(&past_frame, &current_frame,
         sizeof(MARKETFRAME)); // not copying lagging stats
  // ~ copy new data in
...
TIMESTAMP auction::presync(TIMESTAMP t0, TIMESTAMP t1) {
  if (clearat == TS_ZERO) {
    ...
    char myname[64];
    if (verbose)
      gl_output("   ...%s first clearing at %s",
                gl_name(object_header(this), myname, sizeof(myname)),
                gl_strtime(&dt, buffer, sizeof(buffer)) ? buffer
                                                        : "unknown time");
  } else {
    ...
    if (t1 > t0 && ((t1 / TS_SECOND) % period) == 0) {
      ...
      next.from = nullptr; /* in the context of a clearing, from is the marginal
                              resource */
  ...
  if (t1 >= clearat) {
    ...
    char myname[64];
    if (verbose)
      gl_output("   ...%s clearing process started at %s",
                gl_name(object_header(this), myname, sizeof(myname)),
                gl_strtime(&dt, buffer, sizeof(buffer)) ? buffer
                                                        : "unknown time");
    ...
    checkat = gl_globalclock +
              (TIMESTAMP)(3600.0 - fmod(gl_globalclock + 3600.0, 3600.0));
    gl_localtime(clearat, &dt);
    if (verbose)
      gl_output(
          "   ...%s opens for clearing of market_id %d at %s",
          gl_name(object_header(this), name, sizeof(name)), (int32)market_id,
          gl_strtime(&dt, buffer, sizeof(buffer)) ? buffer : "unknown time");
  }
...
TIMESTAMP auction::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  return -clearat; /* soft return t2>t1 on success, t2=t1 for retry, t2<t1 on
                      failure */
}
...
void auction::record_curve(double bu, double su) {
  ...
  for (i = 0; i < offers.getcount(); i++) {
    fprintf(curve_file, "%d,%s,%4d,%s,%.3f,%.6f\n", (int32)market_id, timestr,
            i, offers.getbid(i)->from, offers.getbid(i)->quantity,
  ...
  for (i = 0; i < asks.getcount(); i++) {
    fprintf(curve_file, "%d,%s,%4d,%s,%.3f,%.6f\n", (int32)market_id, timestr,
            i, asks.getbid(i)->from, -asks.getbid(i)->quantity,
            asks.getbid(i)->price);
  }
  ...
  if (CO_EXTRA == curve_log_info) {
    ...
    fprintf(curve_file, "# marginal quantity of %f %s (%f %%)\n",
            cleared_frame.marginal_quantity, this->unit,
            cleared_frame.marginal_frac);
    fprintf(curve_file, "# curve cleared %f at $%f with %s type\n",
            cleared_frame.clearing_quantity, cleared_frame.clearing_price,
            tstr);
  }
...
void auction::clear_market(void) {
  ...
  if (capacity_reference_property != nullptr &&
      special_mode != MD_FIXED_BUYER) {
    char name[256];

    double total_unknown =
        asks.get_total() - asks.get_total_on() - asks.get_total_off();
    double *pRefload =
        gl_get_double(capacity_reference_object, capacity_reference_property);
    double refload;
    ...
    if (pRefload == nullptr) {
      ...
      sprintf(msg,
              ...
              capacity_reference_object->name);
      throw msg;
      /* TROUBLESHOOT
              The specified capacity reference property cannot be retrieved as a
         double.  Verify that it is a double type property.
      */
    } else {
      refload = *pRefload;
    }
      ...
      if (capacity_reference_property->unit != 0) {
        if (gl_convert(capacity_reference_property->unit->name, unit,
          ...
          sprintf(msg,
                  ...
                  capacity_reference_property->name,
                  capacity_reference_property->unit->name, unit);
          throw msg;
          /* TROUBLESHOOT
                  If capacity_reference_property has units specified, the units
             must be convertable to the units used by its auction object.
                  */
        } else if (verbose) {
          gl_output("capacity_reference_property converted %.3f %s to %.3f %s",
                    *pRefload, capacity_reference_property->unit->name, refload,
                    unit);
        }
      } // else assume same units
    }
    if (total_unknown >
        0.001) { // greater than one mW ~ allows rounding errors.  Threshold may
                 // be one kW given a MW-unit'ed market.
      gl_warning("total_unknown is %.0f -> some controllers are not providing "
                 "their states with their bids",
                 total_unknown);
    }
    // unresponsive.from = linkref;
    char capname[1024];
    unresponsive.from =
        (char *)gl_name(capacity_reference_object, capname, sizeof(capname));
    ...
    if (fixed_uncontrollable_load > 0) {
      ...
      unresponsive.quantity =
          (refload - asks.get_total_on() -
           total_unknown / 2); /* estimate load on as 1/2 unknown load */
    }
    unresponsive.bid_id = (int64)capacity_reference_object->id;
    cap_ref_unrep = unresponsive.quantity;
    if (unresponsive.quantity < -0.001) {
      gl_warning("capacity_reference_property %s has negative unresponsive "
                 "load--this is probably due to improper bidding",
                 capacity_reference_property->name, unresponsive.quantity);
    } else if (unresponsive.quantity > 0.001) {
      submit_nolock(unresponsive.from, -unresponsive.quantity,
                    unresponsive.price, unresponsive.bid_id, BS_ON, false,
                    market_id);
      gl_verbose("capacity_reference_property %s has %.3f unresponsive load",
                 capacity_reference_property->name, -unresponsive.quantity);
    }
  }
    ...
    if (strcmp(unit, "") != 0) {
      if (capacity_reference_property->unit != 0) {
        if (gl_convert(capacity_reference_property->unit->name, unit,
          ...
          submit_nolock(
              (char *)object_header(this)->name,
              max_capacity_reference_bid_quantity, capacity_reference_bid_price,
              (int64)object_header(this)->id, BS_ON, false, market_id);
          if (verbose)
            gl_output("Capacity reference object: %s bids %.2f at %.2f",
                      capacity_reference_object->name,
                      max_capacity_reference_bid_quantity,
    ...
    for (unsigned int i = 0; i < offers.getcount(); i++) {
      if (verbose) {
        ...
        if (offer != nullptr) {
          ...
          // Also, continue to apply defensive checks for other pointers like
          // 'unit' and 'from'. const char* safe_unit_str = unit ? unit : "N/A";
          const char *safe_unit_str =
              unit ? static_cast<const char *>(unit) : "N/A";
          const char *safe_from_str =
              offer->from ? offer->from : "unknown_source";

          gl_output("   ...  %4d: %s offers %.3f %s at %.2f $/%s", i,
                    safe_from_str, offer->quantity, safe_unit_str, offer->price,
                    safe_unit_str);
        } else {
          // 4. (Optional but highly recommended) The pointer was NULL.
          // Log a warning or debug message so you know why some data is missing
          // from the output.
          gl_warning("Offer at index %d is NULL and was skipped in output.", i);
        }

        // --- Continue the loop ---
        /*	gl_output("   ...  %4d: %s offers %.3f %s at %.2f
         * $/%s",i,offers.getbid(i)->from,
         * offers.getbid(i)->quantity,unit,offers.getbid(i)->price,unit);*/
      }
    }
    if (fixed_price * fixed_quantity != 0.0) {
      gl_warning("fixed_price and fixed_quantity are set in the same single "
                 "auction market ~ only fixed_price will be used");
    }
    if (fixed_quantity > 0.0) {
      for (unsigned int i = 0;
           i < offers.getcount() && single_quantity < fixed_quantity; ++i) {
      ...
      if (single_quantity > fixed_quantity) {
        ...
        single_quantity = 0.0;
        // Jason: Not sure this is the right way, but seems to work when nobody
        // is bidding (neither supply or demand)
  ...
  case MD_BUYERS:
    ...
    for (unsigned int i = 0; i < asks.getcount(); i++) {
      if (verbose) {
        ...
        // 1. Call the function ONCE and store the result in a local pointer
        // variable.
        auto bid = asks.getbid(i); // Or use the specific pointer type, e.g.,
                                   // const BidObject* bid = ...

        // 2. *** ALWAYS CHECK THE POINTER FOR NULL BEFORE USING IT. ***
        if (bid != nullptr) {
          // 3. Now that we know 'bid' is a valid pointer, it is safe to access
          // its members. We should also continue to apply the lessons from
          // before and check other pointers like 'unit'. const char*
          // safe_unit_str = unit ? unit : "N/A";
          const char *safe_unit_str =
              unit ? static_cast<const char *>(unit) : "N/A";
          const char *safe_from_str = bid->from ? bid->from : "unknown_source";

          gl_output("   ...  %4d: %s asks %.3f %s at %.2f $/%s", i,
                    safe_from_str, bid->quantity, safe_unit_str, bid->price,
                    safe_unit_str);
        } else {
          // 4. (Optional but highly recommended) The pointer was NULL.
        ...
        // --- Continue the loop ---
        // gl_output("   ...  %4d: %s asks %.3f %s at %.2f
        // $/%s",i,asks.getbid(i)->from,
        // asks.getbid(i)->quantity,unit,asks.getbid(i)->price,unit);
      }
    }
    if (fixed_price * fixed_quantity != 0.0) {
      gl_warning("fixed_price and fixed_quantity are set in the same single "
    ...
    if (fixed_quantity > 0.0) {
      for (unsigned int i = 0;
           i < asks.getcount() && single_quantity < fixed_quantity; ++i) {
  ...
  case MD_FIXED_SELLER:
    ...
    asks.clear();
    submit((char *)object_header(this)->name, -fixed_quantity, fixed_price,
           (int64)object_header(this)->id, BS_ON, false, market_id);
    break;
  case MD_FIXED_BUYER:
    asks.sort(true);
  ...
  if (special_mode == MD_SELLERS || special_mode == MD_BUYERS) {
    ...
    if (verbose) {
      ...
      const char *arg6_time = gl_strtime(&dt, time_buffer, sizeof(time_buffer))
                                  ? time_buffer
                                  : "unknown time";

      // 3. Print every single argument to stderr to verify their values before
      // the call. This will immediately show you if any pointer is NULL or if a
      // double is garbage. fprintf(stderr, "DEBUG: Pre-call to gl_output:\n");
      // fprintf(stderr, "  Format: \"... %%s clears %%.2f %%s at $%%.2f/%%s at
      // %%s\"\n"); fprintf(stderr, "  Arg 1 (name): %s\n", arg1_name ?
      // arg1_name : "NULL"); fprintf(stderr, "  Arg 2 (qty):  %.2f\n",
      // arg2_qty); fprintf(stderr, "  Arg 3 (unit): %s\n", arg3_unit ?
      // arg3_unit : "NULL"); fprintf(stderr, "  Arg 4 (price):%.2f\n",
      // arg4_price); fprintf(stderr, "  Arg 5 (unit): %s\n", arg5_unit ?
      // arg5_unit : "NULL"); fprintf(stderr, "  Arg 6 (time): %s\n", arg6_time
      // ? arg6_time : "NULL"); fflush(stderr); // Make sure the debug output
      // appears before a crash

      // 4. Now, make the call using the safe, local variables.
      gl_output("   ...  %s clears %.2f %s at $%.2f/%s at %s", arg1_name,
                arg2_qty, arg3_unit, arg4_price, arg5_unit, arg6_time);

      /*fprintf(stderr, "DEBUG: Post-call to gl_output succeeded.\n");
      fflush(stderr);*/

      // gl_output("   ...  %s clears %.2f %s at $%.2f/%s at %s",
      // gl_name(object_header(this),name,sizeof(name)), next.quantity, unit,
      // next.price, unit, gl_strtime(&dt,buffer,sizeof(buffer))?buffer:"unknown
      // time");
    }
    ...
    if (1) {
      ...
      for (i = 0; i < offers.getcount(); i++) {
        if (verbose) {
          ...
          // 1. Call the function ONCE and store the returned pointer in a local
          // variable.
          auto offer = offers.getbid(i);
          ...
          if (offer != nullptr) {
            ...
            // As a best practice, also guard any other pointers that could be
            // null. const char* safe_unit_str = unit ? unit : "N/A";
            const char *safe_unit_str =
                unit ? static_cast<const char *>(unit) : "N/A";
            const char *safe_from_str =
                offer->from ? offer->from : "unknown_source";
            ...
            gl_output("   ...  %4d: %s offers %.3f %s at %.2f $/%s", i,
                      safe_from_str, offer->quantity, safe_unit_str,
                      offer->price, safe_unit_str);
          } else {
            // 4. (Optional but strongly recommended) Log a warning that the
            // offer was null. This helps in debugging why some data might be
      ...
      for (i = 0; i < asks.getcount(); i++) {
        if (verbose) {
          ...
          if (bid != nullptr) {
            ...
            // We also continue to guard against other potentially null
            // pointers.
            const char *safe_from_str =
                "unknown_seller"; // bid->from ? bid->from : "unknown_seller";
    ...
    i = j = 0;
    clearing_type = CT_NULL;
    while (i < asks.getcount() && j < offers.getcount() &&
    ...
    if (check) { /* there was price agreement or quantity disagreement */
      ...
      if (supply_quantity == demand_quantity) {
        if (i == asks.getcount() || j == offers.getcount()) {
          if (i == asks.getcount() &&
              j == offers.getcount()) { // both sides exhausted at same quantity
            if (a == b) {
              clearing_type = CT_EXACT;
            } else {
              clearing_type = CT_PRICE;
                     ...
                     b == sell->price) { // exhausted buyers, sellers
                                         // unsatisfied at same price
            clearing_type = CT_SELLER;
          } else if (j == offers.getcount() &&
                     a == buy->price) { // exhausted sellers, buyers unsatisfied
                                        // at same price
            clearing_type = CT_BUYER;
          ...
          if (a != buy->price && b != sell->price && a == b) {
            ...
            // possible when a == b, q_buy == q_sell, and either the buyers or
    ...
    if (clear.quantity == 0) // && offers.getcount()>0)
    {
      ...
      if (offers.getcount() > 0 && asks.getcount() == 0) {
        ...
        if (offers.getbid(0)->price == pricecap) {
          ...
          clear.price = offers.getbid(0)->price +
                        (asks.getbid(0)->price - offers.getbid(0)->price) *
                            clearing_scalar;
        }
      }
      ...
      clear.price = -pricecap;
               ...
               clear.quantity == unresponsive_sell) {
    ...
    if (verbose) {
      ...
      char name_buffer[64];
      const char *safe_name_str =
          gl_name(object_header(this), name_buffer, sizeof(name_buffer));

      // 2. Use a dedicated local buffer for the time to prevent buffer
      // aliasing.
      char time_buffer[64];
      const char *safe_time_str =
          gl_strtime(&dt, time_buffer, sizeof(time_buffer)) ? time_buffer
                                                            : "unknown time";
      ...
      gl_output("   ...  %s clears %.2f %s at $%.2f/%s at %s", safe_name_str,
                clear.quantity, safe_unit_str, clear.price, safe_unit_str,
                safe_time_str);

      // gl_output("   ...  %s clears %.2f %s at $%.2f/%s at %s",
      // gl_name(object_header(this), name, sizeof(name)), clear.quantity, unit,
      // clear.price, unit, gl_strtime(&dt, buffer, sizeof(buffer)) ? buffer :
      // "unknown time");
    }
    ...
    if (offers.getcount() > 0 && asks.getcount() == 0) {
      ...
      next.price =
          offers.getbid(0)->price +
          (asks.getbid(0)->price - offers.getbid(0)->price) * clearing_scalar;
    } else if (asks.getcount() == 0 && offers.getcount() == 0) {
      next.price = 0.0;
    }
    next.quantity = 0;
    clearing_type = CT_NULL;
    gl_warning("market '%s' fails to clear due to missing %s",
               gl_name(object_header(this), name, sizeof(name)),
               asks.getcount() == 0
                   ? (offers.getcount() == 0 ? "buyers and sellers" : "buyers")
                   : "sellers");
  }
  ...
  if (latency > 0) {
    ...
    current_frame.total_marginal_quantity =
        cleared_frame.total_marginal_quantity;
...
void auction::record_bid(char *from, double quantity, double real_price,
                         BIDDERSTATE state) {
...
int auction::submit_nolock(char *from, double quantity, double real_price,
                           KEY key, BIDDERSTATE state, bool rebid,
                           int64 mkt_id) {
  char myname[64];
  TIMESTAMP submit_time = gl_globalclock;
  DATETIME dt;
  ...
  if (total_samples < sph24 && quantity < 0 && warmup) {
    if (verbose)
      gl_output("   ...  %s ignoring demand bid during first 24 hours",
                gl_name(object_header(this), myname, sizeof(myname)));
    return 1;
  }
  ...
  if (mkt_id > market_id) { // future market
    gl_error("bidding into future markets is not yet supported");
    /* TROUBLESHOOT
            Tracking bids input markets other than the immediately open one will
  ...
  } else if (mkt_id == market_id && rebid == true) // resubmit
  {
    ...
    if (verbose) {
      ...
      // 3. Use the ternary operator to GUARD AGAINST NULL POINTERS for all
      // string arguments.
      ...
      gl_output("   ...  %s resubmits %s from object %s for %.2f %s at "
                "$%.2f/%s at %s",
                safe_name_str, (quantity < 0 ? "ask" : "offer"), safe_from_str,
                fabs(quantity), safe_unit_str, price, safe_unit_str, time_str);

      // gl_output("   ...  %s resubmits %s from object %s for %.2f %s at
      // $%.2f/%s at %s",
      //	gl_name(object_header(this),myname,sizeof(myname)),
      //quantity<0?"ask":"offer", from, 	fabs(quantity), unit, price, unit,
      //gl_strtime(&dt,buffer,sizeof(buffer))?buffer:"unknown time");
    }
    ...
    if (verbose) {
      ...
      // 2. Use the ternary operator to guard against NULL pointers for all %s
      // arguments.
      const char *safe_name_str =
          gl_name(object_header(this), myname, sizeof(myname));
      ...
      gl_output(
          "   ...  %s receives %s from object %s for %.2f %s at $%.2f/%s at %s",
          (safe_name_str ? safe_name_str
                         : "unknown_object"), // Guard against NULL from gl_name
      ...
      // gl_output("   ...  %s receives %s from object %s for %.2f %s at
      // $%.2f/%s at %s", gl_name(object_header(this),myname,sizeof(myname)),
      // quantity<0?"ask":"offer", from, fabs(quantity), unit, price, unit,
      // gl_strtime(&dt,buffer,sizeof(buffer))?buffer:"unknown time");
    }
    BID bid = {from, b_id, fabs(quantity), price, state};
    ...
    biddef.raw = out;
    return 1;
  } else { // key between cleared market and 'market_id' ~ points to an old
           // market
    if (verbose) {
      char myname[64];
      gl_output(
          " ... %s receives %s from object %s for a previously cleared market",
          gl_name(object_header(this), myname, sizeof(myname)),
          quantity < 0 ? "ask" : "offer", from);
    }
  }
  return 0;
}
...
TIMESTAMP auction::nextclear(void) const {
  return gl_globalclock +
         (TIMESTAMP)(period - (gl_globalclock + period) % period);
}
