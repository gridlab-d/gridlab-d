...
controller2::controller2(MODULE *mod) {
  if (oclass == nullptr) {
    oclass = gl_register_class(mod, "controller2", sizeof(controller2),
    ...
    if (gl_publish_variable(
            oclass,
            // series inputs
            PT_int32, "input_state", PADDR(input_state), PT_double,
            "input_setpoint", PADDR(input_setpoint), PT_bool, "input_chained",
            PADDR(input_chained),
            // outputs
            ...
            PT_DESCRIPTION, "the observed expected value", PT_double,
            "setpoint", PADDR(output_setpoint), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the target setpoint given the input observations",
            // inputs
            PT_double, "sensitivity", PADDR(sensitivity), PT_DESCRIPTION,
            "the sensitivity of the control actuator to observation deviations",
            PT_int64, "period", PADDR(period), PT_DESCRIPTION,
            "the cycle period for the controller logic", PT_char32,
            "expectation_prop", PADDR(expectation_propname), PT_DESCRIPTION,
            "the name of the property to observe for the expected value",
            PT_object, "expectation_obj", PADDR(expectation_obj),
            PT_DESCRIPTION, "the object to watch for the expectation property",
            PT_char32, "setpoint_prop", PADDR(output_setpoint_propname),
            PT_DESCRIPTION,
            "the name of the setpoint property in the parent object", PT_char32,
            "state_prop", PADDR(output_state_propname), PT_DESCRIPTION,
            "the name of the actuator property in the parent object", PT_object,
            "observation_obj", PADDR(observable), PT_DESCRIPTION,
            "the object to observe", PT_char32, "observation_prop",
            PADDR(observation_propname), PT_DESCRIPTION,
            "the name of the observation property", PT_char32,
            "mean_observation_prop", PADDR(observation_mean_propname),
            PT_DESCRIPTION, "the name of the mean observation property",
            PT_char32, "stdev_observation_prop",
            PADDR(observation_stdev_propname), PT_DESCRIPTION,
            "the name of the standard deviation observation property", PT_int32,
            "cycle_length", PADDR(cycle_length), PT_DESCRIPTION,
            "length of time between processing cycles", PT_double,
            "base_setpoint", PADDR(first_setpoint), PT_DESCRIPTION,
            "the base setpoint to base control off of",
            // specific inputs
            PT_double, "ramp_high", PADDR(ramp_high), PT_double, "ramp_low",
            PADDR(ramp_low), PT_double, "range_high", PADDR(range_high),
            PT_double, "range_low", PADDR(range_low),
            // specific outputs
            PT_double, "prob_off", PADDR(prob_off), PT_int32, "output_state",
            PADDR(output_state), PT_double, "output_setpoint",
            PADDR(output_setpoint),
            // enums
            ...
            (enumeration)CM_RAMP, PT_KEYWORD, "CND", (enumeration)CM_CND,
            PT_KEYWORD, "DUTYCYCLE", (enumeration)CM_DUTYCYCLE, PT_KEYWORD,
            "THIRD", (enumeration)CM_THIRD, PT_KEYWORD, "PROBOFF",
            (enumeration)CM_PROBOFF, nullptr) < 1) {
...
int controller2::init(OBJECT *parent) {
  ...
  if (parent == nullptr) {
    ...
    if (expectation_obj != 0) {
      ...
      expectation_addr =
          (void *)((unsigned int64)expectation_obj + sizeof(OBJECT) +
                   (unsigned int64)expectation_prop->addr);
    }
    ...
    if (observable != 0) {
      ...
      if (observation_prop != 0) {
        observation_addr =
            (void *)((unsigned int64)observable + sizeof(OBJECT) +
                     (unsigned int64)observation_prop->addr);
      }
      // observation_mean_addr
      observation_mean_prop =
          gl_get_property(observable, observation_mean_propname);
      ...
      observation_stdev_prop =
          gl_get_property(observable, observation_stdev_propname);
      if (observation_stdev_prop != 0) {
        observation_stdev_addr =
            (void *)((unsigned int64)observable + sizeof(OBJECT) +
                     (unsigned int64)observation_stdev_prop->addr);
      }
    }
    // output_state
    if (output_state_propname[0] != 0) {
      ...
      output_setpoint_addr = (void *)((unsigned int64)parent + sizeof(OBJECT) +
    ...
    if (output_setpoint_propname[0] != 0) {
      ...
      if (output_setpoint_prop == nullptr) {
        GL_THROW("controller2 parent \"%s\" does not contain property \"%s\"",
                 ...
                 output_setpoint_propname);
      }
      output_setpoint_addr =
          (void *)((unsigned int64)parent + sizeof(OBJECT) +
                   (unsigned int64)output_setpoint_prop->addr);
    }
  }

  if (observable == nullptr) {
    GL_THROW(
        "controller2 observable object is undefined, and can not function");
  }

  //
  // make sure that the observable and expectable are ranked above the
  // controller
  //
...
TIMESTAMP controller2::postsync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
...
int controller2::calc_ramp(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (sensitivity == 0.0 || obs_stdev == 0.0) {
    ...
    set_change = (observation - expectation) * fabs(T_limit) /
                 (ramp / sensitivity * obs_stdev);
  }
...
int controller2::calc_proboff(TIMESTAMP t0, TIMESTAMP t1) {
  if (observation_addr == 0 || observation_mean_addr == 0 ||
      observation_stdev_addr == 0) {
...
EXPORT int init_controller2(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    char name[64];
    gl_error("init_controller2(obj=%s): %s", gl_name(obj, name, sizeof(name)),
             msg);
  }
  return 1;
}
...
static TIMESTAMP sync_controller2_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  ...
  try {
    ...
    char name[64];
    gl_error("sync_controller2(obj=%s): %s", gl_name(obj, name, sizeof(name)),
             msg);
  }
  return t2;
}
