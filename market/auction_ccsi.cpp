/** $Id: auction_ccsi.cpp 1182 2008-12-22 22:08:36Z dchassin $
	Copyright (C) 2008 Battelle Memorial Institute
	@file auction_ccsi.cpp
	@defgroup auction_ccsi Template for a new object class
	@ingroup market

	The auction_ccsi object implements the basic auction_ccsi. 
...
CLASS *auction_ccsi::oclass = NULL;
auction_ccsi *auction_ccsi::defaults = NULL;
STATISTIC_CCSI *auction_ccsi::stats = NULL;
TIMESTAMP auction_ccsi::longest_statistic = 0;
int auction_ccsi::statistic_check = -1;
uint32 auction_ccsi::statistic_count = 0;
...
EXPORT int64 get_market_for_time_ccsi(OBJECT *obj, TIMESTAMP ts){
	...
	int64 market_id;
	if(obj == 0){
		gl_error("get_market_for_time_ccsi() was called with a null object pointer");
		/* TROUBLESHOOT
	...
	pAuc = OBJECTDATA(obj, auction_ccsi);
	...
	if(ts < market_time){
		...
		write_bid(key, market_id, -1, BID_CCSI_UNKNOWN);
		return (int64)key;
...
/** auction_ccsi/register_participant_ccsi allow an auction_ccsi to interact propertly with a participating controller.
 ...
 **/
EXPORT int64 register_participant_ccsi(OBJECT *mkt, OBJECT *part)
{
...
auction_ccsi::auction_ccsi(MODULE *module)
{
	if (oclass==NULL)
	{
...
/**/		PT_object, "network", PADDR(network), PT_DESCRIPTION, "the comm network used by object to talk to the market (if any)",
			...
			PT_enumeration, "margin_mode", PADDR(margin_mode),
				PT_KEYWORD, "NORMAL", 0,
				PT_KEYWORD, "DENY", (enumeration)AM_DENY_CCSI,
				PT_KEYWORD, "PROB", (enumeration)AM_PROB_CCSI,
		...
		gl_publish_function(oclass,	"submit_bid_state_ccsi", (FUNCTIONADDR)submit_bid_state_ccsi);
		gl_publish_function(oclass, "get_market_for_time_ccsi", (FUNCTIONADDR)get_market_for_time_ccsi);
		gl_publish_function(oclass, "register_participant_ccsi", (FUNCTIONADDR)register_participant_ccsi);
		defaults = this;
//		immediate = 1;
		memset(this,0,sizeof(auction_ccsi));
	}
}


int auction_ccsi::isa(char *classname)
{
	return strcmp(classname,"auction_ccsi")==0;
}

/* Object creation is called once for each object that is created by the core */
int auction_ccsi::create(void)
{
	STATISTIC_CCSI *stat;
	double val = -1.0;
	memcpy(this,defaults,sizeof(auction_ccsi));
	lasthr = thishr = -1;
	verbose = 0;
...
int auction_ccsi::init(OBJECT *parent)
{
	...
	if(capacity_reference_object != NULL){
		...
		if(capacity_reference_object->flags & OF_INIT != OF_INIT){
			char objname[256];
			gl_verbose("auction_ccsi::init(): deferring initialization on %s", gl_name(obj, objname, 255));
			return 2; // defer
		}
	}
	...
	if(trans_log[0] != 0){
		...
		if(trans_file == 0){
			gl_error("%s (auction_ccsi:%d) unable to open '%s' as a transaction log output file", obj->name?obj->name:"anonymous", obj->id, trans_log.get_string());
			return 0;
		}
	...
	if(curve_log[0] != 0){
		...
		if(curve_file == 0){
			gl_error("%s (auction_ccsi:%d) unable to open '%s' as an aurction curve log output file", obj->name?obj->name:"anonymous", obj->id, trans_log.get_string());
			return 0;
		}
	...
	if(capacity_reference_object != NULL){
		if(capacity_reference_propname[0] != 0){
			...
			if(capacity_reference_property == NULL){
				gl_error("%s (auction_ccsi:%d) capacity_reference_object of type '%s' does not contain specified reference property '%s'", obj->name?obj->name:"anonymous", obj->id, capacity_reference_object->oclass->name, capacity_reference_propname.get_string());
				/* TROUBLESHOOT
			...
			if(capacity_reference_property->ptype != PT_double){
				gl_warning("%s (auction_ccsi:%d) capacity_reference_property '%s' is not a double type property", obj->name?obj->name:"anonymous", obj->id, capacity_reference_propname.get_string());
				/* TROUBLESHOOT
			...
			gl_error("%s (auction_ccsi:%d) capacity_reference_object specified without a reference property", obj->name?obj->name:"anonymous", obj->id);
			/* TROUBLESHOOT
	...
	if(special_mode != MD_NONE){
		if(fixed_quantity < 0.0){
			gl_error("%s (auction_ccsi:%d) is using a one-sided market with a negative fixed quantity", obj->name?obj->name:"anonymous", obj->id);
			/* TROUBLESHOOT
				capacity_reference_object can only be used when capacity_reference_property is specified.
				*/
			return 0;
	...
	latency_stride = sizeof(MARKETFRAME_CCSI) + statistic_count * sizeof(double);
	framedata = (MARKETFRAME_CCSI *)malloc(latency_stride * latency_count);
	memset(framedata, 0, latency_stride * latency_count);
	for(i = 0; i < latency_count; ++i){
		MARKETFRAME_CCSI *frameptr;
		int64 addr = latency_stride * i + (int64)framedata;
		int64 stataddr = addr + sizeof(MARKETFRAME_CCSI);
		int64 nextaddr = addr + latency_stride;
		frameptr = (MARKETFRAME_CCSI *)(addr);
		...
		if(i+1 < latency_count){
			frameptr->next = (MARKETFRAME_CCSI *)(nextaddr);
	...
	if(init_stdev < 0.0){
		gl_error("auction_ccsi init_stdev is negative!");
		/* TROUBLESHOOT
...
int auction_ccsi::init_statistics(){
	STATISTIC_CCSI *sp = 0;
	STATISTIC_CCSI *tail = 0;
	STATISTIC_CCSI statprop;
	...
	for(prop = oclass->pmap; prop != NULL; prop = prop->next){
		...
		memset(&statprop, 0, sizeof(STATISTIC_CCSI));
		period_unit[0] = 0;
		if(sscanf(prop->name, "%[^\n_]_%[^\n_]_%[^\n_]_%[0-9]%[A-Za-z]", frame, price, stat, period, period_unit) >= 4){
			...
			sp = (STATISTIC_CCSI *)malloc(sizeof(STATISTIC_CCSI));
			memcpy(sp, &statprop, sizeof(STATISTIC_CCSI));
...
int auction_ccsi::update_statistics(){
	OBJECT *obj = OBJECTHDR(this);
	STATISTIC_CCSI *current = 0;
...
int auction_ccsi::push_market_frame(TIMESTAMP t1){
	MARKETFRAME_CCSI *frame = 0;
	OBJECT *obj = OBJECTHDR(this);
	STATISTIC_CCSI *stat = stats;
	double *stats = 0;
	...
	frame = (MARKETFRAME_CCSI *)frame_addr;
...
int auction_ccsi::check_next_market(TIMESTAMP t1){
	MARKETFRAME_CCSI *frame = 0;
	frame = (MARKETFRAME_CCSI *)(latency_front * this->latency_stride + (int64)framedata);
	if(frame->start_time > t1 && frame->start_time <= t1 + period){
		OBJECT *obj = OBJECTHDR(this);
		MARKETFRAME_CCSI *nframe = frame;
		unsigned int i = 0;
		STATISTIC_CCSI *stat;
...
TIMESTAMP auction_ccsi::pop_market_frame(TIMESTAMP t1){
	MARKETFRAME_CCSI *frame = 0;
	OBJECT *obj = OBJECTHDR(this);
	STATISTIC_CCSI *stat = stats;
	double *stats = 0;
	...
	memcpy(&past_frame, &current_frame, sizeof(MARKETFRAME_CCSI)); // not copying lagging stats
	// ~ copy new data in
...
TIMESTAMP auction_ccsi::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	if (clearat==TS_ZERO)
	{
		clearat = nextclear();
...
TIMESTAMP auction_ccsi::sync(TIMESTAMP t0, TIMESTAMP t1){
	if (clearat2==TS_ZERO)
	{
		clearat2 = nextclear();
		DATETIME dt;
		gl_localtime(clearat2,&dt);
		update_statistics();
	}
	if (t1>=clearat2)
	{
...
TIMESTAMP auction_ccsi::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	return -clearat; /* soft return t2>t1 on success, t2=t1 for retry, t2<t1 on failure */
}

void auction_ccsi::record_curve(double bu, double su){
	char name[64];
	char timestr[64];
	DATETIME dt;
	unsigned int i = 0;
...
void auction_ccsi::clear_market(void)
{
	unsigned int sph24 = (unsigned int)(3600/period*24);
	BID_CCSI unresponsive;
		...
		if(strcmp(unit, "") != 0){
			if(capacity_reference_property->unit != 0){
				if(gl_convert(capacity_reference_property->unit->name,unit.get_string(),&refload) == 0){
					...
					throw msg;
					/* TROUBLESHOOT
						If capacity_reference_property has units specified, the units must be convertable to the units used by its auction_ccsi object.
						*/
				} else if (verbose){
	...
	} else if ((asks.getcount()>0) && offers.getcount()>0)
	{
		...
		BID_CCSI *buy = asks.getbid(i), *sell = offers.getbid(j);
		BID_CCSI clear = {NULL,0,0};
	...
	if(latency > 0){
		...
		STATISTIC_CCSI *stat = 0;
		OBJECT *obj = OBJECTHDR(this);
		memcpy(&past_frame, &current_frame, sizeof(MARKETFRAME_CCSI)); // just the frame
		// ~ copy new data in
...
int auction_ccsi::submit(char *from, double quantity, double real_price, KEY key, BIDDERSTATE state, bool rebid, int64 mkt_id)
{
	gld_wlock lock(my());
	return submit_nolock(from,quantity,real_price,key,state, rebid, mkt_id);
}
int auction_ccsi::submit_nolock(char *from, double quantity, double real_price, KEY key, BIDDERSTATE state, bool rebid, int64 mkt_id)
{
	char myname[64];
	TIMESTAMP submit_time = gl_globalclock;
	DATETIME dt;
	...
	if(rebid == false){ // new bid ~ rebuild key
		biddef.bid = -1;
		biddef.bid_type = BID_CCSI_UNKNOWN;
		biddef.market = -1;
		biddef.raw = -1;
	} else {
	...
	else if (mkt_id == market_id && rebid == true) // resubmit
	{
		...
		BID_CCSI bid = {from,b_id,fabs(quantity),price,state};
		if (quantity < 0){
		...
		biddef.bid_type = (quantity > 0 ? BID_CCSI_SELL : BID_CCSI_BUY);
...
EXPORT int create_auction_ccsi(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(auction_ccsi::oclass);
		if (*obj!=NULL)
		{
			auction_ccsi *my = OBJECTDATA(*obj,auction_ccsi);
	...
	CREATE_CATCHALL(auction_ccsi);
}

EXPORT int init_auction_ccsi(OBJECT *obj, OBJECT *parent)
{
	try
	{
		if (obj!=NULL)
			return OBJECTDATA(obj,auction_ccsi)->init(parent);
		else
			return 0;
	}
	INIT_CATCHALL(auction_ccsi);
}

EXPORT int isa_auction_ccsi(OBJECT *obj, char *classname)
{
	if(obj != 0 && classname != 0){
		return OBJECTDATA(obj,auction_ccsi)->isa(classname);
	} else {
		return 0;
	}
}

EXPORT TIMESTAMP sync_auction_ccsi(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
