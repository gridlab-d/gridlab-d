...
// ref:
// http://www.digitalmars.com/pnews/read.php?server=news.digitalmars.com&group=c++&artnum=3659
/***************************
 *   erf.cpp
 *   author:  Steve Strand
 *   written: 29-Jan-04
...
static const double rel_error =
    1E-14; // calculate 12^N^N 14 significant figures
...
// only
{
  static const double one_sqrtpi =
      0.564189583547756287; // 1/sqrt(pi)
                            //    if (fabs(x) < 2.2) {
                            //        return 1.0 - erf(x);        //use series
                            //        when fabs(x) < 2.2
                            //    }
...
passive_controller::passive_controller(MODULE *mod) {
  if (oclass == nullptr) {
    oclass = gl_register_class(
        mod, "passive_controller", sizeof(passive_controller),
        PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            ...
            // outputs
            PT_double, "observation", PADDR(observation), PT_ACCESS,
            PA_REFERENCE, PT_DESCRIPTION, "the observed value", PT_double,
            "mean_observation", PADDR(obs_mean), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the observed mean value", PT_double,
            "stdev_observation", PADDR(obs_stdev), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the observed standard deviation value", PT_double,
            "expectation", PADDR(expectation), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the observed expected value",
            // inputs
            PT_double, "period[s]", PADDR(dPeriod), PT_DESCRIPTION,
            "the cycle period for the controller logic", PT_char32,
            "expectation_property", PADDR(expectation_propname), PT_DESCRIPTION,
            "the name of the property to observe for the expected value",
            PT_object, "expectation_object", PADDR(expectation_object),
            PT_DESCRIPTION, "the object to watch for the expectation property",
            PT_char32, "setpoint", PADDR(output_setpoint_propname),
            PT_DESCRIPTION,
            "the name of the setpoint property in the parent object", PT_char32,
            "state_property", PADDR(output_state_propname), PT_DESCRIPTION,
            "the name of the actuator property in the parent object", PT_object,
            "observation_object", PADDR(observation_object), PT_DESCRIPTION,
            "the object to observe", PT_char32, "observation_property",
            PADDR(observation_propname), PT_DESCRIPTION,
            "the name of the observation property",
            /**/ PT_char32, "mean_observation_property",
            PADDR(observation_mean_propname), PT_DESCRIPTION,
            "the name of the mean observation property", PT_char32,
            "stdev_observation_property", PADDR(observation_stdev_propname),
            PT_DESCRIPTION,
            "the name of the standard deviation observation property",
            PT_double, "base_setpoint", PADDR(base_setpoint), PT_DESCRIPTION,
            "the base setpoint to base control off of", PT_double,
            "critical_day", PADDR(critical_day), PT_DESCRIPTION,
            "used to switch between TOU and CPP days, 1 is CPP, 0 is TOU",
            PT_bool, "two_tier_cpp", PADDR(check_two_tier_cpp), PT_double,
            "daily_elasticity", PADDR(dailyElasticity), PT_double,
            "sub_elasticity_first_second", PADDR(subElasticityFirstSecond),
            PT_double, "sub_elasticity_first_third",
            PADDR(subElasticityFirstThird), PT_int32, "second_tier_hours",
            PADDR(secondTierHours), PT_int32, "third_tier_hours",
            PADDR(thirdTierHours), PT_int32, "first_tier_hours",
            PADDR(firstTierHours), PT_double, "first_tier_price",
            PADDR(firstTierPrice), PT_double, "second_tier_price",
            PADDR(secondTierPrice), PT_double, "third_tier_price",
            PADDR(thirdTierPrice), PT_double, "old_first_tier_price",
            PADDR(oldFirstTierPrice), PT_double, "old_second_tier_price",
            PADDR(oldSecondTierPrice), PT_double, "old_third_tier_price",
            PADDR(oldThirdTierPrice), PT_double, "Percent_change_in_price",
            PADDR(dailyElasticityMultiplier), PT_double,
            "Percent_change_in_peakoffpeak_ratio",
            PADDR(peakPriceMultiplier_test), PT_double,
            "Percent_change_in_Criticalpeakoffpeak_ratio",
            PADDR(criticalpeakPriceMultiplier_test), PT_bool,
            "linearize_elasticity", PADDR(linearizeElasticity), PT_double,
            "price_offset", PADDR(price_offset), PT_bool, "pool_pump_model",
            PADDR(pool_pump_model), PT_DESCRIPTION,
            "Boolean flag for turning on the pool pump version of the "
            "DUTYCYCLE control",
            PT_double, "base_duty_cycle", PADDR(base_duty_cycle),
            PT_DESCRIPTION,
            "This is the duty cycle before modification due to the price "
            "signal",

            // Primary Frequency Control implementation
            PT_int32, "trigger_time_under_frequency", PADDR(trigger_time_under),
            PT_DESCRIPTION, "Time to stay in triggered off state in seconds",
            PT_int32, "trigger_time_over_frequency", PADDR(trigger_time_over),
            PT_DESCRIPTION, "Time to stay in triggered on state in seconds",
            PT_int32, "release_time_under_frequency", PADDR(release_time_under),
            PT_DESCRIPTION, "Time to stay in released on state in seconds",
            PT_int32, "release_time_over_frequency", PADDR(release_time_over),
            PT_DESCRIPTION, "Time to stay in released off state in seconds",
            PT_double, "release_point_under_frequency",
            PADDR(release_freq_under), PT_DESCRIPTION,
            "Frequency value for releasing GFA in under frequency mode",
            PT_double, "release_point_over_frequency", PADDR(release_freq_over),
            PT_DESCRIPTION,
            "Frequency value for releasing GFA in over frequency mode",
            PT_double, "trigger_point_under_frequency",
            PADDR(trigger_freq_under), PT_DESCRIPTION,
            "Frequency value for triggereing GFA in under frequency mode",
            PT_double, "trigger_point_over_frequency", PADDR(trigger_freq_over),
            PT_DESCRIPTION,
            "Frequency value for triggereing GFA in over frequency mode",
            PT_double, "frequency", PADDR(frequency), PT_DESCRIPTION,
            "Frequency value", PT_enumeration, "PFC_mode", PADDR(PFC_mode),
            PT_DESCRIPTION,
            "operation mode of the primary frequency controller", PT_KEYWORD,
            "OVER_FREQUENCY", (enumeration)OVER_FREQUENCY, PT_KEYWORD,
            "UNDER_FREQUENCY", (enumeration)UNDER_FREQUENCY, PT_KEYWORD,
            "OVER_UNDER_FREQUENCY", (enumeration)OVER_UNDER_FREQUENCY,
            PT_enumeration, "PFC_state", PADDR(PFC_state), PT_DESCRIPTION,
            "State of the primary frequency controller", PT_KEYWORD, "FREE",
            (enumeration)FREE, PT_KEYWORD, "TRIGGERED_OFF",
            (enumeration)TRIGGERED_OFF, PT_KEYWORD, "TRIGGERED_ON",
            (enumeration)TRIGGERED_ON, PT_KEYWORD, "FORCED_OFF",
            (enumeration)FORCED_OFF, PT_KEYWORD, "FORCED_ON",
            (enumeration)FORCED_ON, PT_KEYWORD, "RELEASED_OFF",
            (enumeration)RELEASED_OFF, PT_KEYWORD, "RELEASED_ON",
            (enumeration)RELEASED_ON, PT_char32, "state_observed",
            PADDR(state_observed_propname), PT_DESCRIPTION,
            "the name of the observed state property in the parent object",
            PT_char32, "power_observed", PADDR(power_observed_propname),
            PT_DESCRIPTION,
            "the name of the observed state property in the parent object",
            PT_int32, "output_observed", PADDR(state_observed), PT_int32,
            "bid_delay", PADDR(bid_delay), PT_DESCRIPTION,
            "time the controller will bid in advance before clearing",
            PT_double, "voltage_lockout[%]", PADDR(voltage_lockout),
            PT_DESCRIPTION,
            "lockout primary frequency control if voltage is deviating % from "
            "nominal",
            PT_double, "voltage_lockout_time[s]",
            PADDR(time_in_voltage_lockout), PT_DESCRIPTION,
            "voltage lockout period", PT_int32, "voltage_lockout_state",
            PADDR(voltage_lockout_state), PT_DESCRIPTION,
            ...
            PT_KEYWORD, "NORMAL", (enumeration)PDT_NORMAL, PT_KEYWORD,
            "EXPONENTIAL", (enumeration)PDT_EXPONENTIAL, PT_KEYWORD, "UNIFORM",
            (enumeration)PDT_UNIFORM, PT_double, "comfort_level",
            PADDR(comfort_level),
            // specific inputs
            PT_double, "range_high", PADDR(range_high), PT_double, "range_low",
            PADDR(range_low), PT_double, "ramp_high", PADDR(ramp_high),
            PT_double, "ramp_low", PADDR(ramp_low),
            // specific outputs
            /**/ PT_double, "prob_off", PADDR(prob_off),
            /**/ PT_int32, "output_state", PADDR(output_state), PT_ACCESS,
            ...
            PT_KEYWORD, "NONE", (enumeration)CM_NONE, PT_KEYWORD, "RAMP",
            (enumeration)CM_RAMP, PT_KEYWORD, "DUTYCYCLE",
            (enumeration)CM_DUTYCYCLE, PT_KEYWORD, "PROBABILITY_OFF",
            (enumeration)CM_PROBOFF, PT_KEYWORD, "ELASTICITY_MODEL",
            (enumeration)CM_ELASTICITY_MODEL, PT_KEYWORD, "DIRECT_LOAD_CONTROL",
            (enumeration)CM_DLC, PT_KEYWORD, "PRIMARY_FREQUENCY_CONTROL",
            (enumeration)CM_PFC, PT_enumeration, "dlc_mode", PADDR(dlc_mode),
            PT_DESCRIPTION,
            "the control mode to use in conjunction with DIRECT_LOAD_CONTROL",
            PT_KEYWORD, "OFF", (enumeration)DLC_OFF, PT_DESCRIPTION,
            "this mode is designed to override standard behavior and turn off "
            "the load",
            PT_KEYWORD, "CYCLING", (enumeration)DLC_CYCLING, PT_DESCRIPTION,
            "this mode is roughly designed to force cycle an AC unit",
            PT_double, "cycle_length_off[s]", PADDR(cycle_off), PT_double,
            "cycle_length_on[s]", PADDR(cycle_on), nullptr) < 1) {
...
void passive_controller::fetch_double(double **prop, const char *name,
                                      OBJECT *parent) {
...
void passive_controller::fetch_int(int **prop, const char *name,
                                   OBJECT *parent) {
  OBJECT *hdr = object_header(this);
...
int passive_controller::init(OBJECT *parent) {
...
#ifdef __APPLE__
  parent =
      hdr->parent; // AppleClang seems to have an issue with the parent pointer
#endif
  PROPERTY *enduseProperty;

  if (parent == nullptr) {
    gl_error("passive_controller has no parent and will be operating in "
             "'dummy' mode");
  } else {
    ...
    if (expectation_object != 0) {
      expectation_property =
          gl_get_property(expectation_object, expectation_propname);
      if (expectation_property == 0) {
        GL_THROW("passive_controller cannot find its expectation property");
      }
      expectation_addr =
          (void *)((unsigned int64)expectation_object + sizeof(OBJECT) +
                   (unsigned int64)expectation_property->addr);
    }

    if (observation_object != 0) {
      // observation_addr
      observation_prop =
          gl_get_property(observation_object, observation_propname);
      if (observation_prop != 0) {
        observation_addr =
            (void *)((unsigned int64)observation_object + sizeof(OBJECT) +
                     (unsigned int64)observation_prop->addr);
      }
      if (pool_pump_model == false) {
        // observation_mean_addr
        observation_mean_prop =
            gl_get_property(observation_object, observation_mean_propname);
        if (observation_mean_prop != 0) {
          observation_mean_addr =
              (void *)((unsigned int64)observation_object + sizeof(OBJECT) +
                       (unsigned int64)observation_mean_prop->addr);
        }
        // observation_stdev_addr
        stdev_observation_property =
            gl_get_property(observation_object, observation_stdev_propname);
        if (stdev_observation_property != 0) {
          observation_stdev_addr =
              (void *)((unsigned int64)observation_object + sizeof(OBJECT) +
                       (unsigned int64)stdev_observation_property->addr);
        }
      }
    }
    // output_state
    if (output_state_propname[0] != 0) {
      ...
      if (output_state_prop == nullptr) {
        GL_THROW(
            "passive_controller parent \"%s\" does not contain property \"%s\"",
            (parent->name ? parent->name : "anon"),
            output_state_propname.get_string());
      }
      output_state_addr = (void *)((unsigned int64)parent + sizeof(OBJECT) +
                                   (unsigned int64)output_state_prop->addr);
    }

    // output_setpoint
    if (control_mode != this->CM_PROBOFF &&
        control_mode != this->CM_ELASTICITY_MODEL &&
        control_mode != this->CM_DLC && control_mode != this->CM_PFC) {
      if (output_setpoint_propname[0] == 0 &&
          output_setpoint_propname[0] == 0) {
        GL_THROW("passive_controller has no output properties");
      }
      if (output_setpoint_propname[0] != 0) {
        output_setpoint_property =
            gl_get_property(parent, output_setpoint_propname);
        if (output_setpoint_property == nullptr) {
          GL_THROW("passive_controller parent \"%s\" does not contain property "
                   "\"%s\"",
                   (parent->name ? parent->name : "anon"),
                   output_setpoint_propname.get_string());
        }
        output_setpoint_addr =
            (void *)((unsigned int64)parent + sizeof(OBJECT) +
                     (unsigned int64)output_setpoint_property->addr);
      }
    }
  }

  if (pool_pump_model == false && control_mode != CM_ELASTICITY_MODEL &&
      control_mode != CM_DLC && control_mode != CM_PFC)
    gl_set_dependent(hdr, expectation_object);

  if (control_mode != CM_PFC ||
      (control_mode == CM_PFC && observation_object != 0)) {
    gl_set_dependent(hdr, observation_object);
    if (observation_object == nullptr) {
      GL_THROW("passive_controller observation_object object is undefined, and "
               "can not function");
    }
  }

  //
  // make sure that the observation_object and expectable are ranked above the
  // controller
  //
  ...
  if (pool_pump_model == true) {
    if (control_mode != CM_DUTYCYCLE)
      GL_THROW(
          "pool pump mode must be used with control mode set to DUTYCYCLE");
    if (firstTierHours == 0 || secondTierHours == 0)
      GL_THROW("Please set first and second tier hours in pool pump duty cycle "
               "mode");
    if (firstTierPrice == 0 || secondTierPrice == 0)
      GL_THROW("Please set first and second tier prices in pool pump duty "
               "cycle mode");
  }

  if (control_mode == CM_DLC) {
    if (secondTierPrice == 0)
      GL_THROW("Please set second tier price in DLC mode. This is used as the "
               "breakpoint to determine when to activate DLC.");
    if (dlc_mode == DLC_CYCLING) {
      if (cycle_off == 0 || cycle_on == 0)
        GL_THROW("Please set cycle_on and cycle_off in DLC modes to greater "
                 "than zero.");
      else {
  ...
  if (control_mode == CM_PFC) {
    if (state_observed_propname[0] != 0) {
      ...
      if (state_observed_prop == nullptr) {
        GL_THROW(
            "passive_controller parent \"%s\" does not contain property \"%s\"",
            (parent->name ? parent->name : "anon"),
            state_observed_propname.get_string());
      }
      state_observed_addr = (void *)((unsigned int64)parent + sizeof(OBJECT) +
                                     (unsigned int64)state_observed_prop->addr);
    } else
    ...
    if (voltage_lockout != 0 ||
        observation_object != 0) { // we need to check the voltage of the parent
    ...
    if (trigger_time_under < 1)
      GL_THROW("Please set trigger time for under frequency to a minimum of 1 "
               "seconds.");
    if (trigger_time_over < 1)
      GL_THROW("Please set trigger time for over frequency to a minimum of 1 "
               "seconds.");
    if (release_time_under < 1)
      GL_THROW("Please set release time for under frequency to a minimum of 1 "
               "seconds.");
    if (release_time_over < 1)
      GL_THROW("Please set release time for over frequency to a minimum of 1 "
               "seconds.");
    if (release_freq_under <= 0)
      GL_THROW("Please set release frequency time for under frequency to a "
               "valid number");
    if (release_freq_over <= 0)
      GL_THROW("Please set release frequency time for over frequency to a "
               "valid number");
  }
  ...
  if (zipLoadParent == true && control_mode == CM_ELASTICITY_MODEL) {
    ...
    if (subElasticityFirstSecond > 0)
      gl_warning(
          "The peak to offpeak Substitution Elasticity is positive.  While "
          "this is allowed, it is typically the reverse of convention, as an "
          "increase in peak to offpeak price ratio generally produces a "
          "reduction in peak load to offpeak load. This is indicated by a "
          "negative peak to offpeak Substitution Elasticity value.");

    if (subElasticityFirstThird > 0)
      gl_warning("The critical peak to offpeak Substitution Elasticity is "
                 "positive.  While this is allowed, it is typically the "
                 "reverse of convention, as an increase in critical peak to "
                 "offpeak price ratio generally produces a reduction in peak "
                 "load to offpeak load. This is indicated by a negative "
                 "critical peak to offpeak Substitution Elasticity value.");

    if (dailyElasticity > 0)
      gl_warning("The Daily Elasticity is positive.  While this is allowed, it "
                 "is typically the reverse of convention, as an increase in "
                 "daily price generally produces a reduction in daily load. "
                 "This is indicated by a negative Daily Elasticity value.");
    ...
    if (critical_day >= 0.5) {
      ...
      if (oldThirdTierPrice == 0) {
        ...
        gl_warning("Old third tier price is missing. System will assume the "
                   "old pricing scheme was a fixed pricing scheme and use the "
                   "old first tier price");
      }
      ...
      if (check_two_tier_cpp != true) {
        ...
        if (oldSecondTierPrice == 0) {
          ...
          gl_warning("Old second tier price is missing. System will assume the "
                     "old pricing scheme was a fixed pricing scheme and use "
                     "the old first tier price");
        }

        if (firstTierHours == 0) {
          firstTierHours = elasticityPeriod - thirdTierHours - secondTierHours;
        } else {
          if ((thirdTierHours + secondTierHours + firstTierHours) !=
              elasticityPeriod)
            GL_THROW("Please set the tier hours correctly in the three tier "
                     "Elasticity Model");
        }
        ...
        if (linearizeElasticity == true) {
          peakPriceMultiplier =
              pow(newPriceRatioSecondFirst / oldPriceRatioSecondFirst,
                  subElasticityFirstSecond);
        } else {

          peakPriceMultiplier =
              1 + ((subElasticityFirstSecond) *
                   (newPriceRatioSecondFirst - oldPriceRatioSecondFirst) /
                   oldPriceRatioSecondFirst);
        ...
        //	GL_THROW("Please set the tier hours correctly in the Elasticity
        //Model");
        //}
      }
      ...
      if (linearizeElasticity == true) {
        criticalPriceMultiplier =
            pow(newPriceRatioThirdFirst / oldPriceRatioThirdFirst,
                subElasticityFirstThird);
      } else {

        criticalPriceMultiplier =
            1 + ((subElasticityFirstThird) *
                 (newPriceRatioThirdFirst - oldPriceRatioThirdFirst) /
                 oldPriceRatioThirdFirst);
      ...
      if (firstTierHours == 0) {
        ...
        if ((secondTierHours + firstTierHours) != elasticityPeriod)
          GL_THROW(
              "Please set the tier hours correctly in the Elasticity Model");
      }
    ...
    // Initialize all the arrays for 24 hours to avoid reinitializing and
    // re-mallocing
  ...
  if (last_cycle == 0 || t1 >= last_cycle + period || period == 0) {
    ...
    case CM_PROBOFF:
      if (calc_proboff(t0, t1) != 0) {
        GL_THROW("error occured when handling the probabilistic cutoff control "
                 "mode");
      }
      break;
    case CM_DUTYCYCLE:
    ...
    case CM_ELASTICITY_MODEL:
      if (calc_elasticity(t0, t1) != 0) {
        GL_THROW(
            "error occured when handling the elasticity model control mode");
      }
      break;
    case CM_DLC:
      if (calc_dlc(t0, t1) != 0) {
...
TIMESTAMP passive_controller::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (control_mode == CM_PFC &&
      observation_object != 0) { // we have a supervisor
    if (t1 == supervisor_next_run -
                  bid_delay) { // time to submit state and power to supervisor

      if (*stateParent == 0) { // water heater is off and we need to bid rated
      ...
      if (0 != strcmp(market->unit, "")) {
        if (0 == gl_convert("kW", market->unit, &powerParentConverted)) {
          gl_error("unable to convert bid units from 'kW' to '%s'",
                   market->unit.get_string());
          return TS_INVALID;
        }
      }

      // double voltage_deviation = fabs(1-(*voltageParent /
      // *nomVoltageParent));
      ...
      submit_bid_state((char *)gl_name(hdr, ctrname, 1024),
                       (char *)gl_name(observation_object, spvrname, 1024),
                       const_cast<char *>("submit_bid_state"),
                       const_cast<char *>("supervisor"),
                       (void *)&controller_bid, (size_t)sizeof(controller_bid));
      controller_bid.rebid = true;
      if (!controller_bid.bid_accepted) {
        return TS_INVALID;
      }
      supervisor_next_run +=
          (TIMESTAMP)*supervisorPeriod; // calculate next time supervisor will
                                        // clear
    }
...
int passive_controller::calc_elasticity(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (zipLoadParent == true) {
    ...
    if (old_critical_day != critical_day) {
      ...
      if (dailyElasticity > 0)
        gl_warning(
            "The Daily Elasticity is positive.  While this is allowed, it is "
            "typically the reverse of convention, as an increase in daily "
            "price generally produces a reduction in daily load. This is "
            "indicated by a negative Daily Elasticity value.");
      ...
      if (critical_day >= 0.5) {
        ...
        if (check_two_tier_cpp != true) {
          ...
          if (oldSecondTierPrice == 0) {
            oldSecondTierPrice = oldFirstTierPrice;
            gl_warning("Old second tier price is missing. System will assume "
                       "the old pricing scheme was a fixed pricing scheme and "
                       "use the old first tier price");
          }

          if (firstTierHours == 0) {
            firstTierHours =
                elasticityPeriod - thirdTierHours - secondTierHours;
          } else {
            if ((thirdTierHours + secondTierHours + firstTierHours) !=
                elasticityPeriod)
              GL_THROW("Please set the tier hours correctly in the three tier "
    ...
    if ((t1 - starttime) % period == 0 && (t1 != starttime) && starttime > 0 &&
      ...
      /*if(fabs(observation - thirdTierPrice) <= price_offset){
              ...
              criticalPeakLoad[ThirdTierArrayIndex] = *(double
      *)current_load_enduse;
      ...
      if (fabs(observation - thirdTierPrice) <= price_offset) {
        ...
        for (int32 i = ArraySize - 1;
             i > (ArraySize - (SecondTierArraySize - SecondTierArrayIndex));
             i--) {
          totalPeakLoad += peakLoad[i] * dPeriod;
        }
        ...
        for (int32 i = ArraySize - 1;
             i > (ArraySize - (FirstTierArraySize - FirstTierArrayIndex));
             i--) {
          totalOffPeakLoad += offPeakLoad[i] * dPeriod;
        }
        ...
        GL_THROW(
            "Market price signal does not match with any of the tier prices");
      }
    }
    ...
    if (starttime > 0) {
      ...
      if (critical_day >= 0.5) {
        ...
        if (check_two_tier_cpp == true) {
          ...
          if (linearizeElasticity == true) {
            if ((totalCriticalPeakLoad * oldThirdTierPrice +
                 totalOffPeakLoad * oldFirstTierPrice) == 0) {
              GL_THROW("Either the last 24 hour Critical Peak and Off Peak "
                       "Load information was input as zero and/or one of the "
                       "old tier prices(Third or First) was input as zero.");
            } else {
              dailyElasticityMultiplier =
                  (totalCriticalPeakLoad * thirdTierPrice +
                   totalOffPeakLoad * firstTierPrice) /
                  (totalCriticalPeakLoad * oldThirdTierPrice +
                   totalOffPeakLoad * oldFirstTierPrice);
            }
          } else {
            if ((totalCriticalPeakLoad * thirdTierPrice +
                 totalOffPeakLoad * firstTierPrice) == 0) {
              GL_THROW("Either the last 24 hour Critical Peak and Off Peak "
                       "Load information was input as zero and/or one of the "
                       "new tier prices(Third or First) was input as zero.");
            } else {
              dailyElasticityMultiplier =
                  (totalCriticalPeakLoad * priceDiffThird +
                   totalOffPeakLoad * priceDiffFirst) /
                  (totalCriticalPeakLoad * thirdTierPrice +
                   totalOffPeakLoad * firstTierPrice);
            }
          }
          ...
          totalCriticalPeakLoad =
              totalClearedLoad - totalPeakLoad - totalOffPeakLoad;

          if (linearizeElasticity == true) {
            if ((totalCriticalPeakLoad * oldThirdTierPrice +
                 totalPeakLoad * oldSecondTierPrice +
                 totalOffPeakLoad * oldFirstTierPrice) == 0) {
              GL_THROW(
                  "Either the last 24 hour Critical Peak, Peak and Off Peak "
                  "Load information was input as zero and/or one of the old "
                  "tier prices(Third, Second or First) was input as zero.");
            } else {
              dailyElasticityMultiplier =
                  (totalCriticalPeakLoad * thirdTierPrice +
                   totalPeakLoad * secondTierPrice +
                   totalOffPeakLoad * firstTierPrice) /
                  (totalCriticalPeakLoad * oldThirdTierPrice +
                   totalPeakLoad * oldSecondTierPrice +
                   totalOffPeakLoad * oldFirstTierPrice);
            ...
            if ((totalPeakLoad * secondTierPrice +
                 totalCriticalPeakLoad * thirdTierPrice +
                 totalOffPeakLoad * firstTierPrice) == 0) {
              GL_THROW(
                  "Either the last 24 hour Critical Peak, Peak and Off Peak "
                  "Load information was input as zero and/or one of the new "
                  "tier prices(Third, Second or First) was input as zero.");
            } else {
              dailyElasticityMultiplier =
                  (totalCriticalPeakLoad * priceDiffThird +
                   totalPeakLoad * priceDiffSecond +
                   totalOffPeakLoad * priceDiffFirst) /
                  (totalPeakLoad * secondTierPrice +
                   totalCriticalPeakLoad * thirdTierPrice +
                   totalOffPeakLoad * firstTierPrice);
        ...
        if (linearizeElasticity == true) {
          if ((totalPeakLoad * oldSecondTierPrice +
               totalOffPeakLoad * oldFirstTierPrice) == 0) {
            GL_THROW("Either the last 24 hour Peak and Off Peak Load "
                     "information was input as zero and/or one of the old tier "
                     "prices(Second or First) was input as zero.");
          } else {
            dailyElasticityMultiplier = (totalPeakLoad * secondTierPrice +
                                         totalOffPeakLoad * firstTierPrice) /
                                        (totalPeakLoad * oldSecondTierPrice +
                                         totalOffPeakLoad * oldFirstTierPrice);
          }
          ...
          if ((totalPeakLoad * secondTierPrice +
               totalOffPeakLoad * firstTierPrice) == 0) {
            GL_THROW("Either the last 24 hour Peak and Off Peak Load "
                     "information was input as zero and/or one of the new tier "
                     "prices(Second or First) was input as zero.");
          } else {
            dailyElasticityMultiplier = (totalPeakLoad * priceDiffSecond +
                                         totalOffPeakLoad * priceDiffFirst) /
                                        (totalPeakLoad * secondTierPrice +
                                         totalOffPeakLoad * firstTierPrice);
          }
      ...
      if (linearizeElasticity == true) {
        qDailyNew =
            totalClearedLoad * pow(dailyElasticityMultiplier, dailyElasticity);
      } else {

        qDailyNew = totalClearedLoad *
                    (1 + ((dailyElasticity)*dailyElasticityMultiplier));
      }
      ...
      if (qDailyNew > 0.0) {
        ...
        if (critical_day >= 0.5) {
          ...
          if (totalOffPeakLoad == 0) {
            ...
            tempCriticalPeakMultiplier =
                (totalCriticalPeakLoad / totalOffPeakLoad) *
                criticalPriceMultiplier;

            if (check_two_tier_cpp != true) {

              tempPeakMultiplier =
                  (totalPeakLoad / totalOffPeakLoad) * peakPriceMultiplier;
            }
          }
        } else {
        ...
        if (fabs(observation - firstTierPrice) <= price_offset) {
          ...
          if (totalOffPeakLoad == 0) {
            ...
            *(double *)output_state_addr =
                1 + (qNewFirst - totalOffPeakLoad) / totalOffPeakLoad;
          }
        } else if (fabs(observation - secondTierPrice) <= price_offset) {
          ...
          if (totalPeakLoad == 0) {
            ...
            *(double *)output_state_addr =
                1 + (qNewSecond - totalPeakLoad) / totalPeakLoad;
          }
          ...
          if (totalCriticalPeakLoad == 0) {
            ...
            *(double *)output_state_addr =
                1 + (qNewThird - totalCriticalPeakLoad) / totalCriticalPeakLoad;
          }
        } else {
          GL_THROW(
              "Market price signal does not match with any of the tier prices");
...
int passive_controller::calc_ramp(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (expectation_addr == 0 || observation_addr == 0 ||
      observation_stdev_addr == 0) {
  ...
  */
  ...
  // T_limit = (observation > expectation && ramp > 0.0 ? range_high :
  // range_low);
  ...
  if (obs_stdev < bid_offset) {
    ...
    set_change =
        (observation - expectation) * fabs(T_limit) / (ramp * obs_stdev);
  }
...
int passive_controller::calc_dutycycle(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (pool_pump_model == true) {
    ...
    // Note:  this method is only currently compatible w/ 2 tier TOU or 2 tier
    // TOU + CPP
    if (thirdTierPrice != 0.0 && observation >= thirdTierPrice) // we're in CPP
    {
      output_state = 1; // turn on override
                        // if (output_setpoint == -1) // if recovery_duty_cycle
                        // hasn't been set, use this value
    ...
    } else if (observation >= secondTierPrice) // we're in high TOU
    {
      ...
      {
        double hour_ratio =
            (double)secondTierHours / (firstTierHours + secondTierHours);
        output_setpoint =
            base_duty_cycle *
            (firstTierPrice / (firstTierPrice + secondTierPrice)) / hour_ratio;
      }
    } else // we're in low TOU
    {
      ...
      {
        double hour_ratio =
            (double)firstTierHours / (firstTierHours + secondTierHours);
        output_setpoint =
            base_duty_cycle *
            (secondTierPrice / (firstTierPrice + secondTierPrice)) / hour_ratio;
      }
    }
  }
  return 0;
...
int passive_controller::calc_proboff(TIMESTAMP t0, TIMESTAMP t1) {
  if (observation_addr == 0 || expectation_addr == 0 ||
      observation_stdev_addr == 0) {
...
int passive_controller::calc_pfc(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (output_state_addr != 0) {
    output_state =
        *(int *)output_state_addr; // keep water heater in previous state
  } else {
  ...
  if (state_observed_addr != 0) {
    state_observed =
        *(double *)state_observed_addr; // get the state of the parent
  } else {
    state_observed = 0;
  }

  if (PFC_state == FREE) {
    time_in_trigger_under = time_in_trigger_over =
        t1; // reset the timers for triggering the device
    if (frequency < trigger_freq_under &&
        (PFC_mode == OVER_UNDER_FREQUENCY || PFC_mode == UNDER_FREQUENCY)) {
      PFC_state = TRIGGERED_OFF;
    } else if (frequency > trigger_freq_over &&
               (PFC_mode == OVER_UNDER_FREQUENCY ||
  ...
  if (PFC_state == FREE || PFC_state == TRIGGERED_OFF ||
      PFC_state == TRIGGERED_ON) {
  ...
  if (voltage_lockout_time + time_in_voltage_lockout > t1) {
    ...
    if (voltage_lockout != 0 &&
        ((*voltageParent / *nomVoltageParent) > (1 + voltage_lockout / 100) ||
         (*voltageParent / *nomVoltageParent) < (1 - voltage_lockout / 100))) {
  ...
  // determine if we need to set the water heater to neutral due to local
  // voltage issues
  //	if (voltage_lockout != 0 && ((*voltageParent / *nomVoltageParent) >
  //(1+voltage_lockout / 100) || (*voltageParent / *nomVoltageParent) <
  //(1-voltage_lockout / 100))) { 		gl_debug("voltage problems on %s Voltage
  //(%.2f) Voltage PU (%.2f) \n",hdr->name,*voltageParent,(*voltageParent /
  //*nomVoltageParent)); 		voltage_lockout_time = t1; 		output_state = 0; //water
  //heater neutral 		voltage_lockout_state = 1;
  //	}
  //	else {
  //		voltage_lockout_state = 0;
  //	}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_passive_controller(OBJECT *obj,
                                                        TIMESTAMP t1,
                                                        PASSCONFIG pass) {
  return sync_passive_controller_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_passive_controller(OBJECT *obj, ...) {
  va_list ap;
  ...
  PASSCONFIG pass;
  va_start(ap, obj);
  t1 = va_arg(ap, TIMESTAMP);
  pass = va_arg(ap, PASSCONFIG);
  va_end(ap);
  return sync_passive_controller_impl(obj, t1, pass);
}
#endif
