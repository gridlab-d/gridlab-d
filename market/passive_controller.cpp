...
int passive_controller::init(OBJECT *parent)
{
	...
	if (control_mode == CM_PFC)
	{
		...
		if (voltage_lockout != 0 || observation_object != 0)
		{ // we need to check the voltage of the parent
...
TIMESTAMP passive_controller::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (control_mode == CM_PFC && observation_object != 0)
	{ // we have a supervisor
		if (t1 == supervisor_next_run - bid_delay)
		{ // time to submit state and power to supervisor

			if (*stateParent == 0)
			{ // water heater is off and we need to bid rated power instead of actual power
...
int passive_controller::calc_ramp(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (ramp_high * ramp_low < 0 || range_high * range_low > 0)
	{ // zero is legit
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_passive_controller(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
