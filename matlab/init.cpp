/** $Id: init.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        classes are defined using the module GLM statement \verbatim module
 matlab::CLASSNAME; \endverbatim To define an object class in Matlab, you then
 refer to the class name, omitting the
        \p matlab:: module prefix of the name.  The follow methods must be
 defined for each object used in an \p matlab module:
        - \e classname where \e classname is the name you give to the Matlab
 class (see Matlab \p class syntax). This is used as the default constructor as
 well as the converter from char.
        ...
        - \p presync is used to perform \p PRETOPDOWN synchronization for
 GridLAB-D
        - \p sync is used to perform \p BOTTOMUP synchronization for GridLAB-D
        - \p postsync is used to perform \p POSTTOPDOWN synchronization for
 GridLAB-D
        - \p char is used to convert to char
        - \p display is used to show the object's data during debugging

        The constructor must be implemented as follows (replace \p CLASSNAME
 with the classes true name)
        @verbatim
        ...
        if (nargsin==0) % setup call from core
                ...
                global passconfig;
                passconfig = 'PRETOPDOWN|BOTTOMUP|POSTTOPDOWN'; % omit those you
 don't need
        ...
        The \p presync, \p sync, and \p postsync methods must be implemented and
 must be implemented if defined in \p passconfig.  Implementation of \p
 {pre,post}sync is always as follows

        @verbatim
        function [r t2] = sync(my,t0,t1) % define separate methods for each type
 of sync
        ...
        The \p char implementation is required by GridLAB-D's internal data
 conversion routines and must convert the object's data to a character string
 that can be embedded in GLM and XML files. The implementation must be as
 follows

        @verbatim
        function s = char(my)

        s = sprintf('var1="%g kV"; var2="%s";',my.var1,my.var2); % adjust for
 your variables

        end	@endverbatim

        The \p display implementation is necessary for Matlab's internal data
 conversion routines and must convert the object's data to a sequences of output
 lines that can be displayed on the screen.  The implementation must be as
 follows
...
char *matlab_server =
    ""; // use "standalone" to engage single use servers for each run
...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  char env[1024];
  _snprintf(env, sizeof(env), "NEVER=%g;INVALID=%g;", TOSERIAL(TS_NEVER),
            TOSERIAL(TS_INVALID));
  ...
  if (pcfg && mxIsChar(pcfg)) {
    ...
    PROPERTY pctype = {0,       "passconfig", PT_set,  1,    PA_PUBLIC,
                       nullptr, &passconfig,  nullptr, keys, nullptr};
    set passdata;
    if (mxGetString(pcfg, passinfo, sizeof(passinfo)) == 0 &&
        callback->convert.string_to_property(&pctype, &passdata, passinfo) >
            0) {
      ...
      if (gl_publish_variable(oclass, PT_delegated, pDelegate, "data", 0,
                              nullptr) < 1)
        GL_THROW("unable to publish properties in %s", __FILE__);
    } else
      gl_error("passconfig is invalid (expected set of NOSYNC, PRETOPDOWN, "
               "BOTTOMUP, and POSTTOPDOWN)",
               passinfo);
  } else
  ...
  if (ans && mxIsStruct(ans)) {
    ...
    gl_error("result of call to matlab::%s did not return a structure",
             argv[0]);
    ...
    *obj = gl_create_object(oclass, sizeof(mxArray *));
    if (*obj != nullptr) {
      ...
      if (engPutVariable(engine, "object", defaults)) {
        gl_error("matlab::%s_create(...) unable to set defaults for object",
                 oclass->name);
        throw "create failed";
      }
      if (parent) {
      ...
      if (engEvalString(engine, createcall) != 0) {
        gl_error("matlab::%s_create(...) unable to evaluate '%s' in Matlab",
                 oclass->name, createcall);
        throw "create failed";
      } else
        gl_matlab_output();
      mxArray *ans = engGetVariable(engine, "ans");
      ...
      if (ans && mxIsClass(ans, oclass->name))
        ...
        *my = nullptr;
        gl_error(
            "matlab::@%s/create(...) failed to return an object of class %s",
...
EXPORT int init_matlab(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    gl_error("init_matlab(obj=%d;%s): %s", obj->id,
...
static TIMESTAMP sync_matlab_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  ...
  try {
    ...
    switch (pass) {
    case PC_PRETOPDOWN:
      sprintf(synccall, "[ans t2]=presync(object,%g,%g)", TOSERIAL(obj->clock),
              TOSERIAL(t1));
      if ((passconfig & (PC_BOTTOMUP | PC_POSTTOPDOWN)) == 0)
        obj->clock = t1;
      break;
    case PC_BOTTOMUP:
      sprintf(synccall, "[ans t2]=sync(object,%g,%g)", TOSERIAL(obj->clock),
              TOSERIAL(t1));
      if ((passconfig & PC_POSTTOPDOWN) == 0)
        obj->clock = t1;
      break;
    case PC_POSTTOPDOWN:
      sprintf(synccall, "[ans t2]=postsync(object,%g,%g)", TOSERIAL(obj->clock),
              TOSERIAL(t1));
      obj->clock = t1;
      break;
    default:
    ...
    if (ans && mxIsClass(ans, oclass->name)) {
      ...
      gl_error("matlab::%s: %s failed to return an object of class %s",
               oclass->name, synccall, oclass->name);
      throw "sync failed";
    }
    ...
    if (pT2 && mxIsNumeric(pT2)) {
      ...
      gl_error("matlab::@%s/%s failed to return a sync time", oclass->name,
               synccall);
      throw "sync failed";
    }
  } catch (char *msg) {
    gl_error("sync_matlab(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return TS_INVALID;
  }
  return t2;
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_matlab(OBJECT *obj, TIMESTAMP t1,
                                            PASSCONFIG pass) {
