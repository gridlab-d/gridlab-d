/** $Id: tape_memory.cpp
        ...
        The Memory tape type was originally created to provide a work buffer for
 Matlab in memory space that GridLAB-D could access.  It works by allocating a
 large block of memory as a global variable, which can be accessed at any time
 by any module.  The tapes specifically wrap this allocated block with tape
 functionality.

        The global variables referenced by a Memory tape MUST exist before the
 tape attempts to �open� them during the first sync call, and the user must
 include the size limits in the tape properties to prevent memory overflow.
        There does not appear to be any method to data-drive the data to or from
 files.  The contents of a Memory tape must be set or retrieved
 programmatically.

        Memory types are arranged as an interleaved array of double timestamps
 and double values within a malloc�ed block. Properly arranging these values,
 and not overrunning the buffer, is the responsibility of the user.
...
int open_player(struct player *my, char *fname, char *flags) {
  ...
  // gl_error(
  fprintf(stderr,
          "open_player(struct player *my='{...}', char *fname='%s', char "
          "*flags='%s'): global '%s' not defined\n",
          fname, flags, fname);
  return 0;
}
...
char *read_player(struct player *my, char *buffer, unsigned int size) {
  ...
  if (my->memory->index >=
      my->memory->buffer->prop->size - 1) /* need two values */
    return nullptr;
  else {
...
void close_player(struct player *my) { free(my->memory); }
...
static void setmap(char *spec, unsigned char *map, int size) {
  ...
  while (*p != '\0') {
    if (*p == '*') {
      ...
      while (isdigit(*p))
        p++;
    } else if (*p == '-') { /* spanning enabled */
      p++;
    } else if (*p == ';') { /* spanning disabled */
      last = -1;
      p++;
    }
  }
...
int open_shaper(struct shaper *my, char *fname, char *flags) {
  ...
  my->status = TS_OPEN;
  my->type = FT_MEMORY;
  /** @todo shape defaults should be read from the shape memory, or better yet,
   * inferred from it (no ticket) */
  ...
  while (fgets(line, sizeof(line), my->fp) != nullptr) {
    ...
    if (strcmp(group, "") != 0 &&
      ...
      if (min[0] != '*')
        // gl_warning("%s(%d) : minutes are ignored in '%s'", memory, linenum,
        // line);
        hours = hourmap(hour);
      days = daymap(day);
      ...
      if (load > peak)
        peak = load; /* keep the highest load in the shape (that's going to be
                        255) */
    } else if (p[0] == '}') { /* end shape group */
      int h, d, m, w;
        ...
        for (w = 0; w < 7; w++) {
          for (d = 0; d < 31; d++) {
            for (h = 0; h < 24; h++) {
              if (scale[m][d][w][h] < 0)
                my->shape[m][d][w][h] =
                    (unsigned char)(-scale[m][d][w][h] / peak * 255 +
                                    0.5); /* negative removes scaled value
                                             indicator */
            }
          }
        }
      ...
      strcpy(group, "");
    } else if (sscanf(p, "%s {", group) == 1) { /* new shape group */
      sum = 0;
    } else { /* syntax error */
      // gl_error(
...
int rewind_shaper(struct shaper *my) { return fseek(my->fp, SEEK_SET, 0); }
...
int open_recorder(struct recorder *my, char *fname, char *flags) {
  ...
  if (my->memory == nullptr) {
    // gl_error(
    fprintf(stderr,
            "open_recorder(struct recorder *my={...}, char *fname='%s', char "
            "*flags='%s'): %s\n",
...
void close_recorder(struct recorder *my) { free(my->memory); }
...
int open_collector(struct collector *my, char *fname, char *flags) {
  ...
  if (my->memory == nullptr) {
    // gl_error(
    fprintf(stderr,
            "open_collector(struct recorder *my={...}, char *fname='%s', char "
            "*flags='%s'): %s\n",
            fname, flags, strerror(errno));
    my->status = TS_DONE;
    return 0;
...
void close_collector(struct collector *my) { free(my->memory); }
