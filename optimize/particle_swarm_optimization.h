...
protected:
  ...
  int32 trials; // maximum number of trials allowed for one point in
                // DISCRETE_ITERATE
private:
  // int32 *trial; // Might be able to do use this instead of trial if you want
  // to distinguish between NULL and user-set trial limit of 0 (for checking)
  ...
  bool feas_check;  // starts at 0, switches to 1 once a feasible solution has
                    // been found
  ...
  double last_y; // y value from previous iteration. Used in DISCRETE_ITERATE to
                 // check convergence.
  ...
  double *pConstraint1; // Constraint variable. One for each, upper and lower
                        // for x and y.
  double *pConstraint2;
  double *pConstraint3;
  double *pConstraint4;
  ...
  bool constraint_broken(bool (*op)(double, double), double value,
                         double x); // detect constraint
public:
