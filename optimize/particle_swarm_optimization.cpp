...
particle_swarm_optimization::particle_swarm_optimization(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "particle_swarm_optimization",
                               sizeof(particle_swarm_optimization), passconfig);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            ...
            PT_double, "no_particles", PADDR(no_particles), PT_DESCRIPTION,
            "The number of agents (particles) in a swarm.", // this might not
                                                            // work if it isn't
                                                            // specified in
                                                            // input
            PT_double, "C1", PADDR(C1), PT_DESCRIPTION,
            "Hookes�s coefficients", //
            PT_double, "C2", PADDR(C2), PT_DESCRIPTION,
            "Hookes�s coefficients", //
            PT_double, "no_unknowns", PADDR(no_unknowns), PT_DESCRIPTION,
            "This is the dimension of solution space. It is equal to the "
            "number of unknowns in a variable to be optimized", //
            PT_double, "max_iterations", PADDR(max_iterations), PT_DESCRIPTION,
            "Total number of iterations.",

            PT_double, "w", PADDR(w), PT_DESCRIPTION, "Weighting factor.",

            PT_double, "variable_1", PADDR(variable_1), PT_DESCRIPTION,
            "variable_1 value", //
            PT_double, "variable_2", PADDR(variable_2), PT_DESCRIPTION,
            "variable_2 value", //
            PT_double, "variable_3", PADDR(variable_3), PT_DESCRIPTION,
            "variable_3 value", //
            ...
            PT_double, "position_lb", PADDR(position_lb), PT_DESCRIPTION,
            "position_lb", //
            PT_double, "position_ub", PADDR(position_ub), PT_DESCRIPTION,
            "position_ub", //

            PT_double, "velocity_lb", PADDR(velocity_lb), PT_DESCRIPTION,
            "velocity_lb", //
            PT_double, "velocity_ub", PADDR(velocity_ub), PT_DESCRIPTION,
            "velocity_ub", //
...
int particle_swarm_optimization::init(OBJECT *parent) {
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'no_particles' in PSO object '%s' must be "
             "a positive integer value",
             gl_name(obj, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'no_unknowns' in PSO object '%s' must be "
             "a positive integer value",
             gl_name(obj, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'max_iterations' in PSO object '%s' must "
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'variable_1' in PSO object '%s' must be a "
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'C1' in PSO object '%s' must be a "
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'C2' in PSO object '%s' must be a "
             "positive integer value",
             gl_name(obj, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  // check the no_particles limit
    ...
    gl_error("The no_particles limit 'variable_2' in PSO object '%s' must be a "
             "positive integer value or zero",
             gl_name(obj, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  if (!variable_3)
    ...
    gl_error("The no_particles limit 'variable_3' in PSO object '%s' must be a "
             "positive integer value or zero",
             gl_name(obj, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
...
// Each case checks all values delta around that case. The last case selects the
// best of those values. If no feasible solution exists, return an error. Note
// that the value from the previous time step will always be tested at the
// current time step, which is okay because the objective function may
  ...
  if (curr_cycle_time >= time_cycle_interval) // Update values
  {
    ...
    for (int particle = 0; particle < no_particles; particle++) {
      for (int dimension = 0; dimension < no_unknowns; dimension++) {
        particle_position[particle][dimension] =
            position_lb +
            (position_ub - position_lb) * gl_random_uniform(RNGSTATE, 0.0, 1.0);
        particle_velocity[particle][dimension] =
            velocity_lb +
            (velocity_ub - velocity_lb) * gl_random_uniform(RNGSTATE, 0.0, 1.0);
      }
    }
    ...
    for (int iteration = 0; iteration < max_iterations; iteration++) {
...

      {
        ...
        // solution = 100*(variable_2 - (variable_1*variable_1))*(variable_2 -
        // (variable_1*variable_1))+ (1-variable_1)*(1-variable_1) ; solution =
        // (0.25*variable_1*variable_1*variable_1*variable_1) +
        // (0.5*variable_2*variable_2) - (variable_1*variable_2) + variable_1 -
        // variable_2;

        // Minimization example
        solution = 2 * variable_1 + 10 * variable_2 +
                   8 * variable_3; // example 2, page 514

        if ((variable_1 + variable_2 + variable_3 >= 6) &&
            (variable_2 + 2 * variable_3 >= 8) &&
            (-variable_1 + 2 * variable_2 + 2 * variable_3 >= 4) &&
        ...
        //	solution = -2*variable_1 + variable_2 - 2*variable_3;//maximize
        //solution = 2*variable_1 - variable_2 + 2*variable_3;(example 2, page
        //500)
        //
        //	if ((variable_1 + 2*variable_2 - 2*variable_3 <= 20) &&
        //(2*variable_1 + variable_2 <= 10) && (variable_2 + 2*variable_3 <=
        //5)&& (variable_1 >=0)&&(variable_2 >=0)&& (variable_3 >=0))
        //		current_fitness[particle] = -solution;
        //	else
        //		current_fitness[particle] = solution-100000000000000;
      }
...

      {
        for (int dimension = 0; dimension < no_unknowns; dimension++) {
          ...
          particle_velocity[particle][dimension] =
              w * particle_velocity[particle][dimension] +
              C1 * rand1 *
                  (pbest[0][dimension] -
                   particle_position[particle][dimension]) +
              C2 * rand2 *
...
bool particle_swarm_optimization::constraint_broken(bool (*op)(double, double),
...
EXPORT int create_particle_swarm_optimization(OBJECT **obj, OBJECT *parent) {
  try {
    *obj = gl_create_object(particle_swarm_optimization::oclass);
    if (*obj != nullptr) {
      particle_swarm_optimization *my =
          OBJECTDATA(*obj, particle_swarm_optimization);
...
static TIMESTAMP sync_particle_swarm_optimization_impl(OBJECT *obj,
                                                       TIMESTAMP t1,
                                                       PASSCONFIG pass) {
...
sync_particle_swarm_optimization(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  return sync_particle_swarm_optimization_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_particle_swarm_optimization(OBJECT *obj,
                                                                 ...) {
  va_list args;
  va_start(args, obj);
