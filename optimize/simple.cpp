...
simple::simple(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "simple", sizeof(simple),
    ...
    if (gl_publish_variable(
            oclass, PT_char1024, "objective", PADDR(objective), PT_DESCRIPTION,
            "Optimization objective value", PT_char1024, "variable",
            PADDR(variable), PT_DESCRIPTION, "Optimization decision variables",
            PT_char1024, "constraint", PADDR(constraint), PT_DESCRIPTION,
            "Optimization constraints", PT_double, "delta", PADDR(delta),
            PT_DESCRIPTION, "Change applied to decision variable", PT_double,
            "epsilon", PADDR(epsilon), PT_DESCRIPTION,
            "Precision of objective value", PT_int32, "trials", PADDR(trials),
            PT_DESCRIPTION, "Limits on number of trials", PT_enumeration,
            "goal", PADDR(goal), PT_DESCRIPTION, "Optimization objective goal",
...
int simple::isa(char *classname) { return strcmp(classname, "simple") == 0; }

/* Object creation is called once for each object that is created by the core */
int simple::create(void) { return 1; /* return 1 on success, 0 on failure */ }
...
int simple::init(OBJECT *parent) {
...
#ifdef __APPLE__
  parent =
      my->parent; // AppleClang seems to have an issue with the parent pointer
#endif
  char buffer[1024];
  struct {
    const char *name;
    ...
    double *value;
      ...
      {"constraint", constraint.get_string(), &pConstraint, &(constrain.op),
       &(constrain.value)},
  };
  int n;
  for (n = 0; n < sizeof(map) / sizeof(map[0]); n++) {
    ...
    if (sscanf(map[n].param, "%[^.:].%[a-zA-Z0-9_.]", oname, pname) != 2) {
      gl_error("%s could not be parsed, expected term in the form "
    ...
    *(map[n].var) = (double *)gl_get_addr(obj, pname);
    ...
    if (map[n].op != nullptr) {
      ...
      switch (sscanf(constraint.get_string(), "%[^ \t<=>!]%[<=>!]%s", varname,
                     op, value)) {
      case 0:
        break;
      case 1:
        gl_error("constraint '%s' in object '%s' is missing a valid comparison "
                 "operator",
                 constraint.get_string(), oname);
        return 0;
      case 2:
        gl_error("constraint '%s' in object '%s' is missing a valid comparison "
                 "value",
      ...
      case 3: {
        if (strcmp(op, "<") == 0)
          ...
          *(map[n].op) = ne;
          ...
          gl_error("constraint '%s' in object '%s' operator '%s' is invalid",
                   constraint.get_string(), oname, op);
          return 0;
        }
      }
        *(map[n].value) = atof(value);
        break;
      default:
        gl_error("constraint '%s' in object '%s' parsing resulted in an "
                 "internal error",
                 constraint.get_string(), oname);
        return 0;
      }
  ...
  if (delta <= 0) {
    gl_error("Decision variable 'delta' in simple optimizer object '%s' must "
             "be a positive value",
             gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  if (epsilon <= 0) {
    gl_error("Objective 'epsilon' in simple optimizer object '%s' must be a "
             "positive value",
             gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  if (pObjective != nullptr)
    ...
    gl_error(
        "The property 'objective' must be set in simple optimizer object '%s'",
        gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  if (pVariable != nullptr)
    ...
    gl_error(
        "The property 'variable' must be set in simple optimizer object '%s'",
        gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return 0;
  }
  ...
  if (trials < 0) {
    gl_error("The trial limit 'trials' in simple optimizer object '%s' must be "
  ...
  gl_verbose("  %s(%s)",
             goal == OG_MINIMUM ? "minimum"
                                : (goal == OG_MAXIMUM ? "maximum" : "extremum"),
             objective.get_string());
...
TIMESTAMP simple::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (trials > 0 && trial > trials) {
    gl_error("The trial limit of %d in simple optimizer object '%s' has been "
             "reached",
             trials, gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return TS_INVALID; /* stop */
  }
  if (isnan(*pObjective) || !isfinite(*pObjective)) {
    gl_error("The objective '%s' in simple optimizer object '%s' is infinite "
             "or indeterminate",
             objective.get_string(),
             gl_name(my, buffer, sizeof(buffer)) ? buffer : "???");
    return TS_INVALID; /* stop */
  ...
  case 2: {    // second order estimate
    ...
    if (fabs(dy) < epsilon)
      ...
      if (ddy == 0) {
        gl_error("The objective '%s' in simple optimizer object '%s' does not "
                 "appear to have a non-zero second derivative near '%s=%g', "
                 "which cannot lead to an extremum",
                 objective.get_string(),
                 gl_name(my, buffer, sizeof(buffer)) ? buffer : "???",
                 variable.get_string(), last_x);
        return TS_INVALID;
      } else if (ddy < 0 && goal == OG_MINIMUM) {
        gl_error(
            "The minimum objective '%s' in '%s' cannot be found from '%s=%g'",
            objective.get_string(),
            gl_name(my, buffer, sizeof(buffer)) ? buffer : "???",
            variable.get_string(), last_x);
        return TS_INVALID;
        ...
        gl_error(
            "The maximum objective '%s' in '%s' cannot be found from '%s=%g'",
            objective.get_string(),
            gl_name(my, buffer, sizeof(buffer)) ? buffer : "???",
            variable.get_string(), last_x);
        return TS_INVALID;
        ...
        // if the constraint is on the decision variable
          ...
          if (violation) {
            ...
            *pVariable = constrain.value;
            gl_verbose("%s constrained to %g", variable.get_string(),
                       constrain.value);
            pass = 3;
          }
...
#else
extern "C" MODULE_API TIMESTAMP sync_simple(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
  TIMESTAMP t1 = va_arg(args, TIMESTAMP);
  PASSCONFIG pass = static_cast<PASSCONFIG>(va_arg(args, int));
  va_end(args);
  return sync_simple_impl(obj, t1, pass);
}
#endif