...
load::load(MODULE *mod) : node(mod)
{
	if(oclass == NULL)
	{
		...
		if(gl_publish_variable(oclass,
			...
			PT_complex, "constant_power_A[W]", PADDR(constant_power[0]),
			PT_complex, "constant_power_B[W]", PADDR(constant_power[1]),
			PT_complex, "constant_power_C[W]", PADDR(constant_power[2]),
