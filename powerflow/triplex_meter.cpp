...
triplex_meter::triplex_meter(MODULE *mod) : triplex_node(mod)
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_double, "measured_real_energy_delta[Wh]", PADDR(measured_real_energy_delta),PT_DESCRIPTION,"delta in metered real energy consumption from last time step",
			PT_double, "measured_reactive_energy[VAh]",PADDR(measured_reactive_energy),PT_DESCRIPTION,"metered reactive energy consumption",
			PT_double, "measured_reactive_energy-delta[VAh]",PADDR(measured_reactive_energy_delta),PT_DESCRIPTION,"delta in metered reactive energy consumption from last time step",
...
//		if (dt > 0 && last_t != dt)
	if (dt > 0)
	{
		measured_real_energy_delta = measured_real_power * TO_HOURS(dt);
		measured_real_energy += measured_real_energy_delta;
		measured_reactive_energy_delta = measured_reactive_power * TO_HOURS(dt);
		measured_reactive_energy += measured_reactive_energy_delta;
	}
