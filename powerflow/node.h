...
#define I_INJ(V, S, Z, I)                                                      \
  (I_S(S, V) + ((Z.IsFinite()) ? I_Z(Z, V) : gld::complex(0.0)) + I_I(I))
...
// #define NF_NONE			0x0000	///< flag indicates node has no
// special conditions */ #define NF_HASSOURCE	0x0001	///< flag indicates node
// has a source for voltage */ #define NF_ISSOURCE		0x0002	///<
// flag indicates node has a source connected directly to it */
...
#define shuntA                                                                 \
  shunt[0] /// phase A shunt admittance accumulator (reset to 1/impedance on
           /// each pass)
#define shuntB                                                                 \
  shunt[1] /// phase B shunt admittance accumulator (reset to 1/impedance on
           /// each pass)
#define shuntC                                                                 \
  shunt[2] /// phase C shunt admittance accumulator (reset to 1/impedance on
           /// each pass)
...
#define SNT_CHILD_NOINIT                                                       \
  0x0002                  ///< defines is a child node that has not been linked
#define SNT_PARENT 0x0004 ///< defines is a parent of a child
#define SNT_DIFF_CHILD                                                         \
  0x0008 ///< defines is a child node, but has different phase-connection that
         ///< our parent
#define SNT_DIFF_PARENT                                                        \
  0x0010 ///< defines a parent, but has a different phase-connection than our
         ///< child
...
typedef struct {
  gld::complex voltage_val[3]; // Voltage values stored - used for "prev"
                               // version
...
private:
  ...
  gld::complex
      current_inj[3]; ///< current injection (total of current+shunt+power)
  TIMESTAMP
      prev_NTime; ///< Previous timestep - used for propogating child properties
  gld::complex last_child_power[4][3];    ///< Previous power values - used for
                                          ///< child object propogation
  gld::complex last_child_power_dy[6][3]; ///< Previous power values joint -
                                          ///< used for child object propogation
  gld::complex
      last_child_current12; ///< Previous current value - used for child object
                            ///< propogation (namely triplex)
  bool deltamode_inclusive; ///< Flag for deltamode functionality, just to
                            ///< prevent having to mask the flags
  gld::complex
      BusHistTerm[3]; ///< Pointer for array used to store load history value
                      ///< for deltamode-based in-rush computations
  double prev_delta_time; ///< Tracking variable for last time deltamode call
                          ///< occurred - used for "once a timestep" in-rush
                          ///< computations
  gld::complex *ahrlloadstore; ///< Pointer for array used to store load history
                               ///< constant ahrl -- associated with inductance
  gld::complex *bhrlloadstore; ///< Pointer for array used to store load history
                               ///< constant bhrl -- associated with inductance
  gld::complex *chrcloadstore; ///< Pointer for array used to store load history
                               ///< constant chrc -- associated with capacitance
  gld::complex *LoadHistTermL; ///< Pointer for array used to store load history
                               ///< value for deltamode-based in-rush
                               ///< computations -- Inductive terms
  gld::complex *LoadHistTermC; ///< Pointer for array used to store load history
                               ///< value for deltamode-based in-rush
                               ///< computations -- Shunt capacitance terms

  gld::complex shunt_change_check[3]; ///< Change tracker for FPI - to trigger
                                      ///< update of "standard" loads
  gld::complex shunt_change_check_dy[6]; ///< Change tracker for FPI - trigger
                                         ///< update of explicit Wye-Delta loads
  gld::complex
      *Extra_Data_Track_FPI; /// Link to extra data tracker information (NR-FPI)
  ...
  bool first_freq_init; // Flag to indicate if the first init for frequency -
                        // prevents from overwriting an off-nominal frequency

  double freq_violation_time_total; // Keeps track of how long the device has
                                    // been in a frequency violation, to see if
                                    // it needs to disconnect or not
  double volt_violation_time_total; // Keeps track of how long the device has
                                    // been in a voltage violation, to see if it
                                    // needs to disconnect or not
  double out_of_violation_time_total; // Tracking variable to see how long we've
                                      // been "outside of bad conditions"
  ...
  // Swing designation
  bool swing_functions_enabled; // Flag to indicate if a bus is behaving as a
                                // swing

  // VFD-related items
  bool VFD_attached; ///< Flag to indicate this is on the to-side of a VFD link
  FUNCTIONADDR VFD_updating_function; ///< Address for VFD updating function, if
                                      ///< it is present
  OBJECT *VFD_object; ///< Object pointer for the VFD - for later function calls

  gld::complex
      *tn_values; // Variable (mostly for FBS) to map triplex multipliers for
                  // neutral current.  Mostly so saves API calls.

public:
  double compute_angle_diff(double angle_B,
                            double angle_A); // Function to do differences, but
                                             // handle the phase wrap/jump
  STATUS shunt_update_fxn(void); // Function to do shunt update for FPI, so
                                 // sequencing happens correctly
...
public:
  ...
  unsigned short
      k; ///< incidence count (number of links connecting to this node) */
  gld::complex
      *prev_voltage_value; // Pointer for array used to store previous voltage
                           // value for Master/Slave functionality
  gld::complex *prev_power_value; // Pointer for array used to store previous
                                  // power value for Master/Slave functionality

  bool reset_island_state; //< Flagging variable - indicates the disabled island
...
public:
  ...
  enum {
    ...
    SWING_PQ = 3 /**< defines a bus that is an unconstrained voltage controlled
                    bus, unless another exists on the system. */
  };
  enumeration bustype;
  enum {
    NOMINAL = 1, ///< bus voltage is nominal
  ...
  double service_status_dbl; ///< double value for service - overrides the
                             ///< enumeration if set
  ...
  gld::complex
      pre_rotated_current[3]; /// bus current that has been rotated already for
                              /// deltamode (direct post to powerflow)
  gld::complex
      deltamode_dynamic_current[3]; /// bus current that is pre-rotated, but
                                    /// also has ability to be reset within
                                    /// powerflow
  ...
  gld::complex current_dy[6];       /// bus current injection (positive = in),
                              /// explicitly specify delta and wye portions
  gld::complex power_dy[6]; /// bus power injection (positive = in), explicitly
                            /// specify delta and wye portions
  gld::complex shunt_dy[6]; /// bus shunt admittance, explicitly specify delta
                            /// and wye portions
  gld::complex
      *full_Y; /// full 3x3 bus shunt admittance - populate as necessary
  gld::complex full_Y_load[3][3]; /// 3x3 bus shunt admittance - meant to update
                                  /// - used for in-rush or FPI
  gld::complex
      *full_Y_all; /// Full 3x3 bus admittance with "other" contributions (self
                   /// of full admittance) - populate as necessary
  DYN_NODE_TYPE node_type; /// Variable to indicate what we are - prevents
                           /// needing a gl_object_isa EVERY...SINGLE...TIME in
                           /// an already slow dynamic simulation
  gld::complex current12;  /// Used for phase 1-2 current injections in triplex
  gld::complex nom_res_curr[3]; /// Used for the inclusion of nominal
                                /// residential currents (for angle adjustments)
  bool house_present;  /// Indicator flag for a house being attached (NR
                       /// primarily)
  bool dynamic_norton; /// Norton-equivalent posting on this bus -- deltamode
                       /// and diesel generator ties
  bool dynamic_norton_child;  /// Flag to indicate a childed node object is
                              /// posting to this bus - prevents a
                              /// double-accumulation in meters
  bool dynamic_generator;     /// Swing-type generator posting on this bus --
                              /// deltamode and other generator ties
  gld::complex *Triplex_Data; /// Link to triplex line for extra current
                              /// calculation information (NR)
  gld::complex *Extra_Data;   /// Link to extra data information (NR)
  unsigned int NR_connected_links[2]; /// Counter for number of connected links
                                      /// in the system
  unsigned int NR_number_child_nodes[2]; /// Counter for number of childed nodes
                                         /// we have (for later NR linking)
  node **NR_child_nodes;                 /// Pointer to childed nodes list
  int *NR_link_table;                    /// Pointer to link list table

  double mean_repair_time; /// Node's mean repair time - mainly for swing at
                           /// this point

  int NR_node_reference; /// Node's reference in NR_busdata
  int *
      NR_subnode_reference; /// Pointer to parent node's reference in NR_busdata
                            /// - just in case things get inited out of synch
  unsigned char
      prev_phases; /// Phase tracking variable for use in reliability calls
...
public:
  ...
  static inline size_t get_full_Y_matrix_offset(void) {
    ...
    return reinterpret_cast<const char *>(&(current_defaults->full_Y_matrix)) -
           reinterpret_cast<const char *>(current_defaults);
  }

  // Inline function to get the value of `full_Y_matrix`.
  inline Eigen::MatrixXcd get_full_Y_matrix(void) { return full_Y_matrix; }
  ...
  // Inline method to get the string representation of the `full_Y_matrix`
  // property.
...
public:
  ...
  static inline size_t get_full_Y_all_matrix_offset(void) {
    ...
    return reinterpret_cast<const char *>(
               &(current_defaults->full_Y_all_matrix)) -
           reinterpret_cast<const char *>(current_defaults);
  }
  ...
  // Inline method to get the string representation of the `full_Y_all_matrix`
  // property.
  inline gld_string get_full_Y_all_matrix_string(void) {
    return get_full_Y_all_matrix_property().get_string();
  }

  // Inline method to set the `full_Y_all_matrix` property from a provided
  // string.
...
public:
  ...
  SIMULATIONMODE inter_deltaupdate_node(unsigned int64 delta_time,
                                        unsigned long dt,
                                        unsigned int iteration_count_val,
                                        bool interupdate_pos);
  ...
  object TopologicalParent; /// Child node's original parent as per the
                            /// topological configuration in the GLM file

  // NR bus status toggle function
  STATUS NR_swap_swing_status(bool desired_status);

  // NR bus swing-status check
  void NR_swing_status_check(bool *swing_status_check_value,
                             bool *swing_pq_status_value);
