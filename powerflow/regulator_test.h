...
class regulator_tests : public test_helper {
  ...
  void test_three_single_phase_wye_regulator() {
    ...
    regulator_configuration *reg_config =
        OBJECTDATA(obj, regulator_configuration);
    reg_config->create();
    ...
    double iI_1 =
        src_node->currentA.Im(); // Imaginary part of the current at node 1
    ...
    validate_re =
        1 - (rV_2 <= node2_phA_V_test.Re()
                 ? (node2_phA_V_test.Re() != 0 ? rV_2 / node2_phA_V_test.Re()
                                               : rV_2)
                 : (rV_2 != 0 ? node2_phA_V_test.Re() / rV_2
                              : node2_phA_V_test.Re()));
    validate_im =
        1 - (iV_2 <= node2_phA_V_test.Im()
                 ? (node2_phA_V_test.Im() != 0 ? iV_2 / node2_phA_V_test.Im()
                                               : iV_2)
                 : (iV_2 != 0 ? node2_phA_V_test.Im() / iV_2
                              : node2_phA_V_test.Im()));
    CPPUNIT_ASSERT(validate_re < VALIDATE_THRESHOLD);
    //		CPPUNIT_ASSERT(validate_im < VALIDATE_THRESHOLD);
