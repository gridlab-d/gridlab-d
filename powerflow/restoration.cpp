...
restoration::restoration(MODULE *mod) : powerflow_library(mod) {
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            oclass, PT_int32, "reconfig_attempts", PADDR(reconfig_attempts),
            PT_DESCRIPTION,
            "Number of reconfigurations/timestep to try before giving up",
            PT_int32, "reconfig_iteration_limit", PADDR(reconfig_iter_limit),
            PT_DESCRIPTION,
            "Number of iterations to let PF go before flagging this as a bad "
            "reconfiguration",
            PT_object, "source_vertex", PADDR(sourceVerObj), PT_DESCRIPTION,
            "Source vertex object for reconfiguration", PT_object,
            "faulted_section", PADDR(faultSecObj), PT_DESCRIPTION,
            "Faulted section for reconfiguration", PT_char1024,
            "feeder_power_limit", PADDR(feeder_power_limit_Pub), PT_DESCRIPTION,
            "Comma-separated power limit (VA) for feeders during "
            "reconfiguration",
            PT_char1024, "feeder_power_links", PADDR(feeder_power_link_Pub),
            PT_DESCRIPTION,
            "Comma-separated list of link-based objects for monitoring power "
            "through",
            PT_char1024, "feeder_vertex_list", PADDR(fVerPub), PT_DESCRIPTION,
            "Comma-separated object list that defines the feeder vertices",
            PT_char1024, "microgrid_power_limit", PADDR(microgrid_limit_Pub),
            PT_DESCRIPTION,
            "Comma-separated power limit (complex VA) for microgrids during "
            "reconfiguration",
            PT_char1024, "microgrid_power_links",
            PADDR(microgrid_power_link_Pub), PT_DESCRIPTION,
            "Comma-separated list of link-based objects for monitoring power "
            "through",
            PT_char1024, "microgrid_vertex_list", PADDR(mVerPub),
            PT_DESCRIPTION,
            "Comma-separated object list that defines the microgrid vertices",
            PT_double, "lower_voltage_limit[pu]", PADDR(voltage_limit[0]),
            PT_DESCRIPTION,
            "Lower voltage limit for the reconfiguration validity checks - per "
            "unit",
            PT_double, "upper_voltage_limit[pu]", PADDR(voltage_limit[1]),
            PT_DESCRIPTION,
            "Upper voltage limit for the reconfiguration validity checks - per "
            "unit",
            PT_char1024, "output_filename", PADDR(logfile_name), PT_DESCRIPTION,
            "Output text file name to describe final or attempted switching "
            "operations",
            PT_bool, "generate_all_scenarios", PADDR(stop_and_generate),
            PT_DESCRIPTION,
            "Flag to determine if restoration reconfiguration and continues, "
            "or explores the full space",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);

    if (gl_publish_function(oclass, "perform_restoration",
                            (FUNCTIONADDR)perform_restoration) == nullptr)
  ...
  if (solver_method == SM_NR) {
    if (restoration_object == nullptr) {
      ...
      if (reconfig_attempts == 0) {
        ...
        /*  TROUBLESHOOT
        The restoration object can have the number of reconfiguration per
        timestep set via the reconfig_attempts property.  If not set, or set to
        0, the system will perform reconfigurations forever and lock up the
        computer (if an acceptable solution is never met).
        */
      }
      ...
      if (reconfig_iter_limit == 0) {
        ...
        /*  TROUBLESHOOT
        The restoration object can have the number of iterations powerflow can
        perform before trying another reconfiguration set.  This is set via teh
        reconfig_iteration_limit property.  If not set, or set to 0, the system
        will perform iterations on this reconfiguration until the overall
        powerflow iteration limit is reached, or the system solves and moves to
      ...
      /*  TROUBLESHOOT
      The restoration object manipulates the configuration of the system.  In
      order to prevent possible conflicts, only one restoration object is
      permitted per model.
      */
    }
    ...
    feeder_power_limit =
        ParseDoubleString(&feeder_power_limit_Pub[0], &numfVer);
    ...
    if (numfVer != working_int_val) {
      GL_THROW("restoration: The number of feeder limits and feeder vertices "
               "must be the same!");
      /*  TROUBLESHOOT
      While specifying the feeder_power_limit and feeder_vertex_list, the same
      number of feeders must be represented.
      */
    }

    // Read in the feeder "power flow" object list
    fLinkObjList =
        ParseObjectString(&feeder_power_link_Pub[0], &working_int_val);

    // Do another check to make sure counts match
    if (numfVer != working_int_val) {
      GL_THROW("restoration: The number of feeder links and feeder vertices "
               "must be the same!");
      /*  TROUBLESHOOT
      While specifying the feeder_power_links and feeder_vertex_list, the same
      number of feeders must be represented.
      */
    }
    ...
    if (fLinkPowerFunc == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to allocate memory for the feeder/microgrid power link
      objects, an error occurred.  Please try again.  If the error persists,
      please submit your code and a bug report via the ticketing system.
      */
    }

    // Do the same for the value
    feeder_power_link_value =
        (gld::complex **)gl_malloc(numfVer * sizeof(gld::complex *));

    // Check it
    if (feeder_power_link_value == nullptr) {
      GL_THROW(
          "Restoration: failed to allocate memory for calculated power values");
      /*  TROUBLESHOOT
      While attempting to allocate memory for pointers to the calculated power
      of links, an error occurred.  Please try again, assuming proper inputs. If
    ...
    for (indexval = 0; indexval < numfVer; indexval++) {
      ...
      fLinkPowerFunc[indexval] = (FUNCTIONADDR)(gl_get_function(
          fLinkObjList[indexval], "update_power_pwr_object"));

      // Check to see if it worked -- would let it be nullptr, but will cause
      // problems later
      if (fLinkPowerFunc[indexval] == nullptr) {
        GL_THROW("Restoration: failed to find power calculation function for "
                 "link:%s",
                 fLinkObjList[indexval]->name ? fLinkObjList[indexval]->name
                                              : "Unnamed");
        /*  TROUBLESHOOT
        While attempting to map the update_power_pwr_object function for a link
        object, a failure occurred. Make sure the object is a link object and
      ...
      feeder_power_link_value[indexval] =
          gl_get_complex_by_name(fLinkObjList[indexval], "power_out");

      // Make sure it worked
      if (feeder_power_link_value[indexval] == nullptr) {
        GL_THROW(
            "Restoration: failed to find calculated power value for link:s",
            fLinkObjList[indexval]->name ? fLinkObjList[indexval]->name
                                         : "Unnamed");
        /*  TROUBLESHOOT
        While attempting to map up the location of the complex, calculated power
        of a link, an error occurred. Please try again.  If the error persists,
        please submit your code and a bug report via the ticketing system.
        */
      }
    }
    ...
    if (numMG != working_int_val) {
      GL_THROW("restoration: The number of microgrid limits and microgrid "
               "vertices must be the same!");
      /*  TROUBLESHOOT
      While specifying the microgrid_power_limit and microgrid_vertex_list, the
      same number of microgrids must be represented.
      */
    }

    // Read in the microgrid "power flow" object list
    mLinkObjList =
        ParseObjectString(&microgrid_power_link_Pub[0], &working_int_val);

    // Do another check to make sure counts match
    if (numMG != working_int_val) {
      GL_THROW("restoration: The number of microgrid links and microgrid "
               "vertices must be the same!");
      /*  TROUBLESHOOT
      While specifying the microgrid_power_links and microgrid_vertex_list, the
      same number of microgrids must be represented.
      */
    }
    ...
    // Do the same for the value
    microgrid_power_link_value =
        (gld::complex **)gl_malloc(numMG * sizeof(gld::complex *));
    ...
    for (indexval = 0; indexval < numMG; indexval++) {
      // Try to map the function
      mLinkPowerFunc[indexval] = (FUNCTIONADDR)(gl_get_function(
          mLinkObjList[indexval], "update_power_pwr_object"));
      ...
      if (mLinkPowerFunc[indexval] == nullptr) {
        GL_THROW("Restoration: failed to find power calculation function for "
                 "link:%s",
                 mLinkObjList[indexval]->name ? mLinkObjList[indexval]->name
                                              : "Unnamed");
        // Defined above
      }

      // Link up the actual power value too, while we're here
      microgrid_power_link_value[indexval] =
          gl_get_complex_by_name(mLinkObjList[indexval], "power_out");
    ...
    if (voltage_limit[1] <= voltage_limit[0]) {
      GL_THROW("restoration: the upper and lower voltage limits are either "
               "equal, or backwards");
      /*  TROUBLESHOOT
      The lower_voltage_limit must be lower than the upper_voltage_limit for the
      restoration object to properly work.
      */
    }
    ...
    GL_THROW("Restoration control is only valid for the Newton-Raphson solver "
             "at this time.");
    /*  TROUBLESHOOT
    The restoration object only works with the Newton-Raphson powerflow solver
    at this time. Other solvers may be integrated at a future date.
    */
  }
...
int restoration::PerformRestoration(int faulting_link) {
  ...
  if (fault_check_object != nullptr) {
    ...
    if (fault_check_fxn == nullptr) {
      // Map the function
      fault_check_fxn = (FUNCTIONADDR)(gl_get_function(
          fault_check_object, "reliability_alterations"));
  ...
  } else // Throw an error
  {
    ...
    object could be found.  Restoration requires one to function.  Please add
    one and try again.
    */
  }
  ...
  if (sourceVerObj == nullptr) {
    gl_warning("restoration: The source vertex for the reconfiguration is not "
               "defined, defaulting to a swing bus");
    /*  TROUBLESHOOT
    To properly operate, the reconfiguration needs the sources vertex of the
    system defined.  If left blank, the first SWING bus will be utilized
    instead.
    */
  ...
  // Simplified readNode_2 code -- create a new LinkedUndigraph item for the
  // number of nodes Steal the NR-based numbering schemes, since they're already
  // done
  top_ori = (LinkedUndigraph *)gl_malloc(
      sizeof(LinkedUndigraph)); // Allocate the storage item

  // Make sure it worked
  if (top_ori == nullptr) {
    GL_THROW("Restoration: failed to allocte graph theory object");
    /*  TROUBLESHOOT
    While attempting to allocate memory for one of the graph theory objects
    inside the restoration object, an error was encountered.  Please try again.
  ...
  // Performed up here so source-connected switch can be removed -- this was
  // done in a separate routine before Set the source vertex
  for (indexval = 0; indexval < NR_bus_count; indexval++) {
  ...
  if (s_ver == -1) {
    ...
    could not be located.  Be sure it is part of the main system island and try
    again. If the error persists, please submit your code and a bug report via
  ...
  for (indexval = 0; indexval < NR_branch_count; indexval++) {
    ...
    if ((NR_branchdata[indexval].lnk_type == 3) ||
        (NR_branchdata[indexval].lnk_type == 4) ||
        (NR_branchdata[indexval].lnk_type ==
         5)) // Normal switch considered sectionalizer, as well
    {
      // Make sure it is closed (if open, just don't add it)
      if (*NR_branchdata[indexval].status == LS_CLOSED) {
        top_ori->addEdge(NR_branchdata[indexval].from,
                         NR_branchdata[indexval].to);
      }
      // Default else, don't add it

      // Update the count of what it is (if not a fuse)
      if (NR_branchdata[indexval].lnk_type !=
          3) // Must be a switch or sectionalizer
      {
        if (*NR_branchdata[indexval].status == LS_CLOSED) // Closed switches
        {
          // See if one side is connected to the source vertex -- if so, don't
          // count it (WSU algorithm does this separately) if
          // ((NR_branchdata[indexval].from != s_ver) &&
          // (NR_branchdata[indexval].to != s_ver))
          {
            num_swi_closed++;
          }
    ...
    else // Not a fuse, switch, or sectionalizer just add it, for now (reclosers
         // always assumed closed)
    {
      top_ori->addEdge(NR_branchdata[indexval].from,
                       NR_branchdata[indexval].to);
  ...
  // Allocate tracking arrays - allocate them all to the same size (multiple
  // passes)
  ...
  for (indexval = 0; indexval < NR_branch_count; indexval++) {
    ...
    if ((NR_branchdata[indexval].lnk_type == 4) ||
        (NR_branchdata[indexval].lnk_type == 5)) {
      // Check out status
      if (*NR_branchdata[indexval].status == LS_CLOSED) {
        // Make sure we're not source-vertex connected, just like above
        // if ((NR_branchdata[indexval].from != s_ver) &&
        // (NR_branchdata[indexval].to != s_ver))
        {
          ...
          sec_swi_loc.data_1[sec_swi_loc.currSize] =
              indexval; // This was originally line number in the GLM.  Not sure
        ...
        // Default else -- closed and connected to the source vertex - ignore it
        // (WSU requirement)
      } else {
        // Switch open - tie
        ...
        // Populate the location matrix thing
        tie_swi_loc.data_1[tie_swi_loc.currSize] =
            indexval; // This was originally line number in the GLM.  Not sure
                      // this is needed anymore
    ...
    else if (NR_branchdata[indexval].lnk_type ==
             6) // If it's a recloser, automatically a sectionalizing switch
    {
  ...
  // Do whatever this readLoads_2 thing does - forms load_matrix, which appears
  // to just represent constant_xxx_x portions, so we're skipping it

  // Source vertex setting is above here -- needed for removing source-connected
  // switches for WSU reconfiguration

  // Do a check to set the fault section -- see if we're reading the GLM or
  // fault_check
  if (faulting_link == -99) {
    // Read the GLM -- make sure it is valid
    if (faultSecObj == nullptr) {
      // Not set, hard-code to zero (swing fault?  Default setting of original
      // MATLAB code)
      f_sec.from_vert = 0;
      f_sec.to_vert = 0;
  ...
  // Original MATLAB trimmed excess portions here (shouldn't need to be done -
  // should be GLM level)
  ...
  for (secindexval = 0; secindexval < feederVertices.currSize; secindexval++) {
    feederVertices_1.data[secindexval] =
        ver_map_1.data[feederVertices.data[secindexval]];
  ...
  // Removes the switches from source and feeders as switches -- tried to do
  // this at the GLM level, but it breaks other things
  modifySecSwiList();
  ...
  return 1; // Always succeeds now, or something (unless it fails earlier -
            // yeah, how's that for logic!?)
}

// Utility functions - string parsing
// Parse comma-separated list of doubles into array
// Assumes 1024-character input array
double *restoration::ParseDoubleString(char *input_string,
                                       int *num_items_found) {
  ...
  // General checks
  if ((num_items >= 0) &&
      (index != 0)) // Moved, but didn't find a comma - assume singular
  {
  ...
  if (num_items != 0) {
    ...
    if (output_array == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to allocate space for a double array for parsing a
      comma-separated list into, an error occurred.  Please try again.  If the
      error persists, please submit your code and a bug report via the ticketing
      system.
      */
    ...
    if (num_items > 1) {
      ...
      while (item_count < (num_items - 1)) {
        ...
        if (parsing_token == nullptr) {
          ...
          /*  TROUBLESHOOT
          While attempting to parse a string of numbers, an unexpected state was
          encountered. Please check your inputs and try again.
          */
  ...
  {
    gl_warning("restoration: Attempt to parse double CSV array resulted in an "
               "empty array - check your inputs");
    /*  TROUBLESHOOT
    While attempting to parse a comma-separated string into a double array, the
    array appears to be empty.  Ensure this is the correct behavior.
    */
  }
...
// Assumes 1024-character input array
gld::complex *restoration::ParseComplexString(char *input_string,
                                              int *num_items_found) {
  int index, num_items, item_count;
  ...
  if (num_items != 0) {
    ...
    if (output_array == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to allocate space for a complex array for parsing a
      comma-separated list into, an error occurred.  Please try again.  If the
      error persists, please submit your code and a bug report via the ticketing
      system.
      */
      ...
      while (item_count < (num_items - 1)) {
        ...
        } else // Continue like normal
        {
          ...
          *parsing_token = '\0';
          ...
          if (complex_token == nullptr) // No complex indicated, so we're either
                                        // full real or full complex - fail
          {
            gl_error("restoration: Attempting to parse a complex character "
                     "token failed");
            /*  TROUBLESHOOT
            While attempting to parse a string of numbers, an unexpected state
            was encountered. Please check your inputs and try again.
            */
    ...
    // see if a complex portion was specified
    if (complex_token == nullptr) // No complex indicated, so we're either full
                                  // real or full complex - fail
    {
      gl_error(
  ...
  {
    gl_warning("restoration: Attempt to parse complex CSV array resulted in an "
               "empty array - check your inputs");
    /*  TROUBLESHOOT
    While attempting to parse a comma-separated string into a complex array, the
    array appears to be empty.  Ensure this is the correct behavior.
    */
  }
...
// Parse comma-separated list of complex values into array
// Assumes 1024-character input array
OBJECT **restoration::ParseObjectString(char *input_string,
                                        int *num_items_found) {
  int index, num_items, item_count;
  char *working_token, *parsing_token;
  OBJECT **output_array;
  ...
  if (num_items != 0) {
    ...
    if (output_array == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to allocate space for a object array for parsing a
      comma-separated list into, an error occurred.  Please try again.  If the
      error persists, please submit your code and a bug report via the ticketing
      system.
      */
  ...
  {
    gl_warning("restoration: Attempt to parse object CSV array resulted in an "
               "empty array - check your inputs");
    /*  TROUBLESHOOT
    While attempting to parse a comma-separated string into a object array, the
    array appears to be empty.  Ensure this is the correct behavior.
    */
  }
...
void restoration::INTVECTalloc(INTVECT *inititem, int allocsizeval) {
  ...
  // Initial check -- see if the array already has something -- if so, free it
  // first Not sure any of these ever get reused, but this will make sure this
  // potential memory hole is closed
  if (inititem->data != nullptr) {
    gl_free((void **)&inititem->data);
...
void restoration::INTVECTfree(INTVECT *inititem) {
  // Remove all of our malloced components - if allocated (check, to prevent
...
void restoration::LOCSETalloc(LOCSET *inititem, int allocsizeval) {
  ...
  // Check it
  if ((inititem->data_1 == nullptr) || (inititem->data_2 == nullptr) ||
      (inititem->data_3 == nullptr) || (inititem->data_4 == nullptr)) {
    GL_THROW("Restoration: failed to allocte graph theory object");
    // Defined elsewhere
  }
...
void restoration::CANDSWOPalloc(CANDSWOP *inititem, int allocsizeval) {
  ...
  // Check second 3 ints
  if ((inititem->data_4 == nullptr) || (inititem->data_5 == nullptr) ||
      (inititem->data_7 == nullptr)) {
    GL_THROW("Restoration: failed to allocte graph theory object");
    // Defined elsewhere
  }
...
void restoration::simplifyTop_1(void) {
  ...
  for (idx = 0; idx < top_ori_VerNum; idx++) {
    ...
    while (v != -1) {
      if ((v > idx) && !isSwitch(idx, v) &&
          ((v != f_sec.from_vert) || (idx != f_sec.to_vert)) &&
          ((v != f_sec.to_vert) || (idx != f_sec.from_vert))) {
...
void restoration::simplifyTop_2(void) {
  ...
  // Simplify top_sim_2
  top_sim_2->simplify(&tie_swi_1, &f_sec_1, s_ver_1, &feederVertices_1,
                      &ver_map_2);
  ...
  // Sectionalizing switches
  allocsizeval =
      top_sim_2->getVerNum(); // Arbitrary change, since it was blowing the
                              // array size below - was getEdgeNum - 1 - this
                              // aligns with FOR loop below
  ...
  for (uIdx = 0; uIdx < top_sim_2_getVerNum; uIdx++) {
    ...
    while (v != nullptr) {
      vIdx = v->data;
      if (!(vIdx > uIdx) &&
          ((((uIdx == f_sec_2.from_vert) && (vIdx == f_sec_2.to_vert)) ||
            ((uIdx == f_sec_2.to_vert) && (vIdx == f_sec_2.from_vert))))) {
        sec_swi_2.data_1[sec_swi_2.currSize] = uIdx;
        sec_swi_2.data_2[sec_swi_2.currSize] = vIdx;
        sec_swi_2.currSize++;

        if (sec_swi_2.currSize >=
            sec_swi_2.maxSize) // Check, since we had problems with the original
                               // code, as written
        {
          // Being equal is just of note for debugging (if we come back, we'll
          // be upset)
          ...
          } else // Must be greater, we're already broken
          {
            ...
            /*  TROUBLESHOOT
            While performing an operation, the maximum size of the array
            allocated was exceeded.  Please submit your code and a bug report
...
void restoration::setFeederVertices_2(void) {
  ...
  // Allocate the temporary vector - assume won't be bigger than
  // feederVertices_1? (failed horribly on feederVertices)
  ...
  for (idx = 0; idx < feederVertices.currSize; idx++) {
    ...
    while ((nodeSet.data[nodeSetIdx] != -1) &&
           (nodeSetIdx < nodeSet.currSize)) {
...
//
// Do this in a much more simplified manner - just look for it matching and
// remove it (GLD approach) This isn't an elegant solution, but works for now
...
void restoration::modifySecSwiList(void) {
  ...
  memcpy(sec_swi_1_temp.data_1, sec_swi_1.data_1,
         sec_swi_1.maxSize * sizeof(int));
  memcpy(sec_swi_1_temp.data_2, sec_swi_1.data_2,
         sec_swi_1.maxSize * sizeof(int));
  memcpy(sec_swi_2_temp.data_1, sec_swi_2.data_1,
         sec_swi_2.maxSize * sizeof(int));
  memcpy(sec_swi_2_temp.data_2, sec_swi_2.data_2,
         sec_swi_2.maxSize * sizeof(int));

  // Including the useless locset thing
  memcpy(sec_swi_loc_temp.data_1, sec_swi_loc.data_1,
         sec_swi_loc.maxSize * sizeof(int));
  memcpy(sec_swi_loc_temp.data_2, sec_swi_loc.data_2,
         sec_swi_loc.maxSize * sizeof(int));
  memcpy(sec_swi_loc_temp.data_3, sec_swi_loc.data_3,
         sec_swi_loc.maxSize * sizeof(int));
  memcpy(sec_swi_loc_temp.data_4, sec_swi_loc.data_4,
         sec_swi_loc.maxSize * sizeof(int));
  ...
  // Zero the old, just because I say so - could theoretically just "-1" out the
  // existing entry But this code would probably not like that and I'm too lazy
  // to track down the instances Assume sec_swi_loc is the same size
  ...
  for (indexval = 0; indexval < sec_swi_temp.currSize; indexval++) {
    if ((sec_swi_temp.data_1[indexval] != s_ver) &&
        (sec_swi_temp.data_2[indexval] != s_ver)) {
      ...
      // Same for loc stuff
      sec_swi_loc.data_1[sec_swi_loc.currSize] =
          sec_swi_loc_temp.data_1[indexval];
      sec_swi_loc.data_2[sec_swi_loc.currSize] =
          sec_swi_loc_temp.data_2[indexval];
      sec_swi_loc.data_3[sec_swi_loc.currSize] =
          sec_swi_loc_temp.data_3[indexval];
      sec_swi_loc.data_4[sec_swi_loc.currSize] =
          sec_swi_loc_temp.data_4[indexval];
  ...
  for (indexval = 0; indexval < sec_swi_1_temp.currSize; indexval++) {
    if ((sec_swi_1_temp.data_1[indexval] != s_ver_1) &&
        (sec_swi_1_temp.data_2[indexval] != s_ver_1)) {
      // Not a source vertex, allow it
  ...
  for (indexval = 0; indexval < sec_swi_2_temp.currSize; indexval++) {
    if ((sec_swi_2_temp.data_1[indexval] != s_ver_2) &&
        (sec_swi_2_temp.data_2[indexval] != s_ver_2)) {
      // Not a source vertex, allow it
...
// No MATLAB description for function -- presumably located Sectionalizing
// switches in some manner
void restoration::mapSecSwi(BRANCHVERTICES *sSW_2, BRANCHVERTICES *sSW,
                            BRANCHVERTICES *sSW_1) {
  ...
  if (uIdx == -1) {
    ...
    /*  TROUBLESHOOT
    While parsing a mapping list of vertices for the graph theory analysis, a
    desired vertex could not be found.  Please try again.  If the error
  ...
  while (idx < sec_swi_1.currSize) {
    if ((sec_swi_1.data_1[idx] == sSW_1->from_vert) &&
        (sec_swi_1.data_2[idx] == sSW_1->to_vert)) {
      sSW->from_vert = sec_swi.data_1[idx];
      sSW->to_vert = sec_swi.data_2[idx];
      break;
    }
    if ((sec_swi_1.data_1[idx] == sSW_1->to_vert) &&
        (sec_swi_1.data_2[idx] == sSW_1->from_vert)) {
...
// No MATLAB description for function -- presumably locates tie switches in some
// manner
void restoration::mapTieSwi(BRANCHVERTICES *tSW_2, BRANCHVERTICES *tSW,
                            BRANCHVERTICES *tSW_1) {
  int idx;

  idx = 0;

  // Initialize to bad values - for giggles (may check later -- was absent from
  // MATLAB)
  tSW->from_vert = -1;
  tSW->to_vert = -1;
  tSW_1->from_vert = -1;
  ...
  while (idx < tie_swi_2.currSize) {
    if ((tie_swi_2.data_1[idx] == tSW_2->from_vert) &&
        (tie_swi_2.data_2[idx] == tSW_2->to_vert)) {
    ...
    if ((tie_swi_2.data_2[idx] == tSW_2->from_vert) &&
        (tie_swi_2.data_1[idx] == tSW_2->to_vert)) {
...
void restoration::renewFaultLocation(BRANCHVERTICES *faultsection) {
  ...
  if (top_res != nullptr) // As an attempt at fixing possible memory leaks, try
                          // to empty them first
  {
    top_res->delAllVer();
  }
  ...
  //  ********************* may get upset about this, so may need to destroy
...
int restoration::spanningTreeSearch(void) {
  int idx, counter, preCounter, feederID, feeder_overloaded, k, tvi, startIdx,
      allocsize;
  ...
  if (numMG > 0) {
    allocsize = 2 * numMG * numfVer *
                (tie_swi.currSize +
                 sec_swi.currSize); // Arbitrarily sized - MATLAB is sloppy
  } else {
    allocsize = 2 * numfVer *
                (tie_swi.currSize +
                 sec_swi.currSize); // Arbitrarily sized - MATLAB is sloppy
  ...
  while (counter < candidateSwOpe.currSize) {
    if (candidateSwOpe.data_5[counter] == 0) {
      ...
      // If it fails, then modifyModel again (should de-toggle all of what was
      // just toggled)
      ...
      if (powerflow_result == -1) {
        return -2; // Serious error occurred, so flag us as "really bad"
                   // basically, the state of the system may be corrupted, so
                   // any subsequent powerflows can't be trusted
      ...
      if (feasible) {
        // Check our desired "approach" on this -- see if we need to undo for
        // the next section or not
      ...
      top_res->deleteEdge(candidateSwOpe_2.data_1[counter],
                          candidateSwOpe_2.data_2[counter]);
      top_res->addEdge(candidateSwOpe_2.data_3[counter],
                       candidateSwOpe_2.data_4[counter]);
      top_res->BFS(s_ver_2);

      // Form the tie switch list for new graph
      // Allocate new_tie_swi -- just in case sizes change, this will realloc it
      // (not quite as elegantly)
      CHORDSETalloc(&new_tie_swi, tie_swi_2.currSize);

      // Copy results in
      memcpy(new_tie_swi.data_1, tie_swi_2.data_1,
             tie_swi_2.currSize * sizeof(int));
      memcpy(new_tie_swi.data_2, tie_swi_2.data_2,
             tie_swi_2.currSize * sizeof(int));
      new_tie_swi.currSize = tie_swi_2.currSize;

      for (idx = 0; idx < new_tie_swi.currSize; idx++) {
        if (((new_tie_swi.data_1[idx] == candidateSwOpe_2.data_3[counter]) &&
             (new_tie_swi.data_2[idx] == candidateSwOpe_2.data_4[counter])) ||
            ((new_tie_swi.data_1[idx] == candidateSwOpe_2.data_4[counter]) &&
             (new_tie_swi.data_2[idx] == candidateSwOpe_2.data_3[counter]))) {
      ...
      for (idx = 0; idx < sec_swi_2.currSize; idx++) {
        ...
        // Save all candidate solutions -- these get appended to the previous
        // ones (no overwritten)
        for (k = 0; k < FCutSet_2.currSize; k++) {
          // General error check
      ...
      // Adjustment from WSU code below - just run a powerflow
      // If it fails, then modifyModel again (should de-toggle all of what was
      // just toggled) Perform the modification
      modifyModel(counter);
      ...
      while (preCounter != 0) {
        top_res->deleteEdge(candidateSwOpe_2.data_1[preCounter],
                            candidateSwOpe_2.data_2[preCounter]);
        top_res->addEdge(candidateSwOpe_2.data_3[preCounter],
                         candidateSwOpe_2.data_4[preCounter]);
      ...
      while (preCounter != 0) {
        for (idx = 0; idx < new_tie_swi.currSize; idx++) {
          if (((new_tie_swi.data_1[idx] ==
                candidateSwOpe_2.data_3[preCounter]) &&
               (new_tie_swi.data_2[idx] ==
                candidateSwOpe_2.data_4[preCounter])) ||
              ((new_tie_swi.data_1[idx] ==
                candidateSwOpe_2.data_4[preCounter]) &&
               (new_tie_swi.data_2[idx] ==
                candidateSwOpe_2.data_3[preCounter]))) {
      ...
      //
      for (idx = 0; idx < sec_swi_2.currSize; idx++) {
        if (((sec_swi_2.data_1[idx] == candidateSwOpe_2.data_1[counter]) &&
             (sec_swi_2.data_2[idx] == candidateSwOpe_2.data_2[counter])) ||
            ((sec_swi_2.data_1[idx] == candidateSwOpe_2.data_2[counter]) &&
             (sec_swi_2.data_2[idx] == candidateSwOpe_2.data_1[counter]))) {
          startIdx = idx + 1;
          break;
        }
      }
...
// MATLAB does this as a sort and is probably more efficient.  This goes "brute
// force route"
void restoration::CHORDSETintersect(CHORDSET *set_1, CHORDSET *set_2,
                                    CHORDSET *intersect) {
  ...
  for (indexa = 0; indexa < leftside->currSize;
       indexa++) // Main loop is on the smaller one, since it will finish faster
  {
    // Loop through the other side
    for (indexb = 0; indexb < rightside->currSize; indexb++) {
      // See if they match
      if ((leftside->data_1[indexa] == rightside->data_1[indexb]) &&
          (leftside->data_2[indexa] == rightside->data_2[indexb])) {
...
void restoration::modifyModel(int counter) {
  ...
  while (preCounter != 0) {
    swi_to_open.data_1[swi_to_open.currSize] =
        candidateSwOpe.data_1[preCounter];
    swi_to_open.data_2[swi_to_open.currSize] =
        candidateSwOpe.data_2[preCounter];
    swi_to_open.currSize++;

    swi_to_close.data_1[swi_to_close.currSize] =
        candidateSwOpe.data_3[preCounter];
    swi_to_close.data_2[swi_to_close.currSize] =
        candidateSwOpe.data_4[preCounter];
    swi_to_close.currSize++;
  ...
  for (idx = 0; idx < swi_to_open.currSize; idx++) {
    for (k = 0; k < sec_swi.currSize; k++) {
      if (((swi_to_open.data_1[idx] == sec_swi.data_1[k]) &&
           (swi_to_open.data_2[idx] == sec_swi.data_2[k])) ||
          ((swi_to_open.data_1[idx] == sec_swi.data_2[k]) &&
           (swi_to_open.data_2[idx] == sec_swi.data_1[k]))) {
  ...
  for (idx = 0; idx < swi_to_close.currSize; idx++) {
    for (k = 0; k < tie_swi.currSize; k++) {
      if (((swi_to_close.data_1[idx] == tie_swi.data_1[k]) &&
           (swi_to_close.data_2[idx] == tie_swi.data_2[k])) ||
          ((swi_to_close.data_1[idx] == tie_swi.data_2[k]) &&
           (swi_to_close.data_2[idx] == tie_swi.data_1[k]))) {
  ...
  // Copy values into the input -- just copy the first column, since it is the
  // index to what we care about
  memcpy(locations_temp.data, loc_sec.data_1, loc_sec.currSize * sizeof(int));
  memcpy(&locations_temp.data[loc_sec.currSize], loc_tie.data_1,
         loc_tie.currSize * sizeof(int));
  ...
  // Now loop through these locations - if they were closed, open them.  If
  // open, close them.
  for (idx = idxstart; idx < locations.currSize; idx++) {
    ...
    if (NR_branchdata[locations.data[idx]].lnk_type == 4) // Normal switch
    {
      ...
      switching_fxn =
          (FUNCTIONADDR)(gl_get_function(swobj, "change_switch_state"));
    ...
    } else if (NR_branchdata[locations.data[idx]].lnk_type == 6) // Recloser
    {
      // See if we can find the switching function
      switching_fxn =
          (FUNCTIONADDR)(gl_get_function(swobj, "change_recloser_state"));

      // Set the return type
      return_is_int_val = false;
    } else if (NR_branchdata[locations.data[idx]].lnk_type ==
               5) // Sectionalizer
    {
      // See if we can find the switching function
      switching_fxn =
          (FUNCTIONADDR)(gl_get_function(swobj, "change_sectionalizer_state"));
    ...
    {
      GL_THROW(
          "Restoration: switch actions were attempted on a non-switch device!");
      /*  TROUBLESHOOT
      While attempting a reconfiguration action, an object that is not a switch,
      sectionalizer, or recloser was attempted to be switched.  This should not
      occur.  Please try again.  If the error persists, pleas submit your code
    ...
    if (switching_fxn == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to map the switch adjustment function for the restoration
      code, an error occurred.  Please try again.  If the error persists, please
    ...
    if (return_is_int_val) // Switches, basically
    {
      ...
      if (*NR_branchdata[locations.data[idx]].status ==
          LS_OPEN) // Was open, close it
      {
        return_val = ((int (*)(OBJECT *, unsigned char, bool))(*switching_fxn))(
            swobj, 0x07, true);
    ...
    } else // Double return - reclosers and sectionalizers
    {
      ...
      if (*NR_branchdata[locations.data[idx]].status ==
          LS_OPEN) // Was open, close it
      {
        return_val_double = ((double (*)(OBJECT *, unsigned char, bool))(
            *switching_fxn))(swobj, 0x07, true);
      } else // Must be closed, open it
      {
        return_val_double = ((double (*)(OBJECT *, unsigned char, bool))(
            *switching_fxn))(swobj, 0x07, false);
      }
    ...
    if (return_val != 1) {
      GL_THROW("Restoration: Unable to perform switching action on %s!",
               swobj->name ? swobj->name : "unnamed");
      /*  TROUBLESHOOT
      While attempting to change the state on a switching device, an error
  ...
  // Call the support check alterations, if there was at least one switching
  // action
  if (switch_occurred) {
    // Call "reliability_alterations" - "link" value of -99 or -77 matter?
    return_val = ((int (*)(OBJECT *, int, bool))(*fault_check_fxn))(
        fault_check_object, -99, true);

    // Check it
    if (return_val != 1) {
      GL_THROW("Restoration: problem occurred altering topology");
      /*  TROUBLESHOOT
      While attempting to modify the system topology based on a switching
      action, an error occurred. Please try again.  If the error persists,
      please submit your code and a bug report via the ticketing system.
      */
    }
  }
...
int restoration::runPowerFlow(void) {
  ...
  if (enable_subsecond_models) // Deltamode is flagged on a module level, which
                               // means powerflow should really be going that
                               // route
  {
    powerflow_type = PF_DYNCALC; // Built on an overarching assumption (to be
                                 // implemented elsewhere) that reconfiguration
                                 // NEVER occurs at the 0th timestep
  } // End deltamode
  else // Normal mode
  {
  ...
  try {
...
#else
    PFresult = NR_Solver.solver_nr(NR_bus_count, NR_busdata, NR_branch_count,
                                   NR_branchdata, &NR_powerflow, powerflow_type,
                                   nullptr, &bad_computation);
#endif
    ...
    if (bad_computation) {
      overallresult = 0; // Flag as a failure to converge -- may be singular,
                         // NaN, etc, but is invalid
    } else if (PFresult < 0) // Failure to converge
    {
      overallresult =
          0; // Flag as a failure to converge (since that's what happened here)
    } else {
      overallresult = 1; //"Succeeded" (solved a powerflow, at least)
    }
  } catch (...) // Don't really care about the error - presumably it is
                // associated with whatever bad powerflow we just did
  {
    overallresult = -1; // Some type of "bad error" occurred - either a throw,
                        // or something worse
  }
...
void restoration::checkPF2(bool *flag, double *overLoad, int *feederID) {
  ...
  if (((oFeederID != 0) && (microgridID != 0)) || (oFeederID == -1) ||
      (microgridID == -1)) {
    *feederID = -1;
...
bool restoration::checkVoltage(void) {
  ...
  for (index = 0; index < NR_bus_count; index++) {
    ...
    {
      // Check each leg individually against nominal -- assumes nominal is 120 V
      // (or suitable equivalent based on 1N/2N, not 12)
      perunitvalue =
          (NR_busdata[index].V[0].Mag()) / NR_busdata[index].volt_base; // 1N

      // Check and see if it violates or not
      if ((perunitvalue < voltage_limit[0]) ||
          (perunitvalue > voltage_limit[1])) // 1N check
      {
      ...
      perunitvalue =
          (NR_busdata[index].V[1].Mag()) / NR_busdata[index].volt_base; // 2N

      // Check and see if it violates or not
      if ((perunitvalue < voltage_limit[0]) ||
          (perunitvalue > voltage_limit[1])) // 2N check
      {
        // Outside of range - set flag and break us
        flag = false;
        break;
    ...
    {
      // Check against individual phases -- check as a bit mask so only
      // "in-service" are caught
      if ((NR_busdata[index].phases & 0x04) == 0x04) // Has phase A
      {
        // Calculate the perunit value for A
        perunitvalue =
            (NR_busdata[index].V[0].Mag()) / NR_busdata[index].volt_base;

        // Check and see if it violates or not
        if ((perunitvalue < voltage_limit[0]) ||
            (perunitvalue > voltage_limit[1])) {
          // Outside of range - set flag and break us
          flag = false;
          break;
        }
      ...
      if ((NR_busdata[index].phases & 0x02) == 0x02) // Has phase B
      {
        // Calculate the perunit value for A
        perunitvalue =
            (NR_busdata[index].V[1].Mag()) / NR_busdata[index].volt_base;
      ...
      if ((NR_busdata[index].phases & 0x01) == 0x01) // Has phase C
      {
        // Calculate the perunit value for A
        perunitvalue =
            (NR_busdata[index].V[2].Mag()) / NR_busdata[index].volt_base;
...
void restoration::checkFeederPower(bool *fFlag, double *overLoad,
                                   int *feederID) {
  ...
  *feederID = 0;
  ...
  for (indexval = 0; indexval < numfVer; indexval++) {
    ...
    ret_value = ((int (*)(OBJECT *))(*fLinkPowerFunc[indexval]))(
        fLinkObjList[indexval]);

    // Check the value -- make sure it worked
    if (ret_value != 1) {
      GL_THROW("Restoration: failure to calculate power from link:%s",
               fLinkObjList[indexval]->name ? fLinkObjList[indexval]->name
                                            : "Unnamed");
      /*  TROUBLESHOOT
      While trying to calculate the power on a link in the system, an error
      occurred.  Please try again.  If the error persists, please submit your
      code and a bug report via the ticketing system.
      */
    }
...
void restoration::checkMG(bool *mFlag, double *overLoad, int *microgridID) {
  ...
  *microgridID = 0;
  ...
  for (indexval = 0; indexval < numMG; indexval++) {
    // Call the power computation function first
    ret_value = ((int (*)(OBJECT *))(*mLinkPowerFunc[indexval]))(
        mLinkObjList[indexval]);
    ...
    if ((microgrid_power_link_value[indexval]->Re() >
         microgrid_limit[indexval].Re()) ||
        (microgrid_power_link_value[indexval]->Im() >
         microgrid_limit[indexval].Im())) {
      *mFlag = false;
      ...
      microgrid_power_over =
          *microgrid_power_link_value[indexval] - microgrid_limit[indexval];
...
void restoration::checkLinePower(bool *lFlag, double *overLoad, int *lineID) {
  ...
  *overLoad = 0.0;
  ...
  // Check to see if line limits are being checked -- if they aren't, just skip
  // this
  if (use_link_limits) {
    // Call the function linked in NR_branchdata for each line to check itself
    for (indexval = 0; indexval < NR_branch_count; indexval++) {
      // Call the function
      ret_value = ((int (*)(OBJECT *, double *, bool *))(
          *NR_branchdata[indexval].limit_check))(NR_branchdata[indexval].obj,
                                                 &lineOverload, &lineFlag);

      // Do a general success check
      if (ret_value != 1) {
        GL_THROW("Restoration: failure to calculate limits from link:%s",
                 NR_branchdata[indexval].name ? NR_branchdata[indexval].name
                                              : "Unnamed");
        /*  TROUBLESHOOT
        While trying to calculate the amount over a limit on a link in the
        system, an error occurred.  Please try again.  If the error persists,
        please submit your code and a bug report via the ticketing system.
        */
      }
...
// Print the results to a text file -- similar to the MATLAB output (adjusted
// slightly)
void restoration::printResult(int IdxSW) {
  ...
  delta_microseconds_value =
      (int)((delta_tval - (int)(delta_tval)) * 1000000 +
            0.5); // microseconds roll-over - biased upward (by 0.5)
  retvalue = gl_localtime(
      tval, &res_event_time); // Could check to make sure this worked, but meh

  // Create the seconds counter
  seconds_dbl_value = (double)(res_event_time.second) +
                      (double)(delta_microseconds_value) / 1000000.0;
  ...
  {
    fprintf(FPOutput,
            "-- Restoration session of %4d-%02d-%02d %02d:%02d:0%2.6f --\n\n",
            res_event_time.year, res_event_time.month, res_event_time.day,
            res_event_time.hour, res_event_time.minute, seconds_dbl_value);
  } else // 10 or above, 2 digit print works
  {
    fprintf(FPOutput,
            "-- Restoration session of %4d-%02d-%02d %02d:%02d:%2.6f --\n\n",
            res_event_time.year, res_event_time.month, res_event_time.day,
            res_event_time.hour, res_event_time.minute, seconds_dbl_value);
  }
  fprintf(FPOutput,
          "Fault section: %d - %d (in original topology), %d - %d (in "
          "simplified topology)\n",
  ...
  // Restoration results
  if (IdxSW >= 0) // Full restoration is successful	-- was "1" in MATLAB,
                  // but that is a counter.  -1 values coded as bad for GLD
  {
  ...
  } else if (candidateSwOpe.data_6 != nullptr) // Partial restoration
  {
    ...
    fprintf(FPOutput,
            "Full restoration failed. Partial restoration performed.\n");
...
void restoration::printCIO(FILE *FPHandle, int IdxSW) {
  ...
  fprintf(FPHandle,
          "Open: %d - %d (in original topology), %d - %d (in simplified "
          "topology)\n",
          candidateSwOpe.data_1[IdxSW], candidateSwOpe.data_2[IdxSW],
          candidateSwOpe_1.data_1[IdxSW], candidateSwOpe_1.data_2[IdxSW]);
  fprintf(FPHandle, "Open: %s - %s (in original topology)\n",
          NR_busdata[candidateSwOpe.data_1[IdxSW]].name,
          NR_busdata[candidateSwOpe.data_2[IdxSW]].name);

  if ((candidateSwOpe.data_3[IdxSW] != s_ver) &&
      (candidateSwOpe.data_4[IdxSW] != s_ver)) {
    fprintf(FPHandle,
            "Close: %d - %d (in original topology), %d - %d (in simplified "
            "topology)\n",
            candidateSwOpe.data_3[IdxSW], candidateSwOpe.data_4[IdxSW],
            candidateSwOpe_1.data_3[IdxSW], candidateSwOpe_1.data_4[IdxSW]);
    fprintf(FPHandle, "Close: %s - %s (in original topology)\n",
            NR_busdata[candidateSwOpe.data_3[IdxSW]].name,
            NR_busdata[candidateSwOpe.data_4[IdxSW]].name);
  } else {
    if (candidateSwOpe.data_3[IdxSW] == s_ver) {
      ...
      if (idx_microgrid != -1) {
        fprintf(FPHandle,
                "Close: %d - Microgrid%d (in original topology), %d - "
                "Microgrid%d (in simplified topology)\n",
                candidateSwOpe.data_4[IdxSW], idx_microgrid,
                candidateSwOpe_1.data_4[IdxSW], idx_microgrid);
...
bool restoration::isSwitch(int iIndex, int jIndex) {
  ...
  for (idx = 0; idx < sec_swi.currSize; idx++) {
    if (((iIndex == sec_swi.data_1[idx]) && (jIndex == sec_swi.data_2[idx])) ||
        ((jIndex == sec_swi.data_1[idx]) && (iIndex == sec_swi.data_2[idx]))) {
      flag = true;
      break;
    }
  }
...
bool restoration::isSwiInFeeder(BRANCHVERTICES *swi_2, int feederID_overload) {
  ...
  if (feederID_overload < 100) // If a feeder overloaded
  {
    ...
    //"Stuck" detector -- not sure why it does sometimes, but this is a kludgy
    //way to prevent it
...
void restoration::PowerflowSave(void) {
  ...
  if (voltage_storage != nullptr) {
    ...
    for (indexval = 0; indexval < NR_bus_count; indexval++) {
      gl_free((void **)&voltage_storage[indexval]);
    }

    // Free us up too
    gl_free((void **)&voltage_storage);
  }

  // Allocate us
  voltage_storage =
      (gld::complex **)gl_malloc(NR_bus_count * sizeof(gld::complex *));
  ...
  if (voltage_storage == nullptr) {
    ...
    an error occurred.  Please try again, assuming proper inputs.  If the error
    persists, please submit your code and a bug report via the ticketing system.
    */
  }
  ...
  for (indexval = 0; indexval < NR_bus_count; indexval++) {
    // Allocate this entry
    voltage_storage[indexval] =
        (gld::complex *)gl_malloc(3 * sizeof(gld::complex));

    // Check it
    if (voltage_storage[indexval] == nullptr) {
      GL_THROW(
          "Restoration: failed to allocate memory for base voltage values!");
      // Defined above
    }
...
// search the linked list for given data.  If found, return index of 1st node.
...
// modifies node data.  Only 1st found data is modified.  If oldData is found,
// modify it and return 1 if oldData is not found, do nothing and return 0
...
// adds a new node after the kth node in the linked list.  If k is out of
// bounds, do nothing. if k = 0, add the new node as the first node
void Chain::addNode(int kIndex, int newData) {
  ...
  if (newNode == nullptr) {
    ...
    /*  TROUBLESHOOT
    While attempting to allocate a new node in one of the restoration program's
    graph theory chains, an error was encountered.  Please try again.  If the
    error persists, please submit your code and a bug report via the ticket
    system.
    */
  }
...
LinkedBase::LinkedBase(int nVer) {
  ...
  if (nVer == 0) {
    ...
    if (adjList == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to allocate a new chain in one of the restoration
      program's graph theory chains, an error was encountered.  Please try
      again.  If the error persists, please submit your code and a bug report
    ...
    for (indexvar = 0; indexvar < numVertices; indexvar++) {
      ...
      // Call the constructor for everything (since doing explicit memory
      // management) Allocate each new object
...
void LinkedBase::delAllVer(void) {
  ...
  if (numVertices != 0) {
    ...
    gl_free((void **)&adjList); // Main pointer
    // Null it too
    adjList = nullptr;
    ...
    source = 0;
    gl_free((void **)&status_value);
    gl_free((void **)&parent_value);
    gl_free((void **)&dist);
    gl_free((void **)&dTime);
    gl_free((void **)&fTime);
...
int LinkedBase::getVerNum(void) { return numVertices; }

// Return the number of edges
int LinkedBase::getEdgNum(void) { return numEdges; }
...
// visits vertices in depth first search and marks them as visited; u is a
// vertex
...
void LinkedBase::copy(LinkedBase *graph1) {
  ...
  addAIsoVer(graph1->numVertices); // This whole routine strikes me as odd, but
                                   // this is theoretically verbatim from MATLAB
...
void LinkedBase::addAIsoVer(int n) {
  ...
  gl_free((void **)&tempBase);

  // nullptr it, for giggles
  tempBase = nullptr;

  // See if interPos has anything -- if it does, delete it first (just to be
  // thorough) Theoretically, only this references it, so prevents orphaning --
  // may need to revisit in the future if doesn't work
  if (iterPos != nullptr) {
    deactivatePos();
  }
  ...
  // Appears to zero things for new vector sizes -- check the first one and
  // release, if needed
  if (status_value != nullptr) {
    // Free everyone first
    gl_free((void **)&status_value);
  ...
  // Update initializations to reflect the increased size (others already
  // alloced and copied in above)
...
bool LinkedDigraph::isEdgeExisting(int iIndex, int jIndex) {
  ...
  if ((iIndex < 0) || (jIndex < 0) || (iIndex >= numVertices) ||
      (jIndex >= numVertices)) {
...
void LinkedDigraph::addEdge(int iIndex, int jIndex) {
  ...
  if (isEdgeExisting(iIndex, jIndex)) {
    GL_THROW(
        "Restoration: Error: Can not add an edge that is already existing!");
  }
...
int LinkedDigraph::getOutDeg(int index) { return LinkedBase::getOutDeg(index); }
...
void LinkedUndigraph::addEdge(int iIndex, int jIndex) {
  ...
  if (isEdgeExisting(iIndex, jIndex)) {
    ...
    gl_warning("Restoration: (addEdge): Edge (%d,%d) is existing", iIndex,
               jIndex);
    return;
  }
  adjList[iIndex]->addNode(0, jIndex);
...
int LinkedUndigraph::getDeg(int index) { return getOutDeg(index); }

// Find fundamental cut set
void LinkedUndigraph::findFunCutSet(CHORDSET *chordSet, BRANCHVERTICES *tBranch,
                                    CHORDSET *cutSet) {
  ...
  for (idx = 0; idx < chordSet->currSize; idx++) {
    ...
    while (curIndex != -1) {
      ...
      debug_counter++; // Added to prevent it from getting stuck (circular
                       // loops)

      // Check the debug counter -- arbitrary check -- these seem like they
    ...
    while (curIndex != -1) {
      ...
      if ((debug_counter > numVertices) && (debug_counter > numEdges)) {
        isCutSetEle =
            false; // Short cirucit next part - effectively a "continue"
        break;     // Death to the loop!
      }
    }
...
void LinkedUndigraph::simplify(CHORDSET *chordset, BRANCHVERTICES *fBranch,
                               int source, INTVECT *fVer, INTVECT *vMap) {
  ...
  // Allocate the "unique" output vector -- assume it will never be bigger than
  // vMap's largest size (assumed to be total number of nodes)
  ...
  // Loop -- still not convinced this isn't a single execution or a race
  // condition - we'll see (******************)
  while (resultval != 0) {
    resultval = isolateDeg12Ver(&iVer);
  }
  ...
  // Free up our mess
  gl_free((void **)&tempvect.data);
  gl_free((void **)&iVer.data);
}

// Find function
// Parses a vector and extracts the indices of any matches of findval (to the
// maximum number in numfind) and places them into foundvect.  If numfind is -1,
// it finds all matches, otherwise it stops after numfind entries foundvect
// currsize variable will be a test for success (+1 = number of entries found,
// so -1 = none found)
void LinkedUndigraph::find_int(INTVECT *inputvect, INTVECT *foundvect,
                               int findval, int numfind) {
  int indexval;

  // Reset output vector as a matter of principle - set it to -1 (only for
  // numfind, unless it is -1)
  if (numfind != -1) {
...
int LinkedUndigraph::isolateDeg12Ver(INTVECT *iVer) {
  ...
  if (iMark == nullptr) {
    GL_THROW("Restoration:Failed to allocate new temp variable");
    /*  TROUBLESHOOT
    While attempting to allocate space for a new temporary variable, an error
    was encountered. Please try again.  If the error persists, please submit
    your code via the ticketing system.
    */
  }
...
void LinkedUndigraph::deleteIsoVer(INTVECT *vMap) {
  ...
  if (vMap->data != nullptr) {
    ...
    if (vMap->maxSize != numVertices) {
      // Free it
      gl_free((void **)&vMap->data);

      // No need to nullptr it, we're just going to allocate it again in a
      // second Changed my mind, nullptr it anyways
      vMap->data = nullptr;
  ...
  for (idx = 0; idx < numIsoVer; idx++) {
    ...
    gl_free((void **)&adjList[isoVerArray.data[idx]]);
  ...
  // Now that it is copied, destroy the old "main list" (elements stay alloced)
  // Theoretically, any of the "extended" items have already been dealloced
  // above in the adjacency list for loop (previous for)
  gl_free((void **)&tempList);
...
void LinkedUndigraph::mergeVer_2(INTVECT *vMap) {
  ...
  // Allocate three working arrays -- assume it won't be any bigger than vMap
  // for now (may need to be revisited)
  isoVerArray.currSize = 0;
  ...
  merge_V.maxSize = vMap->maxSize;     // This should be extremely overkill, but
  ...
  for (idx = 0; idx < n_V_new; idx++) {
    ...
    // **** Seems like there should be an empty check here, but seems to be
    // handled by changes below
  ...
  gl_free((void **)&resVerArray.data);

  // Free up temporary vectors
  gl_free((void **)&V_new.data);
  gl_free((void **)&merge_V.data);
  gl_free((void **)&tempoutVect.data);
}
...
void unique_int(INTVECT *inputvect, INTVECT *outputvect) {
  ...
  // Allocate a working input matrix too -- otherwise we potentially screw up
  // the order elsewhere (keep it separate)
  workmatrix_input.currSize = inputvect->currSize;
  workmatrix_input.maxSize = inputvect->maxSize;
  workmatrix_input.data =
      (int *)gl_malloc(workmatrix_input.maxSize * sizeof(int));
  ...
  // Copy the input into the work matrix - do a memcpy, just to be clever - only
  // copy the current size (we don't need more)
  memcpy(workmatrix_input.data, inputvect->data,
         inputvect->currSize * sizeof(int));

  // Perform the merge sort
  merge_sort_int(workmatrix_input.data, workmatrix_input.currSize,
                 workmatrix.data);
  ...
  while (leftidx < workmatrix_input.currSize) {
    if (workmatrix_input.data[leftidx] != outputvect->data[rightidx]) // Unique
    {
      ...
      if (rightidx > outputvect->maxSize) {
        GL_THROW("Restoration: Larger number of unique entries than expected "
                 "found!");
        /*  TROUBLESHOOT
        While looking for unique vertices in the restoration algorithm, more
        unique values than expected were found and the vector is mis-sized.
        Please submit your code and a bug report via the ticketing system.
        */
      }
  ...
  gl_free((void **)&workmatrix.data);
  gl_free((void **)&workmatrix_input.data);
}
...
void merge_sort_int(int *Input_Array, unsigned int Alen, int *Work_Array) {
  ...
  if (Alen > 0) // Only occurs if over zero
  {
    ...
    right_length =
        Alen - split_point; // Figure out how big the right hand side is
    ...
    do {
      if (*leftside < *rightside)
        ...
        *Final_P++ = *rightside++;
             ...
             (rightside <
              (Input_Array + Alen))); // Sort the list until one of them empties

    while (leftside < (Input_Array +
                       split_point)) // Put any remaining entries into the list
      *Final_P++ = *leftside++;

    while (rightside <
           (Input_Array + Alen)) // Put any remaining entries into the list
      *Final_P++ = *rightside++;

    memcpy(Input_Array, Work_Array,
           sizeof(int) * Alen); // Copy the result back into the input
  } // End length > 0
}
...
extern "C" MODULE_API int isa_restoration(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_restoration_impl(obj, classsname);
}
#endif
