...
load_tracker::load_tracker(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "load_tracker", sizeof(load_tracker),
    ...
    if (gl_publish_variable(
            oclass, PT_object, "target", PADDR(target), PT_DESCRIPTION,
            "target object to track the load of", PT_char256, "target_property",
            PADDR(target_prop), PT_DESCRIPTION,
            "property on the target object representing the load",
            PT_enumeration, "operation", PADDR(operation), PT_DESCRIPTION,
            "operation to perform on complex property types", PT_KEYWORD,
            "REAL", (enumeration)REAL, PT_KEYWORD, "IMAGINARY",
            (enumeration)IMAGINARY, PT_KEYWORD, "MAGNITUDE",
            (enumeration)MAGNITUDE, PT_KEYWORD, "ANGLE",
            (enumeration)ANGLE, // If using, please specify in radians
            PT_double, "full_scale", PADDR(full_scale), PT_DESCRIPTION,
            "magnitude of the load at full load, used for feed-forward control",
            PT_double, "setpoint", PADDR(setpoint), PT_DESCRIPTION,
            "load setpoint to track to", PT_double, "deadband", PADDR(deadband),
            PT_DESCRIPTION, "percentage deadband", PT_double, "damping",
            PADDR(damping), PT_DESCRIPTION, "load setpoint to track to",
            PT_double, "output", PADDR(output), PT_DESCRIPTION,
            "output scaling value", PT_double, "feedback", PADDR(feedback),
            PT_DESCRIPTION, "the feedback signal, for reference purposes",
            nullptr) < 1)
...
int load_tracker::init(OBJECT *parent) {
  ...
  if (target_property == nullptr) {
    GL_THROW("Unable to find property \"%s\" in object %s",
             target_prop.get_string(), target->name);
    /* TROUBLESHOOT
  ...
  case PT_int64:
    break;
  default:
    GL_THROW("Unsupported property type.  Supported types are complex, double "
             "and integer");
    /* TROUBLESHOOT
    Please specify a property whose type is either a double, complex, int16,
    int32, or an int64.
    */
  }
  ...
  // The VALUEPOINTER is a union of pointers so we only need to check one of
  // them....
  if (pointer.d == nullptr) {
    GL_THROW("Unable to bind to property \"%s\" in object %s",
             target_prop.get_string(), target->name);
    /* TROUBLESHOOT
    The property given does not exist for the given property object. Please
    specify an existing property of the target object.
    */
  }
...
void load_tracker::update_feedback_variable() {
  ...
  std::shared_lock<std::shared_mutex> subnode_lock(
      SharedMutexManager::get_mutex(target));
  switch (type) {
  case PT_double:
    feedback = *(pointer.d);
    break;
  ...
  if (setpoint == 0.0) {
    ...
    if (feedback < setpoint) {
      ...
      if (percent_error > (deadband / 100.0)) {
        output *=
            1.0 + ((setpoint - feedback) / feedback) * (1.0 / (1.0 + damping));
      }
    } else {
      double percent_error = (feedback - setpoint) / full_scale;
      if (percent_error > (deadband / 100.0)) {
        output *=
            1.0 - ((feedback - setpoint) / feedback) * (1.0 / (1.0 + damping));
      }
    }
  }
...
TIMESTAMP load_tracker::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if ((solver_method == SM_FBS) ||
      (solver_method == SM_NR && !NR_admit_change)) {
    update_feedback_variable();
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_load_tracker(OBJECT *obj, TIMESTAMP t0,
                                                  PASSCONFIG pass) {
