...
public:
  ...
  double measured_real_max_voltage_in_interval
      [3]; ///< metered real part of maximum voltage over the last interval
  double measured_imag_max_voltage_in_interval
      [3]; ///< metered imaginary part of maximum voltage over the last interval
  double measured_real_min_voltage_in_interval
      [3]; ///< metered real part of minimum voltage over the last interval
  double measured_imag_min_voltage_in_interval
      [3]; ///< metered imaginary part of minimum voltage over the last interval
  double measured_avg_voltage_mag_in_interval
      [3]; ///< metered real part of average voltage over the last interval
  ...
  bool tpmeter_interrupted; ///< Reliability flag - goes active if the customer
                            ///< is in an "interrupted" state
  bool tpmeter_interrupted_secondary; ///< Reliability flag - goes active if the
                                      ///< customer is in a "secondary
  ...
  int16 s_flag; ///< reliability flag that gets set if the meter experienced
                ///< more than n sustained interruptions
  int16 t_flag; ///< reliability flage that gets set if the meter experienced
                ///< more than n events total
  ...
  double
      previous_monthly_energy; ///< Total monthly energy for the previous month
  ...
  OBJECT *power_market; ///< Designates the auction object where prices are read
                        ///< from for bill mode
  PROPERTY *price_prop; ///< Designates what property to observe in the auction
                        ///< object for price
...
private:
  ...
  double previous_energy_total; ///< Used to track what the meter reading was
                                ///< the previous month
...
public:
  ...
  SIMULATIONMODE
  inter_deltaupdate_triplex_meter(unsigned int64 delta_time, unsigned long dt,
