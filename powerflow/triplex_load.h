...
private:
    bool base_load_val_was_nonzero[3]; ///< Tracking variable to make ZIP-fraction
                                       ...
                                       ///< not already zeroed)
    bool ZIP_constant_current[3];      ///< Tracking variable for if ZIP-fraction loads
                                       ///< have overwritten constant_current_X
...
public:
    ...
    SIMULATIONMODE
    inter_deltaupdate_triplex_load(unsigned int64 delta_time, unsigned long dt,
                                   unsigned int iteration_count_val,
                                   bool interupdate_pos);
};
