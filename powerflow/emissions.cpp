...
emissions::emissions(MODULE *mod) : powerflow_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "emissions", sizeof(emissions),
    ...
    if (gl_publish_variable(
            ...
            PT_double, "Nuclear_Order", PADDR(Nuclear_Order), PT_double,
            "Hydroelectric_Order", PADDR(Hydroelectric_Order), PT_double,
            "Solarthermal_Order", PADDR(Solarthermal_Order), PT_double,
            "Biomass_Order", PADDR(Biomass_Order), PT_double, "Wind_Order",
            PADDR(Wind_Order), PT_double, "Coal_Order", PADDR(Coal_Order),
            PT_double, "Naturalgas_Order", PADDR(Naturalgas_Order), PT_double,
            "Geothermal_Order", PADDR(Geothermal_Order), PT_double,
            "Petroleum_Order", PADDR(Petroleum_Order),

            PT_double, "Naturalgas_Max_Out[kWh]", PADDR(Naturalgas_Max_Out),
            PT_double, "Coal_Max_Out[kWh]", PADDR(Coal_Max_Out), PT_double,
            "Biomass_Max_Out[kWh]", PADDR(Biomass_Max_Out), PT_double,
            "Geothermal_Max_Out[kWh]", PADDR(Geothermal_Max_Out), PT_double,
            "Hydroelectric_Max_Out[kWh]", PADDR(Hydroelectric_Max_Out),
            PT_double, "Nuclear_Max_Out[kWh]", PADDR(Nuclear_Max_Out),
            PT_double, "Wind_Max_Out[kWh]", PADDR(Wind_Max_Out), PT_double,
            "Petroleum_Max_Out[kWh]", PADDR(Petroleum_Max_Out), PT_double,
            "Solarthermal_Max_Out[kWh]", PADDR(Solarthermal_Max_Out),

            PT_double, "Naturalgas_Out[kWh]", PADDR(Naturalgas_Out), PT_double,
            "Coal_Out[kWh]", PADDR(Coal_Out), PT_double, "Biomass_Out[kWh]",
            PADDR(Biomass_Out), PT_double, "Geothermal_Out[kWh]",
            PADDR(Geothermal_Out), PT_double, "Hydroelectric_Out[kWh]",
            PADDR(Hydroelectric_Out), PT_double, "Nuclear_Out[kWh]",
            PADDR(Nuclear_Out), PT_double, "Wind_Out[kWh]", PADDR(Wind_Out),
            PT_double, "Petroleum_Out[kWh]", PADDR(Petroleum_Out), PT_double,
            "Solarthermal_Out[kWh]", PADDR(Solarthermal_Out),

            PT_double, "Naturalgas_Conv_Eff[Btu/kWh]",
            PADDR(Naturalgas_Conv_Eff), PT_double, "Coal_Conv_Eff[Btu/kWh]",
            PADDR(Coal_Conv_Eff), PT_double, "Biomass_Conv_Eff[Btu/kWh]",
            PADDR(Biomass_Conv_Eff), PT_double, "Geothermal_Conv_Eff[Btu/kWh]",
            PADDR(Geothermal_Conv_Eff), PT_double,
            "Hydroelectric_Conv_Eff[Btu/kWh]", PADDR(Hydroelectric_Conv_Eff),
            ...
            PT_double, "Solarthermal_Conv_Eff[Btu/kWh]",
            PADDR(Solarthermal_Conv_Eff),

            PT_double, "Naturalgas_CO2[lb/Btu]", PADDR(Naturalgas_CO2),
            PT_double, "Coal_CO2[lb/Btu]", PADDR(Coal_CO2), PT_double,
            "Biomass_CO2[lb/Btu]", PADDR(Biomass_CO2), PT_double,
            "Geothermal_CO2[lb/Btu]", PADDR(Geothermal_CO2), PT_double,
            "Hydroelectric_CO2[lb/Btu]", PADDR(Hydroelectric_CO2), PT_double,
            "Nuclear_CO2[lb/Btu]", PADDR(Nuclear_CO2), PT_double,
            "Wind_CO2[lb/Btu]", PADDR(Wind_CO2), PT_double,
            "Petroleum_CO2[lb/Btu]", PADDR(Petroleum_CO2), PT_double,
            "Solarthermal_CO2[lb/Btu]", PADDR(Solarthermal_CO2),

            PT_double, "Naturalgas_SO2[lb/Btu]", PADDR(Naturalgas_SO2),
            PT_double, "Coal_SO2[lb/Btu]", PADDR(Coal_SO2), PT_double,
            "Biomass_SO2[lb/Btu]", PADDR(Biomass_SO2), PT_double,
            "Geothermal_SO2[lb/Btu]", PADDR(Geothermal_SO2), PT_double,
            "Hydroelectric_SO2[lb/Btu]", PADDR(Hydroelectric_SO2), PT_double,
            "Nuclear_SO2[lb/Btu]", PADDR(Nuclear_SO2), PT_double,
            "Wind_SO2[lb/Btu]", PADDR(Wind_SO2), PT_double,
            "Petroleum_SO2[lb/Btu]", PADDR(Petroleum_SO2), PT_double,
            "Solarthermal_SO2[lb/Btu]", PADDR(Solarthermal_SO2),

            PT_double, "Naturalgas_NOx[lb/Btu]", PADDR(Naturalgas_NOx),
            PT_double, "Coal_NOx[lb/Btu]", PADDR(Coal_NOx), PT_double,
            "Biomass_NOx[lb/Btu]", PADDR(Biomass_NOx), PT_double,
            "Geothermal_NOx[lb/Btu]", PADDR(Geothermal_NOx), PT_double,
            "Hydroelectric_NOx[lb/Btu]", PADDR(Hydroelectric_NOx), PT_double,
            "Nuclear_NOx[lb/Btu]", PADDR(Nuclear_NOx), PT_double,
            "Wind_NOx[lb/Btu]", PADDR(Wind_NOx), PT_double,
            "Petroleum_NOx[lb/Btu]", PADDR(Petroleum_NOx), PT_double,
            "Solarthermal_NOx[lb/Btu]", PADDR(Solarthermal_NOx),

            PT_double, "Naturalgas_emissions_CO2[lb]",
            PADDR(Naturalgas_emissions_CO2), PT_double,
            "Naturalgas_emissions_SO2[lb]", PADDR(Naturalgas_emissions_SO2),
            PT_double, "Naturalgas_emissions_NOx[lb]",
            PADDR(Naturalgas_emissions_NOx),
            ...
            PT_double, "Biomass_emissions_CO2[lb]",
            PADDR(Biomass_emissions_CO2), PT_double,
            "Biomass_emissions_SO2[lb]", PADDR(Biomass_emissions_SO2),
            PT_double, "Biomass_emissions_NOx[lb]",
            PADDR(Biomass_emissions_NOx),

            PT_double, "Geothermal_emissions_CO2[lb]",
            PADDR(Geothermal_emissions_CO2), PT_double,
            "Geothermal_emissions_SO2[lb]", PADDR(Geothermal_emissions_SO2),
            PT_double, "Geothermal_emissions_NOx[lb]",
            PADDR(Geothermal_emissions_NOx),

            PT_double, "Hydroelectric_emissions_CO2[lb]",
            PADDR(Hydroelectric_emissions_CO2), PT_double,
            "Hydroelectric_emissions_SO2[lb]",
            PADDR(Hydroelectric_emissions_SO2), PT_double,
            "Hydroelectric_emissions_NOx[lb]",
            PADDR(Hydroelectric_emissions_NOx),

            PT_double, "Nuclear_emissions_CO2[lb]",
            PADDR(Nuclear_emissions_CO2), PT_double,
            "Nuclear_emissions_SO2[lb]", PADDR(Nuclear_emissions_SO2),
            PT_double, "Nuclear_emissions_NOx[lb]",
            PADDR(Nuclear_emissions_NOx),
            ...
            PT_double, "Petroleum_emissions_CO2[lb]",
            PADDR(Petroleum_emissions_CO2), PT_double,
            "Petroleum_emissions_SO2[lb]", PADDR(Petroleum_emissions_SO2),
            PT_double, "Petroleum_emissions_NOx[lb]",
            PADDR(Petroleum_emissions_NOx),

            PT_double, "Solarthermal_emissions_CO2[lb]",
            PADDR(Solarthermal_emissions_CO2), PT_double,
            "Solarthermal_emissions_SO2[lb]", PADDR(Solarthermal_emissions_SO2),
            PT_double, "Solarthermal_emissions_NOx[lb]",
            PADDR(Solarthermal_emissions_NOx),
            ...
            PT_double, "cycle_interval[s]", PADDR(cycle_interval), nullptr) < 1)
...
int emissions::init(OBJECT *parent) {
  ...
  if (parent != nullptr) {
    ...
    if (gl_object_isa(parent, "meter", "powerflow")) {
      ...
      // Make sure it worked
      if (!parent_meter_total_power->is_valid() ||
          !parent_meter_total_power->is_complex()) {
        GL_THROW(
            "emissions:%d - %s - Unable to map parent object's power value",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While attempting to map the parent meter's measured_power field, the
        emissions object encountered an error.  Please try again.  If the error
        persists, please submit your code and a bug report via the issues
        tracker.
        */
      }
    } else {
      GL_THROW("emissions:%s must be parented to a three-phase meter to work!",
               obj->name);
      /*  TROUBLESHOOT
      The emissions object requires a three-phase meter object to properly work.
      Please parent the emissions object to such a meter and try again.
      */
    }
  } else {
    GL_THROW("emissions:%s must be parented to a three-phase meter to work!",
  ...
  if (cycle_interval <= 0) {
    GL_THROW("emissions:%s must have a positive cycle_interval time",
             obj->name);
    /*  TROUBLESHOOT
    The calculation cycle interval for the emissions object must be a non-zero,
    positive value.  Please specify one and try again.
    */
  }
  ...
  if (Nuclear_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for nuclear generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Hydroelectric_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for hydroelectric generators was set to 0, so these
    generator types will be ignored from the emissions dispatch.  Ensure this
    was desired.
    */
  }
  ...
  if (Solarthermal_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for solar generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Biomass_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for biomass generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Wind_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for wind generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Coal_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for coal generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Naturalgas_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for natural generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
  ...
  if (Geothermal_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for geothermal generators was set to 0, so these
    generator types will be ignored from the emissions dispatch.  Ensure this
    was desired.
    */
  }
  ...
  if (Petroleum_Order == 0) {
    ...
    /*  TROUBLESHOOT
    The dispatch order for petroleum generators was set to 0, so these generator
    types will be ignored from the emissions dispatch.  Ensure this was desired.
    */
  }
...
TIMESTAMP emissions::postsync(TIMESTAMP t0) {
  ...
  if (curr_cycle_time >= time_cycle_interval) // Update values
  {
    ...
    // Order offset by 1 to get 1-9 range.  Zero is left in as a default
    // condition (just in case an order is being swapped)
    for (dispatch_order = 0; dispatch_order < 10; dispatch_order++) {

      // Energy output of any power source based on its maximum capacity and
      // requirement. If energy required is greater than the maximum capacity of
      // the corresponding power source,the source would supply its maximum,
      // otherwise it would supply only the required. The dispacth order is
      // based on the users' choice.

      if (dispatch_order == Nuclear_Order) {
        if ((Nuclear_Max_Out < energy_for_calc.Mag() && energy_requirement) &&
            (dispatch_order != 0))
        ...
        } else // Not needed
        {
          ...
          // Zero the emissions counts, otherwise they maintain previous values
          // (coudl just make local, but are published)
          Nuclear_emissions_CO2 = 0.0;
          Nuclear_emissions_SO2 = 0.0;
          Nuclear_emissions_NOx = 0.0;
        }
      ...
      if (dispatch_order == Hydroelectric_Order) {
        if ((Hydroelectric_Max_Out < energy_for_calc.Mag() &&
             energy_requirement) &&
            (dispatch_order != 0)) {
          Hydroelectric_Out = Hydroelectric_Max_Out;

          Hydroelectric_emissions_CO2 =
              Hydroelectric_Out * Hydroelectric_Conv_Eff * Hydroelectric_CO2;
          Hydroelectric_emissions_SO2 =
              Hydroelectric_Out * Hydroelectric_Conv_Eff * Hydroelectric_SO2;
          Hydroelectric_emissions_NOx =
              Hydroelectric_Out * Hydroelectric_Conv_Eff * Hydroelectric_NOx;
      ...
      if (dispatch_order == Solarthermal_Order) {
        if ((Solarthermal_Max_Out < energy_for_calc.Mag() &&
             energy_requirement) &&
            (dispatch_order != 0)) {
          Solarthermal_Out = Solarthermal_Max_Out;

          Solarthermal_emissions_CO2 =
              Solarthermal_Out * Solarthermal_Conv_Eff * Solarthermal_CO2;
          Solarthermal_emissions_SO2 =
              Solarthermal_Out * Solarthermal_Conv_Eff * Solarthermal_SO2;
          Solarthermal_emissions_NOx =
              Solarthermal_Out * Solarthermal_Conv_Eff * Solarthermal_NOx;
      ...
      if (dispatch_order == Biomass_Order) {
        if ((Biomass_Max_Out < energy_for_calc.Mag() && energy_requirement) &&
            (dispatch_order != 0)) {
      ...
      if (dispatch_order == Wind_Order) {
        if ((Wind_Max_Out < energy_for_calc.Mag() && energy_requirement) &&
            (dispatch_order != 0)) {
      ...
      if (dispatch_order == Coal_Order) {
        if ((Coal_Max_Out < energy_for_calc.Mag() && energy_requirement) &&
            (dispatch_order != 0)) {
      ...
      if (dispatch_order == Naturalgas_Order) {
        if ((Naturalgas_Max_Out < energy_for_calc.Mag()) &&
            energy_requirement && (dispatch_order != 0)) {
          Naturalgas_Out = Naturalgas_Max_Out;

          Naturalgas_emissions_CO2 =
              Naturalgas_Out * Naturalgas_Conv_Eff * Naturalgas_CO2;
          Naturalgas_emissions_SO2 =
              Naturalgas_Out * Naturalgas_Conv_Eff * Naturalgas_SO2;
          Naturalgas_emissions_NOx =
              Naturalgas_Out * Naturalgas_Conv_Eff * Naturalgas_NOx;
      ...
      if (dispatch_order == Geothermal_Order) {
        if ((Geothermal_Max_Out < energy_for_calc.Mag() &&
             energy_requirement) &&
            (dispatch_order != 0)) {
          Geothermal_Out = Geothermal_Max_Out;

          Geothermal_emissions_CO2 =
              Geothermal_Out * Geothermal_Conv_Eff * Geothermal_CO2;
          Geothermal_emissions_SO2 =
              Geothermal_Out * Geothermal_Conv_Eff * Geothermal_SO2;
          Geothermal_emissions_NOx =
              Geothermal_Out * Geothermal_Conv_Eff * Geothermal_NOx;
      ...
      if (dispatch_order == Petroleum_Order) {
        if ((Petroleum_Max_Out < energy_for_calc.Mag() && energy_requirement) &&
            (dispatch_order != 0)) {
          Petroleum_Out = Petroleum_Max_Out;

          Petroleum_emissions_CO2 =
              Petroleum_Out * Petroleum_Conv_Eff * Petroleum_CO2;
          Petroleum_emissions_SO2 =
              Petroleum_Out * Petroleum_Conv_Eff * Petroleum_SO2;
          Petroleum_emissions_NOx =
              Petroleum_Out * Petroleum_Conv_Eff * Petroleum_NOx;
    ...
    // If energy required is greater than the energy available, the rest would
    // be supplied by coal.
    if (energy_for_calc.Mag() > 0) {
      Coal_Out = Coal_Out + energy_for_calc.Mag();
    }

    Total_energy_out = Naturalgas_Out + Coal_Out + Biomass_Out +
                       Geothermal_Out + Hydroelectric_Out + Nuclear_Out +
                       Wind_Out + Petroleum_Out + Solarthermal_Out;

    Total_emissions_CO2 = Naturalgas_emissions_CO2 + Coal_emissions_CO2 +
                          Biomass_emissions_CO2 + Geothermal_emissions_CO2 +
                          Hydroelectric_emissions_CO2 + Nuclear_emissions_CO2 +
                          Wind_emissions_CO2 + Petroleum_emissions_CO2 +
                          Solarthermal_emissions_CO2;

    Total_emissions_SO2 = Naturalgas_emissions_SO2 + Coal_emissions_SO2 +
                          Biomass_emissions_SO2 + Geothermal_emissions_SO2 +
                          Hydroelectric_emissions_SO2 + Nuclear_emissions_SO2 +
                          Wind_emissions_SO2 + Petroleum_emissions_SO2 +
                          Solarthermal_emissions_SO2;

    Total_emissions_NOx = Naturalgas_emissions_NOx + Coal_emissions_NOx +
                          Biomass_emissions_NOx + Geothermal_emissions_NOx +
                          Hydroelectric_emissions_NOx + Nuclear_emissions_NOx +
                          Wind_emissions_NOx + Petroleum_emissions_NOx +
                          Solarthermal_emissions_NOx;
...
static TIMESTAMP sync_emissions_impl(OBJECT *obj, TIMESTAMP t0,
  ...
  try {
    ...
    gl_error("emissions %s (%s:%d): %s", obj->name, obj->oclass->name, obj->id,
             msg);
  } catch (...) {
    gl_error("emissions %s (%s:%d): unknown exception", obj->name,
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_emissions(OBJECT *obj, TIMESTAMP t0,
                                               PASSCONFIG pass) {
  return sync_emissions_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_emissions(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
extern "C" MODULE_API int isa_emissions(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_emissions_impl(obj, classsname);
}
#endif
/**@}**/
