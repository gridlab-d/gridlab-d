...
motor::motor(MODULE *mod) : node(mod) {
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "motor", sizeof(motor),
                          passconfig | PC_UNSAFE_OVERRIDE_OMIT | PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "node", PT_double, "base_power[W]",
            PADDR(Pbase), PT_DESCRIPTION, "base power", PT_double, "n",
            PADDR(n), PT_DESCRIPTION,
            "ratio of stator auxiliary windings to stator main windings",
            PT_double, "Rds[ohm]", PADDR(Rds), PT_DESCRIPTION,
            "d-axis resistance - single-phase model", PT_double, "Rqs[ohm]",
            PADDR(Rqs), PT_DESCRIPTION,
            "q-asis resistance - single-phase model", PT_double, "Rs[ohm]",
            PADDR(Rs), PT_DESCRIPTION, "stator resistance - three-phase model",
            PT_double, "Rr[ohm]", PADDR(Rr), PT_DESCRIPTION, "rotor resistance",
            PT_double, "Xm[ohm]", PADDR(Xm), PT_DESCRIPTION,
            "magnetizing reactance", PT_double, "Xr[ohm]", PADDR(Xr),
            PT_DESCRIPTION, "rotor reactance", PT_double, "Xs[ohm]", PADDR(Xs),
            PT_DESCRIPTION, "stator leakage reactance - three-phase model",
            PT_double, "Xc_run[ohm]", PADDR(Xc1), PT_DESCRIPTION,
            "running capacitor reactance - single-phase model", PT_double,
            "Xc_start[ohm]", PADDR(Xc2), PT_DESCRIPTION,
            "starting capacitor reactance - single-phase model", PT_double,
            "Xd_prime[ohm]", PADDR(Xd_prime), PT_DESCRIPTION,
            "d-axis reactance - single-phase model", PT_double, "Xq_prime[ohm]",
            PADDR(Xq_prime), PT_DESCRIPTION,
            "q-axis reactance - single-phase model", PT_double, "A_sat",
            PADDR(Asat), PT_DESCRIPTION,
            "flux saturation parameter, A - single-phase model", PT_double,
            "b_sat", PADDR(bsat), PT_DESCRIPTION,
            "flux saturation parameter, b - single-phase model", PT_double,
            "H[s]", PADDR(H), PT_DESCRIPTION, "inertia constant", PT_double,
            "J[kg*m^2]", PADDR(Jm), PT_DESCRIPTION, "moment of inertia",
            PT_double, "number_of_poles", PADDR(pf), PT_DESCRIPTION,
            "number of poles", PT_double, "To_prime[s]", PADDR(To_prime),
            PT_DESCRIPTION, "rotor time constant", PT_double,
            "capacitor_speed[%]", PADDR(cap_run_speed_percentage),
            PT_DESCRIPTION,
            "percentage speed of nominal when starting capacitor kicks in",
            PT_double, "trip_time[s]", PADDR(trip_time), PT_DESCRIPTION,
            "time motor can stay stalled before tripping off ", PT_enumeration,
            "uv_relay_install", PADDR(uv_relay_install), PT_DESCRIPTION,
            "is under-voltage relay protection installed on this motor",
            PT_KEYWORD, "INSTALLED", (enumeration)uv_relay_INSTALLED,
            PT_KEYWORD, "UNINSTALLED", (enumeration)uv_relay_UNINSTALLED,
            PT_double, "uv_relay_trip_time[s]", PADDR(uv_relay_trip_time),
            PT_DESCRIPTION,
            "time low-voltage condition must exist for under-voltage "
            "protection to trip ",
            PT_double, "uv_relay_trip_V[pu]", PADDR(uv_relay_trip_V),
            PT_DESCRIPTION,
            "pu minimum voltage before under-voltage relay trips",
            PT_enumeration, "contactor_state", PADDR(contactor_state),
            PT_DESCRIPTION, "the current status of the motor", PT_KEYWORD,
            "OPEN", (enumeration)contactorOPEN, PT_KEYWORD, "CLOSED",
            (enumeration)contactorCLOSED, PT_double, "contactor_open_Vmin[pu]",
            PADDR(contactor_open_Vmin), PT_DESCRIPTION,
            "pu voltage at which motor contactor opens", PT_double,
            "contactor_close_Vmax[pu]", PADDR(contactor_close_Vmax),
            PT_DESCRIPTION, "pu voltage at which motor contactor recloses",
            PT_double, "reconnect_time[s]", PADDR(reconnect_time),
            PT_DESCRIPTION, "time before tripped motor reconnects", PT_int32,
            "time_step_divider", PADDR(ndiv), PT_DESCRIPTION,
            "divide time step by n - single- and three- phase models",

            // Reconcile torque and speed, primarily
            PT_double, "mechanical_torque[pu]", PADDR(Tmech), PT_DESCRIPTION,
            "mechanical torque applied to the motor", PT_double,
            "mechanical_torque_state_var[pu]", PADDR(Tmech_eff), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Internal state variable torque - three-phase model",

            PT_enumeration, "torque_usage_method",
            PADDR(motor_torque_usage_method), PT_DESCRIPTION,
            "Approach for using Tmech on both types", PT_KEYWORD, "DIRECT",
            (enumeration)modelDirectTorque, PT_KEYWORD, "SPEEDFOUR",
            (enumeration)modelSpeedFour, // Fixed at 0.85 + 0.15*speed^4

            PT_int32, "iteration_count", PADDR(iteration_count), PT_DESCRIPTION,
            "maximum number of iterations for steady state model", PT_double,
            "delta_mode_voltage_trigger[%]", PADDR(DM_volt_trig_per),
            PT_DESCRIPTION,
            "percentage voltage of nominal when delta mode is triggered",
            PT_double, "delta_mode_rotor_speed_trigger[%]",
            PADDR(DM_speed_trig_per), PT_DESCRIPTION,
            "percentage speed of nominal when delta mode is triggered",
            PT_double, "delta_mode_voltage_exit[%]", PADDR(DM_volt_exit_per),
            PT_DESCRIPTION, "percentage voltage of nominal to exit delta mode",
            PT_double, "delta_mode_rotor_speed_exit[%]",
            PADDR(DM_speed_exit_per), PT_DESCRIPTION,
            "percentage speed of nominal to exit delta mode", PT_double,
            "maximum_speed_error", PADDR(speed_error), PT_DESCRIPTION,
            "maximum speed error for transitioning modes", PT_double,
            "rotor_speed[rad/s]", PADDR(wr), PT_DESCRIPTION, "rotor speed",
            PT_enumeration, "motor_status", PADDR(motor_status), PT_DESCRIPTION,
            "the current status of the motor", PT_KEYWORD, "RUNNING",
            (enumeration)statusRUNNING, PT_KEYWORD, "STALLED",
            (enumeration)statusSTALLED, PT_KEYWORD, "TRIPPED",
            (enumeration)statusTRIPPED, PT_KEYWORD, "OFF",
            (enumeration)statusOFF, PT_int32, "motor_status_number",
            PADDR(motor_status), PT_DESCRIPTION,
            "the current status of the motor as an integer", PT_enumeration,
            "desired_motor_state", PADDR(motor_override), PT_DESCRIPTION,
            "Should the motor be on or off", PT_KEYWORD, "ON",
            (enumeration)overrideON, PT_KEYWORD, "OFF",
            (enumeration)overrideOFF,

            PT_object, "connected_house", PADDR(mtr_house_pointer),
            PT_DESCRIPTION,
            "house object to monitor the XXX property to determine if the "
            "motor is running",

            PT_enumeration, "connected_house_assumed_mode",
            PADDR(connected_house_assumed_mode), PT_DESCRIPTION,
            "Assumed operation mode of connected_house object", PT_KEYWORD,
            "NONE", (enumeration)house_mode_NONE, PT_KEYWORD, "COOLING",
            (enumeration)house_mode_COOLING, PT_KEYWORD, "HEATING",
            (enumeration)house_mode_HEATING,

            PT_enumeration, "motor_operation_type", PADDR(motor_op_mode),
            PT_DESCRIPTION,
            "current operation type of the motor - deltamode related",
            PT_KEYWORD, "SINGLE-PHASE", (enumeration)modeSPIM, PT_KEYWORD,
            "THREE-PHASE", (enumeration)modeTPIM, PT_enumeration,
            "triplex_connection_type", PADDR(triplex_connection_type),
            PT_DESCRIPTION,
            "Describes how the motor will connect to the triplex devices",
            PT_KEYWORD, "TRIPLEX_1N", (enumeration)TPNconnected1N, PT_KEYWORD,
            "TRIPLEX_2N", (enumeration)TPNconnected2N, PT_KEYWORD, "TRIPLEX_12",
            (enumeration)TPNconnected12,

            // These model parameters are published as hidden
            PT_double, "wb[rad/s]", PADDR(wbase), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "base speed", PT_double, "ws[rad/s]", PADDR(ws),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "system speed", PT_complex,
            "psi_b", PADDR(psi_b), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "backward rotating flux", PT_complex, "psi_f", PADDR(psi_f),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "forward rotating flux",
            PT_complex, "psi_dr", PADDR(psi_dr), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Rotor d axis flux", PT_complex, "psi_qr",
            PADDR(psi_qr), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "Rotor q axis flux", PT_complex, "Ids", PADDR(Ids), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "time before tripped motor reconnects",
            PT_complex, "Iqs", PADDR(Iqs), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "time before tripped motor reconnects", PT_complex, "If", PADDR(If),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "forward current", PT_complex,
            "Ib", PADDR(Ib), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "backward current", PT_complex, "Is", PADDR(Is), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "motor current", PT_complex,
            "electrical_power[VA]", PADDR(motor_elec_power), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "motor power", PT_double,
            "electrical_torque[pu]", PADDR(Telec), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "electrical torque", PT_complex, "Vs", PADDR(Vs),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "motor voltage", PT_bool,
            "motor_trip", PADDR(motor_trip), PT_DESCRIPTION,
            "boolean variable to check if motor is tripped", PT_double, "trip",
            PADDR(trip), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "current time in tripped state", PT_double, "reconnect",
            PADDR(reconnect), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "current time since motor was tripped",

            // These model parameters are published as hidden
            PT_complex, "phips", PADDR(phips), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "positive sequence stator flux", PT_complex,
            "phins_cj", PADDR(phins_cj), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "conjugate of negative sequence stator flux", PT_complex, "phipr",
            PADDR(phipr), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "positive sequence rotor flux", PT_complex, "phinr_cj",
            PADDR(phinr_cj), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "conjugate of negative sequence rotor flux", PT_double,
            "per_unit_rotor_speed[pu]", PADDR(wr_pu), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "rotor speed in per-unit", PT_complex, "Ias[pu]",
            PADDR(Ias), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-a stator current", PT_complex, "Ibs[pu]", PADDR(Ibs),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-b stator current", PT_complex, "Ics[pu]", PADDR(Ics),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-c stator current", PT_complex, "Vas[pu]", PADDR(Vas),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-a stator-to-ground voltage", PT_complex, "Vbs[pu]",
            PADDR(Vbs), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-b stator-to-ground voltage", PT_complex, "Vcs[pu]",
            PADDR(Vcs), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "motor phase-c stator-to-ground voltage", PT_complex, "Ips",
            PADDR(Ips), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "positive sequence stator current", PT_complex, "Ipr", PADDR(Ipr),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "positive sequence rotor current", PT_complex, "Ins_cj",
            PADDR(Ins_cj), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "conjugate of negative sequence stator current", PT_complex,
            "Inr_cj", PADDR(Inr_cj), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "conjugate of negative sequence rotor current", PT_double, "Ls",
            PADDR(Ls), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "stator synchronous reactance", PT_double, "Lr", PADDR(Lr),
            PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "rotor synchronous reactance",
            PT_double, "sigma1", PADDR(sigma1), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "intermediate variable 1 associated with synch. react.", PT_double,
            "sigma2", PADDR(sigma2), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_motor) == nullptr)
...
int motor::create() {
  ...
  uv_relay_time =
      0; // counter for how long we've been in under-voltage condition
  ...
  triplex_connection_type =
      TPNconnected12; // If it does end up being triplex, we default it to L1-L2
...
int motor::init(OBJECT *parent) {
  ...
  // See if we have a house connection defined -- if so, do this after that
  // initializes (to get data)
  if (mtr_house_pointer != nullptr) {
    // Check it's status
    if ((mtr_house_pointer->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("motor:%d - %s ::init(): deferring initialization on %s",
                 obj->id, (obj->name ? obj->name : "Unnamed"),
                 gl_name(mtr_house_pointer, objname, 255));
      return 2; // defer
    }
  }
  ...
  if (num_phases == 1) {
    ...
    GL_THROW(
        "motor:%s -- only single-phase or three-phase motors are supported",
        (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The motor only supports single-phase and three-phase motors at this time.
    Please use one of these connection types.
    */
  }
  ...
  if (mtr_house_pointer != nullptr) {
    ...
    if (!gl_object_isa(mtr_house_pointer, "house", "residential")) {
      GL_THROW("motor:%s -- connected_house must point toward a "
               "residential:house object",
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      The motor connected_house field only supports connections to a
      residential:house object at this time.
      */
    }

    // Make sure our mode is SPIM and triplexy - if not, failure
    if (!triplex_connected) {
      GL_THROW("motor:%s -- When using the house-connected mode, the motor "
               "must be a triplex device",
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      In the house-tied mode, the motor must be a triplex-connected motor.
      Ideally, it should be connected to the same triplex device as the house.
      Three-phase implementations may be supported at a future date.
      */
    }

    // See if a mode is assumed - if not, throw an error, "just because"
    if (connected_house_assumed_mode == house_mode_NONE) {
      GL_THROW("motor:%s -- When using the house-connected mode, an expected "
               "type of operation must be specified",
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      If using the motor in a "house_connected" mode, an assumption on if it is
      running as a heating or cooling motor must be assumed.  This is used to
      extract the motor size from the house.  If left to "NONE", this error will
      persist.
      */
    }

    // Implies it is set, determine which one we are and pull the appropriate
    // properties
    if (connected_house_assumed_mode == house_mode_COOLING) {
      // Make sure it is a compatible type!
      temp_gld_property =
          new gld_property(mtr_house_pointer, "cooling_system_type");

      // Make sure it worked
      if (!temp_gld_property->is_valid() ||
          !temp_gld_property->is_enumeration()) {
        GL_THROW("motor:%s -- Unable to map house property",
                 (obj->name ? obj->name : "Unnamed"));
        // Defined below
      }
      ...
      if ((temp_house_type != 2) &&
          (temp_house_type != 3)) // Not a normal AC or heat pump
      {
        GL_THROW("motor:%s - Mapped house does not support the type of "
                 "heating/cooling mode expected!",
                 (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While attempting to map the motor to a house object, the house does not
        have the appropriate heating or cooling system to connect the motor.  If
        assuming COOLING, the house cooling_system_type must be ELECTRIC or
        HEAT_PUMP.  If assuming HEATING, the house heating_system_type must be
        HEAT_PUMP.  Select the appropriate mode and try again.
        */
      }

      // Map to design cooling capacity
      temp_gld_property =
          new gld_property(mtr_house_pointer, "design_cooling_capacity");
      ...
      // Make sure it is a compatible type!
      temp_gld_property =
          new gld_property(mtr_house_pointer, "heating_system_type");
      ...
      temp_gld_property =
          new gld_property(mtr_house_pointer, "design_heating_capacity");
    ...
    // Perform the conversion to get a power amount for this motor (based on
    // house)
    Pbase = temp_house_capacity_info / temp_house_cop / 3.4120;

    // Set the flag on the house
    temp_gld_property =
        new gld_property(mtr_house_pointer, "external_motor_attached");

    // Make sure it worked
    if (!temp_gld_property->is_valid() || !temp_gld_property->is_bool()) {
      GL_THROW("motor:%s -- Unable to map house external motor property",
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attempting to map the external_motor_attached property of the house
      to set motor state, an error occurred.  Please try again. If the error
      persists, please submit your code and a bug report via the issues tracker.
      */
    }
    ...
    // Map up the property
    mtr_house_state_pointer =
        new gld_property(mtr_house_pointer, "compressor_on");

    // Make sure it worked
    if (!mtr_house_state_pointer->is_valid() ||
        !mtr_house_state_pointer->is_bool()) {
      GL_THROW("motor:%s -- Unable to map house property",
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attempting to map a property of the house needed to operate the
      motor externally, an error occurred.  Please try again. If the error
      persists, please submit your code and a bug report via the issues tracker.
      */
    }

    // Check the initial state - pull the value (not sure it is actually set
    // yet)
  ...
  {
    Jm = H / (1 / 2 * pow(2 / pf, 2) * pow(wbase, 2) /
              Pbase); // no need to calculate Jm, only H is needed
  } else if ((Jm != -999) &&
             (H != -999)) // both Jm and H specified; priority to H
  {
    gl_warning(
        "motor:%s -- both H and J were specified, H will be used, J is ignored",
        (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
  ...
  if (contactor_open_Vmin > contactor_close_Vmax) {
    GL_THROW("motor:%s -- contactor_open_Vmin must be less or equal to than "
  ...
  if (uv_relay_trip_time < 0) {
    GL_THROW(
        "motor:%s -- uv_relay_trip_time must be greater than or equal to 0",
        (obj->name ? obj->name : "Unnamed"));
  }
  if (uv_relay_trip_V < 0 || uv_relay_trip_V > 1) {
    GL_THROW("motor:%s -- uv_relay_trip_V must be greater than or equal to 0 "
...
TIMESTAMP motor::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (motor_op_mode == modeSPIM) {
    ...
    if ((double)t1 == last_cycle) { // if time did not advance, load old values
      SPIMreinitializeVars();
               ...
               last_cycle) { // time advanced, time to update varibles
      SPIMupdateVars();
    } else {
    ...
    if (motor_override == overrideON && ws > 1 && Vs.Mag() > 0.1 &&
        motor_trip == 0) { // motor is currently connected and grid conditions
                           // are not "collapsed"
  ...
  } else // Must be three-phase
  {
    ...
    if ((double)t1 == last_cycle) { // if time did not advance, load old values
      TPIMreinitializeVars();
               ...
               last_cycle) { // time advanced, time to update variables
      TPIMupdateVars();
    } else {
      gl_error("current time is less than previous time");
    }
    ...
    if (motor_override == overrideON && ws_pu > 0.1 && Vas.Mag() > 0.1 &&
        Vbs.Mag() > 0.1 && Vcs.Mag() > 0.1 &&
        motor_trip == 0) { // motor is currently connected and grid conditions
                           // are not "collapsed"
      // This needs to be re-visited to determine the lower bounds of ws_pu and
      // Vas, Vbs and Vcs
  ...
  if (motor_op_mode == modeSPIM) {
    if (((Vs.Mag() < DM_volt_trig) || (wr < DM_speed_trig)) &&
        deltamode_inclusive) {
      // we should not enter delta mode if the motor is tripped or not close to
      // reconnect
      ...
      // we are not tripped and the motor needs to enter delta mode to capture
      // the dynamics
...
// Module-level call
SIMULATIONMODE motor::inter_deltaupdate(unsigned int64 delta_time,
                                        unsigned long dt,
  ...
  if ((delta_time == 0) && (iteration_count_val == 0) &&
      !interupdate_pos) // First run of new delta call
  {
  ...
  if (!interupdate_pos) // Before powerflow call
  {
    ...
    if (motor_op_mode == modeSPIM) {
      ...
      if (deltat_ndiv > 0.0003) {
        gl_warning("Motor: Delta time for the SPIM model needs to be lower "
                   "than 0.0003 seconds");
        /*  TROUBLESHOOT
        The deltamode timestep, or for the motor the deltamodetimestep divided
        by time_step_divider, is too large and can lead to integration/model
        issues.
        */
      }

      if (curr_delta_time ==
          last_cycle) { // if time did not advance, load old values
        SPIMreinitializeVars();
      } else if (curr_delta_time >
                 last_cycle) { // time advanced, time to update varibles
        SPIMupdateVars();
      } else {
        gl_error("current time is less than previous time");
    ...
    } else // Must be three-phase
    {
      ...
      if (deltat_ndiv > 0.0005) {
        gl_warning("Motor: Delta time for the TPIM model needs to be lower "
                   "than 0.0005 seconds");
        // Defined above
      }
  ...
  } else // After powerflow call
  {
    ...
    {
      // figure out if we need to exit delta mode on the next pass
      if ((Vas.Mag() > DM_volt_exit) && (Vbs.Mag() > DM_volt_exit) &&
          (Vcs.Mag() > DM_volt_exit) && (wr > DM_speed_exit) &&
...
void motor::updateFreqVolt() {
  ...
  if (motor_op_mode == modeSPIM) {
    if ((SubNode & (SNT_CHILD | SNT_DIFF_CHILD)) !=
        0) // if we have a parent, reference the voltage and frequency of the
           // parent
    {
      ...
      if (triplex_connected) {
        ...
        {
          Vs = parNode->voltaged[0] /
               (2.0 * parNode->nominal_voltage); // To reflect LL connection
          ws = parNode->curr_freq_state.fmeas[0] * 2.0 * PI;
        }
      } else //"Three-phase" connection
      {
    ...
    } else // No parent, use our own voltage
    {
      if (triplex_connected) {
        ...
        {
          Vs =
              voltaged[0] / (2.0 * nominal_voltage); // To reflect LL connection
          ws = curr_freq_state.fmeas[0] * 2.0 * PI;
        }
      } else //"Three-phase" connection
      {
  ...
  } else // TPIM model
  {
           ...
           // parent
    {
      ...
      ws_pu = (parNode->curr_freq_state.fmeas[0] +
               parNode->curr_freq_state.fmeas[1] +
               parNode->curr_freq_state.fmeas[2]) /
    ...
    } else // No parent, use our own voltage
    {
      ...
      ws_pu = (curr_freq_state.fmeas[0] + curr_freq_state.fmeas[1] +
...
void motor::TPIMUpdateMotorStatus() {
  if (motor_override == overrideOFF || ws_pu <= 0.1 || Vas.Mag() <= 0.1 ||
      Vbs.Mag() <= 0.1 || Vcs.Mag() <= 0.1) {
    motor_status = statusOFF;
  } else if (wr_pu > 1e-10) { // Threshold as very small number - accommodates
                              // for slight powerflow differences where this is
...
void motor::SPIMUpdateProtection(double delta_time) {
  if (motor_override == overrideON) {
    if (wr < 1 && motor_trip == 0 && ws > 1 &&
        Vs.Mag() > 0.1) { // conditions for counting up the time trip timer
      trip = trip + delta_time; // count up by the time since last pass
    } else if ((wr >= 1 && motor_trip == 0) || ws <= 1 ||
               Vs.Mag() <=
                   0.1) { // conditions for counting down the time trip timer
      trip = trip - (delta_time / (reconnect_time / trip_time)) < 0
                 ? 0
                 : trip - (delta_time /
                           (reconnect_time /
                            trip_time)); // count down by the time since last
                                         // cycle scaled by the difference in
                                         // trip and reconnect times
    }

    // Under-voltage protection relay timer
    if (uv_lockout == 0) {
      if (Vs.Mag() <= uv_relay_trip_V && Vs.Mag() > 0.01 &&
          uv_relay_install ==
              uv_relay_INSTALLED) { //  This particular AC motor does have an
                                    //  under-voltage relay that will trip.
                                    //  Assumes voltages less than 0.01 pu
                                    //  indicates a disconnected state and
                                    //  shouldn't be considered under-voltage.
        if (uv_relay_time <= uv_relay_trip_time) { // In under-voltage state and
                                                   // accumulating time
          uv_relay_time = uv_relay_time + delta_time;
          uv_lockout = 0;
        } else { // relay time has accumlated and trips the unit open,
                 // permanently
          motor_override = overrideOFF;
          uv_lockout = 1;
        }
      } else { // Either the voltage is high enough or this motor doesn't have
               // under-voltage protection
        uv_relay_time = 0;
      }
    }

    // Main contactor opening due to low-voltage condition (coil doesn't have
    // enough magnetic force to hold contacts together)

    if (Vs.Mag() <= contactor_open_Vmin && uv_lockout == 0 &&
        contactor_state == contactorCLOSED) { //
    ...
    trip = trip - (delta_time / (reconnect_time / trip_time)) < 0
               ...
               : trip - (delta_time /
                         (reconnect_time /
                          trip_time)); // count down by the time since last
                                       // cycle scaled by the difference in trip
                                       // and reconnect times
  }
  ...
  // Main contactor closing (magnetic force able to hold contacts together) once
  // voltage gets high enough
  if (Vs.Mag() >= contactor_close_Vmax && uv_lockout == 0 &&
      contactor_state == contactorOPEN) { //
...
void motor::TPIMUpdateProtection(double delta_time) {
  if (motor_override == overrideON) {
    if (wr_pu < 0.01 && motor_trip == 0 && ws_pu > 0.1 && Vas.Mag() > 0.1 &&
        Vbs.Mag() > 0.1 &&
        Vcs.Mag() > 0.1) { // conditions for counting up the time trip timer
      trip = trip + delta_time; // count up by the time since last pass
    } else if ((wr_pu >= 0.01 && motor_trip == 0) || ws_pu <= 0.1 ||
               Vas.Mag() <= 0.1 || Vbs.Mag() <= 0.1 ||
               Vcs.Mag() <=
                   0.1) { // conditions for counting down the time trip timer
      trip = trip - (delta_time / (reconnect_time / trip_time)) < 0
                 ? 0
    ...
    // Under-voltage protection relay timer
    if (uv_lockout == 0) {
      if (((Vas.Mag() <= uv_relay_trip_V && Vas.Mag() > 0.001) ||
           (Vbs.Mag() <= uv_relay_trip_V && Vbs.Mag() > 0.001) ||
           (Vcs.Mag() <= uv_relay_trip_V) && Vcs.Mag() > 0.001) &&
          uv_relay_install ==
              uv_relay_INSTALLED) { //  This particular AC motor does have an
                                    //  under-voltage relay that will trip
        if (uv_relay_time <= uv_relay_trip_time) { // In under-voltage state and
                                                   // accumulating time
          // std::cout << "Under voltage: " << Vas.Mag() << "\t" << Vbs.Mag() <<
          // "\t" << Vcs.Mag()<< "\n";
          uv_relay_time = uv_relay_time + delta_time;
          uv_lockout = 0;
        } else { // relay time has accumlated and trips the unit open,
          ...
          uv_lockout = 1;
               ...
               // under-voltage protection
        // std::cout << "Voltage OK: " << Vas.Mag() << "\t" << Vbs.Mag() << "\t"
        // << Vcs.Mag()<< "\n";
        uv_relay_time = 0;
      }
    }
    ...
    // Main contactor opening due to low-voltage condition (coil doesn't have
    // enough magnetic force to hold contacts together)
    if ((Vas.Mag() <= contactor_open_Vmin || Vbs.Mag() <= contactor_open_Vmin ||
         Vcs.Mag() <= contactor_open_Vmin) &&
        uv_lockout == 0 && contactor_state == contactorCLOSED) { //
      // std::cout << "Under voltage, contactor open: " << Vas.Mag() << "\t" <<
      // Vbs.Mag() << "\t" << Vcs.Mag()<< "\n";
      motor_override = overrideOFF;
      contactor_state = contactorOPEN;
    }
  ...
  // Main contactor closing (magnetic force able to hold contacts together) once
  // voltage gets high enough
  if ((Vas.Mag() >= contactor_close_Vmax && Vbs.Mag() >= contactor_close_Vmax &&
       Vcs.Mag() >= contactor_close_Vmax) &&
      uv_lockout == 0 && contactor_state == contactorOPEN) { //
    // std::cout << "Voltage OK, contactor closed: " << Vas.Mag() << "\t" <<
    // Vbs.Mag() << "\t" << Vcs.Mag()<< "\n";
    motor_override = overrideON;
    contactor_state = contactorCLOSED;
  }
...
void motor::SPIMSteadyState(TIMESTAMP t1) {
  ...
  while (fabs(wr_delta) > speed_error && count < iteration_count) {
    ...
    TF[4] = 0;
    TF[5] = (gld::complex(0.0, 1.0) * Xc) / ws_pu +
            (gld::complex(0.0, 1.0) * Xq_prime * ws_pu) + Rqs;
    TF[6] = -(Xm * n * ws_pu) / Xr;
    TF[7] = (Xm * n * ws_pu) / Xr;
    TF[8] = Xm / 2;
    TF[9] = -(gld::complex(0.0, 1.0) * Xm * n) / 2;
    TF[10] =
        (gld::complex(0.0, 1.0) * wr - gld::complex(0.0, 1.0) * ws) * To_prime -
        psi_sat;
    TF[11] = 0;
    TF[12] = Xm / 2;
    TF[13] = (gld::complex(0.0, 1.0) * Xm * n) / 2;
    TF[14] = 0;
    TF[15] = (gld::complex(0.0, 1.0) * wr + gld::complex(0.0, 1.0) * ws) *
                 -To_prime -
             psi_sat;
    ...
    // electrical torque
    Telec = (Xm / Xr) * 2.0 *
            (If.Im() * psi_f.Re() - If.Re() * psi_f.Im() -
             Ib.Im() * psi_b.Re() + Ib.Re() * psi_b.Im());
    ...
    // Default else - direct method, so just read (in case player/else changes
    // it)
    ...
    psi = sqrt(pow(psi_f.Re(), 2) + pow(psi_f.Im(), 2) + pow(psi_b.Re(), 2) +
               pow(psi_b.Im(), 2));
    if (psi <= bsat) {
      psi_sat = 1;
    } else {
...
void motor::TPIMSteadyState(TIMESTAMP t1) {
  ...
  if (motor_status == statusRUNNING) {
    ...
    while (fabs(omgr0_delta) > speed_error && count < iteration_count) {
      ...
      // pre-calculate complex coefficients of the 4 linear equations associated
      // with flux state variables
      ...
      phips = (B1 * C2 * D3 * E4 - B1 * C2 * D4 * E3 - B1 * C3 * D2 * E4 +
               B1 * C3 * D4 * E2 + B1 * C4 * D2 * E3 - B1 * C4 * D3 * E2 -
               B2 * C1 * D3 * E4 + B2 * C1 * D4 * E3 + B2 * C3 * D1 * E4 -
               B2 * C3 * D4 * E1 - B2 * C4 * D1 * E3 + B2 * C4 * D3 * E1 +
               B3 * C1 * D2 * E4 - B3 * C1 * D4 * E2 - B3 * C2 * D1 * E4 +
               B3 * C2 * D4 * E1 + B3 * C4 * D1 * E2 - B3 * C4 * D2 * E1 -
               B4 * C1 * D2 * E3 + B4 * C1 * D3 * E2 + B4 * C2 * D1 * E3 -
               B4 * C2 * D3 * E1 - B4 * C3 * D1 * E2 + B4 * C3 * D2 * E1) /
              (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 +
               A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 - A1 * B4 * C3 * D2 -
               A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 -
               A2 * B3 * C4 * D1 - A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 +
               A3 * B1 * C2 * D4 - A3 * B1 * C4 * D2 - A3 * B2 * C1 * D4 +
               A3 * B2 * C4 * D1 + A3 * B4 * C1 * D2 - A3 * B4 * C2 * D1 -
               A4 * B1 * C2 * D3 + A4 * B1 * C3 * D2 + A4 * B2 * C1 * D3 -
               A4 * B2 * C3 * D1 - A4 * B3 * C1 * D2 + A4 * B3 * C2 * D1);

      phins_cj = -(A1 * C2 * D3 * E4 - A1 * C2 * D4 * E3 - A1 * C3 * D2 * E4 +
                   A1 * C3 * D4 * E2 + A1 * C4 * D2 * E3 - A1 * C4 * D3 * E2 -
                   A2 * C1 * D3 * E4 + A2 * C1 * D4 * E3 + A2 * C3 * D1 * E4 -
                   A2 * C3 * D4 * E1 - A2 * C4 * D1 * E3 + A2 * C4 * D3 * E1 +
                   A3 * C1 * D2 * E4 - A3 * C1 * D4 * E2 - A3 * C2 * D1 * E4 +
                   A3 * C2 * D4 * E1 + A3 * C4 * D1 * E2 - A3 * C4 * D2 * E1 -
                   A4 * C1 * D2 * E3 + A4 * C1 * D3 * E2 + A4 * C2 * D1 * E3 -
                   A4 * C2 * D3 * E1 - A4 * C3 * D1 * E2 + A4 * C3 * D2 * E1) /
                 (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 +
                  A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 - A1 * B4 * C3 * D2 -
                  A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 -
                  A2 * B3 * C4 * D1 - A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 +
      ...
      phipr = (A1 * B2 * D3 * E4 - A1 * B2 * D4 * E3 - A1 * B3 * D2 * E4 +
               A1 * B3 * D4 * E2 + A1 * B4 * D2 * E3 - A1 * B4 * D3 * E2 -
               A2 * B1 * D3 * E4 + A2 * B1 * D4 * E3 + A2 * B3 * D1 * E4 -
               A2 * B3 * D4 * E1 - A2 * B4 * D1 * E3 + A2 * B4 * D3 * E1 +
               A3 * B1 * D2 * E4 - A3 * B1 * D4 * E2 - A3 * B2 * D1 * E4 +
               A3 * B2 * D4 * E1 + A3 * B4 * D1 * E2 - A3 * B4 * D2 * E1 -
               A4 * B1 * D2 * E3 + A4 * B1 * D3 * E2 + A4 * B2 * D1 * E3 -
               A4 * B2 * D3 * E1 - A4 * B3 * D1 * E2 + A4 * B3 * D2 * E1) /
              (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 +
               A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 - A1 * B4 * C3 * D2 -
               A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 -
               A2 * B3 * C4 * D1 - A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 +
      ...
      phinr_cj = -(A1 * B2 * C3 * E4 - A1 * B2 * C4 * E3 - A1 * B3 * C2 * E4 +
                   A1 * B3 * C4 * E2 + A1 * B4 * C2 * E3 - A1 * B4 * C3 * E2 -
                   A2 * B1 * C3 * E4 + A2 * B1 * C4 * E3 + A2 * B3 * C1 * E4 -
                   A2 * B3 * C4 * E1 - A2 * B4 * C1 * E3 + A2 * B4 * C3 * E1 +
                   A3 * B1 * C2 * E4 - A3 * B1 * C4 * E2 - A3 * B2 * C1 * E4 +
                   A3 * B2 * C4 * E1 + A3 * B4 * C1 * E2 - A3 * B4 * C2 * E1 -
                   A4 * B1 * C2 * E3 + A4 * B1 * C3 * E2 + A4 * B2 * C1 * E3 -
                   A4 * B2 * C3 * E1 - A4 * B3 * C1 * E2 + A4 * B3 * C2 * E1) /
                 (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 +
                  A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 - A1 * B4 * C3 * D2 -
                  A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 -
                  A2 * B3 * C4 * D1 - A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 +
      ...
      // iteratively compute speed increment to make sure Telec matches Tmech
      // during steady state mode if it does not match, then update current and
      // Telec using new wr_pu
      omgr0_delta = (Telec - Tmech_eff) / ((double)iteration_count);

      // update the rotor speed to make sure electrical torque traces mechanical
      // torque
...
void motor::SPIMDynamic(double curr_delta_time, double dTime) {
  ...
  for (int i = 0; i < ndiv; i++) {
    ...
    // Flux equation
    psi_b =
        (Ib * Xm) / ((gld::complex(0.0, 1.0) * (ws + wr) * To_prime) + psi_sat);

    psi_f = psi_f + (If * (Xm / To_prime) -
                     (gld::complex(0.0, 1.0) * (ws - wr) + psi_sat / To_prime) *
                         psi_f) *
                        dTime_sub;

    // Calculate saturated flux
    psi = sqrt(psi_f.Re() * psi_f.Re() + psi_f.Im() * psi_f.Im() +
               psi_b.Re() * psi_b.Re() + psi_b.Im() * psi_b.Im());
    if (psi <= bsat) {
      psi_sat = 1;
    } else {
    ...
    Ids = (-(gld::complex(0.0, 1.0) * ws_pu * (Xm / Xr) * psi_dr) + Vs.Mag()) /
          ((gld::complex(0.0, 1.0) * ws_pu * Xd_prime) + Rds);
    Iqs = (-(gld::complex(0.0, 1.0) * ws_pu * (n * Xm / Xr) * psi_qr) +
           Vs.Mag()) /
          ((gld::complex(0.0, 1.0) * ws_pu * Xq_prime) +
           (gld::complex(0.0, 1.0) / ws_pu * Xc) + Rqs);
...
void motor::TPIMDynamic(double curr_delta_time, double dTime) {
  ...
  for (int i = 0; i < ndiv; i++) {
    ...
    dphips_prev_dt =
        (Vap + A1p * phips_prev + C1p * phipr_prev) * wbase; // pu/s
    dphins_cj_prev_dt =
        (~Van + B2p * phins_cj_prev + D2p * phinr_cj_prev) * wbase; // pu/s
    dphipr_prev_dt = (C3p * phipr_prev + A3p * phips_prev) * wbase; // pu/s
    dphinr_cj_prev_dt =
        (D4p * phinr_cj_prev + B4p * phins_cj_prev) * wbase; // pu/s
    domgr0_prev_dt =
        ((~phips_prev * Ips_prev + ~phins_cj_prev * Ins_cj_prev).Im() -
         Tmech_eff - Kfric * wr_pu_prev) /
        (2.0 * H); // pu/s
    ...
    //*** Corrector Step ***//
    // assuming no boundary variable (e.g. voltage) changes during each time
    // step, so predictor and corrector steps are placed in the same class
    // function
    ...
    C3c = -(gld::complex(0.0, 1.0) * (ws_pu - wr_pu) +
            rr_pu / sigma2); // This coeff. is different from predictor

    B4c = rr_pu / sigma2 * lm / Ls;
    D4c = -(gld::complex(0.0, 1.0) * (-ws_pu - wr_pu) +
            rr_pu / sigma2); // This coeff. is different from predictor
    ...
    domgr0_dt =
        1.0 / (2.0 * H) *
        ((~phips * Ips + ~phins_cj * Ins_cj).Im() - Tmech_eff - Kfric * wr_pu);

    // corrector step 3 - integrate
    phips = phips_prev + (dphips_prev_dt + dphips_dt) * dTime_sub / 2.0;
    phins_cj =
        phins_cj_prev + (dphins_cj_prev_dt + dphins_cj_dt) * dTime_sub / 2.0;
    phipr = phipr_prev + (dphipr_prev_dt + dphipr_dt) * dTime_sub / 2.0;
    phinr_cj =
        phinr_cj_prev + (dphinr_cj_prev_dt + dphinr_cj_dt) * dTime_sub / 2.0;
...
int motor::invertMatrix(gld::complex TF[16], gld::complex ITF[16]) {
  ...
  inv[0] = TF[5] * TF[10] * TF[15] - TF[5] * TF[11] * TF[14] -
           TF[9] * TF[6] * TF[15] + TF[9] * TF[7] * TF[14] +
           TF[13] * TF[6] * TF[11] - TF[13] * TF[7] * TF[10];
  inv[4] = -TF[4] * TF[10] * TF[15] + TF[4] * TF[11] * TF[14] +
           TF[8] * TF[6] * TF[15] - TF[8] * TF[7] * TF[14] -
           TF[12] * TF[6] * TF[11] + TF[12] * TF[7] * TF[10];
  inv[8] = TF[4] * TF[9] * TF[15] - TF[4] * TF[11] * TF[13] -
           TF[8] * TF[5] * TF[15] + TF[8] * TF[7] * TF[13] +
           TF[12] * TF[5] * TF[11] - TF[12] * TF[7] * TF[9];
  inv[12] = -TF[4] * TF[9] * TF[14] + TF[4] * TF[10] * TF[13] +
            TF[8] * TF[5] * TF[14] - TF[8] * TF[6] * TF[13] -
            TF[12] * TF[5] * TF[10] + TF[12] * TF[6] * TF[9];
  inv[1] = -TF[1] * TF[10] * TF[15] + TF[1] * TF[11] * TF[14] +
           TF[9] * TF[2] * TF[15] - TF[9] * TF[3] * TF[14] -
           TF[13] * TF[2] * TF[11] + TF[13] * TF[3] * TF[10];
  inv[5] = TF[0] * TF[10] * TF[15] - TF[0] * TF[11] * TF[14] -
           TF[8] * TF[2] * TF[15] + TF[8] * TF[3] * TF[14] +
           TF[12] * TF[2] * TF[11] - TF[12] * TF[3] * TF[10];
  inv[9] = -TF[0] * TF[9] * TF[15] + TF[0] * TF[11] * TF[13] +
           TF[8] * TF[1] * TF[15] - TF[8] * TF[3] * TF[13] -
           TF[12] * TF[1] * TF[11] + TF[12] * TF[3] * TF[9];
  inv[13] = TF[0] * TF[9] * TF[14] - TF[0] * TF[10] * TF[13] -
            TF[8] * TF[1] * TF[14] + TF[8] * TF[2] * TF[13] +
            TF[12] * TF[1] * TF[10] - TF[12] * TF[2] * TF[9];
  inv[2] = TF[1] * TF[6] * TF[15] - TF[1] * TF[7] * TF[14] -
           TF[5] * TF[2] * TF[15] + TF[5] * TF[3] * TF[14] +
           TF[13] * TF[2] * TF[7] - TF[13] * TF[3] * TF[6];
  inv[6] = -TF[0] * TF[6] * TF[15] + TF[0] * TF[7] * TF[14] +
           TF[4] * TF[2] * TF[15] - TF[4] * TF[3] * TF[14] -
           TF[12] * TF[2] * TF[7] + TF[12] * TF[3] * TF[6];
  inv[10] = TF[0] * TF[5] * TF[15] - TF[0] * TF[7] * TF[13] -
            TF[4] * TF[1] * TF[15] + TF[4] * TF[3] * TF[13] +
            TF[12] * TF[1] * TF[7] - TF[12] * TF[3] * TF[5];
  inv[14] = -TF[0] * TF[5] * TF[14] + TF[0] * TF[6] * TF[13] +
            TF[4] * TF[1] * TF[14] - TF[4] * TF[2] * TF[13] -
            TF[12] * TF[1] * TF[6] + TF[12] * TF[2] * TF[5];
  inv[3] = -TF[1] * TF[6] * TF[11] + TF[1] * TF[7] * TF[10] +
           TF[5] * TF[2] * TF[11] - TF[5] * TF[3] * TF[10] -
           TF[9] * TF[2] * TF[7] + TF[9] * TF[3] * TF[6];
  inv[7] = TF[0] * TF[6] * TF[11] - TF[0] * TF[7] * TF[10] -
           TF[4] * TF[2] * TF[11] + TF[4] * TF[3] * TF[10] +
           TF[8] * TF[2] * TF[7] - TF[8] * TF[3] * TF[6];
  inv[11] = -TF[0] * TF[5] * TF[11] + TF[0] * TF[7] * TF[9] +
            TF[4] * TF[1] * TF[11] - TF[4] * TF[3] * TF[9] -
            TF[8] * TF[1] * TF[7] + TF[8] * TF[3] * TF[5];
  inv[15] = TF[0] * TF[5] * TF[10] - TF[0] * TF[6] * TF[9] -
            TF[4] * TF[1] * TF[10] + TF[4] * TF[2] * TF[9] +
            TF[8] * TF[1] * TF[6] - TF[8] * TF[2] * TF[5];
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_motor(OBJECT *obj, TIMESTAMP t0,
                                           PASSCONFIG pass) {
 ...
 * DELTA MODE
 */
EXPORT SIMULATIONMODE interupdate_motor(OBJECT *obj, unsigned int64 delta_time,
                                        unsigned long dt,
                                        unsigned int iteration_count_val,
                                        bool interupdate_pos) {
  motor *my = /*OBJECTDATA(obj,<>)*/ object_data<motor>(obj);
  ...
  try {
    ...
    gl_error("interupdate_motor(obj=%d;%s): %s", obj->id,
