...
TIMESTAMP motor::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (motor_op_mode == modeSPIM)
	{
		...
		{ // if time did not advance, load old values
			SPIMreinitializeVars();
		}
		else if ((double)t1 > last_cycle)
		{ // time advanced, time to update varibles
			SPIMupdateVars();
		}
		else
		{
		...
		if (motor_override == overrideON && ws > 1 && Vs.Mag() > 0.1 && motor_trip == 0)
		{ // motor is currently connected and grid conditions are not "collapsed"
		...
		{ // motor is currently disconnected
			if (triplex_connected)
			{
				// See which type of triplex
				if (triplex_connection_type == TPNconnected1N)
	...
	else // Must be three-phase
	{
		...
		{ // time advanced, time to update variables
			TPIMupdateVars();
		}
		else
		{
...
SIMULATIONMODE motor::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val, bool interupdate_pos)
{
	...
	if (!interupdate_pos) // Before powerflow call
	{
		...
		if (motor_op_mode == modeSPIM)
		{
			...
			else if (curr_delta_time > last_cycle)
			{ // time advanced, time to update varibles
				SPIMupdateVars();
			}
			else
	...
	else // After powerflow call
	{
		...
		{
			// figure out if we need to exit delta mode on the next pass
			if ((Vas.Mag() > DM_volt_exit) && (Vbs.Mag() > DM_volt_exit) && (Vcs.Mag() > DM_volt_exit) && (wr > DM_speed_exit) && ((fabs(wr_pu - wr_pu_prev) * wbase) <= speed_error))
			{
				return SM_EVENT;
			}
			else if (motor_trip == 1 && reconnect < reconnect_time - 1)
...
void motor::TPIMUpdateMotorStatus()
{
	...
	{ // Threshold as very small number - accommodates for slight powerflow differences where this is technically non-zero
		motor_status = statusRUNNING;
	}
	else if (motor_trip == 1)
	{
...
void motor::SPIMUpdateProtection(double delta_time)
{
	...
	{
		if (wr < 1 && motor_trip == 0 && ws > 1 && Vs.Mag() > 0.1)
		{							  // conditions for counting up the time trip timer
			trip = trip + delta_time; // count up by the time since last pass
		}
		else if ((wr >= 1 && motor_trip == 0) || ws <= 1 || Vs.Mag() <= 0.1)
		{																															// conditions for counting down the time trip timer
		...
		if (uv_lockout == 0)
		{
			if (Vs.Mag() <= uv_relay_trip_V && Vs.Mag() > 0.01 && uv_relay_install == uv_relay_INSTALLED)
			{ //  This particular AC motor does have an under-voltage relay that will trip. Assumes voltages less than 0.01 pu indicates a disconnected state and shouldn't be considered under-voltage.
				if (uv_relay_time <= uv_relay_trip_time)
				{ // In under-voltage state and accumulating time
					uv_relay_time = uv_relay_time + delta_time;
					uv_lockout = 0;
				}
				else
				{ // relay time has accumlated and trips the unit open, permanently
					motor_override = overrideOFF;
					uv_lockout = 1;
				}
			}
			else
			{ // Either the voltage is high enough or this motor doesn't have under-voltage protection
				uv_relay_time = 0;
			}
		}
	...
	{																															// motor is off so it is "cooling" down
		trip = trip - (delta_time / (reconnect_time / trip_time)) < 0 ? 0 : trip - (delta_time / (reconnect_time / trip_time)); // count down by the time since last cycle scaled by the difference in trip and reconnect times
	}
	...
	if (trip >= trip_time)
	{ // check if we should trip the motor
		motor_trip = 1;
		trip = 0;
	}
	if (reconnect >= reconnect_time)
	{ // check if we should reconnect the motor
		motor_trip = 0;
		reconnect = 0;
	}
...
void motor::TPIMUpdateProtection(double delta_time)
{
	if (motor_override == overrideON)
	{
		if (wr_pu < 0.01 && motor_trip == 0 && ws_pu > 0.1 && Vas.Mag() > 0.1 && Vbs.Mag() > 0.1 && Vcs.Mag() > 0.1)
		{							  // conditions for counting up the time trip timer
			trip = trip + delta_time; // count up by the time since last pass
		}
		else if ((wr_pu >= 0.01 && motor_trip == 0) || ws_pu <= 0.1 || Vas.Mag() <= 0.1 || Vbs.Mag() <= 0.1 || Vcs.Mag() <= 0.1)
		{																															// conditions for counting down the time trip timer
			trip = trip - (delta_time / (reconnect_time / trip_time)) < 0 ? 0 : trip - (delta_time / (reconnect_time / trip_time)); // count down by the time since last cycle scaled by the difference in trip and reconnect times
		}
		...
		if (uv_lockout == 0)
		{
			if (((Vas.Mag() <= uv_relay_trip_V && Vas.Mag() > 0.001) || (Vbs.Mag() <= uv_relay_trip_V && Vbs.Mag() > 0.001) || (Vcs.Mag() <= uv_relay_trip_V) && Vcs.Mag() > 0.001) && uv_relay_install == uv_relay_INSTALLED)
			{ //  This particular AC motor does have an under-voltage relay that will trip
				if (uv_relay_time <= uv_relay_trip_time)
				{ // In under-voltage state and accumulating time
...
void motor::TPIMSteadyState(TIMESTAMP t1)
{
	...
	if (motor_status == statusRUNNING)
	{
		...
		while (fabs(omgr0_delta) > speed_error && count < iteration_count)
		{
			...
			phips = (B1 * C2 * D3 * E4 - B1 * C2 * D4 * E3 - B1 * C3 * D2 * E4 + B1 * C3 * D4 * E2 + B1 * C4 * D2 * E3 - B1 * C4 * D3 * E2 - B2 * C1 * D3 * E4 + B2 * C1 * D4 * E3 + B2 * C3 * D1 * E4 - B2 * C3 * D4 * E1 - B2 * C4 * D1 * E3 + B2 * C4 * D3 * E1 + B3 * C1 * D2 * E4 - B3 * C1 * D4 * E2 - B3 * C2 * D1 * E4 + B3 * C2 * D4 * E1 + B3 * C4 * D1 * E2 - B3 * C4 * D2 * E1 - B4 * C1 * D2 * E3 + B4 * C1 * D3 * E2 + B4 * C2 * D1 * E3 - B4 * C2 * D3 * E1 - B4 * C3 * D1 * E2 + B4 * C3 * D2 * E1) /
			...
			phins_cj = -(A1 * C2 * D3 * E4 - A1 * C2 * D4 * E3 - A1 * C3 * D2 * E4 + A1 * C3 * D4 * E2 + A1 * C4 * D2 * E3 - A1 * C4 * D3 * E2 - A2 * C1 * D3 * E4 + A2 * C1 * D4 * E3 + A2 * C3 * D1 * E4 - A2 * C3 * D4 * E1 - A2 * C4 * D1 * E3 + A2 * C4 * D3 * E1 + A3 * C1 * D2 * E4 - A3 * C1 * D4 * E2 - A3 * C2 * D1 * E4 + A3 * C2 * D4 * E1 + A3 * C4 * D1 * E2 - A3 * C4 * D2 * E1 - A4 * C1 * D2 * E3 + A4 * C1 * D3 * E2 + A4 * C2 * D1 * E3 - A4 * C2 * D3 * E1 - A4 * C3 * D1 * E2 + A4 * C3 * D2 * E1) /
					   (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 + A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 -
						A1 * B4 * C3 * D2 - A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 - A2 * B3 * C4 * D1 -
						A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 + A3 * B1 * C2 * D4 - A3 * B1 * C4 * D2 - A3 * B2 * C1 * D4 +
						A3 * B2 * C4 * D1 + A3 * B4 * C1 * D2 - A3 * B4 * C2 * D1 - A4 * B1 * C2 * D3 + A4 * B1 * C3 * D2 +
			...
			phipr = (A1 * B2 * D3 * E4 - A1 * B2 * D4 * E3 - A1 * B3 * D2 * E4 + A1 * B3 * D4 * E2 + A1 * B4 * D2 * E3 - A1 * B4 * D3 * E2 - A2 * B1 * D3 * E4 + A2 * B1 * D4 * E3 + A2 * B3 * D1 * E4 - A2 * B3 * D4 * E1 - A2 * B4 * D1 * E3 + A2 * B4 * D3 * E1 + A3 * B1 * D2 * E4 - A3 * B1 * D4 * E2 - A3 * B2 * D1 * E4 + A3 * B2 * D4 * E1 + A3 * B4 * D1 * E2 - A3 * B4 * D2 * E1 - A4 * B1 * D2 * E3 + A4 * B1 * D3 * E2 + A4 * B2 * D1 * E3 - A4 * B2 * D3 * E1 - A4 * B3 * D1 * E2 + A4 * B3 * D2 * E1) /
					(A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 + A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 -
					 A1 * B4 * C3 * D2 - A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 - A2 * B3 * C4 * D1 -
					 A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 + A3 * B1 * C2 * D4 - A3 * B1 * C4 * D2 - A3 * B2 * C1 * D4 +
					 A3 * B2 * C4 * D1 + A3 * B4 * C1 * D2 - A3 * B4 * C2 * D1 - A4 * B1 * C2 * D3 + A4 * B1 * C3 * D2 +
			...
			phinr_cj = -(A1 * B2 * C3 * E4 - A1 * B2 * C4 * E3 - A1 * B3 * C2 * E4 + A1 * B3 * C4 * E2 + A1 * B4 * C2 * E3 - A1 * B4 * C3 * E2 - A2 * B1 * C3 * E4 + A2 * B1 * C4 * E3 + A2 * B3 * C1 * E4 - A2 * B3 * C4 * E1 - A2 * B4 * C1 * E3 + A2 * B4 * C3 * E1 + A3 * B1 * C2 * E4 - A3 * B1 * C4 * E2 - A3 * B2 * C1 * E4 + A3 * B2 * C4 * E1 + A3 * B4 * C1 * E2 - A3 * B4 * C2 * E1 - A4 * B1 * C2 * E3 + A4 * B1 * C3 * E2 + A4 * B2 * C1 * E3 - A4 * B2 * C3 * E1 - A4 * B3 * C1 * E2 + A4 * B3 * C2 * E1) /
					   (A1 * B2 * C3 * D4 - A1 * B2 * C4 * D3 - A1 * B3 * C2 * D4 + A1 * B3 * C4 * D2 + A1 * B4 * C2 * D3 -
						A1 * B4 * C3 * D2 - A2 * B1 * C3 * D4 + A2 * B1 * C4 * D3 + A2 * B3 * C1 * D4 - A2 * B3 * C4 * D1 -
						A2 * B4 * C1 * D3 + A2 * B4 * C3 * D1 + A3 * B1 * C2 * D4 - A3 * B1 * C4 * D2 - A3 * B2 * C1 * D4 +
						A3 * B2 * C4 * D1 + A3 * B4 * C1 * D2 - A3 * B4 * C2 * D1 - A4 * B1 * C2 * D3 + A4 * B1 * C3 * D2 +
...
void motor::TPIMDynamic(double curr_delta_time, double dTime)
{
	...
	for (int i = 0; i < ndiv; i++)
	{
		...
		if (wr_pu < -10.0)
		{ // speeds below -10 should be avoided
			wr_pu = -10.0;
			wr = wr_pu * wbase;
		}
