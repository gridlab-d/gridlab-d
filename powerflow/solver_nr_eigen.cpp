...
int64 solver_nr_eigen(unsigned int bus_count, BUSDATA *bus, unsigned int branch_count, BRANCHDATA *branch,
    ...
    *bad_computations = false;
    ...
    if (powerflow_type != PF_NORMAL) {
        if (powerflow_type ==
            PF_DYNCALC)    //Parse the list -- anything that is a swing and a generator, deflag it out of principle (and to make it work right)
        {
    ...
    if (matrix_solver_method == MM_EXTERN) {
        for (island_loop_index = 0; island_loop_index < NR_islands_detected; island_loop_index++) {
            ...
            powerflow_values->island_matrix_values[island_loop_index].LU_solver_vars = ((void *(*)(
                    void *)) (LUSolverFcns.ext_init))(
    ...
    if (NR_admit_change)    //If an admittance update was detected, fix it
    {
        ...
        if (powerflow_values->BA_diag == NULL) {
            powerflow_values->BA_diag = (Bus_admit *) gl_malloc(bus_count *
                                                                sizeof(Bus_admit));   //BA_diag store the location and value of diagonal elements of Bus Admittance matrix
        ...
        for (indexer = 0; indexer < bus_count; indexer++) // Construct the diagonal elements of Bus admittance matrix.
        {
            ...
            if ((powerflow_type != PF_NORMAL) && (bus[indexer].full_Y != NULL)) {
                ...
                for (jindex = 0; jindex < 3; jindex++) {
                    for (kindex = 0; kindex < 3; kindex++) {
                        tempY[jindex][kindex] = bus[indexer].full_Y[jindex * 3 +
                                                                    kindex];    //Adds in any self-admittance terms (generators)
                    }
                }
            }
            ...
            for (kindexer = 0; kindexer < (bus[indexer].Link_Table_Size); kindexer++) {
                ...
                if ((branch[jindexer].from == indexer) || (branch[jindexer].to ==
                                                           indexer))    //Bus is the from or to side of things - not sure how it would be in link table otherwise, but meh
                {
                               ...
                               0x80)    //Split phase - add in 2x2 element to upper left 2x2
                    {
                        ...
                        } else                                    //To branch
                        {
                            ...
                            //End of SPCT transformer requires slightly different Diagonal components (so when it's the To bus of SPCT and from for other triplex
                            if (((bus[indexer].phases & 0x20) == 0x20) && (branch[jindexer].lnk_type ==
            ...
            for (jindex = 0; jindex < powerflow_values->BA_diag[indexer].size; jindex++) {
                for (kindex = 0; kindex <
                                 powerflow_values->BA_diag[indexer].size; kindex++)            //Store values - assume square matrix - don't bother parsing what doesn't exist.
                {
            ...
            if (bus[indexer].full_Y_all != NULL) {
                for (jindex = 0; jindex < powerflow_values->BA_diag[indexer].size; jindex++) {
                    for (kindex = 0; kindex <
                                     powerflow_values->BA_diag[indexer].size; kindex++)            //Store values - assume square matrix - don't bother parsing what doesn't exist.
                    {
                        bus[indexer].full_Y_all[jindex * 3 +
                                                kindex] = tempY[jindex][kindex];// Store the self admittance terms.
                    }
                }
        ...
        for (island_loop_index = 0; island_loop_index < NR_islands_detected; island_loop_index++) {
            ...
            if (powerflow_values->island_matrix_values[island_loop_index].total_variables >
                powerflow_values->island_matrix_values[island_loop_index].max_total_variables)
        ...
        for (jindexer = 0; jindexer < branch_count; jindexer++)    //Parse all of the branches
        {
            ...
            if ((bus[tempa].Matrix_Loc == -1) || (bus[tempb].Matrix_Loc == -1)) {
                GL_THROW(const_cast<char *>("An element in NR line:%d - %s was not properly localized"),
                         branch[jindexer].obj->id, (branch[jindexer].name ? branch[jindexer].name : "Unnamed"));
                /*  TROUBLESHOOT
            ...
            else                                            //Three phase or some variety
            {
                ...
                else    //SPCT implementation
                {
                    for (jindex = 0; jindex < 3; jindex++)            //rows
                    {
                        ...
                        {
                            for (kindex = 0;
                                 kindex < 3; kindex++)        //Row valid, traverse all columns for SPCT Yfrom
                            {
                                if (((branch[jindexer].Yfrom[jindex * 3 + kindex]).Re() != 0) &&
                                    (bus[tempa].type != 1) && (bus[tempb].type != 1))
                                    powerflow_values->island_matrix_values[island_index_val].size_offdiag_PQ += 1;
        ...
        for (island_loop_index = 0; island_loop_index < NR_islands_detected; island_loop_index++) {
            ...
            if (powerflow_values->island_matrix_values[island_loop_index].Y_offdiag_PQ == NULL) {
                powerflow_values->island_matrix_values[island_loop_index].Y_offdiag_PQ = (Y_NR *) gl_malloc(
                        (powerflow_values->island_matrix_values[island_loop_index].size_offdiag_PQ * 2) *
                        sizeof(Y_NR));   //powerflow_values->island_matrix_values[x].Y_offdiag_PQ store the row,column and value of off_diagonal elements of Bus Admittance matrix in which all the buses are not PV buses.
            ...
            } else if (powerflow_values->island_matrix_values[island_loop_index].size_offdiag_PQ >
                       powerflow_values->island_matrix_values[island_loop_index].max_size_offdiag_PQ)    //Something changed and we are bigger!!
            {
                //Destroy us!
                ...
                powerflow_values->island_matrix_values[island_loop_index].Y_offdiag_PQ = (Y_NR *) gl_malloc(
                        (powerflow_values->island_matrix_values[island_loop_index].size_offdiag_PQ * 2) * sizeof(Y_NR));
        ...
        for (jindexer = 0; jindexer < branch_count; jindexer++)    //Parse through all of the branches
        {
                    ...
                    if ((branch[jindexer].phases & phase_workd) == phase_workd)    //Validity check
                    {
                        for (kindex = 0; kindex < 3; kindex++)    //Loop through columns of admittance matrices
                        {
                            ...
                            if ((branch[jindexer].phases & phase_worke) == phase_worke)    //Valid column too!
                            {
                                ...
                                {
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempa].Matrix_Loc + jindex;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempb].Matrix_Loc + kindex;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = -((branch[jindexer].Yfrom[
                                            jindex * 3 + kindex]).Im());
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempa].Matrix_Loc + jindex + 3;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempb].Matrix_Loc + kindex + 3;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (branch[jindexer].Yfrom[
                                            jindex * 3 + kindex]).Im();
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                }
                                ...
                                {
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempb].Matrix_Loc + jindex;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempa].Matrix_Loc + kindex;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = -((branch[jindexer].Yto[
                                            jindex * 3 + kindex]).Im());
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempb].Matrix_Loc + jindex + 3;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempa].Matrix_Loc + kindex + 3;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (branch[jindexer].Yto[
                                            jindex * 3 + kindex]).Im();
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                }
                     ...
                     ((bus[tempb].phases & 0x80) == 0x80))    //Someone's a triplex
            {
                    ...
                    ((bus[tempb].phases & 0x80) == 0x80))    //Both are triplex, easy case
                {
                    ...
                    {
                        for (kindex = 0;
                             kindex < 2; kindex++)    //Loop through columns of admittance matrices (only 2x2)
                        {
                            ...
                            {
                                GL_THROW(
                                        const_cast<char *>("NR: SPCT to SPCT via triplex connections are unsupported at this time."));
                                /*  TROUBLESHOOT
                            ...
                            else if ((bus[tempa].phases & 0x20) == 0x20)    //From end is a SPCT to
                            {
                                    ...
                                    (bus[tempa].type != 1) && (bus[tempb].type != 1))    //From imags
                                {
                                    ...
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempa].Matrix_Loc + jindex + 2;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempb].Matrix_Loc + kindex + 2;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = -(branch[jindexer].Yfrom[
                                            jindex * 3 + kindex]).Im();
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                }
                                    ...
                                    (bus[tempb].type != 1))    //To imags
                                {
                                    ...
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                            2 * bus[tempb].Matrix_Loc + jindex + 2;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                            2 * bus[tempa].Matrix_Loc + kindex + 2;
                                    powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (branch[jindexer].Yto[
                                            jindex * 3 + kindex]).Im();
                                    powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                                }
                         ...
                         0x80)    //From is the triplex - this implies transformer with or something, we don't support this
                {
                ...
                else    //Only option left is the to must be the triplex - implies SPCT xformer - so only one phase on the three-phase side (we just need to figure out where)
                {
                    ...
                    switch (bus[tempa].phases & 0x07) {
                        case 0x01:    //C
                        {
                            ...
                            {
                                GL_THROW(
                                        const_cast<char *>("NR: A center-tapped transformer has an invalid phase matching"));
                                /*  TROUBLESHOOT
                    ...
                    for (kindex = 0; kindex < 2; kindex++)    //Loop through columns of admittance matrices (only 2x2)
                    {
                        ...
                        {
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                    2 * bus[tempa].Matrix_Loc + temp_index;
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                    2 * bus[tempb].Matrix_Loc + kindex;
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = -((branch[jindexer].Yfrom[
                                    jindex * 3 + kindex]).Im());
                            ...
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                    2 * bus[tempa].Matrix_Loc + temp_index + temp_size;
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                    2 * bus[tempb].Matrix_Loc + kindex + 2;
                            powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (branch[jindexer].Yfrom[
                                    jindex * 3 + kindex]).Im();
            ...
            else                    //Some combination of not-3 phase
            {
                ...
                {
                    GL_THROW(
                            const_cast<char *>("NR: A line's phase was flagged as not full three-phase, but wasn't: (%s) %u %u %u %u"),
                            branch[jindexer].name, branch[jindexer].phases, branch[jindexer].origphases, phase_worka,
                            phase_workb);
                    /*  TROUBLESHOOT
                ...
                if ((temp_index == -1) || (temp_index_b == -1) || (temp_size == -1) || (temp_size_b == -1) ||
                    (temp_size_c == -1)) {
                ...
                if (Full_Mat_A)    //From side is a full ABC and we have AC
                {
                    ...
                    {
                        for (kindex = 0;
                             kindex < temp_size_c; kindex++)    //Loop through columns of admittance matrices
                        {
                            //Indices counted out from Self admittance above.  needs doubling due to complex separation
                            if ((Temp_Ad_A[jindex][kindex].Im() != 0) && (bus[tempa].type != 1) &&
                                (bus[tempb].type != 1))    //From imags
                            {
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                ...
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempa].Matrix_Loc + temp_index + jindex * 2 + temp_size;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                        2 * bus[tempb].Matrix_Loc + temp_index_b + kindex + temp_size_b;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (Temp_Ad_A[jindex][kindex].Im());
                                powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                            }

                            if ((Temp_Ad_B[jindex][kindex].Im() != 0) && (bus[tempa].type != 1) &&
                                (bus[tempb].type != 1))    //To imags
                            {
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                ...
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempb].Matrix_Loc + temp_index_b + jindex + temp_size_b;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                        2 * bus[tempa].Matrix_Loc + temp_index + kindex * 2 + temp_size;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = Temp_Ad_B[jindex][kindex].Im();
                                powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                            }

                            if ((Temp_Ad_A[jindex][kindex].Re() != 0) && (bus[tempa].type != 1) &&
                                (bus[tempb].type != 1))    //From reals
                            {
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempa].Matrix_Loc + temp_index + jindex * 2 + temp_size;
                            ...
                            if ((Temp_Ad_B[jindex][kindex].Re() != 0) && (bus[tempa].type != 1) &&
                                (bus[tempb].type != 1))    //To reals
                            {
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempb].Matrix_Loc + temp_index_b + jindex + temp_size_b;
                ...
                if (Full_Mat_B)    //To side is a full ABC and we have AC
                {
                    for (jindex = 0; jindex < temp_size_c; jindex++)        //Loop through rows of admittance matrices
                    {
                             ...
                             kindex < temp_size_c; kindex++)    //Loop through columns of admittance matrices
                        {
                            ...
                            {
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempa].Matrix_Loc + temp_index + jindex;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                ...
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                        2 * bus[tempb].Matrix_Loc + temp_index_b + kindex * 2 + temp_size_b;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = (Temp_Ad_A[jindex][kindex].Im());
                                powerflow_values->island_matrix_values[island_index_val].indexer += 1;
                            }
                                ...
                                (bus[tempb].type != 1))    //To imags
                            {
                                ...
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                                        2 * bus[tempa].Matrix_Loc + temp_index + kindex;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].Y_value = -(Temp_Ad_B[jindex][kindex].Im());
                                powerflow_values->island_matrix_values[island_index_val].indexer += 1;

                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                        2 * bus[tempb].Matrix_Loc + temp_index_b + jindex * 2 + temp_size_b;
                                powerflow_values->island_matrix_values[island_index_val].Y_offdiag_PQ[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
        ...
        for (jindexer = 0; jindexer < bus_count; jindexer++) {
            if (bus[jindexer].island_number != -1) {
                ...
                for (jindex = 0; jindex < 3; jindex++) {
                    for (kindex = 0; kindex < 3; kindex++) {
                        if ((powerflow_values->BA_diag[jindexer].Y[jindex][kindex]).Re() != 0 &&
                            bus[jindexer].type != 1 && jindex != kindex)
                            powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed += 1;
                        if ((powerflow_values->BA_diag[jindexer].Y[jindex][kindex]).Im() != 0 &&
                            bus[jindexer].type != 1 && jindex != kindex)
                            powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed += 1;
                    }
                }
        ...
        for (island_loop_index = 0; island_loop_index < NR_islands_detected; island_loop_index++) {
            if (powerflow_values->island_matrix_values[island_loop_index].Y_diag_fixed == NULL) {
                powerflow_values->island_matrix_values[island_loop_index].Y_diag_fixed = (Y_NR *) gl_malloc(
                        (powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed * 2) *
                        sizeof(Y_NR));   //powerflow_values->island_matrix_values[island_loop_index].Y_diag_fixed store the row,column and value of the fixed part of the diagonal PQ bus elements of 6n*6n Y_NR matrix.
            ...
            } else if (powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed >
                       powerflow_values->island_matrix_values[island_loop_index].max_size_diag_fixed)        //Something changed and we are bigger!!
            {
                //Destroy us!
                ...
                //Rebuild us, we have the technology
                powerflow_values->island_matrix_values[island_loop_index].Y_diag_fixed = (Y_NR *) gl_malloc(
                        (powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed * 2) * sizeof(Y_NR));
        ...
        for (jindexer = 0; jindexer < bus_count; jindexer++)    //Parse through bus list
        {
            ...
            for (jindex = 0; jindex < powerflow_values->BA_diag[jindexer].size; jindex++) {
                for (kindex = 0; kindex < powerflow_values->BA_diag[jindexer].size; kindex++) {
                    if ((powerflow_values->BA_diag[jindexer].Y[jindex][kindex]).Im() != 0 && bus[jindexer].type != 1 &&
                        jindex != kindex) {
                        powerflow_values->island_matrix_values[island_index_val].Y_diag_fixed[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                2 * powerflow_values->BA_diag[jindexer].row_ind + jindex;
                        powerflow_values->island_matrix_values[island_index_val].Y_diag_fixed[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
                    ...
                    if ((powerflow_values->BA_diag[jindexer].Y[jindex][kindex]).Re() != 0 && bus[jindexer].type != 1 &&
                        jindex != kindex) {
                        powerflow_values->island_matrix_values[island_index_val].Y_diag_fixed[powerflow_values->island_matrix_values[island_index_val].indexer].row_ind =
                                2 * powerflow_values->BA_diag[jindexer].row_ind + jindex;
                        powerflow_values->island_matrix_values[island_index_val].Y_diag_fixed[powerflow_values->island_matrix_values[island_index_val].indexer].col_ind =
    ...
    while (still_iterating_islands == true) {
        ...
        if (powerflow_values->island_matrix_values[island_loop_index].deltaI_NR == NULL) {
            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR = (double *) gl_malloc(
                    (2 * powerflow_values->island_matrix_values[island_loop_index].total_variables) *
        ...
        } else if (powerflow_values->island_matrix_values[island_loop_index].NR_realloc_needed)        //Bigger sized (this was checked above)
        {
            ...
            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR = (double *) gl_malloc(
                    (2 * powerflow_values->island_matrix_values[island_loop_index].total_variables) * sizeof(double));
        ...
        for (indexer = 0; indexer < bus_count; indexer++) //for specific bus k
        {
            ...
            if (bus[indexer].island_number == island_loop_index) {
                ...
                if ((powerflow_type == PF_DYNINIT) &&
                    ...
                    if ((*bus[indexer].dynamics_enabled == true) && (bus[indexer].full_Y != NULL) &&
                        (bus[indexer].DynCurrent != NULL)) {
                        //Form denominator term of Ii, since it won't change
                        temp_complex_1 =
                                (~bus[indexer].V[0]) + (~bus[indexer].V[1]) * avalsq + (~bus[indexer].V[2]) * aval;
                        ...
                        temp_complex_0 = temp_complex_2 * (bus[indexer].full_Y[0] * bus[indexer].V[0] +
                        ...
                        temp_complex_5 = bus[indexer].full_Y[0] * bus[indexer].V[0] +
                                         bus[indexer].full_Y[1] * bus[indexer].V[1] +
                                         bus[indexer].full_Y[2] * bus[indexer].V[2];
                        ...
                        //Row 2 of admittance
                        temp_complex_0 += temp_complex_2 * (bus[indexer].full_Y[3] * bus[indexer].V[0] +
                        ...
                        temp_complex_5 = bus[indexer].full_Y[3] * bus[indexer].V[0] +
                                         bus[indexer].full_Y[4] * bus[indexer].V[1] +
                                         bus[indexer].full_Y[5] * bus[indexer].V[2];
                        temp_complex_4 = ~temp_complex_5;
                        ...
                        //Row 3 of admittance
                        temp_complex_0 += temp_complex_2 * (bus[indexer].full_Y[6] * bus[indexer].V[0] +
                        ...
                        temp_complex_5 = bus[indexer].full_Y[6] * bus[indexer].V[0] +
                                         bus[indexer].full_Y[7] * bus[indexer].V[1] +
                                         bus[indexer].full_Y[8] * bus[indexer].V[2];
                        temp_complex_4 = ~temp_complex_5;
                ...
                for (jindex = 0; jindex <
                                 powerflow_values->BA_diag[indexer].size; jindex++) // rows - for specific phase that exists
                {
                    ...
                    if ((bus[indexer].phases & 0x80) == 0x80)    //Split phase - triplex bus
                    {
                        ...
                        //Column 1
                        tempIcalcReal +=
                                (powerflow_values->BA_diag[indexer].Y[jindex][0]).Re() * (bus[indexer].V[0]).Re() -
                                (powerflow_values->BA_diag[indexer].Y[jindex][0]).Im() *
                                (bus[indexer].V[0]).Im();// equation (7), the diag elements of bus admittance matrix
                        tempIcalcImag +=
                                (powerflow_values->BA_diag[indexer].Y[jindex][0]).Re() * (bus[indexer].V[0]).Im() +
                                (powerflow_values->BA_diag[indexer].Y[jindex][0]).Im() *
                                (bus[indexer].V[0]).Re();// equation (8), the diag elements of bus admittance matrix

                        //Column 2
                        tempIcalcReal +=
                                (powerflow_values->BA_diag[indexer].Y[jindex][1]).Re() * (bus[indexer].V[1]).Re() -
                                (powerflow_values->BA_diag[indexer].Y[jindex][1]).Im() *
                                (bus[indexer].V[1]).Im();// equation (7), the diag elements of bus admittance matrix
                        tempIcalcImag +=
                                (powerflow_values->BA_diag[indexer].Y[jindex][1]).Re() * (bus[indexer].V[1]).Im() +
                                (powerflow_values->BA_diag[indexer].Y[jindex][1]).Im() *
                                (bus[indexer].V[1]).Re();// equation (8), the diag elements of bus admittance matrix
                        ...
                        for (kindexer = 0; kindexer < (bus[indexer].Link_Table_Size); kindexer++) {
                            ...
                            if (branch[jindexer].from == indexer)    //We're the from bus
                            {
                                if ((bus[indexer].phases & 0x20) == 0x20)    //SPCT from bus - needs different signage
                                {
                                    ...
                                    tempIcalcReal += ((branch[jindexer].Yfrom[work_vals_char_0])).Re() *
                                                     ...
                                                     (bus[branch[jindexer].to].V[0]).Im();// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                    tempIcalcImag += ((branch[jindexer].Yfrom[work_vals_char_0])).Re() *
                                                     (bus[branch[jindexer].to].V[0]).Im() +
                                                     ((branch[jindexer].Yfrom[work_vals_char_0])).Im() *
                                                     (bus[branch[jindexer].to].V[0]).Re();// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance

                                    //Column2
                                    tempIcalcReal += ((branch[jindexer].Yfrom[jindex * 3 + 1])).Re() *
                                                     (bus[branch[jindexer].to].V[1]).Re() -
                                                     ((branch[jindexer].Yfrom[jindex * 3 + 1])).Im() *
                                                     (bus[branch[jindexer].to].V[1]).Im();// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                    tempIcalcImag += ((branch[jindexer].Yfrom[jindex * 3 + 1])).Re() *
                                                     (bus[branch[jindexer].to].V[1]).Im() +
                                                     ((branch[jindexer].Yfrom[jindex * 3 + 1])).Im() *
                                                     (bus[branch[jindexer].to].V[1]).Re();// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                            ...
                            else if (branch[jindexer].to == indexer)    //We're the to bus
                            {
                                if (branch[jindexer].v_ratio != 1.0)    //Transformer
                                {
                                    ...
                                    {
                                        GL_THROW(
                                                const_cast<char *>("NR: A split-phase transformer appears to have an invalid phase"));
                                    }
                                    ...
                                    tempIcalcReal += (-(branch[jindexer].Yto[work_vals_char_0])).Re() *
                                                     ...
                                                     (bus[branch[jindexer].from].V[temp_index]).Im();// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                    tempIcalcImag += (-(branch[jindexer].Yto[work_vals_char_0])).Re() *
                                                     (bus[branch[jindexer].from].V[temp_index]).Im() +
                                                     (-(branch[jindexer].Yto[work_vals_char_0])).Im() *
                                                     (bus[branch[jindexer].from].V[temp_index]).Re();// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                ...
                                else                                    //Must be a normal line then
                                {
                                        ...
                                        0x20)    //SPCT from bus - needs different signage
                                    {
                                        ...
                                        tempIcalcReal += ((branch[jindexer].Yto[work_vals_char_0])).Re() *
                                                         ...
                                                         ((branch[jindexer].Yto[work_vals_char_0])).Im() *
                                                         (bus[branch[jindexer].from].V[0]).Im();// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                        tempIcalcImag += ((branch[jindexer].Yto[work_vals_char_0])).Re() *
                                                         (bus[branch[jindexer].from].V[0]).Im() +
                                                         ((branch[jindexer].Yto[work_vals_char_0])).Im() *
                                                         (bus[branch[jindexer].from].V[0]).Re();// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                        ...
                                        tempIcalcReal += ((branch[jindexer].Yto[work_vals_char_0 + 1])).Re() *
                                                         ...
                                                         (bus[branch[jindexer].from].V[1]).Im();// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                        tempIcalcImag += ((branch[jindexer].Yto[work_vals_char_0 + 1])).Re() *
                                                         (bus[branch[jindexer].from].V[1]).Im() +
                                                         ((branch[jindexer].Yto[work_vals_char_0 + 1])).Im() *
                                                         (bus[branch[jindexer].from].V[1]).Re();// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                    }//End SPCT To bus - from diagonal contributions
                                    else        //Normal line connection to normal triplex
                                    {
                                        work_vals_char_0 = jindex * 3;
                        ...
                        if ((bus[indexer].type > 1) && (bus[indexer].swing_functions_enabled ==
                                                        true))    //SWING bus is different (when it really is a SWING bus)
                        {
                            if ((powerflow_type == PF_DYNINIT) &&
                                (bus[indexer].DynCurrent != NULL))    //We're a generator-type bus
                            {
                                ...
                                work_vals_double_0 = sqrt((work_vals_double_3 * work_vals_double_3 +
                                                           work_vals_double_4 * work_vals_double_4));

                                if (work_vals_double_0 >
                                    bus[indexer].max_volt_error)    //Failure check (defaults to voltage convergence for now)
                                {
                            ...
                            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                    2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                    jindex] = 0.0;
                            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                    2 * bus[indexer].Matrix_Loc + jindex] = 0.0;
                        }//End SWING bus cases
                        else    //PQ bus or SWING masquerading as a PQ
                        {
                            //It's I.  Just a direct conversion (P changed above to I as well)
                            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                    2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size + jindex] =
                                    tempPbus - tempIcalcReal;
                            powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                    2 * bus[indexer].Matrix_Loc + jindex] = tempQbus - tempIcalcImag;
                        }//End Normal PQ bus
                    ...
                    else    //Three phase or some variant thereof
                    {
                        ...
                        for (kindex = 0; kindex <
                                         powerflow_values->BA_diag[indexer].size; kindex++)        //cols - Still only for specified phases
                        {
                            ...
                            tempIcalcReal += (powerflow_values->BA_diag[indexer].Y[jindex][kindex]).Re() *
                                             (bus[indexer].V[temp_index]).Re() -
                                             (powerflow_values->BA_diag[indexer].Y[jindex][kindex]).Im() *
                                             (bus[indexer].V[temp_index]).Im();// equation (7), the diag elements of bus admittance matrix
                            tempIcalcImag += (powerflow_values->BA_diag[indexer].Y[jindex][kindex]).Re() *
                                             (bus[indexer].V[temp_index]).Im() +
                                             (powerflow_values->BA_diag[indexer].Y[jindex][kindex]).Im() *
                                             (bus[indexer].V[temp_index]).Re();// equation (8), the diag elements of bus admittance matrix

                            //In-rush load contributions (if any) - only along explicit diagonal
                            if ((bus[indexer].full_Y_load != NULL) && (jindex == kindex)) {
                                tempIcalcReal += (bus[indexer].full_Y_load[temp_index]).Re() *
                                                 (bus[indexer].V[temp_index]).Re() -
                                                 (bus[indexer].full_Y_load[temp_index]).Im() *
                                                 (bus[indexer].V[temp_index]).Im();// equation (7), the diag elements of bus admittance matrix
                                tempIcalcImag += (bus[indexer].full_Y_load[temp_index]).Re() *
                                                 (bus[indexer].V[temp_index]).Im() +
                                                 (bus[indexer].full_Y_load[temp_index]).Im() *
                                                 (bus[indexer].V[temp_index]).Re();// equation (8), the diag elements of bus admittance matrix
                            }
                            ...
                            for (kindexer = 0; kindexer <
                                               (bus[indexer].Link_Table_Size); kindexer++)    //Parse through the branch list
                            {
                                //Apply proper index to jindexer (easier to implement this way)
                                jindexer = bus[indexer].Link_Table[kindexer];
                                ...
                                if (branch[jindexer].from == indexer) {
                                    ...
                                    else    ///Must be a standard line
                                    {
                                        ...
                                        tempIcalcReal += work_vals_double_0 * work_vals_double_2 - work_vals_double_1 *
                                                                                                   work_vals_double_3;// equation (7), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                        tempIcalcImag += work_vals_double_0 * work_vals_double_3 + work_vals_double_1 *
                                                                                                   work_vals_double_2;// equation (8), the off_diag elements of bus admittance matrix are equal to negative value of branch admittance
                                                        ...
                                                        true))    //SWING bus is different (when it really is a SWING bus)
                        {
                            if ((powerflow_type == PF_DYNINIT) && (bus[indexer].DynCurrent != NULL)) {
                                ...
                                if (bus[indexer].full_Y != NULL) {
                                        ...
                                        0)    //Only update SWING on subsequent passes
                                    {
                                        ...
                                        (*bus[indexer].PGenTotal) += temp_complex_2 - temp_complex_3 /
                                                                                      3.0; // both PT and QT = Ssource-Sysource = v conj(I) - v conj(ysource v)
                                    }

                                    //Compute the delta_I, just like below - but don't post it (still zero in calcs)
                                    work_vals_double_0 =
                                            (bus[indexer].V[temp_index_b]).Mag() * (bus[indexer].V[temp_index_b]).Mag();

                                    if (work_vals_double_0 !=
                                        0)    //Only normal one (not square), but a zero is still a zero even after that
                                    {
                                        work_vals_double_1 = (bus[indexer].V[temp_index_b]).Re();
                                        work_vals_double_2 = (bus[indexer].V[temp_index_b]).Im();
                                        work_vals_double_3 =
                                                (tempPbus * work_vals_double_1 + tempQbus * work_vals_double_2) /
                                                (work_vals_double_0) -
                                                tempIcalcReal; // equation(7), Real part of deltaI, left hand side of equation (11)
                                        work_vals_double_4 =
                                                (tempPbus * work_vals_double_2 - tempQbus * work_vals_double_1) /
                                                (work_vals_double_0) -
                                                tempIcalcImag; // Imaginary part of deltaI, left hand side of equation (11)
                            ...
                            if (NR_busdata[indexer].BusHistTerm != NULL)    //See if we're "delta-capable"
                            {
                                powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                        2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                        jindex] = NR_busdata[indexer].BusHistTerm[jindex].Re();
                                powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                        2 * bus[indexer].Matrix_Loc +
                                        jindex] = NR_busdata[indexer].BusHistTerm[jindex].Im();
                            } else {
                                powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                        2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                        jindex] = 0.0;
                        ...
                        else    //PQ bus or SWING masquerading as a PQ
                        {
                                ...
                                0)    //Only normal one (not square), but a zero is still a zero even after that
                            {
                                ...
                                if (NR_busdata[indexer].BusHistTerm != NULL) {
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                            jindex] = (tempPbus * work_vals_double_1 + tempQbus * work_vals_double_2) /
                                                      (work_vals_double_0) +
                                                      NR_busdata[indexer].BusHistTerm[jindex].Re() -
                                                      tempIcalcReal; // equation(7), Real part of deltaI, left hand side of equation (11)
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + jindex] =
                                            (tempPbus * work_vals_double_2 - tempQbus * work_vals_double_1) /
                                            (work_vals_double_0) + NR_busdata[indexer].BusHistTerm[jindex].Im() -
                                            tempIcalcImag; // Imaginary part of deltaI, left hand side of equation (11)
                                } else    //Nope
                                {
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                ...
                                if (NR_busdata[indexer].BusSatTerm != NULL) {
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                            jindex] -= NR_busdata[indexer].BusSatTerm[jindex].Re();
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc +
                                            jindex] -= NR_busdata[indexer].BusSatTerm[jindex].Im();
                                }
                            } else {
                                if (NR_busdata[indexer].BusHistTerm !=
                                    NULL)    //See if extra deltamode term needs to be included
                                {
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                            jindex] = NR_busdata[indexer].BusHistTerm[jindex].Re();
                        ...
                        if (bus[indexer].full_Y != NULL) {
                            if ((bus[indexer].phases & 0x07) == 0x07)    //Make sure is three-phase
                            {
                                ...
                                if ((bus[indexer].type == 0) ||
                                    ((bus[indexer].type > 1) && (bus[indexer].swing_functions_enabled == false))) {
                                    //Add these into the system - added because "generation"
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc +
                                            powerflow_values->BA_diag[indexer].size] += bus[indexer].DynCurrent[0].Re();        //Phase A
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                            1] += bus[indexer].DynCurrent[1].Re();    //Phase B
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc + powerflow_values->BA_diag[indexer].size +
                                            2] += bus[indexer].DynCurrent[2].Re();    //Phase C

                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[2 *
                                                                                                        bus[indexer].Matrix_Loc] += bus[indexer].DynCurrent[0].Im();        //Phase A
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc +
                                            1] += bus[indexer].DynCurrent[1].Im();    //Phase B
                                    powerflow_values->island_matrix_values[island_loop_index].deltaI_NR[
                                            2 * bus[indexer].Matrix_Loc +
                                            2] += bus[indexer].DynCurrent[2].Im();    //Phase C
                                }
                            ...
                            else    //Not three-phase at the moment
                            {
                                ...
                                {
                                    GL_THROW(
                                            const_cast<char *>("NR_solver: bus:%s has tried updating deltamode dynamics, but is not three-phase!"),
                                            bus[indexer].name);
                                    /*  TROUBLESHOOT
        ...
        if (powerflow_values->island_matrix_values[island_loop_index].Y_diag_update == NULL) {
            powerflow_values->island_matrix_values[island_loop_index].Y_diag_update = (Y_NR *) gl_malloc(
                    (4 * powerflow_values->island_matrix_values[island_loop_index].size_diag_update) *
                    sizeof(Y_NR));   //powerflow_values->island_matrix_values[island_loop_index].Y_diag_update store the row,column and value of the dynamic part of the diagonal PQ bus elements of 6n*6n Y_NR matrix.
        ...
        } else if (powerflow_values->island_matrix_values[island_loop_index].size_diag_update >
                   powerflow_values->island_matrix_values[island_loop_index].max_size_diag_update)    //We've exceeded our limits
        {
            ...
            powerflow_values->island_matrix_values[island_loop_index].Y_diag_update = (Y_NR *) gl_malloc(
                    (4 * powerflow_values->island_matrix_values[island_loop_index].size_diag_update) * sizeof(Y_NR));
        ...
        for (jindexer = 0; jindexer < bus_count; jindexer++)    //Parse through bus list
        {
            ...
            if (bus[jindexer].island_number == island_loop_index) {
                if ((bus[jindexer].type > 1) &&
                    (bus[jindexer].swing_functions_enabled == true))    //Swing bus - and we aren't ignoring it
                {
                    for (jindex = 0; jindex < powerflow_values->BA_diag[jindexer].size; jindex++) {
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind =
                                2 * bus[jindexer].Matrix_Loc + jindex;
                        ...
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind =
                                2 * bus[jindexer].Matrix_Loc + jindex;
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].col_ind =
                                powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind +
                                powerflow_values->BA_diag[jindexer].size;
                        ...
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind =
                                2 * bus[jindexer].Matrix_Loc + jindex + powerflow_values->BA_diag[jindexer].size;
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].col_ind =
                                powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind -
                                powerflow_values->BA_diag[jindexer].size;
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].Y_value = (powerflow_values->BA_diag[jindexer].Y[jindex][jindex]).Re();    //Normal admittance portion
                ...
                if ((bus[jindexer].type == 0) || ((bus[jindexer].type > 1) && bus[jindexer].swing_functions_enabled ==
                                                                              false))    //Only do on PQ (or SWING masquerading as PQ)
                {
                    for (jindex = 0; jindex < powerflow_values->BA_diag[jindexer].size; jindex++) {
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind =
                                2 * bus[jindexer].Matrix_Loc + jindex;
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].col_ind = powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].row_ind;
                        powerflow_values->island_matrix_values[island_loop_index].Y_diag_update[indexer].Y_value =
        ...
        if (powerflow_values->island_matrix_values[island_loop_index].size_Amatrix == 0) {
            ...
            if (NR_islands_detected > 1) {
                gl_warning("Empty powerflow connectivity matrix for island %d, your system is empty!",
                           (island_loop_index + 1));
                /*  TROUBLESHOOT
        ...
        if (powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix == NULL) {
            ...
            //Initiliaze it
            sparse_init(powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix,
                        powerflow_values->island_matrix_values[island_loop_index].size_Amatrix,
                        6 * powerflow_values->island_matrix_values[island_loop_index].bus_count);
        } else if (powerflow_values->island_matrix_values[island_loop_index].NR_realloc_needed)    //If one of the above changed, we changed too
        {
            ...
            //Just clear it out
            sparse_reset(powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix,
                         6 * powerflow_values->island_matrix_values[island_loop_index].bus_count);
        }

        //integrate off diagonal components
        for (indexer = 0;
             indexer < powerflow_values->island_matrix_values[island_loop_index].size_offdiag_PQ * 2; indexer++) {
            ...
            sparse_add(powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix, row, col, value, bus,
                       bus_count, powerflow_values, island_loop_index);
        }

        //Integrate fixed portions of diagonal components
        for (indexer = 0;
             indexer < (powerflow_values->island_matrix_values[island_loop_index].size_diag_fixed * 2); indexer++) {
        ...
        for (indexer = 0;
             indexer < (4 * powerflow_values->island_matrix_values[island_loop_index].size_diag_update); indexer++) {
        ...
        if (NRMatDumpMethod != MD_NONE) {
            ...
            //Check our frequency
            if ((NRMatDumpMethod == MD_ALL) || ((NRMatDumpMethod != MD_ALL) &&
                ...
                if (something_has_been_output == true) {
                    ...
                    for (jindexer = 0; jindexer <
                                       powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix->ncols; jindexer++) {
...
//        std::cout<<(char*)matrix_solver_method<<std::endl;
        if (powerflow_values->island_matrix_values[island_loop_index].matrices_LU.a_LU == NULL)    //First run
        {
            ...
            powerflow_values->island_matrix_values[island_loop_index].matrices_LU.a_LU = (double *) gl_malloc(
                    nnz * sizeof(double));
            ...
            powerflow_values->island_matrix_values[island_loop_index].matrices_LU.rows_LU = (int *) gl_malloc(
                    nnz * sizeof(int));
            if (powerflow_values->island_matrix_values[island_loop_index].matrices_LU.rows_LU == NULL)
                GL_THROW(const_cast<char *>("NR: One of the SuperLU solver matrices failed to allocate"));

            powerflow_values->island_matrix_values[island_loop_index].matrices_LU.cols_LU = (int *) gl_malloc(
                    (n + 1) * sizeof(int));
            if (powerflow_values->island_matrix_values[island_loop_index].matrices_LU.cols_LU == NULL)
                GL_THROW(const_cast<char *>("NR: One of the SuperLU solver matrices failed to allocate"));

            /* Create the right-hand side matrix B. */
            powerflow_values->island_matrix_values[island_loop_index].matrices_LU.rhs_LU = (double *) gl_malloc(
                    m * sizeof(double));
            ...
            } else if (matrix_solver_method == MM_EXTERN)    //External routine
            {
                ((void (*)(void *, unsigned int, unsigned int, bool)) (LUSolverFcns.ext_alloc))(
                        powerflow_values->island_matrix_values[island_loop_index].LU_solver_vars, n, n,
                        NR_admit_change);
            } else {
        ...
        } else if (powerflow_values->island_matrix_values[island_loop_index].NR_realloc_needed)    //Something changed, we'll just destroy everything and start over
        {
            ...
            //Update tracking variable
            powerflow_values->island_matrix_values[island_loop_index].prev_m = m;
        } else if (powerflow_values->island_matrix_values[island_loop_index].prev_m !=
                   m)    //Non-reallocing size change occurred
        {
            if (matrix_solver_method == MM_SUPERLU) {
        ...
        sparse_tonr(powerflow_values->island_matrix_values[island_loop_index].Y_Amatrix,
        ...
        if (matrix_solver_method == MM_SUPERLU) {
            ...
            if (mesh_imped_vals != NULL) {
                ...
                if (mesh_imped_vals->NodeRefNum > 0) {
                    tempa = 2 *
                            bus[mesh_imped_vals->NodeRefNum].Matrix_Loc;    //These are already separated into real/imag, so the base is 2x
                } else    //Error
                {
                ...
                for (kindex = 0; kindex < temp_size; kindex++) {
                    ...
                    // solve the system
                    dgssv(&options, &curr_island_superLU_vars->A_LU, curr_island_superLU_vars->perm_c,
                          curr_island_superLU_vars->perm_r, &L_LU, &U_LU, &curr_island_superLU_vars->B_LU, &stat,
                          &info);
            ...
            if (mesh_imped_vals != NULL) {
				...
				*/
                ...
                ((void (*)(void *, bool)) (LUSolverFcns.ext_destroy))(
                        powerflow_values->island_matrix_values[island_loop_index].LU_solver_vars,
                        powerflow_values->island_matrix_values[island_loop_index].new_iteration_required);
            ...
            //Call the solver
            powerflow_values->island_matrix_values[island_loop_index].solver_info = ((int (*)(void *, NR_SOLVER_VARS *,
                                                                                              unsigned int,
                                                                                              unsigned int)) (LUSolverFcns.ext_solve))(
                    powerflow_values->island_matrix_values[island_loop_index].LU_solver_vars,
                    &powerflow_values->island_matrix_values[island_loop_index].matrices_LU, n, 1);
            ...
            if (bus[indexer].island_number == island_loop_index) {
                ...
                if ((bus[indexer].type == 0) ||
                    ...
                    if ((bus[indexer].phases & 0x80) == 0x80)    //Split phase
                    {
                        ...
                        if (CurrConvVal >
                            powerflow_values->island_matrix_values[island_loop_index].max_mismatch_converge)    //Update our convergence check if it is bigger
                    ...
                    {
                        for (jindex = 0;
                             jindex < powerflow_values->BA_diag[indexer].size; jindex++)    //parse through the phases
                        {
                            switch (bus[indexer].phases & 0x07) {
                                case 0x01:    //C
                                {
                            ...
                            if (CurrConvVal >
                                powerflow_values->island_matrix_values[island_loop_index].max_mismatch_converge)    //See if the current differential is the largest found so far or not
                ...
                if ((bus[indexer].ExtraCurrentInjFunc != NULL) && ((bus[indexer].type == 0) ||
                                                                   ((bus[indexer].type > 1) &&
                                                                    (bus[indexer].swing_functions_enabled == false)))) {
                    //Call the function
                    call_return_status = ((STATUS (*)(OBJECT *)) (*bus[indexer].ExtraCurrentInjFunc))(
                            bus[indexer].ExtraCurrentInjFuncObject);

                    //Make sure it worked
                    if (call_return_status == FAILED) {
                        GL_THROW(const_cast<char *>("External current injection update failed for device %s"),
                                 bus[indexer].ExtraCurrentInjFuncObject->name
                                 ? bus[indexer].ExtraCurrentInjFuncObject->name : "Unnamed");
                        /*  TROUBLESHOOT
        ...
        if ((enable_inrush_calculations == true) &&
            (deltatimestep_running > 0))    //Don't even both with this if inrush not on
        {
            ...
            for (indexer = 0; indexer < branch_count; indexer++) {
                ...
                if (branch[indexer].island_number == island_loop_index) {
                    ...
                    if (branch[indexer].ExtraDeltaModeFunc != NULL) {
                        //Call the function
                        func_result_val = ((int (*)(OBJECT *, bool *)) (*branch[indexer].ExtraDeltaModeFunc))(
                                branch[indexer].obj,
                                &(powerflow_values->island_matrix_values[island_loop_index].SaturationMismatchPresent));

                        //Make sure it worked
                        if (func_result_val != 1) {
                            GL_THROW(const_cast<char *>("Extra delta update failed for device %s"),
        ...
        if (powerflow_type == PF_DYNINIT) {
            ...
            if (powerflow_values->island_matrix_values[island_loop_index].new_iteration_required ==
                false)    //Overall powerflow has converged
            {
                if (powerflow_values->island_matrix_values[island_loop_index].swing_converged ==
                    false)    //See if deltaI_sym is being met
                {
                    //See which message to print
                    if (NR_islands_detected > 1) {
                        //Give a verbose message, just cause
                        gl_verbose(
                                "NR_Solver: swing bus failed balancing criteria on island %d - making it a PQ for future dynamics",
                                (island_loop_index + 1));
                    } else    //Single system
                    {
                        //Give a verbose message, just cause
                        gl_verbose(
                                "NR_Solver: swing bus failed balancing criteria - making it a PQ for future dynamics");
                    }
        ...
        if (powerflow_values->island_matrix_values[island_loop_index].new_iteration_required ==
            true)    //We think we need a new one
        {
        ...
        } else    //Should be done
        {
                ...
                0)    //It was a convergence exit
            {
                ...
                if (NR_islands_detected > 1) {
                    ...
                    if (matrix_solver_method == MM_SUPERLU) {
                        gl_verbose("superLU failed out of island %d with return value %d", (island_loop_index + 1),
                                   powerflow_values->island_matrix_values[island_loop_index].solver_info);
                    } else if (matrix_solver_method == MM_EXTERN) {
                        gl_verbose("External LU solver failed out of island %d with return value %d",
                                   (island_loop_index + 1),
                                   powerflow_values->island_matrix_values[island_loop_index].solver_info);
                    }
    ...
    *bad_computations = true;
    ...
    for (island_loop_index = 0; island_loop_index < NR_islands_detected; island_loop_index++) {
            ...
            0)    //Success or really bad failure
        {
            //See which one it is
            if (powerflow_values->island_matrix_values[island_loop_index].new_iteration_required ==
                false)    //We converged to get here!
            {
            ...
            } else if (NR_island_fail_method ==
                       true)    //Really bad failure, but let's see if we're in "special handling mode"
            {
                *bad_computations = false;    //Deflag us, so it keeps going
                ...
                //Call the function
                call_return_status = ((STATUS (*)(OBJECT *, int)) (temp_fxn_val))(fault_check_object,
                                                                                  island_loop_index);

                //Make sure it worked
                if (call_return_status != SUCCESS) {
                    GL_THROW(const_cast<char *>("NR: Failed to remove island %d from the powerflow"),
                             (island_loop_index + 1));
                    /*  TROUBLESHOOT
        ...
        if (bus[indexer].island_number == island_number) {
            if ((bus[indexer].phases & 0x08) == 0x08)    //Delta connected node
            {
                ...
                if (*bus[indexer].dynamics_enabled == true) {
                    ...
                    if ((bus[indexer].I[0] != 0.0) && (adjust_temp_voltage_mag[0] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[0] = ~(adjust_temp_nominal_voltage[0] * ~bus[indexer].I[0] *
                    ...
                    if ((bus[indexer].I[1] != 0.0) && (adjust_temp_voltage_mag[1] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[1] = ~(adjust_temp_nominal_voltage[1] * ~bus[indexer].I[1] *
                    ...
                    if ((bus[indexer].I[2] != 0.0) && (adjust_temp_voltage_mag[2] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[2] = ~(adjust_temp_nominal_voltage[2] * ~bus[indexer].I[2] *
                    ...
                    if ((bus[indexer].phases & 0x10) == 0x10) {
                        ...
                        if ((bus[indexer].extra_var[6] != 0.0) && (adjust_temp_voltage_mag[3] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[3] = ~(adjust_temp_nominal_voltage[3] *
                        ...
                        if ((bus[indexer].extra_var[7] != 0.0) && (adjust_temp_voltage_mag[4] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[4] = ~(adjust_temp_nominal_voltage[4] *
                        ...
                        if ((bus[indexer].extra_var[8] != 0.0) && (adjust_temp_voltage_mag[5] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[5] = ~(adjust_temp_nominal_voltage[5] *
                ...
                if ((bus[indexer].phases & 0x04) == 0x04)    //Has a phase A
                {
                    ...
                    {
                        //Power values
                        undeltacurr[0] += (bus[indexer].V[0] == 0) ? 0 : ~(bus[indexer].extra_var[0] /
                                                                           bus[indexer].V[0]);
                ...
                if ((bus[indexer].phases & 0x02) == 0x02)    //Has a phase B
                {
                    ...
                    {
                        //Power values
                        undeltacurr[1] += (bus[indexer].V[1] == 0) ? 0 : ~(bus[indexer].extra_var[1] /
                                                                           bus[indexer].V[1]);
                ...
                if ((bus[indexer].phases & 0x01) == 0x01)    //Has a phase C
                {
                    ...
                    {
                        //Power values
                        undeltacurr[2] += (bus[indexer].V[2] == 0) ? 0 : ~(bus[indexer].extra_var[2] /
                                                                           bus[indexer].V[2]);
                ...
                for (jindex = 0; jindex < powerflow_values->BA_diag[indexer].size; jindex++) {
                    ...
                    if (jacobian_pass == false)    //current-injection updates
                    {
                        ...
                        tempPbus = (undeltacurr[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Re() +
                                   ...
                                   (bus[indexer].V[temp_index_b]).Im();    // Real power portion of Constant current component multiply the magnitude of bus voltage
                        ...
                        tempQbus = (undeltacurr[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() -
                                   (undeltacurr[temp_index_b]).Im() *
                                   (bus[indexer].V[temp_index_b]).Re();    // Reactive power portion of Constant current component multiply the magnitude of bus voltage
            ...
            else if ((bus[indexer].phases & 0x80) == 0x80)    //Split phase computations
            {
                ...
                if ((bus[indexer].phases & 0x40) == 0x40) {
                    ...
                    delta_current[0] = bus[indexer].house_var[0] /
                                       (~temp_store[0]);        //Just denominator conjugated to keep math right (rest was conjugated in house)
                    ...
                    for (jindex = 0; jindex < 2; jindex++) {
                        if ((bus[indexer].V[jindex]).Mag() != 0)    //Only current
                        {
                            bus[indexer].Jacob_A[jindex] =
                                    ((bus[indexer].V[jindex]).Re() * (bus[indexer].V[jindex]).Im() *
                                     (temp_store[jindex]).Re() +
                                     (temp_store[jindex]).Im() * pow((bus[indexer].V[jindex]).Im(), 2)) /
                                    pow((bus[indexer].V[jindex]).Mag(), 3);// second part of equation(37)
                            bus[indexer].Jacob_B[jindex] =
                                    -((bus[indexer].V[jindex]).Re() * (bus[indexer].V[jindex]).Im() *
                                      (temp_store[jindex]).Im() +
                                      (temp_store[jindex]).Re() * pow((bus[indexer].V[jindex]).Re(), 2)) /
                                    pow((bus[indexer].V[jindex]).Mag(), 3);// second part of equation(38)
                            bus[indexer].Jacob_C[jindex] =
                                    ((bus[indexer].V[jindex]).Re() * (bus[indexer].V[jindex]).Im() *
                                     (temp_store[jindex]).Im() -
                                     (temp_store[jindex]).Re() * pow((bus[indexer].V[jindex]).Im(), 2)) /
                                    pow((bus[indexer].V[jindex]).Mag(), 3);// second part of equation(39)
                            bus[indexer].Jacob_D[jindex] =
                                    ((bus[indexer].V[jindex]).Re() * (bus[indexer].V[jindex]).Im() *
                                     (temp_store[jindex]).Re() -
                                     (temp_store[jindex]).Im() * pow((bus[indexer].V[jindex]).Re(), 2)) /
                                    pow((bus[indexer].V[jindex]).Mag(), 3);// second part of equation(40)
                        } else {
            ...
            else    //Wye-connected system/load
            {
                ...
                if (*bus[indexer].dynamics_enabled == true) {
                    ...
                    if ((bus[indexer].I[0] != 0.0) && (adjust_temp_voltage_mag[3] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[0] = ~(adjust_temp_nominal_voltage[3] * ~bus[indexer].I[0] *
                    ...
                    if ((bus[indexer].I[1] != 0.0) && (adjust_temp_voltage_mag[4] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[1] = ~(adjust_temp_nominal_voltage[4] * ~bus[indexer].I[1] *
                    ...
                    if ((bus[indexer].I[2] != 0.0) && (adjust_temp_voltage_mag[5] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[2] = ~(adjust_temp_nominal_voltage[5] * ~bus[indexer].I[2] *
                    ...
                    if ((bus[indexer].phases & 0x10) == 0x10) {
                        ...
                        if ((bus[indexer].extra_var[6] != 0.0) && (adjust_temp_voltage_mag[0] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[3] = ~(adjust_temp_nominal_voltage[0] *
                        ...
                        if ((bus[indexer].extra_var[7] != 0.0) && (adjust_temp_voltage_mag[1] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[4] = ~(adjust_temp_nominal_voltage[1] *
                        ...
                        if ((bus[indexer].extra_var[8] != 0.0) && (adjust_temp_voltage_mag[2] != 0.0)) {
                            //calculate new value
                            adjusted_constant_current[5] = ~(adjust_temp_nominal_voltage[2] *
                ...
                if ((bus[indexer].phases & 0x10) ==
                    0x10)    //"Different" child load - in this case it must be delta - also must be three phase (just because that's how I forced it to be implemented)
                ...
                for (jindex = 0; jindex < powerflow_values->BA_diag[indexer].size; jindex++) {
                    ...
                    if (jacobian_pass == false)    //Current injection pass
                    {
                        ...
                        tempPbus +=
                                (adjusted_constant_current[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Re() +
                                (adjusted_constant_current[temp_index_b]).Im() *
                                (bus[indexer].V[temp_index_b]).Im();    // Real power portion of Constant current component multiply the magnitude of bus voltage
                        tempPbus += (undeltacurr[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Re() +
                                    (undeltacurr[temp_index_b]).Im() *
                                    (bus[indexer].V[temp_index_b]).Im();    // Real power portion of Constant current from "different" children
                        tempPbus += (bus[indexer].Y[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Re() *
                                    (bus[indexer].V[temp_index_b]).Re() +
                                    (bus[indexer].Y[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                    (bus[indexer].V[temp_index_b]).Im();    // Real power portion of Constant impedance component multiply the square of the magnitude of bus voltage
                        ...
                        tempQbus +=
                                (adjusted_constant_current[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() -
                                (adjusted_constant_current[temp_index_b]).Im() *
                                (bus[indexer].V[temp_index_b]).Re();    // Reactive power portion of Constant current component multiply the magnitude of bus voltage
                        tempQbus += (undeltacurr[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() -
                                    (undeltacurr[temp_index_b]).Im() *
                                    (bus[indexer].V[temp_index_b]).Re();    // Reactive power portion of Constant current from "different" children
                        tempQbus += -(bus[indexer].Y[temp_index_b]).Im() * (bus[indexer].V[temp_index_b]).Im() *
                                    (bus[indexer].V[temp_index_b]).Im() -
                                    (bus[indexer].Y[temp_index_b]).Im() * (bus[indexer].V[temp_index_b]).Re() *
                                    (bus[indexer].V[temp_index_b]).Re();    // Reactive power portion of Constant impedance component multiply the square of the magnitude of bus voltage
                    ...
                    } else    //Jacobian update pass
                    {
                        ...
                        if ((bus[indexer].V[temp_index_b]).Mag() != 0) {
                            ...
                            bus[indexer].Jacob_A[temp_index] +=
                                    ...
                                    (bus[indexer].Y[temp_index_b]).Im();// second part of equation(37)
                            bus[indexer].Jacob_A[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     (undeltacurr[temp_index_b]).Re() +
                                     (undeltacurr[temp_index_b]).Im() * pow((bus[indexer].V[temp_index_b]).Im(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3);// current part of equation (37) - Handles "different" children
                            ...
                            bus[indexer].Jacob_B[temp_index] +=
                                    -((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                      ...
                                      pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(), 3) -
                                    (bus[indexer].Y[temp_index_b]).Re();// second part of equation(38)
                            bus[indexer].Jacob_B[temp_index] +=
                                    -((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                      (undeltacurr[temp_index_b]).Im() +
                                      (undeltacurr[temp_index_b]).Re() * pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3);// current part of equation(38) - Handles "different" children
                            ...
                            bus[indexer].Jacob_C[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     ...
                                     pow((bus[indexer].V[temp_index_b]).Im(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(), 3) -
                                    (bus[indexer].Y[temp_index_b]).Re();// second part of equation(39)
                            bus[indexer].Jacob_C[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     (undeltacurr[temp_index_b]).Im() -
                                     (undeltacurr[temp_index_b]).Re() * pow((bus[indexer].V[temp_index_b]).Im(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3);// Current part of equation(39) - Handles "different" children
                            ...
                            bus[indexer].Jacob_D[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     ...
                                     pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(), 3) -
                                    (bus[indexer].Y[temp_index_b]).Im();// second part of equation(40)
                            bus[indexer].Jacob_D[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     (undeltacurr[temp_index_b]).Re() -
                                     (undeltacurr[temp_index_b]).Im() * pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3);// Current part of equation(40) - Handles "different" children
            ...
            if ((bus[indexer].phases & 0x80) != 0x80)    //Not triplex
            {
                ...
                if (*bus[indexer].dynamics_enabled == true) {
                    ...
                    if ((bus[indexer].I_dy[3] != 0.0) && (adjust_temp_voltage_mag[3] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[3] = ~(adjust_temp_nominal_voltage[3] * ~bus[indexer].I_dy[3] *
                                                         adjust_temp_voltage_mag[3] /
                                                         (bus[indexer].V[0] * adjust_nominal_voltage_val));
                    } else {
                        adjusted_constant_current[3] = complex(0.0, 0.0);
                    ...
                    if ((bus[indexer].I_dy[4] != 0.0) && (adjust_temp_voltage_mag[4] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[4] = ~(adjust_temp_nominal_voltage[4] * ~bus[indexer].I_dy[4] *
                                                         adjust_temp_voltage_mag[4] /
                                                         (bus[indexer].V[1] * adjust_nominal_voltage_val));
                    } else {
                        adjusted_constant_current[4] = complex(0.0, 0.0);
                    ...
                    if ((bus[indexer].I_dy[5] != 0.0) && (adjust_temp_voltage_mag[5] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[5] = ~(adjust_temp_nominal_voltage[5] * ~bus[indexer].I_dy[5] *
                                                         adjust_temp_voltage_mag[5] /
                                                         (bus[indexer].V[2] * adjust_nominal_voltage_val));
                    } else {
                        adjusted_constant_current[5] = complex(0.0, 0.0);
                    ...
                    if ((bus[indexer].I_dy[0] != 0.0) && (adjust_temp_voltage_mag[0] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[0] = ~(adjust_temp_nominal_voltage[0] * ~bus[indexer].I_dy[0] *
                                                         adjust_temp_voltage_mag[0] /
                    ...
                    if ((bus[indexer].I_dy[1] != 0.0) && (adjust_temp_voltage_mag[1] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[1] = ~(adjust_temp_nominal_voltage[1] * ~bus[indexer].I_dy[1] *
                                                         adjust_temp_voltage_mag[1] /
                    ...
                    if ((bus[indexer].I_dy[2] != 0.0) && (adjust_temp_voltage_mag[2] != 0.0)) {
                        //calculate new value
                        adjusted_constant_current[2] = ~(adjust_temp_nominal_voltage[2] * ~bus[indexer].I_dy[2] *
                                                         adjust_temp_voltage_mag[2] /
                        ...
                        if ((bus[indexer].V[temp_index_b]).Mag() != 0) {
                            ...
                            bus[indexer].Jacob_A[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     ...
                                     (undeltacurr[temp_index_b]).Im() * pow((bus[indexer].V[temp_index_b]).Im(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3); // + (undeltaimped[temp_index_b]).Im();// second part of equation(37) - no power term needed
                            bus[indexer].Jacob_B[temp_index] +=
                                    -((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                      (undeltacurr[temp_index_b]).Im() +
                                      (undeltacurr[temp_index_b]).Re() * pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3); // - (undeltaimped[temp_index_b]).Re();// second part of equation(38) - no power term needed
                            bus[indexer].Jacob_C[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     (undeltacurr[temp_index_b]).Im() -
                                     (undeltacurr[temp_index_b]).Re() * pow((bus[indexer].V[temp_index_b]).Im(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3); // - (undeltaimped[temp_index_b]).Re();// second part of equation(39) - no power term needed
                            bus[indexer].Jacob_D[temp_index] +=
                                    ((bus[indexer].V[temp_index_b]).Re() * (bus[indexer].V[temp_index_b]).Im() *
                                     (undeltacurr[temp_index_b]).Re() -
                                     (undeltacurr[temp_index_b]).Im() * pow((bus[indexer].V[temp_index_b]).Re(), 2)) /
                                    pow((bus[indexer].V[temp_index_b]).Mag(),
                                        3); // - (undeltaimped[temp_index_b]).Im();// second part of equation(40) - no power term needed
                        } else    //Zero voltage = only impedance is valid (others get divided by VMag, so are IND) - not entirely sure how this gets in here anyhow
                        {
