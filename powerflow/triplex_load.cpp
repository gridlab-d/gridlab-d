/** $Id: triplex_load.cpp 1182 2012-7-1 2:13 PDT fish334 $
        ...
        Triplex_load objects represent static loads and export both voltages and
   current.
...
triplex_load::triplex_load(MODULE *mod) : triplex_node(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "triplex_load", sizeof(triplex_load),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "triplex_node", PT_enumeration, "load_class",
            ...
            (enumeration)CRITICAL, PT_complex, "constant_power_1[VA]",
            PADDR(constant_power[0]), PT_DESCRIPTION,
            "constant power load on split phase 1, specified as VA", PT_complex,
            "constant_power_2[VA]", PADDR(constant_power[1]), PT_DESCRIPTION,
            "constant power load on split phase 2, specified as VA", PT_complex,
            "constant_power_12[VA]", PADDR(constant_power[2]), PT_DESCRIPTION,
            "constant power load on split phase 12, specified as VA", PT_double,
            "constant_power_1_real[W]", PADDR(constant_power[0].Re()),
            PT_DESCRIPTION,
            "constant power load on spit phase 1, real only, specified as W",
            PT_double, "constant_power_2_real[W]",
            PADDR(constant_power[1].Re()), PT_DESCRIPTION,
            "constant power load on phase 2, real only, specified as W",
            PT_double, "constant_power_12_real[W]",
            PADDR(constant_power[2].Re()), PT_DESCRIPTION,
            "constant power load on phase 12, real only, specified as W",
            PT_double, "constant_power_1_reac[VAr]",
            PADDR(constant_power[0].Im()), PT_DESCRIPTION,
            "constant power load on phase 1, imaginary only, specified as VAr",
            PT_double, "constant_power_2_reac[VAr]",
            PADDR(constant_power[1].Im()), PT_DESCRIPTION,
            "constant power load on phase 2, imaginary only, specified as VAr",
            PT_double, "constant_power_12_reac[VAr]",
            PADDR(constant_power[2].Im()), PT_DESCRIPTION,
            "constant power load on phase 12, imaginary only, specified as VAr",
            PT_complex, "constant_current_1[A]", PADDR(constant_current[0]),
            PT_DESCRIPTION,
            "constant current load on phase 1, specified as Amps", PT_complex,
            "constant_current_2[A]", PADDR(constant_current[1]), PT_DESCRIPTION,
            "constant current load on phase 2, specified as Amps", PT_complex,
            "constant_current_12[A]", PADDR(constant_current[2]),
            PT_DESCRIPTION,
            "constant current load on phase 12, specified as Amps", PT_double,
            "constant_current_1_real[A]", PADDR(constant_current[0].Re()),
            PT_DESCRIPTION,
            "constant current load on phase 1, real only, specified as Amps",
            PT_double, "constant_current_2_real[A]",
            PADDR(constant_current[1].Re()), PT_DESCRIPTION,
            "constant current load on phase 2, real only, specified as Amps",
            PT_double, "constant_current_12_real[A]",
            PADDR(constant_current[2].Re()), PT_DESCRIPTION,
            "constant current load on phase 12, real only, specified as Amps",
            PT_double, "constant_current_1_reac[A]",
            PADDR(constant_current[0].Im()), PT_DESCRIPTION,
            "constant current load on phase 1, imaginary only, specified as "
            "Amps",
            PT_double, "constant_current_2_reac[A]",
            PADDR(constant_current[1].Im()), PT_DESCRIPTION,
            "constant current load on phase 2, imaginary only, specified as "
            "Amps",
            PT_double, "constant_current_12_reac[A]",
            PADDR(constant_current[2].Im()), PT_DESCRIPTION,
            "constant current load on phase 12, imaginary only, specified as "
            "Amps",
            PT_complex, "constant_impedance_1[Ohm]",
            PADDR(constant_impedance[0]), PT_DESCRIPTION,
            "constant impedance load on phase 1, specified as Ohms", PT_complex,
            "constant_impedance_2[Ohm]", PADDR(constant_impedance[1]),
            PT_DESCRIPTION,
            "constant impedance load on phase 2, specified as Ohms", PT_complex,
            "constant_impedance_12[Ohm]", PADDR(constant_impedance[2]),
            PT_DESCRIPTION,
            "constant impedance load on phase 12, specified as Ohms", PT_double,
            "constant_impedance_1_real[Ohm]", PADDR(constant_impedance[0].Re()),
            PT_DESCRIPTION,
            "constant impedance load on phase 1, real only, specified as Ohms",
            PT_double, "constant_impedance_2_real[Ohm]",
            PADDR(constant_impedance[1].Re()), PT_DESCRIPTION,
            "constant impedance load on phase 2, real only, specified as Ohms",
            PT_double, "constant_impedance_12_real[Ohm]",
            PADDR(constant_impedance[2].Re()), PT_DESCRIPTION,
            "constant impedance load on phase 12, real only, specified as Ohms",
            PT_double, "constant_impedance_1_reac[Ohm]",
            PADDR(constant_impedance[0].Im()), PT_DESCRIPTION,
            "constant impedance load on phase 1, imaginary only, specified as "
            "Ohms",
            PT_double, "constant_impedance_2_reac[Ohm]",
            PADDR(constant_impedance[1].Im()), PT_DESCRIPTION,
            "constant impedance load on phase 2, imaginary only, specified as "
            "Ohms",
            PT_double, "constant_impedance_12_reac[Ohm]",
            PADDR(constant_impedance[2].Im()), PT_DESCRIPTION,
            "constant impedance load on phase 12, imaginary only, specified as "
            "Ohms",
            PT_complex, "measured_voltage_1[V]", PADDR(measured_voltage_1),
            PT_DESCRIPTION, "measured voltage on phase 1", PT_complex,
            "measured_voltage_2[V]", PADDR(measured_voltage_2), PT_DESCRIPTION,
            "measured voltage on phase 2", PT_complex, "measured_voltage_12[V]",
            PADDR(measured_voltage_12), PT_DESCRIPTION,
            "measured voltage on phase 12", PT_complex,
            "indiv_measured_power_1[VA]", PADDR(measured_power[0]),
            PT_DESCRIPTION, "current measured power on phase 1", PT_complex,
            "indiv_measured_power_2[VA]", PADDR(measured_power[1]),
            PT_DESCRIPTION, "current measured power on phase 2", PT_complex,
            "indiv_measured_power_12[VA]", PADDR(measured_power[2]),
            PT_DESCRIPTION, "current measured power on phase 12", PT_complex,
            ...
            PT_double, "base_power_1[VA]", PADDR(base_power[0]), PT_DESCRIPTION,
            "in similar format as ZIPload, this represents the nominal power "
            "on phase 1 before applying ZIP fractions",
            PT_double, "base_power_2[VA]", PADDR(base_power[1]), PT_DESCRIPTION,
            "in similar format as ZIPload, this represents the nominal power "
            "on phase 2 before applying ZIP fractions",
            PT_double, "base_power_12[VA]", PADDR(base_power[2]),
            PT_DESCRIPTION,
            "in similar format as ZIPload, this represents the nominal power "
            "on phase 12 before applying ZIP fractions",
            PT_double, "power_pf_1[pu]", PADDR(power_pf[0]), PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 1 constant power portion of load",
            PT_double, "current_pf_1[pu]", PADDR(current_pf[0]), PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 1 constant current portion of load",
            PT_double, "impedance_pf_1[pu]", PADDR(impedance_pf[0]),
            PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 1 constant impedance portion of load",
            PT_double, "power_pf_2[pu]", PADDR(power_pf[1]), PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 2 constant power portion of load",
            PT_double, "current_pf_2[pu]", PADDR(current_pf[1]), PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 2 constant current portion of load",
            PT_double, "impedance_pf_2[pu]", PADDR(impedance_pf[1]),
            PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 2 constant impedance portion of load",
            PT_double, "power_pf_12[pu]", PADDR(power_pf[2]), PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 12 constant power portion of load",
            PT_double, "current_pf_12[pu]", PADDR(current_pf[2]),
            PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 12 constant current portion of load",
            PT_double, "impedance_pf_12[pu]", PADDR(impedance_pf[2]),
            PT_DESCRIPTION,
            "in similar format as ZIPload, this is the power factor of the "
            "phase 12 constant impedance portion of load",
            PT_double, "power_fraction_1[pu]", PADDR(power_fraction[0]),
            PT_DESCRIPTION,
            "this is the constant power fraction of base power on phase 1",
            PT_double, "current_fraction_1[pu]", PADDR(current_fraction[0]),
            PT_DESCRIPTION,
            "this is the constant current fraction of base power on phase 1",
            PT_double, "impedance_fraction_1[pu]", PADDR(impedance_fraction[0]),
            PT_DESCRIPTION,
            "this is the constant impedance fraction of base power on phase 1",
            PT_double, "power_fraction_2[pu]", PADDR(power_fraction[1]),
            PT_DESCRIPTION,
            "this is the constant power fraction of base power on phase 2",
            PT_double, "current_fraction_2[pu]", PADDR(current_fraction[1]),
            PT_DESCRIPTION,
            "this is the constant current fraction of base power on phase 2",
            PT_double, "impedance_fraction_2[pu]", PADDR(impedance_fraction[1]),
            PT_DESCRIPTION,
            "this is the constant impedance fraction of base power on phase 2",
            PT_double, "power_fraction_12[pu]", PADDR(power_fraction[2]),
            PT_DESCRIPTION,
            "this is the constant power fraction of base power on phase 12",
            PT_double, "current_fraction_12[pu]", PADDR(current_fraction[2]),
            PT_DESCRIPTION,
            "this is the constant current fraction of base power on phase 12",
            PT_double, "impedance_fraction_12[pu]",
            PADDR(impedance_fraction[2]), PT_DESCRIPTION,
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_triplex_load) == nullptr)
    ...
    if (gl_publish_function(oclass, "pwr_current_injection_update_map",
        ...
        nullptr)
      GL_THROW("Unable to publish triplex_load current injection update "
               "mapping function");
    ...
    if (gl_publish_function(oclass, "pwr_object_load_update",
                            (FUNCTIONADDR)update_triplex_load_values) ==
        nullptr)
      GL_THROW("Unable to publish triplex_load impedance-conversion/update "
               "function");
...
int triplex_load::create(void) {
  ...
  base_load_val_was_nonzero[0] = base_load_val_was_nonzero[1] =
      base_load_val_was_nonzero[2] = false; // Start deflagged
  ZIP_constant_current[0] = ZIP_constant_current[1] = ZIP_constant_current[2] =
      false; // Start assuming not touched

  prev_load_values[0][0] = prev_load_values[0][1] = prev_load_values[0][2] =
      gld::complex(0.0, 0.0);
  prev_load_values[1][0] = prev_load_values[1][1] = prev_load_values[1][2] =
      gld::complex(0.0, 0.0);
  prev_load_values[2][0] = prev_load_values[2][1] = prev_load_values[2][2] =
      gld::complex(0.0, 0.0);
...
int triplex_load::init(OBJECT *parent) {
  ...
  if (((constant_current[0] != 0.0) && (base_power[0] != 0.0)) ||
    ...
    gl_warning("triplex_load:%s - ZIP fractions will override "
               "constant_current_X values",
               obj->name ? obj->name : "unnamed");
    /*  TROUBLESHOOT
    In triplex_load objects, if constant_current_X is specified and a
    base_power_X has a constant current portion, the base_power_X-based values
    will override constant_current_X and inputs to constant_current_X will be
...
TIMESTAMP triplex_load::postsync(TIMESTAMP t0) {
  ...
  measured_voltage_1.SetPolar(
      voltage1.Mag(), voltage1.Arg()); // Used for testing and xml output
  ...
  measured_power[2] =
      voltage[2] * (~(-(current_inj[1] + current_inj[0]))); // Voltage_N
...
// Here primarily so deltamode players can actually influence things, as well as
// constant current rotations
void triplex_load::triplex_load_update_fxn(void) {
  ...
  if (base_power[0] != 0.0) { // Phase 1
    ...
    if (power_fraction[0] + current_fraction[0] + impedance_fraction[0] !=
        1.0) {
      power_fraction[0] = 1 - current_fraction[0] - impedance_fraction[0];

      OBJECT *obj = object_header(this);

      gl_warning("load:%s - ZIP components on phase 1 did not sum to 1. "
                 "Setting power_fraction to %.2f",
                 obj->name ? obj->name : "unnamed", power_fraction[0]);
      /*  TROUBLESHOOT
      ZIP load fractions must sum to 1.  The values (power_fraction_1,
      impedance_fraction_1, and current_fraction_1) did not sum to 1. To ensure
      that constraint (Z+I+P=1), power_fraction is being calculated and
      overwritten.
      */
    }
    ...
    if (current_fraction[0] != 0.0) {
      ...
      if (current_pf[0] == 0.0) {
        ...
        imag_power =
            real_power * sqrt(1.0 / (current_pf[0] * current_pf[0]) - 1.0);
      }
      ...
      // Calculate then shift the constant current to use the posted voltage as
      // the reference angle
    ...
    if (impedance_fraction[0] != 0.0) {
      ...
      if (impedance_pf[0] == 0.0) {
        ...
        real_power =
            base_power[0] * impedance_fraction[0] * fabs(impedance_pf[0]);
        imag_power =
            real_power * sqrt(1.0 / (impedance_pf[0] * impedance_pf[0]) - 1.0);
      }
  ...
  if (base_power[1] != 0.0) { // Phase 2
    ...
    if (power_fraction[1] + current_fraction[1] + impedance_fraction[1] !=
        1.0) {
      power_fraction[1] = 1 - current_fraction[1] - impedance_fraction[1];

      OBJECT *obj = object_header(this);

      gl_warning("load:%s - ZIP components on phase 2 did not sum to 1. "
                 "Setting power_fraction to %.2f",
                 obj->name ? obj->name : "unnamed", power_fraction[1]);
      /*  TROUBLESHOOT
      ZIP load fractions must sum to 1.  The values (power_fraction_2,
      impedance_fraction_2, and current_fraction_2) did not sum to 1. To ensure
      that constraint (Z+I+P=1), power_fraction is being calculated and
      overwritten.
      */
    }
    ...
    if (current_fraction[1] != 0.0) {
      ...
      if (current_pf[1] == 0.0) {
        ...
        imag_power =
            real_power * sqrt(1.0 / (current_pf[1] * current_pf[1]) - 1.0);
      }
    ...
    if (impedance_fraction[1] != 0.0) {
      ...
      if (impedance_pf[1] == 0.0) {
        ...
        real_power =
            base_power[1] * impedance_fraction[1] * fabs(impedance_pf[1]);
        imag_power =
            real_power * sqrt(1.0 / (impedance_pf[1] * impedance_pf[1]) - 1.0);
      }
  ...
  if (base_power[2] != 0.0) { // Phase 12
    ...
    if (power_fraction[2] + current_fraction[2] + impedance_fraction[2] !=
        1.0) {
      power_fraction[2] = 1 - current_fraction[2] - impedance_fraction[2];

      OBJECT *obj = object_header(this);

      gl_warning("load:%s - ZIP components on phase 12 did not sum to 1. "
                 "Setting power_fraction to %.2f",
                 obj->name ? obj->name : "unnamed", power_fraction[2]);
      /*  TROUBLESHOOT
      ZIP load fractions must sum to 1.  The values (power_fraction_12,
      impedance_fraction_12, and current_fraction_12) did not sum to 1. To
      ensure that constraint (Z+I+P=1), power_fraction is being calculated and
      overwritten.
      */
    }
    // Put in the constant power portion
    ...
    if (current_fraction[2] != 0.0) {
      ...
      if (current_pf[2] == 0.0) {
        ...
        imag_power =
            real_power * sqrt(1.0 / (current_pf[2] * current_pf[2]) - 1.0);
      }
      ...
      // Calculate then shift the constant current to use the posted voltage as
      // the reference angle Note that the 2*nominal_voltage is to account for
      // the 240 V connection
      temp_curr = ~gld::complex(real_power, imag_power) /
                  gld::complex(2.0 * nominal_voltage, 0);
    ...
    if (impedance_fraction[2] != 0.0) {
      ...
      if (impedance_pf[2] == 0.0) {
        ...
        real_power =
            base_power[2] * impedance_fraction[2] * fabs(impedance_pf[2]);
        imag_power =
            real_power * sqrt(1.0 / (impedance_pf[2] * impedance_pf[2]) - 1.0);
      }
      ...
      // Note that nominal_voltage is 120, so need to double for 240, the 4 is
      // 2^2
      constant_impedance[2] =
          ~(gld::complex(4 * nominal_voltage * nominal_voltage, 0) /
            gld::complex(real_power, imag_power));
    } else {
  ...
  for (index_var = 0; index_var < 3; index_var++) {
    if ((constant_current[index_var] != 0.0) &&
        (adjust_temp_voltage_mag[index_var] != 0.0) &&
        !ZIP_constant_current[index_var]) {
      // calculate new value
      adjusted_current[index_var] =
          ~(adjust_temp_nominal_voltage[index_var] *
            ~constant_current[index_var] * adjust_temp_voltage_mag[index_var] /
            (adjust_voltage_val[index_var] *
             adjust_nominal_voltage_val[index_var]));
    } else {
...
void triplex_load::triplex_load_delete_update_fxn(void) {
  ...
  if (NR_solver_algorithm == NRM_FPI) {
    // See if we're a parented load or not - not sure how we'd ever be a
    // "different child", but copy paste!
...
//////////////////////////////////////////////////////////////////////////
// Module-level call
SIMULATIONMODE triplex_load::inter_deltaupdate_triplex_load(
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_triplex_load(OBJECT *obj, TIMESTAMP t0,
                                                  PASSCONFIG pass) {
  return sync_triplex_load_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_triplex_load(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
extern "C" MODULE_API int isa_triplex_load(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_triplex_load_impl(obj, classsname);
}
#endif
...
EXPORT SIMULATIONMODE interupdate_triplex_load(OBJECT *obj,
  ...
  try {
    status = my->inter_deltaupdate_triplex_load(
        delta_time, dt, iteration_count_val, interupdate_pos);
    return status;
  } catch (char *msg) {
    gl_error("interupdate_triplex_load(obj=%d;%s): %s", obj->id,
