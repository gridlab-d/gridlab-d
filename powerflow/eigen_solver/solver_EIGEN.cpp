...
// Allocation function
void LU_alloc(void *ext_array, unsigned int rowcount, unsigned int colcount,
...
// Performs the analysis portion and outputs result
int LU_solve(void *ext_array, NR_SOLVER_VARS *system_info_vars,
  ...
  SpMat mat;
    ...
    for (int row_ref = 0; row_ref <=.rows(); row_ref++) {
      while (col_ref < cols_index[row_ref + 1]) {
        ...
        // check for NaN's
        if (isnan(value)) {
          std::cout << "NaN detected in matrix A value, returning with -1"
                    << std::endl;
          return -1;
        }
    ...
    for (int i = 0; i < cols; i++) {
      ...
      if (isnan(system_info_vars->rhs_LU[i])) {
        std::cout << "NaN detected in matrix b value, returning with -1"
                  << std::endl;
        //            return -1;
      }
    ...
    if (EigenValues->solver->info() != 0) {
      std::cout << "issue with solver: " << EigenValues->solver->info()
                << std::endl;
      //        return -1;
    }
    ...
    for (int i = 0; i <.rows(); i++) {
      ...
      if (isnan(solution[i] || (EigenValues->tracker > 1))) {
        std::cout << "NaN detected in matrix x value, returning with -1"
                  << std::endl;
        //            return -1;
      }
...
// New iteration isn't needed here - numeric gets redone EVERY iteration, so we
// don't care if we were successful or not
