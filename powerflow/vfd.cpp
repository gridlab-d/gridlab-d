...
vfd::vfd(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "vfd", sizeof(vfd),
    ...
    if (gl_publish_variable(
            ...
            PT_double, "rated_motor_speed[1/min]", PADDR(ratedRPM),
            PT_DESCRIPTION, "Rated speed of the VFD in RPM. Default = 1800 RPM",
            PT_double, "desired_motor_speed[1/min]", PADDR(desiredRPM),
            PT_DESCRIPTION,
            "Desired speed of the VFD In ROM. Default = 1800 RPM (max)",
            PT_double, "motor_poles", PADDR(motorPoles), PT_DESCRIPTION,
            "Number of Motor Poles. Default = 4", PT_double,
            "rated_output_voltage[V]", PADDR(voltageLLRating), PT_DESCRIPTION,
            "Line to Line Voltage - VFD Rated voltage. Default to TO node "
            "nominal_voltage",
            PT_double, "rated_horse_power[hp]", PADDR(horsePowerRatedVFD),
            PT_DESCRIPTION, "Rated Horse Power of the VFD. Default = 75 HP",
            PT_double, "nominal_output_frequency[Hz]",
            PADDR(nominal_output_frequency), PT_DESCRIPTION,
            "Nominal VFD output frequency. Default = 60 Hz",

            PT_double, "desired_output_frequency[Hz]", PADDR(desiredFrequency),
            PT_DESCRIPTION,
            "VFD desired output frequency based on the desired RPM", PT_double,
            "current_output_frequency[Hz]", PADDR(currentFrequency),
            PT_DESCRIPTION, "VFD currently output frequency", PT_double,
            "efficiency[%]", PADDR(currEfficiency), PT_DESCRIPTION,
            "Current VFD efficiency based on the load/VFD output Horsepower",

            PT_double, "stable_time[s]", PADDR(stableTime), PT_DESCRIPTION,
            "Time taken by the VFD to reach desired frequency (based on RPM). "
            "Default = 1.45 seconds",

            PT_enumeration, "vfd_state", PADDR(vfdState), PT_DESCRIPTION,
            "Current state of the VFD", PT_KEYWORD, "OFF", (enumeration)VFD_OFF,
            PT_KEYWORD, "CHANGING", (enumeration)VFD_CHANGING, PT_KEYWORD,
            "STEADY_STATE", (enumeration)VFD_STEADY,
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_vfd) == nullptr)
      GL_THROW("Unable to publish VFD deltamode function");
    if (gl_publish_function(oclass, "postupdate_pwr_object",
                            (FUNCTIONADDR)postupdate_vfd) == nullptr)
      GL_THROW("Unable to publish VFD deltamode function");
    ...
    if (gl_publish_function(oclass, "check_limits_pwr_object",
      ...
      GL_THROW(
          "Unable to publish VFD external power limit calculation function");
    ...
    if (gl_publish_function(oclass, "vfd_current_injection_update",
                            (FUNCTIONADDR)current_injection_update_VFD) ==
        nullptr)
      GL_THROW("Unable to publish VFD external current injection calculation "
               "function");
  }
}
...
int vfd::create() {
  ...
  // z_val is part of a "center and fit" consequence of the MATLAB equation fit
  // - computed later 3.497*z^7 - 8.2828*z^6 + 0.97848*z^5 + 8.7113*z^4
  // - 3.2079*z^3 - 4.4504*z^2 + 3.8759*z + 96.014
...
int vfd::init(OBJECT *parent) {
  ...
  if (stableTime <= 0.0) {
    GL_THROW("VFD:%d - %s - the stableTime must be positive", obj->id,
             (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The stableTime value for the VFD is negative.  This must be a positive value
    for the simulation to work properly.
    */
  }
  ...
  if (voltageLLRating <= 0.0) {
    ...
    gl_warning("VFD:%d - %s - rated_output_voltage was not set, defaulting to "
               "TO node nominal",
               obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The rated_output_voltage was not set for the VFD.  It has been defaulted to
    the nominal_voltage value of the TO node.
    */
  } else // Was specified
  {
    ...
    if (temp_diff_value > 0.5) {
      gl_warning("VFD:%d - %s - rated_output_voltage does not match the TO "
                 "node nominal voltage",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      The rated_output_voltage does not match the nominal_voltage of the TO
      node.  This may cause some issues with simulations results.
      */
    }
  ...
  if (temp_voltage_check < 0.0) {
    gl_warning(
        "VFD:%d - %s - FROM node nominal_voltage is not equal to or greater "
        "than TO node nominal_voltage - implies DC-DC converter!e",
        obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The from node nominal_voltage is less than the to node nominal_voltage. This
    implies an internal DC-DC converter, which is not explicityly modeled.
    Efficiency values will not be accurate!
    */
  }

  // Check the nominal output frequency
  if (nominal_output_frequency <= 0.0) {
    gl_warning("VFD:%d - %s - nominal_output_frequency is not set or is "
               "negative - defaulting to system nominal",
               obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The nominal_output_frequency variable was not set properly - defaulting to
    nominal_frequency of the system.
     */
  ...
  // Check rated speed
  if (ratedRPM <= 0.0) {
    GL_THROW("VFD:%d - %s - rated_motor_speed must be positive and non-zero!",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The value specified for rated_motor_speed must be positive and non-zero in
  ...
  // Check the poles too
  if (motorPoles <= 0.0) {
    GL_THROW("VFD:%d - %s - motor_poles must be positive and non-zero!",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The value specified for motor_poles must be positive and non-zero in order
    to properly work.
     */
  ...
  if (horsePowerRatedVFD <= 0.0) {
    GL_THROW("VFD:%d - %s - rated_horse_power must be positive and non-zero!",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The value specified for rated_horse_power must be positive and non-zero in
    order to properly work.
     */
  ...
  if (!(has_phase(PHASE_A) & has_phase(PHASE_B) & has_phase(PHASE_C))) {
    GL_THROW("VFD:%d - %s - Must be three-phase!", obj->id,
             (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The VFD only operates as a three-phase device, at this time.  Please specify
    all phases.
     */
  ...
  if (temp_status_val == FAILED) {
    GL_THROW("VFD:%d - %s - Allocating the dynamic arrays for the frequency "
             "tracking failed",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    While attempting to allocate the dynamic length arrays for the vfd
    operation, an issue was encountered.  Please try again.  If the error
  ...
  // Make sure it worked
  if (temp_fxn == nullptr) {
    GL_THROW("VFD:%d - %s -- Failed to map TO-node flag function", obj->id,
             (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    While attempting to update the flagging on the "TO" node to reflect being
    attached to a VFD, an error occurred. Please try again.  If the error
    persists, please submit an issue
    */
  }
...
void vfd::CheckParameters() {
  ...
  if (desiredRPM < 0.0) {
    GL_THROW("VFD:%d - %s - the desired_motor_speed must be positive", obj->id,
             (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The desired_motor_speed value for the VFD is negative.  This must be a
    positive value for the simulation to work properly.  Backwards spinning
    motors are not supported at this time.
    */
  }
  ...
  desiredFrequency = (desiredRPM * motorPoles) /
                     120.0; // Convert RPM to Hz (with pole pair division)

  if ((desiredFrequency > 0.0) && (desiredFrequency < 3.0)) {
    gl_warning("VFD:%d - %s - the desired_motor_speed results in a frequency "
               "of less than 3.0 Hz",
               obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The desired_motor_speed results in a value that is less than the starting
    frequency of a typical VFD.  This is high discouraged, since it may not
    function properly in this range.
    */
  }

  if (desiredRPM / ratedRPM <= 0.75) {
    gl_warning("VFD:%d - %s - VFD currently at %f%% of rated speed -- VFDs "
               "perform best when running at >= 75 percent output",
               obj->id, (obj->name ? obj->name : "Unnamed"),
               (desiredRPM * 100 / ratedRPM));
    /*  TROUBLESHOOT
    Recommended to run a VFD at 75%.  Lower values are not recommended and can
    result in odd behavior
    */
  }
}

// Function to perform the current update - called by a node object, after
// they've updated (so current is accurate)
STATUS vfd::VFD_current_injection(void) {
  ...
  if (curr_time_value !=
      prev_time_value) // Updates only occur for new time steps
  {
    ...
    if (vfdState == VFD_STEADY) {
      ...
      if (prev_vfdState ==
          VFD_OFF) // Were off, so need to actually be starting up
      {
        // flag us as changing
        ...
        // Reset the array pointer, since we'll be starting earlier (but are
        // already populated)
        curr_array_position = 0;
        ...
        // Override the increment -- it implies we just turned on (otherwise,
        // what drove us here?)
      ...
      } else if (prev_vfdState == VFD_CHANGING) // were changing
      {
        ...
        // Do nothing, we're effectively steady state - just flag us for the
        // sake of doing so
    ...
    if (vfdState == VFD_CHANGING) // We're mid-adjust
    {
      ...
      } else // Update like normal
      {
        ...
        if (index_new_limit > settleFreq_length) {
          ...
          // Populate the array
          for (index_val = curr_array_position; index_val < settleFreq_length;
               index_val++) {
        ...
        {
          // Populate the array
          for (index_val = curr_array_position; index_val < index_new_limit;
               index_val++) {
            // Populate our current entry with the desired frequency
            settleFreq[index_val] = desiredFrequency;
          }
    ...
    if (vfdState != VFD_OFF) {
      phasorVal[0] += (2 * PI * currentFrequency - nominal_output_radian_freq) *
                      diff_time_value;
      phasorVal[1] += (2 * PI * currentFrequency - nominal_output_radian_freq) *
                      diff_time_value;
      phasorVal[2] += (2 * PI * currentFrequency - nominal_output_radian_freq) *
                      diff_time_value;
    }
  ...
  {
    // Update efficiency
    // Relate the frequency output to the estimated out put power (as percentage
    // of rated HP)
    ...
    for (index_val = 0; index_val < 8; index_val++) {
      ...
      } else // Others
      {
        ...
        for (index_val_inner = 1; index_val_inner < index_val;
             index_val_inner++) {
    ...
    // Translate the output power to the input - divide it by 3 here too (for
    // per-phase) Assumes everything coalesces onto a common DC bus Zero check
    // the efficiency
    ...
    for (index_val = 0; index_val < 3; index_val++) {
      ...
      if (fNode->voltage[index_val].Mag() == 0.0) {
        ...
        current_in[index_val] =
            ~(powerInElectrical / fNode->voltage[index_val]);
      }
...
TIMESTAMP vfd::presync(TIMESTAMP t0) { return link_object::presync(t0); }
...
TIMESTAMP vfd::postsync(TIMESTAMP t0) {
  ...
  if (vfdState == VFD_CHANGING) {
    ...
    if (curr_array_position <
        settleFreq_length) // Make sure we're not a shoulder
    {
      // Compute time left
...
gld::complex vfd::complex_exp(double angle) {
  ...
  // Mod us down to around 2pi, since the cos and sin functions get a little
  // flaky with big numbers
...
STATUS vfd::alloc_freq_arrays(double delta_t_val) {
  ...
  // See if we were commanded to reallocate -- this would be done on a zero-th
  // pass of interupdate, most likely (or in postupdate, when transitioning out)
  if (force_array_realloc) {
    // Make sure we actually are allocated first
    if (settleFreq != nullptr) {
      // Free the allocation
      gl_free((void **)&settleFreq);
  ...
  if (settleFreq == nullptr) {
    ...
    if (settleFreq_length <= 0) {
      gl_error("vfd:%d %s -- the stable_time value must result in at least 1 "
               "timestep of averaging!",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attempting to allocate the averaging arrays, an invalid length was
      specified.  Make sure your stable_time value is greater than zero!
      */
    ...
    if (settleFreq == nullptr) {
      ...
      gl_error("vfd:%d %s -- failed to allocate dynamic array", obj->id,
               (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      While allocating a dynamic array, an error occurred.  Please try again. If
      the error persists, please submit an issue to the website. */
    ...
    // Success
    return SUCCESS;
  } else // Already done and we weren't forced, so just assume we were called
         // erroneously
  {
    return SUCCESS;
  }
...
SIMULATIONMODE vfd::inter_deltaupdate_vfd(unsigned int64 delta_time,
                                          unsigned long dt,
  ...
  if ((delta_time == 0) && (iteration_count_val == 0) &&
      !interupdate_pos) // First deltamode call
  {
  ...
  // Most of these items were copied from link.cpp's interupdate (need to
  // replicate)
...
STATUS vfd::post_deltaupdate_vfd(void) {
  // Force it as an update -- otherwise it just exits (which works, but
  // technically has too large of an array)
  force_array_realloc = true;
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_vfd(OBJECT *obj, TIMESTAMP t0,
                                         PASSCONFIG pass) {
...
EXPORT SIMULATIONMODE interupdate_vfd(OBJECT *obj, unsigned int64 delta_time,
  ...
  try {
    status = my->inter_deltaupdate_vfd(delta_time, dt, iteration_count_val,
                                       interupdate_pos);
    return status;
  } catch (char *msg) {
    gl_error("interupdate_vfd(obj=%d;%s): %s", obj->id,
...
EXPORT STATUS postupdate_vfd(OBJECT *obj) {
  ...
  try {
    ...
    gl_error("postupdate_vfd(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return status_val;
  }
}
