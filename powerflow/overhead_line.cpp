...
overhead_line::overhead_line(MODULE *mod) : line(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "overhead_line", sizeof(overhead_line),
    ...
    if (gl_publish_function(oclass, "recalc_distribution_line",
                            (FUNCTIONADDR)recalc_overhead_line) == nullptr)
...
int overhead_line::init(OBJECT *parent) {
  ...
  /*  TROUBLESHOOT
  No overhead line configuration was specified.  Please use object
  line_configuration with appropriate values to specify an overhead line
  configuration
  */
  ...
  configuration object.  Please ensure you have a line_configuration object
  selected.
  */
  ...
  if ((!config->line_spacing ||
       !gl_object_isa(config->line_spacing, "line_spacing")) &&
  ...
  /*  TROUBLESHOOT
  The configuration object for the overhead line is missing the line_spacing
...
void overhead_line::recalc(void) {
  ...
  if (config->impedance11 != 0 || config->impedance22 != 0 ||
    ...
    double daap, dabp, dacp, danp, dbbp, dbcp, dbnp, dccp, dcnp, dnnp, diamA,
        diamB, diamC, diamN;
    ...
    // frequency values Per Kersting (4.39) and (4.40)
    if (enable_frequency_dependence) // See if we may be updating due to
                                     // frequency changes
    {
...
#define GMR(ph)                                                                \
  (has_phase(PHASE_##ph) && config->phase##ph##_conductor                      \
       ? object_data<overhead_line_conductor>(config->phase##ph##_conductor)   \
...
#define DIAM(ph)                                                               \
  (has_phase(PHASE_##ph) && config->phase##ph##_conductor                      \
    ...
    gmr_c = GMR(C);
    gmr_n = GMR(N);
    // res_a = (status==IMPEDANCE_CHANGED && (affected_phases&PHASE_A)) ?
    // resistance/miles : RES(A); res_b = (status==IMPEDANCE_CHANGED &&
    // (affected_phases&PHASE_A)) ? resistance/miles : RES(B); res_c =
    // (status==IMPEDANCE_CHANGED && (affected_phases&PHASE_A)) ?
    // resistance/miles : RES(C); res_n = (status==IMPEDANCE_CHANGED &&
    // (affected_phases&PHASE_A)) ? resistance/miles : RES(N);
    res_a = RES(A);
    res_b = RES(B);
    res_c = RES(C);
    res_n = RES(N);
...
#undef DIAM
    ...
    if (use_line_cap) {
      ...
      if (spacing_val == nullptr) {
        ...
        /*  TROUBLESHOOT
        The line spacing values could not be properly mapped.  Capacitance
        values can not be calculated for this line (and other values may be in
        error).  Please specific a proper line spacing and try again.
        */
      }

      // Start with the assumption the capacitance is valid for appropriate
      // phases
      valid_capacitance = true;
    }
    // Defaulted else - not really needed, since if not use_line_cap, these
    // values are irrelevant

    if (has_phase(PHASE_A)) {
      if (gmr_a > 0.0 && res_a > 0.0)
        z_aa = gld::complex(res_a + freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / gmr_a) + freq_additive_term));
      else
        z_aa = 0.0;
      if (has_phase(PHASE_B) && dab > 0.0)
        z_ab = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dab) + freq_additive_term));
      else
        z_ab = 0.0;
      if (has_phase(PHASE_C) && dac > 0.0)
        z_ac = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dac) + freq_additive_term));
      else
        z_ac = 0.0;
      if (has_phase(PHASE_N) && dan > 0.0)
        z_an = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dan) + freq_additive_term));
      else
        z_an = 0.0;
      ...
      if (use_line_cap) {
        if ((diamA > 0.0) && (spacing_val->distance_AtoE > 0.0)) {
          ...
          if (has_phase(PHASE_B)) {
            // Calc distance
            dabp = calc_image_dist(spacing_val->distance_AtoE,
                                   spacing_val->distance_BtoE,
                                   spacing_val->distance_AtoB);
          ...
          if (has_phase(PHASE_C)) {
            // Calculate distance
            dacp = calc_image_dist(spacing_val->distance_AtoE,
                                   spacing_val->distance_CtoE,
                                   spacing_val->distance_AtoC);
          ...
          if (has_phase(PHASE_N)) {
            ...
            danp = calc_image_dist(spacing_val->distance_AtoE,
                                   spacing_val->distance_NtoE,
                                   spacing_val->distance_AtoN);
        ...
        {
          valid_capacitance = false; // Failed one line of it, so don't include
                                     // capacitance anywhere

          gl_warning("Shunt capacitance of overhead line:%s not calculated - "
                     "invalid values",
                     object_header(this)->name);
          /*  TROUBLESHOOT
          While attempting to calculate the shunt capacitance for an overhead
          line, an invalid parameter was encountered. To calculate shunt
          capacitance, ensure the condutor to earth distance for each phase is
          defined, as well as the diameter of that phases's cable.
          */
    ...
    if (has_phase(PHASE_B)) {
      if (gmr_b > 0.0 && res_b > 0.0)
        z_bb = gld::complex(res_b + freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / gmr_b) + freq_additive_term));
      else
        z_bb = 0.0;
      if (has_phase(PHASE_C) && dbc > 0.0)
        z_bc = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dbc) + freq_additive_term));
      else
        z_bc = 0.0;
      if (has_phase(PHASE_N) && dbn > 0.0)
        z_bn = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dbn) + freq_additive_term));
      else
        z_bn = 0.0;
      ...
      if (use_line_cap) {
        if ((diamB > 0.0) && (spacing_val->distance_BtoE > 0.0)) {
          ...
          if (has_phase(PHASE_C)) {
            // Calculate distance
            dbcp = calc_image_dist(spacing_val->distance_BtoE,
                                   spacing_val->distance_CtoE,
                                   spacing_val->distance_BtoC);
          ...
          if (has_phase(PHASE_N)) {
            // Calculate distance
            dbnp = calc_image_dist(spacing_val->distance_BtoE,
                                   spacing_val->distance_NtoE,
                                   spacing_val->distance_BtoN);
    ...
    if (has_phase(PHASE_C)) {
      if (gmr_c > 0.0 && res_c > 0.0)
        z_cc = gld::complex(res_c + freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / gmr_c) + freq_additive_term));
      else
        z_cc = 0.0;
      if (has_phase(PHASE_N) && dcn > 0.0)
        z_cn = gld::complex(freq_coeff_real,
                            freq_coeff_imag *
                                (log(1.0 / dcn) + freq_additive_term));
      else
        z_cn = 0.0;
      ...
      if (use_line_cap) {
        if ((diamC > 0.0) && (spacing_val->distance_CtoE > 0.0)) {
          ...
          if (has_phase(PHASE_N)) {
            // Calculate the distance
            dcnp = calc_image_dist(spacing_val->distance_CtoE,
                                   spacing_val->distance_NtoE,
                                   spacing_val->distance_CtoN);
    ...
    if (has_phase(PHASE_N) && gmr_n > 0.0 && res_n > 0.0) {
      z_nn = gld::complex(res_n + freq_coeff_real,
                          freq_coeff_imag *
                              (log(1.0 / gmr_n) + freq_additive_term));
      z_nn_inv = z_nn ^ (-1.0);
    ...
    // If we have valid capacitance values and line capacitance is turned on
    // then calculate Yabc_mat otherwise just leave is zeroed out.
    ...
    if (valid_capacitance && use_line_cap) {
      ...
      // Now appropriately invert it - scale for frequency, distance, and
      // microSiemens as well as per Kersting (5.14) and (5.15)
               ...
               has_phase(PHASE_C)) // has A & C
      {
        gld::complex detvalue =
            P_mat[0][0] * P_mat[2][2] - P_mat[0][2] * P_mat[2][0];
      ...
      {
        gld::complex detvalue =
            P_mat[0][0] * P_mat[1][1] - P_mat[0][1] * P_mat[1][0];
      ...
      {
        gld::complex detvalue =
            P_mat[1][1] * P_mat[2][2] - P_mat[1][2] * P_mat[2][1];
      ...
      {
        gld::complex detvalue = P_mat[0][0] * P_mat[1][1] * P_mat[2][2] -
                                P_mat[0][0] * P_mat[1][2] * P_mat[2][1] -
                                P_mat[0][1] * P_mat[1][0] * P_mat[2][2] +
                                P_mat[0][1] * P_mat[2][0] * P_mat[1][2] +
                                P_mat[1][0] * P_mat[0][2] * P_mat[2][1] -
                                P_mat[0][2] * P_mat[1][1] * P_mat[2][0];

        // Invert it
        Yabc_mat[0][0] =
            (P_mat[1][1] * P_mat[2][2] - P_mat[1][2] * P_mat[2][1]) / detvalue *
            cap_freq_mult;
        Yabc_mat[0][1] =
            (P_mat[0][2] * P_mat[2][1] - P_mat[0][1] * P_mat[2][2]) / detvalue *
            cap_freq_mult;
        Yabc_mat[0][2] =
            (P_mat[0][1] * P_mat[1][2] - P_mat[0][2] * P_mat[1][1]) / detvalue *
            cap_freq_mult;
        Yabc_mat[1][0] =
            (P_mat[2][0] * P_mat[1][2] - P_mat[1][0] * P_mat[2][2]) / detvalue *
            cap_freq_mult;
        Yabc_mat[1][1] =
            (P_mat[0][0] * P_mat[2][2] - P_mat[0][2] * P_mat[2][0]) / detvalue *
            cap_freq_mult;
        Yabc_mat[1][2] =
            (P_mat[1][0] * P_mat[0][2] - P_mat[0][0] * P_mat[1][2]) / detvalue *
            cap_freq_mult;
        Yabc_mat[2][0] =
            (P_mat[1][0] * P_mat[2][1] - P_mat[1][1] * P_mat[2][0]) / detvalue *
            cap_freq_mult;
        Yabc_mat[2][1] =
            (P_mat[0][1] * P_mat[2][0] - P_mat[0][0] * P_mat[2][1]) / detvalue *
            cap_freq_mult;
        Yabc_mat[2][2] =
            (P_mat[0][0] * P_mat[1][1] - P_mat[0][1] * P_mat[1][0]) / detvalue *
            cap_freq_mult;
      }
  ...
  if (neg_res) {
    gl_warning("INIT: overhead_line:%s has a negative resistance in it's "
    ...
    overhead_line's impedance matrix. While this is numerically possible, it is
    a physical impossibility. This resulted most likely from a improperly
...
void overhead_line::test_phases(line_configuration *config, const char ph) {
    ...
    if (config->impedance11 == 0.0) {
      condCheck = (config->phaseA_conductor &&
                   !gl_object_isa(config->phaseA_conductor,
                                  "overhead_line_conductor", "powerflow"));
  ...
  if (condCheck)
    GL_THROW("invalid conductor for phase %c of overhead line %s", ph,
             obj->name);
  /*	TROUBLESHOOT  The conductor specified for the indicated phase is not
   * necessarily an overhead line conductor, it may be an underground or
   * triplex-line only conductor */

  if (condNotPres)
    GL_THROW("missing conductor for phase %c of overhead line %s", ph,
             obj->name);
  /*  TROUBLESHOOT
  The conductor specified for the indicated phase for the overhead line is
  missing or invalid.
  */
}
 ...
 *
 * * Outputs *
 * dist1_to_2p - distance from line 1 to line 2's image - same as line 2 to line
 * 1's image (isosceles trapezoid)
 */
double overhead_line::calc_image_dist(double dist1_to_e, double dist2_to_e,
                                      double dist1_to_2) {
  ...
  // Diagonals should be the same, since forms isosceles trapezoid - sqrt(c^2 +
  // ab) - a & b are double sides (above ground to its image)
  dist1_to_2p = sqrt(dist1_to_2 * dist1_to_2 +
                     dist1_to_1p * dist2_to_2p); // sqrt(c^2 + ab)
...
static TIMESTAMP sync_overhead_line_impl(OBJECT *obj, TIMESTAMP t0,
                                         PASSCONFIG pass) {
  try {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_overhead_line(OBJECT *obj, TIMESTAMP t0,
                                                   PASSCONFIG pass) {
  return sync_overhead_line_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_overhead_line(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
extern "C" MODULE_API int isa_overhead_line(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_overhead_line_impl(obj, classsname);
}
#endif
...
EXPORT int create_fault_ohline(OBJECT *thisobj, OBJECT **protect_obj,
...
EXPORT int fix_fault_ohline(OBJECT *thisobj, int *implemented_fault,
                            char *imp_fault_name) {
  int retval;
...
EXPORT int clear_fault_ohline(OBJECT *thisobj, int *implemented_fault,
                              char *imp_fault_name) {
  int retval;
