...
impedance_dump::impedance_dump(MODULE *mod)
{
  if (oclass == nullptr)
  {
    ...
    if (gl_publish_variable(
            ...
            "the group ID to output data for (all links if empty)", PT_char256,
            "filename", PADDR(filename), PT_DESCRIPTION,
...
int impedance_dump::init(OBJECT *parent)
{
  ...
  if (filename[0] == '\0')
  {
    ...
    return 0;
    /* TROUBLESHOOT
    No file name was specifed. Please assign a file name to record the line/link
    impedance values in the impedance_dump object.
    */
  }
...
int impedance_dump::dump(TIMESTAMP t)
{
  ...
  if (group[0] == '\0')
  {
    ...
    ohlines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "overhead_line",
                              FT_END); // find all overhead_lines
    ...
    relays = gl_find_objects(FL_NEW, FT_CLASS, SAME, "relay",
                             FT_END); // find all relays
    sectionalizers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "sectionalizer",
                                     FT_END); // find all sectionalizers
    series_reactors = gl_find_objects(FL_NEW, FT_CLASS, SAME, "series_reactor",
                                      FT_END); // find all series_reactors
    switches = gl_find_objects(FL_NEW, FT_CLASS, SAME, "switch",
                               FT_END); // find all switches
    transformers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "transformer",
                                   FT_END); // find all transformers
    tplines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "triplex_line",
                              FT_END); // find all triplex_lines
  ...
  else
  {
    ...
    ohlines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "overhead_line", AND,
                              FT_GROUPID, SAME, group.get_string(), FT_END);
    reclosers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "recloser", AND,
                                FT_GROUPID, SAME, group.get_string(), FT_END);
    regulators = gl_find_objects(FL_NEW, FT_CLASS, SAME, "regulator", AND,
                                 FT_GROUPID, SAME, group.get_string(), FT_END);
    relays = gl_find_objects(FL_NEW, FT_CLASS, SAME, "relay", AND, FT_GROUPID,
                             SAME, group.get_string(), FT_END);
    sectionalizers =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "sectionalizer", AND,
                        FT_GROUPID, SAME, group.get_string(), FT_END);
    series_reactors =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "series_reactor", AND,
                        FT_GROUPID, SAME, group.get_string(), FT_END);
    switches = gl_find_objects(FL_NEW, FT_CLASS, SAME, "switch", AND,
                               FT_GROUPID, SAME, group.get_string(), FT_END);
    transformers =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "transformer", AND, FT_GROUPID,
                        SAME, group.get_string(), FT_END);
    ...
    uglines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "underground_line", AND,
                              FT_GROUPID, SAME, group.get_string(), FT_END);
    capacitors = gl_find_objects(FL_NEW, FT_CLASS, SAME, "capacitor", AND,
                                 FT_GROUPID, SAME, group.get_string(), FT_END);
  }
      ...
      transformers == nullptr && tplines == nullptr && uglines == nullptr)
  {
    ...
    return 0;
    /* TROUBLESHOOT
    No link objects were found in your .glm file. if you specified a group id
    double check to make sure you have link objects with the specified group id.
    */
  }
  ...
  fprintf(fn, "<?xml-stylesheet "
              "href=\"C:\\Projects\\GridLAB-D_Builds\\ticket_"
              "704\\VS2005\\gridlabd-2_0.xsl\" type=\"text/xsl\"?>\n");
  fprintf(fn, "<gridlabd>\n");
  // write time
  ...
  if (fuses != nullptr)
  {
    ...
    while (obj = gl_find_next(fuses, obj))
    {
      ...
      pFuse[index] = object_data<link_object>(obj);
  ...
  if (reclosers != nullptr)
  {
    ...
    while (obj = gl_find_next(reclosers, obj))
    {
      ...
      pRecloser[index] = object_data<link_object>(obj);
  ...
  if (relays != nullptr)
  {
    ...
    while (obj = gl_find_next(relays, obj))
    {
      ...
      pRelay[index] = object_data<link_object>(obj);
  ...
  if (capacitors != nullptr)
  {
    ...
    while (obj = gl_find_next(capacitors, obj))
    {
      ...
      pCapacitor[index] = object_data<capacitor>(obj);
...
EXPORT int create_impedance_dump(OBJECT **obj, OBJECT *parent)
{
  try
  {
    *obj = gl_create_object(impedance_dump::oclass);
    ...
    {
      impedance_dump *my =
          object_data<impedance_dump>(*obj);
...
{
  impedance_dump *my = object_data<impedance_dump>(obj);
  try
  {
...
EXPORT int isa_impedance_dump_impl(OBJECT *obj, char *classname)
{
  return object_data<impedance_dump>(obj)->isa(
      classname);
}

#ifndef __APPLE__
extern "C" MODULE_API int isa_impedance_dump(OBJECT *obj, char *classname) {
  return isa_impedance_dump_impl(obj, classname);
}
#else
extern "C" MODULE_API int isa_impedance_dump(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
  char *classsname = va_arg(args, char *);
  va_end(args);
  return isa_impedance_dump_impl(obj, classsname);
}
#endif
