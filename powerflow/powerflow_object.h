...
// DPC: this is to remain undefined until GS solver method is reconciled with
// reliability module #define SUPPORT_OUTAGES /**< defined when reliability
// module is supported */
...
/* contact flags */
#define OC_CONTACT                                                             \
  0x1000 /**< contact condition flag (must be combined with at least 2         \
            connections */
...
/* open flags */
#define OC_OPEN                                                                \
  0x2000 /**< open condition flag (combined with at least 1 phase connections) \
          */
...
/** Powerflow solution mode indicators
        The solution modes are used to indicate the solution needed for the
 powerflow object.  Whenever the object is in PS_NORMAL state, the powerflow
 solver will use the normal solution method.  PS_OUTAGE is used to indicate that
        there is no expectation the normal solution method will work.  Alternate
 solutions methods may be indicated using other object-specified PS codes
 **/
...
#define NF_ISSOURCE                                                            \
  0x0002 ///< flag indicates node has a source connected directly to it */
...
public:
  ...
  INRUSHINTMETHOD
      inrush_integration_method; /** Set on an individual object level */
#ifdef SUPPORT_OUTAGES
  set condition;        /**< operating condition (see OC codes) */
  enumeration solution; /**< solution code (PS_NORMAL=0, class-specific solution
                           mode code>0) */
#endif
public:
  static CLASS *oclass;
  ...
  /* is_open checks whether particulars phases are open (no connection when
   * there should be one) */
  inline bool is_open(set ph = PHASE_INFO) const {
    return (condition & OC_OPEN) != 0 && (condition & ph) == ph;
  };

  /* is_open_any checks whether any phase is open (no connection when there
   * should be one) */
  inline bool is_open_any(set ph = PHASE_INFO) const {
    return (condition & OC_OPEN) != 0 && (condition & ph) != 0;
  };

  /* is_contact_any checks whether particulars phases are contacting another */
  inline bool is_contact(set ph = PHASE_INFO) const {
    return (condition & OC_CONTACT) != 0 && (condition & ph) == ph;
  };

  /* is_contact_any checks whether any phase is contacting another */
  inline bool is_contact_any(set ph = PHASE_INFO) const {
    return (condition & OC_CONTACT) != 0 && (condition & ph) != 0;
  };
#endif
  ...
  inline bool has_phase(gld::set ph, gld::set flags = 0) const {
    return ((flags ? flags : phases) & ph) == ph;
  };

  /* phase_index returns the array index of a phase if configured (use flags to
   * test another variable) */
  inline unsigned int phase_index(gld::set ph = 0) const {
    return ((unsigned int)((ph ? ph : phases) & PHASE_ABC) >> 2) % 3;
  };

  /* get_phases acquires the phase part of the flags given */
  inline gld::set get_phases(gld::set flags) const {
    return flags & PHASE_INFO;
  };

  inline OBJECT *objecthdr() const { return object_header(this); }
...
public:
  ...
  int isa_powerflow_object(OBJECT *obj, char *classname);
