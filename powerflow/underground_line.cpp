...
#define UG_GET(ph, name) (has_phase(PHASE_##ph) && config->phase##ph##_conductor ? object_data<underground_line_conductor>(config->phase##ph##_conductor)->name : 0)
		...
		GMRCN(5) = !(has_phase(PHASE_B) && strands_5 > 0) ? 0.0 : pow(GMR(5) * strands_5 * pow(rad_25, (strands_5 - 1)), (1.0 / strands_5));
		GMRCN(6) = !(has_phase(PHASE_C) && strands_6 > 0) ? 0.0 : pow(GMR(6) * strands_6 * pow(rad_36, (strands_6 - 1)), (1.0 / strands_6));

		// Check to see if this is not a tape-shielded line or a concentric neutral line

		if (GMR(4) == 0.0 && GMR(5) == 0.0 && GMR(6) == 0.0 && GMR_S(4) == 0.0 && GMR_S(5) == 0.0 && GMR_S(6) == 0.0)
		{ // the gmr for the neutral strands and the tape shield is zero this is just an insulated underground cable
			not_TS_CN = true;
		}
...
#define DIST(ph1, ph2) (has_phase(PHASE_##ph1) && has_phase(PHASE_##ph2) && config->line_spacing ? object_data<line_spacing>(config->line_spacing)->distance_##ph1##to##ph2 : 0.0)
...
static TIMESTAMP sync_underground_line_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	try
	{
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_underground_line(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
