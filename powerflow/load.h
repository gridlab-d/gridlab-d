...
private:
  ...
  bool base_load_val_was_nonzero[3]; ///< Tracking variable to make ZIP-fraction
                                     ///< loads check for zero conditions (but
                                     ///< not already zeroed)

  gld::complex prev_load_values[3][3]; ///< Tracking variable for accumulators -
                                       ///< make loads behave more like nodes
  gld::complex prev_load_values_dy[3]
                                  [6]; ///< Tracking variable for accumulators -
                                       ///< full connection - make loads behave
                                       ///< more like nodes.
...
public:
  ...
  gld::complex constant_power_dy[6]; // Power load, explicitly specified wye and
                                     // delta -- delta first, then wye
  gld::complex constant_current_dy[6]; // Current load, explicitly specified wye
                                       // and delta -- delta first, then wye
  gld::complex
      constant_impedance_dy[6]; // Impedance load, explicitly specified wye and
                                // delta -- delta first, then wye
  ...
  SIMULATIONMODE inter_deltaupdate_load(unsigned int64 delta_time,
                                        unsigned long dt,
                                        unsigned int iteration_count_val,
                                        bool interupdate_pos);
