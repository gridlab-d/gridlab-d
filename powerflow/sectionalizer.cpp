...
sectionalizer::sectionalizer(MODULE *mod) : switch_object(mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "sectionalizer", sizeof(sectionalizer),
    ...
    if (gl_publish_variable(oclass, PT_INHERIT, "switch", nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);

    if (gl_publish_function(oclass, "change_sectionalizer_state",
                            (FUNCTIONADDR)change_sectionalizer_state) ==
        nullptr)
      GL_THROW("Unable to publish sectionalizer state change function");
    if (gl_publish_function(
            oclass, "sectionalizer_reliability_operation",
            (FUNCTIONADDR)sectionalizer_reliability_operation) == nullptr)
      GL_THROW(
          "Unable to publish sectionalizer reliability operation function");
    if (gl_publish_function(oclass, "change_sectionalizer_faults",
                            (FUNCTIONADDR)sectionalizer_fault_updates) ==
        nullptr)
    ...
    if (gl_publish_function(oclass, "update_power_pwr_object",
      ...
      GL_THROW("Unable to publish sectionalizer external power calculation "
               "function");
    if (gl_publish_function(oclass, "check_limits_pwr_object",
                            (FUNCTIONADDR)calculate_overlimit_link) == nullptr)
      GL_THROW("Unable to publish sectionalizer external power limit "
               "calculation function");
    if (gl_publish_function(oclass, "perform_current_calculation_pwr_link",
                            (FUNCTIONADDR)currentcalculation_link) == nullptr)
      GL_THROW("Unable to publish sectionalizer external current calculation "
               "function");
  }
}

int sectionalizer::isa(char *classname) {
  return strcmp(classname, "sectionalizer") == 0 ||
         switch_object::isa(classname);
}
...
int sectionalizer::create() {
  ...
  switch_banked_mode =
      BANKED_SW;             // Sectionalizer starts with all three phases open
...
static TIMESTAMP sync_sectionalizer_impl(OBJECT *obj, TIMESTAMP t0,
                                         PASSCONFIG pass) {
  try {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_sectionalizer(OBJECT *obj, TIMESTAMP t0,
                                                   PASSCONFIG pass) {
  return sync_sectionalizer_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_sectionalizer(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
EXPORT double change_sectionalizer_state(OBJECT *thisobj,
                                         unsigned char phase_change,
                                         bool state) {
  ...
  if (!state &&
      !restoration_checks_active) // Check routine to find a recloser (or we're
                                  // in reconfiguration mode)
  {
    ...
    if (fault_check_object == nullptr) {
      ...
      /*  TROUBLESHOOT
      A sectionalizer attempted to call a reliability-related function. However,
      this function requires a fault_check object to be present in the system.
      Please add the appropriate object. If the error persists, please submit
    ...
    funadd = (FUNCTIONADDR)(gl_get_function(fault_check_object,
                                            "handle_sectionalizer"));

    // make sure it worked
    if (funadd == nullptr) {
      GL_THROW("Failed to find sectionalizer checking method on object %s",
               fault_check_object->name);
      /*  TROUBLESHOOT
      While attempting to find the fault check method, or its subfunction to
      handle sectionalizers, an error was encountered.  Please ensure a proper
      fault_check object is present in the system. If the error persists, please
      submit your code and a bug report via the trac website.
      */
    }

    // Function call
    recloser_count = ((double (*)(OBJECT *, int))(*funadd))(
        fault_check_object, sectionobj->NR_branch_reference);

    if (recloser_count == 0.0) // Failed :(
    {
      GL_THROW("Failed to handle sectionalizer check on %s", thisobj->name);
      /*  TROUBLESHOOT
      While attempting to handle sectionalizer actions for the specified device,
      an error occurred.  Please try again and ensure all parameters are
      correct.  If the error persists, please submit your code and a bug report
  ...
  if (perform_operation) // Either is a "replace" or a recloser was found -
                         // operation is a go
  {
    // Map the switch
...
EXPORT int sectionalizer_reliability_operation(OBJECT *thisobj,
...
EXPORT int sectionalizer_fault_updates(OBJECT *thisobj,
