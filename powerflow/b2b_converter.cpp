...
//initialize pointers
CLASS* b2bc::oclass = NULL;
CLASS* b2bc::pclass = NULL;

//////////////////////////////////////////////////////////////////////////
// b2bc CLASS FUNCTIONS
//////////////////////////////////////////////////////////////////////////

b2bc::b2bc(MODULE *mod) : link_object(mod)
{
	if(oclass == NULL)
	{
		pclass = link_object::oclass;
		
		oclass = gl_register_class(mod,"b2bc",sizeof(b2bc),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==NULL)
			throw "unable to register class b2bc";
		else
        ...
        if(gl_publish_variable(oclass,
			...
			PT_double, "desired_output_frequency[Hz]", PADDR(desiredFrequency), PT_DESCRIPTION, "b2bc desired output frequency based on the desired RPM",
			PT_double, "current_output_frequency[Hz]", PADDR(currentFrequency), PT_DESCRIPTION, "b2bc currently output frequency",
			PT_double, "efficiency[%]", PADDR(currEfficiency), PT_DESCRIPTION, "Current b2bc efficiency based on the load/b2bc output Horsepower",
			
			PT_double, "stable_time[s]", PADDR(stableTime), PT_DESCRIPTION, "Time taken by the b2bc to reach desired frequency (based on RPM). Default = 1.45 seconds",
			
			PT_enumeration, "b2bc_state", PADDR(b2bcState), PT_DESCRIPTION, "Current state of the b2bc",
				PT_KEYWORD, "OFF", (enumeration)b2bc_OFF,
				PT_KEYWORD, "CHANGING", (enumeration)b2bc_CHANGING,
				PT_KEYWORD, "STEADY_STATE", (enumeration)b2bc_STEADY,
			
			NULL) < 1) GL_THROW("unable to publish properties in %s",__FILE__);
		// functions that interact with other GLD objects (mainly delta mode and powerflow solutions)
		//Publish deltamode functions
		if (gl_publish_function(oclass,	"interupdate_pwr_object", (FUNCTIONADDR)interupdate_b2bc)==NULL)
			GL_THROW("Unable to publish b2bc deltamode function");
		if (gl_publish_function(oclass,	"postupdate_pwr_object", (FUNCTIONADDR)postupdate_b2bc)==NULL)
			GL_THROW("Unable to publish b2bc deltamode function");

		//Publish restoration-related function (current update)         // access to internal funcitons to get interim solution
		if (gl_publish_function(oclass,	"update_power_pwr_object", (FUNCTIONADDR)updatepowercalc_link)==NULL)
			GL_THROW("Unable to publish b2bc external power calculation function");
		if (gl_publish_function(oclass,	"check_limits_pwr_object", (FUNCTIONADDR)calculate_overlimit_link)==NULL)
			GL_THROW("Unable to publish b2bc external power limit calculation function");
		if (gl_publish_function(oclass,	"perform_current_calculation_pwr_link", (FUNCTIONADDR)currentcalculation_link)==NULL)
			GL_THROW("Unable to publish b2bc external current calculation function");

		//Publish external b2bc interfacing function
		if (gl_publish_function(oclass,	"b2bc_current_injection_update", (FUNCTIONADDR)current_injection_update_b2bc)==NULL)
			GL_THROW("Unable to publish b2bc external current injection calculation function");
    }
}

int b2bc::isa(char *classname)
{
	return strcmp(classname,"b2bc")==0 || link_object::isa(classname);
}

int b2bc::create() // defaults... called before glm is read
{
	...
	motorPoles = 4;
	voltageLLRating = -1.0;
	horsePowerRatedb2bc = 75;
	nominal_output_frequency = -1.0; // example of a flag value to check if it is still -1, use the default value if the user did not set it.... or inform user to set if needed
	stableTime = 1.45;

	b2bcState = b2bc_OFF;
	prev_b2bcState = b2bc_OFF;

	desiredFrequency = 0.0;  // forces these variables ot a known value
	...
	//http://www.variablefrequencydrive.org/b2bc-efficiency
...
*/
int b2bc::init(OBJECT *parent)  // the glam values are read before th object initialization and checking of glm properties and default values and one time calculations
{
	...
	if (stableTime <= 0.0)
	{
		GL_THROW("b2bc:%d - %s - the stableTime must be positive",obj->id,(obj->name ? obj->name : "Unnamed")); // will stop simulatoin
		/*  TROUBLESHOOT
		The stableTime value for the b2bc is negative.  This must be a positive value for the simulation to work
		properly.
		*/
	}
	...
	if (voltageLLRating <= 0.0)
	{
		...
		gl_warning("b2bc:%d - %s - rated_output_voltage was not set, defaulting to TO node nominal",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		The rated_output_voltage was not set for the b2bc.  It has been defaulted to the nominal_voltage value of the TO node.
		*/
	}
	else	//Was specified
	{
		...
		{
			gl_warning("b2bc:%d - %s - rated_output_voltage does not match the TO node nominal voltage",obj->id,(obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
	...
	{
		gl_warning("b2bc:%d - %s - FROM node nominal_voltage is not equal to or greater than TO node nominal_voltage - implies DC-DC converter!e",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
	...
	{
		gl_warning("b2bc:%d - %s - nominal_output_frequency is not set or is negative - defaulting to system nominal",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
	...
	if (ratedRPM <= 0.0)
	{
		GL_THROW("b2bc:%d - %s - rated_motor_speed must be positive and non-zero!",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
	...
	if (motorPoles <= 0.0)
	{
		GL_THROW("b2bc:%d - %s - motor_poles must be positive and non-zero!",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
	...
	if (horsePowerRatedb2bc <= 0.0)
	{
		GL_THROW("b2bc:%d - %s - rated_horse_power must be positive and non-zero!",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
	...
	{
		GL_THROW("b2bc:%d - %s - Must be three-phase!",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		The b2bc only operates as a three-phase device, at this time.  Please specify all phases.
		 */
	...
	if (b2bcState == b2bc_STEADY)
	{
		//Update trackers
		prev_desiredRPM = desiredRPM;
		prev_b2bcState = b2bcState;
	...
	{
		GL_THROW("b2bc:%d - %s - Allocating the dynamic arrays for the frequency tracking failed",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		While attempting to allocate the dynamic length arrays for the b2bc operation, an issue was encountered.  Please try again.  If the error
		persists, please submit an issue. */
		
	}
	
	//Map our "TO" node as a proper b2bc
	temp_fxn = (FUNCTIONADDR)(gl_get_function(to,"attach_b2bc_to_pwr_object"));

	//Make sure it worked
	if (temp_fxn == NULL)
	{
		GL_THROW("b2bc:%d - %s -- Failed to map TO-node flag function",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		While attempting to update the flagging on the "TO" node to reflect being attached to a b2bc, an error occurred.
...
void b2bc::CheckParameters() // akb-can be very useful for time varying parameters and checking multiple times and generalize...
{
	OBJECT *obj = OBJECTHDR(this);
	...
	if (desiredRPM < 0.0)
	{
		GL_THROW("b2bc:%d - %s - the desired_motor_speed must be positive",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		The desired_motor_speed value for the b2bc is negative.  This must be a positive value for the simulation to work
	...
	{
		gl_warning("b2bc:%d - %s - the desired_motor_speed results in a frequency of less than 3.0 Hz",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		The desired_motor_speed results in a value that is less than the starting frequency of a typical b2bc.  This is high discouraged, since it may
	...
	{
		gl_warning("b2bc:%d - %s - b2bc currently at %f%% of rated speed -- b2bcs perform best when running at >= 75 percent output",obj->id,(obj->name ? obj->name : "Unnamed"),(desiredRPM*100/ratedRPM));
		/*  TROUBLESHOOT
		Recommended to run a b2bc at 75%.  Lower values are not recommended and can result in odd behavior
		*/
	}
}

//Function to perform the current update - called by a node object, after they've updated (so current is accurate)
STATUS b2bc::b2bc_current_injection(void) // akb- might need to be changed  a bit
{
	...
	if (curr_time_value != prev_time_value)	//Updates only occur for new time steps
	{
		...
		if (b2bcState == b2bc_STEADY)
		{
			...
			if (prev_b2bcState == b2bc_OFF)	//Were off, so need to actually be starting up
			{
				//flag us as changing
				b2bcState = b2bc_CHANGING;
			...
			else if (prev_b2bcState == b2bc_CHANGING)	//were changing
			{
		...
		if (b2bcState == b2bc_CHANGING)	//We're mid-adjust
		{
			//See if we've changed values or not
			if ((prev_b2bcState == b2bc_CHANGING) && (desiredChange == true))
			{
		...
		//Update the b2bc phasor output, if appropriate
		if (b2bcState != b2bc_OFF)
		{
...
TIMESTAMP b2bc::presync(TIMESTAMP t0)  // akb- QSTS update presync - time/ accumulator reset; syc- powerflow step; post sync - after powerflow is updating dependent variables... (pre- reset; sync forward sweep; postsync-backward sweep)
{
	return link_object::presync(t0);
}

TIMESTAMP b2bc::sync(TIMESTAMP t0)
{
	TIMESTAMP t2;
...
TIMESTAMP b2bc::postsync(TIMESTAMP t0)
{
	TIMESTAMP t1, tret;
	int time_left;
...
complex b2bc::complex_exp(double angle) //akb- evaluate if required
{
	complex output_val;
...
STATUS b2bc::alloc_freq_arrays(double delta_t_val)   //akb- evaluate if required - -similar to the check parameters
{
	OBJECT *obj = OBJECTHDR(this);
	int a_index;
	...
	if (settleFreq == NULL)
	{
		...
		{
			gl_error("b2bc:%d %s -- the stable_time value must result in at least 1 timestep of averaging!",obj->id,(obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
		...
		if (settleFreq == NULL)
		{
			...
			gl_error("b2bc:%d %s -- failed to allocate dynamic array",obj->id,(obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
...
SIMULATIONMODE b2bc::inter_deltaupdate_b2bc(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val,bool interupdate_pos)
{
	double dt_value, deltatimedbl;
	STATUS ret_value;

	//See if we're the very first pass/etc
	if ((delta_time == 0) && (iteration_count_val == 0) && (interupdate_pos == false))	//First deltamode call // akb- working with the speciic timestep, initializing diff eqns, etc.
	{
	...
	else	//After the call
	{
		...
		if (b2bcState == b2bc_CHANGING)	//Still adjusting, so bring us back
		{
			return SM_DELTA;
		}
		else
		{
			return SM_EVENT;	//b2bc always just want out // akb to re-iterate in deltam mode SM_ITER will be used - refer diesel dg
		}
	}
}//End module deltamode

//Post update deltamode
STATUS b2bc::post_deltaupdate_b2bc(void)
{
...
{
	b2bc *b2bcObj = OBJECTDATA(obj,b2bc);

	return b2bcObj->b2bc_current_injection();
}

//////////////////////////////////////////////////////////////////////////
// IMPLEMENTATION OF CORE LINKAGE: b2bc
//////////////////////////////////////////////////////////////////////////
...
*/
EXPORT int create_b2bc(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(b2bc::oclass);
		if (*obj!=NULL)
		{
			b2bc *my = OBJECTDATA(*obj,b2bc);
...
* @return 1 on success, 0 on error
*/
EXPORT int init_b2bc(OBJECT *obj)
{
	try {
		b2bc *my = OBJECTDATA(obj,b2bc);
...
*/
EXPORT TIMESTAMP sync_b2bc(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	try
	{
		b2bc *pObj = OBJECTDATA(obj,b2bc);
...
EXPORT int isa_b2bc(OBJECT *obj, char *classname)
{
	return OBJECTDATA(obj,b2bc)->isa(classname);
}
...
EXPORT SIMULATIONMODE interupdate_b2bc(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val, bool interupdate_pos)
{
	...
	try
	{
		status = my->inter_deltaupdate_b2bc(delta_time,dt,iteration_count_val,interupdate_pos);
		return status;
	}
	catch (char *msg)
	{
		gl_error("interupdate_b2bc(obj=%d;%s): %s", obj->id, obj->name?obj->name:"unnamed", msg);
		return status;
	}
}
...
EXPORT STATUS postupdate_b2bc(OBJECT *obj)
{
	...
	try
	{
		status_val = my->post_deltaupdate_b2bc();
		return status_val;
	}
	catch (char *msg)
	{
		gl_error("postupdate_b2bc(obj=%d;%s): %s", obj->id, obj->name?obj->name:"unnamed", msg);
		return status_val;
	}
}
