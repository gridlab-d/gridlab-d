...
triplex_line::triplex_line(MODULE *mod) : line(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "triplex_line", sizeof(triplex_line),
    ...
    if (gl_publish_function(oclass, "recalc_distribution_line",
                            (FUNCTIONADDR)recalc_triplex_line) == nullptr)
...
int triplex_line::init(OBJECT *parent) {
  ...
  if (!has_phase(PHASE_S))
    gl_warning("%s (%s:%d) is triplex but doesn't have phases S set", obj->name,
               obj->oclass->name, obj->id);
  /*  TROUBLESHOOT
  A triplex line has been used, but this triplex line does not have the phase S
  designated to indicate it contains single-phase components.  Without this
  specified, you may get invalid results.
  */
  ...
  if (temp_config->phaseA_conductor != nullptr ||
      temp_config->phaseB_conductor != nullptr) {
...
void triplex_line::phase_conductor_checks(void) {
  ...
  OBJECT *obj = object_header(this);
  triplex_line_configuration *line_config =
      object_data<triplex_line_configuration>(configuration);
  ...
  if ((line_config->impedance11 == 0.0) && (line_config->impedance22 == 0.0)) {
    ...
    if (line_config->phaseA_conductor != nullptr) // 1
    {
      ...
      if (!gl_object_isa(line_config->phaseA_conductor,
                         "triplex_line_conductor", "powerflow")) {
        GL_THROW("triplex_line:%d - %s - configuration does not use a "
                 "triplex_line_conductor for at least one phase!",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        A triplex_line has an object specified for conductor_1, conductor_2, or
        conductor_N that is not a triplex_line_conductor object. Fix this and
        try again.
        */
      }
      // Default else -- it's a valid conductor
    } else {
      GL_THROW("triplex_line:%d - %s - configuration doesn't have a valid "
               "phase conductor!",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLSHOOT
      A triplex_line's triplex_line_configuration does not have the proper phase
      specified for the conductor
       */
    }
    ...
    {
      // Make sure it is a valid conductor
      if (!gl_object_isa(line_config->phaseB_conductor,
                         "triplex_line_conductor", "powerflow")) {
        GL_THROW("triplex_line:%d - %s - configuration does not use a "
                 "triplex_line_conductor for at least one phase!",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
    ...
    {
      // Make sure it is a valid conductor
      if (!gl_object_isa(line_config->phaseC_conductor,
                         "triplex_line_conductor", "powerflow")) {
        GL_THROW("triplex_line:%d - %s - configuration does not use a "
                 "triplex_line_conductor for at least one phase!",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
...
void triplex_line::recalc(void) {
  ...
  if (line_config->impedance11 != 0.0 || line_config->impedance22 != 0.0) {
    gl_warning("Using a 2x2 z-matrix, instead of geometric values, is an "
               "under-determined system. Ground and/or neutral currents will "
               "be incorrect.");
    ...
    triplex_line_conductor *l1 =
        object_data<triplex_line_conductor>(line_config->phaseA_conductor);
    triplex_line_conductor *l2 =
        object_data<triplex_line_conductor>(line_config->phaseB_conductor);
    triplex_line_conductor *lN =
        object_data<triplex_line_conductor>(line_config->phaseC_conductor);

    if (l1 == nullptr || l2 == nullptr || lN == nullptr) {
      GL_THROW("triplex_line_configuration:%d (%s) is missing a conductor "
               "specification.",
               line_config->get_id(), line_config->get_name());
      /* TROUBLESHOOT
      At this point, triplex lines are assumed to have three conductor values:
      conductor_1, conductor_2, and conductor_N.  If any of these are missing,
      the triplex line cannot be specified.  Please verify that your
      triplex_line_configuration object contains all of the neccessary conductor
      values.
      */
    }
    ...
    if (D12 <= 0.0 || D13 <= 0.0) {
      GL_THROW("triplex_line_configuration diameter and/or "
               "insulation_thickness are incorrectly set. Please set both of "
               "these values to a positive value.");
      /* TROUBLESHOOT
      The triplex line configuration requires that the spacing between
      conductors (diameter + 2*insulation_thickness) & (diameter +
      insulation_thickness) must be positive values.  Please look at your
      triplex_line_configuration to verify that one or both of these variables
      are set to positive values.  A good resource for the geometrical
      configuration is William H. Kersting, "Distribution System Modeling and
      Analysis, 3rd Ed.", Chapter 11.
      */
    }

    zp11 = gld::complex(r1, 0) + freq_coeff_real +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / gmr1) + freq_additive_term);
    zp22 = gld::complex(r2, 0) + freq_coeff_real +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / gmr2) + freq_additive_term);
    zp33 = gld::complex(rn, 0) + freq_coeff_real +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / gmrn) + freq_additive_term);
    zp12 = gld::complex(freq_coeff_real, 0.0) +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / D12) + freq_additive_term);
    zp13 = gld::complex(freq_coeff_real, 0.0) +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / D13) + freq_additive_term);
    zp23 = gld::complex(freq_coeff_real, 0.0) +
           gld::complex(0.0, freq_coeff_imag) *
               (log(1 / D23) + freq_additive_term);
    ...
    if ((solver_method == SM_FBS) || (solver_method == SM_NR)) {
      ...
      /*  TROUBLESHOOT
      While computing the impedance values for a triplex_line, an invalid solver
      method was used. Please use only the supported solvers (FBS and NR), or
      adjust the triplex_line code to accommodate your new method.
      */
    }

    // No solver check here -- NR and FBS are the same, and if it is something
    // else, it should have failed above
    tn[0] = -zp13 / zp33;
    tn[1] = -zp23 / zp33;
    tn[2] = 0;

    multiply(length / 5280.0, zs,
             b_mat); // Length comes in ft, convert to miles.
  ...
  if (neg_res) {
    gl_warning("INIT: triplex_line:%s has a negative resistance in it's "
    ...
    triplex_line's impedance matrix. While this is numerically possible, it is a
    physical impossibility. This resulted most likely from a improperly defined
    conductor cable. Please check and modify the conductor objects used for this
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_triplex_line(OBJECT *obj, TIMESTAMP t0,
                                                  PASSCONFIG pass) {
