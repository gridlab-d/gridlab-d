...
public:
  enumeration
      control_method;    ///< Control method for IVVC controller (On or off)
  ...
  double d_max; // Scaling constant for capacitor switching on - typically 0.3 -
                // 0.6
  double d_min; // Scaling constant for capacitor switching off - typically 0.1
                // - 0.4
  OBJECT *substation_lnk_obj; // Power factor measurement link
  char1024 regulator_list;    // List of regulators, separated by commas
  char1024
      capacitor_list; // List of controllable capacitors, separated by commas
  char1024
      measurement_list; // List of available voltage measurement points,
                        // separated by commas with the regulator index between
  char1024
      minimum_voltage_txt; // List of minimum allowable voltage of the system
  char1024
      maximum_voltage_txt; // List of maximum allowable voltage of the system
  ...
  bool pf_signed; // Flag to indicate if a signed pf value should be maintained,
                  // or just a "deadband around 1"
...
private:
  VOLTVARSTATE
      prev_mode; // Previous mode of Volt-VAr controller - used to transitions
  ...
  node ***pMeasurement_list; // Measurement points - they are assumed to be
                             // nodes at some level
  node **RegToNodes; // To Node (Load side) of regulators - for voltage VO
                     // measurements
  ...
  bool Regulator_Change; // Flag to indicate a regulator change is in progress -
                         // used to hold off VAr-based switching
  double *reg_step_up;   // Regulator step for upper taps
  double *reg_step_down; // Regulator step for lower taps (may be same as
                         // reg_step_up)
  ...
  bool TUpdateStatus; // Status variable for TUpdate - used for transitioning
                      // between Active/Standby states
  bool first_cycle;
  regulator_configuration::Control_enum
      *PrevRegState;                   // Previous state of the regulators
  capacitor::CAPCONTROL *PrevCapState; // Previous state of the capacitors
  TIMESTAMP prev_time;
  void size_sorter(
      double *cap_size, int *cap_Index, int cap_num, double *temp_cap_size,
      int *temp_cap_Index); // Capacitor size sorting function (recursive)
  char *
  dbl_token(char *start_token,
            double *dbl_val); // Function to parse a comma-separated list to get
                              // the next double (or the last double)
  char *
