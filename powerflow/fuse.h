...
public:
  ...
  TIMESTAMP postsync(
      TIMESTAMP t0); // Legacy FBS coding in here - may not be needed in future
  fuse(MODULE *mod);
  ...
  void set_fuse_full(
      ...
      char desired_status_C); // Used to set individual phases - 0 = blown, 1 =
                              // good, 2 = don't care (retain current)
  void set_fuse_full_reliability(unsigned char desired_status);
  void set_fuse_faulted_phases(unsigned char desired_status);
  void
  fuse_sync_function(void); // Functionalized since it exists in two spots - no
                            // sense having to update two pieces of code
  OBJECT **get_object(
      OBJECT *obj,
  ...
  unsigned char
      phased_fuse_status; // Used to track individual phase fuse status - mainly
                          // for reliability - use LSB - x0_XABC
  unsigned char faulted_fuse_phases; // Used for phase faulting tracking -
