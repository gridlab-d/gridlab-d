...
typedef struct s_CandSwOpe {
  int *data_1; // Column 1
               // -- Switch to be opened (1)
  int *data_2; // Column 2 - semi-silly to do it this way, but I have reasons
               // -- Switch to be opened (2)
  int *data_3; // Column 3 - Expanded silly, but makes the ML transition easier
               // -- Switch to be closed (1)
  int *data_4; // Column 4 - Expanded silly, but makes the ML transition easier
               // -- Switch to be closed (2)
  int *data_5; // Column 5 - Expanded silly, but makes the ML transition easier
               // -- previous cyclic interfhange operation
  double *data_6; // Column 6 - Expanded silly, but makes the ML transition
                  // easier -- Amount of loads not able to be restored or
                  // overload.  Not sure
  int *data_7; // Column 7 - Expanded silly, but makes the ML transition easier
               // -- Feeder number overloaded?
  int currSize;
  int maxSize;
} CANDSWOP;
...
public:                   // member functions
  ...
  bool modify(int oldData, int newData); // replace oldData with newData and
                                         // return 1.  If no oldData, return 0
  void addNode(int kIndex, int newData); // add new node after kth node
  void deleteNode(int dData); // delete 1st node found containing dData.  If not
                              // found, do nothing
  void copy(Chain *ilist);  // copy a linked list into a new linked list
  void append(int newData); // add a new node containing newData at the end of
                            // the linked list
};
...
public:                // class constructor
  inline ChainIterator() { location = NULL; }

public:                         // member functions
  int initialize(Chain *lList); // initializes the iterator, takes as argument
                                // lList of class Chain
...
public:                    // member properties
  ...
  int *status_value; // should be enumeration - 0:not discovered, 1:discovered
                     // but not finished, 2:finished
  int *parent_value; // the parents of all vertices.  -1 means no parent
  double *dist;      // distance for the source to each vertext (For
                // Breath-First-Search or BFS)
  TIMESTAMP dfs_time; // Previous timestamp - mainly for intialization
  TIMESTAMP *dTime;   // timestamp array I for DFS, records when each vertex is
                    // discovered
  TIMESTAMP *fTime; // timestamp array II for DFS, records when the search
                    // finishes exaining the adjacency list for each vertex
public:                       // member functions
  ...
  void DFS2(int s); // creates a depth-first tree with a specificied root -
                    // connected graph only
  void DFSVisit(int u); // visits vertices in depth first search and marks them
                        // as visited; u is a vertex
  void copy(LinkedBase *graph1); // copy a graph, but only vertices, edges, and
                                 // adjacency.  Properties for iterator and
                                 // graph traversal excluded
...
public:
  ...
  bool isEdgeExisting(int iIndex,
                      int jIndex); // determine if edge <i,j> esists in graph
...
public:
  ...
  void findFunCutSet(CHORDSET *chordSet, BRANCHVERTICES *tBranch,
                     CHORDSET *cutSet); // find fundamental cut set.
  void simplify(CHORDSET *chordset, BRANCHVERTICES *fBranch, int source,
                INTVECT *fVer, INTVECT *vMap); // graph simplification
  // void unique_int(INTVECT *inputvect, INTVECT *outputvect);
  // //Outputvect is unique elements of input vector (sorted)
  void find_int(INTVECT *inputvect, INTVECT *foundvect, int findval,
                int numfind); // foundvect is the matching entries of inputvect
  // void merge_sort_int(int *Input_Array, unsigned int Alen, int *Work_Array);
  // //Merge sort - stolen from solver_nr
  ...
  void
  mergeVer_2(INTVECT *vMap); // merge vertices according to the map of vertices
};
...
public:
  int reconfig_attempts; // Number of reconfigurations/timestep to try before
                         // giving up
  int reconfig_iter_limit; // Number of iterations to let PF go before flagging
                           // this as a bad reconfiguration
  ...
  char1024 feeder_power_limit_Pub; // Published string for feeder_power_limit
                                   // (to be parsed)
  char1024 feeder_power_link_Pub; // Published string for feeder_power_link (to
                                  // be parsed)

  char1024 mVerPub; // Published string for mVer object list (to be parsed)
  char1024 microgrid_limit_Pub; // Published string for microgrid_limit (to be
                                // parsed)
  char1024 microgrid_power_link_Pub; // Published string for
                                     // microgrid_power_link (to be parsed)

  char1024 logfile_name; // Filename for output file of restoration -- if
                         // desired

  double voltage_limit[2]; // Lower and upper limits of load voltages

  bool stop_and_generate; // Flag to either perform the base-WSU functionality
                          // (check all scenarios), or to just do a "first
                          // solution exit" approach False = GLD approach (exit
                          // when first valid reconfig found), true = WSU MATLAB
                          // (generate all)

  // I/O functions for GLD Interface
  int PerformRestoration(
      int faulting_link); // Base function - similar to main class of MATLAB
                          // (called by fault_check)
...
private:
  ...
  INTVECT feederVertices; // ID of the start vertices of feeders in the graph
  ...
  FUNCTIONADDR *fLinkPowerFunc; // Feeder "power update" function list (matches
                                // fLinkObjList)
  double *feeder_power_limit; // Feeder power limits - must match number of
                              // feeder vertices (feeders) - parsed from string
  gld::complex **feeder_power_link_value; // Feeder power output - linked from
                                          // parsed feeder list

  int numMG; // Number of microgrids
  INTVECT
      MGIdx; // Microgrid vertices - specify by object name and parse (string)
  ...
  FUNCTIONADDR *mLinkPowerFunc; // Microgrid "power update" function list
                                // (matches fLinkObjList)
  gld::complex *
      microgrid_limit; // Microgrid power limit - must match number of microgrid
                       // vertices - parsed from string (real & reactive)
  gld::complex **microgrid_power_link_value; // Microgrid power output - linked
                                             // from parsed microgrid list

  FUNCTIONADDR
      fault_check_fxn; // Function address for "reliability_alterations"
                       // function - meant to do topology check after a
  ...
  LinkedUndigraph *top_sim_1; // Graph object for topology after first
                              // simplification -- all non-switch edges deleted
  LinkedUndigraph
      *top_sim_2; // Graph object for topology after second simplification --
                  // vertices whose degrees are 1 or 2 are deleted
  ...
  CHORDSET
      sec_swi; // Set of sectionalizing switches (normally closed), n*2 matrix
  ...
  CHORDSET
      sec_list; // Copy of sec_swi for iterations -- not sure if really needed

  CHORDSET sec_swi_map;   //???
  CHORDSET sec_swi_map_1; //???

  LOCSET tie_swi_loc; // Location of tie switches status in original GLM file
  LOCSET sec_swi_loc; // Location of sectionalizing switches status in original
                      // GLM file
  ...
  INTVECT ver_map_1; // Map between the vertices of the original tree and those
                     // of the 1st simplified tree
  INTVECT ver_map_2; // Map between the vertices of the 1st simplification tree
                     // and the 2nd simplified tree
  ...
  gld::complex **voltage_storage; // Voltage storage - to restore when powerflow
  ...
  void mapSecSwi(BRANCHVERTICES *sSW_2, BRANCHVERTICES *sSW,
                 BRANCHVERTICES *sSW_1);
  void mapTieSwi(BRANCHVERTICES *tSW_2, BRANCHVERTICES *tSW,
...
void unique_int(INTVECT *inputvect,
                INTVECT *outputvect); // Outputvect is unique elements of input
                                      // vector (sorted)
void merge_sort_int(int *Input_Array, unsigned int Alen,
                    int *Work_Array); // Merge sort - stolen from solver_nr
