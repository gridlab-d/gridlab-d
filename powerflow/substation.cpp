/** $Id: substation.cpp
        ...
        Substation object serves as a connecting object between the powerflow
 and network solvers.  The three-phase unbalanced connections of the
 distribution solver are posted as an equivalent per-unit load on the network
 solver.  The network solver per-unit voltage is translated into a balanced
 three-phase voltage for the distribution solver.  This functionality assumes
 the substation is treated as the swing bus for the distribution solver.
...
substation::substation(MODULE *mod) : node(mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "substation", sizeof(substation),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "node",
            // inputs
            PT_complex, "zero_sequence_voltage[V]", PADDR(seq_mat[0]),
            PT_DESCRIPTION,
            "The zero sequence representation of the voltage for the "
            "substation object.",
            PT_complex, "positive_sequence_voltage[V]", PADDR(seq_mat[1]),
            PT_DESCRIPTION,
            "The positive sequence representation of the voltage for the "
            "substation object.",
            PT_complex, "negative_sequence_voltage[V]", PADDR(seq_mat[2]),
            PT_DESCRIPTION,
            "The negative sequence representation of the voltage for the "
            "substation object.",
            PT_double, "base_power[VA]", PADDR(base_power), PT_DESCRIPTION,
            "The 3 phase VA power rating of the substation.", PT_double,
            "power_convergence_value[VA]", PADDR(power_convergence_value),
            PT_DESCRIPTION,
            "Default convergence criterion before power is posted to pw_load "
            "objects if connected, otherwise ignored",
            PT_enumeration, "reference_phase", PADDR(reference_phase),
            PT_DESCRIPTION,
            "The reference phase for the positive sequence voltage.",
            PT_KEYWORD, "PHASE_A", (enumeration)R_PHASE_A, PT_KEYWORD,
            "PHASE_B", (enumeration)R_PHASE_B, PT_KEYWORD, "PHASE_C",
            (enumeration)R_PHASE_C, PT_complex,
            "transmission_level_constant_power_load[VA]",
            PADDR(average_transmission_power_load), PT_DESCRIPTION,
            "the average constant power load to be posted directly to the "
            "pw_load object.",
            PT_complex, "transmission_level_constant_current_load[A]",
            PADDR(average_transmission_current_load), PT_DESCRIPTION,
            "the average constant current load at nominal voltage to be posted "
            "directly to the pw_load object.",
            PT_complex, "transmission_level_constant_impedance_load[Ohm]",
            PADDR(average_transmission_impedance_load), PT_DESCRIPTION,
            "the average constant impedance load at nominal voltage to be "
            "posted directly to the pw_load object.",
            ...
            PT_double, "distribution_real_energy[Wh]",
            PADDR(distribution_real_energy),
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_substation) == nullptr)
    ...
    if (gl_publish_function(oclass, "pwr_current_injection_update_map",
        ...
        nullptr)
      GL_THROW("Unable to publish substation current injection update mapping "
               "function");
...
void substation::fetch_complex(gld::complex **prop, const char *name,
                               OBJECT *parent) {
...
int substation::init(OBJECT *parent) {
  ...
  if (base_power <= 0) {
    gl_warning("substation:%i is using the default base power of 100 VA. This "
               "could cause instability on your system.",
               hdr->id);
  ...
  if (power_convergence_value <= 0.0) {
    ...
    /*  TROUBLESHOOT
    A value was not specified for the convergence criterion required before
    posting an answer up to pw_load.  This value has defaulted to 1% of
    base_power.  If a different threshold is desired, set it explicitly.
    */
  ...
  if (parent != nullptr) {
    if (gl_object_isa(parent, "pw_load", "network")) {
      ...
      if ((parent->flags & OF_INIT) != OF_INIT) {
        char objname[256];
        gl_verbose("substation::init(): deferring initialization on %s",
      ...
      if (fabs(*pTransNominalVoltage - nominal_voltage) > 0.001) {
        gl_error("Nominal voltages of tranmission node (%.1f V) and substation "
                 "(%.1f) do not match!",
                 *pTransNominalVoltage, nominal_voltage);
        /*  TROUBLESHOOT
        The nominal voltage of the transmission node in PowerWorld does not
        match that of the value inside GridLAB-D's substation's nominal_voltage.
        This could cause information mismatch and is therefore not allowed.
        Please set the substation to the same nominal voltage as the
        transmission node.  Use transformers to step the voltage down to an
        appropriate distribution or sub-transmission level.
        */
        return 0; // Fail
      }
      ...
      if (bustype != SWING) {
        gl_warning("substation attached to pw_load and not a SWING bus - "
                   "forcing to SWING");
        /*  TROUBLESHOOT
        When a substation object is connected to PowerWorld via a pw_load
        object, the substation must be designated as a SWING bus.  This
        designation will now be forced upon the bus.
        */
        bustype = SWING;
      } // End bus check
      ...
      has_parent = 1;
    } else // Parent isn't a pw_load, so we just become a normal node - let it
           // handle things
    {
      has_parent = 2; // Flag as "normal" - let node checks sort out if we are
                      // really valid or not
    }
  } // End parent
  else // Parent is null, see what mode we're in
  {
    // Check for sequence voltages - if not set, we're normal (let node checks
    // handle if we're valid)
    if ((seq_mat[0] != 0.0) || (seq_mat[1] != 0.0) || (seq_mat[2] != 0.0)) {
      ...
      if (bustype != SWING) {
        ...
        /*  TROUBLESHOOT
        A substation object appears set to accept sequence voltage values, but
        it is not a SWING bus.  This may end up causing the voltages to be
        converted from sequence, but then overridden by the distribution
        powerflow solver.
        */
      }
    ...
    } else // Else, nothing set, we're a normal old node
    {
      ...
      gl_warning("substation:%s is set up as a normal node, no sequence values "
                 "will be calculated",
                 hdr->name);
      /*  TROUBLESHOOT
      A substation is currently behaving just like a normal powerflow node.  If
      it was desired that it convert a schedule or player of sequence values,
      please initialize those values to non-zero along with the player
      attachment.
      */
    }
  } // End no parent
  ...
  {
    // New requirement to maintain positive sequence ability - three phases must
    // be had, unless we're just a normal node.  Then, we don't care.
    if (!has_phase(PHASE_A | PHASE_B | PHASE_C)) {
      gl_error("substation needs to have all three phases!");
      /*  TROUBLESHOOT
      To meet the requirements for sequence voltage conversions, the substation
      node must have all three phases at the connection point.  If only a single
      phase or subset of full three phase is needed, those must be set in the
      distribution network, typically after a delta-ground wye transformer.
      */
      return 0;
    }
  } // End not a normal node
  // set the reference phase number to shift the phase voltages appropriatly
  // with the positive sequence voltage
...
TIMESTAMP substation::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (t1 != t0 && t0 != 0 &&
      (last_power_A.Re() != 0 && last_power_B.Re() != 0 &&
       last_power_C.Re() != 0)) {
...
TIMESTAMP substation::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if ((solver_method == SM_NR || solver_method == SM_FBS)) {
    if (has_parent == 1) { // has a pw_load as a parent
      ...
      seq_mat[0] =
          0.0; // Force the other two to zero for now - just to make sure
      seq_mat[2] = 0.0;
    }

    // Check to see if a sequence conversion needs to be done
    if (seq_mat[0].Mag() != 0 || seq_mat[1].Mag() != 0 ||
        seq_mat[2].Mag() != 0) {
      voltageA = (seq_mat[0] + seq_mat[1] + seq_mat[2]) * reference_number;
      voltageB = (seq_mat[0] + transformation_matrix[1][1] * seq_mat[1] +
                  transformation_matrix[1][2] * seq_mat[2]) *
                 reference_number;
      voltageC = (seq_mat[0] + transformation_matrix[2][1] * seq_mat[1] +
                  transformation_matrix[2][2] * seq_mat[2]) *
                 reference_number;
    }
  }
  ...
  if (solver_method == SM_NR) {
    ...
    if (result != 1) {
      GL_THROW("Attempt to update current/power on substation:%s failed!",
               obj->name);
      // Defined elsewhere
    }
  }
  ...
  if ((solver_method == SM_NR && !NR_admit_change) || solver_method == SM_FBS) {
    ...
    distribution_load =
        distribution_power_A + distribution_power_B + distribution_power_C;
    ...
    if (has_parent == 1) {
      if ((fabs(dist_power_A_diff) + fabs(dist_power_B_diff) +
           fabs(dist_power_C_diff)) <= power_convergence_value) {
        // Built on three-phase assumption, otherwise sequence components method
        // falls apart - convert these to all use nominal!
        *pConstantCurrentLoad =
            ((~average_transmission_current_load) * (*pTransNominalVoltage)) /
            1000000;
        if (average_transmission_impedance_load.Mag() > 0) {
          *pConstantImpedanceLoad =
              (gld::complex((*pTransNominalVoltage * (*pTransNominalVoltage))) /
               (~average_transmission_impedance_load) / 1000000);
        } else {
          *pConstantImpedanceLoad = 0;
        }
        *pConstantPowerLoad = (average_transmission_power_load +
                               (distribution_power_A + distribution_power_B +
...
SIMULATIONMODE substation::inter_deltaupdate_substation(
  ...
  if (!interupdate_pos) // Before powerflow call
  {
    ...
    if (iteration_count_val == 0) {
      ...
      distribution_real_energy +=
          total_load * ((double)dt / (3600.0 * DT_SECOND));
    }
  ...
  } else             // After the call
  {
    ...
    if ((solver_method == SM_NR && !NR_admit_change) ||
        solver_method == SM_FBS) {
      distribution_power_A = voltageA * (~current_inj[0]);
      distribution_power_B = voltageB * (~current_inj[1]);
      distribution_power_C = voltageC * (~current_inj[2]);
      distribution_load =
      ...
      if (has_parent == 1) {
        if ((fabs(dist_power_A_diff) + fabs(dist_power_B_diff) +
             fabs(dist_power_C_diff)) <= power_convergence_value) {
          // Built on three-phase assumption, otherwise sequence components
          // method falls apart - convert these to all use nominal!
          *pConstantCurrentLoad =
              ((~average_transmission_current_load) * (*pTransNominalVoltage)) /
              1000000;
          if (average_transmission_impedance_load.Mag() > 0) {
            *pConstantImpedanceLoad =
                (gld::complex(
                     (*pTransNominalVoltage * (*pTransNominalVoltage))) /
                 (~average_transmission_impedance_load) / 1000000);
          } else {
            *pConstantImpedanceLoad = 0;
          }
...
static TIMESTAMP sync_substation_impl(OBJECT *obj, TIMESTAMP t0,
                                      PASSCONFIG pass) {
  try {
  ...
  //    catch (char *msg) {
  //        gl_error("sync_substation(obj=%d;%s): %s", obj->id, obj->name ?
  //        obj->name : "unnamed", msg); return TS_INVALID;
  //    } catch (const char *msg) {
  //        gl_error("sync_substation(obj=%d;%s): %s", obj->id, obj->name ?
  //        obj->name : "unnamed", msg); return TS_INVALID;
  //    }
  //    catch(const std::exception& ex) {
  //        gl_error("sync_substation(obj=%d;%s): unhandled exception", obj->id,
  //        obj->name ? obj->name : "unnamed"); gl_error(ex.what()); return
  //        TS_INVALID;
  //    }
  //    return TS_INVALID;
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_substation(OBJECT *obj, TIMESTAMP t0,
                                                PASSCONFIG pass) {
  return sync_substation_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_substation(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
EXPORT int notify_substation(OBJECT *obj, int update_mode, PROPERTY *prop,
                             char *value) {
...
// Deltamode export
EXPORT SIMULATIONMODE interupdate_substation(OBJECT *obj,
  ...
  try {
    status = my->inter_deltaupdate_substation(
        delta_time, dt, iteration_count_val, interupdate_pos);
    return status;
  } catch (char *msg) {
    gl_error("interupdate_substation(obj=%d;%s): %s", obj->id,
