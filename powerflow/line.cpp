/** $Id: line.cpp 1186 2009-01-02 18:15:30Z dchassin $
        ...
        This file contains the definition for 4 types of lines, their
configurations and the conductors associated with these lines.  The line is one
of the major parts of the method used for solving a powerflow network.  In
essense the distribution network can be seen as a series of nodes and links. The
links (or lines as in this file) do most of the work, solving a series of 3
        dimensional matrices, while the nodes (defined in node.cpp) are
containers and aggregators, storing the values to be used in the equation
solvers.

        In general, a line contains a configuration that contains a spacing
object, and one or more conductor objects, exported as phaseA_conductor,
phaseB_conductor, phaseC_conductor, and phaseN_conductor.  The line spacing
object hold information about the distances between each of the phase
conductors, while the conductor objects contain the information on the specifics
of the conductor (resistance, diameter, etc...)

        Line itself is not exported, and cannot be used in a model.  Instead one
of 3 types of lines are exported for use in models; underground lines, overhead
lines, and triplex (or secondary) lines.  Underground lines and overhead lines
use the basic line configuration and line spacing objects, but have different
conductor classes defined for them.  The overhead line conductor is the simplest
of these with just a resistance and a geometric mean radius defined.  The
underground line conductor is more complex, due to the fact that each phase
conductor may have its own neutral.

        Triplex lines are the third type of line exported.  A special triplex
line configuration is also exported which presents 3 line conductors;
l1_conductor, l2_conductor, and lN_conductor.  Also exported is a general
insulation thickness and the total diameter of the line.  The conductor for
triplex lines is much the same as the overhead line conductor.

        All line objects export a phase property that is a set of phases and may
...
line::line(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    line_class = oclass =
        gl_register_class(mod, "line", sizeof(line),
    ...
    if (gl_publish_variable(oclass, PT_INHERIT, "link", PT_object,
                            "configuration", PADDR(configuration), PT_double,
                            "length[ft]", PADDR(length), nullptr) < 1)
      GL_THROW("unable to publish line properties in %s", __FILE__);
    ...
    if (gl_publish_function(oclass, "check_limits_pwr_object",
      ...
      GL_THROW(
          "Unable to publish line external power limit calculation function");
...
gld_property *fNode_nominal, *tNode_nominal;
  ...
  if (!fNode_nominal->is_valid() || !fNode_nominal->is_double()) {
    GL_THROW(
        "line:%d - %s - Unable to map nominal_voltage from connected node!",
        obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBESHOOT
    While attempting to map the nominal_voltage property of the from or to node
    on a line, an error occurred. Please try again.  If the error persists,
    please submit your code and a bug report via the issue tracking system.
    */
  }
  ...
  if (solver_method == SM_NR && length == 0.0)
    throw "Newton-Raphson method does not support zero length lines at this "
          "time";

  // Now see if we are truly "just a line" (not overhead or anything) - if so,
  // do a recalc for us
  if (strcmp(obj->oclass->name, "line") == 0) {
    gl_warning("line:%d - %s - use of an overhead_line, underground_line, or "
               "other line-based subclass is highly recommended!",
               obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    Users are encouraged to use one of the specific sub-classes of line objects,
    not line itself.  Functionality with line objects is limited and mostly
    handled by their appropriate subclasses (e.g., overhead_line,
    underground_line, triplex_line).
    */

    // Make sure a configuration was actually specified
    if ((configuration == nullptr) ||
        (!gl_object_isa(configuration, "line_configuration", "powerflow"))) {
      GL_THROW("line:%d - %s - configuration object either doesn't exist, or "
               "is not a valid configuration object!",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      The configuration field for the line object is either empty, or does not
      contain a line_configuration object.  Please fix this and try again.
      */
    }

    // Perform the calculation assuming we're matrix-based (because we basically
    // have to be)
...
void line::load_matrix_based_configuration(gld::complex Zabc_mat[3][3],
                                           gld::complex Yabc_mat[3][3]) {
  line_configuration *config = object_data<line_configuration>(configuration);
  double miles;
  gld::complex cap_freq_mult;

  // ft -> miles as z-matrix and c-matrix values are specified in Ohms / nF per
  // mile.
  miles = length / 5280.0;

  // Set capacitor frequency/distance/scaling factor (rad/s*S)
  // scale factor allows for the fact that the internal representation of the C
  // matrix is specified in nF/mile and converts back to Siemens
  cap_freq_mult =
      gld::complex(0, (2.0 * PI * nominal_frequency * 0.000000001 * miles));

  // Setting Zabc_mat based on the z-matrix configuration parameters
  // Setting Yabc_mat based on the c-matrix configuration parameters and the
  // application of Kersting (5.15)

  // User defined z-matrix and c-matrix - only assign parts of matrix if phase
  // exists
  ...
  // Make sure use_line_cap is turned on if capacitance values have been
  // specified.  Otherwise we need to warn the user and zero out Yabc_mat as
  // otherwise the powerflow engine will give quirky results.
  if (config->capacitance11 != 0 || config->capacitance22 != 0 ||
      config->capacitance33 != 0) {
    if (!use_line_cap) {
      gl_warning("Shunt capacitance of line:%s specified without setting "
                 "powerflow::line_capacitance = TRUE. Shunt capacitance will "
                 "be ignored.",
...
void line::recalc_line_matricies(gld::complex Zabc_mat[3][3],
                                 gld::complex Yabc_mat[3][3]) {
  ...
  } else if ((has_phase(PHASE_A) && has_phase(PHASE_B) && has_phase(PHASE_C)) ||
             (has_phase(PHASE_D))) // has ABC or D (D=ABC)
  {
    gld::complex detvalue = a_mat[0][0] * a_mat[1][1] * a_mat[2][2] -
                            a_mat[0][0] * a_mat[1][2] * a_mat[2][1] -
                            a_mat[0][1] * a_mat[1][0] * a_mat[2][2] +
                            a_mat[0][1] * a_mat[2][0] * a_mat[1][2] +
                            a_mat[1][0] * a_mat[0][2] * a_mat[2][1] -
                            a_mat[0][2] * a_mat[1][1] * a_mat[2][0];

    // Invert it
    A_mat[0][0] =
        (a_mat[1][1] * a_mat[2][2] - a_mat[1][2] * a_mat[2][1]) / detvalue;
    A_mat[0][1] =
        (a_mat[0][2] * a_mat[2][1] - a_mat[0][1] * a_mat[2][2]) / detvalue;
    A_mat[0][2] =
        (a_mat[0][1] * a_mat[1][2] - a_mat[0][2] * a_mat[1][1]) / detvalue;
    A_mat[1][0] =
        (a_mat[2][0] * a_mat[1][2] - a_mat[1][0] * a_mat[2][2]) / detvalue;
    A_mat[1][1] =
        (a_mat[0][0] * a_mat[2][2] - a_mat[0][2] * a_mat[2][0]) / detvalue;
    A_mat[1][2] =
        (a_mat[1][0] * a_mat[0][2] - a_mat[0][0] * a_mat[1][2]) / detvalue;
    A_mat[2][0] =
        (a_mat[1][0] * a_mat[2][1] - a_mat[1][1] * a_mat[2][0]) / detvalue;
    A_mat[2][1] =
        (a_mat[0][1] * a_mat[2][0] - a_mat[0][0] * a_mat[2][1]) / detvalue;
    A_mat[2][2] =
        (a_mat[0][0] * a_mat[1][1] - a_mat[0][1] * a_mat[1][0]) / detvalue;
  }
...
extern "C" MODULE_API int isa_line(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_line_impl(obj, classsname);
}
#endif
