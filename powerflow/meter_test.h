...
public:
  ...
  void tearDown() { local_callbacks->remove_objects(); }
  ...
  void test_meter_singlephase() {
    ...
    OBJECTDATA(trans_config, transformer_configuration)->V_secondary =
        0.120 * 1000;
    ...
    OBJECTDATA(trans_config, transformer_configuration)
        ->impedance.SetReal(0.011);
    // OBJECTDATA(trans_config,transformer_configuration)->X_pu = 0.018;
    OBJECTDATA(trans_config, transformer_configuration)
        ->impedance.SetImag(0.018);
    OBJECTDATA(trans_config, transformer_configuration)->connect_type =
        transformer_configuration::SINGLE_PHASE_CENTER_TAPPED; // delta-delta
    ...
    OBJECTDATA(ohl2, triplex_line)->phases = OBJECTDATA(node2, node)->phases =
        OBJECTDATA(node3, load)->phases = PHASE_A;
    ...
    triplex_line_configuration *lc =
        OBJECTDATA(line_config, triplex_line_configuration);
    ...
    double rI_1 = OBJECTDATA(node1, node)
                      ->currentA.Re(); // Real part of the current at node 1
    double iI_1 =
        OBJECTDATA(node1, node)
            ->currentA.Im(); // Imaginary part of the current at node 1
#ifdef _TESTING
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->currentA.Re(),
           OBJECTDATA(node1, node)->currentA.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->currentB.Re(),
           OBJECTDATA(node1, node)->currentB.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->currentC.Re(),
           OBJECTDATA(node1, node)->currentC.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->currentA.Re(),
           OBJECTDATA(node2, triplex_node)->currentA.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->currentB.Re(),
           OBJECTDATA(node2, triplex_node)->currentB.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->currentC.Re(),
           OBJECTDATA(node2, triplex_node)->currentC.Im());
#endif

    // Get the percentage of the difference between the calculated value and the
    // test value
    double validate_re =
        1 - (rI_1 <= node1_phA_I_test->Re() ? rI_1 / node1_phA_I_test->Re()
                                            : node1_phA_I_test->Re() / rI_1);
    double validate_im =
        1 - (iI_1 <= node1_phA_I_test->Im() ? iI_1 / node1_phA_I_test->Im()
                                            : node1_phA_I_test->Im() / iI_1);
...
#ifdef _TESTING
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->voltageA.Re(),
           OBJECTDATA(node2, triplex_node)->voltageA.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->voltageB.Re(),
           OBJECTDATA(node2, triplex_node)->voltageB.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node2, triplex_node)->voltageC.Re(),
           OBJECTDATA(node2, triplex_node)->voltageC.Im());
#endif

    double rV_2 = OBJECTDATA(node3, meter)->voltageA.Re();
    double iV_2 = OBJECTDATA(node3, meter)->voltageA.Im();
#ifdef _TESTING
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node3, meter)->voltageA.Re(),
           OBJECTDATA(node3, meter)->voltageA.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node3, meter)->voltageB.Re(),
           OBJECTDATA(node3, meter)->voltageB.Im());
    printf("  %10.6f+%+10.6f\n", OBJECTDATA(node3, meter)->voltageC.Re(),
           OBJECTDATA(node3, meter)->voltageC.Im());
#endif

    validate_re =
        1 - (rV_2 <= node3_phA_V_test->Re() ? rV_2 / node3_phA_V_test->Re()
                                            : node3_phA_V_test->Re() / rV_2);
    validate_im =
        1 - (iV_2 <= node3_phA_V_test->Im() ? iV_2 / node3_phA_V_test->Im()
                                            : node3_phA_V_test->Im() / iV_2);
    CPPUNIT_ASSERT(validate_re < VALIDATE_THRESHOLD);
    CPPUNIT_ASSERT(validate_im < VALIDATE_THRESHOLD);
    ...
    validate_re = 1 - (mP_3 <= meter_power_test ? mP_3 / meter_power_test
                                                : meter_power_test / mP_3);
  ...
  void test_meter_polyphase() {
    ...
    */
    ...
    printf(" I2: %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->currentA.Re(),
           OBJECTDATA(node2, meter)->currentA.Im());
    printf(" I2: %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->currentB.Re(),
           OBJECTDATA(node2, meter)->currentB.Im());
    printf(" I2: %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->currentC.Re(),
           OBJECTDATA(node2, meter)->currentC.Im());
    /*
                    printf(" I2: %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->currentA.Re(),
       OBJECTDATA(node2,node)->currentA.Im()); printf(" I2: %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->currentB.Re(),
       OBJECTDATA(node2,node)->currentB.Im()); printf(" I2: %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->currentC.Re(),
       OBJECTDATA(node2,node)->currentC.Im());
    */

    printf(" I3: %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->currentA.Re(),
           OBJECTDATA(node3, load)->currentA.Im());
    printf(" I3: %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->currentB.Re(),
           OBJECTDATA(node3, load)->currentB.Im());
    printf(" I3: %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->currentC.Re(),
           OBJECTDATA(node3, load)->currentC.Im());
    ...
    validate_re = 1 - (mP_1 <= meter_power_test1 ? mP_1 / meter_power_test1
                                                 : meter_power_test1 / mP_1);
    //		CPPUNIT_ASSERT(validate_re < VALIDATE_THRESHOLD);
    ...
    printf(" V1: %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->voltageA.Re(),
           OBJECTDATA(node1, node)->voltageA.Im());
    printf(" V1:  %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->voltageB.Re(),
           OBJECTDATA(node1, node)->voltageB.Im());
    printf(" V1:  %10.6f+%+10.6f\n", OBJECTDATA(node1, node)->voltageC.Re(),
           OBJECTDATA(node1, node)->voltageC.Im());

    printf(" V2:  %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->voltageA.Re(),
           OBJECTDATA(node2, meter)->voltageA.Im());
    printf(" V2:  %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->voltageB.Re(),
           OBJECTDATA(node2, meter)->voltageB.Im());
    printf(" V2:  %10.6f+%+10.6f\n", OBJECTDATA(node2, meter)->voltageC.Re(),
           OBJECTDATA(node2, meter)->voltageC.Im());
    /*
            printf(" V2:  %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->voltageA.Re(),
       OBJECTDATA(node2,node)->voltageA.Im()); printf(" V2:  %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->voltageB.Re(),
       OBJECTDATA(node2,node)->voltageB.Im()); printf(" V2:  %10.6f+%+10.6f\n",
       OBJECTDATA(node2,node)->voltageC.Re(),
       OBJECTDATA(node2,node)->voltageC.Im());
    */
    printf(" V3:  %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->voltageA.Re(),
           OBJECTDATA(node3, load)->voltageA.Im());
    printf(" V3:  %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->voltageB.Re(),
           OBJECTDATA(node3, load)->voltageB.Im());
    printf(" V3: %10.6f+%+10.6f\n", OBJECTDATA(node3, load)->voltageC.Re(),
           OBJECTDATA(node3, load)->voltageC.Im());
    ...
    validate_re = 1 - (mP_1 <= meter_power_test2 ? mP_1 / meter_power_test2
                                                 : meter_power_test2 / mP_1);
    CPPUNIT_ASSERT(validate_re < VALIDATE_THRESHOLD);
  }
