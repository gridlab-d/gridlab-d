...
/**	Copyright (C) 2008 Battelle Memorial Institute
        ...
        @todo fuse do not reclose ever once blown, implement fuse restoration
   scheme (e.g., scale of hours with circuit outage)
...
fuse::fuse(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "fuse", sizeof(fuse),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "link", PT_enumeration, "phase_A_status",
            PADDR(phase_A_state), PT_KEYWORD, "BLOWN", (enumeration)BLOWN,
            PT_KEYWORD, "GOOD", (enumeration)GOOD, PT_enumeration,
            "phase_B_status", PADDR(phase_B_state), PT_KEYWORD, "BLOWN",
            (enumeration)BLOWN, PT_KEYWORD, "GOOD", (enumeration)GOOD,
            PT_enumeration, "phase_C_status", PADDR(phase_C_state), PT_KEYWORD,
            "BLOWN", (enumeration)BLOWN, PT_KEYWORD, "GOOD", (enumeration)GOOD,
            PT_enumeration, "repair_dist_type", PADDR(restore_dist_type),
            PT_KEYWORD, "NONE", (enumeration)NONE, PT_KEYWORD, "EXPONENTIAL",
            (enumeration)EXPONENTIAL, PT_double, "current_limit[A]",
            PADDR(current_limit), PT_double, "mean_replacement_time[s]",
            PADDR(mean_replacement_time), // Retains compatibility with older
                                          // files
    ...
    if (gl_publish_function(oclass, "check_limits_pwr_object",
      ...
      GL_THROW(
          "Unable to publish fuse external power limit calculation function");
...
int fuse::create() {
  ...
  current_current_values[0] = current_current_values[1] =
      current_current_values[2] = 0.0; // No current by default
...
int fuse::init(OBJECT *parent) {
  ...
  if ((phases & PHASE_S) == PHASE_S) {
    ...
    /*  TROUBLESHOOT
    Fuses do not currently support triplex circuits.  Please place the fuse
    higher upstream in the three-phase power area or utilize another object
    (such as a circuit breaker in a house model) to limit the current flow.
    */
  }
  ...
  if (current_limit < 0.0) {
    ...
    /*  TROUBLESHOOT
    The fuse has a negative value current limit specified.  Please specify a
    positive value for the current and try again.
    */
  }
  ...
  if (current_limit == 0.0) {
    ...
    gl_warning("fuse:%s has a zero current limit - set to 9999.9 Amps",
               obj->name);
    /*  TROUBLESHOOT
    A fuse somehow had a current limit of 0.0 Amps set.  This is invalid, so a
    placeholder value of 9999.0 Amps is used.  Please adjust this value
    accordingly.
    */
  }
  ...
  {
    gl_warning("Fuse:%s has a negative or 0 mean replacement time - defaulting "
               "to 1 hour",
               obj->name);
    /*  TROUBLESHOOT
    A fuse has a negative or zero time specified for mean_replacement_time.  The
    value has therefore been overridden to 1 hour.  If this is unacceptable,
    please change the value in your GLM file.
    */
  ...
  if (solver_method == SM_FBS) {
    gl_warning("Fuses only work for the attached node in the FBS solver, not "
               "any deeper.");
    /*  TROUBLESHOOT
    Under the Forward-Back sweep method, fuses can only affect their directly
    attached downstream node. Due to the nature of the FBS algorithm, nodes
    further downstream (especially constant current loads) will cause an
    oscillatory nature in the voltage and current injections, so they will no
    longer be accurate. Either ignore these values or figure out a way to work
    around this limitation (player objects).
    */
  }
  // check the fuse resistance value to see that it is not zero
  if (solver_method == SM_NR) {
    if (fuse_resistance == 0.0) {
      gl_warning("Fuse:%s fuse_resistance has been set to zero. This will "
      ...
      fuse_resistance to something small but larger that zero.
      */
    }
    if (fuse_resistance < 0.0) {
  ...
  for (jindex = 0; jindex < 3; jindex++) {
    for (kindex = 0; kindex < 3; kindex++) {
      a_mat[jindex][kindex] = d_mat[jindex][kindex] = A_mat[jindex][kindex] =
          0.0;
  ...
  a_mat[0][0] = d_mat[0][0] = A_mat[0][0] =
      (is_closed() && has_phase(PHASE_A) ? 1.0 : 0.0);
  a_mat[1][1] = d_mat[1][1] = A_mat[1][1] =
      (is_closed() && has_phase(PHASE_B) ? 1.0 : 0.0);
  a_mat[2][2] = d_mat[2][2] = A_mat[2][2] =
      (is_closed() && has_phase(PHASE_C) ? 1.0 : 0.0);
    ...
    } else // LS_CLOSED - handle individually
    {
      if (has_phase(PHASE_A)) {
        if (phase_A_state == GOOD) {
          From_Y[0][0] =
              gld::complex(1.0 / fuse_resistance, 1.0 / fuse_resistance);
...
TIMESTAMP fuse::sync(TIMESTAMP t0) {
  ...
  // Code below only applies to NR right now - FBS legacy code has no sync
  // values May need to be appropriately adjusted once FBS supports reliability
  if (solver_method == SM_NR) {
    // Put any fuses back in service, if they're ready
    if (((fix_time[0] <= t0) || (fix_time[1] <= t0) || (fix_time[2] <= t0)) &&
        (event_schedule ==
         nullptr)) // Only needs to be done if reliability isn't present
    {
      // Bring the phases back that are necessary
      if ((fix_time[0] <= t0) &&
          ((NR_branchdata[NR_branch_reference].origphases & 0x04) ==
           0x04)) // Phase A ready and had a phase A
      {
        // Update status
        phase_A_state = GOOD;
      ...
      if ((fix_time[1] <= t0) &&
          ((NR_branchdata[NR_branch_reference].origphases & 0x02) ==
           0x02)) // Phase B ready and had a phase B
      {
        // Update status
        phase_B_state = GOOD;
      ...
      if ((fix_time[2] <= t0) &&
          ((NR_branchdata[NR_branch_reference].origphases & 0x01) ==
           0x01)) // Phase C ready and had a phase C
      {
        // Update status
        phase_C_state = GOOD;
    ...
    if ((NR_branchdata[NR_branch_reference].phases & 0x04) ==
        0x04) // Phase A valid - check it
    {
      // Link::sync is where current in is calculated.  Convert the values
      current_current_values[0] = current_in[0].Mag();

      if ((current_current_values[0] > current_limit) &&
          (phase_A_state == GOOD)) {
        phase_A_state = BLOWN; // Blow the fuse
        gl_warning("Phase A of fuse:%s just blew!", obj->name);
        /*  TROUBLESHOOT
        The current through phase A of the fuse just exceeded the maximum rated
        value. Use a larger value, or otherwise change your system and try
        again.
        */
        ...
        // See if an update is needed (it's A and first, so yes, but just to be
        // generic)
        if (replacement_time == 0) {
          ...
          if (restore_dist_type == EXPONENTIAL) {
            ...
            mean_repair_time =
                gl_random_exponential(RNGSTATE, 1.0 / mean_replacement_time);
    ...
    if ((NR_branchdata[NR_branch_reference].phases & 0x02) ==
        0x02) // Phase B valid - check it
    {
      // Link::sync is where current in is calculated.  Convert the values
      current_current_values[1] = current_in[1].Mag();

      if ((current_current_values[1] > current_limit) &&
          (phase_B_state == GOOD)) {
        phase_B_state = BLOWN; // Blow the fuse

        gl_warning("Phase B of fuse:%s just blew!", obj->name);
        /*  TROUBLESHOOT
        The current through phase B of the fuse just exceeded the maximum rated
        value. Use a larger value, or otherwise change your system and try
        again.
        */
    ...
    if ((NR_branchdata[NR_branch_reference].phases & 0x01) ==
        0x01) // Phase C valid - check it
    {
      // Link::sync is where current in is calculated.  Convert the values
      current_current_values[2] = current_in[2].Mag();

      if ((current_current_values[2] > current_limit) &&
          (phase_C_state == GOOD)) {
        phase_C_state = BLOWN; // Blow the fuse

        gl_warning("Phase C of fuse:%s just blew!", obj->name);
        /*  TROUBLESHOOT
        The current through phase C of the fuse just exceeded the maximum rated
        value. Use a larger value, or otherwise change your system and try
        again.
        */
    ...
    if (fuse_blew) {
      ...
      // Determine who blew and store the time (assumes fuses can be replaced in
      // parallel)
      switch (work_phases) {
      case 0x00: // No fuses blown !??
        GL_THROW(
            "fuse:%s supposedly blew, but doesn't register the right phases",
            obj->name);
        /*  TROUBLESHOOT
        A fuse reported an over-current condition and blew the appropriate link.
        However, it did not appear to fully propogate this condition.  Please
      ...
      {
        // Call the function
        result_val = ((int (*)(OBJECT *, OBJECT *, char *, TIMESTAMP, TIMESTAMP,
                               int, bool))(*event_schedule))(
            *eventgen_obj, obj, fault_val, t0, 0, -1, false);

        // Make sure it worked
        if (result_val != 1) {
          GL_THROW("Attempt to blow fuse:%s failed in a reliability manner",
                   obj->name);
          /*  TROUBLESHOOT
          While attempting to propagate a blown fuse's impacts, an error was
      ...
      else // No object, just fail us out - save the iterations
      {
        ...
        /*  TROUBLESHOOT
        A fuse blew and created an open link.  If the system is not meshed, the
        Newton-Raphson solver will likely fail.  Theoretically, this should be a
        quick fail due to a singular matrix. However, the system occasionally
        gets stuck and will exhaust iteration cycles before continuing. If the
        fuse is blowing and the NR solver still iterates for a long time, this
        may be the case.
        */
      }
    }
...
TIMESTAMP fuse::postsync(TIMESTAMP t0) {
  ...
  // FBS legacy code
  if (solver_method == SM_FBS) {
    // All actual checks and updates are handled in the COMMIT time-step in the
    // fuse_check function below. See which phases we need to check
    if ((phases & PHASE_A) == PHASE_A) // Check A
    {
      if (phase_A_state == GOOD) // Only bother if we are in service
      {
        Ret_Val[0] =
            TS_NEVER; // We're still good, so we don't care when we come back
...
// Function to externally set fuse status - mainly for "out of step" updates
// under NR solver where admittance needs to be updated derived from function
// fuse_sync_function
    ...
    if (status == LS_OPEN) // Fully opened means all go open
    {
      ...
      if (meshed_fault_checking_enabled) // Different operating mode
      {
        NR_branchdata[NR_branch_reference].faultphases =
            NR_branchdata[NR_branch_reference].origphases & 0x07;
      }
    } else // Closed means a phase-by-phase basis
    {
      if (has_phase(PHASE_A)) {
        ...
        } else // Must be open
        {
          ...
          {
            NR_branchdata[NR_branch_reference].faultphases |=
                0x04; // Make sure fault condition is set
          }
        }
      }
      ...
      if (has_phase(PHASE_B)) {
        ...
        } else // Must be open
        {
          ...
          {
            NR_branchdata[NR_branch_reference].faultphases |=
                0x02; // Make sure fault condition is set
          }
        }
      }
      ...
      if (has_phase(PHASE_C)) {
        ...
        } else // Must be open
        {
          ...
          {
            NR_branchdata[NR_branch_reference].faultphases |=
                0x01; // Make sure fault condition is set
          }
        }
      }
    }
    ...
    } else      // Closed means a phase-by-phase basis
    {
      if (has_phase(PHASE_A)) {
        if (phase_A_state == GOOD) {
          ...
          // See if this changed from expectations - this prevents fuse from
          // overriding fault_check/reliability
...
// individual switching ability 0 = blown, 1 = good, 2 = don't care (leave as
// was)
void fuse::set_fuse_full(char desired_status_A, char desired_status_B,
...
// fuse status for reliability faults Prevents system from magically "turning
// back on" a single islanded fuse when removed as a downstream element
void fuse::set_fuse_full_reliability(unsigned char desired_status) {
  ...
  if ((phase_change & 0x04) == 0x04) // Phase A
  {
    ...
    if ((desired_status & 0x04) == 0x04) // Putting it back in
    {
      ...
      if ((phased_fuse_status & 0x04) ==
          0x04) // Fuse A was on when this was faulted
      {
  ...
  if ((phase_change & 0x02) == 0x02) // Phase B
  {
    ...
    if ((desired_status & 0x02) == 0x02) // Putting it back in
    {
      ...
      if ((phased_fuse_status & 0x02) ==
          0x02) // Fuse B was on when this was faulted
      {
  ...
  if ((phase_change & 0x01) == 0x01) // Phase C
  {
    ...
    if ((desired_status & 0x01) == 0x01) // Putting it back in
    {
      ...
      if ((phased_fuse_status & 0x01) ==
          0x01) // Fuse C was on when this was faulted
      {
 ...
 *
 * functionalized so don't have to change 4 entries in 3 different sets every
 * time
 *
...
void fuse::fuse_check(gld::set phase_to_check, gld::complex *fcurr) {
  ...
  if ((phases & phase_to_check) == phase_to_check) // Check phase
  {
    ...
    {
      // Check both directions, that way if we are reverse flowed it doesn't
      // matter
      if (fcurr[indexval].Mag() > current_limit) // We've exceeded the limit
      {
        *valstate = BLOWN; // Trip us
        ...
        // Get an update time
        *fixtime = prev_fuse_time +
                   (int64)(3600 * gl_random_exponential(
                                      RNGSTATE, 1.0 / mean_replacement_time));

        // Announce it for giggles
        gl_warning("Phase %c of fuse:%d (%s) just blew", phase_verbose, hdr->id,
                   hdr->name);
      } else // Still good
      {
    ...
    {
      if (*fixtime <=
          prev_fuse_time) // Technician has arrived and replaced us!!
      {
        // Fix us
        A_mat[indexval][indexval] = d_mat[indexval][indexval] = 1.0;
        ...
        *fixtime = TS_NEVER; // Update the time check just in case
        ...
        gl_warning("Phase %c of fuse:%d (%s) just returned to service",
                   phase_verbose, hdr->id, hdr->name);
      } else // Still driving there or on break, no fixed yet
      {
        // Ensure matrices are up to date in case someone manually blew us (or a
        // third, off state is implemented)
        A_mat[indexval][indexval] = d_mat[indexval][indexval] = 0.0;
      }
    }
  }
...
int fuse::fuse_state(OBJECT *parent) {
  ...
  return 1; // Not sure how we'd ever fail.  If I come up with a reason, we'll
            // check
}
...
#else
// On Apple, we have to do some extra work to export the function with the
// correct name
extern "C" MODULE_API TIMESTAMP sync_fuse(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
// Reliability interface - generalized fuse operation so fuses and other opjects
// can be similarly
...
EXPORT int create_fault_fuse(OBJECT *thisobj, OBJECT **protect_obj,
...
EXPORT int fix_fault_fuse(OBJECT *thisobj, int *implemented_fault,
                          char *imp_fault_name) {
  int retval;
...
EXPORT int clear_fault_fuse(OBJECT *thisobj, int *implemented_fault,
                            char *imp_fault_name) {
  int retval;
