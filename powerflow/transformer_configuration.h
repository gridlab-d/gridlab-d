...
public:
  ...
  enumeration
      install_type;   ///< Defines location of the transformer installation
  double V_primary;   // primary voltage level in L-L value kV
  double V_secondary; // secondary voltage level kV
  double kVA_rating;  // kVA rating of transformer  @todo remove when everything
                      // is implemented for separate phase ratings.
  ...
  gld::complex impedance1;      // Series impedance (only used when you want to
                                // define each individual
  gld::complex impedance2;      //    winding seperately
  gld::complex shunt_impedance; // Shunt impedance - all values are summed and
                                // reflected back to the primary
  ...
  double
      core_coil_weight; // The weight of the core and coil assembly in pounds.
  ...
  double dtheta_TO_R; // top-oil hottest-spot rise over ambient temperature at
                      // rated load, degrees C.
  double dtheta_H_AR; // winding hottest-spot rise over ambient temperature at
                      // rated load, degrees C.
  double installed_insulation_life; // the normal lifetime of the transformer
                                    // insulation at rated load, hours.
  ...
  inline transformer_configuration(CLASS *cl = oclass)
      : powerflow_library(cl) {};
  int create(void);
