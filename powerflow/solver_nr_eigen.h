...
#ifndef _SOLVER_NR_EIGEN
#define _SOLVER_NR_EIGEN
...
// Sparse Matrix triplet allocator list
template<typename Type>
using SPARSE = struct sparse_array {
    bool changed {false};
    std::string name;
...
STATUS NR_array_structure_allocate(NR_SOLVER_STRUCT *struct_of_interest,
                                   int number_of_islands);    /* Allocates NR_SOLVER_STRUCT item for a given number of entries/islands */


class NR_Solver_Eigen {
public:
    explicit NR_Solver_Eigen(unsigned island_count){
      Island_Builders.reserve(island_count);
      A_LU_Islands.reserve(island_count);
      B_LU_Islands.reserve(island_count);
    };
...
private:
    static void sparse_tonr(SPARSE &sm, Eigen::SparseMatrix<double> &A_LU);
    void sparse_add(SPARSE<double> &sm,
    ...
    void sparse_reset(Eigen::SparseMatrix<double> &A_LU, int ncols);
    void sparse_clear(Eigen::SparseMatrix<double> &A_LU);
    void sparse_init(Eigen::SparseMatrix<double> &A_LU, int nels, int ncols);
    void
    compute_load_values(unsigned int bus_count, BUSDATA *bus, NR_SOLVER_STRUCT *powerflow_values, bool jacobian_pass,
                        int island_number);


private:
    std::vector<SPARSE<double>> Island_Builders;

    std::vector<Eigen::SparseMatrix<double>> A_LU_Islands;
    std::vector<Eigen::VectorXd> B_LU_Islands;

//    std::unique_ptr<Eigen::SparseMatrix<double>> A_LU;
//    std::unique_ptr<Eigen::VectorXd> B_LU;

//    Eigen::SparseMatrix<double> A_LU;
//    Eigen::VectorXd B_LU;
#ifdef GLD_USE_KLU
