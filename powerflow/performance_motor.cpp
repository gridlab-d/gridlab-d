...
//@TODO: Still needs to have the stalling and reconnecting portion done.  Right
//now, will return to service immediately
...
performance_motor::performance_motor(MODULE *mod) : node(mod) {
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "performance_motor", sizeof(performance_motor),
    ...
    if (gl_publish_variable(
            ...
            PT_double, "Pbase[W]", PADDR(P_base), PT_DESCRIPTION,
            "motor rated size", PT_complex, "power_value[VA]", PADDR(PowerVal),
            PT_DESCRIPTION, "motor power consumption", PT_double,
            "power_factor[pu]", PADDR(CompPF), PT_DESCRIPTION,
            "Compressor power factor", PT_double, "delta_f_val",
            PADDR(delta_f_val), PT_DESCRIPTION,
            "Frequency change value for compressor sensitivities", PT_double,
            "Vbrk[pu]", PADDR(Vbrk), PT_DESCRIPTION,
            "Compressor motor breakdown voltage", PT_double, "Vstallbrk",
            PADDR(Vstallbrk), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "Intersection point for power curve and power stall curve - "
            "calculated",
            PT_double, "Vstall[pu]", PADDR(Vstall), PT_DESCRIPTION,
            "Compressor stall threshold voltage", PT_double, "Vrst[pu]",
            PADDR(Vrst), PT_DESCRIPTION, "Voltage at which motor can restart",
            PT_double, "P_val[pu]", PADDR(P_val), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "real power portion of motor power consumption calculation",
            PT_double, "Q_val[pu]", PADDR(Q_val), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "reactive power portion of motor power consumption calculation",
            PT_double, "P_0[pu]", PADDR(P_0), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "Computed power curve initialization point - real power", PT_double,
            "Q_0[pu]", PADDR(Q_0), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "Computed power curve initialization point - reactive power",
            PT_double, "K_p1[pu]", PADDR(K_p1), PT_DESCRIPTION,
            "Real power coefficient for running state 1, puP/puV", PT_double,
            "K_q1[pu]", PADDR(K_q1), PT_DESCRIPTION,
            "Reactive power coefficient for running state 1, puQ/puV",
            PT_double, "K_p2[pu]", PADDR(K_p2), PT_DESCRIPTION,
            "Real power coefficient for running state 2, puP/puV", PT_double,
            "K_q2[pu]", PADDR(K_q2), PT_DESCRIPTION,
            "Reactive power coefficient for running state 2, puQ/puV",
            PT_double, "N_p1", PADDR(N_p1), PT_DESCRIPTION,
            "Real power exponent for running state 1", PT_double, "N_q1",
            PADDR(N_q1), PT_DESCRIPTION,
            "Reactive power exponent for running state 1", PT_double, "N_p2",
            PADDR(N_p2), PT_DESCRIPTION,
            "Real power exponent for running state 2", PT_double, "N_q2",
            PADDR(N_q2), PT_DESCRIPTION,
            "Reactive power exponent for running state 2", PT_double,
            "CmpKpf[pu]", PADDR(CmpKpf), PT_DESCRIPTION,
            "Real power frequency sensitivity in puP/puf", PT_double,
            "CmpKqf[pu]", PADDR(CmpKqf), PT_DESCRIPTION,
            "Reactive power frequency sensitivity in puQ/puf", PT_complex,
            "stall_impedance[pu]", PADDR(stall_impedance_pu), PT_DESCRIPTION,
            "compressor stall imepdance", PT_double, "stall_resistance[pu]",
            PADDR(stall_impedance_pu.Re()), PT_DESCRIPTION,
            "compressor stall resistance", PT_double, "stall_reactance[pu]",
            PADDR(stall_impedance_pu.Im()), PT_DESCRIPTION,
            "compressor stall reactance", PT_double, "Tstall[s]", PADDR(Tstall),
            PT_DESCRIPTION, "stall time", PT_double, "reconnect_time[s]",
            PADDR(reconnect_time), PT_DESCRIPTION,
            "reconnect time after a trip", PT_double, "Pinit[pu]", PADDR(Pinit),
            PT_DESCRIPTION,
            "Initial assumed real power loading of connected terminal",
            PT_double, "Vinit[pu]", PADDR(Vinit), PT_DESCRIPTION,
            "Initial assumed voltage value of connected terminal", PT_double,
            "stall_time_tracker[s]", PADDR(stall_time_tracker), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "internal stall time tracker variable",
            PT_double, "reconnect_time_tracker[s]",
            PADDR(reconnect_time_tracker), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "internal reconnect time tracker variable", nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);

    // Publish deltamode functions
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_performance_motor) ==
        nullptr)
...
int performance_motor::create() {
  ...
  // Most default parameters taken from WECC Air Conditioner Motor Model Test
  // Report
  ...
  // Actual equations/implementation follow the LD1PAC user manual for
  // PowerWorld
...
int performance_motor::init(OBJECT *parent) {
  ...
  if (num_phases == 1) {
    ...
    GL_THROW("performance_motor:%s -- only single-phase or three-phase motors "
             "are supported",
  ...
  if (motor_op_mode == modeSPIM) {
    Z_base = (4.0 * nominal_voltage * nominal_voltage) /
             P_base; // Triplex is 12, so has to be double nominal
  ...
  Q_0 = Pinit * (sqrt(1.0 - CompPF * CompPF) / CompPF) -
        K_p1 * pow((1.0 - Vbrk), N_p1);
...
// Module-level call
SIMULATIONMODE performance_motor::inter_deltaupdate(
...
void performance_motor::update_motor_equations(void) {
  ...
  // See which mode we're in - use this to compute "the voltage", since it's a
  ...
  } else // Must be TPIM, but elimination!
  {
    ...
    V_current = (voltage[0].Mag() + voltage[1].Mag() + voltage[2].Mag()) /
                (3.0 * nominal_voltage);
  }
  ...
  if (motor_status == statusRUNNING) {
    ...
    if (V_current > Vbrk) {
      P_val = (P_0 + K_p1 * pow((V_current - Vbrk), N_p1)) *
              (1.0 + CmpKpf * delta_f_val);
      Q_val = (Q_0 + K_q1 * pow((V_current - Vbrk), N_q1)) *
              (1.0 + CmpKqf * delta_f_val);
    } else if ((V_current < Vbrk) && (V_current > Vstallbrk)) {
      P_val = (P_0 + K_p2 * pow((V_current - Vbrk), N_p2)) *
              (1.0 + CmpKpf * delta_f_val);
      Q_val = (Q_0 + K_q2 * pow((V_current - Vbrk), N_q2)) *
              (1.0 + CmpKqf * delta_f_val);
    ...
    // If goes above Vrest, restart - this will delay one timestep, but meh -
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_performance_motor(OBJECT *obj,
                                                       TIMESTAMP t0,
                                                       PASSCONFIG pass) {
...
extern "C" MODULE_API int isa_performance_motor(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_performance_motor_impl(obj, classsname);
}
#endif
 ...
 * DELTA MODE
 */
EXPORT SIMULATIONMODE interupdate_performance_motor(
  ...
  try {
    ...
    gl_error("interupdate_performance_motor(obj=%d;%s): %s", obj->id,
