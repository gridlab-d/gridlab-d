/** $Id: power_metrics.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The metric analysis object reports reliability metrics for a system by
 observing the duration and number of conditions when meter objects are not
 normal.  The following metrics are calculated annual and reported to the \p
 report_file

        -# \e SAIFI (System average interrupts frequency index) indicates how
 often the average customer experiences a system interruption over a predefined
 period of time.  This is calculated using the formula

           \f[ SAIFI = \frac{\Sigma N_{customer\_interruptions}}{N_{customers}}
 \f]

        -# \e SAIDI (System average interrupts duration index) indicates the
 total duration of interrupt for the average customer during a predefined period
 of time.  This is calculated using the formula

           \f[ SAIDI = \frac{\Sigma T_{customer\_interruptions}}{N_{customers}}
 \f]

        -# \e CAIDI (Customer average interrupt duration index) represents the
 average time required to restore service.  This is calculated using the formula

                \f[ CAIDI = \frac{\Sigma
 T_{customer\_interruptions}}{N_{customers\_interrupted}} \f]

        -# \e MAIFI (Momentary average interruption frequency index) represents
 the average frequency of momentary interruptions.  This is calculated using the
 formula

                \f[ MAIFI = \frac{\Sigma T_{{momentary
 customer}\_interruptions}}{N_{customers}} \f]

        See <b>IEEE Std 1366-2003</b> <i>IEEE Guide for Electric Power
 Distribution Reliability Indices</i> for more information on how to compute
...
power_metrics::power_metrics(MODULE *mod) : powerflow_library(mod) {
  if (oclass == nullptr) {
    oclass =
        gl_register_class(mod, "power_metrics", sizeof(power_metrics), 0x00);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_double, "SAIFI", PADDR(SAIFI), PT_DESCRIPTION,
            "Displays annual SAIFI values as per IEEE 1366-2003", PT_double,
            "SAIFI_int", PADDR(SAIFI_int), PT_DESCRIPTION,
            "Displays SAIFI values over the period specified by "
            "base_time_value as per IEEE 1366-2003",
            PT_double, "SAIDI", PADDR(SAIDI), PT_DESCRIPTION,
            "Displays annual SAIDI values as per IEEE 1366-2003", PT_double,
            "SAIDI_int", PADDR(SAIDI_int), PT_DESCRIPTION,
            "Displays SAIDI values over the period specified by "
            "base_time_value as per IEEE 1366-2003",
            PT_double, "CAIDI", PADDR(CAIDI), PT_DESCRIPTION,
            "Displays annual CAIDI values as per IEEE 1366-2003", PT_double,
            "CAIDI_int", PADDR(CAIDI_int), PT_DESCRIPTION,
            "Displays SAIDI values over the period specified by "
            "base_time_value as per IEEE 1366-2003",
            PT_double, "ASAI", PADDR(ASAI), PT_DESCRIPTION,
            "Displays annual AISI values as per IEEE 1366-2003", PT_double,
            "ASAI_int", PADDR(ASAI_int), PT_DESCRIPTION,
            "Displays AISI values over the period specified by base_time_value "
            "as per IEEE 1366-2003",
            PT_double, "MAIFI", PADDR(MAIFI), PT_DESCRIPTION,
            "Displays annual MAIFI values as per IEEE 1366-2003", PT_double,
            "MAIFI_int", PADDR(MAIFI_int), PT_DESCRIPTION,
            "Displays MAIFI values over the period specified by "
            "base_time_value as per IEEE 1366-2003",
            PT_double, "base_time_value[s]", PADDR(stat_base_time_value),
            PT_DESCRIPTION, "time period over which _int values are claculated",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    if (gl_publish_function(oclass, "calc_metrics",
                            (FUNCTIONADDR)calc_pfmetrics) == nullptr)
      GL_THROW("Unable to publish metrics calculation function");
    if (gl_publish_function(oclass, "reset_interval_metrics",
                            (FUNCTIONADDR)reset_pfinterval_metrics) == nullptr)
      GL_THROW("Unable to publish interval metrics reset function");
    if (gl_publish_function(oclass, "reset_annual_metrics",
                            (FUNCTIONADDR)reset_pfannual_metrics) == nullptr)
      GL_THROW("Unable to publish annual metrics reset function");
    if (gl_publish_function(oclass, "init_reliability",
                            (FUNCTIONADDR)init_pf_reliability_extra) == nullptr)
      GL_THROW(
          "Unable to publish powerflow reliability initialization function");
    if (gl_publish_function(oclass, "logfile_extra",
                            (FUNCTIONADDR)logfile_extra) == nullptr)
      GL_THROW("Unable to publish powerflow reliability metrics extra header "
               "function");
  }
}
...
int power_metrics::init(OBJECT *parent) {
  // Nothing to do in here - all variables are outputs for now, so if user
  // overrode them, we'll override them back!
  return 1;
}

// ASAI Calculation - Average Service Availability Index
// ASAI = ((total number of customers * number of hours/year) - sum(restoration
// time per event * number of customers interrupted per event)) / (total number
// of customers * number of hours/year)
void power_metrics::calc_ASAI(void) {
  double yearhours;

  if (num_cust_total != 0) {
    yearhours = num_cust_total * 8760.0; // Nt * hours/year
    ASAI_num += ((double)(outage_length * num_cust_interrupted)) /
                3600.0; // In hours (TIMESTAMP assumed to be seconds-level)
    ASAI_num_int += ((double)(outage_length * num_cust_interrupted)) /
                    3600.0; // In hours (TIMESTAMP assumed to be seconds-level)
...
// SAIDI = sum(restoration time per event * number of customers interrupted per
// event) / total number of customers
void power_metrics::calc_SAIDI(void) {
  if (num_cust_total != 0) {
    SAIDI_num += outage_length_normalized * ((double)(num_cust_interrupted));
    SAIDI_num_int +=
        outage_length_normalized * ((double)(num_cust_interrupted));
...
// SAIFI = sum(number of customers interrupted per event) / total number of
// customers
...
// MAIFI = sum(number momentary interrupts * number customers affected) / total
// number of customers
void power_metrics::calc_MAIFI(void) {
  if (num_cust_total != 0) {
    MAIFI_num +=
        reliability_metrics_recloser_counts * num_cust_momentary_interrupted;
    MAIFI_num_int +=
        reliability_metrics_recloser_counts * num_cust_momentary_interrupted;

    // See if it was a "momentary" outage - if so, add extra data
    if (outage_length <
        300) // Less than 5 minutes - "normal" failures are considered momentary
    {
...
void power_metrics::perform_rel_calcs(int number_int, int number_int_secondary,
                                      int total_cust, TIMESTAMP rest_time_val,
                                      TIMESTAMP base_time_val) {
  ...
  // Momentary gets done all times - just in case a "sustained" outage had
  // momentary connotations
  calc_MAIFI();
}
...
void power_metrics::reset_metrics_variables(bool annual_metrics) {
  ...
  // Now check annual
  if (annual_metrics) // ASAI for whole simulation should really be reset here
                      // as well, but no rolling window implemented
  {
    ASAI_num_int =
        0.0; // Default ASAI to perfect reliability (no interruptions)
  }
}

// Function to check for fault_check object (needed) and to make sure it is in a
// proper mode
void power_metrics::check_fault_check(void) {
  ...
  if (!is_fault_check_tested) {
    ...
    if (fault_check_object == nullptr) {
      GL_THROW("power_metrics failed to map fault_check object - ensure it is "
               "near the top of the GLM!");
      /*  TROUBLESHOOT
      ...
      For best results, put the fault_check object right after all module
      definitions. If the error persists, please submit your code and a bug
    ...
    } else // One is mapped, let's check it
    {
      ...
      // Make sure it worked
      if (!temporary_property->is_valid() ||
          !temporary_property->is_enumeration()) {
        GL_THROW("power_metrics failed to map a property of the fault_check "
                 "object!");
        /*  TROUBLESHOOT
        While attempting to map to one of the fault_check object's properties to
        test it, an error was encountered.  Be sure that property and a
        fault_check object exist and try again.  If the error persists, please
        submit your code via the issues tracker.
        */
      }
      ...
      if ((temp_enum_value != 1) && (temp_enum_value != 2)) {
        GL_THROW(
            "fault_check must be in the proper mode for reliabilty to work!");
        /*  TROUBLESHOOT
        To properly work with reliability, fault_check must be in the ALL or
        ONCHANGE check_mode.  Please set it appropriately and try again.
        */
      }
      ...
      // Map the property
      temporary_property =
          new gld_property(fault_check_object, "reliability_mode");
      ...
      //** See if it has an eventgen object, to know if unscheduled faults may
      //work too **//
      temporary_property =
          new gld_property(fault_check_object, "eventgen_object");

      // Make sure it worked
      if (!temporary_property->is_valid() ||
          !temporary_property->is_objectref()) {
        GL_THROW("power_metrics failed to map a property of the fault_check "
                 "object!");
        // Defined above
      }
      ...
      if (temp_object_value == nullptr) {
        gl_warning("No eventgen object mapped up to %s, unscheduled faults are "
                   "not allowed",
                   fault_check_object->name);
        /*  TROUBLESHOOT
        No eventgen object is specified on the fault_check field
        eventgen_object.  Without this specified, "unscheduled" faults can not
        occur and will simply stop the simulation.  This includes fuses blowing
        and "non-faulted" switch operations.
        */
      }
...
EXPORT int calc_pfmetrics(OBJECT *callobj, OBJECT *calcobj, int number_int,
                          int number_int_secondary, int total_customers,
                          TIMESTAMP rest_time_val, TIMESTAMP base_time_val) {
  // Link to it
  power_metrics *pmetrics_obj;
  pmetrics_obj = object_data<power_metrics>(calcobj);

  // Perform the calculation - pass into a class function so variables can be
  // written willy-nilly
  pmetrics_obj->perform_rel_calcs(number_int, number_int_secondary,
                                  total_customers, rest_time_val,
                                  base_time_val);

  return 1; // Success!
}

// Exported function for reliability module to call to reset "interval
// (base_time_val) metrics
 ...
 */
static TIMESTAMP sync_power_metrics_impl(OBJECT *obj, TIMESTAMP t1,
                                         PASSCONFIG pass) {
  return TS_NEVER;
}

#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_power_metrics(OBJECT *obj, TIMESTAMP t1,
                                                   PASSCONFIG pass) {
  return sync_power_metrics_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_power_metrics(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
