...
fault_check::fault_check(MODULE *mod) : powerflow_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "fault_check", sizeof(fault_check),
                               PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "check_mode", PADDR(fcheck_state),
            PT_DESCRIPTION, "Frequency of fault checks", PT_KEYWORD, "SINGLE",
            (enumeration)SINGLE, PT_KEYWORD, "ONCHANGE", (enumeration)ONCHANGE,
            PT_KEYWORD, "ALL", (enumeration)ALLT, PT_KEYWORD, "SINGLE_DEBUG",
            (enumeration)SINGLE_DEBUG, PT_KEYWORD, "SWITCHING",
            (enumeration)SWITCHING, PT_char1024, "output_filename",
            PADDR(output_filename), PT_DESCRIPTION,
            "Output filename for list of unsupported nodes", PT_bool,
            "reliability_mode", PADDR(reliability_mode), PT_DESCRIPTION,
            "General flag indicating if fault_check is operating under "
            "faulting or restoration mode -- reliability set this",
            PT_bool, "strictly_radial", PADDR(reliability_search_mode),
            PT_DESCRIPTION,
            "Flag to indicate if a system is known to be strictly radial -- "
            "uses radial assumptions for reliability alterations",
            PT_bool, "full_output_file", PADDR(full_print_output),
            PT_DESCRIPTION,
            "Flag to indicate if the output_filename report contains both "
            "supported and unsupported nodes -- if false, just does "
            "unsupported",
            PT_bool, "grid_association", PADDR(grid_association_mode),
            PT_DESCRIPTION,
            "Flag to indicate if multiple, distinct grids are allowed in a "
            "GLM, or if anything not attached to the master swing is removed",
            PT_object, "eventgen_object", PADDR(rel_eventgen), PT_DESCRIPTION,
            "Link to generic eventgen object to handle unexpected faults",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);
    if (gl_publish_function(oclass, "reliability_alterations",
                            (FUNCTIONADDR)powerflow_alterations) == nullptr)
      GL_THROW("Unable to publish remove from service function");
    if (gl_publish_function(oclass, "handle_sectionalizer",
                            (FUNCTIONADDR)handle_sectionalizer) == nullptr)
      GL_THROW("Unable to publish sectionalizer special function");
    if (gl_publish_function(oclass, "island_removal_function",
                            (FUNCTIONADDR)powerflow_disable_island) == nullptr)
      GL_THROW("Unable to publish island deletion function");
    if (gl_publish_function(oclass, "rescan_topology",
                            (FUNCTIONADDR)powerflow_rescan_topo) == nullptr)
...
int fault_check::create(void) {
  ...
  phases = PHASE_A; // Arbitrary - set so powerflow_object shuts up (library
                    // doesn't grant us access to synch)
  ...
  reliability_mode = false; // By default, we find errors and fail - if true,
                            // dumps log, but continues on its merry way

  reliability_search_mode = true; // By default (and for speed), assumes the
                                  // system is truly, strictly radial

  rel_eventgen = nullptr; // No object linked by default

  restoration_fxn = nullptr; // No restoration function mapped by default

  grid_association_mode = false; // By default, we go to normal "Highlander"
                                 // grid (there can be only one!)
...
int fault_check::init(OBJECT *parent) {
  ...
  if (fault_check_object == nullptr) // Make sure we're the only one
  {
    ...
    // Make sure the existing one is not us - just in case the deferred init
    // fired
    if (fault_check_object != obj) {
      GL_THROW("Only one fault_check object is supported at this time");
      /*  TROUBLESHOOT
      At this time, a .GLM file can only contain one fault_check object.  Future
      implementations may change this.  Please restrict yourself to one
      fault_check object in the mean time.
      */
    }
  ...
  if (solver_method == SM_NR) {
    // Swing tracking variable - we have to be one below the SWING, so make sure
    // it settled
    ...
    GL_THROW("Fault checking object is only valid for the Newton-Raphson "
             "solver at this time.");
    /*  TROUBLESHOOT
    The fault_check object only works with the Newton-Raphson powerflow solver
  ...
  if (rel_eventgen != nullptr) {
    if (!gl_object_isa(rel_eventgen, "eventgen")) {
      gl_error("fault_check:%s %s is not an eventgen object. Please specify "
               "the name of an eventgen object.",
               obj->name, rel_eventgen->name);
      return 0;
      /*  TROUBLESHOOT
      The property eventgen_object was given the name of an object that is not
      an eventgen object. Please provide the name of an eventgen object located
      in your file.
      */
    }
  ...
  if (grid_association_mode) {
    if (reliability_search_mode) {
      gl_warning("fault_check:%s was forced into meshed search mode due to "
                 "multiple grids being desired",
                 obj->name ? obj->name : "Unnamed");
      /*  TROUBLESHOOT
      While starting up a fault_check object, the grid_association flag was set.
      This flag requires the meshed topology checking routines to be active, so
      that capability was enabled.  If this is undesired, please deactivate
      grid_association capability.
      */
  ...
  if (NR_island_fail_method && !grid_association_mode) {
    gl_error("fault_check:%s - powerflow::NR_island_fail_method is set, but "
             "grid_association is not!",
             (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The powerflow directive to handle individual island failures
    (NR_island_fail_method) is set to true, but grid_association is not enabled
    in fault_check.  This won't work.  Correct one of them.
    */
    return 0;
  }
  ...
  if (!reliability_search_mode) {
    meshed_fault_checking_enabled =
        true; // Let other powerflow objects know we're a mesh-based check
  }

  // See if we're in "special mode" and set the flag
  if (fcheck_state == SINGLE_DEBUG) {
    // Set the powerflow global -- all checks will be off of this (so no crazy
    // player manipulations allowed)
...
TIMESTAMP fault_check::sync(TIMESTAMP t0) {
  ...
  if (prev_time ==
      0) // First run - see if restoration exists (we need it for now)
  {
    allocate_alterations_values(reliability_mode);

    // Override tret to force a reiteration - mostly to catch "initially
    // islanded nodes" in single runs
    tret = t0;
  }

  if (fault_check_override_mode) // Special mode -- do a single topology check
                                 // and then fail
  {
    perform_check = true;
  } else if ((fcheck_state == SINGLE) &&
             (prev_time == 0)) // Single only occurs on first iteration
  {
    perform_check = true; // Flag for a check
  } // end single check
  else if ((fcheck_state == ONCHANGE) &&
           NR_admit_change) // Admittance change has been flagged
  {
    perform_check = true; // Flag the check
  } // end onchange check
  else if ((fcheck_state == SWITCHING) &&
           NR_admit_change) // Admittance change has been flagged
  {
    perform_check = true;
  } else if (fcheck_state == ALLT) // Must be every iteration then
  {
    ...
    perform_check =
        true; // Flag the check - easist way to force the reassociation
  } else      // Doesn't fit one of the above
  {
    perform_check = false; // Flag not-a-check
  }

  if (perform_check) // Each "check" is identical - split out here to avoid 3x
                     // replication
  {
    ...
    if ((restoration_object != nullptr) && !fault_check_override_mode) {
      ...
      if (restoration_fxn == nullptr) {
        // Map it
        restoration_fxn = (FUNCTIONADDR)(gl_get_function(
            restoration_object, "perform_restoration"));

        // Check it
        if (restoration_fxn == nullptr) {
          GL_THROW("Unable to map restoration function");
          /*  TROUBLESHOOT
          While attempting to map the restoration configuration function, an
      ...
      {
        gl_warning("fault_check interaction with restoration requires meshed "
                   "checking mode - enabling this now");
        /*  TROUBLESHOOT
        The restoration algorithm is improved by using the meshed mode of
        topology checking for faults.  This has been automatically enabled for
        the object.  If this is undesired, do not use fault_check with
        restoration.
        */
      ...
      return_val =
          ((int (*)(OBJECT *, int))(*restoration_fxn))(restoration_object, -99);

      // Make sure it worked
      if (return_val != 1) {
        GL_THROW("Restoration failed to execute properly");
        /*  TROUBLESHOOT
        While attempting to call the restoration/reconfiguration, an error was
        encountered in the function. Please look for other error messages from
        restoration itself.
        */
      }

      // Now break into normal check, just to do one last one to make sure
      // things work
    } // End restoration object not null

    // Perform the appropriate check -- occurs one more time after restoration
    // (if it was called)
    if (reliability_search_mode) {
      ...
      if (fcheck_state !=
          SWITCHING) // Parse the list - see if anything is broken
      {
        for (index = 0; index < NR_bus_count; index++) {
          if ((Supported_Nodes[index][0] == 0) ||
              (Supported_Nodes[index][1] == 0) ||
              (Supported_Nodes[index][2] == 0)) {
            if (output_filename[0] != '\0') // See if there's an output
            {
              write_output_file(t0, 0); // Write it
            }

            // See what mode we are in - only check reliability mode if we
            // aren't in grid association
            if (!reliability_mode && !fault_check_override_mode &&
                !grid_association_mode) {
              GL_THROW("Unsupported phase on node %s", NR_busdata[index].name);
              /*  TROUBLESHOOT
              An unsupported connection was found on the indicated bus in the
              system.  Since reconfiguration is not enabled, the solver will
              fail here on the next iteration, so the system is broken early.
              */
            } else // Must be in reliability mode
            {
              gl_warning("Unsupported phase on node %s",
                         NR_busdata[index].name);
              /*  TROUBLESHOOT
              An unsupported connection was found on the indicated bus in the
              system.  Since reliability is enabled, the solver will simply
              ignore the unsupported components for now.
              */
            }
    ...
    } else // Mesh, maybe
    {
      ...
      //****************** NOTE - is there a better way to do this with islands
      //now -- need to do twice (pre/post) to catch stragglers! ****//
      if (grid_association_mode) {
        associate_grids();
      }
      ...
      if (NR_curr_bus != NR_bus_count) {
        GL_THROW("fault_check: Incomplete initialization detected - this will "
                 "cause issues");
        /*  TROUBLESHOOT
        The fault_check object's topology checking routine tried to run, but
        detected a system that was not completely initialized.  Look for other
        output message relating to completely isolated and unconnected nodes.
        These must be fixed (and islanded via a switch) before the system can
        proceed.
        */
      ...
      override_output =
          output_check_supported_mesh(); // See if anything changed

      // If full output, do an initial dump
      if ((prev_time == 0) && full_print_output &&
          (fcheck_state != SWITCHING)) {
      ...
      // Default else - not a first timestep or the mode of interest, so just go
      // like normal
      ...
      if (override_output && fcheck_state != SWITCHING) {
        ...
        if (!reliability_mode && !fault_check_override_mode &&
          ...
          /*  TROUBLESHOOT
          An unsupported connection was found on the system.  Since
          reconfiguration is not enabled, the solver will possibly fail here on
          the next iteration, so the system is broken early.
          */
        } else // Must be in reliability mode or one of the proper handling
               // modes
        {
          gl_warning("Unsupported phase on a possibly meshed node");
          /*  TROUBLESHOOT
          An unsupported connection was found on the system.  Since reliability
          is enabled, the solver will simply ignore the unsupported components
          for now.
          */
        }
      }
  ...
  if (fault_check_override_mode) {
    gl_error("fault_check:%d %s -- Special debug mode utilized, simulation "
             "terminating",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    The special "SINGLE_DEBUG" mode has been activated in fault_check.  This
    performs a topology check ignoring some of the phase checking built into
    powerflow.  This is meant only for debugging topological issues, so the
    simulation is terminated prematurely, regardless of if any issues were found
    or not.
    */
    return TS_INVALID;
  } else // Standard mode
  {
    return tret; // Return where we want to go.  If >t0, NR will force us back
                 // anyways
  }
}
...
void fault_check::search_links(int node_int) {
  ...
  for (index = 0; index < NR_busdata[node_int].Link_Table_Size;
       index++) // parse through our connected link
  {
    temp_branch =
        NR_branchdata[NR_busdata[node_int].Link_Table[index]]; // Get connecting
                                                               // link
                                                               // information
    ...
    if (((temp_branch.phases & 0x07) != 0x00) &&
        (*temp_branch.status == LS_CLOSED)) {
      for (indexb = 0; indexb < 3; indexb++) // Handle phases
      {
        work_phases = 0x04 >> indexb; // Pull off the phase reference

        if ((temp_branch.phases & work_phases) ==
            work_phases) // We are of the proper phase
        {
          ...
          {
            from_val = true; // Flag us as the from end (so we don't have to
                             // check it again later)
          } else             // Must be the to
          {
            from_val = false; // Flag us as the to end (so we don't have to
                              // check it again later)
          }

          // See if both sides of this link are already set - if so, don't
          // bother going back in
          if ((Supported_Nodes[temp_branch.to][indexb] == 1) &&
              (Supported_Nodes[temp_branch.from][indexb] == 1))
            both_handled = true;
          ...
          if (!both_handled) {
            ...
            if (from_val) // From end
            {
              ...
              if (Supported_Nodes[temp_branch.from][indexb] == 1) {
                ...
                continue; // Somehow is checking, but neither of our ends are
                          // set - skip us for now
              }
            } else // To end
            {
...
void fault_check::search_links_mesh(int node_int) {
  ...
  if (!grid_association_mode) // Nope, do "normally"
  {
    ...
    for (index = 0; index < NR_busdata[node_int].Link_Table_Size; index++) {
      ...
      if ((valid_phases[node_int] != 0x00) ||
          (valid_phases[node_value] != 0x00)) {
        // Are we a switch
        if ((NR_branchdata[device_value].lnk_type == 4) ||
            (NR_branchdata[device_value].lnk_type == 5) ||
            (NR_branchdata[device_value].lnk_type == 6)) {
        ...
        } else if (NR_branchdata[device_value].lnk_type == 3) // Fuse
        {
          ...
          if (*NR_branchdata[device_value].status == 1) {
            ...
            result_phases =
                (((~NR_branchdata[device_value].faultphases) & 0x07) | 0xF8);

            // Mask out the original
            temp_phases |=
                NR_branchdata[device_value].origphases & result_phases;
  ...
  else // Grid association mode
  {
    ...
    for (index = 0; index < NR_busdata[node_int].Link_Table_Size; index++) {
      ...
      // Check our "contributions" against the other end - use this to determine
      // recursion
      temp_compare_phases =
          (valid_phases[node_value] | (valid_phases[node_int] & temp_phases));
      ...
      if (valid_phases[node_value] != temp_compare_phases) {
        // Populate the phase information - store what we just did (no point
        // doing twice)
...
void fault_check::support_check(int swing_node_int) {
  ...
  gl_verbose("  fault_check::support_check:%s",
             NR_busdata[swing_node_int].name);
  ...
  for (index = 0; index < 3; index++) {
    ...
    if ((NR_busdata[swing_node_int].phases & phase_vals) ==
        phase_vals)                               // Has this phase
      Supported_Nodes[swing_node_int][index] = 1; // Flag it as supported
  }

  // Call the node link-erator (node support check) - call it on the swing, the
  // details are handled inside
  search_links(swing_node_int);
}

// Mesh-capable version of support check -- by default, it doesn't support
// restoration object
void fault_check::support_check_mesh(void) {
  ...
  if (!grid_association_mode) // Not needing to do grid association, use normal,
                              // inefficient method
  {
    // Swing node has support - if the phase exists (changed for complete
    // faults)
    valid_phases[0] = NR_busdata[0].phases & 0x07;

    // Call the node link-erator (node support check) - call it on the swing,
    // the details are handled inside Supports possibly meshed topology - brute
    // force method
  ...
  {
    // Traverse the whole bus list, just in case (since may be altered in the
    // future)
    for (indexa = 0; indexa < NR_bus_count; indexa++) {
      // See if we're a SWING node
      if ((NR_busdata[indexa].type == 2) ||
          ((NR_busdata[indexa].type == 3) &&
           NR_busdata[indexa].swing_functions_enabled) ||
          ((*NR_busdata[indexa].busflag & NF_ISSOURCE) ==
           NF_ISSOURCE)) // SWING node, of some form
      {
        // Check and see if we've apparently been "sourced" before
        if ((NR_busdata[indexa].phases & 0x07) !=
            valid_phases[indexa]) // We don't match, so something came to us,
...
void fault_check::reset_support_check(void) {
  ...
  // Reset the node - 0 = unsupported, 1 = supported (not populated here), 2 =
  // N/A (no phase there)
  for (index = 0; index < NR_bus_count; index++) {
...
void fault_check::write_output_file(TIMESTAMP tval, double tval_delta) {
  ...
  // Do a deltamode check - fault check is not really privy to what mode we're
  // in -- it doesn't care
  ...
  for (index = 0; index < NR_bus_count;
       index++) // Loop through all bus values - find the unsupported section
  {
    if (reliability_search_mode) {
    ...
    {
      phase_outs = ((~(valid_phases[index] & NR_busdata[index].origphases)) &
                    (0x07 & NR_busdata[index].origphases));
    }
    ...
    if (phase_outs != 0x00) // Anything unsupported?
    {
      ...
      if (!headerwritten) {
        if (!fault_check_override_mode) {
          ...
          } else // Event-based mode
          {
            ...
            fprintf(FPOutput,
                    "Unsupported at timestamp %lld - %04d-%02d-%02d "
                    "%02d:%02d:%02d =\n\n",
                    tval, temp_time.year, temp_time.month, temp_time.day,
                    temp_time.hour, temp_time.minute, temp_time.second);
          }
        } else // Special debug mode
        {
          // Write it
          fprintf(FPOutput, "Special debug topology check -- phase checks "
                            "bypassed -- Unsupported nodes list\n\n");
        }
      ...
      {
        GL_THROW("Error parsing unsupported phases on node %s",
                 NR_busdata[index].name);
        /*  TROUBLESHOOT
        The output file writing routine for the fault_check object encountered a
        problem determining which phases are unsupported on the indicated node.
        Please try again. If the error persists, submit your code and a bug
  ...
  // See if we made it here without writing a header -- if we're in special
  // mode, indicate as much
  if (!headerwritten && fault_check_override_mode) {
    fprintf(FPOutput, "Special debug topology check -- phase checks bypassed "
                      "-- No unsupported nodes found\n\n");
  }
  ...
  if (full_print_output) {
         ...
         index++) // Loop through all bus values - find the unsupported section
    {
      ...
      if (phase_outs != 0x00) // Supported
      {
        ...
        if (!fault_check_override_mode) // Standard mode
        {
          ...
          if (!headerwritten) {
            ...
            } else // Event-based mode
            {
              ...
              fprintf(FPOutput,
                      "Supported at timestamp %lld - %04d-%02d-%02d "
                      "%02d:%02d:%02d =\n",
                      tval, temp_time.year, temp_time.month, temp_time.day,
                      temp_time.hour, temp_time.minute, temp_time.second);
          ...
          } else if (!supportheaderwritten) // Tiemstamp written, but not second
                                            // header
          {
            if (!grid_association_mode) {
              fprintf(FPOutput, "\nSupported Nodes\n");
        ...
        } else // Special debug mode
        {
          ...
          if (!headerwritten) {
            if (!grid_association_mode) {
              // Write it
              fprintf(FPOutput, "Special debug topology check -- phase checks "
                                "bypassed -- Supported nodes list\n\n");
            } else {
              // Write it
              fprintf(FPOutput,
                      "Special debug topology check -- phase checks bypassed "
                      "-- Supported nodes list -- %d Islands detected\n\n",
                      NR_islands_detected);
            }
        ...
        default: // How'd we get here?
        {
          GL_THROW("Error parsing supported phases on node %s",
                   NR_busdata[index].name);
          /*  TROUBLESHOOT
          The output file writing routine for the fault_check object encountered
          a problem determining which phases are supported on the indicated
          node.  Please try again. If the error persists, submit your code and a
...
// Function to traverse powerflow and remove/restore now unsupported objects'
// phases (so NR solves happy)
void fault_check::support_check_alterations(int baselink_int, bool rest_mode) {
  ...
  if (reliability_search_mode) // Strictly radial assumption, continue as always
  {
    ...
    if ((baselink_int == -99) ||
        (baselink_int == -77)) // Swing or initial-type fault
      base_bus_val = 0;        // Swing is always 0th position
    else                       // Not swing, see what our to side is
    {
      gl_verbose("  fault_check::support_check_alterations:%s:%d",
                 NR_branchdata[baselink_int].name, rest_mode);
      // Get to side
      base_bus_val = NR_branchdata[baselink_int].to;

      // Assuming radial, now make the system happy by removing/restoring
      // unsupported phases
      if (rest_mode) // Restoration
        NR_busdata[base_bus_val].phases |=
            (NR_branchdata[baselink_int].phases & 0x07);
      else // Removal mode
        NR_busdata[base_bus_val].phases &=
            (NR_branchdata[baselink_int].phases & 0x07);
    }
    ...
    // See if the FROM side of our newly restored greatness is supported.  If it
    // isn't, there's no point in proceeding
    if (rest_mode) // Restoration
    {
      gl_verbose("fault_check: alterations support check called restoration on "
                 "bus %s with phases %d",
                 NR_busdata[base_bus_val].name,
                 int(NR_busdata[base_bus_val].phases));

      if ((NR_busdata[base_bus_val].phases & 0x07) !=
          0x00) // We have phase, means OK above us
      {
        // Recurse our way in - adjusted version of original search_links
        // function above (but no storage, because we don't care now)
        support_search_links(base_bus_val, base_bus_val, rest_mode);
      } else
        return; // No phases above us, so must be higher level fault.  No sense
                // proceeding then
    } else      // Destructive!
    {
      gl_verbose("fault_check: alterations support check called removal on bus "
                 "%s with phases %d",
                 NR_busdata[base_bus_val].name,
                 int(NR_busdata[base_bus_val].phases));

      // Recurse our way in - adjusted version of original search_links function
      // above (but no storage, because we don't care now)
      support_search_links(base_bus_val, base_bus_val, rest_mode);
    }
  } // End strictly radial assumption
  else // Not assumed to be strictly radial, or just being safe -- check
       // EVERYTHING
  {
    ...
    if (!restoration_checks_active) {
      ...
      if ((restoration_object != nullptr) && (restoration_fxn != nullptr)) {
        // Call restoration -- fault_checks will occur as part of this
        return_val = ((int (*)(OBJECT *, int))(*restoration_fxn))(
            restoration_object, baselink_int);
    ...
    // Now loop through and remove those components that are not supported
    // anymore -- start from SWING, just because we have to start somewhere
    support_search_links_mesh();
  ...
  // Determine if an output is desired and if we're not in restoration check
  // mode (otherwise, it may flood the output log)
  if (!restoration_checks_active && (output_filename[0] != '\0')) {
    ...
    // Perform the actual write -- may duplicate times (if an event calls after
    // sync, but shows what may be changing from an event)
...
// Checks against support found, not an assumed radial traversion (little
// slower, but more thorough) Based on support_search_links below
void fault_check::support_search_links_mesh(void) {
  ...
  for (indexval = 0; indexval < NR_bus_count; indexval++) {
    // Check our location in the support check -- see if we're supported or not
    // and alter as necessary

    // Mask out our phases -- see what is available - mostly just to avoid
    // alterations if nothing changed
    ...
    if (temp_phases != valid_phases[indexval]) {
      ...
      if (work_phases != 0x00) {
        ...
        if ((NR_busdata[indexval].origphases & 0x80) == 0x80) {
          work_phases |= (NR_busdata[indexval].origphases &
                          0xE0); // SP, House?, To SPCT - flagged on
        } else if (work_phases ==
                   0x07) // Fully connected, we can pass D and diff conns
        {
          work_phases |= (NR_busdata[indexval].origphases & 0x18); // D
        }
      }
      // Default else - we're completely valid phase-less, so just set to no
      // phases
      ...
      for (index = 0; index < NR_busdata[indexval].Link_Table_Size;
           index++) // parse through our connected link
      {
        ...
        {
          // See which phases are even allowed - initial mask of from/to valid
          // phases
          temp_phases = (valid_phases[NR_branchdata[device_index].from] &
                         valid_phases[NR_branchdata[device_index].to]);
          ...
          if (work_phases != 0x00) // We have some valid phasing
          {
            ...
            if ((NR_branchdata[device_index].origphases & 0x80) == 0x80) {
              work_phases |= (NR_branchdata[device_index].origphases &
                              0xE0); // Mask in SPCT-type flags
            }
          } // End valid phases
...
void fault_check::special_object_alteration_handle(int branch_idx) {
  ...
  gl_verbose("  fault_check::special_object_alteration_handle:%s",
             NR_branchdata[branch_idx].name);
  ...
  if (!meshed_fault_checking_enabled) {
    ...
    if (NR_branchdata[branch_idx].lnk_type == 4) {
      ...
      // Make sure it worked
      if (temp_obj == nullptr) {
        GL_THROW("Failed to find switch object:%s for reliability manipulation",
                 NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attemping to map to an object for reliability-based modifications,
        GridLAB-D failed to find the object of interest. Please try again.  If
        they error persists, please submit your code and a bug report to the
        trac website.
        */
      }
      ...
      // map the function
      funadd =
          (FUNCTIONADDR)(gl_get_function(temp_obj, "reliability_operation"));

      // make sure it worked
      if (funadd == nullptr) {
        GL_THROW("Failed to find reliability manipulation method on object %s",
                 NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attempting to handle special reliability actions on a "special"
        device (switch, recloser, etc.), the function required was not located.
        Ensure this object type supports special actions and try again.  If the
        problem persists, please submit a bug report and your code to the trac
        website.
        */
      }
      ...
      return_val =
          ((int (*)(OBJECT *, unsigned char))(*funadd))(temp_obj, temp_phases);

      if (return_val == 0) // Failed :(
      {
        GL_THROW("Failed to handle reliability manipulation on %s",
                 NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attempting to handle special reliability actions on a "special"
    ...
    } else if (NR_branchdata[branch_idx].lnk_type == 3) // See if we're a fuse
    {
      ...
      // Make sure it worked
      if (temp_obj == nullptr) {
        GL_THROW("Failed to find fuse object:%s for reliability manipulation",
                 NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attemping to map to an object for reliability-based modifications,
        GridLAB-D failed to find the object of interest. Please try again.  If
    ...
    else if (NR_branchdata[branch_idx].lnk_type == 6) // Recloser
    {
      ...
      if (temp_obj == nullptr) {
        GL_THROW(
            "Failed to find recloser object:%s for reliability manipulation",
            NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attemping to map to an object for reliability-based modifications,
        GridLAB-D failed to find the object of interest. Please try again.  If
      ...
      funadd = (FUNCTIONADDR)(gl_get_function(
          temp_obj, "recloser_reliability_operation"));
    ...
    else if (NR_branchdata[branch_idx].lnk_type == 5) // Sectionalizer
    {
      ...
      // Make sure it worked
      if (temp_obj == nullptr) {
        GL_THROW("Failed to find sectionalizer object:%s for reliability "
                 "manipulation",
                 NR_branchdata[branch_idx].name);
        /*  TROUBLESHOOT
        While attemping to map to an object for reliability-based modifications,
      ...
      // map the function
      funadd = (FUNCTIONADDR)(gl_get_function(
          temp_obj, "sectionalizer_reliability_operation"));
...
void fault_check::support_search_links(int node_int, int node_start,
                                       bool impact_mode) {
  unsigned int index;
  ...
  gl_verbose("  fault_check::support_search_links:%s:%s:%d",
             NR_busdata[node_int].name, NR_busdata[node_start].name,
             impact_mode);
  // Loop through the connectivity and populate appropriately
  for (index = 0; index < NR_busdata[node_int].Link_Table_Size;
       index++) // parse through our connected link
  {
    ...
    if (temp_branch.from == node_int) // We're the from
    {
      from_val = true; // Flag us as the from end (so we don't have to check it
                       // again later)
    } else             // Must be the to
    {
      from_val = false; // Flag us as the to end (so we don't have to check it
                        // again later)
    }

    if ((node_int == node_start) &&
        !from_val) // We're the TO side of the base node, Oh Noes!
    {
      Alteration_Nodes[temp_branch.from] =
          1;    // Flag us to prevent future issues (not sure how they'd happen)
      continue; // Nothing to do with this link, so I hereby render this
                // iteration useless and proceed to skip it
    } else      // FROM side of any, or not the TO side as the base node
    {
      // See if both sides of this link are already set - if so, don't bother
      // going back in
      if ((Alteration_Nodes[temp_branch.to] == 1) &&
          (Alteration_Nodes[temp_branch.from] == 1))
        both_handled = true;
    }
    ...
    if (!both_handled) {
      ...
      if (from_val) // From end
      {
        ...
        if (!impact_mode) // Removal time
        {
          ...
          if (Alteration_Nodes[temp_branch.from] == 1) {
            ...
            if ((NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &
                 0x80) == 0x80) {
              // See if any support exists
              if ((NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                       .phases &
                   work_phases) == 0x00) // no longer any support
              {
                // Just remove it all
                NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases =
                    0x00;
              }
              // Defaulted else - do nothing
            } else // Not split-phase, normal - continue
            {
              // Remove components - USBs are typically node oriented, so they
              // aren't included here
              NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &=
                  work_phases;
            }

            // Now apply the phases on the TO end of this branch - first off,
            // get base phases - D is omitted (D unsupported for now)
            work_phases =
                NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &
                0x07;

            // See if the line is a SPCT or Triplex - if so, bring the flag in.
            // If not, clear it
            if ((NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &
                 0x80) == 0x80) {
        ...
        else // Restoration time
        {
          // Make sure our FROM end is valid first - just in case
          if (Alteration_Nodes[temp_branch.from] == 1) {
            // Now see if we can even proceed - if we are a fault blocked area,
            // then go no lower
            phase_restrictions =
                ~(NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                      .faultphases &
                  0x07); // Get unrestricted

            // Check our status
            if (*NR_branchdata[NR_busdata[node_int].Link_Table[index]].status ==
                LS_CLOSED) {
              phase_restrictions &=
                  (NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                       .origphases &
                   0x07); // Mask this with what we used to be
            } else {
              phase_restrictions = 0x00; // Nothing available
            }

            if (phase_restrictions ==
                0x00) // No phases are available below here, go to next
            {
              continue;
            } else // At least one phase is valid, proceed
            {
              // Restore our phase portions - determine by our FROM end and
              // restrictions
              work_phases =
                  NR_busdata[temp_branch.from].phases & phase_restrictions;

              if ((temp_branch.origphases & 0x80) ==
                  0x80) // See if we were split phase - if so and no phases are
                        // present, remove that too for good measure
              {
                if (work_phases != 0x00)
                  work_phases |=
                      (NR_branchdata[NR_busdata[node_int].Link_Table[index]]
              ...
              // Restore components - USBs are typically node oriented, so they
              // aren't explicitly included here
              NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases |=
                  work_phases;
              ...
              // See if the line is a SPCT or Triplex - if so, bring the flag
              // in.  If not, clear it
              if ((NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                       .phases &
                   0x80) == 0x80) {
                work_phases |= (NR_busdata[temp_branch.to].origphases &
                                0xE0); // SP, House?, To SPCT - flagged on
              } else if (work_phases ==
                         0x07) // Fully connected, we can pass D and diff conns
              {
                work_phases |=
                    (NR_busdata[temp_branch.to].origphases & 0x18); // D
              }
      ...
      else // To end
      {
        ...
        if (!impact_mode) // Removal time
        {
          ...
          if (Alteration_Nodes[temp_branch.to] ==
              1) // Implies TO is done, but not FROM.  Basically indicates
                 // reverse flow or a mesh - not necessarily good (solver won't
                 // care)
          {
            // Remove our phase portions - determine by our TO end
            work_phases = NR_busdata[temp_branch.to].phases & 0x07;

            if ((temp_branch.phases & 0x80) ==
                0x80) // See if we are split phase - if so and no phases are
                      // present, remove that too for good measure
            {
              if (work_phases != 0x00)
            ...
            // Now apply the phases on the FROM end of this branch - first off,
            // get base phases - D is omitted (D unsupported for now)
            work_phases =
                NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &
                0x07;
        ...
        else // Restoration time
        {
          ...
          if (Alteration_Nodes[temp_branch.to] == 1) {
            ...
            {
              // Restore our phase portions - determine by our TO end and
              // restrictions
              work_phases =
                  NR_busdata[temp_branch.to].phases & phase_restrictions;
              ...
              if ((NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                       .phases &
                   0x80) == 0x80) {
                work_phases |= (NR_busdata[temp_branch.from].origphases &
                                0xE0); // SP, House?, To SPCT - flagged on
              } else if (work_phases ==
                         0x07) // Fully connected, we can pass D and diff conns
              {
...
// FUnctionalized version of the allocation routine, mainly so it can be called
// by events that start before fault_check's presync has occurred.
void fault_check::allocate_alterations_values(bool reliability_mode_bool) {
  unsigned int index;

  gl_verbose("  fault_check::allocate_alterations_values:%d",
             reliability_mode_bool);
  // Make sure we haven't been allocated before
  if (Supported_Nodes == nullptr) {
    if (restoration_object == nullptr && fcheck_state != SWITCHING) {
      gl_verbose(
          "Restoration object not detected!"); // Put down here because the
                                               // variable may not be populated
                                               // in time for init
                                               /*  TROUBLESHOOT
                                               The fault_check object can use a restoration object in the system.  If the
                                               restoration object is present,                                          unsuccessful node support checks will call
                                               the reconfiguration.
                                               */
    }
    ...
    Supported_Nodes =
        (unsigned int **)gl_malloc(NR_bus_count * sizeof(unsigned int *));

    if (Supported_Nodes == nullptr) {
      GL_THROW("fault_check: node status vector allocation failure");
      /*  TROUBLESHOOT
      The fault_check object has failed to allocate the node information vector.
      Please try again and if the problem persists, submit your code and a bug
    ...
    for (index = 0; index < NR_bus_count; index++) {
      Supported_Nodes[index] =
          (unsigned int *)gl_malloc(3 * sizeof(unsigned int));
    ...
    if (reliability_mode_bool || !reliability_search_mode) {
      ...
      if (Alteration_Nodes == nullptr) {
        GL_THROW(
            "fault_check: node alteration status vector allocation failure");
        /*  TROUBLESHOOT
        The fault_check object has failed to allocate the node alteration
        information vector.  Please try again and if the problem persists,
      ...
      if (!reliability_search_mode) {
        ...
        if (Alteration_Links == nullptr) {
          GL_THROW(
              "fault_check: link alteration status vector allocation failure");
          /*  TROUBLESHOOT
          The fault_check object has failed to allocate the node alteration
          information vector.  Please try again and if the problem persists,
          submit your code and a bug report via the trac website.
      ...
      valid_phases =
          (unsigned char *)gl_malloc(NR_bus_count * sizeof(unsigned char));
...
void fault_check::momentary_activation(int node_int) {
  ...
  // Make sure it worked
  if (tmp_obj == nullptr) {
    GL_THROW("Failed to map node:%s during momentary interruption!",
             NR_busdata[node_int].name);
    /*  TROUBLESHOOT
    While mapping a node object for the momentary fault inducing process, the
    node failed to be found. Please try again and ensure the node exists.  If
  ...
  if ((gl_object_isa(tmp_obj, "triplex_meter", "powerflow")) ||
    ...
    if (pval == nullptr) {
      ...
      /*  TROUBLESHOOT
      While attempting to map the momentary outage flag on a node, it failed to
      find said flag. Please try again. If the error persists, please submit
  ...
  for (index = 0; index < NR_busdata[node_int].Link_Table_Size; index++) {
    ...
    if (NR_branchdata[NR_busdata[node_int].Link_Table[index]].from ==
        node_int) {
      // Recurse our way in!
      momentary_activation(
          NR_branchdata[NR_busdata[node_int].Link_Table[index]].to);
    }
...
void fault_check::reset_associated_grid(void) {
  ...
  for (indexval = 0; indexval < NR_branch_count; indexval++) {
    NR_branchdata[indexval].island_number =
        -1; // Start as "not associated" as well
  }
}

// Multiple grid checking items - search for SWING nodes as the search entry
// points, then populate
void fault_check::associate_grids(void) {
  ...
  for (indexval = 0; indexval < NR_bus_count; indexval++) {
    ...
    if (NR_busdata[indexval].type == 3) // SWING_PQ bus
    {
      ...
      } else // Deflag us as a swing
      {
        ...
        // Leave the "swing_topology_entry" flag as-is - used as flag to
        // determine if a SWING_PQ with a generator suddenly became a main
        // source
      }
      // Default else, we've already been hit, skip out
    }
  ...
  // Third loop -- look for NF_ISSOURCE flags - anything attached to this,
  // should be active
  for (indexval = 0; indexval < NR_bus_count; indexval++) {
    // See if we're a source-flagged node
    if ((*NR_busdata[indexval].busflag & NF_ISSOURCE) ==
        NF_ISSOURCE) // Source flagged
    {
      // See if we're already flagged
      if (NR_busdata[indexval].island_number == -1) // We're still unparsed
      {
  ...
  if (NR_islands_detected != grid_counter) {
    // See if we need to free anything first - basically, see if it already
    // exists
    if ((NR_powerflow.island_matrix_values != nullptr) &&
        (NR_islands_detected != 0)) {
      stat_return_val =
          NR_array_structure_free(&NR_powerflow, NR_islands_detected);

      // Make sure it worked
      if (stat_return_val == FAILED) {
        GL_THROW("fault_check: Failed to free up a multi-island NR solver "
                 "array properly");
        /*  TROUBLESHOOT
        While attempting to free up one of the multi-island solver variable
        arrays, an error was encountered.  Please try again.  If the error
        persists, please submit your code and a bug report via the
        ticketing/issues system.
        */
      }
    }
    ...
    // Make sure it worked
    if (stat_return_val == FAILED) {
      GL_THROW("fault_check: Failed to allocate a multi-island NR solver array "
               "properly");
      /*  TROUBLESHOOT
      While attempting to allocate a multi-island solver variable array, an
      error was encountered. Please try again.  If the error persists, please
      submit your code and a bug report via the ticketing/issue system.
      */
    }
...
void fault_check::search_associated_grids(unsigned int node_int,
  ...
  for (index = 0; index < NR_busdata[node_int].Link_Table_Size; index++) {
    ...
    // We're theoretically coming from a "powered node", so see if it has any
    // phase alignment to proceed Only do "in service" items, so go on current
    // phases, not original phases
    if (((NR_busdata[node_int].phases & 0x07) &
         (NR_branchdata[NR_busdata[node_int].Link_Table[index]].phases &
          0x07)) != 0x00) {
      ...
      if (NR_busdata[node_ref].island_number == -1) {
        ...
        NR_branchdata[NR_busdata[node_int].Link_Table[index]].island_number =
            grid_counter;
        ...
        /*  TROUBLESHOOT
        While mapping the associated grid/swing node for a system, a condition
        was encountered where a node tried to belong to two different systems.
        This should not have occurred.  Please submit your code and a bug report
        via the ticketing system.
        */
      } else if (NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                     .island_number != grid_counter) // This line is unassigned
      {
        // See if it is fully unassigned
        if (NR_branchdata[NR_busdata[node_int].Link_Table[index]]
                .island_number == -1) {
          // Just got missed by the iterations, so associate us (ends already
          // handled, so no need to recurse)
          NR_branchdata[NR_busdata[node_int].Link_Table[index]].island_number =
              grid_counter;
        } else {
          // Somehow is associated with a different grid, even though the
          // from/to are the same!
          GL_THROW("fault_check: invalid grid assignment on line %s!",
                   NR_branchdata[NR_busdata[node_int].Link_Table[index]].name);
          /*  TROUBLESHOOT
          While mapping the associated grid/swing node for a line in the system,
          a condition was encountered where the line belonged to a grid/island
          it shouldn't.  This should not have occurred.  Please submit your code
...
STATUS fault_check::disable_island(int island_number) {
  ...
  if (!grid_association_mode) {
    gl_error("fault_check: an island removal call was made, but "
             "grid_association is set to false!");
    /*  TROUBLESHOOT
    The powerflow is attempting to run in multi-islanded mode with individual
    island handling, but fault_check is not set up properly.  Please set
    `grid_association true;` in the fault_check and try again.
    */

    return FAILED;
  }

  // Loop through the buses -- remove if it is in this island (keep SWING
  // functions affected though)
  ...
  // If there's an output file, log it in there too (since this is through an
  ...
  if (output_filename[0] != '\0') // See if there's an output
  {
    ...
    // Put a message in the output file, before the topology change (just to be
    // clear) open the file
    FPOutput = fopen(output_filename, "at");
    ...
    if (deltamodeflag == true) {
      ...
      // Write it
      fprintf(FPOutput,
              "Island %d removed from powerflow at timestamp %0.9f - %s =\n\n",
    ...
    } else // Event-based mode
    {
      ...
      fprintf(FPOutput,
              "Island %d removed from powerflow at timestamp %lld - "
              "%04d-%02d-%02d %02d:%02d:%02d =\n\n",
              (island_number + 1), curr_time_val_TS, temp_time.year,
              temp_time.month, temp_time.day, temp_time.hour, temp_time.minute,
              temp_time.second);
    }
  ...
  // Flag a forced reiteration for the next time something can (not now though,
  // we may be halfway through an NR solver loop)
  force_reassociation = true;

  // Output it - no longer as a verbose (easy to miss)
  gl_warning("fault_check: Removed island %d from the powerflow",
             (island_number + 1));
  /*  TROUBLESHOOT
  One of the islands of teh system was removed (by the multi-island capability).
  This may have been due to a divergent powerflow, or some specifically-command
  action.  If this was not intended, check your file and try again.
  */
...
// Function to progress upstream from a sectionalizer and see if a recloser is
// present (and handle any future sectionalizers it encounters)
EXPORT double handle_sectionalizer(OBJECT *thisobj, int sectionalizer_number) {
  ...
  while (!loop_complete) {
    ...
    for (index = 0; index < NR_busdata[node_val].Link_Table_Size;
         index++) // parse through our connected link
    {
      ...
      if (NR_branchdata[branch_val].to == node_val) {
        ...
        if (NR_branchdata[branch_val].lnk_type == 6) {
          ...
          // Make sure it worked
          if (tmp_obj == nullptr) {
            GL_THROW("Failure to map recloser object %s during sectionalizer "
                     "handling",
                     NR_branchdata[branch_val].name);
            /*  TROUBLESHOOT
            While mapping a recloser as part of a sectionalizer's reliability
            function, said recloser failed to be mapped correctly.  Please try
          ...
          // Make sure it worked
          if (Pval == nullptr) {
            GL_THROW("Failed to map recloser:%s retry count!",
                     NR_branchdata[branch_val].name);
            /*  TROUBLESHOOT
            While attempting to map a reclosers attempt count, an error was
        ...
        else // Not a recloser - onwards and upwards
        {
          ...
          if ((NR_busdata[node_val].type == 2) ||
              ((NR_busdata[node_val].type == 3) &&
               NR_busdata[node_val].swing_functions_enabled)) {
          ...
          {
            proper_exit = true; // Flag proper exiting (not loop limit) so next
                                // can proceed
    ...
    if ((index > NR_busdata[node_val].Link_Table_Size) &&
        !proper_exit) // Full traversion - then failure
    {
...
// Function to remove a divergent island from the powerflow, so it isn't handled
// anymore
...
// Function to prompt a topology rescan, likely when a swing comes back into
// service after failing
EXPORT STATUS powerflow_rescan_topo(OBJECT *thisobj,
