...
public:
  ...
  double m; // empirically derived exponent used to calculate the variation of
            // dtheta_H with changes in load.
  double n; // empirically derived exponent used to calculate the variation of
            // dtheta_TO with changes in load.
  ...
  double dtheta_H;    // winding hottest-spot rise over top-oil temperature,
                      // degrees C.
  double dtheta_H_i;  // initial winding hottest-spot rise over top-oil
                      // temperature, degrees C.
  double dtheta_H_R;  // winding hottest-spot rise over top-oil temperature at
                      // rated load, degrees C.
  double dtheta_H_U;  // ultimate winding hottest-spot rise over top-oil
                      // temperature for current load, degrees C.
  double dtheta_TO;   // top-oil hottest-spot rise over ambient temperature,
                      // degrees C.
  double dtheta_TO_i; // initial top-oil hottest-spot rise over ambient
                      // temperature, degrees C.
  double dtheta_TO_U; // ultimate top-oil hottest-spot rise over ambient
                      // temperature for current load, degrees C.
  double t_TOR; // the oil time constant at rated load with dtheta_TO_i = 0
                // degrees C, hrs.
  double t_TO;  // the oil time constant for any load and for any difference
                // between dtheta_TO_U and dtheta_TO_i, hrs.
  ...
  double aging_step; // maximum timestep before updating thermal and aging model
                     // in seconds.
  TIMESTAMP return_at;
  double last_temp;
  ...
  double flux_vals_inst[6]; // Flux values for transformer when saturation is
                            // enabled
