...
public:
  ...
  SIMULATIONMODE inter_deltaupdate_sync_check(unsigned int64 delta_time,
                                              unsigned long dt,
                                              unsigned int iteration_count_val,
                                              bool interupdate_pos);
...
private:
  /* Flags & Buffer */
  bool reg_dm_flag; // Flag for indicating the registration of deltamode (array
                    // & func)
...
private: /* Published Variables (for Measurements of Recorders) */
  ...
  double volt_A_mag_diff_noabs_pu, volt_B_mag_diff_noabs_pu,
...
private: /* Published Variables*/
  ...
  enum VOLT_COMP_MODE { MAG_DIFF = 0, SEP_DIFF = 1 } volt_compare_mode;
...
private:
  ...
  void init_vars();
  void
  init_diff_prop(double = -1); // Measurement Properties (hidden) for Recorders
