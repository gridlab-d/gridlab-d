...
regulator::regulator(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "regulator", sizeof(regulator),
    ...
    if (gl_publish_variable(
            ...
            "reference to the regulator_configuration object used to determine "
            "regulator properties",
            PT_int16, "tap_A", PADDR(tap[0]), PT_DESCRIPTION,
            "current tap position of tap A", PT_int16, "tap_B", PADDR(tap[1]),
            PT_DESCRIPTION, "current tap position of tap B", PT_int16, "tap_C",
            PADDR(tap[2]), PT_DESCRIPTION, "current tap position of tap C",
            PT_enumeration, "msg_mode", PADDR(msgmode), PT_DESCRIPTION,
            "messages regarding remote node voltage to come internally from "
            "gridlabd or externally through co-simulation. Set to EXTERNAL "
            "only if you have co-simulation enabled",
            PT_KEYWORD, "INTERNAL", (enumeration)msg_INTERNAL, PT_KEYWORD,
            "EXTERNAL", (enumeration)msg_EXTERNAL, PT_complex,
            "remote_voltage_A[V]", PADDR(check_voltage[0]), PT_DESCRIPTION,
            "remote node voltage, Phase A to ground", PT_complex,
            "remote_voltage_B[V]", PADDR(check_voltage[1]), PT_DESCRIPTION,
            "remote node voltage, Phase B to ground", PT_complex,
            "remote_voltage_C[V]", PADDR(check_voltage[2]), PT_DESCRIPTION,
            "remote node voltage, Phase C to ground", PT_double,
            "tap_A_change_count", PADDR(tap_A_change_count), PT_DESCRIPTION,
            "count of all physical tap changes on phase A since beginning of "
            "simulation (plus initial value)",
            PT_double, "tap_B_change_count", PADDR(tap_B_change_count),
            PT_DESCRIPTION,
            "count of all physical tap changes on phase B since beginning of "
            "simulation (plus initial value)",
            PT_double, "tap_C_change_count", PADDR(tap_C_change_count),
            PT_DESCRIPTION,
            "count of all physical tap changes on phase C since beginning of "
            "simulation (plus initial value)",
            PT_object, "sense_node", PADDR(RemoteNode), PT_DESCRIPTION,
            "Node to be monitored for voltage control in remote sense mode",
            PT_double, "regulator_resistance[Ohm]", PADDR(regulator_resistance),
            PT_DESCRIPTION,
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_regulator) == nullptr)
...
int regulator::create() {
  ...
  deltamode_reiter_request =
      false; // By default, we're assumed to not want this
...
int regulator::init(OBJECT *parent) {
  ...
  /*  TROUBLESHOOT
  A regulator configuration was not provided.  Please use object
  regulator_configuration and define the necessary parameters of your regulator
  to continue.  See the media wiki for an example:
  ...
  */
  ...
  /*  TROUBLESHOOT
  An invalid regulator configuration was provided.  Ensure you have proper
  values in each field of the regulator_configuration object and that you
  haven't inadvertantly used a line or transformer configuration as the
  regulator configuration.  See the media wiki for an example:
  http://sourceforge.net/apps/mediawiki/gridlab-d/index.php?title=Power_Flow_Guide
  */

  regulator_configuration *pConfig =
      object_data<regulator_configuration>(configuration);
  ...
  if (pConfig->Control == pConfig->REMOTE_NODE) {
    if (RemoteNode == nullptr) {
      GL_THROW("Remote sensing node not found on regulator:%d - %s", obj->id,
               (obj->name ? obj->name : "Unnamed"));
      /* TROUBLESHOOT
      If you are trying to use REMOTE_NODE, then please specify a sense_node
      within the regulator object.  Otherwise, change your Control method.
      */
    } else if (!gl_object_isa(RemoteNode, "node", "powerflow") &&
               !gl_object_isa(RemoteNode, "meter", "powerflow") &&
               !gl_object_isa(RemoteNode, "load", "powerflow") &&
               !gl_object_isa(RemoteNode, "network_interface")) {
      GL_THROW("Remote sensing node is not a valid object in regulator:%d - %s",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      The object specified in the sense_node property is not a node-type or
      network_interface object, so it will not work for the REMOTE_NODE.
      */
    }
    ...
    if (msgmode == msg_INTERNAL) {
      ...
      if (!RNode_voltage[0]->is_valid() || !RNode_voltage[0]->is_complex()) {
        GL_THROW("Regulator:%d - %s - Unable to map property for remote object",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
        /* TROUBLESHOOT
        While attempting to map a property for the sense_node, a property could
  ...
  for (int i = 0; i < 3; i++) {
    ...
    /*  TROUBLESHOOT
    Check the Type specification in your regulator_configuration object.  It can
    an only be type A or B.
    */
  }

  gld::complex tmp_mat[3][3] = {
      {gld::complex(1, 0) / a_mat[0][0], gld::complex(0, 0),
       gld::complex(0, 0)},
      {gld::complex(0, 0), gld::complex(1, 0) / a_mat[1][1],
       gld::complex(0, 0)},
      {gld::complex(-1, 0) / a_mat[0][0], gld::complex(-1, 0) / a_mat[1][1],
       gld::complex(0, 0)}};
  ...
  case regulator_configuration::WYE_WYE:
    ...
    if (solver_method == SM_NR) {
      if (regulator_resistance == 0.0) {
        gl_warning(
            "Regulator:%s regulator_resistance has been set to zero. This will "
        ...
        regulator_resistance to something small but larger that zero.
        */
      }
      if (regulator_resistance < 0.0) {
      ...
      if (has_phase(PHASE_A)) {
        base_admittance_mat[0][0] =
            gld::complex(1.0 / regulator_resistance, 0.0);
  ...
  case regulator_configuration::OPEN_DELTA_ABBC:
    ...
    gl_warning("Only WYE-WYE configurations are working in either "
               "Newton-Raphson or non-Manual controls");
    /*  TROUBLESHOOT
    For this portion of development, only WYE-WYE configurations are fully
    supported.  Later implementations will include support for other regulator
    configurations.  As it stands, WYE-WYE regulators work in SM_NR for both
    Manual and Automatic controls, while OPEN_DELTA_ABBC is only supported in
    single powerflow simulations.
    */
    break;
  case regulator_configuration::OPEN_DELTA_BCAC:
    throw "Regulator connect type not supported yet";
    /*  TROUBLESHOOT
    Check the connection type specified.  Only a few are available at this time.
    Ones available can be found on the wiki website (
    http://sourceforge.net/apps/mediawiki/gridlab-d/index.php?title=Power_Flow_Guide
    )
    */
    break;
  ...
  for (jindex = 0; jindex < 3; jindex++) {
    if (!TapInitialValue[jindex]) {
      ...
      first_run_flag[jindex] =
          0; // Set so the regulator will operate normally here
    }
  }
  ...
  new_reverse_flow_action[0] = new_reverse_flow_action[1] =
      new_reverse_flow_action[2] = false;
  return result;
}
...
TIMESTAMP regulator::presync(TIMESTAMP t0) {
  ...
  // Toggle the iteration variable -- only for voltage-type adjustments (since
  // it's in presync now)
  if ((solver_method == SM_NR) &&
      ((pConfig->Control == pConfig->OUTPUT_VOLTAGE) ||
       (pConfig->Control == pConfig->REMOTE_NODE)))
  ...
  // Force a "reiteration" if we're checking voltage - consequence of this
  // previously being in true pass of NR
  if ((solver_method == SM_NR) &&
      ((pConfig->Control == pConfig->OUTPUT_VOLTAGE) ||
       (pConfig->Control == pConfig->REMOTE_NODE)) &&
      !iteration_flag) {
  ...
  if ((first_run_flag[0] < 1) || (first_run_flag[1] < 1) ||
      (first_run_flag[2] < 1))
    return t1;
  else if (t1_dbl <= next_time)
    return t1;
  else if (next_time != TS_NEVER_DBL)
    return -next_time; // soft return to next tap change
  else
    return TS_NEVER;
}
  ...
  if (pConfig->Control == pConfig->MANUAL) {
    for (int i = 0; i < 3; i++) {
      if (pConfig->Type == pConfig->A) {
        ...
        /*  TROUBLESHOOT
        Check the Type specification in your regulator_configuration object.  It
        can an only be type A or B.
        */
      }
    }
    ...
    if (pConfig->control_level == pConfig->INDIVIDUAL) {
      // Set flags correctly for each pass, 1 indicates okay to change taps, 0
      // indicates no go
      for (int i = 0; i < 3; i++) {
      ...
      if (pConfig->connect_type == pConfig->WYE_WYE) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          {
            // hit the band center for convergence on first run, otherwise bad
            // initial guess on tap settings can fail on the first timestep
            if (first_run_flag[i] == 0) {
              if (toggle_reverse_flow[i]) {
                tap[i] = reverse_flow_tap[i];
              } else {
                tap[i] = tap[i] + (int16)ceil((pConfig->band_center -
                                               check_voltage[i].Mag()) /
                                              VtapChange);
              }
            ...
            // dwelling has happened, and now waiting for actual physical change
            // time
            else if (mech_flag[i] == 0 && dwell_flag[i] == 1 &&
                     (mech_t_next[i] - curr_time_value) >=
                         pConfig->time_delay) {
              mech_t_next[i] = curr_time_value + pConfig->time_delay;
            }
            // if both flags say it's okay to change the tap, then change the
            // tap
            ...
            // only set the dwell time if we've reached the end of the previous
            // dwell (in case other objects update during that time)
            else if (dwell_flag[i] == 0 &&
                     (dwell_t_next[i] - curr_time_value) >=
                         pConfig->dwell_time) {
              dwell_t_next[i] = curr_time_value + pConfig->dwell_time;
          ...
          } else if (check_voltage[i].Mag() > Vhigh) // lower voltage
          {
            if (first_run_flag[i] == 0) {
              if (toggle_reverse_flow[i]) {
                ...
                tap[i] = tap[i] - (int16)ceil((check_voltage[i].Mag() -
                                               pConfig->band_center) /
                                              VtapChange);
              }
          ...
          // If no tap changes were needed, then this resets dwell_flag to 0 and
          // indicates regulator has no more changes unless system changes
          else {
          ...
          if (pConfig->Type == pConfig->A) {
            ...
            GL_THROW("invalid regulator type");
            /*  TROUBLESHOOT
            Check the Type of regulator specified.  Type can only be A or B at
            this time.
            */
          }
        }
        ...
        GL_THROW("Specified connect type is not supported in automatic modes "
                 "at this time.");
      /* TROUBLESHOOT
      At this time only WYE-WYE regulators are supported in automatic control
      modes. OPEN_DELTA_ABBC will only work in MANUAL control mode and in FBS at
      this time.
      */
    }
      ...
      // Set flags correctly for each pass, 1 indicates okay to change taps, 0
      // indicates no go - we'll store all of banked stuff in index=0
        ...
        if (check_voltage[0].Mag() < Vlow) // raise voltage
        {
          ...
          if (first_run_flag[0] == 0) {
            if (toggle_reverse_flow_banked) {
              ...
              tap[0] = tap[1] = tap[2] =
                  tap[0] +
                  (int16)ceil((pConfig->band_center - check_voltage[0].Mag()) /
                              VtapChange);
            }
          ...
          // dwelling has happened, and now waiting for actual physical change
          // time
          else if (mech_flag[0] == 0 && dwell_flag[0] == 1 &&
                   (mech_t_next[0] - curr_time_value) >= pConfig->time_delay) {
            mech_t_next[0] = curr_time_value + pConfig->time_delay;
          }
        ...
        } else if (check_voltage[0].Mag() > Vhigh) // lower voltage
        {
          if (first_run_flag[0] == 0) {
            if (toggle_reverse_flow_banked) {
              ...
              tap[0] = tap[1] = tap[2] =
                  tap[0] -
                  (int16)ceil((check_voltage[0].Mag() - pConfig->band_center) /
                              VtapChange);
            }
            ...
            if (tap[0] < -pConfig->lower_taps) {
              ...
              mech_flag[0] = 0;
                     ...
                     (dwell_t_next[0] - curr_time_value) >=
                         pConfig->dwell_time) {
            dwell_t_next[0] = curr_time_value + pConfig->dwell_time;
            mech_t_next[0] = dwell_t_next[0] + pConfig->time_delay;
          }
...
void regulator::reg_postPre_fxn(void) {
  ...
  if (solver_method == SM_NR) {
    ...
    for (jindex = 0; jindex < 3; jindex++) {
      Ylefttemp[jindex][jindex] =
          Yto[jindex][jindex] * gld::complex(1, 0) / a_mat[jindex][jindex];
      Yfrom[jindex][jindex] = Ylefttemp[jindex][jindex] * gld::complex(1, 0) /
                              a_mat[jindex][jindex];
    }

    // multiply(invratio,Yto,Ylefttemp);		//Scale from admittance
    // by turns ratio multiply(invratio,Ylefttemp,Yfrom);
    ...
    // multiply(invratio,Yto,To_Y);		//Incorporate turns ratio
    // information into line's admittance matrix.
    // multiply(voltage_ratio,Yfrom,From_Y); //Scales voltages to same "level"
    // for GS //uncomment me

    // Only perform update if a tap has changed.  Since the link calculations
    // are replicated here and it directly accesses the NR memory space, this
    // won't cause any issues.
    if ((prev_tap[0] != tap[0]) || (prev_tap[1] != tap[1]) ||
        (prev_tap[2] != tap[2])) // Change has occurred
    {
      // Flag an update
  ...
  if (tap[0] == pConfig->raise_taps && has_phase(PHASE_A)) {
    ...
    gl_warning("Regulator %s has phase %c at the maximum tap value",
               object_header(this)->name, phaseWarn);
    /*  TROUBLESHOOT
    The regulator has set its taps such that it is at the maximum setting.  This
    may indicate a problem with settings, or your system.
    */
  }
  ...
  if (tap[0] == -pConfig->lower_taps && has_phase(PHASE_A)) {
    ...
    gl_warning("Regulator %s has phase %c at the minimum tap value",
               object_header(this)->name, phaseWarn);
    /*  TROUBLESHOOT
    The regulator has set its taps such that it is at the minimum setting.  This
    may indicate a problem with settings, or your system.
    */
  }
  ...
  if (iteration_flag) {
    ...
    if (prev_time == curr_time_value) {
      ...
      if (tap_A_changed == 1) {
        if (initial_tap_A == tap[0]) {
          ...
          tap_A_change_count--;
          if (tap_A_change_count < 0) {
            gl_error("Unusual control of the regulator has resulted in a "
                     "negative tap change count on phase A.");
            return -1.0;
          }
          tap_A_changed = 0;
      ...
      if (tap_B_changed == 1) {
        if (initial_tap_B == tap[1]) {
          ...
          if (tap_B_change_count < 0) {
            gl_error("Unusual control of the regulator has resulted in a "
                     "negative tap change count on phase B.");
            return -1.0;
          }
          tap_B_changed = 0;
      ...
      if (tap_C_changed == 1) {
        if (initial_tap_C == tap[2]) {
          ...
          if (tap_C_change_count < 0) {
            gl_error("Unusual control of the regulator has resulted in a "
                     "negative tap change count on phase C.");
            return -1.0;
          }
          tap_C_changed = 0;
    ...
    if (pConfig->Control != pConfig->MANUAL) {
      ...
      if (pConfig->reverse_flow_control == pConfig->LOCK_NEUTRAL) {
        if (pConfig->control_level == pConfig->INDIVIDUAL) {
          if ((flow_direction & FD_A_MASK) == FD_A_REVERSE &&
              !toggle_reverse_flow[0]) { // Phase A power in reverse
            toggle_reverse_flow[0] = true;
            reverse_flow_tap[0] = 0;
            new_reverse_flow_action[0] = true;
          } else if ((flow_direction & FD_A_MASK) != FD_A_REVERSE &&
                     toggle_reverse_flow[0]) {
            toggle_reverse_flow[0] = false;
          }
          if ((flow_direction & FD_B_MASK) == FD_B_REVERSE &&
              !toggle_reverse_flow[1]) { // Phase A power in reverse
            toggle_reverse_flow[1] = true;
            reverse_flow_tap[1] = 0;
            new_reverse_flow_action[1] = true;
          } else if ((flow_direction & FD_B_MASK) != FD_B_REVERSE &&
                     toggle_reverse_flow[1]) {
            toggle_reverse_flow[1] = false;
          }
          if ((flow_direction & FD_C_MASK) == FD_C_REVERSE &&
              !toggle_reverse_flow[2]) { // Phase A power in reverse
            toggle_reverse_flow[2] = true;
            reverse_flow_tap[2] = 0;
            new_reverse_flow_action[2] = true;
          } else if ((flow_direction & FD_C_MASK) != FD_C_REVERSE &&
                     toggle_reverse_flow[2]) {
            toggle_reverse_flow[2] = false;
          }
        } else if (pConfig->control_level == pConfig->BANK) {
          if (((flow_direction & FD_A_MASK) == FD_A_REVERSE ||
               (flow_direction & FD_B_MASK) == FD_B_REVERSE ||
               (flow_direction & FD_C_MASK) == FD_C_REVERSE) &&
              !toggle_reverse_flow_banked) { // Phase A power in reverse
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (dwell_flag[i] == 1 && mech_flag[i] == 1) {
          if (check_voltage[i].Mag() < Vlow && tap[i] != pConfig->lower_taps &&
              !new_reverse_flow_action[i]) {
            if (pConfig->control_level == pConfig->INDIVIDUAL &&
                !toggle_reverse_flow[i]) {
              return curr_time_value;
            } else if (pConfig->control_level == pConfig->BANK &&
                       !toggle_reverse_flow_banked) {
              return curr_time_value;
            }
          }

          if (check_voltage[i].Mag() > Vhigh &&
              tap[i] != -pConfig->raise_taps && !new_reverse_flow_action[i]) {
            if (pConfig->control_level == pConfig->INDIVIDUAL &&
                !toggle_reverse_flow[i]) {
              return curr_time_value;
            } else if (pConfig->control_level == pConfig->BANK &&
        ...
        if (new_reverse_flow_action[i] &&
            (toggle_reverse_flow[i] || toggle_reverse_flow_banked))
          return curr_time_value;
      }
    }
  }
...
void regulator::get_monitored_voltage() {
  ...
  case 4: // Line Drop Compensation
  {
    if (pConfig->control_level == pConfig->INDIVIDUAL) {
      ...
      if ((double)pConfig->CT_ratio != 0.0) {
        ...
        curr[0] = tmp_mat2[0][0] * current_in[0] +
                  tmp_mat2[0][1] * current_in[1] +
                  tmp_mat2[0][2] * current_in[2];
        curr[1] = tmp_mat2[1][0] * current_in[0] +
                  tmp_mat2[1][1] * current_in[1] +
                  tmp_mat2[1][2] * current_in[2];
        curr[2] = tmp_mat2[2][0] * current_in[0] +
                  tmp_mat2[2][1] * current_in[1] +
                  tmp_mat2[2][2] * current_in[2];

        for (int i = 0; i < 3; i++)
          check_voltage[i] = V2[i] - (curr[i] / (double)pConfig->CT_ratio) *
                                         gld::complex(pConfig->ldc_R_V[i],
                                                      pConfig->ldc_X_V[i]);
      } else {
        for (int i = 0; i < 3; i++)
      ...
      if ((double)pConfig->CT_ratio != 0.0) {
        ...
        if (pConfig->CT_phase == PHASE_A)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              V2[0] -
              (curr[0] / (double)pConfig->CT_ratio) *
                  gld::complex(pConfig->ldc_R_V[0], pConfig->ldc_X_V[0]);

        else if (pConfig->CT_phase == PHASE_B)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              V2[0] -
              (curr[1] / (double)pConfig->CT_ratio) *
                  gld::complex(pConfig->ldc_R_V[1], pConfig->ldc_X_V[1]);

        else if (pConfig->CT_phase == PHASE_C)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              V2[0] -
              (curr[2] / (double)pConfig->CT_ratio) *
                  gld::complex(pConfig->ldc_R_V[2], pConfig->ldc_X_V[2]);
      } else {
  ...
  case 2: // Output voltage
  {
    if (pConfig->control_level == pConfig->INDIVIDUAL) {
      ...
      if (pConfig->PT_phase == PHASE_A)
        check_voltage[0] = check_voltage[1] = check_voltage[2] =
            ToNode_voltage[0]->get_complex();
      else if (pConfig->PT_phase == PHASE_B)
        check_voltage[0] = check_voltage[1] = check_voltage[2] =
            ToNode_voltage[1]->get_complex();
      else if (pConfig->PT_phase == PHASE_C)
        check_voltage[0] = check_voltage[1] = check_voltage[2] =
            ToNode_voltage[2]->get_complex();
    }
  } break;
  case 3: // Remote Node
  {
...

    {
      if (pConfig->control_level == pConfig->INDIVIDUAL) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          //				gl_warning("check_voltage
          //%f",check_voltage[i]); 				gl_warning("Regulator:%s regulator_resistance
          //has been set to zero. This will result singular matrix. Setting to
          //the global default.",obj->name);
        }
      } else if (pConfig->control_level == pConfig->BANK) {
        if (pConfig->PT_phase == PHASE_A)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              RNode_voltage[0]->get_complex();
        else if (pConfig->PT_phase == PHASE_B)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              RNode_voltage[1]->get_complex();
        else if (pConfig->PT_phase == PHASE_C)
          check_voltage[0] = check_voltage[1] = check_voltage[2] =
              RNode_voltage[2]->get_complex();
      }
    }
  } break;
  default:
  ...
  for (int i = 0; i < sizeof(phase) / sizeof(phase[0]); i++) {
    if (phase[i])
      stream("<TD ALIGN=CENTER COLSPAN=2 "
             "STYLE=\"font-family:courier;\"><NOBR>%d</NOBR></TD>",
             tap[i]);
    else
      stream("<TD ALIGN=CENTER COLSPAN=2 "
    ...
    for (int i = 0; i < sizeof(phase) / sizeof(phase[0]); i++) {
      if (phase[i])
        stream(
            ...
            "NAME=\"tap_%c\" VALUE=\"%d\" /></FORM></TD>",
            (const char *)server.get_string(), port.get_int16(),
            (const char *)get_name(), 'A' + i, tap[i] + 1);
      else
        stream("<TD ALIGN=CENTER COLSPAN=2 "
               "STYLE=\"font-family:courier;\">&mdash;</TD>");
    }
...
SIMULATIONMODE regulator::inter_deltaupdate_regulator(
  ...
  if (!interupdate_pos) // Before powerflow call
  {
    ...
    // Toggle the iteration variable -- only for voltage-type adjustments (since
    // it's in presync now)
    if ((pConfig->Control == pConfig->OUTPUT_VOLTAGE) ||
        (pConfig->Control == pConfig->REMOTE_NODE))
      iteration_flag = !iteration_flag;
  ...
  } else             // After the call
  {
    ...
    if (temp_time == -1.0) {
      return SM_ERROR;
    } else if ((temp_time == curr_time_value) ||
               deltamode_reiter_request) // See if this requested a reiter, or
                                         // if above did
    {
 ...
 */
static TIMESTAMP sync_regulator_impl(OBJECT *obj, TIMESTAMP t0,
                                     PASSCONFIG pass) {
  try {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_regulator(OBJECT *obj, TIMESTAMP t0,
                                               PASSCONFIG pass) {
...
EXPORT SIMULATIONMODE interupdate_regulator(OBJECT *obj,
  ...
  try {
    status = my->inter_deltaupdate_regulator(
        delta_time, dt, iteration_count_val, interupdate_pos);
    return status;
  } catch (char *msg) {
    gl_error("interupdate_regulator(obj=%d;%s): %s", obj->id,
