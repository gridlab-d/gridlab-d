...
underground_line_conductor::underground_line_conductor(MODULE *mod) : powerflow_library(mod)
{
	if (oclass == nullptr)
	{
		...
		if (gl_publish_variable(oclass,
								...
								nullptr) < 1)
			GL_THROW("unable to publish underground_line_conductor properties in %s", __FILE__);
	}
}
...
static TIMESTAMP sync_underground_line_conductor_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	return TS_NEVER;
}

#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_underground_line_conductor(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
