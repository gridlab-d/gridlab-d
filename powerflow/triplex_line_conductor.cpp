...
triplex_line_conductor::triplex_line_conductor(MODULE *mod)
  ...
  if (oclass == nullptr) {
    oclass = gl_register_class(mod, "triplex_line_conductor",
                               sizeof(triplex_line_conductor), 0x00);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_double, "resistance[Ohm/mile]", PADDR(resistance),
            PT_DESCRIPTION, "resistance of cable in ohm/mile", PT_double,
            "geometric_mean_radius[ft]", PADDR(geometric_mean_radius),
            PT_DESCRIPTION, "geometric mean radius of the cable", PT_double,
            ...
            nullptr) < 1)
      GL_THROW("unable to publish triplex_line_conductor properties in %s",
               __FILE__);
  }
}
...
int triplex_line_conductor::init(OBJECT *parent) {
  ...
  if (resistance == 0.0) {
    if (solver_method == SM_NR) {
      GL_THROW("triplex_line_conductor:%d - %s - NR: resistance is zero",
               get_id(), get_name());
      /*  TROUBLESHOOT
      The triplex_line_conductor has a resistance of zero.  This will cause
      problems with the Newton-Raphson solution.  Please put a valid resistance
      value.
      */
    } else // Assumes FBS
    {
      gl_warning("triplex_line_conductor:%d - %s - FBS: resistance is zero",
                 get_id(), get_name());
      /*  TROUBLESHOOT
      The triplex_line_conductor has a resistance of zero.  This will cause
...
static TIMESTAMP sync_triplex_line_conductor_impl(OBJECT *obj, TIMESTAMP t1,
                                                  PASSCONFIG pass) {
  return TS_NEVER;
}
...
EXPORT int isa_triplex_line_conductor_impl(OBJECT *obj, char *classname) {
  return object_data<triplex_line_conductor>(obj)->isa(classname);
}

#ifndef __APPLE__
extern "C" MODULE_API int isa_triplex_line_conductor(OBJECT *obj, char *classname) {
