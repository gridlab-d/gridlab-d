...
jsondump::jsondump(MODULE *mod) {
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            oclass, PT_char32, "group", PADDR(group), PT_DESCRIPTION,
            "the group ID to output data for (all objects if empty)",
            PT_char256, "filename_dump_system", PADDR(filename_dump_system),
            PT_DESCRIPTION, "the file to dump the current data into",
            PT_char256, "filename_dump_reliability",
            PADDR(filename_dump_reliability), PT_DESCRIPTION,
            "the file to dump the current data into", PT_timestamp, "runtime",
            PADDR(runtime), PT_DESCRIPTION, "the time to check voltage data",
            PT_int32, "runcount", PADDR(runcount), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the number of times the file has been written to",
            PT_bool, "write_system_info", PADDR(write_system), PT_DESCRIPTION,
            "Flag indicating whether system information will be written into "
            "JSON file or not",
            PT_bool, "write_reliability", PADDR(write_reliability),
            PT_DESCRIPTION,
            "Flag indicating whether reliabililty information will be written "
            "into JSON file or not",
            PT_bool, "write_per_unit", PADDR(write_per_unit), PT_DESCRIPTION,
            "Output the quantities as per-unit values", PT_double,
            "system_base[VA]", PADDR(system_VA_base), PT_DESCRIPTION,
            "System base power rating for per-unit calculations", PT_double,
            "min_node_voltage[pu]", PADDR(min_volt_value), PT_DESCRIPTION,
            "Per-unit minimum voltage level allowed for nodes", PT_double,
            "max_node_voltage[pu]", PADDR(max_volt_value), PT_DESCRIPTION,
            "Per-unit maximum voltage level allowed for nodes",
...
int jsondump::init(OBJECT *parent) {
  ...
  if ((filename_dump_system[0] != '\0') && write_system && write_per_unit) {
    ...
    if (system_VA_base <= 0.0) {
      gl_warning("jsondump:%d-%s -- If per-unit impedance is desired, a valid "
                 "system_base must be specified - defaulting to 100 MVA",
                 hdr->id, (hdr->name ? hdr->name : "Unnamed"));
      /*  TROUBLESHOOT
      To use the per-unit functionality of the jsdondump object, a valid system
      base must be specified (>0.0).  Either none was specified, or an invalid
      one was specified. As such, the base is adjusted to an assumed base of 100
      MVA.  If this is not a valid base, please specify the proper value. Please
      try again.
      */
...
STATUS jsondump::dump_system(void) {
  ...
  FINDLIST *ohlines, *tplines, *uglines, *switches, *regulators, *regConfs,
      *lineConfs, *tpLineConfs, *TransformerList, *TransConfsList, *nodes,
      *meters, *loads, *inverters, *diesels, *fuses;
  ...
  // metrics JSON value
  nlohmann::json metrics_lines; // Output dictionary for line and line
  ...
  if (group[0] == '\0') {
    ohlines =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "overhead_line", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all overhead_lines
    tplines =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "triplex_line", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all triplex_lines
    uglines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "underground_line", AND,
                              FT_MODULE, SAME, "powerflow",
                              FT_END); // find all underground_lines
    switches = gl_find_objects(FL_NEW, FT_CLASS, SAME, "switch", AND, FT_MODULE,
                               SAME, "powerflow", FT_END); // find all switches
    sectionalizers =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "sectionalizer", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all sectionalizer
    reclosers =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "recloser", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all recloser
    fuses = gl_find_objects(FL_NEW, FT_CLASS, SAME, "fuse", AND, FT_MODULE,
                            SAME, "powerflow", FT_END); // Find all fuses
    regulators =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "regulator", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all regulators
    regConfs = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "regulator_configuration", AND, FT_MODULE, SAME,
        "powerflow", FT_END); // find all regulator configurations
    lineConfs = gl_find_objects(FL_NEW, FT_CLASS, SAME, "line_configuration",
                                AND, FT_MODULE, SAME, "powerflow",
                                FT_END); // find all line configurations
    tpLineConfs = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "triplex_line_configuration", AND, FT_MODULE,
        SAME, "powerflow", FT_END); // find all triplex line configurations
    TransConfsList = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "transformer_configuration", AND, FT_MODULE,
        SAME, "powerflow", FT_END); // find all transformer configurations
    TransformerList =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "transformer", AND, FT_MODULE,
                        SAME, "powerflow", FT_END); // find all transformers
    nodes = gl_find_objects(FL_NEW, FT_CLASS, SAME, "node", AND, FT_MODULE,
                            SAME, "powerflow", FT_END); // Find all nodes
    meters = gl_find_objects(FL_NEW, FT_CLASS, SAME, "meter", AND, FT_MODULE,
                             SAME, "powerflow", FT_END); // Find all meters
    loads = gl_find_objects(FL_NEW, FT_CLASS, SAME, "load", AND, FT_MODULE,
                            SAME, "powerflow", FT_END); // Find all loads

    inverters =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "inverter", AND, FT_MODULE,
                        SAME, "generators", FT_END); // Find all inverters
    diesels =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "diesel_dg", AND, FT_MODULE,
                        SAME, "generators", FT_END); // Find all diesels
  } else {
    ohlines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "overhead_line", AND,
                              FT_GROUPID, SAME, group.get_string(), AND,
                              FT_MODULE, SAME, "powerflow", FT_END);
    tplines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "triplex_line", AND,
                              FT_GROUPID, SAME, group.get_string(), AND,
                              FT_MODULE, SAME, "powerflow", FT_END);
    uglines = gl_find_objects(FL_NEW, FT_CLASS, SAME, "underground_line", AND,
                              FT_GROUPID, SAME, group.get_string(), AND,
                              FT_MODULE, SAME, "powerflow", FT_END);
    switches = gl_find_objects(FL_NEW, FT_CLASS, SAME, "switch", AND,
                               FT_GROUPID, SAME, group.get_string(), AND,
                               FT_MODULE, SAME, "powerflow", FT_END);
    sectionalizers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "sectionalizer",
                                     AND, FT_GROUPID, SAME, group.get_string(),
                                     AND, FT_MODULE, SAME, "powerflow", FT_END);
    reclosers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "recloser", AND,
                                FT_GROUPID, SAME, group.get_string(), AND,
                                FT_MODULE, SAME, "powerflow", FT_END);
    fuses = gl_find_objects(FL_NEW, FT_CLASS, SAME, "fuse", AND, FT_GROUPID,
                            SAME, group.get_string(), AND, FT_MODULE, SAME,
                            "powerflow", FT_END);
    regulators = gl_find_objects(FL_NEW, FT_CLASS, SAME, "regulator", AND,
                                 FT_GROUPID, SAME, group.get_string(), AND,
                                 FT_MODULE, SAME, "powerflow", FT_END);
    regConfs = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "regulator_configuration", AND, FT_GROUPID,
        SAME, group.get_string(), AND, FT_MODULE, SAME, "powerflow", FT_END);
    lineConfs = gl_find_objects(FL_NEW, FT_CLASS, SAME, "line_configuration",
                                AND, FT_GROUPID, SAME, group.get_string(), AND,
                                FT_MODULE, SAME, "powerflow", FT_END);
    tpLineConfs = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "triplex_line_configuration", AND, FT_GROUPID,
        SAME, group.get_string(), AND, FT_MODULE, SAME, "powerflow", FT_END);
    TransConfsList = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "transformer_configuration", AND, FT_GROUPID,
        SAME, group.get_string(), AND, FT_MODULE, SAME, "powerflow", FT_END);
    TransformerList = gl_find_objects(
        FL_NEW, FT_CLASS, SAME, "transformer", AND, FT_GROUPID, SAME,
        group.get_string(), AND, FT_MODULE, SAME, "powerflow", FT_END);
    nodes = gl_find_objects(FL_NEW, FT_CLASS, SAME, "node", AND, FT_GROUPID,
                            SAME, group.get_string(), AND, FT_MODULE, SAME,
                            "powerflow", FT_END);
    meters = gl_find_objects(FL_NEW, FT_CLASS, SAME, "meter", AND, FT_GROUPID,
                             SAME, group.get_string(), AND, FT_MODULE, SAME,
                             "powerflow", FT_END);
    loads = gl_find_objects(FL_NEW, FT_CLASS, SAME, "load", AND, FT_GROUPID,
                            SAME, group.get_string(), AND, FT_MODULE, SAME,
                            "powerflow", FT_END);

    inverters = gl_find_objects(FL_NEW, FT_CLASS, SAME, "inverter", AND,
                                FT_GROUPID, SAME, group.get_string(), AND,
                                FT_MODULE, SAME, "generators", FT_END);
    diesels = gl_find_objects(FL_NEW, FT_CLASS, SAME, "diesel_dg", AND,
                              FT_GROUPID, SAME, group.get_string(), AND,
                              FT_MODULE, SAME, "generators", FT_END);
  }

  if ((ohlines->hit_count == 0) && (tplines->hit_count == 0) &&
      (uglines->hit_count == 0) && (nodes->hit_count == 0) &&
      (meters->hit_count == 0) && (loads->hit_count == 0)) {
    gl_error("No line or bus objects were found.");
    /* TROUBLESHOOT
    No line or bus objects were found in your .glm file. if you specified a
    group id double check to make sure you have line objects with the specified
    group id.
    */
  ...
  metrics_lines["description"] =
      "This file describes the system topology information (bus and lines) and "
  ...
  if (lineConfs->hit_count > 0) {
    ...
    if (pLineConf == nullptr) {
      GL_THROW("jsdondump:%d %s - Unable to allocate memory", objhdr->id,
               (objhdr->name ? objhdr->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attempting to allocate memory for one of the search and output
    ...
    // Define b_mat_pu
    b_mat_pu = (gld::complex *)gl_malloc((lineConfs->hit_count) * 9 *
                                         sizeof(gld::complex));
  ...
  if (tpLineConfs->hit_count > 0) {
    pTpLineConf =
        (OBJECT **)gl_malloc((tpLineConfs->hit_count) * sizeof(OBJECT *));
    ...
    // Define b_mat_tp_pu
    b_mat_tp_pu = (gld::complex *)gl_malloc((tpLineConfs->hit_count) * 9 *
                                            sizeof(gld::complex));
    ...
    b_mat_tp_defined =
        (bool *)gl_malloc((tpLineConfs->hit_count) * sizeof(bool));
  ...
  if (TransConfsList->hit_count > 0) {
    ...
    //
    pTransConf =
        (OBJECT **)gl_malloc((TransConfsList->hit_count) * sizeof(OBJECT *));
    ...
    b_mat_trans_pu = (gld::complex *)gl_malloc((TransConfsList->hit_count) * 9 *
                                               sizeof(gld::complex));
    ...
    b_mat_trans_defined =
        (bool *)gl_malloc((TransConfsList->hit_count) * sizeof(bool));
    ...
    trans_phase_count =
        (int *)gl_malloc((TransConfsList->hit_count) * sizeof(int));
  ...
  if (regConfs->hit_count > 0) {
    ...
    // Define b_mat_trans_pu
    b_mat_reg_pu = (gld::complex *)gl_malloc((regConfs->hit_count) * 9 *
                                             sizeof(gld::complex));
  ...
  if (inverters->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      // Do the tests -- we're in powerflow, so the master definitions still
      // work Phase A
  ...
  if (nodes->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      {
        temp_voltage_base = 1.0; // Divide by unity - does nothing really, but
                                 // easier to code this way
        temp_de_pu_base = get_double_value(
            obj, "nominal_voltage"); // But we do need to get the "real value"
      }
      ...
      // Obtain current voltage - assume that's the reference - per-unitize, if
      // necessary
      ...
      for (indexA = 0; indexA < 3; indexA++) {
        ...
        temp_voltage_output_value =
            temp_complex_voltage_value[indexA].Mag() / temp_voltage_base;
  ...
  // Search for meters too - replicate the nodes list, since we'll just leave
  // them the same way
  ...
  // Search for loads too - replicate the nodes list, since we'll just leave
  // them the same way
  ...
  if (TransformerList->hit_count > 0) {
    ...
    pTransformer = (transformer **)gl_malloc(TransformerList->hit_count *
                                             sizeof(transformer *));
    ...
    while (obj != nullptr) {
      ...
      // write capacity - I wanted to use link emergency limit (A), but its
      // private field, write only default value
      ...
      // If per-unit - adjust the values
      if (write_per_unit) {
        // Compute the per-unit base - use the nominal value off of the
        // secondary
        per_unit_base =
            get_double_value(pTransformer[index]->to, "nominal_voltage");

        // Calculate the base impedance
        temp_impedance_base =
            (per_unit_base * per_unit_base) / (system_VA_base / 3.0);
      } // End per-unit
      else // No per-unit desired
      {
        temp_impedance_base = 1.0; // Divide by unity - does nothing really, but
                                   // easier to code this way
      }
        ...
        if (!b_mat_trans_defined[indexA]) {
          for (indexB = 0; indexB < 3; indexB++) {
            for (indexC = 0; indexC < 3; indexC++) {
              b_mat_trans_pu[indexA * 9 + indexB * 3 + indexC] =
                  pTransformer[index]->b_mat[indexB][indexC] /
                  temp_impedance_base;
            }
          }
  ...
  if (regulators->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      if (write_per_unit) {
        ...
        // secondary
        per_unit_base =
            get_double_value(pRegulator[index]->to, "nominal_voltage");
      ...
      if (found_match_config) {
        if (!b_mat_reg_defined[indexA]) {
          for (indexB = 0; indexB < 3; indexB++) {
            for (indexC = 0; indexC < 3; indexC++) {
              b_mat_reg_pu[indexA * 9 + indexB * 3 + indexC] =
                  pRegulator[index]->b_mat[indexB][indexC] /
                  temp_impedance_base;
            }
          }
  ...
  if (ohlines->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      // write is_transformer (now only write line object, so will only be
      // false)
      ...
      if (found_match_config) {
        if (!b_mat_defined[indexA]) {
          for (indexB = 0; indexB < 3; indexB++) {
            for (indexC = 0; indexC < 3; indexC++) {
              b_mat_pu[indexA * 9 + indexB * 3 + indexC] =
                  (pOhLine[index]->b_mat[indexB][indexC]) /
                  (((pOhLine[index]->length) / 5280.0) / temp_impedance_base);
            }
          }
      ...
      if (found_match_config) {
        if (!b_mat_defined[indexA]) {
          for (indexB = 0; indexB < 3; indexB++) {
            for (indexC = 0; indexC < 3; indexC++) {
              b_mat_pu[indexA * 9 + indexB * 3 + indexC] =
                  (pUgLine[index]->b_mat[indexB][indexC]) /
                  (((pUgLine[index]->length) / 5280.0) / temp_impedance_base);
            }
          }
          b_mat_defined[indexA] = true;
        }
    ...
    while (obj != nullptr) {
      ...
      if (found_match_config) {
        if (!b_mat_tp_defined[indexA]) {
          for (indexB = 0; indexB < 3; indexB++) {
            for (indexC = 0; indexC < 3; indexC++) {
              b_mat_tp_pu[indexA * 9 + indexB * 3 + indexC] =
                  (pTpLine[index]->b_mat[indexB][indexC]) /
                  (((pTpLine[index]->length) / 5280.0) / temp_impedance_base);
            }
          }
  ...
  if (switches->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      if (!b_mat_switch_defined) {
        for (indexB = 0; indexB < 3; indexB++) {
          for (indexC = 0; indexC < 3; indexC++) {
            b_mat_switch_pu[indexB * 3 + indexC] =
                pSwitch[index]->b_mat[indexB][indexC] / temp_impedance_base;
          }
        }
  ...
  if (sectionalizers->hit_count > 0) {
    // Allocate the master array
    pSectionalizer = (sectionalizer **)gl_malloc(sectionalizers->hit_count *
                                                 sizeof(sectionalizer *));
    ...
    while (obj != nullptr) {
      ...
      if (write_per_unit) {
        ...
        // secondary
        per_unit_base =
            get_double_value(pSectionalizer[index]->to, "nominal_voltage");
      ...
      if (!b_mat_switch_defined) {
        for (indexB = 0; indexB < 3; indexB++) {
          for (indexC = 0; indexC < 3; indexC++) {
            b_mat_switch_pu[indexB * 3 + indexC] =
                pSectionalizer[index]->b_mat[indexB][indexC] /
                temp_impedance_base;
          }
        }
        b_mat_switch_defined = true;
  ...
  if (reclosers->hit_count > 0) {
    ...
    // Allocate the master array
    pRecloser =
        (recloser **)gl_malloc(reclosers->hit_count * sizeof(recloser *));
    ...
    while (obj != nullptr) {
      ...
      if (write_per_unit) {
        ...
        // secondary
        per_unit_base =
            get_double_value(pRecloser[index]->to, "nominal_voltage");
      ...
      if (!b_mat_switch_defined) {
        for (indexB = 0; indexB < 3; indexB++) {
          for (indexC = 0; indexC < 3; indexC++) {
            b_mat_switch_pu[indexB * 3 + indexC] =
                pRecloser[index]->b_mat[indexB][indexC] / temp_impedance_base;
          }
        }
        b_mat_switch_defined = true;
        switch_phase_count = phaseCount; // Just store the first one
  ...
  if (fuses->hit_count > 0) {
    ...
    while (obj != nullptr) {
      ...
      if (!b_mat_fuse_defined) {
        for (indexB = 0; indexB < 3; indexB++) {
          for (indexC = 0; indexC < 3; indexC++) {
            b_mat_fuse_pu[indexB * 3 + indexC] =
                pFuse[index]->b_mat[indexB][indexC] / temp_impedance_base;
          }
        }
  ...
  // Write a "fuse config", if it exists - assume that if one exists, this needs
  // written
  if (fuses->hit_count > 0) {
  ...
  // Write a "switch config", if it exists - assume that if one exists, this
  // needs written
  if ((switches->hit_count > 0) || (reclosers->hit_count > 0) ||
      (sectionalizers->hit_count > 0)) {
  ...
  if (lineConfs->hit_count > 0) {
    ...
    for (index = 0; index < lineConfs->hit_count; index++) {
      ...
      if (b_mat_defined[index]) {
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_pu[index * 9 + indexA * 3 + indexB].Re());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_pu[index * 9 + indexA * 3 + indexB].Im());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
    ...
    for (index = 0; index < tpLineConfs->hit_count; index++) {
      ...
      if (b_mat_tp_defined[index]) {
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_tp_pu[index * 9 + indexA * 3 + indexB].Re());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_tp_pu[index * 9 + indexA * 3 + indexB].Im());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
    ...
    for (index = 0; index < TransConfsList->hit_count; index++) {
      ...
      if (b_mat_trans_defined[index]) {
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_trans_pu[index * 9 + indexA * 3 + indexB].Re());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_trans_pu[index * 9 + indexA * 3 + indexB].Im());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
    ...
    for (index = 0; index < regConfs->hit_count; index++) {
      ...
      if (b_mat_reg_defined[index]) {
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_reg_pu[index * 9 + indexA * 3 + indexB].Re());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
        ...
        for (indexA = 0; indexA < 3; indexA++) {
          for (indexB = 0; indexB < 3; indexB++) {
            jsonArray1.push_back(
                b_mat_reg_pu[index * 9 + indexA * 3 + indexB].Im());
          }
          jsonArray2.push_back(jsonArray1);
          jsonArray1.clear();
        }
  ...
  if (lineConfs->hit_count > 0) {
    gl_free((void **)&b_mat_pu);
    gl_free((void **)&b_mat_defined);
  }

  if (tpLineConfs->hit_count > 0) {
    gl_free((void **)&b_mat_tp_pu);
    gl_free((void **)&b_mat_tp_defined);
  }

  if (TransConfsList->hit_count > 0) {
    gl_free((void **)&b_mat_trans_pu);
    gl_free((void **)&b_mat_trans_defined);
    gl_free((void **)&trans_phase_count);
  }

  if (regConfs->hit_count > 0) {
    gl_free((void **)&b_mat_reg_pu);
    gl_free((void **)&b_mat_reg_defined);
    gl_free((void **)&reg_phase_count);
  }

  // Clean up the findlists
  gl_free((void **)&ohlines);
  gl_free((void **)&tplines);
  gl_free((void **)&uglines);
  gl_free((void **)&switches);
  gl_free((void **)&regulators);
  gl_free((void **)&regConfs);
  gl_free((void **)&lineConfs);
  gl_free((void **)&tpLineConfs);
  gl_free((void **)&TransformerList);
  gl_free((void **)&TransConfsList);
  gl_free((void **)&nodes);
  gl_free((void **)&meters);
  gl_free((void **)&loads);
  gl_free((void **)&fuses);
  gl_free((void **)&reclosers);
  gl_free((void **)&sectionalizers);
  gl_free((void **)&inverters);
  gl_free((void **)&diesels);

  return SUCCESS;
}

STATUS jsondump::dump_reliability(void) {
  FINDLIST *powermetrics, *fuses, *reclosers, *sectionalizers, *relays,
  ...
  regulator *regData;
  char *indices1366[] = {
      const_cast<char *>("SAIFI"), const_cast<char *>("SAIDI"),
      const_cast<char *>("CAIDI"), const_cast<char *>("ASAI"),
  ...
  // metrics JSON value
  nlohmann::json metrics_reliability; // Output dictionary for line and line
                                      // configuration metrics
  nlohmann::json metrics_1366;
  nlohmann::json metrics_protection;
  nlohmann::json metrics_others;
  nlohmann::json
      protectionArray; // for storing array of each type of protective device
  nlohmann::json protect_obj;
  nlohmann::json otherArray; // for storing array of each type of capacitor and
                             // regulator devices
  ...
  powermetrics = gl_find_objects(FL_NEW, FT_CLASS, SAME, "power_metrics",
                                 FT_END); // find all power_metrics
  fuses =
      gl_find_objects(FL_NEW, FT_CLASS, SAME, "fuse", FT_END); // find all fuses
  reclosers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "recloser",
                              FT_END); // find all reclosers
  sectionalizers = gl_find_objects(FL_NEW, FT_CLASS, SAME, "sectionalizer",
                                   FT_END); // find all sectionalizers
  //	relays = gl_find_objects(FL_NEW,FT_CLASS,SAME,"relay",FT_END);
  ////find all relays
  capacitors = gl_find_objects(FL_NEW, FT_CLASS, SAME, "capacitor",
                               FT_END); // find all capacitors
  regulators = gl_find_objects(FL_NEW, FT_CLASS, SAME, "regulator",
                               FT_END); // find all regulators
  ...
  if (powermetrics != nullptr) {
    ...
    while (obj = gl_find_next(powermetrics, obj)) {
      ...
      // Loop through the reliability indices to find the value and put into
      // JSON variable
      index1366 = -1;
      while (indices1366[++index1366] != nullptr) {
        ...
        if (temp_double == nullptr) {
          gl_error("Unable to to find property for %s: %s is nullptr",
                   obj->name, indices1366[index1366]);
          return FAILED;
        }
  ...
  gl_free((void **)&powermetrics);
  gl_free((void **)&fuses);
  gl_free((void **)&reclosers);
  gl_free((void **)&sectionalizers);
  gl_free((void **)&relays);
  gl_free((void **)&capacitors);
  gl_free((void **)&regulators);
...
TIMESTAMP jsondump::commit(TIMESTAMP t) {
  ...
  if (write_system && ((t == runtime) || (runtime == TS_NEVER)) &&
      (runcount < 1)) {
    /* dump */
    rv = dump_system();
    ++runcount;
...
double jsondump::get_double_value(OBJECT *obj, const char *name) {
  ...
  if (!pQuantity->is_valid() || !pQuantity->is_double()) {
    GL_THROW("jsdondump:%d %s - Unable to map property %s from object:%d %s",
             objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name,
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    While attempting to map a quantity from another object, an error occurred.
    Please try again. If the error persists, please submit your system and a bug
