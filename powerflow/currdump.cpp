...
currdump::currdump(MODULE *mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "currdump", sizeof(currdump),
    ...
    if (gl_publish_variable(
            ...
            "the time to check current data", PT_char256, "filename",
            PADDR(filename), PT_DESCRIPTION,
            "the file to dump the current data into", PT_int32, "runcount",
            PADDR(runcount), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION,
            "the number of times the file has been written to", PT_enumeration,
            "mode", PADDR(mode), PT_KEYWORD, "RECT", (enumeration)CDM_RECT,
            PT_KEYWORD, "POLAR", (enumeration)CDM_POLAR, nullptr) < 1)
...
int currdump::init(OBJECT *parent) { return 1; }
...
void currdump::dump(TIMESTAMP t) {
  ...
  if (group[0] == 0) {
    ...
    links = gl_find_objects(FL_NEW, FT_MODULE, SAME, "powerflow", AND,
  ...
  fprintf(outfile, "# %s run at %s on %i links\n", filename.get_string(),
          timestr, links->hit_count);
  if (mode == CDM_RECT) {
    fprintf(outfile, "link_name,currA_real,currA_imag,currB_real,currB_imag,"
                     "currC_real,currC_imag\n");
  } else if (mode == CDM_POLAR) {
    fprintf(outfile, "link_name,currA_mag,currA_angle,currB_mag,currB_angle,"
                     "currC_mag,currC_angle\n");
  }
  obj = 0;
  while (obj = gl_find_next(links, obj)) {
    if (gl_object_isa(obj, "link", "powerflow")) {
      ...
      // Check it
      if (!link_current_value_link[0]->is_valid() ||
          !link_current_value_link[0]->is_complex()) {
        GL_THROW("currdump - Unable to map current property of link:%d - %s",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While the currdump object attempted to map the current_in_A,
        current_in_B, or current_in_C, an error occurred.  Please try again.  If
        the error persists, please submit your code via the ticketing and issues
        system.
        */
      }
      ...
      // Check it
      if (!link_current_value_link[1]->is_valid() ||
          !link_current_value_link[1]->is_complex()) {
        GL_THROW(
            const_cast<char *>(
                "currdump - Unable to map current property of link:%d - %s"),
            obj->id, (obj->name ? obj->name : "Unnamed"));
        // Defined above
      }
      ...
      // Check it
      if (!link_current_value_link[2]->is_valid() ||
          !link_current_value_link[2]->is_complex()) {
        GL_THROW(
            const_cast<char *>(
                "currdump - Unable to map current property of link:%d - %s"),
            obj->id, (obj->name ? obj->name : "Unnamed"));
      ...
      if (mode == CDM_RECT) {
        fprintf(outfile, "%s,%f,%f,%f,%f,%f,%f\n",
                (obj->name ? obj->name : namestr), link_current_values[0].Re(),
                link_current_values[0].Im(), link_current_values[1].Re(),
                link_current_values[1].Im(), link_current_values[2].Re(),
                link_current_values[2].Im());
      } else if (mode == CDM_POLAR) {
        fprintf(outfile, "%s,%f,%f,%f,%f,%f,%f\n",
                (obj->name ? obj->name : namestr), link_current_values[0].Mag(),
                link_current_values[0].Arg(), link_current_values[1].Mag(),
                link_current_values[1].Arg(), link_current_values[2].Mag(),
                link_current_values[2].Arg());
      }
