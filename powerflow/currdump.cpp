...
currdump::currdump(MODULE *mod)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_char32,"filename",PADDR(filename),PT_DESCRIPTION,"the file to dump the current data into",
...
//	PROPERTY *vA, *vB, *vC;
	...
	fprintf(outfile,"# %s run at %s on %i links\n", filename, timestr, links->hit_count);
	fprintf(outfile,"node_name,currA_real,currA_imag,currB_real,currB_imag,currC_real,currC_imag\n");
