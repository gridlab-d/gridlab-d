...
series_compensator::series_compensator(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "series_compensator", sizeof(series_compensator),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "link",

            PT_double, "vset_A[pu]", PADDR(vset_value[0]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase A", PT_double, "vset_B[pu]",
            PADDR(vset_value[1]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase B", PT_double, "vset_C[pu]",
            PADDR(vset_value[2]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase C",

            PT_double, "vset_A_0[pu]", PADDR(vset_value_0[0]), PT_DESCRIPTION,
            "Voltage magnitude set point for phase A, changed by the player",
            PT_double, "vset_B_0[pu]", PADDR(vset_value_0[1]), PT_DESCRIPTION,
            "Voltage magnitude set point for phase B, changed by the player",
            PT_double, "vset_C_0[pu]", PADDR(vset_value_0[2]), PT_DESCRIPTION,
            "Voltage magnitude set point for phase C, changed by the player",

            PT_double, "vset_1[pu]", PADDR(vset_value[0]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase 1 of a triplex system",
            PT_double, "vset_2[pu]", PADDR(vset_value[1]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase 2 of a tryplex system",

            PT_double, "vset_1_0[pu]", PADDR(vset_value_0[0]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase 1 of a triplex system",
            PT_double, "vset_2_0[pu]", PADDR(vset_value_0[1]), PT_DESCRIPTION,
            "Voltage magnitude reference for phase 2 of a tryplex system",

            PT_bool, "frequency_regulation", PADDR(frequency_regulation),
            PT_DESCRIPTION,
            "DELTAMODE: Boolean value indicating whether the frequency "
            "regulation of the series compensator is enabled or not",
            PT_bool, "frequency_open_loop_control",
            PADDR(frequency_open_loop_control), PT_DESCRIPTION,
            "DELTAMODE: Boolean value indicating whether the frequency open "
            "loop control of the series compensator is enabled or not",

            PT_double, "t_delay", PADDR(t_delay), PT_DESCRIPTION,
            "the controller will wait for t_delay to take actions", PT_double,
            "t_hold", PADDR(t_hold), PT_DESCRIPTION,
            "Once the controller changes the voltage set point, it will stay "
            "there for t_hold time",
            PT_double, "recover_rate", PADDR(recover_rate), PT_DESCRIPTION,
            "The rate that the voltage goes back to nominal, unit: pu/s",
            PT_double, "frequency_low", PADDR(frequency_low), PT_DESCRIPTION,
            "The low frequency that activates the controller", PT_double,
            "frequency_high", PADDR(frequency_high), PT_DESCRIPTION,
            "The high frequency that activates the controller", PT_double,
            "V_error", PADDR(V_error), PT_DESCRIPTION,
            "Make sure the voltage can go back to nominal",

            PT_double, "voltage_update_tolerance[pu]",
            PADDR(voltage_iter_tolerance), PT_DESCRIPTION,
            "Largest absolute between vset_X and measured voltage that won't "
            "force a reiteration",

            PT_double, "turns_ratio_A", PADDR(turns_ratio[0]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Debug variable - Turns ratio for phase A series compensator "
            "equivalent",
            PT_double, "turns_ratio_B", PADDR(turns_ratio[1]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Debug variable - Turns ratio for phase B series compensator "
            "equivalent",
            PT_double, "turns_ratio_C", PADDR(turns_ratio[2]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Debug variable - Turns ratio for phase C series compensator "
            "equivalent",

            PT_double, "turns_ratio_1", PADDR(turns_ratio[0]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Debug variable - Turns ratio for phase 1 (triplex) series "
            "compensator equivalent",
            PT_double, "turns_ratio_2", PADDR(turns_ratio[1]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION,
            "Debug variable - Turns ratio for phase 2 (triplex) series "
            "compensator equivalent",

            PT_double, "n_max_ext_A", PADDR(n_max_ext[0]), PT_DESCRIPTION,
            "maximum Turn ratio for phase A", PT_double, "n_max_ext_B",
            PADDR(n_max_ext[1]), PT_DESCRIPTION,
            "maximum Turn ratio for phase B", PT_double, "n_max_ext_C",
            PADDR(n_max_ext[2]), PT_DESCRIPTION,
            "maximum Turn ratio for phase C", PT_double, "n_min_ext_A",
            PADDR(n_min_ext[0]), PT_DESCRIPTION,
            "minimum Turn ratio for phase A", PT_double, "n_min_ext_B",
            PADDR(n_min_ext[1]), PT_DESCRIPTION,
            "minimum Turn ratio for phase B", PT_double, "n_min_ext_C",
            PADDR(n_min_ext[2]), PT_DESCRIPTION,
            "minimum Turn ratio for phase C",

            PT_double, "n_max_ext_1", PADDR(n_max_ext[0]), PT_DESCRIPTION,
            "maximum Turn ratio for phase 1 (triplex)", PT_double,
            "n_max_ext_2", PADDR(n_max_ext[1]), PT_DESCRIPTION,
            "maximum Turn ratio for phase 2 (triplex)", PT_double,
            "n_min_ext_1", PADDR(n_min_ext[0]), PT_DESCRIPTION,
            "minimum Turn ratio for phase 1 (triplex)", PT_double,
            "n_min_ext_2", PADDR(n_min_ext[1]), PT_DESCRIPTION,
            "minimum Turn ratio for phase 2 (triplex)",

            PT_double, "kp", PADDR(kp), PT_DESCRIPTION, "proportional gain",
            PT_double, "ki", PADDR(ki), PT_DESCRIPTION, "integrator gain",
            PT_double, "kpf", PADDR(kpf), PT_DESCRIPTION,
            "proportional gain of frequency regulation", PT_double, "f_db_max",
            PADDR(f_db_max), PT_DESCRIPTION, "frequency dead band max",
            PT_double, "f_db_min", PADDR(f_db_min), PT_DESCRIPTION,
            "frequency dead band max", PT_double, "delta_Vmax",
            PADDR(delta_Vmax), PT_DESCRIPTION,
            "upper limit of the frequency regulation output", PT_double,
            "delta_Vmin", PADDR(delta_Vmin), PT_DESCRIPTION,
            "lower limit of the frequency regulation output", PT_double,
            "delta_V", PADDR(delta_V), PT_DESCRIPTION,
            "frequency regulation output",

            PT_double, "V_bypass_max_pu", PADDR(V_bypass_max_pu),
            PT_DESCRIPTION, "the upper limit voltage to bypass compensator",
            PT_double, "V_bypass_min_pu", PADDR(V_bypass_min_pu),
            PT_DESCRIPTION, "the lower limit voltage to bypass compensator",

            PT_enumeration, "phase_A_state", PADDR(phase_states[0]),
            PT_DESCRIPTION, "Defines if phase A is in bypass or not",
            PT_KEYWORD, "NORMAL", (enumeration)ST_NORMAL, PT_KEYWORD, "BYPASS",
            (enumeration)ST_BYPASS, PT_enumeration, "phase_B_state",
            PADDR(phase_states[1]), PT_DESCRIPTION,
            "Defines if phase B is in bypass or not", PT_KEYWORD, "NORMAL",
            (enumeration)ST_NORMAL, PT_KEYWORD, "BYPASS",
            (enumeration)ST_BYPASS, PT_enumeration, "phase_C_state",
            PADDR(phase_states[2]), PT_DESCRIPTION,
            "Defines if phase C is in bypass or not", PT_KEYWORD, "NORMAL",
            (enumeration)ST_NORMAL, PT_KEYWORD, "BYPASS",
            (enumeration)ST_BYPASS, PT_enumeration, "phase_1_state",
            PADDR(phase_states[0]), PT_DESCRIPTION,
            "Defines if phase 1 is in bypass or not", PT_KEYWORD, "NORMAL",
            (enumeration)ST_NORMAL, PT_KEYWORD, "BYPASS",
            (enumeration)ST_BYPASS, PT_enumeration, "phase_2_state",
            PADDR(phase_states[1]), PT_DESCRIPTION,
            "Defines if phase 2 is in bypass or not", PT_KEYWORD, "NORMAL",
            (enumeration)ST_NORMAL, PT_KEYWORD, "BYPASS",
            (enumeration)ST_BYPASS,

            PT_double, "series_compensator_resistance[Ohm]",
            PADDR(series_compensator_resistance), PT_DESCRIPTION,
            "Baseline resistance for the series compensator device - needed "
            "for NR",
            nullptr) < 1)
    ...
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_series_compensator) ==
        nullptr)
...
int series_compensator::create() {
  ...
  pred_state.dn_ini_StateVal[0] = pred_state.dn_ini_StateVal[1] =
      pred_state.dn_ini_StateVal[2] = 0;
  pred_state.n_ini_StateVal[0] = pred_state.n_ini_StateVal[1] =
      pred_state.n_ini_StateVal[2] = 1;
  pred_state.n_StateVal[0] = pred_state.n_StateVal[1] =
      pred_state.n_StateVal[2] = 1;

  next_state.dn_ini_StateVal[0] = next_state.dn_ini_StateVal[1] =
      next_state.dn_ini_StateVal[2] = 0;
  next_state.n_ini_StateVal[0] = next_state.n_ini_StateVal[1] =
      next_state.n_ini_StateVal[2] = 1;
  next_state.n_StateVal[0] = next_state.n_StateVal[1] =
      next_state.n_StateVal[2] = 1;

  curr_state.dn_ini_StateVal[0] = curr_state.dn_ini_StateVal[1] =
      curr_state.dn_ini_StateVal[2] = 0;
  curr_state.n_ini_StateVal[0] = curr_state.n_ini_StateVal[1] =
      curr_state.n_ini_StateVal[2] = 1;
  curr_state.n_StateVal[0] = curr_state.n_StateVal[1] =
      curr_state.n_StateVal[2] = 1;

  // Working variables - set initial ratio to unity
  turns_ratio[0] = turns_ratio[1] = turns_ratio[2] = 1.0;
  prev_turns_ratio[0] = prev_turns_ratio[1] = prev_turns_ratio[2] =
      0.0; // Set different, so it forces an update
  ...
  // Zero the values
  val_ToNode_voltage[0] = val_ToNode_voltage[1] = val_ToNode_voltage[2] =
      gld::complex(0.0, 0.0);
  val_ToNode_voltage_pu[0] = val_ToNode_voltage_pu[1] =
      val_ToNode_voltage_pu[2] = 0.0;
  val_FromNode_voltage[0] = val_FromNode_voltage[1] = val_FromNode_voltage[2] =
      gld::complex(0.0, 0.0);
  val_FromNode_voltage_pu[0] = val_FromNode_voltage_pu[1] =
      val_FromNode_voltage_pu[2] = 0.0;

  vset_value[0] = vset_value[1] = vset_value[2] =
      1.0; // Start out set to unity voltage
  vset_value_0[0] = vset_value_0[1] = vset_value_0[2] =
      1.0; // Start out set to unity voltage
...
int series_compensator::init(OBJECT *parent) {
  ...
  // Loop through and zero the various line matrices (link should really do this
  // -- @TODO - Put this in link.cpp sometime)
  ...
  if (!FromNode_frequency->is_valid() || !FromNode_frequency->is_double()) {
    GL_THROW(
        "series_compensator:%d - %s - Unable to map property for remote object",
        obj->id, (obj->name ? obj->name : "Unnamed"));
    // Defined elsewhere
  }

  val_FromNode_frequency =
      FromNode_frequency->get_double(); // get the frequency
  ...
  if (has_phase(PHASE_S)) // Triplex-enabled
  {
    ...
    if (!ToNode_voltage[0]->is_valid() || !ToNode_voltage[0]->is_complex()) {
      GL_THROW("series_compensator:%d - %s - Unable to map property for remote "
               "object",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      // Defined elsewhere
    }
    ...
    // Make sure it worked
    if (!FromNode_voltage[0]->is_valid() ||
        !FromNode_voltage[0]->is_complex()) {
      GL_THROW("series_compensator:%d - %s - Unable to map property for remote "
               "object",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      // Defined elsewhere
    ...
    // Make sure it worked
    if (!FromNode_voltage[1]->is_valid() ||
        !FromNode_voltage[1]->is_complex()) {
      GL_THROW("series_compensator:%d - %s - Unable to map property for remote "
               "object",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      // Defined elsewhere
  ...
  else // Must be some variant of three-phase
  {
    ...
    if (has_phase(PHASE_A)) {
      ...
      // Make sure it worked
      if (!ToNode_voltage[0]->is_valid() || !ToNode_voltage[0]->is_complex()) {
        GL_THROW("series_compensator:%d - %s - Unable to map property for "
                 "remote object",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
        /* TROUBLESHOOT
        While attempting to map the voltage for the to node, a property could
        not be properly mapped. Please try again.  If the error persists, please
        submit an issue in the ticketing system.
        */
      }
    ...
    {
      ToNode_voltage[0] =
          nullptr; // Null it -- should already be done, but be paranoid
    ...
    if (has_phase(PHASE_B)) {
      ...
      if (!ToNode_voltage[1]->is_valid() || !ToNode_voltage[1]->is_complex()) {
        GL_THROW(
            "series_compensator:%d - %s - Unable to map property for TO node",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        // Defined above
      }
    ...
    if (has_phase(PHASE_C)) {
      ...
      // Make sure it worked
      if (!FromNode_voltage[2]->is_valid() ||
          !FromNode_voltage[2]->is_complex()) {
        GL_THROW(
            "series_compensator:%d - %s - Unable to map property for TO node",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        // Defined above
  ...
  if (!temp_volt_prop->is_valid() || !temp_volt_prop->is_double()) {
    GL_THROW("series_compensator:%d - %s - Unable to map nominal_voltage of "
             "FROM node",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    While mapping the nominal_voltage property of the from node, an error
    occurred.  Please try again. If the error persists, please submit your code
  ...
  if (solver_method == SM_NR) {
    if (series_compensator_resistance <= 0.0) {
      gl_warning("series_compensator:%s series_compensator_resistance is a "
                 "negative or zero value -- setting to the global default.",
                 obj->name);
      /*  TROUBLESHOOT
      Under Newton-Raphson solution method, the impedance matrix cannot be a
      singular matrix for the inversion process (or have an invalid negative
      value). Change the value of series_compensator_resistance to something
      small but larger that zero.
      */
    ...
    // Flag the special link as a rgulator, since right now, we work the same
    // way
    ...
    if (has_phase(PHASE_S)) {
      ...
      base_admittance_mat[0][0] =
          gld::complex(1.0 / series_compensator_resistance, 0.0);
...
TIMESTAMP series_compensator::postsync(TIMESTAMP t0) {
  ...
  if (series_compensator_first_step) {
    ...
    if (has_phase(PHASE_S)) {
      ...
      val_ToNode_voltage_pu[0] =
          val_ToNode_voltage[0].Mag() / val_FromNode_nominal_voltage;
      val_FromNode_voltage_pu[0] =
          val_FromNode_voltage[0].Mag() / val_FromNode_nominal_voltage;
      val_ToNode_voltage_pu[1] =
          val_ToNode_voltage[1].Mag() / val_FromNode_nominal_voltage;
      val_FromNode_voltage_pu[1] =
          val_FromNode_voltage[1].Mag() / val_FromNode_nominal_voltage;
      ...
      if (abs(voltage_difference[0]) > voltage_iter_tolerance) {
        iterate = true;
        curr_state.n_ini_StateVal[0] = turns_ratio[0] =
            vset_value[0] / val_FromNode_voltage_pu[0];
      }

      if (abs(voltage_difference[1]) > voltage_iter_tolerance) {
        iterate = true;
        curr_state.n_ini_StateVal[1] = turns_ratio[1] =
            vset_value[1] / val_FromNode_voltage_pu[1];
      }
    ...
    else // Three-phase variant
    {
      ...
      if (has_phase(PHASE_C)) {
        ...
        // Do the per-unit conversion
        val_ToNode_voltage_pu[2] =
            val_ToNode_voltage[2].Mag() / val_FromNode_nominal_voltage;
        val_FromNode_voltage_pu[2] =
            val_FromNode_voltage[2].Mag() / val_FromNode_nominal_voltage;

        voltage_difference[2] = val_ToNode_voltage_pu[2] - vset_value[2];

        if (abs(voltage_difference[2]) > voltage_iter_tolerance) {
          iterate = true;
          curr_state.n_ini_StateVal[2] = turns_ratio[2] =
              vset_value[2] / val_FromNode_voltage_pu[2];
        }
      } else {
        // Zero everything
  ...
  function_return_val = sercom_postPost_fxn(
      0, 0); // Just give it an arbitary value -- used mostly for delta flagging
...
// Functionalized "presync before link::presync" portions, mostly for deltamode
// functionality
...
// Functionalized version of the code for deltamode - "post-link::presync"
// portions
void series_compensator::sercom_postPre_fxn(void) {
  ...
  if (solver_method == SM_NR) {
    ...
    // Only perform update if a turns_ratio has changed.  Since the link
    // calculations are replicated here and it directly accesses the NR memory
    // space, this won't cause any issues.
    if ((prev_turns_ratio[0] != turns_ratio[0]) ||
        (prev_turns_ratio[1] != turns_ratio[1]) ||
        (prev_turns_ratio[2] != turns_ratio[2])) // Change has occurred
    {
      // See if we're triplex or not
      ...
      // Get matrices for NR (should be the same for triplex vs normal, with
      // this implementation)
...
// Functionalized "postsyc after link::postsync" items -- mostly for deltamode
// compatibility Return values -- 0 = no iteration, 1 = reiterate (deltamode or
// otherwise), 2 = proceed (deltamode) Pass value is for deltamode pass
// information (currently in modified Euler flagging)
int series_compensator::sercom_postPost_fxn(unsigned char pass_value,
  ...
  if (deltatimestep_running > 0) {
    ...
    // This may be a place to do the turns ratio update, or in the above logic
    // (with the return value) Probably where the integration-method code would
    // go Need to flag if deltamode or not -- use "" to do that (possibly inside
    // the function)
    ...
    if (pass_value == 0) {
      ...
      if (frequency_regulation) {
        ...
        else if ((val_FromNode_frequency < frequency_low) && !t_hold_low_flag &&
                 !t_recover_low_flag && !t_delay_high_flag &&
                 !t_hold_high_flag && !t_recover_high_flag) {
        ...
        else if ((val_FromNode_frequency > frequency_high) &&
                 !t_hold_high_flag && !t_recover_high_flag &&
                 !t_delay_low_flag && !t_hold_low_flag &&
                 !t_recover_low_flag) // t_delay status
        {
      ...
      if ((NR_branchdata[NR_branch_reference].origphases & 0x80) == 0x80) {
        ...
        if ((NR_branchdata[NR_branch_reference].phases & 0x80) == 0x80) {
          ...
          for (index_val = 0; index_val < 2; index_val++) {
            vset_value[index_val] =
                delta_V + vset_value_0[index_val]; // get the voltage set point
            ...
            // integrator
            pred_state.dn_ini_StateVal[index_val] =
                (vset_value[index_val] - val_ToNode_voltage_pu[index_val]) * ki;
            pred_state.n_ini_StateVal[index_val] =
                curr_state.n_ini_StateVal[index_val] +
                pred_state.dn_ini_StateVal[index_val] * deltat;
            ...
            pred_state.n_StateVal[index_val] =
                pred_state.n_ini_StateVal[index_val] +
                pred_state.dn_ini_StateVal[index_val] / ki * kp;
        ...
        // Default else - don't do anything -- does this maybe need to be
        // zeroed?
      ...
      else // Some form of three-phase
      {
        ...
        for (index_val = 0; index_val < 3; index_val++) {
          ...
          if ((NR_branchdata[NR_branch_reference].phases & phase_mask) ==
              phase_mask) {
      ...
      if ((NR_branchdata[NR_branch_reference].origphases & 0x80) == 0x80) {
        ...
        if ((NR_branchdata[NR_branch_reference].phases & 0x80) == 0x80) {
          ...
          for (index_val = 0; index_val < 2; index_val++) {
            ...
            next_state.dn_ini_StateVal[index_val] =
                (vset_value[index_val] - val_ToNode_voltage_pu[index_val]) * ki;
            next_state.n_ini_StateVal[index_val] =
                curr_state.n_ini_StateVal[index_val] +
                (pred_state.dn_ini_StateVal[index_val] +
                 next_state.dn_ini_StateVal[index_val]) *
                    deltat / 2;
            ...
            // PI controller output
            next_state.n_StateVal[index_val] =
                next_state.n_ini_StateVal[index_val] +
                next_state.dn_ini_StateVal[index_val] / ki * kp;
    ...
    if ((NR_branchdata[NR_branch_reference].origphases & 0x80) == 0x80) {
      ...
      if ((NR_branchdata[NR_branch_reference].phases & 0x80) == 0x80) {
        ...
        for (index_val = 0; index_val < 2; index_val++) {
          temp_diff_val[index_val] =
              fabs(vset_value[index_val] - val_ToNode_voltage_pu[index_val]);

          // Check it
          if (temp_diff_val[index_val] > voltage_iter_tolerance) {
            return_val = 1; // Arbitrary flag - could be used for other things
                            // in the future
          }
        }
      }
...
SIMULATIONMODE series_compensator::inter_deltaupdate_series_compensator(
  ...
  } else             // After the call
  {
    ...
    //******************* Implementation -- Inside this function will probably
    //be where the logic and/or pred/corr needs to go ******//
    ...
    // Otherwise, it was a proceed forward -- probably returned a future state
    // time
...
extern "C" MODULE_API int isa_series_compensator(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_series_compensator_impl(obj, classsname);
}
#endif
...
EXPORT SIMULATIONMODE interupdate_series_compensator(
  ...
  try {
    status = my->inter_deltaupdate_series_compensator(
        delta_time, dt, iteration_count_val, interupdate_pos);
    return status;
  } catch (char *msg) {
    gl_error("interupdate_series_compensator(obj=%d;%s): %s", obj->id,
