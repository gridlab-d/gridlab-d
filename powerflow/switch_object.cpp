...
switch_object::switch_object(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "switch", sizeof(switch_object),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "link", PT_enumeration, "phase_A_state",
            PADDR(phase_A_state), PT_DESCRIPTION,
            "Defines the current state of the phase A switch", PT_KEYWORD,
            "OPEN", (enumeration)SW_OPEN, PT_KEYWORD, "CLOSED",
            (enumeration)SW_CLOSED, PT_enumeration, "phase_B_state",
            PADDR(phase_B_state), PT_DESCRIPTION,
            "Defines the current state of the phase B switch", PT_KEYWORD,
            "OPEN", (enumeration)SW_OPEN, PT_KEYWORD, "CLOSED",
            (enumeration)SW_CLOSED, PT_enumeration, "phase_C_state",
            PADDR(phase_C_state), PT_DESCRIPTION,
            "Defines the current state of the phase C switch", PT_KEYWORD,
            "OPEN", (enumeration)SW_OPEN, PT_KEYWORD, "CLOSED",
            (enumeration)SW_CLOSED, PT_enumeration, "operating_mode",
            PADDR(switch_banked_mode), PT_DESCRIPTION,
            "Defines whether the switch operates in a banked or per-phase "
            "control mode",
            PT_KEYWORD, "INDIVIDUAL", (enumeration)INDIVIDUAL_SW, PT_KEYWORD,
            "BANKED", (enumeration)BANKED_SW, PT_complex,
            "switch_impedance[Ohm]", PADDR(switch_impedance_value),
            PT_DESCRIPTION, "Impedance value of the swtich when closed",
            PT_double, "switch_resistance[Ohm]",
            PADDR(switch_impedance_value.Re()), PT_DESCRIPTION,
            "Resistance portion of impedance value of the switch when it is "
            "closed.",
            PT_double, "switch_reactance[Ohm]",
            PADDR(switch_impedance_value.Im()), PT_DESCRIPTION,
            "Reactance portion of impedance value of the switch when it is "
            "closed.",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);

    if (gl_publish_function(oclass, "change_switch_state",
                            (FUNCTIONADDR)change_switch_state) == nullptr)
      GL_THROW("Unable to publish switch state change function");
    if (gl_publish_function(oclass, "reliability_operation",
                            (FUNCTIONADDR)reliability_operation) == nullptr)
      GL_THROW("Unable to publish switch reliability operation function");
    if (gl_publish_function(oclass, "create_fault",
                            (FUNCTIONADDR)create_fault_switch) == nullptr)
      GL_THROW("Unable to publish fault creation function");
    if (gl_publish_function(oclass, "fix_fault",
                            (FUNCTIONADDR)fix_fault_switch) == nullptr)
      GL_THROW("Unable to publish fault restoration function");
    if (gl_publish_function(oclass, "clear_fault",
                            (FUNCTIONADDR)clear_fault_switch) == nullptr)
      GL_THROW("Unable to publish fault clearing function");
    if (gl_publish_function(oclass, "change_switch_faults",
                            (FUNCTIONADDR)switch_fault_updates) == nullptr)
      GL_THROW("Unable to publish switch fault correction function");
    if (gl_publish_function(oclass, "change_switch_state_toggle",
                            (FUNCTIONADDR)change_switch_state_toggle) ==
        nullptr)
      GL_THROW("Unable to publish switch toggle function");

    // Publish deltamode functions
    if (gl_publish_function(oclass, "interupdate_pwr_object",
                            (FUNCTIONADDR)interupdate_switch) == nullptr)
    ...
    if (gl_publish_function(oclass, "check_limits_pwr_object",
      ...
      GL_THROW(
          "Unable to publish switch external power limit calculation function");
    if (gl_publish_function(oclass, "perform_current_calculation_pwr_link",
                            (FUNCTIONADDR)currentcalculation_link) == nullptr)
      GL_THROW(
          "Unable to publish switch external current calculation function");

    // Other
    if (gl_publish_function(oclass, "pwr_object_kmldata",
                            (FUNCTIONADDR)switch_object_kmldata) == nullptr)
...
int switch_object::create() {
  ...
  prefault_banked = false;      // Condition of the switch prior to a fault -
                           // individual mode has to be enacted for reliability
...
int switch_object::init(OBJECT *parent) {
  ...
  // Special flag moved to be universal for all solvers - mainly so phase checks
  // catch it now
  SpecialLnk = SWITCH;
  ...
  if (((phase_from | phase_to | phases) & PHASE_S) == PHASE_S) {
    GL_THROW(
        "switch:%d - %s - triplex/S-phase connections unsupported at this time",
        get_id(), get_name());
    /*  TROUBLESHOOT
    A switch was placed connected to a triplex/split-phase node, or was flagged
    as triplex/split-phase itself.  This is not supported at this time - all
    switching must be done on the standard three-phase portion of the system.
    */
  }
  ...
  if (solver_method == SM_FBS) {
    for (indexa = 0; indexa < 3; indexa++) {
      for (indexb = 0; indexb < 3; indexb++) {
        // These have to be zeroed (nature of switch - mostly to zero, but also
        // are zero-impedance in FBS)
    ...
    switch may induce the desired behavior on the system.  If open, it should
    prevent current flow and set the "to" end voltage to zero.  However, this
    may cause problems in many powerflow conditions or not properly solve.
    Consider using the Newton-Raphson solver in conjunction with the reliability
    package, or rearrange your system.
    */
  } else {
  ...
  if (solver_method == SM_NR) {
    ...
    if (switch_impedance_value.Re() == 0.0) {
      gl_warning("Switch:%s switch_resistance has been set to zero. This will "
                 "result singular matrix. Setting to the global default.",
                 obj->name);
      /*  TROUBLESHOOT
      Under Newton-Raphson solution method the impedance matrix cannot be a
      singular matrix for the inversion process. Change the value of
      switch_resistance to something small but larger that zero.
      */
    ...
    // React
    if (switch_impedance_value.Im() == 0.0) {
      gl_warning("Switch:%s switch_reactance has been set to zero. This will "
                 "result singular matrix. Setting to the global default.",
                 obj->name);
      /*  TROUBLESHOOT
      Under Newton-Raphson solution method the impedance matrix cannot be a
      singular matrix for the inversion process. Change the value of
      switch_reactance to something small but larger that zero.
      */
  ...
  if (switch_banked_mode == BANKED_SW) {
    ...
    // Banked mode is operated by majority rule.  If the majority are a
    // different state, all become that state
    phase_total = (double)(has_phase(PHASE_A) + has_phase(PHASE_B) +
                           has_phase(PHASE_C)); // See how many switches we have
    ...
    if (switch_total > 0.5) // In two switches, a stalemate occurs.  We'll
                            // consider this a "maintain status quo" state
    {
      // Initial check, make sure stays open
      if (status == LS_OPEN) {
        phase_A_state = phase_B_state = phase_C_state =
            SW_CLOSED; // Set all to open - phase checks will sort it out
      } else {
        phase_A_state = phase_B_state = phase_C_state =
            SW_CLOSED; // Set all to closed - phase checks will sort it out
      }
    } else // Minority or stalemate
    {
      if (switch_total == 0.5) // Stalemate
      {
        if (status == LS_OPEN) // These check assume phase_X_state will be
                               // manipulated, not status
        {
      ...
      } else // Not stalemate - open all
      {
        status = LS_OPEN;
        phase_A_state = phase_B_state = phase_C_state =
            SW_OPEN; // Set all to open - phase checks will sort it out
      }
    }
...
// functionality
void switch_object::BOTH_switch_sync_pre(unsigned char *work_phases_pre,
                                         unsigned char *work_phases_post) {
  if ((solver_method == SM_NR) &&
      (event_schedule != nullptr)) // NR-reliability-related stuff
  {
  ...
  gl_verbose("  BOTH_switch_sync_pre leaving:%s:%s", work_phases_pre,
             work_phases_post);
}

// Functionalized switch sync call -- after link call -- for deltamode
// functionality
void switch_object::NR_switch_sync_post(unsigned char *work_phases_pre,
                                        unsigned char *work_phases_post,
                                        OBJECT *obj, TIMESTAMP *t0,
                                        TIMESTAMP *t2) {
  unsigned char work_phases,
      work_phases_closed; // work_phases_pre, work_phases_post,
                          // work_phases_closed;
  char fault_val[9];
  ...
  if (!meshed_fault_checking_enabled) {
    ...
    if (*work_phases_pre != *work_phases_post) {
      ...
      case 0x00: // No switches opened !??
        GL_THROW("switch:%s supposedly opened, but doesn't register the right "
                 "phases",
                 obj->name);
        /*  TROUBLESHOOT
        A switch reported changing to an open status.  However, it did not
        appear to fully propogate this condition.  Please try again.  If the
      ...
      if (fault_mode) {
        ...
        for (indexval = 0; indexval < 3; indexval++) {
          ...
          if ((protect_locations[indexval] == -1) &&
              (working_protect_phases[indexval] == 1)) {
            // Flag us as our own device -- otherwise restoration gets a little
            // odd
      ...
      if (event_schedule != nullptr) // Function was mapped - go for it!
      {
        ...
        if (fault_mode) // Restoration - make fail time in the past
        {
          ...
          // Call function
          result_val = ((int (*)(OBJECT *, OBJECT *, char *, TIMESTAMP,
                                 TIMESTAMP, int, bool))(*event_schedule))(
              *eventgen_obj, obj, fault_val, (*t0 - 50), temp_time, impl_fault,
              fault_mode);
        } else // Failing - normal
        {
          result_val = ((int (*)(OBJECT *, OBJECT *, char *, TIMESTAMP,
                                 TIMESTAMP, int, bool))(*event_schedule))(
              *eventgen_obj, obj, fault_val, *t0, TS_NEVER, -1, fault_mode);
        }

        // Make sure it worked
        if (result_val != 1) {
          GL_THROW("Attempt to change switch:%s failed in a reliability manner",
                   obj->name);
          /*  TROUBLESHOOT
          While attempting to propagate a changed switch's impacts, an error was
      ...
      else // No object, just fail us out - save the iterations
      {
        ...
        /*  TROUBLESHOOT
        A switch changed and created an open link.  If the system is not meshed,
        the Newton-Raphson solver will likely fail.  Theoretically, this should
        be a quick fail due to a singular matrix. However, the system
        occasionally gets stuck and will exhaust iteration cycles before
        continuing. If the fuse is blowing and the NR solver still iterates for
        a long time, this may be the case.
        */
      }
    } // End NR call
  } // end not meshed checking mode
  // defaulted else -- meshed checking, but we don't do anything for that (link
  // handles all)
}
...
TIMESTAMP switch_object::presync(TIMESTAMP t0) {
  ...
  if (local_switching) {
    if (switch_banked_mode == BANKED_SW) { // if any phase state has changed,
    ...
    if (phase_A_state == SW_CLOSED)
      phased_switch_status |= 0x04;
    if (phase_B_state == SW_CLOSED)
      phased_switch_status |= 0x02;
    if (phase_C_state == SW_CLOSED)
      phased_switch_status |= 0x01;
    phase_changes = phased_switch_status ^ prev_full_status;
    gl_verbose("switch_object::presync:%s:%ld:%d:%d:%d", get_name(), t0,
               prev_full_status, phased_switch_status, phase_changes);
    if (phased_switch_status != prev_full_status) {
      ...
      if (closing) {
        ...
        link_fault_on(&protect_obj, fault_type, &implemented_fault,
                      &repair_time);
      }
    }
  }
...
TIMESTAMP switch_object::sync(TIMESTAMP t0) {
  ...
  gl_verbose("switch_object::sync:%s:%ld:%d:%d:%d", get_name(), t0,
             phase_A_state, phase_B_state, phase_C_state);
  ...
  if (!event_schedule_map_attempt) {
    ...
    if (fault_check_object != nullptr) {
      ...
      if (*eventgen_obj != nullptr) {
        ...
        event_schedule =
            (FUNCTIONADDR)(gl_get_function(*eventgen_obj, "add_event"));

        // Make sure it was found
        if (event_schedule == nullptr) {
          gl_warning("Unable to map add_event function in eventgen:%s",
                     *(*eventgen_obj)->name);
          /*  TROUBLESHOOT
          While attempting to map the "add_event" function from an eventgen
          object, the function failed to be found.  Ensure the target object in
          fault_check is an eventgen object and this function exists.  If the
          error persists, please submit your code and a bug report via the trac
          website.
          */
        }
      } else { // no event generator, but we might want regular power flow with
               // faults
        ...
        if (!flt_chk_property->is_valid() ||
            !flt_chk_property->is_enumeration()) {
          GL_THROW("switch:%d - %s - Failure to map fault_check property",
                   obj->id, (obj->name ? obj->name : "Unnamed"));
          /*  TROUBLESHOOT
          While attempting to map the "check_mode" property of the fault_check
          object, an error was encountered. Please check your GLM and try again.
          If the error persists, please submit your model and a description to
          the issues system.
          */
        }
  ...
  gl_verbose("  calling link_object::sync:%d:%d:%d", phase_A_state,
             phase_B_state, phase_C_state);
...
// since essentially used in reliability calls as well, so need to make sure the
// two call points are consistent
void switch_object::switch_sync_function(void) {
  ...
  gl_verbose("  switch_sync_function:%s:%d:%d:%d:%d:%d", get_name(), status,
             prev_status, phase_A_state, phase_B_state, phase_C_state);
  pres_status =
      0x00; // Reset individual status indicator - assumes all start open
  ...
  if (!meshed_fault_checking_enabled) //"Normal" mode
  {
    if (switch_banked_mode == BANKED_SW) // Banked mode
    {
      if (status == prev_status) {
        ...
        // different state, all become that state
        phase_total =
            (double)(has_phase(PHASE_A) + has_phase(PHASE_B) +
                     has_phase(PHASE_C)); // See how many switches we have
      ...
      else // Not the same - force the inputs
      {
        if (status == LS_OPEN)
          phase_A_state = phase_B_state = phase_C_state =
              SW_OPEN; // Flag all as open
        else
          phase_A_state = phase_B_state = phase_C_state =
              SW_CLOSED; // Flag all as closed
      } // End not same
      ...
      if (status == LS_OPEN) {
        if (has_phase(PHASE_A)) {
          if (solver_method == SM_NR) {
            ...
            b_mat[0][0] = gld::complex(0.0, 0.0);
            a_mat[0][0] = 0.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[0][0] = 0.0;
            NR_branchdata[NR_branch_reference].phases &=
                0xFB; // Remove this bit
          } else      // Assume FBS
          {
            A_mat[0][0] = gld::complex(0.0, 0.0);
            d_mat[0][0] = gld::complex(0.0, 0.0);
        ...
        if (has_phase(PHASE_B)) {
          if (solver_method == SM_NR) {
            ...
            b_mat[1][1] = gld::complex(0.0, 0.0);
            a_mat[1][1] = 0.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[1][1] = 0.0;
            NR_branchdata[NR_branch_reference].phases &=
                0xFD; // Remove this bit
          } else {
            A_mat[1][1] = gld::complex(0.0, 0.0);
            d_mat[1][1] = gld::complex(0.0, 0.0);
            B_mat[1][1] = gld::complex(0.0, 0.0);
        ...
        if (has_phase(PHASE_C)) {
          if (solver_method == SM_NR) {
            ...
            b_mat[2][2] = gld::complex(0.0, 0.0);
            a_mat[2][2] = 0.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[2][2] = 0.0;
            NR_branchdata[NR_branch_reference].phases &=
                0xFE; // Remove this bit
          } else {
            A_mat[2][2] = gld::complex(0.0, 0.0);
            d_mat[2][2] = gld::complex(0.0, 0.0);
            B_mat[2][2] = gld::complex(0.0, 0.0);
      ...
      else // Must be closed then
      {
        if (has_phase(PHASE_A)) {
          ...
          if (solver_method == SM_NR) {
            ...
            b_mat[0][0] = switch_impedance_value;
            a_mat[0][0] = 1.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[0][0] = 1.0;
            NR_branchdata[NR_branch_reference].phases |=
                0x04; // Ensure we're set
          } else      // Assumed FBS
          {
            A_mat[0][0] = 1.0;
            d_mat[0][0] = 1.0;
        ...
        if (has_phase(PHASE_B)) {
          ...
          if (solver_method == SM_NR) {
            ...
            b_mat[1][1] = switch_impedance_value;
            a_mat[1][1] = 1.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[1][1] = 1.0;
            NR_branchdata[NR_branch_reference].phases |=
                0x02; // Ensure we're set
          } else {
            A_mat[1][1] = 1.0;
            d_mat[1][1] = 1.0;
            B_mat[1][1] = switch_impedance_value;
        ...
        if (has_phase(PHASE_C)) {
          ...
          if (solver_method == SM_NR) {
            ...
            b_mat[2][2] = switch_impedance_value;
            a_mat[2][2] = 1.0; // Update the voltage ratio matrix as well (for
                               // power calcs)
            d_mat[2][2] = 1.0;
            NR_branchdata[NR_branch_reference].phases |=
                0x01; // Ensure we're set
          } else {
            A_mat[2][2] = 1.0;
            d_mat[2][2] = 1.0;
            B_mat[2][2] = switch_impedance_value;
  ...
  else // Meshed fault checking -- phases are handled externally by link.cpp,
       // just like other objects, but only with an eventgen call
  {
    if (status != prev_status) {
      ...
      if (fault_handle_call == nullptr) {
        ...
        if (fault_check_object != nullptr) {
          // Get the link
          fault_handle_call = (FUNCTIONADDR)(gl_get_function(
              fault_check_object, "reliability_alterations"));

          // Make sure it worked
          if (fault_handle_call == nullptr) {
            GL_THROW(
                "switch:%d - %s - Failed to map the topology update function!",
                obj->id, (obj->name ? obj->name : "Unnamed"));
            /*  TROUBLESHOOT
            While attempting to map up the topology reconfigure function, an
            error was encountered.  Please try again. If the error persists,
            please submit your file and an issue report.
            */
          }
        }
      ...
      if (status == LS_CLOSED) {
        ...
        if (fault_handle_call != nullptr) {
          ...
          result_val = ((int (*)(OBJECT *, int, bool))(*fault_handle_call))(
              fault_check_object, NR_branch_reference, true);

          // Make sure it worked
          if (result_val != 1) {
            GL_THROW("switch:%d - %s - Topology update fail!", obj->id,
                     (obj->name ? obj->name : "Unnamed"));
            /*  TROUBLESHOOT
            While attempting to update the topology of the system after a switch
      ...
      } else // Must be open
      {
        ...
        if (fault_handle_call != nullptr) {
          // Call the topology update
          result_val = ((int (*)(OBJECT *, int, bool))(*fault_handle_call))(
              fault_check_object, NR_branch_reference, false);
...
// Function to replicate sync_switch_function, but not call anything (just for
// reliability checks)
unsigned char switch_object::switch_expected_sync_function(void) {
  ...
  gl_verbose("  switch_expected_sync_function:%d:%d:%d", phase_A_state,
             phase_B_state, phase_C_state);
    ...
    if (switch_banked_mode == BANKED_SW) // Banked mode
    {
      if (temp_status == prev_status) {
                                ...
                                // consider this a "maintain status quo" state
        {
          ...
          temp_A_state = temp_B_state = temp_C_state =
              SW_CLOSED; // Set all to closed - phase checks will sort it out
        } else           // Minority or stalemate
        {
          if (switch_total == 0.5) // Stalemate
          {
            if (temp_status == LS_OPEN) // These check assume phase_X_state will
                                        // be manipulated, not status
            {
...
// Function to externally set switch status - mainly for "out of step" updates
// under NR solver where admittance needs to be updated
    ...
    if (status != prev_status) // Something's changed
    {
      ...
      if (status == LS_OPEN) {
        if (has_phase(PHASE_A)) {
          ...
          b_mat[0][0] = gld::complex(0.0, 0.0);
          a_mat[0][0] =
              0.0; // Update the voltage ratio matrix as well (for power calcs)
          d_mat[0][0] = 0.0;
          NR_branchdata[NR_branch_reference].phases &=
              0xFB;                // Ensure we're not set
      ...
      else // Must be closed then
      {
        if (has_phase(PHASE_A)) {
          ...
          b_mat[0][0] = switch_impedance_value;
          a_mat[0][0] =
              1.0; // Update the voltage ratio matrix as well (for power calcs)
          d_mat[0][0] = 1.0;
...
// Function to externally set switch status - mainly for "out of step" updates
// under NR solver where admittance needs to be updated - this function provides
// individual switching ability 0 = open, 1 = closed, 2 = don't care (leave as
// was)
void switch_object::set_switch_full(char desired_status_A,
                                    char desired_status_B,
                                    char desired_status_C) {
  gl_verbose("set_switch_full:%s:%d:%d:%d", get_name(), desired_status_A,
             desired_status_B, desired_status_C);
...
// Function to externally set switch status - mainly for "out of step" updates
// under NR solver where admittance needs to be updated - this function tracks
// switching position for reliability faults Prevents system from magically
// "turning back on" a single islanded switch when removed as a downstream
// element
void switch_object::set_switch_full_reliability(unsigned char desired_status) {
  unsigned char desA, desB, desC, phase_change;

  gl_verbose("set_switch_full_reliability:%s:%d:%d:%d", get_name(),
             phased_switch_status, int(desired_status), local_switching);
  ...
  if ((phase_change & 0x04) == 0x04) // Phase A
  {
    ...
    if ((desired_status & 0x04) == 0x04) // Putting it back in
    {
      ...
      if ((phased_switch_status & 0x04) ==
          0x04) // Switch A was on when this was faulted
      {
      ...
      faulted_switch_phases &=
          0xFB; // Remove it from the "fault-removed" phasing
  ...
  if ((phase_change & 0x02) == 0x02) // Phase B
  {
    ...
    if ((desired_status & 0x02) == 0x02) // Putting it back in
    {
      ...
      if ((phased_switch_status & 0x02) ==
          0x02) // Switch B was on when this was faulted
      {
      ...
      faulted_switch_phases &=
          0xFD; // Remove it from the "fault-removed" phasing
    } else      // Removing it (faulting it)
    {
      if (phase_B_state == SW_CLOSED) {
  ...
  if ((phase_change & 0x01) == 0x01) // Phase C
  {
    ...
    if ((desired_status & 0x01) == 0x01) // Putting it back in
    {
      ...
      if ((phased_switch_status & 0x01) ==
          0x01) // Switch C was on when this was faulted
      {
      ...
      faulted_switch_phases &=
          0xFE; // Remove it from the "fault-removed" phasing
    } else      // Removing it (faulting it)
    {
      if (phase_C_state == SW_CLOSED) {
  ...
  if (!meshed_fault_checking_enabled) {
    ...
    if ((faulted_switch_phases != 0x00) && prefault_banked &&
        (switch_banked_mode == BANKED_SW)) {
      // Put into individual mode
      switch_banked_mode = INDIVIDUAL_SW;
    } else if ((faulted_switch_phases == 0x00) && prefault_banked &&
               (switch_banked_mode == INDIVIDUAL_SW)) {
...
// Function to adjust "faulted phases" block - in case something has tried to
// restore itself
...
// Function to pull phase information from the from/to node, using API - for
// triplex check at this point
gld::set switch_object::node_phase_information(OBJECT *obj) {
  ...
  if (!temp_phase_property->is_valid() || !temp_phase_property->is_set()) {
    GL_THROW("switch:%d - %s - unable to map phase property of connecting node",
             get_id(), get_name());
    /*  TROUBLESHOOT
    While attempting to map the phases of the from or to node of a switch-based
    object, an error occurred. Please try again.  If the error persists, please
    submit a bugfix ticket.
    */
  }
...
SIMULATIONMODE switch_object::inter_deltaupdate_switch(
  ...
  if (!interupdate_pos) // Before powerflow call
  {
    ...
    NR_switch_sync_post(&work_phases_pre, &work_phases_post, hdr, &t0_val,
                        &t2_val);
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_switch(OBJECT *obj, TIMESTAMP t0,
                                            PASSCONFIG pass) {
  return sync_switch_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_switch(OBJECT *obj, ...) {
  va_list args;
  TIMESTAMP t0;
  PASSCONFIG pass;
...
EXPORT int change_switch_state(OBJECT *thisobj, unsigned char phase_change,
  ...
  gl_verbose("  change_switch_state:%d:%d:%d", phase_change, state,
             swtchobj->switch_banked_mode);

  if ((swtchobj->switch_banked_mode == switch_object::BANKED_SW) ||
      meshed_fault_checking_enabled) // Banked mode - all become "state", just
                                     // cause
  {
...
EXPORT int change_switch_state_toggle(OBJECT *thisobj) {
  ...
  if (funadd == nullptr) {
    ...
    /*  TROUBLESHOOT
    While attempting to map the reliability function to manipulate topology, an
    error occurred. Please try again.  If the bug persists, please submit your
  ...
  ext_result =
      ((int (*)(OBJECT *, int, bool))(*funadd))(fault_check_object, 0, false);
...
// Reliability interface - generalized switch operation so switches and other
// opjects can be similarly
EXPORT int reliability_operation(OBJECT *thisobj,
...
EXPORT int create_fault_switch(OBJECT *thisobj, OBJECT **protect_obj,
  ...
  // Try to fault up
  retval = thisswitch->link_fault_on(protect_obj, fault_type, implemented_fault,
                                     repair_time);

  return retval;
}
EXPORT int fix_fault_switch(OBJECT *thisobj, int *implemented_fault,
                            char *imp_fault_name) {
  int retval;
...
EXPORT int clear_fault_switch(OBJECT *thisobj, int *implemented_fault,
                              char *imp_fault_name) {
  int retval;
...
EXPORT int switch_fault_updates(OBJECT *thisobj,
...
// Deltamode export
EXPORT SIMULATIONMODE interupdate_switch(OBJECT *obj, unsigned int64 delta_time,
                                         unsigned long dt,
  ...
  try {
    status = my->inter_deltaupdate_switch(delta_time, dt, iteration_count_val,
                                          interupdate_pos);
    return status;
  } catch (char *msg) {
...
// KML Export
EXPORT int switch_object_kmldata(OBJECT *obj,
