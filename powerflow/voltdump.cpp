...
voltdump::voltdump(MODULE *mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "voltdump", sizeof(voltdump),
    ...
    if (gl_publish_variable(
            ...
            PT_timestamp, "runtime", PADDR(runtime), PT_DESCRIPTION,
            "the time to check voltage data", PT_char256, "filename",
            PADDR(filename), PT_DESCRIPTION,
            "the file to dump the voltage data into", // must keep this for
                                                      // compatibility
            PT_char256, "file", PADDR(filename), PT_DESCRIPTION,
            "the file to dump the voltage data into", // added 2012-07-10, adds
                                                      // convenience
            PT_int32, "runcount", PADDR(runcount), PT_ACCESS, PA_REFERENCE,
            PT_DESCRIPTION, "the number of times the file has been written to",
            PT_enumeration, "mode", PADDR(mode), PT_DESCRIPTION,
            "dumps the voltages in either polar or rectangular notation",
            PT_KEYWORD, "RECT", (enumeration)VDM_RECT, PT_KEYWORD, "POLAR",
            (enumeration)VDM_POLAR, nullptr) < 1)
...
int voltdump::init(OBJECT *parent) { return 1; }
...
void voltdump::dump(TIMESTAMP t) {
  ...
  // Find the objects - note that "FT_CLASS" requires an explicit match (not
  // parent classing), so this would have to be replicated for all different
  // node types to get it to work.
  if (group[0] == 0) {
    nodes = gl_find_objects(FL_NEW, FT_MODULE, SAME, "powerflow", FT_END);
  } else {
    nodes = gl_find_objects(FL_NEW, FT_MODULE, SAME, "powerflow", AND,
  ...
  fprintf(outfile,
          "# %s run at %s on %i powerflow objects (not all are nodes)\n",
          filename.get_string(), timestr, nodes->hit_count);
  if (mode == VDM_RECT)
    fprintf(outfile, "node_name,voltA_real,voltA_imag,voltB_real,voltB_imag,"
                     "voltC_real,voltC_imag\n");
  else if (mode == VDM_POLAR)
    fprintf(outfile, "node_name,voltA_mag,voltA_angle,voltB_mag,voltB_angle,"
  ...
  while (obj = gl_find_next(nodes, obj)) {
    if (gl_object_isa(obj, "triplex_node", "powerflow")) {
      ...
      // Check it
      if (!node_voltage_value_link[0]->is_valid() ||
          !node_voltage_value_link[0]->is_complex()) {
        GL_THROW(
            "voltdump - Unable to map voltage property of triplex_node:%d - %s",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While the voltdump object attempted to map the voltage_1, voltage_2, or
        voltage_N, an error occurred.  Please try again.  If the error persists,
        please submit your code via the ticketing and issues system.
        */
      }
      ...
      // Check it
      if (!node_voltage_value_link[1]->is_valid() ||
          !node_voltage_value_link[1]->is_complex()) {
        GL_THROW(
            "voltdump - Unable to map voltage property of triplex_node:%d - %s",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        // Defined above
      ...
      // Check it
      if (!node_voltage_value_link[2]->is_valid() ||
          !node_voltage_value_link[2]->is_complex()) {
        GL_THROW(
            "voltdump - Unable to map voltage property of triplex_node:%d - %s",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        // Defined above
      ...
      if (!node_voltage_value_link[0]->is_valid() ||
          !node_voltage_value_link[0]->is_complex()) {
        GL_THROW("voltdump - Unable to map voltage property of node:%d - %s",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While the voltdump object attempted to map the voltage_A, voltage_B, or
        voltage_C, an error occurred.  Please try again.  If the error persists,
        please submit your code via the ticketing and issues system.
        */
      }
    ...
    } else // Skip -- this is just some other object -- consequence of the
           // findlist restrictions
    {
      // Next
      continue;
    }
    ...
    if (mode == VDM_RECT) {
      fprintf(outfile, "%s,%f,%f,%f,%f,%f,%f\n",
              (obj->name ? obj->name : namestr), node_voltage_values[0].Re(),
              node_voltage_values[0].Im(), node_voltage_values[1].Re(),
              node_voltage_values[1].Im(), node_voltage_values[2].Re(),
              node_voltage_values[2].Im());
    } else if (mode == VDM_POLAR) {
      fprintf(outfile, "%s,%f,%f,%f,%f,%f,%f\n",
              (obj->name ? obj->name : namestr), node_voltage_values[0].Mag(),
              node_voltage_values[0].Arg(), node_voltage_values[1].Mag(),
              node_voltage_values[1].Arg(), node_voltage_values[2].Mag(),
              node_voltage_values[2].Arg());
    }
