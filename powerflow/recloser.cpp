...
recloser::recloser(MODULE *mod) : switch_object(mod)
{
    if (oclass == nullptr)
    {
        ...
        oclass = gl_register_class(mod, "recloser", sizeof(recloser),
        ...
        if (gl_publish_function(oclass, "change_recloser_state",
                                (FUNCTIONADDR)change_recloser_state) == nullptr)
            GL_THROW("Unable to publish recloser state change function");
        if (gl_publish_function(oclass, "recloser_reliability_operation",
                                (FUNCTIONADDR)recloser_reliability_operation) ==
            nullptr)
            GL_THROW("Unable to publish recloser reliability operation function");
        if (gl_publish_function(oclass, "change_recloser_faults",
                                (FUNCTIONADDR)recloser_fault_updates) == nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_recloser(OBJECT *obj, ...)
{
    va_list args;
    va_start(args, obj);
...
extern "C" MODULE_API int isa_recloser(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_recloser_impl(obj, classsname);
}
#endif
...
EXPORT int recloser_reliability_operation(OBJECT *thisobj,
...
EXPORT int recloser_fault_updates(OBJECT *thisobj,
