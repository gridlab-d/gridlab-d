...
private:
  ...
  void SPIMupdateVars(); // function to update the previous values for the motor
                         // model
  void
  updateFreqVolt(); // function to update frequency and voltage for the motor
  void SPIMUpdateMotorStatus(); // function to update the status of the motor
  void SPIMStateOFF(); // function to ensure that internal model states are
                       // zeros when the motor is OFF
  void
  SPIMreinitializeVars(); // function to reinitialize values for the motor model
  void SPIMUpdateProtection(
      double delta_time); // function to update the protection of the motor
  void SPIMSteadyState(TIMESTAMP t1); // steady state model for the SPIM motor
  void SPIMDynamic(double curr_delta_time,
                   double dTime); // dynamic phasor model for the SPIM motor
  gld::complex complex_exp(double angle);
  int invertMatrix(gld::complex TF[16], gld::complex ITF[16]);

  // TPIM functions
  void TPIMupdateVars(); // function to update the previous values for the motor
                         // model
  void TPIMUpdateMotorStatus(); // function to update the status of the motor
  void TPIMStateOFF(); // function to ensure that internal model states are
                       // zeros when the motor is OFF
  void
  TPIMreinitializeVars(); // function to reinitialize values for the motor model
  void TPIMUpdateProtection(
      double delta_time); // function to update the protection of the motor
  void TPIMSteadyState(TIMESTAMP t1); // steady state model for the TPIM motor
  void TPIMDynamic(double curr_delta_time,
                   double dTime); // dynamic phasor model for the TPIM motor
  ...
  int uv_lockout;
  typedef enum { contactorOPEN = 0, contactorCLOSED = 1 } CONTACTOR_STATE;
