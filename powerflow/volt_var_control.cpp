/** $Id: volt_var_control.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        Baran, M.E. and M.Y Hsu, "Volt/Var Control at Distribution Substations,"
in IEEE Transactions on Power Systems, vol. 14, no. 1, February 1999, pp.
312-318.

        Borozan, V., M.E. Baran, and D. Novosel, "Integrated Volt/Var Control in
Distribution Systems," in Proceedings of the 2001 Power Engineering Society
Winter Meeting, vol. 3, Jan. 28-Feb. 01, 2001, Columbus, OH, USA, pp. 1485-1490.
...
volt_var_control::volt_var_control(MODULE *mod) : powerflow_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "volt_var_control", sizeof(volt_var_control),
                          PC_PRETOPDOWN | PC_POSTTOPDOWN | PC_AUTOLOCK);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);
    if (gl_publish_variable(
            oclass, PT_enumeration, "control_method", PADDR(control_method),
            PT_DESCRIPTION, "IVVC activated or in standby", PT_KEYWORD,
            "ACTIVE", (enumeration)ACTIVE, PT_KEYWORD, "STANDBY",
            (enumeration)STANDBY, PT_double, "capacitor_delay[s]",
            PADDR(cap_time_delay), PT_DESCRIPTION,
            "Default capacitor time delay - overridden by local defintions",
            PT_double, "regulator_delay[s]", PADDR(reg_time_delay),
            PT_DESCRIPTION,
            "Default regulator time delay - overriden by local definitions",
            PT_double, "desired_pf", PADDR(desired_pf), PT_DESCRIPTION,
            "Desired power-factor magnitude at the substation transformer or "
            "regulator",
            PT_double, "d_max", PADDR(d_max), PT_DESCRIPTION,
            "Scaling constant for capacitor switching on - typically 0.3 - 0.6",
            PT_double, "d_min", PADDR(d_min), PT_DESCRIPTION,
            "Scaling constant for capacitor switching off - typically 0.1 - "
            "0.4",
            PT_object, "substation_link", PADDR(substation_lnk_obj),
            PT_DESCRIPTION,
            "Substation link, transformer, or regulator to measure power "
            "factor",
            PT_set, "pf_phase", PADDR(pf_phase), PT_DESCRIPTION,
            "Phase to include in power factor monitoring", PT_KEYWORD, "A",
            (gld::set)PHASE_A, PT_KEYWORD, "B", (gld::set)PHASE_B, PT_KEYWORD,
            "C", (gld::set)PHASE_C, PT_char1024, "regulator_list",
            PADDR(regulator_list), PT_DESCRIPTION,
            "List of voltage regulators for the system, separated by commas",
            PT_char1024, "capacitor_list", PADDR(capacitor_list),
            PT_DESCRIPTION,
            "List of controllable capacitors on the system separated by commas",
            PT_char1024, "voltage_measurements", PADDR(measurement_list),
            PT_DESCRIPTION,
            "List of voltage measurement devices, separated by commas",
            PT_char1024, "minimum_voltages", PADDR(minimum_voltage_txt),
            PT_DESCRIPTION,
            "Minimum voltages allowed for feeder, separated by commas",
            PT_char1024, "maximum_voltages", PADDR(maximum_voltage_txt),
            PT_DESCRIPTION,
            "Maximum voltages allowed for feeder, separated by commas",
            PT_char1024, "desired_voltages", PADDR(desired_voltage_txt),
            PT_DESCRIPTION,
            "Desired operating voltages for the regulators, separated by "
            "commas",
            PT_char1024, "max_vdrop", PADDR(max_vdrop_txt), PT_DESCRIPTION,
            "Maximum voltage drop between feeder and end measurements for each "
            "regulator, separated by commas",
            PT_char1024, "high_load_deadband", PADDR(vbw_high_txt),
            PT_DESCRIPTION,
            "High loading case voltage deadband for each regulator, separated "
            "by commas",
            PT_char1024, "low_load_deadband", PADDR(vbw_low_txt),
            PT_DESCRIPTION,
            "Low loading case voltage deadband for each regulator, separated "
            "by commas",
            PT_bool, "pf_signed", PADDR(pf_signed), PT_DESCRIPTION,
            "Set to true to consider the sign on the power factor.  Otherwise, "
            "it just maintains the deadband of +/-desired_pf",
            nullptr) < 1)
...
int volt_var_control::create(void) {
  ...
  phases = PHASE_A; // Arbitrary - set so powerflow_object shuts up (library
                    // doesn't grant us access to time functions)

  control_method = ACTIVE; // Turn it on by default
  prev_mode = ACTIVE; // Flag so it will go straight in by default (subject to
                      // regulator checks)
  ...
  Regulator_Change =
      false; // Start by assuming no regulator change is occurring
  TRegUpdate = nullptr;
  ...
  pf_phase =
      0; // No phases monitored by default - will drop to link if unpopulated
...
int volt_var_control::init(OBJECT *parent) {
  ...
  char tempchar[1024];
  char numchar[3]; // Assumes we'll never have more than 99 regulators - I hope
                   // this is valid
  OBJECT *temp_obj;
  capacitor **pCapacitor_list_temp;
  double *temp_cap_size;
  int *temp_cap_idx, *temp_cap_idx_work, *temp_meas_idx;
  double cap_adder, temp_double, nom_volt, default_min, default_max,
      default_des, default_max_vdrop, default_vbw_low, default_vbw_high;
  gld::set temp_phase;
  int num_min_volt, num_max_volt, num_des_volt, num_max_vdrop, num_vbw_low,
      num_vbw_high, total_meas;
  bool reg_list_type;
  ...
  if (pf_signed) {
    if ((d_max <= 0.0) || (d_max > 1.0)) {
      GL_THROW("volt_var_control %s: d_max must be a number between 0 and 1",
               obj->name);
      /*  TROUBLESHOOT
      The capacitor threshold value d_max represents a fraction of a capacitor's
      total kVA rating. It must be specified greater than 0, but less than or
      equal to 1.
      */
    }
    ...
    if ((desired_pf <= -1) || (desired_pf > 1)) {
      GL_THROW("volt_var_control %s: Desired signed power factor is outside "
               "the valid range",
               obj->name);
      /*  TROUBLESHOOT
      With a signed power factor enabled, the desired_pf must be between -1
      and 1.  Valid ranges are greater than -1 and less than or equal to 1 (-1
      is not accepted, use 1.0 as unity power factor).
      */
    }
  } else //"Traditional" algorithm
  {
    if ((d_max <= 0.0) || (d_max > 1.0) || (d_min <= 0.0) || (d_min > 1.0)) {
      GL_THROW("volt_var_control %s: d_max and d_min must be a number between "
               "0 and 1",
               obj->name);
      /*  TROUBLESHOOT
      The capacitor threshold values d_max and d_min represent a fraction of a
      capacitor's total kVA rating. They must be specified greater than 0, but
    ...
    if (d_min >= d_max) {
      ...
      /*  TROUBLESHOOT
      The capacitor threshold fraction for d_min must be less than d_max.
      Otherwise, improper (or no) operation will occur.
      */
    }
    ...
    {
      GL_THROW("volt_var_control %s: Desired power factor is negative.  Should "
               "pf_signed be set?",
               obj->name);
      /*  TROUBLESHOOT
      A negative power factor was entered in the "deadband" operation mode of
      the controller.  If a signed power factor value was desired, set the
      pf_signed paramter to true to proceed.  Otherwise, correct the desired
      power factor.
      */
    } else if (desired_pf > 1) // Greater than 1, invalid
    {
      GL_THROW("volt_var_control %s: Desired power factor is outside the valid "
               "range",
               obj->name);
      /*  TROUBLESHOOT
      With a normal power factor enabled, the desired_pf must be between 0
      and 1.  Valid ranges are greater than or equal to 0 and less than or equal
      to 1.
      */
    }
  ...
  if (cap_time_delay < 0) {
    GL_THROW("volt_var_control %s: capacitor_delay must be a positive number!",
             obj->name);
    /*  TROUBLESHOOT
    The default capacitor delay must be a positive number.  Non-causal or
    negative time delays are not permitted.
    */
  }

  if (cap_time_delay < 1) // Has to be positive in leiu of previous error check
  {
    gl_warning("volt_var_control %s: capacitor_delay should be greater than or "
               "equal to 1 to prevent convergence issues.",
               obj->name);
    /*  TROUBLESHOOT
    The capacitor_delay should be greater than or equal to 1.0 to avoid having a
    system oscillate switch positions until the convergence limit is reached.
    */
  }

  if (reg_time_delay < 0) {
    GL_THROW("volt_var_control %s: regulator_delay must be a positive number!",
             obj->name);
    /*  TROUBLESHOOT
    The default regilator delay must be a positive number.  Non-causal or
    negative time delays are not permitted.
    */
  }
  ...
  {
    gl_warning("volt_var_control %s: regulator_delay should be greater than or "
               "equal to 1 to prevent convergence issues.",
               obj->name);
    /*  TROUBLESHOOT
    The regulator_delay should be greater than or equal to 1.0 to avoid having a
    system oscillate switch positions until the convergence limit is reached.
    */
  }
  ...
  if (num_regs == 1) // Only 1 column, make sure something is actually in there
  {
    ...
    if (temp_obj ==
        nullptr) // Not really an object, must be no controllable capacitors
      num_regs = 0;
  }

  if (num_regs > 0) {
    // Allocate the relevant variables - lots of them - single allocs are
    // probably pretty silly, but meh
    pRegulator_list = (regulator **)gl_malloc(num_regs * sizeof(regulator *));

    if (pRegulator_list == nullptr) {
      GL_THROW("volt_var_control %s: regulator storage allocation failed",
               obj->name);
      /*  TROUBLESHOOT
      While attempting to allocate one of the storage vectors for the
      regulators, an error was encountered.  Please try again.  If the error
      persists, please submit your code and a bug report via the trac website.
      */
    }

    pRegulator_configs = (regulator_configuration **)gl_malloc(
        num_regs * sizeof(regulator_configuration *));
    ...
    PrevRegState = (regulator_configuration::Control_enum *)gl_malloc(
        num_regs * sizeof(regulator_configuration::Control_enum));
    ...
    for (index = 0; index < num_regs; index++) {
      ...
      {
        pRegulator_list[index] =
            object_data<regulator>(temp_obj); // Store the regulator

        pRegulator_configs[index] = object_data<regulator_configuration>(
            pRegulator_list[index]->configuration); // Store the configuration

        RegToNodes[index] =
            object_data<node>(pRegulator_list[index]->to); // Store the to node

        if (pRegulator_configs[index]->time_delay != 0) // Delay specified
        {
          RegUpdateTimes[index] =
              pRegulator_configs[index]->time_delay; // use this delay
        } else {
          RegUpdateTimes[index] = reg_time_delay; // Use the default
        }

        reg_step_up[index] = pRegulator_configs[index]->band_center *
                             pRegulator_configs[index]->regulation /
                             ((double)pRegulator_configs[index]
                                  ->raise_taps); // Store upstep value

        reg_step_down[index] = pRegulator_configs[index]->band_center *
                               pRegulator_configs[index]->regulation /
                               ((double)pRegulator_configs[index]
                                    ->lower_taps); // Store downstep value
      ...
      else // General catch
      {
        ...
        GL_THROW("volt_var_control %s: item %s in regulator_list not found",
                 obj->name, token_a);
        /*  TROUBLESHOOT
        While attempting to populate the regulator list, an invalid object was
        found.  Please check the object name.
        */
      }

      // Update the pointer
      token_a = token_a1;
    } // end token FOR - populate regulators, configurations, to nodes, and
      // updates
    ...
    if ((num_des_volt != num_min_volt) || (num_min_volt != num_max_volt)) {
      gl_warning("volt_var_control %s: Desired, minimum, and maximum voltages "
                 "don't match",
                 obj->name);
      /*  TROUBLESHOOT
      The lists for desired_voltages, minimum_voltages, and maximum_voltages
      should all have the same number of elements.  If they don't, if at each
      field has at least one specification, the first for each will be used on
      all regulators.  If they don't all have at least one specification, the
      whole set will be set to default values.
      */
    ...
    for (index = 0; index < num_regs; index++) {
      ...
      if (minimum_voltage[index] >= maximum_voltage[index]) {
        GL_THROW("volt_var_control %s: Minimum voltage limit on regulator %d "
                 "must be less than the maximum voltage limit of the system",
                 obj->name, (index + 1));
        /*  TROUBLESHOOT
        The minimum_voltage value must be less than the maximum_voltage value to
        ensure proper system operation.
        */
      }

      // Make sure desired voltage is within operating limits
      if ((desired_voltage[index] < minimum_voltage[index]) ||
          (desired_voltage[index] > maximum_voltage[index])) {
        GL_THROW("volt_var_control %s: Desired voltage on regulator %d is "
                 "outside the minimum and maximum set points.",
                 obj->name, (index + 1));
        /*  TROUBLESHOOT
        The desired_voltage property is specified in the range outside of the
        minimum_voltage and maximum_voltage set points.  It needs to lie between
        these points for proper operation.  Please adjust the values, or leave
    ...
    if ((num_max_vdrop != num_vbw_low) || (num_vbw_low != num_vbw_high)) {
      gl_warning(
          "volt_var_control %s: VDrop, low V BW, and high V BW don't match",
          obj->name);
      /*  TROUBLESHOOT
      The lists for maximum voltage drop, voltage drop low loading, and voltage
      drop high loading should all have the same number of elements.  If they
      don't, if at each field has at least one specification, the first for each
      will be used on all regulators.  If they don't all have at least one
      specification, the whole set will be set to default values.
      */
    ...
    if (num_max_vdrop >
        1) // One for each regulator - loop precursor (point the tokens)
    {
      // Point to list
    ...
    for (index = 0; index < num_regs; index++) {
      ...
      {
        max_vdrop[index] =
            1.5 * reg_step_up[index]; // Arbitrarily set to 1.5x a step change

        // Set low bandwidth to 2 tap positions out of whack
        vbw_low[index] = 2.0 * reg_step_up[index];

        // Set high bandwidth to 1 tap positions out of whack (more constrained
        // than vbw_low)
      ...
      if (max_vdrop[index] <= 0) {
        GL_THROW("volt_var_control %s: Maximum expected voltage drop specified "
                 "for regulator %d should be a positive non-zero number.",
                 obj->name, (index + 1));
        /*  TROUBLESHOOT
        The max_vdrop property must be greater than 0 for proper operation of
        the coordinated Volt-VAr control scheme.
        */
      }

      // Make sure bandwidth values aren't negative
      if (vbw_low[index] <= 0) {
        GL_THROW("volt_var_control %s: Low loading deadband (bandwidth) for "
                 "regulator %d  must be a positive non-zero number",
                 obj->name, (index + 1));
        /*  TROUBLESHOOT
        The low_load_deadband setting must be greater than 0 for proper
        operation of the Volt-VAr controller.
        */
      }

      if (vbw_high[index] <= 0) {
        GL_THROW("volt_var_control %s: High loading deadband (bandwidth) for "
                 "regulator %d must be a positive non-zero number",
                 obj->name, (index + 1));
        /*  TROUBLESHOOT
        The high_load_deadband setting must be greater than 0 for proper
        operation of the Volt-VAr controller.
        */
      }

      // vbw_high is supposed to be less than vbw_low (heavy loading means we
      // should be more constrained)
      if (vbw_high[index] > vbw_low[index]) {
        gl_warning("volt_var_control %s: Low loading deadband for regulator %d "
                   "is expected to be larger than the high loading deadband",
                   obj->name, (index + 1));
        /*  TROUBLESHOOT
        The high_load_deadband is larger than the low_load_deadband setting. The
        algorithm proposed in the referenced paper suggests low_load_deadband
        should be larger (less constrained) than the high load.  This may need
        to be fixed to be consistent with expected results.
        */
      }
  ...
  else // no regulators found
  {
    ...
    /*  TROUBLESHOOT
    The volt_var_control object requires at one regulator regulator to control
    for coordinated action.  Please specify an appropriate regulator on the
    system.
    */
  }
  ...
  if (control_method == ACTIVE) {
    ...
    {
      if (pRegulator_configs[index]->Control !=
          regulator_configuration::MANUAL) // We're on, but regulator not in
                                           // manual.  Set up as a transition
      {
        prev_mode = STANDBY;
        break; // Only takes one
      }
    }
  } else // We're in standby, so we don't care what the regulator is doing right
         // now
  {
    prev_mode = STANDBY;
  }

  if (substation_lnk_obj ==
      nullptr) // If nothing specified, just use the first feeder regulator
  {
    gl_warning("volt_var_control %s: A link to monitor power-factor was not "
               "specified, using first regulator.",
               obj->name);
    /*  TROUBLESHOOT
    The volt_var_control object requires a link-based object to measure power
    factor.  Since one was not specified, the control scheme will just monitor
    the power values of the first feeder regulator.
    */
  ...
  if (num_caps > 0) {
    if (num_caps == 1) // Only one capacitor
    {
      ...
      if (pCapacitor_list == nullptr) {
        GL_THROW("volt_var_control %s: Failed to allocate capacitor memory",
                 obj->name);
        /*  TROUBLESHOOT
        The volt_var_control failed to allocate memory for the controllable
        capacitors table. Please try again.  If the error persists, please
      ...
      // malloc the update time vector (there is only 1 again, but consistency
      // is key)
      ...
      {
        pCapacitor_list[0] =
            object_data<capacitor>(temp_obj); // Store it as a capacitor

        // Add up the values
        if ((pCapacitor_list[0]->phases_connected & PHASE_D) ==
            PHASE_D) // Delta - grab all three (assumed ABC)
        {
          cap_adder = pCapacitor_list[0]->capacitor_A +
                      pCapacitor_list[0]->capacitor_B +
                      pCapacitor_list[0]->capacitor_C;
        } else // Non-delta
        {
          cap_adder = 0; // Clear the accumulator

          if ((pCapacitor_list[0]->phases_connected & PHASE_A) ==
              PHASE_A) // Add in phase A contribution
            cap_adder += pCapacitor_list[0]->capacitor_A;

          if ((pCapacitor_list[0]->phases_connected & PHASE_B) ==
              PHASE_B) // Add in phase B contribution
            cap_adder += pCapacitor_list[0]->capacitor_B;

          if ((pCapacitor_list[0]->phases_connected & PHASE_C) ==
              PHASE_C) // Add in phase C contribution
      ...
      {
        GL_THROW("volt_var_control %s: Single capacitor list population failed",
                 obj->name);
        /*  TROUBLESHOOT
        While attempting to populate the capacitor list with a single capacitor,
        it somehow failed to recognize this was a valid object.  Please submit
    ...
    else // More than one
    {
      ...
      pCapacitor_list_temp =
          (capacitor **)gl_malloc(num_caps * sizeof(capacitor *));
      ...
      for (index = 0; index < num_caps; index++) {
        ...
        if (temp_obj != nullptr) // Valid object!
        {
          pCapacitor_list_temp[index] =
              object_data<capacitor>(temp_obj); // Store it as a capacitor
          temp_cap_idx[index] = index;          // Store the index

          // Add up the values
          if ((pCapacitor_list_temp[index]->phases_connected & PHASE_D) ==
              PHASE_D) // Delta - grab all three (assumed ABC)
          {
            cap_adder = pCapacitor_list_temp[index]->capacitor_A +
                        pCapacitor_list_temp[index]->capacitor_B +
                        pCapacitor_list_temp[index]->capacitor_C;
          } else // Non-delta
          {
            cap_adder = 0; // Clear the accumulator

            if ((pCapacitor_list_temp[index]->phases_connected & PHASE_A) ==
                PHASE_A) // Add in phase A contribution
              cap_adder += pCapacitor_list_temp[index]->capacitor_A;

            if ((pCapacitor_list_temp[index]->phases_connected & PHASE_B) ==
                PHASE_B) // Add in phase B contribution
              cap_adder += pCapacitor_list_temp[index]->capacitor_B;

            if ((pCapacitor_list_temp[index]->phases_connected & PHASE_C) ==
                PHASE_C) // Add in phase C contribution
          ...
          if (pCapacitor_list_temp[index]->time_delay != 0) {
            CapUpdateTimes[index] =
                pCapacitor_list_temp[index]->time_delay; // Store it
          } else {
        ...
        } else // General catch
        {
          ...
          GL_THROW("volt_var_control %s: item %s in capacitor_list not found",
                   obj->name, token_a);
          /*  TROUBLESHOOT
          While attempting to populate the capacitor list, an invalid object was
          found.  Please check the object name.
          */
        }
      ...
      // Sort the lists on size and distance (assumes put in distance order) -
      // Biggest to smallest, and furthest to closest
      size_sorter(Capacitor_size, temp_cap_idx, num_caps, temp_cap_size,
                  temp_cap_idx_work);
      ...
      gl_free((void **)&pCapacitor_list_temp);
      gl_free((void **)&temp_cap_idx);
      gl_free((void **)&temp_cap_idx_work);
      gl_free((void **)&temp_cap_size);
    } // end more than one cap

    // Populate the "state holder"
    PrevCapState = (capacitor::CAPCONTROL *)gl_malloc(
        num_caps * sizeof(capacitor::CAPCONTROL));

    if (PrevCapState == nullptr) {
      GL_THROW(
          "volt_var_control %s: capacitor previous state allocation failure",
          obj->name);
      /*  TROUBLESHOOT
      While attempting to allocate space for the previous capacitor control
      state vector, a memory allocation error occurred.  Please try again.  If
    ...
    // Another loop, make sure if they are in manual, they have a PT_PHASE
    // defined (other modes will check this inside capacitors)
    for (index = 0; index < num_caps; index++) {
      if (pCapacitor_list[index]->control ==
          capacitor::MANUAL) // Only applies to manual mode
      {
        if ((pCapacitor_list[index]->pt_phase &
             (PHASE_A | PHASE_B | PHASE_C)) == NO_PHASE) {
          temp_obj = object_header(pCapacitor_list[index]);
          gl_warning("Capacitor %s has no pt_phase set, volt_var_control %s "
                     "will pick the first phases_connected",
                     temp_obj->name, obj->name);
          /*  TROUBLESHOOT
          To properly operate capacitors from the volt_var_control object, a
          pt_phase property must be defined.  One was missing, so the
          volt_var_control object assigned the first phase_connected value to
          pt_phase on that capacitor for proper operation.
          */

          if ((pCapacitor_list[index]->phases_connected & PHASE_A) == PHASE_A) {
            pCapacitor_list[index]->pt_phase = PHASE_A;
          } else if ((pCapacitor_list[index]->phases_connected & PHASE_B) ==
                     PHASE_B) {
            pCapacitor_list[index]->pt_phase = PHASE_B;
          } else // We'll just say it has to be C.  Not sure if this is true,
                 // but pt_phase needs to be populated for proper operation
                 // (especially banked)
          {
  ...
  // Only 1 measurement found - figure out where it needs to go - or toss an
  // error
  if (total_meas == 1) {
    ...
    if (measurement_list[0] != 0) {
      ...
      {
        GL_THROW("volt_var_control %s: A measurement is missing a regulator "
                 "association",
                 obj->name);
        // Defined below
      }
  ...
  if ((num_regs == 1) &&
      (total_meas > 1)) // 1 regulator and more than 1 point found
  {
    ...
    {
      GL_THROW("volt_var_control %s: Measurement %s is not a valid node!",
               obj->name, token_a);
      /*  TROUBLESHOOT
      While parsing the measurement list, an invalid object was found.  Please
      verify the object name and try again.
      */
    }

    reg_list_type = false; // Flag as special - single measurement point for a
                           // single regulator
  }
  // Else implies no measurements and/or num_regs > 1 - those will be caught
  // separately below

  // Make sure we found an even number if there is more than one regulator
  // attached - otherwise it is mismatched
  if ((num_regs > 1) ||
      reg_list_type) // more than one regulator, or we're a standard list
  {
    index = total_meas >> 1;
    ...
    if (temp_double != 0) // Should be no remainder
    {
      GL_THROW("volt_var_control %s: A measurement is missing a regulator "
               ...
               obj->name);
      /*  TROUBLESHOOT
      If more than one regulator is present, the measurement_list must be
      specified as a comma-delimited pair with the measurement node and the
      integer of the regulator it is associated with (e.g., 709,2 associates
    ...
    if (total_meas != 0) // There's at least something in there
    {
      ...
      for (index = 0; index < total_meas; index++) {
        ...
        // Make sure we aren't over two digits long - if we need more than 99
        // regulators, I don't want to hear about it
        addy_math = token_b - token_a;

        if (addy_math > 2) // Too many characters
        {
          GL_THROW(
              "volt_var_control %s: Measurement list item pair %d is invalid.",
              obj->name, (index + 1));
          /*  TROUBLESHOOT
          While parsing the measurement list, an invalid pair was encountered.
          This may be due to a typo, or there may not be a proper regulator
          specification.  if more than one regulator is being controlled, the
          list must be specified as "node,reg" pairs.
          */
        }
        ...
        if ((indexa < 0) ||
            (indexa > (num_regs - 1))) // Pre-offset for C indexing
        {
          GL_THROW("volt_var_control %s: Measurement list references a "
                   "nonexistant regulator %d",
                   obj->name, (indexa + 1));
          /*  TROUBLESHOOT
          While parsing the measurement list, an invalid regulator reference
          number was encountered.  These numbers should match the position of
          the regulator in the regulator_list variable.  Please double check
    ...
    // Else is no measurements specified, this will be handled in the general
    // catch below
    ...
    if (temp_meas_idx == nullptr) {
      GL_THROW("volt_var_control %s: measurement list allocation failure",
               obj->name);
      // Defined below
    }
    ...
    for (index = 0; index < num_regs; index++) {
      ...
      {
        pMeasurement_list[index] =
            (node **)gl_malloc(num_meas[index] * sizeof(node *));
      }
      ...
      if (pMeasurement_list[index] == nullptr) {
        GL_THROW("volt_var_control %s: measurement list allocation failure",
                 obj->name);
        /*  TROUBLESHOOT
        While attempting to allocate space for the measurment list, a memory
        allocation error occurred. Please try again.  If the problem persists,
    ...
    for (index = 0; index < total_meas; index++) {
      ...
      if (temp_obj == nullptr) {
        ...
        GL_THROW("volt_var_control %s: measurement object %s was not found!",
                 obj->name, token_a);
        /*  TROUBLESHOOT
        While parsing the measurment list for the volt_var_control object, a
        measurement point was not found.  Please check the name and try again.
        If the problem persists, please submit your code and a bug report via
        the trac website.
        */
      }
      ...
      token_a1 = dbl_token(
          token_a,
          &temp_double); // Pull next - starts on obj, we want to do a count

      token_a = token_a1; // Update the pointer

      indexa = ((int)temp_double) - 1; // Figure out the index

      pMeasurement_list[indexa][temp_meas_idx[indexa]] =
          object_data<node>(temp_obj); // Store it in the list

      temp_meas_idx[indexa]++; // Increment the storage pointer
    }

    // Free up our temporary memory use
    gl_free((void **)&temp_meas_idx);

    // Now loop through again - find any "zero" lists and populate them
    for (index = 0; index < num_regs; index++) {
      if (num_meas[index] == -1) // This was a default case
      {
        pMeasurement_list[index][0] =
            RegToNodes[index]; // Default is just use the load side

        gl_warning("volt_var_control %s: No measurement point specified for "
                   "reg %d - defaulting to load side of regulator",
                   obj->name, (index + 1));
        /*  TROUBLESHOOT
        If no measuremnt points are specified for a regualtor in the
        volt_var_control object, it will default to the load side of the
        regulator.  Please specify measurement points for better control.
        */
  ...
  if ((pf_phase & (PHASE_A | PHASE_B | PHASE_C)) ==
      0) // No phase (that we care about)
  {
    pf_phase =
        (substation_link->phases &
         (PHASE_A | PHASE_B | PHASE_C)); // Just bring in the phases of the link
    gl_warning("volt_var_control %s: Power factor monitored phases not set - "
               "defaulting to substation_link phases",
               obj->name);
    /*  TROUBLESHOOT
    The pf_phases variable was not set, so the volt_var_control object defaulted
    to the phases of the substation_link object. If this is not desired, please
    specify the phases on pf_phases.
    */
  }

  // Make sure the phases are valid - strip off any N or D of either
  temp_phase =
      (pf_phase & (PHASE_A | PHASE_B | PHASE_C)); // Extract the phases we want

  if ((substation_link->phases & temp_phase) != temp_phase) // Mismatch
  {
    GL_THROW("volt_var_control %s: Power factor monitored phases mismatch",
             obj->name);
    /*  TROUBLESHOOT
    One or more of the phases specified in pf_phase does not match with the
    phases of substation_link.  Please double check the values for pf_phases on
    the volt_var_control and phases on the substation_link object.
    */
  }

  if (solver_method == SM_NR) {
    // Set our rank above links - this will put us before regs on the down sweep
    // and before caps on the upsweep (but after current calculations)
    gl_set_rank(obj, 2);
  } else if (solver_method ==
             SM_FBS) // FBS - need to be below substation_link for power/voltage
                     // calculations.  Regs shouldn't be issue
  {
    // Parent us to the substation link to make sure our ranking is happy - do
    // this explicitly. If we already have a parent, over-write it - need to be
    // below voltage updates and power calculations of substation_link
    ...
    if (index < 0) // Make sure it worked
    {
      ...
      /*  TROUBLESHOOT
      An error has occurred while setting the parent field of the
      volt_var_control.  Please submit a bug report and your code so this error
      can be diagnosed further.
      */
    }

    // Rank us 1 below the parent (same as substation link to end)
    gl_set_rank(obj, substation_link->to->rank);
  } else // Any future solvers and GS - GS doesn't play nice with regulators
         // anyways
  {
    GL_THROW(
        "Solver methods other than NR and FBS are unsupported at this time.");
    /*  TROUBLESHOOT
    The volt_var_control object only supports the Newton-Raphson and
    Forward-Back Sweep solvers at this time.  Other solvers (Gauss-Seidel) may
...
TIMESTAMP volt_var_control::presync(TIMESTAMP t0) {
  ...
  if (TUpdateStatus) {
    // Old logic checked for TRegUpdate <= t0 as well - if we have TUpdateStatus
    // true, all times should be t0 anyways
  ...
  {
    first_cycle =
        true; // Flag as first entrant this time step (used to reactive control)
  ...
  {
    if (control_method ==
        ACTIVE) // We're active, implies were in standby before
    {
      for (indexer = 0; indexer < num_regs; indexer++) {
        // Store the state of the regulator
        PrevRegState[indexer] =
            (regulator_configuration::Control_enum)pRegulator_configs[indexer]
                ->Control;
      ...
      for (indexer = 0; indexer < num_caps; indexer++) {
        PrevCapState[indexer] = (capacitor::CAPCONTROL)pCapacitor_list[indexer]
                                    ->control; // Store the old
        pCapacitor_list[indexer]->control =
            capacitor::MANUAL; // Put it into manual mode
      }
    } else // Now in standby, implies were active before
    {
      // Put regulators back into previous mode
      for (indexer = 0; indexer < num_regs; indexer++) {
        pRegulator_configs[indexer]->Control =
            PrevRegState[indexer]; // Put the regulator's mode back
      }
    ...
    for (indexer = 0; indexer < num_regs; indexer++) {
      TRegUpdate[indexer] =
          t0; // Flag us to stay here.  This will force another iteration
    }
  ...
  if (control_method == ACTIVE) // are turned on?
  {
    for (reg_index = 0; reg_index < num_regs; reg_index++) {
      if ((TRegUpdate[reg_index] <= t0) ||
          (TRegUpdate[reg_index] == TS_NEVER)) // See if we're allowed to update
      {
        // Clear flag
        LimitExceed = 0x00;
        ...
        VSet[0] = VSet[1] = VSet[2] =
            desired_voltage[reg_index]; // Default VSet to where we want

        // Initialize tap changes
        prop_tap_changes[0] = prop_tap_changes[1] = prop_tap_changes[2] = 0;

        // Parse through the measurement list - find the lowest voltage - do by
        // PT_PHASE
        for (indexer = 0; indexer < num_meas[reg_index]; indexer++) {
          // See if this node has phase A
          if ((pMeasurement_list[reg_index][indexer]->phases & PHASE_A) ==
              PHASE_A) // Has phase A
          {
            if (pMeasurement_list[reg_index][indexer]->voltage[0].Mag() <
                vmin[0]) // New minimum
            {
          ...
          if ((pMeasurement_list[reg_index][indexer]->phases & PHASE_B) ==
              PHASE_B) // Has phase B
          {
            if (pMeasurement_list[reg_index][indexer]->voltage[1].Mag() <
                vmin[1]) // New minimum
            {
          ...
          if ((pMeasurement_list[reg_index][indexer]->phases & PHASE_C) ==
              PHASE_C) // Has phase A
          {
            if (pMeasurement_list[reg_index][indexer]->voltage[2].Mag() <
                vmin[2]) // New minimum
            {
        ...
        if ((pRegulator_configs[reg_index]->PT_phase & PHASE_A) ==
            PHASE_A) // We have an A
        {
          VDrop[0] = VRegTo[0] - vmin[0]; // Calculate the drop
          VSet[0] = desired_voltage[reg_index] +
                    VDrop[0]; // Calculate where we want to be

          // Make sure it isn't too big or small - otherwise toss a warning (it
          // will cause problem)
          if (VSet[0] > maximum_voltage[reg_index]) // Will exceed
          {
            gl_warning("volt_var_control %s: The set point for phase A will "
                       "exceed the maximum allowed voltage!",
                       object_header(this)->name);
            /*  TROUBLESHOOT
            The set point necessary to maintain the end point voltage exceeds
            the maximum voltage limit specified by the system.  Either increase
            this maximum_voltage limit, or configure your system differently.
            */

            // See what region we are currently in
            if (pRegulator_list[reg_index]->tap[0] > 0) // In raise region
            {
              if ((VRegTo[0] + reg_step_up[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x10; // Flag A as above
              }
            } else // Must be in lower region
            {
              if ((VRegTo[0] + reg_step_down[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x10; // Flag A as above
              }
            }
          ...
          {
            gl_warning("volt_var_control %s: The set point for phase A will "
                       "exceed the minimum allowed voltage!",
                       object_header(this)->name);
            /*  TROUBLESHOOT
            The set point necessary to maintain the end point voltage exceeds
            the minimum voltage limit specified by the system.  Either decrease
            this minimum_voltage limit, or configure your system differently.
            */
        ...
        if ((pRegulator_configs[reg_index]->PT_phase & PHASE_B) ==
            PHASE_B) // We have a B
        {
          VDrop[1] = VRegTo[1] - vmin[1]; // Calculate the drop
          VSet[1] = desired_voltage[reg_index] +
                    VDrop[1]; // Calculate where we want to be

          // Make sure it isn't too big or small - otherwise toss a warning (it
          // will cause problem)
          if (VSet[1] > maximum_voltage[reg_index]) // Will exceed
          {
            gl_warning("volt_var_control %s: The set point for phase B will "
                       "exceed the maximum allowed voltage!",
                       object_header(this)->name);
            ...
            {
              if ((VRegTo[1] + reg_step_up[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x20; // Flag B as above
              }
            } else // Must be in lower region
            {
              if ((VRegTo[1] + reg_step_down[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x20; // Flag B as above
              }
            }
        ...
        if ((pRegulator_configs[reg_index]->PT_phase & PHASE_C) ==
            PHASE_C) // We have a C
        {
          VDrop[2] = VRegTo[2] - vmin[2]; // Calculate the drop
          VSet[2] = desired_voltage[reg_index] +
                    VDrop[2]; // Calculate where we want to be
          // Make sure it isn't too big or small - otherwise toss a warning (it
          // will cause problem)

          if (VSet[2] > maximum_voltage[reg_index]) // Will exceed
          {
            gl_warning("volt_var_control %s: The set point for phase C will "
                       "exceed the maximum allowed voltage!",
                       object_header(this)->name);
            ...
            {
              if ((VRegTo[2] + reg_step_up[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x40; // Flag C as above
              }
            } else // Must be in lower region
            {
              if ((VRegTo[2] + reg_step_down[reg_index]) >
                  maximum_voltage[reg_index]) {
                LimitExceed |= 0x40; // Flag C as above
              }
            }
        ...
        if (pRegulator_configs[reg_index]->control_level ==
            regulator_configuration::INDIVIDUAL) // Individually handled
        {
          // Handle phases
          for (indexer = 0; indexer < 3; indexer++) // Loop through phases
          {
            LimitExceed &=
                0x7F; // Use bit 8 as a validity flag (to save a variable)

            if ((indexer == 0) && ((pRegulator_configs[reg_index]->PT_phase &
                                    PHASE_A) == PHASE_A)) // We have an A
            {
            ...
            if ((indexer == 1) && ((pRegulator_configs[reg_index]->PT_phase &
                                    PHASE_B) == PHASE_B)) // We have a B
            {
            ...
            if ((indexer == 2) && ((pRegulator_configs[reg_index]->PT_phase &
                                    PHASE_C) == PHASE_C)) // We have a C
            {
            ...
            {
              // Make sure we aren't below the minimum or above the maximum
              // first				  (***** This below here \/ \/
              // *************) - sub with step check! **************
              if (((vmin[indexer] > maximum_voltage[reg_index]) ||
                   (VRegTo[indexer] > maximum_voltage[reg_index])) &&
                  ((LimitExceed & temp_var_d) !=
                   temp_var_d)) // Above max, but won't go below
              {
                // Flag us for a down tap
                prop_tap_changes[indexer] = -1;
              } else if (((vmin[indexer] < minimum_voltage[reg_index]) ||
                          (VRegTo[indexer] < minimum_voltage[reg_index])) &&
                         ((LimitExceed & temp_var_u) !=
                          temp_var_u)) // Below min, but won't exceed
              {
              ...
              } else // Normal operation
              {
                ...
                if (VDrop[indexer] > max_vdrop[reg_index]) // High loading
                {
                  ...
                  if (((VSet[indexer] + vbw_high[reg_index]) <
                       VRegTo[indexer]) &&
                      ((LimitExceed & temp_var_d) !=
                       temp_var_d)) // Above deadband, but can go down
                  {
                    // Check the theoretical change - make sure we won't exceed
                    // any limits
                    if (pRegulator_list[reg_index]->tap[indexer] >
                        0) // Step up region
                    {
                      // Find out what a step down will get us theoretically
                      if ((VRegTo[indexer] - reg_step_up[reg_index]) <
                          minimum_voltage[reg_index]) // We'll fall below
                      {
                    ...
                    {
                      // Find out what a step down will get us theoretically
                      if ((VRegTo[indexer] - reg_step_down[reg_index]) <
                          minimum_voltage[reg_index]) // We'll fall below
                      {
                        prop_tap_changes[indexer] = 0; // No change allowed
                      } else                           // Change allowed
                  ...
                  else if (((VSet[indexer] - vbw_high[reg_index]) >
                            VRegTo[indexer]) &&
                           ((LimitExceed & temp_var_u) !=
                            temp_var_u)) // Below deadband, but can go up
                  {
                    // Check the theoretical change - make sure we won't exceed
                    // any limits
                    if (pRegulator_list[reg_index]->tap[indexer] >
                        0) // Step up region
                    {
                      ...
                      if ((VRegTo[indexer] + reg_step_up[reg_index]) >
                          maximum_voltage[reg_index]) // We'll exceed
                      {
                        prop_tap_changes[indexer] = 0; // No change allowed
                      } else                           // Change allowed
                      {
                    ...
                    } else // Must be lower region
                    {
                      // Find out what a step up will get us theoretically
                      if ((VRegTo[indexer] + reg_step_down[reg_index]) >
                          maximum_voltage[reg_index]) // We'll fall exceed
                      {
                        prop_tap_changes[indexer] = 0; // No change allowed
                      } else                           // Change allowed
                      {
                ...
                {
                  // See if we're outside our range
                  if (((VSet[indexer] + vbw_low[reg_index]) <
                       VRegTo[indexer]) &&
                      ((LimitExceed & temp_var_d) !=
                       temp_var_d)) // Above deadband, but can go down
                  {
                  ...
                  else if (((VSet[indexer] - vbw_low[reg_index]) >
                            VRegTo[indexer]) &&
                           ((LimitExceed & temp_var_u) !=
                            temp_var_u)) // Below deadband, but can go up
                  {
          ...
          // Apply the taps - loop through phases (nonexistant phases should
          // just be 0 Default assume no change will occur
          Regulator_Change = false;
          ...
          for (indexer = 0; indexer < 3; indexer++) {
            if (prop_tap_changes[indexer] > 0) // Want to tap up
            {
              ...
              if (pRegulator_list[reg_index]->tap[indexer] >=
                  pRegulator_configs[reg_index]->raise_taps) {
                // Just leave us railed - explicity done in case it somehow ends
                // up over raise_taps (not sure how that would happen)
                pRegulator_list[reg_index]->tap[indexer] =
                    pRegulator_configs[reg_index]->raise_taps;
              } else // Must have room
              {
                ...
                TRegUpdate[reg_index] =
                    t0 +
                    (TIMESTAMP)RegUpdateTimes[reg_index]; // Set return time
              }
            } // end tap up
            else if (prop_tap_changes[indexer] < 0) // Tap down
            {
              // Make sure we aren't railed
              if (pRegulator_list[reg_index]->tap[indexer] <=
                  -pRegulator_configs[reg_index]->lower_taps) {
                // Just leave us railed - explicity done in case it somehow ends
                // up under lower_taps (not sure how that would happen)
                pRegulator_list[reg_index]->tap[indexer] =
                    -pRegulator_configs[reg_index]->lower_taps;
              } else // Must have room
              {
        ...
        {
          // Banked will take first PT_PHASE it matches.  If there's more than
              ...
              PHASE_A) // We have an A
          {
            ...
            temp_var_d = 0x01; // A base lower "Limit" checker
            temp_var_u = 0x10; // A base upper "Limit" checker
          } else if ((pRegulator_configs[reg_index]->PT_phase & PHASE_B) ==
                     PHASE_B) // We have a B
          {
          ...
          if (((vmin[indexer] > maximum_voltage[reg_index]) ||
               (VRegTo[indexer] > maximum_voltage[reg_index])) &&
              ((LimitExceed & temp_var_d) !=
               temp_var_d)) // Above max, but can go down
          {
            // Flag us for a down tap
            prop_tap_changes[0] = prop_tap_changes[1] = prop_tap_changes[2] =
                -1;
          } else if (((vmin[indexer] < minimum_voltage[reg_index]) ||
                      (VRegTo[indexer] < minimum_voltage[reg_index])) &&
                     ((LimitExceed & temp_var_u) !=
                      temp_var_u)) // Below min, but can go up
          {
            // Flag us for an up tap
          ...
          } else // Normal operation
          {
            ...
            {
              // See if we're outside our range
              if (((VSet[indexer] + vbw_high[reg_index]) < VRegTo[indexer]) &&
                  ((LimitExceed & temp_var_d) !=
                   temp_var_d)) // Above deadband, but can go down too
              {
                // Check the theoretical change - make sure we won't exceed any
                // limits
                if (pRegulator_list[reg_index]->tap[indexer] >
                    0) // Step up region
                {
                  ...
                  if ((VRegTo[indexer] - reg_step_up[reg_index]) <
                      minimum_voltage[reg_index]) // We'll fall below
                  {
                    prop_tap_changes[0] = prop_tap_changes[1] =
                        prop_tap_changes[2] = 0; // No change allowed
                  } else                         // Change allowed
                  {
                    prop_tap_changes[0] = prop_tap_changes[1] =
                        prop_tap_changes[2] = -1; // Try to tap us down
                  }
                } else // Must be lower region
                {
                  // Find out what a step down will get us theoretically
                        ...
                        temp_var_u)) // Below deadband, but can go up
              {
                    ...
                    0) // Step up region
                {
                  ...
                  {
                    prop_tap_changes[0] = prop_tap_changes[1] =
                        prop_tap_changes[2] = 1; // Try to tap us up
                  }
                } else // Must be lower region
                {
                  // Find out what a step up will get us theoretically
            ...
            {
              // See if we're outside our range
              if (((VSet[indexer] + vbw_low[reg_index]) < VRegTo[indexer]) &&
                  ((LimitExceed & temp_var_d) !=
                   temp_var_d)) // Above deadband, but can go down
              {
                // Check the theoretical change - make sure we won't exceed any
                ...
                } else // Must be lower region
                {
                  ...
                  } else                         // Change allowed
                  {
                    prop_tap_changes[0] = prop_tap_changes[1] =
                        ...
                        VRegTo[indexer]) &&
                       ((LimitExceed & temp_var_d) !=
                        temp_var_d)) // Below deadband, but can go up
              {
                // Check the theoretical change - make sure we won't exceed any
                // limits
                if (pRegulator_list[reg_index]->tap[indexer] >
          ...
          // Check on the assumption of differential banked (offsets can be
          // present, just all move simultaneously) We'll only check prop->A,
          // since it is banked
          Regulator_Change = false; // Start with no change assumption
          TRegUpdate[reg_index] = TS_NEVER;
          if (prop_tap_changes[0] > 0) // Want a tap up
          {
            // Check individually - set to rail if they are at or exceed - this
            // may lose the offset, but I don't know how they'd ever exceed a
            // limit anyways
            if (pRegulator_list[reg_index]->tap[0] >=
                pRegulator_configs[reg_index]->raise_taps) {
              pRegulator_list[reg_index]->tap[0] =
                  pRegulator_configs[reg_index]->raise_taps; // Set at limit
              limit_hit = true; // Flag that a limit was hit
            }

            if (pRegulator_list[reg_index]->tap[1] >=
                pRegulator_configs[reg_index]->raise_taps) {
            ...
            if (pRegulator_list[reg_index]->tap[2] >=
                pRegulator_configs[reg_index]->raise_taps) {
            ...
            if (!limit_hit) // We can still proceed
            {
              ...
              TRegUpdate[reg_index] =
                  t0 + (TIMESTAMP)RegUpdateTimes[reg_index]; // Set return time
            }
          ...
          else if (prop_tap_changes[0] < 0) // Want a tap down
          {
            ...
            if (pRegulator_list[reg_index]->tap[0] <=
                -pRegulator_configs[reg_index]->lower_taps) {
              pRegulator_list[reg_index]->tap[0] =
                  -pRegulator_configs[reg_index]->lower_taps; // Set at limit
              limit_hit = true; // Flag that a limit was hit
            }
  ...
  {
    if (treg_min ==
        TS_NEVER) // Final check to make sure we don't return "-infinity"
      return TS_NEVER;
    else
      return -treg_min;
  }
...
TIMESTAMP volt_var_control::postsync(TIMESTAMP t0) {
  ...
  if ((control_method == ACTIVE) &&
      !Regulator_Change) // no regulator changes in progress and we're active
  {
    ...
    if (pf_signed) {
      curr_pf_temp = fabs(link_power_vals.Re()) /
                     link_power_vals.Mag(); // Pull in power factor
    ...
    {
      curr_pf = fabs(link_power_vals.Re()) /
                link_power_vals.Mag(); // Pull in power factor
    }

    // Update "proceeding" variable
    if (((solver_method == SM_NR) && first_cycle) ||
        ((solver_method == SM_FBS) && !first_cycle))
      allow_change = true; // Intermediate assignment since FBS likes to mess up
                           // power calculations on the first cycle
    else
      allow_change = false;
    ...
    if (pf_signed) // Consider "signing" on the power factor
    {
      ...
      des_react_pwr_temp =
          fabs(link_power_vals.Re() * sqrt(1 / (desired_pf * desired_pf) - 1));
    ...
    // Now check to see if this is a first pass or not - always "attempt" a
    // change for positive/negative pf - just may be no candidates
    if (pf_check && allow_change && (TCapUpdate <= t0)) {
      change_requested = false; // Start out assuming no change

      // Parse through the capacitor list - see where they sit in the categories
      // - break after one switching operation
      for (index = 0; index < num_caps; index++) {
        // Find the phases being watched, check their switch
        if ((pCapacitor_list[index]->pt_phase & PHASE_A) == PHASE_A)
          bank_status =
              (capacitor::CAPSWITCH)pCapacitor_list[index]->switchA_state;
        else if ((pCapacitor_list[index]->pt_phase & PHASE_B) == PHASE_B)
          bank_status =
              (capacitor::CAPSWITCH)pCapacitor_list[index]->switchB_state;
        else // Must be C, right?
          bank_status =
              (capacitor::CAPSWITCH)pCapacitor_list[index]->switchC_state;

        if (pf_signed) // Consider the sign in switching operations
        {
          // Now perform logic based on where it is
          if ((bank_status == capacitor::CLOSED) &&
              !pf_add_capacitor) // We are on and need to remove someone
          {
          ...
          else if ((bank_status == capacitor::OPEN) &&
                   pf_add_capacitor) // We're off and want to turn someone on
          {
      ...
      // If we are outside our pf range and nothing went off, see if a smaller
      // capacitor can be "prompted" to get us back in range
      if (!change_requested && pf_signed) {
        // See if we're needing a capacitor action to get back in range, or just
        // looking for more adjustments
        if (((curr_pf > 0) && (curr_pf < -desired_pf)) ||
            ((curr_pf < 0) && (curr_pf < desired_pf))) {
          // Parse through the capacitor list - go backwards (smallest first) -
          // break after one operation (if any)
          for (index = (num_caps - 1); index >= 0; index--) {
            ...
            // Now perform logic based on where it is - if anything is found to
            // "fit" the criterion, just enact it
            if ((bank_status == capacitor::CLOSED) &&
                !pf_add_capacitor) // We are on and need to remove someone
            {
              pCapacitor_list[index]->toggle_bank_status(false); // Turn all off
      ...
      if (change_requested) // Something changed
      {
        TCapUpdate =
            t0 +
            (TIMESTAMP)CapUpdateTimes[index]; // Figure out where we want to go

        return t0; // But then stay here, mainly so the capacitor change we just
                   // enacted goes through
...
// Function to parse a comma-separated list to get the next double (or the last
// double)
...
// Function to parse a comma-separated list to get the next object (or the last
// object)
char *volt_var_control::obj_token(char *start_token, OBJECT **obj_val) {
  char workArray[64]; // Hopefully, names will never be over 64 characters -
                      // seems to get upset if we do more
  char *outIndex, *workIndex, *end_token;
  char index;
...
// Function to sort capacitors based on size - assumes they are banked in
// operation (only 1 size provided) Utilizes merge sorting algorithm - sorts
// largest to smallest
void volt_var_control::size_sorter(double *cap_size, int *cap_Index,
                                   int cap_num, double *temp_cap_size,
                                   int *temp_cap_Index) {
  int split_point;
  int right_length;
  ...
  if (num_caps > 0) // Only occurs if over zero
  {
    ...
    right_length =
        cap_num - split_point; // Figure out how big the right hand side is
    ...
    if (split_point > 1)
      size_sorter(leftside, leftsideidx, split_point, temp_cap_size,
                  temp_cap_Index);

    if (right_length > 1)
      size_sorter(rightside, rightsideidx, right_length, temp_cap_size,
                  temp_cap_Index);
    ...
    } while ((leftside < (cap_size + split_point)) &&
             (rightside <
              (cap_size + cap_num))); // Sort the list until one of them empties

    while (leftside <
           (cap_size + split_point)) // Put any remaining entries into the list
    {
      *Final_Val++ = *leftside++;
      *Final_Idx++ = *leftsideidx++;
    }
    ...
    while (rightside <
           (cap_size + cap_num)) // Put any remaining entries into the list
    {
      *Final_Val++ = *rightside++;
      *Final_Idx++ = *rightsideidx++;
    }

    memcpy(cap_size, temp_cap_size,
           sizeof(double) *
               cap_num); // Copy the size result back into the input
    memcpy(cap_Index, temp_cap_Index,
           sizeof(int) * cap_num); // Copy the index result back into the input
 ...
 */
static TIMESTAMP sync_volt_var_control_impl(OBJECT *obj, TIMESTAMP t0,
                                            PASSCONFIG pass) {
  try {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_volt_var_control(OBJECT *obj, TIMESTAMP t0,
                                                      PASSCONFIG pass) {
...
extern "C" MODULE_API int isa_volt_var_control(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_volt_var_control_impl(obj, classsname);
}
#endif
