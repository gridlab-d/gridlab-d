...
line_spacing::line_spacing(MODULE *mod) : powerflow_library(mod) {
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            oclass, PT_double, "distance_AB[ft]", PADDR(distance_AtoB),
            PT_double, "distance_BC[ft]", PADDR(distance_BtoC), PT_double,
            "distance_AC[ft]", PADDR(distance_AtoC), PT_double,
            "distance_AN[ft]", PADDR(distance_AtoN), PT_double,
            "distance_BN[ft]", PADDR(distance_BtoN), PT_double,
            "distance_CN[ft]", PADDR(distance_CtoN), PT_double,
            "distance_AE[ft]", PADDR(distance_AtoE), PT_DESCRIPTION,
            "distance between phase A wire and earth", PT_double,
            "distance_BE[ft]", PADDR(distance_BtoE), PT_DESCRIPTION,
            "distance between phase B wire and earth", PT_double,
            "distance_CE[ft]", PADDR(distance_CtoE), PT_DESCRIPTION,
            "distance between phase C wire and earth", PT_double,
            "distance_NE[ft]", PADDR(distance_NtoE), PT_DESCRIPTION,
            "distance between neutral wire and earth", nullptr) < 1)
      GL_THROW("unable to publish line_spacing properties in %s", __FILE__);
  }
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_line_spacing(OBJECT *obj, TIMESTAMP t1,
                                                  PASSCONFIG pass) {
  return sync_line_spacing_impl(obj, t1, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_line_spacing(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
