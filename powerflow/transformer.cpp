/** $Id: transformer.cpp 1211 2009-01-17 00:45:29Z d3x593 $
        ...
        The transformer is one of the more complex objects in the powerflow
network. Implemented as a link, the transformer configuration exports a type
property that allows the transformer to operate as and single phase transformer,
a wye-wye connected transformer, a delta-grounded wye tranformer, a delta-delta
        transformer, and as a center-tapped transformer.

        The transformer exports a phase property that is a set of phases and may
be set using the bitwise or operator (A|B|C for a 3 phase line).
 @{
**/
...
transformer::transformer(MODULE *mod) : link_object(mod) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "transformer", sizeof(transformer),
    ...
    if (gl_publish_variable(
            oclass, PT_INHERIT, "link", PT_object, "configuration",
            PADDR(configuration), PT_DESCRIPTION,
            "Configuration library used for transformer setup", PT_object,
            "climate", PADDR(climate), PT_DESCRIPTION,
            "climate object used to describe thermal model ambient temperature",
            PT_double, "ambient_temperature[degC]", PADDR(amb_temp),
            PT_DESCRIPTION, "ambient temperature in degrees C", PT_double,
            "top_oil_hot_spot_temperature[degC]", PADDR(theta_TO),
            ...
            PT_double, "percent_loss_of_life", PADDR(life_loss), PT_DESCRIPTION,
            "the percent loss of life", PT_double, "aging_constant",
            PADDR(B_age), PT_DESCRIPTION,
            "the aging rate slope for the transformer insulation", PT_bool,
            "use_thermal_model", PADDR(use_thermal_model), PT_DESCRIPTION,
            "boolean to enable use of thermal model", PT_double,
            "transformer_replacement_count", PADDR(transformer_replacements),
            PT_DESCRIPTION,
            "counter of the number times the transformer has been replaced due "
            "to lifetime failure",
            PT_double, "aging_granularity[s]", PADDR(aging_step),
            PT_DESCRIPTION,
            "maximum timestep before updating thermal and aging model in "
            "seconds",
            //************** TODO -- Figure out if this makes sense to publish
            //it like this *************************/
            PT_double, "phase_A_primary_flux_value[Wb]",
            PADDR(flux_vals_inst[0]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase A on the primary side of the "
            "transformer during saturation calculations",
            PT_double, "phase_B_primary_flux_value[Wb]",
            PADDR(flux_vals_inst[1]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase B on the primary side of the "
            "transformer during saturation calculations",
            PT_double, "phase_C_primary_flux_value[Wb]",
            PADDR(flux_vals_inst[2]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase C on the primary side of the "
            "transformer during saturation calculations",
            PT_double, "phase_A_secondary_flux_value[Wb]",
            PADDR(flux_vals_inst[3]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase A on the secondary side of "
            "the transformer during saturation calculations",
            PT_double, "phase_B_secondary_flux_value[Wb]",
            PADDR(flux_vals_inst[4]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase B on the secondary side of "
            "the transformer during saturation calculations",
            PT_double, "phase_C_secondary_flux_value[Wb]",
            PADDR(flux_vals_inst[5]), PT_DESCRIPTION,
            "instantaneous magnetic flux in phase C on the secondary side of "
            "the transformer during saturation calculations",
            nullptr) < 1)
      GL_THROW("unable to publish properties in %s", __FILE__);

    if (gl_publish_function(oclass, "power_calculation",
                            (FUNCTIONADDR)power_calculation) == nullptr)
    ...
    if (gl_publish_function(oclass, "recalc_transformer_matrices",
                            (FUNCTIONADDR)recalc_transformer_mat) == nullptr)
      GL_THROW("Unable to publish transformer in-rush update function");
    if (gl_publish_function(oclass, "recalc_deltamode_saturation",
                            (FUNCTIONADDR)recalc_deltamode_saturation) ==
        nullptr)
      GL_THROW(
          "Unable to publish transformer in-rush powerflow update function");

    // Publish restoration-related function (current update)
    if (gl_publish_function(oclass, "update_power_pwr_object",
                            (FUNCTIONADDR)updatepowercalc_link) == nullptr)
      GL_THROW(
          "Unable to publish transformer external power calculation function");
    if (gl_publish_function(oclass, "check_limits_pwr_object",
                            (FUNCTIONADDR)calculate_overlimit_link) == nullptr)
      GL_THROW("Unable to publish transformer external power limit calculation "
               "function");
    if (gl_publish_function(oclass, "perform_current_calculation_pwr_link",
                            (FUNCTIONADDR)currentcalculation_link) == nullptr)
      GL_THROW("Unable to publish transformer external current calculation "
               "function");
  }
}
...
void transformer::fetch_double(double **prop, const char *name,
                               OBJECT *parent) {
...
int transformer::init(OBJECT *parent) {
  ...
  /*  TROUBLESHOOT
  A transformer configuration was not provided.  Please use object
  transformer_configuration and define the necessary parameters of your
  ...
  */
  ...
  /*  TROUBLESHOOT
  An invalid transformer configuration was provided.  Ensure you have proper
  values in each field of the transformer_configuration object and that you
  haven't inadvertantly used a line configuration as the transformer
  configuration.
  */
  if ((configuration->flags & OF_INIT) != OF_INIT) {
    char objname[256];
    gl_verbose("transformer::init(): deferring initialization on %s",
               gl_name(configuration, objname, 255));
    return 2; // defer
  }
  ...
  else if (config->connect_type ==
           3) // Flag Delta-Gwye and Split-phase for phase checks
    SpecialLnk = DELTAGWYE;
  else if (config->connect_type ==
           5) // Flag Delta-Gwye and Split-phase for phase checks
    SpecialLnk = SPLITPHASE;
  else // Wye-wye or single phase, but flag anyways
    SpecialLnk = WYEWYE;

  // Populate limits - emergency and continuous are the same - moved above the
  // init so the internal check works
  ...
  if (!tphases_ref->is_set() || !tphases_ref->is_valid()) {
    GL_THROW("Transformer:%s failed to map the phases of the \"to\" node",
             obj->name ? obj->name : "unnamed");
    /*  TROUBLESHOOT
    While attempting to map the phases property of the "to" node of the
    transformer, an error occurred.  Ensure that object actually is a powerflow
    node and try again.  If the error persists, please submit an issue in issue
    tracker.
    */
  }
  ...
  zc = gld::complex(V_base * V_base, 0) / (config->kVA_rating * 1000.0) *
       gld::complex(config->shunt_impedance.Re(), 0) *
       gld::complex(0, config->shunt_impedance.Im()) /
       gld::complex(config->shunt_impedance.Re(), config->shunt_impedance.Im());

  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      a_mat[i][j] = b_mat[i][j] = c_mat[i][j] = d_mat[i][j] = A_mat[i][j] =
          B_mat[i][j] = gld::complex(0.0, 0.0);
  ...
  case transformer_configuration::SINGLE_PHASE:
    ...
    if (has_phase(PHASE_S) || ((tphases & PHASE_S) == PHASE_S)) {
      GL_THROW("Transformer:%s has a triplex phase or a triplex \"to\" node, "
               "but it is not an SPCT transformer",
               obj->name ? obj->name : "unnamed");
      /*  TROUBLESHOOT
      A transformer has triplex characteristics (S phase or a triplex_node
      attached), but it is not configured as an SPCT transformer.  Please check
      your transformer configuration or attached node.
      */
    }
    ...
    if (solver_method == SM_FBS) {
      if (has_phase(PHASE_A)) {
        ...
        d_mat[0][0] =
            1 / nt; //(zc + zt_a) / (gld::complex(nt_a,0) * zc);
                    // A_mat[0][0] = (zc - zt_a) / ( gld::complex(nt_a,0) * (zc
                    // + zt_a)); a_mat[0][0] = gld::complex(1,0) / A_mat[0][0];
      ...
      if (has_phase(PHASE_B)) {
        ...
        d_mat[1][1] =
            1 / nt; //(zc + zt_b) / (gld::complex(nt_b,0) * zc);
                    // A_mat[1][1] = (zc - zt_b) / ( gld::complex(nt_b,0) * (zc
                    // + zt_b)); a_mat[1][1] = gld::complex(1,0) / A_mat[1][1];
      ...
      if (has_phase(PHASE_C)) {
        ...
        d_mat[2][2] =
            1 / nt; //(zc + zt_c) / (gld::complex(nt_c,0) * zc);
                    // A_mat[2][2] = (zc - zt_c) / ( gld::complex(nt_c,0) * (zc
                    // + zt_c)); a_mat[2][2] = gld::complex(1,0) / A_mat[2][2];
      ...
      if (enable_inrush_calculations) {
        //"Main" allocation is done in link.cpp -- this depends on where we want
        //windings, so done here "Main" allocation already done, via link::init
        //above
        ...
        if (YBase_Full == nullptr) {
          GL_THROW("Transformer:%s failed to allocate space for deltamode "
                   "inrush history term",
                   obj->name ? obj->name : "unnamed");
          /*  TROUBLESHOOT
          While attempting to allocate memory for a transformer object to
        ...
        if ((config->magnetization_location == config->PRI_MAG) ||
            (config->magnetization_location ==
             config->BOTH_MAG)) // Primary needed (from)
        {
        ...
        if ((config->magnetization_location == config->SEC_MAG) ||
            (config->magnetization_location ==
             config->BOTH_MAG)) // Secondary needed (to)
        {
      ...
      if (has_phase(PHASE_A)) {
        ...
        base_admittance_mat[0][0] =
            gld::complex(1, 0) / zt; //(zc - zt) / ((zc + zt) * zt);
        b_mat[0][0] = zt;
      }
      if (has_phase(PHASE_B)) {
        // base_admittance_mat[1][1] = (zc - zt) / ((zc + zt) * zt);
        base_admittance_mat[1][1] =
            gld::complex(1, 0) / zt; //(zc - zt) / ((zc + zt) * zt);
        b_mat[1][1] = zt;
      }
      if (has_phase(PHASE_C)) {
        // base_admittance_mat[2][2] = (zc - zt) / ((zc + zt) * zt);
        base_admittance_mat[2][2] =
            gld::complex(1, 0) / zt; //(zc - zt) / ((zc + zt) * zt);
        b_mat[2][2] = zt;
      }

      // Other matrices
      A_mat[0][1] = A_mat[0][2] = A_mat[1][0] = A_mat[1][2] = A_mat[2][0] =
          A_mat[2][1] = 0.0;
      a_mat[0][1] = a_mat[0][2] = a_mat[1][0] = a_mat[1][2] = a_mat[2][0] =
          a_mat[2][1] = 0.0;
      c_mat[0][1] = c_mat[0][2] = c_mat[1][0] = c_mat[1][2] = c_mat[2][0] =
          c_mat[2][1] = 0.0;
      ...
      (forward-back sweep), GS (Gauss-Seidel), and NR (Newton-Raphson).  Please
      use one of these methods and consider submitting a bug report for the
      solver type you tried.
      */
    }
  ...
  case transformer_configuration::DELTA_DELTA:
    ...
    if (solver_method == SM_FBS) {
      ...
      A_mat[0][1] = A_mat[0][2] = A_mat[1][0] = A_mat[1][2] = A_mat[2][0] =
          A_mat[2][1] = gld::complex(-1.0) / (nt * 3.0);
      ...
      base_admittance_mat[0][0] = base_admittance_mat[1][1] =
          base_admittance_mat[2][2] = Izt;
  ...
  case transformer_configuration::DELTA_GWYE:
    ...
    if (solver_method == SM_FBS) {
      ...
      if (voltage_ratio > 1.0) // Step down
      {
        ...
        base_admittance_mat[0][0] = base_admittance_mat[1][1] =
            base_admittance_mat[2][2] = Izt;
        base_admittance_mat[0][1] = base_admittance_mat[0][2] =
            base_admittance_mat[1][0] = 0.0;
        base_admittance_mat[1][2] = base_admittance_mat[2][0] =
            base_admittance_mat[2][1] = 0.0;
  ...
  case transformer_configuration::SINGLE_PHASE_CENTER_TAPPED:
    if (solver_method == SM_FBS) {
      ...
      } else if (has_phase(PHASE_A)) // wye-A
      {
        ...
        if (sa_base == 0)
          GL_THROW("Split-phase tranformer:%d trying to attach to phase A not "
                   "defined in the configuration",
                   obj->id);
        /*  TROUBLESHOOT
        A single-phase, center-tapped transformer is attempting to attach to a
        system with phase A, while its phase A is undefined.  Fix the
        appropriate link or define a new transformer configuration with
        ...
        */
        ...
        if (config->impedance1.Re() == 0.0 && config->impedance1.Im() == 0.0) {
          z0 = gld::complex(0.5 * config->impedance.Re(),
                            0.8 * config->impedance.Im()) *
               gld::complex(za_basehi, 0);
          z1 = gld::complex(config->impedance.Re(),
                            0.4 * config->impedance.Im()) *
               gld::complex(za_baselo, 0);
          z2 = gld::complex(config->impedance.Re(),
                            0.4 * config->impedance.Im()) *
               gld::complex(za_baselo, 0);
        } else {
          z0 = gld::complex(config->impedance.Re(), config->impedance.Im()) *
               gld::complex(za_basehi, 0);
          z1 = gld::complex(config->impedance1.Re(), config->impedance1.Im()) *
               gld::complex(za_baselo, 0);
          z2 = gld::complex(config->impedance2.Re(), config->impedance2.Im()) *
               gld::complex(za_baselo, 0);
        }

        zc = gld::complex(za_basehi, 0) *
             gld::complex(config->shunt_impedance.Re(), 0) *
             gld::complex(0, config->shunt_impedance.Im()) /
             gld::complex(config->shunt_impedance.Re(),
                          config->shunt_impedance.Im());
        zt_b = gld::complex(0, 0);
        zt_c = gld::complex(0, 0);
      ...
      else if (has_phase(PHASE_B)) // wye-B
      {
        ...
        if (sa_base == 0)
          GL_THROW("Split-phase tranformer:%d trying to attach to phase B not "
                   "defined in the configuration",
                   obj->id);
        /*  TROUBLESHOOT
        A single-phase, center-tapped transformer is attempting to attach to a
        system with phase B, while its phase B is undefined.  Fix the
        appropriate link or define a new transformer configuration with
      ...
      } else if (has_phase(PHASE_C)) // wye-C
      {
        ...
        if (sa_base == 0)
          GL_THROW("Split-phase tranformer:%d trying to attach to phase C not "
                   "defined in the configuration",
                   obj->id);
        /*  TROUBLESHOOT
        A single-phase, center-tapped transformer is attempting to attach to a
        system with phase C, while its phase C is undefined.  Fix the
        appropriate link or define a new transformer configuration with
      ...
      B_mat[1][1] =
          gld::complex(-1, 0) * ((z2) + (z0 * zc / ((zc + z0) * nt * nt)));
      B_mat[1][2] = gld::complex(0, 0);
      B_mat[2][0] = gld::complex(0, 0);
      B_mat[2][1] = gld::complex(0, 0);
      B_mat[2][2] = gld::complex(0, 0);
    } else if (solver_method == SM_GS) // This doesn't work yet
    {
      ...
      /*  TROUBLESHOOT
      At this time, the Gauss-Seidel method does not support split-phase
      transformers. This will hopefully be a feature in future releases.
      */
    } else if (solver_method == SM_NR) {
      V_basehi = config->V_primary;
      ...
      if (config->impedance1.Re() == 0.0 && config->impedance1.Im() == 0.0) {
        z0 = gld::complex(0.5 * config->impedance.Re(),
             ...
             gld::complex(za_basehi, 0);
        z1 =
            gld::complex(config->impedance.Re(), 0.4 * config->impedance.Im()) *
            gld::complex(za_baselo, 0);
        z2 =
            gld::complex(config->impedance.Re(), 0.4 * config->impedance.Im()) *
            gld::complex(za_baselo, 0);
      ...
      gld::complex indet;
      indet =
          gld::complex(1.0) / (z1 * z2 * zc * nt * nt +
                               z0 * (z2 * zc + z1 * zc + z1 * z2 * nt * nt));

      // Store all information into base_admittance_mat (pull it out later) -
      // phases handled in link
      //  Yto_00	Yto_01   To_Y00
      //  Yto_10	Yto_11	 To_Y10
      //  From_Y00	From_Y01 YFrom_00
      //
      //  Translates into			base_admittance_mat*[V1; V2;
      //  VSource] = [I1; I2; ISource]
      //       V1 V2 Vsource
      //  I1
      //  I2
      //  ISource
      ...
      // Yfrom
      base_admittance_mat[2][2] =
          (z2 * zc + z1 * zc + z1 * z2 * nt * nt) * indet; // z1+z2;
  ...
  default:
    ...
    /*  TROUBLESHOOT
    An unknown transformer configuration was provided.  Please ensure you are
    using only the defined types of transformer.  Refer to the documentation of
    use the command flag
    --modhelp powerflow:transformer_configuration to see valid transformer
    types.
    */
  }
  ...
  if (use_thermal_model && config->coolant_type != 1) {
    GL_THROW("transformer:%d (%s) coolant_type specified is not handled",
             obj->id, obj->name);
    /*  TROUBLESHOOT
    When using the thermal aging model, the coolant type must be specifed.
    Please select a supported coolant type and add it to your transformer
    configuration.  Currently, only MINERAL_OIL is supported.
    */
  }
  if (use_thermal_model) {
    if (config->coolant_type == 1) {
      if (config->core_coil_weight <= 0) {
        GL_THROW("weight of the core and coil assembly for transformer "
                 "configuration %s must be greater than zero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the core weight must be specified.
        Please specify core_coil_weight in your transformer configuration.
        */
      }
      if (config->tank_fittings_weight <= 0) {
        GL_THROW("weight of the tank fittings for transformer configuration %s "
                 "must be greater than zero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the core weight must be specified.
        Please specify tank_fittings_weight in your transformer configuration.
        */
      }
      if (config->oil_vol <= 0) {
        GL_THROW("the oil volume for transformer configuration %s must be "
                 "greater than zero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the oil volume must be specified as
        a value of zero or greater. Please specify oil_volume in your
        transformer configuration.
        */
      }

      if (config->cooling_type == 1 || config->cooling_type == 2) {
        thermal_capacity = (0.06 * config->core_coil_weight) +
                           (0.04 * config->tank_fittings_weight) +
                           (1.33 * config->oil_vol);
        if (config->cooling_type == 1) {
          ...
          GL_THROW(
              "cooling_type not specified for transformer configuration %s",
              configuration->name);
          /*  TROUBLESHOOT
          When using the thermal aging model, the a cooling_type must be
          specified. Please specify cooling_type in your transformer
          configuration.
          */
        }
      } else if (config->cooling_type > 2) {
        thermal_capacity = (0.06 * config->core_coil_weight) +
                           (0.06 * config->tank_fittings_weight) +
                           (1.93 * config->oil_vol);
        ...
        GL_THROW("cooling_type not specified for transformer configuration %s",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the a cooling_type must be
        specified. Please specify cooling_type in your transformer
      ...
      if (config->full_load_loss == 0 && config->no_load_loss == 0 &&
          config->impedance.Re() == 0 && config->shunt_impedance.Re() == 0) {
        GL_THROW("full-load and no-load losses for transformer configuration "
                 "%s must be nonzero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, full_load_losses and no_load_losses
        must be specified.
        */
      } else if (config->full_load_loss != 0 && config->no_load_loss != 0) {
        R = config->full_load_loss / config->no_load_loss;
      } else if (config->impedance.Re() != 0 &&
                 config->shunt_impedance.Re() != 0)
        R = config->impedance.Re() * config->shunt_impedance.Re();
      if (config->t_W == 0 || config->dtheta_TO_R == 0.0) {
        GL_THROW("winding time constant or rated top-oil hotspot rise for "
                 "transformer configuration %s must be nonzero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the rated_winding_time_constant or
        the rated_top_oil_rise must be given as a non-zero value. Please specify
        one or the other in your transformer configuration.
        */
      }
      if (config->t_W <= 0) {
        GL_THROW("%s: transformer_configuration winding time constant must be "
                 "greater than zero",
                 configuration->name);
        /*  TROUBLESHOOT
        When using the thermal aging model, the rated_winding_time_constant must
        be given as a greater-than-zero value. Please specify as a positive
        value in your transformer configuration.
        */
      }
      ...
      if (climate == nullptr) {
        ...
        climate_list =
            gl_find_objects(FL_NEW, FT_CLASS, SAME, "climate", FT_END);

        if (climate_list == nullptr) {
          // Warn
          gl_warning("No climate data found - using static temperature");
          /*  TROUBLESHOOT
          While attempting to map a climate object for the transformer aging
          model, no climate object could be found.  Static temperature data will
          be used instead.  Please check your code or manually specify a climate
          ...
          */
          ...
          if (climate == nullptr) {
            ...
            /*  TROUBLESHOOT
            While attempting to map a climate object for the transformer aging
            model, no climate object could be found.  Static temperature data
            will be used instead.  Please check your code or manually specify a
            climate object if this is not desired.
            */

            // Nope, just use default static
            ptheta_A =
                &default_outdoor_temperature; // default static temperature
          } else                              // Found one, map the property
          {
            ...
            if (ptheta_A == nullptr) {
              ...
              While attempting to map a climate object for the transformer aging
              model, no climate object could be found.  Static temperature data
              will be used instead.  Please check your code or manually specify
              a climate object if this is not desired.
              */
      ...
      if (config->dtheta_H_AR == 0)
        config->dtheta_H_AR =
            80; // default for an average rise of 65 degrees C is 80 degrees C

      return_at = gl_globalclock;
      if (aging_step < 1)
        aging_step = 300; // default granularity is 5 minutes.
      t_TOR = thermal_capacity * (config->dtheta_TO_R) /
              ((config->full_load_loss) * (config->kVA_rating) * 1000);
...
TIMESTAMP transformer::postsync(TIMESTAMP t0) {
  ...
  if (use_thermal_model) {
    ...
    if ((t0 >= simulation_start_time) && (t0 != time_before)) {
      if (time_before != 0) {
        dt = (double)((t0 - time_before) * TS_SECOND) /
             3600; // calculate the change in time in hours

        // calculate the loss of life
        if (config->installed_insulation_life <= 0) {
          gl_error("%s: transformer configuration installed insulation life "
                   "must be greater than zero",
                   configuration->name);
          /*  TROUBLESHOOT
          During the simulation, installed_insulation_life was found to be zero
          or negative.  Either this was specified incorrectly in the model -
          check to see that installed_insulation_life was set as a positive
          value in your configuration file - or a major error occurred in the
          model.  If the latter, please post on the GridLAB-D forums.
          */
          return TS_INVALID;
        }
        life_loss += F_AA * dt * 100 / config->installed_insulation_life;
        if (life_loss < 100) {
          // calculate the top-oil hot spot temperature using previous
          // parameters from the previous timestep
          dtheta_TO =
              (dtheta_TO_U - dtheta_TO_i) * (1 - exp(-dt / t_TO)) + dtheta_TO_i;
          theta_TO = last_temp + dtheta_TO;

          // calculate the winding hot spot temperature
          dtheta_H = (dtheta_H_U - dtheta_H_i) * (1 - exp(-dt / config->t_W)) +
                     dtheta_H_i;
          theta_H = last_temp + dtheta_TO + dtheta_H;

          // calculate the power ratio for the current timestep, temperature
          // asymptotes, and time constants.
          ...
          dtheta_TO_U =
              (config->dtheta_TO_R) * pow(((K * K * R + 1) / (R + 1)), n);
          t_TO = t_TOR * (((dtheta_TO_U / config->dtheta_TO_R) -
                           (dtheta_TO_i / config->dtheta_TO_R)) /
                          (pow((dtheta_TO_U / config->dtheta_TO_R), (1 / n)) -
                           pow((dtheta_TO_i / config->dtheta_TO_R), (1 / n))));
          dtheta_H_U = dtheta_H_R * pow(K, 2 * m);
        } else if (life_loss >= 100) {
          gl_warning("%s: The transformer has reached its operational "
                     "lifespan. Resetting transformer lifetime parameters.",
                     obj->name);
          /*  TROUBLESHOOT
          During the simulation, while using the transformer thermal aging
          model, the transformer reached the end of its life.  It was assumed
          that the transformer was replaced with a new transformer of the same
          make, model, etc. to prevent failure of the simulation.  All of the
          lifetime parameters are set to assume a "new" transformer.
          */
          life_loss = 0;
          dtheta_TO_i = 0;
          dtheta_H_i = 0;
    ...
    // figure out how much time is left before the transformer lifespan is
    // reached
    F_AA = exp((B_age / 383.14) - (B_age / (theta_H + 273.14)));
    time_left = (config->installed_insulation_life) * (100 - life_loss) /
                (100 * F_AA) * 3600;
    if (time_left < 1) {
      time_left = 1;
    }
    ...
    if (t0 >= return_at) {
      ...
      if (return_at < t0) {
        gl_error("%s: transformer granularity is too small for the minimum "
                 "timestep specified",
                 obj->name);
        /*  TROUBLESHOOT
        During the simulation, the time step that occured was greater than what
        the aging_granularity was set to. The usual cause is becasue the
        aging_granularity wasn't set or set to zero. If setting the
        aging_granularity parameter to something greater than 0 doesn't get rid
        of this error please post on the GridLAB-D forums.
        */
        return TS_INVALID;
      }
      if (result < return_at) {
...
int transformer::transformer_inrush_mat_update(void) {
  ...
  gld::complex Zprim[8][8], ZMprim[8][8], zp_trafo[8][8], zhp_trafo[8][8],
      zmp_trafo[8][8], zmhp_trafo[8][8];
  gld::complex zh_trafo[8][8], zmh_trafo[8][8];
  gld::complex temp_mat[8][8], yp_trafo[8][8], y_trafo[8][8], ym_trafo[8][8],
      temp_store_mat[8][8];
  ...
  Rd = config->impedance.Re() * config->V_secondary * config->V_secondary /
       (config->kVA_rating * 1000.0);
  Xd = config->impedance.Im() * config->V_secondary * config->V_secondary /
       (config->kVA_rating * 1000.0 *
        2.0); // Where'd this /2 come from?  Fixed in later version!?!
  XM = config->V_secondary * config->V_secondary /
           (config->kVA_rating * 1000.0 * config->IM_pu) -
       Xd;

  //******************** DEBUG NOTE - these may need to be moved, depending on
  //where I populate things
  // Compute saturation constants
  A_phi = gld::complex((deltatimestep_running * 2.0 * config->TD_val), 0.0) /
          gld::complex((4.0 * config->TD_val + deltatimestep_running),
                       (4.0 * PI * nominal_frequency * config->TD_val *
                        deltatimestep_running));

  // Form B coefficient -- numerator
  B_phi = gld::complex(
      (4.0 * config->TD_val - deltatimestep_running),
      (-4.0 * PI * nominal_frequency * config->TD_val * deltatimestep_running));

  // Denominator
  work_val_cplex = gld::complex((4.0 * config->TD_val + deltatimestep_running),
                                4.0 * PI * nominal_frequency * config->TD_val *
                                    deltatimestep_running);
  ...
  C_sat = config->IM_pu *
          (config->LA_pu * config->IM_pu - config->phiM_pu + config->phiK_pu);
  D_sat = (-1.0 * B_sat - sqrt((B_sat * B_sat - 4.0 * A_sat * C_sat))) /
          (2.0 * A_sat);
  ...
  if (config->model_inrush_saturation) {
    // Calculate values
    phi_base_Pri =
        config->V_primary / (sqrt(3.0) * 2.0 * PI * nominal_frequency);
    I_base_Pri =
        (config->kVA_rating * 1000.0) / (sqrt(3.0) * config->V_primary);

    phi_base_Sec =
        config->V_secondary / (sqrt(3.0) * 2.0 * PI * nominal_frequency);
    I_base_Sec =
        (config->kVA_rating * 1000.0) / (sqrt(3.0) * config->V_secondary);
  } // Saturation enabled
  ...
  Zprim[0][0] =
      gld::complex((Rd * (Np / Ns) * (Np / Ns)),
                   (Xd * (Np / Ns) * (Np / Ns) + (Np * Np) / (Ns * Ns) * 1e+9));
  Zprim[1][1] = Zprim[0][0];
  Zprim[2][2] = Zprim[0][0];
  Zprim[3][3] = 1e8;
  ...
  if (deltatimestep_running > 0) {
    ...
    for (idex_val = 0; idex_val < 8; idex_val++) {
      for (jdex_val = 0; jdex_val < 8; jdex_val++) {
        ...
        zp_trafo[idex_val][jdex_val] = gld::complex(
            (Rprim + (2.0 * Lprim / deltatimestep_running)), Xprim);
        zhp_trafo[idex_val][jdex_val] = gld::complex(
            (Rprim - (2.0 * Lprim / deltatimestep_running)), Xprim);
        ...
        // Put them into the new matrices
        zmp_trafo[idex_val][jdex_val] = gld::complex(
            (Rprim + (2.0 * Lprim / deltatimestep_running)), Xprim);
        zmhp_trafo[idex_val][jdex_val] = gld::complex(
            (Rprim - (2.0 * Lprim / deltatimestep_running)), Xprim);
      }
    }

    // Form the incident A matrix and it's transpose	-- [1 0 0 -1; etc] type
    // form
    for (idex_val = 0; idex_val < 8; idex_val++) {
    ...
    avaltran_mat[3][0] = avaltran_mat[3][1] = avaltran_mat[3][2] =
        gld::complex(-1.0, 0.0);
    avaltran_mat[7][4] = avaltran_mat[7][5] = avaltran_mat[7][6] =
        gld::complex(-1.0, 0.0);

    // Form y_trafo - A'(inv(zp_trafo)A
    lu_matrix_inverse(&zp_trafo[0][0], &temp_mat[0][0], 8); // inv(zp_trafo)
    lmatrix_mult(&avaltran_mat[0][0], &temp_mat[0][0], &temp_store_mat[0][0],
                 8); // A'*inv
    lmatrix_mult(&temp_store_mat[0][0], &aval_mat[0][0], &y_trafo[0][0],
                 8); // above*A

    // Store out the appropriate portion for current calculations (only 3x3
    // portions)
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        YBase_Full[idex_val * 6 + jdex_val] =
            y_trafo[idex_val][jdex_val]; // Top-left 3x3
        YBase_Full[idex_val * 6 + jdex_val + 3] =
            y_trafo[idex_val][jdex_val + 4]; // Top-right 3x3
        YBase_Full[idex_val * 6 + jdex_val + 18] =
            y_trafo[idex_val + 4][jdex_val]; // Bottom-left 3x3
        YBase_Full[idex_val * 6 + jdex_val + 21] =
            y_trafo[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
    ...
    lmatrix_mult(&avaltran_mat[0][0], &temp_mat[0][0], &temp_store_mat[0][0],
                 8); // A'*inv
    lmatrix_mult(&temp_store_mat[0][0], &aval_mat[0][0], &temp_mat[0][0],
                 8);                                        // inv*A
    lu_matrix_inverse(&temp_mat[0][0], &zh_trafo[0][0], 8); // inv(above)

    //*********** DEBUG
    // zh_trafo is quite a bit off - not sure if it matters (no sat seems to
    // work)
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        ahrlstore[idex_val * 6 + jdex_val] =
            temp_store_mat[idex_val][jdex_val]; // Top-left 3x3
        ahrlstore[idex_val * 6 + jdex_val + 3] =
            temp_store_mat[idex_val][jdex_val + 4]; // Top-right 3x3
        ahrlstore[idex_val * 6 + jdex_val + 18] =
            temp_store_mat[idex_val + 4][jdex_val]; // Bottom-left 3x3
        ahrlstore[idex_val * 6 + jdex_val + 21] =
            temp_store_mat[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        bhrlstore[idex_val * 6 + jdex_val] =
            temp_store_mat[idex_val][jdex_val]; // Top-left 3x3
        bhrlstore[idex_val * 6 + jdex_val + 3] =
            temp_store_mat[idex_val][jdex_val + 4]; // Top-right 3x3
        bhrlstore[idex_val * 6 + jdex_val + 18] =
            temp_store_mat[idex_val + 4][jdex_val]; // Bottom-left 3x3
        bhrlstore[idex_val * 6 + jdex_val + 21] =
            temp_store_mat[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }

    // Create admittance matrix of zmp_trafo -- becomes the current calcuation?,
    // and other portions Form ym_trafo - A'(inv(zmp_trafo)A
    lu_matrix_inverse(&zmp_trafo[0][0], &temp_mat[0][0], 8); // inv(zmp_trafo)
    lmatrix_mult(&avaltran_mat[0][0], &temp_mat[0][0], &temp_store_mat[0][0],
                 8); // A'*inv
    lmatrix_mult(&temp_store_mat[0][0], &aval_mat[0][0], &ym_trafo[0][0],
                 8); // above*A

    // Gets stored below

    // Form zmh_trafo (ymhp_trafo and ymh_trafo only used in here, so skip
    // making them) zmh_trafo = inv(A'(inv_zmhp_trafo)A)
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      ...
      {
        ahmstore[idex_val * 3 + jdex_val] =
            temp_store_mat[idex_val][jdex_val]; // Top-left 3x3
        ahmstore[idex_val * 3 + jdex_val + 9] =
            temp_store_mat[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
    ...
    // Extract the 3x3 portions
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3;
           jdex_val++) // Same funny stack - 6x3 matrix
      {
        bhmstore[idex_val * 3 + jdex_val] =
            temp_store_mat[idex_val][jdex_val]; // Top-left 3x3
        bhmstore[idex_val * 3 + jdex_val + 9] =
            temp_store_mat[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
  ...
  else // Not deltamode, just form up some "normal" matrices
  {
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        temp_mat_small[idex_val][jdex_val] =
            Zprim[idex_val][jdex_val]; // Top-left 3x3
        temp_mat_small[idex_val][jdex_val + 3] =
            Zprim[idex_val][jdex_val + 4]; // Top-right 3x3
        temp_mat_small[idex_val + 3][jdex_val] =
            Zprim[idex_val + 4][jdex_val]; // Bottom-left 3x3
        temp_mat_small[idex_val + 3][jdex_val + 3] =
            Zprim[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        temp_mat_small[idex_val][jdex_val] =
            ZMprim[idex_val][jdex_val]; // Top-left 3x3
        temp_mat_small[idex_val][jdex_val + 3] =
            ZMprim[idex_val][jdex_val + 4]; // Top-right 3x3
        temp_mat_small[idex_val + 3][jdex_val] =
            ZMprim[idex_val + 4][jdex_val]; // Bottom-left 3x3
        temp_mat_small[idex_val + 3][jdex_val + 3] =
            ZMprim[idex_val + 4][jdex_val + 4]; // Bottom-right 3x3
      }
    }
    ...
    for (idex_val = 0; idex_val < 3; idex_val++) {
      for (jdex_val = 0; jdex_val < 3; jdex_val++) {
        ym_trafo[idex_val][jdex_val] =
            temp_other_mat_small[idex_val][jdex_val]; // Top-left 3x3
        ym_trafo[idex_val][jdex_val + 4] =
            temp_other_mat_small[idex_val][jdex_val + 3]; // Top-right 3x3
        ym_trafo[idex_val + 4][jdex_val] =
            temp_other_mat_small[idex_val + 3][jdex_val]; // Bottom-left 3x3
        ym_trafo[idex_val + 4][jdex_val + 4] =
            temp_other_mat_small[idex_val + 3]
                                [jdex_val + 3]; // Bottom-right 3x3
      }
    }
  ...
  // Form up the base admittance -- it's the same for all different
  // magnetization portions YSfrom -- upper left
  for (idex_val = 0; idex_val < 3; idex_val++) {
    for (jdex_val = 0; jdex_val < 3; jdex_val++) {
  ...
  for (idex_val = 0; idex_val < 3; idex_val++) {
    for (jdex_val = 0; jdex_val < 3; jdex_val++) {
      From_Y[idex_val][jdex_val] = -YBase_Full[idex_val * 6 + jdex_val + 3] -
                                   ym_trafo[idex_val][jdex_val + 4];
    }
  }
  ...
  for (idex_val = 0; idex_val < 3; idex_val++) {
    for (jdex_val = 0; jdex_val < 3; jdex_val++) {
      To_Y[idex_val][jdex_val] = -YBase_Full[idex_val * 6 + jdex_val + 18] -
                                 ym_trafo[idex_val + 4][jdex_val];
    }
  }
  ...
  // Now populate the admittance matrices, based on the connections -
  // /*********** Check Both to see if correct? *************/
  if ((config->magnetization_location == config->PRI_MAG) ||
      (config->magnetization_location == config->BOTH_MAG)) // Primary or both
  {
...
int transformer::transformer_saturation_update(bool *deltaIsat) {
  ...
  if ((config->connect_type == config->WYE_WYE) && enable_inrush_calculations &&
      config->model_inrush_saturation) {
    ...
    if (deltaIsat == nullptr) // Init mode
    {
      ...
      saturation_calculated_vals =
          (gld::complex *)gl_malloc(12 * sizeof(gld::complex));

      // Make sure it worked
      if (saturation_calculated_vals == nullptr) {
        GL_THROW("Transformer:%d %s failed to allocate memory for inrush "
                 "saturation tracking",
                 obj->id, obj->name ? obj->name : "Unnamed");
        /*  TROUBLESHOOT
        While attempting to allocate the tracking and calculation matrices for
        the inrush saturation terms, an error was encountered.  Please try
        again.  If the error persists, please submit your code and a bug report
        via the ticketing website.
        */
      }
      ...
      {
        gl_warning("transformer:%d %s is set to model saturation, but has no "
                   "magnetization location",
                   obj->id, obj->name ? obj->name : "Unnamed");
        /*  TROUBLESHOOT
        While attempting to initialize a transformer for inrush saturation
        calculations, it was noticed that no magentization location was
        specified.  No saturation can be modeled in this case.  Please specify
        a winding to include the magnetization on with the
        magnetization_location of the transformer_configuration object.
        */

        // return a "not needed" flag -- no sense calling this if it does
        // nothing
        return -1;
      }
    } // End "init" routine
    else // Actually do the update
    {
      ...
      for (index_loop = 0; index_loop < 6; index_loop++) {
        phi_values[index_loop] =
            A_phi * work_values_voltages[index_loop] + hphi[index_loop];
        saturation_calculated_vals[index_loop + 6] =
            saturation_calculated_vals[index_loop];
      }

      // Apparently reference time matters - sin gets very erratic for
      // POSIX-style "large posterior" times
      //************* TODO ************** Note that this will be influenced by
      //the incidence of the sinusoidal wave
      //********************************* This means it will need to
      //"accumulate" the frequency error for non-nominal sets

      // Compute offset -- same for all cases
      angle_offset = 2.0 * PI * nominal_frequency *
                     (curr_delta_timestep_val - global_time_dbl_val);

      // Determine what to saturation -- check windings
      if ((config->magnetization_location == config->PRI_MAG) ||
          (config->magnetization_location == config->BOTH_MAG)) {
        // Loop
        for (index_loop = 0; index_loop < 3; index_loop++) {
          ...
          Isat_pu_imag_full = temp_double * temp_double +
                              4.0 * D_sat * config->LA_pu; // Part for sqrt
          temp_double = sqrt(Isat_pu_imag_full);
          temp_double += imag_phi_value_pu -
                         config->phiK_pu; // sqrt + imag_phivalue_pu - phiK_pu
          ...
          saturation_calculated_vals[index_loop] =
              Isat_pu * (Isat_pu_imag * I_base_Pri);

          // Compute the difference from the old
          Isat_diff = saturation_calculated_vals[index_loop] -
                      saturation_calculated_vals[index_loop + 6];
      ...
      if ((config->magnetization_location == config->SEC_MAG) ||
        ...
        for (index_loop = 3; index_loop < 6; index_loop++) {
          ...
          // Store the saturation value
          saturation_calculated_vals[index_loop] =
              Isat_pu * (Isat_pu_imag * I_base_Sec);
 ...
 */
static TIMESTAMP sync_transformer_impl(OBJECT *obj, TIMESTAMP t0,
                                       PASSCONFIG pass) {
  try {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_transformer(OBJECT *obj, TIMESTAMP t0,
                                                 PASSCONFIG pass) {
  return sync_transformer_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_transformer(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
...
EXPORT int recalc_deltamode_saturation(OBJECT *obj, bool *deltaIsat) {
  int result;
  result =
      object_data<transformer>(obj)->transformer_saturation_update(deltaIsat);
  return result;
}
...
extern "C" MODULE_API int isa_transformer(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_transformer_impl(obj, classsname);
}
#endif
/**@}**/
