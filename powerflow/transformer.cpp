/** $Id: transformer.cpp 1182 2008-12-22 22:08:36Z dchassin $
...
int transformer::init(OBJECT *parent)
{
	...
	switch (config->connect_type) {
		...
		case transformer_configuration::DELTA_DELTA:
			...
			{
				//Adjust matrices to be similar to Y-Y implementation (algorithm is encoded that way)
				a_mat[0][0] = nt;
				a_mat[1][1] = nt;
				a_mat[2][2] = nt;
		...
		case transformer_configuration::DELTA_GWYE:
			...
			else if (solver_method==SM_GS)
			{
				...
				phaseadjust = complex(sqrt(3.0),-1);
				phaseadjust = phaseadjust/complex(2,0);
