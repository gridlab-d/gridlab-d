		...
		case transformer_configuration::SINGLE_PHASE_CENTER_TAPPED:
			if (solver_method==SM_FBS)
			{
				...
				else if (has_phase(PHASE_A)) // wye-A
				{
					...
					zc =  complex(za_basehi,0) * complex(1000000 * config->impedance.Re(),0) * complex(0,1000000 * config->impedance.Im()) / complex(1000000 * config->impedance.Re(),1000000 * config->impedance.Im());
					zt_b = complex(0,0);
					zt_c = complex(0,0);
