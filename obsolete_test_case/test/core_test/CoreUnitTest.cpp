...
void CoreUnitTest::testModuleLoad() { CPPUNIT_ASSERT(network != nullptr); }

void CoreUnitTest::testNodeLoad() { CPPUNIT_ASSERT(node != nullptr); }

void CoreUnitTest::testLinkLoad() { CPPUNIT_ASSERT(link != nullptr); }

void CoreUnitTest::testTapeLoad() { CPPUNIT_ASSERT(tape != nullptr); }

void CoreUnitTest::testPlayerLoad() { CPPUNIT_ASSERT(player != nullptr); }

void CoreUnitTest::testRecorderLoad() { CPPUNIT_ASSERT(recorder != nullptr); }

void CoreUnitTest::testCollectorLoad() { CPPUNIT_ASSERT(collector != nullptr); }
...
void CoreUnitTest::testPlayerCreate() {
  ...
  // Not sure what this is needed for, or if I should have a separate test for
  // it.
  object_set_value_by_name(obj[3], "loop",
                           "3600"); /* 18000 is about 12y at 1h steps */
...
void CoreUnitTest::testTapeCollectorCreate() {
  ...
  object_set_value_by_name(
      obj[5], "property",
      "count(V.mag),min(V.mag),avg(V.mag),std(V.mag),max(V.mag),min(V.ang),avg("
      "V.ang),std(V.ang),max(V.ang)");
