...
typedef struct {
  ...
  double deltaf;  // frequency error signal *after* adjustment for max/min and
                  // deadband
  double dxi;     // change of PID integrator output
  double xi;      // ouput of PID integrator
  double PIDout;  // PID controller output
  double
      dP; // total change in MW (PIDout when sampling, or older value otherwise)
} SEC_CNTRL_STATE;
...
typedef struct {
  ...
  double dp_dn; // maximum allowable change to setpoint in MW - downward
                // direction (shouid still be positive)
  ...
  double dP[4]; // Change in stepoint [corrector, predictor, actual value set
                // predictor, t-1]
...
private:
  ...
  double curr_timestamp_dbl; // deltamode - tracks last timestep for
                             // reiterations and time tracking
  ...
  std::vector<SEC_CNTRL_PARTICIPANT>
      part_obj; // Vector of participating objects in secondary control
  ...
  double sample_P; // Secondary control should sample unit error values at this
                   // time
  double sample_f; // Secondary control should sample frequency at this time
  bool deadbandflag; // false=frequency within deadband, true=frequency outside
                     // of deadband
  bool tielineflag;  // false=all tie lines within tolerance, true=some tielines
                     // outside of tolerance

  double
      curr_dt; // Stores current deltamode deltat value OR -1 if not available
  double qsts_time; // if the current time is greater than this time then we can
                    // switch to delta mode. -1 indicates unset.
  bool deltaf_flag; // indicates that a non zero deltaf has been detedted at
                    // some poin in this deltamode run.
  SIMULATIONMODE
      prev_simmode_return_value; // store the previously returned simulation
                                 // mode value (in case iterations > 2)
public:
  char1024 participant_input; // command string for creating/modifing secondary
                              // controller participants (or path to csv file)
  int32 participant_count;    // Number of participants in the controller
  double dp_dn_default;       // Default maximum allowable change to setpoint -
                              // downward direction (shouid still be positive)
  double dp_up_default; // Default maximum allowable change to setpoint - upward
                        // direction
  ...
  double underfrequency_limit; // Maximum positive input limit to PID controller
                               // is f0 - underfreuqnecy_limit
  double overfrequency_limit;  // Maximum negative input limit to PID controller
                               // is f0 - overfrequency_limit
  double deadband;             // Deadband for PID controller input in Hz
  double
      tieline_tol; // Generic tie-line error tolerance in p.u. (w.r.t set-point)
  ...
  enum ANTI_WINDUP {
    ...
    FEEDBACK_PIDOUT =
        2, // feedback difference between PIDout and signal passed to generators
    FEEDBACK_INTEGRATOR =
        3 // feedback difference between PIDout and actual "actuated" output
  ...
  double Ts_P; // Sampling interval in sec for unit/tie-line errors (should be
               // >= dt)
  double Ts_f; // Sampling interval in sec for frequency (should be >= dt)
  double deltat_pid;  // Minimum of Ts_p and Ts_f in seconds. Integration step
                      // for PID contorller.
