...
controller_dg::controller_dg(MODULE *mod) {
  if (oclass == nullptr) {
    oclass = gl_register_class(mod, "controller_dg", sizeof(controller_dg),
    ...
    if (gl_publish_variable(
            ...
            PT_char32, "groupid_dg", PADDR(controlled_dgs), PT_DESCRIPTION,
            "the group ID of the dg objects the controller controls.",
            PT_double, "ki", PADDR(ki), PT_DESCRIPTION,
            "parameter of the propotional control", PT_double, "kp", PADDR(kp),
            PT_DESCRIPTION, "parameter of the integral control", PT_double,
            "gain", PADDR(gain), PT_DESCRIPTION, "Gain of the controller",
            PT_double, "ki_QV", PADDR(ki_QV), PT_DESCRIPTION,
            "parameter of the propotional control for secondary voltage "
            "control",
            PT_double, "kp_QV", PADDR(kp_QV), PT_DESCRIPTION,
            "parameter of the integral control for secondary voltage control",
            PT_double, "gain_QV", PADDR(gain_QV), PT_DESCRIPTION,
    ...
    if (gl_publish_function(oclass, "interupdate_controller_object",
                            (FUNCTIONADDR)interupdate_controller_dg) == nullptr)
      GL_THROW("Unable to publish controller_dg deltamode function");
    if (gl_publish_function(oclass, "postupdate_controller_object",
                            (FUNCTIONADDR)postupdate_controller_dg) == nullptr)
      GL_THROW("Unable to publish controller_dg deltamode function");
  }
}
...
int controller_dg::init(OBJECT *parent) {
  ...
  if (controlled_dgs[0] == '\0') {
    ...
    dgs = gl_find_objects(FL_NEW, FT_CLASS, SAME, "diesel_dg", AND, FT_GROUPID,
                          SAME, controlled_dgs.get_string(), FT_END);
    if (dgs == nullptr) {
      gl_error("Although controller given group id, no dgs with given group id "
               "found.");
      /*  TROUBLESHOOT
      While trying to put together a list of all dg objects with the specified
      controller groupid, no such dg objects were found.
      */
  ...
  if (dgs != nullptr) {
    ...
    while (obj = gl_find_next(dgs, obj)) {
      ...
      GenPobj[index].voltage_A_prop =
          map_complex_value(GenPobj[index].obj, "voltage_A");
      GenPobj[index].voltage_B_prop =
          map_complex_value(GenPobj[index].obj, "voltage_B");
      GenPobj[index].voltage_C_prop =
          map_complex_value(GenPobj[index].obj, "voltage_C");
    ...
    for (int i = 0; i < dgs->hit_count; i++) {
      ...
      ctrlGen[i]->curr_state->Vset_ref =
          -1; // Assign negative initial values to Vset_ref as an indicator
    }
  }
  ...
  if (switches != nullptr) {
    ...
    pSwitchObjs =
        (SWITCH_VARS *)gl_malloc(dgs->hit_count * sizeof(pSwitchObjs));
    ...
    while (obj = gl_find_next(switches, obj)) {
      ...
      if (!temp_prop->is_valid() || !temp_prop->is_objectref()) {
        GL_THROW(
            "controller_dg:%d %s Failed to map the switch property 'from'!",
      ...
      // Double check the validity
      if (!temp_prop->is_valid() || !temp_prop->is_objectref()) {
        GL_THROW("controller_dg:%d %s Failed to map the switch property 'to'!",
                 thisobj->id, (thisobj->name ? thisobj->name : "Unnamed"));
        /*  TROUBLESHOOT
        While attempting to map the a property to the switch object, an error
        occurred.  Please try again. If the error persists, please submit your
        GLM and a bug report to the ticketing system.
        */
      }
      ...
      for (int i = 0; i < dgs->hit_count; i++) {
        if (strcmp(temp_from->get_name(), pDG[i].parent->name) == 0 ||
            strcmp(temp_to->get_name(), pDG[i].parent->name) == 0) {
          found = true;
      ...
      if (found) {
        ...
        Switch_Froms[dgswitchFound].voltage_A_prop =
            map_complex_value(fnd_obj, "voltage_A");
        Switch_Froms[dgswitchFound].voltage_B_prop =
            map_complex_value(fnd_obj, "voltage_B");
        Switch_Froms[dgswitchFound].voltage_C_prop =
            map_complex_value(fnd_obj, "voltage_C");
        Switch_Froms[dgswitchFound].voltage_A =
            Switch_Froms[dgswitchFound].voltage_A_prop->get_complex();
        Switch_Froms[dgswitchFound].voltage_B =
            Switch_Froms[dgswitchFound].voltage_B_prop->get_complex();
        Switch_Froms[dgswitchFound].voltage_C =
            Switch_Froms[dgswitchFound].voltage_C_prop->get_complex();
        ...
        pSwitchObjs[dgswitchFound].power_out_A_prop =
            map_complex_value(obj, "power_out_A");
        pSwitchObjs[dgswitchFound].power_out_B_prop =
            map_complex_value(obj, "power_out_B");
        pSwitchObjs[dgswitchFound].power_out_C_prop =
            map_complex_value(obj, "power_out_C");
        pSwitchObjs[dgswitchFound].power_in_prop =
            map_complex_value(obj, "power_in");

        // Map status
        pSwitchObjs[dgswitchFound].status_prop =
            new gld_property(obj, "status");

        // Check it
        if (!pSwitchObjs[dgswitchFound].status_prop->is_valid() ||
            !pSwitchObjs[dgswitchFound].status_prop->is_enumeration()) {
          GL_THROW(
              "controller_dg:%d %s Failed to map the switch property 'status'!",
              thisobj->id, (thisobj->name ? thisobj->name : "Unnamed"));
          /*  TROUBLESHOOT
          While attempting to map the a property to the switch object, an error
          occurred.  Please try again. If the error persists, please submit your
        ...
        // Pull the values
        pSwitchObjs[dgswitchFound].power_out_A =
            pSwitchObjs[dgswitchFound].power_out_A_prop->get_complex();
        pSwitchObjs[dgswitchFound].power_out_B =
            pSwitchObjs[dgswitchFound].power_out_B_prop->get_complex();
        pSwitchObjs[dgswitchFound].power_out_C =
            pSwitchObjs[dgswitchFound].power_out_C_prop->get_complex();
        pSwitchObjs[dgswitchFound].power_in =
            pSwitchObjs[dgswitchFound].power_in_prop->get_complex();
        pSwitchObjs[dgswitchFound].status_val =
            pSwitchObjs[dgswitchFound].status_prop->get_enumeration();
  ...
  if (deltamode_inclusive) {
    ...
    if (!enable_subsecond_models) {
      ...
      */
      ...
      mapped_freq_variable = (double *)gl_get_module_var(
          gl_find_module("powerflow"), "current_frequency");

      // Make sure it isn't empty
      if (mapped_freq_variable == nullptr) {
        GL_THROW("controller_dg:%s - Failed to map frequency checking variable "
      ...
      if (fxn_return_status == FAILED) {
        GL_THROW("controller_dg:%s - failed to add object to generator "
                 "deltamode object list",
                 obj->name ? obj->name : "unnamed");
        /*  TROUBLESHOOT
        The controller_dg object encountered an issue while trying to add itself
  ...
  {
    if (enable_subsecond_models) {
      gl_warning("diesel_dg:%d %s - Deltamode is enabled for the module, but "
                 "not this generator!",
  ...
  // Set rank as 1 so that the controller_dg can be executed after the
  // generators (ranked 0) in sync (and delta-mode) process
...
/* Sync is called at first run, mainly for registration of the delta mode
 * functions */
...
TIMESTAMP controller_dg::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  {
    if (deltamode_inclusive &&
        enable_subsecond_models) // Still "first run", but at least one
                                 // powerflow has completed (call init dyn now)
    {
      ...
      for (int index = 0; index < dgs->hit_count; index++) {
        ...
        if (ret_state == FAILED) {
          GL_THROW(
              "diesel_dg:%s - unsuccessful call to dynamics initialization",
              (obj->name ? obj->name : "unnamed"));
          /*  TROUBLESHOOT
...
// Module-level call
SIMULATIONMODE
controller_dg::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt,
  ...
  // Detect abnormal feeder frequency, switch terminal voltage, and reverse real
  // power flow from generators (absorbing real power) Switch will be turned
  // off. Delay has been added to avoid simultaneously turning off all switches
  OBJECT *obj = nullptr;
  for (int index = 0; index < dgswitchFound; index++) {
    ...
    Switch_Froms[index].voltage_A =
        Switch_Froms[index].voltage_A_prop->get_complex();
    Switch_Froms[index].voltage_B =
        Switch_Froms[index].voltage_B_prop->get_complex();
    Switch_Froms[index].voltage_C =
        Switch_Froms[index].voltage_C_prop->get_complex();
    ...
    pSwitchObjs[index].power_out_A =
        pSwitchObjs[index].power_out_A_prop->get_complex();
    pSwitchObjs[index].power_out_B =
        pSwitchObjs[index].power_out_B_prop->get_complex();
    pSwitchObjs[index].power_out_C =
        pSwitchObjs[index].power_out_C_prop->get_complex();
    pSwitchObjs[index].power_in =
        pSwitchObjs[index].power_in_prop->get_complex();
    pSwitchObjs[index].status_val =
        pSwitchObjs[index].status_prop->get_enumeration();
    ...
    if (phase_A_P > 0 || phase_B_P > 0 || phase_C_P > 0) {
      gl_warning("Reverse real power flow detected at generator switch %s, "
                 "generator is absorbing real power now!",
                 obj->name);
      /*  TROUBLESHOOT
      The diesel generator is absorbing real power, which is not allowed.
      Should check kVA rating and power_out_A/power_out_B/power_out_C value
      settings. Large kVA rating, or small initial power output than the total
      loads may result in negative real power output calculated.
      */
    }

    // Check whether the voltage and frequency is out of limit when the switch
    // is closed
    if ((pSwitchObjs[index].status_val == 1) &&
            (((*mapped_freq_variable > omega_ref / (2.0 * PI) * 1.01) ||
              vtemp[0].Mag() > 1.2 * nominal_voltage ||
              vtemp[1].Mag() > 1.2 * nominal_voltage ||
              vtemp[2].Mag() > 1.2 * nominal_voltage)) ||
      ...
      if (!flag_switchOn || (controlTime == delta_time)) {
        ...
        if (funadd == nullptr) {
          GL_THROW(
              "Failed to find reliability manipulation method on object %s",
              obj->name);
          /*  TROUBLESHOOT
          While attempting to handle special reliability actions on a "special"
          device (switch, recloser, etc.), the function required was not
          located.  Ensure this object type supports special actions and try
          again.  If the problem persists, please submit a bug report and your
        ...
        for (int i = 0; i < 3; i++) {
          return_val = ((int (*)(OBJECT *, unsigned char, bool))(*funadd))(
              obj, openPhases[i], false);
          if (return_val == 0) // Failed :(
          {
            GL_THROW("Failed to handle reliability manipulation on %s",
                     obj->name);
            /*  TROUBLESHOOT
            While attempting to handle special reliability actions on a
            "special" device (switch, recloser, etc.), the function required
            failed to execute properly.  If the problem persists, please submit
            a bug report and your code to the trac website.
            */
          }
        }

        // Set up flags and time control time every time the switch status is
        // changed
        if (!flag_switchOn) {
          flag_switchOn = true;
        }
      ...
      (iteration_count_val == 0)) // First run of new delta call
  {
    ...
    for (int index = 0; index < dgs->hit_count; index++) {
      ...
      memcpy(ctrlGen[index]->next_state, ctrlGen[index]->curr_state,
             sizeof(CTRL_VARS));
    }
  ...
  if (pass_mod == 0) // Predictor pass
  {
    ...
    for (int index = 0; index < dgs->hit_count; index++) {
      ...
      // Call dynamics
      apply_dynamics(ctrlGen[index]->curr_state, &predictor_vals, deltat,
                     index);

      // Apply prediction update
      ctrlGen[index]->next_state->x =
          ctrlGen[index]->curr_state->x + predictor_vals.x * deltat;
      ctrlGen[index]->next_state->Pref_ctrl =
          ctrlGen[index]->next_state->x + predictor_vals.x * (kp / ki);
      ...
      // Apply prediction update
      ctrlGen[index]->next_state->x_QV =
          ctrlGen[index]->curr_state->x_QV + predictor_vals.x_QV * deltat;
      ctrlGen[index]->next_state->Vset_ctrl =
          ctrlGen[index]->next_state->x_QV +
          predictor_vals.x_QV * (kp_QV / ki_QV);
  ...
  } else                  // Corrector pass
  {
    ...
    for (int index = 0; index < dgs->hit_count; index++) {
      ...
      // Call dynamics
      apply_dynamics(ctrlGen[index]->next_state, &corrector_vals, deltat,
                     index);

      // Reconcile updates update
      ctrlGen[index]->next_state->x =
          ctrlGen[index]->curr_state->x +
          (predictor_vals.x + corrector_vals.x) * deltath;
      ctrlGen[index]->next_state->Pref_ctrl =
          ctrlGen[index]->next_state->x +
          (predictor_vals.x + corrector_vals.x) * 0.5 * (kp / ki);
      ...
      // Apply prediction update
      ctrlGen[index]->next_state->x_QV =
          ctrlGen[index]->curr_state->x_QV +
          (predictor_vals.x_QV + corrector_vals.x_QV) * deltat;
      ctrlGen[index]->next_state->Vset_ctrl =
          ctrlGen[index]->next_state->x_QV +
          (predictor_vals.x_QV + corrector_vals.x_QV) * 0.5 * (kp_QV / ki_QV);

      pDG[index].Vset = ctrlGen[index]->next_state->Vset_ctrl;
      pDG[index].Vset_prop->setp<double>(pDG[index].Vset, test_rlock);

      // Copy everything back into curr_state, since we'll be back there
      memcpy(ctrlGen[index]->curr_state, ctrlGen[index]->next_state,
             sizeof(CTRL_VARS));

      // Check convergence - pick the max difference
      temp_double =
          fmax(temp_double, fabs(ctrlGen[index]->curr_state->Pref_ctrl -
                                 prev_Pref_val[index]));
      temp_double =
          fmax(temp_double, fabs(ctrlGen[index]->curr_state->Vset_ctrl -
                                 prev_Vset_val[index]));
...
STATUS controller_dg::post_deltaupdate(gld::complex *useful_value,
...
STATUS controller_dg::apply_dynamics(CTRL_VARS *curr_time,
                                     CTRL_VARS *curr_delta, double deltaT,
                                     int index) {

  // Control of the f/p droop
  curr_delta->x = gain *
                  (curr_time->wref_ctrl - *mapped_freq_variable * 2 * PI) /
                  omega_ref * ki;
  ...
  double V_pos_temp =
      (GenPobj[index].voltage_A.Mag() + GenPobj[index].voltage_B.Mag() +
       GenPobj[index].voltage_C.Mag()) /
      3.0;
  curr_delta->x_QV =
      gain_QV * (curr_time->Vset_ref - V_pos_temp) / nominal_voltage * ki_QV;
...
STATUS controller_dg::init_dynamics(CTRL_VARS *curr_time, int index) {
  OBJECT *obj = nullptr;
  double omega =
      *mapped_freq_variable * 2 *
      PI; // not used here since speed of each generator deirectly used
  ...
  // If Vset_ref has not been set yet, define it as the positive voltage values
  // measured before entering the delta mode
...
gld_property *controller_dg::map_complex_value(OBJECT *obj, const char *name) {
  ...
  if (!pQuantity->is_valid() || !pQuantity->is_complex()) {
    GL_THROW(
        "controller_dg:%d %s - Unable to map property %s from object:%d %s",
        objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name, obj->id,
        (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    While attempting to map a quantity from another object, an error occurred in
    controller_dg.  Please try again. If the error persists, please submit your
...
EXPORT int create_controller_dg(OBJECT **obj, OBJECT *parent) {
  try {
    *obj = gl_create_object(controller_dg::oclass);
    if (*obj != nullptr) {
      controller_dg *my =
          /*OBJECTDATA(*obj, controller_dg)*/ object_data<controller_dg>(*obj);
...
EXPORT int init_controller_dg(OBJECT *obj, OBJECT *parent) {
  try {
    if (obj != nullptr)
      return /*OBJECTDATA(obj,controller_dg)*/ object_data<controller_dg>(obj)
          ->init(parent);
    else
      return 0;
  }
...
static TIMESTAMP sync_controller_dg_impl(OBJECT *obj, TIMESTAMP t0,
                                         PASSCONFIG pass) {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_controller_dg(OBJECT *obj, TIMESTAMP t0,
                                                   PASSCONFIG pass) {
...
interupdate_controller_dg(OBJECT *obj, unsigned int64 delta_time,
  ...
  try {
    ...
    gl_error("interupdate_controller_dg(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return status;
  }
}

EXPORT STATUS postupdate_controller_dg(OBJECT *obj, gld::complex *useful_value,
                                       unsigned int mode_pass) {
  controller_dg *my =
      /*OBJECTDATA(obj, controller_dg)*/ object_data<controller_dg>(obj);
  ...
  try {
    ...
    gl_error("postupdate_controller_dg(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return status;
  }
}
