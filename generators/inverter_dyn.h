...
EXPORT STATUS postupdate_inverter_dyn(OBJECT *obj, gld::complex *useful_value,
                                      unsigned int mode_pass);
EXPORT STATUS inverter_dyn_NR_current_injection_update(OBJECT *obj,
...
private:
  ...
  gld::complex filter_admittance; // Filter admittance value - mostly separate
                                  // to make single-phase easier
  gld::complex value_IGenerated[3]; // Value/accumulator for IGenerated values
  gld::complex prev_value_IGenerated[3]; // Tracking variable for grid following
                                         // "QSTS exit"
  ...
  bool parent_is_a_meter; // Boolean to indicate if the parent object is a
                          // meter/triplex_meter
  bool parent_is_single_phase; // Boolean to indicate if the parent object is
                               // single-phased (main or triplexed)
  bool parent_is_triplex; // Boolean to indicate if the parent object was a
                          // triplex device - minimal usage
  ...
  gld::complex
      value_Circuit_V[3]; ///< value holder for the three L-N voltage fields
  gld::complex value_Line_I[3]; ///< value holder for the three current fields
  gld::complex value_Line_unrotI[3]; ///< value holder for the three pre-rotated
                                     ///< current fields
  ...
  bool Reconnect_Warn_Flag; // Flag to warn on a 1547 recovery that the behavior
                            // is not fully validated

  bool enable_1547_compliance; // Flag to enable IEEE 1547-2003/2014 condition
                               // checking
  double IEEE1547_reconnect_time; // Time after a 1547 violation clears before
                                  // we reconnect
  ...
  // 1547(a) frequency
  double IEEE1547_over_freq_high_band_setpoint; // OF2 set point for IEEE
                                                // 1547a-2014
  double IEEE1547_over_freq_high_band_delay; // OF2 clearing time for
                                             // IEEE1547a-2014
  double IEEE1547_over_freq_high_band_viol_time; // OF2 violation accumulator
  double
      IEEE1547_over_freq_low_band_setpoint; // OF1 set point for IEEE 1547a-2014
  double IEEE1547_over_freq_low_band_delay; // OF1 clearing time for IEEE
                                            // 1547a-2014
  double IEEE1547_over_freq_low_band_viol_time;  // OF1 violation accumulator
  double IEEE1547_under_freq_high_band_setpoint; // UF2 set point for IEEE
                                                 // 1547a-2014
  double IEEE1547_under_freq_high_band_delay; // UF2 clearing time for
                                              // IEEE1547a-2014
  double IEEE1547_under_freq_high_band_viol_time; // UF2 violation accumulator
  double IEEE1547_under_freq_low_band_setpoint;   // UF1 set point for IEEE
                                                // 1547a-2014
  double IEEE1547_under_freq_low_band_delay; // UF1 clearing time for IEEE
                                             // 1547a-2014
  double IEEE1547_under_freq_low_band_viol_time; // UF1 violation accumulator

  // 1547 voltage(a) voltage
  double IEEE1547_under_voltage_lowest_voltage_setpoint; // Lowest voltage
                                                         // threshold for
                                                         // undervoltage
  double IEEE1547_under_voltage_middle_voltage_setpoint; // Middle-lowest
                                                         // voltage threshold
                                                         // for undervoltage
  double IEEE1547_under_voltage_high_voltage_setpoint; // High value of low
                                                       // voltage threshold for
                                                       // undervoltage
  double IEEE1547_over_voltage_low_setpoint; // Lowest voltage value for
                                             // overvoltage
  double
      IEEE1547_over_voltage_high_setpoint; // High voltage value for overvoltage
  double IEEE1547_under_voltage_lowest_delay; // Lowest voltage clearing time
                                              // for undervoltage
  double IEEE1547_under_voltage_middle_delay; // Middle-lowest voltage clearing
                                              // time for undervoltage
  double IEEE1547_under_voltage_high_delay; // Highest voltage clearing time for
                                            // undervoltage
  double IEEE1547_over_voltage_low_delay; // Lowest voltage clearing time for
                                          // overvoltage
  double IEEE1547_over_voltage_high_delay; // Highest voltage clearing time for
                                           // overvoltage
  double
      IEEE1547_under_voltage_lowest_viol_time; // Lowest low voltage threshold
                                               // violation accumulator
  double
      IEEE1547_under_voltage_middle_viol_time; // Middle low voltage threshold
                                               // violation accumulator
  double IEEE1547_under_voltage_high_viol_time; // Highest low voltage threshold
                                                // violation accumulator
  double IEEE1547_over_voltage_low_viol_time; // Lowest high voltage threshold
                                              // violation accumulator
  double IEEE1547_over_voltage_high_viol_time; // Highest high voltage threshold
                                               // violation accumulator
  ...
  double IEEE1547_out_of_violation_time_total; // Tracking variable to see how
                                               // long we've been "outside of
                                               // bad conditions" to re-enable
                                               // the inverter
  gld_property
  ...
  double ieee_1547_delta_return; // Deltamode tracker - made global for
                                 // "off-cycle" checks
  ...
  typedef struct {
    ...
    bool inverter_1547_status; // helper variable to check changes to inverter
                               // 1547 status
  } SETPOINT_CHK;
...
public:
  ...
  enum GRID_FORMING_TYPE { CONSTANT_DC_BUS = 0, DYNAMIC_DC_BUS = 1 };
  enumeration grid_forming_mode; //

  enum P_F_DROOP_SETTING_TYPE { FSET_MODE = 0, PSET_MODE = 1 };
  ...
  gld::complex
      terminal_current_val_pu_prefault[3]; // Pre-fault current, used in Imax
                                           // angle correction only
  bool imax_phase_correction_done[3];
  bool phase_angle_correction; // GLM input - Enable or disable phase angle
                               // correction?
  bool virtual_resistance_correction; // GLM input - Current limiting through
  ...
  double
      delta_w_prev_step; // frequency of the internal voltage at previous step

  Integrator Angle_blk[3]; // Integrator block for calculating phase angle of
  ...
  double E_mag; // internal voltage magnitude, used for grid-forming control,
                // output of voltage control block
  ...
  double Pref_droop_pu_filter;     // Output of Pref droop filter block for
                                   // frequency watt
  ...
  gld::complex I_out_PU_temp[3]; // This is mainly used for current limiting
                                 // function of a grid-forming inverter

  gld::complex power_val[3];       // power
  gld::complex VA_Out;             // complex output power
  gld::complex value_Circuit_V_PS; // Positive sequence voltage of three phase
                                   // terminal voltages
  gld::complex
      value_Circuit_I_PS[3]; // Positive sequence current of three phase
                             // terminal currents, assume no negative or zero
                             // sequence, Phase A equals to positive sequence

  double node_nominal_voltage; // Nominal voltage

  double mdc; // only used when dc bus dynamic is enabled, make sure that the
              // modulation index is enough

  bool frequency_watt; // Boolean value indicating whether the f/p droop curve
                       // is included in the inverter or not
  bool checkRampRate_real; // check the active power ramp rate
  bool volt_var; // Boolean value indicating whether the volt-var droop curve is
                 // included in the inverter or not
  ...
  double Pref_droop_pu_prev; // The value of Pref in last simulation step, note
                             // it is only defined in the predictor pass
  double Qref_droop_pu_prev; // The value of Qref in last simulation step, note
                             // it is only defined in the predictor pass
  ...
  double Vset;  // Vset is the voltage set point in grid-forming inverter,
                // usually 1 pu

  double kpv;   // proportional gain and integral gain of voltage loop
  double kiv;   //
  double mq;    // mq is the Q-V droop gain, usually 0.05 pu
  double E_max; // E_max and E_min are the maximum and minimum of the output of
                // voltage controller
  double E_min; //
  ...
  double
      kppmax; // proportional and integral gains for Pmax and Pmin controllers
  double kipmax;
  double
      kpqmax; // proportional and integral gains for Qmax and Qmin controllers
  double kiqmax;
  double w_lim; // w_lim is the saturation limit
  double V_lim; // the saturation limit of the Qmax and Qmin controllers
  double Pmax;  // Pmax and Pmin are the maximum limit and minimum limit of Pmax
                // controller and Pmin controller
  double Pmin;
  ...
  gld::complex
      e_source[3]; // e_source[i] is the complex value of internal voltage
  gld::complex e_source_pu[3]; // e_source[i] is the complex per-unit value of
                               // internal voltage
  gld::complex
      e_droop[3]; // e_droop is the complex value of the inverter internal
                  // voltage given by the grid-forming droop control
  gld::complex
      e_droop_pu[3]; // e_droop is the complex value of the inverter internal
                     // voltage given by the grid-forming droop control
  double e_source_Re[3];
  double e_source_Im[3];
  gld::complex I_source[3]; // I_source[i] is the complex value when using
                            // current source representation
  double I_source_Re[3];
  double I_source_Im[3];
  double pCircuit_V_Avg_pu; // pCircuit_V_Avg_pu refers to the average value of
                            // three phase voltages, it is per-unit value
  double Xfilter; // Rfilter and Xfilter are the per-unit values of inverter
                  // filter, they are per-unit values
  double Rfilter;
  double freq; // freq is the frequency obtained from the P-f droop controller
  double fset; // frequency set point is for isochronous mode of grid-forming
               // inverters

  double
      kpPLL; // kiPLL and kiPLL are the proportional and integral gains of PLL
  double kiPLL;   //
  ...
  double Tff;   // Tff is the time constant of low pass filter in frequency
                // measurement
  double Tif;   // Tif is the low pass filter when using current source
                // representation
  ...
  double Pref_max; // Pref_max and Pref_min are the upper and lower limits of
                   // power references
  double Pref_min; //
  double V_Avg_pu; // average voltage in volt-var
  double Qref_droop_pu; // Q reference in volt-var
  double Qref_max; // Qref_max and Qref_min are the upper and lower limits of Q
                   // references
  double Qref_min; //
  ...
  double m_Vdc; // modulation index when using grid-forming PV inverter, it is
                // per unit value
  double Vdc_min_pu; // Tha minimum dc bus voltage that the PV grid-forming
                     // inverter can run. It is also the maximum point voltage
                     // of PV panel, it is per unit value
  double I_dc_pu; // equivalent current at the dc side, which is calculated from
                  // the ac side, it is per unit value
