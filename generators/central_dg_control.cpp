...
central_dg_control::central_dg_control(MODULE *module)
{
    if (oclass == nullptr)
    {
        oclass = gl_register_class(
            module, "central_dg_control", sizeof(central_dg_control),
...
int central_dg_control::init(OBJECT *parent)
{
  OBJECT *thisObj = object_header(this);
    ...
    inverter_list = gl_find_objects(FL_NEW, FT_CLASS, SAME, "inverter", AND, FT_GROUPID, SAME,
    ...
    battery_list = gl_find_objects(FL_NEW, FT_CLASS, SAME, "battery", AND, FT_PARENT, FT_CLASS, SAME, "inverter", AND,
                                   FT_PARENT, FT_GROUPID, SAME, controlled_objects.get_string(), FT_END);
    ...
    solar_list = gl_find_objects(FL_NEW, FT_CLASS, SAME, "solar", AND, FT_PARENT, FT_CLASS, SAME, "inverter", AND,
                                 FT_PARENT, FT_GROUPID, SAME, controlled_objects.get_string(), FT_END);
    ...
    if (inverter_set[inverter_filled_to + 1] == nullptr) {
        ...
        // comment in the previous line says "inverter rated complex power", but
        // here it uses the PV Panel's value.)
    ...
    if (feederhead_meter != nullptr)
    {
        ...
        if (gl_object_isa(feederhead_meter, "meter", "powerflow"))
        {
            ...
            {
                GL_THROW("central_dg_control:%d - %s - failed to map feaderhead_meter power property!", obj->id,
                ...
                error persists, please submit a bug report via the issues system.
                */
            }
        ...
        {
            GL_THROW("central_dg_control:%d - %s - feederhead_meter is empty!", obj->id,
                     (obj->name ? obj->name : "Unnamed"));
            /*  TROUBLESHOOT
...
TIMESTAMP central_dg_control::presync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    // should return to their original values or their current scheduled values if
    // a schedule is given. In the case of a schedule, the player values will be
    // played in during presync so no action need be taken. However, if only
...
TIMESTAMP central_dg_control::sync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    // The target power factor is the midpoint of the two values specifying the
    ...
    // If controller action is taken, t1 is returned to force the powerflow to
    // reiterate before checking lower rank control modes. If controller action is
    // not taken, the next lowest rank mode is checked. After all have been
    ...
    while (i > -1)
    {
        if (control_mode_setting[i] != 0)
        {
            ...
            case CONSTANT_PF:
                // Calculate measured power factor at feederhead. Note this is signed
                // power factor. The magnitude is equal to |P|/|S|. The sign is positive
                ...
                // If we're still here then we've come up with a total Q dispatch to
                // correct power factor. Now we need to allocate the Q. For starters,
                // look at total inverter rated S and current real power output to get
