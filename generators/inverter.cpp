...
int inverter::init(OBJECT *parent)
{
    ...
    if (use_multipoint_efficiency)
    {
        switch (inverter_manufacturer)
        { // all manufacturer defaults use the CEC parameters
        case NONE:
            if (p_dco < 0)
            {
...
TIMESTAMP inverter::sync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    if ((value_MeterStatus == 1) && inverter_1547_status) // Make sure the meter is in service
    {
        ...
        else // FOUR_QUADRANT code
        {
            ...
            { // == FQM_VOLT_VAR || == FQM_VOLT_WATT
                if (four_quadrant_control_mode == FQM_VOLT_VAR)
                { // revalidating the parameters, we are in ::sync here, but maybe they changed?
                    if (V1 == -2)
                        V1 = 0.97;
                    if (V2 == -2)
                        V2 = 0.99;
            ...
            else if (four_quadrant_control_mode == FQM_CONSTANT_PQ)
            {
                ...
                if (b_soc != -1)
                {
                    ...
                    if (fabs(temp_VA.Mag()) > p_max)
                    {                                   // Requested power output (P_Out, Q_Out) is greater than inverter rating
                ...
                { // no battery is attached we are attached to a solar panel
                    // Ensuring power rating of inverter is not exceeded.
                    if (fabs(temp_VA.Mag()) > p_max)
                    {                                   // Requested power output (P_Out, Q_Out) is greater than inverter rating
                        if (p_max > fabs(temp_VA.Re())) // Can we reduce the reactive power output and stay within the inverter rating?
                    ...
                    if (fabs(temp_VA.Mag()) > VA_Efficiency)
                    {                                           // Requested power output (P_Out, Q_Out) is greater than solar input
            ...
            { // Volt Var or Volt-Watt Control mode
                if (four_quadrant_control_mode == FQM_VOLT_VAR)
                {
                ...
                if ((phases & 0x10) == 0x10)
                {
                    ...
                    if (vpu > VW_V1)
                    {
                        ...
                        if (pa_vw_limited < 0.0)
                            pa_vw_limited = 0.0;
                        if (power_val[0].Re() > pa_vw_limited)
                            power_val[0].SetReal(pa_vw_limited); // TODO - curtail power absorption for batteries?
                    }
