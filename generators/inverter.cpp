...
inverter::inverter(MODULE *module)
{
	if (oclass==nullptr)
	{
		...
		if (gl_publish_variable(oclass,
			...
			//1547 stuff
			PT_bool, "enable_1547_checks", PADDR(enable_1547_compliance), PT_DESCRIPTION, "Enable IEEE 1547-2003 disconnect checking",
                                PT_double, "reconnect_time[s]", PADDR(reconnect_time), PT_DESCRIPTION, "Time delay after IEEE 1547-2003 violation clears before resuming generation",
                                PT_bool, "inverter_1547_status", PADDR(inverter_1547_status), PT_DESCRIPTION, "Indicator if the inverter is curtailed due to a 1547 violation or not",

			//Select 1547 type to auto-populate
			PT_enumeration, "IEEE_1547_version", PADDR(ieee_1547_version), PT_DESCRIPTION, "Version of IEEE 1547 to use to populate defaults",
			...
			PT_double, "over_freq_high_cutout[Hz]", PADDR(over_freq_high_band_setpoint), PT_DESCRIPTION, "OF2 set point for IEEE 1547a",
                                PT_double, "over_freq_high_disconnect_time[s]", PADDR(over_freq_high_band_delay), PT_DESCRIPTION, "OF2 clearing time for IEEE1547a",
                                PT_double, "over_freq_low_cutout[Hz]", PADDR(over_freq_low_band_setpoint), PT_DESCRIPTION, "OF1 set point for IEEE 1547a",
                                PT_double, "over_freq_low_disconnect_time[s]", PADDR(over_freq_low_band_delay), PT_DESCRIPTION, "OF1 clearing time for IEEE 1547a",
                                PT_double, "under_freq_high_cutout[Hz]", PADDR(under_freq_high_band_setpoint), PT_DESCRIPTION, "UF2 set point for IEEE 1547a",
                                PT_double, "under_freq_high_disconnect_time[s]", PADDR(under_freq_high_band_delay), PT_DESCRIPTION, "UF2 clearing time for IEEE1547a",
                                PT_double, "under_freq_low_cutout[Hz]", PADDR(under_freq_low_band_setpoint), PT_DESCRIPTION, "UF1 set point for IEEE 1547a",
                                PT_double, "under_freq_low_disconnect_time[s]", PADDR(under_freq_low_band_delay), PT_DESCRIPTION, "UF1 clearing time for IEEE 1547a",
			...
			//1547 trip reason
			PT_enumeration, "IEEE_1547_trip_method", PADDR(ieee_1547_trip_method), PT_DESCRIPTION, "Reason for IEEE 1547 disconnect - which threshold was hit",
...
int inverter::init(OBJECT *parent)
{
  ...
  parent = obj->parent; // AppleClang bug workaround - parent is coming in as
	...
	if (enable_1547_compliance)
	{
		...
		gl_warning("inverter:%d-%s - IEEE 1547 checks are enabled, but the modeldoes not support transient mode",obj->id,(obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
		The IEEE 1547 checks have been enabled for the inverter object, but that object does not support transient mode.  This will severely hinder the inverter's
		ability to do these checks, and really isn't the intended mode of operation.  It is recommended to use inverter_dyn for those scenarios.
		*/
	}
...
//Functionalized IEEE 1547 initalizer
