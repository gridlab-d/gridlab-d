...
{	
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_double, "Pref", PADDR(curr_state.Pref), PT_DESCRIPTION, "DELTAMODE: The real power reference.",
			PT_double, "Qref", PADDR(curr_state.Qref), PT_DESCRIPTION, "DELTAMODE: The reactive power reference.",
			PT_double, "epA", PADDR(curr_state.ed[0]), PT_DESCRIPTION, "DELTAMODE: The real power error on phase A.",
			PT_double, "epB", PADDR(curr_state.ed[1]), PT_DESCRIPTION, "DELTAMODE: The real power error on phase B.",
			PT_double, "epC", PADDR(curr_state.ed[2]), PT_DESCRIPTION, "DELTAMODE: The real power error on phase C.",
			PT_double, "eqA", PADDR(curr_state.eq[0]), PT_DESCRIPTION, "DELTAMODE: The reactive power error on phase A.",
			PT_double, "eqB", PADDR(curr_state.eq[1]), PT_DESCRIPTION, "DELTAMODE: The reactive power error on phase B.",
			PT_double, "eqC", PADDR(curr_state.eq[2]), PT_DESCRIPTION, "DELTAMODE: The reactive power error on phase C.",
			PT_double, "delta_ep", PADDR(curr_state.ded), PT_DESCRIPTION, "DELTAMODE: The change in real power error.",
			PT_double, "delta_eq", PADDR(curr_state.deq), PT_DESCRIPTION, "DELTAMODE: The change in reactive power error.",
			PT_double, "md", PADDR(curr_state.md), PT_DESCRIPTION, "DELTAMODE: The d-axis current modulation.",
			PT_double, "mq", PADDR(curr_state.mq), PT_DESCRIPTION, "DELTAMODE: The q-axis current modulation.",
			PT_double, "delta_md", PADDR(curr_state.dmd), PT_DESCRIPTION, "DELTAMODE: The change in d-axis current modulation.",
			PT_double, "delta_mq", PADDR(curr_state.dmq), PT_DESCRIPTION, "DELTAMODE: The change in q-axis current modulation.",
			PT_complex, "Idq", PADDR(curr_state.Idq), PT_DESCRIPTION, "DELTAMODE: The d-axis current.",
			PT_double, "kpd", PADDR(curr_state.kpd), PT_DESCRIPTION, "DELTAMODE: The d-axis integration gain for the current modulation PI controller.",
			PT_double, "kpq", PADDR(curr_state.kpq), PT_DESCRIPTION, "DELTAMODE: The q-axis integration gain for the current modulation PI controller.",
			PT_double, "kid", PADDR(curr_state.kid), PT_DESCRIPTION, "DELTAMODE: The d-axis proportional gain for the current modulation PI controller.",
			PT_double, "kiq", PADDR(curr_state.kiq), PT_DESCRIPTION, "DELTAMODE: The q-axis proportional gain for the current modulation PI controller.",
...
// end of volt -var addition
	...
	a2.SetPolar(1.0,-2.0*PI/3.0);
	a.SetPolar(1.0,2.0*PI/3.0);
	last_VA_Out = 0;
	I_Out[0] = complex(0);
	I_Out[1] = complex(0);
	I_Out[2] = complex(0);
...
TIMESTAMP inverter::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if(inverter_type_v != FOUR_QUADRANT){
		...
		if(deltamode_inclusive && enable_subsecond_models && t1 != t0) {
			last_VA_Out = VA_Out;
			last_I_In = I_In.Re();
			for(int i = 0; i < 3; i++) {
...
if (four_quadrant_control_mode != FQM_VOLT_VAR) {
			...
			} else { // // Volt Var Control mode sync
...
SIMULATIONMODE inverter::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	...
	{
		if(curr_state.kid == 0.0) {
			curr_state.kid = 1.0 / deltat;
		}
		if(curr_state.kiq == 0.0) {
			curr_state.kiq = 1.0 / deltat;
		}
		pred_state.kid = curr_state.kid;
		pred_state.kiq = curr_state.kiq;
		if(iteration_count_val == 0) {
			//Initialize dynamics
			init_dynamics(&curr_state);
			//Send Current Injection to parent
			if((phases & 0x10) == 0x10) {
				*pLine12 += -curr_state.Iac[0];
				I_Out[0]= curr_state.Iac[0];
			}
			if((phases & 0x07) == 0x07) {
				for(int i = 0; i < 3; i++) {
					pLine_I[i] += -curr_state.Iac[i];
					I_Out[i] = curr_state.Iac[i];
				}
			}
			return SM_DELTA_ITER; // iterate so I know what my current power out is
			...
			if((phases & 0x07) == 0x07) {
				VA_Out = (pCircuit_V[0] * ~(I_Out[0]) + (pCircuit_V[1] * ~(I_Out[0])) + (pCircuit_V[2] * ~(I_Out[2])));
			}
			//calculate my current errors
			curr_state.Pout = VA_Out.Re();
			curr_state.Qout = VA_Out.Im();
			if((phases & 0x10) == 0x10) {
				curr_state.ed[0] = ((~(complex(curr_state.Pref, curr_state.Qref)/(*pCircuit_V))) - (~(complex(curr_state.Pout,curr_state.Qout)/(*pCircuit_V)))).Re();
				curr_state.ded[0] = curr_state.ed[0] / deltat;
				curr_state.dmd[0] = (curr_state.kpd * curr_state.ded[0]) + (curr_state.kid * curr_state.ed[0]);
				curr_state.eq[0] = ((~(complex(curr_state.Pref, curr_state.Qref)/(*pCircuit_V))) - (~(complex(curr_state.Pout,curr_state.Qout)/(*pCircuit_V)))).Im();
				curr_state.deq[0] = curr_state.eq[0] / deltat;
				curr_state.dmq[0] = (curr_state.kpq * curr_state.deq[0]) + (curr_state.kiq * curr_state.eq[0]);
				if(fabs(curr_state.ded[0]) <= inverter_convergence_criterion && fabs(curr_state.deq[0]) <= inverter_convergence_criterion) {
					return SM_EVENT;// we have reached steady state
				} else {
					return SM_DELTA;
				}
			}
			if((phases & 0x07) == 0x07) {
				for(i = 0; i < 3; i++) {
					curr_state.ed[i] = ((~(complex(curr_state.Pref/3.0, curr_state.Qref/3.0)/(pCircuit_V[i]))) - (~(complex(curr_state.Pout/3.0,curr_state.Qout/3.0)/(pCircuit_V[i])))).Re();
					curr_state.ded[i] = curr_state.ed[i] / deltat;
					curr_state.dmd[i] = (curr_state.kpd * curr_state.ded[i]) + (curr_state.kid * curr_state.ed[i]);
					curr_state.eq[i] = ((~(complex(curr_state.Pref/3.0, curr_state.Qref/3.0)/(pCircuit_V[i]))) - (~(complex(curr_state.Pout/3.0,curr_state.Qout/3.0)/(pCircuit_V[i])))).Im();
					curr_state.deq[i] = curr_state.eq[i] / deltat;
					curr_state.dmq[i] = (curr_state.kpq * curr_state.deq[i]) + (curr_state.kiq * curr_state.eq[i]);
					if(fabs(curr_state.ded[i]) <= inverter_convergence_criterion && fabs(curr_state.deq[i]) <= inverter_convergence_criterion && result != SM_DELTA) {
						result = SM_EVENT;// we have reached steady state
					} else {
						result = SM_DELTA;
					}
				}
		...
		if((phases & 0x07) == 0x07) {
			for(i = 0; i < 3; i++) {
				...
				pLine_I[i] += I_Out[i];
				pLine_I[i] += -pred_state.Iac[i];
			}
		}

		//update the Pref and Qref values
		update_control_references(&pred_state);
		curr_state.Pref = pred_state.Pref;
		curr_state.Qref = pred_state.Qref;
		return SM_DELTA_ITER;
	} else if(iteration_count_val == 1) {
		//calculate the corrector state
		if ((phases & 0x10) == 0x10) {
			pred_state.Pout = (*pCircuit_V * ~(pred_state.Iac[0])).Re();
			pred_state.Qout = (*pCircuit_V * ~(pred_state.Iac[0])).Im();
			pred_state.ed[0] = ((~(complex(pred_state.Pref, pred_state.Qref)/(*pCircuit_V))) - (~(complex(pred_state.Pout,pred_state.Qout)/(*pCircuit_V)))).Re();
			pred_state.ded[0] = (pred_state.ed[0] - curr_state.ed[0]) / deltat;
			pred_state.dmd[0] = (pred_state.kpd * pred_state.ded[0]) + (pred_state.kid * pred_state.ed[0]);
			...
			pred_state.eq[0] = ((~(complex(pred_state.Pref, pred_state.Qref)/(*pCircuit_V))) - (~(complex(pred_state.Pout,pred_state.Qout)/(*pCircuit_V)))).Im();
			pred_state.deq[0] = (pred_state.eq[0] - curr_state.eq[0]) / deltat;
			pred_state.dmq[0] = (pred_state.kpq * pred_state.deq[0]) + (pred_state.kiq * pred_state.eq[0]);
		...
		if((phases & 0x07) == 0x07) {
			pred_state.Pout = (pCircuit_V[0] * ~(pred_state.Iac[0])).Re() + (pCircuit_V[1] * ~(pred_state.Iac[1])).Re() + (pCircuit_V[2] * ~(pred_state.Iac[2])).Re();
			pred_state.Qout = (pCircuit_V[0] * ~(pred_state.Iac[0])).Im() + (pCircuit_V[1] * ~(pred_state.Iac[1])).Im() + (pCircuit_V[2] * ~(pred_state.Iac[2])).Im();
			for(i = 0; i < 3; i++) {
				pred_state.ed[i] = ((~(complex(pred_state.Pref/3.0, pred_state.Qref/3.0)/(pCircuit_V[i]))) - (~(complex(pred_state.Pout/3.0,pred_state.Qout/3.0)/(pCircuit_V[i])))).Re();
				pred_state.ded[i] = (pred_state.ed[i] - curr_state.ed[i]) / deltat;
				pred_state.dmd[i] = (pred_state.kpd * pred_state.ded[i]) + (pred_state.kid * pred_state.ed[i]);
				...
				pred_state.eq[i] = ((~(complex(pred_state.Pref/3.0, pred_state.Qref/3.0)/(pCircuit_V[i]))) - (~(complex(pred_state.Pout/3.0,pred_state.Qout/3.0)/(pCircuit_V[i])))).Im();
				pred_state.deq[i] = (pred_state.eq[i] - curr_state.eq[i]) / deltat;
				pred_state.dmq[i] = (pred_state.kpq * pred_state.deq[i]) + (pred_state.kiq * pred_state.eq[i]);
...
STATUS inverter::init_dynamics(INV_STATE *curr_time)
{
	complex prev_Idq[3];
...
void inverter::update_control_references(INV_STATE *inv_state)
{
	//FOUR_QUADRANT model (originally written for NAS/CES, altered for PV)
	double VA_Efficiency, temp_PF, temp_QVal;
	...
	else if (four_quadrant_control_mode == FQM_CONSTANT_PQ)
	{
		//Compute desired output - sign convention appears to be backwards
		temp_VA = complex(P_Out,Q_Out);
		// constant PQ addition from 797 //
		if(temp_VA.Mag() > VA_Efficiency){
			if(temp_VA.Re() > VA_Efficiency){
				temp_VA.SetReal(VA_Efficiency);
				temp_VA.SetImag(0.0);
			} else {
				if(Q_Out > 0.0){
					temp_VA.SetImag(sqrt(pow(VA_Efficiency,2.0) - pow(temp_VA.Re(), 2.0)));
				} else {
					temp_VA.SetImag(-sqrt(pow(VA_Efficiency,2.0) - pow(temp_VA.Re(), 2.0)));
				}
			}
	...
	// update control references deltamode volt_var
	else if(four_quadrant_control_mode == FQM_VOLT_VAR){
		if ((phases & 0x10) == 0x10){
			power_A.SetReal(VA_Efficiency);
			VA_Outref = power_A;
		} else {
			VA_Outref = complex(0,0);
			if ((phases & 0x01) == 0x01) {
				power_A.SetReal(VA_Efficiency/number_of_phases_out);
				VA_Outref += power_A;
			}
	...
	// update references
	inv_state->Pref = VA_Outref.Re();
	inv_state->Qref = VA_Outref.Im();
}
