...
inverter::inverter(MODULE *module)
{
	if (oclass==nullptr)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_complex, "last_current[0]", PADDR(last_current[0]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (used to remove from parent so XML files look proper) - element 0",
			PT_complex, "last_current[1]", PADDR(last_current[1]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (used to remove from parent so XML files look proper) - element 1",
			PT_complex, "last_current[2]", PADDR(last_current[2]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (used to remove from parent so XML files look proper) - element 2",
			PT_complex, "last_current[3]", PADDR(last_current[3]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (used to remove from parent so XML files look proper) - element 3",
			PT_complex, "last_power[0]", PADDR(last_power[0]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (as constant power) - used to remove from parent so XML looks right - element 0",
			PT_complex, "last_power[1]", PADDR(last_power[1]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (as constant power) - used to remove from parent so XML looks right - element 1",
			PT_complex, "last_power[2]", PADDR(last_power[2]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (as constant power) - used to remove from parent so XML looks right - element 2",
			PT_complex, "last_power[3]", PADDR(last_power[3]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: Previously applied power output (as constant power) - used to remove from parent so XML looks right - element 3",
			...
			PT_complex, "last_I_Out[0]", PADDR(last_I_Out[0]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for last_I_Out[0]",
			PT_complex, "last_I_Out[1]", PADDR(last_I_Out[1]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for last_I_Out[1]",
			PT_complex, "last_I_Out[2]", PADDR(last_I_Out[2]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for last_I_Out[2]",
			...
			PT_complex, "prev_value_IGenerated[0]", PADDR(prev_value_IGenerated[0]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for prev_value_IGenerated[0]",
			PT_complex, "prev_value_IGenerated[1]", PADDR(prev_value_IGenerated[1]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for prev_value_IGenerated[1]",
			PT_complex, "prev_value_IGenerated[2]", PADDR(prev_value_IGenerated[2]), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for prev_value_IGenerated[2]",
				...
				if (childed_connection == true)
				{
					...
					if (!emh::is_element_valid(temp_child_complex_array,0,0))
					{
						...
						for (temp_idx_x=0; temp_idx_x<3; temp_idx_x++)
						{
							for (temp_idx_y=0; temp_idx_y<3; temp_idx_y++)
							{
								temp_child_complex_array(temp_idx_x,temp_idx_y) = gld::complex(0.0,0.0);
							}
						}
					}
					else	//Already populated, make sure it is the right size!
