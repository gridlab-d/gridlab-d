...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  gl_global_create("generators::default_line_voltage", PT_double,
                   &default_line_voltage, PT_UNITS, "V", PT_DESCRIPTION,
                   "line voltage (L-N) to use when no circuit is attached",
                   nullptr);
  gl_global_create("generators::enable_subsecond_models", PT_bool,
                   &enable_subsecond_models, PT_DESCRIPTION,
                   "Enable deltamode capabilities within the generators module",
                   nullptr);
  gl_global_create("generators::all_generator_delta", PT_bool,
                   &all_generator_delta, PT_DESCRIPTION,
                   "Forces all generator objects that are capable to "
                   "participate in deltamode",
                   nullptr);
  gl_global_create("generators::deltamode_timestep", PT_double,
                   &deltamode_timestep_publish, PT_UNITS, "ns", PT_DESCRIPTION,
                   "Desired minimum timestep for deltamode-related simulations",
                   nullptr);
  gl_global_create("generators::default_temperature_value", PT_double,
                   &default_temperature_value, PT_UNITS, "degF", PT_DESCRIPTION,
                   "Temperature when no climate module is detected", nullptr);
...
void schedule_deltamode_start(TIMESTAMP tstart) {
  if (enable_subsecond_models) // Make sure the overall mode is enabled
  {
    ...
    GL_THROW("generators: a call was made to deltamode functions, but "
             "subsecond models are not enabled!");
    /*  TROUBLESHOOT
    The schedule_deltamode_start function was called by an object when
    generators' overall enabled_subsecond_models flag was not set.  The
...
// prioritize = boolean that will elevate current object to the top of the
// generator object deltamode execution list Returns a status - SUCCESS/FAIL
STATUS add_gen_delta_obj(OBJECT *obj, bool prioritize) {
  ...
  temp_gen_obj.preudpate_fxn =
      (FUNCTIONADDR)(gl_get_function(obj, "preupdate_gen_object"));
  temp_gen_obj.interupdate_fxn =
      (FUNCTIONADDR)(gl_get_function(obj, "interupdate_gen_object"));
  temp_gen_obj.post_delta_fxn =
      (FUNCTIONADDR)(gl_get_function(obj, "postupdate_gen_object"));
  ...
  try {
    ...
    gl_error(
        "generator:add_gen_delta_obj - failed allocation for obj:%d-%s - %s",
        obj->id, (obj->name ? obj->name : "Unnamed"), msg);
    return_status = FAILED;
  } catch (...) {
    gl_error("powerflow:add_gen_delta_obj - pre-pass function call: unknown "
             "exception for obj:%d - %s",
...
EXPORT unsigned long preupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models) {
    if (deltamode_timestep_publish <= 0.0) {
      gl_error("generators::deltamode_timestep must be a positive, non-zero "
               "number!");
      /*  TROUBLESHOOT
      The value for deltamode_timestep, as specified as the module level in
      generators, must be a positive, non-zero number. Please use such a number
      and try again.
      */
      ...
      for (curr_object_number = 0; curr_object_number < delta_object.size();
           curr_object_number++) {
        // See if it has a function
        if (delta_object[curr_object_number].preudpate_fxn != nullptr) {
          // Call the function
          status_value = ((STATUS (*)(OBJECT *, TIMESTAMP, unsigned int64))(
              *delta_object[curr_object_number].preudpate_fxn))(
              delta_object[curr_object_number].obj, t0, dt);

          // Make sure we passed
          if (status_value != SUCCESS) {
            GL_THROW("generators - object:%d %s failed to run the object-level "
                     "preupdate function",
                     delta_object[curr_object_number].obj->id,
                     (delta_object[curr_object_number].obj->name
                          ? delta_object[curr_object_number].obj->name
                          : "Unnamed"));
            /*  TROUBLESHOOT
            While attempting to call a preupdate function for a generator
            deltamode object, an error occurred. Please look to the console
  ...
  if (enable_subsecond_models) {
    ...
    for (curr_object_number = 0; curr_object_number < delta_object.size();
      ...
      if (delta_object[curr_object_number].interupdate_fxn != nullptr) {
        ...
        if ((delta_object[curr_object_number].obj->in_svc_double <=
             gl_globaldeltaclock) &&
            (delta_object[curr_object_number].obj->out_svc_double >=
             gl_globaldeltaclock)) {
          // Call the actual function
          function_status = ((SIMULATIONMODE (*)(OBJECT *, unsigned int64,
                                                 unsigned long, unsigned int))(
              *delta_object[curr_object_number].interupdate_fxn))(
              delta_object[curr_object_number].obj, delta_time, dt,
              iteration_count_val);
        ...
        if (function_status == SM_DELTA) {
          gl_verbose(
              "Generator object:%d - %s - requested deltamode to continue",
              delta_object[curr_object_number].obj->id,
              (delta_object[curr_object_number].obj->name
                   ? delta_object[curr_object_number].obj->name
                   : "Unnamed"));
          ...
          gl_verbose(
              "Generator object:%d - %s - requested a deltamode reiteration",
              delta_object[curr_object_number].obj->id,
              (delta_object[curr_object_number].obj->name
                   ? delta_object[curr_object_number].obj->name
                   : "Unnamed"));
          ...
          gl_error(
              "Generator object:%s - deltamode function returned an error!",
              delta_object[curr_object_number].obj->name);
          /*  TROUBLESHOOT
        ...
        {
          gl_verbose(
              "Generator object:%d - %s - requested exit to event-driven mode",
              delta_object[curr_object_number].obj->id,
              (delta_object[curr_object_number].obj->name
                   ? delta_object[curr_object_number].obj->name
                   : "Unnamed"));
...
EXPORT STATUS postupdate(MODULE *module, TIMESTAMP t0, unsigned int64 dt) {
  ...
  if (enable_subsecond_models) {
    ...
    extracted_freq = (double *)gl_get_module_var(gl_find_module("powerflow"),
                                                 "current_frequency");
    ...
    if (extracted_freq == 0) {
      ...
      /*  TROUBLESHOOT
      While performing a deltamode update, mapping the current powerflow
      frequency value failed. Please try again.  If the error persists, please
      submit a bug report via the trac website.
      */
      return FAILED;
    }
    ...
    // Loop through delta objects and update the execution times and frequency
    // values - only does "0" pass
    for (curr_object_number = 0; curr_object_number < delta_object.size();
         curr_object_number++) {
      ...
      if (delta_object[curr_object_number].post_delta_fxn != nullptr) {
        ...
        if ((delta_object[curr_object_number].obj->in_svc_double <=
          ...
          // Call the actual function
          function_status =
              ((STATUS (*)(OBJECT *, gld::complex *, unsigned int))(
                  *delta_object[curr_object_number].post_delta_fxn))(
                  delta_object[curr_object_number].obj, &temp_complex, 0);
        } else // Not in service
          function_status = SUCCESS;

        // Make sure we worked
        if (function_status == FAILED) {
          gl_error("Generator object:%s - failed post-deltamode update",
                   delta_object[curr_object_number].obj->name);
          /*  TROUBLESHOOT
    ...
    return SUCCESS;
  } else { // Not sure how it would ever get here, but just jump out if that's
           // the case
    return SUCCESS;
  }
}
