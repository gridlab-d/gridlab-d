...
int solar::init(OBJECT *parent)
{
	...
	if (deltamode_inclusive)
	{
		...
		else
		{
			...
			{
				GL_THROW("solar:%s - failed to add object to generator deltamode object list", obj->name ? obj->name : "unnamed");
				/*  TROUBLESHOOT
				The solar object encountered an issue while trying to add itself to the generator deltamode object list.  If the error
...
extern "C" MODULE_API int isa_solar(OBJECT *obj, ...) {
  ...
  va_end(args);
  return isa_solar_impl(obj, classsname);
}
#endif
