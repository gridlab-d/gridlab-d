...
ibr_gfl::ibr_gfl(MODULE *module)
{
	if (oclass == nullptr)
	{
		...
		if (gl_publish_variable(oclass,
			...
			// ibr_gfl_NOTE: Any variables that need to be published should go here.
...
int ibr_gfl::init(OBJECT *parent)
{
	...
	else //This particular model isn't enabled
	{
		if (enable_subsecond_models)
		{
			gl_warning("ibr_gfl:%d %s - Deltamode is enabled for the module, but not this object!", obj->id, (obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
