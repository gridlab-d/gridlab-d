...
typedef struct {
  ...
  double V_mea_delayed[3]; // Delay measured terminal voltage values seen by the
                           // inverter
...
private:
  ...
  gld_property *pIGenerated[3]; // Link to direct current injections to
                                // powerflow at bus-level
  gld::complex generator_admittance[3][3]; // Generator admittance matrix
                                           // converted from sequence values
  gld::complex prev_VA_out[3]; // Previous state tracking variable for ramp-rate
                               // calculations
  gld::complex curr_VA_out[3]; // Current state tracking variable for ramp-rate
                               // calculations
  gld::complex value_IGenerated[3]; // Value/accumulator for IGenerated values
  gld::complex prev_value_IGenerated[3]; // Tracking variable for
                                         // Norton-equivalent initializations
  double Pref_prev; // Previous Pref value in the same time step for non-VSI
                    // droop mode ramp-rate calculations
  double Qref_prev[3]; // Previous Qref value in the same time step for non-VSI
                       // droop mode ramp-rate calculations

  double
      current_convergence_criterion; // Convergence criterion for initialization
                                     // of some Norton-equivalent models

  SIMULATIONMODE desired_simulation_mode; // deltamode desired simulation mode
                                          // after corrector pass - prevents
                                          // starting iterations again
...
public:
  ...
  enum LOAD_FOLLOW_STATUS {
    ...
    DISCHARGE = 1,
    CHARGE = 2
  } load_follow_status; // Status variable for what the load_following mode is
                        // doing

  gld::complex VA_Out;
  gld::complex VA_Out_past;
  double P_Out; // P_Out and Q_Out are set by the user as set values to output
                // in CONSTANT_PQ mode
  double Q_Out;
  ...
  double Qref;
  double
      Qref_PI[3]; // Qref is set differently for each phase in PI control mode
  ...
  bool inverter_droop_fp; // Boolean value indicating whether the f/p droop
                          // curve is included in the inverter or not
  ...
  double
      Tvol_delay;  // Time delay for inverter terminal voltage seen by inverter
  ...
  bool inverter_droop_vq; // Boolean value indicating whether the v/q droop
                          // curve is included in the inverter or not
  ...
  double V_angle_past[3]; // Voltage angle  measured at inverter voltage source
                          // behind filter before entering the delta mode
  double V_angle[3]; // Voltage angle measured at inverter voltage source beind
                     // filter after entering the delta mode
  double V_mag_ref[3]; // Initial voltage magnitude of VSI terminal voltage,
  ...
  double Tp_delay; // Time delay for feeder real power changes seen by inverter
                   // droop control
  double Tq_delay; // Time delay for feeder reactive power changes seen by
                   // inverter droop control

  bool checkRampRate_real; // Flag to enable ramp rate/slew rate checking for
                           // active power
  ...
  bool checkRampRate_reactive; // Flag to enable ramp rate/slew rate checking
                               // for reactive power
  double rampUpRate_reactive; // Maximum power increase rate for reactive power
  double rampDownRate_reactive; // Maximum power decrease rate for reactive
                                // power
  ...
  gld::complex last_current[4]; // Previously applied power output (used to
                                // remove from parent so XML files look proper)
  gld::complex
      last_power[4]; // Previously applied power output (as constant power) -
                     // used to remove from parent so XML looks right
  bool islanded; // ces/nas islanding special boolean.

  // properties for multipoint efficiency model. The model used is from Sandia
  // National Laboratory's 2007 paper "Performance Model For Grid-Connected
  ...
  double c_o;   // The parameter describing the curvature of the relationship
              // between ac power and dc power at the reference operating
              // condition. default is 0.
  double c_1; // Coefficient allowing p_dco to vary linearly with dc voltage.
              // default is 0.
  double c_2; // Coefficient allowing p_so to vary linearly with dc voltage.
              // default is 0.
  double c_3; // Coefficient allowing c_o to vary linearly with dc voltage.
              // default is 0.
  double C1;
  double C2;
  double C3;
  ...
  enumeration
      inverter_manufacturer; // known manufacturer to set some presets else use
                             // variables themselves for custom inverter.
  ...
  double excess_input_power; // Variable tracking excess power on the input that
                             // is not placed to the output

  // properties for four quadrant load following control mode
  OBJECT *sense_object; // Object to sense the power level from and provide
                        // adjustments toward
  double max_charge_rate; // Maximum rate of charge for recharging the battery
  double max_discharge_rate; // Maximum discharge rate for extracting energy
                             // from the battery
  ...
  double
      discharge_on_threshold; // Threshold to enable discharging of the battery
  double discharge_off_threshold; // Threshold to disable discharging of the
                                  // battery
  double charge_lockout_time; // Time a charge operation is held before another
                              // dispatch operation is allowed
  double discharge_lockout_time; // Time a discharge operation is held before
                                 // another dispatch operation is allowed

  // properties for pf regulation mode - first control method
  double pf_reg_activate; // Lowest acceptable power-factor level below which
                          // power-factor regulation will activate.
  double pf_reg_deactivate; // Lowest acceptable power-factor above which no
                            // power-factor regulation is needed.

  // properties for pf regulation mode - second control method
  double pf_target_var; // Desired power-factor to maintain (signed)
  double pf_reg_high;   // Upper limit for power factor regulation - if above
                      // (other side), go full opposite
  double pf_reg_low; // Lower limit for power factor regulation - if above, just
                     // leave it be (don't regulator)

  double pf_reg_activate_lockout_time; // Mandatory pause between the
                                       // deactivation of power-factor
                                       // regulation and it reactivation

  // Properties for group load-following
  double charge_threshold; // Level at which all inverters in the group will
                           // begin charging attached batteries. Regulated
                           // minimum load level.
  double discharge_threshold; // Level at which all inverters in the group will
                              // begin discharging attached batteries. Regulated
                              // maximum load level.
  double group_max_charge_rate; // Sum of the charge rates of the inverters
                                // involved in the group load-following.
  double
      group_max_discharge_rate; // Sum of the discharge rates of the inverters
                                // involved in the group load-following.
  double
      group_rated_power; // Sum of the inverter power ratings of the inverters
                         // involved in the group power-factor regulation.

  double inverter_convergence_criterion; // The convergence criteria for the
                                         // dynamic inverter to exit deltamode

  TIMESTAMP inverter_start_time;
  bool inverter_first_step;
  int64 first_iteration_current_injection; // Initialization variable - mostly
                                           // so SWING_PQ buses initalize
                                           // properly for deltamode
  ...
  // 1547 variables
  bool enable_1547_compliance; // Flag to enable IEEE 1547-2003 condition
                               // checking
  double
      reconnect_time; // Time after a 1547 violation clears before we reconnect
  bool inverter_1547_status; // Flag to indicate if we are online, or
                             // "curtailed" due to 1547 mapping
  ...
  double under_voltage_lowest_voltage_setpoint; // Lowest voltage threshold for
                                                // undervoltage
  double under_voltage_middle_voltage_setpoint; // Middle-lowest voltage
                                                // threshold for undervoltage
  double under_voltage_high_voltage_setpoint; // High value of low voltage
                                              // threshold for undervoltage
  ...
  double under_voltage_lowest_delay; // Lowest voltage clearing time for
                                     // undervoltage
  double under_voltage_middle_delay; // Middle-lowest voltage clearing time for
                                     // undervoltage
  double under_voltage_high_delay; // Highest voltage clearing time for
                                   // undervoltage
  double over_voltage_low_delay; // Lowest voltage clearing time for overvoltage
  double
      over_voltage_high_delay; // Highest voltage clearing time for overvoltage
  double under_voltage_lowest_viol_time; // Lowest low voltage threshold
                                         // violation accumulator
  double under_voltage_middle_viol_time; // Middle low voltage threshold
                                         // violation accumulator
  double under_voltage_high_viol_time; // Highest low voltage threshold
                                       // violation accumulator
  double over_voltage_low_viol_time; // Lowest high voltage threshold violation
                                     // accumulator
  double over_voltage_high_viol_time; // Highest high voltage threshold
                                      // violation accumulator
  ...
  bool disable_volt_var_if_no_input_power; // if true turn off Volt/VAr behavior
                                           // when no input power (i.e. at night
                                           // for a solar system)
  double delay_time; // delay time time between seeing a voltage value and
                     // responding with appropiate VAr setting (seconds)
  double max_var_slew_rate; // maximum rate at which inverter can change its VAr
                            // output (VAr/second) *not sure if this is defined
                            // anywhere else i.e. power electronics classes
  double max_pwr_slew_rate; // maximum rate at which inverter can change its
                            // power output for frequency regulation (W/second)
                            // *not sure if this is defined anywhere else i.e.
  ...
  std::vector<std::pair<double, double>>
      VoltVArSched; // Volt/VAr schedule -- i realize I'm using goofball data
                    // types, what would be the GridLABD-esque way of
                    // implementing this data type?
  std::vector<std::pair<double, double>>
      freq_pwrSched; // freq-power schedule -- i realize I'm using goofball data
                     // types, what would be the GridLABD-esque way of
                     // implementing this data type?
private:
  // Comaptibility variables - used to be in power_electronics
  bool parent_is_a_meter; // Boolean to indicate if the parent object is a
                          // meter/triplex_meter
  bool parent_is_triplex; // Boolean to indicate if the parent object is
                          // triplex-oriented (for variable exchange)
  enumeration attached_bus_type; // Determines attached bus type - mostly for
                                 // VSI and grid-forming functionality

  FUNCTIONADDR
      swing_test_fxn; // Function to map to swing testing function, if needed
  ...
  gld_property
      *pLine_unrotI[3];    ///< pointer to the three pre-rotated current fields
  ...
  gld_property
      *pMeterStatus; ///< Pointer to service_status variable on meter parent

  // Default or "connecting point" values for powerflow interactions
  gld::complex
      value_Circuit_V[3]; ///< value holeder for the three L-N voltage fields
  gld::complex value_Line_I[3]; ///< value holeder for the three current fields
  gld::complex value_Line_unrotI[3]; ///< value holeder for the three
                                     ///< pre-rotated current fields
  gld::complex
      value_Power[3];         ///< value holeder for power value on meter parent
  ...
  enumeration value_MeterStatus; ///< value holder for service_status variable
                                 ///< on meter parent
  gld::complex value_Meter_I[3]; ///< value holder for meter measured current on
                                 ///< three lines
  ...
  FUNCTIONADDR
      powerCalc; // Address for power_calculate in link object, if it is a link
  bool sense_is_link; // Boolean flag for if the sense object is a link or a
                      // node
  gld_property *sense_power; // Link to measured power value fo sense_object
  double lf_dispatch_power;  // Amount of real power to try and dispatch to meet
                            // thresholds
  TIMESTAMP next_update_time; // TIMESTAMP of next dispatching change allowed
  bool lf_dispatch_change_allowed; // Flag to indicate if a change in dispatch
                                   // is allowed

  // pf regulation variables
  bool pf_reg_dispatch_change_allowed; // Flag to indicate if a change in
                                       // dispatch is allowed for power factor
                                       // regulation
  double pf_reg_dispatch_VAR; //(Reactive only?) power dispatched to meet power
                              //factor regulation threshold.
  TIMESTAMP
      pf_reg_next_update_time; // TIMESTAMP of next dispatching change allowed
  ...
  // 1547 variables
  double out_of_violation_time_total; // Tracking variable to see how long we've
                                      // been "outside of bad conditions" to
                                      // re-enable the inverter
  gld_property
      *pFrequency; // Pointer to frequency value for checking 1547 compliance
  double value_Frequency;      // Value storage for current frequency value
  double node_nominal_voltage; // Nominal voltage for per-unit-izing for 1547
                               // checks
  double ieee_1547_double; // Deltamode tracker - made global for "off-cycle"
                           // checks

  // Feeder frequency determined by the inverters
  gld_property *mapped_freq_variable; // Mapping to frequency variable in
                                      // powerflow module - deltamode updates

  gld_property *pbus_full_Y_mat; // Link to the full_Y bus variable -- used for
                                 // Norton equivalents
  gld_property
      *pGenerated; // Link to pGenerated value - used for Norton equivalents

  // VSI mode tracker - used to initialize current injection pre-deltamode
  bool VSI_esource_init;

  double ki_Vterminal; ///< The integrator gain for the VSI terminal voltage
                       ///< modulation
  double kp_Vterminal; ///< The proportional gain for the VSI terminal voltage
                       ///< modulation
...
public:
  ...
  SIMULATIONMODE inter_deltaupdate(unsigned int64 delta_time, unsigned long dt,
                                   unsigned int iteration_count_val);
