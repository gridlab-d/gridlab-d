...
rectifier::rectifier(MODULE *module) {
  if (oclass == nullptr) {
    oclass = gl_register_class(module, "rectifier", sizeof(rectifier),
    ...
    if (gl_publish_variable(
            ...
            PT_KEYWORD, "ONE_PULSE", (enumeration)ONE_PULSE, PT_KEYWORD,
            "TWO_PULSE", (enumeration)TWO_PULSE, PT_KEYWORD, "THREE_PULSE",
            (enumeration)THREE_PULSE, PT_KEYWORD, "SIX_PULSE",
            (enumeration)SIX_PULSE, PT_KEYWORD, "TWELVE_PULSE",
            (enumeration)TWELVE_PULSE,

            PT_enumeration, "generator_mode", PADDR(gen_mode_v), PT_KEYWORD,
            "UNKNOWN", (enumeration)UNKNOWN, PT_KEYWORD, "CONSTANT_V",
            ...
            PT_complex, "V_Out[V]", PADDR(V_Out), PT_double, "V_Rated[V]",
            PADDR(V_Rated), PT_complex, "I_Out[A]", PADDR(I_Out), PT_complex,
            "VA_Out[VA]", PADDR(VA_Out), PT_complex, "voltage_A[V]",
            PADDR(voltage_out[0]), PT_complex, "voltage_B[V]",
            PADDR(voltage_out[1]), PT_complex, "voltage_C[V]",
            PADDR(voltage_out[2]), PT_complex, "current_A[V]",
            PADDR(current_out[0]), PT_complex, "current_B[V]",
            PADDR(current_out[1]), PT_complex, "current_C[V]",
            PADDR(current_out[2]), PT_complex, "power_out_A[VA]",
            PADDR(power_out[0]), PT_complex, "power_out_B[VA]",
            PADDR(power_out[1]), PT_complex, "power_out_C[VA]",
            PADDR(power_out[2]),

            // PT_int64, "generator_mode_choice", PADDR(generator_mode_choice),

            PT_set, "phases", PADDR(phases), PT_KEYWORD, "A", (gld::set)PHASE_A,
            PT_KEYWORD, "B", (gld::set)PHASE_B, PT_KEYWORD, "C",
            (gld::set)PHASE_C, PT_KEYWORD, "N", (gld::set)PHASE_N, PT_KEYWORD,
            "S", (gld::set)PHASE_S, nullptr) < 1)
...
int rectifier::init(OBJECT *parent) {
  ...
  if (parent != nullptr && gl_object_isa(parent, "inverter")) {
    ...
    if (!pCircuit_V->is_valid() || !pCircuit_V->is_double()) {
      GL_THROW("rectifier:%d - %s - Unable to map parent inverter property",
               obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      While attempting to map the parent inverter property, the rectifier
      encoutnered an error.  Please try again. If the error persists, please
      submit your model and information via the ticketing system.
      */
    }
    ...
    GL_THROW("Rectifier:%d - %s -- Rectifiers must be parented to inverters",
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
    A rectifier either lacks a parent, or has a parent that is not an inverter.
    Rectifiers only support inverter objects as parents, at this time.  Please
    parent it to an inverter.
    */
  }
...
TIMESTAMP rectifier::presync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
...
TIMESTAMP rectifier::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  case SUPPLY_DRIVEN: {
    ...
    // Write the current
    // NOTE: This isn't checked/bounded because right now this only works with
    // an inverter as a parent (no unparanted or other)
