...
rectifier::rectifier(MODULE *module)
{
    if (oclass == nullptr)
    {
        ...
        if (gl_publish_variable(
                ...
                PT_double, "VA_Out[VA]", PADDR(VA_Out),
                PT_double, "voltage_A[V]", PADDR(voltage_out[0]),
                PT_double, "voltage_B[V]", PADDR(voltage_out[1]),
                PT_double, "voltage_C[V]", PADDR(voltage_out[2]),
                PT_double, "current_A[V]", PADDR(current_out[0]),
                PT_double, "current_B[V]", PADDR(current_out[1]),
                PT_double, "current_C[V]", PADDR(current_out[2]),
                PT_double, "power_out_A[VA]", PADDR(power_out[0]),
                PT_double, "power_out_B[VA]", PADDR(power_out[1]),
                PT_double, "power_out_C[VA]", PADDR(power_out[2]),
...
int rectifier::init(OBJECT *parent)
{
    ...
    if (parent != nullptr)
    {
        ...
        {
            char objname[256];
            gl_verbose("rectifier::init(): deferring initialization on %s", gl_name(parent, objname, 255));
            return 2; // defer
        }
    }
