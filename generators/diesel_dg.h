...
typedef struct {
  ...
  //	double x_cvr;			// State variable for PI controller for
  //CVR control 	double xerr_cvr;		// State variable for PID
  //controller for CVR control
...
typedef struct {
  ...
  double w_ref; // Reference frequency/bias for generator object (governor), but
                // rad/s (align with other generators)
  double f_set; // Reference frequency in Hz
  double vset;  // Reference per-unit voltage/bias for generator object (AVR)
  double vseta; // Reference per-unit voltage/bias for generator object (AVR)
                // before going into bound check
  double vsetb; // Reference per-unit voltage/bias for generator object (AVR)
                // after going into bound check
  double vadd; // Additioal value added to the field voltage before going into
               // the bound
  double vadd_a; // Additioal value added to the field voltage before going into
                 // the bound before going into bound check
  double Pref; // Reference real power output/bias (per-unit) for generator
               // object (governor)
  double Qref; // Reference reactive power output/bias (per-unit) for generator
               // object (AVR)
} MAC_INPUTS;
...
private:
  ...
  gld::complex value_prev_Power[3]; /// Storage variable for previous power -
                                    /// mostly for accumulator handling
  ...
  bool first_run; ///< Flag for first run of the diesel_dg object - eliminates
                  ///< t0==0 dependence
  bool only_first_init;    ///< Flag to indicate if we should limit the dynamics
                           ///< initialization to only the very first run of
                           ///< deltamode
  bool first_init_status;  ///< Flag to see if that first init has actually
                           ///< occurred
  bool is_isochronous_gen; ///< Flag to indicate if we're isochronous, mostly to
                           ///< help keep us in deltamode
  ...
  gld_property *pbus_full_Y_mat; // Link to the full_Y bus variable -- used for
                                 // Norton equivalents
  gld_property *pbus_full_Y_all_mat; // Link to the full_Y_all bus variable --
                                     // used for Norton equivalents
  gld_property *pPGenerated; // Link to bus PGenerated field - mainly used for
                             // SWING generator
  gld_property *pIGenerated[3]; // Link to direct current injections to
                                // powerflow at bus-level (prerot current)
  gld::complex
      value_IGenerated[3]; // Accumulator/holding variable for direct current
                           // injections at bus-level (pre-rotated current)
  gld::complex
      prev_value_IGenerated[3]; // Tracking variable - mostly for initial
                                // powerflow convergence of Norton equivalent
  gld::complex generator_admittance[3][3]; // Generator admittance matrix
                                           // converted from sequence values
  gld::complex full_bus_admittance_mat[3]
                                      [3]; // Full self-admittance of Ybus form
                                           // - pulled from node connection
  ...
  unsigned int torque_delay_write_pos; // Indexing variable for writing the
                                       // torque_delay_buffer
  unsigned int torque_delay_read_pos; // Indexing variable for reading
                                      // torque_delay_buffer
  unsigned int x5a_delayed_len;       // Length of the torque delay buffer
  unsigned int x5a_delayed_write_pos; // Indexing variable for writing the
                                      // torque_delay_buffer
  unsigned int
      x5a_delayed_read_pos; // Indexing variable for reading torque_delay_buffer
  double prev_rotor_speed_val; // Previous value of rotor speed - used for
                               // delta-exiting convergence check
  double prev_voltage_val[3]; // Previous value of voltage magnitude - used for
                              // delta-exiting convergence check
  gld::complex
      last_power_output[3]; // Tracking variable for previous power output -
                            // used to do super-second frequency adjustments
  TIMESTAMP prev_time;  // Tracking variable for previous "new time" run
  double prev_time_dbl; // Tracking variable for previous "new time" run --
                        // deltamode capable
  ...
  double Overload_Limit_Value; // The computed maximum output power, based on
                               // the Rated_VA and the Overload_Limit_Value
...
public:
  ...
  bool gov_ggv1_fsrt_enable; // Enables/disables top fsrt of low-value-select
                             // (load limiter)
  bool gov_ggv1_fsra_enable; // Enables/disables middle fsra of low-value select
                             // (acceleration limiter)
  bool gov_ggv1_fsrn_enable; // Enables/disables lower fsrn of low-value select
  ...
  MAC_INPUTS
      gen_base_set_vals; // Base set points for the various control objects
  MAC_INPUTS gen_base_set_vals_check; // Check to see if set points for the
                                      // various control objects changed.
  PDISPATCH pdispatch_exp; // Dispatch setpoint in p.u., exposed via glm
  bool Vset_defined; // Flag indicating whether Vset has been defined in glm
                     // file or not

  // AVR properties (Simplified Exciter System (SEXS) - Industry acronym, I
  // swear)
  ...
  double ki_Pconstant; // ki for the PI controller implemented in P constant
                       // delta mode
  double kp_Pconstant; // kp for the PI controller implemented in P constant
                       // delta mode

  bool P_constant_mode; // Flag indicating whether P constant mode is imployed
  double ki_Qconstant;  // ki for the PI controller implemented in Q constant
                        // delta mode
  double kp_Qconstant;  // kp for the PI controller implemented in Q constant
                        // delta mode
  ...
  bool CVRenabled; // Flag indicating whether CVR control is enabled or not
                   // inside the exciter
  ...
  //	double gov_gast_TD;				//Governor combustion
  //delay (s)

  // Governor properties (GGOV1)
  double gov_ggv1_r; // Permanent droop, p.u.
  unsigned int
      gov_ggv1_rselect; // Feedback signal for droop, = 1 selected electrical
                        // power, = 0 none (isochronous governor), = -1 fuel
                        // valve stroke ( true stroke),= -2 governor output (
                        // requested stroke)
  double
      gov_ggv1_Tpelec; // Electrical power transducer time constant, sec. (>0.)
  ...
  unsigned int gov_ggv1_Flag; // Switch for fuel source characteristic, = 0 for
  ...
  // double gov_ggv1_rup;				//Maximum rate of load
  // limit increase double gov_ggv1_rdown;			//Maximum rate
  // of load limit decrease

  // P_CONSTANT mode properties
  double
      pconstant_Tpelec; // Electrical power transducer time constant, sec. (>0.)
  ...
  unsigned int pconstant_Flag; // Switch for fuel source characteristic, = 0 for
                               // fuel flow independent of speed, = 1 fuel flow
                               // proportional to speed

  // Fuel useage and emmisions
  bool fuelEmissionCal; // Boolean value indicating whether calculation of fuel
                        // and emissions are enabled
  ...
  TIMESTAMP last_time;
  double dg_1000_a; // Parameter to calculate fuel usage (gal)based on VA power
                    // output (for 1000 kVA rating dg)
  double dg_1000_b; // Parameter to calculate fuel usage (gal)based on VA power
                    // output (for 1000 kVA rating dg)
...
public:
  ...
  void convert_Ypn0_to_Yabc(gld::complex Y0, gld::complex Y1, gld::complex Y2,
                            gld::complex *Yabcmat);
