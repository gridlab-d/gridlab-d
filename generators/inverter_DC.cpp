...
CLASS *inverter_DC::oclass = nullptr;
inverter_DC *inverter_DC::defaults = nullptr;

static PASSCONFIG clockpass = PC_BOTTOMUP;

/* Class registration is only called once to register the class with the core */
inverter_DC::inverter_DC(MODULE *module)
{
	if (oclass == nullptr)
	{
		oclass = gl_register_class(module, "inverter_DC", sizeof(inverter_DC), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
		if (oclass == nullptr)
			throw "unable to register class inverter_DC";
		else
		...
		memset(this, 0, sizeof(inverter_DC));

		if (gl_publish_function(oclass, "preupdate_gen_object", (FUNCTIONADDR)preupdate_inverter_DC) == nullptr)
			GL_THROW("Unable to publish inverter_DC deltamode function");
		if (gl_publish_function(oclass, "interupdate_gen_object", (FUNCTIONADDR)interupdate_inverter_DC) == nullptr)
			GL_THROW("Unable to publish inverter_DC deltamode function");
		// if (gl_publish_function(oclass, "postupdate_gen_object", (FUNCTIONADDR)postupdate_inverter_DC) == nullptr)
		// 	GL_THROW("Unable to publish inverter_DC deltamode function");
		if (gl_publish_function(oclass, "current_injection_update", (FUNCTIONADDR)inverter_DC_NR_current_injection_update) == nullptr)
			GL_THROW("Unable to publish inverter_DC current injection update function");
		if (gl_publish_function(oclass, "register_gen_DC_object", (FUNCTIONADDR)inverter_DC_DC_object_register) == nullptr)
			GL_THROW("Unable to publish inverter_DC DC registration function");
	}
}

//Isa function for identification
int inverter_DC::isa(char *classname)
{
	return strcmp(classname,"inverter_DC")==0;
}

/* Object creation is called once for each object that is created by the core */
int inverter_DC::create(void)
{
	Test_1=909;
	////////////////////////////////////////////////////////
	// DELTA MODE
...
int inverter_DC::init(OBJECT *parent)
{
	...
	if (parent != nullptr)
	{
		...
		{
			char objname[256];
			gl_verbose("inverter_DC::init(): deferring initialization on %s", gl_name(parent, objname, 255));
			return 2; // defer
		}
	}
	...
	{
		S_base = 1000;
		gl_warning("inverter_DC:%d - %s - The rated power of this inverter must be positive - set to 1 kVA.",obj->id,(obj->name ? obj->name : "unnamed"));
		/*  TROUBLESHOOT
		The inverter_DC has a rated power that is negative.  It defaulted to a 1 kVA inverter.  If this is not desired, set the property properly in the GLM.
		*/
	}
	...
	if (parent != nullptr)
	{
			...
			gl_object_isa(parent, "triplex_meter", "powerflow") || gl_object_isa(parent, "triplex_node", "powerflow") || gl_object_isa(parent, "triplex_load", "powerflow"))
		{
			...
			if (deltamode_inclusive)
			{
				...
				if (parent->parent != nullptr)
				{
					...
					{
						GL_THROW("inverter_DC:%s failed to map Norton-equivalence deltamode variable from %s",obj->name?obj->name:"unnamed",parent->name?parent->name:"unnamed");
						//Defined elsewhere
					}
			...
			{
				gl_warning("inverter_DC:%s is in grid-forming mode, but is not a three-phase connection.  This is untested and may not behave properly.", obj->name ? obj->name : "unnamed");
				/*  TROUBLESHOOT
				The inverter_DC was set up to be grid-forming, but is either a triplex or a single-phase-connected inverter.  This implementaiton is not fully tested and may either not
			...
			{
				GL_THROW("inverter_DC:%s failed to map bustype variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
			...
			{
				GL_THROW("inverter_DC:%s failed to map measured_frequency variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
			...
			else if (gl_object_isa(tmp_obj, "meter", "powerflow") || gl_object_isa(tmp_obj, "node", "powerflow") || gl_object_isa(tmp_obj, "load", "powerflow"))
			{
				...
				else //Not three-phase
				{
					...
					{
						GL_THROW("inverter_DC:%s is not connected to a single-phase or three-phase node - two-phase connections are not supported at this time.", obj->name ? obj->name : "unnamed");
						/*  TROUBLESHOOT
						The inverter_DC only supports single phase (A, B, or C or triplex) or full three-phase connections.  If you try to connect it differntly than this, it will not work.
						*/
					}
				} //End non-three-phase
			...
			{
				GL_THROW("inverter_DC:%d %s failed to map the nominal_frequency property", obj->id, (obj->name ? obj->name : "Unnamed"));
				/*  TROUBLESHOOT
			...
			if (deltamode_inclusive)
			{
				if (control_mode != GFL_CURRENT_SOURCE)
				{
					...
					if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_bool())
					{
						GL_THROW("inverter_DC:%s failed to map Norton-equivalence deltamode variable from %s", obj->name ? obj->name : "unnamed", tmp_obj->name ? tmp_obj->name : "unnamed");
						//Defined elsewhere
					}
				...
				else
				{
					...
					if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_bool())
					{
						GL_THROW("inverter_DC:%s failed to map generator deltamode variable from %s", obj->name ? obj->name : "unnamed", tmp_obj->name ? tmp_obj->name : "unnamed");
						/*  TROUBLESHOOT
				...
				if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_double())
				{
					gl_error("inverter_DC:%d %s failed to map the nominal_voltage property", obj->id, (obj->name ? obj->name : "Unnamed"));
					/*  TROUBLESHOOT
				...
				if (control_mode != GFL_CURRENT_SOURCE)
				{
					...
					else //Already populated, make sure it is the right size!
					{
						...
						{
							GL_THROW("inverter_DC:%s exposed Norton-equivalent matrix is the wrong size!", obj->name ? obj->name : "unnamed");
							/*  TROUBLESHOOT
			...
			{
				GL_THROW("inverter_DC failed to map powerflow status variable");
				/*  TROUBLESHOOT
		...
		{
			GL_THROW("inverter_DC must have a valid powerflow object as its parent, or no parent at all");
			/*  TROUBLESHOOT
			Check the parent object of the inverter.  The inverter_DC is only able to be childed via to powerflow objects.
			Alternatively, you can also choose to have no parent, in which case the inverter_DC will be a stand-alone application
	...
	else //Must not have a parent
	{
		...
		gl_warning("inverter_DC:%d has no parent meter object defined; using static voltages", obj->id);
		/*  TROUBLESHOOT
		An inverter_DC in the system does not have a parent attached.  It is using static values for the voltage.
		*/
	...
	if (deltamode_inclusive)
	{
		...
		{
			gl_warning("inverter_DC:%s indicates it wants to run deltamode, but the module-level flag is not set!", obj->name ? obj->name : "unnamed");
			/*  TROUBLESHOOT
			The inverter_DC object has the deltamode_inclusive flag set, but not the module-level enable_subsecond_models flag.  The generator
		...
		else
		{
			...
			{
				GL_THROW("inverter_DC:%s - failed to add object to generator deltamode object list", obj->name ? obj->name : "unnamed");
				/*  TROUBLESHOOT
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				{
					GL_THROW("inverter_DC:%d-%s - initalizing the IEEE 1547 checks failed",get_id(),get_name());
					/*  TROUBLESHOOT
			...
			else	//Grid forming
			{
				...
				//Send a warning
				gl_warning("inverter_DC:%d - %s - Grid-forming inverters do not support IEEE 1547 checks at this time - deactivated",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	else //This particular model isn't enabled
	{
		if (enable_subsecond_models)
		{
			gl_warning("inverter_DC:%d %s - Deltamode is enabled for the module, but not this inverter!", obj->id, (obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
			The inverter_DC is not flagged for deltamode operations, yet deltamode simulations are enabled for the overall system.  When deltamode
			triggers, this inverter_DC may no longer contribute to the system, until event-driven mode resumes.  This could cause issues with the simulation.
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				gl_warning("inverter_DC:%d-%s - IEEE 1547 checks are enabled, but the model is not deltamode-enabled",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	{
		gl_warning("inverter_DC:%d %s - The output apparent power is larger than the rated apparent power, P and Q are capped", obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
TIMESTAMP inverter_DC::presync(TIMESTAMP t0, TIMESTAMP t1)
{
...
TIMESTAMP inverter_DC::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
	...
	if (first_sync_delta_enabled) //Deltamode first pass
	{
		...
		if (deltamode_inclusive && enable_subsecond_models) //We want deltamode - see if it's populated yet
		{
			...
			if (parent_is_a_meter)
			{
				...
				if (test_fxn == nullptr)
				{
					GL_THROW("inverter_DC:%s - failed to map additional current injection mapping for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
void inverter_DC::update_iGen(gld::complex VA_Out)
{
	char loop_var;
...
void inverter_DC::check_and_update_VA_Out(OBJECT *obj)
{
	if (grid_forming_mode == DYNAMIC_DC_BUS)
	{
		// Update the P_DC
		...
		if (!dc_interface_objects.empty())
		{
			...
			for (temp_idx = 0; temp_idx < dc_interface_objects.size(); temp_idx++)
			{
				...
				if (fxn_return_status == FAILED)
				{
					...
					//Error it up
					GL_THROW("inverter_DC:%d - %s - DC object update for object:%d - %s - failed!", obj->id, (obj->name ? obj->name : "Unnamed"), temp_obj->id, (temp_obj->name ? temp_obj->name : "Unnamed"));
					//Defined above
				}
			}
		}
...
TIMESTAMP inverter_DC::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
...
//////////////////////////////////////////////////////////////////////////
//Preupdate
STATUS inverter_DC::pre_deltaupdate(TIMESTAMP t0, unsigned int64 delta_time)
{
	STATUS stat_val;
	...
	if (stat_val != SUCCESS)
	{
		gl_error("inverter_DC failed pre_deltaupdate call");
		/*  TROUBLESHOOT
		While attempting to call the pre_deltaupdate portion of the inverter_DC code, an error
	...
	if (control_mode == GRID_FORMING)
	{
		...
		if (funadd == nullptr)
		{
			gl_error("inverter_DC:%s -- Failed to find node swing swapper function", (hdr->name ? hdr->name : "Unnamed"));
			/*  TROUBLESHOOT
			While attempting to map the function to change the swing status of the parent bus, the function could not be found.
			Ensure the inverter_DC is actually attached to something.  If the error persists, please submit your code and a bug report
		...
		{
			gl_error("Failed to swap SWING status of node:%s on inverter_DC:%s", (hdr->parent->name ? hdr->parent->name : "Unnamed"), (hdr->name ? hdr->name : "Unnamed"));
			/*  TROUBLESHOOT
...
//Module-level call
SIMULATIONMODE inverter_DC::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	...
	if (control_mode == GRID_FORMING)
	{
				...
				if (!dc_interface_objects.empty())
				{
					...
					for (temp_dc_idx = 0; temp_dc_idx < dc_interface_objects.size(); temp_dc_idx++)
					{
						...
						if (fxn_DC_status == FAILED)
						{
							...
							//Error it up
							GL_THROW("inverter_DC:%d - %s - DC object update for object:%d - %s - failed!", obj->id, (obj->name ? obj->name : "Unnamed"), temp_DC_obj->id, (temp_DC_obj->name ? temp_DC_obj->name : "Unnamed"));
							//Defined elsewhere
						}
					}
		...
		else if (iteration_count_val == 1) // Corrector pass
		{
			if (parent_is_single_phase) // single phase/split-phase implementation
			{
				...
				memcpy(&curr_state, &next_state, sizeof(INV_DC_STATE));
		...
		if ((value_MeterStatus == 1) && inverter_1547_status)
		{
			...
			if (iteration_count_val == 0) // Predictor pass
			{
				...
				if (parent_is_single_phase) // single phase/split-phase implementation
				{
					...
					{
						Pref = S_base;
						gl_warning("inverter_DC:%d %s - The dispatched active power is larger than the rated apparent power, the output active power is capped at the rated apparent power", obj->id, (obj->name ? obj->name : "Unnamed"));
						/*  TROUBLESHOOT
						The active power Pref for the inverter_DC is above the rated value.  It has been capped at the rated value.
						*/
					}
					...
					else //When volt-var is disabled, Q is limited by Qref
					{
						if(frequency_watt)
						{
							...
							{
								Qref = sqrt(S_base*S_base - Pref*Pref);
								gl_warning("inverter_DC:%d %s - The inverter output apparent power is larger than the rated apparent power, the output reactive power is capped", obj->id, (obj->name ? obj->name : "Unnamed"));
								/*  TROUBLESHOOT
								The reactive power Qref for the inverter_DC is above the rated value.  It has been capped at the rated value minus any active power output.
								*/
							}
...
STATUS inverter_DC::init_dynamics(INV_DC_STATE *curr_time)
{
	OBJECT *obj = OBJECTHDR(this);
...
// STATUS inverter_DC::post_deltaupdate(gld::complex *useful_value, unsigned int mode_pass)
// {
// 	//If we have a meter, reset the accumulators
// 	if (parent_is_a_meter)
// 	{
...
//Map Complex value
gld_property *inverter_DC::map_complex_value(OBJECT *obj, const char *name)
{
	...
	{
		GL_THROW("inverter_DC:%d %s - Unable to map property %s from object:%d %s", objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name, obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
//Map double value
gld_property *inverter_DC::map_double_value(OBJECT *obj, const char *name)
{
	gld_property *pQuantity;
	OBJECT *objhdr = OBJECTHDR(this);
...
void inverter_DC::pull_complex_powerflow_values(void)
{
...
void inverter_DC::reset_complex_powerflow_accumulators(void)
{
	int indexval;
...
void inverter_DC::push_complex_powerflow_values(bool update_voltage)
{
...
STATUS inverter_DC::updateCurrInjection(int64 iteration_count,bool *converged_failure)
{
	double temp_time;
	OBJECT *obj = OBJECTHDR(this);
	gld::complex temp_VA;
	...
	if (parent_is_a_meter)
	{
		...
		if (attached_bus_type > 1) //SWING or SWING_PQ
		{
			...
			if (swing_test_fxn == nullptr)
			{
				...
				if (swing_test_fxn == nullptr)
				{
					GL_THROW("inverter_DC:%s - failed to map swing-checking for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
STATUS inverter_DC::DC_object_register(OBJECT *DC_object)
{
	FUNCTIONADDR temp_add = nullptr;
	DC_inv_OBJ_FXNS temp_DC_struct;
	OBJECT *obj = OBJECTHDR(this);
	...
	{
		gl_error("inverter_DC:%s - failed to map DC update for object %s", (obj->name ? obj->name : "unnamed"), (DC_object->name ? DC_object->name : "unnamed"));
		/*  TROUBLESHOOT
    ...
    if (gl_object_isa(DC_object,"energy_storage","generators"))
    {
		...
		{
			GL_THROW("inverter_DC:%s - failed to map battery SOC ", (obj->name ? obj->name : "unnamed"));
			/*  TROUBLESHOOT
...
STATUS inverter_DC::initalize_IEEE_1547_checks(void)
{
	...
	else
	{
		...
		gl_warning("inverter_DC:%d - %s does not have a valid parent - 1547 checks have been disabled",get_id(),get_name());
		/*  TROUBLESHOOT
...
double inverter_DC::perform_1547_checks(double timestepvalue)
{
	...
	if ((value_Frequency > IEEE1547_over_freq_low_band_setpoint) || (value_Frequency < IEEE1547_under_freq_high_band_setpoint))
	{
		...
		{
			GL_THROW("inverter_DC:%d - %s - IEEE-1547 Checks - invalid  state!",get_id(),get_name());
			/*  TROUBLESHOOT
	...
	if (!inverter_1547_status)
	{
		if (IEEE1547_out_of_violation_time_total > IEEE1547_reconnect_time)
		{
			...
			{
				gl_warning("inverter_DC - Reconnections after an IEEE-1547 cessation are not fully validated.  May cause odd transients.");
				/*  TROUBLESHOOT
				The simple/base IEEE-1547 functionality in the inverter_DC object only handles the cessation/disconnect side.  Upon reconnecting,
...
EXPORT int create_inverter_DC(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(inverter_DC::oclass);
		if (*obj != nullptr)
		{
			inverter_DC *my = OBJECTDATA(*obj, inverter_DC);
	...
	CREATE_CATCHALL(inverter_DC);
}

EXPORT int init_inverter_DC(OBJECT *obj, OBJECT *parent)
{
	try
	{
		if (obj != nullptr)
			return OBJECTDATA(obj, inverter_DC)->init(parent);
		else
			return 0;
	}
	INIT_CATCHALL(inverter_DC);
}

EXPORT TIMESTAMP sync_inverter_DC(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	TIMESTAMP t2 = TS_NEVER;
	inverter_DC *my = OBJECTDATA(obj, inverter_DC);
	try
	...
	SYNC_CATCHALL(inverter_DC);
	return t2;
}

EXPORT int isa_inverter_DC(OBJECT *obj, char *classname)
{
	return OBJECTDATA(obj,inverter_DC)->isa(classname);
}

EXPORT STATUS preupdate_inverter_DC(OBJECT *obj, TIMESTAMP t0, unsigned int64 delta_time)
{
	inverter_DC *my = OBJECTDATA(obj, inverter_DC);
	...
	{
		gl_error("preupdate_inverter_DC(obj=%d;%s): %s", obj->id, (obj->name ? obj->name : "unnamed"), msg);
		return status_output;
	}
}

EXPORT SIMULATIONMODE interupdate_inverter_DC(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	inverter_DC *my = OBJECTDATA(obj, inverter_DC);
	...
	{
		gl_error("interupdate_inverter_DC(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
		return status;
	}
}

// EXPORT STATUS postupdate_inverter_DC(OBJECT *obj, gld::complex *useful_value, unsigned int mode_pass)
// {
// 	inverter_DC *my = OBJECTDATA(obj, inverter_DC);
...
// 	catch (char *msg)
// 	{
// 		gl_error("postupdate_inverter_DC(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
// 		return status;
// 	}
// }

//// Define export function that update the VSI current injection IGenerated to the grid
EXPORT STATUS inverter_DC_NR_current_injection_update(OBJECT *obj, int64 iteration_count, bool *converged_failure)
{
	STATUS temp_status;
...
EXPORT STATUS inverter_DC_DC_object_register(OBJECT *this_obj, OBJECT *DC_obj)
{
	STATUS temp_status;

	//Map us
	inverter_DC *this_inv = OBJECTDATA(this_obj, inverter_DC);
