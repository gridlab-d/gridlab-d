...
public:
  ...
  enum SOLAR_POWER_MODEL { BASEEFFICIENT = 0, FLATPLATE = 1, PV_CURVE = 2 };
  ...
  // Variables for temperature correction - obtained from Sandia database for
  // module types
  double module_acoeff;  // Temperature correction coefficient a
  double module_bcoeff;  // Temperature correction coefficient b
  double module_dTcoeff; // Temperature difference coefficient associated with
                         // insolation heating
  ...
  bool fix_angle_lat;         // Fix tilt angle to latitude (replicates NREL SAM
                      // function)
  double soiling_factor; // Soiling factor to be applied - makes user
                         // specifiable
  ...
  FUNCTIONADDR calc_solar_radiation; // Function pointer to climate's calculate
                                     // solar radiation in degrees
...
private:
  double orientation_azimuth_corrected; // Corrected azimuth, for crazy
                                        // "0=equator" referenced models
