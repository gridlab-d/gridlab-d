...
typedef struct
{
    ...
    double Pref_ctrl;  // Reference real power output/bias (per-unit) for
                       // controller_dg
    double wref_ctrl;  // Reference frequency/bias for conrtoller_dg

    double Vset_ref; // Vset reference for each DG, values obtained at the
                     // begining of the delta mode
    double x_QV;
    double Vset_ctrl;
...
private:
    ...
    double *prev_Pref_val; // Previous value of x - used for delta-exiting
                           // convergence check
    double *prev_Vset_val; // Previous value of Vset - used for delta-exiting
                           // convergence check

    int dgswitchFound; // Index for storing found switches that connected to the
                       // generators
    ...
    bool deltamode_inclusive;     // Boolean for deltamode calls - pulled from object
                                  // flags
    double *mapped_freq_variable; // Mapping to frequency variable in powerflow
                                  // module - deltamode updates
...
public:
    ...
    STATUS apply_dynamics(CTRL_VARS *curr_time, CTRL_VARS *curr_delta,
                          double deltaT, int index);
