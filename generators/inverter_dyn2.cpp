...
CLASS *inverter_dyn2::oclass = nullptr;
inverter_dyn2 *inverter_dyn2::defaults = nullptr;
...
inverter_dyn2::inverter_dyn2(MODULE *module)
{
	if (oclass == nullptr)
	{
		...
		if (gl_publish_function(oclass, "preupdate_gen_object", (FUNCTIONADDR)preupdate_inverter_dyn2) == nullptr)
			GL_THROW("Unable to publish inverter_dyn2 deltamode function");
		if (gl_publish_function(oclass, "interupdate_gen_object", (FUNCTIONADDR)interupdate_inverter_dyn2) == nullptr)
			GL_THROW("Unable to publish inverter_dyn2 deltamode function");
		// if (gl_publish_function(oclass, "postupdate_gen_object", (FUNCTIONADDR)postupdate_inverter_dyn2) == nullptr)
		// 	GL_THROW("Unable to publish inverter_dyn2 deltamode function");
		if (gl_publish_function(oclass, "current_injection_update", (FUNCTIONADDR)inverter_dyn2_NR_current_injection_update) == nullptr)
			GL_THROW("Unable to publish inverter_dyn2 current injection update function");
		if (gl_publish_function(oclass, "register_gen_DC_object", (FUNCTIONADDR)inverter_dyn2_DC_object_register) == nullptr)
...
int inverter_dyn2::init(OBJECT *parent)
{
	...
	if (parent != nullptr)
	{
			...
			gl_object_isa(parent, "triplex_meter", "powerflow") || gl_object_isa(parent, "triplex_node", "powerflow") || gl_object_isa(parent, "triplex_load", "powerflow"))
		{
			...
			{
				GL_THROW("inverter_dyn2:%s failed to map measured_frequency variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
	...
	if (deltamode_inclusive)
	{
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				{
					GL_THROW("inverter_dyn2:%d-%s - initalizing the IEEE 1547 checks failed",get_id(),get_name());
					/*  TROUBLESHOOT
			...
			else	//Grid forming
			{
				...
				//Send a warning
				gl_warning("inverter_dyn2:%d - %s - Grid-forming inverters do not support IEEE 1547 checks at this time - deactivated",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	else //This particular model isn't enabled
	{
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				gl_warning("inverter_dyn2:%d-%s - IEEE 1547 checks are enabled, but the model is not deltamode-enabled",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	{
		gl_warning("inverter_dyn2:%d %s - The output apparent power is larger than the rated apparent power, P and Q are capped", obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
void inverter_dyn2::update_iGen(gld::complex VA_Out)
{
	char loop_var;
...
void inverter_dyn2::check_and_update_VA_Out(OBJECT *obj)
{
	...
	if (control_mode == GRID_FORMING)
	{
		...
		else if (iteration_count_val == 1) // Corrector pass
		{
			if (parent_is_single_phase) // single phase/split-phase implementation
			{
				...
				memcpy(&curr_state, &next_state, sizeof(INV_DYN2_STATE));
		...
		if ((value_MeterStatus == 1) && inverter_1547_status)
		{
			...
			if (iteration_count_val == 0) // Predictor pass
			{
				...
				if (parent_is_single_phase) // single phase/split-phase implementation
				{
					...
					{
						Pref = S_base;
						gl_warning("inverter_dyn2:%d %s - The dispatched active power is larger than the rated apparent power, the output active power is capped at the rated apparent power", obj->id, (obj->name ? obj->name : "Unnamed"));
						/*  TROUBLESHOOT
						The active power Pref for the inverter_dyn2 is above the rated value.  It has been capped at the rated value.
						*/
					}
					...
					else //When volt-var is disabled, Q is limited by Qref
					{
						if(frequency_watt)
						{
							...
							{
								Qref = sqrt(S_base*S_base - Pref*Pref);
								gl_warning("inverter_dyn2:%d %s - The inverter output apparent power is larger than the rated apparent power, the output reactive power is capped", obj->id, (obj->name ? obj->name : "Unnamed"));
								/*  TROUBLESHOOT
								The reactive power Qref for the inverter_dyn2 is above the rated value.  It has been capped at the rated value minus any active power output.
								*/
							}
...
STATUS inverter_dyn2::init_dynamics(INV_DYN2_STATE *curr_time)
{
	OBJECT *obj = OBJECTHDR(this);
...
// STATUS inverter_dyn2::post_deltaupdate(gld::complex *useful_value, unsigned int mode_pass)
// {
// 	//If we have a meter, reset the accumulators
// 	if (parent_is_a_meter)
// 	{
...
//Map Complex value
gld_property *inverter_dyn2::map_complex_value(OBJECT *obj, const char *name)
{
...
//Map double value
gld_property *inverter_dyn2::map_double_value(OBJECT *obj, const char *name)
{
	gld_property *pQuantity;
	OBJECT *objhdr = OBJECTHDR(this);
...
STATUS inverter_dyn2::updateCurrInjection(int64 iteration_count,bool *converged_failure)
{
	double temp_time;
	OBJECT *obj = OBJECTHDR(this);
	gld::complex temp_VA;
...
STATUS inverter_dyn2::DC_object_register(OBJECT *DC_object)
{
    ...
    if (gl_object_isa(DC_object,"energy_storage","generators"))
    {
		...
		{
			GL_THROW("inverter_dyn2:%s - failed to map battery SOC ", (obj->name ? obj->name : "unnamed"));
			/*  TROUBLESHOOT
...
STATUS inverter_dyn2::initalize_IEEE_1547_checks(void)
{
	...
	else
	{
		...
		gl_warning("inverter_dyn2:%d - %s does not have a valid parent - 1547 checks have been disabled",get_id(),get_name());
		/*  TROUBLESHOOT
...
double inverter_dyn2::perform_1547_checks(double timestepvalue)
{
	...
	if ((value_Frequency > IEEE1547_over_freq_low_band_setpoint) || (value_Frequency < IEEE1547_under_freq_high_band_setpoint))
	{
		...
		{
			GL_THROW("inverter_dyn2:%d - %s - IEEE-1547 Checks - invalid  state!",get_id(),get_name());
			/*  TROUBLESHOOT
	...
	if (!inverter_1547_status)
	{
		if (IEEE1547_out_of_violation_time_total > IEEE1547_reconnect_time)
		{
			...
			{
				gl_warning("inverter_dyn2 - Reconnections after an IEEE-1547 cessation are not fully validated.  May cause odd transients.");
				/*  TROUBLESHOOT
				The simple/base IEEE-1547 functionality in the inverter_dyn2 object only handles the cessation/disconnect side.  Upon reconnecting,
...
// EXPORT STATUS postupdate_inverter_dyn2(OBJECT *obj, gld::complex *useful_value, unsigned int mode_pass)
// {
// 	inverter_dyn2 *my = OBJECTDATA(obj, inverter_dyn2);
...
EXPORT STATUS inverter_dyn2_NR_current_injection_update(OBJECT *obj, int64 iteration_count, bool *converged_failure)
{
	STATUS temp_status;
