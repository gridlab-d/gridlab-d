...
private:
  ...
  double internal_battery_load; // the power out of the battery on the source
                                // side of the internal resistance
  ...
  bool deltamode_inclusive; // Boolean for deltamode calls - pulled from object
                            // flags
  bool enableDelta; // is true only if battery is use_internal_battery_model,
                    // and its parent inverter is FQM_CONSTANT_PQ mode
...
public:
  ...
  // note battery panel will always operate under the SUPPLY_DRIVEN generator
  // mode
  enum GENERATOR_STATUS { OFFLINE = 1, ONLINE = 2 };
  enumeration gen_status_v;

  enum ADDITIONAL_CONTROLS { AC_NONE = 0, AC_LINEAR_TEMPERATURE = 1 };
...
public:
  ...
  double check_state_change_time_delta(unsigned int64 delta_time,
                                       unsigned long dt);
