#ifndef GLD_GENERATORS_inverter_DC_H_
#define GLD_GENERATORS_inverter_DC_H_
...
//inverter_DC class
class inverter_DC : public gld_object
{
private:
	...
	INV_DC_STATE pred_state; ///< The predictor state of the inverter in delamode
	INV_DC_STATE next_state; ///< The next state of the inverter in delamode
	...
	std::vector<DC_inv_OBJ_FXNS> dc_interface_objects;
	double P_DC;
	double V_DC;
	double I_DC;
	double SOC;
...
public:
	...
	INV_DC_STATE curr_state; ///< The current state of the inverter in deltamode
	...
	STATUS init_dynamics(INV_DC_STATE *curr_time);
	STATUS DC_object_register(OBJECT *DC_object);

public:
	static CLASS *oclass;
	static inverter_DC *defaults;
	static CLASS *plcass;
};

#endif // GLD_GENERATORS_inverter_DC_H_
