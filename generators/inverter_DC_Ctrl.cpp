...
CLASS *inverter_DC_Ctrl::oclass = nullptr;
inverter_DC_Ctrl *inverter_DC_Ctrl::defaults = nullptr;

static PASSCONFIG clockpass = PC_BOTTOMUP;

/* Class registration is only called once to register the class with the core */
inverter_DC_Ctrl::inverter_DC_Ctrl(MODULE *module)
{
	if (oclass == nullptr)
	{
		oclass = gl_register_class(module, "inverter_DC_Ctrl", sizeof(inverter_DC_Ctrl), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
		if (oclass == nullptr)
			throw "unable to register class inverter_DC_Ctrl";
		else
			oclass->trl = TRL_PROOF;                                               // HM: what does TRL_PROOF do?
		...
		if (gl_publish_variable(oclass,
			...
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			PT_bool, "DC_Ctrl", PADDR(DC_Ctrl), PT_DESCRIPTION, "DELTAMODE: Boolean used to indicate whether inverter control the DC-link voltage or not",
			PT_double, "Kp_dc", PADDR(Kp_dc), PT_DESCRIPTION, "DELTAMODE: Proportional gain of the DC-Link PI controller",
			PT_double, "Ki_dc", PADDR(Ki_dc), PT_DESCRIPTION, "DELTAMODE: Integral gain of the DC-Link PI controller",
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		memset(this, 0, sizeof(inverter_DC_Ctrl));

		if (gl_publish_function(oclass, "preupdate_gen_object", (FUNCTIONADDR)preupdate_inverter_DC_Ctrl) == nullptr)
			GL_THROW("Unable to publish inverter_DC_Ctrl deltamode function");
		if (gl_publish_function(oclass, "interupdate_gen_object", (FUNCTIONADDR)interupdate_inverter_DC_Ctrl) == nullptr)
			GL_THROW("Unable to publish inverter_DC_Ctrl deltamode function");
		// if (gl_publish_function(oclass, "postupdate_gen_object", (FUNCTIONADDR)postupdate_inverter_DC_Ctrl) == nullptr)
		// 	GL_THROW("Unable to publish inverter_DC_Ctrl deltamode function");
		if (gl_publish_function(oclass, "current_injection_update", (FUNCTIONADDR)inverter_DC_Ctrl_NR_current_injection_update) == nullptr)
			GL_THROW("Unable to publish inverter_DC_Ctrl current injection update function");
		if (gl_publish_function(oclass, "register_gen_DC_object", (FUNCTIONADDR)inverter_DC_Ctrl_DC_object_register) == nullptr)
			GL_THROW("Unable to publish inverter_DC_Ctrl DC registration function");
	}
}

//Isa function for identification
int inverter_DC_Ctrl::isa(char *classname)
{
	return strcmp(classname,"inverter_DC_Ctrl")==0;
}

/* Object creation is called once for each object that is created by the core */
int inverter_DC_Ctrl::create(void)
{
	Test_1=909;
	////////////////////////////////////////////////////////
	// DELTA MODE
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // Pref=0.0    // Default Pref used in Dc_Ctrl mode as Pref is not supplied by the modeler
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	DC_Ctrl=true;
    Vdc_ref_pu=0.83333;   // Default value for the DC voltage reference in pu
    Kp_dc = 0.1*4;      // Default values of the DC-Link proportional gain
    Ki_dc = 1.0*1.2;      // Deafult values of the DC-Link integrator gain
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	C_pu = 0.1;	 //per unit
	kpVdc = 35;	 // per unit, rad/s - This is for the min Vdc control
	kiVdc = 350; //per unit, rad/s^2 - - This is for the min Vdc control
	kdVdc = 0;	 // per unit, rad - This is for the min Vdc control
	mdc = 1.2;	 //
...
int inverter_DC_Ctrl::init(OBJECT *parent)
{
	...
	if (parent != nullptr)
	{
		...
		{
			char objname[256];
			gl_verbose("inverter_DC_Ctrl::init(): deferring initialization on %s", gl_name(parent, objname, 255));
			return 2; // defer
		}
	}
	...
	{
		S_base = 1000;
		gl_warning("inverter_DC_Ctrl:%d - %s - The rated power of this inverter must be positive - set to 1 kVA.",obj->id,(obj->name ? obj->name : "unnamed"));
		/*  TROUBLESHOOT
		The inverter_DC_Ctrl has a rated power that is negative.  It defaulted to a 1 kVA inverter.  If this is not desired, set the property properly in the GLM.
		*/
	}
	...
	if (parent != nullptr)
	{
			...
			gl_object_isa(parent, "triplex_meter", "powerflow") || gl_object_isa(parent, "triplex_node", "powerflow") || gl_object_isa(parent, "triplex_load", "powerflow"))
		{
			...
			if (deltamode_inclusive)
			{
				...
				if (parent->parent != nullptr)
				{
					...
					{
						GL_THROW("inverter_DC_Ctrl:%s failed to map Norton-equivalence deltamode variable from %s",obj->name?obj->name:"unnamed",parent->name?parent->name:"unnamed");
						//Defined elsewhere
					}
			...
			{
				gl_warning("inverter_DC_Ctrl:%s is in grid-forming mode, but is not a three-phase connection.  This is untested and may not behave properly.", obj->name ? obj->name : "unnamed");
				/*  TROUBLESHOOT
				The inverter_DC_Ctrl was set up to be grid-forming, but is either a triplex or a single-phase-connected inverter.  This implementaiton is not fully tested and may either not
			...
			{
				GL_THROW("inverter_DC_Ctrl:%s failed to map bustype variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
			...
			{
				GL_THROW("inverter_DC_Ctrl:%s failed to map measured_frequency variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
			...
			else if (gl_object_isa(tmp_obj, "meter", "powerflow") || gl_object_isa(tmp_obj, "node", "powerflow") || gl_object_isa(tmp_obj, "load", "powerflow"))
			{
				...
				else //Not three-phase
				{
					...
					{
						GL_THROW("inverter_DC_Ctrl:%s is not connected to a single-phase or three-phase node - two-phase connections are not supported at this time.", obj->name ? obj->name : "unnamed");
						/*  TROUBLESHOOT
						The inverter_DC_Ctrl only supports single phase (A, B, or C or triplex) or full three-phase connections.  If you try to connect it differntly than this, it will not work.
						*/
					}
				} //End non-three-phase
			...
			{
				GL_THROW("inverter_DC_Ctrl:%d %s failed to map the nominal_frequency property", obj->id, (obj->name ? obj->name : "Unnamed"));
				/*  TROUBLESHOOT
			...
			if (deltamode_inclusive)
			{
				if (control_mode != GFL_CURRENT_SOURCE)
				{
					...
					if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_bool())
					{
						GL_THROW("inverter_DC_Ctrl:%s failed to map Norton-equivalence deltamode variable from %s", obj->name ? obj->name : "unnamed", tmp_obj->name ? tmp_obj->name : "unnamed");
						//Defined elsewhere
					}
				...
				else
				{
					...
					if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_bool())
					{
						GL_THROW("inverter_DC_Ctrl:%s failed to map generator deltamode variable from %s", obj->name ? obj->name : "unnamed", tmp_obj->name ? tmp_obj->name : "unnamed");
						/*  TROUBLESHOOT
				...
				if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_double())
				{
					gl_error("inverter_DC_Ctrl:%d %s failed to map the nominal_voltage property", obj->id, (obj->name ? obj->name : "Unnamed"));
					/*  TROUBLESHOOT
				...
				if (control_mode != GFL_CURRENT_SOURCE)
				{
					...
					else //Already populated, make sure it is the right size!
					{
						...
						{
							GL_THROW("inverter_DC_Ctrl:%s exposed Norton-equivalent matrix is the wrong size!", obj->name ? obj->name : "unnamed");
							/*  TROUBLESHOOT
			...
			{
				GL_THROW("inverter_DC_Ctrl failed to map powerflow status variable");
				/*  TROUBLESHOOT
		...
		{
			GL_THROW("inverter_DC_Ctrl must have a valid powerflow object as its parent, or no parent at all");
			/*  TROUBLESHOOT
			Check the parent object of the inverter.  The inverter_DC_Ctrl is only able to be childed via to powerflow objects.
			Alternatively, you can also choose to have no parent, in which case the inverter_DC_Ctrl will be a stand-alone application
	...
	else //Must not have a parent
	{
		...
		gl_warning("inverter_DC_Ctrl:%d has no parent meter object defined; using static voltages", obj->id);
		/*  TROUBLESHOOT
		An inverter_DC_Ctrl in the system does not have a parent attached.  It is using static values for the voltage.
		*/
	...
	if (deltamode_inclusive)
	{
		...
		{
			gl_warning("inverter_DC_Ctrl:%s indicates it wants to run deltamode, but the module-level flag is not set!", obj->name ? obj->name : "unnamed");
			/*  TROUBLESHOOT
			The inverter_DC_Ctrl object has the deltamode_inclusive flag set, but not the module-level enable_subsecond_models flag.  The generator
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				{
					GL_THROW("inverter_DC_Ctrl:%d-%s - initalizing the IEEE 1547 checks failed",get_id(),get_name());
					/*  TROUBLESHOOT
			...
			else	//Grid forming
			{
				...
				//Send a warning
				gl_warning("inverter_DC_Ctrl:%d - %s - Grid-forming inverters do not support IEEE 1547 checks at this time - deactivated",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	else //This particular model isn't enabled
	{
		if (enable_subsecond_models)
		{
			gl_warning("inverter_DC_Ctrl:%d %s - Deltamode is enabled for the module, but not this inverter!", obj->id, (obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
			The inverter_DC_Ctrl is not flagged for deltamode operations, yet deltamode simulations are enabled for the overall system.  When deltamode
			triggers, this inverter_DC_Ctrl may no longer contribute to the system, until event-driven mode resumes.  This could cause issues with the simulation.
		...
		if (enable_1547_compliance)
		{
			...
			if (control_mode != GRID_FORMING)
			{
				...
				gl_warning("inverter_DC_Ctrl:%d-%s - IEEE 1547 checks are enabled, but the model is not deltamode-enabled",get_id(),get_name());
				/*  TROUBLESHOOT
	...
	{
		gl_warning("inverter_DC_Ctrl:%d %s - The output apparent power is larger than the rated apparent power, P and Q are capped", obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
TIMESTAMP inverter_DC_Ctrl::presync(TIMESTAMP t0, TIMESTAMP t1)
{
...
TIMESTAMP inverter_DC_Ctrl::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
	...
	if (first_sync_delta_enabled) //Deltamode first pass
	{
		...
		if (deltamode_inclusive && enable_subsecond_models) //We want deltamode - see if it's populated yet
		{
			...
			if (parent_is_a_meter)
			{
				...
				if (test_fxn == nullptr)
				{
					GL_THROW("inverter_DC_Ctrl:%s - failed to map additional current injection mapping for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
void inverter_DC_Ctrl::update_iGen(gld::complex VA_Out)
{
	char loop_var;
...
TIMESTAMP inverter_DC_Ctrl::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
...
//////////////////////////////////////////////////////////////////////////
//Preupdate
STATUS inverter_DC_Ctrl::pre_deltaupdate(TIMESTAMP t0, unsigned int64 delta_time)
{
	STATUS stat_val;
	...
	if (stat_val != SUCCESS)
	{
		gl_error("inverter_DC_Ctrl failed pre_deltaupdate call");
		/*  TROUBLESHOOT
		While attempting to call the pre_deltaupdate portion of the inverter_DC_Ctrl code, an error
	...
	if (control_mode == GRID_FORMING)
	{
		...
		if (funadd == nullptr)
		{
			gl_error("inverter_DC_Ctrl:%s -- Failed to find node swing swapper function", (hdr->name ? hdr->name : "Unnamed"));
			/*  TROUBLESHOOT
			While attempting to map the function to change the swing status of the parent bus, the function could not be found.
			Ensure the inverter_DC_Ctrl is actually attached to something.  If the error persists, please submit your code and a bug report
		...
		{
			gl_error("Failed to swap SWING status of node:%s on inverter_DC_Ctrl:%s", (hdr->parent->name ? hdr->parent->name : "Unnamed"), (hdr->name ? hdr->name : "Unnamed"));
			/*  TROUBLESHOOT
...
//Module-level call
SIMULATIONMODE inverter_DC_Ctrl::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	...
	if (control_mode == GRID_FORMING)
	{
				...
				if (!dc_interface_objects.empty())
				{
					...
					for (temp_dc_idx = 0; temp_dc_idx < dc_interface_objects.size(); temp_dc_idx++)
					{
						...
						if (fxn_DC_status == FAILED)
						{
							...
							//Error it up
							GL_THROW("inverter_DC_Ctrl:%d - %s - DC object update for object:%d - %s - failed!", obj->id, (obj->name ? obj->name : "Unnamed"), temp_DC_obj->id, (temp_DC_obj->name ? temp_DC_obj->name : "Unnamed"));
							//Defined elsewhere
						}
					}
		...
		else if (iteration_count_val == 1) // Corrector pass
		{
			if (parent_is_single_phase) // single phase/split-phase implementation
			{
				...
				memcpy(&curr_state, &next_state, sizeof(INV_DC_Ctrl_STATE));
	...
	else if ((control_mode == GRID_FOLLOWING) || (control_mode == GFL_CURRENT_SOURCE))
	{
		...
		{
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			if ((iteration_count_val == 0) && (delta_time == 0) && (control_mode == GRID_FOLLOWING) && (DC_Ctrl))
		    {
			  P_DC = I_DC = 0; // Clean the buffer, only on the very first delta timestep
		    }
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			...
			//{
				...
				//In order to keep the rest of the inverter up-to-date, call the DC bus routines
				if ((control_mode == GRID_FOLLOWING) && (DC_Ctrl))
				{
					//Old way in Grid Forming Idc is independednt variable
					//I_dc_pu = P_out_pu / curr_state.Vdc_pu; // Calculate the equivalent dc current, including the dc capacitor

					if (!dc_interface_objects.empty())
					{
						//Old way in grid Forming V_DC is updated as a state of the inverter - Update V_DC, just in case
						//V_DC = curr_state.Vdc_pu * Vdc_base;
						//I_DC= curr_state.Idc_pu * (S_base/Vdc_base)
						//Loop through and call the DC objects
						for (temp_dc_idx = 0; temp_dc_idx < dc_interface_objects.size(); temp_dc_idx++)
						{
							//DC object, calling object (us), init mode (true/false)
                        ...
                        // V_DC is updated now from the DC Object 
						//I_DC is done now.  
					} //End DC object update
				}//End DYNAMIC_DC_BUS
			...
			if (iteration_count_val == 0) // Predictor pass
			{
				...
				if (parent_is_single_phase) // single phase/split-phase implementation
				{
					...
					{
						Pref = S_base;
						gl_warning("inverter_DC_Ctrl:%d %s - The dispatched active power is larger than the rated apparent power, the output active power is capped at the rated apparent power", obj->id, (obj->name ? obj->name : "Unnamed"));
						/*  TROUBLESHOOT
						The active power Pref for the inverter_DC_Ctrl is above the rated value.  It has been capped at the rated value.
						*/
					}
					...
					//Function end
					//++++++++++++++++++++Iteration ==0, Single Phase ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					// DC_Ctrl Function 
					// 1- Update DC Variables
					if (((control_mode == GRID_FOLLOWING) || (control_mode == GFL_CURRENT_SOURCE)) && (DC_Ctrl)) // consider the dynamics of PV dc bus
					{
						//I_dc_pu = P_out_pu / curr_state.Vdc_pu; // Calculate the equivalent dc current, including the dc capacitor
						...
						//OLD: I_PV_pu = I_DC / Idc_base; // Calculate the current from PV panel

						//OLD: pred_state.dVdc_pu = (I_PV_pu - I_dc_pu) / C_pu;

						//OLD: pred_state.Vdc_pu = curr_state.Vdc_pu + pred_state.dVdc_pu * deltat;
						//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
						V_DC_pu=V_DC/Vdc_base;	
						//curr_state.Vdc_pu=V_DC_pu;// Update Current Vdc_pu value
						Pdc_ref_pu = Vdc_ctrl_blk.getoutput(V_DC_pu-Vdc_ref_pu,deltat,PREDICTOR); 
						//if (Pdc_ref_pu > Pref_max)
						//{
						//	Pdc_ref_pu = Pref_max;
						//}

						//if (Pdc_ref_pu < Pref_min)
						//{
						//	Pdc_ref_pu = Pref_min;
						//}	
						//++++++++++++++++++++++++++++++++++++++++++++++++++++++
						// Do not forget to update the dc current state later
					}

					// End of DC_Ctrl, 1- Update 
					...
					else //When volt-var is disabled, Q is limited by Qref
					{
						if(frequency_watt)
						{
							...
							{
								Qref = sqrt(S_base*S_base - Pref*Pref);
								gl_warning("inverter_DC_Ctrl:%d %s - The inverter output apparent power is larger than the rated apparent power, the output reactive power is capped", obj->id, (obj->name ? obj->name : "Unnamed"));
								/*  TROUBLESHOOT
								The reactive power Qref for the inverter_DC_Ctrl is above the rated value.  It has been capped at the rated value minus any active power output.
								*/
							}
					...
					if (control_mode == GRID_FOLLOWING)
					{
						...
						if (DC_Ctrl)
						{
							power_val[0] = e_source[0] * ~value_IGenerated[0]; // Real values
							VA_Out=power_val[0];
							P_DC=VA_Out.Re();
						 	I_DC=VA_Out.Re()/V_DC; // Update I_DC even though the DC-Link will not use it during the predictor stage		
						}
						//+++++++++++++++++++++++++++++++++++++++++++++++++++
					}
					...
					if ((grid_following_mode == BALANCED_POWER)||(grid_following_mode == POSITIVE_SEQUENCE))
					{
						...
						for (i = 0; i < 3; i++)
						{
							...
							if (DC_Ctrl)
							{
								if (grid_following_mode == BALANCED_POWER)
								{
									igd_ref[i] = Pdc_ref_pu / ugd_pu[i];
									igq_ref[i] = -Qref / S_base / ugd_pu[i];
								}
								else if (grid_following_mode == POSITIVE_SEQUENCE)
								{
									igd_ref[i] = Pdc_ref_pu / ugd_pu_PS;
									igq_ref[i] = -Qref / S_base / ugd_pu_PS;
								}
								//+++++++++++++++++++ If volt_var is enabled, igq[i] needs to be recalculated below ++++++++++++++++++++
								if (volt_var)
								{
									if(grid_following_mode == BALANCED_POWER)
									{
							...
							if (control_mode == GRID_FOLLOWING)
							{
								...
								power_val[i] = e_source[i] * ~value_IGenerated[i];								
							...
							else if (control_mode == GFL_CURRENT_SOURCE)
							{
								...
								if(DC_Ctrl)
								{
									terminal_current_val[i] = value_IGenerated[i] - filter_admittance * value_Circuit_V[i];

									//Update per-unit value
									terminal_current_val_pu[i] = terminal_current_val[i] / I_base;
									power_val[i] = value_Circuit_V[i] * ~terminal_current_val[i];
									//Update power output variables, just so we can see what is going on
								}
								//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			...
			else if (iteration_count_val == 1) // Corrector pass
			{
				...
				if (parent_is_single_phase) // single phase/split-phase implementation
				{
					...
					if (((control_mode == GRID_FOLLOWING) || (control_mode == GFL_CURRENT_SOURCE)) && (DC_Ctrl)) // consider the dynamics of PV dc bus
					{
						...
						V_DC_pu=V_DC/Vdc_base;	
						//curr_state.Vdc_pu=V_DC_pu;// Update Current Vdc_pu value
						Pdc_ref_pu = Vdc_ctrl_blk.getoutput(V_DC_pu-Vdc_ref_pu,deltat,CORRECTOR); 
						//if (Pdc_ref_pu > Pref_max)
						//{
						//	Pdc_ref_pu = Pref_max;
						//}
					...
					//+++++++++++++++++++ Iteration 1, Single Phase ++++++++++++++++++++++++++++++++++++++++++
					// Function: Calculate current references - Iteration == 1, Single Phase
					if (DC_Ctrl)
					{
						igd_ref[0] = Pdc_ref_pu / ugd_pu[0];
						igq_ref[0] = -Qref / S_base / ugd_pu[0];
					...
					if(control_mode == GRID_FOLLOWING)
					{
						...
						// Function end
						//                  Iteration == 1, Single Phase 
						// +++++++ Calculate DC Power and Update I_DC: Iterationc== 1, Single Phase +++++++++++++++++++++++++++
						if (DC_Ctrl)
						{
							power_val[0] = e_source[0] * ~value_IGenerated[0]; // Real values
							VA_Out = power_val[0];
				...
				else //Three-phase
				{
					if ((grid_following_mode == BALANCED_POWER)||(grid_following_mode == POSITIVE_SEQUENCE))
					{
						...
						//Function end
						//                     Iteration == 1, Three Phase 
						//++++++++++++ V_DC Update and Control: Iteration==1, Three Phase ++++++++++++++++++++++++++++++++++++++++++++
						// DC_Ctrl Function 
						// 1- Update DC Variables
						if (((control_mode == GRID_FOLLOWING) || (control_mode == GFL_CURRENT_SOURCE)) && (DC_Ctrl)) // consider the dynamics of PV dc bus
						{
...
STATUS inverter_DC_Ctrl::init_dynamics(INV_DC_Ctrl_STATE *curr_time)
{
	OBJECT *obj = OBJECTHDR(this);
		...
		if ((phases & 0x07) == 0x07)
		{
			...
			if (grid_forming_mode == DYNAMIC_DC_BUS) // consider the dynamics of PV dc bus, and the internal voltage magnitude needs to be recalculated
			{
				...
				if (!dc_interface_objects.empty())
				{
					...
					//Theoretically, the DC objects have now set V_DC and I_DC appropriately - updated equations would go here
				} //End DC object update
	...
	else if ((control_mode == GRID_FOLLOWING) || (control_mode == GFL_CURRENT_SOURCE))
	{
		...
		if (parent_is_single_phase) // single phase/split-phase implementation
		{
			...
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			//         Grid-Following:   VDC Dynamic Initialization - Single Phase
...
// STATUS inverter_DC_Ctrl::post_deltaupdate(gld::complex *useful_value, unsigned int mode_pass)
// {
// 	//If we have a meter, reset the accumulators
// 	if (parent_is_a_meter)
// 	{
...
//Map Complex value
gld_property *inverter_DC_Ctrl::map_complex_value(OBJECT *obj, const char *name)
{
	...
	{
		GL_THROW("inverter_DC_Ctrl:%d %s - Unable to map property %s from object:%d %s", objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name, obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
//Map double value
gld_property *inverter_DC_Ctrl::map_double_value(OBJECT *obj, const char *name)
{
	gld_property *pQuantity;
	OBJECT *objhdr = OBJECTHDR(this);
...
void inverter_DC_Ctrl::pull_complex_powerflow_values(void)
{
...
void inverter_DC_Ctrl::reset_complex_powerflow_accumulators(void)
{
	int indexval;
...
void inverter_DC_Ctrl::push_complex_powerflow_values(bool update_voltage)
{
...
STATUS inverter_DC_Ctrl::updateCurrInjection(int64 iteration_count,bool *converged_failure)
{
	double temp_time;
	OBJECT *obj = OBJECTHDR(this);
	gld::complex temp_VA;
	...
	if (parent_is_a_meter)
	{
		...
		if (attached_bus_type > 1) //SWING or SWING_PQ
		{
			...
			if (swing_test_fxn == nullptr)
			{
				...
				if (swing_test_fxn == nullptr)
				{
					GL_THROW("inverter_DC_Ctrl:%s - failed to map swing-checking for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
STATUS inverter_DC_Ctrl::DC_object_register(OBJECT *DC_object)
{
	FUNCTIONADDR temp_add = nullptr;
	DC_Ctrl_OBJ_FXNS temp_DC_struct;
	OBJECT *obj = OBJECTHDR(this);
	...
	{
		gl_error("inverter_DC_Ctrl:%s - failed to map DC update for object %s", (obj->name ? obj->name : "unnamed"), (DC_object->name ? DC_object->name : "unnamed"));
		/*  TROUBLESHOOT
    ...
    if (gl_object_isa(DC_object,"energy_storage","generators"))
    {
		...
		{
			GL_THROW("inverter_DC_Ctrl:%s - failed to map battery SOC ", (obj->name ? obj->name : "unnamed"));
			/*  TROUBLESHOOT
...
STATUS inverter_DC_Ctrl::initalize_IEEE_1547_checks(void)
{
	...
	else
	{
		...
		gl_warning("inverter_DC_Ctrl:%d - %s does not have a valid parent - 1547 checks have been disabled",get_id(),get_name());
		/*  TROUBLESHOOT
...
double inverter_DC_Ctrl::perform_1547_checks(double timestepvalue)
{
	...
	if ((value_Frequency > IEEE1547_over_freq_low_band_setpoint) || (value_Frequency < IEEE1547_under_freq_high_band_setpoint))
	{
		...
		{
			GL_THROW("inverter_DC_Ctrl:%d - %s - IEEE-1547 Checks - invalid  state!",get_id(),get_name());
			/*  TROUBLESHOOT
	...
	if (!inverter_1547_status)
	{
		if (IEEE1547_out_of_violation_time_total > IEEE1547_reconnect_time)
		{
			...
			{
				gl_warning("inverter_DC_Ctrl - Reconnections after an IEEE-1547 cessation are not fully validated.  May cause odd transients.");
				/*  TROUBLESHOOT
				The simple/base IEEE-1547 functionality in the inverter_DC_Ctrl object only handles the cessation/disconnect side.  Upon reconnecting,
...
EXPORT int create_inverter_DC_Ctrl(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(inverter_DC_Ctrl::oclass);
		if (*obj != nullptr)
		{
			inverter_DC_Ctrl *my = OBJECTDATA(*obj, inverter_DC_Ctrl);
	...
	CREATE_CATCHALL(inverter_DC_Ctrl);
}

EXPORT int init_inverter_DC_Ctrl(OBJECT *obj, OBJECT *parent)
{
	try
	{
		if (obj != nullptr)
			return OBJECTDATA(obj, inverter_DC_Ctrl)->init(parent);
		else
			return 0;
	}
	INIT_CATCHALL(inverter_DC_Ctrl);
}

EXPORT TIMESTAMP sync_inverter_DC_Ctrl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	TIMESTAMP t2 = TS_NEVER;
	inverter_DC_Ctrl *my = OBJECTDATA(obj, inverter_DC_Ctrl);
	try
	...
	SYNC_CATCHALL(inverter_DC_Ctrl);
	return t2;
}

EXPORT int isa_inverter_DC_Ctrl(OBJECT *obj, char *classname)
{
	return OBJECTDATA(obj,inverter_DC_Ctrl)->isa(classname);
}

EXPORT STATUS preupdate_inverter_DC_Ctrl(OBJECT *obj, TIMESTAMP t0, unsigned int64 delta_time)
{
	inverter_DC_Ctrl *my = OBJECTDATA(obj, inverter_DC_Ctrl);
	...
	{
		gl_error("preupdate_inverter_DC_Ctrl(obj=%d;%s): %s", obj->id, (obj->name ? obj->name : "unnamed"), msg);
		return status_output;
	}
}

EXPORT SIMULATIONMODE interupdate_inverter_DC_Ctrl(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	inverter_DC_Ctrl *my = OBJECTDATA(obj, inverter_DC_Ctrl);
	...
	{
		gl_error("interupdate_inverter_DC_Ctrl(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
		return status;
	}
}

// EXPORT STATUS postupdate_inverter_DC_Ctrl(OBJECT *obj, gld::complex *useful_value, unsigned int mode_pass)
// {
// 	inverter_DC_Ctrl *my = OBJECTDATA(obj, inverter_DC_Ctrl);
...
// 	catch (char *msg)
// 	{
// 		gl_error("postupdate_inverter_DC_Ctrl(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
// 		return status;
// 	}
// }

//// Define export function that update the VSI current injection IGenerated to the grid
EXPORT STATUS inverter_DC_Ctrl_NR_current_injection_update(OBJECT *obj, int64 iteration_count, bool *converged_failure)
{
	STATUS temp_status;
...
EXPORT STATUS inverter_DC_Ctrl_DC_object_register(OBJECT *this_obj, OBJECT *DC_obj)
{
	STATUS temp_status;

	//Map us
	inverter_DC_Ctrl *this_inv = OBJECTDATA(this_obj, inverter_DC_Ctrl);
