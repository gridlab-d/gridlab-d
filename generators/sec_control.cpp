...
sec_control::sec_control(MODULE *module) {
  if (oclass == NULL) {
    oclass = gl_register_class(module, "sec_control", sizeof(sec_control),
    ...
    if (gl_publish_variable(
            oclass, PT_char1024, "participant_input", PADDR(participant_input),
            PT_DESCRIPTION,
            "command string for creating/modifing secondary controller "
            "participants",
            PT_int32, "participant_count", PADDR(participant_count),
            PT_DESCRIPTION,
            "Number of objects currrently participating in secondary control.",
            // PID controller input
            PT_double, "f0[Hz]", PADDR(f0), PT_DESCRIPTION,
            "Nominal frequency in Hz", PT_double, "underfrequency_limit[Hz]",
            PADDR(underfrequency_limit), PT_DESCRIPTION,
            "Maximum positive input limit to PID controller is f0 - "
            "underfreuqnecy_limit",
            PT_double, "overfrequency_limit[Hz]", PADDR(overfrequency_limit),
            PT_DESCRIPTION,
            "Maximum negative input limit to PID controller is f0 - "
            "overfrequency_limit",
            PT_double, "deadband[Hz]", PADDR(deadband), PT_DESCRIPTION,
            "Deadpand for PID controller input in Hz", PT_double,
            "tieline_tol[pu]", PADDR(tieline_tol), PT_DESCRIPTION,
            "Generic tie-line error tolerance in p.u. (w.r.t set point)",
            PT_double, "B[MW/Hz]", PADDR(B), PT_DESCRIPTION,
            "frequency bias in MW/Hz",
            // PID controller
            PT_double, "kpPID[pu]", PADDR(kpPID), PT_DESCRIPTION,
            "PID proportional gain in pu", PT_double, "kiPID[pu]", PADDR(kiPID),
            PT_DESCRIPTION, "PID integral gain in pu/s", PT_double, "kdPID[pu]",
            PADDR(kdPID), PT_DESCRIPTION, "PID derivative gain in pu*s",

            PT_enumeration, "anti_windup", PADDR(anti_windup), PT_DESCRIPTION,
            "Integrator windup handling", PT_KEYWORD, "NONE", (enumeration)NONE,
            PT_DESCRIPTION, "No anti-windup active", PT_KEYWORD,
            "ZERO_IN_DEADBAND", (enumeration)ZERO_IN_DEADBAND, PT_DESCRIPTION,
            "Zero integrator when frequency is within deadband", PT_KEYWORD,
            "FEEDBACK_PIDOUT", (enumeration)FEEDBACK_PIDOUT, PT_DESCRIPTION,
            "Feedback difference between PIDout and signal passed to "
            "generators",
            PT_KEYWORD, "FEEDBACK_INTEGRATOR", (enumeration)FEEDBACK_INTEGRATOR,
            PT_DESCRIPTION,
            "Feedback difference between PIDout and signal passed to "
            "generators (neglecting proportional and derivative paths)",

            //
            PT_double, "Ts[s]", PADDR(Ts), PT_DESCRIPTION,
            "Secondary controller sampling period in sec.", PT_double,
            "Ts_P[s]", PADDR(Ts_P), PT_DESCRIPTION,
            "Secondary controller input sampling period in sec for "
            "unit/tie-line errors.",
            PT_double, "Ts_f[s]", PADDR(Ts_f), PT_DESCRIPTION,
            "Secondary controller input sampling period in sec for frequency.",
            PT_double, "Tlp[s]", PADDR(Tlp_default), PT_DESCRIPTION,
            "Default low pass time constant for participants in sec. Default "
            "is 0, meaning no low pass filter.",
            //
            PT_bool, "sampleflag", PADDR(sampleflag), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "Boolean flag whether output should be published or not by "
            "secondary controller",
            PT_bool, "sampleflag_P", PADDR(sampleflag_P), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "Boolean flag whether unit error inputs should be sampled", PT_bool,
            "sampleflag_f", PADDR(sampleflag_f), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Boolean flag whether frequency should be sampled",
            PT_double, "sample_time", PADDR(sample_time), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Next update time for the secondary controller",
            PT_double, "sample_P", PADDR(sample_P), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Next update time for sampling unit error values",
            PT_double, "sample_f", PADDR(sample_f), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Next update time for sampling frequency error",
            PT_bool, "deadbandflag", PADDR(deadbandflag), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "false=frequency within deadband, true=frequency outside of "
            "deadband",
            PT_bool, "tielineflag", PADDR(tielineflag), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "false=all tie lines within tolerance, true=some tielines outside "
            "of tolerance",
            PT_double, "deltat_pid", PADDR(deltat_pid), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "Integration step for PID controller. Minimum of Ts_p and Ts_f",
            // States
            PT_double, "perr(t)[MW]", PADDR(curr_state.perr[0]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "Power error in current iteration [MW]",
            PT_double, "perr(t-1)[MW]", PADDR(curr_state.perr[1]), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "Power error in previous timestep [MW]",
            PT_double, "uniterr[MW]", PADDR(curr_state.uniterr), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "Sum of unit errors in MW.", PT_double,
            "deltaf[Hz]", PADDR(curr_state.deltaf), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION,
            "frequency error signal *after* adjustment for max/min and "
            "deadband in Hz",
            PT_double, "dxi[MW]", PADDR(curr_state.dxi), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "Change in PID integrator output", PT_double,
            "xi[MW]", PADDR(curr_state.xi), PT_ACCESS, PA_HIDDEN,
            PT_DESCRIPTION, "PID integrator output", PT_double, "PIDout[MW]",
            PADDR(curr_state.PIDout), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION,
            "PID output", PT_double, "dP[MW]", PADDR(curr_state.dP), PT_ACCESS,
            PA_HIDDEN, PT_DESCRIPTION, "Delta P signal [MW]", NULL) < 1)
...
int sec_control::create(void) {
  ...
  Tlp_default = -1.0;   // Note: should actually be >= dt which is > 0 (or 0 to
                        // indicate no low pass filter [default])
...
int sec_control::init(OBJECT *parent) {
  ...
  if (parent != NULL) {
    if ((parent->flags & OF_INIT) != OF_INIT) {
      char objname[256];
      gl_verbose("sec_control::init(): deferring initialization on %s",
  ...
  if ((pFrequency->is_valid() != true) || (pFrequency->is_double() != true)) {
    GL_THROW("sec_control:%s failed to map measured_frequency variable from %s",
  ...
  if (deltamode_inclusive) {
    ...
    if (enable_subsecond_models != true) {
      gl_warning("sec_control:%s indicates it wants to run deltamode, but the "
                 "module-level flag is not set!",
                 obj->name ? obj->name : "unnamed");
      /*  TROUBLESHOOT
      The sec_control object has the deltamode_inclusive flag set, but not the
      ...
      */
      ...
      if (fxn_return_status == FAILED) {
        GL_THROW("sec_control:%s - failed to add object to generator deltamode "
                 "object list",
                 obj->name ? obj->name : "unnamed");
        /*  TROUBLESHOOT
        The sec_control object encountered an issue while trying to add itself
  ...
  {
    if (enable_subsecond_models == true) {
      gl_warning("sec_control:%d %s - Deltamode is enabled for the module, but "
                 "not this controller!",
                 obj->id, (obj->name ? obj->name : "Unnamed"));
      /*  TROUBLESHOOT
      The sec_control is not flagged for deltamode operations, yet deltamode
      simulations are enabled for the overall system.  When deltamode triggers,
      this sec_control may no longer contribute to the system, until
  ...
  // Reason is, that the default value should be dt, which is not known at this
  // point. Keeping at -1 is an indication for later that they should be
  // adjusted. Similarly for dp_dn_default and dp_up_default, we'd like to pick
  // those on an object by object basis, e.g. rating
...
// Check if sampling time ts is less than dt and if it is set it to another
// default
void sec_control::ts_lb_check(double &ts, double dt, double def,
                              const char *name) {
  if (ts < dt) {
    gl_warning("sec_control: %s = %0.5f < dt = %0.5f. Setting %s = %0.5f", name,
               ts, dt, name, def);
    ts = def;
  }
}
...
//  For example with t=0 and dt = 0.01 then by the end of this step we will
//  reach t=0.01. If we want to sample at t=0.01 then we need to sample in this
//  pass i.e. at cur_timestamp_dbl+deltat
void sec_control::sample_time_update(bool &flag, double &time, double curr_time,
                                     double ts) {
  if ((curr_time) >= time) {
...
TIMESTAMP sec_control::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  //**************** Bottom-up execution events for QSTS - usually the actual
  //"logic" before a powerflow call ****************//
...
void sec_control::participant_tlp_check(SEC_CNTRL_PARTICIPANT *obj) {
  if (curr_dt > 0) // in delta mode therefore deltat is present
  {
    ...
    if ((obj->Tlp < curr_dt) &&
        (obj->Tlp != 0)) // Specified but is too small or 0 (i.e. ignore)
    {
      gl_warning("sec_control: participant %s: Tlp = %0.5f < dt = %0.5f. "
                 "Setting Tlp = 0 (i.e. ignore)",
                 obj->ptr->name, obj->Tlp, curr_dt);
      obj->Tlp = 0;
    }
  }
}
...
// Module-level call
SIMULATIONMODE
sec_control::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt,
  ...
  //**************** Primary deltamode executor - dynamics usually go/get called
  //here ****************//
  ...
  if (iteration_count_val == 0) // Predictor pass
  {
    ...
    if (delta_time == 0) {
      ...
      // These are done here because in pre_deltaupdate we don't have deltat
      // yet.
    ...
    sample_time_update(sampleflag, sample_time, curr_timestamp_dbl + deltat,
                       Ts); // update output sampling
    sample_time_update(sampleflag_P, sample_P, curr_timestamp_dbl + deltat,
                       Ts_P); // update unit error sampling
    sample_time_update(sampleflag_f, sample_f, curr_timestamp_dbl + deltat,
                       Ts_f); // update frequency sampling

    // Only process error signals and PID controller if one of the inputs is
    // being sampled
    if (sampleflag_f || sampleflag_P) {
      ...
      case FEEDBACK_PIDOUT:
        ...
        // PIDout when not sampling and 0 otherwise
        curr_state.dxi += curr_state.PIDout *
                          (sampleflag * (deadbandflag || tielineflag) - 1);
        next_state.xi = curr_state.xi + deltat_pid * curr_state.dxi;
        break;
      case FEEDBACK_INTEGRATOR:
        // feedback difference between PIDout and actual "actuated" output
        // (neglecting proportional and derivative paths)
        curr_state.dxi +=
            curr_state.xi * (sampleflag * (deadbandflag || tielineflag) - 1);
        next_state.xi = curr_state.xi + deltat_pid * curr_state.dxi;
        break;
      default:
        break;
      }
      next_state.PIDout = next_state.xi +
                          (kpPID + kdPID / deltat_pid) * curr_state.perr[0] -
                          kdPID / deltat_pid * curr_state.perr[1];
      //=== End PID controller
    }
    ...
    for (auto &obj : part_obj) {
      ...
      // update change for each participant based on participation factor alpha
      // and low pass filter.
      if (obj.Tlp == 0) {
        // no low pass filter, just pass the fraction of change if sampling,
        // else 0
        obj.dP[1] = (sampleflag && (deadbandflag || tielineflag))
                        ? obj.dP[0] + obj.alpha * next_state.dP
                        : 0;
        obj.dP[3] = 0; // since no Low pass filter, there is no memory and no
                       // need for this t-1 value
      } else {
        obj.dP[0] *= !(sampleflag && (deadbandflag ||
                                      tielineflag)); // zero on sample instances
        ...
        obj.dP[3] =
            (sampleflag && (deadbandflag || tielineflag)) ? 0 : obj.dP[0];
      }

      //***************code to update object setpoints********************
      // on the predictor pass we just want to update with the calculated value
      // dP[1] if we are sampling. If we are *between* sampling instances we
      // want to pass the incremental change from the previous step.
      update_pdisp(obj, obj.dP[1] - obj.dP[3]);
    }
    simmode_return_value =
        SM_DELTA_ITER; // call iteration to get to corrector pass
  } else if (iteration_count_val == 1) // corrector pass
  {
    ...
    if (sampleflag_f || sampleflag_P) {
      ...
      next_state.xi =
          curr_state.xi + deltath_pid * (curr_state.dxi + next_state.dxi);
      switch (anti_windup) {
      case ZERO_IN_DEADBAND:
        if (!deadbandflag && !tielineflag) {
          // if frequency is within deadband, reset the integrator
      ...
      case FEEDBACK_PIDOUT:
        // feedback difference between PIDout and actual "actuated" output
        next_state.dxi += curr_state.PIDout *
                          (sampleflag * (deadbandflag || tielineflag) - 1);
        next_state.xi =
            curr_state.xi + deltath_pid * (curr_state.dxi + next_state.dxi);
        break;
    ...
    for (auto &obj : part_obj) {
      ...
      if (obj.Tlp == 0) {
        ...
        obj.dP[0] = (sampleflag && (deadbandflag || tielineflag))
                        ? obj.alpha * next_state.dP
                        : 0;
      } else {
      ...
      // Since various limits can affect the passed value and what is actually
      // set, what we *really* want is obj.dP[0] - obj.dP[2], where obj.dP[2]
      // stores the final set value via the call to update_pdisp.
    ...
    if (deadbandflag || tielineflag) {
      ...
      // Frequency/tieline error within deadband, wait 2xTs and then switch to
      // QSTS
      if (deltaf_flag) {
        if (qsts_time < 0) {
    ...
    // only update state on input sampling instances, othewise, state is not
    // updated.
    if (sampleflag_f || sampleflag_P) {
...
void sec_control::update_pdisp(SEC_CNTRL_PARTICIPANT &obj, double val) {
  ...
  if (val != 0) {
    ...
    if ((val + poffset * obj.rate) > obj.dp_up) {
      gl_warning("sec_control: Object %s desired setpoint change of %0.6f MW, "
                 "would result in a total change of %0.6f > than maximum %0.6f "
                 "MW. Changing to limit.",
                 obj.ptr->name, val, val + poffset * obj.rate, obj.dp_up);
      val = obj.dp_up - poffset * obj.rate;
    } else if ((val + poffset * obj.rate) < -obj.dp_dn) {
      gl_warning("sec_control: Object %s desired setpoint change of %0.6f MW, "
                 "would result in a total change of %0.6f < than maximum "
                 "-%0.6f MW. Changing to limit.",
                 obj.ptr->name, val, val + poffset * obj.rate, obj.dp_dn);
      val = -obj.dp_dn - poffset * obj.rate;
    }

    // ===== Handle dispatch limits (these are in p.u.)
    if ((pdisp + poffset + val / obj.rate) > obj.pmax) {
      gl_warning("sec_control: generator %s is dispatched at %0.4f pu. The "
                 "desired setpoint change of %0.4f pu will exceed the upper "
                 "limit of %0.4f pu. Setting to upper limit.",
                 obj.ptr->name, pdisp + poffset, val / obj.rate, obj.pmax);
      val = (obj.pmax - pdisp - poffset) * obj.rate;
    } else if ((pdisp + poffset + val / obj.rate) < obj.pmin) {
      gl_warning("sec_control: generator %s is dispatched at %0.4f pu. The "
                 "desired setpoint change of %0.4f pu will exceed the lower "
                 "limit of %0.4f pu. Setting to lower limit.",
                 obj.ptr->name, pdisp + poffset, val / obj.rate, obj.pmin);
...
void sec_control::get_perr(void) {
  if (sampleflag_f) // only update delta f if sampling
  {
    if (overfrequency_limit < f0) {
      gl_warning("sec_control: overfrequency_limit (%0.3f Hz) is SMALLER than "
                 "nominal (%0.3f Hz). Setting to default (%0.3f Hz)",
                 overfrequency_limit, f0, f0 + frequency_delta_default);
      overfrequency_limit = f0 + frequency_delta_default;
    }
    if (underfrequency_limit > f0) {
      gl_warning("sec_control: under (%0.3f Hz) is GREATER than nominal (%0.3f "
                 "Hz). Setting to default (%0.3f Hz)",
                 overfrequency_limit, f0, f0 - frequency_delta_default);
      overfrequency_limit = f0 - frequency_delta_default;
    }
    ...
    if (deltaf > (f0 - underfrequency_limit)) // limit maximum positive
                                              // deviation (underfrequency)
    {
      deltaf = f0 - underfrequency_limit;
    } else if (deltaf < (f0 - overfrequency_limit)) // limit maximum negative
                                                    // deviation (overfrequency)
    {
    ...
    next_state.deltaf =
        deltaf; // saving in case sample_P and sample_f don't coincide
  }
  if (sampleflag_P) // only update unit error if sampling
  {
    ...
    for (auto &obj : part_obj) {
      if (gl_object_isa(obj.ptr, "link", "powerflow")) {
        ...
        pset = (obj.pdisp->get_double() + obj.poffset->get_double()) *
               obj.rate;       // get current set point in units!
        pout = get_pelec(obj); // get current output
        uniterr += (pset - pout);
      }
    }
    next_state.uniterr =
        uniterr; // saving in case sample_P and sample_f don't coincide
  }
  // the power error is the frequency error times bias *minus* the total
  // production error.
...
double sec_control::get_tieline_error(SEC_CNTRL_PARTICIPANT &obj) {
  double pset = (obj.pdisp->get_double() + obj.poffset->get_double()) *
                1e-6;           // set point in MW
  double pout = get_pelec(obj); // current flow in MW (avg of in and out)
  double d =
      (pset != 0)
          ? pset
          : pout; // denominator is pset unless that is 0, then we use pout
  double out;
  if (((pset - pout) <= (obj.pmax * d)) && ((pset - pout) >= (obj.pmin * d))) {
    ...
    // alpha should be -1 if from node is in region and +1 if to node is in
    // region.
...
STATUS sec_control::post_deltaupdate(gld::complex *useful_value,
...
gld_property *sec_control::map_complex_value(OBJECT *obj, const char *name) {
  ...
  if ((pQuantity->is_valid() != true) || (pQuantity->is_complex() != true)) {
    GL_THROW("sec_control:%d %s - Unable to map property %s from object:%d %s",
    ...
    the secondary controller.  Please try again. If the error persists, please
    submit your system and a bug report via the ticketing system.
    */
  }
...
gld_property *sec_control::map_enum_value(OBJECT *obj, const char *name) {
  ...
  // Check it
  if ((pQuantity->is_valid() != true) ||
      (pQuantity->is_enumeration() != true)) {
    GL_THROW("sec_control:%d %s - Unable to map property %s from object:%d %s",
             objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name,
             obj->id, (obj->name ? obj->name : "Unnamed"));
    /*  TROUBLESHOOT
...
std::vector<std::string> sec_control::split(const std::string &s,
                                            char delimiter) {
...
// from
// https://www.techiedelight.com/trim-string-cpp-remove-leading-trailing-spaces/#:~:text=We%20can%20use%20a%20combination,functions%20to%20trim%20the%20string.
std::string sec_control::trim(const std::string &s) { return rtrim(ltrim(s)); }
...
std::vector<SEC_CNTRL_PARTICIPANT>::iterator
sec_control::find_obj(std::string name) {
...
double sec_control::get_pelec(SEC_CNTRL_PARTICIPANT &obj) {
  ...
  if (gl_object_isa(obj.ptr, "diesel_dg", "generators")) {
    ...
    out = (get_complex_value(obj.ptr, "VA_Out")).Re() * 1e-6;
             ...
             gl_object_isa(obj.ptr, "switch", "powerflow")) {
    // Average is taken to get the same value for both regions.
    out = (get_complex_value(obj.ptr, "power_in").Re() +
           get_complex_value(obj.ptr, "power_out").Re()) /
          2.0 * 1e-6;
  }
...
void sec_control::add_obj(std::vector<std::string> &vals) {
  ...
  tmp.poffset = map_double_value(
      tmp.ptr, "pdispatch_offset"); // offset to controller setpont
  // Handle based on object type
  if (gl_object_isa(tmp.ptr, "diesel_dg", "generators")) {
    ...
    tmp.rate = get_double_value(tmp.ptr, "Rated_VA") *
               1e-6; // convert to MW for our purposes
    tmp.pmax = 1;    // p.u
    tmp.pmin = 0;    // p.u.

    // unless a default rate is given, the default is the whole range, i.e.
    // pmax-pmin
    tmp.dp_dn = (dp_dn_default < 0) ? ((tmp.pmax - tmp.pmin) * tmp.rate)
                                    : dp_dn_default;
    tmp.dp_up = (dp_up_default < 0) ? ((tmp.pmax - tmp.pmin) * tmp.rate)
                                    : dp_up_default;

  } // END diesel_dg specific
  else if (gl_object_isa(tmp.ptr, "inverter_dyn", "generators")) {
    // ===== Inverter_dyn options ==========
    tmp.rate = get_double_value(tmp.ptr, "rated_power") *
               1e-6; // convert to MW for our purposes
    ...
    if (inv_mode == 0) // GRID_FORMING
    {
      ...
      tmp.pmin = get_double_value(tmp.ptr, "Pmin"); // This is in p.u.
               ...
               (inv_mode == 2)) // GRID_FOLLOWING or GFL_CURRENT_SOURCE
    {
      ...
      GL_THROW("sec_control: inverter_dyn control_mode %d is not supported. "
               "For object %s.",
               inv_mode, tmp.ptr->name);
      /*  TROUBLESHOOT
      The secondary controller object is currently only able to interact with
      generator objects of type diese_dg and inverter_dyn. More objects might be
      added in the future.
      */
    }
    ...
    tmp.dp_up = (dp_up_default < 0) ? ((tmp.pmax - tmp.pmin) * tmp.rate)
           ...
           gl_object_isa(tmp.ptr, "switch", "powerflow")) {
    // ==== tie-line options ========
    tmp.rate = 1; // not uses for links. 1 so that any units-to-pu conversions
                  // will not do anything
    // pmax/dp_up and pmin/dp_dn are used as deadband values for the tie-lines.
    tmp.pmax =
        tieline_tol; // positive tolerance in p.u. (pset-pactual)/pset <= pmax
    tmp.pmin = -tieline_tol; // negative tolerance in p.u. (pset - pactual)/pset
                             // >= pmin
    // calculate initial dp_up/dp_dn. these are currently NOT USED!!!
    tmp.dp_dn = tmp.pmin *
                (tmp.pdisp->get_double() + tmp.poffset->get_double()) *
                1e-6; // Tie line tolerance in MW, lower magnitude
    tmp.dp_up = tmp.pmax *
                (tmp.pdisp->get_double() + tmp.poffset->get_double()) *
                1e-6; // Tie line tolerance in MW, higher magnitude
  } // END link_object specific
  else {
    GL_THROW("sec_control: currently only diesel_dg, inverter_dyn and "
             "link_object objects supported. For object %s.",
             tmp.ptr->name);
    /*  TROUBLESHOOT
    The secondary controller object is currently only able to interact with
    generator objects of type diese_dg and inverter_dyn. More objects might be
  ...
  participant_tlp_check(tmp); // recheck  Tlp to make sure it is not too small
                              // or set default if in delta mode.
...
void sec_control::mod_obj(std::vector<std::string> &vals) {
  ...
  if (tmp != part_obj.end()) {
    ...
    try {
      ...
      participant_tlp_check(
          tmpptr); // recheck Tlp to make sure it is not too small
    } catch (std::out_of_range const &e) {
      // do nothing these are optional.
    }
  }
...
void sec_control::parse_glm(std::string &s) {
  ...
  std::vector<PARSE_KEYS> keys = {{ADD, 3, s.find("ADD")},
                                  {MODIFY, 6, s.find("MODIFY")},
                                  {REMOVE, 6, s.find("REMOVE")}};
  ...
  for (int i = 0; i < keys.size(); i++) {
    ...
    std::size_t end =
        keys[i + 1].pos != s.npos ? keys[i + 1].pos - start : s.npos;
...
static TIMESTAMP sync_sec_control_impl(OBJECT *obj, TIMESTAMP t1,
                                       PASSCONFIG pass) {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_sec_control(OBJECT *obj, TIMESTAMP t1,
                                                 PASSCONFIG pass) {
...
EXPORT STATUS preupdate_sec_control(OBJECT *obj, TIMESTAMP t0,
  ...
  try {
    ...
    gl_error("preupdate_sec_control(obj=%d;%s): %s", obj->id,
...
interupdate_sec_control(OBJECT *obj, unsigned int64 delta_time,
  ...
  try {
    ...
    gl_error("interupdate_sec_control(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return status;
  }
}

EXPORT STATUS postupdate_sec_control(OBJECT *obj, gld::complex *useful_value,
                                     unsigned int mode_pass) {
  sec_control *my = /*OBJECTDATA(obj,<>)*/ object_data<sec_control>(obj);
  ...
  try {
    ...
    gl_error("postupdate_sec_control(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
    return status;
  }
}