...
ibr_gfm_vsm::ibr_gfm_vsm(MODULE *module)
{
	if (oclass == nullptr)
	{
		...
		if (gl_publish_variable(oclass,
			...
			// ibr_gfm_vsm_NOTE: Any variables that need to be published should go here.
...
int ibr_gfm_vsm::init(OBJECT *parent)
{
	...
	else //This particular model isn't enabled
	{
		if (enable_subsecond_models)
		{
			gl_warning("ibr_gfm_vsm:%d %s - Deltamode is enabled for the module, but not this object!", obj->id, (obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
...
STATUS ibr_gfm_vsm::init_dynamics()
{
	OBJECT *obj = OBJECTHDR(this);
