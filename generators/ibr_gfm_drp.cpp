...
CLASS *ibr_gfm_drp::oclass = nullptr;
ibr_gfm_drp *ibr_gfm_drp::defaults = nullptr;

static PASSCONFIG clockpass = PC_BOTTOMUP;

/* Class registration is only called once to register the class with the core */
ibr_gfm_drp::ibr_gfm_drp(MODULE *module)
{
	if (oclass == nullptr)
	{
		oclass = gl_register_class(module, "ibr_gfm_drp", sizeof(ibr_gfm_drp), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
		if (oclass == nullptr)
			throw "unable to register class ibr_gfm_drp";
		else
		...
		if (gl_publish_variable(oclass,
			...
			// ibr_gfm_drp_NOTE: Any variables that need to be published should go here.
		...
		memset(this, 0, sizeof(ibr_gfm_drp));

		if (gl_publish_function(oclass, "preupdate_gen_object", (FUNCTIONADDR)preupdate_ibr_gfm_drp) == nullptr)
			GL_THROW("Unable to publish ibr_gfm_drp deltamode function");
		if (gl_publish_function(oclass, "interupdate_gen_object", (FUNCTIONADDR)interupdate_ibr_gfm_drp) == nullptr)
			GL_THROW("Unable to publish ibr_gfm_drp deltamode function");
		// if (gl_publish_function(oclass, "postupdate_gen_object", (FUNCTIONADDR)postupdate_ibr_gfm_drp) == nullptr)
		// 	GL_THROW("Unable to publish ibr_gfm_drp deltamode function");
		if (gl_publish_function(oclass, "current_injection_update", (FUNCTIONADDR)ibr_gfm_drp_NR_current_injection_update) == nullptr)
			GL_THROW("Unable to publish ibr_gfm_drp current injection update function");
	}
}

//Isa function for identification
int ibr_gfm_drp::isa(char *classname)
{
	return strcmp(classname,"ibr_gfm_drp")==0;
}

/* Object creation is called once for each object that is created by the core */
//**** This sets up object defaults.  This is done before the GLM is read, so GLM properties will override these ****//
int ibr_gfm_drp::create(void)
{
	////////////////////////////////////////////////////////
	// DELTA MODE
	////////////////////////////////////////////////////////
...
int ibr_gfm_drp::init(OBJECT *parent)
{
	...
	if (parent != nullptr)
	{
		...
		{
			char objname[256];
			gl_verbose("ibr_gfm_drp::init(): deferring initialization on %s", gl_name(parent, objname, 255));
			return 2; // defer
		}
	}
	...
	{
		S_base = 1000;
		gl_warning("ibr_gfm_drp:%d - %s - The rated power of this inverter must be positive - set to 1 kVA.",obj->id,(obj->name ? obj->name : "unnamed"));
		/*  TROUBLESHOOT
		The ibr_gfm_drp has a rated power that is negative.  It defaulted to a 1 kVA inverter.  If this is not desired, set the property properly in the GLM.
		*/
	}
	...
	if (parent != nullptr)
	{
			...
			gl_object_isa(parent, "triplex_meter", "powerflow") || gl_object_isa(parent, "triplex_node", "powerflow") || gl_object_isa(parent, "triplex_load", "powerflow"))
		{
			...
			if (deltamode_inclusive)
			{
				...
				if (parent->parent != nullptr)
				{
					...
					{
						GL_THROW("ibr_gfm_drp:%s failed to map Norton-equivalence deltamode variable from %s",obj->name?obj->name:"unnamed",parent->name?parent->name:"unnamed");
						//Defined elsewhere
					}
			...
			{
				GL_THROW("ibr_gfm_drp:%s failed to map bustype variable from %s", obj->name ? obj->name : "unnamed", obj->parent->name ? obj->parent->name : "unnamed");
				/*  TROUBLESHOOT
			...
			else if (gl_object_isa(tmp_obj, "meter", "powerflow") || gl_object_isa(tmp_obj, "node", "powerflow") || gl_object_isa(tmp_obj, "load", "powerflow"))
			{
				...
				else //Not three-phase
				{
					...
					{
						GL_THROW("ibr_gfm_drp:%s is not connected to a single-phase or three-phase node - two-phase connections are not supported at this time.", obj->name ? obj->name : "unnamed");
						/*  TROUBLESHOOT
						The ibr_gfm_drp only supports single phase (A, B, or C or triplex) or full three-phase connections.  If you try to connect it differntly than this, it will not work.
						*/
					}
				} //End non-three-phase
			...
			{
				GL_THROW("ibr_gfm_drp:%d %s failed to map the nominal_frequency property", obj->id, (obj->name ? obj->name : "Unnamed"));
				/*  TROUBLESHOOT
			...
			if (deltamode_inclusive)
			{
				...
				if (!temp_property_pointer->is_valid() || !temp_property_pointer->is_double())
				{
					gl_error("ibr_gfm_drp:%d %s failed to map the nominal_voltage property", obj->id, (obj->name ? obj->name : "Unnamed"));
					/*  TROUBLESHOOT
				...
				{
					GL_THROW("ibr_gfm_drp:%s failed to map Norton-equivalence deltamode variable from %s", obj->name ? obj->name : "unnamed", tmp_obj->name ? tmp_obj->name : "unnamed");
					/*  TROUBLESHOOT
					While attempting to set up the deltamode interfaces and calculations with powerflow, the required interface could not be mapped.
					Please check your GLM and try again.  If the error persists, please submit a trac ticket with your code.
					*/
				...
				else //Already populated, make sure it is the right size!
				{
					...
					{
						GL_THROW("ibr_gfm_drp:%s exposed Norton-equivalent matrix is the wrong size!", obj->name ? obj->name : "unnamed");
						/*  TROUBLESHOOT
			...
			{
				GL_THROW("ibr_gfm_drp failed to map powerflow status variable");
				/*  TROUBLESHOOT
		...
		{
			GL_THROW("ibr_gfm_drp must have a valid powerflow object as its parent, or no parent at all");
			/*  TROUBLESHOOT
			Check the parent object of the inverter.  The ibr_gfm_drp is only able to be childed via to powerflow objects.
			Alternatively, you can also choose to have no parent, in which case the ibr_gfm_drp will be a stand-alone application
	...
	else //Must not have a parent
	{
		...
		gl_warning("ibr_gfm_drp:%d has no parent meter object defined; using static voltages", obj->id);
		/*  TROUBLESHOOT
		An ibr_gfm_drp in the system does not have a parent attached.  It is using static values for the voltage.
		*/
	...
	if (deltamode_inclusive)
	{
		...
		{
			gl_warning("ibr_gfm_drp:%s indicates it wants to run deltamode, but the module-level flag is not set!", obj->name ? obj->name : "unnamed");
			/*  TROUBLESHOOT
			The ibr_gfm_drp object has the deltamode_inclusive flag set, but not the module-level enable_subsecond_models flag.  The generator
		...
		else
		{
			...
			{
				GL_THROW("ibr_gfm_drp:%s - failed to add object to generator deltamode object list", obj->name ? obj->name : "unnamed");
				/*  TROUBLESHOOT
				The ibr_gfm_drp object encountered an issue while trying to add itself to the generator deltamode object list.  If the error
	...
	else //This particular model isn't enabled
	{
		if (enable_subsecond_models)
		{
			gl_warning("ibr_gfm_drp:%d %s - Deltamode is enabled for the module, but not this object!", obj->id, (obj->name ? obj->name : "Unnamed"));
			/*  TROUBLESHOOT
			The ibr_gfm_drp is not flagged for deltamode operations, yet deltamode simulations are enabled for the overall system.  When deltamode
			triggers, this ibr_gfm_drp may no longer contribute to the system, until event-driven mode resumes.  This could cause issues with the simulation.
	...
	{
		gl_warning("ibr_gfm_drp:%d %s - The output apparent power is larger than the rated apparent power, P and Q are capped", obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
TIMESTAMP ibr_gfm_drp::presync(TIMESTAMP t0, TIMESTAMP t1)
{
...
TIMESTAMP ibr_gfm_drp::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
	...
	if (first_sync_delta_enabled) //Deltamode first pass
	{
		...
		if (deltamode_inclusive && enable_subsecond_models) //We want deltamode - see if it's populated yet
		{
			if (parent_is_a_meter)
			{
				...
				if (test_fxn == nullptr)
				{
					GL_THROW("ibr_gfm_drp:%s - failed to map additional current injection mapping for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
TIMESTAMP ibr_gfm_drp::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	OBJECT *obj = OBJECTHDR(this);
...
STATUS ibr_gfm_drp::pre_deltaupdate(TIMESTAMP t0, unsigned int64 delta_time)
{
	STATUS stat_val;
	...
	if (stat_val != SUCCESS)
	{
		gl_error("ibr_gfm_drp failed pre_deltaupdate call");
		/*  TROUBLESHOOT
		While attempting to call the pre_deltaupdate portion of the ibr_gfm_drp code, an error
...
SIMULATIONMODE ibr_gfm_drp::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
...
STATUS ibr_gfm_drp::init_dynamics()
{
	OBJECT *obj = OBJECTHDR(this);
...
// STATUS ibr_gfm_drp::post_deltaupdate(gld::complex *useful_value, unsigned int mode_pass)
// {
...
//Map Complex value
gld_property *ibr_gfm_drp::map_complex_value(OBJECT *obj, const char *name)
{
	...
	{
		GL_THROW("ibr_gfm_drp:%d %s - Unable to map property %s from object:%d %s", objhdr->id, (objhdr->name ? objhdr->name : "Unnamed"), name, obj->id, (obj->name ? obj->name : "Unnamed"));
		/*  TROUBLESHOOT
...
//Map double value
gld_property *ibr_gfm_drp::map_double_value(OBJECT *obj, const char *name)
{
	gld_property *pQuantity;
	OBJECT *objhdr = OBJECTHDR(this);
...
void ibr_gfm_drp::pull_complex_powerflow_values(void)
{
...
void ibr_gfm_drp::push_complex_powerflow_values(bool update_voltage)
{
...
STATUS ibr_gfm_drp::updateCurrInjection(int64 iteration_count,bool *converged_failure)
{
	double temp_time;
	OBJECT *obj = OBJECTHDR(this);
	...
	if (parent_is_a_meter)
	{
		...
		if (attached_bus_type > 1) //SWING or SWING_PQ
		{
			...
			if (swing_test_fxn == nullptr)
			{
				...
				if (swing_test_fxn == nullptr)
				{
					GL_THROW("ibr_gfm_drp:%s - failed to map swing-checking for node:%s", (obj->name ? obj->name : "unnamed"), (obj->parent->name ? obj->parent->name : "unnamed"));
					/*  TROUBLESHOOT
...
EXPORT int create_ibr_gfm_drp(OBJECT **obj, OBJECT *parent)
{
	try
	{
		*obj = gl_create_object(ibr_gfm_drp::oclass);
		if (*obj != nullptr)
		{
			ibr_gfm_drp *my = OBJECTDATA(*obj, ibr_gfm_drp);
	...
	CREATE_CATCHALL(ibr_gfm_drp);
}

EXPORT int init_ibr_gfm_drp(OBJECT *obj, OBJECT *parent)
{
	try
	{
		if (obj != nullptr)
			return OBJECTDATA(obj, ibr_gfm_drp)->init(parent);
		else
			return 0;
	}
	INIT_CATCHALL(ibr_gfm_drp);
}

EXPORT TIMESTAMP sync_ibr_gfm_drp(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	TIMESTAMP t2 = TS_NEVER;
	ibr_gfm_drp *my = OBJECTDATA(obj, ibr_gfm_drp);
	try
	...
	SYNC_CATCHALL(ibr_gfm_drp);
	return t2;
}

EXPORT int isa_ibr_gfm_drp(OBJECT *obj, char *classname)
{
	return OBJECTDATA(obj,ibr_gfm_drp)->isa(classname);
}

EXPORT STATUS preupdate_ibr_gfm_drp(OBJECT *obj, TIMESTAMP t0, unsigned int64 delta_time)
{
	ibr_gfm_drp *my = OBJECTDATA(obj, ibr_gfm_drp);
	...
	{
		gl_error("preupdate_ibr_gfm_drp(obj=%d;%s): %s", obj->id, (obj->name ? obj->name : "unnamed"), msg);
		return status_output;
	}
}

EXPORT SIMULATIONMODE interupdate_ibr_gfm_drp(OBJECT *obj, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	ibr_gfm_drp *my = OBJECTDATA(obj, ibr_gfm_drp);
	...
	{
		gl_error("interupdate_ibr_gfm_drp(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
		return status;
	}
}

// EXPORT STATUS postupdate_ibr_gfm_drp(OBJECT *obj, gld::complex *useful_value, unsigned int mode_pass)
// {
// 	ibr_gfm_drp *my = OBJECTDATA(obj, ibr_gfm_drp);
...
// 	catch (char *msg)
// 	{
// 		gl_error("postupdate_ibr_gfm_drp(obj=%d;%s): %s", obj->id, obj->name ? obj->name : "unnamed", msg);
// 		return status;
// 	}
// }
...
EXPORT STATUS ibr_gfm_drp_NR_current_injection_update(OBJECT *obj, int64 iteration_count, bool *converged_failure)
{
	STATUS temp_status;
