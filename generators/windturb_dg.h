...
private:
  ...
  double Power_Curve[2][100]; // Look-up table carrying power curve values.
                              // Maximum points limited to 100. Equals default
                              // (defined in .cpp) or user defined power curve
  ...
  bool NR_first_run;
  double internal_model_current_convergence; // Variable to set
                                             // convergence/reiteration context
...
public:
  ...
  double WSadj;    // Wind speed after all adjustments have been made (height,
                // terrain, etc)
  double Wind_Speed;
  ...
  bool power_curve_pu; // Flag when set indicates that user provided power curve
                       // has power values in pu. Defaults to false in .cpp
