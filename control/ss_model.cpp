...
ss_model::ss_model(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gld_class::create(module, "ss_model", sizeof(ss_model),
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "form", get_form_offset(), PT_DESCRIPTION,
            "desired canonical controller type", PT_KEYWORD, "CCF", CF_CCF,
            PT_KEYWORD, "OCF", CF_OCF, PT_double, "timestep",
            get_timestep_offset(), PT_UNITS, "s", PT_DESCRIPTION,
            "discrete timestep", PT_char1024, "Y", get_Y_offset(),
            PT_DESCRIPTION, "transfer function numerator coefficients",
            PT_char1024, "U", get_U_offset(), PT_DESCRIPTION,
            "transfer function denominator coefficients", PT_char1024, "x",
            get_x_offset(), PT_DESCRIPTION, "state variable values",
            PT_char1024, "u", get_u_offset(), PT_DESCRIPTION,
            "ss_model variable names", PT_char1024, "y", get_y_offset(),
            PT_DESCRIPTION, "output variable names", PT_char1024, "H",
            get_H_offset(), PT_DESCRIPTION, "transfer function", PT_char1024,
            "A", get_y_offset(), PT_DESCRIPTION, "canonical A matrix",
            PT_char1024, "B", get_y_offset(), PT_DESCRIPTION,
            "canonical B matrix", PT_char1024, "C", get_y_offset(),
            PT_DESCRIPTION, "canonical C matrix", PT_char1024, "D",
            get_y_offset(), PT_DESCRIPTION, "canonical D matrix",
...
unsigned int ss_model::scan_references(char *input, gld_property *target[],
                                       unsigned max) {
  unsigned int n;
  char *p;
  char ref[1024];
  for (p = input, n = 0; *p != '\0' && n < max;) {
    ...
    if (target[n]->get_type() != PT_double &&
        target[n]->get_type() != PT_random)
