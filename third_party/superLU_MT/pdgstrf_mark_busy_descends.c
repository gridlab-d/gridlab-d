...
void
pdgstrf_mark_busy_descends(int pnum, int jcol, int *etree, 
 ...
 */
    ...
    GlobalLU_t *Glu = pdgstrf_shared->Glu;
	...
	/* -----------------------------------------------------------
	   ...
	   ----------------------------------------------------------- */
	if (pdgstrf_shared->pan_status[bcol_reg].type == RELAXED_SNODE) {
#if 0	    
...
#endif
	    fsupc = bcol_reg;
	    w = pdgstrf_shared->pan_status[fsupc].size;
	    bcol_reg += w;
