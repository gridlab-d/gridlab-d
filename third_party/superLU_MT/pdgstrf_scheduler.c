...
void
pdgstrf_scheduler(const int pnum, const int n, const int *etree, 
		  //int *cur_pan, int *bcol, pdgstrf_shared_t *pxgstrf_shared)
 ...
 * =======
 *
 * pdgstrf_scheduler() gets a panel for the processor to work on. 
 ...
 *
 */
	pxgstrf_shared_t *pdgstrf_shared = pxgstrf_shared; //sj

    register int dad, dad_ukids, jcol, w, j;
    int *fb_cols = pdgstrf_shared->fb_cols;
    queue_t *taskq = &pdgstrf_shared->taskq;
#ifdef PROFILE
    double t;
#endif
...
#endif;
#if ( MACH==SUN )
    mutex_lock( &pdgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_lock( &pdgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==SGI || MACH==ORIGIN )
#pragma critical lock(pdgstrf_shared->lu_locks[SCHED_LOCK])
#elif ( MACH==CRAY_PVP )
...
{   /* ---- START CRITICAL SECTION ---- */
	...
	dad_ukids = --pdgstrf_shared->pan_status[dad].ukids;
	...
	{
	    STATE( jcol ) = BUSY;
	    w = pdgstrf_shared->pan_status[jcol].size;

	    for (j = jcol; j < jcol+w; ++j) pdgstrf_shared->spin_locks[j] = 1;
	    dad = DADPANEL (jcol);
	    if ( dad < n && pdgstrf_shared->pan_status[dad].ukids == 1 ) {
		STATE( dad ) = CANPIPE;
		/*>> j = Enqueue(taskq, dad);*/
    ...
    *cur_pan = jcol;
    ...
    printf("(%d) Exit C.S. tasks_remain %d, cur_pan %d\n", 
	   pnum, pdgstrf_shared->tasks_remain, jcol);
#endif
...
#if ( MACH==SUN )
    /* Exit C.S. */
    mutex_unlock( &pdgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_unlock( &pdgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==CRAY_PVP )
...
//	 //pdgstrf_shared_t *pxgstrf_shared)
...
//	*cur_pan = 0;
//    } else {
//	w = pdgstrf_shared->pan_status[*cur_pan].size;
//	*cur_pan += w;
//    }
