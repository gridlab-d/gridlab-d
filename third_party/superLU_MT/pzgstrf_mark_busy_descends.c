...
void
pzgstrf_mark_busy_descends(int pnum, int jcol, int *etree, 
 ...
 */
    ...
    GlobalLU_t *Glu = pzgstrf_shared->Glu;
	...
	/* -----------------------------------------------------------
	   ...
	   ----------------------------------------------------------- */
	if (pzgstrf_shared->pan_status[bcol_reg].type == RELAXED_SNODE) {
#if 0	    
	    if ( pzgstrf_shared->pan_status[bcol_reg].size < 0 )
	  	fsupc = bcol_reg + pzgstrf_shared->pan_status[bcol_reg].size;
	    else fsupc = bcol_reg;
#endif
	    fsupc = bcol_reg;
	    w = pzgstrf_shared->pan_status[fsupc].size;
	    bcol_reg += w;
...
#if 0
	    if ( pzgstrf_shared->spin_locks[bcol_reg] ) /* WORSE PERFORMANCE!! */
		await( &pzgstrf_shared->spin_locks[bcol_reg] );
#endif
	    xsup = Glu->xsup;
