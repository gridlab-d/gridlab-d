...
void
psgstrf_scheduler(const int pnum, const int n, const int *etree, 
		  //int *cur_pan, int *bcol, psgstrf_shared_t *pxgstrf_shared)
		  int *cur_pan, int *bcol, pxgstrf_shared_t *pxgstrf_shared) //sj
{
/*
 ...
 * =======
 *
 * psgstrf_scheduler() gets a panel for the processor to work on. 
 ...
 *
 */
	pxgstrf_shared_t *psgstrf_shared = pxgstrf_shared; //sj

    register int dad, dad_ukids, jcol, w, j;
    int *fb_cols = psgstrf_shared->fb_cols;
    queue_t *taskq = &psgstrf_shared->taskq;
#ifdef PROFILE
    double t;
#endif
...
#endif;
#if ( MACH==SUN )
    mutex_lock( &psgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_lock( &psgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==SGI || MACH==ORIGIN )
#pragma critical lock(psgstrf_shared->lu_locks[SCHED_LOCK])
#elif ( MACH==CRAY_PVP )
...
{   /* ---- START CRITICAL SECTION ---- */
	...
	dad_ukids = --psgstrf_shared->pan_status[dad].ukids;
	...
	{
	    STATE( jcol ) = BUSY;
	    w = psgstrf_shared->pan_status[jcol].size;

	    for (j = jcol; j < jcol+w; ++j) psgstrf_shared->spin_locks[j] = 1;
	    dad = DADPANEL (jcol);
	    if ( dad < n && psgstrf_shared->pan_status[dad].ukids == 1 ) {
		STATE( dad ) = CANPIPE;
		/*>> j = Enqueue(taskq, dad);*/
    ...
    *cur_pan = jcol;
    ...
    printf("(%d) Exit C.S. tasks_remain %d, cur_pan %d\n", 
	   pnum, psgstrf_shared->tasks_remain, jcol);
#endif
...
#if ( MACH==SUN )
    /* Exit C.S. */
    mutex_unlock( &psgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_unlock( &psgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==CRAY_PVP )
...
//	 //psgstrf_shared_t *pxgstrf_shared)
...
//	*cur_pan = 0;
//    } else {
//	w = psgstrf_shared->pan_status[*cur_pan].size;
//	*cur_pan += w;
//    }
