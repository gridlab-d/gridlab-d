...
void
pcgstrf_scheduler(const int pnum, const int n, const int *etree, 
		  //int *cur_pan, int *bcol, pcgstrf_shared_t *pxgstrf_shared)
 ...
 * =======
 *
 * pcgstrf_scheduler() gets a panel for the processor to work on. 
 ...
 */
    ...
    int *fb_cols = pcgstrf_shared->fb_cols;
    queue_t *taskq = &pcgstrf_shared->taskq;
#ifdef PROFILE
    double t;
#endif
...
#endif;
#if ( MACH==SUN )
    mutex_lock( &pcgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_lock( &pcgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==SGI || MACH==ORIGIN )
#pragma critical lock(pcgstrf_shared->lu_locks[SCHED_LOCK])
#elif ( MACH==CRAY_PVP )
...
{   /* ---- START CRITICAL SECTION ---- */
	...
	dad_ukids = --pcgstrf_shared->pan_status[dad].ukids;
	...
	{
	    STATE( jcol ) = BUSY;
	    w = pcgstrf_shared->pan_status[jcol].size;

	    for (j = jcol; j < jcol+w; ++j) pcgstrf_shared->spin_locks[j] = 1;
	    dad = DADPANEL (jcol);
	    if ( dad < n && pcgstrf_shared->pan_status[dad].ukids == 1 ) {
		STATE( dad ) = CANPIPE;
		/*>> j = Enqueue(taskq, dad);*/
    ...
    *cur_pan = jcol;
    ...
    printf("(%d) Exit C.S. tasks_remain %d, cur_pan %d\n", 
	   pnum, pcgstrf_shared->tasks_remain, jcol);
#endif
...
#if ( MACH==SUN )
    /* Exit C.S. */
    mutex_unlock( &pcgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_unlock( &pcgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==CRAY_PVP )
...
#undef POSTORDER
	...
	*cur_pan = 0;
	...
	w = pcgstrf_shared->pan_status[*cur_pan].size;
	*cur_pan += w;
    }
