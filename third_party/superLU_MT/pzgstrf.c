...
void
pzgstrf(superlumt_options_t *superlumt_options, SuperMatrix *A, int *perm_r,
 ...
 * =======
 *
 * PZGSTRF computes an LU factorization of a general sparse nrow-by-ncol
 ...
 */
    ...
    /* --------------------------------------------------------------
       ...
       --------------------------------------------------------------*/
    pzgstrf_threadarg = pzgstrf_thread_init(A, L, U, superlumt_options,
    ...
    pzgstrf_thread( &(pzgstrf_threadarg[0]) );
...
#pragma parallel
#pragma shared  (pzgstrf_threadarg)
...
#pragma _CRI taskloop private (i,nprocs)  shared (pzgstrf_threadarg)
...
#pragma omp parallel for shared (pzgstrf_threadarg) private (i)
    /* Stand-alone task loop */
    for (i = 0; i < nprocs; ++i) {
        pzgstrf_thread( &(pzgstrf_threadarg[i]) );
    }
...
/* _OPENMP */
    ...
    printf("pzgstrf() is not parallelized on this machine.\n");
...
#if ( PRNTlevel==1 )
    printf(".. pzgstrf_thread() returns info %d, usrtime %.2f, wtime %.2f\n",
           *info, usrtime, wtime);
#endif

    /* check_mem_leak("after pzgstrf_thread()"); */
