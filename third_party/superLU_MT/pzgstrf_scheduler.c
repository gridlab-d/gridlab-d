...
void
pzgstrf_scheduler(const int pnum, const int n, const int *etree, 
		  //int *cur_pan, int *bcol, pzgstrf_shared_t *pxgstrf_shared)
 ...
 * =======
 *
 * pzgstrf_scheduler() gets a panel for the processor to work on. 
 ...
 *
 */
	pxgstrf_shared_t *pzgstrf_shared = pxgstrf_shared; //sj

    register int dad, dad_ukids, jcol, w, j;
    int *fb_cols = pzgstrf_shared->fb_cols;
    queue_t *taskq = &pzgstrf_shared->taskq;
#ifdef PROFILE
    double t;
#endif
...
#endif;
#if ( MACH==SUN )
    mutex_lock( &pzgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_lock( &pzgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==SGI || MACH==ORIGIN )
#pragma critical lock(pzgstrf_shared->lu_locks[SCHED_LOCK])
#elif ( MACH==CRAY_PVP )
...
{   /* ---- START CRITICAL SECTION ---- */
	...
	dad_ukids = --pzgstrf_shared->pan_status[dad].ukids;
	...
	{
	    STATE( jcol ) = BUSY;
	    w = pzgstrf_shared->pan_status[jcol].size;

	    for (j = jcol; j < jcol+w; ++j) pzgstrf_shared->spin_locks[j] = 1;
	    dad = DADPANEL (jcol);
	    if ( dad < n && pzgstrf_shared->pan_status[dad].ukids == 1 ) {
		STATE( dad ) = CANPIPE;
		/*>> j = Enqueue(taskq, dad);*/
    ...
    *cur_pan = jcol;
    ...
    printf("(%d) Exit C.S. tasks_remain %d, cur_pan %d\n", 
	   pnum, pzgstrf_shared->tasks_remain, jcol);
#endif
...
#if ( MACH==SUN )
    /* Exit C.S. */
    mutex_unlock( &pzgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==DEC || MACH==PTHREAD )
    pthread_mutex_unlock( &pzgstrf_shared->lu_locks[SCHED_LOCK] );
#elif ( MACH==CRAY_PVP )
...
//	 //pzgstrf_shared_t *pxgstrf_shared)
...
//	*cur_pan = 0;
//    } else {
//	w = pzgstrf_shared->pan_status[*cur_pan].size;
//	*cur_pan += w;
//    }
