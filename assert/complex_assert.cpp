...
complex_assert::complex_assert(MODULE *module)
{
    if (oclass == nullptr)
    {
        ...
        if (gl_publish_variable(oclass,
                                ...
                                PT_complex, "once_value", get_once_value_offset(), PT_ACCESS, PA_HIDDEN, PT_DESCRIPTION, "CHECKPOINT_VAR: internal variable for ONCE state value",
        ...
        else if (get_operation() == ANGLE)
        {
            ...
            if (_isnan(angle_error) || fabs(angle_error) > within)
            {
                gl_error("Assert failed on %s: Angle of %s (%g) not within %f of given value %g",
                         get_parent()->get_name(), get_target().c_str(), x.Arg(), within, value.Arg());
