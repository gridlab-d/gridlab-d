/* int_assert
 ...
 Very simple test that compares double values to any corresponding double value.
 If the test fails at any time, it t.rows() a 'zero' to the commit function and
 breaks the simulator out with a failure code.
 */
...
int_assert::int_assert(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "int_assert", sizeof(int_assert),
    ...
    if (gl_publish_variable(
            ...
            PT_enumeration, "status", get_status_offset(), PT_KEYWORD,
            "ASSERT_TRUE", (enumeration)ASSERT_TRUE, PT_KEYWORD, "ASSERT_FALSE",
            (enumeration)ASSERT_FALSE, PT_KEYWORD, "ASSERT_NONE",
            (enumeration)ASSERT_NONE, PT_enumeration, "once", get_once_offset(),
            PT_KEYWORD, "ONCE_FALSE", (enumeration)ONCE_FALSE, PT_KEYWORD,
            "ONCE_TRUE", (enumeration)ONCE_TRUE, PT_KEYWORD, "ONCE_DONE",
            (enumeration)ONCE_DONE, PT_enumeration, "within_mode",
            get_within_mode_offset(), PT_KEYWORD, "WITHIN_VALUE",
            (enumeration)IN_ABS, PT_KEYWORD, "WITHIN_RATIO",
            (enumeration)IN_RATIO, PT_int32, "value", get_value_offset(),
...
int int_assert::init(OBJECT *parent) {
  ...
  /*  TROUBLESHOOT
   ...
   see that you have specified a value for "within" and it is not negative.
   */
  return 1;
}
...
TIMESTAMP int_assert::commit(TIMESTAMP t1, TIMESTAMP t2) {
  ...
  if (!target_prop.is_valid() || !(target_prop.get_type() == PT_int16 ||
                                   target_prop.get_type() == PT_int32 ||
                                   target_prop.get_type() == PT_int64)) {
    ...
    /*  TROUBLESHOOT
     ...
     for the object that you are pointing to.  Refer to the documentation of the
  ...
  if (status == ASSERT_TRUE) {
    ...
    if (theDiff > 0xFFFFFFFF || theDiff < -0xFFFFFFFF) {
      gl_warning("int_assert may be incorrect, difference range outside 32-bit "
                 "abs() range");
    }
