/* enum_assert
   ...
   Very simple test that compares either integers or can be used to compare
   enumerated values to their corresponding integer values.  If the test fails
   at any time, it t.rows() a 'zero' to the commit function and breaks the
   simulator out with a failure code.
*/
...
enum_assert::enum_assert(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(module, "enum_assert", sizeof(enum_assert),
    ...
    if (gl_publish_variable(
            ...
            (enumeration)ASSERT_NONE, PT_int32, "value", get_value_offset(),
            PT_DESCRIPTION, "Value to assert", PT_char1024, "target",
...
int enum_assert::init(OBJECT *parent) { return 1; }
...
EXPORT SIMULATIONMODE update_enum_assert(OBJECT *obj, TIMESTAMP t0,
