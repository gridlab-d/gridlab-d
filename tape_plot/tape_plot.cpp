/** $Id: tape_plot.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        Tape Files read or write comma-separated values in a flat file on the
local computer.  Output tape files will typically clobber existing files, but
the user can change the default file mode to write+ or append in the tape
definition in the input file. The "#" symbol at the beginning of a line denotes
a comment.  These comments and blank lines are skipped in input files.  Leading
...
EXPORT int open_shaper(struct shaper *my, char *fname, char *flags) {
  ...
  my->status = TS_OPEN;
  my->type = FT_FILE;
  /* TODO: these should be read from the shape file, or better yet, inferred
   * from it */
  ...
  while (fgets(line, sizeof(line), my->fp) != nullptr) {
    ...
    if (strcmp(group, "") != 0 &&
      ...
      if (sscanf(line, "%s %s %s %s %[^,],%[^,\n]", min, hour, day, month,
                 weekday, value) < 6) {
        // gl_error
        fprintf(stderr, "%s(%d) : shape '%s' has specification '%s'", file,
                linenum, group, line);
        continue;
      }
      /* minutes are ignored right now */
      if (min[0] != '*') // gl_warning
        fprintf(stdout, "%s(%d) : minutes are ignored in '%s'", file, linenum,
                line);
      hours = hourmap(hour);
      days = daymap(day);
      ...
      // gl_error(
      fprintf(stderr, "%s(%d) : shape specification '%s' is not valid", file,
              linenum, line);
    }
  }
  return 1;
...
EXPORT void write_default_plot_commands_rec(struct recorder *my,
                                            char32 extension) {
  ...
  if (my->plotcommands[0] == '\0' || strcmp(my->plotcommands, "") == 0) {
    ...
    // j = strlen(my->columns)>0  ? 0: fprintf(my->fp, "plot \'-\' using 1:2
    // title '%s' with lines;\n", my->property);
    if (strlen(my->columns) > 0) {
      ...
      k = 2;
      for (item = strtok(list, ","); item != nullptr;
           item = strtok(nullptr, ",")) {
        if (k == 2) {
          fprintf(my->fp, "plot \'-\' using 1:%i title \'%s\' with lines", k,
                  item);
          /* only handles one column properly as-is, will be fixed with the tape
           * module update -MH */
          fprintf(my->fp, "\n");
          break;
        } // else {
        //	fprintf(my->fp, ", \\\n\t\'-\' using 1:%i title \'%s\' with
        //lines", k, item);
        //}
        ++k;
      }
...
void set_recorder(struct recorder *my) { global_rec = my; }
...
EXPORT void close_recorder(struct recorder *my) {
...
#endif
  if (my->output == SCREEN)
    snprintf(gnuplot, sizeof(gnuplot), "%s -persist", plotcmd);
  else
    snprintf(gnuplot, sizeof(gnuplot), "%s", plotcmd);
  char fname[sizeof(char32)];
  char type[sizeof(char32)];
  ...
  if (my->fp == nullptr)
    ...
    snprintf(command, sizeof(command), "%s %s", gnuplot, fname);
...
EXPORT void write_default_plot_commands_col(struct collector *my,
                                            char32 extension) {
  char fname[sizeof(char32)];
  char type[sizeof(char32)];
  char buf[sizeof(char1024)];
  ...
  if (my->plotcommands[0] == '\0' || strcmp(my->plotcommands, "") == 0) {
    ...
    fprintf(my->fp, "show output; \n");
    fprintf(my->fp, "set timefmt \"%%Y-%%m-%%d %%H:%%M:%%S\";\n");
    j = strlen(my->columns) > 0
            ? 0
            : fprintf(my->fp, "plot \'-\' using 1:2 with lines;\n");
    return;
  }
  strcpy(plotcommands, my->plotcommands);
