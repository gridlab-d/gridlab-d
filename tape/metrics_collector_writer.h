...
const string
    m_below_10_percent_NormVol_Duration("below_10_percent_NormVol_Duration");
...
const string
    m_air_temperature_setpoint_cooling("air_temperature_setpoint_cooling");
const string
    m_air_temperature_setpoint_heating("air_temperature_setpoint_heating");
...
private:
  ...
  void writeMetadata(nlohmann::json &meta, nlohmann::json &metadata,
                     char *time_str, char256 filename_house);
  ...
  void hdfWrite(char256 filename, const std::unique_ptr<H5::CompType> &mtype,
                void *ptr, int structKind, int size);
...
private:
  nlohmann::json
      metrics_writer_billing_meters;    // Final output dictionary for
                                        // triplex_meters and non-swing meters
  nlohmann::json metrics_writer_houses; // Final output dictionary for houses
                                        // and water heater
  nlohmann::json
      metrics_writer_inverters; // Final output dictionary for inverters
  nlohmann::json metrics_writer_feeders; // Final output dictionary for feeders
  nlohmann::json
      metrics_writer_capacitors; // Final output dictionary for capacitors
  nlohmann::json
      metrics_writer_regulators; // Final output dictionary for regulators
  nlohmann::json
      metrics_writer_transformers; // Final output dictionary for transformers
  nlohmann::json metrics_writer_lines; // Final output dictionary for lines
  nlohmann::json
      metrics_writer_evchargerdets; // Final output dictionary for evcharger det

  nlohmann::json ary_billing_meters; // array storage for billing meter metrics
  nlohmann::json
      ary_houses; // array storage for house (and water heater) metrics
  ...
  int interim_length;  // integer interim length (seconds to write out intervals
                      // and then clear)
  int interim_cnt; // integer interim count (count to write out intervals)
  int line_cnt;    // integer write line count (count how many write_line in a
                // interim write)
