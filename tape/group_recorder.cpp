...
void new_group_recorder(MODULE *mod) { new group_recorder(mod); }
...
group_recorder::group_recorder(MODULE *mod) {
  if (oclass == nullptr) {
...
#endif
    oclass = gl_register_class(mod, const_cast<char *>("group_recorder"),
                               sizeof(group_recorder), PC_POSTTOPDOWN);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            ...
            "output file name", PT_char1024, "group", PADDR(group_def),
            PT_DESCRIPTION, "group definition string", PT_double, "interval[s]",
            PADDR(dInterval), PT_DESCRIPTION,
            ...
            "causes the group_recorder to stop the simulation should there be "
            "a problem opening or writing with the group_recorder",
            PT_bool, "print_units", PADDR(print_units), PT_DESCRIPTION,
            "flag to append units to each written value, if applicable",
            PT_char256, "property", PADDR(property_name), PT_DESCRIPTION,
            "property to record", PT_int32, "limit", PADDR(limit),
            PT_DESCRIPTION, "the maximum number of lines to write to the file",
            PT_bool, "format", PADDR(format), PT_DESCRIPTION,
            "determines whether output timestamp is formatted to be formatted "
            "as human-readable (default) or epoch",
            PT_enumeration, "complex_part", PADDR(complex_part), PT_DESCRIPTION,
            "the complex part to record if complex properties are gathered",
            PT_KEYWORD, "NONE", NONE, PT_KEYWORD, "REAL", REAL, PT_KEYWORD,
            "IMAG", IMAG, PT_KEYWORD, "MAG", MAG, PT_KEYWORD, "ANG_DEG", ANG,
            PT_KEYWORD, "ANG_RAD", ANG_RAD, nullptr) < 1) {
      ; // GL_THROW("unable to publish properties in %s",__FILE__);
    }

    if (gl_publish_function(oclass, const_cast<char *>("obj_postupdate_fxn"),
                            (FUNCTIONADDR)group_recorder_postroutine) ==
        nullptr)
      GL_THROW(const_cast<char *>("Unable to publish deltamode postupdate "
                                  "function for group_recorder"));
...
int group_recorder::create() {
  ...
  // offnominal_time = false; // By default, assume we'll be handled like normal
  // time strict = true;			 // By default, errors are
  // errors - make the user override them return 1;

  // Initialize all members to a known, safe default state.
  // Do NOT copy from a 'defaults' object.
  ...
  obj_list = nullptr;
  prop_ptr = nullptr;
  prev_line_buffer = nullptr;
  line_buffer = nullptr;

  // Integers/sizes should be zero
  obj_count = 0;
  write_count = 0;
  line_size = 0;
  write_ct = 0;
  limit = 0; // from GL_ATOMIC
  ...
  strict = true; // This was your intended default
  print_units = false;
  format = false;
  interval_write = false;
  buffer_valid = false;

  // Enums and other members
  tape_status = TS_INIT;
  complex_part = NONE;

  // The char buffers will be populated by the core loader from the .glm file.
  // They are default-initialized to empty strings by their own constructors.
...
int group_recorder::init(OBJECT *obj) {
  ...
  if ('\0' == filename[0]) {
    ...
    if (strict) {
      ...
      gl_warning(
          ...
          filename.get_string());
      /* TROUBLESHOOT
              group_recorder requires a filename.  If none is provided, a
         filename will be generated using a combination of the classname and the
         core-assigned ID number.
      */
    }
  }
  ...
  if (-1 > write_interval) {
    gl_error("group_recorder::init(): invalid write_interval of %i, must be -1 "
             "or greater",
             write_interval);
    /* TROUBLESHOOT
            The group_recorder interval must be -1, 0, or a positive number of
       seconds.
    */
    return 0;
  }
  ...
  if (0 == items) {
    if (strict) {
      gl_error("group_recorder::init(): unable to construct a set with group "
               "definition");
      /* TROUBLESHOOT
              An error occured while attempting to build and populate the find
         list with the specified group definition.
       */
      return 0;
    } else {
      gl_warning("group_recorder::init(): unable to construct a set with group "
                 "definition");
      tape_status = TS_ERROR;
      return 1; // nothing more to do
    }
  ...
  if (0 == rec_file) {
    if (strict) {
      gl_error("group_recorder::init(): unable to open file '%s' for writing",
               filename.get_string());
      return 0;
    } else {
      gl_warning("group_recorder::init(): unable to open file '%s' for writing",
                 filename.get_string());
      /* TROUBLESHOOT
              If the group_recorder cannot open the specified output file, it
         will
       */
      tape_status = TS_ERROR;
      return 1;
  ...
  for (gr_obj = gl_find_next(items, 0); gr_obj != 0;
    ...
    if (prop_ptr == nullptr) {
      gl_error("group_recorder::init(): unable to find property '%s' in an "
               "object of type '%s'",
               property_name.get_string(), gr_obj->oclass->name);
      /* TROUBLESHOOT
              An error occured while reading the specified property in one of
         the objects.
       */
      return 0;
    }
    ...
    try {
      if (obj_list == nullptr) {
        ...
        if (obj_list == nullptr) {
          GL_THROW("group_recorder::init(): memory allocation failed for first "
                   "object");
        }
      } else {
      ...
      gl_error("group_recorder::init(): error adding object to list: %s",
               e.what());
      return 0;
    }
  }
  ...
  if (!print_units && obj_list != nullptr) {
  ...
  tape_status = TS_OPEN;
  if (0 == write_header()) {
    gl_error("group_recorder::init(): an error occured when writing the file "
             "header");
    /* TROUBLESHOOT
            Unexpected IO error.
     */
  ...
  if (min_ts_value.is_valid() != true) {
    gl_error("group_recorder::init() - %s - Failed to map minimum_timestep "
             "variable!");
    /*  TROUBLESHOOT
    While mapping the global minimum_timestamp variable, an error occurred.  Try
    again.  If the error persists, submit an issue via GitHub
    */
    return 0;
  }
  ...
  if ((thisobj->flags) & OF_DELTAMODE) {
    ...
    /* Make sure it worked */
    if (retvalue == 0) {
      /* Error message is inside the delta_add_tape_device function, just fail
       * us */
      return 0;
    }

    /* Set the flag - deltamode qualifies as something that may have "odd update
     * intervals"*/
    offnominal_time = true;
  }
...
TIMESTAMP group_recorder::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (0 == write_interval) { //
    ...
    // Extra check for offnominal time steps (deltamode or minimum_timestep) -
    // make sure it didn't get stuck
    if (offnominal_time) {
      // See if we stagnated
...
int group_recorder::commit(TIMESTAMP t1, double t1dbl, bool deltacall) {
  ...
  if ((TS_ERROR == tape_status) && strict) {
    gl_error("group_recorder::commit(): the object has error'ed and is halting "
             "the simulation");
    /* TROUBLESHOOT
  ...
  // See if we're deltamode -- if so, just make an update for t1 for the various
  // items
  if (deltacall) {
    t1 = (TIMESTAMP)t1dbl;
    ...
    if (0 == write_interval) { //
      ...
      if (0 == write_line(t1, t1dbl, deltacall)) {
        gl_error("group_recorder::sync(): error when writing the values to the "
                 "file");
        /* TROUBLESHOOT
                Placeholder.
        */
  ...
  if (write_interval > 0) {
    if ((interval_write && !deltacall) || deltacall) {
      ...
      if (0 == write_line(t1, t1dbl, deltacall)) {
        gl_error("group_recorder::commit(): error when writing the values to "
                 "the file");
        return 0;
      }
      last_write = t1;
...
int group_recorder::write_header() {
  ...
  if (0 == rec_file) {
    ...
    /* TROUBLESHOOT
            group_recorder claimed to be open and attempted to write to a file
       when a file had not successfully	opened.
     */
    tape_status = TS_ERROR;
  ...
  if (0 > fprintf(rec_file, "# timestamp")) {
    return 0;
  }
  for (qol = obj_list; qol != 0; qol = qol->next) {
    if (0 != qol->obj->name) {
      if (0 > fprintf(rec_file, ",%s", qol->obj->name)) {
        return 0;
      }
    } else {
      if (0 >
          fprintf(rec_file, ",%s:%i", qol->obj->oclass->name, qol->obj->id)) {
        return 0;
      }
    }
  }
...
int group_recorder::read_line() {
  ...
  if (-1 == write_interval) {
    // FIX: Check if prev_line_buffer was allocated. If not, allocate it now.
    if (prev_line_buffer == nullptr) {
      prev_line_buffer = (char *)malloc(line_size);
      if (0 == prev_line_buffer) {
        gl_error(
            "group_recorder::read_line(): malloc failure for prev_line_buffer");
        return 0;
      }
      memset(prev_line_buffer, 0, line_size);
    }

    // Now the swap is safe
  ...
  for (curr = obj_list; curr != 0; curr = curr->next) {
    ...
    if (curr->prop.ptype == PT_complex) {
      ...
      if (0 == cptr) {
        gl_error("group_recorder::read_line(): unable to get complex property "
                 "'%s' from object '%s'",
                 curr->prop.name, gl_name(curr->obj, objname, 127));
        /* TROUBLESHOOT
      ...
      // If no specific part is requested, format the whole complex number
      // safely.
      if (complex_part == NONE) {
        // Manual, safe formatting to mimic the default complex output.
        // This safely handles NaN values by letting snprintf format them
        // without overflow.
        double real_part = cptr->Re();
        double imag_part = cptr->Im();
        snprintf(buffer, sizeof(buffer), "%+.12lg%+.12lgj", real_part,
                 imag_part);
      } else {
        double part_value = 0.0;
        // switch on part
        ...
        // gl_error("group_recorder::read_line(): inconsistant complex_part
        // states!"); return 0;
        case REAL:
          part_value = cptr->Re();
          break;
        ...
        default: // <-- ADD A DEFAULT CASE
          gl_error("group_recorder::read_line(): invalid value for "
                   "complex_part, not recording value");
      ...
      offset = gl_get_value(curr->obj, get_addr(curr->obj, &(curr->prop)),
                            buffer, 127, &(curr->prop));
      if (0 == offset) {
        gl_error("group_recorder::read_line(): unable to get value for '%s' in "
                 "object '%s'",
                 curr->prop.name, curr->obj->name);
        /* TROUBLESHOOT
                An error occured while reading the specified property in one of
           the objects.
         */
    ...
    // space needed = 1 (for comma) + offset (for string) + 1 (for null
    // terminator)
    size_t space_needed = offset + 2;
    if ((index + space_needed) > line_size) {
      gl_error("group_recorder::read_line(): potential buffer overflow from a "
               "too-short automatically sized output value buffer");
      /* TROUBLESHOOT
              A potential buffer overflow was caught, most likely due to
         incorrect property representation assumptions for buffer allocation.
       */
      return 0;
    }
    // write to line_buffer
    // * lead with a comma on all entries, assume leading timestamp will NOT
    // print a comma
    size_t remaining_space = line_size - index;
    int chars_written =
        snprintf(line_buffer + index, remaining_space, ",%s", buffer);

    if (chars_written < 0 || (size_t)chars_written >= remaining_space) {
      // snprintf failed or the output was truncated.
      gl_error("group_recorder::read_line(): buffer overflow while writing "
               "data line.");
      return 0;
    }
    index += chars_written; // Use the actual characters written
  }
...
int group_recorder::write_line(TIMESTAMP t1, double t1dbl, bool deltacall) {
  ...
  if (TS_OPEN != tape_status) {
    gl_error("group_recorder::write_line(): trying to write line when the tape "
             "is not open");
    // could be ERROR or CLOSED, should not have happened
    return 0;
  }
  if (0 == rec_file) {
    gl_error("group_recorder::write_line(): no output file open and state is "
             "'open'");
    /* TROUBLESHOOT
            group_recorder claimed to be open and attempted to write to a file
       when a file had not successfully	opened.
  ...
  if (line_size <= 0 || line_buffer == 0) {
    gl_error("group_recorder::write_line(): output buffer not initialized "
             "(read_line() not called)");
    /* TROUBLESHOOT
            read_line was not called before write_line, indicating an internal
       logic error.
     */
    tape_status = TS_ERROR;
    return 0;
  ...
  // write time_str
  // recorder.c uses multiple formats, in the sense of "formatted or not".  This
  // has been fixed to match
  if (!format) {
    if (!deltacall) {
      if (0 == gl_localtime(t1, &dt)) {
        gl_error("group_recorder::write_line(): error when converting the sync "
                 "time");
        /* TROUBLESHOOT
    ...
    if (0 == gl_strtime(&dt, time_str, sizeof(time_str))) {
      gl_error("group_recorder::write_line(): error when writing the sync time "
               "as a string");
      /* TROUBLESHOOT
...
int group_recorder::flush_line() {
  ...
  if (0 == rec_file) {
    ...
    /* TROUBLESHOOT
            group_recorder claimed to be open and attempted to flush to a file
       when a file had not successfully	opened.
     */
    tape_status = TS_ERROR;
    return 0;
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_group_recorder(OBJECT *obj, TIMESTAMP t0,
                                                    PASSCONFIG pass) {
...
// Deltamode -- exposed function for commit (slightly different) to get the post
// update call
