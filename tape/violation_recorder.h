...
//	quickobjlist(OBJECT *o, PROPERTY *p){obj = o; next = 0; memcpy(&prop, p,
//sizeof(PROPERTY));} 	~quickobjlist(){if(next != 0) delete next;} 	void
//tack(OBJECT *o, PROPERTY *p){if(next){next->tack(o, p);} else {next = new
//quickobjlist(o, p);}} 	OBJECT *obj; 	PROPERTY prop; 	quickobjlist *next;
// };
...
public:
  ...
  void tack(OBJECT *o) {
    if (obj) {
      if (next) {
        ...
        // Core-callback allocation - replicates the function, but done manually
        // here
        next = (vobjlist *)gl_malloc(sizeof(vobjlist));

        if (next == NULL) {
          GL_THROW(const_cast<char *>("violation_recorder - Failed to allocate "
          ...
          again.  If the error persists, please submit you code and a bug report
  ...
  void add_ref(OBJECT *o) { ref_obj = o; }
  int length() {
    if (next) {
  ...
  void update_last(int i, double v, TIMESTAMP t,
                   int s) { // ugh.. this is a bit ugly
    if (i > 2 || i < 0)
      return;
    last_v[i] = v;
    last_t[i] = t;
      ...
      if (next) {
        ...
        if (next == NULL) {
          GL_THROW(const_cast<char *>(
              "violation_recorder - Failed to allocate space for unique list"));
          /*  TROUBLESHOOT
          While attempting to allocate the memory for a list of unique objects
          within the violation recorder, an error occurred.  Please check your
          file and try again.  If the error persists, please submit you code and
...
private:
  ...
  int check_line_thermal_limit(TIMESTAMP, vobjlist *, uniqueList *, int, double,
                               double);
  int check_xfrmr_thermal_limit(TIMESTAMP, vobjlist *, uniqueList *, int,
                                double, double);
  ...
  int fails_static_condition(OBJECT *, char *, double, double, double,
                             double *);
  int fails_static_condition(double, double, double, double, double *);
  int fails_dynamic_condition(vobjlist *, int, char *, TIMESTAMP, double,
                              double, double, double, double *);
  int fails_continuous_condition(vobjlist *, int, const char *, TIMESTAMP,
  ...
  int write_summary();
  // Memory allocation functions - functionalized for ease of use
  // (copy-paste-itis)
