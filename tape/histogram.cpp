...
void new_histogram(MODULE *mod) { new histogram(mod); }
...
histogram::histogram(MODULE *mod) {
  if (oclass == nullptr) {
...
#endif
    oclass =
        gl_register_class(mod, "histogram", sizeof(histogram), PC_PRETOPDOWN);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_char1024, "filename", PADDR(filename), PT_DESCRIPTION,
            "the name of the file to write", PT_char8, "filetype", PADDR(ftype),
            PT_DESCRIPTION, "the format to output a histogram in", PT_char32,
            "mode", PADDR(mode), PT_DESCRIPTION, "the mode of file output",
            PT_char1024, "group", PADDR(group), PT_DESCRIPTION,
            "the GridLAB-D group expression to use for this histogram",
            PT_char1024, "bins", PADDR(bins), PT_DESCRIPTION,
            "the specific bin values to use", PT_char256, "property",
            PADDR(property), PT_DESCRIPTION, "the property to sample",
            PT_double, "min", PADDR(min_val), PT_DESCRIPTION,
            "the minimum value of the auto-sized bins to use", PT_double, "max",
            PADDR(max_val), PT_DESCRIPTION,
            "the maximum value of the auto-sized bins to use", PT_double,
            "samplerate[s]", PADDR(sampling_interval), PT_DESCRIPTION,
            "the rate at which samples are read", PT_double, "countrate[s]",
            PADDR(counting_interval), PT_DESCRIPTION,
            "the reate at which bins are counted and written", PT_int32,
            "bin_count", PADDR(bin_count), PT_DESCRIPTION,
            "the number of auto-sized bins to use", PT_int32, "limit",
            PADDR(limit), PT_DESCRIPTION, "the number of samples to write",
            nullptr) < 1)
...
int parse_bin_val(char *tok, BIN *bin) {
  ...
  if (isdigit(*pos)) {
    ...
    low_set = 1;
             ...
             pos[1] == '.') { /* "everything less than the other value" */
    bin->low_val = -DBL_MAX;
    bin->low_inc = 0;
  } else if (*pos == '-') {
    bin->low_val = -DBL_MAX;
...
/**
 *	Local convenience function, used to extract the complex part of a
 * property string (if any) and add a null character to the property string
 * accordingly.
...
void histogram::test_for_complex(char *tprop, char *tpart) {
  ...
  if (sscanf(property.get_string(), "%[^.\n\t ].%s", tprop, tpart) == 2) {
    if (0 == memcmp(tpart, "real", 4)) {
      comp_part = REAL;
    } else if (0 == memcmp(tpart, "imag", 3)) {
      comp_part = IMAG;
    } else if (0 == memcmp(tpart, "mag", 3)) {
      comp_part = MAG;
    } else if (0 == memcmp(tpart, "ang", 3)) {
      comp_part = ANG;
    } else {
      comp_part = NONE;
...
int histogram::init(OBJECT *parent) {
  ...
  if (parent == nullptr) /* better have a group... */
  {
    ...
    while (group_obj = gl_find_next(group_list, group_obj)) {
      ...
      if (prop == nullptr) {
        gl_error("Histogram group is unable to find prop '%s' in class '%s' "
                 "for group '%s'",
                 property.get_string(), group_obj->oclass->name,
                 group.get_string());
        return 0;
      }
      /* check to see if all the group objects are in the same class, allowing
       * us to cache the target property */
      if (oclass == nullptr) {
    ...
    if (prop == nullptr) {
      gl_error(
          "Histogram parent '%s' of class '%s' does not contain property '%s'",
          parent->name ? parent->name : "(anon)", parent->oclass->name,
          property.get_string());
      return 0;
    } else {
  ...
  if ((bin_count > 0) && (min_val < max_val)) {
    ...
    if (bin_list == nullptr) {
      gl_error("Histogram malloc error: unable to alloc %i * %i bytes for %s",
               bin_count, sizeof(BIN),
               obj->name ? obj->name : "(anon. histogram)");
      return 0;
    }
  ...
  /*
   *	This will parse the bin strings as specified.  The valid basic form is
   * "(a..b)".  Brackets are optional, [ & ] are inclusive.  A dash may replace
   * the '..'. If a or b is not present, the bin will fill in a positve/negative
   * infinity.
   */
  {
    char *cptr = bins;
    ...
    /* would be better to count the number of times strtok succeeds, but this
     * should work -mh */
    // while(*cptr != 0){
    ...
    cptr = strtok(bincpy, ",\t\r\n\0");
    if (prop->ptype == PT_complex || prop->ptype == PT_double ||
        prop->ptype == PT_int16 || prop->ptype == PT_int32 ||
        prop->ptype == PT_int64 || prop->ptype == PT_float ||
        prop->ptype == PT_real) {
      for (i = 0; i < bin_count && cptr != nullptr; ++i) {
        if (parse_bin_val(cptr, bin_list + i) == 0) {
          gl_error("Histogram unable to parse '%s' in %s", cptr,
                   obj->name ? obj->name : "(unnamed histogram)");
          return 0;
        }
        cptr = strtok(nullptr,
                      ",\t\r\n\0"); /* minor efficiency gain to use the
                                       incremented pointer from parse_bin */
      }
    ...
    if (i < bin_count) {
      gl_error("Histrogram encountered a problem parsing bins for %s",
               obj->name ? obj->name : "(unnamed histogram)");
      return 0;
    }
  ...
  if (tf == nullptr)
    return 0;
  ops = tf->histogram; /* same mentality as a recorder, 'cept for the header
                          properties */
  if (ops == nullptr)
    return 0;
...
int histogram::feed_bins(OBJECT *obj) {
  ...
  case PT_complex:
    cval = (prop_ptr ? *gl_get_complex(obj, prop_ptr)
                     : *gl_get_complex_by_name(obj, property));
    ...
    case ANG:
      ...
      break;
    default:
      gl_error("Complex property with no part defined in %s",
  ...
  case PT_double:
    if (ival == 0)
      value = (prop_ptr ? *gl_get_double(obj, prop_ptr)
                        : *gl_get_double_by_name(obj, property.get_string()));
    for (i = 0; i < bin_count; ++i) {
    ...
    break;
  case PT_int16:
    ival = (prop_ptr ? *gl_get_int16(obj, prop_ptr)
                     : *gl_get_int16_by_name(obj, property.get_string()));
    value = 1.0;
  case PT_int32:
    if (value == 0.0) {
      ival = (prop_ptr ? *gl_get_int32(obj, prop_ptr)
                       : *gl_get_int32_by_name(obj, property.get_string()));
      value = 1.0;
    }
  case PT_int64:
    if (value == 0.0) {
      ival = (prop_ptr ? *gl_get_int64(obj, prop_ptr)
                       : *gl_get_int64_by_name(obj, property.get_string()));
      value = 1.0;
    }
  case PT_enumeration:
    if (value == 0.0) {
      ival = (prop_ptr ? *gl_get_enum(obj, prop_ptr)
                       : *gl_get_enum_by_name(obj, property.get_string()));
      value = 1.0;
    }
  case PT_set:
    if (value == 0.0) {
      ival = (prop_ptr ? *gl_get_set(obj, prop_ptr)
                       : *gl_get_set_by_name(obj, property.get_string()));
      value = 1.0;
    }
...
TIMESTAMP histogram::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if ((sampling_interval == -1.0 && t_count > t1) || sampling_interval == 0.0 ||
  ...
  if ((counting_interval == -1.0 && t_count < t1) || counting_interval == 0.0 ||
  ...
  return (next_count < next_sample && counting_interval > 0.0 ? next_count
                                                              : next_sample);
}
...
EXPORT int init_histogram(OBJECT *obj) {
  ...
  try {
    ...
    GL_THROW("%s (histogram:%d): %s", (obj->name ? obj->name : "(unnamed)"),
             obj->id, msg);
    return 0;
  }
}

static TIMESTAMP sync_histogram_impl(OBJECT *obj, TIMESTAMP t0,
                                     PASSCONFIG pass) {
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_histogram(OBJECT *obj, TIMESTAMP t0,
                                               PASSCONFIG pass) {
  return sync_histogram_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_histogram(OBJECT *obj, ...) {
  va_list args;
  TIMESTAMP t0;
  PASSCONFIG pass;
