...
private:
  ...
  vol_violation findOutLimit(bool firstCall, double array[], bool checkAbove,
                             double limitVal, int size);
  ...
  double *reactive_power_array; // array storing reactive power measured at the
                                // meter
  double *voltage_vll_array; // average line-to-line (or hot-to-hot) voltage at
                             // the meter
  double *voltage_vln_array; // average line-to-neutral (or hot-to-neutral)
                             // voltage at the meter
  double *
      voltage_unbalance_array; // array storing voltage unbalance per ANSI C84.1
  double price_parent;
  double bill_parent;
  ...
  double *air_temperature_array; // array storing air_temperature measured at
                                 // the house
  ...
  // since real_power_array and reactive_power_array have been defined for
  // triplex_meter already

  // Parameters related to capacitor and regulator objects
  double *count_array; // these _count member variables are doubles in
                       // capacitor.h and regulator.h
  ...
  double *real_power_loss_array; // array storing real power losses for the
                                 // whole feeder
  double *reactive_power_loss_array; // array storing real power losses for the
                                     // whole feeder
  ...
  int curr_index; // Index [0..interval_length-1] for current position of
                  // averaging array
};
