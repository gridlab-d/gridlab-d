/** $Id: player.c 1182 2008-12-22 22:08:36Z dchassin $
    ...
    - \p filetype specifies the source file extension, default is \p "txt".
 Valid types are \p txt, \p odbc, and \p memory.
    ...
    +1h,-1.0-0.2j
    \endverbatim
    The first line specifies the starting time of the tape and the initial
 value.  The value must be formatted as a string that is readable for the type
 of the data that is to receive the recording.  The remaining lines may have
 either absolute timestamps or relative times (indicated by a leading + sign).
 Relative times are useful if the \p loop parameter is used.  When a loop is
 performed, only lines with relative timestamps are read and all absolute times
 are ignored.
 @{
 **/
...
PROPERTY *player_link_properties(struct player *player, OBJECT *obj,
                                 char *property_list) {
  char *item;
  ...
  for (item = strtok(list, ","); item != nullptr; item = strtok(nullptr, ",")) {
    ...
    if (2 == sscanf(item, "%[A-Za-z0-9_.][%[^]\n,]", pstr.get_string(),
      ...
      if (unit == nullptr) {
        gl_error("sync_player:%d: unable to find unit '%s' for property '%s'",
                 obj->id, ustr.get_string(), pstr.get_string());
        return nullptr;
      }
      item = pstr;
    }
    ...
    if (prop != nullptr && target != nullptr) {
      if (unit != nullptr && target->unit == nullptr) {
        gl_warning("sync_player:%d: property '%s' is unitless, ignoring unit "
                   "conversion",
                   obj->id, item);
      } else if (unit != nullptr &&
                 0 == gl_convert_ex(target->unit, unit, &scale)) {
        gl_error(
            "sync_player:%d: unable to convert property '%s' units to '%s'",
...
int player_write_properties(struct player *my, OBJECT *thisplyr, OBJECT *obj,
                            PROPERTY *prop, const char *buffer) {
  int count = 0;
  const char delim[] = ",\n\r\t";
  char1024 bufcpy;
  ...
  for (p = prop; p != nullptr; p = p->next) {
    if (token == nullptr) {
      gl_error("sync_player:%d: not enough values on line: %s", thisplyr->id,
               buffer);
      return -1;
    }
    if (gl_set_value(obj, get_addr(obj, p), token, p) <= 0) {
      ...
      /*  TROUBLESHOOT
      While attempting to set a property value from a player, an error occurred.
...
static void rewind_player(struct player *my) { (*my->ops->rewind)(my); }

static void close_player(struct player *my) { (my->ops->close)(my); }
...
TIMESTAMP player_read(OBJECT *obj) {
  ...
  /* TODO move this to tape.c and make the variable available to all classes in
   * tape */
  static enum { UNKNOWN, ISO, US, EURO } dateformat = UNKNOWN;
  if (dateformat == UNKNOWN) {
    static char global_dateformat[8] = "";
    gl_global_getvar("dateformat", global_dateformat,
  ...
  while (true) {
    ...
    if (sscanf(result, "%63[^,],%1023[^\n\r;]", tbuf, valbuf) == 2) {
      trim(tbuf, timebuf);
      trim(valbuf, value);
      if (sscanf(timebuf, "%d-%d-%d %d:%d:%lf %4s", &Y, &m, &d, &H, &M, &S,
                 tz) == 7) {
      ...
      } else if (sscanf(timebuf, "%d-%d-%d %d:%d:%lf", &Y, &m, &d, &H, &M,
                        &S) >= 4) {
        // struct tm dt = {S,M,H,d,m-1,Y-1900,0,0,0};
        DATETIME dt;
        switch (dateformat) {
      ...
      } else if (sscanf(timebuf, "%" FMT_INT64 "d%1s", &t1, unit) == 2) {
        {
          ...
          if (result[0] == '+') { /* timeshifts have leading + */
            ...
            strcpy(my->next.value.get_string(), value.get_string() + voff);
          } else if (my->loop == my->loopnum) { /* absolute times are ignored on
                                                   all but first loops */
            my->next.ts = t1;
            while (value[voff] == ' ') {
              ++voff;
        ...
        if (my->loop == my->loopnum) {
          ...
          if ((obj->flags & OF_DELTAMODE) ==
              OF_DELTAMODE) /* Only request deltamode if we're explicitly
                               enabled */
          {
            if (my->all_events_delta) {
              if (my->sim_start_time < t1) {
  ...
  return my->next.ns == 0 ? my->next.ts
...
static TIMESTAMP sync_player_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  {             /* post this value */
    ...
    if (my->target != nullptr) {
      OBJECT *target =
          obj->parent ? obj->parent : obj; /* target myself if no parent */
      return_val =
  ...
  /* Apply an intermediate value, if necessary - mainly for "DELTA players in
   * non-delta situations" */
  if ((my->target != nullptr) && (my->delta_track.ts < t0) &&
      (my->delta_track.ns != 0)) {
    OBJECT *target =
        obj->parent ? obj->parent : obj; /* target myself if no parent */
    return_val = player_write_properties(my, obj, target, my->target,
                                         my->delta_track.value);
  ...
  /* Delta-mode catch - if we're not explicitly in delta mode and a nano-second
   * values pops up, try to advance past it */
  if (((obj->flags & OF_DELTAMODE) != OF_DELTAMODE) && (my->next.ns != 0) &&
      (my->status == TS_OPEN)) {
    /* Initial value for next time */
    temp_t = t1;

    /* Copied verbatim from above - just in case it still needs to be done - if
     * a first timestep is a deltamode timestep */
    if (my->target == nullptr)
      my->target = player_link_properties(my, obj->parent, my->property);
    if (my->target == nullptr) {
      gl_error("sync_player: Unable to find property \"%s\" in object %s",
               my->property.get_string(), obj->name ? obj->name : "(anon)");
      my->status = TS_ERROR;
      return TS_INVALID;
    }
    ...
    while ((my->next.ns != 0) && (my->next.ts <= t0)) {
      ...
      /* Apply the "current value", if it is relevant (player_next_value
       * contains the previous, so this will override it) */
    ...
    t1 = temp_t;
             ...
             (delta_mode_needed < t0)) /* Code to catch when gets stuck in
                                          deltamode (exits early) */
  {
    if (my->next.ts < t0) {
      ...
      /* Perform the update */
      temp_t = player_read(obj); // FIXME: I feel like this was intended to be
                                 // used, but currently isn't
      /* Check for error */
      if (temp_t == TS_INVALID) {
        return TS_INVALID;
      }
  ...
  /* Catch for next delta instances that may occur -- if doesn't pass a
   * reiteration, gets missed when nothing driving*/
  if ((obj->flags & OF_DELTAMODE) == OF_DELTAMODE) {
    ...
    /* Check for a final stoppping point -- if player value goes ns beyond
     * current stop time, it will get stuck and iterate */
    if ((my->next.ts == gl_globalstoptime) && (my->next.ns != 0) &&
        (t1 == gl_globalstoptime)) {
      /* Push it one second forward, just so GLD thinks it's done and can get
       * out */
      t1++;
    }
  }
