...
EXPORT int create_shaper(OBJECT **obj, OBJECT *parent) {
  *obj = gl_create_object(shaper_class);
  if (*obj != nullptr) {
    ...
    my->magnitude =
        1.0; /* magnitude of 1 event (only used when QUEUE is enabled) */
...
static void rewind_shaper(struct shaper *my) { my->ops->rewind(my); }

static void close_shaper(struct shaper *my) { my->ops->close(my); }
...
static TIMESTAMP shaper_read(OBJECT *obj, TIMESTAMP t0, unsigned int n) {
  ...
  struct tm *tval = localtime(
      &t); /* TODO: this should use machine local time, but sim local time */
  ...
  if (my->events <= 0) /* direct shape */
  {
    ...
    my->targets[n].value =
        my->magnitude *
        my->shape[tval->tm_mon][tval->tm_mday][tval->tm_wday][tval->tm_hour] *
        my->scale;
  ...
  } else /* shape queue */
  {
    ...
    /** @todo shaper queues shouldn't be uniform over the time step, but
     * cumulative uniform (tape, medium priority) */
    my->targets[n].ts =
        t0 + (TIMESTAMP)gl_random_uniform(&(obj->rng_state), 0, my->step);
  }
...
static TIMESTAMP shaper_update(OBJECT *obj, TIMESTAMP t0, unsigned int n) {
  ...
  } else if (*(my->targets[n].addr) == 0 &&
             gl_random_bernoulli(&(obj->rng_state), my->targets[n].value) ==
                 1) /* shape queue event starts */
  {
    ...
    *(my->targets[n].addr) = my->magnitude;
    ...
    t1 = my->targets[n].ts =
        t0 + (TIMESTAMP)(event_size * my->step * my->interval / my->magnitude);
    my->targets[n].value -= event_size; /* remove event from queue */
  } else if (*(my->targets[n].addr) != 0 &&
             t0 == my->targets[n].ts) /* shape queue event ends */
  {                                   /* event ends */
...
static TIMESTAMP sync_shaper_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  struct shaper *my = object_data<struct shaper>(obj);
  TIMESTAMP t1 = TS_NEVER;
  ...
  if (my->status == TS_OPEN) {
    ...
    if (my->targets == nullptr && my->group[0] != '\0') {
      ...
      my->targets =
          (SHAPERTARGET *)gl_malloc(sizeof(SHAPERTARGET) * my->n_targets);
      ...
      for (; (item = gl_find_next(object_list, item)) != nullptr; n++) {
        ...
        if (prop != nullptr) {
          if (prop->ptype == PT_double) {
            ...
            my->targets[n].addr = (double *)(gl_get_addr(
                item, prop->name)); /* pointer => int64 */
            if (my->targets[n].addr < (double *)(item + 1)) {
              if (my->targets[n].addr != nullptr)
                GL_THROW(
                    const_cast<char *>(
                        "gl_get_addr(OBJECT *obj=[%s (%s:%d)], char "
                        "*name='%s') return an invalid non-nullptr pointer"),
                    item->name ? item->name : "unnamed object",
                    item->oclass->name, obj->id, prop->name);
              else
                GL_THROW(
                    const_cast<char *>("property '%s' not found in %s (%s:%d)"),
                    prop->name, item->name ? item->name : "unnamed object",
                    item->oclass->name, item->id);
            }
            ...
            gl_warning("object %s:%d property %s is not a double",
                       item->oclass->name, item->id, prop->name);
        } else {
          gl_error("object %s:%d property %s not found in object %s",
                   obj->oclass->name, obj->id, my->property.get_string(),
                   item->oclass->name, item->id);
        }
      }
    }
  ...
  obj->clock = t0;
  if (t1 == 0) {
    gl_error("shaper:%i will return t1==0 ~ check the shaper's target "
             "property, \"%s\"",
             obj->id, my->property.get_string());
  }
  return t1 != TS_NEVER
             ? -t1
             : TS_NEVER; /* negative indicates a "soft" event which is only
                            considered for stepping, not for stopping */
}

#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_shaper(OBJECT *obj, TIMESTAMP t0,
                                            PASSCONFIG pass) {
  return sync_shaper_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_shaper(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
