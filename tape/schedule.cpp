...
range_list *cron_part(char *part, int first = 0) {
  ...
  for (i = 0; i < len && part[i] != 0; ++i) {
    if (isdigit(part[i])) {
      ...
      // make sure we've read a value, or that we're not interrupting a dash
      // operator
      if (has_num == 0) {
        gl_error("invalid cron part: comma without prior value, or comma "
                 "interrupting a dash operator");
        return 0;
      }
...
schedule_list *parse_cron(char *cron_str) {
  ...
  if (oobct > 0) {
    gl_error("out-of-bound cron time ranges can cause incorrect schedule "
             "calculations");
    if (mohr != 0)
      delete mohr;
    if (hodr != 0)
  ...
  listct = mohr->get_count() * hodr->get_count() * domr->get_count() *
           moyr->get_count() * dowr->get_count();
  if (listct < 1) {
    gl_error("non-postive product of the range list counts, something strange "
             "happened");
    return 0;
  }

  sheap = (schedule_list **)malloc(sizeof(schedule_list *) *
                                   listct); // not really heaped so much as a
                                            // combination of the range lists...
      ...
      for (k = 0; k < domr->get_count(); ++k) {
        for (l = 0; l < hodr->get_count(); ++l) {
          for (m = 0; m < mohr->get_count(); ++m) {
            sheap[idx] = new schedule_list(moh[m], hod[l], dom[k], moy[j],
                                           dow[i], value);
            ++idx;
          }
        }
...
schedule_list::schedule_list(range_list *moh, range_list *hod, range_list *dom,
                             range_list *moy, range_list *dow, double val) {
  moh_start = moh->min;
  moh_end = moh->max;
  hod_start = hod->min;
  hod_end = hod->max;
  ...
  } else if (dow_start == -1 && dow_end == -1 && dom_start != -1 &&
             dom_end != -1) {
...
schedule::schedule(MODULE *module) {
  ...
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(module, "schedule", sizeof(schedule), PC_PRETOPDOWN);
    if (oclass == nullptr)
      GL_THROW("unable to register object class implemented by %s", __FILE__);

    // publish the class properties
    if (gl_publish_variable(
            oclass, PT_double, "value", PADDR(currval), PT_ACCESS, PA_REFERENCE,
            PT_double, "default_value", PADDR(default_value), PT_timestamp,
            "next_ts", PADDR(next_ts), PT_ACCESS, PA_REFERENCE, PT_enumeration,
            "state", PADDR(state), PT_ACCESS, PA_REFERENCE, PT_KEYWORD, "INIT",
            TS_INIT, PT_KEYWORD, "OPEN", TS_OPEN, PT_KEYWORD, "DONE", TS_DONE,
            PT_KEYWORD, "ERROR", TS_ERROR, PT_char256, "error_msg",
            PADDR(errmsg), PT_ACCESS, PA_REFERENCE, PT_char256, "filename",
            PADDR(filename), PT_char1024, "schedule", PADDR(sched),
            nullptr) < 1)
  ...
  for (lptr = sched_list; lptr != nullptr; lptr = lptr->next) {
    ...
    for (child = lptr; child != nullptr; child = child->group_right) {
      ...
      if (child->dow_or_dom == schedule_list::wor_skip) {
        ...
        res = test_sched_dt(child->dom_start, child->dom_end, dt.day) +
              test_sched_dt(child->dow_start, child->dow_end, dt.weekday);
      } else {
      ...
      res += test_sched_dt(child->moh_start, child->moh_end,
                           dt.minute > 59 ? 59 : dt.minute);
      if (res == 2) {
        currval = child->scale;
...
TIMESTAMP schedule::sync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }
...
void new_schedule(MODULE *mod) { new schedule(mod); }
