...
int metrics_collector_writer::write_line(TIMESTAMP t1)
{
	...
	while (obj = gl_find_next(metrics_collectors, obj))
	{
		...
		if ((strcmp(temp_metrics_collector->parent_string, "triplex_meter") == 0) || (strcmp(temp_metrics_collector->parent_string, "meter") == 0))
		{
		...
		else if (strcmp(temp_metrics_collector->parent_string, "house") == 0)
		{
			...
			if (!house_objects.contains(key))
			{ // already made this house
	...
	try
	{
		...
		else
		{
			...
			if (size)
			{
				...
				{
				case 1:
					dataset->write(((std::vector<BillingMeter> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 2:
					dataset->write(((std::vector<House> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 3:
					dataset->write(((std::vector<Inverter> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 4:
					dataset->write(((std::vector<Capacitor> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 5:
					dataset->write(((std::vector<Regulator> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 6:
					dataset->write(((std::vector<Feeder> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 7:
					dataset->write(((std::vector<Transformer> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 8:
					dataset->write(((std::vector<Line> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				case 9:
					dataset->write(((std::vector<EVChargerDet> *)ptr)->data(), *mtype, memspace, *filespace);
					break;
				}
