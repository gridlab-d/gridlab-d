	...
	if (my->type == FT_FILE && my->multifile[0] != 0)
	{
		...
		if (my->inputfp != nullptr)
		{
			...
			if (0 != fgets(inbuffer, 1024, my->inputfp))
			{
				...
				{ // invalid input file or somesuch, we could error ... or we can trample onwards with our output file.
					rep = 0;
					fprintf(my->multifp, "# repetition %i\n", rep);
				}
...
RECORDER_MAP *link_multi_properties(OBJECT *obj, char *property_list)
{
	...
	for (itemptr = strtok(list, ","); itemptr != nullptr; itemptr = strtok(nullptr, ","))
	{
		...
		// if(2 == sscanf(itemptr, "%[^:]:%[^\n\r\0]", objstr, itemstr)){
		if (2 == sscanf(itemptr, " %[^:]:%s", objstr, itemstr))
		{ // changed this line because of conflicts in rh5
			item = itemstr;
