/** $Id: multi_recorder.c 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - \p format specifies whether to use raw timestamp instead of date-time
 format
        - \p interval specifies the sampling interval to use (0 means every
 pass, -1 means only transients)
        - \p limit specifies the maximum length limit for the number of samples
 taken
        - \p trigger specifies a trigger condition on a property to start
 recording the condition is specified in the format \p property \p comparison \p
 value
...
/*
 *	The idea behind the multi-recorder is that it is easier to reuse the
 * recorder structure with logic that anticipates the use of a different
 * mechanism for determining the object to associate with each property listed
 * for the recorder. The differences assumed by the multi-recorder are that the
 * parent is not used to determine the object that each property is associated
 * with, or that the recorder is associated with, that the property list will
 * include the name of the object that the property will correspond to (and
 * corresponds with the parent object if no object is parsed from the string),
 * and that the parent property may be null.  The multi-recorder will also use
 * the RECORDER_MAP struct in order to store the target properties, rather than
 * the PROPERTY struct. -Matt Hauer <d3p988>
 */
...
static int multi_recorder_open(OBJECT *obj) {
  ...
  if (my->type == FT_FILE && my->multifile[0] != 0) {
    if (my->interval < 1) {
      gl_error("multi_recorder: transient recorders cannot use multi-run "
               "output files");
      return 0;
    }
    ...
    if (my->multifp == nullptr) {
      gl_error("multi_recorder: unable to open \'%s\' for multi-run output",
               my->multitempfile.get_string());
      return 0;
    } else {
    ...
    if (my->inputfp != nullptr) {
      ...
      do {
        if (0 != fgets(inbuffer, 1024, my->inputfp)) {
          ...
          if (strncmp(inbuffer, "# file", strlen("# file")) == 0) {
            ...
            if (obj->parent != 0) {
              ...
              if (0 != strncmp(target, data, strlen(data))) {
                gl_error("multi_recorder:%i: re-recording target mismatch: was "
                         "%s, now %s",
                         obj->id, data, target.get_string());
                return 0;
              }
            } else {
            ...
            ;
          } else if (strncmp(inbuffer, "# interval", strlen("# interval")) ==
                     0) {
            // verify same interval
            int interval = atoi(data);
            if (interval != my->interval) {
              gl_error("multi_recorder:%i: re-recording interval mismatch: was "
                       "%i, now %i",
                       obj->id, interval, my->interval);
              return 0;
            }
            ...
            int limit = atoi(data);
            if (limit != my->limit) {
              gl_error("multi_recorder:%i: re-recording limit mismatch: was "
                       "%i, now %i",
                       obj->id, limit, my->limit);
              return 0;
            }
          } else if (strncmp(inbuffer, "# property", strlen("# property")) ==
                     0) {
            // verify same columns
            if (0 != strncmp(my->property, data, strlen(my->property))) {
              gl_error("multi_recorder:%i: re-recording property mismatch: was "
                       "%s, now %s",
                       obj->id, data, my->property.get_string());
              return 0;
            }
          ...
          gl_error("multi_recorder: error reading multi-read input file \'%s\'",
                   my->multifile.get_string());
          return 0;
          break;
        }
      ...
      if (0 != fgets(inbuffer, 1024, my->inputfp)) {
        ...
        if (strncmp(inbuffer, "# repetition", replen) == 0) {
          char *trim;
          rep = atoi(inbuffer.get_string() + replen +
                     1); // skip intermediate space
          ++rep;
          fprintf(my->multifp, "# repetition %i\n", rep);
          trim = strchr(inbuffer, '\n');
          if (trim)
            *trim = 0;
        } else { // invalid input file or somesuch, we could error ... or we can
                 // trample onwards with our output file.
          rep = 0;
          fprintf(my->multifp, "# repetition %i\n", rep);
        }
        ...
        while (tprop != nullptr) {
          ...
          if (len > lenmax) {
            gl_error(
                "multi_recorder: multi-run recorder output full property list "
                "is larger than the buffer, please start a new file!");
            return 0;
            break; // will still print everything up to this one
          }
          strncpy(propstr.get_string() + i, shortstr.get_string(),
      ...
      while (tprop != nullptr) {
        ...
        len = (int)strlen(shortstr);
        if (len > lenmax) {
          gl_error("multi_recorder: multi-run recorder output full property "
                   "list is larger than the buffer, please start a new file!");
          return 0;
          break; // will still print everything up to this one
        }
        strncpy(propstr.get_string() + i, shortstr.get_string(),
  ...
  // set out_property here
  {
    ...
    case HU_ALL:
      ...
      for (token = strtok(unit_buffer, ","); token != nullptr;
        ...
        if (prop_ptr == 0) {
          ...
          if (myobj == 0) {
            gl_error("multi_recorder:%d: unable to find object '%s'", obj->id,
                     objstr);
            return 0;
          }
        }
        ...
        if (unit_ptr == 0) {
          ...
          if (prop == 0) {
            gl_error("multi_recorder:%d: unable to find property '%s' for "
                     "object '%s'",
                     obj->id, propstr, myobj->name);
            return 0;
          }
          ...
          // has explicit unit
          if (2 ==
              sscanf(bigpropstr, "%[A-Za-z0-9_.][%[^]\n0]", propstr, unitstr)) {
            unit = gl_find_unit(unitstr);
            if (unit == 0) {
              gl_error("multi_recorder:%d: unable to find unit '%s' for "
                       "property '%s'",
                       obj->id, unitstr, propstr);
              return 0;
            }
        ...
        /*					// breakpoint
                                                if(3 == sscanf(token,
           "%[A-Za-z0-9_.][%[^]\n,\0]:%[A-Za-z0-9_.][%[^]\n,\0]", objstr,
           propstr, unitstr)){ myobj = gl_get_object(objstr); } else if(2 ==
           sscanf(token, "%[A-Za-z0-9_.][%[^]\n,\0]", propstr, unitstr)){ unit =
           gl_find_unit(unitstr); if(unit == 0){ gl_error("multi_recorder:%d:
           unable to find unit '%s' for property '%s'", obj->id, unitstr,
           propstr); return 0;
                                                        }
                                                }
                                                prop =
           gl_get_property(obj->parent, propstr); if(prop->unit != 0 && unit ==
           0){ unit = prop->unit;
                                                }*/
        ...
        if (myobj != obj->parent) {
          ...
          if (unit != 0) {
            snprintf(my->out_property.get_string() + offset, sizeof(my->out_property.get_string()) - offset, "%s%s:%s[%s]",
                    (first ? "" : ","), myobj->name, propstr,
            ...
            snprintf(my->out_property.get_string() + offset, sizeof(my->out_property.get_string()) - offset, "%s%s:%s",
                    (first ? "" : ","), myobj->name, propstr);
            offset +=
                strlen(propstr) + (first ? 0 : 1 + strlen(myobj->name) + 1);
          }
        } else {
...
static void close_multi_recorder(struct recorder *my) {
  ...
  if (my->multifp) {
    if (0 != fclose(my->multifp)) {
      gl_error("multi_recorder: unable to close multi-run temp file '%s'",
               my->multitempfile.get_string());
      perror("fclose(): ");
    }
    ...
    if (my->inputfp != nullptr) {
      ...
      if (0 != remove(my->multifile)) { // old file
        gl_error(
            "multi_recorder: unable to remove out-of-data multi-run file '%s'",
            my->multifile.get_string());
        perror("remove(): ");
      }
    }
    if (0 != rename(my->multitempfile, my->multifile)) {
      gl_error("multi_recorder: unable to rename multi-run file '%s' to '%s'",
               my->multitempfile.get_string(), my->multifile.get_string());
      perror("rename(): ");
    }
  }
}
  ...
  if (my->multifp != nullptr) {
    ...
    if (my->inputfp != nullptr) {
      ...
      do {
        if (0 == fgets(inbuffer, 1024, my->inputfp)) {
          if (feof(my->inputfp)) {
            // if there is no more data to append.rows() to, we're done with the
            // aggregate fclose(my->multifp); // happens in close()
      ...
      if (in_ts == nullptr) {
        gl_error("multi_recorder: unable to indentify a timestamp within the "
                 "line read from ");
      }
      ...
      if (strcmp(in_ts, ts) != 0) {
        gl_warning("multi_recorder: timestamp mismatch between current input "
                   "line and simulation time");
      }
...
RECORDER_MAP *link_multi_properties(OBJECT *obj, char *property_list) {
  ...
  char objstr[128];
  char itemstr[128];
  RECORDER_MAP *first = nullptr, *last = nullptr, *rmap;
  ...
  for (itemptr = strtok(list, ","); itemptr != nullptr;
       itemptr = strtok(nullptr, ",")) {
    cpart = 0;
    cid = -1;
    prop = nullptr;
    target = nullptr;
    ...
    // if(2 == sscanf(itemptr, "%[^:]:%[^\n\r\0]", objstr, itemstr)){
    if (2 == sscanf(itemptr, " %[^:]:%s", objstr,
                    itemstr)) { // changed this line because of conflicts in rh5
      item = itemstr;
      ...
      if (obj == 0) {
        gl_error("multi_recorder: no parent object and no specified target "
                 "object in '%s'",
                 itemstr);
        return 0;
      }
    ...
    if (2 == sscanf(item, "%[A-Za-z0-9_.][%[^]\n0]", pstr.get_string(),
      ...
      if (unit == nullptr) {
        gl_error("multi_recorder: unable to find unit '%s' for property '%s' "
                 "in object '%s %i'",
                 ustr.get_string(), pstr.get_string(), target_obj->oclass->name,
                 target_obj->id);
        return nullptr;
      }
      item = pstr;
    ...
    /* must occur w/ *cpart=0 before gl_get_property in order to properly
     * reformat the property name string */
    cpart = strchr(item, '.');
    if (cpart != nullptr) {
    ...
    if (rmap != nullptr && target != nullptr) {
      if (unit != nullptr && target->unit == nullptr) {
        gl_error("multi_recorder:%d: property '%s' is unitless, ignoring unit "
                 "conversion",
                 obj->id, item);
      } else if (unit != nullptr &&
                 0 == gl_convert_ex(target->unit, unit, &scale)) {
        gl_error(
            "multi_recorder:%d: unable to convert property '%s' units to '%s'",
            obj->id, item, ustr.get_string());
        return nullptr;
      }
      if (first == nullptr) {
        first = rmap;
      ...
      gl_error("multi_recorder: property '%s' not found in object '%s'", item,
               name);
      return nullptr;
    }
...
int read_multi_properties(struct recorder *my, OBJECT *obj, RECORDER_MAP *rmap,
  ...
  fake.unit = 0;
  for (r = rmap; r != nullptr && offset < size - 33; r = r->next) {
    if (offset > 0) {
      strcpy(buffer + offset++, ",");
    }
    //		offset += gl_get_value(r->obj, GETADDR(r->obj, &(r->prop)),
    //buffer + offset, size - offset - 1, &(r->prop)); /* pointer => int64 */
    if (r->prop.ptype == PT_double) {
      switch (my->line_units) {
      case LU_ALL:
        // cascade into 'default', as prop->unit should've been set, if there's
        // a unit available.
      case LU_DEFAULT:
        offset +=
            gl_get_value(r->obj, get_addr(r->obj, &(r->prop)), buffer + offset,
                         size - offset - 1, &(r->prop)); /* pointer => int64 */
        break;
      case LU_NONE:
        // copy value into local value, use fake PROP, feed into gl_get_vaule
        value = *gl_get_double(r->obj, &(r->prop));
        p2 = gl_get_property(r->obj, r->prop.name, nullptr);
        if (p2 == 0) {
          gl_error("multi_recorder: unable to locate %s.%s for LU_NONE", r->obj,
                   r->prop.name);
          return 0;
        }
        if (r->prop.unit != 0 && p2->unit != 0) {
          if (0 == gl_convert_ex(p2->unit, r->prop.unit, &value)) {
            gl_error("multi_recorder: unable to convert %s to %s for LU_NONE",
                     r->prop.unit, p2->unit);
          } else { // converted
            offset +=
                gl_get_value(r->obj, &value, buffer + offset, size - offset - 1,
                             &fake); /* pointer => int64 */
            ;
          }
        } else {
          offset += gl_get_value(r->obj, get_addr(r->obj, &(r->prop)),
                                 buffer + offset, size - offset - 1,
                                 &(r->prop)); /* pointer => int64 */
          ;
        }
        break;
      default:
      ...
      // offset += gl_get_value(obj,    GETADDR(obj,    p), buffer+offset,
      // size-offset-1, p); /* pointer => int64 */
      offset +=
          gl_get_value(r->obj, get_addr(r->obj, &(r->prop)), buffer + offset,
                       size - offset - 1, &(r->prop)); /* pointer => int64 */
    }
...
static TIMESTAMP sync_multi_recorder_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  if (my->status == TS_DONE) {
    close_multi_recorder(
        my); /* note: potentially called every sync pass for multiple timesteps,
                catch fp==nullptr in tape ops */
    return TS_NEVER;
  }
  ...
  if(obj->parent == nullptr){
          char tb[32];
          snprintf(buffer, sizeof(buffer), "'%s' lacks a parent object", obj->name ? obj->name :
  tb, snprintf(tb, sizeof(tb), "recorder:%i", obj->id)); close_recorder(my); my->status =
  TS_ERROR; return sync_multi_recorder_error(&obj, &my, buffer);
  }*/
  ...
  /* connect to property */
  if (my->rmap == nullptr) {
    my->rmap = link_multi_properties(
        obj->parent, my->property); // allowable use of obj->parent
  }
  ...
  my->property, obj->parent->oclass->name, obj->parent->id);
          close_multi_recorder(my);
          my->status = TS_ERROR;
  ...
  if (my->rmap == nullptr) {
    snprintf(buffer, sizeof(buffer), "'%s' contains a property reference that was not found",
            my->property.get_string());
    close_multi_recorder(my);
    my->status = TS_ERROR;
    return sync_multi_recorder_error(&obj, &my, buffer);
  ...
  if ((my->rmap != nullptr) && (my->interval == 0 || my->interval == -1)) {
    if (read_multi_properties(my, obj->parent, my->rmap, buffer,
                              sizeof(buffer)) == 0) // vestigal use of parent
    {
      // snprintf(buffer, sizeof(buffer),"unable to read property '%s' of %s %d", my->property,
      // obj->parent->oclass->name, obj->parent->id);
      snprintf(buffer, sizeof(buffer), "unable to read a property");
      close_multi_recorder(my);
      my->status = TS_ERROR;
      return sync_multi_recorder_error(&obj, &my, buffer);
    }
  }
  if ((my->rmap != nullptr) && (my->interval > 0)) {
  ...
  if (comparison != NONE) {
    ...
    if (actual != desired ||
        (my->status == TS_INIT && !multi_recorder_open(obj))) {
      // Fail out directly
...
EXPORT int isa_multi_recorder_impl(OBJECT *obj, char *classname) {
  return strcmp(classname, "multi_recorder") == 0;
}

#ifndef __APPLE__
extern "C" MODULE_API int isa_multi_recorder(OBJECT *obj, char *classname) {
  return isa_multi_recorder_impl(obj, classname);
}
#else
extern "C" MODULE_API int isa_multi_recorder(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
  char *classsname = va_arg(args, char *);
  va_end(args);
  return isa_multi_recorder_impl(obj, classsname);
}
#endif

TIMESTAMP sync_multi_recorder_error(OBJECT **obj, struct recorder **my,
                                    char2048 buffer) {
