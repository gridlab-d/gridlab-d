...
int csv_data_only = 0; /* enable this option to suppress addition of lines
                          starting with # in CSV */
int csv_keep_clean =
    0; /* enable this option to keep data flushed at end of line */
...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  gl_global_create(const_cast<char *>("tape::flush_interval"), PT_int32,
                   &flush_interval, nullptr);
  gl_global_create(const_cast<char *>("tape::csv_data_only"), PT_int32,
                   &csv_data_only, nullptr);
  gl_global_create(const_cast<char *>("tape::csv_keep_clean"), PT_int32,
                   &csv_keep_clean, nullptr);

  /* control delta mode */
  gl_global_create(const_cast<char *>("tape::delta_mode_needed"), PT_timestamp,
                   &delta_mode_needed, nullptr);

  /* register the first class implemented, use SHARE to reveal variables */
  player_class = gl_register_class(module, const_cast<char *>("player"),
                                   sizeof(struct player), PC_PRETOPDOWN);
  if (!player_class) {
    gl_error("Failed to register 'player_class'.");
    return nullptr;
  }
  // fprintf(stderr, "Player class registered successfully with name=%s and
  ...
  if (gl_publish_function(player_class, "create",
                          (FUNCTIONADDR)create_player) == NULL) {
    gl_error("publish_function: Failed to publish 'create_player' for "
             "'player_class'. Exiting...");
  ...
  recorder_class =
      gl_register_class(module, const_cast<char *>("recorder"),
                        sizeof(struct recorder), PC_POSTTOPDOWN | PC_OBSERVER);
  ...
  if (gl_publish_function(recorder_class, "create_recorder",
                          (FUNCTIONADDR)create_recorder) == NULL)
  ...
  if (gl_publish_variable(
          recorder_class, PT_double, "interval[s]",
          ((char *)&(my.dInterval) - (char *)&my), PT_enumeration, "output",
          ((char *)&(my.output) - (char *)&my), PT_KEYWORD, "SCREEN", SCREEN,
          PT_KEYWORD, "EPS", EPS, PT_KEYWORD, "GIF", GIF, PT_KEYWORD, "JPG",
          JPG, PT_KEYWORD, "PDF", PDF, PT_KEYWORD, "PNG", PNG, PT_KEYWORD,
          "SVG", SVG, PT_enumeration, "header_units",
          ((char *)&(my.header_units) - (char *)&my), PT_KEYWORD, "DEFAULT",
          HU_DEFAULT, PT_KEYWORD, "ALL", HU_ALL, PT_KEYWORD, "NONE", HU_NONE,
          PT_enumeration, "line_units",
          ((char *)&(my.line_units) - (char *)&my), PT_KEYWORD, "DEFAULT",
          LU_DEFAULT, PT_KEYWORD, "ALL", LU_ALL, PT_KEYWORD, "NONE", LU_NONE,
          nullptr) < 1)
    GL_THROW(
        const_cast<char *>("Could not publish property output for recorder"));

  /* register the first class implemented, use SHARE to reveal variables */
  shaper_class = gl_register_class(module, const_cast<char *>("shaper"),
                                   sizeof(struct shaper), PC_PRETOPDOWN);
  ...
  /* register the other classes as needed, */
  multi_recorder_class =
      gl_register_class(module, const_cast<char *>("multi_recorder"),
                        sizeof(struct recorder), PC_POSTTOPDOWN | PC_OBSERVER);
  multi_recorder_class->trl = TRL_QUALIFIED;
  if (gl_publish_variable(
          multi_recorder_class, PT_double, "interval[s]",
          ((char *)&(my.dInterval) - (char *)&my), PT_char1024, "property",
          ((char *)&(my.property) - (char *)&my), PT_char32, "trigger",
          ((char *)&(my.trigger) - (char *)&my), PT_char1024, "file",
          ((char *)&(my.file) - (char *)&my), PT_char8, "filetype",
          ((char *)&(my.filetype) - (char *)&my), PT_char32, "mode",
          ((char *)&(my.mode) - (char *)&my), PT_char1024, "multifile",
          ((char *)&(my.multifile) - (char *)&my), PT_int32, "limit",
          ((char *)&(my.limit) - (char *)&my), PT_char1024, "plotcommands",
          ((char *)&(my.plotcommands) - (char *)&my), PT_char32, "xdata",
          ((char *)&(my.xdata) - (char *)&my), PT_char32, "columns",
          ((char *)&(my.columns) - (char *)&my), PT_char32, "format",
          ((char *)&(my.format) - (char *)&my), PT_enumeration, "output",
          ((char *)&(my.output) - (char *)&my), PT_KEYWORD, "SCREEN", SCREEN,
          PT_KEYWORD, "EPS", EPS, PT_KEYWORD, "GIF", GIF, PT_KEYWORD, "JPG",
          JPG, PT_KEYWORD, "PDF", PDF, PT_KEYWORD, "PNG", PNG, PT_KEYWORD,
          "SVG", SVG, PT_enumeration, "header_units",
          ...
          nullptr) < 1)
    GL_THROW(const_cast<char *>(
        "Could not publish property output for multi_recorder"));

  /* register the other classes as needed, */
  collector_class =
      gl_register_class(module, const_cast<char *>("collector"),
                        sizeof(struct collector), PC_POSTTOPDOWN | PC_OBSERVER);
  ...
  if (gl_publish_variable(collector_class, PT_double, "interval[s]",
                          ((char *)&(my2.dInterval) - (char *)&my2),
                          nullptr) < 1)
    GL_THROW(
        const_cast<char *>("Could not publish property output for collector"));
...
EXPORT int check(void) {
  ...
  /* check players */
  {
    ...
    while ((obj = gl_find_next(players, obj)) != nullptr) {
      ...
      if (gl_findfile(pData->file, nullptr, F_OK, fpath, sizeof(fpath)) ==
          nullptr) {
        errcount++;
        gl_error("player %s (id=%d) uses the file '%s', which cannot be found",
                 obj->name ? obj->name : "(unnamed)", obj->id,
                 pData->file.get_string());
      }
    }
  ...
  /* check shapers */
  {
    OBJECT *obj = nullptr;
    FINDLIST *shapers =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "shaper", FT_END);
    while ((obj = gl_find_next(shapers, obj)) != nullptr) {
      struct shaper *pData = object_data<struct shaper>(obj);
      if (gl_findfile(pData->file, nullptr, F_OK, fpath, sizeof(fpath)) ==
          nullptr) {
        errcount++;
        gl_error("shaper %s (id=%d) uses the file '%s', which cannot be found",
                 obj->name ? obj->name : "(unnamed)", obj->id,
                 pData->file.get_string());
      }
    }
...
/* DELTA MODE SUPPORT */
/*
        Delta mode is supported by maintaining a list of recorders that are
   enabled for high-speed sampling.  This list is populated for sync_recorder
   based on whether OF_DELTAMODE is set.  Objects in the list are recorded to
   the output during the interupdate() operation and not during the object
   update (which is too slow and could happen at the wrong time).
 */
...
int delta_add_tape_device(OBJECT *obj, DELTATAPEOBJ tape_type) {
  ...
  if (temp_ll_item == nullptr) {
    gl_error("tape object:%d - unable to allocate space for deltamode",
             obj->id);
    /*  TROUBLESHOOT
    While attempting to allocate space for a tape module deltamode-capable
...
void enable_deltamode(TIMESTAMP t) {
  if ((t < delta_mode_needed) &&
      ((t - gl_globalclock) < 0x7fffffff)) // cannot exceed 31 bit integer
    delta_mode_needed = t;
}
...
EXPORT unsigned long deltamode_desired(int *flags) {
  ...
  if ((delta_mode_needed != TS_NEVER) && (tdiff < 0x7fffffff) &&
      (tdiff >= 0)) /* 31 bit limit */
...
EXPORT SIMULATIONMODE interupdate(MODULE *module, TIMESTAMP t0,
  ...
  double clock_val =
      (double)gl_globalclock + (double)delta_time / (double)DT_SECOND;
  ...
  auto integer_clock =
      (TIMESTAMP)clock_val; /* Whole seconds - update from global clock because
                               we could be in delta for over 1 second */
  int microseconds =
      (int)((clock_val - (int)(clock_val)) * 1000000 +
            0.5); /* microseconds roll-over - biased upward (by 0.5) */

  /* Recorders should only "fire" on the 0th iteration - may need to adjust if
   * "0" is implemented in deltamode */
  if (iteration_count_val == 0) {
    /* Set up recorder clock - ignore "first" entry since that is just steady
     * state */
    if ((integer_clock != t0) || (microseconds != 0)) {
      DATETIME rec_date_time;
      auto rec_integer_clock =
          (TIMESTAMP)recorder_delta_clock; /* Whole seconds - update from global
                                              clock because we could be in delta
                                              for over 1 second */
      int rec_microseconds =
          (int)((recorder_delta_clock - (int)(recorder_delta_clock)) * 1000000 +
                0.5); /* microseconds roll-over - biased upward (by 0.5) */
      ...
      while (index_item != nullptr) {
        if (index_item->obj_type == RECORDER) {
          ...
          char value[1024];
          // extern int read_properties(struct recorder *my, OBJECT *obj,
          // PROPERTY *prop, char *buffer, int size);

          /* See if we're in service */
          if ((obj->in_svc_double <= gl_globaldeltaclock) &&
              (obj->out_svc_double >= gl_globaldeltaclock)) {
            if (read_properties(my, obj->parent, my->target, value,
                                sizeof(value))) {
              if (!my->ops->write(my, recorder_timestamp, value)) {
                gl_error("recorder:%d: unable to write sample to file",
                         obj->id);
                return SM_ERROR;
              }
    ...
    if (index_item->obj_type == PLAYER) {
      ...
      if ((obj->in_svc_double <= gl_globaldeltaclock) &&
        ...
        if (t <= clock_val) {
          ...
          /* Check to make sure we've be initialized -- if a deltamode timestep
           * is first, it may be before this was initialized */
          if (my->target == nullptr) /* Not set yet */
          {
            /*  This fails on Mac builds under 4.0 for some odd reason.
            Commenting code and putting an error for now. Will be investigated
            further for 4.1 release.
            */

            gl_error("deltamode player: player '%s' has a deltamode timestep "
                     "starting it - please avoid this",
                     (obj->name ? obj->name : "(anon)"));
            /*  TROUBLESHOOT
            Starting a player with a deltamode timestep causes segmentation
            faults.  A fix was developed, but still has issues with certain
            platforms and must be investigated further.  For now, start all
            player files with non-deltamode timesteps.  This issues is
            documented at #1001 on the GridLAB-D Git site, and it expected to be
            resolved in release 4.1.
            */

            return SM_ERROR;

            /* *** Commenting -- all existing comment braces turned to ***
            *** Make sure we actually opened the file (this part is new -
            paranoia check *** if (my->status != TS_OPEN)
            {
                    gl_error("deltamode player: player '%s' has not been opened
            yet!",obj->name?obj->name:"(anon)");
                    ***  TROUBLESHOOT
                    While attempting to start a player with a deltamode
            timestep, it was found the player hasn't been opened yet. Please
            submit your code and a bug report via the issue system.  To work
            around this, put a non-deltamode timestep at the beginning of your
            player file
                    ***
            ...
            *** Make sure it worked ***
            if (my->target==nullptr){
                    gl_error("deltamode player: Unable to find property '%s' in
            object %s", my->property, obj->name?obj->name:"(anon)");
                    ***  TROUBLESHOOT
                    While attempting to link up the property of a player in
            deltamode, the property could not be found.  Make sure the object
            ...
            *** Do an initialization of it ***
            if (my->target!=nullptr)
            {
                    OBJECT *target = obj->parent ? obj->parent : obj; *** target
            myself if no parent *** player_write_properties(my, obj, target,
            my->target, my->next.value);
            }
          ...
          while (t <= clock_val) {
            ...
            // Set the value
            ret_value =
                gl_set_value(temp_obj, get_addr(temp_obj, my->target),
                             my->next.value, my->target); /* pointer => int64 */

            if (ret_value == 0) {
              gl_error("player:%d: %s - failure to set or convert property",
                       obj->id, (obj->name ? obj->name : "Unnamed"));
              /*  TROUBLESHOOT
              While trying to set the value from a player, an error occurred.
        ...
        if (my->next.ts != TS_NEVER) {
          ...
          /* Check to see if we're within the next second too, so we aren't
           * stuck in delta */
          if ((t >= clock_val) && (t < (clock_val + 1.0))) {
            ...
            if ((my->next.ns != 0) || my->all_events_delta) {
              mode = SM_DELTA;
              gl_verbose(
                  "Tape object:%d - %s - requested deltamode to continue",
                  obj->id, (obj->name ? obj->name : "Unnamed"));
            }
          }
        } /*End not TS_NEVER */
        else /* Player is done - set the flag variables, just to prevent some
                other issues in sync_player */
        {
          /* These prevent the code on player.c:488 from overwriting with an
           * older value */
          my->delta_track.ns = 0;
...
EXPORT STATUS postupdate(MODULE *module, TIMESTAMP t0, unsigned int64 dt) {
  ...
  TIMESTAMP rec_integer_clock =
      TS_INVALID; // FIXME: make sure this makes sense.
  int rec_microseconds;
  ...
  /* Only look for recorders first -- ensures they pull the last value before a
  ...
  while (index_item != nullptr) {
    if (index_item->obj_type == RECORDER) {
      if (!recorder_init_items) {
        ...
        rec_integer_clock =
            (TIMESTAMP)recorder_delta_clock; /* Whole seconds - update from
                                                global clock because we could be
                                                in delta for over 1 second */
        rec_microseconds =
            (int)((recorder_delta_clock - (int)(recorder_delta_clock)) *
                      1000000 +
                  0.5); /* microseconds roll-over - biased upward (by 0.5) */
        if (gl_localtime(rec_integer_clock, &rec_date_time) != 0) {
          if (global_dateformat[0] == '\0')
      ...
      if ((obj->in_svc_double <= gl_globaldeltaclock) &&
          (obj->out_svc_double >= gl_globaldeltaclock)) {
        if (read_properties(myrec, obj->parent, myrec->target, value,
                            sizeof(value))) {
          ...
          /* Update recorders so they don't "minimum timestep up" to where we
           * exited! */
      ...
      /* Map up function - have to do a modified version (C call), which is why
       * this looks odd*/
      ...
      if (temp_fxn == nullptr) {
        ...
        /*  TROUBLESHOOT
        While trying to map up a deltamode function for group_recorders, the
        function was not found. Please try again.  If the error persists, please
        submit your code and a bug report via the ticketing system.
        */
      ...
      /* Call the function */
      return_val = ((int (*)(OBJECT *, double))(*temp_fxn))(
          index_item->obj, gl_globaldeltaclock);

      /* Make sure it worked */
      if (return_val != 1) {
        gl_error("Failed to perform postupdate for group_recorder object");
        /*  TROUBLESHOOT
        While trying to perform the final write for a group recorder deltamode
        call, an error occurred.  Please look at the output console for more
        details.
        */
  ...
  while (index_item != nullptr) {
    ...
    if (index_item->obj_type == PLAYER) {
      ...
      if ((obj->in_svc_double <= gl_globaldeltaclock) &&
          (obj->out_svc_double >= gl_globaldeltaclock)) {
        if (((myplayer->next.ns != 0) && (myplayer->next.ts != t0)) ||
            ((myplayer->next.ts != t0) &&
             myplayer->all_events_delta)) /* See if we need to go back into
                                             deltamode, but make sure we aren't
                                             stuck! */
