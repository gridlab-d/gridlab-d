...
int violation_recorder::check_violation_3(TIMESTAMP t1)
{
	...
	for (curr = tplx_node_obj_list; curr != 0; curr = curr->next)
	{
		...
		nominal = get_observed_double_value(curr->obj, p_ptr) * 2.;
		if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2))
		{ // We are now checking all objects regardless of parent // && gl_object_isa(curr->obj->parent,"transformer")) {
...
int violation_recorder::check_violation_6(TIMESTAMP t1)
{
	...
	for (curr = inverter_obj_list; curr != 0; curr = curr->next)
	{
		if (curr->obj == 0)
			continue;
		if (has_phase(curr->obj, PHASE_S))
		{ // inverter connected to a triplex system, only checking one phase here
		...
		{ // assume we are a three phase inverter
			if (has_phase(curr->obj, PHASE_A))
			{
...
int violation_recorder::write_summary()
{
	...
	{
		if (0 > fprintf(f, "    TRANSFORMER (%i of %i transformers in violation),%i\n", xfrmr_list_v1->length(), xfrmr_obj_list->length(), get_violation_count(VIOLATION1, XFMR)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    OVERHEAD LINE (%i of %i lines in violation),%i\n", ohl_list_v1->length(), ohl_obj_list->length(), get_violation_count(VIOLATION1, OHLN)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    UNDERGROUND LINE (%i of %i lines in violation),%i\n", ugl_list_v1->length(), ugl_obj_list->length(), get_violation_count(VIOLATION1, UGLN)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX LINE (%i of %i lines in violation),%i\n", tplxl_list_v1->length(), tplxl_obj_list->length(), get_violation_count(VIOLATION1, TPXL)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    NODE (%i of %i nodes in violation),%i\n", node_list_v2->length(), node_obj_list->length(), get_violation_count(VIOLATION2, NODE)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX NODE (%i of %i nodes in violation),%i\n", tplx_node_list_v2->length(), tplx_node_obj_list->length(), get_violation_count(VIOLATION2, TPXN)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX METER (%i of %i meters in violation),%i\n", tplx_meter_list_v2->length(), tplx_mtr_obj_list->length(), get_violation_count(VIOLATION2, TPXM)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    COMMERCIAL METER (%i of %i meters in violation),%i\n", comm_meter_list_v2->length(), comm_mtr_obj_list->length(), get_violation_count(VIOLATION2, CMTR)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX NODE (%i of %i nodes in violation),%i\n", tplx_node_list_v3->length(), tplx_node_obj_list->length(), get_violation_count(VIOLATION3, TPXN)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX METER (%i of %i meters in violation),%i\n", tplx_meter_list_v3->length(), tplx_mtr_obj_list->length(), get_violation_count(VIOLATION3, TPXM)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    COMMERCIAL METER (%i of %i meters in violation),%i\n", comm_meter_list_v3->length(), comm_mtr_obj_list->length(), get_violation_count(VIOLATION3, CMTR)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "VIOLATION6 TOTAL (%i of %i inverters in violation),%i\n", inverter_list_v6->length(), inverter_obj_list->length(), get_violation_count(VIOLATION6)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    TRIPLEX METER (%i of %i meters in violation),%i\n", tplx_meter_list_v7->length(), tplx_mtr_obj_list->length(), get_violation_count(VIOLATION7, TPXM)))
		{
			return 0;
		}
	}
	...
	{
		if (0 > fprintf(f, "    COMMERCIAL METER (%i of %i meters in violation),%i\n", comm_meter_list_v7->length(), comm_mtr_obj_list->length(), get_violation_count(VIOLATION7, CMTR)))
		{
			return 0;
		}
	}
