...
int memory_open_player(struct player *my, char *fname, char *flags) {
  ...
  gl_error("memory_open_player(struct player *my='{...}', char *fname='%s', "
           "char *flags='%s'): global '%s' not defined",
           fname, flags, fname);
  return 0;
}
...
void memory_close_player(struct player *my) { free(my->memory); }
  ...
  while (fgets(line, sizeof(line), my->fp) != nullptr) {
    ...
    if (strcmp(group, "") != 0 &&
      ...
      if (sscanf(line, "%s %s %s %s %[^,],%[^,\n]", minute, hour, day, month,
                 weekday, value) < 6) {
        gl_error("%s(%d) : shape '%s' has specification '%s'", memory, linenum,
                 group, line);
        continue;
      }
      ...
      gl_error("%s(%d) : shape specification '%s' is not valid", memory,
               linenum, line);
    }
  }
  return 1;
...
int memory_open_recorder(struct recorder *my, char *fname, char *flags) {
  ...
  my->memory = (MEMORY *)malloc(sizeof(MEMORY));
  if (my->memory == nullptr) {
    gl_error("memory_open_recorder(struct recorder *my={...}, char "
             "*fname='%s', char *flags='%s'): %s",
  ...
  if (my->memory->buffer == nullptr) {
    gl_error("memory_open_recorder(struct recorder *my={...}, char "
             "*fname='%s', char *flags='%s'): global '%s' not found",
             fname, flags, fname);
    my->status = TS_DONE;
    return 0;
  }
...
void memory_close_recorder(struct recorder *my) { free(my->memory); }
...
int memory_open_collector(struct collector *my, char *fname, char *flags) {
  ...
  my->memory = (MEMORY *)malloc(sizeof(MEMORY));
  if (my->memory == nullptr) {
    gl_error("memory_open_collector(struct recorder *my={...}, char "
             "*fname='%s', char *flags='%s'): %s",
             fname, flags, strerror(errno));
    my->status = TS_DONE;
    return 0;
...
void memory_close_collector(struct collector *my) { free(my->memory); }
