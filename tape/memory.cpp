 ...
 * players
 */
int memory_open_player(player *my, char *fname, char *flags)
{
    /* "-" means stdin */
    ...
    gl_error("memory_open_player(player *my='{...}', char *fname='%s', char *flags='%s'): global '%s' not defined", fname,flags,fname);
    return 0;
}

char *memory_read_player(player *my,char *buffer,unsigned int size)
{
...
int memory_rewind_player(player *my)
{
    my->memory->index = 0;
    return 1;
}

void memory_close_player(player *my)
{
    free(my->memory);
}
...
int memory_open_shaper(shaper *my, char *fname, char *flags)
{
...
char *memory_read_shaper(shaper *my,char *buffer,unsigned int size)
{
    return fgets(buffer,size,my->fp);
}

int memory_rewind_shaper(shaper *my)
{
    return fseek(my->fp,SEEK_SET,0);
}

void memory_close_shaper(shaper *my)
{
}
 ...
 * recorders
 */
int memory_open_recorder(recorder *my, char *fname, char *flags)
{
    ...
    if (my->memory==NULL)
    {
        gl_error("memory_open_recorder(recorder *my={...}, char *fname='%s', char *flags='%s'): %s", fname, flags, strerror(errno));
        my->status = TS_DONE;
        return 0;
    }
    my->memory->buffer = gl_global_find(fname);
    if (my->memory->buffer==NULL)
    {
        gl_error("memory_open_recorder(recorder *my={...}, char *fname='%s', char *flags='%s'): global '%s' not found", fname, flags, fname);
        my->status = TS_DONE;
        return 0;
    }
    my->memory->index = 0;
...
int memory_write_recorder(recorder *my, char *timestamp, char *value)
{
...
void memory_close_recorder(recorder *my)
{
    free(my->memory);
}
 ...
 * collectors
 */
int memory_open_collector(collector *my, char *fname, char *flags)
{
    time_t now=time(NULL);
    OBJECT *obj=OBJECTHDR(my);
    ...
    if (my->memory==NULL)
    {
        gl_error("memory_open_collector(recorder *my={...}, char *fname='%s', char *flags='%s'): %s", fname, flags, strerror(errno));
        my->status = TS_DONE;
        return 0;
    }
    my->memory->buffer = gl_global_find(fname);
    if (my->memory->buffer==NULL)
    {
        gl_error("memory_open_collector(recorder *my={...}, char *fname='%s', char *flags='%s'): global '%s' not found", fname, flags, fname);
        my->status = TS_DONE;
        return 0;
    }
    my->memory->index = 0;
...
int memory_write_collector(collector *my, char *timestamp, char *value)
{
    if (my->memory->index >= my->memory->buffer->prop->size-1)
        return 0;
    else
...
void memory_close_collector(collector *my)
{
    free(my->memory);
}
