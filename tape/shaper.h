...
struct shaper {
  ...
  double magnitude; /**< the magnitude of a queue event (only used when \p
                       events > 0) */
  double events;    /* the number of queue events per interval (\p events = 0 =>
                       direct shape)*/
  /* private */
  TAPEOPS *ops;
  FILETYPE type;
  ...
  int16 step;   /* the duration of a single step in the shape integral (usually
                   3600s) */
