...
int file_open_player(struct player *my, char *fname, char *flags) {
  ...
  /* "-" means stdin */
  my->fp = (strcmp(fname, "-") == 0
                ? stdin
                : (gl_findfile(fname, nullptr, R_OK, ff, sizeof(ff))
                       ? fopen(ff, flags)
                       : nullptr));
  if (my->fp == nullptr) {
...
int file_rewind_player(struct player *my) { return fseek(my->fp, SEEK_SET, 0); }
  ...
  while (fgets(line, sizeof(line), my->fp) != nullptr) {
    ...
    if (strcmp(group, "") != 0 &&
      ...
      if (sscanf(line, "%s %s %s %s %[^,],%[^,\n]", min, hour, day, month,
                 weekday, value) < 6) {
        gl_error("%s(%d) : shape '%s' has specification '%s'", file, linenum,
                 group, line);
        continue;
      }
      /* minutes are ignored right now */
      if (min[0] != '*')
        gl_warning("%s(%d) : minutes are ignored in '%s'", file, linenum, line);
      hours = hourmap(hour);
      days = daymap(day);
      ...
      gl_error("%s(%d) : shape specification '%s' is not valid", file, linenum,
               line);
    }
  }
  return 1;
...
int file_rewind_shaper(struct shaper *my) { return fseek(my->fp, SEEK_SET, 0); }
...
void file_flush_recorder(struct recorder *my) { fflush(my->fp); }
...
void file_flush_collector(struct collector *my) { fflush(my->fp); }
