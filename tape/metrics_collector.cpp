...
PROPERTY *metrics_collector::propTriplexNomV = nullptr;
PROPERTY *metrics_collector::propTriplexV1 = nullptr;
PROPERTY *metrics_collector::propTriplexV2 = nullptr;
PROPERTY *metrics_collector::propTriplexV12 = nullptr;
PROPERTY *metrics_collector::propTriplexPrice = nullptr;
PROPERTY *metrics_collector::propTriplexBill = nullptr;
PROPERTY *metrics_collector::propTriplexP = nullptr;
PROPERTY *metrics_collector::propTriplexQ = nullptr;
PROPERTY *metrics_collector::propMeterNomV = nullptr;
PROPERTY *metrics_collector::propMeterVa = nullptr;
PROPERTY *metrics_collector::propMeterVb = nullptr;
PROPERTY *metrics_collector::propMeterVc = nullptr;
PROPERTY *metrics_collector::propMeterVab = nullptr;
PROPERTY *metrics_collector::propMeterVbc = nullptr;
PROPERTY *metrics_collector::propMeterVca = nullptr;
PROPERTY *metrics_collector::propMeterPrice = nullptr;
PROPERTY *metrics_collector::propMeterBill = nullptr;
PROPERTY *metrics_collector::propMeterP = nullptr;
PROPERTY *metrics_collector::propMeterQ = nullptr;
PROPERTY *metrics_collector::propHouseLoad = nullptr;
PROPERTY *metrics_collector::propHouseHVAC = nullptr;
PROPERTY *metrics_collector::propHouseAirTemp = nullptr;
PROPERTY *metrics_collector::propHouseCoolSet = nullptr;
PROPERTY *metrics_collector::propHouseHeatSet = nullptr;
PROPERTY *metrics_collector::propHouseSystemMode = nullptr;
PROPERTY *metrics_collector::propWaterLoad = nullptr;
PROPERTY *metrics_collector::propWaterSetPoint = nullptr;
PROPERTY *metrics_collector::propWaterDemand = nullptr;
PROPERTY *metrics_collector::propWaterTemp = nullptr;

PROPERTY *metrics_collector::propWaterLSetPoint = nullptr;
PROPERTY *metrics_collector::propWaterUSetPoint = nullptr;
PROPERTY *metrics_collector::propWaterLTemp = nullptr;
PROPERTY *metrics_collector::propWaterUTemp = nullptr;
PROPERTY *metrics_collector::propWaterElemLMode = nullptr;
PROPERTY *metrics_collector::propWaterElemUMode = nullptr;

PROPERTY *metrics_collector::propInverterS = nullptr;
PROPERTY *metrics_collector::propCapCountA = nullptr;
PROPERTY *metrics_collector::propCapCountB = nullptr;
PROPERTY *metrics_collector::propCapCountC = nullptr;
PROPERTY *metrics_collector::propRegCountA = nullptr;
PROPERTY *metrics_collector::propRegCountB = nullptr;
PROPERTY *metrics_collector::propRegCountC = nullptr;
PROPERTY *metrics_collector::propSwingSubLoad = nullptr;
PROPERTY *metrics_collector::propSwingMeterS = nullptr;

PROPERTY *metrics_collector::propTransformerOverloaded = nullptr;
PROPERTY *metrics_collector::propLineOverloaded = nullptr;

PROPERTY *metrics_collector::propChargeRate = nullptr;
PROPERTY *metrics_collector::propBatterySOC = nullptr;

bool metrics_collector::log_set =
    true; // if false, the first (some class) instance will print messages to
          // console

void new_metrics_collector(MODULE *mod) { new metrics_collector(mod); }
...
#endif
    oclass = gl_register_class(mod, const_cast<char *>("metrics_collector"),
                               sizeof(metrics_collector), PC_POSTTOPDOWN);
    if (oclass == nullptr)
      GL_THROW(const_cast<char *>(
                   "unable to register object class implemented by %s"),
               __FILE__);

    if (gl_publish_variable(oclass, PT_double, "interval[s]",
                            PADDR(interval_length_dbl), PT_DESCRIPTION,
                            "Interval at which the metrics_collector output is "
...
int metrics_collector::create() {
  ...
  time_array = nullptr;
  real_power_array = nullptr;
  reactive_power_array = nullptr;
  voltage_vll_array = nullptr;
  voltage_vln_array = nullptr;
  voltage_unbalance_array = nullptr;
  total_load_array = nullptr;
  hvac_load_array = nullptr;
  wh_load_array = nullptr;
  air_temperature_array = nullptr;
  set_cooling_array = nullptr;
  set_heating_array = nullptr;
  count_array = nullptr;
  real_power_loss_array = nullptr;
  reactive_power_loss_array = nullptr;
  wh_setpoint_array = nullptr;
  wh_demand_array = nullptr;
  wh_temp_array = nullptr;

  wh_l_setpoint_array = nullptr;
  wh_u_setpoint_array = nullptr;
  wh_l_temp_array = nullptr;
  wh_u_temp_array = nullptr;

  trans_overload_status_array = nullptr;
  line_overload_status_array = nullptr;

  charge_rate_array = nullptr;
  battery_SOC_array = nullptr;
...
int metrics_collector::init(OBJECT *parent) {
  ...
  if (parent == nullptr) {
    gl_error("metrics_collector must have a parent (triplex meter, house, "
             "waterheater, inverter, substation, or meter)");
    /*  TROUBLESHOOT
  ...
  // Find parent, if not defined, or if the parent is not a supported class,
  // throw an exception
  if (gl_object_isa(parent, "triplex_meter")) {
    parent_string = const_cast<char *>("triplex_meter");
    if (propTriplexNomV == nullptr)
      propTriplexNomV = gl_get_property(parent, "nominal_voltage");
    if (propTriplexPrice == nullptr)
      propTriplexPrice = gl_get_property(parent, "price");
    if (propTriplexBill == nullptr)
      propTriplexBill = gl_get_property(parent, "monthly_bill");
    if (propTriplexP == nullptr)
      propTriplexP = gl_get_property(parent, "measured_real_power");
    if (propTriplexQ == nullptr)
      propTriplexQ = gl_get_property(parent, "measured_reactive_power");
    if (propTriplexV1 == nullptr)
      propTriplexV1 = gl_get_property(parent, "voltage_1");
    if (propTriplexV2 == nullptr)
      propTriplexV2 = gl_get_property(parent, "voltage_2");
    if (propTriplexV12 == nullptr)
      propTriplexV12 = gl_get_property(parent, "voltage_12");
    if (!log_set) {
      log_set = log_me = true;
    }
  } else if (gl_object_isa(parent, "house")) {
    parent_string = const_cast<char *>("house");
    if (propHouseLoad == nullptr)
      propHouseLoad = gl_get_property(parent, "total_load");
    if (propHouseHVAC == nullptr)
      propHouseHVAC = gl_get_property(parent, "hvac_load");
    if (propHouseAirTemp == nullptr)
      propHouseAirTemp = gl_get_property(parent, "air_temperature");
    if (propHouseCoolSet == nullptr)
      propHouseCoolSet = gl_get_property(parent, "cooling_setpoint");
    if (propHouseHeatSet == nullptr)
      propHouseHeatSet = gl_get_property(parent, "heating_setpoint");
    if (propHouseSystemMode == nullptr)
      propHouseSystemMode = gl_get_property(parent, "system_mode");
  } else if (gl_object_isa(parent, "waterheater")) {
    parent_string = const_cast<char *>("waterheater");
    if (propWaterLoad == nullptr)
      propWaterLoad = gl_get_property(parent, "actual_load");
    if (propWaterSetPoint == nullptr)
      propWaterSetPoint = gl_get_property(parent, "tank_setpoint");
    if (propWaterDemand == nullptr)
      propWaterDemand = gl_get_property(parent, "water_demand");
    if (propWaterTemp == nullptr)
      propWaterTemp = gl_get_property(parent, "temperature");

    if (propWaterLSetPoint == nullptr)
      propWaterLSetPoint = gl_get_property(parent, "lower_tank_setpoint");
    if (propWaterUSetPoint == nullptr)
      propWaterUSetPoint = gl_get_property(parent, "upper_tank_setpoint");
    if (propWaterLTemp == nullptr)
      propWaterLTemp = gl_get_property(parent, "lower_tank_temperature");
    if (propWaterUTemp == nullptr)
      propWaterUTemp = gl_get_property(parent, "upper_tank_temperature");
    if (propWaterElemLMode == nullptr)
      propWaterElemLMode =
          gl_get_property(parent, "lower_heating_element_state");
    if (propWaterElemUMode == nullptr)
      propWaterElemUMode =
          gl_get_property(parent, "upper_heating_element_state");
  } else if (gl_object_isa(parent, "inverter")) {
    parent_string = const_cast<char *>("inverter");
    if (propInverterS == nullptr)
      propInverterS = gl_get_property(parent, "VA_Out");
  } else if (gl_object_isa(parent, "capacitor")) {
    parent_string = const_cast<char *>("capacitor");
    if (propCapCountA == nullptr)
      propCapCountA = gl_get_property(parent, "cap_A_switch_count");
    if (propCapCountB == nullptr)
      propCapCountB = gl_get_property(parent, "cap_B_switch_count");
    if (propCapCountC == nullptr)
      propCapCountC = gl_get_property(parent, "cap_C_switch_count");
  } else if (gl_object_isa(parent, "regulator")) {
    parent_string = const_cast<char *>("regulator");
    if (propRegCountA == nullptr)
      propRegCountA = gl_get_property(parent, "tap_A_change_count");
    if (propRegCountB == nullptr)
      propRegCountB = gl_get_property(parent, "tap_B_change_count");
    if (propRegCountC == nullptr)
      propRegCountC = gl_get_property(parent, "tap_C_change_count");
    ...
    if ((pval == nullptr) || (pval->ptype != PT_enumeration)) {
      GL_THROW(
          const_cast<char *>(
              "metrics_collector:%s failed to map bustype variable from %s"),
          obj->name ? obj->name : "unnamed",
          parent->name ? parent->name : "unnamed");
      /*  TROUBLESHOOT
      While attempting to set up the deltamode interfaces and calculations with
      powerflow, the required interface could not be mapped. Please check your
      GLM and try again.  If the error persists, please submit a trac ticket
      with your code.
      */
    }
    ...
    if (*meter_bustype != 2) {
      gl_error("If a metrics_collector is attached to a substation, it must be "
               "a SWING bus");
      return 0;
    }
    parent_string = const_cast<char *>("swingbus");
    if (propSwingSubLoad == nullptr)
      propSwingSubLoad =
          gl_get_property(parent, const_cast<char *>("distribution_load"));
  } else if (gl_object_isa(parent, const_cast<char *>("meter"))) {
    parent_string = const_cast<char *>("meter"); // unless it's a swing bus
    if (propMeterNomV == nullptr)
      propMeterNomV = gl_get_property(parent, "nominal_voltage");
    if (propMeterPrice == nullptr)
      propMeterPrice = gl_get_property(parent, "price");
    if (propMeterBill == nullptr)
      propMeterBill = gl_get_property(parent, "monthly_bill");
    if (propMeterP == nullptr)
      propMeterP = gl_get_property(parent, "measured_real_power");
    if (propMeterQ == nullptr)
      propMeterQ = gl_get_property(parent, "measured_reactive_power");
    if (propMeterVa == nullptr)
      propMeterVa = gl_get_property(parent, "voltage_A");
    if (propMeterVb == nullptr)
      propMeterVb = gl_get_property(parent, "voltage_B");
    if (propMeterVc == nullptr)
      propMeterVc = gl_get_property(parent, "voltage_C");
    if (propMeterVab == nullptr)
      propMeterVab = gl_get_property(parent, "voltage_AB");
    if (propMeterVbc == nullptr)
      propMeterVbc = gl_get_property(parent, "voltage_BC");
    if (propMeterVca == nullptr)
      propMeterVca = gl_get_property(parent, "voltage_CA");
    PROPERTY *pval = gl_get_property(parent, const_cast<char *>("bustype"));
    if ((pval != nullptr) && (pval->ptype == PT_enumeration)) {
      auto *meter_bustype = (enumeration *)get_addr(parent, pval);
      if (*meter_bustype == 2) {
        parent_string = const_cast<char *>("swingbus");
        if (propSwingMeterS == nullptr)
          propSwingMeterS =
              gl_get_property(parent, const_cast<char *>("measured_power"));
      }
    }
  } else if (gl_object_isa(parent, "transformer")) {
    parent_string = const_cast<char *>("transformer");
    if (propTransformerOverloaded == nullptr)
      propTransformerOverloaded = gl_get_property(parent, "overloaded_status");
  } else if (gl_object_isa(parent, "line")) {
    parent_string = const_cast<char *>("line");
    if (propLineOverloaded == nullptr)
      propLineOverloaded = gl_get_property(parent, "overloaded_status");
  } else if (gl_object_isa(parent, "evcharger_det")) {
    parent_string = const_cast<char *>("evcharger_det");
    if (propChargeRate == nullptr)
      propChargeRate = gl_get_property(parent, "actual_charge_rate");
    if (propBatterySOC == nullptr)
      propBatterySOC = gl_get_property(parent, "battery_SOC");
  } else {
    gl_error("metrics_collector allows only these parents: triplex meter, "
             "house, waterheater, inverter, substation, meter, capacitor, "
             "regulator, transformer, line.");
    /*  TROUBLESHOOT
    Check the parent object of the metrics_collector.
    */
    return 0;
  ...
  if (strcmp(parent_string, "triplex_meter") == 0) {
    ...
    if (metrics == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate JSON metrics array"),
               obj->id);
    }
    ...
    // In this work, only when a metrics_collector is attached to a swing-bus,
    // the feeder losses are recorded Find all the link objects for collecting
    // loss values
    link_objects = gl_find_objects(FL_NEW, FT_MODULE, SAME, "powerflow",
                                   FT_END); // find all link objects
    if (link_objects == nullptr) {
      gl_warning("No link objects were found.");
      /* TROUBLESHOOT
      No link objects were found in your .glm file. In this way, there is no
      losses calculated.
      */
    }
    if (parent->name != nullptr) {
    ...
    if (metrics == nullptr) {
      GL_THROW(
          "metrics_collector %d::init(): Failed to allocate JSON metrics array",
          obj->id);
    }
  ...
  if (interval_length <= 0) {
    gl_error("metrics_collector::init(): invalid interval of %i, must be "
             "greater than 0",
             interval_length);
    /* TROUBLESHOOT
  ...
  if (time_array == nullptr) {
    GL_THROW(const_cast<char *>(
                 "metrics_collector %d::init(): Failed to allocate time array"),
             obj->id);
    /*  TROUBLESHOOT
    While attempting to allocate the array, an error was encountered.
    Please try again.  If the error persists, please submit a bug report via the
    Trac system.
    */
  }
  ...
  if ((strcmp(parent_string, "triplex_meter") == 0) ||
      (strcmp(parent_string, "meter") == 0)) {
    // Allocate real power array
    real_power_array = (double *)gl_malloc(vector_length * sizeof(double));
    // Check
    if (real_power_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate real power array"),
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
      the Trac system.
      */
    }
    ...
    // Check
    if (reactive_power_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate reactive power array"),
               obj->id);
      /*  TROUBLESHOOT
    ...
    // Check
    if (voltage_vln_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate average vln voltage array"),
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    voltage_unbalance_array =
        (double *)gl_malloc(vector_length * sizeof(double));
    // Check
    if (voltage_unbalance_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate voltage unbalance array"),
               obj->id);
      /*  TROUBLESHOOT
  ...
  // If parent is house
    ...
    // Check
    if (air_temperature_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate air_temperature array"),
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    // Check
    if (set_cooling_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate set_cooling_array array"),
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    // Check
    if (set_heating_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate set_heating_array array"),
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
  ...
  // If parent is waterheater
    ...
    // Check
    if (wh_load_array == nullptr) {
      GL_THROW(
          const_cast<char *>(
              "metrics_collector %d::init(): Failed to allocate wh_load array"),
          obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    // Check
    if (wh_setpoint_array == nullptr) {
      GL_THROW(
          "metrics_collector %d::init(): Failed to allocate wh_setpoint array",
          obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    // Check
    if (wh_demand_array == nullptr) {
      GL_THROW(
          "metrics_collector %d::init(): Failed to allocate wh_demand array",
          obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    // Check
    if (wh_temp_array == nullptr) {
      GL_THROW("metrics_collector %d::init(): Failed to allocate wh_temp array",
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    if (wh_l_setpoint_array == NULL) {
      GL_THROW("metrics_collector %d::init(): Failed to allocate wh_l_setpoint "
               "array",
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
    ...
    if (wh_u_setpoint_array == NULL) {
      GL_THROW("metrics_collector %d::init(): Failed to allocate wh_u_setpoint "
               "array",
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
    ...
    if (wh_l_temp_array == NULL) {
      GL_THROW(
          "metrics_collector %d::init(): Failed to allocate wh_l_temp array",
          obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
    ...
    if (wh_u_temp_array == NULL) {
      GL_THROW(
          "metrics_collector %d::init(): Failed to allocate wh_u_temp array",
          obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
      Please try again.  If the error persists, please submit a bug report via
  ...
  // If parent is meter
    ...
    // Check
    if (real_power_loss_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate real power loss array"),
               obj->id);
      /*  TROUBLESHOOT
    ...
    reactive_power_loss_array =
        (double *)gl_malloc(vector_length * sizeof(double));
    // Check
    if (reactive_power_loss_array == nullptr) {
      GL_THROW(const_cast<char *>("metrics_collector %d::init(): Failed to "
                                  "allocate reactive power loss array"),
               obj->id);
      /*  TROUBLESHOOT
    ...
    // Check
    if (trans_overload_status_array == nullptr) {
      GL_THROW("metrics_collector %d::init(): Failed to allocate overload "
               "status array",
               obj->id);
      /*  TROUBLESHOOT
      While attempting to allocate the array, an error was encountered.
    ...
    for (int i = 0; i < vector_length; i++)
      trans_overload_status_array[i] = 0;
  } else if (strcmp(parent_string, "line") == 0) {
    line_overload_status_array = (int *)gl_malloc(vector_length * sizeof(int));
    // Check
    if (line_overload_status_array == nullptr) {
      GL_THROW("metrics_collector %d::init(): Failed to allocate overload "
               "status array",
               obj->id);
      /*  TROUBLESHOOT
  ...
  // else not possible come to this step
    ...
    gl_error(
        "metrics_collector must have a triplex meter, meter, house, "
        "waterheater, inverter, swing-bus, transformer or line as its parent");
    /*  TROUBLESHOOT
    Check the parent object of the metrics_collector. The metrics_collector is
    only able to be childed via a triplex meter or a house or an inverter when
    connecting into powerflow systems.
    */
    return 0;
  }
...
int metrics_collector::commit(TIMESTAMP t1) {
  ...
  if (write_now) {
    if (0 == write_line(t1, obj)) {
      gl_error("metrics_collector::commit(): error when aggregating values "
               "over the interval");
      return 0;
    }
    write_now = false;
...
int metrics_collector::read_line(OBJECT *obj) {
  ...
  if (strcmp(parent_string, "triplex_meter") == 0) {
    ...
    reactive_power_array[curr_index] =
        *gl_get_double(obj->parent, propTriplexQ);
    ...
    // compliance with C84.1; unbalance defined as max deviation from average /
    // average, here based on 1-N and 2-N
    ...
    voltage_unbalance_array[curr_index] =
        vdev / vll; // max deviation from Vll / average Vll
  } else if (strcmp(parent_string, "house") == 0) {
    ...
    air_temperature_array[curr_index] =
        *gl_get_double(obj->parent, propHouseAirTemp);
    set_cooling_array[curr_index] =
        *gl_get_double(obj->parent, propHouseCoolSet);
    set_heating_array[curr_index] =
        *gl_get_double(obj->parent, propHouseHeatSet);
    system_mode = *gl_get_enum(obj->parent, propHouseSystemMode);
  } else if (strcmp(parent_string, "waterheater") == 0) {
    wh_load_array[curr_index] = *gl_get_double(obj->parent, propWaterLoad);
    wh_setpoint_array[curr_index] =
        *gl_get_double(obj->parent, propWaterSetPoint);
    ...
    wh_l_setpoint_array[curr_index] =
        *gl_get_double(obj->parent, propWaterLSetPoint);
    wh_u_setpoint_array[curr_index] =
        *gl_get_double(obj->parent, propWaterUSetPoint);
    ...
    double opcount = *gl_get_double(obj->parent, propCapCountA) +
                     *gl_get_double(obj->parent, propCapCountB) +
                     *gl_get_double(obj->parent, propCapCountC);
    count_array[curr_index] = opcount;
  } else if (strcmp(parent_string, "regulator") == 0) {
    double opcount = *gl_get_double(obj->parent, propRegCountA) +
                     *gl_get_double(obj->parent, propRegCountB) +
                     *gl_get_double(obj->parent, propRegCountC);
    count_array[curr_index] = opcount;
  } else if (strcmp(parent_string, "swingbus") == 0) {
    // Get VAfeeder values
    ...
    while (obj = gl_find_next(link_objects, obj)) {
      ...
      if (strcmp(oclassName, "overhead_line") == 0 ||
          strcmp(oclassName, "underground_line") == 0 ||
          strcmp(oclassName, "triplex_line") == 0 ||
          strcmp(oclassName, "transformer") == 0 ||
          strcmp(oclassName, "regulator") == 0 ||
          strcmp(oclassName, "switch") == 0 ||
          strcmp(oclassName, "fuse") == 0) {
  ...
  // else not possible come to this step
    ...
    gl_error("metrics_collector must have a triplex meter, house, waterheater, "
             "inverter, swing-bus, capacitor, regulator, transformer or line "
             "as its parent");
    /*  TROUBLESHOOT
    Check the parent object of the metrics_collector. The metrics_collector is
    only able to be childed via a triplex meter or a house or a waterheater or
    an inverter or a swing-bus when connecting into powerflow systems.
    */
    return 0;
  }
  ...
  if (curr_index == vector_length) {
    gl_error("metrics_collector wrapped its buffer for aggregating data, %d %d "
             "%ld\n",
             curr_index, vector_length, gl_globalclock);
    /*  TROUBLESHOOT
    The metrics_collector allocates a buffer large enough to cover its
    aggregation interval at 1-second time steps. Is the time step smaller than
    1s? Program logic error?
    */
  }
  return 1;
}
...
void metrics_collector::log_to_console(char *msg, TIMESTAMP t) {
  if (log_me) {
    printf("** %s: t = %" FMT_INT64 "d, next_write = %" FMT_INT64
           "d, curr_index = %i\n",
...
/**
        @return 1 on successful write, 0 on unsuccessful write, error, or when
 not ready
 **/
...
int metrics_collector::write_line(TIMESTAMP t1, OBJECT *obj) {
  ...
  bool bOverran = false;
  if (t1 > next_write) { // interpolate at the aggregation interval's actual end
                         // point
    bOverran = true;
  ...
  if ((strcmp(parent_string, "triplex_meter") == 0) ||
    ...
    metrics[MTR_REAL_ENERGY] =
        findAverage(real_power_array, curr_index) * interval_length / 3600;
    metrics[MTR_REAC_ENERGY] =
        findAverage(reactive_power_array, curr_index) * interval_length / 3600;

    //		metrics[MTR_BILL] = metrics[MTR_REAL_ENERGY] * price_parent /
    //1000; // price unit given is [$/kWh]
    ...
    double normVol = 1.0, aboveRangeA, belowRangeA, aboveRangeB, belowRangeB,
           belowOutage;
    if (strcmp(parent_string, "triplex_meter") == 0) {
    ...
    struct vol_violation vol_Vio = findOutLimit(first_write, voltage_vll_array,
                                                true, aboveRangeA, curr_index);
    metrics[MTR_ABOVE_A_DUR] = vol_Vio.durationViolation;
#ifdef ALL_MTR_METRICS
    metrics[MTR_ABOVE_A_CNT] = vol_Vio.countViolation;
#endif
    // Voltage below Range A
    vol_Vio = findOutLimit(first_write, voltage_vll_array, false, belowRangeA,
                           curr_index);
    ...
    vol_Vio = findOutLimit(first_write, voltage_vll_array, true, aboveRangeB,
                           curr_index);
    ...
    vol_Vio = findOutLimit(first_write, voltage_vll_array, false, belowRangeB,
                           curr_index);
    ...
    vol_Vio = findOutLimit(first_write, voltage_vll_array, false, belowOutage,
                           curr_index);
  ...
  // If parent is transformer
    ...
    metrics[TRANS_OVERLOAD_PERC] =
        countPerc(trans_overload_status_array, curr_index);
  }
  // If parent is line
  else if (strcmp(parent_string, "line") == 0) {
    metrics[LINE_OVERLOAD_PERC] =
        countPerc(line_overload_status_array, curr_index);
  } else if (strcmp(parent_string, "swingbus") == 0) {
    // real power data
    ...
    // Energy data
    metrics[FDR_REAL_ENERGY] =
        findAverage(real_power_array, curr_index) * interval_length / 3600;
    metrics[FDR_REAC_ENERGY] =
        findAverage(reactive_power_array, curr_index) * interval_length / 3600;
    ...
    metrics[FDR_AVG_REAC_LOSS] =
        findAverage(reactive_power_loss_array, curr_index);
    metrics[FDR_MED_REAC_LOSS] =
        findMedian(reactive_power_loss_array, curr_index);
  } else if (strcmp(parent_string, "evcharger_det") == 0) {
...
void metrics_collector::copyHistories(int from, int to) {
  ...
  if (voltage_vll_array)
    voltage_vll_array[to] = voltage_vll_array[from];
  if (voltage_vln_array)
    voltage_vln_array[to] = voltage_vln_array[from];
  if (voltage_unbalance_array)
    voltage_unbalance_array[to] = voltage_unbalance_array[from];
  if (total_load_array)
    total_load_array[to] = total_load_array[from];
  if (hvac_load_array)
    hvac_load_array[to] = hvac_load_array[from];
  if (air_temperature_array)
    air_temperature_array[to] = air_temperature_array[from];
  if (set_cooling_array)
    set_cooling_array[to] = set_cooling_array[from];
  if (set_heating_array)
    set_heating_array[to] = set_heating_array[from];
  if (wh_load_array)
    wh_load_array[to] = wh_load_array[from];
  if (count_array)
    count_array[to] = count_array[from];
  if (real_power_array)
    real_power_array[to] = real_power_array[from];
  if (reactive_power_array)
    reactive_power_array[to] = reactive_power_array[from];
  if (real_power_loss_array)
    real_power_loss_array[to] = real_power_loss_array[from];
  if (reactive_power_loss_array)
    reactive_power_loss_array[to] = reactive_power_loss_array[from];
  if (trans_overload_status_array)
    trans_overload_status_array[to] = trans_overload_status_array[from];
  if (line_overload_status_array)
    line_overload_status_array[to] = line_overload_status_array[from];
  if (charge_rate_array)
    charge_rate_array[to] = charge_rate_array[from];
  if (battery_SOC_array)
    battery_SOC_array[to] = battery_SOC_array[from];
}

void metrics_collector::interpolate(double *a, int idx, double denom,
                                    double top) {
...
void metrics_collector::interpolateHistories(int idx, TIMESTAMP t) {
  ...
  if (voltage_vll_array)
    interpolate(voltage_vll_array, idx, denom, top);
  if (voltage_vln_array)
    interpolate(voltage_vln_array, idx, denom, top);
  if (voltage_unbalance_array)
    interpolate(voltage_unbalance_array, idx, denom, top);
  if (total_load_array)
    interpolate(total_load_array, idx, denom, top);
  if (hvac_load_array)
    interpolate(hvac_load_array, idx, denom, top);
  if (air_temperature_array)
    interpolate(air_temperature_array, idx, denom, top);
  if (set_cooling_array)
    interpolate(set_cooling_array, idx, denom, top);
  if (set_heating_array)
    interpolate(set_heating_array, idx, denom, top);
  if (wh_load_array)
    interpolate(wh_load_array, idx, denom, top);
  if (count_array)
    interpolate(count_array, idx, denom, top);
  if (real_power_array)
    interpolate(real_power_array, idx, denom, top);
  if (reactive_power_array)
    interpolate(reactive_power_array, idx, denom, top);
  if (real_power_loss_array)
    interpolate(real_power_loss_array, idx, denom, top);
  if (reactive_power_loss_array)
    interpolate(reactive_power_loss_array, idx, denom, top);
  if (charge_rate_array)
    interpolate(charge_rate_array, idx, denom, top);
  if (battery_SOC_array)
    interpolate(battery_SOC_array, idx, denom, top);
}
...
double metrics_collector::findAverage(double array[], int length) {
  ...
  for (int i = 1; i < length; i++) {
    double dA =
        0.5 * (array[i] + array[i - 1]) * (time_array[i] - time_array[i - 1]);
    sum += dA;
  }
  return sum / tlen;
}

// helper class to find the median of a histogram, skipping explicit linear
// interpolation
struct HistBar {
  double height; // aka frequency of observations, or # samples at this average
                 // array value
  double v1;
  double width;
  double avg;
  ...
  bool operator<(const HistBar &bar) const { return avg < bar.avg; }
};
...
double metrics_collector::findMedian(double array[], int length) {
  ...
  for (int i = 1; i < length; i++) {
    Histogram.push_back(
        HistBar(time_array[i - 1], time_array[i], array[i - 1], array[i]));
  ...
  for (int i = 0; i < length - 1; i++) {
    if ((cume + Histogram[i].height) >= 0.5 * nobs) { // found it
      return Histogram[i].v1 +
             (0.5 * nobs - cume) * Histogram[i].width / Histogram[i].height;
    }
...
vol_violation metrics_collector::findOutLimit(bool firstCall, double array[],
                                              bool checkAbove, double limitVal,
                                              int length) {
    ...
    if (checkAbove) {
      if (y1 > 0.0 || y2 > 0.0) {
        ...
        if (crossed) {
          ...
          assert(t >= 0.0 && t <= dt);
          if (y1 >
              0.0) { // add the violating sub-interval to the total duration
  ...
  if (log_me) {
    printf("limit %g count %i duration %g\n", limitVal, result.countViolation,
           result.durationViolation);
  }
  return result;
}
...
EXPORT int create_metrics_collector(OBJECT **obj, OBJECT *parent) {
  ...
  try {
    *obj = gl_create_object(metrics_collector::oclass);
    ...
    std::cerr << "Unhandled general exception in " << __func__ << ": "
              << ex.what() << std::endl;
  }
  return rv;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_metrics_collector(OBJECT *obj,
                                                       TIMESTAMP t0,
                                                       PASSCONFIG pass) {
