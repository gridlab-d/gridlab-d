...
int metrics_collector::init(OBJECT *parent)
{
	...
	else if (gl_object_isa(parent, "waterheater"))
	{
		...
		if (propWaterElemLMode == nullptr)
			propWaterElemLMode = gl_get_property(parent, "lower_heating_element_state");
		if (propWaterElemUMode == nullptr)
			propWaterElemUMode = gl_get_property(parent, "upper_heating_element_state");
	}
	...
	else if (gl_object_isa(parent, "substation"))
	{ // must be a swing bus
		...
		if (propSwingSubLoad == nullptr)
			propSwingSubLoad = gl_get_property(parent,
		...
		if ((pval != nullptr) && (pval->ptype == PT_enumeration))
		{
			...
			if (*meter_bustype == 2)
			{
				...
				if (propSwingMeterS == nullptr)
					propSwingMeterS = gl_get_property(parent,
...
int metrics_collector::read_line(OBJECT *obj)
{
	...
	{
		double opcount = *gl_get_double(obj->parent, propCapCountA) + *gl_get_double(obj->parent, propCapCountB) + *gl_get_double(obj->parent, propCapCountC);
		count_array[curr_index] = opcount;
	}
	else if (strcmp(parent_string, "regulator") == 0)
	{
		double opcount = *gl_get_double(obj->parent, propRegCountA) + *gl_get_double(obj->parent, propRegCountB) + *gl_get_double(obj->parent, propRegCountC);
		count_array[curr_index] = opcount;
	}
	else if (strcmp(parent_string, "swingbus") == 0)
	{
...
int metrics_collector::write_line(TIMESTAMP t1, OBJECT *obj)
{
	...
	bool bOverran = false;
	if (t1 > next_write)
	{ // interpolate at the aggregation interval's actual end point
		bOverran = true;
...
double metrics_collector::findMedian(double array[], int length)
{
	...
	{
		if ((cume + Histogram[i].height) >= 0.5 * nobs)
		{ // found it
		...
		if (checkAbove)
		{
			if (y1 > 0.0 || y2 > 0.0)
			{
				...
				if (crossed)
				{
					...
					assert(t >= 0.0 && t <= dt);
					if (y1 > 0.0)
					{ // add the violating sub-interval to the total duration
				...
				{ // this whole interval violates the limit
					result.durationViolation += dt;
				}
			}
		}
		else
		{ // below; mirrors the checkAbove case
			if (y1 < 0.0 || y2 < 0.0)
			{
				if (y1 * y2 <= 0.0)
					crossed = true;
