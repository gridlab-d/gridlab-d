...
public:
  ...
  ~quickobjlist() {
    ...
    quickobjlist *current = next;
    next = nullptr; // Prevent double-free

    while (current != nullptr) {
      quickobjlist *temp = current->next;
  ...
  void tack(OBJECT *o, PROPERTY *p) {
    // Add validation
    if (o == nullptr) {
      GL_THROW("quickobjlist::tack(): object pointer is null");
      return;
    }

    if (p == nullptr) {
      GL_THROW("quickobjlist::tack(): property pointer is null for object %s",
    ...
    if (next) {
      ...
      try {
        ...
        if (!next) {
          GL_THROW("quickobjlist::tack(): memory allocation failed");
        }
      } catch (std::bad_alloc &) {
        GL_THROW("quickobjlist::tack(): memory allocation failed (exception)");
      }
    }
  }
...
public:
  ...
  inline bool get_strict(void) { return strict; }
  ...
  inline void set_strict(bool p) { strict = p; }
...
public:
  ...
  inline bool get_print_units(void) { return print_units; }
  ...
  inline void set_print_units(bool p) { print_units = p; }
...
public:
  ...
  inline int32 get_limit(void) { return limit; }
  ...
  inline void set_limit(int32 p) { limit = p; }
...
public:
  ...
  inline bool get_format(void) { return format; }
  ...
  inline void set_format(bool p) { format = p; }
...
protected:
  CPLPT complex_part; // Member variable of type `char[256]`.

public:
  // Static inline method to calculate the byte offset of the member
  // `complex_part`.
  ...
  inline CPLPT get_complex_part(void) {
    auto &mtx = SharedMutexManager::get_mutex(my());
    std::shared_lock<std::shared_mutex> lock(mtx);
    return complex_part;
  }
  ...
  inline void set_complex_part(const CPLPT &str) {
    auto &mtx = SharedMutexManager::get_mutex(my());
    std::unique_lock<std::shared_mutex> lock(mtx);
    complex_part = str;
...
public:
  ...
  inline double get_dInterval(void) { return dInterval; }
  ...
  inline void set_dInterval(double p) { dInterval = p; }
...
public:
  ...
  static inline size_t get_dFlush_interval_offset(void) {
    ...
    return reinterpret_cast<const char *>(
               &(current_defaults->dFlush_interval)) -
           reinterpret_cast<const char *>(current_defaults);
  }

  // Inline function to get the value of `dFlush_interval`.
  inline double get_dFlush_interval(void) { return dFlush_interval; }
  ...
  inline void set_dFlush_interval(double p) { dFlush_interval = p; }
};
