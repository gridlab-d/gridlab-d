...
EXPORT MODULE *subload(char *modname, MODULE **pMod, CLASS **cptr, int argc,
                       char **argv) {
  MODULE *mod = (MODULE *)malloc(sizeof(MODULE));
  CLASS *c = nullptr;
  memset(mod, 0, sizeof(MODULE));
  gl_output("glmjava: trying to subload \"%s\"", modname);
  java_init(callback, (JAVACALLBACKS *)getvar("jcallback", nullptr, nullptr),
            mod, modname, argc, argv);
  c = *cptr;
  while (c != nullptr) {
...
EXPORT TIMESTAMP sync_java(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  if (cfunc == nullptr) {
    gl_error("sync_java: unable to find long %s.sync(long, long, int)",
             obj->oclass->name);
    return 0;
  }
...
int notify_java(OBJECT *obj, NOTIFYMODULE msg) {
  ...
  jmethodID cfunc =
      jnienv->GetStaticMethodID(cls, "notify", "(JLjava.lang.String;)I");
  if (cfunc == nullptr) {
    /* acceptable omission */
    // gl_error("notify_java: unable to find int %s.notify(long, int)",
    // obj->oclass->name);
...
EXPORT int isa_java(OBJECT *obj, char *classname) {
  ...
  jmethodID cfunc =
      jnienv->GetStaticMethodID(cls, "isa", "(JLjava.lang.String;)I");
  if (cfunc == nullptr) {
    gl_error("isa_java: unable to find int %s.isa(long, String)",
             obj->oclass->name);
    return 0;
  }
...
EXPORT int64 plc_java(OBJECT *obj, TIMESTAMP t0) {
  ...
  if (cfunc == nullptr) {
    // reasonable omission
    // gl_error("plc_java: unable to find long %s.plc(long, long)",
    // obj->oclass->name);
...
EXPORT int recalc_java(OBJECT *obj) {
  ...
  if (cfunc == nullptr) {
    // reasonable omission
    // gl_error("recalc_java: unable to find void %s.recalc(long)",
    // obj->oclass->name);
...
CLASS *java_init(CALLBACKS *fntable, JAVACALLBACKS *jfntable, MODULE *module,
                 char *modulename, int argc, char *argv[]) {
  JavaVM *jvm = nullptr;
  ...
  jmethodID init_mid = jnienv->GetStaticMethodID(
      cls, "init", "(JLjava/lang/String;I[Ljava/lang/String;)J");

  if (init_mid == nullptr) {
    gl_error("javamod:init.cpp: unable to find \"int %s.init(long, string, "
             "int, string[])\"",
             modulename);
    return nullptr;
  }

  jobjectArray args = jnienv->NewObjectArray(
      argc, jnienv->FindClass("[Ljava/lang/String;"), nullptr);
  if (args == nullptr) {
    gl_error("javamod:init.cpp: unable to allocate args[] for %s.init()",
             modulename);
    return nullptr;
  }
  ...
  if (jmodname == nullptr) {
    gl_error("javamod:init.cpp: unable to allocate jmodname for %s.init()",
             modulename);
  }

  gl_output("javamod:init.cpp(): moduleaddr = %x", module);

  int64 rv = jnienv->CallStaticLongMethod(cls, init_mid, (int64)module,
                                          jmodname, argc, jargv);
  if (jnienv->ExceptionOccurred()) {
    jnienv->ExceptionDescribe();
  }
