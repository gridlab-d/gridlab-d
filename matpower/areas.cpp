...
#endif
{
  ...
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(oclass,
                            ...
                            PT_int16, "AREA", PADDR(AREA), PT_DESCRIPTION,
                            "area number", PT_int16, "REFBUS", PADDR(REFBUS),
                            PT_DESCRIPTION, "reference bus in this area",
                            nullptr) < 1)
...
EXPORT int init_areas(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    gl_error("init_areas(obj=%d;%s): %s", obj->id,
...
static TIMESTAMP sync_areas_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  ...
  try {
    ...
    gl_error("sync_areas(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
  }
  return TS_INVALID;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_areas(OBJECT *obj, TIMESTAMP t1,
                                           PASSCONFIG pass) {
