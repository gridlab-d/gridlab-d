...
#endif
{
  ...
  if (oclass == nullptr) {
    ...
    if (gl_publish_variable(
            ...
            PT_enumeration, "BUS_TYPE", PADDR(BUS_TYPE), PT_DESCRIPTION,
            "bus type", PT_KEYWORD, "PQ", 1, PT_KEYWORD, "PV", 2, PT_KEYWORD,
            "REF", 3, PT_KEYWORD, "ISOLATE", 4, PT_double, "PD[MW]", PADDR(PD),
            PT_DESCRIPTION, "real power demand", PT_double, "QD[MVAr]",
            PADDR(QD), PT_DESCRIPTION, "reactive power demand", PT_double,
            "GS[MW]", PADDR(GS), PT_DESCRIPTION, "shunt conductance", PT_double,
            "BS[MVAr]", PADDR(BS), PT_DESCRIPTION, "shunt susceptance",
            // PT_int16, "BUS_AREA", PADDR(BUS_AREA), PT_DESCRIPTION, "area
            // number",
            PT_double, "VM", PADDR(VM), PT_DESCRIPTION, "voltage magnitude",
            PT_double, "VA", PADDR(VA), PT_DESCRIPTION, "voltage angle",
            PT_double, "BASE_KV[kV]", PADDR(BASE_KV), PT_DESCRIPTION,
            "base voltage", PT_int16, "ZONE", PADDR(ZONE), PT_DESCRIPTION,
            "lose zone", PT_double, "VMAX", PADDR(VMAX), PT_DESCRIPTION,
            "maximum voltage magnitude", PT_double, "VMIN", PADDR(VMIN),
            PT_DESCRIPTION, "minimum voltage magnitude", PT_int16, "BASE_MVA",
            PADDR(BASE_MVA), PT_DESCRIPTION, "base MVA", PT_double, "LAM_P",
            PADDR(LAM_P), PT_DESCRIPTION,
            "Lagrange multiplier on real power mismatch (mu/MW)", PT_double,
            "LAM_Q", PADDR(LAM_Q), PT_DESCRIPTION,
            "Lagrange multiplier on reactive power mistmatch (mu/MW)",
            PT_double, "MU_VMAX", PADDR(MU_VMAX), PT_DESCRIPTION,
            "Kuhn-Tucker multiplier on upper voltage limit (mu/p.u.)",
            PT_double, "MU_VMIN", PADDR(MU_VMIN), PT_DESCRIPTION,
            "Kuhn-Tucker multiplier on lower voltage limit (mu/p.u.)",
            PT_enumeration, "IFHEADER", PADDR(ifheader), PT_DESCRIPTION,
            "if connected with distribution network", PT_KEYWORD, "Yes", 1,
            PT_KEYWORD, "No", 0,

            PT_complex, "CVoltageA", PADDR(CVoltageA), PT_DESCRIPTION,
            "complex voltage phase A of distribution network: Cvoltage.Mag() = "
            "VM, Cvoltage.Arg() = VA;",
            PT_complex, "CVoltageB", PADDR(CVoltageB), PT_DESCRIPTION,
            "complex voltage phase B of distribution network: Cvoltage.Mag() = "
            "VM, Cvoltage.Arg() = VA;",
            PT_complex, "CVoltageC", PADDR(CVoltageC), PT_DESCRIPTION,
            "complex voltage phase C of distribution network: Cvoltage.Mag() = "
            "VM, Cvoltage.Arg() = VA;",
            PT_double, "V_nom", PADDR(V_nom), PT_DESCRIPTION,
            "Normalized voltage of distribution network", PT_complex, "feeder0",
            PADDR(feeder0), PT_DESCRIPTION, "Feeder 0 Complex power",
            PT_complex, "feeder1", PADDR(feeder1), PT_DESCRIPTION,
            "Feeder 1 Complex power", PT_complex, "feeder2", PADDR(feeder2),
            PT_DESCRIPTION, "Feeder 2 Complex power", PT_complex, "feeder3",
            PADDR(feeder3), PT_DESCRIPTION, "Feeder 3 Complex power",
            PT_complex, "feeder4", PADDR(feeder4), PT_DESCRIPTION,
            "Feeder 4 Complex power", PT_complex, "feeder5", PADDR(feeder5),
            PT_DESCRIPTION, "Feeder 5 Complex power", PT_complex, "feeder6",
            PADDR(feeder6), PT_DESCRIPTION, "Feeder 6 Complex power",
            PT_complex, "feeder7", PADDR(feeder7), PT_DESCRIPTION,
            "Feeder 7 Complex power", PT_complex, "feeder8", PADDR(feeder8),
            PT_DESCRIPTION, "Feeder 8 Complex power", PT_complex, "feeder9",
            PADDR(feeder9), PT_DESCRIPTION, "Feeder 9 Complex power",
...
int bus::create(void) {
  ...
  /* TODO: set the context-free initial value of properties, such as random
   * distributions */
...
int bus::init(OBJECT *parent) {
  ...
  if (bus_this->BUS_TYPE == 3) {
    ...
    */
    ...
    branch *branch_obj;
    FINDLIST *branch_list =
        gl_find_objects(FL_NEW, FT_CLASS, SAME, "branch", FT_END);
    ...
    while (gl_find_next(gen_list, obj_gen) != nullptr) {
      ...
      if (find_gen == bus_map.end()) {
        // gl_error("Cannot find bus %s at generator
        // %s",Gen_parent_name,obj_gen->name);
        exit(1);
      }
...
TIMESTAMP bus::presync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  //{
  //	gl_error("There are %d branch bus and %d branch. Not
  //equal!\n",BUS_NUM_BRANCH,BRANCH_NUM); 	exit(1);
  // }
  ...
  if (tempbus->ifheader == 1) {
    ...
    sum_PD = tempbus->feeder0.Re() + tempbus->feeder1.Re() +
             tempbus->feeder2.Re() + tempbus->feeder3.Re() +
             tempbus->feeder4.Re() + tempbus->feeder5.Re() +
             tempbus->feeder6.Re() + tempbus->feeder7.Re() +
             tempbus->feeder8.Re() + tempbus->feeder9.Re();

    sum_QD = tempbus->feeder0.Im() + tempbus->feeder1.Im() +
             tempbus->feeder2.Im() + tempbus->feeder3.Im() +
             tempbus->feeder4.Im() + tempbus->feeder5.Im() +
...
TIMESTAMP bus::sync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  {
    // Run OPF solver
    if (solver_matpower(bus_BUS_I, branch_F_BUS, branch_T_BUS, gen_GEN_BUS,
                        gen_NCOST, tempbus->BASE_MVA) == 1) {
      gl_error("OPF Failed");
      exit(1);
    }
  }
...
TIMESTAMP bus::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  // printf("Bus %d, Real power %f, reactive power
...
EXPORT int init_bus(OBJECT *obj, OBJECT *parent) {
  try {
    ...
    gl_error("init_bus(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
  }
  return 0;
}

static TIMESTAMP sync_bus_impl(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass) {
  TIMESTAMP t2 = TS_NEVER;
  ...
  try {
    ...
    gl_error("sync_bus(obj=%d;%s): %s", obj->id,
             obj->name ? obj->name : "unnamed", msg);
  }
  return TS_INVALID;
}
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_bus(OBJECT *obj, TIMESTAMP t1,
                                         PASSCONFIG pass) {
  return sync_bus_impl(obj, t1, pass);
}
#else
/* On Mac OS X, the symbol name is different when compiled in C++ */
extern "C" MODULE_API TIMESTAMP _sync_bus(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
