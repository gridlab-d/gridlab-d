/** $Id: wholesale.cpp 4738 2014-07-03 00:55:39Z dchassin $
...
// int solver_matpower(double *rbus, unsigned int nbus, double *rgen, unsigned
// int ngen, 	double *rbranch, unsigned int nbranch, double *rgencost, unsigned
//int ngencost, 	double *rareas,	unsigned int nareas)
int solver_matpower(vector<unsigned int> bus_BUS_I,
  ...
  /*
          areas *list_areas;
          FINDLIST *areas_list =
     gl_find_objects(FL_NEW,FT_CLASS,SAME,"areas",FT_END); temp_obj = nullptr;
  ...
  */
  ...
  FINDLIST *gen_cost_list =
  gl_find_objects(FL_NEW,FT_CLASS,SAME,"gen_cost",FT_END); temp_obj = nullptr;
  ...
  */
  ...
  // FINDLIST *baseMVA_list =
  // gl_find_objects(FL_NEW,FT_CLASS,SAME,"baseMVA",FT_END); temp_obj = nullptr;
  ...
  rgencost =
      (double *)calloc(ngen * (GENCOST_ATTR + max_order), sizeof(double));
  ...
  rgencost = (double *)
  calloc(ngencost*(GENCOST_ATTR+max_order),sizeof(double));
  ...
  *mxGetPr(basemva_array) = rbaseMVA;
  ...
  for (unsigned int i = 0; i < nbus; i++) {
    ...
    */
    ...
    setObjectValue_Double2Complex_inDegree(
        temp_obj, "CVoltageA", obus[i + 7 * nbus] * obus[i + 9 * nbus] * 1000,
        obus[i + 8 * nbus]);
    setObjectValue_Double2Complex_inDegree(
        temp_obj, "CVoltageB", obus[i + 7 * nbus] * obus[i + 9 * nbus] * 1000,
        obus[i + 8 * nbus] + 2 / 3 * PI);
    setObjectValue_Double2Complex_inDegree(
        temp_obj, "CVoltageC", obus[i + 7 * nbus] * obus[i + 9 * nbus] * 1000,
        obus[i + 8 * nbus] - 2 / 3 * PI);
    setObjectValue_Double(temp_obj, "V_nom",
                          obus[i + 7 * nbus] * obus[i + 9 * nbus] * 1000);
  ...
  for (unsigned int i = 0; i < ngen; i++) {
    ...
    */
    ...
    for (unsigned int j = 0; j < NCOST; j++) {
      price +=
          pow(ogen[i + 1 * ngen], NCOST - 1 - j) * rgencost[i + (4 + j) * ngen];
