/** $Id: link.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The following operations are performed on each branch during a bottom-up
 sync event.

        The effective admittance Y is calculated by including half of the line
 charging capacitance \p B [Kundur 1993, p. 258]:

                \f[ \widetilde{Y}_{eff} = \widetilde{Y} + \jmath \frac{B}{2} \f]

        The admittance coefficient is the inverse of the transformer turns ratio
 \p n, if any [Kundur 1993, p. 236]:

                \f[ c \leftarrow \frac{1}{n} \f]

        The effective line self-admittance is the product of the admittance
 coefficient and component admittance

                \f[ \widetilde{Y}_c = \widetilde{Y}_{eff} c \f]

        Add the self-admittance and the shunt admittances to the busses [Kundur
 1993, p. 259]

                \f[ \Sigma \widetilde{Y}_{from} \leftarrow \Sigma
 \widetilde{Y}_{from} + \widetilde{Y}_c + \widetilde{Y}_c (c-1) \f]

                \f[ \Sigma \widetilde{Y}_{to} \leftarrow \Sigma
 \widetilde{Y}_{to} + \widetilde{Y}_c + \widetilde{Y}_{eff} (1-c) \f]
 ...
 \n
        ...
        Add the current injections to the busses
                ...
                \f[ \Sigma \widetilde{YV}_{from} \leftarrow \Sigma
 \widetilde{YV} - \widetilde{I}_{from} \f] \n

                \f[ \Sigma \widetilde{YV}_{to} \leftarrow \Sigma \widetilde{YV}
 - \widetilde{I}_{to} \f]

        Compute the line current (from -> to)

                \f[ \widetilde{I} \leftarrow \widetilde{I}_{from} -
 \widetilde{I}_{to} \f] \n
...
link::link(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    link_class = oclass = gl_register_class(
        mod, "link", sizeof(link), PC_BOTTOMUP | PC_UNSAFE_OVERRIDE_OMIT);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(oclass, PT_complex, "Y", PADDR(Y), PT_complex, "I",
                            PADDR(I), PT_double, "B", PADDR(B), PT_object,
                            "from", PADDR(from), PT_object, "to", PADDR(to),
                            nullptr) < 1)
...
TIMESTAMP link::sync(TIMESTAMP t0) {
  ...
  if (debug_link == 1) {
    ...
    if (obj->id == first) {
      printf("\n");
      printf("Link  From -> To             R        X        B        Y        "
             "         Ifrom             Ito               I               \n");
      printf("===== ====================== ======== ======== ======== "
             "================= ================= ================= "
             "=================\n");
    }
    printf("%2d-%2d %-9.9s -> %-9.9s %+8.4f %+8.4f %+8.4f %+8.4f%+8.4fj "
           "%+8.4f%+8.4fj %+8.4f%+8.4fj %+8.4f%+8.4fj\n",
           from->id, to->id, from->name, to->name, (complex(1, 0) / Y).Re(),
           (complex(1, 0) / Y).Im(), B, Y.Re(), Y.Im(), Ifrom.Re(), Ifrom.Im(),
           Ito.Re(), Ito.Im(), I.Re(), I.Im());
  }
#endif // _DEBUG
