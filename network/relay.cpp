/** $Id: relay.cpp 4738 2014-07-03 00:55:39Z dchassin $
 ...
 \f$T_p=TimeDial\left(A_p+\frac{B_p}{M_p^{C_p}-1}\right)\f$.

        The reset time is determined by
 \f$T_r=TimeDial\left(\frac{B_r}{1-M_r^{C_r}}\right)\f$.
        ...
        The current curves are implemented according to \e SEL-551 \e
 Instruction \e Manual
        \htmlonly (<a
 href="http://www.google.com/search?hl=en&q=SEL-551+Manual">Google that</a>)
 \endhtmlonly
...
relay::relay(MODULE *mod) : link(mod) {
  ...
  if (oclass == nullptr) {
    ...
    relay_class = oclass =
        gl_register_class(mod, "relay", sizeof(relay), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "Curve", PADDR(Curve), PT_KEYWORD, "FC_U1",
            FC_U1, PT_KEYWORD, "FC_U2", FC_U2, PT_KEYWORD, "FC_U3", FC_U3,
            PT_KEYWORD, "FC_U4", FC_U4, PT_KEYWORD, "FC_U5", FC_U5, PT_double,
            "TimeDial", PADDR(TimeDial), PT_double, "SetCurrent",
            PADDR(SetCurrent), PT_enumeration, "State", PADDR(State),
            PT_KEYWORD, "FS_CLOSED", FS_CLOSED, PT_KEYWORD, "FS_TRIPPED",
            FS_TRIPPED, PT_KEYWORD, "FS_RECLOSED", FS_RECLOSED, PT_KEYWORD,
            "FS_LOCKOUT", FS_LOCKOUT, PT_KEYWORD, "FS_FAULT", FS_FAULT,
            nullptr) < 1)
