...
regulator::regulator(MODULE *mod) : link(mod) {
  ...
  if (oclass == nullptr) {
    ...
    regulator_class = oclass =
        gl_register_class(mod, "regulator", sizeof(regulator), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(oclass, PT_enumeration, "Type", PADDR(Type),
                            PT_KEYWORD, "RT_LTC", RT_LTC, PT_KEYWORD, "RT_VR",
                            RT_VR, PT_double, "Vmax", PADDR(Vmax), PT_double,
                            "Vmin", PADDR(Vmin), PT_double, "Vstep",
                            PADDR(Vstep), PT_object, "CTlink", PADDR(CTlink),
                            PT_object, "PTbus", PADDR(PTbus), PT_double,
                            "TimeDelay", PADDR(TimeDelay), nullptr) < 1)
