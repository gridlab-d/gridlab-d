...
fuse::fuse(MODULE *mod) : link(mod) {
  ...
  if (oclass == nullptr) {
    ...
    fuse_class = oclass =
        gl_register_class(mod, "fuse", sizeof(fuse), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_double, "TimeConstant", PADDR(TimeConstant), PT_double,
            "SetCurrent", PADDR(SetCurrent), PT_double, "SetBase",
            PADDR(SetBase), PT_double, "SetScale", PADDR(SetScale), PT_double,
            "SetCurve", PADDR(SetCurve), PT_double, "TresetAvg",
            PADDR(TresetAvg), PT_double, "TresetStd", PADDR(TresetStd),
            PT_enumeration, "State", PADDR(State), PT_KEYWORD, "FS_GOOD",
            FS_GOOD, PT_KEYWORD, "FS_BLOWN", FS_BLOWN, PT_KEYWORD, "FS_FAULT",
            FS_FAULT, nullptr) < 1)
...
TIMESTAMP fuse::sync(TIMESTAMP t0) {
  ...
  case FS_GOOD:
    Y = 100;     // fuse is good
    if (M > 1) { // compute fuse time
      double t = TimeConstant * (SetBase + SetScale / (pow(M, SetCurve) - 1));
      TIMESTAMP t1 = Tstate + (TIMESTAMP)(t * TS_SECOND);
      if (t1 <= t0) { // fuse blows now
        State = FS_BLOWN;
        Tstate = t0;
        Treset =
            t0 + (TIMESTAMP)(gl_random_normal(RNGSTATE, TresetAvg, TresetStd) *
                             TS_SECOND);
        return TS_NEVER;
    ...
    } else if (M <= 1) { // fuse doesn't blow; reset timer
      /// @todo it would be better to model how much damage the fuse incurred
      /// (network, low priority) (ticket #128)
      Tstate = t0;
      return TS_NEVER;
    }
    break;
