/** $Id: check.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - <b>Connectivity</b>: this verifies that all nodes have a least one
 link and that all links have both a \p to and \p from node.
        - <b>Swing bus</b>: this verifies that all islands have at least one
 swing bus.
...
EXPORT int check(void) {
  ...
  FINDLIST *swings = gl_find_objects(FL_NEW, FT_CLASS, SAME, "node", AND,
                                     FT_PROPERTY, "type", EQ, "3", FT_END);
  ...
  while ((obj = gl_find_next(nodes, obj)) != nullptr) {
    ...
    if (bus->type == SWING) {
      if (areas[n].swing != nullptr)
        gl_warning(
            "flow area %d has more than one swing bus (node:%d and node:%d)", n,
            areas[n].swing->id, obj->id);
      else
        areas[n].swing = obj;
    }
