...
capbank::capbank(MODULE *mod) : link(mod) {
  ...
  if (oclass == nullptr) {
    ...
    capbank_class = oclass =
        gl_register_class(mod, "capbank", sizeof(capbank), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_double, "KVARrated", PADDR(KVARrated), PT_double,
            "Vrated", PADDR(Vrated), PT_enumeration, "state", PADDR(state),
            PT_KEYWORD, "CAPS_IN", CAPS_IN, PT_KEYWORD, "CAPS_OUT", CAPS_OUT,
            PT_object, "CTlink", PADDR(CTlink), PT_object, "PTnode",
            PADDR(PTnode), PT_double, "VARopen", PADDR(VARopen), PT_double,
            "VARclose", PADDR(VARclose), PT_double, "Vopen", PADDR(Vopen),
            PT_double, "Vclose", PADDR(Vclose), nullptr) < 1)
