...
transformer::transformer(MODULE *mod) : link(mod) {
  ...
  if (oclass == nullptr) {
    ...
    transformer_class = oclass =
        gl_register_class(mod, "transformer", sizeof(transformer), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "Type", PADDR(Type), PT_KEYWORD, "TT_YY",
            TT_YY, PT_KEYWORD, "TT_YD", TT_YD, PT_KEYWORD, "TT_DY", TT_DY,
            PT_KEYWORD, "TT_DD", TT_DD, PT_double, "Sbase", PADDR(Sbase),
            PT_double, "Vbase", PADDR(Vbase), PT_double, "Zpu", PADDR(Zpu),
            PT_double, "Vprimary", PADDR(Vprimary), PT_double, "Vsecondary",
            PADDR(Vsecondary), nullptr) < 1)
