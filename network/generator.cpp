...
int generator_state_to_string(void *addr, char *value, int size) {
  ...
  case generator::STOPPED:
    if (size > 7)
      ...
      return 0;
    return 1;
  case generator::STANDBY:
    if (size > 7)
      strcpy(value, "STANDBY");
    else
      return 0;
    return 1;
  case generator::ONLINE:
    if (size > 6)
      ...
      return 0;
    return 1;
  case generator::TRIPPED:
    if (size > 7)
      strcpy(value, "TRIPPED");
    else
      return 0;
    return 1;
  default:
...
generator::generator(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    generator_class = oclass =
        gl_register_class(mod, "generator", sizeof(generator), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_double, "Pdesired_MW", PADDR(Pdesired_MW), PT_double,
            "Qdesired_MVAR", PADDR(Qdesired_MVAR), PT_int32, "Qcontrolled",
            PADDR(Qcontrolled), PT_double, "Pmax_MW", PADDR(Pmax_MW), PT_double,
            "Qmin_MVAR", PADDR(Qmin_MVAR), PT_double, "Qmax_MVAR",
            PADDR(Qmax_MVAR), PT_double, "QVa", PADDR(QVa), PT_double, "QVb",
            PADDR(QVb), PT_double, "QVc", PADDR(QVc),
            ...
            STOPPED, PT_KEYWORD, "STANDBY", STANDBY, PT_KEYWORD, "ONLINE",
            ONLINE, PT_KEYWORD, "TRIPPED", TRIPPED,
...
int generator::init(node *parent) {
  ...
  if (parent->type == PQ && !Qcontrolled) {
    ...
    gl_error("generator:%d is Qcontrolled but is not connected to a PQ bus",
             obj->id);
    return 0;
  } else if (parent->type != PQ && Qcontrolled) {
    OBJECT *obj = object_header(this);
    gl_error("generator:%d is not Qcontrolled but is connected to a PQ bus",
             obj->id);
    return 0;
  } else if (Qcontrolled && Qdesired_MVAR < Qmin_MVAR &&
             Qdesired_MVAR > Qmax_MVAR) {
    OBJECT *obj = object_header(this);
...
TIMESTAMP generator::sync(TIMESTAMP t0) {
  ...
  case SWING:
    // check for overlimit
    if (pBus->S.Re() > Pmax_MW || pBus->S.Im() < Qmin_MVAR ||
        pBus->S.Im() > Qmax_MVAR) {
      // this happens because we don't support redispatching yet (after
      // redispatch we could do tripping).
      gl_error("generator:%d exceeded limits when connected to swing bus; "
               "clock stopped",
               obj->id);
      return TS_ZERO;
    }
...
static TIMESTAMP sync_generator_impl(OBJECT *obj, TIMESTAMP t0,
                                     PASSCONFIG pass) {
  TIMESTAMP t1 = TS_NEVER;
  if (pass == PC_BOTTOMUP)
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_generator(OBJECT *obj, TIMESTAMP t0,
                                               PASSCONFIG pass) {
  return sync_generator_impl(obj, t0, pass);
}
#else
extern "C" MODULE_API TIMESTAMP sync_generator(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
