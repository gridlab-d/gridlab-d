...
meter::meter(MODULE *mod) : node(mod) {
  ...
  if (oclass == nullptr) {
    ...
    meter_class = oclass =
        gl_register_class(mod, "meter", sizeof(meter), PC_BOTTOMUP);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "type", PADDR(type), PT_KEYWORD,
            "MT_ONEPHASE", MT_ONEPHASE, PT_KEYWORD, "MT_THREEPHASE",
            MT_THREEPHASE, PT_complex, "demand[kW]", PADDR(demand), PT_complex,
            "meter[kWh]", PADDR(meterkWh), PT_complex, "line1_current[A]",
            PADDR(line1_current), PT_complex, "line2_current[A]",
            PADDR(line2_current), PT_complex, "line3_current[A]",
            PADDR(line3_current), PT_complex, "line1_admittance[1/Ohm]",
            PADDR(line1_admittance), PT_complex, "line2_admittance[1/Ohm]",
            PADDR(line2_admittance), PT_complex, "line3_admittance[1/Ohm]",
            PADDR(line3_admittance), PT_complex, "line1_power[S]",
            PADDR(line1_power), PT_complex, "line2_power[S]",
            PADDR(line2_power), PT_complex, "line3_power[S]",
            PADDR(line3_power), PT_complex, "line1_volts[V]", PADDR(V),
            PT_complex, "line2_volts[V]", PADDR(V), PT_complex,
            "line3_volts[V]", PADDR(V), nullptr) < 1)
...
#else
extern "C" MODULE_API TIMESTAMP sync_meter(OBJECT *obj, ...) {
  va_list args;
  va_start(args, obj);
