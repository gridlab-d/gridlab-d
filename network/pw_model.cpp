/**
        ...
        An interface with PowerWorld and a single model file.  Constructs and
 destructs the SimAuto COM connection. Tracks if any of the attached loads has
 requested a model calculation, and detects if the powerflow could not converge
 to a solution.
 **/
...
pw_model::pw_model(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gld_class::create(module, "pw_model", sizeof(pw_model),
    ...
    if (gl_publish_variable(
            ...
            PT_char1024, "model_name", PADDR(model_name), PT_DESCRIPTION,
            "the file path for the PowerWorld model to run",
            //			PT_char1024, "load_list_file",
            //PADDR(load_list_file), PT_DESCRIPTION, "the optional file to write
            //a list of loads into.  if present, GLD will exit after writing
            //loads.", 			PT_char1024, "out_file", PADDR(out_file), PT_DESCRIPTION,
            //"the file to write the list of objects into", 			PT_char32,
            //"out_file_type", PADDR(out_file_type),PT_DESCRIPTION,"the type of
            //object to list in out_file", 			PT_char1024, "field_file",
            //PADDR(field_file),PT_DESCRIPTION,"the file to write the list of
            //fields of a type of object into", 			PT_char32, "field_type",
            //PADDR(field_type),PT_DESCRIPTION,"the type of object to list the
            //fields for in field_file",
            PT_bool, "update_flag", get_update_flag_offset(), PT_ACCESS,
            PA_REFERENCE, PT_DESCRIPTION,
            "flag set by pw_load objects signaling the loads need updating",
            PT_int32, "exchange_count", get_exchange_count_offset(), PT_ACCESS,
            PA_REFERENCE, PT_DESCRIPTION,
            "number of times PowerWorld and GridLAB-D have exchanged data",
            PT_bool, "valid_flag", get_valid_flag_offset(), PT_ACCESS,
            PA_REFERENCE, PT_DESCRIPTION,
            "flag set by pw_model signaling if the PowerWorld model solved "
            "successfully",
            nullptr) < 1) {
      char msg[256];
...
int pw_model::init(OBJECT *parent) {
  ...
  if (0 != model_name[0]) {
    ...
    case CO_E_CLASSSTRING:
      ...
      /*  TROUBLESHOOT
      GridLAB-D was unable to find the clsid value for PowerWorld SimAuto inside
      the registry.  Please ensure PowerWorld and SimAuto are installed and
      registered correctly and try again.
      */
      return 0;
      break;
    case REGDB_E_WRITEREGDB:
      gl_error(
          "An error occured writing the CLSID to the registry."); // I would
                                                                  // imagine
                                                                  // this should
                                                                  // never
                                                                  // happen -
                                                                  ...
                                                                  // registry?
      /*  TROUBLESHOOT
      An error occurred while writing the CLSID value to the registry.  Please
    ...
    if (0 == A) {
      ...
      /*  TROUBLESHOOT
      GridLAB-D was unable to allocate one of the necessary variables for the
      SimAuto interface. Please try again.  If the error persists, please submit
    ...
    output =
        A->OpenCase(model_name.get_string()); // must catch RV in a _variant_t!
    if (0 == check_COM_output(output)) {
      gl_error("OpenCase() failed");
      /*  TROUBLESHOOT
      PowerWorld failed to open the case specified.  Please ensure the case
      exists, is a valid PowerWorld file, and that it is located in the proper
    ...
    if (0 == check_COM_output(output)) {
      ...
      /*	TROUBLESHOOT
      PowerWorld failed to enter the RUN mode.  Please ensure there are no model
      errors and that PowerWorld is functioning correctly and try again.
      */
      return 0;
    }
    ...
    if (0 == check_COM_output(output)) {
      ...
      /* TROUBLESHOOT
              SimAuto reported a problem when asked to simulate the model.
         Review the error message on the console.
       */
      return 0;
    }
  ...
  if (out_file[0] != 0) {
    ...
    // assume 'load'
    if ((0 == strcmp("load", out_file_type)) ||
        (0 == strcmp("Load", out_file_type))) {
...
//	}
  ...
  if (trouble) {
    ...
    /* TROUBLESHOOT
            The pw_model requires either a valid PWB model file path, or for an
       alternate mode to be triggered.  Please check the model_name definition,
       and the path to the desired model file.
     */
    return 0;
  }
...
/**
        If the update flag is set, command PowerWorld to find the powerflow
 solution, then
        @return TS_NEVER given normal operations, t1 (reiterate) if the
 PowerWorld model is in an invalid state, TS_INVALID given SimAuto errors.
 **/
TIMESTAMP pw_model::presync(TIMESTAMP t1) {
  ...
  if (update_flag) {
    ...
    if (0 == check_COM_output(output)) {
      gl_error("RunScriptCommand(SolvePowerFlow;) failed");
      /*  TROUBLESHOOT
      PowerWorld failed to solve the powerflow correctly.  Please ensure the
      model is correct and the system condition is valid and try again.
      */
      return TS_INVALID;
    }
    gl_verbose("RunScriptCommand(SolvePowerFlow) succeeded");
...
TIMESTAMP pw_model::postsync(TIMESTAMP t1) { return TS_NEVER; }
...
int pw_model::finalize() {
  ...
  if (0 == check_COM_output(output)) {
    ...
    /*  TROUBLESHOOT
    PowerWorld encountered an error while trying to close the open case.  Please
    check to ensure your model is correct.
    */
    return 0;
  }
