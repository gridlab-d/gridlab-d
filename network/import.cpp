/** $Id: import.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        \htmlonly (<a href="http://www.google.com/search?hl=en&q=IEEE power flow
 file format CDF">find it</a>) \endhtmlonly
 @{
 **/
...
int import_cdf(char *file) {
  ...
  int errors = 0;
  OBJECT *bus[10000];
  memset(bus, 0, sizeof(bus)); // CDF does not allow more than 9999 as a bus no
  OBJECT *branch[10000];
  memset(branch, 0,
         sizeof(branch)); // CDF does not allow more than 9999 as a bus no
  ...
  while (fgets(line, sizeof(line), fp) != nullptr) {
    ...
    case LD_BUS:
      if (atoi(line) > 0) {
        ...
        if (sscanf(line,
                   "%d %12[^\n] %d %d%d %lg %lg %lg %lg %lg %lg %lg %lg %lg "
                   "%lg %lg %lg %" FMT_INT64 "d",
                   &n, name, &area, &zone, &type, &Vm, &Va, &Lr, &Li, &Gr, &Gi,
                   &bV, &dV, &Qh, &Ql, &G, &B, &remote) == 18 &&
            n_bus < 10000) {
          ...
          p->S.SetRect((Lr - Gr) / mvabase,
                       (Li - Gi) /
                           mvabase); /* loads are positive, gen is negative */
          p->base_kV = bV;
          p->desired_kV = dV;
          p->Qmax_MVAR = Qh;
          p->Qmin_MVAR = Ql;
          p->G = G;
          p->B = B;
          /** @todo using remote_bus_id is just plain wrong, try using a lookup
           * based on getting object node:bus_id **/
    ...
    case LD_BRANCH:
      if (atoi(line) > 0) {
        ...
        double R, X, B, n = 0;
        if (sscanf(line,
                   "%d %d %d %d %d %d %lg %lg %lg %*lg %*lg %*lg %*d %*d %lg",
                   &from, &to, &area, &zone, &type, &circuit, &R, &X, &B,
                   &n) >= 9 &&
            n_branch < 10000) {
