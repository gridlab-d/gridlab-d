...
int pw_recorder::get_pw_values() { return 1; }
...
pw_recorder::pw_recorder(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gld_class::create(module, "pw_recorder", sizeof(pw_recorder),
    ...
    if (gl_publish_variable(
            oclass, PT_object, "model", get_model_offset(), PT_DESCRIPTION,
            "pw_model object for the PowerWorld model this recorder is "
            "monitoring",
            PT_char1024, "outfile", get_outfile_name_offset(), PT_DESCRIPTION,
            "Output file name", PT_char256, "obj_class",
            get_obj_classname_offset(), PT_DESCRIPTION,
            "PowerWorld object class of object to record", PT_char1024,
            "key_strings", get_key_strings_offset(), PT_DESCRIPTION,
            "Key string values for PowerWorld object to record", PT_char1024,
            "key_values", get_key_values_offset(), PT_DESCRIPTION,
            "Key values for PowerWorld object to record", PT_char1024,
            "properties", get_properties_offset(), PT_DESCRIPTION,
            "Properties desired to record from PowerWorld object", PT_int64,
            "interval", get_interval_offset(), PT_DESCRIPTION,
            "Interval of output for pw_recorder object", PT_int64, "limit",
...
int pw_recorder::create() { return 1; }
...
int pw_recorder::init(OBJECT *parent) {
  ...
  if (0 == model) {
    if (0 == parent) {
      gl_error(
          "pw_recorder::init(): object \'%s\' does not specify a model object",
          gl_name(model, objname, 63));
      /*  TROUBLESHOOT
      A model object was not explicitly specified or connected as a parent to
      the pw_recorder object.  Please make this connection and try again.
      */
      return 0;
    } else {
      // isa parent pw_model?
      if (!gl_object_isa(parent, "pw_model")) {
        gl_error("pw_recorder::init(): parent of '%s' is not a pw_model",
                 gl_name(model, objname, 63));
        /*  TROUBLESHOOT
        The pw_recorder object does not have the model field specified and is
        not parented to a pw_model object.  Please specify a pw_model object in
        the model field or parent the pw_recorder to one and try again.
        */
        return 0;
      }
      model = parent;
    ...
    if (!gl_object_isa(model, "pw_model")) {
      gl_error("pw_recorder::init(): model of '%s' is not a pw_model",
               gl_name(model, objname, 63));
      /*  TROUBLESHOOT
      The object specified in the model field of the pw_recorder is not a
      pw_model object.
      */
      return 0;
    }
  ...
  if ((model->flags & OF_INIT) != OF_INIT) {
    gl_verbose("pw_recorder::init(): deferring initialization on '%s'",
               gl_name(model, objname, 63));
    return 2; // defer
  }
  ...
  if (0 == key_strings[0]) {
    gl_error("pw_recorder::init(): key_strings not defined for '%s'",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    The key strings required for the pw_recorder are not specified.  Please
    specify these values and try again.
    */
    return 0;
  }
  ...
  if (0 == key_values[0]) {
    gl_error("pw_recorder::init(): key_values not defined for '%s'",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    The key value field required for the pw_recorder is not specified.  Please
    specify these values and try again.
    */
    return 0;
  }
  ...
  if (0 == properties[0]) {
    gl_error("pw_recorder::init(): properties not defined for '%s'",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    The recording properties list is not defined for the pw_recorder object.
    Please specify these properties and try again.
    */
    return 0;
  }
  ...
  if (0 == build_keys()) {
    gl_error("pw_recorder::init(): error when building keys for '%s'",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    An error occurred while building the keys for the pw_recorder.  Ensure your
    property values, key strings, and key values are correct and try again.
    */
    return 0;
  }
  ...
  if (0 == GPSE()) {
    gl_error("pw_recorder::init(): error when calling "
             "GetParameterSingleElement wrapper function for %s",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    An error occurred while trying to extract a value from PowerWorld.  Please
    check that your key string, key values, and property values are correct and
    try again.
    */
    return 0;
  }
  ...
  if (interval < -1) {
    gl_error("pw_recorder::init(): invalid interval in '%s'",
             gl_name(model, objname, 63));
    /*  TROUBLESHOOT
    An invalid interval was specified in the pw_recorder.  Please specify a
    value in seconds, 0 (all iterations), or -1 (on change).
    */
    return 0;
  }
  ...
  if (limit < 1) {
    gl_verbose("pw_recorder::init(): '%s' will perform unlimited writes",
               gl_name(my(), objname, 63));
    limit = -1;
  }
  ...
  if (0 == outfile) {
    gl_error("pw_recorder::init(): unable to open outfile '%s' for writing",
             outfile_name);
    /*  TROUBLESHOOT
    pw_recorder was unable to open the output file.  Please ensure it isn't open
    in another program and try again.
    */
    return 0;
  } else {
    if (!write_header()) {
      gl_error("pw_recorder::init(): unable to write header for '%s'",
               gl_name(my(), objname, 63));
      /*  TROUBLESHOOT
      pw_recorder was unable to write the header in the output file.  Please
      ensure the file isn't open in another program and try again.
      */
      return 0;
    }
    is_ready = true;
...
int pw_recorder::precommit(TIMESTAMP t1) { return 1; }
...
TIMESTAMP pw_recorder::presync(TIMESTAMP t1) {
  if (!cModel->get_valid_flag()) {
    ...
    if (0 == GPSE()) {
      ...
      /*	TROUBLESHOOT
      pw_recorder encountered an error when attempting to retrieve a parameter
      from PowerWorld. Please try again.  If the error persists, please submit
      your code an an error report via the trac website.
       */
      return TS_INVALID;
    }
  }
...
TIMESTAMP pw_recorder::sync(TIMESTAMP t1) {
  ...
  if ((interval > 0) && is_ready && outfile) {
    if (t1 >= (last_write + interval)) {
      interval_write = true;
      rt =
          last_write + interval + interval; // last_write is updated in commit()
    } else {
...
TIMESTAMP pw_recorder::commit(TIMESTAMP t1, TIMESTAMP t2) {
  if (outfile && is_ready) {
    ...
    //		return last_write+interval;	// now handled by sync(), for
    //the most part
  }
  return TS_NEVER;
}

int pw_recorder::isa(char *classname) { return 1; }

/**
        build_keys
        Validates the contents of key_strings, key_values, and properties, then
        converts them into a VARIANT array for later use in
 GetParameterSingleElement().
...
int pw_recorder::build_keys() {
  ...
  // compare counts
  if (0 == key_str_ct) {
    gl_error("pw_recorder::build_keys(): '%s' did not parse any key strings",
             gl_name(my(), objname, 255));
    /*  TROUBLESHOOT
    pw_recorder was unable to parse the key strings specified.  Please ensure
    they are correct and try again.
    */
    return 0;
  }
  if (0 == key_val_ct) {
    gl_error("pw_recorder::build_keys(): '%s' did not parse any key values",
             gl_name(my(), objname, 255));
    /*  TROUBLESHOOT
    pw_recorder was unable to parse the key values specified.  Please ensure
    they are correct and try again.
    */
    return 0;
  }
  if (key_str_ct != key_val_ct) {
    gl_error("pw_recorder::build_keys(): '%s' has %i key properties and %i key "
             "values listed",
             gl_name(my(), objname, 255), key_str_ct, key_val_ct);
    /*  TROUBLESHOOT
    pw_recorder was unable to parse the key properties specified.  Please ensure
    they are correct and try again.
    */
    return 0;
  }
  ...
  try {
    ...
    /*  TROUBLESHOOT
    An unknown exception was encountered by the pw_recorder object.  Please try
...
/**
        GPSE
        Wrapper for SimAuto's GetParameterSingleElement.  Fetches a stack of
 properties and writes the results to the specified output file.
...
int pw_recorder::write_header() {
  ...
  fprintf(outfile, "# date...... %s",
          asctime(localtime(&now))); // adds its own newline
#ifdef _WIN32
...
int pw_recorder::write_line(TIMESTAMP t1) {
  ...
  fprintf(outfile, "%s,%s\n", (const char *)time_output,
          (const char *)line_output);
  fflush(outfile);
