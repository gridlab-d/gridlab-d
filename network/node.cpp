/** $Id: node.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - \p PQ buses are nodes that have both constant real and reactive power
 injections
        - \p PV buses are for nodes that have constant real power injection but
 can control reactive power injection
        - \p SWING buses are nodes that are designated to absorb the residual
 error and for generators that can control both real and reactive power
 injections
        ...
        The \e PQ bus is the most commonly found bus type in electric network
 models.  PQ buses are nodes where both the real power (\e P) and reactive power
 (\e Q) are given.  In these cases, the updated voltage at for a node is found
 from an existing (non-zero) voltage using

        \f[ \widetilde V \leftarrow \frac{P - \jmath Q}{\widetilde{YY} \left(
 \widetilde V^* - \Sigma \widetilde{YV} \right)} \f]
        ...
        The \e PV bus is the next most common bus type in electric network
 models.  PV buses are nodes where the real power (\e P) is given, but the
 reactive power (\e Q) must be determined at each iteration. In these cases, the
 updated voltage for a node is found by calculating

        \f[ Q \leftarrow - \Im \left[ \widetilde V^* \left( \widetilde{YY}
 \widetilde V + \Sigma \widetilde{YV} \right) \right] \f]

        If \e Q exceeds the \e Q limits [\p Qmin, \p Qmax] then the angle is
 fixed to the correspond limit and the bus is solved as a \e PQ bus.

        @section SWING SWING bus

        The \e SWING bus occurs at least one in any given island of a network
 models.  Large models may have more the one \e SWING bus, particularly if areas
 of the network are only lightly coupled by relatively high impedance links.  \e
 SWING bus nodes are nodes where both the real power (\e P) and reactive power
 (\e Q) must be determined at each iteration.  In these cases, the updated
 voltage for a node is found by

        \f[ \widetilde S \leftarrow \left[ \widetilde V^* \left( \widetilde{YY}
 \widetilde V + \Sigma \widetilde{YV} \right) \right] \f]
...
// #define HYBRID // enables hybrid solver that can be used to perform state
// estimation (DPC 10/07: this is not fully functional)
...
node::node(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    node_class = oclass = gl_register_class(mod, "node", sizeof(node),
                                            PC_PRETOPDOWN | PC_POSTTOPDOWN |
                                                PC_UNSAFE_OVERRIDE_OMIT);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_complex, "V", PADDR(V), PT_complex, "S", PADDR(S),
            PT_double, "G", PADDR(G), PT_double, "B", PADDR(B), PT_double,
            "Qmax_MVAR", PADDR(Qmax_MVAR), PT_double, "Qmin_MVAR",
            PADDR(Qmin_MVAR), PT_enumeration, "type", PADDR(type), PT_KEYWORD,
            "PQ", PQ, PT_KEYWORD, "PQV", PQV, PT_KEYWORD, "PV", PV, PT_KEYWORD,
            "SWING", SWING, PT_int16, "flow_area_num", PADDR(flow_area_num),
            PT_double, "base_kV", PADDR(base_kV),
#ifdef HYBRID
            PT_complex, "Vobs", PADDR(Vobs), PT_double, "Vstdev", PADDR(Vstdev),
#endif
            nullptr) < 1)
...
int node::init(OBJECT *parent) {
  ...
  if (swing_hdr->oclass != hdr->oclass || swing->type != SWING) {
    gl_error("node %s:%d parent is not a swing bus", hdr->oclass->name,
             hdr->id);
    return 0;
  }
#ifdef HYBRID
...
void node::add_inj_residual(node *pNode) {
  if (pNode != this) {
    pNode->Sr2 =
        (~(~pNode->V * ((pNode->Ys + complex(pNode->G, pNode->B)) * pNode->V +
                        pNode->YVs)) -
         pNode->S)
            .Mag();
...
double node::get_obs_probability(void) const {
  ...
  double pr = exp(
      -0.5 *
      r2); /// @todo there should be a 1/sqrt(2*pi) coeff on the observability
           /// probability, yet it works. (network, low priority)
  if (pr > 1)
    gl_warning("node:%d observation probability exceeds 1!",
...
TIMESTAMP node::postsync(TIMESTAMP t0) {
  ...
  if (debug_node > 0) {
    ...
    if (obj->id == first) {
      printf("\n");
      printf(
          "Node           Type  V                 Vobs              Stdev    "
          "Power             G        B        dV       Pr{Vobs} r2     Sr2\n");
      printf("============== ===== ================= ================= "
             "======== ================= ======== ======== ======== ======== "
             "====== ======\n");
    }
    if (((debug_node & 1) == 1 && dV.Mag() > convergence_limit) // only on dV
        ...
        || ((debug_node & 4) == 4 &&
            get_inj_residual() > 0.001) // non-zero power residual
#endif
    ) {
      printf("%2d (%-9.9s) %5s %+8.4f%+8.3fd %+8.4f%+8.3fd %8.5f %+8.4f%+8.4fj "
             "%+8.5f ",
             obj->id, obj->name,
             type == SWING ? "SWING" : (type == PQ ? "PQ   " : "PV"), V.Mag(),
             V.Arg() * 180 / 3.1416, Vobs.Mag(), Vobs.Arg() * 180 / 3.1416,
             Vstdev, S.Re(), S.Im(), G, B, dV.Mag());
#ifdef HYBRID
      printf("%+8.5f %8.5f ", get_obs_probability(), r2);
      if (Vstdev > 0)
        printf("%8.5f %6.3f %6.3f\n", get_obs_probability(), r2,
               get_inj_residual());
      else
        printf("   --      --   %6.3f\n", get_inj_residual());
#else
...
static TIMESTAMP sync_node_impl(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass) {
  ...
  try {
    ...
    gl_error("%s(%s:%d): %s", obj->name ? obj->name : "(anon)",
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP sync_node(OBJECT *obj, TIMESTAMP t0,
                                          PASSCONFIG pass) {
