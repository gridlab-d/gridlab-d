/** $Id: machine.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The machine object implements the PLC code.  On Windows machines, it
 compiles the code using \p mingw, which must be installed on your computer and
 is provided with the GridLAB-D distribution for that purpose.  On Linux
 machines it uses	\p gcc to compile the code.

        The following is a sample of PLC code that implements a simple
...
void sendx(machine *src, char *to, void *str, unsigned int len) {
  ...
  if (obj) {
    ...
    gl_error("sendx(machine=%x, to='%s', str='%-.8s%s', len=%d): message "
             "recipient '%s' not found",
             src, to, (char *)str, len > 8 ? "..." : "", len, to);
}
...
int machine::compile(char *source) {
  ...
  fprintf(fp,
          "/* this code automatically generated by gridlab-d " __FILE__ "*/\n");
  ...
  /* compile source */
  gl_verbose("compiling %s from %s using incpath '%s'...", ofile, cfile,
             incpath);
  unlink(ofile);
  if (exec_cmd("%s -I\"%s\" %s -c \"%s\" -o \"%s\"",
               getenv("CC") ? getenv("CC") : "gcc", incpath,
               getenv("CFLAGS") ? getenv("CFLAGS") : "", cfile, ofile) != 0)
    return -1;
  ...
  if (exec_cmd("%s %s -shared -o \"%s\" -Wl,\"%s\"",
               ...
               getenv("LDFLAGS") ? getenv("LDFLAGS") : "", lfile, ofile) != 0)
    return -1;
...
int machine::init(OBJECT *parent) {
  ...
  for (item = _data; item->name != nullptr; item++) {
    ...
    if (p == nullptr) {
      gl_error("PCL data item %s is not found in object %s:%d", item->name,
...
int machine::run(double dt) {
  ...
  static PLCDEV dev = {(unsigned int)((*pGlobalClock) / TS_SECOND),
                       {&sendx, &recvx}};
  return (*_code)(this, dt, &dev);
}

void machine::connect(comm *ptr) { net = ptr; }

void machine::deliver(Message *msg) { _link.add(msg); }

Message *machine::receive(void) { return _link.take(); }

void machine::send(Message *msg) { net->route(msg); }
