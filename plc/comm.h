/** $Id: comm.h 4738 2014-07-03 00:55:39Z dchassin $
        ...
        The PLC communications object implements message handling between PLC
 objects. Each communications networks is defined by a single object that
 handles all messages between PLC objects linked to it.  The network has the
 following properties
        - \p latency defines the message delivery latency (note this is a
 functional distribution).
        - \p reliability to define the probability with which a message is
 delivered at all, and
        - \p bitrate to define the data rate at which messages are delivered
 (longer messages take longer to be delivered if bitrate is non-zero),
