/** $Id: plc.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        - \p _data	This is the data structure for variables that must be
 linked
        - \p _init	This is the code that must be called after the DLL is
 loaded but before \p _code is called for the first time.  This must return the
 number of seconds before the first call to \p _code
        - \p _code    This is the code that must be called each time to PLC
 syncs. This must return the number of seconds before the next sync.
        ...
        @endcode
        with the last entry having a \p nullptr name (this is \e very
 important).

        The compiler will automatically create a conforming DLL from a \p .plc
 file having the following typical structure
...
plc::plc(MODULE *mod) {
  ...
  if (oclass == nullptr) {
    ...
    oclass =
        gl_register_class(mod, "plc", sizeof(plc), PC_BOTTOMUP | PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(oclass, PT_char1024, "source", PADDR(source),
                            PT_object, "network", PADDR(network), nullptr) < 1)
...
int plc::init(OBJECT *parent) {
  ...
  if (network) {
    ...
    gl_verbose("machine %x has connected to network '%s'", controller,
               network->name ? network->name : "anonymous");
  }
