/** $Id: comm.cpp 4738 2014-07-03 00:55:39Z dchassin $
        ...
        Messages are sent from one PLC object to another using PLC code.  The
 following functions are available to send and receive messages.

        - \code void SNDMSG(char *destination, char *string); \endcode is used
 to send a string message to a PLC object.

        - \code void SNDDAT(char *destination, void *buffer, unsigned int size);
 \endcode is used to send a binary message to a PLC object.

        - \code int RCVMSG(char source[], char buffer[]); \endcode is used
        receive a string messages from a PLC object.  The buffer must be an
 array for which sizeof() returns the correct maximum length.

        - \code int RCVDAT(char source[], void *buffer, unsigned int size);
 \endcode is used to receive a binary message from a PLC object.
...
void Queue::add(Message *msg) {
  ...
  gl_verbose("message %" FMT_INT64 "d '%-8.8s%s' added to queue %x",
             msg->get_id(), msg->get_data(), msg->get_size() > 8 ? "..." : "",
             this);
}
Message *Queue::peek(void) { return first; }
...
Message *Queue::next(Message *msg) { return msg ? msg->get_next() : first; }
...
// comm implementation
CLASS *comm::oclass =
    nullptr; ///< a pointer to the registered object class definition
comm *comm::defaults =
    nullptr; ///< a pointer to the default values for new objects

void comm::route(Message *msg) {
  msg->set_deliverytime((TIMESTAMP)(gl_randomvalue(rtype, a, b) / TS_SECOND) +
                        object_header(this)->clock);
  DATETIME dt;
  char ts[64] = "unknown time";
  gl_localtime(msg->get_deliverytime(), &dt);
  gl_strtime(&dt, ts, sizeof(ts));
  gl_verbose("message %" FMT_INT64 "d from %x to %x will be delivered at %s",
             msg->get_id(), msg->get_src(), msg->get_dst(), ts);
  queue.add(msg);
}

void comm::route(comm *net, Message *msg) { net->queue.add(msg); }

/** Register the new object class and construct the default object properties */
comm::comm(MODULE *mod) : queue() {
  // first time init
  if (oclass == nullptr) {
    // register the class definition
    oclass = gl_register_class(mod, "comm", sizeof(comm),
    ...
    if (gl_publish_variable(oclass, PT_char256, "latency", PADDR(latency),
...
TIMESTAMP comm::sync(TIMESTAMP t0) {
  ...
  while ((msg = queue.next(msg)) != nullptr) {
    if (msg->get_deliverytime() <= t0) {
      ...
      gl_localtime(t0, &dt);
      gl_strtime(&dt, ts, sizeof(ts));
      gl_warning("message %" FMT_INT64 "d delivery timeout at %s",
                 msg->get_id(), ts);
      msg = queue.drop(msg, true);
    }
  }
  return queue.next_delivery_time(); // return TS_ZERO on failure, or TIMESTAMP
                                     // for pending event
}
