...
cacheitem *cache::find_item(VARMAP *var) {
  ...
  for (n = 0; n < tail; n++) {
    ...
    if (strcmp((const char *)var->local_name,
               (const char *)m->get_var()->local_name) == 0 &&
        strcmp((const char *)var->remote_name,
               (const char *)m->get_var()->remote_name) == 0) {
      item = m;
      break;
    }
  }
...
Retry:
  ...
  if (index[id] != nullptr) {
    ...
    if (item->get_object() == o && item->get_property() == p &&
        strcmp(item->get_remote(), r) == 0)
  ...
  index[id] = this;
  var = v;
  value = new char[1025]; // TODO look into using prop->width instead to save
                          // some memory
  memset(value, 0, 1025);
  xltr = nullptr;
  next = nullptr;
...
void cacheitem::grow(void) {
  ...
  gl_verbose("cacheitem::grow(): double cache index size to %d entries",
             newsize);
  cacheitem **newindex = new cacheitem *[newsize];
  memset(newindex, 0, sizeof(newindex[0]) * newsize);

  // TODO this takes a while--put a mechanism in place to keep/use a list of
  // non-null entries copy old ids to new ids
  cacheitem *n;
...
bool cacheitem::write(char *buffer) {
  ...
  if (len < 1025) // TODO look into using prop->width instead to save memory
  {
    ...
    if (*value != '\0' && strcmp(buffer, value) != 0)
      gl_warning("cacheitem::write(): cache already contains different data "
                 "for local '%s'/remote '%s'",
                 var->local_name, var->remote_name);
