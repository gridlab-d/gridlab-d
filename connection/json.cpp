...
int json_translate(char *local, size_t local_len, char *remote,
                   size_t remote_len, TRANSLATIONFLAG flag, ...) {
  ...
  if (flag == TF_DATA) // default is data
  {
    ...
    if (var->dir == DXD_READ) // from remote to local
    {
      ...
      size_t len;
      if (sscanf(remote,
                 "<property><name>%1024[^<]</name><value>%1024[^<]</value></"
                 "property>",
                 name, value) != 2) {
        gl_error("json_translate(char *remote='%s',VARMAP "
                 "*var->local_name='%s'): remote data format is not correct",
                 remote, var->local_name);
        return 0;
      } else if ((len = strlen(value)) > local_len) {
        gl_error("json_translate(char *remote='%s',VARMAP "
                 "*var->local_name='%s'): remote data too big for local value",
                 remote, var->local_name);
        return 0;
      } else if (strcmp(name, var->local_name) != 0) {
        gl_error(
            "json_translate(char *remote='%s',VARMAP *var->local_name='%s'): "
            "local name does not match remote data",
            remote, var->local_name);
        return 0;
      } else {
        strcpy(local, value);
        // TODO recursively process remaining payload in remote if varmap
...
json::json(MODULE *module) : native(module) {
  ...
  oclass = gld_class::create(module, "json", sizeof(json),
                             PC_AUTOLOCK | PC_PRETOPDOWN | PC_BOTTOMUP |
                                 PC_POSTTOPDOWN | PC_OBSERVER);
  if (oclass == nullptr)
    throw "connection/json::json(MODULE*): unable to register class "
          "connection:json";
  else
    oclass->trl = TRL_UNKNOWN;

  defaults = this;
  if (gl_publish_variable(oclass, PT_INHERIT, "native", PT_double, "version",
                          get_version_offset(), PT_DESCRIPTION, "json version",
                          // TODO add published properties here
                          nullptr) < 1)
    throw "connection/json::json(MODULE*): unable to publish properties of "
          "connection:json";

  if (!gl_publish_loadmethod(
          oclass, "link",
          reinterpret_cast<int (*)(void *, char *)>(loadmethod_json_link)))
    throw "connection/json::json(MODULE*): unable to publish link method of "
          "connection:json";
  if (!gl_publish_loadmethod(
          oclass, "option",
          reinterpret_cast<int (*)(void *, char *)>(loadmethod_json_option)))
    throw "connection/json::json(MODULE*): unable to publish option method of "
          "connection:json";
}

// export data to json
int json_export(connection_transport *transport,
                const char *tag, // nullptr for group close, name for group open
                                 // (or nullptr if none)
...
{
  // test sequence
  // JSONLIST *data = json::parse("{\"method\" : \"init\", \"params\" :
  // {\"application\" : \"gridlabd\", \"version\" : 3.0, \"modelname\" :
  // \"modelname.glm\"}, \"id\" : 1}"); char *method = json::get(data,"method");
  ...
  // gl_debug("method=%s; id=%s; version=%s; application=%s; modelname=%s",
  // method,id,version,application,modelname); JSONLIST *params =
  // json::find(data,"params"); version = json::get(params,"version");
  // application = json::get(params,"application");
  // modelname = json::get(params,"modelname");
  // gl_debug("method=%s; id=%s; version=%s; application=%s; modelname=%s",
  // method,id,version,application,modelname);
  ...
  if (vlen == 0) // compare only
  {
    if (strcmp(v, value) == 0)
      return 1;
    gl_error("json_import(tag='%s',...) incoming value '%s' did not match "
             "expected/required value '%s'",
             tag, value, v);
    return 0;
    ...
    gl_error("json_import(tag='%s',...) incoming longer than %d bytes allowed",
             tag, vlen);
    return 0;
  }
}
...
int json::init(OBJECT *parent) {
  ...
  if (get_connection()->client_initiated(
          MSG_CRITICAL, MSG_INITIATE, MSG_TAG, "method", "init", MSG_OPEN,
          "params", MSG_STRING, "application", sizeof(appname) - 1, appname,
          MSG_REAL, "version", &appversion, MSG_STRING, "modelname",
          sizeof(name), name, MSG_CLOSE, MSG_COMPLETE, &id, nullptr) < 0) {
    error("handshake initiation failed");
    return 0;
  }
  if (get_connection()->server_response(MSG_CRITICAL, MSG_INITIATE, MSG_TAG,
                                        "result", "init", MSG_COMPLETE, &id,
                                        nullptr) < 0) {
  ...
  if (get_connection()->client_initiated(MSG_CRITICAL, MSG_INITIATE, MSG_TAG,
                                         "method", "input", MSG_OPEN, "schema",
                                         MSG_SCHEMA, DXD_READ, MSG_CLOSE,
                                         MSG_COMPLETE, &id, nullptr) < 0) {
    error("schema exchange failed");
    return 0;
  }
  if (get_connection()->server_response(MSG_CRITICAL, MSG_INITIATE, MSG_TAG,
                                        "result", "input", MSG_COMPLETE, &id,
                                        nullptr) < 0) {
    error("handshake response failed");
    return 0;
  }
  ...
  if (get_connection()->client_initiated(MSG_CRITICAL, MSG_INITIATE, MSG_TAG,
                                         "method", "output", MSG_OPEN, "schema",
                                         MSG_SCHEMA, DXD_WRITE, MSG_CLOSE,
                                         MSG_COMPLETE, &id, nullptr) < 0) {
    error("schema exchange failed");
    return 0;
  }
  if (get_connection()->server_response(MSG_CRITICAL, MSG_INITIATE, MSG_TAG,
                                        "result", "output", MSG_COMPLETE, &id,
                                        nullptr) < 0) {
    error("handshake response failed");
    return 0;
  }
...
int json::link(char *value) { return native::link(value); }
...
int json::option(char *value) {
  ...
  if (sscanf(value, "%[^:]:%[^\n]", target, command) == 2) {
    ...
    gl_error("connection/json::option(char *value='%s'): unable to parse "
             "option argument",
             value);
    return 0;
  }
...
int json::precommit(TIMESTAMP t) { return native::precommit(t); }

TIMESTAMP json::presync(TIMESTAMP t) { return native::presync(t); }

TIMESTAMP json::sync(TIMESTAMP t) { return native::sync(t); }

TIMESTAMP json::postsync(TIMESTAMP t) { return native::postsync(t); }

TIMESTAMP json::commit(TIMESTAMP t0, TIMESTAMP t1) {
  return native::commit(t0, t1);
}

int json::prenotify(PROPERTY *p, char *v) { return native::prenotify(p, v); }

int json::postnotify(PROPERTY *p, char *v) { return native::postnotify(p, v); }

int json::finalize(void) { return native::finalize(); }

TIMESTAMP json::plc(TIMESTAMP t) { return native::plc(t); }

void json::term(TIMESTAMP t) { return native::term(t); }
...
static void json_read_number(JSONLIST *item) {
  if (item->type == JT_REAL)
    ...
    gl_warning("json_read_number(JSONLIST *item={string:'%s',...}): ignoring "
               "attempt to read non-number type",
               item->string);
}
...
static void json_dump(JSONLIST *item, unsigned int indent = 0) {
  ...
  for (; item != nullptr; item = item->next) {
    if (item->type == JT_LIST) {
      ...
      gl_debug(" %s\"%s\" : \"%s\"%s", indentchars, item->tag, item->string,
               item->next == nullptr ? "" : ",");
    else if (item->type == JT_VOID)
      gl_debug(" %s\"%s\" : (void)%s", indentchars, item->tag,
               item->next == nullptr ? "" : ",");
    else
      gl_debug(" %s\"%s\" : %s%s", indentchars, item->tag, item->string,
               item->next == nullptr ? "" : ",");
...
JSONLIST *json::parse(char *buffer) {
  ...
  for (p = buffer; *p != '\0'; p++) {
    ...
    default:
      gl_error(
          "json::parse(char *buffer='%s'): parser state error at position %d",
          buffer, (int)(p - buffer));
      goto Error;
    }
  }
  ...
  return head;
Syntax:
  gl_error("json::parse(char *buffer='%s'): syntax error at position %d: "
           "...%-16.16s...",
           buffer, (int)(p - buffer), p);
Error:
  destroy(head);
  return nullptr;
}
