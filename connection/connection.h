...
///
/// The connection module implements general data exchange between gridlabd and
/// other applications. See
/// http://sourceforge.net/apps/mediawiki/gridlab-d/index.php?title=Dev:Connection
/// for details.
///
/// @{
...
typedef int(EXCHANGETRANSLATOR)(connection_transport *, const char *tag,
                                char *v, size_t vlen, int options);
...
typedef enum {
  ...
  MSG_CONTINUE = 0x01, ///< msg part of a group of message (automatically new if
                       ///< first in group)
  MSG_INITIATE, ///< open a new message group (if current group exists then
                ///< gives warning/error)
  ...
  MSG_STRING,   ///< group element is a string (followed by
                ///< "name",maxsize,"value")
  ...
  MSG_CRITICAL, ///< flag message as critical (must preceded MSG_INITIATE to
...
typedef enum {
  ...
  CS_EXTREME,  ///< extreme security (everything forbidden unless explicitly
               ///< permitted)
  CS_PARANOID, ///< paranoid security (#CS_EXTREME plus violation start lockout
               ///< clock)
...
public:
  ...
  virtual const char *
  get_mode_name(void) = 0; ///< get a string describing the transport type
...
public:
  ...
  void set_translators(EXCHANGETRANSLATOR *out, EXCHANGETRANSLATOR *in,
                       TRANSLATOR *data);

  cacheitem *create_cache(VARMAP *);

  int client_initiated(MESSAGEFLAG,
                       ...); ///< message group initiation (strings only)
  int server_response(MESSAGEFLAG,
                      ...); ///< message group response (strings only)
  // int client_initiated(char *tag, char *fmt, ...); ///< message item
  // (formatted)
  ...
  int exchange(EXCHANGETRANSLATOR *, const char *tag,
               char *string); // tag/value exchange (match only)
  int exchange(EXCHANGETRANSLATOR *, const char *tag, size_t len,
               char *string); // string exchange (copy back)
  int exchange(EXCHANGETRANSLATOR *, const char *tag,
               double &real); // double exchange
  int exchange(EXCHANGETRANSLATOR *, const char *tag,
               int64 &integer);                           // int64 exchange
  ...
  int exchange_open(EXCHANGETRANSLATOR *,
                    char *tag);             // start tagged group exchange
  ...
  int route_function(char *remotename, char *functionname, void *data,
                     size_t datalen, int64 &result);
};
...
typedef SIMULATIONMODE (*DELTAINTERUPDATE)(void *,
...
typedef SIMULATIONMODE (*DELTACLOCKUPDATE)(void *, double, unsigned long,
                                           SIMULATIONMODE);
...
void register_object_deltaclockupdate(void *data,
                                      DELTACLOCKUPDATE deltaClockUpdate);
#endif /// @}
