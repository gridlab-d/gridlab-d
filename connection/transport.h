...
public:
  ...
  get_transport() = 0; ///< get the type of transport
  virtual const char *
  get_transport_name(void) = 0; ///< get a string describing the transport type
  ...
  get_transport(const char *s); ///< get the type of transport from a name
  static connection_transport *
  new_instance(CONNECTIONTRANSPORT e); ///< create a new instance of a transport
  ...
  inline void set_delimiter(const char *d) {
    delimiter = const_cast<char *>(d);
  };
...
protected:
  ...
  void *(*translator)(
      char *, void *); // callback function to create/replace translation
public:
  inline void set_translator(void *(*fnc)(char *, void *)) {
    translator = fnc;
