...
EXPORT CLASS *init(CALLBACKS *fntable, MODULE *module, int argc, char *argv[]) {
  ...
  gl_global_create(
      "connection::security", PT_enumeration, &connection_security,
      PT_DESCRIPTION, "default connection security level", PT_KEYWORD, "NONE",
      (enumeration)CS_NONE, PT_KEYWORD, "LOW", (enumeration)CS_LOW, PT_KEYWORD,
      "STANDARD", (enumeration)CS_STANDARD, PT_KEYWORD, "HIGH",
      (enumeration)CS_HIGH, PT_KEYWORD, "EXTREME", (enumeration)CS_EXTREME,
      PT_KEYWORD, "PARANOID", (enumeration)CS_PARANOID, nullptr);
  gl_global_create("connection::lockout", PT_double, &connection_lockout,
                   PT_UNITS, "s", PT_DESCRIPTION,
                   "default connection security lockout time", nullptr);
  gl_global_create("connection::enable_subsecond_models", PT_bool,
                   &enable_subsecond_models, PT_DESCRIPTION,
                   "Enable deltamode capabilities within the connection module",
                   nullptr);
...
EXPORT TIMESTAMP clock_update(TIMESTAMP t1) {
  ...
  // t1 is ok to use.  Do not return TS_INVALID or any values less that
  // global_clock CAVEAT: this will be called multiple times until all modules
  // that use clock_update agree on the value of t1.
...
EXPORT int deltamode_desired(int *flags) {
  return DT_INFINITY; /* Returns time in seconds to when the next deltamode
                         transition is desired */

  /* Return DT_INFINITY if this module doesn't have any specific triggering
   * times */
}
...
EXPORT unsigned long preupdate(MODULE *module, TIMESTAMP t0,
  ...
  if (enable_subsecond_models == true) {
    // NOTE: that given how this is used currently, preupdate could just be a
    // return DT_INFINITY for all cases
    ...
    if (double_timestep_value <= 0.0) {
      gl_error("connection::preupdate: deltamode_timestep must be a positive, "
               "non-zero number!");
      /*  TROUBLESHOOT
      The value for global_deltamode_timestep, must be a positive, non-zero
      number. Please use such a number and try again.
      */
...
EXPORT SIMULATIONMODE interupdate(MODULE *module, TIMESTAMP t0,
  ...
  for (item = dInterUpdateList; item != nullptr; item = item->next) {
    result =
        item->deltainterupdate(item->data, iteration_count_val, t0, delta_time);
    switch (result) {
    case SM_DELTA_ITER:
      rv = SM_DELTA_ITER;
      break;
...
EXPORT SIMULATIONMODE deltaClockUpdate(MODULE *module, double t1,
                                       unsigned long timestep,
                                       SIMULATIONMODE systemmode) {
  ...
  if (dClockUpdateList == nullptr) {
    ...
    return SM_ERROR; // fall through return value resolves return-type warning.
