...
EXPORT SIMULATIONMODE dInterupdate(void *ptr, unsigned int dIntervalCounter,
                                   TIMESTAMP t0, unsigned int64 dt) {
  helics_msg *my = (helics_msg *)ptr;
  return my->deltaInterUpdate(dIntervalCounter, t0, dt);
}

EXPORT SIMULATIONMODE dClockupdate(void *ptr, double t1, unsigned long timestep,
                                   SIMULATIONMODE sysmode) {
  helics_msg *my = (helics_msg *)ptr;
...
helics_msg::helics_msg(MODULE *module) {
  ...
  oclass = gld_class::create(module, "helics_msg", sizeof(helics_msg),
                             PC_AUTOLOCK | PC_PRETOPDOWN | PC_BOTTOMUP |
                                 PC_POSTTOPDOWN | PC_OBSERVER);
  if (oclass == nullptr)
    throw "connection/helics_msg::helics_msg(MODULE*): unable to register "
          "class connection:helics_msg";
  else
    oclass->trl = TRL_UNKNOWN;

  defaults = this;
  if (gl_publish_variable(
          oclass, PT_double, "version", get_version_offset(), PT_DESCRIPTION,
          "helics_msg version", PT_enumeration, "message_type",
          PADDR(message_type), PT_DESCRIPTION,
          "set the type of message format you wish to construct", PT_KEYWORD,
          "GENERAL", enumeration(HMT_GENERAL), PT_DESCRIPTION,
          "use this for sending a general HELICS topic/value pair.", PT_KEYWORD,
          "JSON", enumeration(HMT_JSON), PT_DESCRIPTION,
          "use this for want to send a bundled json formatted messag in a "
          "single topic.",
          PT_int32, "publish_period", PADDR(publish_period), PT_DESCRIPTION,
          "use this with json bundling to set the period [s] at which data is "
          "published.",
          // TODO add published properties here
          nullptr) < 1)
    throw "connection/helics_msg::helics_msg(MODULE*): unable to publish "
          "properties of connection:helics_msg";
  if (!gl_publish_loadmethod(oclass, "configure",
                             [](void *val, char *str) -> int {
                               return loadmethod_helics_msg_configure(
                                   static_cast<OBJECT *>(val), str);
                             }))
    throw "connection/helics_msg::helics_msg(MODULE*): unable to publish "
          "configure method of connection:helics_msg";
}
...
int helics_msg::configure(char *value) {
  ...
  if (strcmp(configFile, "") != 0) {
    ...
    gl_error("helics_msg::configure(): No configuration file was give. Please "
  ...
  if (exitCode.get_int16() != 0) {
    ...
    if (fed_state != HELICS_STATE_FINALIZE) {
      if (fed_state != HELICS_STATE_ERROR) {
        ...
        string error_msg =
            fed_name +
            string(":The GridLAB-D Federate encountered an internal Error.");
...
int helics_msg::init(OBJECT *parent) {
...
#else
  gl_error(
      "helics_msg::init ~ helics was not linked with GridLAB-D at compilation. "
      "helics_msg cannot be used if helics was not linked with GridLAB-D.");
  rv = 0;
#endif
  if (rv == 0) {
    return 0;
  ...
  if (gld_helics_federate == nullptr) {
    try {
      gld_helics_federate =
          new helicscpp::CombinationFederate(*federate_configuration_file);
          ...
          if (pub.isValid()) {
            ...
            try {
              ...
              if (config_info.contains("message_type")) {
                individual_message_type =
                    config_info["message_type"].get<string>();
                if (individual_message_type.compare("JSON") == 0) {
                  ...
                  json_gld_pub->objectPropertyBundle =
                      config_info["publication_info"];
                  json_publication *gldProperty = nullptr;
                  for (auto &it : json_gld_pub->objectPropertyBundle.items()) {
                    const string gldObjName = it.key();
                    string gldPropName;
                    int n =
                        json_gld_pub->objectPropertyBundle[gldObjName].size();
                    for (int i = 0; i < n; i++) {
                      gldPropName =
                          json_gld_pub->objectPropertyBundle[gldObjName][i]
                              .get<string>();
                      gldProperty =
                          new json_publication(gldObjName, gldPropName);
                  ...
                  throw("The info field of the publication:%s defines an "
                        "unknown message_type:%s. Valid message types are JSON "
                        "and GENERAL",
                        pub.getName(), individual_message_type.c_str());
                }
              } else {
                gld_pub = new helics_value_publication();
                gld_pub->name = string(pub.getName());
              ...
              gl_verbose("helics_msg::init(): Successfully processed helics "
                         "publication %s.",
                         pub.getName());
            } catch (...) {
              throw("Couldn't parse the HelicsPublication %s's info field.",
                    pub.getName());
            }
          }
        }
          ...
          if (sub.isValid()) {
            ...
            try {
              config_info = config_info =
                  nlohmann::json::parse(config_info_temp);
              if (config_info.contains("message_type")) {
                individual_message_type =
                    config_info["message_type"].get<string>();
                if (individual_message_type.compare("JSON") == 0) {
                  ...
                  throw("The info field of the subscription:%s defines an "
                        "unknown message_type:%s. Valid message types are JSON "
                        "and GENERAL",
                        sub.getTarget(), individual_message_type.c_str());
                }
              } else {
                gld_sub = new helics_value_subscription();
                gld_sub->target = string(sub.getTarget());
              ...
              throw("Couldn't parse the HelicsSubscription %s's info field.",
                    sub.getTarget());
            }
          }
        }
        for (idx = 0; idx < ep_count; idx++) {
          ...
          if (ep.isValid()) {
            ...
            try {
              ...
              if (config_info.contains("message_type")) {
                ...
                if (individual_message_type.compare("JSON") == 0) {
                  if (!dest.empty()) {
                    ...
                    json_gld_ep_pub->objectPropertyBundle =
                        config_info["publication_info"];
                    json_publication *gldProperty = nullptr;
                    for (auto &it :
                         json_gld_ep_pub->objectPropertyBundle.items()) {
                      const string gldObjName = it.key();
                      string gldPropName;
                      int n = json_gld_ep_pub->objectPropertyBundle[gldObjName]
                                  .size();
                      for (int i = 0; i < n; i++) {
                        gldPropName =
                            json_gld_ep_pub->objectPropertyBundle[gldObjName][i]
                                .get<string>();
                        gldProperty =
                            new json_publication(gldObjName, gldPropName);
                        json_gld_ep_pub->jsonPublications.push_back(
                            gldProperty);
                      }
                    }
                    json_gld_ep_pub->HelicsPublicationEndpoint = ep;
                    json_helics_endpoint_publications.push_back(
                        json_gld_ep_pub);
                    gl_verbose("helics_msg::init(): registering publishing "
                               "endpoint: %s",
                               json_gld_ep_pub->name.c_str());
                    if (config_info.contains("receives_messages")) {
                      if (config_info["receives_messages"].get<bool>()) {
                        json_gld_ep_sub =
                            new json_helics_endpoint_subscription();
                        json_gld_ep_sub->name = ep.getName();
                        json_gld_ep_sub->HelicsSubscriptionEndpoint = ep;
                        json_helics_endpoint_subscriptions.push_back(
                            json_gld_ep_sub);
                        gl_verbose("helics_msg::init(): registering "
                                   "subscribing endpoint: %s",
                                   json_gld_ep_sub->name.c_str());
                      }
                    }
                  } else {
                    ...
                    json_helics_endpoint_subscriptions.push_back(
                        json_gld_ep_sub);
                    gl_verbose("helics_msg::init(): registering subscribing "
                               "endpoint: %s",
                               json_gld_ep_sub->name.c_str());
                  }
                } else if (individual_message_type.compare("GENERAL") == 0) {
                  if (config_info.contains("object") &&
                      config_info.contains("property")) {
                    if (!dest.empty()) {
                      gld_ep_pub = new helics_endpoint_publication();
                      gld_ep_pub->name = string(ep.getName());
                      gld_ep_pub->destination = dest;
                      gld_ep_pub->objectName =
                          config_info["object"].get<string>();
                      gld_ep_pub->propertyName =
                          config_info["property"].get<string>();
                      gld_ep_pub->HelicsPublicationEndpoint = ep;
                      helics_endpoint_publications.push_back(gld_ep_pub);
                      gl_verbose("helics_msg::init(): registering publishing "
                                 "endpoint: %s",
                                 gld_ep_pub->name.c_str());
                    } else {
                      gld_ep_sub = new helics_endpoint_subscription();
                      gld_ep_sub->name = string(ep.getName());
                      gld_ep_sub->objectName =
                          config_info["object"].get<string>();
                      gld_ep_sub->propertyName =
                          config_info["property"].get<string>();
                      gld_ep_sub->HelicsSubscriptionEndpoint = ep;
                      helics_endpoint_subscriptions.push_back(gld_ep_sub);
                      gl_verbose("helics_msg::init(): registering subscribing "
                                 "endpoint: %s",
                                 gld_ep_sub->name.c_str());
                    }
                  }
                  if (config_info.contains("publication_info")) {
                    gld_ep_pub = new helics_endpoint_publication();
                    gld_ep_pub->name = string(ep.getName());
                    gld_ep_pub->destination = dest;
                    gld_ep_pub->objectName =
                        config_info["publication_info"]["object"].get<string>();
                    gld_ep_pub->propertyName =
                        config_info["publication_info"]["property"]
                            .get<string>();
                    gld_ep_pub->HelicsPublicationEndpoint = ep;
                    helics_endpoint_publications.push_back(gld_ep_pub);
                    gl_verbose("helics_msg::init(): registering publishing "
                  ...
                  if (config_info.contains("subscription_info")) {
                    ...
                    gld_ep_sub->name = string(ep.getName());
                    gld_ep_sub->objectName =
                        config_info["subscription_info"]["object"]
                            .get<string>();
                    gld_ep_sub->propertyName =
                        config_info["subscription_info"]["property"]
                            .get<string>();
                    gld_ep_sub->HelicsSubscriptionEndpoint = ep;
                    helics_endpoint_subscriptions.push_back(gld_ep_sub);
                    gl_verbose("helics_msg::init(): registering subscribing "
                ...
                if (config_info.contains("publication_info")) {
                  ...
                  gld_ep_pub->objectName =
                      config_info["publication_info"]["object"].get<string>();
                  gld_ep_pub->propertyName =
                      config_info["publication_info"]["property"].get<string>();
                  gld_ep_pub->HelicsPublicationEndpoint = ep;
                  helics_endpoint_publications.push_back(gld_ep_pub);
                  gl_verbose(
                      "helics_msg::init(): registering publishing endpoint: %s",
                      gld_ep_pub->name.c_str());
                }
                if (config_info.contains("subscription_info")) {
                  gld_ep_sub = new helics_endpoint_subscription();
                  gld_ep_sub->name = string(ep.getName());
                  gld_ep_sub->objectName =
                      config_info["subscription_info"]["object"].get<string>();
                  gld_ep_sub->propertyName =
                      config_info["subscription_info"]["property"]
                          .get<string>();
                  gld_ep_sub->HelicsSubscriptionEndpoint = ep;
              ...
              throw("Couldn't parse the HelicsEndpoint %s's info field.",
                    ep.getName());
            }
          }
        }
            ...
            if (!dest.empty()) {
              ...
              try {
                json_gld_ep_pub->objectPropertyBundle =
                    nlohmann::json::parse(config_info_temp);
                json_publication *gldProperty = nullptr;
                for (auto &it : json_gld_ep_pub->objectPropertyBundle.items()) {
                  const string gldObjName = it.key();
                  string gldPropName;
                  int n =
                      json_gld_ep_pub->objectPropertyBundle[gldObjName].size();
                  for (int i = 0; i < n; i++) {
                    gldPropName =
                        json_gld_ep_pub->objectPropertyBundle[gldObjName][i]
                            .get<string>();
              ...
              gl_verbose(
                  "helics_msg::init(): registering subscribing endpoint: %s",
                  json_gld_ep_sub->name.c_str());
            }
          }
        }
      ...
      gl_error("helics_msg::init: an unexpected error occurred when trying to "
               "create a CombinationFederate using the configuration string: "
               "\n%s\nThis is the error that was caught:\n%s",
               federate_configuration_file->c_str(), e.what());
      return 0;
    }
  }
#endif
  for (vector<helics_value_publication *>::iterator pub =
           helics_value_publications.begin();
       pub != helics_value_publications.end(); pub++) {
    if ((*pub)->pObjectProperty == nullptr) {
      ...
      if ((*pub)->objectName.compare("global") != 0) {
        ...
        if (!(*pub)->pObjectProperty->is_valid()) {
          rv = 0;
          gl_error("helics_msg::init(): There is no object %s with property %s",
                   (char *)(*pub)->objectName.c_str(),
                   (char *)(*pub)->propertyName.c_str());
          break;
        }
      } else {
        ...
        if (!(*pub)->pObjectProperty->is_valid()) {
          rv = 0;
          gl_error("helics_msg::init(): There is no global property %s",
                   (char *)(*pub)->propertyName.c_str());
          break;
        }
      }
  ...
  for (vector<helics_value_subscription *>::iterator sub =
           helics_value_subscriptions.begin();
       sub != helics_value_subscriptions.end(); sub++) {
    if ((*sub)->pObjectProperty == nullptr) {
      ...
      if ((*sub)->objectName.compare("global") != 0) {
        ...
        if (!(*sub)->pObjectProperty->is_valid()) {
          rv = 0;
          gl_error("helics_msg::init(): There is no object %s with property %s",
                   (char *)(*sub)->objectName.c_str(),
                   (char *)(*sub)->propertyName.c_str());
          break;
        }
      } else {
  ...
  for (vector<helics_endpoint_publication *>::iterator pub =
           helics_endpoint_publications.begin();
       pub != helics_endpoint_publications.end(); pub++) {
    if ((*pub)->pObjectProperty == nullptr) {
      const char *pObjName = (*pub)->objectName.c_str();
      const char *pPropName = (*pub)->propertyName.c_str();
      char *pObjBuf = new char[strlen(pObjName) + 1];
  ...
  for (vector<helics_endpoint_subscription *>::iterator sub =
           helics_endpoint_subscriptions.begin();
       sub != helics_endpoint_subscriptions.end(); sub++) {
    if ((*sub)->pObjectProperty == nullptr) {
      const char *pObjName = (*sub)->objectName.c_str();
      const char *pPropName = (*sub)->propertyName.c_str();
      char *pObjBuf = new char[strlen(pObjName) + 1];
  ...
  for (vector<helics_value_publication *>::iterator pub =
    ...
    if ((*pub)->HelicsPublication.isValid()) {
      if ((*pub)->pObjectProperty->is_complex()) {
        if (string("complex").compare((*pub)->HelicsPublication.getType()) !=
            0) {
          gl_error(
              "helics_msg::init: The registered publication %s is intended to "
              "publish a complex type but the publication has a type = %s",
              ((*pub)->name).c_str(), (*pub)->HelicsPublication.getType());
          return 0;
        }
      } else if ((*pub)->pObjectProperty->is_integer()) {
        if (string((*pub)->HelicsPublication.getType()).find("int") ==
            string::npos) {
          gl_error(
              "helics_msg::init: The registered publication %s is intended to "
              "publish an integer type but the publication has a type = %s",
              ((*pub)->name).c_str(), (*pub)->HelicsPublication.getType());
          return 0;
        }
      } else if ((*pub)->pObjectProperty->is_double()) {
        if (string("double").compare((*pub)->HelicsPublication.getType()) !=
            0) {
          gl_error(
              "helics_msg::init: The registered publication %s is intended to "
              "publish a double type but the publication has a type = %s",
              ((*pub)->name).c_str(), (*pub)->HelicsPublication.getType());
          return 0;
        }
      } else {
        if (string("string").compare((*pub)->HelicsPublication.getType()) !=
            0) {
          gl_error(
              "helics_msg::init: The registered publication %s is intended to "
              "publish a string type but the publication has a type = %s",
              ((*pub)->name).c_str(), (*pub)->HelicsPublication.getType());
          return 0;
        }
      }
    } else {
      gl_error("helics_msg::init: There is no registered publication with a "
               "name = %s",
               (*pub)->name.c_str());
      return 0;
    }
  ...
  for (vector<helics_value_subscription *>::iterator sub =
    ...
    if ((*sub)->HelicsSubscription.isValid()) {
      if ((*sub)->pObjectProperty->is_complex()) {
        if (string("complex").compare((*sub)->HelicsSubscription.getType()) !=
            0) {
          gl_error("helics_msg::init: The registered subscription %s is "
                   "intended to subscribe to a complex type but the "
                   "subscription has a type = %s",
                   ((*sub)->target.c_str()),
                   (*sub)->HelicsSubscription.getType());
          return 0;
        }
      } else if ((*sub)->pObjectProperty->is_integer()) {
        if (string((*sub)->HelicsSubscription.getType()).find("int") ==
            string::npos) {
          gl_error("helics_msg::init: The registered subscription %s is "
                   "intended to subscribe to an integer type but the "
                   "subscription has a type = %s",
                   ((*sub)->target.c_str()),
                   (*sub)->HelicsSubscription.getType());
          return 0;
        ...
        if (string("double").compare((*sub)->HelicsSubscription.getType()) !=
            0) {
          gl_error(
              "helics_msg::init: The registered subscription %s is intended to "
              "subscribe to double type but the subscription has a type = %s",
              ((*sub)->target.c_str()), (*sub)->HelicsSubscription.getType());
          return 0;
        }
      } else {
        if (string("string").compare((*sub)->HelicsSubscription.getType()) !=
            0) {
          gl_error(
              "helics_msg::init: The registered subscription %s is intended to "
              "subscribe to string type but the subscription has a type = %s",
              ((*sub)->target.c_str()), (*sub)->HelicsSubscription.getType());
          return 0;
        }
      }
    } else {
      gl_error("helics_msg::init: There is no registered subscription with a "
               "key = %s",
               (*sub)->target.c_str());
      return 0;
    }
  ...
  for (vector<helics_endpoint_publication *>::iterator pub =
    ...
    if (!(*pub)->HelicsPublicationEndpoint.isValid()) {
      gl_error(
          "helics_msg::init: There is no registered endpoint with a name = %s",
          (*pub)->name.c_str());
      return 0;
    }
  }
  ...
  for (vector<helics_endpoint_subscription *>::iterator sub =
    ...
    if (!(*sub)->HelicsSubscriptionEndpoint.isValid()) {
      gl_error("helics_msg::init: There is no registered endpoint with a "
               "target = %s",
               (*sub)->name.c_str());
      return 0;
    }
...
TIMESTAMP helics_msg::sync(TIMESTAMP t1) { return TS_NEVER; }

TIMESTAMP helics_msg::postsync(TIMESTAMP t1) { return TS_NEVER; }

TIMESTAMP helics_msg::commit(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }

SIMULATIONMODE
helics_msg::deltaInterUpdate(unsigned int delta_iteration_counter, TIMESTAMP t0,
                             unsigned int64 dt) {
  int result = 0;
...
SIMULATIONMODE helics_msg::deltaClockUpdate(double t1, unsigned long timestep,
                                            SIMULATIONMODE sysmode) {
#if HAVE_HELICS
  if (t1 > last_delta_helics_time) {
    ...
    if (sysmode == SM_EVENT) {
      t = (HelicsTime)(((dt + (1000000000.0 / 2.0)) -
                        fmod((dt + (1000000000.0 / 2.0)), 1000000000.0)) /
                       1000000000.0);
    } else {
      t = (HelicsTime)(((dt + ((double)(timestep) / 2.0)) -
                        fmod((dt + ((double)(timestep) / 2.0)),
                             (double)timestep)) /
                       1000000000.0);
    }
    gld_helics_federate->setProperty(
        HELICS_PROPERTY_TIME_PERIOD,
        (HelicsTime)(((double)timestep) / DT_SECOND));
    ...
    if (helics_t_ns != t_ns) {
      gl_error("helics_msg::deltaClockUpdate: Cannot return anything other "
               "than the time GridLAB-D requested in deltamode. Time requested "
               "%f. Time returned %f.",
               (double)t, (double)helics_t);
      return SM_ERROR;
    } else {
...
TIMESTAMP helics_msg::clk_update(TIMESTAMP t1) {
  ...
  if (exitDeltamode == true) {
    ...
    gld_helics_federate->setProperty(
        HELICS_PROPERTY_TIME_PERIOD,
        1.0); // 140 is the option for the period property.
#endif
    exitDeltamode = false;
  }
  if (t1 > last_approved_helics_time) {
    ...
    //		HelicsTime = ((TIMESTAMP)helics::time_request(t))/1000000000 +
    //initial_sim_time;
    ...
    if (helics_t <= gl_globalclock) {
      gl_error("helics_msg::clock_update: Cannot return the current time or "
...
int helics_msg::publishVariables() {
  ...
  for (vector<helics_value_publication *>::iterator pub =
    ...
    if ((*pub)->pObjectProperty->is_complex()) {
      ...
      gl_verbose("helics_msg: calling publish(%f%+fj) on publication %s",
                 real_part, imag_part, (*pub)->name.c_str());
      (*pub)->HelicsPublication.publish(complex_temp);
    } else if ((*pub)->pObjectProperty->is_integer()) {
      int64_t integer_temp = (*pub)->pObjectProperty->get_integer();
      gl_verbose("helics_msg: calling publishInt(%d) on publication %s",
                 integer_temp, (*pub)->name.c_str());
      (*pub)->HelicsPublication.publish(integer_temp);
    } else if ((*pub)->pObjectProperty->is_double()) {
      double double_temp = 0;
      (*pub)->pObjectProperty->getp(double_temp);
      gl_verbose("helics_msg: calling publish(%f) on publication %s",
                 double_temp, (*pub)->name.c_str());
      ...
      if (buffer_size <= 0) {
        ...
        gl_verbose("helics_msg: Calling publish(\"%s\") on publication %s",
                   temp_value.c_str(), (*pub)->name.c_str());
  ...
  for (vector<helics_endpoint_publication *>::iterator pub =
    ...
    if ((*pub)->pObjectProperty->is_complex()) {
      ...
      complex_temp = {real_part, imag_part};
      buffer_size =
          snprintf(&buffer[0], 1023, "%.3f%+.3fj", real_part, imag_part);
    } else {
      buffer_size = (*pub)->pObjectProperty->to_string(&buffer[0], 1023);
    }
    ...
    try {
      if (gld_helics_federate->getCurrentMode() == HELICS_STATE_EXECUTION) {
        helicscpp::Message *msg =
            new helicscpp::Message((*pub)->HelicsPublicationEndpoint);
        msg->data(message_buffer);
        gl_verbose("calling helics sendMessage on endpoint %s. Message: %s",
                   (*pub)->name.c_str(), message_buffer.c_str());
      ...
      std::cout << e.what()
                << std::endl; // information from length_error printed
    }
#endif
    memset(&buffer[0], '\0', 1024);
  }
#if HAVE_HELICS
  for (vector<json_helics_value_publication *>::iterator pub =
           json_helics_value_publications.begin();
       pub != json_helics_value_publications.end(); pub++) {
    jsonPublishData.clear();
    for (vector<json_publication *>::iterator o =
             (*pub)->jsonPublications.begin();
         o != (*pub)->jsonPublications.end(); o++) {
      prop = (*o)->prop;
      if (prop->is_valid()) {
        if (prop->is_double()) {
          jsonPublishData[(*o)->object_name][(*o)->property_name] =
              prop->get_double();
    ...
    jsonMessageStr = jsonPublishData.dump(4);
    gl_verbose("publishing json message on publication %s: %s",
               (*pub)->name.c_str(), jsonMessageStr.c_str());
    (*pub)->HelicsPublication.publish(jsonMessageStr);
  }
  for (vector<json_helics_endpoint_publication *>::iterator pub =
           json_helics_endpoint_publications.begin();
       pub != json_helics_endpoint_publications.end(); pub++) {
    jsonPublishData.clear();
    for (vector<json_publication *>::iterator o =
             (*pub)->jsonPublications.begin();
         o != (*pub)->jsonPublications.end(); o++) {
    ...
    gl_verbose("sending JSON message on endpoint %s: %s", (*pub)->name.c_str(),
               jsonMessageStr.c_str());
    (*pub)->HelicsPublicationEndpoint.sendMessage(*msg);
  }
#endif
  return 1;
...
int helics_msg::subscribeVariables() {
  ...
  for (vector<helics_value_subscription *>::iterator sub =
    ...
    if ((*sub)->HelicsSubscription.isUpdated()) {
      try {
        if ((*sub)->pObjectProperty->is_complex()) {
          gl_verbose("helics_msg: Calling getComplex on subscription %s",
                     (*sub)->target.c_str());
          complex_temp = (*sub)->HelicsSubscription.getComplex();
          if (!std::isnan(complex_temp.real()) &&
              !std::isnan(complex_temp.imag())) {
          ...
          gl_verbose("helics_msg: Calling getInteger on subscription %s",
                     (*sub)->target.c_str());
          ...
          gl_verbose("helics_msg: Calling getDouble on subscription %s",
                     (*sub)->target.c_str());
          ...
          gl_verbose("helics_msg: Calling getString on subscription %s",
                     (*sub)->target.c_str());
  ...
  for (vector<helics_endpoint_subscription *>::iterator sub =
           helics_endpoint_subscriptions.begin();
       sub != helics_endpoint_subscriptions.end(); sub++) {
    gl_verbose("Has message status for endpoint %s: %s", (*sub)->name.c_str(),
               (*sub)->HelicsSubscriptionEndpoint.hasMessage() ? "True"
                                                               : "False");
    if ((*sub)->HelicsSubscriptionEndpoint.hasMessage()) {
      helicscpp::Message mesg;
      int pendingMessages =
          (int)(*sub)->HelicsSubscriptionEndpoint.pendingMessageCount();
      for (int i = 0; i < pendingMessages; i++) {
        gl_verbose("calling getMessage() for endpoint %s",
                   (*sub)->name.c_str());
  ...
  for (vector<json_helics_value_subscription *>::iterator sub =
           json_helics_value_subscriptions.begin();
       sub != json_helics_value_subscriptions.end(); sub++) {
    if ((*sub)->HelicsSubscription.isUpdated()) {
      gl_verbose("JSON subscription %s updated.", (*sub)->target.c_str());
      value = (*sub)->HelicsSubscription.getString();
      try {
        jsonMessage = nlohmann::json::parse(value);
        for (auto &o : jsonMessage.items()) {
          objectName = o.key();
          for (auto &p : o.value().items()) {
            propertyName = p.key();
            ...
            if (gldProperty->is_valid()) {
              if (gldProperty->is_integer()) {
                int64_t itmp =
                    jsonMessage[objectName][propertyName].get<int64_t>();
                ...
                double dtmp =
                    jsonMessage[objectName][propertyName].get<double>();
                gldProperty->setp(dtmp);
              } else {
                string stmp =
                    jsonMessage[objectName][propertyName].get<string>();
                char sbuf[1024] = "";
                strncpy(sbuf, stmp.c_str(), 1023);
                gldProperty->from_string(sbuf);
              }
            } else {
              gl_error("helics_msg::subscribeVariables(): There is no object "
                       "%s with property %s",
                       objectName.c_str(), propertyName.c_str());
              delete gldProperty;
              return 0;
            }
            delete gldProperty;
        ...
        throw("Couldn't parse the json message for HelicsSubscription %s.",
              (*sub)->target.c_str());
      }
    }
  }
  for (vector<json_helics_endpoint_subscription *>::iterator sub =
           json_helics_endpoint_subscriptions.begin();
       sub != json_helics_endpoint_subscriptions.end(); sub++) {
    if ((*sub)->HelicsSubscriptionEndpoint.hasMessage()) {
      helicscpp::Message mesg;
      int pendingMessages =
          (int)(*sub)->HelicsSubscriptionEndpoint.pendingMessageCount();
      ...
      try {
        jsonMessage = nlohmann::json::parse(message_buffer);
        for (auto &o : jsonMessage.items()) {
          objectName = o.key();
          for (auto &p : o.value().items()) {
            propertyName = p.key();
        ...
        throw("Couldn't parse the json message for HelicsEndpoint %s.",
              (*sub)->name.c_str());
      }
    }
  }
...
int helics_msg::publishJsonVariables() {
  ...
  for (vector<json_helics_value_publication *>::iterator pub =
    ...
    for (vector<json_publication *>::iterator o =
      ...
      if (prop->is_valid()) {
        if (prop->is_double()) {
          jsonPublishData[simName][(*o)->object_name][(*o)->property_name] =
              prop->get_double();
        } else if (prop->is_complex()) {
          double real_part = prop->get_part("real");
          double imag_part = prop->get_part("imag");
...
int helics_msg::subscribeJsonVariables() {
  ...
  for (vector<json_helics_value_subscription *>::iterator sub =
    ...
    if ((*sub)->HelicsSubscription.isUpdated()) {
      ...
      try {
        jsonMessage = nlohmann::json::parse(value);
        if (!jsonMessage.contains(simName.c_str())) {
          jsonData = jsonMessage;
          for (auto &o : jsonData.items()) {
            objectName = o.key();
            for (auto &p : o.value().items()) {
              propertyName = p.key();
              const char *expr1 = objectName.c_str();
              ...
              if (gldProperty->is_valid()) {
                if (gldProperty->is_integer()) {
                  int64_t itmp =
                      jsonData[objectName][propertyName].get<int64_t>();
                  gld_type property_type = gldProperty->get_type();
                  if (property_type == PT_int64) {
                    gldProperty->setp(itmp);
                  } else if (property_type == PT_int32) {
                  ...
                  double dtmp =
                      jsonData[objectName][propertyName].get<double>();
                  gldProperty->setp(dtmp);
                } else {
                  string stmp =
                      jsonData[objectName][propertyName].get<string>();
                  char sbuf[1024] = "";
                  strncpy(sbuf, stmp.c_str(), 1023);
                  gldProperty->from_string(sbuf);
                }
...
extern "C" size_t helics_from_hex(void *buf, size_t len, const char *hex, size_t
hexlen)
{
        char *p = (char*)buf;
        char lo = nullptr;
...
extern "C" void outgoing_helics_function(char *from, char *to, char *funcName,
char *funcClass, void *data, size_t len)
{
        int64 result = -1;
        ...
        if(relay == nullptr){
                throw("helics_msg::outgoing_route_function: the relay function
for function name %s could not be found.", funcName);
        }
        if( relay->drtn != DXD_WRITE){
                throw("helics_msg:outgoing_helics_function: the relay function
for the function name ?s could not be found.", funcName);
        }
        char message[3000] = "";
...
string(relay->remotename); if( relay->ctype == CT_PUBSUB){ #if HAVE_HELICS
//			helics::publish(key, payload.str());
                        //TODO call appropriate helics function for publishing a
value #endif } else if( relay->ctype == CT_ROUTE){ string sender = string((const
char *)from); string recipient = string((const char *)to); #if HAVE_HELICS
//			helics::route(sender, recipient, key, payload.str());
                        //TODO call appropriate helics function for publishing a
communication message #endif
                }
        }
}

extern "C" FUNCTIONADDR add_helics_function(helics_msg *route, const char
*fclass, const char *flocal, const char *rclass, const char *rname, TRANSLATOR
*xlate, DATAEXCHANGEDIRECTION direction, COMMUNICATIONTYPE ctype)
{
        // check for existing of relay (note only one relay is allowed per class
pair) FUNCTIONSRELAY *relay = find_helics_function(rclass, rname); if (
relay!=nullptr )
        {
                gl_error("helics_msg::add_helics_function(rclass='%s',
rname='%s') a relay function is already defined for '%s/%s'",
rclass,rname,rclass,rname); return 0;
        }
        ...
        {
                gl_error("helics_msg::add_helics_function(rclass='%s',
rname='%s') memory allocation failed", rclass,rname); return 0;
        }
...
extern "C" FUNCTIONSRELAY *find_helics_function(const char *rclass, const
char*rname)
{
        ...
        {
                if (strcmp(relay->remotename, rname)==0 &&
strcmp(relay->remoteclass, rclass)==0) return relay;
        }
        return nullptr;
}*/
