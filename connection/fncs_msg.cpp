...
EXPORT SIMULATIONMODE fncs_dInterupdate(void *ptr,
                                        unsigned int dIntervalCounter,
                                        TIMESTAMP t0, unsigned int64 dt) {
  fncs_msg *my = (fncs_msg *)ptr;
  return my->deltaInterUpdate(dIntervalCounter, t0, dt);
}

EXPORT SIMULATIONMODE fncs_dClockupdate(void *ptr, double t1,
...
fncs_msg::fncs_msg(MODULE *module) {
  ...
  oclass = gld_class::create(module, "fncs_msg", sizeof(fncs_msg),
                             PC_AUTOLOCK | PC_PRETOPDOWN | PC_BOTTOMUP |
                                 PC_POSTTOPDOWN | PC_OBSERVER);
  if (oclass == nullptr)
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to register class "
          "connection:fncs_msg";
  else
  ...
  if (gl_publish_variable(
          ...
          "fncs_msg version", PT_enumeration, "message_type",
          PADDR(message_type), PT_DESCRIPTION,
          "set the type of message format you wish to construct", PT_KEYWORD,
          "GENERAL", enumeration(MT_GENERAL), PT_DESCRIPTION,
          "use this for sending a general fncs topic/value pair", PT_KEYWORD,
          "JSON", enumeration(MT_JSON), PT_DESCRIPTION,
          "use this for wanting to send a bundled json formatted message in a "
          "single topic",
          PT_int32, "gridappd_publish_period",
          PADDR(real_time_gridappsd_publish_period), PT_DESCRIPTION,
          "use this with json bundling to set the period [s] at which data is "
          "published.",
          // PT_KEYWORD, "JSON_SB", enumeration(MT_JSON_SB), PT_DESCRIPTION,
          // "use this for wanting to subsribe a bundled json formatted message
          // in a single topic",
          PT_bool, "aggregate_publications", PADDR(aggregate_pub),
          PT_DESCRIPTION, "enable FNCS flag to aggregate publications", PT_bool,
          "aggregate_subscriptions", PADDR(aggregate_sub), PT_DESCRIPTION,
          "enable FNCS flag to aggregate subscriptions",
          // TODO add published properties here
          nullptr) < 1)
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish "
          "properties of connection:fncs_msg";
  if (!gl_publish_loadmethod(
          oclass, "route", (int (*)(void *, char *))loadmethod_fncs_msg_route))
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish route "
          "method of connection:fncs_msg";
  if (!gl_publish_loadmethod(
          oclass, "option",
          (int (*)(void *, char *))loadmethod_fncs_msg_option))
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish option "
          "method of connection:fncs_msg";
  if (!gl_publish_loadmethod(
          oclass, "publish",
          (int (*)(void *, char *))loadmethod_fncs_msg_publish))
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish publish "
          "method of connection:fncs_msg";
  if (!gl_publish_loadmethod(
          oclass, "subscribe",
          (int (*)(void *, char *))loadmethod_fncs_msg_subscribe))
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish subscribe "
          "method of connection:fncs_msg";
  if (!gl_publish_loadmethod(
          oclass, "configure",
          (int (*)(void *, char *))loadmethod_fncs_msg_configure))
    throw "connection/fncs_msg::fncs_msg(MODULE*): unable to publish configure "
          "method of connection:fncs_msg";
}
...
int fncs_msg::option(char *value) {
  ...
  if (sscanf(value, "%[^:]:%[^\n]", target, command) == 2) {
    ...
    gl_error(
        "fncs_msg::option(char *value='%s'): unable to parse option argument",
        value);
    return 0;
  }
  cmd = &command[0];
  if (strncmp(target, "connection", 10) == 0) {
    gl_warning("connection is always a client when operating with fncs. "
               "ingnoring option.");
    rv = 1;
...
int fncs_msg::configure(char *value) {
  ...
  if (strcmp(configFile, "") != 0) {
      ...
      if (message_type == MT_GENERAL) { // renke debug
        while (!ifile.eof()) {
          ...
          if (confLine.find("publish") != string::npos) {
            gl_verbose("fncs_msg.configure(): processing line: %s",
                       confLine.c_str());
            value = confLine.substr(confLine.find("publish") + 9,
                                    confLine.length() -
                                        confLine.find("publish") - 9 - 2);
            ...
            gl_verbose("fncs_msg.configure(): processing line: %s",
                       confLine.c_str());
            value = confLine.substr(confLine.find("subscribe") + 11,
                                    confLine.length() -
                                        confLine.find("subscribe") - 11 - 2);
            strcpy(cval, value.c_str());
            rv = subscribe(&cval[0]);
            if (rv == 0) {
              return 0;
            ...
            gl_verbose("fncs_msg.configure(): processing line: %s",
                       confLine.c_str());
            value = confLine.substr(confLine.find("route") + 7,
                                    confLine.length() - confLine.find("route") -
                                        7 - 2);
            strcpy(cval, value.c_str());
            rv = route(&cval[0]);
            if (rv == 0) {
            ...
            gl_verbose("fncs_msg.configure(): processing line: %s",
                       confLine.c_str());
            value = confLine.substr(confLine.find("option") + 8,
                                    confLine.length() -
                                        confLine.find("option") - 8 - 2);
            strcpy(cval, value.c_str());
            rv = option(&cval[0]);
            if (rv == 0) {
              return 0;
        ...
        while (ifile >> json_config_line) { // Place the entire contents of the
                                            // file into a stringstream
          json_config_stream << json_config_line << "\n";
        }
        json_config_string = json_config_stream.str();
        gl_verbose("fncs_msg::configure(): json string read from configure "
                   "file: %s .\n",
    ...
    //}
      ...
      gl_error(
          "fncs_msg::configure(): failed to open the configuration file %s \n",
...
int fncs_msg::init(OBJECT *parent) {
...
#else
  gl_error(
      "fncs_msg::init ~ fncs was not linked with GridLAB-D at compilation. "
      "fncs_msg cannot be used if fncs was not linked with GridLAB-D.");
  rv = 0;
#endif
  if (rv == 0) {
    return 0;
  ...
  // resolve all the json_configure gl_properties for publishing json style
  // string, renke
  // TODO
  ...
  for (int isize = 0; isize < nsize; isize++) {
    if (!vjson_publish_gld_property_name[isize]->is_header) {
      const string gld_object_name =
          vjson_publish_gld_property_name[isize]->object_name;
      if (gld_object_name.compare("globals") != 0) {
        const string gld_property_name =
            vjson_publish_gld_property_name[isize]->object_name + "." +
            vjson_publish_gld_property_name[isize]->object_property;
        const char *expr1 =
            vjson_publish_gld_property_name[isize]->object_name.c_str();
        const char *expr2 =
            vjson_publish_gld_property_name[isize]->object_property.c_str();
        char *bufObj = new char[strlen(expr1) + 1];
        char *bufProp = new char[strlen(expr2) + 1];
        strcpy(bufObj, expr1);
        strcpy(bufProp, expr2);
        vjson_publish_gld_property_name[isize]->prop =
            new gld_property(bufObj, bufProp);

        if (vjson_publish_gld_property_name[isize]->prop->is_valid()) {
          gl_verbose(
              "connection: local variable '%s' resolved OK, object id %d",
              gld_property_name.c_str(),
              vjson_publish_gld_property_name[isize]
                  ->prop->get_object()
                  ->id); // renke debug
        } else {
          gl_warning("connection: local variable '%s' cannot be resolved. This "
                     "local variable will not be publish.",
                     gld_property_name.c_str());
        }
      } else {
        const string gld_global_name =
            vjson_publish_gld_property_name[isize]->object_property;
        const char *expr1 =
            vjson_publish_gld_property_name[isize]->object_property.c_str();
        ...
        vjson_publish_gld_property_name[isize]->prop =
            new gld_property(bufProp);
        if (vjson_publish_gld_property_name[isize]->prop->is_valid()) {
          gl_verbose(
              "fncs_msg::init: Global variable '%s' resolved OK",
              vjson_publish_gld_property_name[isize]->object_property.c_str());
        } else {
          gl_warning(
              "fncs_msg::init: Global variable '%s' cannot be resolved This "
              "global variable will not be published.",
              vjson_publish_gld_property_name[isize]->object_property.c_str());
        }
      }
    } else {
      if (vjson_publish_gld_property_name[isize]->object_property.compare(
              "parent") == 0) {
        char objname[1024];
        const char *oName =
            vjson_publish_gld_property_name[isize]->object_name.c_str();
        ...
        vjson_publish_gld_property_name[isize]->hdr_val =
            string(vjson_publish_gld_property_name[isize]->obj->parent->name);
      }
    }
  }
  ...
  for (int isize = 0; isize < nsize; isize++) {
    ...
    if (vObj != nullptr) {
      if ((vObj->flags & OF_INIT) != OF_INIT) {
        gl_warning(
            "%d, fncs_msg::init(): vjson_publish_gld_property %s.%s not "
            "initialized!:  \n",
            isize, vjson_publish_gld_property_name[isize]->object_name.c_str(),
            vjson_publish_gld_property_name[isize]->object_property.c_str());
        defer = true;
      }
    }
  ...
  fncs_step = 1;
  // parsing the global variable from string==>int (code copied from
  // regulator.cpp)
  char temp_buff[128];
  ...
  if (message_type == MT_GENERAL) {
    ...
    zplfile << "time_delta = " << fncs_step << "ns"
            << endl; // TODO: minimum timestep needs to take into account
                     // deltamode steps eventually.
        ...
        for (pMap = vmap[n]->getfirst(); pMap != nullptr; pMap = pMap->next) {
          if (pMap->dir == DXD_READ) {
            ...
            for (i = 0; i < inVariableTopics.size(); i++) {
              if ((*inFunctionTopics)[i].compare(string(pMap->remote_name)) ==
                  0) {
                uniqueTopic = false;
              }
            }
            if (uniqueTopic == true) {
              ...
              zplfile << "        topic = " << string(pMap->remote_name)
                      << endl;
              if (dft.empty() != true) {
  ...
  gridappsd_publish_time =
      initial_sim_time + (TIMESTAMP)real_time_gridappsd_publish_period;
  return rv;
}
...
TIMESTAMP fncs_msg::commit(TIMESTAMP t0, TIMESTAMP t1) {
  ...
  if (message_type == MT_JSON) {
    if (real_time_gridappsd_publish_period == 0 ||
        t1 == gridappsd_publish_time) {
      if (real_time_gridappsd_publish_period > 0) {
        gridappsd_publish_time =
            t1 + (TIMESTAMP)real_time_gridappsd_publish_period;
      }
      if (t1 < gl_globalstoptime) {
...
SIMULATIONMODE fncs_msg::deltaInterUpdate(unsigned int delta_iteration_counter,
                                          TIMESTAMP t0, unsigned int64 dt) {
  int result = 0;
...
SIMULATIONMODE fncs_msg::deltaClockUpdate(double t1, unsigned long timestep,
                                          SIMULATIONMODE sysmode) {
  ...
  if (t1 > last_delta_fncs_time) {
    ...
    if (sysmode == SM_EVENT) {
      t = (fncs::time)((dt + (1000000000.0 / 2.0)) -
                       fmod((dt + (1000000000.0 / 2.0)), 1000000000.0));
    } else {
      t = (fncs::time)((dt + ((double)(timestep) / 2.0)) -
    ...
    rv = SM_EVENT;
    if (fncs_time != t) {
      gl_error("fncs_msg::deltaClockUpdate: Cannot return anything other than "
               "the time GridLAB-D requested in deltamode.");
      return SM_ERROR;
    } else {
      last_delta_fncs_time =
          (double)(fncs_time) / 1000000000.0 + (double)(initial_sim_time);
    }
  }
#endif
  return rv; // We should've only gotten here by being in SM_DELTA to begin
             // with.
}
...
TIMESTAMP fncs_msg::clk_update(TIMESTAMP t1) {
  ...
  if (t1 > last_approved_fncs_time) {
    ...
    fncs_time =
        ((TIMESTAMP)fncs::time_request(t)) / 1000000000 + initial_sim_time;
#endif

    if (fncs_time <= gl_globalclock) {
      gl_error("fncs_msg::clock_update: Cannot return the current time or less "
...
int fncs_msg::get_varmapindex(const char *name) {
  const char *varmapname[] = {
      "",           "allow",    "forbid",   "init",   "precommit",
      "presync",    "sync",     "postsync", "commit", "prenotify",
      "postnotify", "finalize", "plc",      "term"};
  int n;
  for (n = 1; n < 14; n++) {
...
int fncs_msg::fncs_link(char *value, COMMUNICATIONTYPE comtype) {
  ...
  char argument[1024] = "";
  VARMAP *mp = nullptr;
  // parse argument to fill the relay function link list and the varmap link
  // list.
  if (sscanf(value, "%[^:]:%[^\n]", command, argument) == 2) {
    if (strncmp(command, "init", 4) == 0) {
      gl_warning("fncs_msg::publish: It is not possible to pass information at "
                 "init time with fncs. communication is ignored");
      rv = 1;
...
int fncs_msg::parse_fncs_function(char *value, COMMUNICATIONTYPE comtype) {
  ...
  if (sscanf(value, "%[^/]/%[^-<>\t ]%*[\t ]%[-<>]%*[\t ]%[^\n]", localClass,
             localFuncName, direction, topic) != 4) {
  ...
  if (fclass == nullptr) {
    gl_error("fncs_msg::parse_fncs_function(const char *spec='%s'): local "
  ...
  if (strcmp(direction, "->") == 0) {
    ...
    if (flocal != nullptr)
      gl_warning("fncs_msg::parse_fncs_function(const char *spec='%s'): "
                 "outgoing call definition of '%s' overwrites existing "
                 "function definition in class '%s'",
                 value, localFuncName, localClass);

    sscanf(topic, "%[^/]/%[^\n]", remoteClassName, remoteFuncName);
    // get relay function
    flocal = add_fncs_function(this, localClass, localFuncName, remoteClassName,
                               remoteFuncName, nullptr, DXD_WRITE, comtype);
    ...
    if (rv == 0) {
      gl_error("fncs_msg::parse_fncs_function(const char *spec='%s'): failed "
               "to define the function '%s' in local class '%s'.",
               value, localFuncName, localClass);
      return rv;
    }
    // setup incoming call
    ...
    if (flocal == nullptr) {
      gl_error("fncs_msg::parse_fncs_function(const char *spec='%s'): local "
               "function '%s' is not valid.",
               value, localFuncName);
      return 0;
    }
    flocal = add_fncs_function(this, localClass, localFuncName, "", topic,
                               nullptr, DXD_READ, comtype);
    if (flocal == nullptr) {
      rv = 1;
    }
  }
      ...
      if (s > 0) {
        for (int i = 0; i < s; i++) {
          ...
          if (sscanf(message,
                     ...
                     "\"function\":\"%[^\"]\", \"data\":\"%[^\"]\", \"data "
                     "length\":\"%[^\"]\"}\"",
                     from, to, funcName, payloadString,
                     payloadLengthstr) != 5) {
            throw("fncs_msg::incomming_fncs_function: unable to parse function "
                  "message %s",
                  message);
          }
          ...
          if (strcmp(funcName, relay->localcall) != 0) {
            throw("fncs_msg::incomming_fncs_function: The remote side function "
                  "call, %s, is not the same as the local function name, %s.",
                  funcName, relay->localcall);
          }
          ...
          // unhex raw payload
          rplen = fncs_from_hex(rawPayload, (size_t)payloadLength,
                                payloadString, (size_t)payloadStringLength);
          if (rplen < strlen(payloadString)) {
            throw("fncs_msg::incomming_fncs_function: unable to decode "
                  "function payload %s.",
                  payloadString);
          }
          // call local function
          ...
          if (obj == nullptr) {
            throw("fncs_msg::incomming_fncs_function: the to object does not "
                  "exist. %s.",
                  to);
          }
          funcAddr = (FUNCTIONADDR)(gl_get_function(obj, relay->localcall));
          ((void (*)(char *, char *, char *, char *, void *, size_t))(
              *funcAddr))(from, to, relay->localcall, relay->localclass,
                          rawPayload, (size_t)payloadLength);
        }
      }
    }
  }
        ...
        if (pub_value == true) {
          if (mp->ctype == CT_PUBSUB) {
            ...
            if (sscanf(mp->local_name, "%[^.].", fromBuf) != 1) {
              gl_error("fncs_msg::publishVariables: unable to parse 'from' "
                       "name from %s.",
                       mp->local_name);
              return 0;
            }
            if (sscanf(mp->remote_name, "%[^/]/%[^\n]", toBuf, keyBuf) != 2) {
              gl_error("fncs_msg::publishVariables: unable to parse 'to' and "
                       "'key' from %s.",
                       mp->remote_name);
              return 0;
            }
...
int fncs_msg::subscribeVariables(varmap *rmap) {
  ...
  for (mp = rmap->getfirst(); mp != nullptr; mp = mp->next) {
    if (mp->dir == DXD_READ) {
      if (mp->ctype == CT_PUBSUB) {
#if HAVE_FNCS
        if (std::find(updated_events.begin(), updated_events.end(),
                      string(mp->remote_name)) != updated_events.end()) {
...
int fncs_msg::publishJsonVariables() // Renke add
{
  ...
  // gl_verbose("entering fncs_msg::publishJsonVariables():
  // vjson_publish_gld_property_name size: %d \n", nvecsizeproname);
  // gl_verbose("entering fncs_msg::publishJsonVariables():
  // vjson_publish_gld_property size: %d \n", nvecsize);
    ...
    if (!vjson_publish_gld_property_name[isize]->is_header) {
      ...
      if (gldpro_obj->is_valid()) {
        if (gldpro_obj->is_double()) {
          publish_json_data
              [simName][vjson_publish_gld_property_name[isize]->object_name]
              [vjson_publish_gld_property_name[isize]->object_property] =
                  gldpro_obj->get_double();
          ...
          publish_json_data
              [simName][vjson_publish_gld_property_name[isize]->object_name]
              [vjson_publish_gld_property_name[isize]->object_property] =
                  gldpro_obj->get_integer();
        } else if (gldpro_obj->is_character() || gldpro_obj->is_enumeration() ||
                   gldpro_obj->is_complex() || gldpro_obj->is_objectref() ||
                   gldpro_obj->is_set()) {
          char chtmp[1024];
          ...
          publish_json_data
              [simName][vjson_publish_gld_property_name[isize]->object_name]
              [vjson_publish_gld_property_name[isize]->object_property] =
                  gldpro_obj->get_timestamp();
        } else {
          gl_error(
              "fncs_msg::publishJsonVariables(): the type of the gld_property: "
              "%s.%s is not a recognized type! \n",
              vjson_publish_gld_property_name[isize]->object_name.c_str(),
              vjson_publish_gld_property_name[isize]->object_property.c_str());
          return 0;
        }
  ...
  gl_verbose(
      "fncs_msg::publishJsonVariables() fncs_publish: key: %s value %s \n",
      skey.c_str(), pubjsonstr.c_str());
...
int fncs_msg::subscribeJsonVariables() // Renke add
{
  ...
  valStream << endl;
  gl_verbose("fncs_msg::subscribeJsonVariables(), skey: %s, reading json data "
             "as string: %s",
             skey.c_str(), valStream.str().c_str());
  for (string &value : values) {
    if (value.empty() == false) {
      ...
      try {
        ...
        gl_warning("fncs_msg::subscribeJsonVariables(), failed to parse the "
                   "received json string! Ignoring message.");
        return 1;
      }
      if (!subscribe_json_data_full.contains(simName)) {
        gl_warning("fncs_msg::subscribeJsonVariables(), Message doesn't "
                   "contain the name of this federate. Ignoring message.");
        return 1;
      }
      ...
      for (auto &it : subscribe_json_data.items()) {
        ...
        for (auto &it1 : it.value().items()) {
          ...
          // gl_verbose("fncs_msg::subscribeJsonVariables(): %s is get from json
          // data \n", gldObjpropertyName.c_str());
          if (gldpro_obj->is_valid()) {
            // gl_verbose("connection: local variable '%s' resolved OK, object
            // id %d", gldObjpropertyName.c_str(),
            // gldpro_obj->get_object()->id); get the value of the property
            nlohmann::json sub_value =
                subscribe_json_data[gldObjectName][gldPropertyName];
            ...
            if (sub_value.is_number_integer() && gldpro_obj->is_integer()) {
              ...
              gl_verbose("fncs_msg::subscribeJsonVariables(): %s is set value "
                         "with int: %d \n",
            ...
            // if the gl_property type is char*, enumeration, or complex number
                     ...
                     (gldpro_obj->is_complex() || gldpro_obj->is_character() ||
                      gldpro_obj->is_enumeration())) {
              ...
              gl_error("fncs_msg::fncs json subscribe: fncs type does not "
                       "match property type: %s",
                       gldObjpropertyName.c_str());
              delete gldpro_obj;
              return 0;
            }
            ...
            gl_warning("connection: local variable '%s' cannot be resolved. "
                       "The local variable will not be updated.",
                       gldObjpropertyName.c_str());
            delete gldpro_obj;
          }
...
int fncs_msg::publish_fncsjson_link() // Renke add
{
  ...
  for (auto &it : publish_json_config.items()) {
    ...
    // gl_verbose("fncs_msg.publish_fncsjson_link(): gldObjectName: %s, nsize:
    ...
    for (int isize = 0; isize < nsize; isize++) {
      ...
      // gldObjpropertyName = gldObjpropertyName + gldPropertyName;
      // gl_verbose("fncs_msg.publish_fncsjson_link(): processing json configure
      // publish properties: %s \n", gldObjpropertyName.c_str());  //renke debug
...
extern "C" size_t fncs_from_hex(void *buf, size_t len, const char *hex,
                                size_t hexlen) {
  char *p = (char *)buf;
  char lo = (char)0;
  char hi = (char)0;
...
extern "C" void outgoing_fncs_function(char *from, char *to, char *funcName,
                                       char *funcClass, void *data,
                                       size_t len) {
  int64 result = -1;
  ...
  if (relay == nullptr) {
    throw("fncs_msg::outgoing_route_function: the relay function for function "
          "name %s could not be found.",
          funcName);
  }
  if (relay->drtn != DXD_WRITE) {
    throw("fncs_msg:outgoing_fncs_function: the relay function for the "
          "function name ?s could not be found.",
          funcName);
  }
  char message[3000] = "";
...
extern "C" FUNCTIONADDR
add_fncs_function(fncs_msg *route, const char *fclass, const char *flocal,
                  const char *rclass, const char *rname, TRANSLATOR *xlate,
                  DATAEXCHANGEDIRECTION direction, COMMUNICATIONTYPE ctype) {
  ...
  if (relay != nullptr) {
    gl_error("fncs_msg::add_fncs_function(rclass='%s', rname='%s') a relay "
             "function is already defined for '%s/%s'",
             rclass, rname, rclass, rname);
    return 0;
  }
  ...
  if (relay == nullptr) {
    gl_error("fncs_msg::add_fncs_function(rclass='%s', rname='%s') memory "
             "allocation failed",
             rclass, rname);
    return 0;
  }
...
extern "C" FUNCTIONSRELAY *find_fncs_function(const char *rclass,
                                              const char *rname) {
  // TODO: this is *very* inefficient -- a hash should be used instead
  FUNCTIONSRELAY *relay;
  for (relay = first_fncsfunction; relay != nullptr; relay = relay->next) {
    if (strcmp(relay->remotename, rname) == 0 &&
        strcmp(relay->remoteclass, rclass) == 0)
      return relay;
  }
  return nullptr;
}
