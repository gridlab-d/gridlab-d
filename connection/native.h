...
typedef struct s_functionrelay {
  ...
  COMMUNICATIONTYPE ctype; ///< identifies which fncs communication function to
                           ///< call. Only used for communicating with fncs.
} FUNCTIONRELAY;
...
public:
  ...
  inline enumeration get_mode(void) { return mode; }
  ...
  inline void set_mode(enumeration p) { mode = p; }
  ...
  inline void set_mode(char *str) { get_mode_property().from_string(str); }
...
public:
  ...
  static inline size_t get_transport_offset(void) {
    native *current_defaults = get_defaults();
    return reinterpret_cast<const char *>(&(current_defaults->transport)) -
           reinterpret_cast<const char *>(current_defaults);
  }

  // Inline function to get the value of `transport`.
  inline enumeration get_transport(void) { return transport; }
  ...
  inline void set_transport(enumeration p) { transport = p; }
...
public:
  ...
  static inline size_t get_timestep_offset(void) {
    native *current_defaults = get_defaults();
    return reinterpret_cast<const char *>(&(current_defaults->timestep)) -
           reinterpret_cast<const char *>(current_defaults);
  }

  // Inline function to get the value of `timestep`.
  inline double get_timestep(void) { return timestep; }
  ...
  inline void set_timestep(double p) { timestep = p; }
...
public:
  ...
  int64 function(char *remotename, char *functionname, void *data,
                 size_t datalen, TRANSLATOR *xlate_out = NULL,
                 TRANSLATOR *xlate_in = NULL);
