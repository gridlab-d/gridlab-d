...
typedef int(TRANSLATOR)(char *local, size_t local_len, char *remote,
                        size_t remote_len, TRANSLATIONFLAG flags, ...);
...
public:
  cacheitem(VARMAP *var); ///< creates a new cache entry (invalidates
                          ///< pre-existing CACHEIDs)
  static void init();
...
public:
  ...
  static CACHEID
  get_id(VARMAP *var, size_t modulo = 0); ///< get the CACHEID for a cache tuple
  static inline cacheitem *
  get_item(CACHEID id) ///< get the cache item from the id
  {
    return index[id];
  };
  ...
  inline gld_property get_object_property(void) {
    return gld_property(get_object(), get_property());
  }
  ...
  bool
  read(char *buffer,
       size_t len); ///< read the item from the cache if buffer is large enough
  bool write(char *value); ///< write the item to the cache if space available
  bool copy_from_object(void);
  bool copy_to_object(void);
  inline void
  set_translator(TRANSLATOR *fn) ///< set the translator to use when copying
                                 ///< cache to and from transport buffers
  {
    xltr = fn;
  };
