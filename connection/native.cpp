...
int native::parse_function(const char *specs) {
  ...
  if (sscanf(specs, "%[^/]/%[^-<>\t ]%*[\t ]%[-<>]%*[\t ]%[^/]/%[^\n]",
    ...
    gl_error(
        "native::add_function(const char *spec='%s'): specification is invalid",
        specs);
    return 0;
  }
  ...
  if (strcmp(direction, "->") == 0) {
    ...
    if (fclass == nullptr) {
      gl_error("native::add_function(const char *spec='%s'): local class '%s' "
               "does not exist",
               specs, localclass);
      return 0;
    }
    ...
    if (flocal != nullptr)
      gl_warning("native::add_function(const char *spec='%s'): outgoing call "
                 "definition of '%s' overwrites existing function definition "
                 "in class '%s'",
                 specs, localname, localclass);

    // get relay function
    flocal = add_relay_function(this, localclass, "", remoteclass, remotename,
                                nullptr, DXD_WRITE);
    if (flocal == nullptr)
      return 0;
  ...
  // setup incoming call
    ...
    gl_error("native::add_function(const char *spec='%s'): incoming calls not "
             "implemented",
             specs);
    return 0;
  }
  ...
  // TODO
    ...
    gl_error("native::add_function(const char *spec='%s'): bidirection calls "
             "not implemented",
             specs);
    return 0;
  }
  exception("connection/native::add_function(const char *spec='%s'): internal "
...
native::native(MODULE *module) {
  if (oclass == nullptr) {
    ...
    if (oclass == nullptr)
      throw "connection/native::native(MODULE*): unable to register class "
            "connection:native";
    else
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "mode", get_mode_offset(), PT_DESCRIPTION,
            "connection mode", PT_KEYWORD, "SERVER", (enumeration)CM_SERVER,
            PT_KEYWORD, "CLIENT", (enumeration)CM_CLIENT, PT_KEYWORD, "NONE",
            (enumeration)CM_NONE, PT_enumeration, "transport",
            get_transport_offset(), PT_DESCRIPTION, "connection transport",
            PT_KEYWORD, "UDP", (enumeration)CT_UDP, PT_KEYWORD, "TCP",
            (enumeration)CT_TCP, PT_KEYWORD, "NONE", (enumeration)CT_NONE,
            PT_double, "timestep", get_timestep_offset(), PT_DESCRIPTION,
            "timestep between updates", PT_UNITS, "s",
            // TODO add published properties here
            nullptr) < 1)
      throw "connection/native::native(MODULE*): unable to publish properties "
            "of connection:native";

    if (!gl_publish_loadmethod(
            oclass, "link",
            reinterpret_cast<int (*)(void *, char *)>(loadmethod_native_link)))
      throw "connection/native::native(MODULE*): unable to publish link method "
            "of connection:native";
    if (!gl_publish_loadmethod(oclass, "option",
                               reinterpret_cast<int (*)(void *, char *)>(
                                   loadmethod_native_option)))
      throw std::runtime_error("connection/native::native(MODULE*): unable to "
                               "publish option method of connection:native");
    mode = 0;
    transport = 0;
  ...
  if (sscanf(value, "%[^:]:%[^\n]", command, argument) == 2) {
    ...
    if (strcmp(command, "function") == 0) {
      ...
      if (n != NONE)
        ...
        gl_error(
            "connection/native::link(char *value='%s') map '%s' not recognized",
            value, command);
        return 0;
      }
    }
  } else {
    gl_error("connection/native::link(char *value='%s'): unable to parse link "
             "argument",
             value);
    return 0;
  }
...
int native::option(char *value) {
  ...
  if (sscanf(value, "%[^:]:%[^\n]", target, command) == 2) {
    ...
    gl_error("connection/native::option(char *value='%s'): unable to parse "
             "option argument",
             value);
    return 0;
  }
...
int native::option(char *target, char *command) {
  if (strcmp(target, "connection") == 0) {
    if (get_connection() == nullptr) {
      ...
      if (m == nullptr) {
        gl_error("connection/native::option(char *target='%s', char "
                 "*command='%s'): connection with options '%s' failed",
      ...
      gl_error("connection/native::option(char *target='%s', char "
               "*command='%s'): connection mode already set",
               target, command);
    return 0;
  } else if (get_connection() == nullptr) {
    gl_error("option(char *target='%s', char *command='%s'): connection mode "
             "hasn't be established",
             target, command);
    return 0;
  } else
...
int native::precommit(TIMESTAMP t, TRANSLATOR *xlate) {
  if (get_connection()->update(map[PRECOMMIT], "precommit", xlate) < 0) {
    gl_error("connection/native::precommit(TIMESTAMP t=%lld, TRANSLATOR "
             "*xltr=%p): update failed",
             t, xlate);
    return 0;
  } else
    return 1;
...
TIMESTAMP native::presync(TIMESTAMP t, TRANSLATOR *xlate) {
  if (get_connection()->update(map[PRESYNC], "presync", xlate) < 0) {
    gl_error("connection/native::presync(TIMESTAMP t=%lld, TRANSLATOR "
             "*xltr=%p): update failed",
             t, xlate);
    return TS_ZERO;
  } else
...
TIMESTAMP native::sync(TIMESTAMP t, TRANSLATOR *xlate) {
  if (get_connection()->update(map[SYNC], "sync", xlate) < 0) {
    gl_error("connection/native::sync(TIMESTAMP t=%lld, TRANSLATOR *xltr=%p): "
             "update failed",
             t, xlate);
    return TS_ZERO;
  } else
...
TIMESTAMP native::postsync(TIMESTAMP t, TRANSLATOR *xlate) {
  if (get_connection()->update(map[POSTSYNC], "postsync", xlate) < 0) {
    gl_error("connection/native::postsync(TIMESTAMP t=%lld, TRANSLATOR "
             "*xltr=%p): update failed",
             t, xlate);
    return TS_ZERO;
  } else
...
TIMESTAMP native::commit(TIMESTAMP t0, TIMESTAMP t1, TRANSLATOR *xlate) {
  if (get_connection()->update(map[COMMIT], "commit", xlate) < 0) {
    gl_error("connection/native::commit(TIMESTAMP t0=%lld, TIMESTAMP t1=%lld, "
             "TRANSLATOR *xltr=%p): update failed",
             t0, t1, xlate);
    return TS_ZERO;
  } else
...
int native::prenotify(PROPERTY *p, char *v, TRANSLATOR *xlate) {
  if (get_connection()->update(map[PRENOTIFY], "prenotify", xlate) < 0) {
    gl_error("connection/native::prenotify(PROPERTY *p={name='%s'}, char "
             "*v='%s', TRANSLATOR *xltr=%p): update failed",
             p->name, v, xlate);
    return 0;
  } else
    return 1;
...
int native::postnotify(PROPERTY *p, char *v, TRANSLATOR *xlate) {
  if (get_connection()->update(map[POSTNOTIFY], "postnotify", xlate) < 0) {
    gl_error("connection/native::postnotify(PROPERTY *p={name='%s'}, char "
             "*v='%s', TRANSLATOR *xltr=%p): update failed",
             p->name, v, xlate);
    return 0;
  } else
...
int native::finalize(TRANSLATOR *xlate) {
  if (get_connection()->update(map[FINALIZE], "finalize", xlate) < 0) {
    gl_error("connection/native::finalize(TRANSLATOR *xltr=%p): update failed",
             xlate);
    return 0;
  } else
    return 1;
...
TIMESTAMP native::plc(TIMESTAMP t, TRANSLATOR *xlate) {
  if (get_connection()->update(map[PLC], "plc", xlate) < 0) {
    gl_error("connection/native::plc(TIMESTAMP t=%lld, TRANSLATOR *xltr=%p): "
             "update failed",
             t, xlate);
    return 0;
  } else
...
/*int64 native::function(char*remotename,char*functionname,void*data,size_t
datalen)
{
        int64 result = 0;
        if (
get_connection()->route_function(remotename,functionname,data,datalen,result)<0
) gl_error("connection/native::function(%s,%s,...): update failed",
remotename,functionname); return result;
}*/
...
static size_t convert_to_hex(char *out, size_t max, const char *in,
                             size_t len) {
  size_t hlen = 0;
...
static size_t convert_from_hex(void *buf, size_t len, const char *hex,
                               size_t hexlen) {
  char *p = (char *)buf;
  char lo = 0;
  char hi = 0;
...
extern "C" void outgoing_route_function(char *from, char *to, char *funcName,
  ...
  connection->xlate_out(transport, "function", relay->remotename, 0,
                        ETO_QUOTES);
...
extern "C" FUNCTIONADDR add_relay_function(native *route, const char *fclass,
                                           const char *flocal,
                                           const char *rclass,
                                           const char *rname, TRANSLATOR *xlate,
  ...
  if (relay != nullptr) {
    gl_error("connection_transport::create_relay_function(rclass='%s', "
  ...
  if (relay == nullptr) {
    gl_error("connection_transport::create_relay_function(rclass='%s', "
             "rname='%s') memory allocation failed",
             rclass, rname);
    return 0;
  }
...
extern "C" FUNCTIONRELAY *find_relay_function(const char *rname,
                                              const char *rclass) {
  // TODO: this is *very* inefficient -- a hash should be used instead
  FUNCTIONRELAY *relay;
  for (relay = first_relayfunction; relay != nullptr; relay = relay->next) {
    if (strcmp(relay->remoteclass, rclass) == 0 &&
        strcmp(relay->remotename, rname) == 0)
      return relay;
  }
  return nullptr;
}
