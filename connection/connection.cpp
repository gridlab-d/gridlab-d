...
connection_mode *connection_mode::new_instance(const char *options) {
  ...
  char temp[4096];
  strcpy(temp, options);
  while ((tag = strtok_r(tag == nullptr ? temp : nullptr, ",", &next_tag)) !=
         nullptr) {
    if (connection == nullptr) {
      ...
      case CM_CLIENT:
        ...
        break;
      default:
        gl_error("connection_mode::new_instance(char *options='%s'): unknown "
                 "connection mode '%s'",
                 options, tag);
        return nullptr;
        break;
      ...
      if (n == 0) {
        gl_error("connection_mode::new_instance(char *options='%s'): unable to "
                 "parse tag '%s'",
                 options, tag);
        return nullptr;
      } else if (n == 1) {
        // TODO add flag options here
        gl_error("connection_mode::new_instance(char *options='%s'): "
                 "unrecognized tag '%s'",
                 options, tag);
        return nullptr;
      } else if (n == 2) {
      ...
      if (!connection->option(tag)) {
        gl_error("connection_mode::new_instance(char *options='%s'): "
                 "unrecognized connection option '%s'",
                 options, tag);
        return nullptr;
      }
    }
...
OK:
  if (connection == nullptr) {
    gl_warning("connection_mode::new_instance(char *options='%s'): connection "
               "mode not specified",
               options);
  }
  if (connection->get_transport() == nullptr) {
    gl_warning("connection_mode::new_instance(char *options='%s'): transport "
               "not specified, using default UDP",
               options);
...
void connection_mode::set_transport(CONNECTIONTRANSPORT t) {
  ...
  if (transport == nullptr)
    gl_warning(
        "connection_mode::set_transport(): unable to create new instance");
...
int connection_mode::init(void) { return get_transport()->init(); }
int connection_mode::option(char *target, char *command) {
  ...
  if (strcmp(target, "transport") == 0 ||
      strcmp(target, transport->get_transport_name()) == 0)
  ...
  if (strcmp(target, "connection") == 0) {
    ...
    gl_error("connection/connection_mode::option(char *target='%s', char "
             "*command='%s'): target not recognized",
             target, command);
  return 0;
}

int connection_mode::update(VARMAP *var, DATAEXCHANGEDIRECTION dir,
                            TRANSLATOR *xlate) {
  gld_property *target = var->obj;
  if (!target->is_valid()) {
    gl_error("connection_mode::update(VARMAP *var=%p, TRANSLATOR *xlate=%p): "
             "target '%s' is not valid",
             var, xlate, var->local_name);
    return 0;
  }

  if ((dir == DXD_READ && var->dir == DXD_READ)) {
    if (!read_cache.read(var, xlate))
      gl_warning("connection_mode::update(VARMAP *var=%p, TRANSLATOR "
                 "*xlate=%p): unable to read/translate data from %s",
                 var, xlate, var->remote_name);
    else
    ...
    if (!write_cache.write(var, xlate))
      gl_warning("connection_mode::update(VARMAP *var=%p, TRANSLATOR "
                 "*xlate=%p): unable to write/translate data for %s",
                 var, xlate, var->remote_name);
    else
...
int connection_mode::update(varmap *varlist, const char *tag,
                            TRANSLATOR *xlate) {
  int count = 0;
  VARMAP *v;
  ...
  if (count > 0) {
    ...
    int64 id;
    // send outgoing data
    if (client_initiated(MSG_INITIATE, MSG_TAG, "method", tag, MSG_OPEN, "data",
                         MSG_DATA, &write_cache, MSG_CLOSE, MSG_COMPLETE, &id,
                         nullptr) < 0)
      return -1;
    // recieve incoming data
    if (server_response(MSG_INITIATE, MSG_TAG, "result", tag, MSG_OPEN, "data",
                        MSG_DATA, &read_cache, MSG_CLOSE, MSG_COMPLETE, &id,
                        nullptr) < 0)
...
int connection_mode::exchange(EXCHANGETRANSLATOR *xlate, bool critical) {
  if (!transport->message_open()) {
    ...
    debug(9, "message queue control: MSG_INITIATE (critical=%s)",
          critical ? "yes" : "no");
...
int connection_mode::exchange_data(EXCHANGETRANSLATOR *xlate, cache *list) {
  ...
  for (n = 0; n < list->get_count(); n++) {
    ...
    if (!xlate(transport, item->get_remote(), item->get_buffer(),
               item->get_size(), ETO_QUOTES))
      return 0;
  }
  return 1;
}
...
int connection_mode::server_response(MESSAGEFLAG flag, ...) {
  ...
  case CM_CLIENT:
    if (recv() > 0) {
      debug(0, "receiving message: length=[%d] buffer=[%s]",
            transport->get_position(), transport->get_input());
      while (stop == false) {
        msgcount++;
        switch (flag) {
        case MSG_CRITICAL:
        ...
        case MSG_INITIATE:
          if (exchange(xlate_in, critical) < 0) {
            error(
                "connection_mode::exchange(): transport status control error");
            count = -msgcount;
            stop = true;
          }
          if ((flag = (MESSAGEFLAG)va_arg(flg, int)) == 0) {
        ...
        case MSG_SCHEMA: {
          ...
          debug(9, "connection_mode::exchange(...) MSG_SCHEMA('%s',cache=0x%p)",
                dir == DXD_READ ? "input"
                                : (dir == DXD_WRITE ? "output" : "invalid"),
                dir == DXD_READ ? &read_cache
                                : (dir == DXD_WRITE ? &write_cache : nullptr));
          if (dir == DXD_READ) {
            ...
            error("exchange(): schema exchange failed, invalid data direction "
                  "supplied");
            count = -msgcount;
            stop = true;
          }
        ...
        case MSG_TAG: {
          ...
          debug(9, "connection_mode::exchange(...) MSG_TAG('%s','%s')", tag,
                value);
          if (exchange(xlate_in, tag, value) < 0) {
            error("connection_mode::exchange(): tagged value exchange failed "
                  "for ('%s','%s')",
                  tag, value);
            count = -msgcount;
            stop = true;
        ...
        case MSG_STRING: {
          ...
          debug(9, "client_initiated(...) MSG_STRING('%s',%d,'%s')", tag, len,
                buf);
          if (exchange(xlate_in, tag, len, buf) < 0) {
            error("connection_mode::exchange(): string value exchange failed "
                  "for ('%s',%d,'%s')",
                  tag, len, buf);
            count = -msgcount;
            stop = true;
        ...
        case MSG_REAL: {
          ...
          if (exchange(xlate_in, tag, *data) < 0) {
            error("connection_mode::exchange(): double value exchange failed "
                  "for ('%s',0x%p=%lf)",
                  tag, data, *data);
            count = -msgcount;
            stop = true;
        ...
        case MSG_INTEGER: {
          ...
          debug(9, "connection_mode::exchange(...) MSG_INTEGER('%s',0x%p=%ll)",
                tag, data, *data);
          if (exchange(xlate_in, tag, *data) < 0) {
            error("connection_mode::exchange(): double value exchange failed "
                  "for ('%s',0x%p=%lf)",
        ...
        case MSG_OPEN: {
          ...
          if (exchange_open(xlate_in, tag) < 0) {
            error(
                "connection_mode::exchange(): start group '%s' exchange failed",
                tag);
            count = -msgcount;
            stop = true;
            break;
        ...
        case MSG_CLOSE:
          ...
          break;
        default:
          warning("connection_mode::exchange(...) unrecognized message control "
                  "flag %d was ignored",
                  flag);
          if ((flag = (MESSAGEFLAG)va_arg(flg, int)) == 0) {
            stop = true;
  ...
  case CM_SERVER:
    while (stop == false) {
      ...
      case MSG_TAG: {
        ...
        if (exchange(xlate_out, tag, value) < 0) {
          error("connection_mode::exchange(): tagged value exchange failed for "
                "('%s','%s')",
                tag, value);
          count = -msgcount;
          stop = true;
      ...
      case MSG_STRING: {
        ...
        if (exchange(xlate_out, tag, len, buf) < 0) {
          error("connection_mode::exchange(): string value exchange failed for "
                "('%s',%d,'%s')",
                tag, len, buf);
          count = -msgcount;
          stop = true;
      ...
      case MSG_REAL: {
        ...
        if (exchange(xlate_out, tag, *data) < 0) {
          error("connection_mode::exchange(): double value exchange failed for "
                "('%s',0x%p=%lf)",
                tag, data, *data);
          count = -msgcount;
          stop = true;
      ...
      case MSG_OPEN: {
        ...
        if (exchange_open(xlate_out, tag) < 0) {
          error("connection_mode::exchange(): start group '%s' exchange failed",
                tag);
          count = -msgcount;
          stop = true;
          break;
    ...
    va_end(flg);
    if (count == 0) {
      debug(0, "sending message: length=[%d] buffer=[%s]",
            transport->get_position(), transport->get_input());
