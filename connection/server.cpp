...
int server::create(void) {
  ...
  local.set_addr(connection_security < CS_STANDARD ? INADDR_ANY
                                                   : INADDR_LOOPBACK);
...
int server::init(void) {
  ...
  if (connection_security > CS_STANDARD && maxclients == 0)
    maxclients = 64;

  if (type == SOCK_STREAM) {
    if (local.listen(maxclients) != 0) {
      gl_error("%s: unable to listen for incoming connections",
               local.get_saddr());
...
void server::accept_tcp(void) {
  ...
  if (client->accept(local)) {
    ...
    /*if (
       pthread_create(client->get_proc(),nullptr,msg_handler,(void*)client)!=0 )
            gl_error("%s: unable to start stream handler for %s",
       local.get_saddr(), client->get_saddr());*/
  ...
  // only an error if not interrupted
    ...
    gl_error("%s: server accept error %d", local.get_saddr(),
             client->get_lasterror());
}
...
void server::accept_udp(void) {
  ...
  switch (::recvfrom(local.get_socket(), nullptr, 0, MSG_PEEK,
                     (struct sockaddr *)client->get_info(),
                     (socklen_t *)&infosize)) {
  case 0:
  ...
  default:
    ...
    try {
      ...
      client->proc =
          std::thread([this, client]() { this->msg_handler(client); });
...
void server::process_msg(Socket &client) {
  ...
  int infosize = client.get_infosize();
  int len =
      ::recvfrom(local.get_socket(), buffer, sizeof(buffer), 0,
                 (struct sockaddr *)client.get_info(), (socklen_t *)&infosize);
  if (len > 0) {
...
void server::set_type(int t) {
  if (type != 0)
    throw "server::set_type(int): unable to set socket type after server is "
          "created";
  type = t;
}
