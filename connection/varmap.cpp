/// $Id$
///
/// Maintains a list of variables that are to be sent or received during an
/// event
///

#include "connection.h"

varmap::varmap() { map = nullptr; }
...
int varmap::add(char *spec, COMMUNICATIONTYPE comtype) {
  ...
  if (strcmp(dir, "<-") == 0) {
    ...
    gl_error("varmap::add(char *spec='%s'): '%s' is not a valid data exchange "
             "direction",
             spec, dir);
    return 0;
  }
  ...
  if (next == nullptr) {
    gl_error("varmap::add(char *spec='%s'): unable to allocate memory for new "
             "VARMAP",
             spec);
    return 0;
  }
  ...
  if (next->local_name == nullptr) {
    gl_error("varmap::add(char *local='%s', char *remote='%s'): unable to "
             "allocate memory for VARMAP local name",
             local, remote);
    return 0;
  }
  ...
  if (sscanf(remote, "%1024[^;];%*[ ]%1023[^\n]", remName, threshold) != 2) {
    ...
    if (next->remote_name == nullptr) {
      gl_error("varmap::add(char *local='%s', char *remote='%s'): unable to "
               "allocate memory for VARMAP remote name",
               local, remote);
      return 0;
    }
  ...
  for (item = getfirst(); item != nullptr; item = getnext(item)) {
    ...
    if (item->obj->is_valid())
      gl_debug("connection: local variable '%s' resolved ok, object id %d",
               item->local_name, item->obj->get_object()->id);
    else
      gl_error("connection: local variable '%s' cannot be resolved",
               item->local_name);
  }
}
