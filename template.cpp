...
public:
  ...
  int commit();
  int notify(NOTIFYMODULE type,
             PROPERTY *prop); // type = {NM_PREUPDATE, NM_POSTUPDATE, NM_RESET}
};
...
classtemplate::classtemplate(MODULE *module) {
  if (oclass == nullptr) {
    ...
    oclass = gl_register_class(mod, "house", sizeof(house_e),
...
int classtemplate::create(OBJECT *parent) { return 1; }
...
TIMESTAMP classtemplate::sync(TIMESTAMP t0, TIMESTAMP t1) { return TS_NEVER; }

TIMESTAMP classtemplate::postsync(TIMESTAMP t0, TIMESTAMP t1) {
  return TS_NEVER;
}

// EXPORT TIMESTAMP classtemplate_sync(OBJECT *obj, TIMESTAMP t0, PASSCONFIG
// pass)
//{

static TIMESTAMP classtemplate_sync_impl(OBJECT *obj, TIMESTAMP t0,
                                         PASSCONFIG pass) {
  classtemplate *my = OBJECTDATA(obj, classtemplate);
...
#ifndef __APPLE__
extern "C" MODULE_API TIMESTAMP classtemplate_sync(OBJECT *obj, TIMESTAMP t0,
                                                   PASSCONFIG pass) {
  return classtemplate_sync_impl(obj, t0, pass);
}
#else
...
int classtemplate::init(OBJECT *parent) { return 1; }
...
EXPORT int classtemplate_init(OBJECT *obj) {
  try {
    ...
    gl_error("classtemplate:%d (%s) %s", obj->id,
             obj->name ? obj->name : "anonymous", msg);
    return 0;
  }
}

int classtemplate::commit(TIMESTAMP t) { return 1; }
...
int classtemplate::prenotify(PROPERTY *prop) { return 1; }

int classtemplate::postnotify(PROPERTY *prop) { return 1; }
