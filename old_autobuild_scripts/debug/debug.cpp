/** $Id: debug.cpp 4738 2014-07-03 00:55:39Z dchassin $
...
bool add_notification(OBJECT *target, PROPERTY *prop, OBJECT *recipient,
                      int (*call)(NOTIFICATION *)) {
...
bool break_on = false;
void break_enable(void) { break_on = true; }
...
void break_disable(void) { break_on = false; }

g_debug::g_debug(MODULE *module) {
  if (oclass == nullptr) {
    // register to receive notice for first top down. bottom up, and second top
    // down synchronizations
    oclass = gld_class::create(module, "debug", sizeof(g_debug),
                               PC_BOTTOMUP | PC_PRETOPDOWN | PC_POSTTOPDOWN |
                                   PC_AUTOLOCK);
    if (oclass == nullptr)
    ...
    if (gl_publish_variable(
            oclass, PT_enumeration, "type", get_type_offset(), PT_DESCRIPTION,
            "debug point type (none or break)", PT_KEYWORD, "NONE",
            (enumeration)DBT_NONE, PT_KEYWORD, "BREAK", (enumeration)DBT_BREAK,
            PT_enumeration, "status", get_status_offset(), PT_DESCRIPTION,
            "debug point status (active or inactive)", PT_KEYWORD, "INACTIVE",
            (enumeration)DBS_INACTIVE, PT_KEYWORD, "ACTIVE",
            (enumeration)DBS_ACTIVE, PT_set, "event", get_event_offset(),
            PT_DESCRIPTION, "debug event (init, sync, etc.)", PT_KEYWORD,
            "NONE", (gld::set)DBE_NONE, PT_KEYWORD, "INIT", (gld::set)DBE_INIT,
            PT_KEYWORD, "PRECOMMIT", (gld::set)DBE_PRECOMMIT, PT_KEYWORD,
            "PRESYNC", (gld::set)DBE_PRESYNC, PT_KEYWORD, "SYNC",
            (gld::set)DBE_SYNC, PT_KEYWORD, "POSTSYNC", (gld::set)DBE_POSTSYNC,
            PT_KEYWORD, "COMMIT", (gld::set)DBE_COMMIT, PT_KEYWORD, "FINALIZE",
            (gld::set)DBE_FINALIZE, PT_KEYWORD, "ALL", (gld::set)DBE_ALL,
            PT_timestamp, "stoptime", get_stoptime_offset(), PT_DESCRIPTION,
            "the time at which to stop", PT_char1024, "target",
            get_target_offset(), PT_DESCRIPTION, "the target property to test",
            PT_char32, "part", get_part_offset(), PT_DESCRIPTION,
            "the target property part to test", PT_enumeration, "relation",
            get_relation_offset(), PT_DESCRIPTION,
            "the relation to use for the test", PT_KEYWORD, "==", TCOP_EQ,
            PT_KEYWORD, "<", TCOP_LT, PT_KEYWORD, "<=", TCOP_LE, PT_KEYWORD,
            ">", TCOP_GT, PT_KEYWORD, ">=", TCOP_GE, PT_KEYWORD, "!=", TCOP_NE,
            PT_KEYWORD, "inside", TCOP_IN, PT_KEYWORD, "outside", TCOP_NI,
            PT_char1024, "value", get_value_offset(), PT_DESCRIPTION,
            "the value to compare with for binary tests", PT_char1024, "within",
            get_value2_offset(), PT_DESCRIPTION,
            "the bounds within which the value must bed compared", PT_char1024,
            "lower", get_value_offset(), PT_DESCRIPTION,
            "the lower bound to compare with for interval tests", PT_char1024,
            "upper", get_value2_offset(), PT_DESCRIPTION,
            "the upper bound to compare with for interval tests", PT_set,
            "options", get_options_offset(), PT_DESCRIPTION,
...
int g_debug::init(OBJECT *parent) {
  ...
  if (get_parent())
    message("%s init(OBJECT *parent={name:'%s';...})", buffer,
            get_parent()->get_name());
  else
  ...
  if (get_parent() != nullptr) {
    ...
    if (!target_property.is_valid()) {
      gl_error("%s: target '%s' of parent '%s' is not valid", get_name(),
               target.get_string(), pname);
      return FAILED;
    }
    gl_verbose("%s: target '%s.%s' is ok", get_name(), pname,
               target.get_string());
    message("initial value of '%s.%s' is '%s'", pname, target.get_string(),
            (const char *)target_property.get_string());
    message("initial debug type is '%s'",
            (const char *)gld_property(my(), "type").get_string());
  } else {
...
bool g_debug::stop_condition(DEBUGEVENT context) {
  ...
  if ((stoptime == TS_ZERO || stoptime >= gl_globalclock) && context & event) {
    message("stoptime condition met at %s",
            (const char *)gld_clock(stoptime).get_string());
    status = DBS_ACTIVE;
  }
  ...
  if (strcmp(part, "") == 0 && target_prop.compare(relation, value, value2))
    status = DBS_ACTIVE;
  if (strcmp(part, "") != 0 &&
      target_prop.compare(relation, value, value2, part))
    status = DBS_ACTIVE;
...
size_t g_debug::message(char *fmt, ...) {
  if (global_debug.get_bool()) {
    ...
    if (debug_window) {
      ...
      gld_clock now;
      len = fprintf(stdout, "\nDEBUG [%s] %s %u%s ",
                    now.to_string(buffer, sizeof(buffer)) > 0 ? buffer : "???",
                    get_name(), command_num, fmt ? ":" : ">");
      if (fmt != nullptr) {
...
bool g_debug::get_command(void) {
  ...
  while (run) {
    ...
    char cmd[64];
    char args[1024] = "";
    char *last = (history[(history_next - 1) % history_size] == nullptr)
                     ? nullptr
                     : history[(history_next - 1) % history_size];
    ...
    if (sscanf(buffer, "%s %[^\n]", cmd, args) > 0) {
      ...
      if (last == nullptr || strcmp(last, buffer) != 0) {
        if (history[history_next] != nullptr)
...
bool g_debug::cmd_help(char *args) {
  if (strcmp(args, "") == 0) {
    gl_output("Available command (type 'command help' for more or 'info "
              "command' for details):");
...
bool g_debug::cmd_break(char *args) { return true; }
bool g_debug::cmd_watch(char *args) { return true; }
bool g_debug::cmd_list(char *args) {
  ...
  for (n = 0; n < list.get_size(); n++) {
    ...
    // service status
    char svc = 'A';
    if (obj->get_in_svc() > gl_globalclock)
      svc = 'P';
    else if (obj->get_out_svc() < gl_globalclock)
      svc = 'R';
    ...
    if (get_options() & DBO_DETAILS) {
      ...
      sprintf(details, "%s %c%c%c%c%c%c %s/%s/%d ", valid_to,
              ...
              obj->get_flags() & OF_INIT
                  ? 'I'
                  : (obj->get_flags() & OF_DEFERRED ? 'i' : '-'),
              module ? module->get_name() : "[runtime]", oclass->get_name(),
              obj->get_id());
    }

    printf("%c%c%c%c%c%c %4d %-24s %-16s %-16s %s\n", svc, pretopdown, bottomup,
           posttopdown, lock, plc, rank, buffer, oname, pname, details);
  }
  return true;
}
bool g_debug::cmd_print(char *args) {
  ...
  if (args[0] == '.') {
    ...
    // print only request args
      ...
      if (!parent.is_valid()) {
        message("property '%s' is not found in object '%s'", args,
                get_object(obj)->get_name());
        return true;
      }
...
bool g_debug::cmd_signal(char *args) {
  ...
  struct {
    ...
    unsigned int sig;
      ...
      {"ABRT", SIGABRT}, {"FPE", SIGFPE},   {"ILL", SIGILL},
      {"INT", SIGINT},   {"SEGV", SIGSEGV}, {"TERM", SIGTERM},
  };
  int n;
...
static CONSOLEWINDOW debugwnd = {
    0,
    0,         // height,width not determined yet
    -10,       // hpos
    32,        // vpos
...
unsigned long console_object_input_handler(unsigned int input) { return 0; }
unsigned long console_data_input_handler(unsigned int input) { return 0; }
unsigned long console_message_input_handler(unsigned int input) { return 0; }
...
int g_debug::console_message(const char *fmt, ...) {
  ...
  if (-(debugwnd.msgs.buffer.next - debugwnd.msgs.buffer.first) %
          debugwnd.nmsg ==
      debugwnd.hpos)
    debugwnd.msgs.buffer.first =
        (debugwnd.msgs.buffer.first + 1) % debugwnd.nmsg;
  return pos;
}
...
void g_debug::console_show_objlist(gld_object *parent, int level) {
  ...
  for (i = 0, obj = gld_object::get_first();
       i < debugwnd.height + debugwnd.hpos - 2 && obj != nullptr;
       i++, obj = obj->get_next()) {
...
bool g_debug::show_line(int row, const char *label, const char *fmt, ...) {
  ...
  if (row < 2 || row > (int)(debugwnd.height + debugwnd.hpos - 1))
    return false;
  ...
  if (is_selected && debugwnd.pane == 1)
    attroff(A_BOLD);
  return true;
}
void g_debug::console_show_object(void) {
  ...
  show_line(++row, "parent", "%s",
            get_parent() ? get_parent()->get_name() : "");
  show_line(++row, "rank", "%d", get_rank());
  show_line(++row, "clock", "%s",
            (const char *)gld_clock(get_clock()).get_string());
  show_line(++row, "valid", "%s",
            (const char *)gld_clock(get_valid_to()).get_string());
  show_line(++row, "skew", "%s",
            (const char *)gld_clock(get_schedule_skew()).get_string());
  //	show_line(++row,"forecast","");
  show_line(++row, "latitude", "%.6f",
            callback->geography.latitude.to_string(get_latitude(), buffer,
                                                   sizeof(buffer))
                ? buffer
                : "(NA)");
  show_line(++row, "longitude", "%.6f",
            callback->geography.longitude.to_string(get_longitude(), buffer,
                                                    sizeof(buffer))
                ? buffer
                : "(NA)");
  show_line(++row, "in", "%s",
            (const char *)gld_clock(get_in_svc()).get_string());
  show_line(++row, "out", "%s",
            (const char *)gld_clock(get_out_svc()).get_string());
  //	show_line(++row,"time","");
  //	show_line(++row,"space","");
...
void g_debug::console_show_messages(void) {
  ...
  for (msg = debugwnd.msgs.buffer.first;
       msg != debugwnd.msgs.buffer.next && row <= debugwnd.height;
       msg = (msg + 1) % debugwnd.nmsg, row++) {
  ...
  while (run) {
    ...
    if (refresh_flags == R_ALL) {
      ...
      mvprintw(0, 0, "GridLAB-D Debug Console - Version %d.%d.%d-%d (%s)",
               major.get_int32(), minor.get_int32(), patch.get_int32(),
               build.get_int32(), b.get_buffer());
      mvprintw(0, (int)(debugwnd.width / 2 - m.get_length() / 2 - 2), " [%s] ",
               m.get_buffer());
    ...
    if (refresh_flags & R_CLOCK) {
      ...
      mvprintw(0, (int)(debugwnd.width - n.get_length() - 1), " %s",
               n.get_buffer());
    }
    ...
    case KEY_DOWN:
      if (debugwnd.highlight == WH_PANE) {
        if (selected_item < (int)(debugwnd.height + debugwnd.hpos - 3))
          selected_item++;
...
static const char *helpdata[] = {
    ...
    " Left    Move the cursor to the parent item (closes the child item) (if "
    "any)\n"
...
void g_debug::console_load(void) {
  ...
  if (fp == nullptr)
    return;
  fscanf(fp, "%d,%d,%d,%d", &debugwnd.height, &debugwnd.width, &debugwnd.hpos,
         &debugwnd.vpos);
  fclose(fp);
}
...
void g_debug::console_save(void) {
  ...
  if (fp == nullptr)
    return;
  fprintf(fp, "%d,%d,%d,%d", debugwnd.height, debugwnd.width, debugwnd.hpos,
          debugwnd.vpos);
  fclose(fp);
}
